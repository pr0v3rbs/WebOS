...
----------------------------------------------------------------------------------------*/
#define VD2_RC0_BASE		(VDEC_REG_BASE + 0x3C000)
#define VD2_RC1_BASE		(VDEC_REG_BASE + 0x3E000)
...
#define _GET_INT_SRC(_src)																\
	( (_src < VDC_HAL_INT_SRC_RESET_BASE)?												\
		(_src - VDC_HAL_INT_SRC_BASE + INTERRUPT_CORE_0) :								\
		(_src - VDC_HAL_INT_SRC_RESET_BASE + INTERRUPT_SRST_DONE) )
...
void	VDC_HAL_Init(void)
{
	...
	SetReg(AD_M1_INTR_ENABLE, (GetReg(AD_M1_INTR_ENABLE) | (VDEC_HW_INT_EN)));
...
void	VDC_HAL_EnableInterrupt(UINT8 ui8CoreNum, UINT8 ui8IntrSrc)
{
	...
	{
	case VDEC_CORE_NUM_HEVC:
		SetReg(AD_M1_CODE_INTRR_MUX, (GetReg(AD_M1_CODE_INTRR_MUX) | (1 << VDEC_CORE_NUM_HEVC)));
		break;
	case VDEC_CORE_NUM_CNM:
		VD2_RC0_RdFL(intr_i_en);
		VD2_RC0_Wr(intr_i_en, (VD2_RC0_Rd(intr_i_en) | (1 << _GET_INT_SRC(ui8IntrSrc))));
		VD2_RC0_WrFL(intr_i_en);
		SetReg(AD_M1_CODE_INTRR_MUX, (GetReg(AD_M1_CODE_INTRR_MUX) | (1 << VDEC_CORE_NUM_CNM)));
		break;
    case VDEC_CORE_NUM_G1:
		SetReg(AD_M1_CODE_INTRR_MUX, (GetReg(AD_M1_CODE_INTRR_MUX) | (1 << VDEC_CORE_NUM_G1)));
        break;
	default:
		break;
	}
...
void	VDC_HAL_ClearInterrupt(UINT8 ui8CoreNum, UINT8 ui8IntrSrc)
{
	...
	case VDEC_CORE_NUM_CNM:
		...
		VD2_RC0_Wr(intr_i_cl, (1 << _GET_INT_SRC(ui8IntrSrc)));
...
void	VDC_HAL_ClearInterruptMask(UINT8 ui8CoreNum, UINT32 ui32IntrMsk)
{
	...
	case VDEC_CORE_NUM_CNM:
		VD2_RC0_RdFL(intr_i_cl);
		VD2_RC0_Wr(intr_i_cl, _GET_INT_SRC_MASK(ui32IntrMsk));
		VD2_RC0_WrFL(intr_i_cl);
		break;
	default:
		break;
...
BOOLEAN	VDC_HAL_IsInterruptEnable(UINT8 ui8CoreNum, UINT8 ui8IntrSrc)
{
	...
	{
	case VDEC_CORE_NUM_HEVC:
		bRet = !!(GetReg(AD_M1_CODE_INTRR_MUX) & (1 << VDEC_CORE_NUM_HEVC));
		break;
	case VDEC_CORE_NUM_CNM:
		VD2_RC0_RdFL(intr_i_en);
		bRet = !!((VD2_RC0_Rd(intr_i_en) & (1 << _GET_INT_SRC(ui8IntrSrc))) & (GetReg(AD_M1_CODE_INTRR_MUX) & (1 << VDEC_CORE_NUM_CNM)));
		break;
	default:
		break;
	}
...
UINT32	VDC_HAL_GetInterruptStatus(UINT8 ui8CoreNum)
{
	...
	case VDEC_CORE_NUM_CNM:
		...
		ui32Status = VD2_RC0_Rd(intr_i_st);

		if ( ui32Status & (1 << INTERRUPT_CORE_0) ) {
			ui32RetVal |= (1 << (VDC_HAL_INT_SRC_BASE + 0));
		}

		if ( ui32Status & (1 << INTERRUPT_CORE_1) ) {
			ui32RetVal |= (1 << (VDC_HAL_INT_SRC_BASE + 1));
		}

		if ( ui32Status & (1 << INTERRUPT_SRST_DONE) ) {
