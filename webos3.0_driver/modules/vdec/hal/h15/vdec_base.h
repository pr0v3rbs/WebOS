...
#define VDEC1_REG_BASE              (0xC8530000)
#define VDEC2_REG_BASE              (0xC8560000)
#endif // __XTENSA__
...
#define VDEC_BASE_GX(i)		\
	({ \
	 unsigned int base;	\
	 if ((i)==0) \
	 base = VDEC1_REG_BASE + 0xc00; \
	 else if ((i)==1)	\
	 base = VDEC2_REG_BASE + 0xc00; \
	 else if ((i)==2) \
	 base = VDEC_REG_BASE + 0x1200; \
	 else \
	 base = 0x0; \
	 base;	\
	 })
...
#define vdec_alloc_dpb(size,align,name)	({ unsigned int a; \
	extern unsigned int ipc_vdec_alloc_dpb (int, unsigned int, const char *); \
	a = ipc_vdec_alloc_dpb (size, align, name); \
	a; })
#define vdec_free_dpb(addr)			{ \
	extern void ipc_vdec_free_dpb (unsigned int); \
	ipc_vdec_free_dpb (addr); }
...
#define vdec_alloc_dpb(size,align,name)	({ \
		unsigned long r = hma_alloc_user ("dpb", size, align, name); \
		if (r) \
			r -= VDEC_BUS_OFFSET; \
		r; \
	})
#define vdec_free_dpb(addr)		hma_free ("dpb", addr+VDEC_BUS_OFFSET)
...
#define vdec_alloc2(s,a,n)		vdec_alloc_dpb(s,a,n)
#define vdec_free2(a)			vdec_free_dpb(a)
...
#define vdec_alloc_from(pool,size,align,name)	({							\
	unsigned long r = 0x00;													\
																			\
	if ( pool == NULL ) {													\
	} else if ( strcmp(pool, "hevc_sb") == 0 ) {							\
		r = vdec_alloc3(size,align,name);									\
	} else if ( strcmp(pool, "hevc") == 0 ) {								\
		r = vdec_alloc1(size,align,name);									\
	} else if ( strcmp(pool, "dpb") == 0 ) {								\
		r = vdec_alloc_dpb(size,align,name);								\
	} else {																\
		r = vdec_alloc(size,align,name);									\
	}																		\
	r;																		\
})
...
#ifdef __XTENSA__
#define vdec_chip()		LX_CHIP_H15
#define vdec_chip_rev()		((vdec_chip() << 8) | 0xb0)
