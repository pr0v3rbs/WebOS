...
void PDEC_HAL_Init(void)
{
	...
	ui32IntEn = RD32(AD_M1_INTR_ENABLE);
	ui32IntEn |= TS_HW_INT_EN;
	WR32(AD_M1_INTR_ENABLE, ui32IntEn);

	ui32IntEn = RD32(AD_M1_TS_INTR_EN);
	ui32IntEn |= TS_PEDC_HW_INT_EN;
	WR32(AD_M1_TS_INTR_EN, ui32IntEn);
...
void PDEC_HAL_Reset(UINT8 ui8PdecCh)
{
	...
	{
		log_error ("PDEC%d: Channel Number\n", ui8PdecCh);
		return;
	}
...
void PDEC_HAL_SkipPicFrame(UINT8 ui8PdecCh, S_PDEC_SKIP_FRAME_T eSkipFrame)
{
	...
	case PDEC_SKIP_BP_FRAME :
		...
		break;
	default :
		log_error ("PDEC%d: Skip Frame: %d\n", ui8PdecCh, eSkipFrame);
	}
...
********************************************************************************
*/
void PDEC_HAL_CPB_SetWrPtr(UINT8 ui8PdecCh, UINT32 ui32CpbWrPtr)
{
	if( ui8PdecCh >= PDEC_NUM_OF_CHANNEL )
	{
		log_error ("PDEC%d: Channel Number\n", ui8PdecCh);
	...
	gsPdecHal[ui8PdecCh].stpReg->reg_cpb_wptr = ui32CpbWrPtr;
}
...
UINT32 PDEC_HAL_AUIB_GetStatus(UINT8 ui8PdecCh)
{
	...
	if( status_aub.reg_aub_status == 0x00 )
		...
		log_error ("PDEC%d: reg_aub_status(0x%X) Field of STATUS_AUB\n",
				ui8PdecCh, status_aub.reg_aub_status );
...
static void _PDEC_UTIL_AUI_Parse(UINT32 *pui32PdecAuiVirPtr, S_PDEC_AUI_T *pPdecAui)
{
	...
	{
		log_error ( "[PDEC] PDEC AUI Address: 0x%X\n", ui32PdecAuiAddr);
		return;
	}
...
********************************************************************************
*/
void PDEC_HAL_AUIB_SetWrPtr(UINT8 ui8PdecCh, UINT32 ui32AuibRdPtr)
{
	if( ui8PdecCh >= PDEC_NUM_OF_CHANNEL )
	{
		log_error ("PDEC%d: Channel Number\n", ui8PdecCh);
	...
	gsPdecHal[ui8PdecCh].stpReg->reg_aub_wptr = ui32AuibRdPtr;
}
...
********************************************************************************
*/
void PDEC_HAL_AUIB_SetRdPtr(UINT8 ui8PdecCh, UINT32 ui32AuibRdPtr)
{
	if( ui8PdecCh >= PDEC_NUM_OF_CHANNEL )
	{
		log_error ("PDEC%d: Channel Number\n", ui8PdecCh);
	...
	gsPdecHal[ui8PdecCh].stpReg->reg_aub_rptr = ui32AuibRdPtr;
}
...
void PDEC_HAL_AUIB_Pop(UINT8 ui8PdecCh, S_PDEC_AUI_T *pPdecAui)
{
	...
	{
		log_error ("PDEC%d: Not Initialised\n", ui8PdecCh);
		return;
	}
	...
	{
		log_error ("PDEC%d: AUIB Underrun\n", ui8PdecCh);
		return;
	}
...
UINT32 *PDEC_HAL_GetLatestDecodedAUI(UINT8 ui8PdecCh, S_PDEC_AUI_T *pPdecAui)
{
	...
	if( ui32AuibWrOffset == ui32AuibRdOffset )
	{
		log_debug ("PDEC%d: AUIB Overflow or Underrun - Offset: 0x%X\n",
				ui8PdecCh, ui32AuibWrOffset);
		return 0x0;
	}
	...
	if( ui32AuibWrOffset == ui32AuibRdOffset )
	{
		log_debug ("PDEC%d: Writing AUIB - Offset: 0x%X\n", ui8PdecCh, ui32AuibWrOffset);
		return 0x0;
	}
...
void PDEC_HAL_SetHEVCDetc(UINT8 ui8PdecCh, UINT8 ui8AuStartMode, UINT16 ui16AuDetEn)
{

    REG_PDEC_ST_HEVC_DET_MODE stRegPdecStHevcDetMode;

    stRegPdecStHevcDetMode.reg_hevc_au_det_en = ui8AuStartMode;
    stRegPdecStHevcDetMode.reg_hevc_au_start_mode = ui16AuDetEn;


	gsPdecHal[ui8PdecCh].stpReg->reg_pdec_st_hevc_det_mode = stRegPdecStHevcDetMode;
}
