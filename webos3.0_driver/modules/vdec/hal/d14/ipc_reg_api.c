 ...
 *---------------------------------------------------------------------------------------*/
#define VDEC_IPC_REG_BASE				(VDEC_BASE + 0x0800)
...
static void IPC_REG_CPB_SetCpbInfoReg(UINT8 ch)
{
	UINT32 ui32RdOffset = stpIPC_Reg->cpb[ch].rd_offset;
	UINT32 ui32WrOffset = stpIPC_Reg->cpb[ch].wr_offset;
	UINT32 ui32UseBytes = 0;
	SINT32 si32Remained = 0;

	/* if tvp */
	if (stpIPC_Reg->ves_db)
		return;

	if( ui32WrOffset >= ui32RdOffset )
		ui32UseBytes = ui32WrOffset - ui32RdOffset;
	else
		ui32UseBytes = ui32WrOffset + VDEC_CPB_SIZE - ui32RdOffset;

	si32Remained = VDEC_CPB_MARGIN - ui32UseBytes;
	if(si32Remained < 0) si32Remained = 0;

	DX_IPC_HAL_SetReg(DX_REG_CPB_INFO, si32Remained);
}

void IPC_REG_CPB_SetWrOffset(UINT8 ch, UINT32 ui32WrOffset)
{
	stpIPC_Reg->cpb[ch].wr_offset = ui32WrOffset;
	IPC_REG_CPB_SetCpbInfoReg(ch);
}
