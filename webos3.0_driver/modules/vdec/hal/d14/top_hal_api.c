...
void	TOP_HAL_EnableVsyncIntr(VDEC_INTR_T s)	{ VCP_CH_VCP_INTR_MASK((s-VSYNC0)) = 0xffffffbf; }
int	TOP_HAL_DisableVsyncIntr(VDEC_INTR_T s)	{ VCP_CH_VCP_INTR_MASK((s-VSYNC0)) = 0xffffffff; return 0; }
void	TOP_HAL_ClearVsyncIntr(VDEC_INTR_T s)	{ VCP_CH_VCP_INTR_REG((s-VSYNC0)) = 1<<6; }

void	TOP_HAL_ClearVsyncIntrMsk(VDEC_INTR_T s)
{
	if (s & (1<<VSYNC0))
		VCP_CH_VCP_INTR_REG(0) = 1<<6;
	if (s & (1<<VSYNC1))
		VCP_CH_VCP_INTR_REG(1) = 1<<6;
}
UINT8	TOP_HAL_IsVsyncIntrEnable(VDEC_INTR_T s)	{ return !(VCP_CH_VCP_INTR_MASK((s-VSYNC0)) & (1<<6)) ; }
UINT32	TOP_HAL_GetVsyncIntrStatus()	{
	return VCP_CH_VCP_INTR_REG(0)|((VCP_CH_VCP_INTR_REG(1)&(1<<6))<<1); }
#endif
