...
/*-----------------------------------------------------------------------------
	0x00e4 reg_pdec_hevc_det_mode ''
------------------------------------------------------------------------------*/
typedef struct {
	UINT32
	reg_hevc_au_det_en		: 9,  //0:8 HEVC NAL detection mode
					      // [8] : FD(Filer Data) detection
					      // [7] : EOB(end of bitstream) NAL detection
					      // [6] : EOS(end of sequence) NAL detection
					      // [5] : AUD(Access Unit Delimiter) NAL detection
					      // [4] : Slice segment NAL detection
					      // [3] : SEI NAL detection
					      // [2] : PPS NAL detection
					      // [1] : SPS NAL detection
					      // [0] : VPS(Video parameter set) NAL detection
					: 6,  //    		reserved
	reg_hevc_au_start_mode		: 2;  //16:17
					      // [17] : 1:AUB & CPB data writing after detected SPS
					      // [16] : 1:AUB & CPB data writing after detected VPS
} REG_PDEC_ST_HEVC_DET_MODE;
...
typedef struct {
	...
	REG_PDEC_STCC                   	reg_pdec_stcc                   ;	// 0x00e0 : ''
	REG_PDEC_ST_HEVC_DET_MODE		reg_pdec_st_hevc_det_mode	;	// 0x00e4 : ''
	UINT32                          	                 __rsvd_01[   6];	// 0x00e4 ~ 0x00f8
