...
#ifdef __XTENSA__
#define BASE_AVLIPSYNC_LOCK()	do{}while(0)
#define BASE_AVLIPSYNC_UNLOCK()	do{}while(0)
#else
#include "base_drv.h"
#endif
...
#define ADEC_M14_A0_BASE		0xC0009000
#define ADEC_M14_B0_BASE		0xC000C000
#define	VDEC_AV_LIPSYNC_BASE(ch)	(VDEC_BASE + 0x08F0 + (ch * 0x8))	// write
#define	ADEC_M14_A0_AV_LIPSYNC_BASE(ch)	(ADEC_M14_A0_BASE + 0x06C0 + (ch * 0x8)) // read
#define	ADEC_M14_B0_AV_LIPSYNC_BASE(ch)	(ADEC_M14_B0_BASE + 0x06C0 + (ch * 0x8)) // read
...
void AV_LipSync_HAL_Init(void)
{
	...
	{
#ifdef __XTENSA__
		static AV_LIPSYNC_BASE_REG_T _vreg, _areg;
		vdec_base[a] = &_vreg;
		adec_base[a] = &_areg;
#else
		...
		if( vdec_chip_rev() >= VDEC_CHIP_REV( M14, B0) )
		{
			adec_base[a] = (volatile AV_LIPSYNC_BASE_REG_T *)
				ioremap (ADEC_M14_B0_AV_LIPSYNC_BASE(a),
						sizeof (AV_LIPSYNC_BASE_REG_T));
		}
		else
		{
			adec_base[a] = (volatile AV_LIPSYNC_BASE_REG_T *)
				ioremap (ADEC_M14_A0_AV_LIPSYNC_BASE(a),
						sizeof (AV_LIPSYNC_BASE_REG_T));
		}
#endif
