...
static bool is_user_resume_host(struct usb_host_data const *data)
{
	if (of_machine_is_compatible("lge,lg1210") && data->is_userresume)
		return true;
	return false;
}
...
static int lg115x_init_usb(void)
{
	...
	for_each_matching_node(np, usb_host_ids) {
		...
		pdev->dev.power.is_userresume = is_user_resume_host(data);
		...
		if(phy_np) {
			...
			phy_pdev->dev.power.is_userresume = pdev->dev.power.is_userresume;
			/*
