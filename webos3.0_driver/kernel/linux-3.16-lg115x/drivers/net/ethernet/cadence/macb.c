...
static irqreturn_t macb_interrupt(int irq, void *dev_id)
{
	...
	status = bp->macbgem_ops.mog_read_isr(bp);
...
static void macb_reset_hw(struct macb *bp)
{
	...
	macb_writel(bp, IDR, -1);
	bp->macbgem_ops.mog_read_isr(bp);
}
...
static u32 gem_read_isr(struct macb *bp)
{
...
static int __init macb_probe(struct platform_device *pdev)
{
	...
	if (macb_is_gem(bp)) {
		...
		if (bp->caps & MACB_CAPS_ISR_CLEAR_ON_WRITE)
			bp->macbgem_ops.mog_read_isr = gem_read_isr;
		else
			bp->macbgem_ops.mog_read_isr = macb_read_isr;
	} else {
