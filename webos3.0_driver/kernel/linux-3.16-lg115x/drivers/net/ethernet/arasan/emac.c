...
#define EMAC_FRAME_LEN	(ETH_FRAME_LEN + ETH_FCS_LEN)
...
static int emac_mac_open(struct net_device *dev)
{
	...
	writel_relaxed(ETH_ALEN * 2, priv->regs_virt + MAC_RX_START_THRESHOLD);
	writel_relaxed(ETH_FRAME_LEN + ETH_FCS_LEN,
	               priv->regs_virt + MAC_TX_START_THRESHOLD);

	/* start both Rx/Tx MAC in proper mode */
	writel_relaxed(0x09, priv->regs_virt + MAC_RX_CTRL);
...
static int emac_handle_rx(struct net_device *dev, int budget)
{
	...
	while (rx < budget && !priv->rxd_virt[priv->rxd_next].own) {
		...
		dma = dma_map_single(&priv->pdev->dev, skb->data,
		                     EMAC_FRAME_LEN, DMA_FROM_DEVICE);

		priv->rxd_virt[priv->rxd_next].size1 = EMAC_FRAME_LEN;
...
static int emac_alloc_rx(struct net_device *dev)
{
	...
	for (i = 0; i < priv->rxd_size; ++i) {
		...
		priv->rxd_virt[i].size1 = EMAC_FRAME_LEN;
