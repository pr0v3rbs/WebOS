...
static int lg1k_usb3_phy_init(struct lg1k_usbphy *lgphy)
{
	...
	parm0.val = readl_relaxed(lgphy->phy_reg + phy_offset(3, param0));
	parm1.val = readl_relaxed(lgphy->phy_reg + phy_offset(3, param1));
	parm2.val = readl_relaxed(lgphy->phy_reg + phy_offset(3, param2));
	parm3.val = readl_relaxed(lgphy->phy_reg + phy_offset(3, param3));
	parm6.val = readl_relaxed(lgphy->phy_reg + phy_offset(3, param6));

	parm0.reg.phy_lane0_ext_pclk_req = 1;
	writel_relaxed(parm0.val, lgphy->phy_reg + phy_offset(3, param0));

	parm1.reg.phy_ssc_range = 1;
	writel_relaxed(parm1.val, lgphy->phy_reg + phy_offset(3, param1));

	parm3.reg.phy_pcs_tx_swing_full = 0x7F;
	parm3.reg.phy_los_bias = 0x5;
	writel_relaxed(parm3.val, lgphy->phy_reg + phy_offset(3, param3));

	parm2.reg.phy_tx_vboost_lvl = 0x4;
	parm2.reg.phy_txpreempamptune = 0x3;
	parm2.reg.phy_los_level = 0x7;
	writel_relaxed(parm2.val, lgphy->phy_reg + phy_offset(3, param2));

	parm6.reg.phy_pcs_rx_los_mask_val = 0x0;
	writel_relaxed(parm6.val, lgphy->phy_reg + phy_offset(3, param6));
...
static int lg1k_usb3_reset(struct lg1k_usbphy *lgphy)
{
	...
	phy_reset.val = readl_relaxed(lgphy->phy_reg + phy_offset(3, reset));
	host_reset.val = readl_relaxed(lgphy->host_reg + host_offset(3, reset));

	phy_reset.reg.usb3_phy_reset = 1;
	writel_relaxed(phy_reset.val, lgphy->phy_reg + phy_offset(3, reset));

	host_reset.reg.usb3_host_bus_reset = 1;
	host_reset.reg.usb3_host_core_reset = 1;
	writel_relaxed(host_reset.val, lgphy->host_reg + host_offset(3, reset));
...
static int lg1k_usb3_gbl_setup(struct lg1k_usbphy *lgphy)
{
	...
	val = readl_relaxed(lgphy->gbl_reg + 0x100);
	val |= (0x3 << 2);
	writel_relaxed(val, lgphy->gbl_reg + 0x100);
	val = 0x0;
	writel_relaxed(val, lgphy->gbl_reg + 0x10C);
	val = readl_relaxed(lgphy->gbl_reg + 0x12C);
	val |= (0x1 << 14);
	writel_relaxed(val, lgphy->gbl_reg + 0x12C);
	val = readl_relaxed(lgphy->gbl_reg + 0x200);
	val |= (0x1 << 3);
	writel_relaxed(val, lgphy->gbl_reg + 0x200);
...
static inline void lg1k_usb3_wait_cr_ack(void __iomem *ack_addr, int ack_val)
{
	...
	while (cnt < timeout) {
		parm5.val = readl_relaxed(ack_addr);
...
static void lg1k_usb3_cr_addr_capture(struct lg1k_usbphy *lgphy, u16 addr)
{
	...
	parm4.val = readl_relaxed(lgphy->phy_reg + phy_offset(3, param4));

	parm4.reg.phy_cr_data_in = addr;
	writel_relaxed(parm4.val, lgphy->phy_reg + phy_offset(3, param4));
...
static u16 lg1k_usb3_cr_read(struct lg1k_usbphy *lgphy, u16 addr)
{
	...
	parm5.val = readl_relaxed(lgphy->phy_reg + phy_offset(3, param5));
...
static int lg1k_usb3_port_up(struct usb_phy *phy)
{
	...
	/* gbl_reg = base_addr + 0xC000 */
	val = readl_relaxed(lgphy->gbl_reg + 0x2C0);
	val &= ~(1 << 31);
	writel_relaxed(val, lgphy->gbl_reg + 0x2C0);

	msleep(100);

	val = readl_relaxed(lgphy->base_addr + 0x420);
	val |= (1 << 9);
	writel_relaxed(val, lgphy->base_addr + 0x420);
...
static int lg1k_usb3_port_reset(struct usb_phy *phy)
{
	...
	val = readl_relaxed(base+0x420);
	val &= ~(1<<9);
	writel_relaxed(val, lgphy->base + 0x420);
	msleep(100);
	...
	phy_rst.val = readl_relaxed(lgphy->phy_reg + phy_offset(3, reset));
	phy_rst.reg.usb3_phy_reset = 0;
	writel_relaxed(phy_rst.val, lgphy->phy_reg + phy_offset(3, reset));
...
static int lg1k_usb3_suspend(struct device *dev)
{
...
static const struct dev_pm_ops lg1k_usb3_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(lg1k_usb3_suspend, lg1k_usb3_resume)
