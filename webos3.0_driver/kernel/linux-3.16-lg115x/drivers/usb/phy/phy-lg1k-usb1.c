...
static int lg1k_usb1_suspend(struct device *dev)
{
...
static int lg1k_usb1_resume(struct device *dev)
{
	...
	dev_info(dev, "resume phy. turn off vbus to trigger\n");
	lg1k_usbphy_set_vbus(&lgphy->phy, false);
...
static const struct dev_pm_ops lg1k_usb1_pm_ops = {
	SET_LATE_SYSTEM_SLEEP_PM_OPS(lg1k_usb1_suspend, lg1k_usb1_resume)
};
