...
/** @file
 *
 *  Application interface header for gfx device
 *
 ...
 *	raxis.lim (2010/11/03) -- GFX_MAX_SURFACE is moved from gfx_cfg.h to gfx_kapi.h
 */
...
/**
 @def GFX_IORW_ALLOC_SURFACE
	...
	@param surface_create_param [IN/OUT] pointer to LX_GFX_SURFACE_SETTING_PARAM_T
	@return RET_OK if success, none zero if otherwise.

 	This IOCTL allocates a surface dynamically based on the requested surface setting.
	According to LG1150 document, engine should align width by 32 byte.
	So application should always check stride information when it read/write the surface directly.
 ...
 @def GFX_IOW_FREE_SURFACE
	...
	Application shoule confirm the surface is note used anymore when this IOCTL called.
 ...
 @def GFX_IOW_SET_SURFACE_PALETTE
	...
	This functon may return RET_ERROR when platte data contains different pixel format from
	surface or current surface is not configured to index color format.
	This restriction to be discussed later.
*/
#define GFX_IORW_ALLOC_SURFACE			_IOWR(GFX_IOC_MAGIC, 3, LX_GFX_SURFACE_SETTING_PARAM_T )
#define GFX_IOW_FREE_SURFACE			_IOW (GFX_IOC_MAGIC, 4, UINT32 )
#define GFX_IORW_QUERY_SURFACE_SETTING	_IOWR(GFX_IOC_MAGIC, 5, LX_GFX_SURFACE_QUERY_PARAM_T )
#define	GFX_IOW_SET_SURFACE_PALETTE		_IOW (GFX_IOC_MAGIC, 6, LX_GFX_SURFACE_PALETTE_PARAM_T )
#define	GFX_IOR_GET_SURFACE_PALETTE		_IOR (GFX_IOC_MAGIC, 7, LX_GFX_SURFACE_PALETTE_PARAM_T )
...
/**
 ...
 @def GFX_IORW_GET_SURFACE_MEM_INFO
	...
	This ioctl gets the physical memory information, offset from base address of surface memory area.
	Application should know the offset information to call mmap() system call.\n
	Remember that physical memory and offset information is never used in application.
	It is used for another ioctl or system call.\n
	DTV software doesn't need to know this information, so this ioctl should be called at
	the internal kadaptor module.

	@note kadaptor should support surface_mmap, surface_munmap.
*/
#define GFX_IOW_SYNC_SURFACE            _IOW ( GFX_IOC_MAGIC, 10, UINT32 )
#define	GFX_IORW_GET_SURFACE_MEM_INFO	_IOWR( GFX_IOC_MAGIC, 11, LX_GFX_SURFACE_MEM_INFO_PARAM_T )

/**
 @def GFX_IOW_DEV_FILE_CTRL
	update attribute flag for each gfx device.
	this attribute is only valid for each gfx device open instance.


*/
#define GFX_IOW_DEV_FILE_CTRL			_IOW( GFX_IOC_MAGIC, 12, LX_GFX_DEV_FILE_CTRL_T )

/**
 @def GFX_IOW_SURFACE_CACHE_CTRL
	control L1/L2 cache for surface memory region.
	this ioctl is only valid when gfx device file is enabled to support cached mmap().
*/
#define GFX_IOW_SURFACE_CACHE_CTRL		_IOW( GFX_IOC_MAGIC, 13, LX_GFX_SURFACE_CACHE_CTRL_T )
 ...
 *
*/
#define GFX_IOW_BLEND					_IOW( GFX_IOC_MAGIC, 30, LX_GFX_MANUAL_BLEND_CTRL_PARAM_T )
#define GFX_IOW_MANUAL_BLEND			_IOW( GFX_IOC_MAGIC, 31, LX_GFX_MANUAL_BLEND_CTRL_PARAM_T )
//#define GFX_IOW_SET_ALPHA_CONV_TBL	_IOW( GFX_IOC_MAGIC, 32, LX_GFX_ALPHA_CONV_TBL_T )
...
*/
#define GFX_IOW_SET_CSC_CONV_TBL		_IOW( GFX_IOC_MAGIC, 33, LX_GFX_CSC_TBL_T )

/** get command delay between command queue request
 *
 */
#define GFX_IOR_GET_CMD_DELAY			_IOR( GFX_IOC_MAGIC, 34, UINT32 )

/** set command delay between command queue request
 *
 * Command delay means the delay in unit of clock. \n
 * Large value makes gfx engine slow. Small value makes gfx engine fast but the whole system
 * perfomance will be degraded since gfx engine consumes large amount of memory bandwidth.
 *
 * @note Valid range is between 0 and 0x7ff (11bit).
 *		Device driver will accept maximum 0x7ff even though application requests the big delay value.\n
 * 		Device driver sould have the default command delay value and initialized with that value.\n
 *
 * @todo We should make the adaptive method to imporve the performance of both gfx engine and system.
 */
#define GFX_IOW_SET_CMD_DELAY			_IOW( GFX_IOC_MAGIC, 35, UINT32 )

/** set batch run mode
 *
 *	This IOCTL controls which module issues 'GFX_BATCH_RUN' command.
 *	If auto mode is set, driver generates GFX_BATCH_RUN whenever it is necessary.
 *	If manual mode is set, driver makes CQ and GFX_START for each GFX operation but GFX_BATCH_RUN is
 *	generated by application with GFX_IO_START_BATCH_RUN ioctl.
 *
 *	Application will change batch run mode to 'auto' when it generates many GFX operation in sequence.
 *	I expect that auto mode will be reduce CPU interrupt.
 *
 *	@note Driver shoule be started with default 'auto' mode \n
 *		  Batch run mode will be changed in run time and driver shold meet this requirement.
 *
 *	@param batch_run_mode [IN] batch run mode
 *	@return OK
 */
#define	GFX_IOW_SET_BATCH_RUN_MODE		_IOW( GFX_IOC_MAGIC, 36, LX_GFX_BATCH_RUN_MODE_T )

/** get batch run mode
 *	@param batch_run_mode [OUT] batch run mode
 *	@return OK
 */
#define GFX_IOR_GET_BATCH_RUN_MODE		_IOR( GFX_IOC_MAGIC, 37, LX_GFX_BATCH_RUN_MODE_T )
 ...
 *
 */
#define	GFX_IOW_SET_GRAPHIC_SYNC_MODE	_IOW( GFX_IOC_MAGIC, 39, LX_GFX_GRAPHIC_SYNC_MODE_T )

/** check if all CQs are processed and wait until ALL CQs are processed.
 *
 *	This IOCTL should block the calling process when timeout value is greater than zero and
 *	GFX are not processed all CQs.
 *	When timeout value is 0, driver should not block the calling process and just return
 *	current status ( OK or ERR_TIMEOUT )
 *
 *	@param timeout [IN] waiting time in unit of msec time.
 *	@return OK if All CQs processed, RET_TIMEOUT if timeout elapsed, NOT_OK otherwise
 *
 *	@note
 */
#define GFX_IOW_WAIT_FOR_SYNC			_IOW( GFX_IOC_MAGIC, 40, UINT32 )
 ...
 *	position is defined as the coordinate point or offset from zero point such as x, y, z.\n
 */
...
typedef struct
{
	 ...
	 *
	 */
	UINT32					b_hw_scaler:1,
							xxx:31;

	/** default cmd delay for surface to surface blit
	 *	if POSD bandwidth problem occur during GFX blit, you should adjust cmd delay value
	 */
	UINT16					surface_blit_cmd_delay;

	/** default cmd delay for surface to POSD blit.
	 *	if POSD bandwidth problem occur during GFX blit, you should adjust cmd delay value
	 */
	UINT16					screen_blit_cmd_delay;

	/** default GFX sync wait timeout
	 *	during GFX H/W read, process & writes data to memory, application should wait for its completion.
	 ...
	 * 	too long value may lead to some graphic still(stuck) during heady memory bandwidth.
	 */
	UINT32					sync_wait_timeout;

	/** retry count when GFX timoeut occur
	 *	currently, this value is fixed to 2
	 */
	UINT8					sync_fail_retry_count;
	UINT8					rsvd[7];

	struct
	{
		/** maximum surface width supported by GFX H/W. minimum value is 1 */
		UINT16				max_surface_width;

		/** maximum surface stride supported by GFX H/W. minimum value is 1 */
		UINT16				max_surface_stride;

		/** minimum width of scaler H/W */
		UINT16				min_scaler_input_width;

		UINT32				rsvd[6];	/** reserved field = 24 byte */
	}
	hw_limit;

	struct
	{
		UINT8				bad_dst_addr_stuck;
		UINT8				scaler_read_buf_stuck;
		UINT8				srcblt_op_stuck;
		UINT8				write_op_stuck;

		UINT8				rsvd[12];	/** reserved field = 12 byte */
	}
	workaround;
}
LX_GFX_CFG_T;
 ...
 * If you want to fill the contstant color to the surface, you need to set this pixel format to
 * GFX src port.
 ...
 * This pixel format is used to describe the surface containg only alpha channel.
 ...
 * Each pixel is express as 4byte (1byte alpha, 1byte red, 1byte gree, 1byte blue)
 *
 * @note
 *
 *  @li Defined value in GFX is different from OSD. So this type should be used in only surface operation.
 ...
 *
 *	This struct is to used to pass color data between application and device driver.
 *
 * @note
 * @li Device driver should convert automatically when color data is different from the pixel format of
 * surface.\n
 * @li It is stronly recommended color data has the same pixel format with surface.\n
*/
typedef struct
{
	LX_GFX_PIXEL_FORMAT_T	pixel_format;	/**< pixel format of color */
	UINT32					pixel_value;	/**< real color data */
}
LX_GFX_COLOR_T;

/** structure for CLUT palette.
 *
 * Palette is used only when FB device/surface is configured to indexed colors.\n
 * For 256 indexed colors, you should pass 256 palette data.
 * But it is possible for you to use less than 256 colors.\n
 * It is stronly recommended that pixel format in palette should be the same as pixel format of FB device.
 * Each entry in palette_data is expressed as ARGB8888 or AYCbCr8888.
 *
 * @note\n
 * -# When insufficient palette data are provided, device driver should preserve the other index data
 * which is not covered by this palette data.\n
 * -# Device driver should copy palette_data field to local storage.
 *
 * @todo\n
 * -# Check what pixel format is for palette_data. Is it only ARGB8888 ? Yes. Only ARGB8888 acceptible.
*/
typedef struct
{
#if 0 /* SPEC OUT */
	...
	UINT32 					*palette_data;	/**< pointer to palette data */
}
LX_GFX_PALETTE_T;
...
typedef struct
{
	UINT32					surface_fd;			/**< surface handle */
 ...
 * It means that surface doesn't have any physical buffer and it should not treated
 * as the normal sourface.
 * This type is used for blitting/wirting constant color such as fill, blended_fill.
 *
 * @li @b LX_GFX_SURFACE_TYPE_MEM_BUFFER \n
 * Normal surface type. Real memory buffer is allocated in gfx engine.
 * So gfx engine should free memory buffer when free operation requested by application.
 * Almost surface operation will use this type.
 *
 * @li @b LX_GFX_SURFACE_TYPE_EXTERN_MEM_BUFFER \n
 * This surface type is supported to pack color from the unpacked Y/C, Y/Cb/Cr, Cb/Cr data.
 * Real memory buffer is provided by the other module such as the video decoder.
 * So device driver should not allocate/free this memory buffer.
 * <b>2010/02/23 : </b> OSD as target surface also uses this surface type since OSD buffer is
 *					 already allocated.\n
 *
 * Please refer to external_mem_ptr field in LX_GFX_SURFACE_SETTING_T.
 *
 * @note If surface type is NONE or EXTERN_MEM_BUFFER, GFX_IORW_MEM_MAP_SURFACE is not
 *       supported.
 */
typedef enum
{
...
typedef struct
{
	/** surface fd
	 *
	 *	this value is needed when application searches surface without surface_fd
	 */
	UINT32					surface_fd;

	/** surface type
	 *
	 * Only LX_GFX_SURFACE_TYPE_MEM_BUFFER and LX_GFX_SURFACE_TYPE_EXTERN_MEM_BUFFER are  allowed.\n
	 ...
	 *
	 *  @note DO NOT use LX_GFX_PIXEL_FORMAT_INDEX_0 ( this is used in the internal GFX gine )
	 */
	 ...
	 *  GFX allows one byte alignment for 8bpp color, 2 byte for 16bpp color and 4 byte for 32bpp color.
	 *
	 *	Alignment is expressed as power of 2.\n
	 *	For example, alignment value 0 means 1 byte aligned, value 1 means 2 byte aligend,
	 *	value 2 means 4 byt aligned and so on.
	 *
	 *	@note Device driver may not use alignment information while creating new surface.\n
	 *		  But application should set this field accroding the pixel format.
	 *
	 *  <b>2010/02/23 : </b> alignment seems useless. please DO NOT use this field.
	 */
	UINT32					alignment;
	 ...
	 *	As you know, stride is calculated as width multiplied by byte size of pixel format.
	 *	But system may require N byte aligend width. So application should use stride information
	 *  when reading/writing surface buffer directly.
	 *
	 *	@note This field is not used at surface creation. So let this value to be initialized
	 *  to zero when creating surface.\n
	 *	This field is set to the exact value when querying the surface information.
	 *
	 *  <b>2010/02/23 : </b> when surface type is LX_GFX_SURFACE_TYPE_EXTERN_MEM_BUFFER,
	 ...
	 *
	 *	This field is only valid when surface type is LX_GFX_SURFACE_TYPE_EXTERN_MEM_BUFFER.
	 *	This field contains the preallocated memory buffer used as surfface buffer.
	 *	So let this field to zero for other surface type.
	 *	Device driver should not allocate new buffer but use this buffer as surface buffer.
	 *
	 *	Real value in memory pointer is "physical" memory address.
	 *	So application cannot use this field and should just pass to gfx engine without any
	 *	operation or modification.
	 *
	 *	This field is set to the exact value when querying the surface information
	 *	even though this field is not used in application. Application may print surface
	 *	information in debug code and it will be helpful to device driver developer.\n
	 *
	 *	If surface type is LX_GFX_SURFACE_TYPE_NONE, phy_memptr is always set to 0 ( NULL )
	 */
	UINT32					phys_addr;

	/** preallocated mmap pointer
	 *	[note] this information is processed at kadaptor layer not passed to the kernel driver.
 	 * 	[note] this information is only valid when surface memory is from the external memory area.
	 */
	void*					mmap_ptr;
}
...
typedef struct
{
#define	LX_GFX_SURFACE_QUERY_TYPE_SURFACE_FD		0	/* query by surface_fd */
#define	LX_GFX_SURFACE_QUERY_TYPE_SURFACE_OFFSET	1	/* query by surface_offset */
#define	LX_GFX_SURFACE_QUERY_TYPE_SURFACE_PHYS_ADDR	2	/* query by surface_phys_addr */
	UINT32						query_type;
 ...
 *
 * Remember that offset and length field are valid only when the surface is allocated
 * at the surface memory area. In other words, if surface is virtually create using the external memory
...
typedef struct
{
	UINT32					phys_addr;		/**< physical address of the surface */
...
typedef struct
{
	...
	UINT32	virt_addr;							/**< virtual address returned by mmap */
	UINT32	length;								/**< memory length */
}
...
typedef enum
{
	LX_GFX_BATCH_RUN_MODE_AUTO	= 0,	/**< command queue is controlled by device driver */
	LX_GFX_BATCH_RUN_MODE_MANUAL		/**< application has the right to control command queue */
}
LX_GFX_BATCH_RUN_MODE_T;

typedef enum
{
	LX_GFX_GRAPHIC_SYNC_MODE_AUTO = 0,	/**< graphic sync is done inside device driver */
	LX_GFX_GRAPHIC_SYNC_MODE_MANUAL,	/**< application should call GFX_IOW_WAIT_FOR_SYNC for graphic sync */
}
 ...
 * -# When blitting, you can't combine raster operation with  operation ( This is hardware restriction ).
 *    So if you need the raster operation with blend, you should request two blit operation.
 ...
 *	This flag is used to convert source surface to premultiplied format during the blit operation.\n
 *	LG1150 performs premultification in the internal blend block and doesn't update surface itself.\n
 *	This flag should be used for the source port and the blend operation.
 *	In other words, this flag isn't effective for the raster operation or simple write, copy operation.\n
 *
 *
 * @li @b LX_GFX_PORT_FLAG_DEMULTIPLY \n\n
 *	This flag is used to convert premultipled blended image to non-premultiplied image.
 ...
 *	When this flag enabled, GFX engine checks whether every pixel is inside/outside color key range
 ...
 * LG1150 supports 32bit bitmask while reading 32bit pixel. So you can clear the specified bits of
 ...
 *
 * @note\n
 * -# DO not mix LX_GFX_PORT_FLAG_PREMULTIPLY and LX_GFX_PORT_FLAG_DEMULTIPLY to the same port.
 *	  As described above, premultify is for the source port and demultifly is fot the destination port. \n
 * -# DO not mix LX_GFX_PORT_FLAG_COLORKEY_INSIDE and LX_GFX_PORT_FLAG_COLORKEY_OUTSIDE. \n
 * -# For hardware restriction, All flags for source port should not be set for src2 port.
 *	  As you can see, src2 port doesn't have color key processor, bitmask processor,
 * 	  COC processor and CSC processor. Accordint to GFX manual, src2 port is used only for mask surface and
 *	  YC interleaving.
 *
 */
typedef enum
 ...
 *
 *	@note Device driver should perform the color space conversion automatically.\n
 *		  Even though LG1150 support the color order converson, we will not use that feature.
 *		  If you need the color space conversion, please contact the driver developer.\n
 */
typedef struct
{
	/** port flag to control the color key processor, bitmask processor and multiplying operation
	 *  in the blender block.
	 */
	LX_GFX_PORT_FLAG_T		port_flag;

	/** surface type ( one of constant color, gfx allocated source and externally allocated source */
	LX_GFX_SURFACE_TYPE_T	surface_type;

	/** surface handle
 	 *
  	 * This field is used when surface type is LX_GFX_SURFACE_TYPE_MEM_BUFFER or
	 * LX_GFX_SURFACE_TYPE_EXTERN_MEM_BUFFER. Otherwise, this field should be set to 0x0.
	 */
	UINT32					surface_fd;
	 ...
	 *
	 *	Becuase LG1150 GFX doesn't support hardware clipping, application should process software
	 *	clipping. So device driver may not check any clipping.\n
	 *
	 *	And LG1150 gfx doesn't support stretch blit, dst_rect.w and dst_rect.h shoule be
	 *	same as src_rect.w and src_rect.h.
	 * 	Device driver may return error when dst_rect.w and dst_rect.h is different from src_rec.w
	 *	and src_rect.h.\n
	 */
	LX_RECT_T				rect;

	/** replace color data if source pixel is inside/outside color key range */
	UINT32					colorkey_tgt_color;
	/** lower limit(range) for color key */
	UINT32					colorkey_lower_limit;
	/** upper limit(range) for color key */
	UINT32					colorkey_upper_limit;
...
#define	LX_CSC_COEF_SEL_3	0x3 /* RGB -> YC( 0~255)	*/
	 ...
	 *	in(00:07)	-- 0x3
	 *
	 *  raxis.lim (2010/12/16) GFX regiseter manuals seems to be wrong... swap field order !!!!!!!
	 */
	struct
	{
...
typedef enum
{
	 ...
	 * As you know, both color key and bitmask processor in src0, src1 port is very simple
	 * but difficult to modify surface to your desired form.\n
	...
	/** global alpha flag
	 *
	 * By enabling this flag, alpha channel in source surface should be ignored and use attached
	 * global alpha value.\n
	 * In other words, during blend operation device driver should use global_alpha value for
	 * all calculation.
	 *
	 * @note Global alpha is very command operation in the porter duff .
	 *		 For example, Yahoo! TV Widget uses global alpha when it requests SRC_OVER .\n
 ...
 *
 *	Before using this structure port configuration needed. Refer to LX_GFX_PORT_T;
 *
 *	Each field of this structure is matched to GFX register.
 *	Please refer to LG1150 GFX register manual.
 *
 * @note This structure is used to control blender device directly by application.\n
...
typedef struct
{
	...
	#define LX_GFX_OP_MODE_WRITE			0x0		/* write such as simple fillrect */
	#define LX_GFX_OP_MODE_ONE_SRC_PORT		0x1		/* srcblt such as simple surface copy */
	#define LX_GFX_OP_MODE_TWO_SRC_PORT		0x2		/* rop or blend with two src port */
	#define LX_GFX_OP_MODE_THREE_SRC_PORT	0x3		/* rop or blend with three src port */
	 ...
	 *
	 * 	@note LX_GFX_OUT_SEL_SRC0_PORT ~ LX_GFX_OUT_SEL_SRC2_PORT is not used in application.
	 *		  This value is used only for SOC debugging.
	 */
							out_sel:3,
	 ...
	 *		But normally we should apply the same composition rule for each color field.
	 *		So comp_rule_color will be applied to the all color field.
	 *		If you need to assign the different composition rule, modify the below data field.
	 *		Refer LG1150 GFX manual.
	 */
							comp_rule_alpha:2,
							comp_rule_color:2,
	/* 12bit reserved */
	 ...
	 *		But normally we should apply the same mux factor for each color field.
	 *		So mux_factorX_color will be applied to the all color field.
	 *		If you need to assign the different mux factor, modify the below data field.
	 *		Refer LG1150 GFX manual.
	 */
	UINT32					mux_factor0_alpha:4,
							mux_factor1_alpha:4,
							mux_factor0_color:4,
							mux_factor1_color:4,
	 ...
	 *		since stretch block is located at the end of blend/rop block, stretch is done after the gfx operation.
	 *		for simple scenario, stretch is only supported for the simple copy operation.
	 *		refer to gfx_kadp.c
	 */
 ...
 *
 *	LG1150 uses four level of alpha value for converting between 16bpp and 32bpp.
 *
 ...
 *
 *	@note You need to discuss the real alpha value with design team or chip engineer.\n
 *		  I think this table seems not used since GP doesn't use 16bpp surface anymore.\n
 *		  Device driver should have the default table for alpha conversion and initialized with
 *		  the default value.
 *
 ...
 *
 * 	@note Device driver should have the default table for color space conversion and initialized
 ...
 *
 *	This data is used only when filter mode is LX_GFX_CHROMA_FILTER_MODE_4_TAP_FILTER.
 *	Device driver uses only 12bit data for each coefficient.
 *
 * @note Device driver should have the default table for the chroma filter and initialized with
 *		 the default value.
 *
...
typedef struct
{
	UINT32					surface_max_num;		/**< maximum number which is managed by kdriver */
	UINT32					surface_alloc_num;		/**< current allocated surface num */

	void*					surface_mem_base;		/**< base address of surface memory */
	UINT32					surface_mem_length;		/**< length of surface memory */

	UINT32					surface_mem_alloc_size;	/**< byte size of allocated surface */
	UINT32					surface_mem_free_size;	/**< byte size of free memory */
