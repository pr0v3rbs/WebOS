    ...
    )
{
    ...
    if (kernel->hardware->chipPowerState == gcvPOWER_ON)
    {
        gckOS_GetTicks(&currentTime);

        gckOS_TicksAfter(
            currentTime,
            kernel->eventObj->lastCommitStamp + gckOS_MsecToTicks(5000),
            &isAfter);
    ...
    gcmkVERIFY_OK(gckOS_StartTimer(kernel->os, kernel->stuckTimer, 500));
}
...
#define gcdEVENT_MASK 0x3FFFFFFF
    ...
    for (i = 0; i < gcmCOUNTOF(eventObj->queues); i++)
    {
        gcsEVENT_QUEUE_PTR queue;
        gcsEVENT_PTR record = gcvNULL;

        queue = &eventObj->queues[i];
        record = queue->head;
