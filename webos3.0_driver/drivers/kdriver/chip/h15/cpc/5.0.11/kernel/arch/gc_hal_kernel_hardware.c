    ...
    )
{
    ...
    gcmkONERROR(
        gckOS_GetPhysicalAddress(
            Hardware->os, Hardware->kernel->mmu->mtlbLogical, &config));
    ...
    gckHARDWARE_Execute(Hardware, Hardware->lastWaitLink, 16);

    if (command->offset > 16)
    {
        command->offset = 0;
    }
