 ...
 ************************************************************************************/
 ...
 UINT16 _gFullVQIPkerrCount = 7;
UINT16 _gFullVQPkerrThreshold = 300;
UINT16 _gFullISDBTPkerrThreshold = 200;

UINT8	_gPhn_state = 0;

UINT8 gPowerOnInitISDBT = 1;
unsigned int gPowerOnInitStartTimeISDBT = 0;

UINT16	_gDelayForStableTime = LX_DEMOD_LOCK_DEALY_200;
UINT8	_gchdiffTh = 0xf;
BOOLEAN _gtpblockon = FALSE;
...
//--------------------------------------------------------------------
//static int _DEMOD_H13_VSB_Get_NarrowPacket_Error(UINT32 *pError);
...
*/
int DEMOD_H13_GetMSE(UINT16 *pMSE)
{
...
************************************************************************************/
/**
 * DEMOD_H13_VSB_Set_DefaultRegisterValue.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_VSB_Set_DefaultRegisterValue(void)
{
...
#if 0
#endif
	if (RET_OK != DEMOD_H13_TPOutEnable(TRUE))
	{
		DEMOD_PRINT( "^r^[DEMOD H13] FAIL (%s:%d) DEMOD_H13_TPOutEnable(TRUE) !!!\n", __F__, __L__);
...
************************************************************************************/
/**
 * DEMOD_H13_VSB_Set_NeverLockWaitTime
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_VSB_Set_NeverLockWaitTime(UINT32 waitMs)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_VSB_CochannelExist(BOOLEAN *pCochannel)
{
...
int DEMOD_H13_VSB_PreMonitor(void)
{
...
//	UINT8	npncombw;
	...
	//printk(" ###chungiii DEBUG ## PRE MONITOR  _gchdiffTh value[0x%x]\n", _gchdiffTh);
	...
	if( (mainstrong == 1) && (chdiff_7_0>_gchdiffTh) && (snrref_5_0<=0x12))
	{
		_gtpblockon = TRUE;
		DEMOD_TRACE(" ###chungiii DEBUG ## _gtpblockon  TRUE\n");
	...
	{
		_gtpblockon = FALSE;
		DEMOD_TRACE(" ###chungiii DEBUG ## _gtpblockon  FALSE\n");
		DEMOD_TRACE(" ###chungiii DEBUG ## mainstrong[%d]\n",mainstrong );
		DEMOD_TRACE(" ###chungiii DEBUG ## delaydiff_7_0[0x%x]\n", delaydiff_7_0);
		DEMOD_TRACE(" ###chungiii DEBUG ## chdiff_7_0 [0x%x]\n", chdiff_7_0);
		DEMOD_TRACE(" ###chungiii DEBUG ## snrref_5_0 [0x%x]\n", snrref_5_0);
...
//		EQ_V_WrFL(eq_v_ctrl48);
	...
	else if(_gchdiffTh == 0x12)
	{
		_gchdiffTh = 0x15;
	}
	else if(_gchdiffTh == 0x15)
	{
		_gchdiffTh = 0x18;
	}
	else
...
int DEMOD_H13_VSB_Monitor(DEMOD_CTX_T *pDemodInfo)
{
...
//	DEMOD_H13_Get_Packet_Error(&packeterror); //jeongpil.yun--> packet error  should be read at once within one second !! this line conflicts with other functions.
	...
	EQ_V_Rd01(eq_v_mon17, ncombdet, ncombdet);
	//printk(" ###chungiii DEBUG ## NTSC IS DETECTED [%s][%d]\n",ncombdet ? "NO" : "YES", ncombdet);
	//printk(" ###chungiii _gDelayForStableTime [%d]\n",_gDelayForStableTime);
	...
	else
	{
		if(tperrcount25ms>0)
		{
			DEMOD_TRACE(" ###chungiii DEBUG ## tperrcount25ms>0  _gPhn_state=0\n");
			_gPhn_state = 0;
		}
		else
		{
			DEMOD_TRACE(" ###chungiii DEBUG ## tperrcount25ms<0  _gPhn_state=1\n");
			_gPhn_state = 1;
		}
...
************************************************************************************/
/**
 * DEMOD_H13_VSB_Get_MSEdynStatus.
...
int DEMOD_H13_VSB_Get_MSEdynStatus(BOOLEAN *pbEnable)
{
	UINT8 	msedyn;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_VSB_Get_TotalInfo( void)
{
...
//	LX_DEMOD_VSB_AUTO_CTRL_SIG_STATUS 	bcMonSignal;
	...
	if (RET_OK  != DEMOD_H13_Get_NeverLockStatus(LX_DEMOD_ATSC, &chScanStatus))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_Get_NeverLockStatus() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_H13_GetOperMode(&operMode))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_GetOperMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_H13_Get_IFAGC(&ifAGC))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_Get_IFAGC() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_H13_Get_Lock(LX_DEMOD_CARR_LOCK, &carrLock))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_Get_Lock() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_H13_Get_Lock(&syncLock))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_Get_Lock() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_H13_Get_CarrierFreqOffset( &offsetFrq))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_H13_VSB_CochannelExist( &cochannel))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_VSB_CochannelExist() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	...
	if (RET_OK  != DEMOD_H13_GetCrAcquisionTime(&cracquisionTimeMs))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_GetTrAcquisionTime() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_H13_Get_VABER(&viterbiBer))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_Get_VABER() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
...
************************************************************************************/
/**
 * DEMOD_H13_VSB_Obtaining_Signal_Lock.
...
int DEMOD_H13_VSB_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_RETUNE_RF:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_H13_VSB_Set_DefaultRegisterValue())
				{
					DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_VSB_Set_DefaultRegisterValue() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				else
				{
					...
					if (RET_OK != DEMOD_H13_TPOutEnable(FALSE))
					{
						DEMOD_PRINT( "^r^[DEMOD H13] FAIL (%s:%d) DEMOD_H13_TPOutEnable(FALSE) !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				{
					if(RET_OK != DEMOD_H13_Get_Lock(LX_DEMOD_CARR_LOCK, &lockState))
					{
						DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_Get_Lock() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.vsb.tuneMode)
					{
						...
						else
						{
							...
							//if(RET_OK != DEMOD_H13_VSB_CochannelExist(&Cochannel, &staticGhost))
							//{
							//	DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_VSB_CochannelExist() !!!\n", __F__, __L__);
							//	DEMOD_RETURN_FAIL(__F__);
							//}
							#if 1
							if(RET_OK != DEMOD_H13_VSB_PreMonitor())
							{
								DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_VSB_PreMonitor() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							#endif

							//if(RET_OK != _DEMOD_H13_VSB_Get_NarrowPacket_Error(&packetError))
							...
							else
							{
								...
								if( ((packetError < LX_DEMOD_PACKET_ERROR_THD_100) && (Cochannel == FALSE)) && (_gtpblockon == FALSE) )
								{
									//for debug chungiii
									//OS_MsecSleep(200);
								...
								else
								{
									...
									if(Cochannel == TRUE)
									{
										...
										if(_gDelayForStableTime == LX_DEMOD_LOCK_DEALY_200)
										{
											printk("chungiiii ######### change to _gDelayForStableTime[400]\n");
											_gDelayForStableTime = LX_DEMOD_LOCK_DEALY_500;
											//break;
										}
									}
									...
									if(elapsedTime > LX_DEMOD_LOCK_DEALY_400)
									{
										...
										if(staticGhost == TRUE)
										{
											...
											if(_gDelayForStableTime == LX_DEMOD_LOCK_DEALY_200)
											{
												printk("chungiiii ######### change to _gDelayForStableTime[500]\n");
												_gDelayForStableTime = LX_DEMOD_LOCK_DEALY_400;
												break;
											}
										}
										#endif
...
************************************************************************************/
/**
 * DEMOD_H13_VSB_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_H13_VSB_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
...
#endif
#if 1
	if(RET_OK != DEMOD_H13_VSB_Monitor(pDemodInfo))
	{
		DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_VSB_Monitor() !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if(pDemodInfo->signalStatus.packetError >= _gFullVQPkerrThreshold)
		{
			...
			if(_gGBB_VQI_UnlockCount_full_packeterr == _gFullVQIPkerrCount)
			{
...
/**
 * DEMOD_H13_Get_VSB_NarrowPacket_Error
...
************************************************************************************/
/**
 * DEMOD_H13_QAM_SoftwareResetFEC.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_QAM_SoftwareResetFEC(void)
{
	UINT8 	softrstfecb;
...
int DEMOD_H13_QAM_Monitor(void)
{
...
************************************************************************************/
/**
 * DEMOD_H13_QAM_ModeAutoDetection.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_QAM_ModeAutoDetection(BOOLEAN bEnable)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_QAM_256Mode(void)
{
...
************************************************************************************/
/**
 * DEMOD_H13_QAM_Set_DefaultRegisterValue.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_QAM_Set_DefaultRegisterValue(void)
{
...
************************************************************************************/
/**
 * DEMOD_H13_QAM_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_QAM_Set_NeverLockWaitTime(UINT32 waitMs)
{
	UINT8 scsyslockchktime_7_0;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_QAM_Get_TotalInfo( void)
{
#if 0
	...
	if (RET_OK  != DEMOD_H13_Get_NeverLockStatus(LX_DEMOD_QAM, &chScanStatus))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_Get_NeverLockStatus() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	...
	if (RET_OK  != DEMOD_H13_Get_Lock(LX_DEMOD_TR_LOCK, &timingLock))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) LX_DEMOD_LOCK_STATE_T() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_H13_Get_Lock(LX_DEMOD_FEC_LOCK, &fecLock))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_Get_Lock() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	...
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_Get_SpectrumStatus() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  !=  DEMOD_H13_GetTrAcquisionTime(&tracquisionTimeMs))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_GetTrAcquisionTime() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	...
	if (RET_OK  !=  DEMOD_H13_GetFecAcquisionTime(&fecacquisionTimeMs))
	{
		DEMOD_PRINT( "^r^[DEMOD H13 DTV] FAIL (%s:%d) DEMOD_H13_GetFecAcquisionTime() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
...
************************************************************************************/
/**
 * DEMOD_H13_QAM_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_H13_QAM_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_RETUNE_RF:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_H13_QAM_Set_DefaultRegisterValue())
				{
					DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_QAM_Set_DefaultRegisterValue() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.qam.tuneMode)
				{
					if(RET_OK != DEMOD_H13_QAM_ModeAutoDetection(TRUE))
					{
						DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_QAM_ModeAutoDetection() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				}
				else
				{
					if(RET_OK != DEMOD_H13_QAM_ModeAutoDetection(FALSE))
					{
						DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_QAM_ModeAutoDetection() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if(RET_OK != DEMOD_H13_Set_SpectrumCtrl(FALSE, pDemodInfo->setCfgParam.qam.bSpectrumInv))
					{
						DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_Set_SpectrumCtrl() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
					if(LX_DEMOD_64QAM == pDemodInfo->setCfgParam.qam.qamMode)
					{
						if(RET_OK != DEMOD_H13_QAM_64Mode())
						{
							DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_QAM_64Mode() !!!\n", __F__, __L__);
							DEMOD_RETURN_FAIL(__F__);
						}
					}
					else if(LX_DEMOD_256QAM == pDemodInfo->setCfgParam.qam.qamMode)
					{
						if(RET_OK != DEMOD_H13_QAM_256Mode())
						{
							DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_QAM_256Mode() !!!\n", __F__, __L__);
							DEMOD_RETURN_FAIL(__F__);
						}
					}
					else
...
//				unsigned int 			elapsedTime;
				...
				if(RET_OK != DEMOD_H13_QAM_EQ_Signal_Detector(TRUE))
				{
					DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_QAM_EQ_Signal_Detector() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if(RET_OK != DEMOD_H13_QAM_EQ_Signal_Detector(FALSE))
					{
						DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_QAM_EQ_Signal_Detector() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
...
************************************************************************************/
/**
 * DEMOD_H13_QAM_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_H13_QAM_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if(pDemodInfo->signalStatus.packetError >= _gFullVQPkerrThreshold)
		{
			...
			if(_gGBB_VQI_UnlockCount_full_packeterr == _gFullVQIPkerrCount)
			{
				...
				DEMOD_PRINT("[DEMOD H13](%s) DEMOD_H13_QAM_ModeAutoDetection(TRUE) !!!\n", __F__);
...
/**
 * DEMOD_H13_QAM_EQ_Signal_Detector.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_QAM_EQ_Signal_Detector(BOOLEAN OnOff)
{
	UINT8	nousenosigdet;
...
/**
 * DEMOD_H13_ISDBT_SetDefaultRegisterValue
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Set_DefaultRegisterValue(void)
{
	...
	if(gPowerOnInitISDBT == 1)
	{
		...
		gPowerOnInitStartTimeISDBT = jiffies_to_msecs(jiffies);

	}
	else if(gPowerOnInitISDBT == 0)
	{
		crgearshen = 0x1;
		Common_RdFL(common_ctrl68);  //regAddr = 0x0044, COMMON_CTRL[68]
		Common_Wr01(common_ctrl68, crgearshen, crgearshen);
...
************************************************************************************/
/**
 * DEMOD_H13_ISDBT_Set_Config_auto
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Set_Config_auto (BOOLEAN bAutoDetect)
...
************************************************************************************/
/**
 * DEMOD_H13_ISDBT_Set_PartialConfig
 * Set hierachical signal path and bandwidth.
 * This function is used when DEMOD_H13_ISDBT_Set_Config_auto is enabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Set_PartialConfig (LX_DEMOD_ISDBT_CONFIG_T *pIsdbtConfigParam)
{
...
************************************************************************************/
/**
 * DEMOD_H13_ISDBT_Set_FullConfig
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Set_FullConfig (LX_DEMOD_ISDBT_CONFIG_T *pIsdbtConfigParam)
{
...
************************************************************************************/
/**
 * DEMOD_H13_ISDBT_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Set_NeverLockWaitTime(UINT32 waitMs)
{
	UINT8 scsyslockchktime_7_0;
...
************************************************************************************/
/**
 * DEMOD_H13_ISDBT_Set_CrGearshEn
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Set_CrGearshEn(BOOLEAN bEnable)
{
	UINT8 crgearshen;
...
************************************************************************************/
/**
 * DEMOD_H13_ISDBT_Set_PhaseNoiseEnhancement
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Set_PhaseNoiseEnhancement(BOOLEAN bEnhanced)
{
	...
	if (RET_OK != DEMOD_H13_ISDBT_Get_FFTMode( &txModeLock, &fftMode ))
	{
		DEMOD_PRINT("[DEMOD_13] FAIL (%s:%d) DEMOD_H13_ISDBT_Get_FFTMode() !!!\n", __F__, __L__);

		return RET_ERROR;
	}

	if (RET_OK != DEMOD_H13_ISDBT_Get_DelaySpreadStatus(&delaySpread))
	{
		DEMOD_PRINT("[DEMOD_13] FAIL (%s:%d) DEMOD_H13_ISDBT_Get_DelaySpreadStatus() !!!\n", __F__, __L__);
...
************************************************************************************/
/**
 * DEMOD_H13_ISDBT_Get_DelaySpreadStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Get_DelaySpreadStatus(UINT16 *pDelaySpread)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Get_FFTMode( LX_DEMOD_LOCK_STATE_T *pTxModeLock, LX_DEMOD_FFTMODE_T *pFftMode )
{
...
************************************************************************************/
/**
 * DEMOD_H13_ISDBT_Get_GuradIntervalMode
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Get_GuradIntervalMode( LX_DEMOD_GIMODE_T *pGIMode )
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Get_TMCCInfo( LX_DEMOD_ISDBT_TMCC_CONFIG_T *pTmccParams)
{
	...
	if(NULL == pTmccParams)
	{
 	...
 	if (RET_OK != DEMOD_H13_ISDBT_Get_FFTMode( &txModeLock, &fftMode ))
 	{
 		DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_ISDBT_Get_FFTMode() !!!\n", __F__, __L__);
 		return RET_ERROR;
	}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Get_TotalInfo( void)
{
	...
	if (RET_OK != DEMOD_H13_ISDBT_Get_GuradIntervalMode( &gIMode ))
	{
	...
	if (RET_OK != DEMOD_H13_ISDBT_Get_TMCCInfo(&tmccParams))
	{
		DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_ISDBT_GetTMCCInfo() !!!\n", __F__, __L__);
	}
	...
	{
		DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_Get_NeverLockStatus() !!!\n", __F__, __L__);
	...
	if (RET_OK != DEMOD_H13_ISDBT_Get_DelaySpreadStatus(&delaySpread))
	{
		DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_ISDBT_Get_DelaySpreadStatus() !!!\n", __F__, __L__);
		delaySpread = 0xffff;
	}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Get_IFO_LOCK( BOOLEAN *pIfoLock)
{
	UINT8	syslock;
...
************************************************************************************/
/**
 * DEMOD_H13_ISDBT_Get_CochanDetIndicator
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_Get_CochanDetIndicator( BOOLEAN *pDetStatus)
{
...
************************************************************************************/
/**
 * DEMOD_H13_ISDBT_EqualizereReset.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H13_ISDBT_EqualizereReset(void)
{
...
************************************************************************************/
/**
 * DEMOD_H13_ISDBT_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_H13_ISDBT_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_RETUNE_RF:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_H13_ISDBT_Set_DefaultRegisterValue())
				{
					DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_H13_ISDBT_Set_DefaultRegisterValue() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				{
					if(RET_OK != DEMOD_H13_ISDBT_Set_Config_auto(TRUE))
					{
						DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_H13_ISDBT_Set_Config_auto() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if(RET_OK != DEMOD_H13_ISDBT_Set_PartialConfig(&(pDemodInfo->setCfgParam.isdbt)))
					{
				...
				else
				{
					...
					if(RET_OK != DEMOD_H13_ISDBT_Set_PartialConfig(&(pDemodInfo->setCfgParam.isdbt)))
					{
						DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_H13_ISDBT_Set_PartialConfig() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_H13_SoftwareReset())
				{
					DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_H13_SoftwareReset() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.isdbt.tuneMode)
				{
					...
					if(RET_OK != DEMOD_H13_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_ON, &scanProcessing, &scanLock))
					{
						DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_H13_NeverlockScan() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
					pDemodInfo->controlState = LX_DEMOD_TUNE_SCAN;
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				if(RET_OK != DEMOD_H13_Get_Lock(LX_DEMOD_PSYNC_LOCK, &lockState))
				{
					DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_H13_Get_Lock() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.isdbt.tuneMode)
					{
						...
						else
						{
							if(RET_OK != DEMOD_H13_Get_Packet_Error(&packetError))
							{
								DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_H13_Get_Packet_Error() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							else
							{
#if 0
								if(RET_OK != DEMOD_H13_ISDBT_Set_PhaseNoiseEnhancement(TRUE))
								{
									DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_H13_ISDBT_Set_PhaseNoiseEnhancement() !!!\n", __F__, __L__);
									DEMOD_RETURN_FAIL(__F__);
								}
#endif
								if(packetError < LX_DEMOD_PACKET_ERROR_THD)
								{
									...
									if (RET_OK != DEMOD_H13_Get_CarrierFreqOffset(&freqOffset))
									{
										DEMOD_PRINT( "^r^[DEMOD] FAIL (%s:%d) DEMOD_H13_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
										DEMOD_RETURN_FAIL(__F__);
									}
									pDemodInfo->freqOffset = freqOffset;
...
************************************************************************************/
/**
 * DEMOD_H13_ISDBT_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_H13_ISDBT_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if(pDemodInfo->signalStatus.packetError >= _gFullISDBTPkerrThreshold)
		{
			_gGBB_VQI_UnlockCount_full_packeterr++;
			DEMOD_PRINT( "^r^[DEMOD H13](%s) _gGBB_VQI_UnlockCount_full_packeterr = %d !!!\n", __F__,_gGBB_VQI_UnlockCount_full_packeterr);
		...
		else
		{
			...
			if(gPowerOnInitISDBT == 2)
			{

				powerOnElapsedTime = jiffies_to_msecs(jiffies) - gPowerOnInitStartTimeISDBT;
				...
				if(powerOnElapsedTime >= 60000)
				{
					...
					if (RET_OK != DEMOD_H13_ISDBT_Set_CrGearshEn(TRUE ))
					{
						DEMOD_PRINT("[DEMOD H13] FAIL (%s:%d) DEMOD_H13_ISDBT_Set_CrGearshEn() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				}
			}
