...
static volatile LX_AUD_REG_H13_T *g_pRealRegAud = NULL;
static LX_AUD_REG_H13_T	*g_pTempRegAud = NULL;
...
SINT32 H13_AUDIO_IOREMAP( void )
{
	...
	if(g_pRealRegAud == NULL)
	{
		...
		g_pRealRegAud = (volatile LX_AUD_REG_H13_T *)ioremap(pstAudRegCfg->reg_base_addr, pstAudRegCfg->reg_size);
		if(g_pRealRegAud == NULL)
		{
	...
	{
		//ioremap for HW register
		g_pTempRegAud = (LX_AUD_REG_H13_T *)kmalloc(sizeof(LX_AUD_REG_H13_T), GFP_KERNEL);
		if(g_pTempRegAud == NULL)
		{
			AUD_KDRV_ERROR("ERROR : can't allocate for register\n");
			return -EIO;
...
SINT32 H13_AUDIO_WriteReg ( LX_AUD_REG_ADDRESS_T addr, UINT32 data )
{
	...
	{
		AUD_H13_Wr(aud_dbg_print_mask, data);
		AUD_H13_WrFL(aud_dbg_print_mask);
	}
	else if(addr == LX_AUD_REG_INT0_CLR)
	{
		AUD_H13_Wr(aud_int0_clr, data);
		AUD_H13_WrFL(aud_int0_clr);
	}
	else if(addr == LX_AUD_REG_INT1_CLR)
	{
		AUD_H13_Wr(aud_int1_clr, data);
		AUD_H13_WrFL(aud_int1_clr);
	}
	else if(addr == LX_AUD_REG_DSP0_GSTC)
	{
		AUD_H13_Wr(aud_dsp0_gstc, data);
		AUD_H13_WrFL(aud_dsp0_gstc);
	}
	else if(addr == LX_AUD_REG_DSP1_GSTC)
	{
		AUD_H13_Wr(aud_dsp1_gstc, data);
		AUD_H13_WrFL(aud_dsp1_gstc);
	}
	else if(addr == LX_AUD_REG_DEBUG_DSP0_RESET)
	{
		AUD_H13_Wr(aud_dbg_dsp0_reset, data);
		AUD_H13_WrFL(aud_dbg_dsp0_reset);
	}
	else if(addr == LX_AUD_REG_DEBUG_DSP1_RESET)
	{
		AUD_H13_Wr(aud_dbg_dsp1_reset, data);
		AUD_H13_WrFL(aud_dbg_dsp1_reset);
	}
	else if(addr == LX_AUD_REG_DEC0_CBT)
	{
		AUD_H13_Wr(aud_dec0_cbt, data);
		AUD_H13_WrFL(aud_dec0_cbt);
	}
	else if(addr == LX_AUD_REG_DEC0_SBT)
	{
		AUD_H13_Wr(aud_dec0_sbt, data);
		AUD_H13_WrFL(aud_dec0_sbt);
	}
	else if(addr == LX_AUD_REG_DEC1_CBT)
	{
		AUD_H13_Wr(aud_dec1_cbt, data);
		AUD_H13_WrFL(aud_dec1_cbt);
	}
	else if(addr == LX_AUD_REG_DEC1_SBT)
	{
		AUD_H13_Wr(aud_dec1_sbt, data);
		AUD_H13_WrFL(aud_dec1_sbt);
	}
	else if(addr == LX_AUD_REG_GSTC_LOW)
	{
		AUD_H13_Wr(aud_gstcc0lreg, data);
		AUD_H13_WrFL(aud_gstcc0lreg);
	}
	else if(addr == LX_AUD_REG_LEVELMO_L)
	{
		AUD_H13_Wr(aud_levelmo_l, data);
		AUD_H13_WrFL(aud_levelmo_l);
	}
	else if(addr == LX_AUD_REG_LEVELMO_R)
	{
		AUD_H13_Wr(aud_levelmo_r, data);
		AUD_H13_WrFL(aud_levelmo_r);
	}
	else if(addr == LX_AUD_REG_AAD_BYPASS)
	{
		//Enable SCART bypass function for AUD reg.
		AUD_H13_RdFL(aad_bypass);
		AUD_H13_Wr01(aad_bypass, aad2scartbypassctrl, data);
		AUD_H13_WrFL(aad_bypass);
	}
	else if(addr == LX_AUD_REG_DSP0_IRQ)
	{
		AUD_H13_Wr(aud_dirq0, data);
		AUD_H13_WrFL(aud_dirq0);
	}
	else if(addr == LX_AUD_REG_DSP1_IRQ)
	{
		AUD_H13_Wr(aud_dirq1, data);
		AUD_H13_WrFL(aud_dirq1);
	}
...
UINT32 H13_AUDIO_ReadReg ( LX_AUD_REG_ADDRESS_T addr )
{
	...
	if(addr == LX_AUD_REG_DEBUG_PRINT_MASK)
	{
		AUD_H13_RdFL(aud_dbg_print_mask);
		data = AUD_H13_Rd(aud_dbg_print_mask);
	}
	else if(addr == LX_AUD_REG_INT0_CLR)
	{
		AUD_H13_RdFL(aud_int0_clr);
		data = AUD_H13_Rd(aud_int0_clr);
	}
	else if(addr == LX_AUD_REG_INT1_CLR)
	{
		AUD_H13_RdFL(aud_int1_clr);
		data = AUD_H13_Rd(aud_int1_clr);
	}
	else if(addr == LX_AUD_REG_DSP0_GSTC)
	{
		AUD_H13_RdFL(aud_dsp0_gstc);
		data = AUD_H13_Rd(aud_dsp0_gstc);
	}
	else if(addr == LX_AUD_REG_DSP1_GSTC)
	{
		AUD_H13_RdFL(aud_dsp1_gstc);
		data = AUD_H13_Rd(aud_dsp1_gstc);
	}
	else if(addr == LX_AUD_REG_DEBUG_DSP0_RESET)
	{
		AUD_H13_RdFL(aud_dbg_dsp0_reset);
		data = AUD_H13_Rd(aud_dbg_dsp0_reset);
	}
	else if(addr == LX_AUD_REG_DEBUG_DSP1_RESET)
	{
		AUD_H13_RdFL(aud_dbg_dsp1_reset);
		data = AUD_H13_Rd(aud_dbg_dsp1_reset);
	}
	else if(addr == LX_AUD_REG_DEC0_CBT)
	{
		AUD_H13_RdFL(aud_dec0_cbt);
		data = AUD_H13_Rd(aud_dec0_cbt);
	}
	else if(addr == LX_AUD_REG_DEC0_SBT)
	{
		AUD_H13_RdFL(aud_dec0_sbt);
		data = AUD_H13_Rd(aud_dec0_sbt);
	}
	else if(addr == LX_AUD_REG_DEC1_CBT)
	{
		AUD_H13_RdFL(aud_dec1_cbt);
		data = AUD_H13_Rd(aud_dec1_cbt);
	}
	else if(addr == LX_AUD_REG_DEC1_SBT)
	{
		AUD_H13_RdFL(aud_dec1_sbt);
		data = AUD_H13_Rd(aud_dec1_sbt);
	}
	else if(addr == LX_AUD_REG_GSTC_LOW)
	{
		AUD_H13_RdFL(aud_gstcc0lreg);
		data = AUD_H13_Rd(aud_gstcc0lreg);
	}
	else if(addr == LX_AUD_REG_LEVELMO_L)
	{
		AUD_H13_RdFL(aud_levelmo_l);
		data = AUD_H13_Rd(aud_levelmo_l);
	}
	else if(addr == LX_AUD_REG_LEVELMO_R)
	{
		AUD_H13_RdFL(aud_levelmo_r);
		data = AUD_H13_Rd(aud_levelmo_r);
	}
	else if(addr == LX_AUD_REG_AAD_BYPASS)
	{
		AUD_H13_RdFL(aad_bypass);
		AUD_H13_Rd01(aad_bypass, aad2scartbypassctrl, data);
	}
	else if(addr == LX_AUD_REG_DSP0_IRQ)
	{
		AUD_H13_RdFL(aud_dirq0);
		data = AUD_H13_Rd(aud_dirq0);
	}
	else if(addr == LX_AUD_REG_DSP1_IRQ)
	{
		AUD_H13_RdFL(aud_dirq1);
		data = AUD_H13_Rd(aud_dirq1);
	}
...
BOOLEAN H13_AUDIO_CheckResetStatus ( UINT8 ui8ResetFlag )
{
	...
	if(ui8ResetFlag == AUD_OPTION_DSP0)
	{
		...
		AUD_H13_RdFL(aud_swreset);
		AUD_H13_Rd01(aud_swreset, dsp0bresetout, dsp0bresetout);	//12 dsp0bresetout : reset for DSP0
	...
	else if(ui8ResetFlag == AUD_OPTION_DSP1)
	{
		...
		AUD_H13_RdFL(aud_swreset);
		AUD_H13_Rd01(aud_swreset, dsp1bresetout, dsp1bresetout);	//13 dsp1bresetout : reset for DSP1
...
SINT32 H13_AUDIO_SetReset ( UINT8 ui8ResetFlag )
{
	...
	if (ui8ResetFlag == AUD_OPTION_DSP0)
	{
		...
		AUD_H13_Wr01(aud_swreset, dsp0ocdresetout, 1);		//10	dsp0ocdresetout    	reset for On chip debugger of DSP0
		AUD_H13_Wr01(aud_swreset, dsp0bresetout, 1);		//12	dsp0bresetout      	reset for DSP0
	}
	else if (ui8ResetFlag == AUD_OPTION_DSP1)
	{
		/* Reset g_AudKdrv.IsDSP0Ready value */
		...
		AUD_H13_Wr01(aud_swreset, dsp1ocdresetout, 1);		//11	dsp1ocdresetout    	reset for On chip debugger of DSP1
		AUD_H13_Wr01(aud_swreset, dsp1bresetout, 1);		//13	dsp1bresetout      	reset for DSP1
	}
	else if (ui8ResetFlag == AUD_OPTION_AAD)
	{
		AUD_H13_Wr01(aud_swreset, aadresetout, 1);			//14 	aadresetout        	reset for operation of Analog audio decoder
		AUD_H13_Wr01(aud_swreset, aadapbresetout, 1);		//15 	aadapbresetout     	reset for AAD register interface
	}
	else if (ui8ResetFlag == AUD_OPTION_OTHERS)
	{
		AUD_H13_Wr01(aud_swreset, aresetout, 1);		//0	 	aresetout          	reset for operation of peripherals
		AUD_H13_Wr01(aud_swreset, apbresetout, 1);		//1	 	apbresetout        	reset for register interface if this set, all registers are cleared
		AUD_H13_Wr01(aud_swreset, aversetout, 1);			//2  	aversetout         	reset for TE data input interface,
		AUD_H13_Wr01(aud_swreset, veresetout, 0);			 //3  	veresetout         	reset for TE data output interface (encoder)
		AUD_H13_Wr01(aud_swreset, fs20resetout, 1);		//4	 	fs20resetout       	reset for audio PCM output interface
		AUD_H13_Wr01(aud_swreset, fs21resetout, 1);		//5		fs21resetout       	reset for audio SPDIF output interface
		AUD_H13_Wr01(aud_swreset, fs23resetout, 1);		//6 	fs23resetout		reset for audio HWSRC0 data input
		AUD_H13_Wr01(aud_swreset, fs24resetout, 1);		//7 	fs24resetout		reset for audio HWSRC0 datat output
		AUD_H13_Wr01(aud_swreset, asrcrstout, 1);		//9	 	asrcrstout         	reset for audio HWSRC operation
		AUD_H13_Wr01(aud_swreset, aadresetout, 1);		//14 	aadresetout        	reest for operation of Analog audio decoder
		AUD_H13_Wr01(aud_swreset, aadapbresetout, 1);	//15 	aadapbresetout     	reset for AAD register interface
		AUD_H13_Wr01(aud_swreset, uartresetout, 1);		//16	uartresetout		reset for audio UART
	}
	else if (ui8ResetFlag == AUD_OPTION_ALL)
	{
		...
		AUD_H13_Wr01(aud_swreset, apbresetout, 1);				//1 	apbresetout 		reset for register interface if this set, all registers are cleared
		AUD_H13_Wr01(aud_swreset, aversetout, 1);				//2 	aversetout			reset for TE data input interface,
		AUD_H13_Wr01(aud_swreset, veresetout, 1);				//3 	veresetout			reset for TE data output interface (encoder)
		AUD_H13_Wr01(aud_swreset, fs20resetout, 1);				//4 	fs20resetout		reset for audio PCM output interface
		AUD_H13_Wr01(aud_swreset, fs21resetout, 1);			//5 	fs21resetout		reset for audio SPDIF output interface
		AUD_H13_Wr01(aud_swreset, fs23resetout, 1);			//6 	fs23resetout		reset for audio HWSRC0 data input
		AUD_H13_Wr01(aud_swreset, fs24resetout, 1);			//7 	fs24resetout		reset for audio HWSRC0 datat output
	...
	AUD_H13_WrFL(aud_swreset);
	AUD_KDRV_PRINT("Reset reg val [0x%x]\n", AUD_H13_Rd(aud_swreset) );
	...
	if (ui8ResetFlag == AUD_OPTION_DSP0)
	{
		...
		AUD_H13_Wr(aud_int0_en, 0x1);
		AUD_H13_WrFL(aud_int0_en);
	}
	else if (ui8ResetFlag == AUD_OPTION_DSP1)
	{
		udelay(AUD_RESET_DELAY);	//IC => 50 clk : 125ns
		...
		AUD_H13_Wr(aud_int1_en, 0x1);
		AUD_H13_WrFL(aud_int1_en);
	}
	else if (ui8ResetFlag == AUD_OPTION_AAD)
	{
...
SINT32 H13_AUDIO_ClearReset ( UINT8 ui8ResetFlag )
{
	...
	AUD_H13_Rd01(aud_swreset, aresetout, aresetout);		//0 	aresetout			reset for operation of peripherals
	AUD_H13_Rd01(aud_swreset, apbresetout, apbresetout);		//1	apbresetout 		reset for register interface if this set, all registers are cleared

	if (ui8ResetFlag == AUD_OPTION_DSP0)
	{
		//Disable audio interrupt DSP0
		AUD_H13_Wr(aud_int0_en, 0x0);
		AUD_H13_WrFL(aud_int0_en);

		AUD_H13_Wr01(aud_swreset, dsp0ocdresetout, 0);		//10 dsp0ocdresetout    	reset for On chip debugger of DSP0
		AUD_H13_Wr01(aud_swreset, dsp0bresetout, 0);		//12 dsp0bresetout      	reset for DSP0
	}
	else if (ui8ResetFlag == AUD_OPTION_DSP1)
	{
		//Disable audio interrupt DSP1
		AUD_H13_Wr(aud_int1_en, 0x0);
		AUD_H13_WrFL(aud_int1_en);

		AUD_H13_Wr01(aud_swreset, dsp1ocdresetout, 0);		//11 dsp1ocdresetout    	reset for On chip debugger of DSP1
		AUD_H13_Wr01(aud_swreset, dsp1bresetout, 0);		//13 dsp1bresetout      	reset for DSP1
	}
	else if (ui8ResetFlag == AUD_OPTION_AAD)
	{
		AUD_H13_Wr01(aud_swreset, aadresetout, 0);			//14 aadresetout        	reest for operation of Analog audio decoder
		AUD_H13_Wr01(aud_swreset, aadapbresetout, 0);		//15 aadapbresetout     	reset for AAD register interface
	}
	else if (ui8ResetFlag == AUD_OPTION_OTHERS)
	{
		//This reset is cleared oncely after booting
		if(apbresetout == 0)
			AUD_H13_Wr01(aud_swreset, aresetout, 0);		//0	 	aresetout          	reset for operation of peripherals

		//This reset is cleared oncely after booting
		if(apbresetout == 0)
			AUD_H13_Wr01(aud_swreset, apbresetout, 0);		//1	 	apbresetout        	reset for register interface if this set, all registers are cleared

		AUD_H13_Wr01(aud_swreset, aversetout, 0);					//2   aversetout         	reset for TE data input interface,
		AUD_H13_Wr01(aud_swreset, veresetout, 0);			//3	 	veresetout         	reset for TE data output interface (encoder)

		AUD_H13_Wr01(aud_swreset, fs20resetout, 0);			//4	 	fs20resetout       	reset for audio PCM output interface
		AUD_H13_Wr01(aud_swreset, fs21resetout, 0);			//5	 	fs21resetout      	reset for audio SPDIF output interface
		AUD_H13_Wr01(aud_swreset, fs23resetout, 0);			//6 	fs23resetout		reset for audio HWSRC0 data input
		AUD_H13_Wr01(aud_swreset, fs24resetout, 0);			//7 	fs24resetout		reset for audio HWSRC0 datat output
		AUD_H13_Wr01(aud_swreset, asrcrstout, 0);			//9	 	asrcrstout         	reset for audio HWSRC operation
		AUD_H13_Wr01(aud_swreset, aadresetout, 0);			//14 	aadresetout        	reest for operation of Analog audio decoder
		AUD_H13_Wr01(aud_swreset, aadapbresetout, 0);		//15 	aadapbresetout     	reset for AAD register interface
		AUD_H13_Wr01(aud_swreset, uartresetout, 0);			//16	uartresetout		reset for audio UART
	}
	else if (ui8ResetFlag == AUD_OPTION_ALL)
	{
		//This reset is cleared oncely after booting
	...
	if (g_AudKdrv.IsDSP0Reseted == 1 && ui8ResetFlag == AUD_OPTION_DSP0)
	{
		...
		//Stop AUD DSP 0
		AUD_H13_Wr(aud_runstall0, 0x1);
		AUD_H13_WrFL(aud_runstall0);
	...
	else if (g_AudKdrv.IsDSP1Reseted == 1 && ui8ResetFlag == AUD_OPTION_DSP1)
	{
		...
		//Stop AUD DSP 1
		AUD_H13_Wr(aud_runstall1, 0x1);
		AUD_H13_WrFL(aud_runstall1);
...
SINT32 H13_AUDIO_InitRegister ( void )
{
	...
	AUD_H13_Wr(aud_dsp0offset5, pstMemCfgAud->dsp0_memory_base + DSP0_VIR_ADDR_OFFSET);
	AUD_H13_WrFL(aud_dsp0offset5);

	AUD_H13_Wr(aud_dsp0offset6, pstMemCfgAud->dsp0_memory_base);
	AUD_H13_WrFL(aud_dsp0offset6);

	//Share area for DSP0
	AUD_H13_Wr(aud_dsp0offset61, pstMemCfgAud->dpb0_memory_base);
	AUD_H13_WrFL(aud_dsp0offset61);

	//set address for DSP1
	AUD_H13_Wr(aud_dsp1offset5, pstMemCfgAud->dsp1_memory_base + DSP1_VIR_ADDR_OFFSET);
	AUD_H13_WrFL(aud_dsp1offset5);

	AUD_H13_Wr(aud_dsp1offset6, pstMemCfgAud->dsp1_memory_base);
	AUD_H13_WrFL(aud_dsp1offset6);

	//Share area for DSP1
	AUD_H13_Wr(aud_dsp1offset61, pstMemCfgAud->dpb0_memory_base);
	AUD_H13_WrFL(aud_dsp1offset61);

	AUD_KDRV_DEBUG_TMP("ADEC_RegInit - aud_dsp0offset5[0x%08X]\n", AUD_H13_Rd(aud_dsp0offset5));
	AUD_KDRV_DEBUG_TMP("ADEC_RegInit - aud_dsp0offset6[0x%08X]\n", AUD_H13_Rd(aud_dsp0offset6));
	AUD_KDRV_DEBUG_TMP("ADEC_RegInit - aud_dsp0offset6[0x%08X]\n", AUD_H13_Rd(aud_dsp0offset61));
	AUD_KDRV_DEBUG_TMP("ADEC_RegInit - aud_dsp1offset5[0x%08X]\n", AUD_H13_Rd(aud_dsp1offset5));
	AUD_KDRV_DEBUG_TMP("ADEC_RegInit - aud_dsp1offset6[0x%08X]\n", AUD_H13_Rd(aud_dsp1offset6));
	AUD_KDRV_DEBUG_TMP("ADEC_RegInit - aud_dsp1offset6[0x%08X]\n", AUD_H13_Rd(aud_dsp1offset61));

	//Set base address and size
	AUD_H13_Wr(aud_mem_share_base, pstMemCfgAud->dpb0_memory_base);
	AUD_H13_WrFL(aud_mem_share_base);
	AUD_H13_RdFL(aud_mem_share_base);

	//Set a DSP1 Share Memory Address
	g_AudKdrv.p_aud_mem_share_base = (void *)&g_pRealRegAud->aud_mem_share_base;

	AUD_H13_Wr(aud_mem_dsp0_base, pstMemCfgAud->dsp0_memory_base);
	AUD_H13_WrFL(aud_mem_dsp0_base);

	AUD_H13_Wr(aud_mem_dsp1_base, pstMemCfgAud->dsp1_memory_base);
	AUD_H13_WrFL(aud_mem_dsp1_base);

	//Sound Engine parameters
	AUD_H13_Wr(aud_mem_se_param_base, pstMemCfgAud->se_memory_base);
	AUD_H13_WrFL(aud_mem_se_param_base);
	AUD_H13_Wr(aud_mem_se_param_size, pstMemCfgAud->se_memory_size);
	AUD_H13_WrFL(aud_mem_se_param_size);

	//IPC
	// from ARM to DSP0 (DSP0 - Write: IPC_WRITE_TO_DSP0)
	AUD_H13_Wr(aud_mem_ipc_a2d_base, pstMemCfgAud->ipc_memory_base);
	AUD_H13_WrFL(aud_mem_ipc_a2d_base);
	AUD_H13_Wr(aud_mem_ipc_a2d_size, AUDIO_IPC_BUF_SIZE);
	AUD_H13_WrFL(aud_mem_ipc_a2d_size);

	// from DSP0 to ARM (DSP0 - Read:  IPC_READ_FROM_DSP0)
	AUD_H13_Wr(aud_mem_ipc_d2a_base, pstMemCfgAud->ipc_memory_base + (AUDIO_IPC_BUF_SIZE * 1));
	AUD_H13_WrFL(aud_mem_ipc_d2a_base);
	AUD_H13_Wr(aud_mem_ipc_d2a_size, AUDIO_IPC_BUF_SIZE);
	AUD_H13_WrFL(aud_mem_ipc_d2a_size);

	// from ARM to DSP1 (DSP1 - Write: IPC_WRITE_TO_DSP1)
	AUD_H13_Wr(aud_mem_ipc_a2p_base, pstMemCfgAud->ipc_memory_base + (AUDIO_IPC_BUF_SIZE * 2));
	AUD_H13_WrFL(aud_mem_ipc_a2p_base);
	AUD_H13_Wr(aud_mem_ipc_a2p_size, AUDIO_IPC_BUF_SIZE);
	AUD_H13_WrFL(aud_mem_ipc_a2p_size);

	// from DSP1 to ARM (DSP1 - Read: IPC_READ_FROM_DSP1)
	AUD_H13_Wr(aud_mem_ipc_p2a_base, pstMemCfgAud->ipc_memory_base + (AUDIO_IPC_BUF_SIZE * 3));
	AUD_H13_WrFL(aud_mem_ipc_p2a_base);
	AUD_H13_Wr(aud_mem_ipc_p2a_size, AUDIO_IPC_BUF_SIZE);
	AUD_H13_WrFL(aud_mem_ipc_p2a_size);

	// from DSP0 to DSP1
	AUD_H13_Wr(aud_mem_ipc_d2p_base, pstMemCfgAud->ipc_memory_base + (AUDIO_IPC_BUF_SIZE * 4));
	AUD_H13_WrFL(aud_mem_ipc_d2p_base);
	AUD_H13_Wr(aud_mem_ipc_d2p_size, AUDIO_IPC_BUF_SIZE);
	AUD_H13_WrFL(aud_mem_ipc_d2p_size);

	// from DSP1 to DSP0
	AUD_H13_Wr(aud_mem_ipc_p2d_base, pstMemCfgAud->ipc_memory_base + (AUDIO_IPC_BUF_SIZE * 5));
	AUD_H13_WrFL(aud_mem_ipc_p2d_base);
	AUD_H13_Wr(aud_mem_ipc_p2d_size, AUDIO_IPC_BUF_SIZE);
	AUD_H13_WrFL(aud_mem_ipc_p2d_size);

	// DEBUG - DSP0
	AUD_H13_Wr(aud_mem_ipc_dbgd_base, pstMemCfgAud->ipc_memory_base + (AUDIO_IPC_BUF_SIZE * 6));
	AUD_H13_WrFL(aud_mem_ipc_dbgd_base);
	AUD_H13_Wr(aud_mem_ipc_dbgd_size, (AUDIO_IPC_BUF_SIZE << 1));
	AUD_H13_WrFL(aud_mem_ipc_dbgd_size);

	// DEBUG - DSP1
	AUD_H13_Wr(aud_mem_ipc_dbgp_base, pstMemCfgAud->ipc_memory_base + (AUDIO_IPC_BUF_SIZE * 8));
	AUD_H13_WrFL(aud_mem_ipc_dbgp_base);
	AUD_H13_Wr(aud_mem_ipc_dbgp_size, (AUDIO_IPC_BUF_SIZE << 1));
	AUD_H13_WrFL(aud_mem_ipc_dbgp_size);

	// Basetime of Lipsync
	AUD_H13_Wr(aud_dec0_cbt, 0xFFFFFFFF);	// Init DEC0 Clock Basetime
	AUD_H13_WrFL(aud_dec0_cbt);
	AUD_H13_Wr(aud_dec0_sbt, 0xFFFFFFFF);	// Init DEC0 Stream Basetime
	AUD_H13_WrFL(aud_dec0_sbt);
	AUD_H13_Wr(aud_dec1_cbt, 0xFFFFFFFF);	// Init DEC1 Clock Basetime
	AUD_H13_WrFL(aud_dec1_cbt);
	AUD_H13_Wr(aud_dec1_sbt, 0xFFFFFFFF);	// Init DEC1 Stream Basetime
	AUD_H13_WrFL(aud_dec1_sbt);
	AUD_H13_Wr(aud_basepts, 0xFFFFFFFF);	// Init MHEG Stream Basetime
	AUD_H13_WrFL(aud_basepts);
	AUD_H13_Wr(aud_basestc, 0xFFFFFFFF);	// Init MHEG Clock Basetime
	AUD_H13_WrFL(aud_basestc);

	// Presented PTS at DEC0
	AUD_H13_Wr(aud_presented_pts_lip0, 0xFFFFFFFF);	// Init Presented PTS at DEC0
	AUD_H13_WrFL(aud_presented_pts_lip0);

	// Presented PTS at DEC1
	AUD_H13_Wr(aud_presented_pts_lip1, 0xFFFFFFFF);	// Init Presented PTS at DEC1
	AUD_H13_WrFL(aud_presented_pts_lip1);

	// TP Buffer size at DEC0
	AUD_H13_Wr01(aud_tpm_bufsize, max_size, 0x3A0);	// Init TP Buffer Max size at DEC0
	AUD_H13_Wr01(aud_tpm_bufsize, free_size, 0x380);	// Init TP Buffer Free size at DEC0
	AUD_H13_WrFL(aud_tpm_bufsize);

	// TP Buffer size at DEC1
	AUD_H13_Wr01(aud_tpa_bufsize, max_size, 0x3A0);	// Init TP Buffer Max size at DEC1
	AUD_H13_Wr01(aud_tpa_bufsize, free_size, 0x380);	// Init TP Buffer Free size at DEC1
	AUD_H13_WrFL(aud_tpa_bufsize);

	//Check a chip revision
	if(lx_chip_rev() >= LX_CHIP_REV(H13, B0))
	{
		// Write a Main SOC Chip Version
		AUD_H13_Wr(aud_ver_chip, lx_chip_rev());	// Init Main SOC Chip Version
		AUD_H13_WrFL(aud_ver_chip);
	}
	else
	{
		AUD_KDRV_ERROR("<<< %s : LX_CHIP_REV => Unknown(0x%X)\n", __F__, lx_chip_rev());
		AUD_H13_Wr(aud_ver_chip, LX_CHIP_REV(H13, B0));	// Init Main SOC Chip Version
		AUD_H13_WrFL(aud_ver_chip);
	}
...
SINT32 H13_AUDIO_InitRegForCheckbit ( void )
{
	...
	for(j = 0; j < AUD_BUF_IF_REG_NUM; j++) // i means  wo, ro, wi, ri
	{
		for(k = 1; k < AUD_BUFFER_TYPE_MAX; k++)
		{
			...
			for(i = searchStart; i < searchEnd; i++)
			{
				AUD_H13_WrREG(((UINT32 *) _gAuBufMemCfg[i].pRegBase + j), 0xA0000000);
				AUD_KDRV_PRINT("0x%p = [%x]\n",((UINT32 *) _gAuBufMemCfg[i].pRegBase + j), AUD_H13_RdREG(((UINT32 *) _gAuBufMemCfg[i].pRegBase + j)));
			}
		}
	}

	//DDCO(SPDIF)
	AUD_H13_Wr(aud_buf_reverse_wo, 0xA0000000);
	AUD_H13_WrFL(aud_buf_reverse_wo);

	AUD_H13_Wr(aud_buf_reverse_ro, 0xA0000000);
	AUD_H13_WrFL(aud_buf_reverse_ro);

	AUD_H13_Wr(aud_buf_reverse_wi, 0xA0000000);
	AUD_H13_WrFL(aud_buf_reverse_wi);

	AUD_H13_Wr(aud_buf_reverse_ri, 0xA0000000);
	AUD_H13_WrFL(aud_buf_reverse_ri);
 ...
 * @see
*/
SINT32 H13_AUDIO_ClearDSP1VerInfo ( void )
{
	//Clear aud_dsp1_state reg. for codec ver and DSP1 running location
	AUD_H13_RdFL(aud_ver_dsp1);
	AUD_H13_Wr01(aud_ver_dsp1, ext,   0x0);
	AUD_H13_Wr01(aud_ver_dsp1, flag,  0x0);
	AUD_H13_Wr01(aud_ver_dsp1, fix,   0x0);
	AUD_H13_Wr01(aud_ver_dsp1, minor, 0x0);
	AUD_H13_Wr01(aud_ver_dsp1, major, 0x0);
	AUD_H13_WrFL(aud_ver_dsp1);

	//Clear aud_dsp1_state reg. for codec ver and DSP1 running location
	AUD_H13_RdFL(aud_ver_dsp1_common);
	AUD_H13_Wr01(aud_ver_dsp1_common, ext,   0x0);
	AUD_H13_Wr01(aud_ver_dsp1_common, flag,  0x0);
	AUD_H13_Wr01(aud_ver_dsp1_common, fix,   0x0);
	AUD_H13_Wr01(aud_ver_dsp1_common, minor, 0x0);
	AUD_H13_Wr01(aud_ver_dsp1_common, major, 0x0);
	AUD_H13_WrFL(aud_ver_dsp1_common);

	AUD_KDRV_DEBUG_TMP("H13_AUDIO_ClearDSP1VerInfo : Done!!!\n");
	return RET_OK;
}
...
void H13_AUDIO_DisplayDSP1VerInfo ( UINT64 startTick )
{
	...
	//Get codec version
	AUD_H13_RdFL(aud_ver_dsp1);
	AUD_H13_Rd01(aud_ver_dsp1, ext,   ext);
	AUD_H13_Rd01(aud_ver_dsp1, flag,  flag);
	AUD_H13_Rd01(aud_ver_dsp1, fix,   fix);
	AUD_H13_Rd01(aud_ver_dsp1, minor, minor);
	AUD_H13_Rd01(aud_ver_dsp1, major, major);
	...
	{
		AUD_H13_RdFL(aud_ver_dsp1_common);
		AUD_H13_Rd01(aud_ver_dsp1_common, ext,   ext);
		AUD_H13_Rd01(aud_ver_dsp1_common, flag,  flag);
		AUD_H13_Rd01(aud_ver_dsp1_common, fix,   fix);
		AUD_H13_Rd01(aud_ver_dsp1_common, minor, minor);
		AUD_H13_Rd01(aud_ver_dsp1_common, major, major);
 ...
 * @see
*/
SINT32 H13_AUDIO_ClearDSP0VerInfo ( void )
{
	//Clear aud_dsp0_state reg.
	AUD_H13_RdFL(aud_ver_dsp0);
	AUD_H13_Wr01(aud_ver_dsp0, ext,   0x0);
	AUD_H13_Wr01(aud_ver_dsp0, flag,  0x0);
	AUD_H13_Wr01(aud_ver_dsp0, fix,   0x0);
	AUD_H13_Wr01(aud_ver_dsp0, minor, 0x0);
	AUD_H13_Wr01(aud_ver_dsp0, major, 0x0);
	AUD_H13_WrFL(aud_ver_dsp0);

	// Clear DSP0's Common Layer Version
	AUD_H13_RdFL(aud_ver_dsp0_common);
	AUD_H13_Wr01(aud_ver_dsp0_common, ext,   0x0);
	AUD_H13_Wr01(aud_ver_dsp0_common, flag,  0x0);
	AUD_H13_Wr01(aud_ver_dsp0_common, fix,   0x0);
	AUD_H13_Wr01(aud_ver_dsp0_common, minor, 0x0);
	AUD_H13_Wr01(aud_ver_dsp0_common, major, 0x0);
	AUD_H13_WrFL(aud_ver_dsp0_common);

	AUD_KDRV_DEBUG_TMP("H13_AUDIO_ClearDSP0VerInfo : Done!!!\n");
	return RET_OK;
}
...
void H13_AUDIO_DisplayDSP0VerInfo ( UINT64 startTick )
{
	...
	AUD_H13_RdFL(aud_ver_dsp0);
	AUD_H13_Rd01(aud_ver_dsp0, ext,   ext);
	AUD_H13_Rd01(aud_ver_dsp0, flag,  flag);
	AUD_H13_Rd01(aud_ver_dsp0, fix,   fix);
	AUD_H13_Rd01(aud_ver_dsp0, minor, minor);
	AUD_H13_Rd01(aud_ver_dsp0, major, major);
	...
	{
		AUD_H13_RdFL(aud_ver_dsp0_common);
		AUD_H13_Rd01(aud_ver_dsp0_common, ext,   ext);
		AUD_H13_Rd01(aud_ver_dsp0_common, flag,  flag);
		AUD_H13_Rd01(aud_ver_dsp0_common, fix,   fix);
		AUD_H13_Rd01(aud_ver_dsp0_common, minor, minor);
		AUD_H13_Rd01(aud_ver_dsp0_common, major, major);
