...
#include "os_util.h"
#include "hdmi_hw_h13.h"
#include "hdmi_reg_h13.h"
#include "sys_regs.h"
 ...
 */
extern LX_HDMI_REG_T *pH13HdmiRegCfg;
extern HDMI_DATA_T *gH13BootData;

static HDMI_PORT_CONNECTION_TYPE_T _gH13HDMIConnectState = HDMI_PORT_NOT_CONNECTED;
static LX_HDMI_TIMING_INFO_T _gH13PrevTiming = {0,};
static UINT32 _gH13TimingReadCnt = 0;
static UINT32 _gH13HDMIState = HDMI_STATE_DISABLE;

static LX_HDMI_AVI_COLORSPACE_T	_gH13PrevPixelEncoding = LX_HDMI_AVI_COLORSPACE_RGB;
static LX_HDMI_INFO_PACKET_T _gH13PrevAVIPacket = {0, };
static LX_HDMI_INFO_PACKET_T _gH13PrevVSIPacket = {0, };
static BOOLEAN _gH13AVIReadState 	  = FALSE;
static BOOLEAN _gH13VSIState 	  = FALSE;
static BOOLEAN _gH13AVIChangeState 	  = FALSE;

//audio
static BOOLEAN _gH13AudioMuteState 	  = FALSE;
static BOOLEAN _gH13AudioArcStatus 	  = FALSE;	//This variable is used ARC Enabled Status to recover ARC mode after PHY reset.
static UINT32  _gH13IntrAudioState 	  = HDMI_AUDIO_INIT_STATE;
static UINT32  _gH13HdmiPortStableCount  = 0;	//This count is used to detect if HDMI Switch port is changed.
static UINT32  _gH13HdmiFreqErrorCount   = 0;	//This count is used to detect if HDMI TMDS Frequency and Frequency is different.
static UINT32  _gH13HdmiAudioThreadCount = 0;	//_gH13HdmiAudioThreadCount is increased in _HDMI_H13_Periodic_Audio_Task function
static UINT64  _gH13IntrBurstInfoCount	 = 0;	//This count is used to detect if burst info. interrupt is asserted count.
static UINT64  _gH13TaskBurstInfoCount	 = 0;	//This count is used to save _gH13IntrBurstInfoCount in hdmi task.
static UINT64  _gH13BurstInfoPrevJiffies = 0;	//This Jiffies is used to save previous burst info. interrupt jiffies.
static LX_HDMI_AUDIO_INFO_T _gH13HdmiAudioInfo = {LX_HDMI_AUDIO_DEFAULT, LX_HDMI_SAMPLING_FREQ_NONE};

//interrupt variables
static UINT32 _gH13Intr_vf_stable = HDMI_VIDEO_INIT_STATE;
static UINT32 _gH13Intr_avi = HDMI_AVI_INIT_STATE;
static UINT32 _gH13Intr_src_mute = HDMI_SOURCE_MUTE_CLEAR_STATE;
static UINT32 _gH13Intr_vsi = HDMI_VSI_INIT_STATE;

static UINT32 _gH13Force_thread_sleep  = 0;	//enable  = 2, disable  =0
static UINT32 _gH13HDMI_thread_running = 1;	//enable  = 1, disable  =0
static UINT32 _gH13HDMI_interrupt_registered = 0;

static UINT32 _gH13ChipPlatform = 0;

//wait_queue_head_t	WaitQueue_HDMI;
DECLARE_WAIT_QUEUE_HEAD(WaitQueue_HDMI_H13);

//HDMI Audio Variables
DEFINE_SPINLOCK(_gIntrHdmiH13AudioLock);
DEFINE_SPINLOCK(_gIntrHdmiH13VideoLock);
DEFINE_SPINLOCK(_gHdmiH13RegReadLock);

#define LINK_H13_RdFL(_r)											\
									do { 								\
										unsigned long __flags;				\
										spin_lock_irqsave(&_gHdmiH13RegReadLock, __flags);	\
										_LINK_H13_RdFL(_r);				\
										_LINK_H13_RdFL(_r);				\
										spin_unlock_irqrestore(&_gHdmiH13RegReadLock, __flags);	\
									} while(0)
 ...
 *	access functions
 */
static struct task_struct	*stH13HDMI_Thread;

PHY_REG_CTRL_H13_T  gPHY_REG_CTRL_H13;
static volatile HDMI_LINK_REG_H13_T *pstLinkReg;
static volatile HDMI_LINK_REG_H13_T *pstLinkShadowReg;


/******************************************************************************
 *				Local function
 *****************************************************************************/
static int _HDMI_H13_GetExtendTimingInfo(LX_HDMI_TIMING_INFO_T *info, UINT32 tmdsClock);
static int _HDMI_H13_ClearSWResetAll(void);			//H13D Chip - Ctop control
static int _HDMI_H13_PhyRunReset(void);
static int _HDMI_H13_RunReset(void);
static int _HDMI_H13_GetPortConnection(void);
static int _HDMI_H13_SetInternalMute(LX_HDMI_MUTE_CTRL_T interMute);
static int _HDMI_H13_SetVideoBlank(LX_HDMI_AVI_COLORSPACE_T space);
static int _HDMI_H13_EnableInterrupt(void);
static int _HDMI_H13_Thread(void *data);
irqreturn_t _HDMI_H13_IRQHandler(int irq, void *dev);
static void _HDMI_H13_Periodic_Task(void);
static void _HDMI_H13_ModifyUDFormat(LX_HDMI_TIMING_INFO_T *info, UINT32 tmdsClock);
static void _HDMI_H13_CheckUDExtInfo(LX_HDMI_TIMING_INFO_T *info, UINT32 tmdsClock);

//audio related.
static int _HDMI_H13_SetAudio(void);
static int _HDMI_H13_GetAudioInfo(void);
static int _HDMI_H13_GetAudioTypeAndFreq(LX_HDMI_AUDIO_TYPE_T *audioType, LX_HDMI_SAMPLING_FREQ_T *samplingFreq);
static int _HDMI_H13_GetAudioFreqFromTmdsClock(LX_HDMI_SAMPLING_FREQ_T *samplingFreqFromTmds);
static int _HDMI_H13_DebugAudioInfo(LX_HDMI_DEBUG_AUDIO_INFO_T *pAudioDebugInfo);

int HDMI_H13_HWInitial(void)
{
	gPHY_REG_CTRL_H13.shdw.addr = (UINT32 *)OS_KMalloc(sizeof(PHY_REG_H13_T));

#ifndef	USE_QEMU_SYSTEM
	pstLinkReg =(HDMI_LINK_REG_H13_T *)ioremap(pH13HdmiRegCfg->link_reg_base_addr, pH13HdmiRegCfg->link_reg_size);
#else
	pstLinkReg =(HDMI_LINK_REG_H13_T *)ioremap(pH13HdmiRegCfg->link_qemu_base_addr, pH13HdmiRegCfg->link_reg_size);
#endif

	HDMI_PRINT("HDMI LINK REG CFG \n");
	HDMI_PRINT("LINK REG Base Address	0x%08x\n", pH13HdmiRegCfg->link_reg_base_addr);
	HDMI_PRINT("LINK REG Size 			0x%08x\n", pH13HdmiRegCfg->link_reg_size);
	HDMI_PRINT("LINK REG Qumu Address 	0x%08x\n", pH13HdmiRegCfg->link_qemu_base_addr);
	...
	pstLinkShadowReg = (HDMI_LINK_REG_H13_T *)kmalloc(sizeof(HDMI_LINK_REG_H13_T), GFP_KERNEL);
	...
	//*
	_HDMI_H13_ClearSWResetAll();			//H13D Chip - Ctop control

	LINK_H13_RdFL(phy_00);
	LINK_H13_Wr01(phy_00, phy_enable, 0x1);
	LINK_H13_Wr01(phy_00, phy_pdb, 0x1);
	LINK_H13_Wr01(phy_00, phy_rstn, 0x0);			///< Phy Enable
	LINK_H13_Wr01(phy_00, phy_arc_pdb, 0x0);		///< Phy Enable
	LINK_H13_WrFL(phy_00);
	...
	LINK_H13_RdFL(phy_00);
	LINK_H13_Wr01(phy_00, phy_rstn, 0x1);			///< Phy Enable
	LINK_H13_WrFL(phy_00);

	_HDMI_H13_PhyRunReset();	// init Phy register
	_HDMI_H13_RunReset();		// init Link register

	//audio setting
	_HDMI_H13_SetAudio();

	_HDMI_H13_EnableInterrupt();

	stH13HDMI_Thread = kthread_create(_HDMI_H13_Thread, (void*)NULL, "hdmi_thread");
	if (stH13HDMI_Thread)
	{
		wake_up_process(stH13HDMI_Thread);
		HDMI_DEBUG("HDMI Thread [%d]\n", stH13HDMI_Thread->pid);
	}
	else
	{
	...
	HDMI_H13_SetPowerControl(0);

	HDMI_TRACE("H13 HDMI Initialize \n" );
	return RET_OK;
}
...
int HDMI_H13_Thread_Control(int sleep_enable)
{
	if (sleep_enable)
	{
		_gH13Force_thread_sleep = 2;
		_gH13HDMIState = HDMI_STATE_DISABLE;
		HDMI_DEBUG("H13 sleeping HDMI Thread \n");
	}
	else
	{
		_gH13HDMIState = HDMI_STATE_IDLE;
		_gH13Force_thread_sleep = 0;
		wake_up_interruptible(&WaitQueue_HDMI_H13);
		_gH13HDMI_thread_running = 1;
		HDMI_DEBUG("H13 Starting HDMI Thread\n");
	}
...
{
	ratio->eAspectRatio = (((_gH13PrevAVIPacket.dataBytes[0] &0xff0000)>>16) &0x30)>>4;		// M1M0
...
int HDMI_H13_GetColorDomain(LX_HDMI_COLOR_DOMAIN_T *color)
{
	...
	if (_gH13Intr_vf_stable == HDMI_VIDEO_INIT_STATE || _gH13Intr_avi == HDMI_AVI_INIT_STATE)
	{
		return RET_OK;
	}

	if (_gH13Intr_avi > HDMI_AVI_INIT_STATE)
	{
		color->bHdmiMode =  gH13BootData->mode;

		if (color->bHdmiMode)
		{
			if (_gH13AVIReadState)
				temp = _gH13PrevAVIPacket.dataBytes[0];
			else
			{
				LINK_H13_RdFL(packet_19);
				temp = LINK_H13_Rd(packet_19);
			}
...
int HDMI_H13_GetTimingInfo(LX_HDMI_TIMING_INFO_T *info)
{
	...
	do{
		if (_gH13HDMI_thread_running == 1 && _gH13HDMIState < HDMI_STATE_READ)
			break;

		if (_gH13HDMI_thread_running == 1 && _gH13HDMIState == HDMI_STATE_STABLE)
			goto func_exit;

		LINK_H13_RdFL(video_04);
		LINK_H13_Rd01(video_04, reg_h_tot, bufTiming.hTotal);		///< Horizontal total pixels
		LINK_H13_Rd01(video_04, reg_v_tot, bufTiming.vTotal);		///< Vertical total lines

		LINK_H13_RdFL(video_05);
		LINK_H13_Rd01(video_05, reg_h_av, bufTiming.hActive);		///< Horizontal active pixel
		LINK_H13_Rd01(video_05, reg_v_av, bufTiming.vActive);		///< Vertical active lines
#if 0
		LINK_H13_RdFL(video_06);
		LINK_H13_Rd01(video_06, reg_h_fp, bufTiming.hStart);		///< Horizontal start pixel (Front Porch)
		LINK_H13_Rd01(video_06, reg_v_fp, bufTiming.vStart);		///< Vertical start lines (Front Porch)
#endif
		LINK_H13_RdFL(video_07);
		LINK_H13_Rd01(video_07, reg_h_bp, bufTiming.hStart);	///< Horizontal start pixel (Back Porch)
		LINK_H13_Rd01(video_07, reg_v_bp, bufTiming.vStart);	///< Vertical start lines (Back Porch)

		LINK_H13_RdFL(video_10);
		LINK_H13_Rd01(video_10, reg_intrl, bufTiming.scanType);
		bufTiming.scanType^=1; 							///< Scan type (0 : interlace, 1 : progressive) 	info->scanType ^= 1;

		bufTiming.extInfo = 0; 	///< Full 3D Timing

		///* pixel clock *///
		PHY_REG_H13_RdFL(tmds_freq_1);
		PHY_REG_H13_RdFL(tmds_freq_2);

		PHY_REG_H13_Rd01(tmds_freq_1, tmds_freq, up_freq);
		PHY_REG_H13_Rd01(tmds_freq_2, tmds_freq, down_freq);
		tmdsClock = ((up_freq << 8) + down_freq); 	// XXX.XX KHz

		if (CHK_UHD_BD())
			_HDMI_H13_ModifyUDFormat(&bufTiming, tmdsClock);		// modify 297MHz format

		_HDMI_H13_GetExtendTimingInfo(&bufTiming, tmdsClock);		// get extend information

		LINK_H13_RdFL(video_02);
		LINK_H13_Rd01(video_02, reg_cmode_tx, colorDepth);
		...
		{
			HDMI_PRINT("[%d] %s : _HDMI_H13_GetExtendTimingInfo hActive, vActive  = 0\n", __L__, __F__);
			//memset(&bufTiming , 0 , sizeof(LX_HDMI_TIMING_INFO_T));
			goto func_exit;
		}
		...
		tmdsClock = ((up_freq << 8) + down_freq); 	// XXX.XX KHz
#endif
		LINK_H13_RdFL(video_00);
		LINK_H13_Rd01(video_00, reg_pr_tx, pixelRepet);
		...
		if ((_gH13HDMIState > HDMI_STATE_NO_SCDT && _gH13HDMIState <= HDMI_STATE_READ) ||_gH13HDMI_thread_running == 0)
		{
			if (_gH13Intr_vf_stable > HDMI_VIDEO_INIT_STATE)
			{
				CTOP_CTRL_H13A0_RdFL(ctr26);
				CTOP_CTRL_H13A0_Rd01(ctr26, phy_ppll_sel, tmp32);

				if (pixelRepet > 0)
				{
					if (tmp32 == 0)
					{
						CTOP_CTRL_H13A0_Wr01(ctr26, phy_ppll_sel, 0x2);
						CTOP_CTRL_H13A0_WrFL(ctr26);
				...
				else
				{
					if (tmp32 != 0)
					{
						CTOP_CTRL_H13A0_Wr01(ctr26, phy_ppll_sel, 0x0);
						CTOP_CTRL_H13A0_WrFL(ctr26);
...
func_cnt:
		...
		if ( _gH13TimingReadCnt > 100) 	goto func_exit;

		if ( memcmp(&_gH13PrevTiming, &bufTiming, sizeof(LX_HDMI_TIMING_INFO_T)) == 0 )
		{
			_gH13TimingReadCnt++;

			if (_gH13HDMIState == HDMI_STATE_READ && _gH13TimingReadCnt < 9)
				HDMI_VIDEO("[%d] %s : hActive[%d] vActive [%d] ReadCnt[%d] \n", __L__, __F__, bufTiming.hActive, bufTiming.vActive,_gH13TimingReadCnt);
		}
...
func_exit:
		...
		memcpy(&bufTiming, &_gH13PrevTiming, sizeof(LX_HDMI_TIMING_INFO_T));
	} while(0);

	info->hFreq 	= _gH13PrevTiming.hFreq 	= bufTiming.hFreq; 			///< Horizontal frequency(100 Hz unit) = Dot Freq / hTotal
	info->vFreq 	= _gH13PrevTiming.vFreq 	= bufTiming.vFreq; 			///< Veritical frequency(1/10 Hz unit) = hFreq / vTotal
	info->hTotal	= _gH13PrevTiming.hTotal 	= bufTiming.hTotal; 		///< Horizontal total pixels
	info->vTotal 	= _gH13PrevTiming.vTotal	= bufTiming.vTotal; 		///< Vertical total lines
	info->hStart	= _gH13PrevTiming.hStart	= bufTiming.hStart; 		///< Horizontal start pixel (Back Porch)
	info->vStart 	= _gH13PrevTiming.vStart	= bufTiming.vStart;			///< Vertical start lines (Back Porch)
	info->hActive 	= _gH13PrevTiming.hActive	= bufTiming.hActive;		///< Horizontal active pixel
	info->vActive 	= _gH13PrevTiming.vActive	= bufTiming.vActive; 		///< Vertical active lines
	info->scanType 	= _gH13PrevTiming.scanType	= bufTiming.scanType; 		///< Scan type (0 : interlace, 1 : progressive) 	info->scanType ^= 1;
	info->extInfo	= _gH13PrevTiming.extInfo	= bufTiming.extInfo; 	///< Full 3D Timing
	info->state = _gH13HDMIState;
...
int HDMI_H13_GetAviPacket(LX_HDMI_INFO_PACKET_T *packet)
{
	...
	{
		gAVIReadCnt = 0;
		memset(&_gH13PrevAVIPacket, 0 , sizeof(LX_HDMI_INFO_PACKET_T));
		//_gH13PrevAVIPacket.InfoFrameType = LX_HDMI_INFO_AVI;
		return RET_OK;
	}
	...
	if (_gH13Intr_avi == HDMI_AVI_INIT_STATE)
	{
		...
		if ( _gH13HDMI_thread_running == 1)
			packet->dataBytes[7] = gH13BootData->mode;		//HDMI Mode
		else
		{
			LINK_H13_RdFL(system_control_00);
			LINK_H13_Rd01(system_control_00, reg_hdmi_mode, data[7]);
			packet->dataBytes[7] = gH13BootData->mode = data[7];		//HDMI Mode
		}
		return RET_OK;
	}

	if ( _gH13HDMI_thread_running == 1 && _gH13Intr_avi == HDMI_AVI_STABLE_STATE)
		goto func_exit;

	if (_gH13Intr_avi == HDMI_AVI_CHANGE_STATE)
	{
		//gAVIReadCnt = 0;
		spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
		_gH13Intr_avi = HDMI_AVI_INTERRUPT_STATE;
		_gH13AVIReadState = FALSE;
		spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
	}

	if (gAVIReadCnt > 300)
	{
		spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
		_gH13Intr_avi = HDMI_AVI_STABLE_STATE;
		spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
	}

	LINK_H13_RdFL(packet_18);
	header = LINK_H13_Rd(packet_18);		// reg_pkt_avi_hdr_0 (AVI Packet Version), reg_pkt_avi_hdr_1 (AVI Packet Length)
	packet->header = (header &0xffff);

	LINK_H13_RdFL(packet_19);
	LINK_H13_RdFL(packet_20);
	LINK_H13_RdFL(packet_21);
	LINK_H13_RdFL(packet_22);
	LINK_H13_RdFL(packet_23);
	LINK_H13_RdFL(packet_24);
	LINK_H13_RdFL(packet_25);

	data[0] = LINK_H13_Rd(packet_19);
	data[1] = LINK_H13_Rd(packet_20);
	data[2] = LINK_H13_Rd(packet_21);
	data[3] = LINK_H13_Rd(packet_22);
	data[4] = LINK_H13_Rd(packet_23);
	data[5] = LINK_H13_Rd(packet_24);
	data[6] = LINK_H13_Rd(packet_25);
	...
	if (_gH13PrevPixelEncoding != csc)
	{
		_HDMI_H13_SetVideoBlank(csc);
		_gH13PrevPixelEncoding = csc;
	}

	_gH13AVIReadState = TRUE;

	if ( memcmp(&_gH13PrevAVIPacket, packet, sizeof(LX_HDMI_INFO_PACKET_T)) != 0 )
	{
		gAVIReadCnt = 0;
		HDMI_VIDEO("[%d] %s : changed AVI Packet / InfoFrameType gPre[%d] /packet[%d] \n", __L__, __F__, _gH13PrevAVIPacket.InfoFrameType, packet->InfoFrameType);
		HDMI_VIDEO("AVI header	gPre[%d] /packet[%d] \n", _gH13PrevAVIPacket.header, packet->header);
		HDMI_VIDEO("AVI data	gPre[0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x] \n", _gH13PrevAVIPacket.dataBytes[0], _gH13PrevAVIPacket.dataBytes[1], _gH13PrevAVIPacket.dataBytes[2], \
			_gH13PrevAVIPacket.dataBytes[3], _gH13PrevAVIPacket.dataBytes[4], _gH13PrevAVIPacket.dataBytes[5], _gH13PrevAVIPacket.dataBytes[6], _gH13PrevAVIPacket.dataBytes[7]);
		...
		memcpy(&_gH13PrevAVIPacket, packet, sizeof(LX_HDMI_INFO_PACKET_T));
		return RET_OK;
	}
	else
	{
...
func_exit:
	memcpy(packet, &_gH13PrevAVIPacket,  sizeof(LX_HDMI_INFO_PACKET_T));
	HDMI_PRINT("[%d] %s \n", __L__, __F__);
...
int HDMI_H13_GetVsiPacket(LX_HDMI_INFO_PACKET_T *packet)
{
	...
	//HDMI_DEBUG("%s, %d _gH13Intr_vf_stable= %d,_gH13Intr_vsi = %d, \n\n",__F__, __L__, _gH13Intr_vf_stable, _gH13Intr_vsi);

	if (_gH13Intr_vf_stable == HDMI_VIDEO_INIT_STATE ||_gH13Intr_vsi < HDMI_VSI_INTERRUPT_STATE)
	{
		gVSIReadCnt = 0;
		memset(&_gH13PrevVSIPacket, 0 , sizeof(LX_HDMI_INFO_PACKET_T));
		_gH13PrevVSIPacket.InfoFrameType = LX_HDMI_INFO_VSI;
		return RET_OK;
	}

	if (_gH13VSIState == TRUE)
	{
		if (lx_chip_rev() >= LX_CHIP_REV(H13, B0))
		{
			LINK_H13_RdFL(interrupt_enable_00);
			LINK_H13_Wr01(interrupt_enable_00, intr_no_vsi_enable, 0x1);			///< 20 intr_no_vsi_enable
			LINK_H13_WrFL(interrupt_enable_00);
			_gH13VSIState = FALSE;
			HDMI_DEBUG("[%d] %s : No VSI intra enable  \n", __LINE__, __func__);
		}
	}

	if ( _gH13HDMI_thread_running == 1 && _gH13Intr_vsi == HDMI_VSI_STABLE_STATE)
		goto func_exit;

	if (gVSIReadCnt > 200)
	{
		spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
		_gH13Intr_vsi = HDMI_VSI_STABLE_STATE;
		spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
	}

	LINK_H13_RdFL(packet_75);
	header = LINK_H13_Rd(packet_75);		// reg_pkt_avi_hdr_0 (AVI Packet Version), reg_pkt_avi_hdr_1 (AVI Packet Length)
	packet->header = (header &0xffff);

	LINK_H13_RdFL(packet_76);
	LINK_H13_RdFL(packet_77);
	LINK_H13_RdFL(packet_78);
	LINK_H13_RdFL(packet_79);
	LINK_H13_RdFL(packet_80);
	LINK_H13_RdFL(packet_81);
	LINK_H13_RdFL(packet_82);

	data[0] = LINK_H13_Rd(packet_76);
	data[1] = LINK_H13_Rd(packet_77);
	data[2] = LINK_H13_Rd(packet_78);
	data[3] = LINK_H13_Rd(packet_79);
	data[4] = LINK_H13_Rd(packet_80);
	data[5] = LINK_H13_Rd(packet_81);
	data[6] = LINK_H13_Rd(packet_82);
	...
	if ( memcmp(&_gH13PrevVSIPacket, packet, sizeof(LX_HDMI_INFO_PACKET_T)) != 0 )
	{
		gVSIReadCnt = 0;
		HDMI_VIDEO("[%d] %s : changed VSI Packet / InfoFrameType gPre[%d] /packet[%d] \n", __L__, __F__, _gH13PrevVSIPacket.InfoFrameType, packet->InfoFrameType);
		HDMI_VIDEO("VSI header	gPre[%d] /packet[%d] \n", _gH13PrevVSIPacket.header, packet->header);
		HDMI_VIDEO("VSI data	gPre[0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x] \n", _gH13PrevVSIPacket.dataBytes[0], _gH13PrevVSIPacket.dataBytes[1], _gH13PrevVSIPacket.dataBytes[2], \
			_gH13PrevVSIPacket.dataBytes[3], _gH13PrevVSIPacket.dataBytes[4], _gH13PrevVSIPacket.dataBytes[5], _gH13PrevVSIPacket.dataBytes[6], _gH13PrevVSIPacket.dataBytes[7]);
		HDMI_VIDEO("VSI data	packet[0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x] \n", 	packet->dataBytes[0], packet->dataBytes[1], packet->dataBytes[2], \
			packet->dataBytes[3], packet->dataBytes[4], packet->dataBytes[5], packet->dataBytes[6], packet->dataBytes[7]);

		memcpy(&_gH13PrevVSIPacket, packet, sizeof(LX_HDMI_INFO_PACKET_T));
		return RET_OK;
	}
	else
	{
...
func_exit:
	memcpy(packet, &_gH13PrevVSIPacket, sizeof(LX_HDMI_INFO_PACKET_T));
	HDMI_PRINT("[%d] %s \n", __L__, __F__);
...
int HDMI_H13_GetSpdPacket(LX_HDMI_INFO_PACKET_T *packet)
{
	...
	LINK_H13_RdFL(packet_59);
	header = LINK_H13_Rd(packet_59);
	packet->header = (header &0xffff);

	LINK_H13_RdFL(packet_60);
	LINK_H13_RdFL(packet_61);
	LINK_H13_RdFL(packet_62);
	LINK_H13_RdFL(packet_63);
	LINK_H13_RdFL(packet_64);
	LINK_H13_RdFL(packet_65);
	LINK_H13_RdFL(packet_66);

	data[0] = LINK_H13_Rd(packet_60);
	data[1] = LINK_H13_Rd(packet_61);
	data[2] = LINK_H13_Rd(packet_62);
	data[3] = LINK_H13_Rd(packet_63);
	data[4] = LINK_H13_Rd(packet_64);
	data[5] = LINK_H13_Rd(packet_65);
	data[6] = LINK_H13_Rd(packet_66);
...
int HDMI_H13_SetPowerControl(UINT32 power)
{
	if (power)
	{
		...
		PHY_REG_H13_RdFL(cr_mode_sel_resetb);
		PHY_REG_H13_Wr01(cr_mode_sel_resetb, cr_mode_sel_resetb, 0x0);
		PHY_REG_H13_WrFL(cr_mode_sel_resetb);

		OS_MsecSleep(1);	// ms delay

		PHY_REG_H13_RdFL(cr_mode_sel_resetb);
		PHY_REG_H13_Wr01(cr_mode_sel_resetb, cr_mode_sel_resetb, 0x1);
		PHY_REG_H13_WrFL(cr_mode_sel_resetb);
	}
	else
	{
		LINK_H13_RdFL(phy_00);
		LINK_H13_Wr01(phy_00, phy_pdb,  0x0);
		LINK_H13_Wr01(phy_00, phy_rstn, 0x0);		///< Phy Disable
		LINK_H13_WrFL(phy_00);
	}
...
{
	LINK_H13_RdFL(phy_00);
	LINK_H13_Wr01(phy_00, hpd0_out, hpd->bEnable);
	LINK_H13_WrFL(phy_00);
...
int HDMI_H13_GetStatus(LX_HDMI_STATUS_T *status)
{
	...
	do {
		if (_gH13HDMIState < HDMI_STATE_SCDT)
			break;

		status->bHdmiMode = gH13BootData->mode;

		status->eHotPlug = _gH13HDMIConnectState;

		LINK_H13_RdFL(video_02);
		LINK_H13_Rd01(video_02, reg_cmode_tx, status->eColorDepth);

		status->eColorDepth = (status->eColorDepth &0x60)>>5;

		LINK_H13_RdFL(video_00);
		LINK_H13_Rd01(video_00, reg_pr_tx, status->pixelRepet);

		status->csc = _gH13PrevPixelEncoding;
		csc = (((_gH13PrevAVIPacket.dataBytes[0] &0xff00)>>8) &0x60)>>5;
		if (_gH13PrevPixelEncoding == csc)		HDMI_DEBUG(" _gH13PrevPixelEncoding = %d , csc = %d \n", _gH13PrevPixelEncoding, csc);
		...
		HDMI_VIDEO("HDMI_H13_GetStatus  depth = 0x%x tmds =%d\n", status->eColorDepth, tmdsClock);
		HDMI_VIDEO("[HDMI] %d HDMI  State  = %d _gH13Force_thread_sleep= %d  _gH13HDMI_thread_running =%d \n",\
			__L__, _gH13HDMIState,_gH13Force_thread_sleep, _gH13HDMI_thread_running);
		HDMI_VIDEO("[HDMI]  AVI State[%d] VSI State[%d]  SRC Mute State[%d] \n",  _gH13Intr_avi, _gH13Intr_vsi, _gH13Intr_src_mute );
		HDMI_VIDEO("[HDMI]  H13 : 20140529 \n");
	} while (0);
...
int HDMI_H13_GetAudioInfo(LX_HDMI_AUDIO_INFO_T *pAudioInfo)
{
	...
	pAudioInfo->audioType	 = _gH13HdmiAudioInfo.audioType;
	pAudioInfo->samplingFreq = _gH13HdmiAudioInfo.samplingFreq;

	//Debug print
	if ((_gH13HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_1S) == 0)
	{
		HDMI_AUDIO("HDMI_GetAudioInfo : type = %d, freq = %d, state = %d, mute = %d\n",	\
					pAudioInfo->audioType, pAudioInfo->samplingFreq, _gH13IntrAudioState, _gH13AudioMuteState);
	}
	else
	{
...
int HDMI_H13_GetAudioCopyInfo(LX_HDMI_AUDIO_COPY_INFO_T *pCopyInfo)
{
	...
	LINK_H13_RdFL(system_control_00);
	LINK_H13_Rd01(system_control_00, reg_hdmi_mode, reg_hdmi_mode);

	//Get a hdmi audio copyright information.
	if (reg_hdmi_mode == 1)	//HDMI Mode
	{
		//Read  reg_achst_byte0 reg.
		LINK_H13_RdFL(audio_07);
		LINK_H13_Rd01(audio_07, reg_achst_byte0, reg_achst_byte0);
		...
		LINK_H13_RdFL(audio_08);
		LINK_H13_Rd01(audio_08, reg_achst_byte1, reg_achst_byte1);
		...
		if ((_gH13HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_1S) == 0)
		{
			HDMI_AUDIO("HDMI_H13_GetAudioCopyInfo : CopyInfo = %d(mode = %d)(0x%X, 0x%X)\n", pCopyInfo->copyInfo, reg_hdmi_mode, reg_achst_byte0, reg_achst_byte1);
		}
		else
		{
			HDMI_PRINT("HDMI_H13_GetAudioCopyInfo : CopyInfo = %d(mode = %d)(0x%X, 0x%X)\n", pCopyInfo->copyInfo, reg_hdmi_mode, reg_achst_byte0, reg_achst_byte1);
		}
	}
	else	//DVI Mode
	{
	...
	HDMI_PRINT("HDMI_H13_GetAudioCopyInfo : CopyInfo = %d(mode = %d)\n", pCopyInfo->copyInfo, reg_hdmi_mode);
	return ret;
}
...
{
	//ARC source
	LINK_H13_RdFL(heac_00);
	LINK_H13_Wr01(heac_00, reg_arc_src, 0x0);
	LINK_H13_WrFL(heac_00);

	//Enable or Disable ARC port
	LINK_H13_RdFL(phy_00);
	LINK_H13_Wr01(phy_00, phy_arc_pdb, (UINT32)pArcCtrl->bARCEnable);
	LINK_H13_WrFL(phy_00);

	//Update _gAudioArcStatus
	_gH13AudioArcStatus = pArcCtrl->bARCEnable;
...
int HDMI_H13_SetMute(LX_HDMI_MUTE_CTRL_T *pMuteCtrl)
{
	...
	LINK_H13_RdFL(system_control_02);
	LINK_H13_Rd01(system_control_02, reg_mute_vid, videoMuteState);
	...
	if (pMuteCtrl->eMode == LX_HDMI_VIDEO_MUTE || pMuteCtrl->eMode == LX_HDMI_AV_MUTE)
	{
		...
		{
			LINK_H13_Wr01(system_control_02, reg_mute_vid, pMuteCtrl->bVideoMute);
			LINK_H13_WrFL(system_control_02);
	...
	if (pMuteCtrl->eMode == LX_HDMI_AUDIO_MUTE || pMuteCtrl->eMode == LX_HDMI_AV_MUTE)
	{
		...
		if (_gH13AudioMuteState != pMuteCtrl->bAudioMute)
		{
			if (pMuteCtrl->bAudioMute == TRUE)
			{
				//Mute audio data
				LINK_H13_RdFL(audio_05);
				LINK_H13_Wr01(audio_05, reg_i2s_sd_en, 0x0);	//I2S SD Output Disable(4 Channel)
				LINK_H13_WrFL(audio_05);
			}
			else
			{
				//Un-mute audio data
				LINK_H13_RdFL(audio_05);
				LINK_H13_Wr01(audio_05, reg_i2s_sd_en, 0xF);	//I2S SD Output Enable(4 Channel)
				LINK_H13_WrFL(audio_05);
			}

			//Update _gH13AudioMuteState
			_gH13AudioMuteState = pMuteCtrl->bAudioMute;
		...
		if (pMuteCtrl->bAudioMute == FALSE)
		{
			HDMI_DEBUG("[%d]SetMute : type = %d, freq = %d, mute = %d, PSC = %d\n", \
						__L__, _gH13HdmiAudioInfo.audioType, _gH13HdmiAudioInfo.samplingFreq, _gH13AudioMuteState, _gH13HdmiPortStableCount);
		}
	}
...
int HDMI_H13_GetAudioDebugInfo(LX_HDMI_DEBUG_AUDIO_INFO_T *pAudioDebugInfo)
{
	...
	(void)_HDMI_H13_DebugAudioInfo(pAudioDebugInfo);

	HDMI_DEBUG("HDMI_H13_GetAudioDebugInfo : type = %d, freq = %d, state = %d, mute = %d, PSC = %d, video = %d\n", \
				_gH13HdmiAudioInfo.audioType, _gH13HdmiAudioInfo.samplingFreq, _gH13IntrAudioState, _gH13AudioMuteState, _gH13HdmiPortStableCount, _gH13HDMIState);
...
static void _HDMI_H13_CheckUDExtInfo(LX_HDMI_TIMING_INFO_T *info, UINT32 tmdsClock)
{
	...
	do {
		...
		if (_gH13Intr_avi > HDMI_AVI_INIT_STATE)
		{
			vic = ((_gH13PrevAVIPacket.dataBytes[1] &0xff) &0x7f);	// VIC6~VIC0
...
static int _HDMI_H13_GetExtendTimingInfo(LX_HDMI_TIMING_INFO_T *info, UINT32 tmdsClock)
{
	...
	do {
		...
		if (_gH13Intr_vsi < HDMI_VSI_INTERRUPT_STATE)
		{
			_HDMI_H13_CheckUDExtInfo(&bufTiming, tmdsClock);		// check UD format
			...
			{
				LINK_H13_RdFL(packet_20);		//AVI
				tmp32 = LINK_H13_Rd(packet_20);
	...
	if ( ( _gH13HDMI_thread_running == 0 &&  _gH13TimingReadCnt < 23 && _gH13TimingReadCnt > 20) ||
		(_gH13HDMIState >= HDMI_STATE_READ && _gH13TimingReadCnt < 6 && _gH13TimingReadCnt > 3) )
	{
		//HDMI_VIDEO(" VIC  = %d [gpreAVI - %d] \n", videoIdCode, __gH13PrevAVIPacket.eAviPacket.VideoIdCode);
...
static int _HDMI_H13_SetInternalMute(LX_HDMI_MUTE_CTRL_T interMute)
{
	...
	if (interMute.eMode == LX_HDMI_VIDEO_MUTE || interMute.eMode == LX_HDMI_AV_MUTE)
	{
		...
		{
			LINK_H13_Wr01(system_control_02, reg_mute_vid, interMute.bVideoMute);
			LINK_H13_WrFL(system_control_02);
	...
	{
		//Check a previous state
		if (_gH13AudioMuteState != interMute.bAudioMute)
		{
			...
			//Update _gH13AudioMuteState
			_gH13AudioMuteState = interMute.bAudioMute;
...
static int _HDMI_H13_SetVideoBlank(LX_HDMI_AVI_COLORSPACE_T space)
{
	...
	/* Blank Red */
	LINK_H13_RdFL(video_09);
	LINK_H13_Wr01(video_09, reg_vid_blank_r, blank_r);	//ACR Enable(Audio Clock Generation Function Activation)
	LINK_H13_WrFL(video_09);

	/* Blank Blue */
	LINK_H13_RdFL(video_10);
	LINK_H13_Wr01(video_10, reg_vid_blank_b, blank_b);	//ACR Enable(Audio Clock Generation Function Activation)
	LINK_H13_WrFL(video_10);

	HDMI_VIDEO("[%d] %s : changed video blank color space [%d]  \n",  __L__, __F__, space);
	return RET_OK;
}

static int _HDMI_H13_ClearSWResetAll(void)
{
	CTOP_CTRL_H13A0_RdFL(ctr00);
	CTOP_CTRL_H13A0_Wr01(ctr00, swrst_hdmi_dto, 0x1);
	CTOP_CTRL_H13A0_Wr01(ctr00, swrst_sys, 0x1);
	CTOP_CTRL_H13A0_Wr01(ctr00, swrst_hdcp, 0x1);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_tmds, 0x1);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_vfifo_w, 0x1);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_afifo_w, 0x1);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_pix_pip, 0x1);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_vfifo_r, 0x1);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_aud, 0x1);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_afifo_r, 0x1);
	CTOP_CTRL_H13A0_WrFL(ctr00);

	OS_MsecSleep(1);	// ms delay

	CTOP_CTRL_H13A0_RdFL(ctr00);
	CTOP_CTRL_H13A0_Wr01(ctr00, swrst_hdmi_dto, 0x0);
	CTOP_CTRL_H13A0_Wr01(ctr00, swrst_sys, 0x0);
	CTOP_CTRL_H13A0_Wr01(ctr00, swrst_hdcp, 0x0);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_tmds, 0x0);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_vfifo_w, 0x0);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_afifo_w, 0x0);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_pix_pip, 0x0);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_vfifo_r, 0x0);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_aud, 0x0);
	CTOP_CTRL_H13A0_Wr01(ctr00, reg_afifo_r, 0x0);
	CTOP_CTRL_H13A0_WrFL(ctr00);
...
static int _HDMI_H13_GetPortConnection(void)
{
	...
	LINK_H13_RdFL(system_control_00);
	LINK_H13_Rd01(system_control_00, reg_scdt, scdt);
	LINK_H13_Rd01(system_control_00, reg_hdmi_mode, mode);
	...
	if (scdt == HDMI_PORT_CONNECTED)
	{
		if (portCnt != 0)
		{
			if (CHK_UHD_BD())
			{
				//2013.06.27 sh.myoung - UD source sync error
				HDMI_H13_SetPowerControl(0);
				OS_MsecSleep(50);	// ms delay
				HDMI_H13_SetPowerControl(1);
				OS_MsecSleep(50);	// ms delay
			}
			HDMI_DEBUG("[%d] %s : Connection \n", __LINE__, __func__);
		}
		portCnt = 0;
	}
	else
	{
		...
		{
			spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
			_gH13Intr_vf_stable = HDMI_VIDEO_INIT_STATE;
			_gH13Intr_avi = HDMI_AVI_INIT_STATE;
			_gH13Intr_vsi = HDMI_VSI_INIT_STATE;
			_gH13Intr_src_mute = HDMI_SOURCE_MUTE_CLEAR_STATE;
			_gH13TimingReadCnt = 0;
			_gH13AVIReadState 	  = FALSE;
			_gH13VSIState = FALSE;
			_gH13AVIChangeState = FALSE;
			spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);

			_HDMI_H13_SetInternalMute(muteCtrl);	// Video All Clear

			///* Reset phy pll for pixel repetition  *///
			CTOP_CTRL_H13A0_RdFL(ctr26);
			CTOP_CTRL_H13A0_Rd01(ctr26, phy_ppll_sel, ctrl_repeat);
			...
			{
				LINK_H13_RdFL(interrupt_enable_00);
				LINK_H13_Rd01(interrupt_enable_00, intr_no_vsi_enable, noVsi_Intra);			///< 20 intr_no_vsi_enable

				if (noVsi_Intra != 0)
				{
					LINK_H13_Wr01(interrupt_enable_00, intr_no_vsi_enable, 0x0);			///< 20 intr_no_vsi_enable
					LINK_H13_WrFL(interrupt_enable_00);
	...
	spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
	_gH13HDMIConnectState = scdt;
	gH13BootData->mode = mode;
	spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
...
static int _HDMI_H13_PhyRunReset(void)
{
	if (lx_chip_rev() >= LX_CHIP_REV(H13, B0))	//for only H13 B0
	{
		PHY_REG_H13_RdFL(cr_vbgr_ctrl);
		PHY_REG_H13_Wr01(cr_vbgr_ctrl, cr_vbgr_ctrl, 0x5);		//2014-03-29 14UD change 3 -> 5
		PHY_REG_H13_WrFL(cr_vbgr_ctrl);

		PHY_REG_H13_RdFL(tmds_clk_inv);
		PHY_REG_H13_Wr01(tmds_clk_inv, tmds_clk_inv, 0x1);
		PHY_REG_H13_WrFL(tmds_clk_inv);

		// set eq at 2012-11-17
		//PHY_REG_H13_RdFL(dr_n1);
		//PHY_REG_H13_Wr01(dr_n1, dr_n1, 0x7);
		//PHY_REG_H13_WrFL(dr_n1);
	}

	// HDMI_ADAPTIVE_LOOP_Manual
	PHY_REG_H13_RdFL(eq_cs_rs_sel);
	PHY_REG_H13_Wr01(eq_cs_rs_sel, eq_rs_sel, 0x3);
	PHY_REG_H13_Wr01(eq_cs_rs_sel, eq_cs_sel, 0x3);
	PHY_REG_H13_WrFL(eq_cs_rs_sel);

	PHY_REG_H13_RdFL(eq_rs);
	PHY_REG_H13_Wr01(eq_rs, eq_rs, 0x1);		//2014-03-29 14UD change 3 -> 1
	PHY_REG_H13_WrFL(eq_rs);

	PHY_REG_H13_RdFL(cr_kvco_offset);
	PHY_REG_H13_Wr01(cr_kvco_offset, cr_kvco_offset, 0x1);		//2012-11-22 change 2 -> 1
	PHY_REG_H13_WrFL(cr_kvco_offset);

/*	// HDCP Problem to the Master for L9 -- need cheak  : default value 0x1
	PHY_REG_H13_RdFL(eq_af_en_avg_width);
	PHY_REG_H13_Wr01(eq_af_en_avg_width, eq_avg_width, 0x0);
	PHY_REG_H13_WrFL(eq_af_en_avg_width);
*/

	PHY_REG_H13_RdFL(idr_adj);
	PHY_REG_H13_Wr01(idr_adj, idr_adj, 0x3);
	PHY_REG_H13_WrFL(idr_adj);

	PHY_REG_H13_RdFL(eq_igmc_ictl);
	PHY_REG_H13_Wr01(eq_igmc_ictl, eq_igmc, 0x1);		//2014-03-29 14UD change 4 -> 1
	PHY_REG_H13_WrFL(eq_igmc_ictl);
...
static int _HDMI_H13_RunReset(void)
{
	...
	LINK_H13_RdFL(video_00);
	LINK_H13_Wr01(video_00, reg_pr_cmu_sync, 0x1);
	LINK_H13_WrFL(video_00);

	// Negative polarity for H13 UD and 3D_SS_Full format
	// Support positive and negative polarity for H13 DE HDMI data bridge
	LINK_H13_RdFL(video_03);
	LINK_H13_Wr01(video_03, reg_neg_pol_en, 0x1);
	LINK_H13_WrFL(video_03);

	// HPD PIN Control Enable
	LINK_H13_RdFL(phy_00);
	LINK_H13_Wr01(phy_00, hpd0_oen, 0x0);
	LINK_H13_WrFL(phy_00);

/*	// set internal test hdcp key
	LINK_H13_RdFL((UINT32*)&glink_818_bx);
	LINK_H13_Wr01.reg_dbg_hdcp_key_bak_en, 0x0)
	LINK_H13_WrFL((UINT32*)&glink_818_bx);
*/
	...
	//Enable or Disable ARC port
	LINK_H13_RdFL(phy_00);
	LINK_H13_Wr01(phy_00, phy_arc_pdb, (UINT32)_gH13AudioArcStatus);
	LINK_H13_WrFL(phy_00);

	HDMI_DEBUG("[%d] %s :  ARC = %s \n", __L__, __F__, _gH13AudioArcStatus? "On":"Off");
	return RET_OK;
}
...
static int _HDMI_H13_SetAudio(void)
{
	//ARC source
	LINK_H13_RdFL(heac_00);
	LINK_H13_Wr01(heac_00, reg_arc_src, 0x1);
	LINK_H13_WrFL(heac_00);

	//Channel 0
	LINK_H13_RdFL(system_control_00);
	LINK_H13_Wr01(system_control_00, reg_aac_en, 0x1);	//Auto Audio Path Configuration Enable(N, CTS value is auto configured.)
	LINK_H13_WrFL(system_control_00);

	LINK_H13_RdFL(audio_00);
	LINK_H13_Wr01(audio_00, reg_acr_en, 		 0x0);	//ACR Enable(Audio Clock Generation Function Activation)
	LINK_H13_Wr01(audio_00, reg_acr_n_fs, 		 0x1);	//0 : 128Fs, 1 : 256 Fs, 2 : 512Fs(default : 256Fs)
	LINK_H13_Wr01(audio_00, reg_acr_clk_aud_div, 0x1);	//0 : 128Fs, 1 : 256 Fs, 2 : 512Fs(default : 256Fs)
	LINK_H13_Wr01(audio_00, reg_acr_ncts_rx_en,  0x0);	//Use N, CTS value for audio clock generation
	LINK_H13_Wr01(audio_00, reg_acr_adj_thr, 	 0x3);	//Threshold Value for Clock Frequency Auto Adjustment for proper FIFO running, not required
	LINK_H13_Wr01(audio_00, reg_acr_adj_en, 	 0x1);	//Enable Clock Frequency Auto Adjustment for proper FIFO running
	LINK_H13_WrFL(audio_00);

	LINK_H13_RdFL(audio_05);
	LINK_H13_Wr01(audio_05, reg_i2s_sck_edge, 1);	//I2S Format for falling edge
	LINK_H13_Wr01(audio_05, reg_i2s_sd_en,  0xF);	//I2S SD Output Enable(4 Channel)
	LINK_H13_Wr01(audio_05, reg_i2s_out_en,   1);	//I2S Output Enable
	LINK_H13_Wr01(audio_05, reg_i2s_sd0_map,  0);	//I2S SD 0 Output Channel Mappings
	LINK_H13_Wr01(audio_05, reg_i2s_sd1_map,  1);	//I2S SD 1 Output Channel Mappings
	LINK_H13_Wr01(audio_05, reg_i2s_sd2_map,  2);	//I2S SD 2 Output Channel Mappings
	LINK_H13_Wr01(audio_05, reg_i2s_sd3_map,  3);	//I2S SD 3 Output Channel Mappings
	LINK_H13_WrFL(audio_05);

	//Set audio mute state
	_gH13AudioMuteState = FALSE;
...
*/static int _HDMI_H13_GetAudioInfo(void)
{
	...
	//Increase _gH13HdmiAudioThreadCount;
	_gH13HdmiAudioThreadCount++;

	//Check HDMI port connection
	LINK_H13_RdFL(system_control_00);
	LINK_H13_Rd01(system_control_00, reg_scdt, reg_scdt);

	if (reg_scdt == 0)	//Check HDMI, DVI Sync Detect
	{
		//spin lock for protection : lock
		spin_lock_irqsave(&_gIntrHdmiH13AudioLock, flags);

		//Clear _gH13IntrAudioState
		_gH13IntrAudioState = HDMI_AUDIO_INIT_STATE;

		//Clear _gH13HdmiFreqErrorCount
		_gH13HdmiFreqErrorCount = 0;

		//spin lock for protection : unlock
		spin_unlock_irqrestore(&_gIntrHdmiH13AudioLock, flags);

		if ((_gH13HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_10S) == 0)
			HDMI_AUDIO("[%d]HDMI_GetAudioInfo : HDMI / DVI Not Connected(state = %d)!!!\n", __L__, _gH13IntrAudioState);
	...
	intrVideoState = _gH13HDMIState;
	//intrVideoState = HDMI_STATE_READ;	//temp for test

	//copy audio interrupt state
	intrAudioState = _gH13IntrAudioState;

	//spin lock for protection : unlock
	spin_unlock_irqrestore(&_gIntrHdmiH13AudioLock, flags);

	//Check _gH13IntrAudioState is stably changed.
	if ( (intrAudioState == HDMI_AUDIO_STABLE_STATE && intrVideoState >= HDMI_STATE_READ)
	  ||(intrAudioState == HDMI_AUDIO_STABLE_STATE && _gH13HdmiPortStableCount > HDMI_AUDIO_PORT_STABLE_TIME_5S) )	//To protect a abnormal or transient video and audio info drop state.
	{
		if (_gH13HdmiAudioInfo.audioType == LX_HDMI_AUDIO_AC3_IEC60958)
		...
		if ((_gH13HdmiAudioThreadCount % audioRecheckTime) == 0)	// 0.5 or 1 seconds, Thread calls a every 20 ms.
		{
			//Check a current mute status for workaround
			if (_gH13AudioMuteState == TRUE)
			{
				//Unmute audio data for abnormal state
				muteCtrl.eMode		= LX_HDMI_AUDIO_MUTE;
				muteCtrl.bAudioMute = FALSE;
				_HDMI_H13_SetInternalMute(muteCtrl);
			...
			(void)_HDMI_H13_GetAudioTypeAndFreq(&audioType, &samplingFreq);

			//Get a sampling frequency from TMDS clock
			(void)_HDMI_H13_GetAudioFreqFromTmdsClock(&samplingFreqFromTmds);

			//H13 A0 IP Bug : non-PCM interrupt is not triggerred if non-PCM(AC-3) is changed to non-PCM(DTS).
			if ( (audioType			  != _gH13HdmiAudioInfo.audioType)
			  ||(samplingFreqFromTmds != _gH13HdmiAudioInfo.samplingFreq) )
			{
				HDMI_DEBUG("[%d]HDMI_GetAudioInfo : type = %d<-%d, tmds freq = %d<-%d, freq = %d, state = %d, mute = %d, PSC = %d(%d)\n", \
							__L__, audioType, _gH13HdmiAudioInfo.audioType, samplingFreqFromTmds, _gH13HdmiAudioInfo.samplingFreq,	\
							samplingFreq, _gH13IntrAudioState, _gH13AudioMuteState, _gH13HdmiPortStableCount, audioRecheckTime);

				//spin lock for protection for audio
				spin_lock_irqsave(&_gIntrHdmiH13AudioLock, flags);
				_gH13IntrAudioState = HDMI_AUDIO_INTERRUPT_STATE;
				_gH13HdmiFreqErrorCount = 0;
				spin_unlock_irqrestore(&_gIntrHdmiH13AudioLock, flags);
				...
				//Reset _gH13IntrBurstInfoCount
				_gH13IntrBurstInfoCount = 0;

				goto func_exit;
			}
		}

		//Increase _gH13HdmiPortStableCount(If HDMI port is changed, this count is cleared.)
		spin_lock_irqsave(&_gIntrHdmiH13AudioLock, flags);
		_gH13HdmiPortStableCount++;
		spin_unlock_irqrestore(&_gIntrHdmiH13AudioLock, flags);
		...
		if ((_gH13HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_10S) == 0)
		{
			HDMI_AUDIO("[%d]HDMI_GetAudioInfo : type = %d, freq = %d, state = %d, PSC = %d. mute = %d\n",	\
						__L__, _gH13HdmiAudioInfo.audioType, _gH13HdmiAudioInfo.samplingFreq,	\
						_gH13IntrAudioState, _gH13HdmiPortStableCount, _gH13AudioMuteState);
		}
	...
	LINK_H13_RdFL(video_05);
	LINK_H13_Rd01(video_05, reg_h_av, ui16HActive);
	LINK_H13_Rd01(video_05, reg_v_av, ui16VActive);
	...
	if (reg_hdmi_mode)
	{
		...
		if (audioType == LX_HDMI_AUDIO_PCM)
		{
			  ...
			  &&(samplingFreqFromTmds != LX_HDMI_SAMPLING_FREQ_44_1KHZ) )
			{
				...
				if (_gH13HdmiFreqErrorCount < HDMI_AUDIO_FREQ_ERROR_TIME_500MS)
				{
					//Increase _gH13HdmiFreqErrorCount.
					spin_lock_irqsave(&_gIntrHdmiH13AudioLock, flags);
					_gH13HdmiFreqErrorCount++;
					spin_unlock_irqrestore(&_gIntrHdmiH13AudioLock, flags);

					//Set a no audio mode
					audioType = LX_HDMI_AUDIO_NO_AUDIO;

					if ((_gH13HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0)
					{
		...
		if ((_gH13HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0)
		{
			HDMI_AUDIO("[%d]GetAudioInfo : type = %d, freq = %d, mute = %d, PSC = %d\n", \
						__L__, audioType, samplingFreq, _gH13AudioMuteState, _gH13HdmiPortStableCount);
		}
	}
	else
	{
	...
	if ( (audioType	  == _gH13HdmiAudioInfo.audioType)
	  &&(samplingFreq == _gH13HdmiAudioInfo.samplingFreq)
	  /* &&(_gH13HdmiAudioInfo.audioType >= LX_HDMI_AUDIO_PCM) */
	  &&(intrVideoState >= HDMI_STATE_READ) )
	{
		//spin lock for protection : lock
		spin_lock_irqsave(&_gIntrHdmiH13AudioLock, flags);

		//Set a _gH13IntrAudioState
		_gH13IntrAudioState = HDMI_AUDIO_STABLE_STATE;
	...
	//For debug print
	if ( (audioType != _gH13HdmiAudioInfo.audioType) || (samplingFreq != _gH13HdmiAudioInfo.samplingFreq) )
	{
		if ( ((audioType == LX_HDMI_AUDIO_DVI) || (audioType == LX_HDMI_AUDIO_NO_AUDIO))	\
		  &&((_gH13HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0) )
		{
			//Set a debug print status
			bDebugEnabled = TRUE;
		}
	...
	if (bDebugEnabled == TRUE)
	{
		...
		LINK_H13_RdFL(audio_09);
		LINK_H13_Rd01(audio_09, reg_burst_pc_0, reg_burst_pc_0);

		HDMI_AUDIO("[%d]GetAudioInfo : type = %d(%d), freq = %d, mute = %d, PSC = %d\n", \
					__L__, audioType, reg_burst_pc_0 & 0x1F, samplingFreq, _gH13AudioMuteState, _gH13HdmiPortStableCount);
	}
...
func_exit:
	if (audioType == LX_HDMI_AUDIO_NO_AUDIO)
	{
		...
		_gH13HdmiAudioInfo.audioType	= LX_HDMI_AUDIO_NO_AUDIO;
		_gH13HdmiAudioInfo.samplingFreq = LX_HDMI_SAMPLING_FREQ_NONE;

		//Clear _gH13HdmiPortStableCount
		_gH13HdmiPortStableCount = 0;
	...
	else
	{
		...
		_gH13HdmiAudioInfo.audioType	= audioType;
		_gH13HdmiAudioInfo.samplingFreq = samplingFreq;
	}

	HDMI_ATASK("_H13_GetAudioInfo : type = %d, freq = %d(A = %d, V = %d)\n", audioType, samplingFreq, intrAudioState, intrVideoState);
	return RET_OK;
}
...
static int _HDMI_H13_GetAudioTypeAndFreq(LX_HDMI_AUDIO_TYPE_T *audioType, LX_HDMI_SAMPLING_FREQ_T *samplingFreq)
{
	...
	if (reg_hdmi_mode)
	{
		...
		if (reg_achst_byte0 & HDMI_AUDIO_SAMPLE_NON_PCM_MASK)	//bit 1, 0 : PCM, 1 : non-PCM
		{
			...
			if ((reg_burst_pc_0 & BURST_INFO_PAYLOAD_ERROR_BIT_MASK) == 0) //bit 7, 0 : No Error, 1 : Error
			{
				...
				switch(reg_burst_pc_0 & BURST_INFO_AUDIO_TYPE_BIT_MASK)	//bit 4 ~ 0
				{
					...
					case BURST_INFO_AUDIO_TYPE_NULL:
						*audioType = _gH13HdmiAudioInfo.audioType;
						if ((_gH13HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0)
							HDMI_DEBUG("_HDMI_H13_GetAudioTypeAndFreq : BURST_INFO_AUDIO_TYPE_NULL(audioType= %d)\n", *audioType);
						break;
				...
				{
					if ((_gH13HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0)
						HDMI_DEBUG("_HDMI_H13_GetAudioTypeAndFreq : BURST_INFO_AUDIO_TYPE_PAUSE(Pc = %d)!!!\n", reg_burst_pc_0);
				}
			...
			else
			{
				*audioType = LX_HDMI_AUDIO_PCM;
				...
				HDMI_ERROR("_HDMI_H13_GetAudioTypeAndFreq : Burst Info Error = %d\n", (reg_burst_pc_0 & BURST_INFO_PAYLOAD_ERROR_BIT_MASK));
			}
		...
		else
		{
			*audioType = LX_HDMI_AUDIO_PCM;
			...
			if (_gH13IntrBurstInfoCount != _gH13TaskBurstInfoCount)
			{
				//Update _gH13TaskBurstInfoCount value.
				_gH13TaskBurstInfoCount = _gH13IntrBurstInfoCount;
				...
				if (*audioType == LX_HDMI_AUDIO_AC3_IEC60958)
				{
					...
					if (_gH13IntrAudioState == HDMI_AUDIO_GET_INFO_STATE)
					{
						HDMI_DEBUG("_HDMI_H13_GetAudioTypeAndFreq : Forced AC-3.(byte0 = 0x%X, Pc = 0x%X)\n", reg_achst_byte0, reg_burst_pc_0);
					}
				}
			}
			else
		...
		LINK_H13_RdFL(audio_08);
		LINK_H13_Rd01(audio_08, reg_achst_byte3, reg_achst_byte3);
		...
		switch(reg_achst_byte3 & HDMI_AUDIO_SAMPLE_BIT_MASK) 	//bit 0 ~ 3
		{
			...
			default:
				*samplingFreq = LX_HDMI_SAMPLING_FREQ_NONE;
				...
				HDMI_AUDIO("_HDMI_H13_GetAudioTypeAndFreq(Channel Status) : samplingFreq = %d\n", *samplingFreq);
				break;
		}
	}
	else
	...
	HDMI_ATASK("_H13_GetAudioTypeAndFreq : type = %d, freq = %d\n", *audioType, *samplingFreq);
	return ret;
}

/**
 *  _HDMI_H13_GetAudioFreqFromTmdsClock
 *
 *  @return int
*/
static int _HDMI_H13_GetAudioFreqFromTmdsClock(LX_HDMI_SAMPLING_FREQ_T *samplingFreqFromTmds)
{
	...
	PHY_REG_H13_Rd01(tmds_freq_1, tmds_freq, ui8TmdsClockHigh);
	PHY_REG_H13_Rd01(tmds_freq_2, tmds_freq, ui8TmdsClockLow);
	...
	//Get ACR N H/W value.
	LINK_H13_RdFL(audio_01);
	LINK_H13_Rd01(audio_01, reg_acr_n_tx, ui64AcrN);

	//Get ACR CTS H/W value.
	LINK_H13_RdFL(audio_03);
	LINK_H13_Rd01(audio_03, reg_acr_cts_tx, ui64AcrCts);
...
static int _HDMI_H13_DebugAudioInfo(LX_HDMI_DEBUG_AUDIO_INFO_T *pAudioDebugInfo)
{
	...
	HDMI_DEBUG("IEC60958 Channel Status Byte0 = 0x%X(0x%X)\n", reg_achst_byte0, LINK_H13_Rd(audio_07));

	//Read  reg_achst_byte1 ~ 4 reg.
	LINK_H13_RdFL(audio_08);
	LINK_H13_Rd01(audio_08, reg_achst_byte1, reg_achst_byte1);
	LINK_H13_Rd01(audio_08, reg_achst_byte2, reg_achst_byte2);
	LINK_H13_Rd01(audio_08, reg_achst_byte3, reg_achst_byte3);
	LINK_H13_Rd01(audio_08, reg_achst_byte4, reg_achst_byte4);

	//Check IEC60958 Channel Status Byte1 ~ 4
	HDMI_DEBUG("IEC60958 Channel Status Byte1 = 0x%X, Byte2 = 0x%X, Byte3 = 0x%X, Byte4 = 0x%X(0x%X)\n",	\
				reg_achst_byte1, reg_achst_byte2, reg_achst_byte3, reg_achst_byte4, LINK_H13_Rd(audio_08));
	...
	if (reg_achst_byte0 & HDMI_AUDIO_SAMPLE_NON_PCM_MASK)
	{
		...
		LINK_H13_RdFL(audio_09);
		LINK_H13_Rd01(audio_09, reg_burst_pc_0, reg_burst_pc_0);
		LINK_H13_Rd01(audio_09, reg_burst_pc_1, reg_burst_pc_1);
		LINK_H13_Rd01(audio_09, reg_burst_pd_0, reg_burst_pd_0);
		LINK_H13_Rd01(audio_09, reg_burst_pd_1, reg_burst_pd_1);
...
static int _HDMI_H13_Thread(void *data)
{
	while(1)
	{
		_HDMI_H13_GetPortConnection();	//check connection
		_HDMI_H13_Periodic_Task();	//Video Task
		_HDMI_H13_GetAudioInfo();	//Audio Task

		OS_MsecSleep(HDMI_THREAD_TIMEOUT);

		if (_gH13Force_thread_sleep > 1)
		{
			HDMI_PRINT("User force sleep HDMI Thread\n");
			//_gH13Force_thread_sleep  = 1;
			_gH13HDMI_thread_running = 0;
			//interruptible_sleep_on(&WaitQueue_HDMI_H13);
			wait_event_interruptible(WaitQueue_HDMI_H13 , _gH13Force_thread_sleep == 0);
		}
	}
	return RET_OK;
}
...
{
	LINK_H13_RdFL(interrupt_enable_00);
	LINK_H13_Wr01(interrupt_enable_00, intr_new_aud_enable, 0x0);		///< 0 intr_new_aud_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_new_spd_enable, 0x0);		///< 1 intr_new_spd_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_new_unr_enable, 0x0);		///< 2 intr_new_unr_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_cts_chg_enable, 0x0);		///< 3 intr_cts_chg_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_n_chg_enable, 0x0);			///< 4 intr_n_chg_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_fs_chg_enable, 0x1);		///< 5 intr_fs_chg_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_2pcm_chg_enable, 0x1);		///< 6 intr_2pcm_chg_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_2npcm_chg_enable, 0x1);		///< 7 intr_2npcm_chg_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_spdif_err_enable, 0x0);		///< 8 intr_spdif_err_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_afifo_undr_enable, 0x0);	///< 9 intr_afifo_undr_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_afifo_ovrr_enable, 0x0);	///< 10 intr_afifo_ovrr_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_burst_info_enable, 0x1);	///< 11 intr_burst_info_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_vf_stable_enable, 0x1);		///< 12 intr_vf_stable_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_vid_chg_enable, 0x0);		///< 13 intr_vid_chg_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_vr_chg_enable, 0x0);		///< 14 intr_vr_chg_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_hr_chg_enable, 0x0);		///< 15 intr_hr_chg_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_po_chg_enable, 0x0);		///< 16 intr_po_chg_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_il_chg_enable, 0x0);		///< 17 intr_il_chg_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_vfifo_undr_enable, 0x0);	///< 18 intr_vfifo_undr_enable
	LINK_H13_Wr01(interrupt_enable_00, intr_vfifo_ovrr_enable, 0x0);	///< 19 intr_vfifo_ovrr_enable
	if (lx_chip_rev() >= LX_CHIP_REV(H13, B0))
	{
		LINK_H13_Wr01(interrupt_enable_00, intr_no_vsi_enable, 0x0);			///< 20 intr_no_vsi_enable
		//intr_achst_5b_chg_int_enable is not working, so disabled.
		LINK_H13_Wr01(interrupt_enable_00, intr_achst_5b_chg_int_enable, 0x0);	///< 21 intr_achst_5b_chg_int_enable
	}
	LINK_H13_WrFL(interrupt_enable_00);

	LINK_H13_RdFL(interrupt_enable_01);
	LINK_H13_Wr01(interrupt_enable_01, intr_plug_in_enable, 0x0);		///< 0 intr_plug_in_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_plug_out_enable, 0x0);		///< 1 intr_plug_out_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_scdt_rise_enable, 0x0);		///< 2 intr_scdt_rise_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_scdt_fall_enable, 0x0);		///< 3 intr_scdt_fall_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_vsync_enable, 0x0);			///< 4 intr_vsync_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_hdmi_mode_enable, 0x0);		///< 5 intr_hdmi_mode_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_set_mute_enable, 0x1);		///< 6 intr_set_mute_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_clr_mute_enable, 0x0);		///< 7 intr_clr_mute_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_auth_init_enable, 0x0);		///< 8 intr_auth_init_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_auth_done_enable, 0x0);		///< 9 intr_auth_done_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_hdcp_err_enable, 0x0);		///< 10 intr_hdcp_err_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_ecc_err_enable, 0x0);		///< 11 intr_ecc_err_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_terc4_err_enable, 0x0);		///< 12 intr_terc4_err_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_acr_err_enable, 0x0);		///< 13 intr_acr_err_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_asp_err_enable, 0x0);		///< 14 intr_asp_err_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_no_avi_enable, 0x0);		///< 15 intr_no_avi_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_no_gcp_enable, 0x0);		///< 16 intr_no_gcp_enable

	LINK_H13_Wr01(interrupt_enable_01, intr_chg_avi_b12345_enable, 0x1);	///< 17 intr_chg_avi_b12345_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_chg_avi_to_rgb_enable, 0x1);	///< 18 intr_chg_avi_to_rgb_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_chg_avi_to_ycc444_enable, 0x1);	///< 19 intr_chg_avi_to_ycc444_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_chg_avi_to_ycc422_enable, 0x1);	///< 20 intr_chg_avi_to_ycc422_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_new_avi_enable, 0x0);			///< 21 intr_new_avi_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_new_vsi_enable, 0x0);			///< 22 intr_new_vsi_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_chg_vsi_vformat_enable, 0x1);	///< 23 intr_chg_vsi_vformat_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_new_gcp_enable, 0x0);			///< 24 intr_new_gcp_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_new_asp_enable, 0x0);			///< 25 intr_new_asp_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_new_acr_enable, 0x1);			///< 26 intr_new_acr_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_new_gbd_enable, 0x0);			///< 27 intr_new_gbd_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_new_acp_enable, 0x0);			///< 28 intr_new_acp_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_new_isrc1_enable, 0x0);			///< 29 intr_new_isrc1_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_new_isrc2_enable, 0x0);			///< 30 intr_new_isrc2_enable
	LINK_H13_Wr01(interrupt_enable_01, intr_new_mpg_enable, 0x0);			///< 31 intr_new_mpg_enable
	LINK_H13_WrFL(interrupt_enable_01);

	if (_gH13HDMI_interrupt_registered < 1)
	{
		if (request_irq(H13_IRQ_HDMI, _HDMI_H13_IRQHandler, 0, "hdmi", NULL))
		{
			HDMI_ERROR("request_irq is failed\n");
		}
		else
			_gH13HDMI_interrupt_registered = 1;
	}

	HDMI_DEBUG("HDMI_H13_EnableInterrupt\n");
	return RET_OK;
}
...
{
	LINK_H13_RdFL(interrupt_enable_00);
	LINK_H13_Wr(interrupt_enable_00, 0x0);
	LINK_H13_WrFL(interrupt_enable_00);

	LINK_H13_RdFL(interrupt_enable_01);
	LINK_H13_Wr(interrupt_enable_01, 0x0);
	LINK_H13_WrFL(interrupt_enable_01);

	free_irq(H13_IRQ_HDMI, NULL);
	HDMI_DEBUG("HDMI_H13_DisableInterrupt\n");
...
irqreturn_t _HDMI_H13_IRQHandler(int irq, void *dev)
{
...
#if 1
	LINK_H13_RdFL(interrupt_00);
	intra0 = LINK_H13_Rd(interrupt_00);

	LINK_H13_RdFL(interrupt_01);
	intra1 = LINK_H13_Rd(interrupt_01);
	...
	if ((intra0 &0x100000)>>20)
	{
		...
		spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
		_gH13Intr_vf_stable = HDMI_VIDEO_INTERRUPT_STATE;
		spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
		...
		if (_gH13HDMIState >= HDMI_STATE_READ)
		{
			HDMI_TASK("[%d] %s : -Changed -  _gH13Intr_vf_stable [STABLE_STATE] => [READ_STATE]     /   HDMI_STABLE[%d] / PSC[%d]\n", __L__, __F__, _gH13HDMIState, _gH13HdmiPortStableCount );
			spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
			_gH13Intr_vf_stable = HDMI_VIDEO_READ_STATE;
			//_gH13HDMIState = HDMI_STATE_READ;
			_gH13TimingReadCnt = 0;
			_gH13Intr_avi = HDMI_AVI_INTERRUPT_STATE;
			_gH13AVIReadState = FALSE;
			spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
			...
			//Check a HDMI Port Stable Count
			if ( (CHK_UHD_BD() && _gH13HdmiPortStableCount > HDMI_AUDIO_RECHECK_TIME_500MS)	\
			  ||(CHK_FHD_BD() && _gH13HdmiPortStableCount > HDMI_AUDIO_PORT_STABLE_TIME_5S) )
			{
				//spin lock for protection for audio
				spin_lock_irqsave(&_gIntrHdmiH13AudioLock, flags);
				_gH13IntrAudioState = HDMI_AUDIO_STABLE_STATE;
	...
	if ((intra1 &0x20000)>>17) 	//	if (pstLinkReg->interrupt_01.intr_chg_avi_b12345)
	{
		...
		HDMI_TASK("[%d][HDMI_AVI] state [%d]  preCSS[%d] CSC = %d \n", __L__, _gH13HDMIState, _gH13PrevPixelEncoding, csc);

		if (_gH13HDMIState >= HDMI_STATE_READ)//(_gH13Intr_vf_stable == HDMI_VIDEO_STABLE_STATE) 			// for PS3 CSC
		{
			if (_gH13PrevPixelEncoding != csc)
			{
				//spin lock for protection
				spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
				_gH13Intr_avi = HDMI_AVI_CHANGE_STATE;
				_gH13TimingReadCnt = 0;
				_gH13AVIReadState = FALSE;
				_gH13AVIChangeState = TRUE;
				spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);

				HDMI_TASK("[%d] %s : -Changed  - _gH13Intr_avi [STABLE_STATE] => [CHANGE_STATE]     /   HDMI_STABLE[%d] \n", __L__, __F__,_gH13HDMIState );
	...
	if ((intra1 &0x40)>>6) 	//	if (pstLinkReg->interrupt_01.intr_set_mute)
	{
		...
		//if (_gH13Intr_vf_stable > HDMI_VIDEO_INIT_STATE)
		{
			if (_gH13Intr_src_mute != HDMI_SOURCE_MUTE_STATE)
			{
				muteCtrl.eMode		= LX_HDMI_VIDEO_MUTE;
				muteCtrl.bVideoMute = TRUE;
				_HDMI_H13_SetInternalMute(muteCtrl);
				...
				spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
				_gH13Intr_src_mute = HDMI_SOURCE_MUTE_STATE;
				_gH13TimingReadCnt = 0;
				spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);

				HDMI_DEBUG("[%d] intr_set_mute : HDMI_SOURCE MUTE Enable[%d] \n", __L__, _gH13Intr_src_mute );
			}
		}
	}
	...
	if (lx_chip_rev() >= LX_CHIP_REV(H13, B0))
	{
		if ((intra1 &0x1000000)>>24) 	//	if (pstLinkReg->interrupt_01.intr_no_vsi)
		{
			...
			HDMI_TASK("[%d] %s : -Changed  - _gH13Intr_vsi [STABLE_STATE] => [NO_DATA_STAT]     /   HDMI_STABLE[%d] \n", __L__, __F__,_gH13HDMIState );
			HDMI_TASK("[%d] %s : -Changed  - _gH13Intr_vf_stable [STABLE_STATE] => [READ_STATE]     /   HDMI_STABLE[%d] \n", __L__, __F__,_gH13HDMIState );
			spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
			_gH13Intr_vsi = HDMI_VSI_NO_DATA_STATE;
			_gH13Intr_vf_stable = HDMI_VIDEO_READ_STATE;
			_gH13TimingReadCnt = 0;
			_gH13Intr_avi = HDMI_AVI_INTERRUPT_STATE;
			_gH13AVIReadState = FALSE;
			...
			LINK_H13_RdFL(interrupt_enable_00);	// fix bug
			LINK_H13_Wr01(interrupt_enable_00, intr_no_vsi_enable, 0x0);			///< 20 intr_no_vsi_enable
			LINK_H13_WrFL(interrupt_enable_00);

			// Enable new_vsi interrupt
			// When resolution change LG demo divx player, sometimes no vsi interrupt occur resulting in no 3D operation.
			LINK_H13_RdFL(interrupt_enable_01);
			LINK_H13_Wr01(interrupt_enable_01, intr_new_vsi_enable, 0x1);			///< 22 intr_no_vsi_enable
			LINK_H13_WrFL(interrupt_enable_01);

			HDMI_DEBUG("[%d] %s : No VSI intra disable & New VSI Enabled \n", __LINE__, __func__);
		}
	...
	if ((intra1 &0x800000)>>23) 	//	if (pstLinkReg->interrupt_01.intr_chg_vsi_vformat)
	{
		...
		spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
		_gH13Intr_vsi = HDMI_VSI_INTERRUPT_STATE;
		_gH13VSIState = TRUE;
		spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
	...
	else if ((intra1 &0x400000)>>22) 	// new vsi interrupt
	{
		HDMI_DEBUG("intr_new_vsi\n");

		if( _gH13Intr_vsi == HDMI_VSI_NO_DATA_STATE)
		{
			spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
			_gH13Intr_vsi = HDMI_VSI_INTERRUPT_STATE;
			_gH13VSIState = TRUE;
			...
			HDMI_TASK("[%d] %s : -Changed  - _gH13Intr_vsi [HDMI_VSI_NO_DATA_STATE] => [HDMI_VSI_INTERRUPT_STATE]     /   HDMI_STABLE[%d] \n", __L__, __F__,_gH13HDMIState );
		...
		LINK_H13_RdFL(interrupt_enable_01);
		LINK_H13_Wr01(interrupt_enable_01, intr_new_vsi_enable, 0x0);			///< 22 intr_no_vsi_enable
		LINK_H13_WrFL(interrupt_enable_01);
	}
...
//audio related. => start
	if ((intra0 &0x4)>>2) 	//if (pstLinkReg->interrupt_00.intr_new_acr)
	{
		...
		/* interrupt_enable_01 reg. is not read because of H13 A0 chip bug, so use global variable. */
		if (lx_chip_rev() >= LX_CHIP_REV(H13, B0))
		{
			LINK_H13_RdFL(interrupt_enable_01);
		}

		LINK_H13_Wr01(interrupt_enable_01, intr_scdt_fall_enable, 0x1);		///< 3 intr_scdt_fall_enable
		LINK_H13_Wr01(interrupt_enable_01, intr_new_acr_enable,   0x0);		///< 26 intr_new_acr_enable
		LINK_H13_WrFL(interrupt_enable_01);

		//To protect EMI issue, ACR is disabled in DVI mode because of avoiding abnormal clock generation.
		LINK_H13_RdFL(audio_00);
		LINK_H13_Wr01(audio_00, reg_acr_en, 0x1);	//ACR Enable(Audio Clock Generation Function Activation)
		LINK_H13_WrFL(audio_00);
	...
	if ((intra1 &0x8)>>3) 	//if (pstLinkReg->interrupt_01.intr_scdt_fall)
	{
		...
		//To protect EMI issue, ACR is disabled in DVI mode because of avoiding abnormal clock generation.
		LINK_H13_RdFL(audio_00);
		LINK_H13_Wr01(audio_00, reg_acr_en, 0x0);	//ACR Disable(Audio Clock Generation Function Activation)
		LINK_H13_WrFL(audio_00);
	...
	if ((intra0 &0x80000)>>19)	//	if (pstLinkReg->interrupt_00.intr_burst_info)
	{
		...
		elapsedTime = jiffies_to_msecs(jiffies - _gH13BurstInfoPrevJiffies);

		//Save _gH13BurstInfoPrevJiffies
		_gH13BurstInfoPrevJiffies = jiffies;

		if (_gH13IntrBurstInfoCount == 0 && elapsedTime > HDMI_AUDIO_BURST_INFO_RECHECK_80MS)
		{
			//spin lock for protection for audio
			spin_lock_irqsave(&_gIntrHdmiH13AudioLock, flags);
			_gH13IntrAudioState = HDMI_AUDIO_INTERRUPT_STATE;
			...
			if (_gH13HdmiPortStableCount > HDMI_AUDIO_PORT_STABLE_TIME_5S)
			{
				//Check a vaild Pc data for payload
				LINK_H13_RdFL(audio_09);
				HDMI_DEBUG("intr_burst_info(%llu), Pd_Pc = 0x%X, elapsedTime = %llu\n",	\
							_gH13IntrBurstInfoCount, LINK_H13_Rd(audio_09), elapsedTime);
			}
		}

		//Increase _gH13IntrBurstInfoCount
		_gH13IntrBurstInfoCount++;

		HDMI_INTR("intr_burst_info(%llu)\n", _gH13IntrBurstInfoCount);

		//if ((_gH13HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0)
			//HDMI_DEBUG("intr_burst_info(%llu), elapsedTime(%llu)\n", _gH13IntrBurstInfoCount, elapsedTime);
	}
	else
	{
		//_gH13IntrBurstInfoCount is normal toggled in FHD Model.
		if (CHK_FHD_BD())
		{
			//Reset _gH13IntrBurstInfoCount
			_gH13IntrBurstInfoCount = 0;
		...
		else
		{
			//Check a vaild Pc data for payload
			LINK_H13_RdFL(audio_09);
			HDMI_INTR("_gH13IntrBurstInfoCount(%llu), Pd_Pc = 0x%X\n", _gH13IntrBurstInfoCount, LINK_H13_Rd(audio_09));
		}
	}
//audio related. => end.
	...
	if ((intra0 &0x1000000)>>24)	//	if (pstLinkReg->interrupt_00.intr_po_chg)
	{
		HDMI_INTR("intr_po_chg\n");
	}

	if ((intra0 &0x2000000)>>25)	//	if (pstLinkReg->interrupt_00.intr_il_chg)
	{
		HDMI_INTR("intr_il_chg\n");
	}

	if ((intra0 &0x4000000)>>26)	//	if (pstLinkReg->interrupt_00.intr_vfifo_undr)
	{
		HDMI_INTR("intr_vfifo_undr\n");
	}

	if ((intra0 &0x8000000)>>27)	//	if (pstLinkReg->interrupt_00.intr_vfifo_ovrr)
	{
	...
	if (lx_chip_rev() >= LX_CHIP_REV(H13, B0))
	{
		if ((intra0 &0x10000000)>>28)	//	if (pstLinkReg->interrupt_00.intr_achst_5b_chg_int)
		{
...
///////////////////////////////////////////////////////////////////////////////
	...
	if (pstLinkReg->interrupt_01.intr_plug_out)
	{
...
/////////////////AVI
	...
	if (intra0)
	{
		LINK_H13_Wr(interrupt_00, intra0);
		LINK_H13_WrFL(interrupt_00);
	}

	if (intra1)
	{
		LINK_H13_Wr(interrupt_01, intra1);
		LINK_H13_WrFL(interrupt_01);
	}
...
static void _HDMI_H13_Periodic_Task(void)
{
	...
	do {
		if (_gH13HDMIState == HDMI_STATE_IDLE && _gH13HDMIState == HDMI_STATE_DISABLE)
			gPeriodicState = _gH13HDMIState;

		if (_gH13HDMIConnectState == HDMI_PORT_NOT_CONNECTED)			// change check connection in the thread function by 20121206
		{
		...
		{
			if (_gH13HDMIState <= HDMI_STATE_NO_SCDT)
			{
			...
			if (_gH13Intr_src_mute == HDMI_SOURCE_MUTE_STATE)
			{
				LINK_H13_RdFL(packet_34);
				LINK_H13_Rd01(packet_34, reg_pkt_gcp_cmute, muteClear);
				LINK_H13_Rd01(packet_34, reg_pkt_gcp_smute, temp);
			...
			if (_gH13Intr_vf_stable == HDMI_VIDEO_READ_STATE)
			{
				HDMI_TASK("[%d] %s : -Changed- HDMI_STATE_[%d]  => HDMI_STATE_READ && _gH13Intr_vf_stable[READ_STATE => INTERRUPT_STATE] \n", __L__, __F__, gPeriodicState);
		...
		switch(gPeriodicState)
		{
			...
			case HDMI_STATE_SCDT:		// if (HDMI_SCDT_COUNT == 4)  5 call
				{
					...
					{
						gScdtCnt++;
						if (_gH13Intr_vf_stable == HDMI_VIDEO_INTERRUPT_STATE)
						{
							gPeriodicState = HDMI_STATE_CHECK_MODE;
						...
						if (gScdtCnt > 50)			// when no vf_stable Interrupt
						{
							...
							spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
							_gH13Intr_vf_stable = HDMI_VIDEO_INTERRUPT_STATE;
							if (_gH13Intr_avi == HDMI_AVI_INIT_STATE)		_gH13Intr_avi = HDMI_AVI_INTERRUPT_STATE;
							if (_gH13Intr_vsi == HDMI_VSI_INIT_STATE)		_gH13Intr_vsi = HDMI_VSI_INTERRUPT_STATE;
							spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
			...
			case HDMI_STATE_CHECK_AVI_NO:
				{
					...
					if (gH13BootData->mode == 1)
					{
						gAviCnt ++;
						if (_gH13Intr_avi <= HDMI_AVI_INTERRUPT_STATE &&  gAviCnt < 50)		// No read AVI
						{
							LINK_H13_RdFL(packet_18);
							aviHeader = LINK_H13_Rd(packet_18);		// reg_pkt_avi_hdr_0 (AVI Packet Version), reg_pkt_avi_hdr_1 (AVI Packet Length)
			...
			case HDMI_STATE_READ:
				{
					...
					else if (_gH13TimingReadCnt > HDMI_READ_COUNT)
						gPeriodicState = HDMI_STATE_STABLE;

					HDMI_TASK("[%d] %s : HDMI State = [%d]  _gH13TimingReadCnt = %d \n", __L__, __F__, gPeriodicState, _gH13TimingReadCnt);
				}
				break;
			...
			case HDMI_STATE_STABLE:
				{
					spin_lock_irqsave(&_gIntrHdmiH13VideoLock, flags);
					_gH13Intr_vf_stable = HDMI_VIDEO_STABLE_STATE;
					spin_unlock_irqrestore(&_gIntrHdmiH13VideoLock, flags);
				}
				break;
	...
	_gH13HDMIState = gPeriodicState;

	if (_gH13HDMIState < HDMI_STATE_STABLE)
...
int HDMI_H13_GetRegister(UINT32 addr , UINT32 *value)
{
	...
	else if (addr >= 0x3800 && addr <=0x38AC)
	{
		slave = ((addr &0xff00)>>8)<<1;
		reg = (addr &0xff);

		REG_ReadI2C(PHY_REG_H13_I2C_IDX, slave, reg, &data);
		*value = (UINT32)data;
...
int HDMI_H13_SetRegister(UINT32 addr , UINT32 value)
{
	...
	else if (addr >= 0x3800 && addr <=0x38AC)
	{
		...
		REG_WriteI2C(PHY_REG_H13_I2C_IDX, slave, reg, data);
...
/**
 * @brief Get size data in AVI packet
 *
 * @param width
 * @param height
 *
...
int HDMI_H13_Get_AVISizeData(int *width, int *height)
{
	...
	LINK_H13_RdFL(packet_20);		//AVI[7:4]
	*width = ( LINK_H13_Rd(packet_20) >> 16 ) & 0xFF; // AVI[7:6] contains width upper/lower

	LINK_H13_RdFL(packet_21);		//AVI[11:8]
	*height = LINK_H13_Rd(packet_21) & 0xFF;	// AVI[9:8] contains height upper/lower
