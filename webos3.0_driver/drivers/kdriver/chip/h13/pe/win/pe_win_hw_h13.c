...
----------------------------------------------------------------------------------------*/
#define PE_WIN_HW_H13_FHD_H_MAX		1920
#define PE_WIN_HW_H13_FHD_V_MAX		1080
#define PE_WIN_HW_H13_SHP_WIN		0
#define PE_WIN_HW_H13_CEN_WIN		1
#define PE_WIN_HW_H13_WIN_EN_OFF	0
#define PE_WIN_HW_H13_WIN_EN__ON	1

/** 20130516, sj.youm, 
* fix 3d(tb,ss) apl window setting with fhd input for ext frc,3dfmt case
*/
#define PE_WIN_HW_H13_FIX_FHD3D_APL_FOR_EXT_FRC3DFMT_CASE

/*----------------------------------------------------------------------------------------
	Macro Definitions
----------------------------------------------------------------------------------------*/
#define PE_WIN_HW_H13_ERROR	printk

#define PE_WIN_HW_H13_DBG_PRINT(fmt,args...)	\
	if(_g_win_hw_h13_trace) printk("[%x,%x][%s,%d] "fmt,PE_CHIP_VER,g_pe_kdrv_ver_mask,__F__,__L__,##args)
#define PE_WIN_HW_H13_CHECK_CODE(_checker,_action,fmt,args...)	\
	{if(_checker){PE_WIN_HW_H13_ERROR(fmt,##args);_action;}}

#define PE_WIN_HW_H13_DIVIDE(x,y)	(((((x)<<10)/y)+512)>>10)
...
----------------------------------------------------------------------------------------*/
static void PE_WIN_HW_H13_SetVerWinReverse(UINT32 *y0,UINT32 *y1,UINT32 v_max);

/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
static UINT32 _g_win_hw_h13_trace=0x0;	//default should be off.
static PE_WIN_HW_H13_SETTINGS_T _g_pe_win_hw_h13_info;
...
int PE_WIN_HW_H13_Init(PE_CFG_CTRL_T *pstParams)
{
	...
	do{
		...
		{
			memset(&_g_pe_win_hw_h13_info,0xffff,sizeof(PE_WIN_HW_H13_SETTINGS_T));
			PE_WIN_HW_H13_DBG_PRINT("pe1 win ctrl init, inside.\n");
	        PE_P1L_H13B0_QWr05(pe1_win1_ctrl_00,	win0_en,		0x1,\
													win1_en,		0x1,\
													win01_en,		0x1,\
													win_en,			0x1,\
            										win_outside,	0x0);
	        PE_P1L_H13B0_QWr05(pe1_win2_ctrl_00,	win0_en,		0x1,\
													win1_en,		0x1,\
													win01_en,		0x1,\
													win_en,			0x1,\
            										win_outside,	0x0);
	        PE_P1L_H13B0_QWr05(pe1_win3_ctrl_00,	win0_en,		0x1,\
													win1_en,		0x1,\
													win01_en,		0x1,\
													win_en,			0x1,\
            										win_outside,	0x0);
		...
		else if(PE_KDRV_VER_H13AX)
		{
			memset(&_g_pe_win_hw_h13_info,0xffff,sizeof(PE_WIN_HW_H13_SETTINGS_T));
			PE_WIN_HW_H13_DBG_PRINT("pe1 win ctrl init, inside.\n");
	        PE_P1L_H13_QWr02(pe1_win1_ctrl_00,	win_en,			0x1,\
            									win_outside,	0x0);
	        PE_P1L_H13_QWr02(pe1_win2_ctrl_00,	win_en,			0x1,\
            									win_outside,	0x0);
	        PE_P1L_H13_QWr02(pe1_win3_ctrl_00,	win_en,			0x1,\
            									win_outside,	0x0);
		}
		else
		{
			PE_WIN_HW_H13_DBG_PRINT("nothing to do\n");	ret = RET_OK;
		}
	}while(0);
	return ret;
}
...
int PE_WIN_HW_H13_SetDebugSettings(LX_PE_DBG_SETTINGS_T *pstParams)
{
	...
	do{
		...
		PE_WIN_HW_H13_DBG_PRINT("wid:%d,type:0x%x,[0x%x]print:0x%x,bypass:0x%x\n",\
			pstParams->win_id,pstParams->type,(0x1<<PE_ITEM_PKTMODL(WIN)),\
			pstParams->print_lvl,pstParams->bypass);
		/* set print level */
		if(pstParams->type&LX_PE_DBG_LV)
		{
			_g_win_hw_h13_trace = (pstParams->print_lvl & (0x1<<PE_ITEM_PKTMODL(WIN)))? 0x1:0x0;
		}
	}while(0);
	return ret;
}
...
int PE_WIN_HW_H13_SetEnhancingWindow(PE_TSK_H13_CFG_T *pCfg)
{
	...
	UINT32 is_seamless_internal;	//amazon
	...
	PE_WIN_HW_H13_CTRL_T *win_info=&_g_pe_win_hw_h13_info.win_inf;
	do {
		CHECK_KNULL(pCfg);
		if(PE_KDRV_VER_H13BX)
		{
			is_seamless_internal = \
				((ctrl0_info->m_win_v_size==(PE_WIN_HW_H13_FHD_V_MAX<<1)) && \
				(disp0_info->fmt_type==LX_PE_FMT_UHD) && \
				(disp0_info->out_type==LX_PE_OUT_2D))? 1:0;
			if(disp0_info->out_type==LX_PE_OUT_UD)
			{
				h_max=PE_WIN_HW_H13_FHD_H_MAX<<1;
				v_max=PE_WIN_HW_H13_FHD_V_MAX<<1;
			...
			else
			{
				if (is_seamless_internal)
				{
					h_max=PE_WIN_HW_H13_FHD_H_MAX<<1;
					v_max=PE_WIN_HW_H13_FHD_V_MAX<<1;
					x_ofst[0] = ((ctrl0_info->m_win_h_size+ctrl1_info->m_win_h_size)<(PE_WIN_HW_H13_FHD_H_MAX<<1))? 3:0;
					y_ofst[0] = (ctrl0_info->m_win_v_size<(PE_WIN_HW_H13_FHD_V_MAX<<1))? 3:0;
					x_ofst[1] = y_ofst[1] = 0;
				}
				else
				{
					h_max=PE_WIN_HW_H13_FHD_H_MAX;
					v_max=PE_WIN_HW_H13_FHD_V_MAX;
					x_ofst[0] = (ctrl0_info->m_win_h_size<PE_WIN_HW_H13_FHD_H_MAX)? 3:0;
					x_ofst[1] = (ctrl1_info->m_win_h_size<PE_WIN_HW_H13_FHD_H_MAX)? 3:0;
					y_ofst[0] = (ctrl0_info->m_win_v_size<PE_WIN_HW_H13_FHD_V_MAX)? 3:0;
					y_ofst[1] = (ctrl1_info->m_win_v_size<PE_WIN_HW_H13_FHD_V_MAX)? 3:0;
				}
			}
			...
			else
			{
				if (is_seamless_internal)
				{
					...
					x1[LX_PE_WIN_0] = x0[LX_PE_WIN_0]+(ctrl0_info->m_win_h_size+ctrl1_info->m_win_h_size)-1;
			...
			{
				PE_WIN_HW_H13_SetVerWinReverse(&act0_y0,&act0_y1,v_max);
				PE_WIN_HW_H13_SetVerWinReverse(&act1_y0,&act1_y1,v_max);
				PE_WIN_HW_H13_SetVerWinReverse(&win0_y0,&win0_y1,v_max);
				PE_WIN_HW_H13_SetVerWinReverse(&win1_y0,&win1_y1,v_max);
			}
			...
			if(disp0_info->mode.is_ext_frc)	/* external frc */
			{
				switch(disp0_info->out_type)
				{
					...
					case LX_PE_OUT_SS:
					case LX_PE_OUT_UD:
						win0_en[PE_WIN_HW_H13_SHP_WIN]=(is_resize_case)? \
							PE_WIN_HW_H13_WIN_EN_OFF:PE_WIN_HW_H13_WIN_EN__ON;
						win1_en[PE_WIN_HW_H13_SHP_WIN]=(is_resize_case)? \
							PE_WIN_HW_H13_WIN_EN_OFF:PE_WIN_HW_H13_WIN_EN__ON;
						win01_en[PE_WIN_HW_H13_SHP_WIN]=(is_resize_case)? \
							PE_WIN_HW_H13_WIN_EN_OFF:PE_WIN_HW_H13_WIN_EN__ON;
						win0_en[PE_WIN_HW_H13_CEN_WIN]=PE_WIN_HW_H13_WIN_EN__ON;
						win1_en[PE_WIN_HW_H13_CEN_WIN]=PE_WIN_HW_H13_WIN_EN__ON;
						win01_en[PE_WIN_HW_H13_CEN_WIN]=PE_WIN_HW_H13_WIN_EN__ON;
						break;
					case LX_PE_OUT_PIP:
						if(ctrl0_info->hv_same)	//split window case
						{
							win0_en[PE_WIN_HW_H13_SHP_WIN]=PE_WIN_HW_H13_WIN_EN__ON;
							win1_en[PE_WIN_HW_H13_SHP_WIN]=(ctrl0_info->mute_on)? \
								PE_WIN_HW_H13_WIN_EN_OFF:PE_WIN_HW_H13_WIN_EN__ON;
							win01_en[PE_WIN_HW_H13_SHP_WIN]=PE_WIN_HW_H13_WIN_EN_OFF;
							win0_en[PE_WIN_HW_H13_CEN_WIN]=PE_WIN_HW_H13_WIN_EN__ON;
							win1_en[PE_WIN_HW_H13_CEN_WIN]=PE_WIN_HW_H13_WIN_EN__ON;
							win01_en[PE_WIN_HW_H13_CEN_WIN]=PE_WIN_HW_H13_WIN_EN_OFF;
						}
						else
						{
							win0_en[PE_WIN_HW_H13_SHP_WIN]=PE_WIN_HW_H13_WIN_EN__ON;
							win1_en[PE_WIN_HW_H13_SHP_WIN]=PE_WIN_HW_H13_WIN_EN_OFF;
							win01_en[PE_WIN_HW_H13_SHP_WIN]=PE_WIN_HW_H13_WIN_EN_OFF;
							win0_en[PE_WIN_HW_H13_CEN_WIN]=PE_WIN_HW_H13_WIN_EN__ON;
							win1_en[PE_WIN_HW_H13_CEN_WIN]=PE_WIN_HW_H13_WIN_EN_OFF;
							win01_en[PE_WIN_HW_H13_CEN_WIN]=PE_WIN_HW_H13_WIN_EN_OFF;
						}
						break;
					default:
			...
			{
				PE_WIN_HW_H13_DBG_PRINT("[inf]ex_frc:%d,rev:%d,out:%d,ofst[0]:%d,%d,ofst[1]:%d,%d\n",\
					disp0_info->mode.is_ext_frc,disp0_info->mode.is_reverse,disp0_info->out_type,\
					x_ofst[0],y_ofst[0],x_ofst[1],y_ofst[1]);
				PE_WIN_HW_H13_DBG_PRINT("[inf]o_win0:%d,%d,%d,%d,o_win1:%d,%d,%d,%d\n",\
				...
				PE_WIN_HW_H13_DBG_PRINT("[set]act0 : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
					win_info->act0_x0,win_info->act0_y0,win_info->act0_x1,\
					win_info->act0_y1,act0_x0,act0_y0,act0_x1,act0_y1);
				PE_WIN_HW_H13_DBG_PRINT("[set]act1 : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
					win_info->act1_x0,win_info->act1_y0,win_info->act1_x1,\
					win_info->act1_y1,act1_x0,act1_y0,act1_x1,act1_y1);
	            PE_P1L_H13B0_QWr02(pe1_win2_ctrl_01,	win_w0_x0,	act0_x0,\
	            										win_w0_y0,	act0_y0);
	            PE_P1L_H13B0_QWr02(pe1_win2_ctrl_02,	win_w0_x1,	act0_x1,\
	            										win_w0_y1,	act0_y1);
	            PE_P1L_H13B0_QWr02(pe1_win2_ctrl_03,	win_w1_x0,	act1_x0,\
	            										win_w1_y0,	act1_y0);
	            PE_P1L_H13B0_QWr02(pe1_win2_ctrl_04,	win_w1_x1,	act1_x1,\
	            										win_w1_y1,	act1_y1);
				#ifdef PE_WIN_HW_H13_FIX_FHD3D_APL_FOR_EXT_FRC3DFMT_CASE
				if(disp0_info->mode.is_ext_frc && disp0_info->out_type==LX_PE_OUT_TB)
				{
		            PE_P1L_H13B0_QWr02(pe1_apl_ctrl_00,	apl_win_ctrl_x0, act0_x0,\
		            									apl_win_ctrl_y0, (act0_y0<<1));
		            PE_P1L_H13B0_QWr02(pe1_apl_ctrl_01,	apl_win_ctrl_x1, act0_x1,\
		            									apl_win_ctrl_y1, ((act0_y1<<1)+1));
				}
				else if(disp0_info->mode.is_ext_frc && disp0_info->out_type==LX_PE_OUT_SS)
				{
		            PE_P1L_H13B0_QWr02(pe1_apl_ctrl_00,	apl_win_ctrl_x0, (act0_x0<<1),\
		            									apl_win_ctrl_y0, act0_y0);
		            PE_P1L_H13B0_QWr02(pe1_apl_ctrl_01,	apl_win_ctrl_x1, ((act0_x1<<1)+1),\
				...
				(win_info->win1_x1 != win1_x1)|| (win_info->win1_y1 != win1_y1))
			{
				...
				PE_WIN_HW_H13_DBG_PRINT("[inf]o_win0:%d,%d,%d,%d,o_win1:%d,%d,%d,%d\n",\
					...
					ctrl0_info->m_win_v_size,ctrl1_info->m_win_h_ofst,ctrl1_info->m_win_v_ofst,\
					ctrl1_info->m_win_h_size,ctrl1_info->m_win_v_size);
				PE_WIN_HW_H13_DBG_PRINT("[set]win0 : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
					win_info->win0_x0,win_info->win0_y0,win_info->win0_x1,\
					win_info->win0_y1,win0_x0,win0_y0,win0_x1,win0_y1);
				PE_WIN_HW_H13_DBG_PRINT("[set]win1 : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
					win_info->win1_x0,win_info->win1_y0,win_info->win1_x1,\
					win_info->win1_y1,win1_x0,win1_y0,win1_x1,win1_y1);
	            PE_P1L_H13B0_QWr02(pe1_win1_ctrl_01,	win_w0_x0,	win0_x0,\
	            										win_w0_y0,	win0_y0);
	            PE_P1L_H13B0_QWr02(pe1_win1_ctrl_02,	win_w0_x1,	win0_x1,\
	            										win_w0_y1,	win0_y1);
	            PE_P1L_H13B0_QWr02(pe1_win1_ctrl_03,	win_w1_x0,	win1_x0,\
	            										win_w1_y0,	win1_y0);
	            PE_P1L_H13B0_QWr02(pe1_win1_ctrl_04,	win_w1_x1,	win1_x1,\
	            										win_w1_y1,	win1_y1);
	            PE_P1L_H13B0_QWr02(pe1_win3_ctrl_01,	win_w0_x0,	win0_x0,\
	            										win_w0_y0,	win0_y0);
	            PE_P1L_H13B0_QWr02(pe1_win3_ctrl_02,	win_w0_x1,	win0_x1,\
	            										win_w0_y1,	win0_y1);
	            PE_P1L_H13B0_QWr02(pe1_win3_ctrl_03,	win_w1_x0,	win1_x0,\
	            										win_w1_y0,	win1_y0);
	            PE_P1L_H13B0_QWr02(pe1_win3_ctrl_04,	win_w1_x1,	win1_x1,\
	            										win_w1_y1,	win1_y1);
			...
			/* shp win en */
			if((win_info->win0_en[PE_WIN_HW_H13_SHP_WIN] != win0_en[PE_WIN_HW_H13_SHP_WIN]) || \
				(win_info->win1_en[PE_WIN_HW_H13_SHP_WIN] != win1_en[PE_WIN_HW_H13_SHP_WIN]) || \
				(win_info->win01_en[PE_WIN_HW_H13_SHP_WIN] != win01_en[PE_WIN_HW_H13_SHP_WIN]))
			{
				PE_WIN_HW_H13_DBG_PRINT("[inf]ex_frc:%d,rev:%d,out:%d,ofst[0]:%d,%d,ofst[1]:%d,%d\n",\
					disp0_info->mode.is_ext_frc,disp0_info->mode.is_reverse,disp0_info->out_type,\
					x_ofst[0],y_ofst[0],x_ofst[1],y_ofst[1]);
				PE_WIN_HW_H13_DBG_PRINT("[inf]o_win0:%d,%d,%d,%d,o_win1:%d,%d,%d,%d\n",\
					...
					ctrl0_info->m_win_v_size,ctrl1_info->m_win_h_ofst,ctrl1_info->m_win_v_ofst,\
					ctrl1_info->m_win_h_size,ctrl1_info->m_win_v_size);
				PE_WIN_HW_H13_DBG_PRINT("[set]shp_win_en(0,1,01):%d,%d,%d \n",\
					win_info->win0_en[PE_WIN_HW_H13_SHP_WIN], win_info->win1_en[PE_WIN_HW_H13_SHP_WIN],\
					win_info->win01_en[PE_WIN_HW_H13_SHP_WIN]);
				PE_P1L_H13B0_QWr03(pe1_win1_ctrl_00,	win0_en,	win0_en[PE_WIN_HW_H13_SHP_WIN],\
														win1_en,	win1_en[PE_WIN_HW_H13_SHP_WIN],\
														win01_en,	win01_en[PE_WIN_HW_H13_SHP_WIN]);
				PE_P1L_H13B0_QWr03(pe1_win3_ctrl_00,	win0_en,	win0_en[PE_WIN_HW_H13_SHP_WIN],\
														win1_en,	win1_en[PE_WIN_HW_H13_SHP_WIN],\
														win01_en,	win01_en[PE_WIN_HW_H13_SHP_WIN]);
				win_info->win0_en[PE_WIN_HW_H13_SHP_WIN] = win0_en[PE_WIN_HW_H13_SHP_WIN];
				win_info->win1_en[PE_WIN_HW_H13_SHP_WIN] = win1_en[PE_WIN_HW_H13_SHP_WIN];
				win_info->win01_en[PE_WIN_HW_H13_SHP_WIN] = win01_en[PE_WIN_HW_H13_SHP_WIN];
			}
			/* cen win en */
			if((win_info->win0_en[PE_WIN_HW_H13_CEN_WIN] != win0_en[PE_WIN_HW_H13_CEN_WIN]) || \
				(win_info->win1_en[PE_WIN_HW_H13_CEN_WIN] != win1_en[PE_WIN_HW_H13_CEN_WIN]) || \
				(win_info->win01_en[PE_WIN_HW_H13_CEN_WIN] != win01_en[PE_WIN_HW_H13_CEN_WIN]))
			{
				PE_WIN_HW_H13_DBG_PRINT("[inf]ex_frc:%d,rev:%d,out:%d,ofst[0]:%d,%d,ofst[1]:%d,%d\n",\
					disp0_info->mode.is_ext_frc,disp0_info->mode.is_reverse,disp0_info->out_type,\
					x_ofst[0],y_ofst[0],x_ofst[1],y_ofst[1]);
				PE_WIN_HW_H13_DBG_PRINT("[inf]o_win0:%d,%d,%d,%d,o_win1:%d,%d,%d,%d\n",\
					...
					ctrl0_info->m_win_v_size,ctrl1_info->m_win_h_ofst,ctrl1_info->m_win_v_ofst,\
					ctrl1_info->m_win_h_size,ctrl1_info->m_win_v_size);
				PE_WIN_HW_H13_DBG_PRINT("[set]cen_win_en(0,1,01):%d,%d,%d \n",\
					win_info->win0_en[PE_WIN_HW_H13_CEN_WIN], win_info->win1_en[PE_WIN_HW_H13_CEN_WIN],\
					win_info->win01_en[PE_WIN_HW_H13_CEN_WIN]);
				PE_P1L_H13B0_QWr03(pe1_win2_ctrl_00,	win0_en,	win0_en[PE_WIN_HW_H13_CEN_WIN],\
														win1_en,	win1_en[PE_WIN_HW_H13_CEN_WIN],\
														win01_en,	win01_en[PE_WIN_HW_H13_CEN_WIN]);
				win_info->win0_en[PE_WIN_HW_H13_CEN_WIN] = win0_en[PE_WIN_HW_H13_CEN_WIN];
				win_info->win1_en[PE_WIN_HW_H13_CEN_WIN] = win1_en[PE_WIN_HW_H13_CEN_WIN];
				win_info->win01_en[PE_WIN_HW_H13_CEN_WIN] = win01_en[PE_WIN_HW_H13_CEN_WIN];
			}
			/* etc */
			if((win_info->h_max != h_max)|| (win_info->v_max != v_max))
			{
				...
				PE_WIN_HW_H13_DBG_PRINT("[inf]o_win0:%d,%d,%d,%d,o_win1:%d,%d,%d,%d\n",\
					...
					ctrl0_info->m_win_v_size,ctrl1_info->m_win_h_ofst,ctrl1_info->m_win_v_ofst,\
					ctrl1_info->m_win_h_size,ctrl1_info->m_win_v_size);
				PE_WIN_HW_H13_DBG_PRINT("[set]h,v_max : prev:%d,%d, cur:%d,%d\n",\
					win_info->h_max,win_info->v_max,h_max,v_max);
				#ifdef PE_CCM_HW_H13B_USE_GMC_PXL_REP
				PE_P1L_H13B0_QWr02(pe1_gmc_ctrl_00,		pxl_rep_xpos,	0x0,\
														pxl_rep_ypos,	0x0);
				PE_P1L_H13B0_QWr02(pe1_gmc_ctrl_01,		pxl_rep_width,	h_max,\
														pxl_rep_height,	v_max);
				#endif
				PE_P1L_H13B0_QWr02(pe1_top_ctrl_01,		width,	h_max,\
														height,	v_max);
		...
		else if(PE_KDRV_VER_H13AX)
		{
			...
			if(ctrl0_info->m_win_h_size <= (x_ofst[0]<<1) || \
				ctrl0_info->m_win_v_size <= (y_ofst[0]<<1))
			{
				x0[LX_PE_WIN_0] = 0;
				y0[LX_PE_WIN_0] = 0;
				x1[LX_PE_WIN_0] = h_max-1;
			...
			if(ctrl1_info->m_win_h_size <= (x_ofst[1]<<1) || \
				ctrl1_info->m_win_v_size <= (y_ofst[1]<<1))
			{
				x0[LX_PE_WIN_1] = 0;
				y0[LX_PE_WIN_1] = 0;
				x1[LX_PE_WIN_1] = h_max-1;
			...
			if((win_info->act0_x0 != act0_x0)|| (win_info->act0_y0 != act0_y0)|| \
				...
				(win_info->act1_x1 != act1_x1)|| (win_info->act1_y1 != act1_y1))
			{
				PE_WIN_HW_H13_DBG_PRINT("[inf]is_ext_frc:%d,out:%d,ofst[0]:%d,%d,ofst[1]:%d,%d\n",\
					disp0_info->mode.is_ext_frc,disp0_info->out_type,x_ofst[0],y_ofst[0],x_ofst[1],y_ofst[1]);
				PE_WIN_HW_H13_DBG_PRINT("[inf]o_win0:%d,%d,%d,%d,o_win1:%d,%d,%d,%d\n",\
					ctrl0_info->m_win_h_ofst,ctrl0_info->m_win_v_ofst,ctrl0_info->m_win_h_size,\
					ctrl0_info->m_win_v_size,ctrl1_info->m_win_h_ofst,ctrl1_info->m_win_v_ofst,\
					ctrl1_info->m_win_h_size,ctrl1_info->m_win_v_size);
				...
				PE_WIN_HW_H13_DBG_PRINT("[set]act1 : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
					win_info->act1_x0,win_info->act1_y0,win_info->act1_x1,\
					win_info->act1_y1,act1_x0,act1_y0,act1_x1,act1_y1);
	            PE_P1L_H13_QWr02(pe1_win2_ctrl_01,	win_w0_x0,	act0_x0,\
	            									win_w0_y0,	act0_y0);
	            PE_P1L_H13_QWr02(pe1_win2_ctrl_02,	win_w0_x1,	act0_x1,\
	            									win_w0_y1,	act0_y1);
	            PE_P1L_H13_QWr02(pe1_win2_ctrl_03,	win_w1_x0,	act1_x0,\
	            									win_w1_y0,	act1_y0);
	            PE_P1L_H13_QWr02(pe1_win2_ctrl_04,	win_w1_x1,	act1_x1,\
	            									win_w1_y1,	act1_y1);
	            PE_P1L_H13_QWr02(pe1_apl_ctrl_00,	apl_win_ctrl_x0, act0_x0,\
	            									apl_win_ctrl_y0, act0_y0);
	            PE_P1L_H13_QWr02(pe1_apl_ctrl_01,	apl_win_ctrl_x1, act0_x1,\
	            									apl_win_ctrl_y1, act0_y1);
				win_info->act0_x0 = act0_x0;
				win_info->act0_y0 = act0_y0;
				win_info->act0_x1 = act0_x1;
				...
				(win_info->win1_x1 != win1_x1)|| (win_info->win1_y1 != win1_y1))
			{
				...
				PE_WIN_HW_H13_DBG_PRINT("[set]win1 : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
					win_info->win1_x0,win_info->win1_y0,win_info->win1_x1,\
					win_info->win1_y1,win1_x0,win1_y0,win1_x1,win1_y1);
	            PE_P1L_H13_QWr02(pe1_win1_ctrl_01,	win_w0_x0,	win0_x0,\
	            									win_w0_y0,	win0_y0);
	            PE_P1L_H13_QWr02(pe1_win1_ctrl_02,	win_w0_x1,	win0_x1,\
	            									win_w0_y1,	win0_y1);
	            PE_P1L_H13_QWr02(pe1_win1_ctrl_03,	win_w1_x0,	win1_x0,\
	            									win_w1_y0,	win1_y0);
	            PE_P1L_H13_QWr02(pe1_win1_ctrl_04,	win_w1_x1,	win1_x1,\
	            									win_w1_y1,	win1_y1);
				win_info->win0_x0 = win0_x0;
				win_info->win0_y0 = win0_y0;
				win_info->win0_x1 = win0_x1;
			...
			if((win_info->h_max != h_max)|| (win_info->v_max != v_max))
			{
				...
				PE_WIN_HW_H13_DBG_PRINT("[set]h,v_max : prev:%d,%d, cur:%d,%d\n",\
					win_info->h_max,win_info->v_max,h_max,v_max);
	            PE_P1L_H13_QWr02(pe1_win3_ctrl_01,	win_w0_x0,		0x0,\
	            									win_w0_y0,		0x0);
	            PE_P1L_H13_QWr02(pe1_win3_ctrl_02,	win_w0_x1,		(h_max-1),\
	            									win_w0_y1,		(v_max-1));
	            PE_P1L_H13_QWr02(pe1_win3_ctrl_03,	win_w1_x0,		0x0,\
	            									win_w1_y0,		0x0);
	            PE_P1L_H13_QWr02(pe1_win3_ctrl_04,	win_w1_x1,		0x0,\
	            									win_w1_y1,		0x0);
				PE_P1L_H13_QWr02(pe1_gmc_ctrl_00,	pxl_rep_xpos,	0x0,\
													pxl_rep_ypos,	0x0);
				PE_P1L_H13_QWr02(pe1_gmc_ctrl_01,	pxl_rep_width,	h_max,\
													pxl_rep_height,	v_max);
				win_info->h_max = h_max;
				win_info->v_max = v_max;
			}
...
int PE_WIN_HW_H13_SetLrcrWindow(PE_TSK_H13_CFG_T *pCfg)
{
	...
	PE_INF_H13_CTRL_T *ctrl0_info = &pCfg->inf.ctrl_info[LX_PE_WIN_0];
	PE_INF_H13_CTRL_T *ctrl1_info = &pCfg->inf.ctrl_info[LX_PE_WIN_1];
	PE_WIN_HW_H13_LRCR_CTRL_T *lrcr_info=&_g_pe_win_hw_h13_info.lrcr_inf;
	...
	do {
		...
		if(PE_KDRV_VER_H13)
		{
			...
			if(ctrl0_info->m_win_v_size)
			{
				if(PE_KDRV_VER_H13BX)
				{
					/* [h13b0] 2x because of something like dual_tb in case of 2dto3d */
			...
			*/
			switch(disp0_info->out_type)
			{
				case LX_PE_OUT_2D:
				case LX_PE_OUT_2D_3D:
					if(lrc_ctrl->win_mod==PE_HST_HW_H13_LRC_HST_WIN_MOD_TB0 || \
						lrc_ctrl->win_mod==PE_HST_HW_H13_LRC_HST_WIN_MOD_TB1 || \
						lrc_ctrl->win_mod==PE_HST_HW_H13_LRC_HST_WIN_MOD_SS0 || \
						lrc_ctrl->win_mod==PE_HST_HW_H13_LRC_HST_WIN_MOD_SS1)
					{
						if(lrc_ctrl->win_mod==PE_HST_HW_H13_LRC_HST_WIN_MOD_TB0 || \
							lrc_ctrl->win_mod==PE_HST_HW_H13_LRC_HST_WIN_MOD_TB1)
						{
						...
						if(l_x1>PE_WIN_HW_H13_FHD_H_MAX)	l_x1=PE_WIN_HW_H13_FHD_H_MAX;
						if(l_y1>PE_WIN_HW_H13_FHD_V_MAX)	l_y1=PE_WIN_HW_H13_FHD_V_MAX;
						if(r_x1>PE_WIN_HW_H13_FHD_H_MAX)	r_x1=PE_WIN_HW_H13_FHD_H_MAX;
						if(r_y1>PE_WIN_HW_H13_FHD_V_MAX)	r_y1=PE_WIN_HW_H13_FHD_V_MAX;
				...
				(lrcr_info->histo_l_x1!=cur_histo_l_win[2])||(lrcr_info->histo_l_y1!=cur_histo_l_win[3]))
			{
				...
				{
					PE_WIN_HW_H13_DBG_PRINT("[inf]out:%d,o_win0:%d,%d,%d,%d,o_win1:%d,%d,%d,%d\n",\
					...
					PE_WIN_HW_H13_DBG_PRINT("[set]histo_l_win : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
				...
				if(PE_KDRV_VER_H13BX)
				{
					...
					PE_P1L_H13B0_RdFL(pe1_lrcr_ctrl_03);
					PE_P1L_H13B0_Wr01(pe1_lrcr_ctrl_02, x0,		cur_histo_l_win[0]);
					PE_P1L_H13B0_Wr01(pe1_lrcr_ctrl_02, y0,		cur_histo_l_win[1]);
					PE_P1L_H13B0_Wr01(pe1_lrcr_ctrl_03, x1,		cur_histo_l_win[2]);
					PE_P1L_H13B0_Wr01(pe1_lrcr_ctrl_03, y1,		cur_histo_l_win[3]);
					PE_P1L_H13B0_WrFL(pe1_lrcr_ctrl_02);
					PE_P1L_H13B0_WrFL(pe1_lrcr_ctrl_03);
				}
				else
				{
					PE_DPP_H13_RdFL(debd_pp_lrcr_ctrl_02);
					PE_DPP_H13_RdFL(debd_pp_lrcr_ctrl_03);
					PE_DPP_H13_Wr01(debd_pp_lrcr_ctrl_02, x0,	cur_histo_l_win[0]);
					PE_DPP_H13_Wr01(debd_pp_lrcr_ctrl_02, y0,	cur_histo_l_win[1]);
					PE_DPP_H13_Wr01(debd_pp_lrcr_ctrl_03, x1,	cur_histo_l_win[2]);
					PE_DPP_H13_Wr01(debd_pp_lrcr_ctrl_03, y1,	cur_histo_l_win[3]);
					PE_DPP_H13_WrFL(debd_pp_lrcr_ctrl_02);
					PE_DPP_H13_WrFL(debd_pp_lrcr_ctrl_03);
				}
				...
				if((disp0_info->out_type==LX_PE_OUT_2D || \
					...
					lrc_ctrl->win_mod>=PE_HST_HW_H13_LRC_HST_WIN_MOD_TB0 && \
					lrc_ctrl->win_mod<=PE_HST_HW_H13_LRC_HST_WIN_MOD_SS1)
				{
					...
					else if(lrc_ctrl->win_mod==PE_HST_HW_H13_LRC_HST_WIN_MOD_SS0)
					{
					...
					else if(lrc_ctrl->win_mod==PE_HST_HW_H13_LRC_HST_WIN_MOD_TB1)
					{
					...
					else	//PE_HST_HW_H13_LRC_HST_WIN_MOD_SS1
					{
				...
				(lrcr_info->histo_r_x1!=cur_histo_r_win[2])||(lrcr_info->histo_r_y1!=cur_histo_r_win[3]))
			{
				...
				if(disp0_info->out_type!=LX_PE_OUT_2D && disp0_info->out_type!=LX_PE_OUT_2D_3D)
				{
					PE_WIN_HW_H13_DBG_PRINT("[inf]out:%d,o_win0:%d,%d,%d,%d,o_win1:%d,%d,%d,%d\n",\
						...
						ctrl0_info->m_win_h_size,ctrl0_info->m_win_v_size,ctrl1_info->m_win_h_ofst,\
						ctrl1_info->m_win_v_ofst,ctrl1_info->m_win_h_size,ctrl1_info->m_win_v_size);
					PE_WIN_HW_H13_DBG_PRINT("[set]histo_r_win : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
				...
				if(PE_KDRV_VER_H13BX)
				{
					...
					PE_P1L_H13B0_RdFL(pe1_lrcr_ctrl_05);
					PE_P1L_H13B0_Wr01(pe1_lrcr_ctrl_04, x0,		cur_histo_r_win[0]);
					PE_P1L_H13B0_Wr01(pe1_lrcr_ctrl_04, y0,		cur_histo_r_win[1]);
					PE_P1L_H13B0_Wr01(pe1_lrcr_ctrl_05, x1,		cur_histo_r_win[2]);
					PE_P1L_H13B0_Wr01(pe1_lrcr_ctrl_05, y1,		cur_histo_r_win[3]);
					PE_P1L_H13B0_WrFL(pe1_lrcr_ctrl_04);
					PE_P1L_H13B0_WrFL(pe1_lrcr_ctrl_05);
				}
				else
				{
					PE_DPP_H13_RdFL(debd_pp_lrcr_ctrl_04);
					PE_DPP_H13_RdFL(debd_pp_lrcr_ctrl_05);
					PE_DPP_H13_Wr01(debd_pp_lrcr_ctrl_04, x0,	cur_histo_r_win[0]);
					PE_DPP_H13_Wr01(debd_pp_lrcr_ctrl_04, y0,	cur_histo_r_win[1]);
					PE_DPP_H13_Wr01(debd_pp_lrcr_ctrl_05, x1,	cur_histo_r_win[2]);
					PE_DPP_H13_Wr01(debd_pp_lrcr_ctrl_05, y1,	cur_histo_r_win[3]);
					PE_DPP_H13_WrFL(debd_pp_lrcr_ctrl_04);
					PE_DPP_H13_WrFL(debd_pp_lrcr_ctrl_05);
				}
				...
				(lrcr_info->apply_r_x1!=cur_apply_r_win[2])||(lrcr_info->apply_r_y1!=cur_apply_r_win[3]))
			{
				PE_WIN_HW_H13_DBG_PRINT("[inf]out:%d,o_win0:%d,%d,%d,%d,o_win1:%d,%d,%d,%d\n",\
					...
					ctrl0_info->m_win_h_size,ctrl0_info->m_win_v_size,ctrl1_info->m_win_h_ofst,\
					ctrl1_info->m_win_v_ofst,ctrl1_info->m_win_h_size,ctrl1_info->m_win_v_size);
				PE_WIN_HW_H13_DBG_PRINT("[set]apply_r_win : prev:%d,%d,%d,%d, cur:%d,%d,%d,%d\n",\
				...
				if(PE_KDRV_VER_H13BX)
				{
					...
					PE_P1L_H13B0_RdFL(pe1_lrcr_ctrl_07);
					PE_P1L_H13B0_Wr01(pe1_lrcr_ctrl_06, x0,		cur_apply_r_win[0]);
					PE_P1L_H13B0_Wr01(pe1_lrcr_ctrl_06, y0,		cur_apply_r_win[1]);
					PE_P1L_H13B0_Wr01(pe1_lrcr_ctrl_07, x1,		cur_apply_r_win[2]);
					PE_P1L_H13B0_Wr01(pe1_lrcr_ctrl_07, y1,		cur_apply_r_win[3]);
					PE_P1L_H13B0_WrFL(pe1_lrcr_ctrl_06);
					PE_P1L_H13B0_WrFL(pe1_lrcr_ctrl_07);
				}
				else
				{
					PE_DPP_H13_RdFL(debd_pp_lrcr_ctrl_06);
					PE_DPP_H13_RdFL(debd_pp_lrcr_ctrl_07);
					PE_DPP_H13_Wr01(debd_pp_lrcr_ctrl_06, x0,	cur_apply_r_win[0]);
					PE_DPP_H13_Wr01(debd_pp_lrcr_ctrl_06, y0,	cur_apply_r_win[1]);
					PE_DPP_H13_Wr01(debd_pp_lrcr_ctrl_07, x1,	cur_apply_r_win[2]);
					PE_DPP_H13_Wr01(debd_pp_lrcr_ctrl_07, y1,	cur_apply_r_win[3]);
					PE_DPP_H13_WrFL(debd_pp_lrcr_ctrl_06);
					PE_DPP_H13_WrFL(debd_pp_lrcr_ctrl_07);
				}
...
int PE_WIN_HW_H13_SetTnrMeasureWin(PE_TSK_H13_CFG_T *pCfg)
{
	...
	PE_INF_H13_CTRL_T *ctrl0_info = &pCfg->inf.ctrl_info[LX_PE_WIN_0];
	PE_INF_H13_CTRL_T *ctrl1_info = &pCfg->inf.ctrl_info[LX_PE_WIN_1];
	PE_WIN_HW_H13_TNR_CTRL_T *tnr_info=&_g_pe_win_hw_h13_info.tnr_inf;
	do{
		CHECK_KNULL(pCfg);
		if(PE_KDRV_VER_H13)
		{
			...
			if(tnr_info->h_size0!=h_size[0] || tnr_info->v_size0!=v_size[0])
			{
				...
				if(PE_KDRV_VER_H13BX)
				{
					PE_P0L_H13B0_RdFL(p0l_tnr_ctrl_14);
					PE_P0L_H13B0_RdFL(p0l_tnr_ctrl_15);
					PE_P0L_H13B0_Wr01(p0l_tnr_ctrl_14, x0,	cur_x0[0]);
					PE_P0L_H13B0_Wr01(p0l_tnr_ctrl_14, y0,	cur_y0[0]);
					PE_P0L_H13B0_Wr01(p0l_tnr_ctrl_15, x1,	cur_x1[0]);
					PE_P0L_H13B0_Wr01(p0l_tnr_ctrl_15, y1,	cur_y1[0]);
					PE_P0L_H13B0_WrFL(p0l_tnr_ctrl_14);
					PE_P0L_H13B0_WrFL(p0l_tnr_ctrl_15);
				}
				else	//PE_KDRV_VER_H13AX
				{
					PE_P0L_H13_RdFL(p0l_tnr_ctrl_14);
					PE_P0L_H13_RdFL(p0l_tnr_ctrl_15);
					PE_P0L_H13_Wr01(p0l_tnr_ctrl_14, x0,	cur_x0[0]);
					PE_P0L_H13_Wr01(p0l_tnr_ctrl_14, y0,	cur_y0[0]);
					PE_P0L_H13_Wr01(p0l_tnr_ctrl_15, x1,	cur_x1[0]);
					PE_P0L_H13_Wr01(p0l_tnr_ctrl_15, y1,	cur_y1[0]);
					PE_P0L_H13_WrFL(p0l_tnr_ctrl_14);
					PE_P0L_H13_WrFL(p0l_tnr_ctrl_15);
				}
				PE_WIN_HW_H13_DBG_PRINT("[L][size:%4d,%4d -> %4d,%4d][ofst:%4d,%4d][x,y:%4d,%4d,%4d,%4d]\n\n",\
			...
			if(tnr_info->h_size1!=h_size[1] || tnr_info->v_size1!=v_size[1])
			{
				...
				if(PE_KDRV_VER_H13BX)
				{
					PE_P0R_H13B0_RdFL(p0r_tnr_ctrl_14);
					PE_P0R_H13B0_RdFL(p0r_tnr_ctrl_15);
					PE_P0R_H13B0_Wr01(p0r_tnr_ctrl_14, x0,	cur_x0[1]);
					PE_P0R_H13B0_Wr01(p0r_tnr_ctrl_14, y0,	cur_y0[1]);
					PE_P0R_H13B0_Wr01(p0r_tnr_ctrl_15, x1,	cur_x1[1]);
					PE_P0R_H13B0_Wr01(p0r_tnr_ctrl_15, y1,	cur_y1[1]);
					PE_P0R_H13B0_WrFL(p0r_tnr_ctrl_14);
					PE_P0R_H13B0_WrFL(p0r_tnr_ctrl_15);
				}
				else	//PE_KDRV_VER_H13AX
				{
					PE_P0R_H13_RdFL(p0r_tnr_ctrl_14);
					PE_P0R_H13_RdFL(p0r_tnr_ctrl_15);
					PE_P0R_H13_Wr01(p0r_tnr_ctrl_14, x0,	cur_x0[1]);
					PE_P0R_H13_Wr01(p0r_tnr_ctrl_14, y0,	cur_y0[1]);
					PE_P0R_H13_Wr01(p0r_tnr_ctrl_15, x1,	cur_x1[1]);
					PE_P0R_H13_Wr01(p0r_tnr_ctrl_15, y1,	cur_y1[1]);
					PE_P0R_H13_WrFL(p0r_tnr_ctrl_14);
					PE_P0R_H13_WrFL(p0r_tnr_ctrl_15);
				}
				PE_WIN_HW_H13_DBG_PRINT("[R][size:%4d,%4d -> %4d,%4d][ofst:%4d,%4d][x,y:%4d,%4d,%4d,%4d]\n\n",\
			...
			if(_g_win_hw_h13_trace)
			{
				if(count>PE_PRINT_COUNT_NUM)
				{
					PE_WIN_HW_H13_DBG_PRINT("[state][L:%4d,%4d -> %4d,%4d][R:%4d,%4d -> %4d,%4d]\n\n",\
...
{
	int ret = RET_OK;
	PE_WIN_HW_H13_SETTINGS_T *pInfo = &_g_pe_win_hw_h13_info;
	PE_WIN_HW_H13_CTRL_T *win_info = &_g_pe_win_hw_h13_info.win_inf;
	PE_WIN_HW_H13_LRCR_CTRL_T *lrcr_info = &_g_pe_win_hw_h13_info.lrcr_inf;
	PE_WIN_HW_H13_TNR_CTRL_T *tnr_info = &_g_pe_win_hw_h13_info.tnr_inf;
	do{
		...
		if(PE_KDRV_VER_H13)
		{
			memcpy(pstParams,pInfo,sizeof(PE_WIN_HW_H13_SETTINGS_T));
			PE_WIN_HW_H13_DBG_PRINT("act0:%d,%d,%d,%d, act1:%d,%d,%d,%d\n",\
			...
			PE_WIN_HW_H13_DBG_PRINT("win0:%d,%d,%d,%d, win1:%d,%d,%d,%d\n",\
			...
			PE_WIN_HW_H13_DBG_PRINT("h,v_max:%d,%d, w0,w1,w01_en:[%d,%d],[%d,%d],[%d,%d]\n",\
			...
			PE_WIN_HW_H13_DBG_PRINT("histo_l,r, apply_r : %d,%d,%d,%d, %d,%d,%d,%d, %d,%d,%d,%d\n",\
				lrcr_info->histo_l_x0,lrcr_info->histo_l_y0,lrcr_info->histo_l_x1,lrcr_info->histo_l_y1,\
				lrcr_info->histo_r_x0,lrcr_info->histo_r_y0,lrcr_info->histo_r_x1,lrcr_info->histo_r_y1,\
				lrcr_info->apply_r_x0,lrcr_info->apply_r_y0,lrcr_info->apply_r_x1,lrcr_info->apply_r_y1);
			PE_WIN_HW_H13_DBG_PRINT("tnr win h,v size0:%d,%d, size1:%d,%d\n",\
