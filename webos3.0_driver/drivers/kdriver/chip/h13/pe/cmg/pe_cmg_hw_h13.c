...
----------------------------------------------------------------------------------------*/
static void PE_CMG_HW_H13B_Init_CenRegister(void);
 ...
 * @author
 */
int PE_CMG_HW_H13_Init(PE_CFG_CTRL_T *pstParams)
{
	int ret = RET_OK;
	do{
		...
		{
			PE_CMG_HW_H13B_Init_CenRegister();
			PE_P1L_H13B0_QWr01(pe1_cen_ctrl_00,	reg_cen_bypass,	0x1);	//default
...
static void PE_CMG_HW_H13B_Init_CenRegister(void)
{
	UINT32 i;
	/* hue */
	PE_P1L_H13B0_Wr(pe1_cen_ia_ctrl,	0x00001000);
	PE_P1L_H13B0_WrFL(pe1_cen_ia_ctrl);
	for(i=0; i<(LX_PE_CMG_REGION_NUM*LX_PE_CMG_TBLPOINT); i++)
	{
		PE_P1L_H13B0_Wr(pe1_cen_ia_data,0x00000000);
		PE_P1L_H13B0_WrFL(pe1_cen_ia_data);
	}
	/* saturation */
	PE_P1L_H13B0_Wr(pe1_cen_ia_ctrl,	0x00001100);
	PE_P1L_H13B0_WrFL(pe1_cen_ia_ctrl);
	for(i=0; i<(LX_PE_CMG_REGION_NUM*LX_PE_CMG_TBLPOINT); i++)
	{
		PE_P1L_H13B0_Wr(pe1_cen_ia_data,0x00000000);
	...
	/* value */
	PE_P1L_H13B0_Wr(pe1_cen_ia_ctrl,	0x00001200);
	PE_P1L_H13B0_WrFL(pe1_cen_ia_ctrl);
	for(i=0; i<(LX_PE_CMG_REGION_NUM*LX_PE_CMG_TBLPOINT); i++)
	{
		PE_P1L_H13B0_Wr(pe1_cen_ia_data,0x00000000);
	...
	/* debug color */
	PE_P1L_H13B0_Wr(pe1_cen_ia_ctrl,	0x00001300);
	PE_P1L_H13B0_WrFL(pe1_cen_ia_ctrl);
	...
	/* delta */
	PE_P1L_H13B0_Wr(pe1_cen_ia_ctrl,	0x00001400);
	PE_P1L_H13B0_WrFL(pe1_cen_ia_ctrl);
	...
	/* master gain */
	PE_P1L_H13B0_Wr(pe1_cen_ia_ctrl,	0x00001500);
	PE_P1L_H13B0_WrFL(pe1_cen_ia_ctrl);
	for(i=0; i<(LX_PE_CMG_REGION_NUM); i++)
	{
		PE_P1L_H13B0_Wr(pe1_cen_ia_data,0x00000000);
	...
	/* normal mode */
	PE_P1L_H13B0_Wr(pe1_cen_ia_ctrl,	0x00008000);
	PE_P1L_H13B0_WrFL(pe1_cen_ia_ctrl);
	return;
}
/**
...
int PE_CMG_HW_H13_SetDebugSettings(LX_PE_DBG_SETTINGS_T *pstParams)
{
	...
	do{
		...
		PE_CMG_HW_H13_DBG_PRINT("wid:%d,type:0x%x,[0x%x]print:0x%x,bypass:0x%x\n",\
			pstParams->win_id,pstParams->type,(0x1<<PE_ITEM_PKTMODL(CMG)),\
			pstParams->print_lvl,pstParams->bypass);
		/* set print level */
		if(pstParams->type&LX_PE_DBG_LV)
		{
			_g_cmg_hw_h13_trace = (pstParams->print_lvl & (0x1<<PE_ITEM_PKTMODL(CMG)))? 0x1:0x0;
		}
		/* set bypass */
		if(pstParams->type&LX_PE_DBG_BY)
		{
			if(PE_KDRV_VER_H13BX)
			{
				...
				{
					PE_CMG_HW_H13_DBG_PRINT("cen bypass.\n");
				...
				{
					PE_CMG_HW_H13_DBG_PRINT("cen on.\n");
					if(PE_CHECK_WIN0(pstParams->win_id))
					{
...
int PE_CMG_HW_H13_SetCenEnable(LX_PE_CMG_ENABLE_T *pstParams)
{
	...
	do {
		CHECK_KNULL(pstParams);
		PE_CHECK_WINID(pstParams->win_id);
		PE_CMG_HW_H13_DBG_PRINT(" set[%d]: cen enable:%d\n",pstParams->win_id,pstParams->enable);
		if(PE_KDRV_VER_H13BX)
		{
			if(PE_CHECK_WIN0(pstParams->win_id))
			{
...
int PE_CMG_HW_H13_GetCenEnable(LX_PE_CMG_ENABLE_T *pstParams)
{
	...
	do {
		...
		PE_CMG_HW_H13_DBG_PRINT(" get[%d]: cen enable:%d\n",pstParams->win_id,pstParams->enable);
	} while(0);
	return ret;
}
/**
...
int PE_CMG_HW_H13_SetCenRegion(LX_PE_CMG_REGION_T *pstParams)
{
	...
	#ifdef PE_CMG_HW_H13_CHK_CEN_DOUBLE_SET
	UINT32 set_flag=0;
...
int PE_CMG_HW_H13_SetCenColorCtrl(LX_PE_CMG_COLOR_CTRL_T *pstParams)
{
	...
	do {
		CHECK_KNULL(pstParams);
		PE_CHECK_WINID(pstParams->win_id);
		PE_CMG_HW_H13_DBG_PRINT("set pstParams[%d] : sat:%d\n",pstParams->win_id,pstParams->saturation);
		if(PE_KDRV_VER_H13BX)
		{
			if(PE_CHECK_WIN0(pstParams->win_id))
			{
...
int PE_CMG_HW_H13_GetCenColorCtrl(LX_PE_CMG_COLOR_CTRL_T *pstParams)
{
	...
	do {
		...
		PE_CMG_HW_H13_DBG_PRINT("get pstParams[%d] : sat:%d\n",pstParams->win_id,pstParams->saturation);
	} while (0);
	return ret;
}
/**
...
int PE_CMG_HW_H13_SetClearWhiteCtrl(LX_PE_CMG_CW_CTRL_T *pstParams)
{
	...
	do{
		...
		if(PE_KDRV_VER_H13BX)
		{
			PE_CMG_HW_H13_DBG_PRINT("set[%d]:en:%d, yc_sel:%d, x:%d,%d,%d,%d,%d, y:%d,%d,%d,%d,%d\n"\
...
int PE_CMG_HW_H13_GetClearWhiteCtrl(LX_PE_CMG_CW_CTRL_T *pstParams)
{
	...
	do{
		...
		if(PE_KDRV_VER_H13BX)
		{
			...
			PE_CMG_HW_H13_DBG_PRINT("get[%d]:en:%d, yc_sel:%d, x:%d,%d,%d,%d,%d, y:%d,%d,%d,%d,%d\n"\
				"sel[0~3]%d,%d,%d,%d,[4~7]%d,%d,%d,%d,[8~11]%d,%d,%d,%d,[12~15]%d,%d,%d,%d, rgn_g:%d\n",\
				pstParams->win_id,pstParams->cw_en,pstParams->gain_sel,pstParams->gain_x[0],pstParams->gain_x[1],\
				pstParams->gain_x[2],pstParams->gain_x[3],pstParams->gain_x[4],pstParams->gain_y[0],\
				pstParams->gain_y[1],pstParams->gain_y[2],pstParams->gain_y[3],pstParams->gain_y[4],\
