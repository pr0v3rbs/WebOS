...
int PE_CMN_HW_H13_Init(PE_CFG_CTRL_T *pstParams)
{
	...
	do{
		...
		{
			memset_0xff(_g_pe_cmn_hw_h13_info);
			_g_pe_cmn_hw_h13_info.dbg_print_lvl = 0x0;
			_g_pe_cmn_hw_h13_info.dbg_bypass[LX_PE_WIN_0] = 0x0;
			_g_pe_cmn_hw_h13_info.dbg_bypass[LX_PE_WIN_1] = 0x0;
...
{
	int ret = RET_OK;
	PE_CMN_HW_H13_SETTINGS_T *pInfo = &_g_pe_cmn_hw_h13_info;
	do{
		CHECK_KNULL(pstParams);
		PE_CHECK_WINID(pstParams->win_id);
		PE_CMN_HW_H13_DBG_PRINT("set[%d] type:0x%x, print:0x%x, bypass:0x%x, fwc_en,dbg:%d,%d\n",\
			pstParams->win_id,pstParams->type,pstParams->print_lvl,pstParams->bypass,\
			pstParams->fwc.ctrl_en,pstParams->fwc.dbg_en);
		/* set print level */
		if(pstParams->type&LX_PE_DBG_LV)
		{
			_g_cmn_hw_h13_trace = (pstParams->print_lvl & (0x1<<PE_ITEM_PKTMODL(CMN)))? 0x1:0x0;
		...
		if(pstParams->type&LX_PE_DBG_BY)
		{
			if(PE_KDRV_VER_H13BX)
			{
				...
				{
					PE_CMN_HW_H13_DBG_PRINT("cont,bri,sat disable, hue bypass.\n");
				...
				{
					PE_CMN_HW_H13_DBG_PRINT("cont,bri,sat enable.\n");
					if(PE_CHECK_WIN0(pstParams->win_id))
					{
			...
			{
				PE_CMN_HW_H13_DBG_PRINT("do nothing.\n");	ret = RET_OK;
			}
			if(PE_CHECK_WIN0(pstParams->win_id))
			{
				pInfo->dbg_bypass[LX_PE_WIN_0] = pstParams->bypass;
		...
		if(pstParams->type&LX_PE_DBG_FW)
		{
			if(PE_KDRV_VER_H13)
			{
				...
				PE_CMN_HW_H13_CHECK_CODE(ret,break,"[%s,%d] PE_FWI_H13_SetFwCtrl() error.\n",__F__,__L__);
			}
			else
			{
				PE_CMN_HW_H13_DBG_PRINT("do nothing.\n");	ret = RET_OK;
...
int PE_CMN_HW_H13_GetDebugSettings(LX_PE_DBG_SETTINGS_T *pstParams)
{
	...
	do{
		...
		if(PE_KDRV_VER_H13)
		{
			...
			PE_CMN_HW_H13_CHECK_CODE(ret,break,"[%s,%d] PE_FWI_H13_GetCurFwiSettings() error.\n",__F__,__L__);
			pstParams->fwc.ctrl_en = param.fw_ctrl.enable;
		}
		else
		{
			PE_CMN_HW_H13_DBG_PRINT("not use fwc.\n");
			pstParams->fwc.dbg_en = 0;
			pstParams->fwc.ctrl_en = 0;
		}
		PE_CMN_HW_H13_DBG_PRINT("get[%d] type:0x%x, print:0x%x, bypass:0x%x, fwc_en,dbg:%d,%d\n",\
			pstParams->win_id,pstParams->type,pstParams->print_lvl,pstParams->bypass,\
			pstParams->fwc.ctrl_en,pstParams->fwc.dbg_en);
	}while(0);
	return ret;
...
int PE_CMN_HW_H13_SetContrast(LX_PE_CMN_CONTRAST_T *pstParams)
{
	...
	do{
		CHECK_KNULL(pstParams);
		PE_CHECK_WINID(pstParams->win_id);
		PE_CMN_HW_H13_DBG_PRINT("set pstParams[%d] : %d\n",pstParams->win_id,pstParams->contrast);
		if(PE_KDRV_VER_H13BX)
		{
			if(PE_CHECK_WIN0(pstParams->win_id))
			{
...
int PE_CMN_HW_H13_GetContrast(LX_PE_CMN_CONTRAST_T *pstParams)
{
	...
	do{
		...
		PE_CMN_HW_H13_DBG_PRINT("get pstParams[%d] : %d\n",pstParams->win_id,pstParams->contrast);
	}while(0);
	return ret;
}
/**
...
int PE_CMN_HW_H13_SetLevelCtrl(LX_PE_CMN_LEVEL_CTRL_T *pstParams)
{
	...
	do{
		CHECK_KNULL(pstParams);
		PE_CHECK_WINID(pstParams->win_id);
		PE_CMN_HW_H13_DBG_PRINT("set pstParams[%d] : ofs:%d, center:%d, gain:%d, tp:%d\n",\
...
int PE_CMN_HW_H13_GetLevelCtrl(LX_PE_CMN_LEVEL_CTRL_T *pstParams)
{
	...
	do{
		...
		PE_CMN_HW_H13_DBG_PRINT("get pstParams[%d] : ofs:%d, center:%d, gain:%d, tp:%d\n",\
			pstParams->win_id,pstParams->offset,pstParams->center,\
			pstParams->gain,pstParams->tp_on);
	}while(0);
	return ret;
