...
/*----------------------------------------------------------------------------------------
	Macro Definitions
----------------------------------------------------------------------------------------*/
#define PE_CSC_HW_H13_ERROR	printk
#define PE_CSC_HW_H13_DBG_PRINT(fmt,args...)	\
	if(_g_csc_hw_h13_trace) printk("[%x,%x][%s,%d] "fmt,PE_CHIP_VER,g_pe_kdrv_ver_mask,__F__,__L__,##args)
#define PE_CSC_HW_H13_CHECK_CODE(_checker,_action,fmt,args...)	\
	if(_checker){PE_CSC_HW_H13_ERROR(fmt,##args);_action;}
...
/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
static UINT32 _g_csc_hw_h13_trace=0x0;		//default should be off.
...
int PE_CSC_HW_H13_Init(PE_CFG_CTRL_T *pstParams)
{
	...
	do{
		...
		{
			PE_CSC_HW_H13_DBG_PRINT("init csc.\n");
		...
		{
			PE_CSC_HW_H13_DBG_PRINT("nothing to do.\n");	ret = RET_OK;
		}
	}while(0);
	return ret;
}
...
int PE_CSC_HW_H13_SetDebugSettings(LX_PE_DBG_SETTINGS_T *pstParams)
{
	...
	do{
		...
		PE_CSC_HW_H13_DBG_PRINT("wid:%d,type:0x%x,[0x%x]print:0x%x,bypass:0x%x\n",\
			pstParams->win_id,pstParams->type,(0x1<<PE_ITEM_PKTMODL(CSC)),\
			pstParams->print_lvl,pstParams->bypass);
		/* set print level */
		if(pstParams->type&LX_PE_DBG_LV)
		{
			_g_csc_hw_h13_trace = (pstParams->print_lvl & (0x1<<PE_ITEM_PKTMODL(CSC)))? 0x1:0x0;
		}
		/* set bypass */
		if(pstParams->type&LX_PE_DBG_BY)
		{
			if(PE_KDRV_VER_H13BX)
			{
				...
				{
					PE_CSC_HW_H13_DBG_PRINT("degamma,pcc,gamma,s scaler : disable.\n");
				...
				{
					PE_CSC_HW_H13_DBG_PRINT("outcsc,degamma,pcc,gamma : enable.\n");
					if(PE_CHECK_WIN0(pstParams->win_id))
					{
			...
			{
				PE_CSC_HW_H13_DBG_PRINT("do nothing.\n");	ret = RET_OK;
			}
		}
	}while(0);
	return ret;
...
int PE_CSC_HW_H13_SetColorGamut(LX_PE_CSC_GAMUT_T *pstParams)
{
	...
	do{
		CHECK_KNULL(pstParams);
		PE_CHECK_WINID(pstParams->win_id);
		PE_CSC_HW_H13_DBG_PRINT("set pstParams[%d] : 0x%x,0x%x,0x%x, 0x%x,0x%x,0x%x, 0x%x,0x%x,0x%x\n",\
...
int PE_CSC_HW_H13_GetColorGamut(LX_PE_CSC_GAMUT_T *pstParams)
{
	...
	do{
		...
		PE_CSC_HW_H13_DBG_PRINT("get pstParams[%d] : 0x%x,0x%x,0x%x, 0x%x,0x%x,0x%x, 0x%x,0x%x,0x%x\n",\
...
int PE_CSC_HW_H13_SetPostCsc(LX_PE_CSC_POST_T *pstParams)
{
	...
	do{
		CHECK_KNULL(pstParams);
		PE_CHECK_WINID(pstParams->win_id);
		PE_CSC_HW_H13_DBG_PRINT("set pstParams[%d] : \n"\
...
int PE_CSC_HW_H13_GetPostCsc(LX_PE_CSC_POST_T *pstParams)
{
	...
	do{
		...
		PE_CSC_HW_H13_DBG_PRINT("get pstParams[%d] : \n"\
			"coef:%x,%x,%x, %x,%x,%x, %x,%x,%x, ofs:%x,%x,%x, %x,%x,%x\n",\
			pstParams->win_id,pstParams->matrix[0],pstParams->matrix[1],pstParams->matrix[2],\
			pstParams->matrix[3],pstParams->matrix[4],pstParams->matrix[5],pstParams->matrix[6],\
			pstParams->matrix[7],pstParams->matrix[8],pstParams->offset[0],pstParams->offset[1],\
...
int PE_CSC_HW_H13_SetInputCsc(LX_PE_CSC_INPUT_T *pstParams)
{
	...
	do{
		CHECK_KNULL(pstParams);
		PE_CHECK_WINID(pstParams->win_id);
		PE_CSC_HW_H13_DBG_PRINT("set pstParams[%d] : en:%d\n"\
			"coef:%x,%x,%x, %x,%x,%x, %x,%x,%x, ofs:%x,%x,%x, %x,%x,%x\n",\
		...
		if(PE_KDRV_VER_H13BX)
		{
			...
			if(PE_CHECK_WIN0(win_id))
			{
				...
				ret = PE_FWI_H13_SetCviCsc(pstParams);
				PE_CSC_HW_H13_CHECK_CODE(ret,break,"[%s,%d] PE_FWI_H13_SetCviCsc() error.\n",__F__,__L__);
			}
...
int PE_CSC_HW_H13_GetInputCsc(LX_PE_CSC_INPUT_T *pstParams)
{
	...
	do{
		...
		PE_CSC_HW_H13_DBG_PRINT("get pstParams[%d] : en:%d\n"\
			"coef:%x,%x,%x, %x,%x,%x, %x,%x,%x, ofs:%x,%x,%x, %x,%x,%x\n",\
			pstParams->win_id,pstParams->enable,\
			pstParams->matrix[0],pstParams->matrix[1],pstParams->matrix[2],\
			pstParams->matrix[3],pstParams->matrix[4],pstParams->matrix[5],\
...
int PE_CSC_HW_H13_SetxvYCC(LX_PE_CSC_XVYCC_T *pstParams)
{
	...
	do{
		CHECK_KNULL(pstParams);
		PE_CHECK_WINID(pstParams->win_id);
		PE_CSC_HW_H13_DBG_PRINT("set pstParams[%d] : en:%d, sclr:0x%x\n",\
...
int PE_CSC_HW_H13_GetxvYCC(LX_PE_CSC_XVYCC_T *pstParams)
{
	...
	do{
		...
		PE_CSC_HW_H13_DBG_PRINT("get pstParams[%d] : en:%d, sclr:0x%x\n",\
			pstParams->win_id,pstParams->enable,pstParams->scaler);
	}while(0);
	return ret;
}
