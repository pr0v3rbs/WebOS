...
/*----------------------------------------------------------------------------------------
	Macro Definitions
----------------------------------------------------------------------------------------*/
#define PE_FWI_H13_ERROR	PE_PRINT_ERROR
...
#define PE_FWI_H13_IPC0_CHK_NULL()		\
	PE_FWI_H13_CHECK_CODE((pe_fwi_h13_mcu0_ipc==NULL || \
	pe_fwi_h13_mcu0_ipc->m_ipc_client->write==NULL || \
	pe_fwi_h13_mcu0_ipc->m_ipc_client->read==NULL), \
	ret = RET_ERROR;break, "[%s,%d] mcu0_ipc is null.\n", __F__, __L__)

#define PE_FWI_H13_IPC0_CHK_READY()		\
	PE_FWI_H13_CHECK_CODE(pe_fwi_h13_mcu0_ipc->p_ready==NULL, \
	ret = RET_ERROR;break, "[%s,%d] mcu0_ipc is not ready.\n", __F__, __L__)

#define PE_FWI_H13_IPC0_SET_DATA(cmd, msg, size)		\
	pe_fwi_h13_mcu0_ipc->m_ipc_client->write( cmd , msg , size )

#define PE_FWI_H13_IPC1_CHK_NULL()		\
	PE_FWI_H13_CHECK_CODE((pe_fwi_h13_mcu1_ipc==NULL || \
	pe_fwi_h13_mcu1_ipc->m_ipc_client->write==NULL || \
	pe_fwi_h13_mcu1_ipc->m_ipc_client->read==NULL), \
	ret = RET_ERROR;break, "[%s,%d] mcu1_ipc is null.\n", __F__, __L__)

#define PE_FWI_H13_IPC1_CHK_READY()		\
	PE_FWI_H13_CHECK_CODE(pe_fwi_h13_mcu1_ipc->p_ready==NULL, \
	ret = RET_ERROR;break, "[%s,%d] mcu1_ipc is not ready.\n", __F__, __L__)

#define PE_FWI_H13_IPC1_SET_DATA(cmd, msg, size)		\
	pe_fwi_h13_mcu1_ipc->m_ipc_client->write( cmd , msg , size )
...
/*----------------------------------------------------------------------------------------
	global Variables
----------------------------------------------------------------------------------------*/
struct ipc_handle *pe_fwi_h13_mcu0_ipc = NULL;
struct ipc_handle *pe_fwi_h13_mcu1_ipc = NULL;

/*----------------------------------------------------------------------------------------
	Static Function Prototypes Declarations
----------------------------------------------------------------------------------------*/
static int PE_FWI_H13_IPC_SetData(UINT32 mcuId, UINT32 ipcCmd, void *pMsg, UINT32 msgCnt);
...
static int PE_FWI_H13_IPC_SetData(UINT32 mcuId, UINT32 ipcCmd, void *pMsg, UINT32 msgCnt)
{
	...
	do{
		...
		if (PE_KDRV_VER_H13)
		{
			if (mcuId == MCU_ID_0)
			{
				PE_FWI_H13_IPC0_CHK_NULL();
				PE_FWI_H13_IPC0_CHK_READY();
				PE_FWI_H13_IPC0_SET_DATA(ipcCmd, pMsg, msgCnt);
			}
			else if (mcuId == MCU_ID_1)
			{
				PE_FWI_H13_IPC1_CHK_NULL();
				PE_FWI_H13_IPC1_CHK_READY();
				PE_FWI_H13_IPC1_SET_DATA(ipcCmd, pMsg, msgCnt);
			}
			else
			{
				PE_FWI_H13_ERROR("[%s,%d] mcuId(%d) is invalid.\n", __F__, __L__, mcuId);
				ret = RET_ERROR;
			}
		}
		else
...
static int PE_FWI_H13_SetFwSettings(PE_FWI_PQ_TABLE_HEAD_T header, UINT8 *pTable, UINT32 tblSize)
{
	...
	do{
		...
		ret = PE_FWI_H13_IPC_SetData(MCU_ID_0, ipcCmd, pData, dataSize);
		PE_FWI_H13_CHECK_CODE(ret, break, \
			"[%s,%d] PE_FWI_H13_IPC_SetData() error.\n", __F__, __L__);
	}while (0);
...
int PE_FWI_H13_IPC_Open(void)
{
	...
	do{
		if (PE_KDRV_VER_H13)
		{
			...
			/* check pe_fwi_h13_mcu0_ipc is already opened */
			if(pe_fwi_h13_mcu0_ipc == NULL)
			{
				pe_fwi_h13_mcu0_ipc = ipc_hal_open("pe", MCU_ID_0);
				PE_FWI_H13_CHECK_CODE(!pe_fwi_h13_mcu0_ipc, ret = RET_ERROR;break, \
					"[%s,%d] pe_fwi_h13_mcu0_ipc open fail.\n", __F__, __L__);
			}
			/* check pe_fwi_h13_mcu1_ipc is already opened */
			if(pe_fwi_h13_mcu1_ipc == NULL)
			{
				pe_fwi_h13_mcu1_ipc = ipc_hal_open("pe", MCU_ID_1);
				PE_FWI_H13_CHECK_CODE(!pe_fwi_h13_mcu1_ipc, ret = RET_ERROR;break, \
					"[%s,%d] pe_fwi_h13_mcu1_ipc open fail.\n", __F__, __L__);
			}
		}
		else
		{
...
int PE_FWI_H13_IPC_Close(void)
{
	...
	do{
		if (PE_KDRV_VER_H13)
		{
			...
			if (pe_fwi_h13_mcu0_ipc)
			{
				ipc_hal_close(pe_fwi_h13_mcu0_ipc);
				pe_fwi_h13_mcu0_ipc = NULL;
			}
			if (pe_fwi_h13_mcu1_ipc)
			{
				ipc_hal_close(pe_fwi_h13_mcu1_ipc);
				pe_fwi_h13_mcu1_ipc = NULL;
			}
		}
		else
		{
...
int PE_FWI_H13_Init(PE_CFG_CTRL_T *pstParams)
{
	...
	do{
		...
		if (PE_KDRV_VER_H13)
		{
			...
			memset(&(_g_pe_fwi_h13_info.cen_ctrl[0]), 0x0, \
				sizeof(PE_FWI_H13_CEN_CTRL)*LX_PE_CMG_REGION_NUM);
			/* fw_ctrl.enable(=0:by register, =1:by fw) */
			if (PE_KDRV_VER_H13BX)
			#ifdef PE_FWI_H13_NOT_USE_SHP_FW
				fw_ctrl.enable = 0x0;
			#else
				fw_ctrl.enable = 0x1;
			#endif
			...
			PE_FWI_H13_CHECK_CODE(ret, break, \
				"[%s,%d] PE_FWI_H13_SetFwCtrl() error.\n", __F__, __L__);
			if (PE_KDRV_VER_H13BX)
			{
				PE_P1L_H13B0_QWr01(pe1_wb_ctrl_01, reg_shp_fw_en, 0x1);
			}
		}
		else
		{
	...
	do{
		...
		if (PE_KDRV_VER_H13)
		{
			if (pInfo->fw_ctrl.enable!=pstParams->enable)
			{
				...
				PE_FWI_H13_CHECK_CODE(ret, break, \
					"[%s,%d] PE_FWI_H13_SetFwSettings() error.\n", __F__, __L__);
	...
	do{
		...
		if (PE_KDRV_VER_H13BX)
		{
			...
			* because of avoiding peaking transition artifact */
			...
			{
				#ifdef PE_FWI_H13_NOT_USE_SHP_FW
					cur_fw_ctrl.enable = 0x0;
				#else
					cur_fw_ctrl.enable = 0x1;
				#endif
			...
			if (pre_fw_ctrl.enable!=cur_fw_ctrl.enable && !cur_fw_ctrl.enable)
			{
				...
				PE_FWI_H13_CHECK_CODE(ret, break, \
					"[%s,%d] PE_FWI_H13_SetShpHorCtrl() error.\n", __F__, __L__);
				ret = PE_FWI_H13_SetShpMiscCtrl(&shp_m_ctrl);
				PE_FWI_H13_CHECK_CODE(ret, break, \
					"[%s,%d] PE_FWI_H13_SetShpMiscCtrl() error.\n", __F__, __L__);
				ret = PE_FWI_H13_SetEdgeGainCtrl(&shp_e_ctrl);
				PE_FWI_H13_CHECK_CODE(ret, break, \
					"[%s,%d] PE_FWI_H13_SetEdgeGainCtrl() error.\n", __F__, __L__);
				ret = PE_FWI_H13_SetTextureGainCtrl(&shp_t_ctrl);
				PE_FWI_H13_CHECK_CODE(ret, break, \
					"[%s,%d] PE_FWI_H13_SetTextureGainCtrl() error.\n", __F__, __L__);
			}
		}
		else
		{
...
int PE_FWI_H13_SetFwTnrEnable(UINT32 on_off)
{
	...
	do{
		if (PE_KDRV_VER_H13)
		{
			PE_FWI_H13_DBG_PRINT("set on_off : %d\n", on_off);
			ret = PE_FWI_H13_IPC_SetData(MCU_ID_0, ipc_cmd, &on_off, sizeof(on_off));
			PE_FWI_H13_CHECK_CODE(ret, break, \
				"[%s,%d] PE_FWI_H13_IPC_SetData() error.\n", __F__, __L__);
		}
		else
...
int PE_FWI_H13_SetCviCsc(LX_PE_CSC_INPUT_T *pstParams)
{
	...
	do{
		...
		if (PE_KDRV_VER_H13)
		{
			PE_FWI_H13_DBG_PRINT("set pstParams[%d] : en:%d\n"\
			...
			ret = PE_FWI_H13_IPC_SetData(MCU_ID_0, \
...
int PE_FWI_H13_SetShpMiscCtrl(PE_FWI_H13_MISC_CTRL *pstParams)
{
	...
	do{
		...
		if (PE_KDRV_VER_H13)
		{
			...
			{
				PE_FWI_H13_DBG_PRINT("gt01ab:pre[%d, %d, %d, %d]->"\
					"cur[%d, %d, %d, %d], same..\n", \
	...
	do{
		...
		if (PE_KDRV_VER_H13)
		{
			if (!pInfo->fw_ctrl.enable)
			{
				...
				PE_FWI_H13_CHECK_CODE(ret, break, \
					"[%s,%d] PE_INF_H13_GetCurInfSettings() error.\n", __F__, __L__);
				...
				* with main atv on split window mode  */
				...
				PE_P1L_H13B0_RdFL(pe1_coring_ctrl_09);
				PE_P1L_H13B0_RdFL(pe1_dp_ctrl_01);
				PE_P1L_H13B0_Wr01(pe1_coring_ctrl_09, 	edge_gain_b, 	cur_gain[0]);
				PE_P1L_H13B0_Wr01(pe1_coring_ctrl_09, 	edge_gain_w, 	cur_gain[1]);
				PE_P1L_H13B0_Wr01(pe1_dp_ctrl_01, 		edge_gain_b, 	cur_gain[2]);
				PE_P1L_H13B0_Wr01(pe1_dp_ctrl_01, 		edge_gain_w, 	cur_gain[3]);
				PE_P1L_H13B0_WrFL(pe1_coring_ctrl_09);
				PE_P1L_H13B0_WrFL(pe1_dp_ctrl_01);
				#else
				PE_FWI_H13_DBG_PRINT("fw_ctrl.enable:%d, just return\n", \
					pInfo->fw_ctrl.enable);
				ret = RET_OK;
			...
			{
				PE_FWI_H13_DBG_PRINT("eg_b, w:pre[%d, %d, %d, %d]->"\
					"cur[%d, %d, %d, %d], same..\n", \
		...
		if (PE_KDRV_VER_H13)
		{
			if (!pInfo->fw_ctrl.enable)
			{
				...
				* with main atv on split window mode  */
				...
				PE_P1L_H13B0_RdFL(pe1_coring_ctrl_09);
				PE_P1L_H13B0_RdFL(pe1_dp_ctrl_01);
				PE_P1L_H13B0_Wr01(pe1_coring_ctrl_09, 	texture_gain_b, 	cur_gain[0]);
				PE_P1L_H13B0_Wr01(pe1_coring_ctrl_09, 	texture_gain_w, 	cur_gain[1]);
				PE_P1L_H13B0_Wr01(pe1_dp_ctrl_01, 		texture_gain_b, 	cur_gain[2]);
				PE_P1L_H13B0_Wr01(pe1_dp_ctrl_01, 		texture_gain_w, 	cur_gain[3]);
				PE_P1L_H13B0_WrFL(pe1_coring_ctrl_09);
				PE_P1L_H13B0_WrFL(pe1_dp_ctrl_01);
				#else
				PE_FWI_H13_DBG_PRINT("fw_ctrl.enable:%d, just return\n", \
			...
			{
				PE_FWI_H13_DBG_PRINT("tg_b, w:pre[%d, %d, %d, %d]->"\
					"cur[%d, %d, %d, %d], same..\n", \
...
int PE_FWI_H13_SetTnrCtrl(UINT32 win_id, PE_FWI_H13_TNR_CTRL *pstParams)
{
	...
	do{
		...
		if (PE_KDRV_VER_H13)
		{
			...
			{
				PE_FWI_H13_DBG_PRINT("wid:%d, core, m_en[%d]:"\
	...
	do{
		...
		if (PE_KDRV_VER_H13)
		{
				...
				(pInfo->cen_ctrl[pstParams->rgn_num].ma_gain!=pstParams->ma_gain))
			{
				...
				ret = PE_FWI_H13_IPC_SetData(MCU_ID_1, ipc_cmd, &fw_ctrl0, tblsize0);
				PE_FWI_H13_CHECK_CODE(ret, break, \
					"[%s,%d][GAIN0] PE_FWI_H13_IPC1_SETDATA() error.\n", __F__, __L__);
				ipc_cmd = BE_PE_SET_CEN_GAIN1;
				ret = PE_FWI_H13_IPC_SetData(MCU_ID_1, ipc_cmd, &fw_ctrl1, tblsize1);
				PE_FWI_H13_CHECK_CODE(ret, break, \
					"[%s,%d][GAIN1] PE_FWI_H13_IPC1_SETDATA() error.\n", __F__, __L__);
...
/**
 * set dce histo read ctrl
 *
 * @param   *pstParams [in] PE_FWI_H13_HISTO_CTRL_T
 * @return  OK if success, ERROR otherwise.
 * @see
 * @author
 */
int PE_FWI_H13_SetDceHistoReadCtrl(PE_FWI_H13_HISTO_CTRL_T *pstParams)
{
	...
	do{
		...
		{
			UINT32 data;
			if(pstParams->ctrl>=PE_FWI_H13_DCE_HISTO_NUM)
			{
				PE_FWI_H13_DBG_PRINT("histo_ctrl(%d) is over, just return\n", \
					pstParams->ctrl);
				ret = RET_OK;
				break;
			}
			if(pstParams->ctrl==PE_FWI_H13_DCE_HISTO_RD0 || \
				pstParams->ctrl==PE_FWI_H13_DCE_HISTO_RD1)
			{
				data = (ipc_cnt)? \
					PE_FWI_H13_DCE_HISTO_RD1:PE_FWI_H13_DCE_HISTO_RD0;
				if(ipc_cnt)	ipc_cnt=0;
				else		ipc_cnt=1;
			}
			else
			{
			...
			if(_g_fwi_h13_trace)
			{
				...
				{
					PE_FWI_H13_DBG_PRINT("histo_ctrl:%d, data:%d\n", \
						pstParams->ctrl, data);
					count=0;
				}
				count++;
			}
			ipc_cmd = BE_PE_GET_HISTO;
			ret = PE_FWI_H13_IPC_SetData(MCU_ID_1, ipc_cmd, &data, sizeof(UINT32));
			PE_FWI_H13_CHECK_CODE(ret, break, \
				"[%s, %d][HISTO][%d] PE_FWI_H13_IPC_SetData() error.\n", __F__, __L__, data);
		}
		else
		{
			PE_FWI_H13_DBG_PRINT("nothing to do.\n");	ret = RET_OK;
