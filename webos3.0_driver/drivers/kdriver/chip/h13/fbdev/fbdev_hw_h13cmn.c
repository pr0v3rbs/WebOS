...
static void		FBDEV_H13CMN_InitOSDMerge	(void);
static void		FBDEV_H13CMN_ExitOSDMerge	(void);

static BOOLEAN	FBDEV_H13CMN_IsReverseOSD	( int fb_dev_id );

static void		FBDEV_H13CMN_ReInitHW		(void);
...
static int	_g_fbdev_h13_s3d_auto_conn_path	= 0x1;	/* in UHD board, S3D request will change OSD conn to BLEND path automatically */
module_param_named( fbdev_h13_s3d_auto_conn_path, _g_fbdev_h13_s3d_auto_conn_path , int, 0644 );

static int	_g_fbdev_h13_s3d_crsr_sep_path	= 0x0;	/* in UHD board, cursor will keep sep path even though OSD is set to blend path */
module_param_named( fbdev_h13_s3d_crsr_sep_path, _g_fbdev_h13_s3d_crsr_sep_path, int, 0644 );

static int	_g_fbdev_h13_conn_change_tune_delay= 50;	/* sep/blend connection path change delay in ms */
module_param_named( fbdev_h13_conn_change_tune_delay, _g_fbdev_h13_conn_change_tune_delay, int, 0644 );
...
static BOOLEAN	FBDEV_H13CMN_IsReverseOSD	( int fb_dev_id )
{
	...
	OSD_H13_RdFL( osd_mixer_ctrl_sync ); v0 = OSD_H13_Rd00(osd_mixer_ctrl_sync, reg_sync_v_reverse);

	switch(fb_dev_id)
	{
		case 0: OSD_H13_RdFL( osd0_ctrl_main ); v1= OSD_H13_Rd00(osd0_ctrl_main, osd0_v_reverse_en); break;
		case 1: OSD_H13_RdFL( osd1_ctrl_main ); v1= OSD_H13_Rd00(osd1_ctrl_main, osd1_v_reverse_en); break;
		case 2: OSD_H13_RdFL( osd2_ctrl_main ); v1= OSD_H13_Rd00(osd2_ctrl_main, osd2_v_reverse_en); break;
		case 3: OSD_H13_RdFL( osd3_ctrl_main ); v1= OSD_H13_Rd00(osd3_ctrl_main, osd3_v_reverse_en); break;
 ...
 *
 */
static void FBDEV_H13CMN_FillOSDRegion( int fb_dev_id, UINT32 color )
{
 ...
 *
 */
static void FBDEV_H13CMN_EnableOSDHdrFlush( ULONG data )
{
 ...
 *
 */
void FBDEV_H13CMN_RunBootLogoTimer( UINT32 ms, BOOLEAN bForce )
{
#if (CONFIG_LX_BOOT_LOGO==1)
	...
	if (!bForce)
	{
		FBDEV_H13CMN_FillOSDRegion(CONFIG_LX_BOOT_LOGO_FBDEV, 0xff000000 /* black */ );
	}
	#endif
		OS_InitTimer ( &osd_en_timer );
		OS_StartTimer( &osd_en_timer, FBDEV_H13CMN_EnableOSDHdrFlush, OS_TIMER_TIMEOUT, ms, 0x0 );
	}
#else
	FBDEV_H13CMN_EnableOSDHdrFlush(0x0);
#endif
}
 ...
 *	[OK]
 */
int FBDEV_H13CMN_InitOSDLayer ( int layer )
{
	int ret = RET_ERROR;
    ...
    switch( layer )
    {
        case LX_FBDEV_ID_OSD0:
			...
			FBDEV_WARN("enable OSD(0) to keep UHDOSD(L) ON\n");
            g_fbdev_h13_reg->osd0_ctrl_main.osd0_en 				= 1;			/* enable OSD to keep logo (left OSD in UHD disp) */
#else
            g_fbdev_h13_reg->osd0_ctrl_main.osd0_en 				= 0;			/* disable OSD by default */
#endif
#ifdef FBDEV_USE_HDR_RDY_REG
        ...
        case LX_FBDEV_ID_OSD1:
            ...
            g_fbdev_h13_reg->osd1_ctrl_main.osd1_en 				= 1;			/* keep logo */
#else
			if(g_fbdev_cfg->uhd_disp_mode)
			{
				FBDEV_WARN("enable OSD(1) to keep UHDOSD(R) ON\n");
            	g_fbdev_h13_reg->osd1_ctrl_main.osd1_en 			= 1;			/* enable OSD to keep logo (right OSD in UHD disp) */
			}
			else
			{
	            g_fbdev_h13_reg->osd1_ctrl_main.osd1_en				= 0;			/* disable OSD by default */
			}
#endif
#ifdef FBDEV_USE_HDR_RDY_REG
        ...
        case LX_FBDEV_ID_OSD2:
            g_fbdev_h13_reg->osd2_ctrl_main.osd2_en 				= 0;			/* disable OSD by default */
#ifdef FBDEV_USE_HDR_RDY_REG
        ...
        case LX_FBDEV_ID_CSR0:
            g_fbdev_h13_reg->osd3_ctrl_main.osd3_en 				= 0;			/* disable OSD by default */
#ifdef FBDEV_USE_HDR_RDY_REG
...
static void		FBDEV_H13CMN_ReInitHW		(void)
{
	...
	FBDEV_H13_InitOSDLayer(0);
	FBDEV_H13_InitOSDLayer(1);
	FBDEV_H13_InitOSDLayer(3);
	...
	/* when 2.5K UHD mode, video path is fixed to FHD(1920x1080) */
	if ( FBDEV_H13_CHK_25K_DISP() && FBDEV_H13_CHK_BOSD_PATH() )
	{
		disp_width = 1920;
		disp_height= 1080;
	}

	OSD_H13_RdFL( osd_common_disp_size );
	OSD_H13_Wr02( osd_common_disp_size, reg_common_disp_size_w, disp_width,
										reg_common_disp_size_h, disp_height );

	OSD_H13_WrFL( osd_common_disp_size );
}

/*========================================================================================
    Implementation Group (OSD Hdr Processing)
========================================================================================*/
/** OSD hdr writer for FHD-size OSD mode */
static void	FBDEV_H13CMN_WriteFHDHdrRegs( int fb_dev_id, FBDEV_H13_OSD_HDR_T* osd_hdr )
{
	UINT32* osd_hdr_u32_ptr = (UINT32*)osd_hdr;
	...
	{
		case 0:
			FBDEV_REG_MEMCPY( &g_fbdev_h13_reg->osd0_ctrl_hdr0, osd_hdr_u32_ptr, 7);
#ifdef FBDEV_USE_HDR_RDY_REG
			if ( wctx->status.b_osd_hdr_flush )
			g_fbdev_h13_reg->osd0_ctrl_main.osd0_hdr_ready_flag = 0x1;
#endif
		break;
		case 1:
			FBDEV_REG_MEMCPY( &g_fbdev_h13_reg->osd1_ctrl_hdr0, osd_hdr_u32_ptr, 7);
#ifdef FBDEV_USE_HDR_RDY_REG
			if ( wctx->status.b_osd_hdr_flush )
			g_fbdev_h13_reg->osd1_ctrl_main.osd1_hdr_ready_flag = 0x1;
#endif
		break;
		case 2:
			FBDEV_REG_MEMCPY( &g_fbdev_h13_reg->osd2_ctrl_hdr0, osd_hdr_u32_ptr, 7);
#ifdef FBDEV_USE_HDR_RDY_REG
			if ( wctx->status.b_osd_hdr_flush )
		...
		break;
		case 3:
			FBDEV_REG_MEMCPY( &g_fbdev_h13_reg->osd3_ctrl_hdr0, osd_hdr_u32_ptr, 7);
#ifdef FBDEV_USE_HDR_RDY_REG
			if ( wctx->status.b_osd_hdr_flush )
			g_fbdev_h13_reg->osd3_ctrl_main.osd3_hdr_ready_flag = 0x1;
#endif
...
/** OSD hdr writer for UHD Display mode */
static void	FBDEV_H13CMN_WriteUHDHdrRegs( int fb_dev_id, FBDEV_H13_OSD_HDR_T* osd_hdr )
{
	int	i;
	int	disp_width, disp_height, disp_stride;

	FBDEV_H13_OSD_HDR_T	uhd_osd_hdr[2];

	UINT32	fb_stride	= osd_hdr->osd_hdr_wpl<<3;
	UINT32	fb_bitmap	= osd_hdr->osd_hdr_ptr_bmp;
	...
	/* get display info */
	disp_width = LX_RES_GET_WIDTH(lx_chip_graphic_res());
	disp_stride= LX_RES_GET_WIDTH(lx_chip_graphic_res())<<2;
	disp_height= LX_RES_GET_HEIGHT(lx_chip_graphic_res());

    /* when 2.5K UHD mode, video path is fixed to FHD(1920x1080) */
	if ( FBDEV_H13_CHK_25K_DISP() && FBDEV_H13_CHK_BOSD_PATH() )
	{
		disp_width = 1920;	/* 2K video fixed */
		disp_height= 1080;	/* 2K video fixed */
	}
	 ...
	 */
	FBDEV_CHECK_CODE( !g_fbdev_cfg->uhd_disp_mode, 			return, "invalid call. not UHD mode\n");
	FBDEV_CHECK_CODE( osd_hdr->osd_hdr_w_mem > disp_width,	osd_hdr->osd_hdr_w_mem = disp_width,"in.w  %d > disp %d\n", osd_hdr->osd_hdr_w_mem, disp_width );
	FBDEV_CHECK_CODE( osd_hdr->osd_hdr_w_out > disp_width,	osd_hdr->osd_hdr_w_out = disp_width,"out.w %d > disp %d\n", osd_hdr->osd_hdr_w_out, disp_width );

	/* H13 frame buffer address should be algined by 16byte.
	 * So Left buffer & Right buffer address should be checked before writing OSD header.
	 *
	 * - frame buffer stride should be aligned by 16 byte.
	 * - input width should be multiple of 8 (32 byte aligned for ARGB format) 
	 *   = left & right framebuffers are aligned by 16byte :)
	 */
	FBDEV_CHECK_CODE(  fb_stride % 16,					return, "mis-aligned stride %d (should be 16x)\n", fb_stride );
	FBDEV_CHECK_DEBUG( osd_hdr->osd_hdr_w_mem % 8,		return, "mis-aligned width  %d (should be 8x)\n", osd_hdr->osd_hdr_w_mem );
	FBDEV_CHECK_DEBUG( fb_dev_id != LX_FBDEV_ID_OSD0,	return, "invalid fb(%d) for UHD mode\n", fb_dev_id );
	...
	for (i=0;i<2;i++)
	{
		memcpy( &uhd_osd_hdr[i], osd_hdr, sizeof(FBDEV_H13_OSD_HDR_T));

		uhd_osd_hdr[i].osd_hdr_w_out			= disp_width>>1;				/* 1920 */
		uhd_osd_hdr[i].osd_hdr_h_out			= disp_height;					/* 2160 */
		uhd_osd_hdr[i].osd_hdr_xpos				= (disp_width>>1)*i;			/* 1920*i */
		uhd_osd_hdr[i].osd_hdr_ypos				= 0;
		uhd_osd_hdr[i].osd_hdr_w_mem			= (osd_hdr->osd_hdr_w_mem>>1);	/* 1920 */
		uhd_osd_hdr[i].osd_hdr_h_mem			= osd_hdr->osd_hdr_h_mem;;		/* 2160 */

		uhd_osd_hdr[i].osd_hdr_wpl				= fb_stride>>3;					/* stride/8 */
		uhd_osd_hdr[i].osd_hdr_format			= osd_hdr->osd_hdr_format;		/* 32bpp fixed */
		uhd_osd_hdr[i].osd_hdr_depth			= osd_hdr->osd_hdr_depth;		/* 32bpp fixed */
		uhd_osd_hdr[i].osd_hdr_global_alpha		= 0xff;							/* No gralpha fixed */

		uhd_osd_hdr[i].osd_hdr_ptr_bmp			= fb_bitmap + i*((osd_hdr->osd_hdr_w_mem>>1)<<2);	/* bitmap_ptr + i*7680 (1920*4) */

		FBDEV_DEBUG("UHD HDR[%d] in %d,%d, xy %d,%d, out %d,%d, wpl %d, fmt %d, depth %d, bmp 0x%x\n", i,
			uhd_osd_hdr[i].osd_hdr_w_mem, uhd_osd_hdr[i].osd_hdr_h_mem,
			uhd_osd_hdr[i].osd_hdr_xpos , uhd_osd_hdr[i].osd_hdr_ypos,
			uhd_osd_hdr[i].osd_hdr_w_out, uhd_osd_hdr[i].osd_hdr_h_out,
			uhd_osd_hdr[i].osd_hdr_wpl, uhd_osd_hdr[i].osd_hdr_format, uhd_osd_hdr[i].osd_hdr_depth, uhd_osd_hdr[i].osd_hdr_ptr_bmp );
	}

	/* show POSD1 if it is off state */
	if ( 0x1 == g_fbdev_h13_reg->osd0_ctrl_main.osd0_en && 0x0 == g_fbdev_h13_reg->osd1_ctrl_main.osd1_en )
	{
		g_fbdev_h13_reg->osd1_ctrl_main.osd1_en = 0x1;
	}

	/* set disp size */
	OSD_H13_RdFL( osd_common_disp_size );
   	OSD_H13_Wr02( osd_common_disp_size, reg_common_disp_size_w, disp_width,		/* 3840 */
                                   		reg_common_disp_size_h, disp_height);	/* 2160 */
	OSD_H13_WrFL( osd_common_disp_size );

	/* write osd header */
	FBDEV_H13CMN_WriteFHDHdrRegs(0, &uhd_osd_hdr[0] );
	FBDEV_H13CMN_WriteFHDHdrRegs(1, &uhd_osd_hdr[1] );
}
 ...
 *
 */
void	FBDEV_H13CMN_WriteHdrRegs	( int fb_dev_id, FBDEV_H13_OSD_HDR_T* osd_hdr, FBDEV_H13_OSD_EXHDR_T* osd_exhdr )
{
	ULONG   flags;
	...
	if ( g_reg_io_ctx->hdr_op_method == FBDEV_HDR_OP_MMIO )
	{
		FBDEV_WARN("MMIO writer is OBSOLETED\n");
		memcpy( (void*)hCanvas->osd_hdr_addr, (void*)osd_hdr, sizeof(FBDEV_H13_OSD_HDR_T) );
	}
	else
	{
		if ( g_fbdev_cfg->uhd_disp_mode )
		{
			FBDEV_H13CMN_WriteUHDHdrRegs( fb_dev_id, osd_hdr );
		}
		else
		{
			FBDEV_H13CMN_WriteFHDHdrRegs( fb_dev_id, osd_hdr );
		}
	}

	/* extened osd header for 3DOSD
	 * 3DOSD is used only for FHD model. normally 3DOSD is not processed in H13 SOC in UHD model.
	 */
	if ( lx_chip_plt() == LX_CHIP_PLT_FHD )
	{
		...
		BOOLEAN b_reverse_en= FBDEV_H13CMN_IsReverseOSD(fb_dev_id);

		UINT32	l_bmp_addr	= osd_hdr->osd_hdr_ptr_bmp;
		UINT32	r_bmp_addr	= l_bmp_addr;

		/* raxis.lim (2012/07/12) -- setup disparity
		 * maximum (effective) available disparity is between -0x40 and +0x40 in H13.
		...
		switch( fb_dev_id )
		{
			case 0:
				...
				OSD_H13_Rd02( osd0_ctrl_3d_disparity,   osd0_l_3d_disparity_value,  val0,
														osd0_r_3d_disparity_value,  val1);
				if ( val0 != disp_val_l || val1 != disp_val_r )
				{
					OSD_H13_Wr02( osd0_ctrl_3d_disparity,   osd0_l_3d_disparity_value,  disp_val_l,
															osd0_r_3d_disparity_value,  disp_val_r );
					OSD_H13_WrFL( osd0_ctrl_3d_disparity );
				}

				OSD_H13_RdFL( osd0_ctrl_scaler );
				if ( b_2d3d_en != OSD_H13_Rd00(osd0_ctrl_scaler,osd0_sc_vdouble) )
				{
				...
				if ( b_s3d_en != OSD_H13_Rd00(osd0_ctrl_main,osd0_s3d_en) )
				{
			...
			case 1:
				...
				OSD_H13_Rd02( osd1_ctrl_3d_disparity,	osd1_l_3d_disparity_value,  val0,
														osd1_r_3d_disparity_value,	val1 );
				if ( val0 != disp_val_l || val1 != disp_val_r )
				{
					OSD_H13_Wr02( osd1_ctrl_3d_disparity,	osd1_l_3d_disparity_value,  disp_val_l,
															osd1_r_3d_disparity_value,	disp_val_r );
					OSD_H13_WrFL( osd1_ctrl_3d_disparity );
				}

				OSD_H13_RdFL( osd1_ctrl_scaler );
				if ( b_2d3d_en != OSD_H13_Rd00(osd1_ctrl_scaler, osd1_sc_vdouble) )
				{
				...
				if ( b_s3d_en != OSD_H13_Rd00( osd1_ctrl_main, osd1_s3d_en) )
				{
 ...
 * @param	bool enable : 0 : display off , 1 : display on
 * @return	RET_OK if success, RET_ERROR otherwise.
 */
int FBDEV_H13CMN_SetOSDEnable(int layer, int enable)
{
	 ...
	 * the real composition is done in FBDEV_H13_SetOSDEnable.
	 */
	FBDEV_CHECK_DEBUG( comp_ctrl->comp_opt != FB_COMP_OPT_NONE, return RET_OK, "osd merge enabled.\n");

	switch(layer)
	{
		case LX_FBDEV_ID_OSD0:	g_fbdev_h13_reg->osd0_ctrl_main.osd0_en = val;
								if(g_fbdev_cfg->uhd_disp_mode) g_fbdev_h13_reg->osd1_ctrl_main.osd1_en = val;
								break;
		case LX_FBDEV_ID_OSD1:	g_fbdev_h13_reg->osd1_ctrl_main.osd1_en = val;
								break;
		case LX_FBDEV_ID_OSD2:	g_fbdev_h13_reg->osd2_ctrl_main.osd2_en = val;
								break;
		case LX_FBDEV_ID_CSR0:	g_fbdev_h13_reg->osd3_ctrl_main.osd3_en = val;
								break;
	}
 ...
 *
 */
int FBDEV_H13CMN_GetOSDEnable(int layer)
{
	int ret = 0;
	FBDEV_H13_FB_COMPOSITOR_T* comp_ctrl = &g_fbdev_h13_hw_ctx.comp_ctrl;
...
int FBDEV_H13CMN_SetTridCtrl( int fb_dev_id, LX_FBDEV_TRID_CTRL_T* ctrl )
{
	...
	/* if UHD board & S3D auto conn path is enabled, change OSD path to BLEND if necessary */
#ifdef FBDEV_USE_UHD_S3D_AUTO_CONN_PATH
	FBDEV_DEBUG("3D REQ. mode 0x%x, disp 0x%x\n", ctrl->mode, ctrl->disparity );

	if( lx_chip_plt() != LX_CHIP_PLT_FHD && _g_fbdev_h13_s3d_auto_conn_path )
	{
		...
		FBDEV_H13CMN_SetConnCtrl(&conn_ctrl);
	}
	else
#endif
	{
...
int		FBDEV_H13CMN_SetViewCtrl ( LX_FBDEV_VIEW_CTRL_T* ctrl )
{
    ...
    for (i=0; i<LX_FBDEV_ID_MAX; i++ )
    {
		if ( ctrl->view_status[i] == 0x0 || ctrl->view_status[i] == 0x1 )
			FBDEV_H13CMN_SetOSDEnable( i, ctrl->view_status[i] );
    }
 ...
 *
 */
int     FBDEV_H13CMN_SetConnCtrl           (LX_FBDEV_CONN_CTRL_T* pConnCtrl)
{
	int		i;
	BOOLEAN	osd_view_status[LX_FBDEV_ID_MAX];
    LX_FBDEV_CONN_CTRL_T* conn_ctrl = &g_fbdev_h13_hw_ctx.conn_ctrl;

	FBDEV_WARN("OSD conn change. conn_type 0x%x (%s disp)\n", pConnCtrl->conn_type, (g_fbdev_cfg->uhd_disp_mode)? "UHD":"FHD" );

	FBDEV_CHECK_DEBUG( conn_ctrl->conn_type == pConnCtrl->conn_type, return RET_OK, "ignore the dup calls (0x%x)\n", conn_ctrl->conn_type );

	memcpy(conn_ctrl, pConnCtrl, sizeof(LX_FBDEV_CONN_CTRL_T));

	/* diable OSD before changing OSD config */
	for (i=0; i<LX_FBDEV_ID_MAX; i++ )
	{
		osd_view_status[i] = FBDEV_H13_GetOSDEnable(i);
		FBDEV_H13_SetOSDEnable( i, FALSE );
	}

	/* sleep until OSD is stablized. vsync wait. 33 ms ~ 40 ms */
	usleep_range(33*1000, 40*1000); //_g_fbdev_h13_conn_change_tune_delay);

	if ( pConnCtrl->conn_type & LX_FBDEV_CONN_OSD_VIDEO_SEPARATE )
	{
		/* DPPA clk divider = 0x0 :297MHz (two H-LVDS), 0x1: 148.5MHz (single H-LVDS) */
		UINT32	dppa_clk_divide_val = (FBDEV_H13_CHK_25K_DISP())? 0x0 : 0x1;

		FBDEV_DEBUG("H13::SEPARATE\n");

		OS_WrReg( 0xc003500c, 0x00000004 );			/* DPPA data path ctrl */
		OS_WrReg( 0xc0035010, 0xc2210011 );			/* DPPA sync path ctrl */

		OSD_H13_RdFL( osd_mixer_ctrl_blend );		/* OSD MIXER PATH ctrl */
		OSD_H13_Wr( osd_mixer_ctrl_blend, 0x00170000 );
		OSD_H13_WrFL( osd_mixer_ctrl_blend );

		/* CTOP.CTR24 - clock dividing selection for DPPA */
		CTOP_CTRL_H13B0_RdFL(ctr24);
		CTOP_CTRL_H13B0_Wr01(ctr24, dppa_osd_clk_sel, dppa_clk_divide_val );
		CTOP_CTRL_H13B0_WrFL(ctr24);
		OS_MsecSleep(_g_fbdev_h13_conn_change_tune_delay);

		/* OSD S.OSD HDR disable */
		OSD_H13_RdFL( osd_common_sep_osd_ctrl );	
		OSD_H13_Wr01( osd_common_sep_osd_ctrl, reg_common_sep_osd_hdr_en, 0x1 );
		OSD_H13_WrFL( osd_common_sep_osd_ctrl );
		OS_MsecSleep(_g_fbdev_h13_conn_change_tune_delay);
	}
	else if ( pConnCtrl->conn_type & LX_FBDEV_CONN_OSD_VIDEO_BLEND )
	{
		/* DPPA clk divider = sync with video clock divider */
		UINT32 dppa_clk_divide_val;

		FBDEV_DEBUG("H13::BLEND\n");

		if ( !g_fbdev_cfg->uhd_disp_mode && _g_fbdev_h13_s3d_crsr_sep_path )
		{
			/* do nothing. keep SEP path */
		}
		else
		{
			/* OSD S.OSD HDR disable */
			OSD_H13_RdFL( osd_common_sep_osd_ctrl );	
			OSD_H13_Wr01( osd_common_sep_osd_ctrl, reg_common_sep_osd_hdr_en, 0x0 );
			OSD_H13_WrFL( osd_common_sep_osd_ctrl );
			OS_MsecSleep(_g_fbdev_h13_conn_change_tune_delay);
		}

		OS_WrReg( 0xc003500c, 0x00000000 );			/* DPPA data path ctrl */
		OS_WrReg( 0xc0035010, 0x00000000 );			/* DPPA sync path ctrl */

		OSD_H13_RdFL( osd_mixer_ctrl_blend );		/* OSD MIXER PATH ctrl */

		if ( !g_fbdev_cfg->uhd_disp_mode && _g_fbdev_h13_s3d_crsr_sep_path )
		{
			OSD_H13_Wr( osd_mixer_ctrl_blend, 0x00044040 );	/* cursor fixed to sep path & graphic plane to blend path */
		}
		else
		{
			OSD_H13_Wr( osd_mixer_ctrl_blend, 0x00000000 );	/* all layer to blend path */
		}
		OSD_H13_WrFL( osd_mixer_ctrl_blend );

		/* CTOP.CTR24 - clock dividing selection */
		CTOP_CTRL_H13B0_RdFL(ctr24);
		CTOP_CTRL_H13B0_Rd01(ctr24, dppa_pe1_clk_sel, dppa_clk_divide_val );
		CTOP_CTRL_H13B0_Wr01(ctr24, dppa_osd_clk_sel, dppa_clk_divide_val );
		CTOP_CTRL_H13B0_WrFL(ctr24);
...
func_exit:
	/* restore OSD visibility */
	for (i=0; i<LX_FBDEV_ID_MAX; i++ )
	{
		FBDEV_H13_SetOSDEnable( i, osd_view_status[i] );
	}
 ...
 *
 */
void FBDEV_H13CMN_DoOSDMerge (int fb_dev_id, BOOLEAN b_flip)
{
	int	i;
	...
	FBDEV_H13_OSD_HDR_T osd_s_hdr[LX_FBDEV_ID_MAX];
	int					osd_s_surf[LX_FBDEV_ID_MAX] = { -1, -1, -1, -1 };	/* surface for app POSD */
	FBDEV_WIN_CTX_T* 	wctx[LX_FBDEV_ID_MAX];

	FBDEV_H13_OSD_HDR_T out_fb_hdr;
	FBDEV_H13_OSD_EXHDR_T out_fb_exhdr;
	int 	out_fb_surf= -1;
	...
	for (i=0; i<LX_FBDEV_ID_MAX; i++)
	{
		...
		memcpy( &osd_s_hdr[i], &g_fbdev_h13_hw_ctx.osd_hdr_sav[i], sizeof(FBDEV_H13_OSD_HDR_T));
	}
...
#endif
	memcpy(&out_fb_hdr, &g_fbdev_h13_hw_ctx.osd_hdr_sav[ comp_ctrl->fb_dev_id ], sizeof(FBDEV_H13_OSD_HDR_T));
	memset(&out_fb_hdr, 0x0, sizeof(FBDEV_H13_OSD_EXHDR_T));
...
crsr_render_done:
	...
	FBDEV_H13CMN_WriteHdrRegs	( comp_ctrl->fb_dev_id, &out_fb_hdr, &out_fb_exhdr );
...
========================================================================================*/
int		FBDEV_H13CMN_SetPivotCtrl	( LX_FBDEV_PIVOT_CTRL_T* ctrl )
{
	...
	{
		FBDEV_H13CMN_InitOSDMerge( );

		/* enable only osd merged layer */
		g_fbdev_h13_reg->osd0_ctrl_main.osd0_en = (comp_ctrl->fb_dev_id==0)?0x1:0x0;
		g_fbdev_h13_reg->osd1_ctrl_main.osd1_en = (comp_ctrl->fb_dev_id==1)?0x1:0x0;
		g_fbdev_h13_reg->osd3_ctrl_main.osd3_en = 0;
	}
	else
	{
		/* restore osd on/off status */
		g_fbdev_h13_reg->osd0_ctrl_main.osd0_en = comp_ctrl->osd_view_status[0] ? 0x1: 0x0;
		g_fbdev_h13_reg->osd1_ctrl_main.osd1_en = comp_ctrl->osd_view_status[1] ? 0x1: 0x0;
		g_fbdev_h13_reg->osd3_ctrl_main.osd3_en = comp_ctrl->osd_view_status[3] ? 0x1: 0x0;
	}
...
void	FBDEV_H13CMN_HandleCLUT ( int fb_dev_id, struct scanvas* hCanvas, FBDEV_H13_OSD_HDR_T* osd_hdr )
{
	...
	if ( hCanvas->bits_per_pixel == 8 )
	{
		...
		/* convert clut to 32bpp surface */
		{
			...
			/* UHD path, osd layers uses two POSD to make full screen */
			if(g_fbdev_cfg->uhd_disp_mode)
			{
				FBDEV_HW_DEBUG("UHD mode. 8to32 scale (%dx%d) -> (%dx%d)\n", hCanvas->input_win.w, hCanvas->input_win.h, FBDEV_H13_CLUT_VOSD_WIDTH, FBDEV_H13_CLUT_VOSD_HEIGHT );
				...
				hCanvas->input_win.w = FBDEV_H13_CLUT_VOSD_WIDTH;

				hCanvas->input_win.y = (hCanvas->input_win.y*FBDEV_H13_CLUT_VOSD_HEIGHT)/fb_height;
				hCanvas->input_win.h = FBDEV_H13_CLUT_VOSD_HEIGHT;

			}
			else /* normal path, each layer uses single POSD */
			{
				...
				{
					FBDEV_HW_DEBUG("FHD mode. 8to32 blit (%dx%d)\n", hCanvas->input_win.w, hCanvas->input_win.h );
				...
				{
					FBDEV_HW_DEBUG("FHD mode. 8to32 scale (%dx%d) -> (%dx%d)\n", hCanvas->input_win.w, hCanvas->input_win.h, FBDEV_H13_CLUT_VOSD_WIDTH, FBDEV_H13_CLUT_VOSD_HEIGHT );
					GFX_StretchSurface( posd_surf, vosd_surf, 0, 0, hCanvas->input_win.w, hCanvas->input_win.h, 0, 0, FBDEV_H13_CLUT_VOSD_WIDTH, FBDEV_H13_CLUT_VOSD_HEIGHT );
...
void	FBDEV_H13CMN_HandleVOSD ( int fb_dev_id, struct scanvas* hCanvas, FBDEV_H13_OSD_HDR_T* osd_hdr )
{
	...
	if ( hCanvas->blend_alpha_en && hCanvas->blend_alpha < 0xff )
		opmask |= FBDEV_H13_VOSD_OPMASK_BLEND_ALPHA;
	...
	/* make virtual OSD screen */
	{
		...
		{
			case FBDEV_H13_VOSD_OPMASK_BLEND_ALPHA:
			{
			...
			case FBDEV_H13_VOSD_OPMASK_DOWN_SCALE|FBDEV_H13_VOSD_OPMASK_BLEND_ALPHA:
			default:
			{
#if 0
				/* accoding to down-scale test, ther seems 2 line screen garbage at the right side in vertical lines.
...
{
	FBDEV_CHECK_DEBUG( lx_chip_plt() != LX_CHIP_PLT_FHD, return, "fb(%d) H13:UHD_OSD - ignore 3D handler\n", fb_dev_id );
...
void	FBDEV_H13CMN_HandleReverseOSD ( int fb_dev_id, struct scanvas* hCanvas, FBDEV_H13_OSD_HDR_T* osd_hdr )
{
	 ...
	 */
	b_reverse_en = FBDEV_H13CMN_IsReverseOSD(fb_dev_id);
	...
	switch ( fb_dev_id )
	{
		case 0:
			OSD_H13_RdFL( osd0_ctrl_scaler );
				OSD_H13_Rd01( osd0_ctrl_scaler, osd0_sc_v_phase_off, v_phase_off );

				if ( v_phase_off != _g_fbdev_h13_v_phase_offset )
				{
		...
		case 1:
			OSD_H13_RdFL( osd1_ctrl_scaler );
				OSD_H13_Rd01( osd1_ctrl_scaler, osd1_sc_v_phase_off, v_phase_off );
...
void     FBDEV_H13CMN_HandleISR ( void )
{
	...
	{
		if ( g_fbdev_h13_hw_ctx.comp_ctrl.my_vsync >= 0 && g_fbdev_h13_hw_ctx.comp_ctrl.my_vsync != g_fbdev_vsync_cnt )
		{
 ...
 *
 */
int	FBDEV_H13CMN_RunResume	( void )
{
	FBDEV_H13CMN_ReInitHW();

	FBDEV_H13CMN_RunBootLogoTimer( 1500, TRUE );

	return RET_OK;
}

int	FBDEV_H13CMN_RunSuspend	( void )
{
	/* do nothing */
	return RET_OK;
}
