...
#include "de_kapi.h"
extern int DE_REG_M14_GetSourceStatus(LX_DE_SRC_STATUS_T *pstParams);
#endif
...
#define GFX_WRITE_REG(reg,val)	\
    __raw_writel(val , (volatile void*)((ULONG)(g_gfx_reg_m14) + (ULONG)OFFSET(GFX_M14_REG_T,reg)))

#define GFX_READ_REG(reg)	\
    __raw_readl( (volatile void*)((ULONG)(g_gfx_reg_m14) + (ULONG)OFFSET(GFX_M14_REG_T,reg)))

//#define SHOW_GFX_REGISTER //only debugging
#if defined(SHOW_GFX_REGISTER)

#define PRINT_READ_REG(reg)	\
   do { printk("GFX REG 0x%x -> 0x%0.8x\n",((UINT32)OFFSET(GFX_M14_REG_T,reg)), *((UINT32 *)(&g_gfx_reg_m14_cache->reg)) ); } while(0);
...
GFX_M14_SCALE_VPHASE_DATA_T;

typedef struct
{
#define	GFX_SCALE_PHASE_DATA_NUM	16
	GFX_M14_SCALE_HPHASE_DATA_T		hphase[GFX_SCALE_PHASE_DATA_NUM];		/* hhpase data */
	GFX_M14_SCALE_VPHASE_DATA_T		vphase[GFX_SCALE_PHASE_DATA_NUM];		/* vhpase data */
}
GFX_M14_SCALE_FILTER_DATA_T;
...
/*----------------------------------------------------------------------------------------
	External Variables
----------------------------------------------------------------------------------------*/
extern volatile GFX_M14_REG_T	*g_gfx_reg_m14;
extern GFX_M14_REG_T 			*g_gfx_reg_m14_cache;

/*----------------------------------------------------------------------------------------
	global Variables
----------------------------------------------------------------------------------------*/
volatile UINT32 g_ui_ack_mode_m14 = 0;

#ifdef	KDRV_CONFIG_PM
unsigned int g_pm_cmd_delay_m14	= 0;
unsigned int g_pm_intr_ctrl_m14	= 0;
#endif
...
/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
static int	_g_gfx_m14_bw_val			= 0x7ff;
static int	_g_gfx_m14_bw_val_req		= 0x7ff;
module_param_named( gfx_m14_bw_val, _g_gfx_m14_bw_val_req, int, 0644 );

#define	GFX_M14_CUSTOM_FILTER_MAX	4

static int _g_gfx_m14_scaler_index = -1;
module_param_named( gfx_m14_scaler_index, _g_gfx_m14_scaler_index, int, 0644 );

static GFX_M14_SCALE_FILTER_DATA_T 	g_gfx_m14_custom_scale_filter_data[GFX_M14_CUSTOM_FILTER_MAX] = 
{
 ...
 *
 */
static int		GFX_M14_GetBestHVScalerIndex ( GFX_SCALER_CONFIGURE_T* scaler )
{
	int	h_scale_ratio = 0;
	int	v_scale_ratio = 0;

	if ( _g_gfx_m14_scaler_index >= 0 ) return _g_gfx_m14_scaler_index;
 ...
 *
 */
void	GFX_M14_InitScaler(void)
{
	int	i,j;
	GFX_M14_SCALE_FILTER_DATA_T*	filter_data;

	GFX_WRITE_REG( scaler_frame_start, 0x1 );					/* scaler frame start (reset) */

	for ( j=0; j< GFX_M14_CUSTOM_FILTER_MAX; j++ )				/* download custome filter data */
	{
		filter_data = &(g_gfx_m14_custom_scale_filter_data[j]);	

		for ( i=0; i< GFX_SCALE_PHASE_DATA_NUM; i++ )
		{
			g_gfx_reg_m14->scaler_hphase_data0 = filter_data->hphase[i].data0;
			g_gfx_reg_m14->scaler_hphase_data1 = filter_data->hphase[i].data1;
			g_gfx_reg_m14->scaler_hphase_data2 = filter_data->hphase[i].data2;
			g_gfx_reg_m14->scaler_hphase_addr  = (j<<4) | filter_data->hphase[i].addr;
		}

		for ( i=0; i< GFX_SCALE_PHASE_DATA_NUM; i++ )
		{
			g_gfx_reg_m14->scaler_vphase_data0 = filter_data->vphase[i].data0;
			g_gfx_reg_m14->scaler_vphase_data1 = filter_data->vphase[i].data1;
			g_gfx_reg_m14->scaler_vphase_addr  = (j<<4) | filter_data->vphase[i].addr;
		}
	}
}
 ...
 *
 */
int GFX_M14_RunFlushCommand(void)
{
	GFX_TRACE_BEGIN();
 ...
 *
 */
int GFX_M14_RunStartCommand(void)
{
	GFX_TRACE_BEGIN();

	/* write B/W limit if changed */
	if ( _g_gfx_m14_bw_val != _g_gfx_m14_bw_val_req	)
	{
		_g_gfx_m14_bw_val_req &= 0x7ff;

		GFX_WARN("+++ bw_limit changed 0x%x -> 0x%x\n", _g_gfx_m14_bw_val, _g_gfx_m14_bw_val_req );
		_g_gfx_m14_bw_val = _g_gfx_m14_bw_val_req;
		OS_WrReg( 0xc000e4f8, _g_gfx_m14_bw_val );
	}
 ...
 *
 */
int GFX_M14_PauseCommand(void)
{
	GFX_TRACE_BEGIN();
	g_gfx_reg_m14->gfx_reset.gfx_reset				= 0x0;
	g_gfx_reg_m14->gfx_batch_run.gfx_batch_run		= 0x0;
	g_gfx_reg_m14->gfx_start.gfx_start				= 0x0;
	g_gfx_reg_m14->gfx_pause.gfx_pause				= 0x1;
	GFX_TRACE_END();
 ...
 *
 */
int GFX_M14_ResumeCommand(void)
{
	GFX_TRACE_BEGIN();
	g_gfx_reg_m14->gfx_reset.gfx_reset				= 0x0;
	g_gfx_reg_m14->gfx_batch_run.gfx_batch_run		= 0x0;
	g_gfx_reg_m14->gfx_start.gfx_start				= 0x0;
	g_gfx_reg_m14->gfx_pause.gfx_pause				= 0x2;
	GFX_TRACE_END();
 ...
 */
int GFX_M14_SwResetCommand(void)
{
 ...
 *
	 */
int GFX_M14_StopCommand(void)
{
	return GFX_M14_SwResetCommand();
}

/** get command queue status 
 *
 */
void GFX_M14_GetComQueueStatus(GFX_CMD_QUEUE_CTRL_T *queue)
{
	GFX_TRACE_BEGIN();
 ...
 */
BOOLEAN GFX_M14_IsGfxIdle(void)
{
    BOOLEAN ret;
	...
	ret = ( g_gfx_reg_m14_cache->gfx_status0.exec_status	== 0 &&
			g_gfx_reg_m14_cache->gfx_status1.cmd_que_status== 0 )? TRUE : FALSE;
//   ret = ( g_gfx_reg_m14->status0.bwCmdStatus == 0 &&
//          g_gfx_reg_m14->status0.abwProcessingLine == 0 &&
//          g_gfx_reg_m14->status1.bwCmdQueueStatus == 0 )? TRUE : FALSE ;

//	printk("GFX IDLE = (%s) %d-%d-%d\n", (ret)? "IDLE":"BUSY", 
//			g_gfx_reg_m14_cache->gfx_status0.exec_status, g_gfx_reg_m14_cache->gfx_status0.exec_line, g_gfx_reg_m14_cache->gfx_status1.cmd_que_status );
 ...
 *
 */
void GFX_M14_GetOperationStatus(GFX_CMD_OP_T *operate)
{
	GFX_TRACE_BEGIN();
	...
	operate->bChromaFilterEnable = 0 ; //g_gfx_reg_m14->op_mode.bwEnChromaFilter;
 ...
 *
 *	@note operation status should be read at GFX_M14_GetOperationStatus
 */
void GFX_M14_SetOperationStatus(GFX_CMD_OP_T *operate)
{
	GFX_TRACE_BEGIN();
 ...
 *
 */
void GFX_M14_GetInputConfigure(int iPort, GFX_PORT_CONFIGURE_T *port)
{
	GFX_TRACE_BEGIN();
...
void GFX_M14_SetInputConfigure(GFX_PORT_CONFIGURE_T *port)
{
	GFX_TRACE_BEGIN();
 ...
 *
 */
void GFX_M14_GetBlendConfigure( GFX_BLEND_CONFIGURE_T *blend)
{
	UINT32	color_a;
	UINT32	color_c;
...
//	blend->ulBlendConstant	= g_gfx_reg_m14->out_const_ctrl.abwConstantCtrl;
	GFX_Rd01( rop_ctrl,		rop_ctrl,		blend->sRaster );
//	blend->sRaster			= g_gfx_reg_m14->out_rop_ctrl.abwRasterSel;
 ...
 *
 *	@note out_sel should be read before this function at GFX_M14_GetBlendConfigure
 */
void GFX_M14_SetBlendingOut(GFX_ENUM_OUT_T type)
{
	GFX_TRACE_BEGIN();
 ...
 *
 * 	@note all data should be read before this function at GFX_M14_GetBlendConfigure
 */
void GFX_M14_SetBlendConfigure( GFX_BLEND_CONFIGURE_T *blend)
{
	UINT32	color_a, color_c;
	GFX_TRACE_BEGIN();
	
	GFX_Wr01( out_sel, 	out_sel,	blend->sOut );
//	blend->sOut 		= g_gfx_reg_m14->out_sel.abwOutSel;
...
void GFX_M14_GetOutputConfigure(GFX_OUT_CONFIGURE_T *port)
{
	GFX_TRACE_BEGIN();
...
void GFX_M14_SetOutputConfigure(GFX_OUT_CONFIGURE_T *port)
{
	GFX_TRACE_BEGIN();
...
static int GFX_M14_SelectScalerOrder( GFX_SCALER_CONFIGURE_T* s )
{
	int	vh_order;
...
void GFX_M14_SetScalerConfigure(GFX_SCALER_CONFIGURE_T* scaler)
{
	switch( scaler->cmd )
	{
		...
		{
			int scaler_set_idx = GFX_M14_GetBestHVScalerIndex( scaler );
			int	vh_order= GFX_M14_SelectScalerOrder( scaler );
#if 1
...
#else
			g_gfx_reg_m14->scaler_line_init.scaler_line_init = 0x1;	/* init scaler line */
			g_gfx_reg_m14->scaler_line_init.scaler_line_init = 0x0;
#endif
		...
		default:
		{
		    g_gfx_reg_m14->scaler_bypass.scaler_bypass = 0x1;
		}
		break;
	}
}

void GFX_M14_SetInterruptMode(UINT32 uiMode)
{
	/* do nothing */
}

/* what is it ? */
void GFX_M14_SetAckMode(UINT32 *mode)
{
	GFX_TRACE_BEGIN();
	g_ui_ack_mode_m14 = *mode;
	g_ui_ack_mode_m14 = 1;
	GFX_TRACE_END();
}

/* what is it ? */
void GFX_M14_GetAckMode(UINT32 *mode)
{
	*mode = g_ui_ack_mode_m14;
}

int GFX_M14_SetCLUT(int port, int size , UINT32 *data)
{
	int i , ret = 0;
	...
	switch(port)
	{
        case 0:
        {
            g_gfx_reg_m14->r0_clut_ctrl.clut0_addr	= 0;
            g_gfx_reg_m14->r0_clut_ctrl.clut0_rw	= 0;
#ifdef GFX_PALETTE_AUTO_INC
            g_gfx_reg_m14->r0_clut_ctrl.clut0_wai	= 1;
#else
            g_gfx_reg_m14->r0_clut_ctrl.clut0_wai	= 0;
#endif
            for(i=0 ; i<size ; i++)
            {
#ifdef GFX_PALETTE_AUTO_INC
...
#else
                g_gfx_reg_m14->r0_clut_ctrl.clut0_addr = i;
#endif
                g_gfx_reg_m14->r0_clut_data			= *data++;
            }
        }
        break;
        ...
        case 1:
        {
            g_gfx_reg_m14->r1_clut_ctrl.clut1_addr	= 0;
            g_gfx_reg_m14->r1_clut_ctrl.clut1_rw	= 0;
#ifdef GFX_PALETTE_AUTO_INC
            g_gfx_reg_m14->r1_clut_ctrl.clut1_wai	= 1;
#else
            g_gfx_reg_m14->r1_clut_ctrl.clut1_wai	= 0;
#endif
            for(i=0 ; i<size ; i++)
            {
#ifdef GFX_PALETTE_AUTO_INC
...
#else
                g_gfx_reg_m14->r1_clut_ctrl.clut1_addr = i;
#endif
                g_gfx_reg_m14->r1_clut_data			= *data++;
			}
        }
        break;
...
static UINT16	GFX_M14_CalcAutoCommandDelay(void)
{
#ifdef GFX_AUTO_CMD_DELAY
	int	rc;
	...
	rc = DE_REG_M14_GetSourceStatus(&de_src_param);
 ...
 *
 */
void GFX_M14_SetCommandDelayReg(UINT16 delay)
{
	UINT32	curr_delay;
	...
	if ( delay == (UINT16)-1 )
	{
		delay = GFX_M14_CalcAutoCommandDelay();
	}
...
*/
UINT16 GFX_M14_GetCommandDelayReg(void)
{
	UINT16	val;
	GFX_TRACE_BEGIN();
	val = g_gfx_reg_m14->gfx_cmd_dly.gfx_cmd_dly;
	GFX_TRACE_END();
	return val;
}
 ...
 */
void GFX_M14_DumpRegister(void)
{
    int i;
    UINT32* reg_ptr = (UINT32*)g_gfx_reg_m14;
...
int GFX_M14_RunSuspend(void)
{
	/* do nothing */
	return RET_OK;
}

int GFX_M14_RunResume(void)
{
	GFX_InitHW( );
	return RET_OK;
}

int	GFX_M14_RunClkOnCommand(void)
{
	return RET_OK;
}

int GFX_M14_RunClkOffCommand(void)
{
	return RET_OK;
}

BOOLEAN GFX_M14_ClkCheck(void)
{
	return TRUE;
}
