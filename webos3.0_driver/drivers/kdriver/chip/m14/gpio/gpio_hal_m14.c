...
#include "../../chip/m14/os/linux_irqs.h"
...
----------------------------------------------------------------------------------------*/
#ifdef INCLUDE_M14_CHIP_KDRV
UINT32 		gpio_intr_num_m14[GPIO_IRQ_NUM_NR] = {M14_IRQ_GPIO0_2,M14_IRQ_GPIO3_5,M14_IRQ_GPIO6_8,M14_IRQ_GPIO9_11,M14_IRQ_GPIO12_14,M14_IRQ_GPIO15_17};
...
========================================================================================*/
 ...
 void _GPIO_ISR_GPIO_M14( unsigned int ui32IRQNum, unsigned int IntData )
{
	UINT32	gpio_array_n;
	...
	GPIONIC(gpio_array_n) =ui32Status; // write only
		...
		if ( ui32Status & ( 1 << ucGpio ) )
		{
			...
			if ( GPIONIE(gpio_array_n) & ( 1 << ucGpio ) )
			{
				...
				if (gpio_isr_info[realGpio].count_enable == 0)
				{
					...
					GPIO_DEBUG("_GPIO_ISR_GPIO_M14		pin[%d] value[%d]\n",realGpio, value);
