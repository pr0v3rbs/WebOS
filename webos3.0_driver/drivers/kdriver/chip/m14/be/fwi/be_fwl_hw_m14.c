...
#include "ipc_drv.h"
#include "be_fwl_hw_m14.h"
#if CONFIG_LX_M14_CHIP_FIRMWARE
#include "frc_fw_m14a0.h"
#include "sre_fw_m14a0.h"
#include "wcg_fw_m14a0.h"
#include "led_fw_m14a0.h"
#include "hdr_fw_m14a0.h"

#include "hdr_fw_m14b0.h"
#endif
...
/*----------------------------------------------------------------------------------------
	global Variables
----------------------------------------------------------------------------------------*/
BE_FWL_HAL_FW_T gHeader_Firmware_M14A0[BE_FW_DWLD_TYPE_MAXNUM] = 
	{
#if CONFIG_LX_M14_CHIP_FIRMWARE
		{ BE_FW_DWLD_TYPE_FRC, sizeof(ui32FrcFirmware_M14A0), (char *)ui32FrcFirmware_M14A0, TRUE },
		{ BE_FW_DWLD_TYPE_SRE, sizeof(ui32SreFirmware_M14A0), (char *)ui32SreFirmware_M14A0, FALSE },
		{ BE_FW_DWLD_TYPE_WCG, sizeof(ui32WcgFirmware_M14B0), (char *)ui32WcgFirmware_M14B0, FALSE },
		{ BE_FW_DWLD_TYPE_LED, sizeof(ui32LedFirmware_M14B0), (char *)ui32LedFirmware_M14B0, FALSE },
		{ BE_FW_DWLD_TYPE_HDR, sizeof(ui32HdrFirmware_M14A0), (char *)ui32HdrFirmware_M14A0, FALSE }
#else
...
BE_FWL_HAL_FW_T gHeader_Firmware_M14B0[BE_FW_DWLD_TYPE_MAXNUM] =
	{
#if CONFIG_LX_M14_CHIP_FIRMWARE
		{ BE_FW_DWLD_TYPE_FRC, 0, NULL, TRUE },	
		{ BE_FW_DWLD_TYPE_SRE, 0, NULL, FALSE },
		{ BE_FW_DWLD_TYPE_WCG, sizeof(ui32WcgFirmware_M14B0), (char *)ui32WcgFirmware_M14B0, FALSE },
		{ BE_FW_DWLD_TYPE_LED, sizeof(ui32LedFirmware_M14B0), (char *)ui32LedFirmware_M14B0, FALSE },
		{ BE_FW_DWLD_TYPE_HDR, sizeof(ui32HdrFirmware_M14B0), (char *)ui32HdrFirmware_M14B0, FALSE }
#else
		{ BE_FW_DWLD_TYPE_FRC, 0, NULL, TRUE },
		{ BE_FW_DWLD_TYPE_SRE, 0, NULL, FALSE },
		{ BE_FW_DWLD_TYPE_WCG, 0, NULL, FALSE },
...
========================================================================================*/
BE_FWL_HAL_FW_T *BE_FWL_HW_M14_GetFwHeaderInfo(UINT8 ucIndex)
{
	...
	{
		return &gHeader_Firmware_M14B0[ucIndex];
	}
	else
	{
		return &gHeader_Firmware_M14A0[ucIndex];
	}		
}
 ...
 */
UINT32 BE_FWL_HW_M14_FW_Download(UINT8 ucIndex, UINT8 *pucBuff, UINT32 ulSize, BOOLEAN bIsMcuFw)
{
	int ret = RET_OK;
	...
	do {
		...
		if(bIsMcuFw && (lx_chip_rev() < LX_CHIP_REV( M14, B0)))
		{
			BOOLEAN         run_stall = 0;
			
			DPPB_TOP_M14_RdFL(top_runstall);
			DPPB_TOP_M14_Rd01(top_runstall, runstall, run_stall);
			if(!run_stall) break;
		}
		...
		if(bIsMcuFw && (lx_chip_rev() < LX_CHIP_REV( M14, B0)))
		{
			DPPB_TOP_M14_RdFL(top_tr_addr_offset_0);
			DPPB_TOP_M14_Wr01(top_tr_addr_offset_0, tr_addr_offset_0, pFwMem->fw_base);
			DPPB_TOP_M14_WrFL(top_tr_addr_offset_0);
			
			msleep(10); // wait ddr to ddr transition

			DPPB_TOP_M14_Wr01(top_runstall, runstall, 0);
			DPPB_TOP_M14_WrFL(top_runstall);
 ...
 */
BOOLEAN BE_FWL_HW_M14_MCUisAlive(void)
{
	...
	else
	{
		...
		DPPB_TOP_M14_FLWr(top_pdebugenable, TRUE);
		DPPB_TOP_M14_FLRd(top_pdebugpc, pc);
		if(pc >= 0x40000600)
		{
			isFwRun = TRUE;
		}
		DPPB_TOP_M14_FLWr(top_pdebugenable, FALSE);
	}
 ...
 */
UINT32 BE_FWL_HW_M14_GetFwMemBaseSize(UINT8 ucIndex)
{
	int ret = RET_OK;
...
int BE_FWL_HW_M14_FrcFwInit(void)
{
	int ret = RET_OK;
...
int BE_FWL_HW_M14_SreFwInit(UINT32 u32FwBaseAddr)
{
	int ret = RET_OK;
...
int BE_FWL_HW_M14_WcgFwInit(UINT32 u32FwBaseAddr)
{
	int ret = RET_OK;
...
int BE_FWL_HW_M14_LedFwInit(UINT32 u32FwBaseAddr)
{
	int ret = RET_OK;
...
int BE_FWL_HW_M14_HdrFwInit(UINT32 u32FwBaseAddr)
{
	int ret = RET_OK;
	...
	do{
		if (lx_chip_rev() >= LX_CHIP_REV( M14, B0))
		{
			BE_IPC_B0_HDR_LOAD_LUT_T fwHdrLoadLutParam;

			fwHdrLoadLutParam.bHDRLutLoad = TRUE;
			ret = BE_KIPC_SetData(DPPB_PACK_CMD(BE_HDR_B0_LOAD_LUT), &fwHdrLoadLutParam, sizeof(BE_IPC_B0_HDR_LOAD_LUT_T));
			if(ret) BREAK_WRONG(ret);
		}
		else
		{
