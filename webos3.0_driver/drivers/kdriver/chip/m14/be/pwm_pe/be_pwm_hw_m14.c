...
#include "be_def.h"
#include "be_pwm_hw_m14.h"

#include "../reg/be_reg_frc_m14.h"
...
/*----------------------------------------------------------------------------------------
	External Variables
----------------------------------------------------------------------------------------*/
extern FRC_DVO_REG_M14_T gFRC_DVO_M14;
...
static BOOLEAN gUseClkFor120HzDisp = TRUE;
...
int BE_PWM_HW_M14_Init(void)
{
	...
	do {
		...
		if (lx_chip_rev() >= LX_CHIP_REV( M14, B0))
		{
			...
			CTOP_CTRL_M14B0_RdFL(FDD, ctr00);
			regField1 = CTOP_CTRL_M14B0_RdFd(FDD, ctr00, frc_ddo_sel);
			CTOP_CTRL_M14B0_RdFL(LEFT, ctr92);
			regField2 = CTOP_CTRL_M14B0_RdFd(LEFT, ctr92, disp_od_fout_ctrl);

			if ((regField1 == 0x1 && regField2 == 0x1) || (regField1 == 0x0 && regField2 == 0x2)) // pixel clk 148.5MHz
			{
				gUseClkFor120HzDisp = FALSE;
			}
		}
...
int BE_PWM_HW_M14_SetPwmControl(BE_PWM_CTRL_T *pstParams)
{
	int ret = RET_OK;
...
int BE_PWM_HW_M14_SetPwmAdaptFreqControl(BE_PWM_ADAPT_FREQ_CTRL_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		if (freqSet)
		{
			...
			ret = BE_PWM_HW_M14_SetPwmSync(&stParams);
...
int BE_PWM_HW_M14_SetPwmFrequency(BE_PWM_FREQ_T *pstParams)
{
	int ret = RET_OK;
	BE_PWM_ID_T nPort;
	UINT32 u32Frequency;
	...
	do {
		...
		ret = PWM_SetFrequency(nPort, u32Frequency);
		if (ret) BREAK_WRONG(ret);
		...
		if((lx_chip_rev() < LX_CHIP_REV( M14, B0))
			&& gPwmInfo[nPort].ctrl.pwm_en && gPwmInfo[nPort].ctrl.pwm_method
			&& (u32PrevFrequency != pstParams->frequency))
		{
			switch(nPort)
			{
				case BE_PWM0 :
					FRC_DVO_M14_RdFL(dvo_pwm0_ctrl0);
					FRC_DVO_M14_Wr01(dvo_pwm0_ctrl0, pwm0_en, 0);
					FRC_DVO_M14_WrFL(dvo_pwm0_ctrl0);

					FRC_DVO_M14_RdFL(dvo_pwm0_ctrl0);
					FRC_DVO_M14_Wr01(dvo_pwm0_ctrl0, pwm0_en, gPwmInfo[nPort].ctrl.pwm_en);
					FRC_DVO_M14_WrFL(dvo_pwm0_ctrl0);
					break;

				case BE_PWM1 :
					FRC_DVO_M14_RdFL(dvo_pwm1_ctrl0);
					FRC_DVO_M14_Wr01(dvo_pwm1_ctrl0, pwm1_en, 0);
					FRC_DVO_M14_WrFL(dvo_pwm1_ctrl0);

					FRC_DVO_M14_RdFL(dvo_pwm1_ctrl0);
					FRC_DVO_M14_Wr01(dvo_pwm1_ctrl0, pwm1_en, gPwmInfo[nPort].ctrl.pwm_en);
					FRC_DVO_M14_WrFL(dvo_pwm1_ctrl0);
					break;

				case BE_PWM2 :
					FRC_DVO_M14_RdFL(dvo_pwm2_ctrl0);
					FRC_DVO_M14_Wr01(dvo_pwm2_ctrl0, pwm2_en, 0);
					FRC_DVO_M14_WrFL(dvo_pwm2_ctrl0);

					FRC_DVO_M14_RdFL(dvo_pwm2_ctrl0);
					FRC_DVO_M14_Wr01(dvo_pwm2_ctrl0, pwm2_en, gPwmInfo[nPort].ctrl.pwm_en);
					FRC_DVO_M14_WrFL(dvo_pwm2_ctrl0);
					break;
...
int BE_PWM_HW_M14_SetPwmDutyCycle(BE_PWM_DUTY_T *pstParams)
{
	int ret = RET_OK;
...
int BE_PWM_HW_M14_GetPwmInfo(BE_EXTERNAL_PWM_T *pstParams)
{
	int ret = RET_OK;
	UINT32 freq_mode = 0;
	UINT32 time_low[4] = { 0 };
...
int BE_PWM_HW_M14_SetPwmSync(BE_PWM_FREQ_T *pstParams)
{
	int ret = RET_OK;
	UINT32 pwmFreq;
	UINT32 prevPwmfreq;
...
static int PWM_SetFrequency(UINT32 pwmId, UINT32 freq)
{
	...
	if( gPwmInfo[pwmId].ctrl.pwm_method == PWM_FREE_OFF_S ) // locking mode
	{
		...
		if (gUseClkFor120HzDisp) //For temp
		{
			freq = freq/2;
		}
		...
		switch ( freq )
		{
			...
			default :
				gPwmInfo[pwmId].ordr				  = PWM_ORD_1ST_S;
				gPwmInfo[pwmId].pxls				  = 2475000 * 60 / freq;
				gPwmInfo[pwmId].ctrl.pwm_free_width= 2475000 * 60 / freq;
				gPwmInfo[pwmId].ctrl.pwm_method	  = TRUE;
				gPwmInfo[pwmId].ctrl.pwm_freq_mode = 0;
				break;
		}
	...
	else	// freeeun mode
	{
		...
		if (gUseClkFor120HzDisp) //For temp
		{
			gPwmInfo[pwmId].pxls				   = 2475000 * 120 / freq;
			gPwmInfo[pwmId].ctrl.pwm_free_width = 2475000 * 120 / freq;
		}
		else
		{
			gPwmInfo[pwmId].pxls				   = 2475000 * 60 / freq;
...
static int _REG_SetPwmCtrl(UINT32 pwmId, BOOLEAN zeroDuty)
{
	...
	switch (pwmId) {
		case 0 :
			FRC_DVO_M14_RdFL(dvo_pwm0_ctrl0);
			FRC_DVO_M14_RdFL(dvo_pwm0_ctrl1);
			FRC_DVO_M14_RdFL(dvo_pwm0_ctrl2);
			FRC_DVO_M14_Wr01(dvo_pwm0_ctrl0, pwm0_en, gPwmInfo[pwmId].ctrl.pwm_en);
			FRC_DVO_M14_Wr01(dvo_pwm0_ctrl0, pwm0_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
			FRC_DVO_M14_Wr01(dvo_pwm0_ctrl0, pwm0_resolution, gPwmInfo[pwmId].ctrl.pwm_resolution);
			FRC_DVO_M14_Wr01(dvo_pwm0_ctrl0, pwm0_inv, gPwmInfo[pwmId].ctrl.pwm_inv);
			FRC_DVO_M14_Wr01(dvo_pwm0_ctrl0, pwm0_sel, gPwmInfo[pwmId].ctrl.pwm_sel);
			FRC_DVO_M14_Wr01(dvo_pwm0_ctrl0, pwm0_width_falling_pos, gPwmInfo[pwmId].ctrl.pwm_width_falling_pos);
			FRC_DVO_M14_Wr01(dvo_pwm0_ctrl1, pwm0_free_width, gPwmInfo[pwmId].ctrl.pwm_free_width);
			FRC_DVO_M14_Wr01(dvo_pwm0_ctrl2, pwm0_method, gPwmInfo[pwmId].ctrl.pwm_method);
			FRC_DVO_M14_Wr01(dvo_pwm0_ctrl2, pwm0_fc_h_disp, gPwmInfo[pwmId].ctrl.pwm_fc_h_disp);
			if(zeroDuty && (!gPwmInfo[pwmId].ctrl.pwm_fc_h_disp))
			{
				FRC_DVO_M14_Wr01(dvo_pwm0_ctrl2, pwm0_fc_l_disp, 1);
			}
			else
			{
				FRC_DVO_M14_Wr01(dvo_pwm0_ctrl2, pwm0_fc_l_disp, gPwmInfo[pwmId].ctrl.pwm_fc_l_disp);
			}
			FRC_DVO_M14_WrFL(dvo_pwm0_ctrl0);
			FRC_DVO_M14_WrFL(dvo_pwm0_ctrl1);
			FRC_DVO_M14_WrFL(dvo_pwm0_ctrl2);
			break;
		case 1 :
			FRC_DVO_M14_RdFL(dvo_pwm1_ctrl0);
			FRC_DVO_M14_RdFL(dvo_pwm1_ctrl1);
			FRC_DVO_M14_RdFL(dvo_pwm1_ctrl2);
			FRC_DVO_M14_Wr01(dvo_pwm1_ctrl0, pwm1_en, gPwmInfo[pwmId].ctrl.pwm_en);
			FRC_DVO_M14_Wr01(dvo_pwm1_ctrl0, pwm1_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
			FRC_DVO_M14_Wr01(dvo_pwm1_ctrl0, pwm1_resolution, gPwmInfo[pwmId].ctrl.pwm_resolution);
			FRC_DVO_M14_Wr01(dvo_pwm1_ctrl0, pwm1_inv, gPwmInfo[pwmId].ctrl.pwm_inv);
			FRC_DVO_M14_Wr01(dvo_pwm1_ctrl0, pwm1_sel, gPwmInfo[pwmId].ctrl.pwm_sel);
			FRC_DVO_M14_Wr01(dvo_pwm1_ctrl0, pwm1_width_falling_pos, gPwmInfo[pwmId].ctrl.pwm_width_falling_pos);
			FRC_DVO_M14_Wr01(dvo_pwm1_ctrl1, pwm1_free_width, gPwmInfo[pwmId].ctrl.pwm_free_width);
			FRC_DVO_M14_Wr01(dvo_pwm1_ctrl2, pwm1_method, gPwmInfo[pwmId].ctrl.pwm_method);
			FRC_DVO_M14_Wr01(dvo_pwm1_ctrl2, pwm1_fc_h_disp, gPwmInfo[pwmId].ctrl.pwm_fc_h_disp);
			if(zeroDuty && (!gPwmInfo[pwmId].ctrl.pwm_fc_h_disp))
			{
				FRC_DVO_M14_Wr01(dvo_pwm1_ctrl2, pwm1_fc_l_disp, 1);
			}
			else
			{
				FRC_DVO_M14_Wr01(dvo_pwm1_ctrl2, pwm1_fc_l_disp, gPwmInfo[pwmId].ctrl.pwm_fc_l_disp);
			}
			FRC_DVO_M14_WrFL(dvo_pwm1_ctrl0);
			FRC_DVO_M14_WrFL(dvo_pwm1_ctrl1);
			FRC_DVO_M14_WrFL(dvo_pwm1_ctrl2);
			break;
		case 2 :
			FRC_DVO_M14_RdFL(dvo_pwm2_ctrl0);
			FRC_DVO_M14_RdFL(dvo_pwm2_ctrl1);
			FRC_DVO_M14_RdFL(dvo_pwm2_ctrl2);
			FRC_DVO_M14_Wr01(dvo_pwm2_ctrl0, pwm2_en, gPwmInfo[pwmId].ctrl.pwm_en);
			FRC_DVO_M14_Wr01(dvo_pwm2_ctrl0, pwm2_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
			FRC_DVO_M14_Wr01(dvo_pwm2_ctrl0, pwm2_resolution, gPwmInfo[pwmId].ctrl.pwm_resolution);
			FRC_DVO_M14_Wr01(dvo_pwm2_ctrl0, pwm2_inv, gPwmInfo[pwmId].ctrl.pwm_inv);
			FRC_DVO_M14_Wr01(dvo_pwm2_ctrl0, pwm2_sel, gPwmInfo[pwmId].ctrl.pwm_sel);
			FRC_DVO_M14_Wr01(dvo_pwm2_ctrl0, pwm2_width_falling_pos, gPwmInfo[pwmId].ctrl.pwm_width_falling_pos);
			FRC_DVO_M14_Wr01(dvo_pwm2_ctrl1, pwm2_free_width, gPwmInfo[pwmId].ctrl.pwm_free_width);
			FRC_DVO_M14_Wr01(dvo_pwm2_ctrl2, pwm2_method, gPwmInfo[pwmId].ctrl.pwm_method);
			FRC_DVO_M14_Wr01(dvo_pwm2_ctrl2, pwm2_fc_h_disp, gPwmInfo[pwmId].ctrl.pwm_fc_h_disp);
			if(zeroDuty && (!gPwmInfo[pwmId].ctrl.pwm_fc_h_disp))
			{
				FRC_DVO_M14_Wr01(dvo_pwm2_ctrl2, pwm2_fc_l_disp, 1);
			}
			else
			{
				FRC_DVO_M14_Wr01(dvo_pwm2_ctrl2, pwm2_fc_l_disp, gPwmInfo[pwmId].ctrl.pwm_fc_l_disp);
			}
			FRC_DVO_M14_WrFL(dvo_pwm2_ctrl0);
			FRC_DVO_M14_WrFL(dvo_pwm2_ctrl1);
			FRC_DVO_M14_WrFL(dvo_pwm2_ctrl2);
			break;
		case 3 :
			FRC_DVO_M14_RdFL(dvo_pwm3_ctrl0);
			FRC_DVO_M14_Wr01(dvo_pwm3_ctrl0, pwm3_en, gPwmInfo[pwmId].ctrl.pwm_en);
			FRC_DVO_M14_Wr01(dvo_pwm3_ctrl0, pwm3_freq_mode, gPwmInfo[pwmId].ctrl.pwm_freq_mode);
			FRC_DVO_M14_WrFL(dvo_pwm3_ctrl0);
			break;
		default :
			break;
	}
...
static int _REG_SetPwmTiming(UINT32 pwmId)
{
	...
	if (lx_chip_rev() >= LX_CHIP_REV( M14, B0))
	{
		switch (pwmId) {
			case 0 :
				FRC_DVO_M14B0_RdFL(dvo_pwm_v_load_write);
				FRC_DVO_M14_RdFL(dvo_pwm0_v_r);
				FRC_DVO_M14_RdFL(dvo_pwm0_v_f);
				FRC_DVO_M14B0_RdFL(dvo_pwm0_v_sub_r);
				FRC_DVO_M14B0_RdFL(dvo_pwm0_v_sub_f);
				FRC_DVO_M14B0_Wr01(dvo_pwm_v_load_write, pwm0_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
				FRC_DVO_M14_Wr01(dvo_pwm0_v_r, pwm0_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
				FRC_DVO_M14_Wr01(dvo_pwm0_v_r, pwm0_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
				FRC_DVO_M14_Wr01(dvo_pwm0_v_f, pwm0_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
				FRC_DVO_M14_Wr01(dvo_pwm0_v_f, pwm0_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
				FRC_DVO_M14B0_Wr01(dvo_pwm_v_load_write, pwm0_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
				FRC_DVO_M14B0_Wr01(dvo_pwm0_v_sub_r, pwm0_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
				FRC_DVO_M14B0_Wr01(dvo_pwm0_v_sub_r, pwm0_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
				FRC_DVO_M14B0_Wr01(dvo_pwm_v_load_write, pwm0_v_sub_f_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
				FRC_DVO_M14B0_Wr01(dvo_pwm0_v_sub_f, pwm0_v_sub_f, gPwmInfo[pwmId].t2nd.pwm_v_f);
				FRC_DVO_M14B0_Wr01(dvo_pwm0_v_sub_f, pwm0_v_sub_f_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
				FRC_DVO_M14B0_WrFL(dvo_pwm_v_load_write);
				FRC_DVO_M14_WrFL(dvo_pwm0_v_r);
				FRC_DVO_M14_WrFL(dvo_pwm0_v_f);
				FRC_DVO_M14B0_WrFL(dvo_pwm0_v_sub_r);
				FRC_DVO_M14B0_WrFL(dvo_pwm0_v_sub_f);
				break;
			case 1 :
				FRC_DVO_M14B0_RdFL(dvo_pwm_v_load_write);
				FRC_DVO_M14_RdFL(dvo_pwm1_v_r);
				FRC_DVO_M14_RdFL(dvo_pwm1_v_f);
				FRC_DVO_M14B0_RdFL(dvo_pwm1_v_sub_r);
				FRC_DVO_M14B0_RdFL(dvo_pwm1_v_sub_f);
				FRC_DVO_M14B0_Wr01(dvo_pwm_v_load_write, pwm1_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
				FRC_DVO_M14_Wr01(dvo_pwm1_v_r, pwm1_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
				FRC_DVO_M14_Wr01(dvo_pwm1_v_r, pwm1_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
				FRC_DVO_M14_Wr01(dvo_pwm1_v_f, pwm1_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
				FRC_DVO_M14_Wr01(dvo_pwm1_v_f, pwm1_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
				FRC_DVO_M14B0_Wr01(dvo_pwm_v_load_write, pwm1_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
				FRC_DVO_M14B0_Wr01(dvo_pwm1_v_sub_r, pwm1_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
				FRC_DVO_M14B0_Wr01(dvo_pwm1_v_sub_r, pwm1_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
				FRC_DVO_M14B0_Wr01(dvo_pwm_v_load_write, pwm1_v_sub_f_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
				FRC_DVO_M14B0_Wr01(dvo_pwm1_v_sub_f, pwm1_v_sub_f, gPwmInfo[pwmId].t2nd.pwm_v_f);
				FRC_DVO_M14B0_Wr01(dvo_pwm1_v_sub_f, pwm1_v_sub_f_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
				FRC_DVO_M14B0_WrFL(dvo_pwm_v_load_write);
				FRC_DVO_M14_WrFL(dvo_pwm1_v_r);
				FRC_DVO_M14_WrFL(dvo_pwm1_v_f);
				FRC_DVO_M14B0_WrFL(dvo_pwm1_v_sub_r);
				FRC_DVO_M14B0_WrFL(dvo_pwm1_v_sub_f);
				break;
			case 2 :
				FRC_DVO_M14B0_RdFL(dvo_pwm_v_load_write);
				FRC_DVO_M14_RdFL(dvo_pwm2_v_r);
				FRC_DVO_M14_RdFL(dvo_pwm2_v_f);
				FRC_DVO_M14B0_RdFL(dvo_pwm2_v_sub_r);
				FRC_DVO_M14B0_RdFL(dvo_pwm2_v_sub_f);
				FRC_DVO_M14B0_Wr01(dvo_pwm_v_load_write, pwm2_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
				FRC_DVO_M14_Wr01(dvo_pwm2_v_r, pwm2_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
				FRC_DVO_M14_Wr01(dvo_pwm2_v_r, pwm2_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
				FRC_DVO_M14_Wr01(dvo_pwm2_v_f, pwm2_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
				FRC_DVO_M14_Wr01(dvo_pwm2_v_f, pwm2_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
				FRC_DVO_M14B0_Wr01(dvo_pwm_v_load_write, pwm2_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
				FRC_DVO_M14B0_Wr01(dvo_pwm2_v_sub_r, pwm2_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
				FRC_DVO_M14B0_Wr01(dvo_pwm2_v_sub_r, pwm2_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
				FRC_DVO_M14B0_Wr01(dvo_pwm_v_load_write, pwm2_v_sub_f_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
				FRC_DVO_M14B0_Wr01(dvo_pwm2_v_sub_f, pwm2_v_sub_f, gPwmInfo[pwmId].t2nd.pwm_v_f);
				FRC_DVO_M14B0_Wr01(dvo_pwm2_v_sub_f, pwm2_v_sub_f_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
				FRC_DVO_M14B0_WrFL(dvo_pwm_v_load_write);
				FRC_DVO_M14_WrFL(dvo_pwm2_v_r);
				FRC_DVO_M14_WrFL(dvo_pwm2_v_f);
				FRC_DVO_M14B0_WrFL(dvo_pwm2_v_sub_r);
				FRC_DVO_M14B0_WrFL(dvo_pwm2_v_sub_f);
				break;
			default :
				break;
		}
	...
	else
	{
		switch (pwmId) {
			case 0 :
				FRC_DVO_M14_RdFL(dvo_pwm_v_load_write);
				FRC_DVO_M14_RdFL(dvo_pwm0_v_r);
				FRC_DVO_M14_RdFL(dvo_pwm0_v_f);
				FRC_DVO_M14A0_RdFL(dvo_pwm0_v_sub);
				FRC_DVO_M14_Wr01(dvo_pwm_v_load_write, pwm0_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
				FRC_DVO_M14_Wr01(dvo_pwm0_v_r, pwm0_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
				FRC_DVO_M14_Wr01(dvo_pwm0_v_r, pwm0_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
				FRC_DVO_M14_Wr01(dvo_pwm0_v_f, pwm0_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
				FRC_DVO_M14_Wr01(dvo_pwm0_v_f, pwm0_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
				FRC_DVO_M14_Wr01(dvo_pwm_v_load_write, pwm0_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
				FRC_DVO_M14A0_Wr01(dvo_pwm0_v_sub, pwm0_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
				FRC_DVO_M14A0_Wr01(dvo_pwm0_v_sub, pwm0_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
				FRC_DVO_M14_WrFL(dvo_pwm_v_load_write);
				FRC_DVO_M14_WrFL(dvo_pwm0_v_r);
				FRC_DVO_M14_WrFL(dvo_pwm0_v_f);
				FRC_DVO_M14A0_WrFL(dvo_pwm0_v_sub);
				break;
			case 1 :
				FRC_DVO_M14_RdFL(dvo_pwm_v_load_write);
				FRC_DVO_M14_RdFL(dvo_pwm1_v_r);
				FRC_DVO_M14_RdFL(dvo_pwm1_v_f);
				FRC_DVO_M14A0_RdFL(dvo_pwm1_v_sub);
				FRC_DVO_M14_Wr01(dvo_pwm_v_load_write, pwm1_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
				FRC_DVO_M14_Wr01(dvo_pwm1_v_r, pwm1_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
				FRC_DVO_M14_Wr01(dvo_pwm1_v_r, pwm1_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
				FRC_DVO_M14_Wr01(dvo_pwm1_v_f, pwm1_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
				FRC_DVO_M14_Wr01(dvo_pwm1_v_f, pwm1_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
				FRC_DVO_M14_Wr01(dvo_pwm_v_load_write, pwm1_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
				FRC_DVO_M14A0_Wr01(dvo_pwm1_v_sub, pwm1_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
				FRC_DVO_M14A0_Wr01(dvo_pwm1_v_sub, pwm1_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
				FRC_DVO_M14_WrFL(dvo_pwm_v_load_write);
				FRC_DVO_M14_WrFL(dvo_pwm1_v_r);
				FRC_DVO_M14_WrFL(dvo_pwm1_v_f);
				FRC_DVO_M14A0_WrFL(dvo_pwm1_v_sub);
				break;
			case 2 :
				FRC_DVO_M14_RdFL(dvo_pwm_v_load_write);
				FRC_DVO_M14_RdFL(dvo_pwm2_v_r);
				FRC_DVO_M14_RdFL(dvo_pwm2_v_f);
				FRC_DVO_M14A0_RdFL(dvo_pwm2_v_sub);
				FRC_DVO_M14_Wr01(dvo_pwm_v_load_write, pwm2_v_we, gPwmInfo[pwmId].t1st.pwm_v_we);
				FRC_DVO_M14_Wr01(dvo_pwm2_v_r, pwm2_v_r, gPwmInfo[pwmId].t1st.pwm_v_r);
				FRC_DVO_M14_Wr01(dvo_pwm2_v_r, pwm2_v_r_id, gPwmInfo[pwmId].t1st.pwm_v_id);
				FRC_DVO_M14_Wr01(dvo_pwm2_v_f, pwm2_v_f, gPwmInfo[pwmId].t1st.pwm_v_f);
				FRC_DVO_M14_Wr01(dvo_pwm2_v_f, pwm2_v_f_id, gPwmInfo[pwmId].t1st.pwm_v_id);
				FRC_DVO_M14_Wr01(dvo_pwm_v_load_write, pwm2_v_sub_we, gPwmInfo[pwmId].t2nd.pwm_v_we);
				FRC_DVO_M14A0_Wr01(dvo_pwm2_v_sub, pwm2_v_sub, gPwmInfo[pwmId].t2nd.pwm_v_r);
				FRC_DVO_M14A0_Wr01(dvo_pwm2_v_sub, pwm2_v_sub_id, gPwmInfo[pwmId].t2nd.pwm_v_id);
				FRC_DVO_M14_WrFL(dvo_pwm_v_load_write);
				FRC_DVO_M14_WrFL(dvo_pwm2_v_r);
				FRC_DVO_M14_WrFL(dvo_pwm2_v_f);
				FRC_DVO_M14A0_WrFL(dvo_pwm2_v_sub);
				break;
			default :
				break;
		}
...
static int _REG_GetPwmTiming(UINT32 freq_mode, UINT32 *pRising, UINT32 *pFall)
{
	...
	do {
		...
		{
			case 0 :
				FRC_DVO_M14_RdFL(dvo_pwm3_0_low);
				FRC_DVO_M14_RdFL(dvo_pwm3_0_high);
				FRC_DVO_M14_Rd01(dvo_pwm3_0_low, pwm3_0_low, *pRising);
				FRC_DVO_M14_Rd01(dvo_pwm3_0_high, pwm3_0_high, *pFall);
				break;
			case 1 :
				FRC_DVO_M14_RdFL(dvo_pwm3_1_low);
				FRC_DVO_M14_RdFL(dvo_pwm3_1_high);
				FRC_DVO_M14_Rd01(dvo_pwm3_1_low, pwm3_1_low, *pRising);
				FRC_DVO_M14_Rd01(dvo_pwm3_1_high, pwm3_1_high, *pFall);
				break;
			case 2 :
				FRC_DVO_M14_RdFL(dvo_pwm3_2_low);
				FRC_DVO_M14_RdFL(dvo_pwm3_2_high);
				FRC_DVO_M14_Rd01(dvo_pwm3_2_low, pwm3_2_low, *pRising);
				FRC_DVO_M14_Rd01(dvo_pwm3_2_high, pwm3_2_high, *pFall);
				break;
			case 3 :
				FRC_DVO_M14_RdFL(dvo_pwm3_3_low);
				FRC_DVO_M14_RdFL(dvo_pwm3_3_high);
				FRC_DVO_M14_Rd01(dvo_pwm3_3_low, pwm3_3_low, *pRising);
				FRC_DVO_M14_Rd01(dvo_pwm3_3_high, pwm3_3_high, *pFall);
				break;
			default :
