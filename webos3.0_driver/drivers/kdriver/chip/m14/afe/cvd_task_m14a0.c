...
----------------------------------------------------------------------------------------*/
static CVD_STATE_T CVD_M14Ax_Check_Color_System_Support(LX_AFE_CVD_SUPPORT_COLOR_SYSTEM_T	forced_color_system);
...
int	CVD_M14Ax_Task_3cdetect(void)
{
...
//	UINT32 rbuf1_empty, rbuf2_empty, rbuf3_empty, rbuf4_empty, wbuf_ful, wbuf_empty;
	...
	//CVD_M14Ax_Check_CVD_Memory();

#ifdef FAST_VLINE625_DETECT
	CVD_M14Ax_Fast_Vline_625_Using_Vdetec_Vcount();
#endif
	/* Read Status CVD Registers */
	g_CVD_Status_3CS.no_signal_flag = CVD_M14Ax_Get_No_Signal_Flag();
	g_CVD_Status_3CS.h_lock_flag = CVD_M14Ax_Get_HLock_Flag();
	g_CVD_Status_3CS.v_lock_flag = CVD_M14Ax_Get_VLock_Flag();
	...
	//HV_Lock_Flag = g_CVD_Status_3CS.h_lock_flag || g_CVD_Status_3CS.v_lock_flag;
	g_CVD_Status_3CS.vline_625_flag = CVD_M14Ax_Get_Vline_625_Flag();
	g_CVD_Status_3CS.cvd_pal_flag = CVD_M14Ax_Get_PAL_Flag();
	g_CVD_Status_3CS.cvd_secam_flag = CVD_M14Ax_Get_SECAM_Flag();//gogosing
	g_CVD_Status_3CS.cvd_chromalock_flag = CVD_M14Ax_Get_Chromalock_Flag();
	g_CVD_Status_3CS.status_noise = CVD_M14Ax_Get_Noise_Status();
	g_CVD_Status_3CS.no_burst_flag = CVD_M14Ax_Get_NoBurst_Flag();
	g_CVD_Status_3CS.cvd_fc_flag = CVD_M14Ax_Get_FC_Flag(LX_CVD_MAIN);
	g_CVD_Status_3CS.cvd_cordic_freq = CVD_M14Ax_Get_Cordic_Freq(LX_CVD_MAIN);
	g_CVD_Status_3CS.cvd_cordic_freq += CVD_M14Ax_Get_Cordic_Freq(LX_CVD_MAIN);
	g_CVD_Status_3CS.cvd_cordic_freq += CVD_M14Ax_Get_Cordic_Freq(LX_CVD_MAIN);
	g_CVD_Status_3CS.cvd_cordic_freq += CVD_M14Ax_Get_Cordic_Freq(LX_CVD_MAIN);
	...
	//g_CVD_Status_3CS.cvd_cordic_freq = g_CVD_Status_3CS.cvd_cordic_freq /5;
		g_CVD_Status_3CS.status_agc_gain = CVD_M14Ax_Get_Status_AGC_Gain();
		g_CVD_Status_3CS.hnon_standard_flag = CVD_M14Ax_Get_HNon_Standard_Flag();
		g_CVD_Status_3CS.vnon_standard_flag = CVD_M14Ax_Get_VNon_Standard_Flag();
	/*
	 ...
	 */
	g_CVD_Status_3CS.vline_625_reg = CVD_M14Ax_Get_Vline_625_Reg();
	...
	g_CVD_Status_3CS.cs0_pal_flag = CVD_M14Ax_Get_PAL_Flag_CS0();
	g_CVD_Status_3CS.cs0_secam_flag = CVD_M14Ax_Get_SECAM_Flag_CS0();//gogosing
	g_CVD_Status_3CS.cs0_chromalock_flag = CVD_M14Ax_Get_Chromalock_Flag_CS0();
	g_CVD_Status_3CS.cs0_fc_flag = CVD_M14Ax_Get_FC_Flag_CS0();
	g_CVD_Status_3CS.cs0_cordic_freq = CVD_M14Ax_Get_Cordic_Freq_CS0();
	g_CVD_Status_3CS.cs0_cordic_freq += CVD_M14Ax_Get_Cordic_Freq_CS0();
	g_CVD_Status_3CS.cs0_cordic_freq += CVD_M14Ax_Get_Cordic_Freq_CS0();
	g_CVD_Status_3CS.cs0_cordic_freq += CVD_M14Ax_Get_Cordic_Freq_CS0();
	g_CVD_Status_3CS.cs0_cordic_freq += CVD_M14Ax_Get_Cordic_Freq_CS0();
	...
	g_CVD_Status_3CS.cs1_pal_flag = CVD_M14Ax_Get_PAL_Flag_CS1();
	g_CVD_Status_3CS.cs1_secam_flag = CVD_M14Ax_Get_SECAM_Flag_CS1();//gogosing
	g_CVD_Status_3CS.cs1_chromalock_flag = CVD_M14Ax_Get_Chromalock_Flag_CS1();
	g_CVD_Status_3CS.cs1_fc_flag = CVD_M14Ax_Get_FC_Flag_CS1();
	g_CVD_Status_3CS.cs1_cordic_freq = CVD_M14Ax_Get_Cordic_Freq_CS1();
	g_CVD_Status_3CS.cs1_cordic_freq += CVD_M14Ax_Get_Cordic_Freq_CS1();
	g_CVD_Status_3CS.cs1_cordic_freq += CVD_M14Ax_Get_Cordic_Freq_CS1();
	g_CVD_Status_3CS.cs1_cordic_freq += CVD_M14Ax_Get_Cordic_Freq_CS1();
	//g_CVD_Status_3CS.cs1_cordic_freq += CVD_M14Ax_Get_Cordic_Freq_CS1();
	...
	g_CVD_Status_3CS.cvd_burst_mag = CVD_M14Ax_Get_CVD_Burst_Mag_Value();

	g_CVD_Status_3CS.cs0_secam_flag &= CVD_M14Ax_Get_SECAM_Flag_CS0();//130204
	...
	switch(g_CVD_Status_3CS.cvd_color_system)
	{
		case CVD_STATE_VideoNotReady:
			...
			if(g_CVD_Status_3CS.cvd_no_signal_count > 4) 
			{
				...
				{
					CVD_M14Ax_Reset_hdct(1);
					CVD_DEBUG("!!! CVD HDCT WORKAROUND !!!\n");
					OS_MsecSleep(5);
					CVD_M14Ax_Reset_hdct(0);
				}
#endif
			}
		...
		case CVD_STATE_VideoReadyToDetect:
			...
			else if ((check_supporting_color_system = CVD_M14Ax_Check_Color_System_Support(g_CVD_Status_3CS.color_system_support)) > 0)
		...
		case CVD_STATE_NTSC:
			...
			else if(g_CVD_Status_3CS.cs1_pal_flag && g_CVD_Status_3CS.cs1_chromalock_flag && (g_CVD_Status_3CS.cs1_cordic_freq <= (FC_MORE_THRESHOLD+10)) && (g_CVD_Status_3CS.cs1_cordic_freq >= FC_LESS_THRESHOLD) && (g_CVD_Status_3CS.color_system_support & LX_COLOR_SYSTEM_PAL_M)) // Transition to PALm
				g_CVD_Status_3CS.cvd_next_state = CVD_STATE_PALm;
	...
	//	g_CVD_Status_3CS.port_change_detected = FALSE;
		...
		CVD_M14Ax_No_Color_Cstate_Control(&g_CVD_Status_3CS, 1);
		...
		if(g_CVD_Status_3CS.in_atv_tunning == FALSE)
		{
			...
			CVD_M14Ax_vf_nstd_control(1);	//default value when state changed
			// Set Hnon_std_threshold to '0' when state changed (WA for 3DComb buffer control error)
			CVD_M14Ax_Set_HNon_Standard_Threshold(0);	//121222 : kim.min
		}
#endif
		...
		if( g_CVD_Status_3CS.cvd_prev_color_system == CVD_STATE_NTSC)
		{
			...
			CVD_M14Ax_Set_CVD_CAGC(CVD_BURST_MAG_STATE_BIG,&g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_prev_color_system], g_CVD_Status_3CS.cvd_prev_color_system);
			...
			CVD_M14Ax_Set_CVD_Saturation_Value(g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_prev_color_system].reg_saturation);
			}
			 */

#ifdef L9_HSTATE_FIXED_CONTROL_FOR_STABLE_SYNC
			//		CVD_CSD_DEBUG("Set hstate_fixed value to 0 (default) \n");
			CVD_M14Ax_Set_Hstate_Fixed(0);
			CVD_M14Ax_Set_Hstate_Max(5);
#endif
#ifdef L9_CHANGE_FIELD_DETECT_MODE_ON_DONG_GO_DONG_RAK

			CVD_M14Ax_Set_for_Field_Detect_Mode(2);
#endif
	...
	//		CVD_M14Ax_AGC_Peak_En_Control(2, 0, 0);	// force on peak_en
			...
			{
				CVD_M14Ax_Burst_Gate_Control(g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_prev_color_system].reg_burst_gate_start_new, g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_prev_color_system].reg_burst_gate_end_new);
...
#endif
#ifdef L9_ADAPTIVE_BURST_GATE_END_MODE
			CVD_M14Ax_Set_for_Burst_Gate_End_On_Noisy(g_SetColorSystem_3CS_M14A0[CVD_STATE_PALm].reg_burst_gate_end_new ,0);
#ifdef L9_BYPASS_AGC_ON_ADAPTIVE_BURST_GATE_END_MODE
			CVD_M14Ax_AGC_Bypass(0);
#endif
#endif
		}
		// #2.3 	: State Change & Previously Locked State is PALi
		else if( g_CVD_Status_3CS.cvd_prev_color_system == CVD_STATE_PALi) {
#ifdef	M14_PAL_STABLE_VSYNC_WORKAROUND
			CVD_M14Ax_Set_Vsync_Cntl(&g_SetColorSystem_3CS_M14A0[CVD_STATE_PALi], 0);	// direct vsync
#endif
#ifdef L9_HSTATE_FIXED_CONTROL_FOR_STABLE_SYNC
			//		CVD_CSD_DEBUG("Set hstate_fixed value to 0 (default) \n");
			CVD_M14Ax_Set_Hstate_Fixed(0);
		...
		if(g_CVD_Status_3CS.in_atv_tunning == FALSE)
			CVD_M14Ax_AGC_Bypass_Function(0, 0, 1 );
		//CVD_AGC_Bypass_Function((UINT32)g_CVD_Status_3CS.status_noise, (UINT32)g_CVD_Status_3CS.h_lock_flag, (UINT32)g_CVD_Status_3CS.no_signal_flag );
		...
		if(g_CVD_Status_3CS.cvd_next_state < CVD_STATE_VideoNotReady)
		{
			...
			if( (g_CVD_Status_3CS.cvd_prev_color_system != g_CVD_Status_3CS.cvd_next_state) || (g_CVD_Status_3CS.cvd_color_system==CVD_STATE_VideoNotReady) || ( g_CVD_Status_3CS.prev_av_rf_mode != g_CVD_Status_3CS.in_rf_mode) )
			{
				...
				if( ( g_CVD_Status_3CS.cvd_prev_color_system != g_CVD_Status_3CS.cvd_next_state) || (g_CVD_Status_3CS.port_change_detected == TRUE) || ( g_CVD_Status_3CS.prev_av_rf_mode != g_CVD_Status_3CS.in_rf_mode))
				{
					 ...
					 */
					{
						...
						CVD_M14Ax_Reset_hdct(1);
						CVD_M14Ax_Reset_cdct(1);
#ifdef M14_COMB2D_ONLY_CONTROL
						/*
						//120718 : comb2d_only on control when color system change
						CVD_DEBUG("$$$ Comb2d only [1] $$$\n");
						CVD_M14Ax_Read_Buffer_Status(&rbuf1_empty, &rbuf2_empty, &rbuf3_empty, &rbuf4_empty, &wbuf_ful, &wbuf_empty);
...
#ifdef	M14_3DCOMB_ERROR_WORKAROUND
					CVD_M14Ax_3DComb_Workaround(1);
#endif

					CVD_M14Ax_Set_Color_System_3CS(g_CVD_Status_3CS.cvd_next_state);
				...
				else
				{
					...
					CVD_M14Ax_Set_comb2d_only(g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_next_state].reg_comb2d_only);
					 */
#endif
				}
				...
				if(g_CVD_Status_3CS.in_atv_tunning == FALSE)
				{
					if( (g_CVD_Status_3CS.in_rf_mode == TRUE) \
						...
						CVD_M14Ax_Enable_Stable_Sync_Mode(1);		//for RF nosignal (dcrestore_lpf_en ON)
					else
						CVD_M14Ax_Enable_Stable_Sync_Mode(0);		//default state

					if ((g_CVD_Status_3CS.in_rf_mode == TRUE) )
					   CVD_M14Ax_Hstart_Position_Compensator(&g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_next_state]);
				}
#endif
				...
				{
					CVD_M14Ax_Set_PALm_Stable_HSync_Mode(1);
				}
#endif
#ifdef M14_ENABLE_PALM_STABLE_SYNC_MODE
			...
			//Program_Color_System_Main_only_system_set(g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_next_state]);
		...
		else if(g_CVD_Status_3CS.cvd_next_state != CVD_STATE_VideoReadyToDetect)  // No Signal or color system change
		{
...
#ifdef	M14_3DCOMB_ERROR_WORKAROUND
			//			CVD_M14Ax_3DComb_Workaround(0);
#ifdef M14_CDTO_CONTROL
			CVD_M14Ax_3DComb_Workaround(4);
#endif
#endif
			...
			CVD_M14Ax_Reset_vdct(1);
			OS_MsecSleep(5);
			CVD_M14Ax_Reset_vdct(0);
#endif
			...
			//		CVD_M14Ax_agc_half_en_control(1);//gogosing agc
#endif
		...
		//CVD_M14Ax_NoBurst_CKill_Control(&g_CVD_Status_3CS, 1);
		g_CVD_Status_3CS.port_change_detected = FALSE;
	} // #2 	: State Change Detected 
			...
			if(g_CVD_Status_3CS.in_atv_tunning == FALSE) {
				...
				if(g_CVD_Status_3CS.cvd_lock_stable_count > CVD_STABLE_COUNT_20) {
					...
					if ( (g_CVD_Status_3CS.cvd_color_system == CVD_STATE_NTSC)  && (g_CVD_Status_3CS.in_rf_mode == TRUE) && (g_CVD_Status_3CS.in_atv_tunning == FALSE)) {
						gHstateMaxCounter++;
						if(CVD_M14Ax_Read_VCR_Detected()) vcr_detect_count++;

						if(gHstateMaxCounter >= 20)
						{
							if(vcr_detect_count >=  CVD_HSTATE_FIX_VCR_COUNT_THRESHOLD)
								CVD_M14Ax_Set_Hstate_Max(3);
							else CVD_M14Ax_Set_Hstate_Max(5);
					...
					if ( (g_CVD_Status_3CS.cvd_color_system == CVD_STATE_PALm) && (g_CVD_Status_3CS.in_rf_mode == TRUE)  )
					{
						...
						{
							if ((g_CVD_Status_3CS.cvd_color_system == CVD_STATE_PALm) && (g_CVD_Status_3CS.in_rf_mode == TRUE) ) {
								gVariance_Status_Cdto_Inc = CVD_M14Ax_Differential_Status_Cdto_Inc_Value();
								gAccumulated_Cordic_Freq_Value = CVD_M14Ax_Read_Cordic_Freq_Value();
							}
						...
						else if(gAdaptive_Burst_Gate_Counter >= CVD_ADAPTIVE_BURST_GATE_COUNT_ITERATION)
						{
							...
							{
								CVD_M14Ax_Set_for_Burst_Gate_End_On_Noisy(g_SetColorSystem_3CS_M14A0[CVD_STATE_PALm].reg_burst_gate_end_new ,Adaptive_Burst_Gate_Enable);
#ifdef L9_BYPASS_AGC_ON_ADAPTIVE_BURST_GATE_END_MODE
								CVD_M14Ax_AGC_Bypass(Adaptive_Burst_Gate_Enable);
#endif
							}
					...
					if( (g_CVD_Status_3CS.in_atv_tunning == FALSE) && (g_CVD_Status_3CS.in_rf_mode == TRUE) && (g_CVD_Status_3CS.cvd_color_system == CVD_STATE_PALm))
						CVD_M14Ax_AGC_Bypass_Function((UINT32)g_CVD_Status_3CS.avg_status_noise, (UINT32)g_CVD_Status_3CS.h_lock_flag, (UINT32)g_CVD_Status_3CS.no_signal_flag );
					...
					if ((g_CVD_Status_3CS.cvd_color_system == CVD_STATE_NTSC) && (g_CVD_Status_3CS.in_rf_mode == TRUE))
					{
						...
						if( (avg_noise_level > 0x240) && (g_CVD_Status_3CS.cvd_burst_mag > 0x1800) )	// for NS Home-shopping field stream
							CVD_M14Ax_Enable_Stable_Sync_Mode(3);
					}
#endif
#ifdef M14_ENABLE_PALM_STABLE_SYNC_MODE
					avg_noise_level = cvd_noise_sum / (g_CVD_Status_3CS.cvd_lock_stable_count + 1);
							...
							&& (avg_noise_level > 0x3F0))
					{
						...
						CVD_M14Ax_Set_PALm_Stable_VSync_Mode(1);
					}
					else
						CVD_M14Ax_Set_PALm_Stable_VSync_Mode(0);
#endif
					...
					if ( (g_CVD_Status_3CS.cvd_color_system == CVD_STATE_PALi)&& (g_CVD_Status_3CS.in_rf_mode == TRUE)\
							&& (g_CVD_Status_3CS.in_atv_tunning == FALSE)) 
						CVD_M14Ax_Set_Vsync_Cntl(&g_SetColorSystem_3CS_M14A0[CVD_STATE_PALi], 2);	// PLL vsync
#endif
				...
				else if(g_CVD_Status_3CS.cvd_lock_stable_count == (CVD_STABLE_COUNT_1 + 2) )
				{
					...
					CVD_M14Ax_Set_comb2d_only_md(g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_next_state].reg_comb2d_only);
#endif
					g_CVD_Status_3CS.cvd_lock_stable_count++;
				}
				...
				{
					CVD_M14Ax_Init_Level_Detector(g_CVD_Status_3CS.cvd_color_system);
					...
					if ((g_CVD_Status_3CS.cvd_color_system == CVD_STATE_NTSC) && (g_CVD_Status_3CS.in_rf_mode == TRUE))
					{
							CVD_M14Ax_Enable_Stable_Sync_Mode(2);
					}
#endif

					//121206
#ifdef M14_ADAPTIVE_AGC_PEAK_EN_CONTROL
				    CVD_M14Ax_AGC_Peak_En_Control(2, 0, 0); // force on peak_en //gogosing agc
					if(!g_CVD_Status_3CS.use_internal_demod)	// if using internal demod, agc_half_en should be '1'
						CVD_M14Ax_agc_half_en_control(0);//gogosing agc
#endif
					...
					CVD_M14Ax_vf_nstd_control(0);	//for fast 3DComb operation
...
#ifdef L9_CHANGE_FIELD_DETECT_MODE_ON_DONG_GO_DONG_RAK
					if ((g_CVD_Status_3CS.cvd_color_system == CVD_STATE_NTSC) && (g_CVD_Status_3CS.in_rf_mode == TRUE) )
						CVD_M14Ax_Set_for_Field_Detect_Mode(1);
#endif
					//CVD_M14Ax_Program_Color_System_PostJob(&g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_next_state]);
	...
	//				CVD_M14Ax_AGC_Peak_En_Control(0, 0, 0);	//force off peak_en
					...
					if ( (g_CVD_Status_3CS.cvd_color_system == CVD_STATE_PALi)&& (g_CVD_Status_3CS.in_rf_mode == TRUE)\
							&& (g_CVD_Status_3CS.in_atv_tunning == FALSE)) 
						CVD_M14Ax_Set_Vsync_Cntl(&g_SetColorSystem_3CS_M14A0[CVD_STATE_PALi], 1);	// vsync_l_cnt & vactive_start
#endif
				}
				...
				else if(g_CVD_Status_3CS.cvd_lock_stable_count < CVD_STABLE_COUNT_20)
				{
					...
					{
						CVD_M14Ax_Program_Color_System_Size(&g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_color_system]);
						CVD_M14Ax_3DComb_Workaround(2);
						CVD_M14Ax_Program_Color_System_Size_md(&g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_color_system]);
						CVD_M14Ax_3DComb_Workaround(3);
					}
#ifdef M14_CDTO_CONTROL
					...
					{
						/*
						   if( (CVD_M14Ax_Get_comb2d_only() == 0) && CVD_M14Ax_Get_comb2d_only_md() )
						   {
						   CVD_DEBUG("$$$ Comb2d only MD [0] : stable count[%d]$$$\n", g_CVD_Status_3CS.cvd_lock_stable_count);
						   CVD_M14Ax_Set_comb2d_only_md(g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_next_state].reg_comb2d_only);
						   }
						   else if(g_CVD_Status_3CS.h_lock_flag && g_CVD_Status_3CS.v_lock_flag && g_CVD_Status_3CS.cvd_chromalock_flag && CVD_M14Ax_Get_comb2d_only()\
						 ...
						 */
						if(g_CVD_Status_3CS.h_lock_flag && g_CVD_Status_3CS.v_lock_flag && g_CVD_Status_3CS.cvd_chromalock_flag && CVD_M14Ax_Get_comb2d_only()\
								&& (g_CVD_Status_3CS.hnon_standard_flag == 0) && (g_CVD_Status_3CS.vnon_standard_flag == 0) )
						{
#if 0
#ifdef M14_COMB2D_ONLY_CONTROL
				...
				if(g_CVD_Status_3CS.cvd_lock_stable_count > 0)
				{
					...
					//CVD_M14Ax_NoBurst_CKill_Control(&g_CVD_Status_3CS, 0);

					//120711 : adaptive cstate control for sub-carrier pull-in-range test
					//CVD_M14Ax_Cstate_Control_for_SC_PullInRange(&g_CVD_Status_3CS, 0);
					CVD_M14Ax_No_Color_Cstate_Control(&g_CVD_Status_3CS, 0);
				}
				...
				if(g_CVD_Status_3CS.cvd_lock_stable_count > CVD_STABLE_COUNT_1)
				{
					...
					if (( g_CVD_Pattern_Detection_t.bEnable == TRUE) && (g_CVD_Status_3CS.in_rf_mode == TRUE) )
						CVD_M14Ax_Pattern_Detection(&g_CVD_Pattern_Detection_t );
#if 1
					...
					if( (g_CVD_Status_3CS.cvd_color_system == CVD_STATE_PALi) && (g_CVD_Status_3CS.in_rf_mode == TRUE) )
					{
						...
						burst_mag_next = CVD_M14Ax_Get_CVD_Burst_Mag_Flag(g_CVD_Status_3CS.cvd_color_system);

						if ( (burst_mag_next != burst_mag_prev)  )
						{
							CVD_WA_DEBUG("burst_mag_prev = %d , burst_mag_next = %d \n",(int)burst_mag_prev,(int)burst_mag_next);
							CVD_M14Ax_Set_CVD_CAGC(burst_mag_next,&g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_color_system], g_CVD_Status_3CS.cvd_color_system);
					...
					else if( (g_CVD_Status_3CS.cvd_color_system == CVD_STATE_NTSC) && (g_CVD_Status_3CS.in_rf_mode == TRUE) )
					{
						...
						{
							CVD_M14Ax_Set_Motion_Expand(1);
							CVD_M14Ax_Set_CCR(0);
						}
						else 
						{
							CVD_M14Ax_Set_Motion_Expand(0);
							CVD_M14Ax_Set_CCR(1);
						}
						...
						burst_mag_array[0] = CVD_M14Ax_Get_CVD_Burst_Mag_Value();
						...
						if( g_CVD_Pattern_Detection_t.pattern_found )
						{
							...
							{
								CVD_M14Ax_Burst_Gate_Control(0x2a, 0x49);
							...
							{
								CVD_M14Ax_Burst_Gate_Control(0x2e, 0x49);
								//CVD_M14Ax_Burst_Gate_Control(0x30, 0x49);
						...
						if( (burst_mag_avg > 0xbc0) )
							...
							if( (burst_mag_avg > 0xbc0) || ( g_CVD_Pattern_Detection_t.pattern_found != 1)
								...
								//cagc_target = g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSC].reg_saturation;

								if(g_CVD_Status_3CS.cvd_black_level == LX_AFE_CVD_BLACK_LEVEL_HIGH)	// NTSC-J
								{
									cagc_target = g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSCj].reg_cagc;

									if(CVD_M14Ax_Get_CAGC_Value() != g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSCj].reg_cagc)
									{
										CVD_WA_DEBUG("NTSC-J Mode : Set CAGC to [0x%x]\n", g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSCj].reg_cagc);
										CVD_M14Ax_Set_CAGC_Value(g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSCj].reg_cagc);
										CVD_M14Ax_Set_Black_Level(LX_AFE_CVD_BLACK_LEVEL_HIGH, LX_CVD_INPUT_SOURCE_ATTRIBUTE_RF);
									}
								}
								else if(g_CVD_Status_3CS.cvd_black_level == LX_AFE_CVD_BLACK_LEVEL_LOW)	// NTSC-M
								{
									cagc_target = g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSC].reg_cagc;
									if(CVD_M14Ax_Get_CAGC_Value() != g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSC].reg_cagc)
									{
										CVD_WA_DEBUG("NTSC-M Mode : Set CAGC to [0x%x]\n", g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSC].reg_cagc);
										CVD_M14Ax_Set_CAGC_Value(g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSC].reg_cagc);
										CVD_M14Ax_Set_Black_Level(LX_AFE_CVD_BLACK_LEVEL_LOW, LX_CVD_INPUT_SOURCE_ATTRIBUTE_RF);
									}
								}
								else
								{
								...
								if(burst_mag_next > g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSC].reg_saturation)
									burst_mag_next = g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSC].reg_saturation;
								...
								&& (g_CVD_Status_3CS.cvd_lock_stable_count > (CVD_STABLE_COUNT_1 + 10 )) )
						{
							...
							cagc_written = CVD_M14Ax_Get_CAGC_Value();
					...
					else if( (g_CVD_Status_3CS.cvd_color_system == CVD_STATE_NTSC) && (g_CVD_Status_3CS.in_rf_mode == FALSE) )	// For NTSC AV
					{
						if(g_CVD_Status_3CS.cvd_black_level == LX_AFE_CVD_BLACK_LEVEL_HIGH)	// NTSC-J
						{
							cagc_target = g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSCj].reg_cagc_av;

							if(CVD_M14Ax_Get_CAGC_Value() != g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSCj].reg_cagc_av)
							{
								CVD_WA_DEBUG("NTSC-J Mode : Set CAGC to [0x%x]\n", g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSCj].reg_cagc_av);
								CVD_M14Ax_Set_CAGC_Value(g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSCj].reg_cagc_av);
								CVD_M14Ax_Set_Black_Level(LX_AFE_CVD_BLACK_LEVEL_HIGH, LX_CVD_INPUT_SOURCE_ATTRIBUTE_AV);
							}
						}
						else if(g_CVD_Status_3CS.cvd_black_level == LX_AFE_CVD_BLACK_LEVEL_LOW)	// NTSC-M
						{
							cagc_target = g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSC].reg_cagc_av;
							if(CVD_M14Ax_Get_CAGC_Value() != g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSC].reg_cagc_av)
							{
								CVD_WA_DEBUG("NTSC-M Mode : Set CAGC to [0x%x]\n", g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSC].reg_cagc_av);
								CVD_M14Ax_Set_CAGC_Value(g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSC].reg_cagc_av);
								CVD_M14Ax_Set_Black_Level(LX_AFE_CVD_BLACK_LEVEL_LOW, LX_CVD_INPUT_SOURCE_ATTRIBUTE_AV);
							}
						}
						else
						{
		...
		if ( lx_chip_rev( ) >= LX_CHIP_REV( L9, B0 ) )
		{
			CVD_M14Ax_swrst_CS( CVD_SEL_CS_CS0);
			CVD_M14Ax_swrst_CS( CVD_SEL_CS_CS1);
		}
#endif
...
//	CVD_M14Ax_Comb2d_Only_Test(0,0,0,0,0,0);
...
int CVD_M14Ax_Channel_Change_Control(BOOLEAN bATV_Channel_Change)
{
...
int CVD_M14Ax_Set_Color_System_3CS(CVD_STATE_T cvd_next_state)
{
	...
	if(g_CVD_Status_3CS.cvd_color_system <= CVD_STATE_SECAM)
	{
		...
		/*
		   CVD_M14Ax_Program_Color_System_buffer_only(&g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_color_system]);
		   CVD_M14Ax_Program_Color_System_Main_Multi(&g_SetColorSystem_3CS_M14A0[CVD_STATE_NTSC]);
		 */
		CVD_M14Ax_Program_Color_System_Main_Multi(&g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cvd_color_system]);


		CVD_M14Ax_Program_Color_System_CS(CVD_SEL_CS_CS0, &g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cs0_color_system]);
		CVD_M14Ax_Program_Color_System_CS(CVD_SEL_CS_CS1, &g_SetColorSystem_3CS_M14A0[g_CVD_Status_3CS.cs1_color_system]);
