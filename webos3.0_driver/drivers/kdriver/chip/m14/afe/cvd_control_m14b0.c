...
#include "cvd_pe_m14b0/CVD_L_AV_NTSC_443_default_m14b.h"
#include "cvd_pe_m14b0/CVD_L_AV_NTSC_M_default_m14b.h"
#include "cvd_pe_m14b0/CVD_L_AV_PAL60_default_m14b.h"
#include "cvd_pe_m14b0/CVD_L_AV_PALI_default_m14b.h"
#include "cvd_pe_m14b0/CVD_L_AV_PALM_default_m14b.h"
#include "cvd_pe_m14b0/CVD_L_AV_PALNc_default_m14b.h"
#include "cvd_pe_m14b0/CVD_L_AV_SECAM_default_m14b.h"
#include "cvd_pe_m14b0/CVD_L_RF_NTSC_M_default_m14b.h"
#include "cvd_pe_m14b0/CVD_L_RF_PALI_default_m14b.h"
#include "cvd_pe_m14b0/CVD_L_RF_PALM_default_m14b.h"
#include "cvd_pe_m14b0/CVD_L_RF_PALNc_default_m14b.h"
#include "cvd_pe_m14b0/CVD_L_RF_SECAM_default_m14b.h"

#include "cvd_pe_m14b0/CVD_L_RF_NTSC_M_Black_Level_High.h"
#include "cvd_pe_m14b0/CVD_L_RF_NTSC_M_Black_Level_Low.h"
#include "cvd_pe_m14b0/CVD_L_AV_NTSC_M_Black_Level_High.h"
#include "cvd_pe_m14b0/CVD_L_AV_NTSC_M_Black_Level_Low.h"
...
----------------------------------------------------------------------------------------*/
 ...
 CVD_SET_SYSTEM_3CS_M14B0_T g_SetColorSystem_3CS_M14B0[]=
{ 
...
int CVD_M14Bx_AGC_Peak_En_Control(int force_peak_enable, int avg_noise, int avg_agc_gain)
{
	int agc_peak_en_value;

	agc_peak_en_value = CVD_M14Bx_Get_AGC_Peak_En_Value();
	...
	if (force_peak_enable == 2) {
		CVD_M14Bx_Set_AGC_Peak_En(1);
	}
	else if (force_peak_enable == 0) {
		CVD_M14Bx_Set_AGC_Peak_En(0);
	}
...
// Not Used
int CVD_M14Bx_NoBurst_CKill_Control(CVD_STATUS_3CS_T *pCVD_Status, int clear_noburst_ckill)
{
...
int CVD_M14Bx_No_Color_Cstate_Control(CVD_STATUS_3CS_T *pCVD_Status, int cstate_reset)
{
...
// Not Used
int CVD_M14Bx_Burst_Gate_Control_for_NoBurst(CVD_STATUS_3CS_T *pCVD_Status, int clear_burst_gate)
{
	...
	if(clear_burst_gate) {
		...
		if(no_color_count > 5) {
			...
			CVD_M14Bx_Burst_Gate_Control(g_SetColorSystem_3CS_M14B0[g_CVD_Status_3CS.cvd_color_system].reg_burst_gate_start_new, 0xC0);
		...
		if(g_CVD_Status_3CS.no_color_detected == 1) {
			...
			CVD_M14Bx_Burst_Gate_Control(g_SetColorSystem_3CS_M14B0[g_CVD_Status_3CS.cvd_color_system].reg_burst_gate_start_new, g_SetColorSystem_3CS_M14B0[g_CVD_Status_3CS.cvd_color_system].reg_burst_gate_end_new);
		}
...
// Not Used
int CVD_M14Bx_Cstate_Control_for_SC_PullInRange(CVD_STATUS_3CS_T *pCVD_Status, int init_cstate) 
{
	...
	if(init_cstate) {
		...
		if(chroma_unlock_count > 5) {
			...
			CVD_M14Bx_Program_Color_System_PostJob(&g_SetColorSystem_3CS_M14B0[g_CVD_Status_3CS.cvd_color_system]);
...
int CVD_M14Bx_Pattern_Detection(CVD_PATTERN_DETECTION_T *pCVD_Pattern_Detection_t )
{
	...
	CVD_M14Bx_Get_PE0_Motion_Value(&pe0_motion_value_t.tnr_x_avg_t[0], &pe0_motion_value_t.tnr_x_avg_s[0], &pe0_motion_value_t.tpd_s_status[0] );

	CVD_M14Bx_ColorBar_Detector(&colorbar_diff_sum, &colorbar_75_100);
...
//	CVD_DEBUG("PE0 : avg_t[0x%x], avg_s[0x%x], s_status[0x%x] \n", pe0_motion_value_t.avg_tnr_x_avg_t, pe0_motion_value_t.avg_tnr_x_avg_s, pe0_motion_value_t.avg_tpd_s_status);
	...
	pCVD_Pattern_Detection_t->global_motion_value = CVD_M14Bx_Get_Global_Motion_Value();
...
int CVD_M14Bx_Set_PQ_Mode(LX_AFE_CVD_PQ_MODE_T cvd_pq_mode)
{
	...
	switch(cvd_pq_mode)
	{
		case LX_CVD_PQ_NTSC_M:		// AV NTSC
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_ntsc_m_l_default_m14b;
			size	= sizeof(cvd_av_ntsc_m_l_default_m14b) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_NTSC_M_RF:	// RF NTSC
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_ntsc_m_l_default_m14b;
			size	= sizeof(cvd_rf_ntsc_m_l_default_m14b) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL:			// AV PAL
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_pali_l_default_m14b;
			size	= sizeof(cvd_av_pali_l_default_m14b) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL_RF:		// RF PAL
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_pali_l_default_m14b;
			size	= sizeof(cvd_rf_pali_l_default_m14b) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_SECAM:		// AV SECAM
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_secam_l_default_m14b;
			size	= sizeof(cvd_av_secam_l_default_m14b) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_SECAM_RF:	// RF SECAM
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_secam_l_default_m14b;
			size	= sizeof(cvd_rf_secam_l_default_m14b) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_NTSC_443:		// AV NTSC 443
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_ntsc_443_l_default_m14b;
			size	= sizeof(cvd_av_ntsc_443_l_default_m14b) / sizeof(CVD_REG_PARAM_T) ;
			break;
		...
		case LX_CVD_PQ_PAL_60:			// AV PAL 60
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_pal60_l_default_m14b;
			size	= sizeof(cvd_av_pal60_l_default_m14b) / sizeof(CVD_REG_PARAM_T) ;
			break;
		...
		case LX_CVD_PQ_PAL_M:		// AV PAL M
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_palm_l_default_m14b;
			size	= sizeof(cvd_av_palm_l_default_m14b) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL_M_RF:	// RF PAL M
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_palm_l_default_m14b;
			size	= sizeof(cvd_rf_palm_l_default_m14b) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL_NC:		// AV PAL NC
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_av_palnc_l_default_m14b;
			size	= sizeof(cvd_av_palnc_l_default_m14b) / sizeof(CVD_REG_PARAM_T) ;
			break;
		case LX_CVD_PQ_PAL_NC_RF:	// RF PAL NC
			pCVD_PE_Param_t = (CVD_REG_PARAM_T *)cvd_rf_palnc_l_default_m14b;
			size	= sizeof(cvd_rf_palnc_l_default_m14b) / sizeof(CVD_REG_PARAM_T) ;
			break;
		default:
	...
	CVD_M14Bx_Set_PE_Params(pCVD_PE_Param_t, size, cvd_pq_mode);
...
/* On Brazil (PAL-M) weak RF Signal, AGC gain sometimes became high value result in white picture.
   ...
   */
int CVD_M14Bx_AGC_Bypass_Function(UINT32 noise_status, UINT32 hlock_flag, UINT32 no_signal_flag )
{
	int ret = 0;
	...
	if( ( noise_status == CVD_M14B0_NOISE_MAX) && ( hlock_flag == 0 )&& ( no_signal_flag == 0)) {
		if(bypass_enable == 0) {
			bypass_enable = 1;
			CVD_M14Bx_AGC_Bypass(bypass_enable);
			//CVD_M14Bx_Set_dcrestore_accum_width(0x1b);
			//111109 by kim.min for weak cable RF signals.
			CVD_M14Bx_Set_dcrestore_accum_width(DCRESTORE_ACCUM_WIDTH_FOR_WEAK_RF);
		...
		if(g_CVD_Status_3CS.cvd_prev_color_system < CVD_STATE_VideoNotReady)
			CVD_M14Bx_Set_dcrestore_accum_width(g_SetColorSystem_3CS_M14B0[g_CVD_Status_3CS.cvd_prev_color_system].reg_dcrestore_accum_width);
		CVD_DEBUG("## Disable AGC Bypass\n");
	}
	/*
	   if(noise_status == CVD_M14B0_NOISE_MAX)
	   CVD_M14Bx_Set_dcrestore_accum_width(0x28);
	   else if(noise_status < 0x100)
	   CVD_M14Bx_Set_dcrestore_accum_width(0x22);
	 */
	OS_UnlockMutex(&g_CVD_Sema);
	return ret;
}

int CVD_M14Bx_Init_Global_Params(void)
{
...
int CVD_M14Bx_Set_for_Tunning(BOOLEAN bEnable)
{
	int ret = 0;
	...
	if(bEnable == TRUE)
	{
		...
		CVD_M14Bx_Set_dcrestore_accum_width(DCRESTORE_ACCUM_WIDTH_DEFAULT);
		CVD_M14Bx_Set_Noise_Threshold(NOISE_THRESHOLD_DEFAULT);

		// restore default value of reg_hnon_std_threshold
#ifdef L9_FAST_3DCOMB_WORKAROUND
		CVD_M14Bx_Set_HNon_Standard_Threshold(g_SetColorSystem_3CS_M14B0[CVD_STATE_NTSC].reg_hnon_std_threshold);
#endif
#ifdef M14_FAST_3DCOMB_WORKAROUND
		//Enable vf_nstd_en when auto tuning
		CVD_M14Bx_vf_nstd_control(1);	
		// restore default value of reg_hnon_std_threshold
		CVD_M14Bx_Set_HNon_Standard_Threshold(g_SetColorSystem_3CS_M14B0[CVD_STATE_NTSC].reg_hnon_std_threshold);
#endif
		...
		//Auto Programming
		CVD_M14Bx_Enable_Stable_Sync_Mode(0);
#endif
	...
	else
	{
		...
		if(g_CVD_Status_3CS.cvd_prev_color_system < CVD_STATE_VideoNotReady)
			CVD_M14Bx_Set_dcrestore_accum_width(g_SetColorSystem_3CS_M14B0[g_CVD_Status_3CS.cvd_prev_color_system].reg_dcrestore_accum_width);
		CVD_M14Bx_Set_Noise_Threshold(NOISE_THRESHOLD_WEAK_RF);
	}
	OS_UnlockMutex(&g_CVD_Sema);
	return ret;
}

int CVD_M14Bx_Get_Reg_States_Detail(LX_AFE_CVD_STATES_DETAIL_T	*pcvd_states_detail_t)
{
	/* Read Status Registers */
	pcvd_states_detail_t->No_Signal_Flag = CVD_M14Bx_Get_No_Signal_Flag();
	pcvd_states_detail_t->HLock_Flag = CVD_M14Bx_Get_HLock_Flag();
	pcvd_states_detail_t->VLock_Flag = CVD_M14Bx_Get_VLock_Flag();
	pcvd_states_detail_t->VLine_625_Flag = CVD_M14Bx_Get_Vline_625_Flag();
	pcvd_states_detail_t->Pal_Flag = CVD_M14Bx_Get_PAL_Flag();
	pcvd_states_detail_t->Secam_Flag = CVD_M14Bx_Get_SECAM_Flag();
	pcvd_states_detail_t->Chromalock_Flag = CVD_M14Bx_Get_Chromalock_Flag();
	pcvd_states_detail_t->NoBurst_Flag = CVD_M14Bx_Get_NoBurst_Flag();
	pcvd_states_detail_t->FC_Flag = CVD_M14Bx_Get_FC_Flag(LX_CVD_MAIN);
	pcvd_states_detail_t->cvd_cordic_freq = CVD_M14Bx_Get_Cordic_Freq(LX_CVD_MAIN);
	pcvd_states_detail_t->status_noise = CVD_M14Bx_Get_Noise_Status();

	pcvd_states_detail_t->Pal_Flag_CS0 = CVD_M14Bx_Get_PAL_Flag_CS0();
	pcvd_states_detail_t->Secam_Flag_CS0 = CVD_M14Bx_Get_SECAM_Flag_CS0();
	pcvd_states_detail_t->Chromalock_Flag_CS0 = CVD_M14Bx_Get_Chromalock_Flag_CS0();
	pcvd_states_detail_t->FC_Flag_CS0 = CVD_M14Bx_Get_FC_Flag_CS0();
	pcvd_states_detail_t->cvd_cordic_freq_CS0 = CVD_M14Bx_Get_Cordic_Freq_CS0();

	pcvd_states_detail_t->Pal_Flag_CS1 = CVD_M14Bx_Get_PAL_Flag_CS1();
	pcvd_states_detail_t->Secam_Flag_CS1 = CVD_M14Bx_Get_SECAM_Flag_CS1();
	pcvd_states_detail_t->Chromalock_Flag_CS1 = CVD_M14Bx_Get_Chromalock_Flag_CS1();
	pcvd_states_detail_t->FC_Flag_CS1 = CVD_M14Bx_Get_FC_Flag_CS1();
	pcvd_states_detail_t->cvd_cordic_freq_CS1 = CVD_M14Bx_Get_Cordic_Freq_CS1();
	CVD_M14Bx_Get_PE0_Motion_Value(&pcvd_states_detail_t->tnr_x, &pcvd_states_detail_t->tnr_s, &pcvd_states_detail_t->s_status );
	pcvd_states_detail_t->motion_value = CVD_M14Bx_Get_Global_Motion_Value();
	pcvd_states_detail_t->agc_gain = CVD_M14Bx_Get_Status_AGC_Gain();
	pcvd_states_detail_t->cagc = CVD_M14Bx_Get_CAGC_Value();
	//pcvd_states_detail_t->saturation = CVD_M14Bx_Get_Saturation_Value();
	pcvd_states_detail_t->saturation = CVD_M14Bx_Get_oadj_c_coeff_value();
	pcvd_states_detail_t->hnon_standard = CVD_M14Bx_Get_HNon_Standard_Flag();
	pcvd_states_detail_t->vnon_standard = CVD_M14Bx_Get_VNon_Standard_Flag();
	pcvd_states_detail_t->burst_mag = CVD_M14Bx_Get_CVD_Burst_Mag_Value();
	pcvd_states_detail_t->vdetect_vcount = CVD_M14Bx_Get_Vdetect_Vcount_Value();
	pcvd_states_detail_t->agc_peak_en = CVD_M14Bx_Get_AGC_Peak_En_Value();
	pcvd_states_detail_t->agc_bypass = CVD_M14Bx_Get_AGC_Bypass_Value();
	...
	pcvd_states_detail_t->status_clamp_updn = CVD_M14Bx_Get_status_clamp_updn();

	CVD_M14Bx_ColorBar_Detector(&pcvd_states_detail_t->colorbar_diff, &pcvd_states_detail_t->colorbar_75_100);

	CVD_M14Bx_Get_AFE_CVBS_status(&pcvd_states_detail_t->cvbs_icon, &pcvd_states_detail_t->cvbs_pdb, &pcvd_states_detail_t->cvbs_cp, &pcvd_states_detail_t->cvbs_insel);
	CVD_M14Bx_Get_AFE_VBUF_status(&pcvd_states_detail_t->buf_ycm, &pcvd_states_detail_t->buf_sel1, &pcvd_states_detail_t->buf_pdb1, &pcvd_states_detail_t->vdac_pdb);
...
int CVD_M14Bx_ColorBar_Detector(UINT32 *pdiff_sum, UINT8 *pcolorbar_75_100)
{
	...
	CVD_M14Bx_Get_ColorBar_Level(composite_level);
...
int CVD_M14Bx_Comb2d_Only_Test(int set_params, int enable, int set_on_time, int set_off_time, int set_md_on_time, int set_md_off_time)
{
#if 0
...
int CVD_M14Bx_Set_Black_Level(LX_AFE_CVD_BLACK_LEVEL_T black_level, LX_AFE_CVD_SOURCE_ATTRIBUTE_T rf_av_mode)
{
	...
	CVD_M14Bx_Set_OADJ(pCVD_Black_Level_Param_t, size);
