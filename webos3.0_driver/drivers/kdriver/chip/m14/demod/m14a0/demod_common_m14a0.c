...
int DEMOD_M14A0_ResetHW(void)
{
	...
	ACE_REG_M14A0_RdFL(gbb_0);

	ACE_REG_M14A0_Wr01(gbb_0, reset, 1);

	ACE_REG_M14A0_WrFL(gbb_0);

	OS_MsecSleep(10);


	ACE_REG_M14A0_Wr01(gbb_0, reset, 0);
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_SetI2C(void)
{
	DEMOD_PRINT("[KDRV_DEMOD] DEMOD_SetI2C Start !! \n");

	ACE_REG_M14A0_RdFL(gbb_0);

	ACE_REG_M14A0_Wr01(gbb_0, i2csel, 0);
...
int DEMOD_M14A0_SetInclk(LX_DEMOD_INCLK_SEL_T *inclk)
{

	DEMOD_PRINT("[KDRV_DEMOD] DEMOD_SetInclk Start !! \n");

	ACE_REG_M14A0_RdFL(gbb_afe_0);

	switch(*inclk)
	{
		case LX_DEMOD_ARM_SETTING:
		{
			ACE_REG_M14A0_Wr01(gbb_afe_0, inck_sel, 0);
		}
		break;
		...
		case LX_DEMOD_EXTERN_PIN:
		{
			...
			ACE_REG_M14A0_Wr01(gbb_afe_0, inck_sel, 1);
		}
		break;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_AdcInit(void)
{
	DEMOD_PRINT("[KDRV_DEMOD] DEMOD_M14A0_AdcInit Start !! \n");

	ACE_REG_M14A0_RdFL(dmd_adc_2);

	ACE_REG_M14A0_Wr02(dmd_adc_2, dmd_adc_pdbm, 1, dmd_adc_pdb, 1 );

	ACE_REG_M14A0_WrFL(dmd_adc_2);

	if(lx_chip_rev() == LX_CHIP_REV(M14, A1))		  
	{    
	   
  		ACE_REG_M14A0_RdFL(dmd_adc_1);	 

 		ACE_REG_M14A0_Wr01(dmd_adc_1, dmd_adc_vrefsel, 0x3);    

		ACE_REG_M14A0_WrFL(dmd_adc_1);	 
	}    

	 


	DEMOD_PRINT("[KDRV_DEMOD] DEMOD_M14A0_AdcInit end !! \n");
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_RepeaterEnable(BOOLEAN bEnable)
{
	UINT8 ni2crpten;
	...
	DEMOD_PRINT("^r^[DEMOD M14A0] success!!  DVB_RepeaterEnable =  %s ", (bEnable == TRUE)?"ON":"OFF");
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_SoftwareReset(void)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Serial_Control(BOOLEAN bEnable)
{
	...
	DEMOD_PRINT("^r^[DEMOD M14A0 DTV] success!!  DEMOD_M14A0_VQI_Serial_Control =  %s ", (bEnable == TRUE)? "SERIAL":"PARALLEL");
...
int DEMOD_M14A0_Power_Save(BOOLEAN bEnable)
{

	UINT8 	gbbpdb;
	UINT8	softrstb;

	DEMOD_PRINT("[DEMOD M14A0 DTV] @ ######chungiii   %s =====\n",__F__);
	...
	DEMOD_PRINT("[DEMOD M14A0] success!!  DEMOD_M14A0_Power_Save =  %s  %d\n ", (bEnable == TRUE)? "ON":"OFF", gbbpdb);
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_StdOperModeContrl( LX_DEMOD_OPER_MODE_T operMode, LX_DEMOD_BWMODE_T bandWd, LX_DEMOD_SYMRDET_MODE_T symbr)
{
	UINT8 	stdopmode_7_0;
	...
	switch(operMode)
	{
		...
		default :
			DEMOD_PRINT("[DEMOD M14A0](%s,%d) Invaild Opermode parameter !!!\n",__F__, __L__);
			return RET_ERROR;
	}
	...
	switch(bandWd)
	{
		...
		default :
			DEMOD_PRINT("[DEMOD M14A0](%s,%d) Invalid Bandwidth parameter !!!\n",__F__, __L__);
			return RET_ERROR;
	}
	...
	switch(symbr)
	{
		...
		default :
			DEMOD_PRINT("[DEMOD M14A0](%s,%d) Invalid Symbol rate parameter !!!\n",__F__, __L__);
			return RET_ERROR;
	}
	...
	*/
	...
	DEMOD_PRINT("[DEMOD M14A0] Sampling frequency is %d [MHz] (%d)!!!\n",samplingFreq/65536, samplingFreq );
 ...
 * @author 	Jeongpil.yun(jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_NeverlockScan(DEMOD_CTX_T *pDemodInfo, LX_DEMOD_FLAG_T ResetOn, LX_DEMOD_FLAG_T *pScanProcessing, LX_DEMOD_LOCK_STATE_T *pScanLock)
{
		...
		case LX_DEMOD_IDLE :
			...
			if(elapsedTime > _maxTimeOut)
			{
				*pScanProcessing	= LX_DEMOD_FLAG_FINISH;
				...
				DEMOD_PRINT( "[DEMOD M14A0] NEVERSCAN : LX_DEMOD_NEVERLOCK DETECTION is on initializing \n");
		...
		case LX_DEMOD_DETECTION_PROCESSING:
			{
				...
				if(elapsedTime > _maxTimeOut)
				{
					*pScanProcessing 	= LX_DEMOD_FLAG_FINISH;
					_demodScanLock		= LX_DEMOD_UNLOCKED;
					DEMOD_PRINT( "[DEMOD M14A0] NEVERSCAN : LX_DEMOD_NEVERLOCK DETECTION is on processing \n");
		...
		case LX_DEMOD_DETECTION_FAIL:
			{
				*pScanProcessing 	= LX_DEMOD_FLAG_FINISH;
				_demodScanLock		= LX_DEMOD_UNLOCKED;
				DEMOD_PRINT( "[DEMOD M14A0] NEVERSCAN :  LX_DEMOD_NEVERLOCK DETECTION FAIL  \n");
			}
			break;
		...
		case LX_DEMOD_DETECTED:
			{
				*pScanProcessing 	= LX_DEMOD_FLAG_FINISH;
				...
				DEMOD_PRINT( "[DEMOD M14A0] NEVERSCAN :  LX_DEMOD_NEVERLOCK DETECTED  !! \n");
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_TPOutEnable(BOOLEAN bEnable)
{

	UINT8 	tpouten;

	DEMOD_PRINT(" @DEMOD_M14A0_TPOutCLKEnable\n");
	...
	DEMOD_TRACE(" DEMOD_M14A0_TPOutEnable(%s)\n",(bEnable == TRUE) ? "ENABLE":"DISABLE");
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Set_IF_Frq(UINT32 ifFrq)
{
	...
	DEMOD_PRINT(" @DEMOD_M14A0_Set_IF_Frq\n");
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Set_AGCPolarity(LX_DEMOD_AGC_POLAR_T agcPola)
{

	UINT8 	agcoutpol;

	DEMOD_PRINT(" @DEMOD_M14A0_DVBT_Set_AGCPolarity\n");
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Set_SpectrumCtrl(BOOLEAN bAutoEnable, BOOLEAN bSpecInvEnable)
{
	...
	if(bAutoEnable == TRUE)
	{
		...
		Common_WrFL(common_ctrl2);
		DEMOD_PRINT( "[DEMOD M14A0 DTV] Success  AUTO -> %s !!!\n", (bAutoEnable == TRUE)?"ON":"OFF");
	}
	else
	{
		...
		DEMOD_PRINT( "[DEMOD M14A0 DTV] Success Auto -> manual, Inversion ->  %s !!!\n",(bSpecInvEnable == TRUE)?"ON":"OFF");
	}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_Id(UINT16 *pId)
{
...
************************************************************************************/
/**
 * DEMOD_M14A0_Get_SpectrumInvStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_SpectrumStatus(LX_DEMOD_LOCK_STATE_T *pSpectrumAuto, LX_DEMOD_LOCK_STATE_T *pSpectrumInv)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_IFAGC(UINT16 *pIfAgc)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_OperMode(LX_DEMOD_OPER_MODE_T *pOperMode)
{
...
************************************************************************************/
/**
 * DEMOD_M14A0_Get_NeverLockStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_NeverLockStatus(LX_DEMOD_OPER_MODE_T scanMode, LX_DEMOD_LOCK_STATE_T *pNeverLockStatus)
{
	UINT8 neverlock_1_0;
	UINT8 srdstatus_1_0;
	UINT8 tpslockeq_d;
	...
	*pNeverLockStatus = LX_DEMOD_UNKOWN_STATE;
	...
	DEMOD_PRINT("[DEMOD M14A0](%s) neverlock_1_0 = %d !!!\n", __F__,neverlock_1_0);
	DEMOD_PRINT("[DEMOD M14A0](%s) srdstatus_1_0 = %d !!!\n", __F__,srdstatus_1_0);
	DEMOD_PRINT("[DEMOD M14A0](%s) tpslockeq_d = %d !!!\n", __F__,tpslockeq_d);

	if(scanMode == LX_DEMOD_QAM)
	{
		if((neverlock_1_0 == 3) || (srdstatus_1_0 == 3))
		{
			DEMOD_PRINT("[DEMOD M14A0](%s)neverlock_1_0[%d] srdstatus_1_0[%d]!!!\n", __F__,neverlock_1_0,srdstatus_1_0);
			neverlock_1_0 = 3;
		}
	}
	else if(scanMode == LX_DEMOD_ISDBT)
	{
		...
		{
			DEMOD_PRINT("[DEMOD M14A0](%s)neverlock_1_0[%d] tpslockeq_d[%d]!!!\n", __F__,neverlock_1_0,tpslockeq_d);
			neverlock_1_0 = 1;
		}
	...
	DEMOD_PRINT("[DEMOD M14A0](%s) SYSLOCK = %s !!!\n", __F__,(syslock == 1) ? "LOCK" :"NOT LOCK");
...
************************************************************************************/
/**
 * DEMOD_M14A0_Get_CarrierFreqOffset
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_CarrierFreqOffset(SINT32 *pFreqOff)
{
		...
		*pFreqOff = carrierOffset;
		...
		DEMOD_PRINT("[DEMOD M14A0] Carrier Offset freq = %d [KHz] !!!\n",*pFreqOff);
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_TPIFStatus(BOOLEAN *pbEnable)
{
	UINT8 	tpifen;
	...
	DEMOD_PRINT("[DEMOD M14A0] TP IF function is %s!!\n",(*pbEnable == TRUE) ? "Active" : "Inactive");
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_VABER(UINT32 *pVber)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_Packet_Error(UINT32 *pError)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_BandWidthMode( LX_DEMOD_BWMODE_T *ChannelBW )
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_QAMMode( LX_DEMOD_RF_MODE_T *pQamMode )
{
	...
	*pQamMode = LX_DEMOD_UNKOWN_MODE;
	...
	switch(mode)
	{
		...
		default :
			DEMOD_PRINT("[DEMOD M14A0] FAIL,(%s:%d) UNKNOWN RF MODE !!!\n", __F__, __L__);
			return RET_ERROR;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_MseInfo( LX_DEMOD_MSE_T *pSigMse)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_Get_Lock(LX_DEMOD_LOCK_STATUS_T whatLock, LX_DEMOD_LOCK_STATE_T *lockState)
{
...
//	DEMOD_PRINT("[DEMOD M14A0 DTV] @ %s =====\n",__F__);
	...
	if (RET_OK != DEMOD_M14A0_Get_OperMode(&operMode))
	{
		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_GetOperMode() !!!\n", __F__, __L__);
		...
		case LX_DEMOD_PSYNC_LOCK:
			{
				...
				{
//					DEMOD_M14A0_DVBT_Get_IFO_LOCK(&ifoLock);
...
int DEMOD_M14A0_VSB_Get_SysLockTime(UINT16 *pSyslocktime)
{
...
int DEMOD_M14A0_VSB_Get_FecLockTime(UINT16 *pFeclocktime)
{
...
int DEMOD_M14A0_ANALOG_ResetHW(void)
{
...
////
	ACE_REG_M14A0_RdFL(bbdemod_control_0);

	ACE_REG_M14A0_Wr01(bbdemod_control_0, bbdemod_control, 0);
...
////
	...
	ACE_REG_M14A0_RdFL(abb_1);

	ACE_REG_M14A0_Wr01(abb_1, abb_reset, 1);

	ACE_REG_M14A0_WrFL(abb_1);

	OS_MsecSleep(10);


	ACE_REG_M14A0_Wr01(abb_1, abb_reset, 0);
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ANALOG_SetI2C(void)
{
	DEMOD_PRINT("[KDRV_DEMOD] DEMOD_M14A0_ANALOG_SetI2C Start !! \n");

	ACE_REG_M14A0_RdFL(abb_0);

	ACE_REG_M14A0_Wr01(abb_0, abb_i2csel, 0);

	ACE_REG_M14A0_WrFL(abb_0);

	DEMOD_PRINT("[KDRV_DEMOD] DEMOD_M14A0_ANALOG_SetI2C end !! \n");
	return RET_OK;
}
...
************************************************************************************/
/**
 * DEMOD_M14A0_ANALOG_Set_AbbMode.
 * select inclk mode
 *
 * @param inclk  [IN] LX_DEMOD_INCLK_SEL_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_M14A0_ANALOG_Set_AbbMode(BOOLEAN bIsAbbMode)
{

	DEMOD_PRINT("[KDRV_DEMOD] DEMOD_M14A0_ANALOG_Set_AbbMode Start !! \n");


	if(TRUE == bIsAbbMode)
	{
		ACE_REG_M14A0_RdFL(bbdemod_control_0);	 // slave/reg addr = 0x10/0x46
		
		ACE_REG_M14A0_Wr01(bbdemod_control_0, bbdemod_control, 0); //0 : ABB, 1: GBB
		
		ACE_REG_M14A0_WrFL(bbdemod_control_0);


#if 0
		ACE_REG_M14A0_RdFL(abb_2);  // slave/reg addr = 0x12/0x15  --> handed over to CVD part 
		
		ACE_REG_M14A0_Wr01(abb_2, abb_mux_sel, 0);
		
		ACE_REG_M14A0_WrFL(abb_2);


		ACE_REG_M14A0_RdFL(abb_3);   // slave/reg addr = 0x12/0x16   --> handed over to audio part
		
		ACE_REG_M14A0_Wr01(abb_3, abb_mux_sel_aad, 0);
	...
	else
	{
		...
		ACE_REG_M14A0_Wr01(bbdemod_control_0, bbdemod_control, 1); //0 : ABB, 1: GBB
		
		ACE_REG_M14A0_WrFL(bbdemod_control_0);


#if 0
		ACE_REG_M14A0_RdFL(abb_2);  // slave/reg addr = 0x12/0x15
		
		ACE_REG_M14A0_Wr01(abb_2, abb_mux_sel, 1);//0 : ABB, 1: GBB
		
		ACE_REG_M14A0_WrFL(abb_2);


		ACE_REG_M14A0_RdFL(abb_3);   // slave/reg addr = 0x12/0x16
		
		ACE_REG_M14A0_Wr01(abb_3, abb_mux_sel_aad, 1);//0 : ABB, 1: GBB
	...
	DEMOD_PRINT("[KDRV_DEMOD] DEMOD_M14A0_ANALOG_Set_AbbMode End !! \n");
...
int DEMOD_M14A0_ANALOG_Set_WorkAround(void)
{
	UINT8	adcclkpllsel;

	
	DEMOD_PRINT("[KDRV_DEMOD] DEMOD_M14A0_ANALOG_Set_WorkAround Start !! \n");

	ACE_REG_M14A0_RdFL(abb_1);

	ACE_REG_M14A0_Wr01(abb_1, abb_reset, 1);  //1. abb enable

	ACE_REG_M14A0_WrFL(abb_1);

	ACE_REG_M14A0_RdFL(dmd_adc_2);	 // slave/reg addr = 0x12/0x12
	
	ACE_REG_M14A0_Wr01(dmd_adc_2, dmd_adc_pdbm, 1); // ADC power down (0: power down, 1: power on) <- from top register
	
	ACE_REG_M14A0_WrFL(dmd_adc_2);

	
	adcclkpllsel = 0x0;
	Common_RdFL(common_ctrl4); // regAddr = 0x0004[3]
	Common_Wr01(common_ctrl4, adcclkpllsel, adcclkpllsel);   // x-tal, or pLL  select ->xtal !! 
	Common_WrFL(common_ctrl4);

	DEMOD_PRINT("[KDRV_DEMOD]  DEMOD_M14A0_ANALOG_Set_WorkAround End !! \n");
