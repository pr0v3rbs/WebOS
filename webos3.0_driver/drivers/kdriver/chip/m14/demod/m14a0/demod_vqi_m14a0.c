 ...
 ************************************************************************************/
 ...
 UINT16 _gM14A0GBB_FullVQIPkerrCount = 7;
UINT16 _gM14A0GBB_FullVQPkerrThreshold = 300;
UINT16 _gM14A0GBB_FullISDBTPkerrThreshold = 200;

UINT8	_gM14A0GBB_Phn_state = 0;

UINT8 gM14A0GBB_PowerOnInitISDBT = 1;
unsigned int gM14A0GBB_PowerOnInitStartTimeISDBT = 0;

UINT16	_gM14A0GBB_DelayForStableTime = LX_DEMOD_LOCK_DEALY_200;
UINT8	_gM14A0GBB_chdiffTh = 0xf;
BOOLEAN _gM14A0GBB_tpblockon = FALSE;


/***********************************************************************************
	Static Variable & Function Prototypes Declarations
 ************************************************************************************/
static UINT16	_gM14A0GBB_GBB_VQI_UnlockCount_full_packeterr = 0;
...
//--------------------------------------------------------------------
//static int _DEMOD_M14A0_VSB_Get_NarrowPacket_Error(UINT32 *pError);
...
*/
int DEMOD_M14A0_GetMSE(UINT16 *pMSE)
{
	...
	*pMSE = tempMseValue;
	DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] check MSE chungiii### [%d](%s:%d)  \n",tempMseValue, __F__, __L__);
...
************************************************************************************/
/**
 * DEMOD_M14A0_VSB_Set_DefaultRegisterValue.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_VSB_Set_DefaultRegisterValue(void)
{
...
//	EQ_V_WrFL(eq_v_ctrl28);
	...
	adcsel1v = 1;
	adcclkpllsel = 0;
	adcmsbinv = ((lx_chip_rev() == LX_CHIP_REV( M14, A0)) ? 0 : 1);
	Common_RdFL(common_ctrl4);   //regAddr = 0x0004[7], COMMON_CTRL[4]
	Common_Wr03(common_ctrl4, adcsel1v, adcsel1v, adcclkpllsel, adcclkpllsel,adcmsbinv,adcmsbinv); //for M14
	...
	_gM14A0GBB_GBB_VQI_UnlockCount_full_packeterr = 0;
	_gM14A0GBB_Phn_state = 0;
	_gM14A0GBB_chdiffTh = 0xf;
	_gM14A0GBB_tpblockon = FALSE;

#if 0
#endif
	if (RET_OK != DEMOD_M14A0_TPOutEnable(TRUE))
	{
...
************************************************************************************/
/**
 * DEMOD_M14A0_VSB_Set_NeverLockWaitTime
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_VSB_Set_NeverLockWaitTime(UINT32 waitMs)
{
	UINT8 scsyslockchktime_7_0;

	DEMOD_PRINT("[DEMOD M14A0 DTV] @ %s =====\n",__F__);
...
************************************************************************************/
/**
 * DEMOD_M14A0_VSB_CochannelExist.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_VSB_CochannelExist(BOOLEAN *pCochannel)
{
...
int DEMOD_M14A0_VSB_PreMonitor(void)
{
...
//	UINT8	npncombw;
	...
	//printk(" ###chungiii DEBUG ## PRE MONITOR  _gM14A0GBB_chdiffTh value[0x%x]\n", _gM14A0GBB_chdiffTh);
	...
	if( (mainstrong == 1) && (chdiff_7_0>_gM14A0GBB_chdiffTh) && (snrref_5_0<=0x12))
	{
		_gM14A0GBB_tpblockon = TRUE;
		DEMOD_TRACE(" ###chungiii DEBUG ## _gM14A0GBB_tpblockon  TRUE\n");
	...
	{
		_gM14A0GBB_tpblockon = FALSE;
		DEMOD_TRACE(" ###chungiii DEBUG ## _gM14A0GBB_tpblockon  FALSE\n");
		DEMOD_TRACE(" ###chungiii DEBUG ## mainstrong[%d]\n",mainstrong );
		DEMOD_TRACE(" ###chungiii DEBUG ## delaydiff_7_0[0x%x]\n", delaydiff_7_0);
		DEMOD_TRACE(" ###chungiii DEBUG ## chdiff_7_0 [0x%x]\n", chdiff_7_0);
		DEMOD_TRACE(" ###chungiii DEBUG ## snrref_5_0 [0x%x]\n", snrref_5_0);
...
//	Common_Rd01(common_mon16,aiccrejstatus_3_0,aiccrejstatus_3_0);
	...
	if((mainstrong==1) && (dlyspread_7_0<0xc0) && (preghost==0))
	{
		...
		//if(chdiff_7_0>0x7)
		if(chdiff_7_0>_gM14A0GBB_chdiffTh)
		{
...
//		EQ_V_WrFL(eq_v_ctrl48);
	...
	if(_gM14A0GBB_chdiffTh == 0xf)
	{
		_gM14A0GBB_chdiffTh = 0x12;
	}
	else if(_gM14A0GBB_chdiffTh == 0x12)
	{
		_gM14A0GBB_chdiffTh = 0x15;
	}
	else if(_gM14A0GBB_chdiffTh == 0x15)
	{
		_gM14A0GBB_chdiffTh = 0x18;
	}
	else
	{
		_gM14A0GBB_chdiffTh = 0x18;
...
int DEMOD_M14A0_VSB_Monitor(DEMOD_CTX_T *pDemodInfo)
{
...
//	DEMOD_M14A0_Get_Packet_Error(&packeterror); //jeongpil.yun--> packet error  should be read at once within one second !! this line conflicts with other functions.
	...
	EQ_V_Rd01(eq_v_mon17, ncombdet, ncombdet);
	//printk(" ###chungiii DEBUG ## NTSC IS DETECTED [%s][%d]\n",ncombdet ? "NO" : "YES", ncombdet);
	//printk(" ###chungiii _gM14A0GBB_DelayForStableTime [%d]\n",_gM14A0GBB_DelayForStableTime);
	...
	else
	{
		if(tperrcount25ms>0)
		{
			DEMOD_TRACE(" ###chungiii DEBUG ## tperrcount25ms>0  _gM14A0GBB_Phn_state=0\n");
			_gM14A0GBB_Phn_state = 0;
		}
		else
		{
			DEMOD_TRACE(" ###chungiii DEBUG ## tperrcount25ms<0  _gM14A0GBB_Phn_state=1\n");
			_gM14A0GBB_Phn_state = 1;
		}
...
************************************************************************************/
/**
 * DEMOD_M14A0_VSB_Get_MSEdynStatus.
...
int DEMOD_M14A0_VSB_Get_MSEdynStatus(BOOLEAN *pbEnable)
{
	UINT8 	msedyn;
	UINT8	tpmainstrongphn;

	DEMOD_PRINT("[DEMOD M14A0] == @ %s!!!\n",__F__);
	...
	DEMOD_PRINT("[DEMOD M14A0] MSEdynStatus is %s!!\n",(*pbEnable == TRUE) ? "Enable" : "Disable");
	DEMOD_PRINT("[DEMOD M14A0] tpmainstrongphn is [%d]!!\n",tpmainstrongphn);
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_VSB_Get_TotalInfo( void)
{
...
//	LX_DEMOD_VSB_AUTO_CTRL_SIG_STATUS 	bcMonSignal;
	...
	if (RET_OK  != DEMOD_M14A0_Get_NeverLockStatus(LX_DEMOD_ATSC, &chScanStatus))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_Get_NeverLockStatus() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_M14A0_GetOperMode(&operMode))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_GetOperMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_M14A0_Get_IFAGC(&ifAGC))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_Get_IFAGC() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_M14A0_Get_Lock(LX_DEMOD_CARR_LOCK, &carrLock))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_Get_Lock() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_M14A0_Get_Lock(&syncLock))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_Get_Lock() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_M14A0_Get_CarrierFreqOffset( &offsetFrq))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_M14A0_VSB_CochannelExist( &cochannel))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_VSB_CochannelExist() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

 	//MSE

	if (RET_OK != DEMOD_M14A0_Get_MseInfo(&sigMse))
	{
		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_DVBC_Get_SNR() !!!\n", __F__, __L__);
		return RET_ERROR;
	}


	if (RET_OK  != DEMOD_M14A0_GetCrAcquisionTime(&cracquisionTimeMs))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_GetTrAcquisionTime() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_M14A0_Get_VABER(&viterbiBer))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_Get_VABER() !!!\n", __F__, __L__);
		return RET_ERROR;
	}


	DEMOD_PRINT("[DEMOD M14A0 DTV] Channel status  = %s !!!\n",(chScanStatus == LX_DEMOD_DETECTION_PROCESSING) ? "not completed" :
	...
	DEMOD_PRINT("[DEMOD M14A0 DTV] Operation Mode  = %s !!!\n",(operMode == LX_DEMOD_64QAM) ? "64QAM" :
													 (operMode == LX_DEMOD_QAM256_MODE) ? "256QAM" :
													 (operMode == LX_DEMOD_VSB_MODE) ? "VSB" :  "unknown" );

	DEMOD_PRINT("[DEMOD M14A0 DTV] IF AGC Gain = 0x%x !!!\n",ifAGC);
	DEMOD_PRINT("[DEMOD M14A0 DTV] Cochannel (NTSC) is  %s !!!\n", (cochannel == TRUE) ? " existent" : " Non-existent");
	DEMOD_PRINT("[DEMOD M14A0 DTV] Carrier LOCK = %s (%d ms)!!!\n",(carrLock == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked", cracquisionTimeMs);
	DEMOD_PRINT("[DEMOD M14A0 DTV] Sync LOCK = %s !!!\n", (syncLock == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD M14A0 DTV] Carrier offset Frequency = %d [KHz] !!!\n",offsetFrq);

	DEMOD_PRINT("[DEMOD M14A0] mse = %d, constPwr = %d, !!!\n",sigMse.mse, sigMse.constPwr);
	DEMOD_PRINT("[DEMOD M14A0 DTV] Viterbi BER = %d !!!\n",viterbiBer);

#ifndef  DEMOD_PRINT
	printk("[DEMOD M14A0 DTV] Channel status  = %s !!!\n",(chScanStatus == LX_DEMOD_DETECTION_PROCESSING) ? "not completed" :
												(chScanStatus == LX_DEMOD_MODE_CHANGE) ? "mode change" :
												(chScanStatus == LX_DEMOD_NEXT_CHANNEL) ? "next channel" :
												(chScanStatus == LX_DEMOD_LOCKED) ? "channel exist" : "unkown" );

	printk("[DEMOD M14A0 DTV] Operation Mode  = %s !!!\n",(operMode == LX_DEMOD_64QAM) ? "64QAM" :
													 (operMode == LX_DEMOD_QAM256_MODE) ? "256QAM" :
													 (operMode == LX_DEMOD_VSB_MODE) ? "VSB" :	"unknown" );

	printk("[DEMOD M14A0 DTV] IF AGC Gain = 0x%x !!!\n",ifAGC);
	printk("[DEMOD M14A0 DTV] Cochannel (NTSC) is  %s !!!\n", (cochannel == TRUE) ? " existent" : " Non-existent");
	printk("[DEMOD M14A0 DTV] Carrier LOCK = %s (%d ms)!!!\n",(carrLock == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked", cracquisionTimeMs);
	printk("[DEMOD M14A0 DTV] Sync LOCK = %s !!!\n", (syncLock == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	printk("[DEMOD M14A0 DTV] Carrier offset Frequency = %d [KHz] !!!\n",offsetFrq);

	printk("[DEMOD M14A0] mse = %d, constPwr = %d, !!!\n",sigMse.mse, sigMse.constPwr);
	printk("[DEMOD M14A0 DTV] Viterbi BER = %d !!!\n",viterbiBer);
...
************************************************************************************/
/**
 * DEMOD_M14A0_VSB_Obtaining_Signal_Lock.
...
int DEMOD_M14A0_VSB_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_RETUNE_RF:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_M14A0_VSB_Set_DefaultRegisterValue())
				{
					DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_VSB_Set_DefaultRegisterValue() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}


				pDemodInfo->controlState = LX_DEMOD_TUNE_START;
				DEMOD_PRINT( "^R^[DEMOD M14A0] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_TUNE_START..... ^0^\n");
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_M14A0_SoftwareReset())
				{
					DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_SoftwareReset() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.vsb.tuneMode)
				{
					...
					if(RET_OK != DEMOD_M14A0_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_ON, &scanProcessing, &scanLock))
					{
						DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_NeverlockScan() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
					pDemodInfo->controlState = LX_DEMOD_TUNE_SCAN;
					DEMOD_PRINT( "^R^[DEMOD M14A0] LX_DEMOD_TUNE_START : Going to LX_DEMOD_TUNE_SCAN..... ^0^\n");
				}
				else
				{

					if (RET_OK != DEMOD_M14A0_TPOutEnable(FALSE))
					{
						DEMOD_PRINT( "^r^[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_TPOutEnable(FALSE) !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					pDemodInfo->scanJobFlag = LX_DEMOD_FLAG_FINISH;
					pDemodInfo->controlState = LX_DEMOD_SYNC_CHECKING;
					DEMOD_PRINT( "^R^[DEMOD M14A0] LX_DEMOD_TUNE_START : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
		...
		case LX_DEMOD_TUNE_SCAN:			/***************************************************/
			{
				...
				if((LX_DEMOD_FLAG_COMPLETE == pDemodInfo->scanJobFlag) || (LX_DEMOD_FLAG_FINISH == pDemodInfo->scanJobFlag))
				{
					...
					DEMOD_PRINT( "^c^[DEMOD M14A0] ERR (%s:%d) Already Done !!!\n", __F__, __L__);
					DEMOD_PRINT( "^R^[DEMOD M14A0] LX_DEMOD_TUNE_SCAN : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
					return RET_OK;
				}


				if(RET_OK != DEMOD_M14A0_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_OFF, &scanProcessing, &scanLock))
				{
					DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_NeverlockScan() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if((LX_DEMOD_FLAG_FINISH == scanProcessing) && (LX_DEMOD_LOCKED == scanLock))
				{
					...
					DEMOD_PRINT( "^G^[DEMOD M14A0] (%s:%d) Signal is Found  !!!\n", __F__, __L__);
					DEMOD_PRINT( "^R^[DEMOD M14A0] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
				...
				else if((LX_DEMOD_FLAG_FINISH == scanProcessing)&& (LX_DEMOD_UNLOCKED == scanLock))
				{
					...
					DEMOD_PRINT( "^G^[DEMOD M14A0] (%s:%d) Signal is NOT Found  !!!\n", __F__, __L__);
				...
				else
				{
					...
					DEMOD_PRINT( "^G^[DEMOD M14A0] (%s:%d) Detection is on processing	!!!\n", __F__, __L__);
				}
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				if(RET_OK != DEMOD_M14A0_Get_Lock(LX_DEMOD_PSYNC_LOCK, &lockState))
				{
					DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_Get_Lock() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					DEMOD_PRINT( "^R^[DEMOD M14A0] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_SIGNAL_STABLE..... ^0^\n");
				}
				else
				{
					if(RET_OK != DEMOD_M14A0_Get_Lock(LX_DEMOD_CARR_LOCK, &lockState))
					{
						DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_Get_Lock() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE: 		/********************************************/
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.vsb.tuneMode)
					{
						...
						else
						{
							...
							//if(RET_OK != DEMOD_M14A0_VSB_CochannelExist(&Cochannel, &staticGhost))
							//{
							//	DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_VSB_CochannelExist() !!!\n", __F__, __L__);
							//	DEMOD_RETURN_FAIL(__F__);
							//}
							#if 1
							if(RET_OK != DEMOD_M14A0_VSB_PreMonitor())
							{
								DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_VSB_PreMonitor() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							#endif

							//if(RET_OK != _DEMOD_M14A0_VSB_Get_NarrowPacket_Error(&packetError))
							if(RET_OK != DEMOD_M14A0_Get_Packet_Error(&packetError))
							{
								DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_Get_Packet_Error() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							else
							{
								...
								if( ((packetError < LX_DEMOD_PACKET_ERROR_THD_100) && (Cochannel == FALSE)) && (_gM14A0GBB_tpblockon == FALSE) )
								{
									//for debug chungiii
									//OS_MsecSleep(200);
									...
									if (RET_OK != DEMOD_M14A0_Get_CarrierFreqOffset(&freqOffset))
									{
										DEMOD_PRINT( "^r^[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
										DEMOD_RETURN_FAIL(__F__);
									}
									...
									DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] LX_DEMOD_SIGNAL_STABLE : MSG_ATSC_FRONT_END_LOCKED..... ^^^0^^^\n");
									DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^^0^^^\n", elapsedTime);
									DEMOD_PRINT( "^R^[DEMOD M14A0] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^^^0^^^\n", freqOffset);

									if (RET_OK != DEMOD_Get_TunedCFG(pDemodInfo))
									{
										DEMOD_PRINT( "^r^[DEMOD M14A0] FAIL (%s:%d) DEMOD_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
										DEMOD_RETURN_FAIL(__F__);
									}
									if (RET_OK != DEMOD_Get_signal_status(pDemodInfo))
									{
										DEMOD_PRINT( "^r^[DEMOD M14A0] FAIL (%s:%d) DEMOD_Get_signal_status() !!!\n", __F__, __L__);
										DEMOD_RETURN_FAIL(__F__);
									}
								...
								else
								{
									...
									if(Cochannel == TRUE)
									{
										printk( "^r^[DEMOD M14A0] (%s:%d) check() !!!\n", __F__, __L__);
										if(_gM14A0GBB_DelayForStableTime == LX_DEMOD_LOCK_DEALY_200)
										{
											printk("chungiiii ######### change to _gM14A0GBB_DelayForStableTime[400]\n");
											_gM14A0GBB_DelayForStableTime = LX_DEMOD_LOCK_DEALY_500;
											//break;
										}
									}
									...
									if(elapsedTime > LX_DEMOD_LOCK_DEALY_400)
									{
										...
										if(staticGhost == TRUE)
										{
											...
											if(_gM14A0GBB_DelayForStableTime == LX_DEMOD_LOCK_DEALY_200)
											{
												printk("chungiiii ######### change to _gM14A0GBB_DelayForStableTime[500]\n");
												_gM14A0GBB_DelayForStableTime = LX_DEMOD_LOCK_DEALY_400;
												break;
											}
										}
										#endif
										...
										DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] LX_DEMOD_SIGNAL_STABLE : should be MSG_ATSC_FRONT_END_LOCKED..... ^^0^^\n");
										DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^0^^\n", elapsedTime);
										DEMOD_PRINT( "^R^[DEMOD M14A0] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^^0^^\n", freqOffset);
				...
				{
					pDemodInfo->controlState	= LX_DEMOD_SYNC_CHECKING;
					DEMOD_PRINT( "^R^[DEMOD M14A0] LX_DEMOD_SYNC_CHECKING : Going back to LX_DEMOD_SYNC_CHECKING..... **;\n");
...
************************************************************************************/
/**
 * DEMOD_M14A0_VSB_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_M14A0_VSB_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	...
	if(RET_OK != DEMOD_M14A0_Get_Lock(LX_DEMOD_PSYNC_LOCK, &lockState))
	{
		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_Get_Lock() !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}
#if 0
...
#endif
#if 1
	if(RET_OK != DEMOD_M14A0_VSB_Monitor(pDemodInfo))
	{
		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_VSB_Monitor() !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if(pDemodInfo->signalStatus.packetError >= _gM14A0GBB_FullVQPkerrThreshold)
		{
			_gM14A0GBB_GBB_VQI_UnlockCount_full_packeterr++;
			DEMOD_PRINT( "^r^[DEMOD M14A0](%s) _gM14A0GBB_GBB_VQI_UnlockCount_full_packeterr = %d !!!\n", __F__,_gM14A0GBB_GBB_VQI_UnlockCount_full_packeterr);

			if(_gM14A0GBB_GBB_VQI_UnlockCount_full_packeterr == _gM14A0GBB_FullVQIPkerrCount)
			{
				DEMOD_PRINT( "^r^[DEMOD M14A0] oops !! (%s) Force to NOT LOCK !!\n", __F__);
				...
				if(RET_OK != DEMOD_M14A0_SoftwareReset())
				{
					DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_SoftwareReset() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				DEMOD_PRINT("[DEMOD M14A0](%s) DEMOD_SoftwareReset() !!!\n", __F__);
		...
		else
		{
			...
			if(pDemodInfo->lockCount == 2)
			{
				...
				pDemodInfo->bNotifiedUnlock = FALSE;
					...
					DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] MONITORING FUNC : MSG_ATSC_FRONT_END_LOCKED..... ^0^\n");
			...
			if (RET_OK != DEMOD_Get_TunedCFG(pDemodInfo))
			{
				DEMOD_PRINT( "^r^[DEMOD M14A0] FAIL (%s:%d) DEMOD_Get_TunedCFG() !!!\n", __F__, __L__);
				DEMOD_RETURN_FAIL(__F__);
			}
	...
	else
	{
		...
		if((LX_DEMOD_UNLOCKED != pDemodInfo->lockStatus)  && (TRUE != pDemodInfo->bNotifiedUnlock))
		{
			...
			if(pDemodInfo->unLockCount == 4)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] MONITORING FUNC : MSG_ATSC_FRONT_END_NOT_LOCKED..... -_-\n");
		...
		else
		{
			...
			if(pDemodInfo->unLockLongCount == 20)
			{
				...
				DEMOD_PRINT( "^R^[DEMOD M14A0] MONITORING FUNC  : Going to LX_DEMOD_RETUNE_RF..... ^0^\n");
...
/**
 * DEMOD_M14A0_Get_VSB_NarrowPacket_Error
...
************************************************************************************/
/**
 * DEMOD_M14A0_QAM_SoftwareResetFEC.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_QAM_SoftwareResetFEC(void)
{
	UINT8 	softrstfecb;
...
int DEMOD_M14A0_QAM_Monitor(void)
{
...
************************************************************************************/
/**
 * DEMOD_M14A0_QAM_ModeAutoDetection.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_QAM_ModeAutoDetection(BOOLEAN bEnable)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_QAM_64Mode(void)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_QAM_256Mode(void)
{
...
************************************************************************************/
/**
 * DEMOD_M14A0_QAM_Set_DefaultRegisterValue.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_QAM_Set_DefaultRegisterValue(void)
{
	...
	DEMOD_PRINT("[DEMOD M14A0 DTV] ####chungiii check !!! %s =====\n",__F__);
...
************************************************************************************/
/**
 * DEMOD_M14A0_QAM_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_QAM_Set_NeverLockWaitTime(UINT32 waitMs)
{
	UINT8 scsyslockchktime_7_0;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_QAM_Get_TotalInfo( void)
{
#if 0
	...
	if (RET_OK  != DEMOD_M14A0_Get_NeverLockStatus(LX_DEMOD_QAM, &chScanStatus))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_Get_NeverLockStatus() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	...
	if (RET_OK  != DEMOD_M14A0_Get_Lock(LX_DEMOD_TR_LOCK, &timingLock))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) LX_DEMOD_LOCK_STATE_T() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  != DEMOD_M14A0_Get_Lock(LX_DEMOD_FEC_LOCK, &fecLock))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_Get_Lock() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	...
	if (RET_OK  != DEMOD_M14A0_Get_SpectrumStatus(&spectrumAuto, &spectrumInv))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_Get_SpectrumStatus() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	if (RET_OK  !=  DEMOD_M14A0_GetTrAcquisionTime(&tracquisionTimeMs))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_GetTrAcquisionTime() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	...
	if (RET_OK  !=  DEMOD_M14A0_GetFecAcquisionTime(&fecacquisionTimeMs))
	{
		DEMOD_PRINT( "^r^[DEMOD M14A0 DTV] FAIL (%s:%d) DEMOD_M14A0_GetFecAcquisionTime() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
...
************************************************************************************/
/**
 * DEMOD_M14A0_QAM_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_M14A0_QAM_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_RETUNE_RF:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_M14A0_QAM_Set_DefaultRegisterValue())
				{
					DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_QAM_Set_DefaultRegisterValue() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.qam.tuneMode)
				{
					if(RET_OK != DEMOD_M14A0_QAM_ModeAutoDetection(TRUE))
					{
						DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_QAM_ModeAutoDetection() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				}
				else
				{
					if(RET_OK != DEMOD_M14A0_QAM_ModeAutoDetection(FALSE))
					{
						DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_QAM_ModeAutoDetection() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if(RET_OK != DEMOD_M14A0_Set_SpectrumCtrl(FALSE, pDemodInfo->setCfgParam.qam.bSpectrumInv))
					{
						DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_Set_SpectrumCtrl() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
					if(LX_DEMOD_64QAM == pDemodInfo->setCfgParam.qam.qamMode)
					{
						if(RET_OK != DEMOD_M14A0_QAM_64Mode())
						{
							DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_QAM_64Mode() !!!\n", __F__, __L__);
							DEMOD_RETURN_FAIL(__F__);
						}
					}
					else if(LX_DEMOD_256QAM == pDemodInfo->setCfgParam.qam.qamMode)
					{
						if(RET_OK != DEMOD_M14A0_QAM_256Mode())
						{
							DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_QAM_256Mode() !!!\n", __F__, __L__);
							DEMOD_RETURN_FAIL(__F__);
						}
					}
					else
					{
							DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) Invalid QAM Mode !!!\n", __F__, __L__);
							DEMOD_RETURN_FAIL(__F__);
					}
...
//				unsigned int 			elapsedTime;
				...
				if(RET_OK != DEMOD_M14A0_QAM_EQ_Signal_Detector(TRUE))
				{
					DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_QAM_EQ_Signal_Detector() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
...
//					{
					...
					pDemodInfo->controlState = LX_DEMOD_TUNE_SCAN;
					DEMOD_PRINT( "[DEMOD M14A0] LX_DEMOD_TUNE_START : (auto) Going to LX_DEMOD_TUNE_SCAN..... ^0^\n");
...
//					}
				...
				else
				{
					...
					DEMOD_PRINT( "[DEMOD M14A0] LX_DEMOD_TUNE_START :(manual) Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				if(RET_OK != DEMOD_M14A0_Get_Lock(LX_DEMOD_FEC_LOCK, &lockState))
				{
					DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_Get_Lock() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if(RET_OK != DEMOD_M14A0_QAM_EQ_Signal_Detector(FALSE))
					{
						DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_QAM_EQ_Signal_Detector() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
						...
						else
						{
							...
							else
							{
								if(packetError < LX_DEMOD_PACKET_ERROR_THD)
								{
									...
									DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] LX_DEMOD_SIGNAL_STABLE : MSG_QAM_FRONT_END_LOCKED..... ^^^0^^^\n");
									DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^^0^^^\n", elapsedTime);
									DEMOD_PRINT( "^R^[DEMOD M14A0] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^0^\n", freqOffset);
...
************************************************************************************/
/**
 * DEMOD_M14A0_QAM_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_M14A0_QAM_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if(pDemodInfo->signalStatus.packetError >= _gM14A0GBB_FullVQPkerrThreshold)
		{
			...
			if(_gM14A0GBB_GBB_VQI_UnlockCount_full_packeterr == _gM14A0GBB_FullVQIPkerrCount)
			{
				...
				DEMOD_PRINT("[DEMOD M14A0](%s) DEMOD_M14A0_QAM_ModeAutoDetection(TRUE) !!!\n", __F__);
		...
		else
		{
			...
			if(RET_OK != DEMOD_M14A0_QAM_EQ_Signal_Detector(FALSE))
			{
				...
				pDemodInfo->bNotifiedUnlock = FALSE;
					...
					DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] MONITORING FUNC : MSG_QAM_FRONT_END_LOCKED..... ^0^\n");
	...
	else
	{
		...
		if((LX_DEMOD_UNLOCKED != pDemodInfo->lockStatus)  && (TRUE != pDemodInfo->bNotifiedUnlock))
		{
			...
			if(pDemodInfo->unLockCount == 4)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] MONITORING FUNC : MSG_QAM_FRONT_END_NOT_LOCKED..... -_-\n");
		...
		else
		{
			...
			{
				pDemodInfo->controlState = LX_DEMOD_RETUNE_RF;
				DEMOD_PRINT( "^R^[DEMOD M14A0] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_RETUNE_RF..... ^0^\n");
...
/**
 * DEMOD_M14A0_QAM_EQ_Signal_Detector.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_QAM_EQ_Signal_Detector(BOOLEAN OnOff)
{
	UINT8	nousenosigdet;
...
/**
 * DEMOD_M14A0_ISDBT_SetDefaultRegisterValue
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Set_DefaultRegisterValue(void)
{
	...
	if(gM14A0GBB_PowerOnInitISDBT == 1)
	{
		gM14A0GBB_PowerOnInitISDBT = 2;
		...
		gM14A0GBB_PowerOnInitStartTimeISDBT = jiffies_to_msecs(jiffies);

	}
	else if(gM14A0GBB_PowerOnInitISDBT == 0)
	{
		crgearshen = 0x1;
		Common_RdFL(common_ctrl68);  //regAddr = 0x0044, COMMON_CTRL[68]
		Common_Wr01(common_ctrl68, crgearshen, crgearshen);
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Set_Config_auto
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Set_Config_auto (BOOLEAN bAutoDetect)
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Set_PartialConfig
 * Set hierachical signal path and bandwidth.
 * This function is used when DEMOD_M14A0_ISDBT_Set_Config_auto is enabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Set_PartialConfig (LX_DEMOD_ISDBT_CONFIG_T *pIsdbtConfigParam)
{
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when DEMOD_M14A0_ISDBT_Set_Config_auto is disabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Set_FullConfig (LX_DEMOD_ISDBT_CONFIG_T *pIsdbtConfigParam)
{
	...
	switch(pIsdbtConfigParam->FFTMode)
	{
		case LX_DEMOD_DVBT_ISDBT_FFT_2K :
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] Set FFT Mode = %s !!!\n","2K ");
			}
			break;
		...
		default :
			{
				mdfftmode_di_1_0 = 0x1;
				DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d)UNKNOWN FFT mode !!!\n",__F__, __L__);
			}
			return RET_ERROR;
	}
	...
	switch(pIsdbtConfigParam->gIMode)
	{
		case LX_DEMOD_DVBT_ISDBT_GIMODE_1_4 :
			{
				mdgimode_di_1_0 = 0x3;
				DEMOD_PRINT("[DEMOD M14A0] Set Guard Interval = %s !!!\n","1_4 ");
			}
			break;
		...
		default :
			{
				mdgimode_di_1_0 = 0x0;
				DEMOD_PRINT("[DEMOD M14A0] FAIL,(%s:%d) UNKNOWN Guard Interval  !!!\n", __F__, __L__);
			}
			return RET_ERROR;
	}
	...
	switch(pIsdbtConfigParam->tmcc.layerAQamMode)
	{
		case LX_DEMOD_QPSK:
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] Set QAM mode = %s !!!\n","QPSK");
			}
			break;
		...
		default :
			{
				tmccqammodea_i_2_0 = 0x0;
				DEMOD_PRINT("[DEMOD M14A0] FAIL,(%s:%d) UNKNOWN QAM mode !!!\n", __F__, __L__);
			}
			return RET_ERROR;
	}
	...
	switch(pIsdbtConfigParam->tmcc.layerACodeRate)
	{
		case LX_DEMOD_CODE_1_2 :
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] Set layer A code rate = %s !!!\n","1_2");
			}
			break;
		...
		default :
			{
				tmcccoderatea_i_2_0 = 0x0;
				DEMOD_PRINT("[DEMOD M14A0] FAIL,(%s:%d) UNKNOWN layer A  code rate !!!\n", __F__, __L__);
			}
			return RET_ERROR;
		...
		case LX_DEMOD_ISDBT_TIME_INTERLEAVING_0 :
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] Set layer A Lenth of time interleaving = %s !!!\n","0");
			}
			break;
		...
		default :
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] FAIL,(%s:%d) layer A  Lenth of time interleaving reserved !!!\n", __F__, __L__);
			}
			return RET_ERROR;
		...
		case LX_DEMOD_ISDBT_TIME_INTERLEAVING_0 :
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] Set layer B Lenth of time interleaving = %s !!!\n","0");
			}
			break;
		...
		default :
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] FAIL,(%s:%d) layer B  Lenth of time interleaving reserved !!!\n", __F__, __L__);
			}
			return RET_ERROR;
		...
		case LX_DEMOD_ISDBT_TIME_INTERLEAVING_0 :
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] Set layer C Lenth of time interleaving = %s !!!\n","0");
			}
			break;
		...
		default :
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] FAIL,(%s:%d) layer C  Lenth of time interleaving reserved !!!\n", __F__, __L__);
			}
			return RET_ERROR;
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Set_NeverLockWaitTime(UINT32 waitMs)
{
	UINT8 scsyslockchktime_7_0;
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Set_CrGearshEn
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Set_CrGearshEn(BOOLEAN bEnable)
{
	UINT8 crgearshen;
	...
	// 0 : disable
	// 1 : enable
	DEMOD_PRINT("[DEMOD M14A0 DTV](%s)(%s) \n",__F__, (bEnable == TRUE) ? "ON" : "OFF");
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Set_PhaseNoiseEnhancement
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Set_PhaseNoiseEnhancement(BOOLEAN bEnhanced)
{
	...
	if (RET_OK != DEMOD_M14A0_ISDBT_Get_FFTMode( &txModeLock, &fftMode ))
	{
		DEMOD_PRINT("[DEMOD_13] FAIL (%s:%d) DEMOD_M14A0_ISDBT_Get_FFTMode() !!!\n", __F__, __L__);

		return RET_ERROR;
	}

	if (RET_OK != DEMOD_M14A0_ISDBT_Get_DelaySpreadStatus(&delaySpread))
	{
		DEMOD_PRINT("[DEMOD_13] FAIL (%s:%d) DEMOD_M14A0_ISDBT_Get_DelaySpreadStatus() !!!\n", __F__, __L__);
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Get_DelaySpreadStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Get_DelaySpreadStatus(UINT16 *pDelaySpread)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Get_FFTMode( LX_DEMOD_LOCK_STATE_T *pTxModeLock, LX_DEMOD_FFTMODE_T *pFftMode )
{
	...
	*pFftMode = LX_DEMOD_DVBT_ISDBT_FFT_UNKNOWN;
	...
	if(modelock_di)
	{
		*pTxModeLock = LX_DEMOD_LOCKED;
		...
		DEMOD_PRINT("[DEMOD M14A0] TX mode LOCKED !!!\n");
	...
	{
		*pTxModeLock = LX_DEMOD_UNLOCKED;
		DEMOD_PRINT("[DEMOD M14A0] TX mode is NOT LOCKED !!!\n");
	}
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Get_GuradIntervalMode
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Get_GuradIntervalMode( LX_DEMOD_GIMODE_T *pGIMode )
{
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Get_TMCCInfo
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Get_TMCCInfo( LX_DEMOD_ISDBT_TMCC_CONFIG_T *pTmccParams)
{
	...
	if(NULL == pTmccParams)
	{
		DEMOD_PRINT("[DEMOD M14A0] ERROR, LX_DEMOD_ISDBT_TMCC_CONFIG_T pTmccParams is NULL !!!\n");
		return RET_ERROR;
	}
 	...
 	if (RET_OK != DEMOD_M14A0_ISDBT_Get_FFTMode( &txModeLock, &fftMode ))
 	{
 		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_ISDBT_Get_FFTMode() !!!\n", __F__, __L__);
 		return RET_ERROR;
	}
	...
	*/
		...
		case 0x0:
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] Get layer A  QAM mode = %s !!!\n","DQPSK");
			}
			break;
	...
	switch(tmcccoderatear_i_2_0)
	{
		case 0x0 :
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] Get layer A code rate = %s !!!\n","1_2");
			}
			break;
		...
		default :
			{
				DEMOD_PRINT("[DEMOD M14A0] UNKNOWN Code rate !!!\n");
			}
			break;
			...
			case 0x0 :
				{
					...
					DEMOD_PRINT("[DEMOD M14A0] Get layer A Lenth of time interleaving = %s !!!\n","0");
				}
				break;
			...
			default :
				{
					...
					DEMOD_PRINT("[DEMOD M14A0] Get layer A  Lenth of time interleaving reserved !!!\n");
				}
				break;
	...
	*/
		...
		case 0x0:
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] Get layer B  QAM mode = %s !!!\n","DQPSK");
			}
			break;
	...
	DEMOD_PRINT("[DEMOD M14A0] Get layer B	Seg # = %d !!!\n",pTmccParams->layerBSegmentNum);
		...
		case 0x0 :
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] Get layer B code rate = %s !!!\n","1_2");
			}
			break;
			...
			case 0x0 :
				{
					...
					DEMOD_PRINT("[DEMOD M14A0] Get layer B Lenth of time interleaving = %s !!!\n","0");
				}
				break;
			...
			default :
				{
					...
					DEMOD_PRINT("[DEMOD M14A0] Get layer B  Lenth of time interleaving reserved !!!\n");
				}
				break;
	...
	*/
		...
		case 0x0:
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] Get layer C  QAM mode = %s !!!\n","DQPSK");
			}
			break;
	...
	DEMOD_PRINT("[DEMOD M14A0] Get layer C	Seg # = %d !!!\n",pTmccParams->layerCSegmentNum);
		...
		case 0x0 :
			{
				...
				DEMOD_PRINT("[DEMOD M14A0] Get layer C code rate = %s !!!\n","1_2");
			}
			break;
			...
			case 0x0 :
				{
					...
					DEMOD_PRINT("[DEMOD M14A0] Get layer C Lenth of time interleaving = %s !!!\n","0");
				}
				break;
			...
			default :
				{
					...
					DEMOD_PRINT("[DEMOD M14A0] Get layer C  Lenth of time interleaving reserved !!!\n");
				}
				break;
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Get_TotalInfo
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Get_TotalInfo( void)
{
	...
	DEMOD_PRINT("[DEMOD M14A0 DTV] @ %s =====\n",__F__);
 	...
 	if (RET_OK != DEMOD_M14A0_Get_BandWidthMode( &channelBW ))
 	{
 		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_DVBT_Get_BandWidthMode() !!!\n", __F__, __L__);
 		return RET_ERROR;
 	}

 	isdbtConfigParam.channelBW = channelBW;
 	DEMOD_PRINT("[DEMOD M14A0] Bandwidth = %s !!!\n",
	...
	DEMOD_PRINT("[DEMOD M14A0] Tx mode = %s, FFTMode = %s  !!!\n",
		(txModeLock == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked",
	...
	if (RET_OK != DEMOD_M14A0_ISDBT_Get_GuradIntervalMode( &gIMode ))
	{
		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_DVBT_Get_GuradIntervalMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	isdbtConfigParam.gIMode = gIMode;
	DEMOD_PRINT("[DEMOD M14A0] Guard Intterval = %s  !!!\n",
	...
	if (RET_OK != DEMOD_M14A0_ISDBT_Get_TMCCInfo(&tmccParams))
	{
		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_ISDBT_GetTMCCInfo() !!!\n", __F__, __L__);
	}

	isdbtConfigParam.tmcc = tmccParams;

	if (RET_OK != DEMOD_M14A0_Get_Lock(LX_DEMOD_AGC_LOCK, &agcLockStatus))
	{
		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_Get_Lock() !!!\n", __F__, __L__);
	...
	{
		if (RET_OK != DEMOD_M14A0_Get_IFAGC(&ifagcValue))
		{
			DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_Get_IFAGC() !!!\n", __F__, __L__);
			ifagcValue = 0;
		}
	}

	scanMode = LX_DEMOD_ISDBT;
	if (RET_OK != DEMOD_M14A0_Get_NeverLockStatus(scanMode, &neverLockStatus))
	{
		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_Get_NeverLockStatus() !!!\n", __F__, __L__);
		neverLockStatus = LX_DEMOD_UNKOWN_STATE;
	}

	if (RET_OK !=  DEMOD_M14A0_Get_SpectrumStatus(&spectrumAuto, &spectrumInv))
	{
		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_Get_SpectrumStatus() !!!\n", __F__, __L__);
		spectrumInv = LX_DEMOD_UNKOWN_STATE;
	}

	if (RET_OK != DEMOD_M14A0_ISDBT_Get_DelaySpreadStatus(&delaySpread))
	{
		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_ISDBT_Get_DelaySpreadStatus() !!!\n", __F__, __L__);
		delaySpread = 0xffff;
	}

	//TP Error count
	if (RET_OK !=  DEMOD_M14A0_Get_Packet_Error(&errorCount))
	{
		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_Get_Packet_Error() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	DEMOD_PRINT("[DEMOD M14A0] TP Error Count= %d !!!\n",errorCount);

	if (RET_OK != DEMOD_M14A0_Get_CarrierFreqOffset( &FreqOff ))
	{
		DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
		FreqOff = 0xffff;
	}
	...
	DEMOD_PRINT("[DEMOD M14A0] AGC LOCK is %s !!!\n",(agcLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD M14A0] IF AGC value = %d(0x%x) !!!\n",ifagcValue,ifagcValue);
	DEMOD_PRINT("[DEMOD M14A0] Neverlock is %s !!!\n",(neverLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD M14A0] Spectrum mode = %s !!!\n",(spectrumInv == LX_DEMOD_NORMAL) ? " Normal" :
																	(spectrumInv == LX_DEMOD_INVERSION) ? "Inversion" : 	" Unknown");
	DEMOD_PRINT("[DEMOD M14A0] Delay Spread = %d !!!\n",delaySpread);
	DEMOD_PRINT("[DEMOD M14A0] FFT mode LOCK = %s !!!\n",(txModeLock == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD M14A0] offset = %d [KHz]!!!\n",FreqOff);
	DEMOD_PRINT("[DEMOD M14A0] mse = %d, constPwr = %d, !!!\n",sigMse.mse, sigMse.constPwr);

	DEMOD_PRINT("[DEMOD M14A0] AGC LOCK is %s !!!\n",(agcLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD M14A0] IF AGC value = %d !!!\n",ifagcValue);
	DEMOD_PRINT("[DEMOD M14A0] Neverlock is %s !!!\n",(neverLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD M14A0] Spectrum mode = %s !!!\n",(spectrumInv == LX_DEMOD_NORMAL) ? " Normal" :
																	(spectrumInv == LX_DEMOD_INVERSION) ? "Inversion" : 	" Unknown");
	DEMOD_PRINT("[DEMOD M14A0] Delay Spread = %d !!!\n",delaySpread);
	...
	// Get FFT mode
	DEMOD_PRINT("[DEMOD M14A0] FFTMode = %s  !!!\n",
	...
	DEMOD_PRINT("[DEMOD M14A0]  [Layer A] QAM mode  = %s  !!!\n",
	...
	DEMOD_PRINT("[DEMOD M14A0]  [Layer A] code rate = %s  !!!\n",
	...
	DEMOD_PRINT("[DEMOD M14A0]  [Layer A] time interleaveing len  = %s	!!!\n",
	...
	DEMOD_PRINT("[DEMOD M14A0] [Layer A] the Num of Segment   = %d	!!!\n",isdbtConfigParam.tmcc.layerASegmentNum);


	// Get Layer B QAM mode
	DEMOD_PRINT("[DEMOD M14A0]  [Layer B] QAM mode  = %s	!!!\n",
	...
	DEMOD_PRINT("[DEMOD M14A0]  [Layer B] code rate = %s	!!!\n",
	...
	DEMOD_PRINT("[DEMOD M14A0]  [Layer B] time interleaveing len	= %s	!!!\n",
	...
	DEMOD_PRINT("[DEMOD M14A0] [Layer B] the Num of Segment	= %d	!!!\n",isdbtConfigParam.tmcc.layerBSegmentNum);


	// Get Layer C QAM mode
	DEMOD_PRINT("[DEMOD M14A0]  [Layer C] QAM mode  = %s	!!!\n",
	...
	DEMOD_PRINT("[DEMOD M14A0]  [Layer C] code rate = %s	!!!\n",
	...
	DEMOD_PRINT("[DEMOD M14A0]  [Layer C] time interleaveing len	= %s	!!!\n",
	...
	DEMOD_PRINT("[DEMOD M14A0] [Layer C] the Num of Segment	= %d	!!!\n",isdbtConfigParam.tmcc.layerCSegmentNum);
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Get_IFO_LOCK
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Get_IFO_LOCK( BOOLEAN *pIfoLock)
{
	UINT8	syslock;
	...
	DEMOD_PRINT("[DEMOD M14A0](%s) IFO LOCK = %s !!!\n", __F__, (syslock == 1) ? "LOCK" :"NOT LOCK");
	DEMOD_PRINT("[DEMOD M14A0](%s) IFO value = %d !!!\n", __F__,intfreqoffsetvalue);
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Get_CochanDetIndicator
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_Get_CochanDetIndicator( BOOLEAN *pDetStatus)
{
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_EqualizereReset.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14A0_ISDBT_EqualizereReset(void)
{
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_M14A0_ISDBT_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_RETUNE_RF:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_M14A0_ISDBT_Set_DefaultRegisterValue())
				{
					DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_M14A0_ISDBT_Set_DefaultRegisterValue() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				{
					if(RET_OK != DEMOD_M14A0_ISDBT_Set_Config_auto(TRUE))
					{
						DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_M14A0_ISDBT_Set_Config_auto() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if(RET_OK != DEMOD_M14A0_ISDBT_Set_PartialConfig(&(pDemodInfo->setCfgParam.isdbt)))
					{
				...
				else
				{
					...
					if(RET_OK != DEMOD_M14A0_ISDBT_Set_PartialConfig(&(pDemodInfo->setCfgParam.isdbt)))
					{
						DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_M14A0_ISDBT_Set_PartialConfig() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_M14A0_SoftwareReset())
				{
					DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_M14A0_SoftwareReset() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.isdbt.tuneMode)
				{
					...
					if(RET_OK != DEMOD_M14A0_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_ON, &scanProcessing, &scanLock))
					{
						DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_M14A0_NeverlockScan() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
					pDemodInfo->controlState = LX_DEMOD_TUNE_SCAN;
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				if(RET_OK != DEMOD_M14A0_Get_Lock(LX_DEMOD_PSYNC_LOCK, &lockState))
				{
					DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_M14A0_Get_Lock() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.isdbt.tuneMode)
					{
						...
						else
						{
							if(RET_OK != DEMOD_M14A0_Get_Packet_Error(&packetError))
							{
								DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_M14A0_Get_Packet_Error() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							else
							{
#if 0
								if(RET_OK != DEMOD_M14A0_ISDBT_Set_PhaseNoiseEnhancement(TRUE))
								{
									DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_M14A0_ISDBT_Set_PhaseNoiseEnhancement() !!!\n", __F__, __L__);
									DEMOD_RETURN_FAIL(__F__);
								}
#endif
								if(packetError < LX_DEMOD_PACKET_ERROR_THD)
								{
									...
									if (RET_OK != DEMOD_M14A0_Get_CarrierFreqOffset(&freqOffset))
									{
										DEMOD_PRINT( "^r^[DEMOD] FAIL (%s:%d) DEMOD_M14A0_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
										DEMOD_RETURN_FAIL(__F__);
									}
									pDemodInfo->freqOffset = freqOffset;
									...
									DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] LX_DEMOD_SIGNAL_STABLE : MSG_ISDBT_FRONT_END_LOCKED..... ^^^0^^^\n");
									DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^^0^^^\n", elapsedTime);
									DEMOD_PRINT( "^R^[DEMOD M14A0] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^^^0^^^\n", freqOffset);
...
************************************************************************************/
/**
 * DEMOD_M14A0_ISDBT_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_M14A0_ISDBT_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if(pDemodInfo->signalStatus.packetError >= _gM14A0GBB_FullISDBTPkerrThreshold)
		{
			_gM14A0GBB_GBB_VQI_UnlockCount_full_packeterr++;
			DEMOD_PRINT( "^r^[DEMOD M14A0](%s) _gM14A0GBB_GBB_VQI_UnlockCount_full_packeterr = %d !!!\n", __F__,_gM14A0GBB_GBB_VQI_UnlockCount_full_packeterr);
		...
		else
		{
			...
			if(gM14A0GBB_PowerOnInitISDBT == 2)
			{

				powerOnElapsedTime = jiffies_to_msecs(jiffies) - gM14A0GBB_PowerOnInitStartTimeISDBT;

				DEMOD_PRINT("[DEMOD M14A0](%s:%d) powerOnElapsedTime = %d !!!\n", __F__, __L__,powerOnElapsedTime);

				if(powerOnElapsedTime >= 60000)
				{
					gM14A0GBB_PowerOnInitISDBT = 0;

					if (RET_OK != DEMOD_M14A0_ISDBT_Set_CrGearshEn(TRUE ))
					{
						DEMOD_PRINT("[DEMOD M14A0] FAIL (%s:%d) DEMOD_M14A0_ISDBT_Set_CrGearshEn() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				}
			}
			...
			if(pDemodInfo->lockCount == 1)
			{
				...
				pDemodInfo->bNotifiedUnlock = FALSE;
					...
					DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] MONITORING FUNC : MSG_ISDBT_FRONT_END_LOCKED..... ^0^\n");
	...
	else
	{
		...
		if((LX_DEMOD_UNLOCKED != pDemodInfo->lockStatus)  && (TRUE != pDemodInfo->bNotifiedUnlock))
		{
			...
			if(pDemodInfo->unLockCount == 2)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD M14A0] MONITORING FUNC : MSG_ISDBT_FRONT_END_NOT_LOCKED..... -_-\n");
