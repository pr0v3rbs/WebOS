 ...
 ************************************************************************************/
 ...
 UINT16 _gM14B0GBB_FullDVBPkerrCount = 9;
UINT16 _gM14B0GBB_FullDVBPkerrThreshold = 200;

extern UINT8 gM14B0GBB_PowerOnInit;
unsigned int 			gM14B0GBB_PowerOnInitStartTimeDVB = 0;


/***********************************************************************************
	Static Variable & Function Prototypes Declarations
 ************************************************************************************/
static	UINT16			_gM14B0GBB_DVB_UnlockCount_full_packeterr	= 0;


/***********************************************************************************
************************************************************************************/
/**
 * DEMOD_M14B0_DVBC_AutoSymbolRateDet
...
*/
int DEMOD_M14B0_DVBC_AutoSymbolRateDet(LX_DEMOD_SYMRDET_MODE_T symrateMode, UINT16 SymbolRate )
{
	...
	switch(symrateMode)
	{
		...
		case LX_DEMOD_SYMR_FIXED_MODE :
		{
			...
			srdfbsymrate_16 = 0;
			DEMOD_PRINT("[DEMOD M14 GBB] symbol_temp = 0x%llx \n",symbol_temp);
			DEMOD_PRINT("[DEMOD M14 GBB] srdfbsymrate0_15_8 = 0x%x \n",srdfbsymrate_15_8);
			DEMOD_PRINT("[DEMOD M14 GBB] srdfbsymrate0_7_0 = 0x%x \n",srdfbsymrate_7_0);
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBC_IsSymbolRateAuto
 * Enable/Diable automatic symbol rate detection on DVB-C mode.
 *
 * @param  *bAuto - BOOLEAN.
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBC_IsSymbolRateAuto(BOOLEAN *bAuto)
{
	...
	{
		*bAuto = FALSE;
		DEMOD_PRINT("[DEMOD M14 GBB] Auto Symbolrate detection is disabled  !!!\n");
	}
	else
	{
		*bAuto = TRUE;
		DEMOD_PRINT("[DEMOD M14 GBB] Auto Symbolrate detection is enabled  !!!\n");
	}
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBC_IsQammodeAutoDet
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBC_IsQammodeAutoDet (BOOLEAN *bAutoQam)
{
	...
	{
		*bAutoQam = TRUE;
		DEMOD_PRINT("[DEMOD M14 GBB] Auto QAM mode detection is enabled  !!!\n");
	}
	else
	{
		*bAutoQam = FALSE;
		DEMOD_PRINT("[DEMOD M14 GBB] Auto QAM mode detection is disabled  !!!\n");
	}
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBC_Set_DefaultRegisterValue
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBC_Set_DefaultRegisterValue(void)
{
	...
	UINT8 crgearshen;
	UINT8 samgsend_cqs_3_0;
	UINT8 samgsstart_cqs_3_0;
...
//	unsigned int			powerOnElapsedTime;
	...
	Common_Wr03(common_ctrl4, adcvrefsel_0, adcvrefsel_0, adcclkpllsel, adcclkpllsel, adcisel_2_0, adcisel_2_0);
	...
	// ADCISEL[2:0] : Selection of ADC bias current
	// 000 : 70%
	// 001 : 80%
	// 010 : 90%
	// 011 : 100%
	...
	daften = 1;  //DAFTEN
	Common_RdFL(common_ctrl10);   //regAddr = 0x000a, COMMON_CTRL[10]
	Common_Wr01(common_ctrl10, daften, daften);
	...
	agcrefmode = 0;
	Common_RdFL(common_ctrl18);   //regAddr = 0x0012, COMMON_CTRL[18]
	Common_Wr01(common_ctrl18, agcrefmode, agcrefmode);
	Common_WrFL(common_ctrl18);
	//AGCREFMODE :Selection of AGC reference value source
	// 0 : use internally pre-defined value
	// 1 : use the value of AGCREF(@0x15~0x0x16)

	agcref_15_8 = 0x1e;
	Common_RdFL(common_ctrl21);   //regAddr = 0x0015, COMMON_CTRL[21]
	Common_Wr01(common_ctrl21, agcref_15_8, agcref_15_8);
	Common_WrFL(common_ctrl21);
	//AGCREF[15:8] :Set of reference value for gain error calculation
	// The reference value is compared with the power of incoming signal for gain error calculation
	// reference value = AGCREF & "000"

	agcref_7_0 = 0xe4;
	Common_RdFL(common_ctrl22);   //regAddr = 0x0016, COMMON_CTRL[21]
	Common_Wr01(common_ctrl22, agcref_7_0, agcref_7_0);
	Common_WrFL(common_ctrl22);
	//AGCREF[7:0] :Set of reference value for gain error calculation
	// The reference value is compared with the power of incoming signal for gain error calculation
	// reference value = AGCREF & "000"

	daftfreqdeltastep_1_0 = 1;
	Sync_RdFL(sync_ctrl13);  //regAddr = 0x100d, SYNC_CTRL[13]
	Sync_Wr01(sync_ctrl13, daftfreqdeltastep_1_0, daftfreqdeltastep_1_0);
	Sync_WrFL(sync_ctrl13);
	//DAFTFREQDELTASTEP[1:0] : Selection of frequency step for DAFT
	// 00 : 50KHz step
	// 01 : 100KHz step
	// 10 : 200KHz step (default)
	// 11 : 400KHz step

	samgsend_cqs_3_0 = 0xc;
	samgsstart_cqs_3_0 = 0x6;
	...
	//SAMGSEND_CQS[3:0] : Set of CQS TR gear shifting ending step for wide version.
...
/*****************************************************************************************************/
	...
	qmdqammode2_2_0 = 3;  // 3rd QAM (128QAM)
	qmdqammode3_2_0 = 0;  // 4th QAM (16QAM)
	qmdqammode4_2_0 = 1;  // 5th QAM (32QAM)
...
/*****************************************************************************************************/
	...
	if(gM14B0GBB_PowerOnInit == 1)
	{
		...
		gM14B0GBB_PowerOnInitStartTimeDVB = jiffies_to_msecs(jiffies);
	}
	else if(gM14B0GBB_PowerOnInit == 0)
	{
		if (RET_OK != DEMOD_M14B0_DVBT_Set_CrGearshEn(TRUE ))
		{
			DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Set_CrGearshEn() !!!\n", __F__, __L__);
	...
	_gM14B0GBB_DVB_UnlockCount_full_packeterr = 0;
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBC_Set_Config_auto
 * Control automatic setting of DVBC config.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBC_Set_Config_auto (BOOLEAN bAutoDetect)
{
	...
	if(bAutoDetect == TRUE)
	{
		...
		//QAM mode setting
		qmdqammode0_2_0 = 0x2; //return to default (256 QAM)
		Sync_RdFL(sync_ctrl26);  //regAddr = 0x101a, SYNC_CTRL[26]
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBC_Set_QamMode (LX_DEMOD_RF_MODE_T constel)
{
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBC_Set_NeverLockWaitTime
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBC_Set_NeverLockWaitTime(UINT32 waitMs)
{
	...
	scsyslockchktime_7_0 = 0x8c; //1400ms
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBC_Set_HighAgcReference
 * Set AGC reference high .
 *
 * @param ctrl 	[IN] BOOLEAN
 * @return if Succeeded - RET_OK else-RET_ERROR
 * @see
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBC_Set_HighAgcReference(BOOLEAN ctrl)
{
	UINT8 agcrefmode;
	UINT8 agcref_15_8;
	UINT8 agcref_7_0;
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBC_Get_SymbolRateDetect
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBC_Get_SymbolRateDetect(UINT16 *pSymbolRate)
{
		...
		*pSymbolRate = (UINT16)(symbolRateKHz);
		...
		DEMOD_PRINT("[DEMOD M14 GBB] Symbol rate is %d [KHz]!!!\n",symbolRateKHz);
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBC_Get_SymbolRateStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBC_Get_SymbolRateStatus(LX_DEMOD_SYMBOLRATE_STATE_T *pSymbolRateDet)
{
	...
	if(1 == srdlock)
	{
		...
		DEMOD_PRINT("[DEMOD M14 GBB] Success (%s:%d) Symbol rate is detected !!!\n", __F__, __L__);
		if (RET_OK != DEMOD_M14B0_DVBC_Get_SymbolRateDetect(&symbolRate))
		{
			DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) Symbol rate couldn't be read !!!\n", __F__, __L__);
			return RET_ERROR;
		}
	...
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) Symbol rate couldn't be detected !!!\n", __F__, __L__);
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBC_Get_QamModeDetectStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBC_Get_QamModeDetectStatus(LX_DEMOD_QAMMODE_STATE_T *pQamModeDet)
{
	...
	if(qmden == 1)
	{
		...
		switch(qmdstatus_1_0)
		{
			...
			case 0x02 :
			{
				...
				if (RET_OK != DEMOD_M14B0_Get_QAMMode(&qamMode ))
				{
					DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVB_Get_QAMMode() !!!\n", __F__, __L__);
					return RET_ERROR;
				}
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBC_Obtaining_Signal_Lock.
...
int DEMOD_M14B0_DVBC_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_RETUNE_RF:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_M14B0_DVBC_Set_DefaultRegisterValue())
				{
					DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Set_DefaultRegisterValue() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}

				if(RET_OK != DEMOD_M14B0_DVBC_Set_NeverLockWaitTime(LX_DEMOD_DVBC_NEVERLOCK_WAIT_TIME))
				{
					DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Set_NeverLockWaitTime() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				if((LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.dvbc.tuneMode) && (LX_DEMOD_TUNE_SPEC_DVBC_FIXED_DATA != pDemodInfo->setCfgParam.dvbc.tuneMode))
				{
					if(RET_OK != DEMOD_M14B0_DVBC_Set_Config_auto(TRUE))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Set_Config_auto() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if(RET_OK != DEMOD_M14B0_DVBC_AutoSymbolRateDet(LX_DEMOD_SYMR_FULL_SCAN_MODE, LX_DEMOD_SYMBOL_RATE_6900_KHZ))
					{
...
#else
					if(RET_OK != DEMOD_M14B0_DVBC_Set_Config_auto(FALSE))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Set_Config_auto() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if(RET_OK != DEMOD_M14B0_DVBC_AutoSymbolRateDet(LX_DEMOD_SYMR_FULL_SCAN_MODE, pDemodInfo->setCfgParam.dvbc.symbolRate))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_AutoSymbolRateDet() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if(RET_OK != DEMOD_M14B0_DVBC_Set_QamMode(pDemodInfo->setCfgParam.dvbc.qamMode))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Set_QamMode() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				...
				DEMOD_PRINT( "^R^[DEMOD M14 GBB] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_TUNE_START..... ^0^\n");
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				{
					DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_SoftwareReset() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if((LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.dvbc.tuneMode) && (LX_DEMOD_TUNE_SPEC_DVBC_FIXED_DATA != pDemodInfo->setCfgParam.dvbc.tuneMode))
				{
					...
					if(RET_OK != DEMOD_M14B0_DVBC_Set_HighAgcReference(TRUE))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Set_HighAgcReference(TRUE) !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if(RET_OK != DEMOD_M14B0_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_ON, &scanProcessing, &scanLock))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_NeverlockScan() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
					pDemodInfo->controlState = LX_DEMOD_TUNE_SCAN;
					DEMOD_PRINT( "^R^[DEMOD M14 GBB] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_TUNE_SCAN..... ^0^\n");
				...
				else
				{
					...
					if(RET_OK != DEMOD_M14B0_DVBC_Set_HighAgcReference(TRUE))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Set_HighAgcReference(FALSE) !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
					...
					DEMOD_PRINT( "^R^[DEMOD M14 GBB] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
		...
		case LX_DEMOD_TUNE_SCAN:			/***************************************************/
			{
				...
				if((LX_DEMOD_FLAG_COMPLETE == pDemodInfo->scanJobFlag) || (LX_DEMOD_FLAG_FINISH == pDemodInfo->scanJobFlag))
				{
					...
					DEMOD_PRINT( "^c^[DEMOD M14 GBB] ERR (%s:%d) Already Done !!!\n", __F__, __L__);
					DEMOD_PRINT( "^R^[DEMOD M14 GBB] LX_DEMOD_TUNE_SCAN : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
					return RET_OK;
				}

				if(RET_OK != DEMOD_M14B0_NeverlockScan(pDemodInfo, LX_DEMOD_FLAG_OFF, &scanProcessing, &scanLock))
				{
					DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_NeverlockScan() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if((LX_DEMOD_FLAG_FINISH == scanProcessing) && (LX_DEMOD_LOCKED == scanLock))
				{
					...
					DEMOD_PRINT( "^G^[DEMOD M14 GBB] (%s:%d) Signal is Found  !!!\n", __F__, __L__);
					DEMOD_PRINT( "^R^[DEMOD M14 GBB] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
				...
				else if((LX_DEMOD_FLAG_FINISH == scanProcessing)&& (LX_DEMOD_UNLOCKED == scanLock))
				{
					...
					DEMOD_PRINT( "^G^[DEMOD M14 GBB] (%s:%d) Signal is NOT Found  !!!\n", __F__, __L__);
				...
				else
				{
					...
					DEMOD_PRINT( "^G^[DEMOD M14 GBB] (%s:%d) Detection is on processing	!!!\n", __F__, __L__);
				}
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				{
					DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_Get_Lock() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					DEMOD_PRINT( "^R^[DEMOD M14 GBB] LX_DEMOD_SYNC_CHECKING : Going to LX_DEMOD_SIGNAL_STABLE..... ^0^\n");
				}
				else
				{
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE: 		/********************************************/
			{
				...
				if(RET_OK != DEMOD_M14B0_Get_Lock(LX_DEMOD_FEC_LOCK, &lockState))
				{
					DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_Get_Lock() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					if(RET_OK != DEMOD_M14B0_DVBC_Set_HighAgcReference(FALSE))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Set_HighAgcReference(FALSE) !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
					...
					if (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.dvbc.tuneMode)
					{
						...
						else
						{
							...
							{
								DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_VSB_PreMonitor() !!!\n", __F__, __L__);
								DEMOD_RETURN_FAIL(__F__);
							}
							else
							{
								if(packetError < LX_DEMOD_PACKET_ERROR_THD)
								{
									...
									{
										DEMOD_PRINT( "^r^[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
										DEMOD_RETURN_FAIL(__F__);
									}
									...
									DEMOD_PRINT_ALWAYS( "[DEMOD M14 GBB] LX_DEMOD_SIGNAL_STABLE : MSG_DVBC_FRONT_END_LOCKED..... ^^^0^^^\n");
									DEMOD_PRINT_ALWAYS( "[DEMOD M14 GBB] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^^0^^^\n", elapsedTime);
									DEMOD_PRINT( "^R^[DEMOD M14 GBB] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^^^0^^^\n", freqOffset);
					...
					else
					{
						...
						{
							if (RET_OK != DEMOD_M14B0_DVBC_Get_SymbolRateStatus(&symbolRateDet))
							{
							...
							DEMOD_PRINT_ALWAYS( "[DEMOD M14 GBB] symbolRateDiff  = %d\n",symbolRateDiff);
							DEMOD_PRINT_ALWAYS( "[DEMOD M14 GBB] symbolRateDet.symbolRate = %d, pDemodInfo->setCfgParam.dvbc.symbolRate = %d\n",symbolRateDet.symbolRate, pDemodInfo->setCfgParam.dvbc.symbolRate);
							if(((symbolRateDet.status == LX_DEMOD_DETECTED) && (symbolRateDiff > 60)) || (symbolRateDet.status != LX_DEMOD_DETECTED))
							{
								if(RET_OK != DEMOD_M14B0_DVBC_AutoSymbolRateDet(LX_DEMOD_SYMR_FIXED_MODE, pDemodInfo->setCfgParam.dvbc.symbolRate))
								{
									DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_AutoSymbolRateDet() !!!\n", __F__, __L__);
									DEMOD_RETURN_FAIL(__F__);
								}
					...
					{
						DEMOD_PRINT( "^r^[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_Get_TunedCFG() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if (RET_OK != DEMOD_Get_signal_status(pDemodInfo))
					{
						DEMOD_PRINT( "^r^[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_Get_signal_status() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				...
				{
					pDemodInfo->controlState	= LX_DEMOD_SYNC_CHECKING;
					DEMOD_PRINT( "^R^[DEMOD M14 GBB] LX_DEMOD_SYNC_CHECKING : Going back to LX_DEMOD_SYNC_CHECKING..... **;\n");
...
int DEMOD_M14B0_DVBC_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
...
//	unsigned int			powerOnElapsedTime;
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if(pDemodInfo->signalStatus.packetError >= _gM14B0GBB_FullDVBPkerrThreshold)
		{
			_gM14B0GBB_DVB_UnlockCount_full_packeterr++;
			DEMOD_PRINT( "^r^[DEMOD M14 GBB](%s) _gM14B0GBB_DVB_UnlockCount_full_packeterr = %d !!!\n", __F__,_gM14B0GBB_DVB_UnlockCount_full_packeterr);

			if(_gM14B0GBB_DVB_UnlockCount_full_packeterr == _gM14B0GBB_FullDVBPkerrCount)
			{
				DEMOD_PRINT( "^r^[DEMOD M14 GBB] oops !! (%s) Force to NOT LOCK !!\n", __F__);
				_gM14B0GBB_DVB_UnlockCount_full_packeterr = 0;
				...
				DEMOD_PRINT("[DEMOD M14 GBB](%s) DEMOD_M14B0_DVBT_Set_Config_auto(TRUE) !!!\n", __F__);

				if(RET_OK != DEMOD_M14B0_SoftwareReset())
				{
					DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_SoftwareReset() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}
				DEMOD_PRINT("[DEMOD M14 GBB](%s) DEMOD_SoftwareReset() !!!\n", __F__);
		...
		else
		{
			...
			if(pDemodInfo->lockCount == 2)
			{
				...
				pDemodInfo->bNotifiedUnlock = FALSE;
					...
					DEMOD_PRINT_ALWAYS( "[DEMOD M14 GBB] MONITORING FUNC : MSG_DVBC_FRONT_END_LOCKED..... ^0^\n");
	...
	else
	{
		...
		if((LX_DEMOD_UNLOCKED != pDemodInfo->lockStatus)  && (TRUE != pDemodInfo->bNotifiedUnlock))
		{
			...
			if(pDemodInfo->unLockCount == 4)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD M14 GBB] MONITORING FUNC : MSG_DVBC_FRONT_END_NOT_LOCKED..... -_-\n");
		...
		else
		{
			...
			if(pDemodInfo->unLockLongCount == 50)
			{
				...
				DEMOD_PRINT( "^R^[DEMOD M14 GBB] MONITORING FUNC : Going to LX_DEMOD_RETUNE_RF..... ^0^\n");
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBC_Get_DvbInfo(void)

{
	...
	//IF AGC
	if (RET_OK != DEMOD_M14B0_Get_IFAGC(&ifAGCGain))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_Get_IFAGC() !!!\n", __F__, __L__);

		return RET_ERROR;
	}

	DEMOD_PRINT("[DEMOD M14 GBB] IF AGC Gain = 0x%x !!!\n",ifAGCGain);

 	//NeverLock status
	if (RET_OK != DEMOD_M14B0_Get_NeverLockStatus(LX_DEMOD_DVBC, &neverLockStatus))
	{

		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Get_NeverLockStatus() !!!\n", __F__, __L__);
	...
	switch(neverLockStatus)
	{
		...
		case LX_DEMOD_IDLE :
			DEMOD_PRINT("[DEMOD M14 GBB] Initialization process  !!!\n");
			break;

		case LX_DEMOD_DETECTION_PROCESSING :
			DEMOD_PRINT("[DEMOD M14 GBB] Signal Checking process !!!\n");
			break;

		case LX_DEMOD_DETECTED :
			DEMOD_PRINT("[DEMOD M14 GBB] normal State (available channel)  !!!\n");
			break;

		case LX_DEMOD_DETECTION_FAIL :
			DEMOD_PRINT("[DEMOD M14 GBB] neverlock state (empty channel) !!!\n");

			break;

		default :

			DEMOD_PRINT("[DEMOD M14 GBB] INVALID neverLockStatus !!!\n");
	...
	if (RET_OK != DEMOD_M14B0_Get_Lock(LX_DEMOD_CARR_LOCK,&carrLockState) )
	{
		DEMOD_PRINT("[LX_DEMOD_DTV] FAIL (%s:%d) DEMOD_M14B0_Get_Lock() !!!\n", __F__, __L__);
		return RET_ERROR;
	...
	if(carrLockState == LX_DEMOD_LOCKED)
	{
		...
		DEMOD_PRINT("[DEMOD M14 GBB] DVB CARRIER is LOCKED  !!!\n");

	}
	else
	{
		DEMOD_PRINT("[DEMOD M14 GBB] DVB CARRIER is NOT LOCKED  !!!\n");

	}

	// Sampling frequency LOCK status and offset monitoring
	if (RET_OK != DEMOD_M14B0_Get_Lock( LX_DEMOD_TR_LOCK, &samplingLock))
	{
	...
	{
		DEMOD_PRINT("[DEMOD M14 GBB] Sampling frequency is Locked   !!!\n");
	}
	else
	{
		DEMOD_PRINT("[DEMOD M14 GBB] Sampling frequency is NOT Locked !!!\n");
	}


	//Symbol rate detection

	if (RET_OK != DEMOD_M14B0_DVBC_IsSymbolRateAuto(&isAutoSymbol))
	{

		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Get_SymbolRateStatus() !!!\n", __F__, __L__);
	...
	if(TRUE == isAutoSymbol)
	{
...

		{
...

			{
				...
				DEMOD_PRINT("[DEMOD M14 GBB] Symbol rate status is idle status!!!\n");
...

			{
				...
				DEMOD_PRINT("[DEMOD M14 GBB] Symbol rate  is detected !!!\n");

				DEMOD_PRINT("[DEMOD M14 GBB] Symbol rate = %d [KHz] !!!\n",symbolRateDet.symbolRate);
...

			{
				...
				DEMOD_PRINT("[DEMOD M14 GBB] Symbol rate detection is processing !!!\n");
...

			{
				...
				DEMOD_PRINT("[DEMOD M14 GBB] Symbol rate detection is failed !!!\n");
			...
			default :
			{
				DEMOD_PRINT("[DEMOD M14 GBB] INVALID Symbol rate detection status  !!!\n");
	...
	if (RET_OK != DEMOD_M14B0_DVBC_IsQammodeAutoDet(&isAutoQammode))
	{

		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_IsQammodeAutoDet() !!!\n", __F__, __L__);
	...
	if(TRUE == isAutoQammode)
	{
		...
		if (RET_OK != DEMOD_M14B0_DVBC_Get_QamModeDetectStatus(&qamModeDet))
		{
			DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Get_QamModeDetectStatus() !!!\n", __F__, __L__);
		...
		switch(qamModeDet.status)
		{
			case LX_DEMOD_IDLE :
			{
				DEMOD_PRINT("[DEMOD M14 GBB] QAM mode detect is idle status!!!\n");
			...
			case LX_DEMOD_DETECTED :
			{
				...
				DEMOD_PRINT("[DEMOD M14 GBB] QAM mode is detected!!!\n");

				DEMOD_PRINT("[DEMOD M14 GBB] QAM mode is %s !!!\n",(qamModeDet.qamMode == LX_DEMOD_16QAM) ? "16QAM" :
			...
			case LX_DEMOD_DETECTION_PROCESSING :
			{
				...
				DEMOD_PRINT("[DEMOD M14 GBB] QAM mode detection is processing !!!\n");
			...
			case LX_DEMOD_DETECTION_FAIL :
			{
				...
				DEMOD_PRINT("[DEMOD M14 GBB] QAM mode detection is failed !!!\n");
			...
			case LX_DEMOD_DETECTION_MANUAL :
			{
				...
				DEMOD_PRINT("[DEMOD M14 GBB] Manual QAM mode detection mode !!!\n");
			...
			default :
			{
				DEMOD_PRINT("[DEMOD M14 GBB] INVALID QAM mode detection  !!!\n");
	...
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_Get_SpectrumStatus() !!!\n", __F__, __L__);
		spectrumInv = LX_DEMOD_UNKOWN_STATE;
	}

	if(LX_DEMOD_INVERSION == spectrumInv)
	{
		DEMOD_PRINT("[DEMOD M14 GBB] Spectrum Inversion !!!\n");
	}
	else
	{
		DEMOD_PRINT("[DEMOD M14 GBB] Spectrum Normal !!!\n");
	}

	//Frame lock status
	if (RET_OK != DEMOD_M14B0_Get_Lock(LX_DEMOD_FEC_LOCK, &fecLockState) )
	{
		DEMOD_PRINT("[LX_DEMOD_DTV] FAIL (%s:%d) DEMOD_M14B0_Get_Lock() !!!\n", __F__, __L__);
		return RET_ERROR;
	...
	{
		DEMOD_PRINT("[DEMOD M14 GBB] DVB FEC IS NOT LOCKED !!!\n");
	}
	else
	{
		DEMOD_PRINT("[DEMOD M14 GBB] DVB FEC LOCK OK !!!\n");
	}
	...
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Get_SNR() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	DEMOD_PRINT("[DEMOD M14 GBB] mse = %d, constPwr = %d, !!!\n",sigMse.mse, sigMse.constPwr);


 	//BER after Viterbi
	if (RET_OK != DEMOD_M14B0_Get_VABER(&vBer))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Get_SNR() !!!\n", __F__, __L__);
	...
	DEMOD_PRINT("[DEMOD M14 GBB] vBER = %d  !!!\n",vBer);

	//TP Error count
	errorMode = LX_DEMOD_I2C_READ_BASED;
	if (RET_OK !=  DEMOD_M14B0_Get_Packet_Error(&errorCount))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBC_Get_TPError() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	DEMOD_PRINT("[DEMOD M14 GBB] TP Error Count= %d !!!\n",errorCount);


#ifndef DEMOD_PRINT

	printk("[DEMOD M14 GBB] IF AGC Gain = 0x%x !!!\n",ifAGCGain);
	printk("[DEMOD M14 GBB] DVB CARRIER is %s  !!!\n",(carrLockState == LX_DEMOD_LOCKED) ? "locked" : "NOT locked" );
	printk("[DEMOD M14 GBB] Carrier offset Frequency = %d [KHz] !!!\n",carrierLock.carrOffset);
	printk("[DEMOD M14 GBB] Sampling frequency is %s   !!!\n",(samplingLock == LX_DEMOD_LOCKED) ? "locked" : "NOT locked");
	printk("[DEMOD M14 GBB] QAM mode is %s   !!!\n",(qamModeDet.status == LX_DEMOD_DETECTED) ? "detected" : "NOT detected");
	...
	printk("[DEMOD M14 GBB] FEC lock is %s !!!\n",(fecLockState == LX_DEMOD_LOCKED) ? "locked" : "NOT locked");
	printk("[DEMOD M14 GBB] MSE = %d, constPwr = %d !!!\n",sigMse.mse, sigMse.constPwr);
	printk("[DEMOD M14 GBB] vBER = %d  !!!\n",vBer);
	printk("[DEMOD M14 GBB] TP Error Count= %d !!!\n",errorCount);
...
/**
 * DEMOD_M14B0_DVBT_Set_DefaultRegisterValue
 * Set defualt register value on DVB-T.
 * This function is applied only to A0 version, not to A1 version.
 *
 * @param none
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Set_DefaultRegisterValue(LX_DEMOD_BWMODE_T band)
{
	UINT8 adcvrefsel_0;
	UINT8 adcclkpllsel;
	UINT8 adcisel_2_0;
...
//	UINT8 ctfnrlen_2_0;
	...
	UINT8 crafcgstime_di_3_0;

	UINT8 crgearshen;
	UINT8 stocirbnd_7_0;

	UINT8 fstoadjmode_d6m;
	...
	UINT8 crafcgstepst_di_3_0;
	UINT8 crafcgsteped_di_3_0;

	UINT8 tiofffltset_sel;
	UINT8 tioffdlysp_sel;
	...
	fstoadjmode_d6m = 0x0;
	EQ_DI_RdFL(eq_di_ctrl28); //regAddr = 0x221c, EQ_DI_CTRL[28]
	EQ_DI_Wr01(eq_di_ctrl28,fstoadjmode_d6m, fstoadjmode_d6m);
	EQ_DI_WrFL(eq_di_ctrl28);
	// FSTOADJMODE_D6M : Selection of FSTO adjustment algorithm of DVB-T 6M bandwidth
	// 0 : using correct
	// 1 : ISI


	crafcgstepst_di_3_0 = 0x6;
	crafcgsteped_di_3_0 = 0xa;
	Sync_RdFL(sync_ctrl79);  //regAddr = 0x104f, SYNC_CTRL[79]
	Sync_Wr02(sync_ctrl79,crafcgstepst_di_3_0,crafcgstepst_di_3_0, crafcgsteped_di_3_0, crafcgsteped_di_3_0);
	Sync_WrFL(sync_ctrl79);
	// CRAFCGSTEPST_DI[3:0]: Set of DVB-T, ISDB-T AFC gain gear shifting initial step
	// CRAFCGSTEPED_DI[3:0]: Set of DVB-T, ISDB-T AFC gain gear shifting final step

	tiofffltset_sel = 0x3;
	tioffdlysp_sel = 0x3;
	EQ_DI_RdFL(eq_di_ctrl91); //regAddr = 0x225b, EQ_DI_CTRL[91]
	EQ_DI_Wr02(eq_di_ctrl91,tiofffltset_sel, tiofffltset_sel, tioffdlysp_sel,tioffdlysp_sel);
	EQ_DI_WrFL(eq_di_ctrl91);
	...
	if(LX_DEMOD_BW_6MHZ == band)
	{
		...
		*/
		...
		EQ_DI_RdFL(eq_di_ctrl23); //regAddr = 0x2217, EQ_DI_CTRL[23]
		EQ_DI_Wr01(eq_di_ctrl23,cstostartcnt_1_0, cstostartcnt_1_0);
		EQ_DI_WrFL(eq_di_ctrl23);
	...
	{
		// co-channel performance for 8MHz
		aiccopmode0_1_0 = 1;
		Common_RdFL(common_ctrl36);   //regAddr = 0x0024,  COMMON_CTRL[36]
		Common_Wr01(common_ctrl36, aiccopmode0_1_0, aiccopmode0_1_0);
		...
		//AICCFIXFREQ0[23:0] : Set of fixed frequency for 1st AICC rejector
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Set_Config_auto
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Set_Config_auto (BOOLEAN bAutoDetect)
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Set_PartialConfig
 * Set hierachical signal path and bandwidth.
 * This function is used when LX_DEMOD_M14B0_DVBT_Set_Config_auto is enabled.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Set_PartialConfig (LX_DEMOD_DVBT_CONFIG_T *pDvbtConfigParam)
{
	...
	{
		hierhpsel_d = 0x1;
		DEMOD_PRINT("[DEMOD M14 GBB] High Priority signal is selected  !!!\n");
	}
	else
	{
		hierhpsel_d = 0x0;
		DEMOD_PRINT("[DEMOD M14 GBB] Low Priority signal is selected  !!!\n");
	}
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Set_FullConfig
 * Set all config parameters on DVB-T.
 * This function is used when LX_DEMOD_M14B0_DVBT_Set_Config_auto is disabled.
 *
 * @param *pDvbtConfigParam 	[IN] LX_DEMOD_DVBT_CONFIG_T - DVB-T config. structure
 * @return if Succeeded - RET_OK else-RET_ERROR
 * @see
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Set_FullConfig (LX_DEMOD_DVBT_CONFIG_T *pDvbtConfigParam)
{
	...
	switch(pDvbtConfigParam->FFTMode)
	{
		case LX_DEMOD_DVBT_ISDBT_FFT_2K :
			{
				...
				DEMOD_PRINT("[DEMOD M14 GBB] Set FFT Mode = %s !!!\n","2K ");
			}
			break;
		...
		default :
			{
				mdfftmode_di_1_0 = 0x1;
				DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d)UNKNOWN FFT mode !!!\n",__F__, __L__);
			}
			return RET_ERROR;
	}
	...
	switch(pDvbtConfigParam->gIMode)
	{
		case LX_DEMOD_DVBT_ISDBT_GIMODE_1_4 :
			{
				mdgimode_di_1_0 = 0x3;
				DEMOD_PRINT("[DEMOD M14 GBB] Set Guard Interval = %s !!!\n","1_4 ");
			}
			break;
		...
		default :
			{
				mdgimode_di_1_0 = 0x0;
				DEMOD_PRINT("[DEMOD M14 GBB] FAIL,(%s:%d) UNKNOWN Guard Interval  !!!\n", __F__, __L__);
			}
			return RET_ERROR;
	}
	...
	switch(pDvbtConfigParam->hierachMode)
	{
		case LX_DEMOD_DVB_TPS_HIERACHY_NONE :
			{
				...
				DEMOD_PRINT("[DEMOD M14 GBB] Set Hierachical Mode = %s !!!\n","NONE");
			}
			break;
		...
		default :
			{
				tpshiermode_d_2_0 = 0x0;
				DEMOD_PRINT("[DEMOD M14 GBB] FAIL,(%s:%d) UNKNOWN Hierachical Mode  !!!\n", __F__, __L__);
			}
			return RET_ERROR;
	}
	...
	switch(pDvbtConfigParam->qamMode)
	{
		case LX_DEMOD_QPSK :
			{
				...
				DEMOD_PRINT("[DEMOD M14 GBB] Set QAM mode = %s !!!\n","QPSK");
			}
			break;
		...
		default :
			{
				tpsqammode_d_1_0 = 0x0;
				DEMOD_PRINT("[DEMOD M14 GBB] FAIL,(%s:%d) UNKNOWN QAM mode !!!\n", __F__, __L__);
			}
			return RET_ERROR;
	}
	...
	if(pDvbtConfigParam->bSetHp)  //code rate of high priority signal manual set
	{
		switch(pDvbtConfigParam->hpCodeRate)
		{
			case LX_DEMOD_CODE_1_2 :
				{
					...
					DEMOD_PRINT("[DEMOD M14 GBB] Set HP code rate = %s !!!\n","1_2");
				}
				break;
			...
			default :
				{
					tpshpcoderate_d_2_0 = 0x0;
					DEMOD_PRINT("[DEMOD M14 GBB] FAIL,(%s:%d) UNKNOWN HP code rate !!!\n", __F__, __L__);
				}
				return RET_ERROR;
		}
	...
	else  //code rate of low priority signal manual set
	{
		switch(pDvbtConfigParam->lpCodeRate)
		{
			case LX_DEMOD_CODE_1_2 :
				{
					...
					DEMOD_PRINT("[DEMOD M14 GBB] Set LP code rate = %s !!!\n","1_2");
				}
				break;
			...
			default :
				{
					tpslpcoderate_d_2_0 = 0x0;
					DEMOD_PRINT("[DEMOD M14 GBB] FAIL,(%s:%d) UNKNOWN LP code rate !!!\n", __F__, __L__);
				}
				return RET_ERROR;
		}
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Set_NeverLockWaitTime
 * Set neverlock waiting period.
 *
 * @param waitMs 	[IN] UINT32 - unit: ms
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Set_NeverLockWaitTime(UINT32 waitMs)
{
	UINT8 scsyslockchktime_7_0;
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Set_TafcWaitSymbol
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Set_TafcWaitSymbol(BOOLEAN bDelayed)
{
/*
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Set_CTFNR_Control
 * Set TAFC wait symbol number.
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Set_CTFNR_Control(LX_DEMOD_CTFNR_CTRL_T ctfnrCtrl)
{
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Set_CirPwrControl
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Set_CirPwrControl(LX_DEMOD_CIR_POWER_CTRL_T cirPwrCtrl)
{
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Set_CrGearshEn
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Set_CrGearshEn(BOOLEAN bEnable)
{
	UINT8 crgearshen;
	UINT8 crafcgstime_di_3_0;
	...
	crafcgstime_di_3_0 = 0x1;
	Sync_RdFL(sync_ctrl71);  //regAddr = 0x1047, SYNC_CTRL[71]
	Sync_Wr01(sync_ctrl71,crafcgstime_di_3_0,crafcgstime_di_3_0);
	Sync_WrFL(sync_ctrl71);
	//CRAFCGSTIME_DI[3:0]: Set of DI AFC gear shifting time step (sec).
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Set_CirNormalize
 * Set Cir Power level and NR level
 *
 * @param none 	[IN] void
 * @return if Succeeded - RET_OK else-RET_ERROR
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Set_CirNormalize(void)
{
	...
	if(RET_OK != DEMOD_M14B0_DVBT_Get_DelaySpreadStatus(&delaySpread))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Get_DelaySpreadStatus() !!!\n", __F__, __L__);
		DEMOD_RETURN_FAIL(__F__);
	}

	if(delaySpread <= 430)
	{
		if (RET_OK != DEMOD_M14B0_DVBT_Set_CTFNR_Control(LX_DEMOD_CTFNR_1_16))
		{
			DEMOD_PRINT("[DEMOD H14] FAIL (%s:%d) DEMOD_M14B0_DVBT_Set_CTFNR_Control !!!\n", __F__, __L__);
			return RET_ERROR;
		}
	...
	if(validCirCount != 2 )
	{
		...
		{
			if (RET_OK != DEMOD_M14B0_DVBT_Set_CirPwrControl(LX_DEMOD_CIR_POWER_MID))
			{
				DEMOD_PRINT("[DEMOD H14] FAIL (%s:%d) DEMOD_M14B0_DVBT_Set_CirPwrControl !!!\n", __F__, __L__);
				return RET_ERROR;
			}
		...
		{
			if(RET_OK != DEMOD_M14B0_DVBT_Get_PreGhostDetection(&IsPreGhostExist))
			{
				DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Get_DelaySpreadStatus() !!!\n", __F__, __L__);
				DEMOD_RETURN_FAIL(__F__);
			}
	...
	{
		if (RET_OK != DEMOD_M14B0_DVBT_Get_CirPwrControl(&cirPwrCtrl))
		{
			DEMOD_PRINT("[DEMOD H14] FAIL (%s:%d) DEMOD_M14B0_DVBT_Set_CirPwrControl !!!\n", __F__, __L__);
			return RET_ERROR;
		}
		...
		if(cirPwrCtrl == LX_DEMOD_CIR_POWER_HIGH)
		{
		    ...
		    {
				if (RET_OK != DEMOD_M14B0_DVBT_Set_CirPwrControl(LX_DEMOD_CIR_POWER_LOW))
				{
					DEMOD_PRINT("[DEMOD H14] FAIL (%s:%d) DEMOD_M14B0_DVBT_Set_CirPwrControl !!!\n", __F__, __L__);
					return RET_ERROR;
				}
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Get_CirPwrControl
 * Set Cir Power level and NR level
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_CirPwrControl(LX_DEMOD_CIR_POWER_CTRL_T *pCirPwrCtrl)
{
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Get_PreGhostDetection
 * Set Cir Power level and NR level
 *
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_PreGhostDetection(BOOLEAN *pIsExist)
{
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Get_DelaySpreadStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_DelaySpreadStatus(UINT16 *pDelaySpread)
{
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Get_Hierach_HPSel
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_Hierach_HPSel( BOOLEAN *pBSetHp )
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_FFTMode( LX_DEMOD_LOCK_STATE_T *pTxModeLock, LX_DEMOD_FFTMODE_T *pFftMode )
{
	...
	*pFftMode = LX_DEMOD_DVBT_ISDBT_FFT_UNKNOWN;
	...
	if(modelock_di)
	{
		*pTxModeLock = LX_DEMOD_LOCKED;
		...
		DEMOD_PRINT("[DEMOD M14 GBB] TX mode LOCKED !!!\n");
	...
	{
		*pTxModeLock = LX_DEMOD_UNLOCKED;
		DEMOD_PRINT("[DEMOD M14 GBB] TX mode is NOT LOCKED !!!\n");
	}
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Get_GuradIntervalMode
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_GuradIntervalMode( LX_DEMOD_GIMODE_T *pGIMode )
{
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Get_HierachyMode
 * Get the information of hierachical mode on DVB-T.
 *
 * @param *pHierachMode[OUT] DEMOD_M14B0_DVB_TPS_HIERACHY_T - alpha = none, 1, 2 and 4.
 * @return if Succeeded - RET_OK else-RET_ERROR
 * @see
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_HierachyMode( LX_DEMOD_DVB_TPS_HIERACHY_T *pHierachMode )
{
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Get_LpCoderRate
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_LpCoderRate( LX_DEMOD_CODERATE_T *pLpCodeRate )
{
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Get_HpCoderRate
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_HpCoderRate( LX_DEMOD_CODERATE_T *pHpCodeRate )
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_CellId(UINT8 *pCellIDLength, UINT16 *cell_id)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_TpsInfo( LX_DEMOD_DVBT_CONFIG_T *pTpsParams)
{
	BOOLEAN bSetHp = TRUE;
	...
	if(NULL == pTpsParams)
	{
		DEMOD_PRINT("[DEMOD M14 GBB] ERROR, DEMOD_M14B0_DVBT_CONFIG_T TpsParams is NULL !!!\n");
		return RET_ERROR;
	}

	// Get bSetHp

	if (RET_OK != DEMOD_M14B0_DVBT_Get_Hierach_HPSel( &bSetHp ))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Get_Hierach_HPSel() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	pTpsParams->bSetHp = bSetHp;
	DEMOD_PRINT("[DEMOD M14 GBB] Set HP selection = %s !!!\n",(bSetHp == TRUE) ? " HP Mode" : " LP Mode");
	...
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Get_BandWidthMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	pTpsParams->channelBW = channelBW;
	DEMOD_PRINT("[DEMOD M14 GBB] Bandwidth = %s !!!\n",
	...
	// Get FFT mode
	if (RET_OK != DEMOD_M14B0_DVBT_Get_FFTMode( &txModeLock, &FFTMode ))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Get_FFTMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	pTpsParams->FFTMode = FFTMode;
	DEMOD_PRINT("[DEMOD M14 GBB] Tx mode = %s, FFTMode = %s  !!!\n",
	...
	if (RET_OK != DEMOD_M14B0_DVBT_Get_GuradIntervalMode( &gIMode ))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Get_GuradIntervalMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	pTpsParams->gIMode = gIMode;
	DEMOD_PRINT("[DEMOD M14 GBB] Guard Intterval = %s  !!!\n",
	...
	if (RET_OK != DEMOD_M14B0_DVBT_Get_HierachyMode( &hierachMode ))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Get_HierachyMode() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	pTpsParams->hierachMode = hierachMode;
	DEMOD_PRINT("[DEMOD M14 GBB] Hierachical Mode = %s  !!!\n",
	...
	// Get LP coder rate
	if (RET_OK != DEMOD_M14B0_DVBT_Get_LpCoderRate( &lpCodeRate ))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Get_LpCoderRate() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	pTpsParams->lpCodeRate = lpCodeRate;
	DEMOD_PRINT("[DEMOD M14 GBB] LP code rate = %s  !!!\n",
	...
	if (RET_OK != DEMOD_M14B0_DVBT_Get_HpCoderRate( &hpCodeRate ))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Get_HpCoderRate() !!!\n", __F__, __L__);
		return RET_ERROR;
	}

	pTpsParams->hpCodeRate = hpCodeRate;
	DEMOD_PRINT("[DEMOD M14 GBB] HP code rate = %s  !!!\n",
	...
	DEMOD_PRINT("[DEMOD M14 GBB] modulation  = %s  !!!\n",
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Get_TotalInfo
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_TotalInfo( void)
{
	...
	if (RET_OK != DEMOD_M14B0_Get_NeverLockStatus(LX_DEMOD_DVBT, &neverLockStatus))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVB_Get_NeverLockStatus() !!!\n", __F__, __L__);
	...
	if (RET_OK != DEMOD_M14B0_DVBT_Get_FFTMode( &txModeLock, &fftMode ))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Get_SpectrumInvStatus() !!!\n", __F__, __L__);
		delaySpread = 0xffff;
	}
	//TP Error count
	errorMode = LX_DEMOD_I2C_READ_BASED;
	if (RET_OK !=  DEMOD_M14B0_Get_Packet_Error(&errorCount))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Get_TPError() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	DEMOD_PRINT("[DEMOD M14 GBB] TP Error Count= %d !!!\n",errorCount);
	...
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
		FreqOff = 0xffff;
	}
	...
	DEMOD_PRINT("[DEMOD M14 GBB] AGC LOCK is %s !!!\n",(agcLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD M14 GBB] IF AGC value = %d(0x%x) !!!\n",ifagcValue,ifagcValue);
	DEMOD_PRINT("[DEMOD M14 GBB] Neverlock is %s !!!\n",(neverLockStatus == LX_DEMOD_DETECTED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD H13 DTV] Spectrum auto = %s, Inversion  = %s !!!\n",(spectrumAuto == LX_DEMOD_AUTO) ? " Auto" : " Manual",
																					(spectrumInv == LX_DEMOD_INVERSION) ? " Inverted" : " Normal" );
	DEMOD_PRINT("[DEMOD M14 GBB] Delay Spread = %d !!!\n",delaySpread);
	DEMOD_PRINT("[DEMOD M14 GBB] FFT mode LOCK = %s !!!\n",(txModeLock == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	DEMOD_PRINT("[DEMOD M14 GBB] offset = %d [KHz]!!!\n",FreqOff);
	DEMOD_PRINT("[DEMOD M14 GBB] mse = %d, constPwr = %d, !!!\n",sigMse.mse, sigMse.constPwr);

	if (RET_OK != DEMOD_M14B0_DVBT_Get_TpsInfo( &tppsParams))
	{
		DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Get_TpsInfo() !!!\n", __F__, __L__);
		return RET_ERROR;
	}
	...
	printk("[DEMOD M14 GBB] AGC LOCK is %s !!!\n",(agcLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	printk("[DEMOD M14 GBB] IF AGC value = %d !!!\n",ifagcValue);
	printk("[DEMOD M14 GBB] Neverlock is %s !!!\n",(neverLockStatus == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");
	printk("[DEMOD M14 GBB] Spectrum mode = %s !!!\n",(spectrumInv == LX_DEMOD_NORMAL) ? " Normal" :
																	(spectrumInv == LX_DEMOD_INVERSION) ? "Inversion" : 	" Unknown");
	printk("[DEMOD M14 GBB] Delay Spread = %d !!!\n",delaySpread);
	printk("[DEMOD M14 GBB] FFT mode LOCK = %s !!!\n",(txModeLock == LX_DEMOD_LOCKED) ? " Locked" : " NOT Locked");


	// Get bSetHp
	printk("[DEMOD M14 GBB] Set HP selection = %s !!!\n",(tppsParams->bSetHp == TRUE) ? " HP Mode" : " LP Mode");

	// Get channelBW
	printk("[DEMOD M14 GBB] Bandwidth = %s  !!!\n",
	...
	// Get FFT mode
	printk("[DEMOD M14 GBB] FFTMode = %s  !!!\n",
	...
	// Get Guard Interval mode
	printk("[DEMOD M14 GBB] Guard Intterval = %s  !!!\n",
	...
	printk("[DEMOD M14 GBB] Hierachical Mode = %s  !!!\n",
	...
	// Get LP coder rate
	printk("[DEMOD M14 GBB] LP code rate = %s  !!!\n",
	...
	// Get HP coder rate
	printk("[DEMOD M14 GBB] HP code rate = %s  !!!\n",
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_IFO_LOCK( BOOLEAN *pIfoLock)
{
	UINT8	syslock;
	...
	DEMOD_PRINT("[DEMOD M14 GBB](%s) IFO LOCK = %s !!!\n", __F__, (syslock == 1) ? "LOCK" :"NOT LOCK");
	DEMOD_PRINT("[DEMOD M14 GBB](%s) IFO value = %d !!!\n", __F__,intfreqoffsetvalue);
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Get_CochanDetIndicator
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_Get_CochanDetIndicator( BOOLEAN *pDetStatus)
{
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_EqualizereReset.
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_M14B0_DVBT_EqualizereReset(void)
{
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Obtaining_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_M14B0_DVBT_Obtaining_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
		...
		case LX_DEMOD_RETUNE_RF:			/***************************************************/
			{
				...
				if(RET_OK != DEMOD_M14B0_DVBT_Set_DefaultRegisterValue(pDemodInfo->setCfgParam.dvbt.channelBW))
				{
					DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Set_DefaultRegisterValue() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}

				DEMOD_PRINT("[DEMOD M14 GBB] [DEMOD M14 GBB] MONITORING FUNC : change to CIR power level(0x86) !!!\n");
				if(RET_OK != DEMOD_M14B0_DVBT_Set_NeverLockWaitTime(LX_DEMOD_DVBT_NEVERLOCK_WAIT_TIME))
				{
					DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Set_NeverLockWaitTime() !!!\n", __F__, __L__);
					DEMOD_RETURN_FAIL(__F__);
				}

				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.dvbt.tuneMode)
				{
					if(RET_OK != DEMOD_M14B0_DVBT_Set_Config_auto(TRUE))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_DVBT_Set_Config_auto() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if(RET_OK != DEMOD_M14B0_DVBT_Set_PartialConfig(&(pDemodInfo->setCfgParam.dvbt)))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_DVBT_Set_PartialConfig() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
				}
				else
				{
					...
					if(RET_OK != DEMOD_M14B0_DVBT_Set_PartialConfig(&(pDemodInfo->setCfgParam.dvbt)))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Set_PartialConfig() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

#else
					if(RET_OK != DEMOD_M14B0_DVBT_Set_Config_auto(FALSE))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Set_Config_auto() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}

					if(RET_OK != DEMOD_M14B0_DVBT_Set_FullConfig(&(pDemodInfo->setCfgParam.dvbt)))
					{
						DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Set_FullConfig() !!!\n", __F__, __L__);
						DEMOD_RETURN_FAIL(__F__);
					}
		...
		case LX_DEMOD_TUNE_START:			/***************************************************/
			{
				...
				if(LX_DEMOD_TUNE_NORMAL != pDemodInfo->setCfgParam.dvbt.tuneMode)
				{
					...
					pDemodInfo->controlState = LX_DEMOD_TUNE_SCAN;
					DEMOD_PRINT( "^R^[DEMOD M14 GBB] LX_DEMOD_TUNE_START : Going to LX_DEMOD_TUNE_SCAN..... ^0^\n");
				...
				else
				{
					...
					DEMOD_PRINT( "^R^[DEMOD M14 GBB] LX_DEMOD_TUNE_START : Going to LX_DEMOD_SYNC_CHECKING..... ^0^\n");
		...
		case LX_DEMOD_SYNC_CHECKING : 		/********************************************/
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					DEMOD_M14B0_DVBT_Set_TafcWaitSymbol(TRUE);
					...
					if(fftMode == LX_DEMOD_DVBT_ISDBT_FFT_2K)
					{
						...
						tiauto_2_0 = 0x5;
						EQ_DI_RdFL(eq_di_ctrl80); //regAddr = 0x2250, EQ_DI_CTRL[80]
						EQ_DI_Wr01(eq_di_ctrl80,tiauto_2_0, tiauto_2_0);
						EQ_DI_WrFL(eq_di_ctrl80);
						// TIAUTO[2:0] : Control of TI(Time Interpolator) off auto enable for each mode
						// MSB : 8K FFT mode, 2nd bit : 4K FFT mode, LSB : 2K FFT mode
						// 0 : disabled, 1 : enabled

						mbupth2k_3_0 = 0x7;
						EQ_DI_RdFL(eq_di_ctrl83); //regAddr = 0x2253, EQ_DI_CTRL[83]
						EQ_DI_Wr01(eq_di_ctrl83,mbupth2k_3_0, mbupth2k_3_0);
						EQ_DI_WrFL(eq_di_ctrl83);
						// MBUPTH2K[3:0] : Set of upper threshold of difference between current symbol and previous symbol(CTF power variation) for 2K FFT mode
						// if it exceed threshold, becomes ti off status

						mblockholdenb = 0x0;
						mblockctrl = 0x0;
						EQ_DI_RdFL(eq_di_ctrl86); //regAddr = 0x2256, EQ_DI_CTRL[86]
						EQ_DI_Wr02(eq_di_ctrl86,mblockholdenb, mblockholdenb,mblockctrl,mblockctrl );
						EQ_DI_WrFL(eq_di_ctrl86);
						// MBLOCKCTRL : Control of stolock consideration for TI
						// 0 : TI off function working without stolock
						// 1 : TI off function working when stolock on

						// MBLOCKHOLDENB : Control of mobile detection lock hold mode(0 : disabled, 1 : enabled)

						crafcgstepst_di_3_0 = 0x8;
						crafcgsteped_di_3_0 = 0xa;
						Sync_RdFL(sync_ctrl79);  //regAddr = 0x104f, SYNC_CTRL[79]
						Sync_Wr02(sync_ctrl79,crafcgstepst_di_3_0,crafcgstepst_di_3_0, crafcgsteped_di_3_0, crafcgsteped_di_3_0);
						Sync_WrFL(sync_ctrl79);
						...
						crfgain_di_1_0 = 0x1;
						Sync_RdFL(sync_ctrl72);  //regAddr = 0x1048, SYNC_CTRL[72]
						Sync_Wr01(sync_ctrl72,crfgain_di_1_0,crfgain_di_1_0);
						Sync_WrFL(sync_ctrl72);
						//CRFGAIN_DI[1:0]: Selection of Frequency domain AFC gain
						// 00 : 1x/4
						// 01 : 1x/2
						// 10 : 1x
						// 11 : 2x
						...
						{
							DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_M14B0_TPOutEnable(FALSE) !!!\n", __F__, __L__);
						}
						OS_MsecSleep(200);

						if(RET_OK != DEMOD_M14B0_TPOutEnable(TRUE))
						{
							DEMOD_PRINT("[DEMOD] FAIL (%s:%d) DEMOD_M14B0_TPOutEnable(TRUE) !!!\n", __F__, __L__);
						}
	   	...
	   	case LX_DEMOD_SIGNAL_STABLE: 		/********************************************/
			{
				...
				if(LX_DEMOD_LOCKED == lockState)
				{
					...
					else
					{
						if(RET_OK != DEMOD_M14B0_Get_Packet_Error(&packetError))
						{
							DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_Get_Packet_Error() !!!\n", __F__, __L__);
							DEMOD_RETURN_FAIL(__F__);
						}
						else
						{
							...
							if(packetError < LX_DEMOD_PACKET_ERROR_THD)
							{
								...
								if((LX_DEMOD_BW_6MHZ == pDemodInfo->setCfgParam.dvbt.channelBW) /* && (LX_DEMOD_TUNE_NORMAL == pDemodInfo->setCfgParam.dvbt.tuneMode)*/)
								{
									...
									if (RET_OK != DEMOD_M14B0_Get_QAMMode( &qamMode ))
									{
										DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_Get_QAMMode() !!!\n", __F__, __L__);
										DEMOD_RETURN_FAIL(__F__);
									}
									...
									{	// To resolve pre-echo stream TW 8K, 1/4
										...
										if (RET_OK != DEMOD_M14B0_DVBT_Set_CirNormalize())
										{
											DEMOD_PRINT("[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_DVBT_Set_CirNormalize() !!!\n", __F__, __L__);
											DEMOD_RETURN_FAIL(__F__);
										}
										DEMOD_PRINT("[DEMOD M14 GBB] (%s) DEMOD_M14B0_DVBT_Set_CirNormalize() !!!\n", __F__ );
								...
								if (RET_OK != DEMOD_M14B0_Get_CarrierFreqOffset(&freqOffset))
								{
									DEMOD_PRINT( "^r^[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_M14B0_Get_CarrierFreqOffset() !!!\n", __F__, __L__);
									DEMOD_RETURN_FAIL(__F__);
								}
								pDemodInfo->freqOffset = freqOffset;
								...
								DEMOD_PRINT_ALWAYS( "[DEMOD M14 GBB] LX_DEMOD_SIGNAL_STABLE : MSG_DVBT_FRONT_END_LOCKED..... ^^^0^^^\n");
								DEMOD_PRINT_ALWAYS( "[DEMOD M14 GBB] LX_DEMOD_SIGNAL_STABLE : Elapsed Time = %d [ms] ..... ^^^0^^^\n", elapsedTime);
								DEMOD_PRINT( "^R^[DEMOD M14 GBB] LX_DEMOD_SIGNAL_STABLE : Frequency offset = %d [KHz] ..... ^^^0^^^\n", freqOffset);
							...
							{
								DEMOD_PRINT( "^R^[DEMOD M14 GBB] LX_DEMOD_SIGNAL_STABLE : Packet error = %d .....  --_--\n", packetError);
								elapsedTime = jiffies_to_msecs(jiffies) - sigStablestartTime;
...
************************************************************************************/
/**
 * DEMOD_M14B0_DVBT_Monitoring_Signal_Lock.
 * Set channel change parameters
 *
 * @param *pDemodInfo [IN] DEMOD_CTX_T
 * @return if Succeeded - RET_OK else-RET_ERROR
...
int DEMOD_M14B0_DVBT_Monitoring_Signal_Lock(DEMOD_CTX_T *pDemodInfo)
{
	...
	if(LX_DEMOD_LOCKED == lockState)
	{
		...
		if (RET_OK != DEMOD_Get_signal_status(pDemodInfo))
		{
			DEMOD_PRINT( "[DEMOD M14 GBB] FAIL (%s:%d) DEMOD_Get_signal_status() !!!\n", __F__, __L__);
			DEMOD_RETURN_FAIL(__F__);
		}
		...
		if(pDemodInfo->signalStatus.packetError >= _gM14B0GBB_FullDVBPkerrThreshold)
		{
			...
			if(_gM14B0GBB_DVB_UnlockCount_full_packeterr == _gM14B0GBB_FullDVBPkerrCount)
			{
				...
				pDemodInfo->bNotifiedLock	= FALSE;
				pDemodInfo->signalStatus.bSignalValid = FALSE;
				DEMOD_PRINT_ALWAYS( "[DEMOD M14 GBB] MONITORING FUNC : MSG_DVBT_FRONT_END_NOT_LOCKED..... -_-;;\n");
		...
		else
		{
			...
			{
				pDemodInfo->lockCount++;
				DEMOD_PRINT( "^r^[DEMOD M14 GBB](%s) pDemodInfo->lockCount = %d !!!\n", __F__,pDemodInfo->lockCount);
			...
			if(pDemodInfo->lockCount == 2)
			{
				...
				DEMOD_PRINT_ALWAYS( "[DEMOD M14 GBB] MONITORING FUNC : MSG_DVBT_FRONT_END_LOCKED..... ^0^\n");
	...
	else
	{
		...
		{
			pDemodInfo->unLockCount++;
			DEMOD_PRINT( "^r^[DEMOD M14 GBB](%s) pDemodInfo->unLockCount = %d !!!\n", __F__,pDemodInfo->unLockCount);
