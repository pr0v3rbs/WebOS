...
static int _gM14B0_TMDS_ERROR_count_tcs_fail = 0;
static int _gM14B0_tcs_fail_restart_count = 0;

static int _gM14B0_TERC4_ERROR_intr_count[4] = {0,};
static int _gM14B0_HDCP_ERROR_intr_count = 0;
static int _gM14B0_HDCP_ERROR_count = 0;
...
static int _HDMI_M14B0_Disable_TERC4_Error_Interrupt(int port_num);
static int _HDMI_M14B0_Enable_TERC4_Error_Interrupt(int port_num);
static int _HDMI_M14B0_Disable_HDCP_Error_Interrupt(void);
static int _HDMI_M14B0_Enable_HDCP_Error_Interrupt(void);
#ifdef NOT_USED_NOW
...
static int _g_mhl_odt_value = 3;	//default : 46 Ohm (141027 : new M14Bx Board )// default : 50Ohm
/******************************************************************************
 *				DATA STRUCTURES
 *****************************************************************************/
/**
...
int HDMI_M14B0_HWInitial(void)
{
	...
	_gM14B0ChipPlatform = lx_chip_plt() + 0xC;
	//141019 : Port2 Portable Tester
	//140930 : MHL ODT 46 Ohm
	//140916 : SS BD-P1600
	//140826 : G-phone MHL
	//140701 : BDP S370 EQ
	//140514 : MHL CT
	//140430 for driver version
	HDMI_M14B0_Key_Control(HDMI_KEY_SAVE);
	HDMI_M14B0_Key_Control(HDMI_KEY_CLEAR);
...
//	int loop, port_authed, port_enc_en;
	...
	do {
		...
		_gM14B0_tcs_fail_restart_count = 0;
...
int HDMI_M14B0_GetTimingInfo(LX_HDMI_TIMING_INFO_T *info)
{
	...
	//	_HDMI_M14B0_GetExtendTimingInfo(&bufTiming);		// get extend information
		...
		tmdsClock = _gM14B0HDMIPhyInform.tmds_clock[0]; 	// XXX.XX KHz
#else
...
int HDMI_M14B0_GetAviPacket(LX_HDMI_INFO_PACKET_T *packet)
{
	...
	if ( _gM14B0HDMIPhyInform.tmds_clock[0] > 29600 )
	{
		gAVIReadCnt = 0;
		HDMI_VIDEO("[%d] %s : not support 4Kx2K over 296Mhz [%d0 Khz]\n", __L__, __F__,_gM14B0HDMIPhyInform.tmds_clock[0]);
		memset(&_gM14B0PrevAVIPacket, 0 , sizeof(LX_HDMI_INFO_PACKET_T));
		return RET_OK;
	}
...
int HDMI_M14B0_GetStatus(LX_HDMI_STATUS_T *status)
{
	...
	do {
		...
		HDMI_VIDEO("[HDMI]  M14B0 : 20140916(BD-P1600) \n");
	} while (0);
 ...
 *  @return int
*/
int HDMI_M14B0_GetAudioCopyInfo(LX_HDMI_AUDIO_COPY_INFO_T *pCopyInfo)
{
	int ret = RET_OK;
		...
		switch(bufTiming.extInfo)		//if (eVideoFormat == LX_FORMAT_3D_FORMAT)
		{
			...
			case LX_HDMI_EXT_4K_2K:
				if ( !((bufTiming.hActive == 3840 && bufTiming.vActive == 2160) || (bufTiming.hActive == 4096 && bufTiming.vActive == 2160) ||\
					(bufTiming.hActive == 1920 && bufTiming.vActive == 2160 && _gM14B0HDMIPhyInform.tmds_clock[0] > 29000) || \
					(bufTiming.hActive == 2048 && bufTiming.vActive == 2160 && _gM14B0HDMIPhyInform.tmds_clock[0] > 29000) ) )
				{
					bufTiming.extInfo = LX_HDMI_EXT_2D_FORMAT;
...
//						_HDMI_M14B0_TMDS_HDCP_ResetPort(port_num);
					...
					if (port_num == _gM14B0HDMIPhyInform.prt_sel) {
						_gM14B0HDMIPhyInform.ecc_error_count[port_num] = 0;
						_gM14B0HDMIPhyInform.hdcp_error_count[port_num] = 0;
					}
					_gM14B0HDMIPhyInform.terc4_error_count[port_num] = 0;
					_gM14B0HDMIPhyInform.loop_count[port_num] = 0;
					_gM14B0HDMIPhyInform.scdt_detect_count[port_num] = 0;
					_gM14B0HDMIPhyInform.scdt_fall_count[port_num] = 0;
				}
...
//				_gM14B0HDMIPhyInform.hpd_out[port_num] = _gM14B0HDMIPhyInform.hdmi5v[port_num];
	...
	for (i=0;i<4;i++) {
		if(_gM14B0HDMIPhyInform.scdt[i] > 0)
			_gM14B0HDMIPhyInform.scdt_detect_count[i] ++;
	}

	LINK_M14_Rd01(system_control_00, reg_hdmi_mode_prt0, _gM14B0HDMIPhyInform.hdmi_mode[0]);
	LINK_M14_Rd01(system_control_00, reg_hdmi_mode_prt1, _gM14B0HDMIPhyInform.hdmi_mode[1]);
	LINK_M14_Rd01(system_control_00, reg_hdmi_mode_prt2, _gM14B0HDMIPhyInform.hdmi_mode[2]);
	LINK_M14_Rd01(system_control_00, reg_hdmi_mode_prt3, _gM14B0HDMIPhyInform.hdmi_mode[3]);
	...
	for (i=0;i<4;i++) {
		if(_gM14B0HDMIPhyInform.hdmi5v[i] > 0)
			_gM14B0HDMIPhyInform.loop_count[i] ++;
	}
	...
	if (access_phy[0] || access_phy[1] ||access_phy[2] ||access_phy[3] )
	{
		...
		_gM14B0HDMIPhyInform.tmds_clock[0] = ((up_freq << 8) + down_freq); 	// XXX.XX KHz

		if ( _gM14B0HDMIPhyInform.hpd_enable && (_gM14B0HDMIPhyInform.tmds_clock[0] == 0) && (_gM14B0HDMIPhyInform.hdmi5v[_gM14B0HDMIPhyInform.prt_sel] ))	// hdmi 5v , not no tmds clock
		{
			int cr_done, cr_lock;
			...
			if (cr_lock && cr_done)	// no tmds clock, but CR lock done ????
			{
				...
				if(tmds_clock_abnormal_count == 10) {
					...
					HDMI_NOTI("@@@ Port[%d] Disable EQ Calibration Mode : abnormal count [%d]\n", \
							_gM14B0HDMIPhyInform.prt_sel, tmds_clock_abnormal_count);

					/* SS BD-P1600 workaround code */
					PHY_REG_M14B0_RdFL(eq_cal_mode);
					PHY_REG_M14B0_Wr01(eq_cal_mode,eq_cal_mode,0x0);
					PHY_REG_M14B0_WrFL(eq_cal_mode);
				}
			}
			else
			...
			if (tmds_clock_abnormal_count > 50)	//clock abnormal for long time
			{
				...
				_g_Disable_EQ_Offset_Cal[_gM14B0HDMIPhyInform.prt_sel] = 1;
		...
		_gM14B0HDMIPhyInform.tmds_clock[0] = 0;
	...
	for (port_num=0;port_num<4;port_num++)
	{
		...
		if ( _SCDT_Fall_Detected[port_num] )
		{
			...
			_gM14B0HDMIPhyInform.scdt_fall_count[port_num] ++;
			...
			&& _gM14B0HDMIPhyInform.phy_pdb[port_num] && _gM14B0HDMIPhyInform.phy_rstn[port_num])		// port detected but TCS not done yet
		{
			...
			if( abs( _Curr_TMDS_Clock[port_num] - _Prev_TMDS_Clock[port_num]) > 10 ) //100kHz
			{
				...
				/*
				   ...
				   else if( (_Stable_TMDS_Count[port_num] == HDMI_STABLE_TMDS_COUNT_MAX) && ( _Curr_TMDS_Clock[port_num] > 0) )
				   {
				 ...
				 */
			   if( _Curr_TMDS_Clock[port_num] > 0)
			   {
				   HDMI_NOTI("---- HDMI_PHY port [%d] : Clock Changed, Restart TCS \n", port_num );
				   PHY_REG_M14B0_RdFL(eq_cs_rs_sel);
				   PHY_REG_M14B0_Wr01(eq_cs_rs_sel, eq_cs_man_sel,0x0);
				   PHY_REG_M14B0_WrFL(eq_cs_rs_sel);
			...
			else if(_gM14B0HDMIPhyInform.tcs_done[port_num] == 0)
			{
				...
				if (tcs_done_value)
				{
					...
					else
					{
						...
						/* HDMI Portable Tester on Port2 : CS should be set to below 0x8 */
						if (tcs_max_value < 0x14 )
						{
							if(tcs_min_value <= 2)
								PHY_REG_M14B0_Wr01(eq_cs_man, eq_cs_man, /*tcs_min_value + */ 6);

						}
						else if(tcs_max_value == 0x14) // HDMI Portable Tester on other ports : CS should be below 0xC
						{
							if(tcs_min_value <= 2)
								PHY_REG_M14B0_Wr01(eq_cs_man, eq_cs_man, 0xa);
						}
						else if (tcs_min_value + 10 < 0x1f)		// for 15m cable
						{
							if(tcs_min_value == 0)
						...
						PHY_REG_M14B0_Rd01(eq_cs_man, eq_cs_man, _EQ_cs_man_value[port_num]);
					...
					_gM14B0HDMIPhyInform.tcs_min[port_num][0] = tcs_min_value;
					_gM14B0HDMIPhyInform.tcs_max[port_num][0] = tcs_max_value;
				}
			}
...
//		_HDMI_M14B0_Disable_TMDS_Error_Interrupt();
	...
	_HDMI_M14B0_Enable_TMDS_Error_Interrupt();
	_HDMI_M14B0_Enable_HDCP_Error_Interrupt();
	_HDMI_M14B0_Enable_TERC4_Error_Interrupt(_gM14B0HDMIPhyInform.prt_sel);

	if(_gM14B0_TMDS_ERROR_intr_count > 0)
	{
		_gM14B0_TMDS_ERROR_intr_count = 0;
		_gM14B0_TMDS_ERROR_count ++;
		_gM14B0HDMIPhyInform.ecc_error_count[0] ++;

		if( (_gM14B0HDMIPhyInform.tcs_done[_gM14B0HDMIPhyInform.prt_sel] == 1) && (_EQ_cs_man_value[_gM14B0HDMIPhyInform.prt_sel] == 4) && _gM14B0_tcs_fail_restart_count < 3 )
			_gM14B0_TMDS_ERROR_count_tcs_fail++;
		else
			_gM14B0_TMDS_ERROR_count_tcs_fail = 0;

		if(_gM14B0_TMDS_ERROR_count > 3)
		{
			HDMI_NOTI("!!! TMDS Errors [%d] : force scdt to zero , TCS[0x%x] !!!\n", _gM14B0_TMDS_ERROR_count, _EQ_cs_man_value[_gM14B0HDMIPhyInform.prt_sel] );
			_gM14B0_TMDS_ERROR_count = 0;
			scdt = 0;

			if(_gM14B0_TMDS_ERROR_count_tcs_fail > 200) // TCS Failed (TMDS error for 4-sec and tcs value == 4)
			{
				HDMI_NOTI("!!!!! TCS Failed : count [%d] ,TCS[0x%x], restart_count[%d] !!!!!\n", _gM14B0_TMDS_ERROR_count_tcs_fail, _EQ_cs_man_value[_gM14B0HDMIPhyInform.prt_sel] ,_gM14B0_tcs_fail_restart_count);
				HDMI_NOTI("!!!!! Restart TCS  !!!!!\n");

				_gM14B0_tcs_fail_restart_count++;
				_gM14B0_TMDS_ERROR_count_tcs_fail = 0;
...

				{
					...
					_gM14B0HDMIPhyInform.tcs_done[_gM14B0HDMIPhyInform.prt_sel] = 0;
	...
	if(_gM14B0_HDCP_ERROR_intr_count > 0)
	{
		_gM14B0_HDCP_ERROR_intr_count = 0;
		_gM14B0_HDCP_ERROR_count ++;
		_gM14B0HDMIPhyInform.hdcp_error_count[0] ++;

		if(_gM14B0_HDCP_ERROR_count > 3)
		{
			HDMI_NOTI("!!! HDCP Errors [%d] !!!\n", _gM14B0_HDCP_ERROR_count );
			_gM14B0_HDCP_ERROR_count = 0;
		}
	...
	if(_gM14B0_TERC4_ERROR_intr_count[0] > 0) {
		_gM14B0_TERC4_ERROR_intr_count[0] = 0;
		_gM14B0HDMIPhyInform.terc4_error_count[0] ++;
	}

	if(_gM14B0_TERC4_ERROR_intr_count[1] > 0) {
		_gM14B0_TERC4_ERROR_intr_count[1] = 0;
		_gM14B0HDMIPhyInform.terc4_error_count[1] ++;
	}

	if(_gM14B0_TERC4_ERROR_intr_count[2] > 0) {
		_gM14B0_TERC4_ERROR_intr_count[2] = 0;
		_gM14B0HDMIPhyInform.terc4_error_count[2] ++;
	}

	if(_gM14B0_TERC4_ERROR_intr_count[3] > 0) {
		_gM14B0_TERC4_ERROR_intr_count[3] = 0;
		_gM14B0HDMIPhyInform.terc4_error_count[3] ++;
	}
	...
	if ( (_gM14B0_TMDS_ERROR_EQ > 16) && ( elasped_time_eq < 3000 ) )	// 140403
	{
		...
		PHY_REG_M14B0_Rd01(eq_cs_man, eq_cs_man, _EQ_cs_man_value[_gM14B0HDMIPhyInform.prt_sel]);
		PHY_REG_M14B0_WrFL(eq_cs_man);
...
#ifdef M14_THREAD_READ_PHY_REG_VALUE
	ui64TmdsClock = (UINT64)(_gM14B0HDMIPhyInform.tmds_clock[0] ) * 10000;
#else
	PHY_REG_M14B0_RdFL(tmds_freq_1);
	PHY_REG_M14B0_RdFL(tmds_freq_2);
	...
	*/
	...
	while(1)
	{
		...
		if (_gM14B0Force_thread_sleep > 1)
		{
			...
			wait_event_interruptible(WaitQueue_HDMI_M14B0 , _gM14B0Force_thread_sleep == 0);
		}
		...
		if (_gHdmi_no_connection_count > 10)
		{
			...
			*/
			...
			if( wait_event_interruptible(WaitQueue_HDMI_M14B0 , _gM14B0HDMIState > HDMI_STATE_DISABLE ) < 0)
			{
...
#ifdef M14B0_HDCP_ERROR_DEBUG
	LINK_M14_Wr01(interrupt_enable_04, intr_auth_init_prt1_enable        , 0x1);		///< 20 intr_auth_init_prt1_enable
#else
...
#ifdef M14B0_HDCP_ERROR_DEBUG
	LINK_M14_Wr01(interrupt_enable_04, intr_auth_done_prt1_enable        , 0x1);		///< 23 intr_auth_done_prt1_enable
#else
...
irqreturn_t _HDMI_M14B0_IRQHandler(int irq, void *dev)
{
	...
	{
		_HDMI_M14B0_Disable_HDCP_Error_Interrupt();

		if( gHDMI_Phy_Control.link_pkt_error_workaround == 1) {
			_gM14B0_HDCP_ERROR_intr_count ++;
		}
	...
	{
		_HDMI_M14B0_Disable_TERC4_Error_Interrupt(0);

		if( gHDMI_Phy_Control.link_pkt_error_workaround == 1) {
			_gM14B0_TERC4_ERROR_intr_count[0] ++;
		}
	...
	{
		_HDMI_M14B0_Disable_TERC4_Error_Interrupt(1);

		if( gHDMI_Phy_Control.link_pkt_error_workaround == 1) {
			_gM14B0_TERC4_ERROR_intr_count[1] ++;
		}
	...
	{
		_HDMI_M14B0_Disable_TERC4_Error_Interrupt(2);

		if( gHDMI_Phy_Control.link_pkt_error_workaround == 1) {
			_gM14B0_TERC4_ERROR_intr_count[2] ++;
		}
	...
	{
		_HDMI_M14B0_Disable_TERC4_Error_Interrupt(3);

		if( gHDMI_Phy_Control.link_pkt_error_workaround == 1) {
			_gM14B0_TERC4_ERROR_intr_count[3] ++;
		}
	...
	{
		if ( _gM14B0HDMI_thread_running > 0)
		{
...
#ifdef M14B0_HDCP_ERROR_DEBUG
	if (LINK_M14_Rd_fld(interrupt_04, intr_auth_done_prt1))
		HDMI_INTR("@@@@@ intr_auth_done_prt1\n");

	if (LINK_M14_Rd_fld(interrupt_04, intr_auth_init_prt1))
		HDMI_INTR("@@@@@ intr_auth_init_prt1\n");
#endif
	...
	do {
		...
		else
		{
			...
			if (gM14BootData->mode == 1)		// for HDMI Mode
			{
				...
				{
#ifdef M14B0_HDCP_ERROR_DEBUG
					HDMI_NOTI("HDMI HDCP Information Changed !!!\n");

					for (temp = 0; temp < 4;temp++)
					{
						if( (hdcp_enc_en_info[temp] != _gM14B0HDMIPhyInform.hdcp_enc_en[temp]) || ( hdcp_authed_info[temp] !=  _gM14B0HDMIPhyInform.hdcp_authed[temp]) )
							HDMI_NOTI("@@@@@ Port[%d] : HDCP_ENC_EN[%d]=>[%d], HDCP_AUTHED[%d]=>[%d]\n", temp, hdcp_enc_en_info[temp], _gM14B0HDMIPhyInform.hdcp_enc_en[temp], \
...
int HDMI_M14B0_Get_Phy_Status(LX_HDMI_PHY_INFORM_T *sp_hdmi_phy_status)
{
	...
	_gM14B0HDMIPhyInform.tcs_result[0][0] = _EQ_cs_man_value[0];
	_gM14B0HDMIPhyInform.tcs_result[1][0] = _EQ_cs_man_value[1];
	_gM14B0HDMIPhyInform.tcs_result[2][0] = _EQ_cs_man_value[2];
	_gM14B0HDMIPhyInform.tcs_result[3][0] = _EQ_cs_man_value[3];
	...
	if(cd_sense_changed)
		{
			...
			if (phy_pdb_prt3 && phy_rstn_prt3)
			{
				...
				if (cd_sense)
				{
					...
					PHY_REG_M14B0_RdFL(odt_ctrl);
					PHY_REG_M14B0_Wr01(odt_ctrl,odt_ctrl,_g_mhl_odt_value);		// 0x0 //140514 : new board impedance 55=>50Ohm
					PHY_REG_M14B0_WrFL(odt_ctrl);
#endif
...
static int _HDMI_M14B0_MHL_Reconnect_Thread(void *data)
{
	...
	while(1)
	{
		...
		wait_event_interruptible(WaitQueue_MHL_M14B0 , _MHL_Reconnect_Thread_wakeup !=0 );
...
static int _HDMI_M14B0_MHL_PDB_Thread(void *data)
{
	...
	while(1)
	{
		...
		wait_event_interruptible(WaitQueue_MHL_PDB_M14B0 , _MHL_PDB_Thread_wakeup !=0 );
...
int HDMI_M14B0_Get_Driver_Status(LX_HDMI_DRIVER_STATUS_T *spHDMI_Driver_Status)
{
	...
	spHDMI_Driver_Status->TimingReadCnt[0] = _gM14B0TimingReadCnt;
	spHDMI_Driver_Status->HDMIState[0] = _gM14B0HDMIState;
	spHDMI_Driver_Status->PrevPixelEncoding[0] = _gM14B0PrevPixelEncoding;
	memcpy(&spHDMI_Driver_Status->PrevAVIPacket[0], &_gM14B0PrevAVIPacket, sizeof(LX_HDMI_INFO_PACKET_T));
	memcpy(&spHDMI_Driver_Status->PrevVSIPacket[0], &_gM14B0PrevVSIPacket, sizeof(LX_HDMI_INFO_PACKET_T));
	spHDMI_Driver_Status->AVIReadState[0] = _gM14B0AVIReadState;
	spHDMI_Driver_Status->VSIState[0] = _gM14B0VSIState;
	spHDMI_Driver_Status->AVIChangeState[0] = _gM14B0AVIChangeState;
	spHDMI_Driver_Status->PortSelected[0] = _gM14B0PortSelected;
	spHDMI_Driver_Status->MHLContentOff = _gM14B0MHLContentOff;

	spHDMI_Driver_Status->AudioMuteState[0] = _gM14B0AudioMuteState;
	spHDMI_Driver_Status->AudioArcStatus[0] = _gM14B0AudioArcStatus;
	spHDMI_Driver_Status->IntrAudioState[0] = _gM14B0IntrAudioState;
	spHDMI_Driver_Status->HdmiPortStableCount[0] = _gM14B0HdmiPortStableCount;
	spHDMI_Driver_Status->HdmiFreqErrorCount[0] = _gM14B0HdmiFreqErrorCount;
	spHDMI_Driver_Status->HdmiAudioThreadCount[0] = _gM14B0HdmiAudioThreadCount;
	spHDMI_Driver_Status->IntrBurstInfoCount[0] = _gM14B0IntrBurstInfoCount;
	spHDMI_Driver_Status->TaskBurstInfoCount[0] = _gM14B0TaskBurstInfoCount;
	memcpy(&spHDMI_Driver_Status->HdmiAudioInfo[0], &_gM14B0HdmiAudioInfo, sizeof(LX_HDMI_AUDIO_INFO_T));

	spHDMI_Driver_Status->Intr_vf_stable[0] = _gM14B0Intr_vf_stable;
	spHDMI_Driver_Status->Intr_avi[0] = _gM14B0Intr_avi;
	spHDMI_Driver_Status->Intr_src_mute[0] = _gM14B0Intr_src_mute;
	spHDMI_Driver_Status->Intr_vsi[0] = _gM14B0Intr_vsi;
	...
	spHDMI_Driver_Status->CHG_AVI_count_MHL[0] = _gM14B0CHG_AVI_count_MHL;
	spHDMI_Driver_Status->CHG_AVI_count_EQ[0] = _gM14B0CHG_AVI_count_EQ;
	spHDMI_Driver_Status->TMDS_ERROR_EQ[0] = _gM14B0_TMDS_ERROR_EQ;

	spHDMI_Driver_Status->Driver_Revision = 0x14102700;
	...
	for (port_num = 0; port_num <4; port_num++)
	{
				...
				&& (_gM14B0HDMIPhyInform.scdt[port_num] ) )
		{
			...
			if (hdcp_unauthed_count[port_num] > 200)	//for about 4-sec
			{
				...
				if( (port_num == 3) &&  _gM14B0HDMIPhyInform.cd_sense && (_gM14B0HDMIPhyInform.cbus_st == 3) && _gM14B0HDMIPhyInform.cbus_conn_done)
				{
					int cbus_st_count, state_sink_clr_hpd, state_sink_set_hpd;
...
int HDMI_M14B0_Key_Control(LX_HDMI_CONTROL_KEY_TYPE_T key_control_cmd)
{
	int ret = RET_OK;
...
int HDMI_M14B0_Set_MHL_ODT(LX_HDMI_MHL_ODT_IMPEDANCE_T odt_value)
{
	int ret = RET_OK;
