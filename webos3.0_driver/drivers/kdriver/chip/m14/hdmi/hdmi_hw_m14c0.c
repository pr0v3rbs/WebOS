 ...
 *				DEFINES
 *****************************************************************************/
#define WORK_NEEDS_TO_BE_DONE_BY_DOSA 0
...
//#define M14C0_MHL_CONNECTION_BUG_WORKAROUND
...
#define HDMI_STABLE_TMDS_COUNT_MAX 1
/**
 *	Global variables of the driver
 */
extern OS_SEM_T	g_HDMI_Sema;
extern LX_HDMI_REG_T *pM14C0HdmiRegCfg;
extern HDMI_DATA_T *gM14BootData;

extern  HDMI_PHY_CONTROL_T	gHDMI_Phy_Control;
static HDMI_PORT_CONNECTION_TYPE_T _gM14C0HDMIConnectState = HDMI_PORT_NOT_CONNECTED;
static LX_HDMI_TIMING_INFO_T _gM14C0PrevTiming = {0,};
static UINT32 _gM14C0TimingReadCnt = 0;
static UINT32 _gM14C0HDMIState = HDMI_STATE_DISABLE;

static LX_HDMI_AVI_COLORSPACE_T	_gM14C0PrevPixelEncoding = LX_HDMI_AVI_COLORSPACE_RGB;
static LX_HDMI_INFO_PACKET_T _gM14C0PrevAVIPacket = {0, };
static LX_HDMI_INFO_PACKET_T _gM14C0PrevVSIPacket = {0, };
static BOOLEAN _gM14C0AVIReadState 	  = FALSE;
static BOOLEAN _gM14C0VSIState 	  = FALSE;
static BOOLEAN _gM14C0AVIChangeState 	  = FALSE;
static BOOLEAN _gM14C0PortSelected 	  = FALSE;
static UINT32 _gM14C0MHLContentOff = 0;	// MHL RAP support(if content off is '1' set timing info to '0' to enable AV mute.

//audio
static BOOLEAN _gM14C0AudioDebugState	  = FALSE; 	//This variable is used Audio Debug Status.
static BOOLEAN _gM14C0AudioMuteState 	  = FALSE;
static BOOLEAN _gM14C0AudioArcStatus 	  = FALSE;	//This variable is used ARC Enabled Status to recover ARC mode after PHY reset.
static UINT32  _gM14C0IntrAudioState 	  = HDMI_AUDIO_INIT_STATE;
static UINT32  _gM14C0HdmiPortStableCount  = 0;	//This count is used to detect if HDMI Switch port is changed.
static UINT32  _gM14C0HdmiFreqErrorCount   = 0;	//This count is used to detect if HDMI TMDS Frequency and Frequency is different.
static UINT32  _gM14C0HdmiAudioThreadCount = 0;	//_gM14C0HdmiAudioThreadCount is increased in _HDMI_M14C0_Periodic_Audio_Task function
static UINT64  _gM14C0IntrBurstInfoCount   = 0;	//This count is used to detect if burst info. interrupt is asserted count.
static UINT64  _gM14C0TaskBurstInfoCount   = 0;	//This count is used to save _gM14C0IntrBurstInfoCount in hdmi task.
static UINT32  _gM14C0CtsInterruptCount    = 0;	//This count is used to debug if CTS interrupt is raised.
static UINT64  _gM14C0BurstInfoPrevJiffies = 0;	//This Jiffies is used to save previous burst info. interrupt jiffies.
static LX_HDMI_AUDIO_INFO_T _gM14C0HdmiAudioInfo = {LX_HDMI_AUDIO_DEFAULT, LX_HDMI_SAMPLING_FREQ_NONE};

//interrupt variables
static UINT32 _gM14C0Intr_vf_stable = HDMI_VIDEO_INIT_STATE;
static UINT32 _gM14C0Intr_avi = HDMI_AVI_INIT_STATE;
static UINT32 _gM14C0Intr_src_mute = HDMI_SOURCE_MUTE_CLEAR_STATE;
static UINT32 _gM14C0Intr_vsi = HDMI_VSI_INIT_STATE;

static UINT32 _gM14C0Force_thread_sleep  = 0;	//enable  = 2, disable  =0
static UINT32 _gM14C0HDMI_thread_running = 0;	//enable  = 1, disable  =0
static UINT32 _gM14C0HDMI_interrupt_registered = 0;

static UINT32 _gM14C0ChipPlatform = 0;

//static int _gPathen_set_count = 0;
static int _gM14C0CHG_AVI_count_MHL = -1;
static int _gM14C0CHG_AVI_count_EQ = -1;
static int _gM14C0_TMDS_ERROR_EQ = -1;
static int _gM14C0_TMDS_ERROR_EQ_2nd = -1;
static int _gM14C0_TMDS_ERROR_intr_count = 0;
static int _gM14C0_TMDS_ERROR_count = 0;
static int _gM14C0_TMDS_ERROR_count_tcs_fail = 0;
static int _gM14C0_tcs_fail_restart_count = 0;

static int _gM14C0_TERC4_ERROR_intr_count[4] = {0,};
static int _gM14C0_HDCP_ERROR_intr_count = 0;
static int _gM14C0_HDCP_ERROR_count = 0;

static int _gM14C0_HDCP_recovery_count = 0;
static int _gM14C0_TMDS_ERROR_count_HDCP_recovery = 0;
static int _gM14C0_HDCP_ERROR_count_HDCP_recovery = 0;

static int _HDMI_M14C0_ResetPortControl(int reset_enable);
static int _HDMI_M14C0_TMDS_ResetPort(int port_num);
static int _HDMI_M14C0_HDCP_ResetPort(int port_num);
#ifdef NOT_USED_NOW
static int _HDMI_M14C0_TMDS_HDCP_ResetPort(int port_num);
static int _HDMI_M14C0_TMDS_HDCP_ResetPort_Control(int port_num, int reset);
#endif	//#ifdef NOT_USED_NOW
static int _HDMI_M14C0_TMDS_ResetPort_Control(int port_num, int reset);
static int _HDMI_M14C0_HDCP_ResetPort_Control(int port_num, int reset);
static int _HDMI_M14C0_SetPortConnection(void);
static int _HDMI_M14C0_EnablePort(int prt_num, int enable);
static int _HDMI_M14C0_Get_ManMHLMode(void);
static int _HDMI_M14C0_Set_ManMHLMode(int man_mhl_mode, int man_mhl_value);
static int _HDMI_M14C0_Disable_TMDS_Error_Interrupt(void);
static int _HDMI_M14C0_Enable_TMDS_Error_Interrupt(void);
static int _HDMI_M14C0_Disable_TERC4_Error_Interrupt(int port_num);
static int _HDMI_M14C0_Enable_TERC4_Error_Interrupt(int port_num);
static int _HDMI_M14C0_Disable_HDCP_Error_Interrupt(void);
static int _HDMI_M14C0_Enable_HDCP_Error_Interrupt(void);
#ifdef NOT_USED_NOW
static int _HDMI_M14C0_Set_HDCP_Unauth(int port_num, int unauth_nosync, int unauth_mode_chg);
#endif	//#ifdef NOT_USED_NOW
static int _HDMI_M14C0_Check_RAP_Content(int clear);
static int _HDMI_M14C0_MHL_Check_Status(int init);
static int _HDMI_M14C0_HDCP_Check_Status(void);

#ifdef ODT_PDB_OFF_ON_WORKAROUND
static int _HDMI_M14C0_Phy_Enable_Register_Access(int port_num);
static int _HDMI_M14C0_EQ_PDB_control(int reset);
#endif
static void _HDMI_M14C0_ModifyUDFormat(LX_HDMI_TIMING_INFO_T *info, UINT32 tmdsClock);
static void _HDMI_M14C0_CheckUDExtInfo(LX_HDMI_TIMING_INFO_T *info, UINT32 tmdsClock);
static int _HDMI_M14C0_Check_MHL_PP_mode(void);
static int _HDMI_M14C0_Check_MHP_PP_link_mode(void);
static int _HDMI_M14C0_Get_HDCP_RI(int port_num, int *hdcp_ri_value);

static	UINT8 HDCP_AN_Data_Zero[8] ={0,} ;
static	UINT8 HDCP_AKSV_Data_Zero[5] ={0,} ;
static LX_HDMI_PHY_INFORM_T _gM14C0HDMIPhyInform = {0,};
...
static int _HDMI_M14C0_TCS_addtional_value_init = 0xa;		//default : 0x8
static int _HDMI_M14C0_TCS_addtional_value_2nd = 0x5;			//default : 0x8
static int _HDMI_M14C0_TCS_addtional_value_3rd = 0x0f;		//default : 0x8


static int _HDMI_M14C0_TCS_mode_sel = 0x3;		// 0: TCS out, 1:CK_MODE, 2:CS manual value, 3 : TCS out channel selection
static int _HDMI_M14C0_hdmi_dr_n1_value = 0x2;	//141218(PC38363)  before : 0x7
static int _HDMI_M14C0_eq_eval_time_value = 0x1;	//141218(PC38363)  before : 0xffff

static int _HDMI_M14C0_TCS_result[4][3] = {{0,},};
static int _HDMI_M14C0_TCS_add[4] = {0,};

//wait_queue_head_t	WaitQueue_HDMI;
static DECLARE_WAIT_QUEUE_HEAD(WaitQueue_HDMI_M14C0);
static DECLARE_WAIT_QUEUE_HEAD(WaitQueue_MHL_M14C0);
static DECLARE_WAIT_QUEUE_HEAD(WaitQueue_MHL_PDB_M14C0);

static DEFINE_SPINLOCK(_gIntrHdmiM14C0VideoLock);
#ifdef NOT_USED_NOW
static DEFINE_SPINLOCK(_gHdmiM14C0RegReadLock);
#endif	//#ifdef NOT_USED_NOW

//HDMI Audio Variables
static DEFINE_SPINLOCK(_gIntrHdmiM14C0AudioLock);
...
static int	_gM14C0_HDMI_MHLAutoModeCnt = 0;
...
static int _g_mhl_enable_flag = 1;		//default MHL mode enabled
static UINT32	_g_Disable_EQ_Offset_Cal[4] = {0,};

static int _g_mhl_odt_value = 3;	// default : 46Ohm
/******************************************************************************
 *				DATA STRUCTURES
 *****************************************************************************/
/**
...
PHY_REG_CTRL_M14C0_T  gPHY_REG_CTRL_M14C0;
static volatile HDMI_LINK_REG_M14C0_T *pstLinkReg;
static volatile HDMI_LINK_REG_M14C0_T *pstLinkShadowReg;
 ...
 *				Local function
 *****************************************************************************/
static int _HDMI_M14C0_GetExtendTimingInfo(LX_HDMI_TIMING_INFO_T *info, UINT32 tmdsClock);
static int _HDMI_M14C0_ClearSWResetAll(void);			//M14C0D Chip - Ctop control
static int _HDMI_M14C0_PhyRunReset(void);
static int _HDMI_M14C0_RunReset(void);
static int _HDMI_M14C0_GetMHLConection(void);
static int _HDMI_M14C0_GetPortConnection(void);
static int _HDMI_M14C0_SetInternalMute(LX_HDMI_MUTE_CTRL_T interMute);
static int _HDMI_M14C0_SetVideoBlank(LX_HDMI_AVI_COLORSPACE_T space);
static int _HDMI_M14C0_EnableInterrupt(void);
static int _HDMI_M14C0_Thread(void *data);
irqreturn_t _HDMI_M14C0_IRQHandler(int irq, void *dev);
static void _HDMI_M14C0_Periodic_Task(void);

//audio related.
static int _HDMI_M14C0_SetAudio(void);
static int _HDMI_M14C0_GetAudioInfo(void);
static int _HDMI_M14C0_GetAudioTypeAndFreq(LX_HDMI_AUDIO_TYPE_T *audioType, LX_HDMI_SAMPLING_FREQ_T *samplingFreq);
static int _HDMI_M14C0_GetAudioFreqFromTmdsClock(LX_HDMI_SAMPLING_FREQ_T *samplingFreqFromTmds);
static int _HDMI_M14C0_DebugAudioInfo(LX_HDMI_DEBUG_AUDIO_INFO_T *pAudioDebugInfo);

static int _HDMI_M14C0_PhyOn(int port_num);
static int _HDMI_M14C0_PhyOn_5V(int port_num);
static int _HDMI_M14C0_PhyOff(int port_num);
#ifdef NOT_USED_NOW
static int _HDMI_M14C0_ResetPort(int port_num);
#endif	//#ifdef NOT_USED_NOW
static int _HDMI_M14C0_Get_EDID_Rd(int port_num , int *edid_rd_done, int *edid_rd_cnt);
static int _HDMI_M14C0_Get_HDCP_info(int port_num , UINT32 *hdcp_authed, UINT32 *hdcp_enc_en);
#ifdef NOT_USED_NOW
static int _HDMI_M14C0_Write_EDID_Data_A4P(UINT32 *pedid_data);
#endif	//#ifdef NOT_USED_NOW
static int _HDMI_M14C0_Get_HDMI5V_Info_A4P(int *pHDMI5V_Status_PRT0, int *pHDMI5V_Status_PRT1 ,int *pHDMI5V_Status_PRT2, int *pHDMI5V_Status_PRT3);
static int _HDMI_M14C0_Get_HDMI5V_Info(int port_num, int *pHDMI5V_Status);
static int _HDMI_M14C0_Set_HPD_Out(int prt_sel, int hpd_out_value);
static int _HDMI_M14C0_Set_HPD_Out_A4P(int HPD_Out_PRT0, int HPD_Out_PRT1 ,int HPD_Out_PRT2, int HPD_Out_PRT3);
static int _HDMI_M14C0_Get_HPD_Out_A4P(int *pHPD_Out_PRT0, int *pHPD_Out_PRT1 ,int *pHPD_Out_PRT2, int *pHPD_Out_PRT3);

static int _HDMI_M14C0_Get_AN_Data(int port_num, UINT8 *AN_Data);
static int _HDMI_M14C0_MHL_Reconnect_Thread(void *data);
static int _HDMI_M14C0_MHL_PDB_Thread(void *data);
static int _HDMI_M14C0_Phy_Reset(int port_num);
static int _HDMI_M14C0_Check_Aksv_Exist(int port_num);
static int _HDMI_M14C0_swrst_TMDS_sel_control(int reset);

int HDMI_M14C0_HWInitial(void)
{
	gPHY_REG_CTRL_M14C0.shdw.addr = (UINT32 *)OS_KMalloc(sizeof(PHY_REG_M14C0_T));

#ifndef	USE_QEMU_SYSTEM
	pstLinkReg =(HDMI_LINK_REG_M14C0_T *)ioremap(pM14C0HdmiRegCfg->link_reg_base_addr, pM14C0HdmiRegCfg->link_reg_size);
#else
	pstLinkReg =(HDMI_LINK_REG_M14C0_T *)ioremap(pM14C0HdmiRegCfg->link_qemu_base_addr, pM14C0HdmiRegCfg->link_reg_size);
#endif

	HDMI_PRINT("HDMI LINK REG CFG \n");
	HDMI_PRINT("LINK REG Base Address	0x%08x\n", pM14C0HdmiRegCfg->link_reg_base_addr);
	HDMI_PRINT("LINK REG Size 			0x%08x\n", pM14C0HdmiRegCfg->link_reg_size);
	HDMI_PRINT("LINK REG Qumu Address 	0x%08x\n", pM14C0HdmiRegCfg->link_qemu_base_addr);
	...
	pstLinkShadowReg = (HDMI_LINK_REG_M14C0_T *)kmalloc(sizeof(HDMI_LINK_REG_M14C0_T), GFP_KERNEL);
	...
	_gM14C0HDMIPhyInform.phy_status = HDMI_PHY_STATUS_DISCONNECTED;

	_gM14C0HDMIPhyInform.hpd_pol[0] = 1;
	_gM14C0HDMIPhyInform.hpd_pol[1] = 1;
	_gM14C0HDMIPhyInform.hpd_pol[2] = 1;
	_gM14C0HDMIPhyInform.hpd_pol[3] = 1;
	...
	_gM14C0ChipPlatform = lx_chip_plt();
	HDMI_M14C0_Key_Control(HDMI_KEY_SAVE);
	HDMI_M14C0_Key_Control(HDMI_KEY_CLEAR);

	stHDMI_Thread = kthread_create(_HDMI_M14C0_Thread, (void*)NULL, "hdmi_thread");
	if (stHDMI_Thread)
	{
		wake_up_process(stHDMI_Thread);
		_gM14C0HDMI_thread_running = 1;
	...
	stMHL_Reconnect_Thread = kthread_create(_HDMI_M14C0_MHL_Reconnect_Thread, (void*)NULL, "mhl_reconnect_thread");
	...
	stMHL_PDB_Thread = kthread_create(_HDMI_M14C0_MHL_PDB_Thread, (void*)NULL, "mhl_pdb_thread");
	if (stMHL_PDB_Thread)
	{
...
//	HDMI_M14C0_SetPowerControl(0);

	HDMI_TRACE("M14C0 HDMI Initialize \n" );
	return RET_OK;
}
...
//	int loop, port_authed, port_enc_en;
	...
	spin_lock_irqsave(&_gIntrHdmiM14C0AudioLock, flags);

	//Clears a global value for audio info.
	_gM14C0HdmiAudioInfo.audioType	  = LX_HDMI_AUDIO_NO_AUDIO;
	_gM14C0HdmiAudioInfo.samplingFreq = LX_HDMI_SAMPLING_FREQ_NONE;

	//Clear _gM14C0IntrAudioState
	_gM14C0IntrAudioState = HDMI_AUDIO_INIT_STATE;

	//Reset _gM14C0IntrBurstInfoCount
	_gM14C0IntrBurstInfoCount = 0;

	//Clear _gM14C0HdmiFreqErrorCount
	_gM14C0HdmiFreqErrorCount = 0;

	_gM14C0HDMIState = HDMI_STATE_IDLE;

	if( port_selected != *port)
	{
		_HDMI_M14C0_swrst_TMDS_sel_control(1);
		_gM14C0Intr_vf_stable = HDMI_VIDEO_INIT_STATE;
		_gM14C0Intr_avi = HDMI_AVI_INIT_STATE;
		_gM14C0Intr_vsi = HDMI_VSI_INIT_STATE;
		_gM14C0Intr_src_mute = HDMI_SOURCE_MUTE_CLEAR_STATE;
	}
	_gM14C0TimingReadCnt = 0;
	_gM14C0AVIReadState = FALSE;
	_gM14C0VSIState = FALSE;
	_gM14C0AVIChangeState = FALSE;

	//spin lock for protection : unlock
	spin_unlock_irqrestore(&_gIntrHdmiM14C0AudioLock, flags);
	...
	do {
		...
		//_gM14C0CHG_AVI_count_EQ = 0;
		if( gHDMI_Phy_Control.sw_eq_algorithm_workaround)
		{
			_gM14C0_TMDS_ERROR_EQ = 0;
			_gM14C0_TMDS_ERROR_EQ_2nd = -1;
		}
		_gM14C0_tcs_fail_restart_count = 0;
//		_HDMI_M14C0_Enable_TMDS_Error_Interrupt();
		_TCS_Done_Time[*port] = jiffies_to_msecs(jiffies);


#ifdef M14_CODE_FOR_MHL_CTS
		_HDMI_M14C0_Check_RAP_Content(1);
#endif

		if (gHDMI_Phy_Control.check_an_data)
		{
			if (port_selected == 0)
				_HDMI_M14C0_Get_AN_Data(port_selected, gHDMI_Phy_Control.AN_Data_Prt0);
			else if (port_selected == 1)
				_HDMI_M14C0_Get_AN_Data(port_selected, gHDMI_Phy_Control.AN_Data_Prt1);
			else if (port_selected == 2)
				_HDMI_M14C0_Get_AN_Data(port_selected, gHDMI_Phy_Control.AN_Data_Prt2);
			else if (port_selected == 3)
				_HDMI_M14C0_Get_AN_Data(port_selected, gHDMI_Phy_Control.AN_Data_Prt3);
		...
		{
			if (gHDMI_Phy_Control.check_an_data)
				_HDMI_M14C0_TMDS_ResetPort(port_selected);
			else
				_HDMI_M14C0_TMDS_ResetPort(port_selected);
		}
		...
		{
			if (*port != 0)
				_HDMI_M14C0_PhyOff(0);
			if (*port != 1)
				_HDMI_M14C0_PhyOff(1);
			if (*port != 2)
				_HDMI_M14C0_PhyOff(2);
			if (*port != 3)
				_HDMI_M14C0_PhyOff(3);
		}
		...
		if (*port != 0)
			_HDMI_M14C0_Set_HDCP_Unauth(0, 1, 1);
		if (*port != 1)
			_HDMI_M14C0_Set_HDCP_Unauth(1, 1, 1);
		if (*port != 2)
			_HDMI_M14C0_Set_HDCP_Unauth(2, 1, 1);
		if (*port != 3)
			_HDMI_M14C0_Set_HDCP_Unauth(3, 1, 1);
#endif

#ifdef M14_CODE_FOR_MHL_CTS
		if (_gM14C0HDMIPhyInform.cd_sense)
		{
#if 0
		...
		if(*port == 3) {
			...
			LINK_M14_Wr01(phy_link_06, reg_man_pp_mode, 0x0);
			LINK_M14_Wr01(phy_link_06, reg_man_pp_value, 0x0);
			LINK_M14_WrFL(phy_link_06);
		}
		else {
			LINK_M14_RdFL(phy_link_06);
			LINK_M14_Wr01(phy_link_06, reg_man_pp_mode, 0x1);
			LINK_M14_Wr01(phy_link_06, reg_man_pp_value, 0x0);
			LINK_M14_WrFL(phy_link_06);
		}
		...
		_HDMI_M14C0_Get_HDMI5V_Info(*port, &hdmi5v_status);
		...
		if (gHDMI_Phy_Control.check_an_data && hdmi5v_status && (prev_port_an_data_zero) )
		{
			...
			spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
			gHDMI_Phy_Control.port_to_change = *port;
			gHDMI_Phy_Control.port_change_start = 1;
			spin_unlock_irqrestore(&_gIntrHdmiM14C0VideoLock, flags);
#if 0
			for (loop=0;loop<100;loop++)
			{
				_HDMI_M14C0_Get_AN_Data(*port, HDCP_AN_Data);
				...
				if ( HDCP_AN_Data[0] || HDCP_AN_Data[1] || HDCP_AN_Data[2] || HDCP_AN_Data[3] || \
					...
					break;
				/*
				   _HDMI_M14C0_Get_HDCP_info(*port , &port_authed, &port_enc_en);
		...
		else
		{
			...
			else
			{
				...
				if (*port != 0)
					_HDMI_M14C0_EnablePort(0, 0);
				if (*port != 1)
					_HDMI_M14C0_EnablePort(1, 0);
				if (*port != 2)
					_HDMI_M14C0_EnablePort(2, 0);
				if (*port != 3)
					_HDMI_M14C0_EnablePort(3, 0);

				/* Enable port_num */
				_HDMI_M14C0_EnablePort(*port, 1);
			}
	...
	if (_gM14C0AudioArcStatus == TRUE)
	{
		...
		LINK_M14_Wr01(phy_link_00, phy_arc_odt_sel_prt0, 0x2);			///< Port1 PHY ARC Temination
		LINK_M14_Wr01(phy_link_00, phy_arc_pdb_prt0, (UINT32)_gM14C0AudioArcStatus);	///< Port1 PHY ARC PDB
	...
	if( port_selected != *port)
		_HDMI_M14C0_swrst_TMDS_sel_control(0);

	HDMI_DEBUG("[%d] %s :  ARC = %s \n", __L__, __F__, _gM14C0AudioArcStatus? "On":"Off");
	return ret;
}
...
int HDMI_M14C0_Thread_Control(int sleep_enable)
{
	if (sleep_enable)
	{
		_gM14C0Force_thread_sleep = 2;
		_gM14C0HDMIState = HDMI_STATE_DISABLE;
		HDMI_NOTI("M14C0 sleeping HDMI Thread \n");
	}
	else
	{
		wake_up_interruptible(&WaitQueue_HDMI_M14C0);
		_gM14C0HDMIState = HDMI_STATE_IDLE;
		_gM14C0Force_thread_sleep = 0;
		_gM14C0HDMI_thread_running = 1;
		HDMI_NOTI("M14C0 Starting HDMI Thread\n");
	}
...
{
	ratio->eAspectRatio = (((_gM14C0PrevAVIPacket.dataBytes[0] &0xff0000)>>16) &0x30)>>4;		// M1M0
...
int HDMI_M14C0_GetColorDomain(LX_HDMI_COLOR_DOMAIN_T *color)
{
	...
	if (_gM14C0Intr_vf_stable == HDMI_VIDEO_INIT_STATE || _gM14C0Intr_avi == HDMI_AVI_INIT_STATE)
	{
		return RET_OK;
	}

	if (_gM14C0Intr_avi > HDMI_AVI_INIT_STATE)
	{
		color->bHdmiMode =  gM14BootData->mode;

		if (color->bHdmiMode)
		{
			if (_gM14C0AVIReadState)
				temp = _gM14C0PrevAVIPacket.dataBytes[0];
			else
			{
...
int HDMI_M14C0_GetTimingInfo(LX_HDMI_TIMING_INFO_T *info)
{
	...
	do{
		if (_gM14C0HDMI_thread_running == 1 && _gM14C0HDMIState < HDMI_STATE_READ)
			break;

		if (_gM14C0HDMIState < HDMI_STATE_SCDT)	// 140408 : added for condition when no connection, but prev timing exist
			break;

		if (_gM14C0PortSelected == FALSE)
			break;

		if (_gM14C0HDMI_thread_running == 1 && _gM14C0HDMIState == HDMI_STATE_STABLE && _gM14C0PrevTiming.vFreq > 0)
			goto func_exit;
		...
		tmdsClock = _gM14C0HDMIPhyInform.tmds_clock[0]; 	// XXX.XX KHz
#else
		PHY_REG_M14C0_RdFL(tmds_freq_1);
		PHY_REG_M14C0_RdFL(tmds_freq_2);

		PHY_REG_M14C0_Rd01(tmds_freq_1,tmds_freq,up_freq);
		PHY_REG_M14C0_Rd01(tmds_freq_2,tmds_freq,down_freq);
		tmdsClock = ((up_freq << 8) + down_freq); 	// XXX.XX KHz
#endif
		_HDMI_M14C0_ModifyUDFormat(&bufTiming, tmdsClock);		// modify 297MHz format

		_HDMI_M14C0_GetExtendTimingInfo(&bufTiming, tmdsClock);		// get extend information
	...
	//	_HDMI_M14C0_GetExtendTimingInfo(&bufTiming);		// get extend information

		if (bufTiming.vActive == 0 || bufTiming.hActive == 0 )
		{
			HDMI_PRINT("[%d] %s : _HDMI_M14C0_GetExtendTimingInfo hActive, vActive  = 0\n", __L__, __F__);
			//memset(&bufTiming , 0 , sizeof(LX_HDMI_TIMING_INFO_T));
			goto func_exit;
		}
		...
		if (tmdsClock != 0 && bufTiming.vTotal > 0 && bufTiming.hTotal > 0 )		// XXX.XX KHz
		{
			...
			if( ( _HDMI_M14C0_Check_MHL_PP_mode() > 0 ) && ( _gM14C0HDMIPhyInform.prt_sel == 3) && (_gM14C0HDMIPhyInform.cd_sense == 1) )
				bufTiming.vFreq = bufTiming.vFreq << 1;
		...
		if ((_gM14C0HDMIState > HDMI_STATE_NO_SCDT && _gM14C0HDMIState <= HDMI_STATE_READ) ||_gM14C0HDMI_thread_running == 0)
		{
			if (_gM14C0Intr_vf_stable > HDMI_VIDEO_INIT_STATE)
			{
		...
		// M14C0 Added for 420 format
		if( ( _gM14C0PrevPixelEncoding == LX_HDMI_AVI_COLORSPACE_YCBCR420) && (bufTiming.hActive >= 1920) && (bufTiming.vActive == 2160) && (bufTiming.scanType == 1)) {
...
func_cnt:
		...
		//if ( _gM14C0TimingReadCnt > 100) 	goto func_exit;

		if ( memcmp(&_gM14C0PrevTiming, &bufTiming, sizeof(LX_HDMI_TIMING_INFO_T)) == 0 )
		{
			if (_gM14C0TimingReadCnt <= 100)
				_gM14C0TimingReadCnt++;

			if (_gM14C0HDMIState == HDMI_STATE_READ && _gM14C0TimingReadCnt < 9)
				HDMI_VIDEO("[%d] %s : hActive[%d] vActive [%d] ReadCnt[%d] \n", __L__, __F__, bufTiming.hActive, bufTiming.vActive,_gM14C0TimingReadCnt);
		}
...
func_exit:
		...
		memcpy(&bufTiming, &_gM14C0PrevTiming, sizeof(LX_HDMI_TIMING_INFO_T));
	} while(0);

	info->hFreq 	= _gM14C0PrevTiming.hFreq 	= bufTiming.hFreq; 			///< Horizontal frequency(100 Hz unit) = Dot Freq / hTotal
	info->vFreq 	= _gM14C0PrevTiming.vFreq 	= bufTiming.vFreq; 			///< Veritical frequency(1/10 Hz unit) = hFreq / vTotal
	info->hTotal	= _gM14C0PrevTiming.hTotal 	= bufTiming.hTotal; 		///< Horizontal total pixels
	info->vTotal 	= _gM14C0PrevTiming.vTotal	= bufTiming.vTotal; 		///< Vertical total lines
	info->hStart	= _gM14C0PrevTiming.hStart	= bufTiming.hStart; 		///< Horizontal start pixel (Back Porch)
	info->vStart 	= _gM14C0PrevTiming.vStart	= bufTiming.vStart;			///< Vertical start lines (Back Porch)
	info->hActive 	= _gM14C0PrevTiming.hActive	= bufTiming.hActive;		///< Horizontal active pixel
	info->vActive 	= _gM14C0PrevTiming.vActive	= bufTiming.vActive; 		///< Vertical active lines
	info->scanType 	= _gM14C0PrevTiming.scanType	= bufTiming.scanType; 		///< Scan type (0 : interlace, 1 : progressive) 	info->scanType ^= 1;
	info->extInfo	= _gM14C0PrevTiming.extInfo	= bufTiming.extInfo; 	///< Full 3D Timing
	info->state = _gM14C0HDMIState;

	/* Workaround Code for FP vactive timiming with Non-FP 3D information */
	if ( _gM14C0Intr_vsi >= HDMI_VSI_STABLE_STATE)
	{
				  ...
				  && (vactive_reg_value != 503) ) )
		{
			...
			if(abnormal_FP_count == 3)
			{
				...
				HDMI_NOTI("#### Port[%d], 3D[%d], Vactive[%d], Vactive_reg[%d] #\n", \
						_gM14C0HDMIPhyInform.prt_sel, info->extInfo, info->vActive, vactive_reg_value);
				HDMI_NOTI("#########################################\n");
				HDMI_NOTI("#########################################\n");

				_g_abnormal_3d_format = 1;
				/*
				_HDMI_M14C0_TMDS_HDCP_ResetPort_Control(_gM14C0HDMIPhyInform.prt_sel, 0);

				PHY_REG_M14C0_RdFL(cr_mode_sel_resetb);
				PHY_REG_M14C0_Wr01(cr_mode_sel_resetb, cr_mode_sel_resetb,0x0);
				PHY_REG_M14C0_WrFL(cr_mode_sel_resetb);

				OS_MsecSleep(10);	// ms delay

				PHY_REG_M14C0_RdFL(cr_mode_sel_resetb);
				PHY_REG_M14C0_Wr01(cr_mode_sel_resetb, cr_mode_sel_resetb,0x1);
				PHY_REG_M14C0_WrFL(cr_mode_sel_resetb);

				OS_MsecSleep(10);	// ms delay

				_HDMI_M14C0_TMDS_HDCP_ResetPort_Control(_gM14C0HDMIPhyInform.prt_sel, 1);
				*/
			}
		}
		else
	...
	if (_gM14C0MHLContentOff && (_gM14C0HDMIPhyInform.prt_sel == 3) && (_gM14C0HDMIPhyInform.cd_sense == 1) )
		memset(info , 0 , sizeof(LX_HDMI_TIMING_INFO_T));

	/*	test for abnormal source
	if (bufTiming.vFreq > 600 && _gM14C0HDMIState == HDMI_STATE_READ)
	{
		HDMI_VIDEO("[%d] %s : VFreq [%d] ReadCnt[%d] \n", __L__, __F__, bufTiming.vFreq, _gM14C0TimingReadCnt);
		memset(info , 0 , sizeof(LX_HDMI_TIMING_INFO_T));
	}
	*/
...
int HDMI_M14C0_GetAviPacket(LX_HDMI_INFO_PACKET_T *packet)
{
	...
	{
		gAVIReadCnt = 0;
		memset(&_gM14C0PrevAVIPacket, 0 , sizeof(LX_HDMI_INFO_PACKET_T));
		//_gM14C0PrevAVIPacket.InfoFrameType = LX_HDMI_INFO_AVI;
		//HDMI_VIDEO("[%d] %s : _gM14C0Intr_vf_stable AVI  / _gM14C0Intr_vf_stable \n", __L__, __F__ );
#ifdef HDMI_MODE_WHEN_NO_AVI_WORKAROUND
		if ( _gM14C0HDMI_thread_running == 1)
...
#ifdef M14C0_BRINGUP_TODO
	if ( _gM14C0HDMIPhyInform.tmds_clock[0] > 29600 )
	{
		gAVIReadCnt = 0;
		HDMI_VIDEO("[%d] %s : not support 4Kx2K over 296Mhz [%d0 Khz]\n", __L__, __F__,_gM14C0HDMIPhyInform.tmds_clock[0]);
		memset(&_gM14C0PrevAVIPacket, 0 , sizeof(LX_HDMI_INFO_PACKET_T));
		return RET_OK;
	}
#endif

	if ( _gM14C0HDMI_thread_running == 1 && _gM14C0Intr_avi == HDMI_AVI_STABLE_STATE)
		goto func_exit;

	if (_gM14C0Intr_avi == HDMI_AVI_CHANGE_STATE)
	{
		//gAVIReadCnt = 0;
		spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
		_gM14C0Intr_avi = HDMI_AVI_INTERRUPT_STATE;
		_gM14C0AVIReadState = FALSE;
		spin_unlock_irqrestore(&_gIntrHdmiM14C0VideoLock, flags);
	}
	...
	{
		spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
		_gM14C0Intr_avi = HDMI_AVI_STABLE_STATE;
		spin_unlock_irqrestore(&_gIntrHdmiM14C0VideoLock, flags);
	}
	...
	if (_gM14C0PrevPixelEncoding != csc)
	{
		HDMI_NOTI("CSC change [%d] => [%d]\n", _gM14C0PrevPixelEncoding , csc);
		_HDMI_M14C0_SetVideoBlank(csc);
		_gM14C0PrevPixelEncoding = csc;
	}

	_gM14C0AVIReadState = TRUE;

	if ( memcmp(&_gM14C0PrevAVIPacket, packet, sizeof(LX_HDMI_INFO_PACKET_T)) != 0 )
	{
		gAVIReadCnt = 0;
		HDMI_VIDEO("[%d] %s : changed AVI Packet / InfoFrameType gPre[%d] /packet[%d] \n", __L__, __F__, _gM14C0PrevAVIPacket.InfoFrameType, packet->InfoFrameType);
		HDMI_VIDEO("AVI header	gPre[%d] /packet[%d] \n", _gM14C0PrevAVIPacket.header, packet->header);
		HDMI_VIDEO("AVI data	gPre[0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x] \n", _gM14C0PrevAVIPacket.dataBytes[0], _gM14C0PrevAVIPacket.dataBytes[1], _gM14C0PrevAVIPacket.dataBytes[2], \
			_gM14C0PrevAVIPacket.dataBytes[3], _gM14C0PrevAVIPacket.dataBytes[4], _gM14C0PrevAVIPacket.dataBytes[5], _gM14C0PrevAVIPacket.dataBytes[6], _gM14C0PrevAVIPacket.dataBytes[7]);
		...
		memcpy(&_gM14C0PrevAVIPacket, packet, sizeof(LX_HDMI_INFO_PACKET_T));
		return RET_OK;
	}
	else
	{
...
func_exit:
	memcpy(packet, &_gM14C0PrevAVIPacket,  sizeof(LX_HDMI_INFO_PACKET_T));
	HDMI_PRINT("[%d] %s \n", __L__, __F__);
...
int HDMI_M14C0_GetVsiPacket(LX_HDMI_INFO_PACKET_T *packet)
{
	...
	//HDMI_DEBUG("%s, %d _gM14C0Intr_vf_stable= %d,_gM14C0Intr_vsi = %d, \n\n",__F__, __L__, _gM14C0Intr_vf_stable, _gM14C0Intr_vsi);

	if (_gM14C0Intr_vf_stable == HDMI_VIDEO_INIT_STATE ||_gM14C0Intr_vsi < HDMI_VSI_INTERRUPT_STATE)
	{
		//HDMI_VIDEO("%s, %d _gM14C0Intr_vf_stable= %d,_gM14C0Intr_vsi = %d, \n\n",__F__, __L__, _gM14C0Intr_vf_stable, _gM14C0Intr_vsi);
		gVSIReadCnt = 0;
		memset(&_gM14C0PrevVSIPacket, 0 , sizeof(LX_HDMI_INFO_PACKET_T));
		_gM14C0PrevVSIPacket.InfoFrameType = LX_HDMI_INFO_VSI;
		return RET_OK;
	}
	...
	if ( _gM14C0HDMI_thread_running == 1 && _gM14C0Intr_vsi == HDMI_VSI_STABLE_STATE)
		goto func_exit;

	if (gVSIReadCnt > 200)
	{
		spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
		_gM14C0Intr_vsi = HDMI_VSI_STABLE_STATE;
		spin_unlock_irqrestore(&_gIntrHdmiM14C0VideoLock, flags);
	}
	...
	if ( memcmp(&_gM14C0PrevVSIPacket, packet, sizeof(LX_HDMI_INFO_PACKET_T)) != 0 )
	{
		gVSIReadCnt = 0;
		HDMI_VIDEO("[%d] %s : changed VSI Packet / InfoFrameType gPre[%d] /packet[%d] \n", __L__, __F__, _gM14C0PrevVSIPacket.InfoFrameType, packet->InfoFrameType);
		HDMI_VIDEO("VSI header	gPre[%d] /packet[%d] \n", _gM14C0PrevVSIPacket.header, packet->header);
		HDMI_VIDEO("VSI data	gPre[0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x] \n", _gM14C0PrevVSIPacket.dataBytes[0], _gM14C0PrevVSIPacket.dataBytes[1], _gM14C0PrevVSIPacket.dataBytes[2], \
			_gM14C0PrevVSIPacket.dataBytes[3], _gM14C0PrevVSIPacket.dataBytes[4], _gM14C0PrevVSIPacket.dataBytes[5], _gM14C0PrevVSIPacket.dataBytes[6], _gM14C0PrevVSIPacket.dataBytes[7]);
		HDMI_VIDEO("VSI data	packet[0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x][0x%x] \n", 	packet->dataBytes[0], packet->dataBytes[1], packet->dataBytes[2], \
			packet->dataBytes[3], packet->dataBytes[4], packet->dataBytes[5], packet->dataBytes[6], packet->dataBytes[7]);

		memcpy(&_gM14C0PrevVSIPacket, packet, sizeof(LX_HDMI_INFO_PACKET_T));
		return RET_OK;
	}
	else
	{
...
func_exit:
	memcpy(packet, &_gM14C0PrevVSIPacket, sizeof(LX_HDMI_INFO_PACKET_T));
	HDMI_PRINT("[%d] %s \n", __L__, __F__);
...
int HDMI_M14C0_SetPowerControl(UINT32 power)
{
	...
	if (power)
	{
		_gM14C0HDMIPhyInform.power_control = 1;
	}
	else if (!gHDMI_Phy_Control.all_port_pdb_enable)
	{
		_gM14C0HDMIPhyInform.power_control = 0;
		...
		_HDMI_M14C0_Set_HPD_Out_A4P(0,0,0,0);
	}
...
int HDMI_M14C0_SetHPD(LX_HDMI_HPD_T *hpd)
{
	...
	if (hpd->bEnable)
		_gM14C0HDMIPhyInform.hpd_enable = 1;
	else
	{
		_gM14C0HDMIPhyInform.hpd_enable = 0;
			...
			_HDMI_M14C0_Set_HPD_Out_A4P(!_gM14C0HDMIPhyInform.hpd_pol[0], \
					!_gM14C0HDMIPhyInform.hpd_pol[1], \
					!_gM14C0HDMIPhyInform.hpd_pol[2], \
					!_gM14C0HDMIPhyInform.hpd_pol[3]);
	}
...
int HDMI_M14C0_GetStatus(LX_HDMI_STATUS_T *status)
{
	...
	do {
		if (_gM14C0HDMIState < HDMI_STATE_SCDT)
			break;

		status->bHdmiMode = gM14BootData->mode;

		status->eHotPlug = _gM14C0HDMIConnectState;
		...
		status->csc = _gM14C0PrevPixelEncoding;
		csc = (((_gM14C0PrevAVIPacket.dataBytes[0] &0xff00)>>8) &0x60)>>5;
		if (_gM14C0PrevPixelEncoding == csc)		HDMI_DEBUG(" _gM14C0PrevPixelEncoding = %d , csc = %d \n", _gM14C0PrevPixelEncoding, csc);
		...
		HDMI_VIDEO("HDMI_M14C0_GetStatus  depth = 0x%x tmds =%d\n", status->eColorDepth, tmdsClock);
		HDMI_VIDEO("[HDMI] %d HDMI  State  = %d _gM14C0Force_thread_sleep= %d  _gM14C0HDMI_thread_running =%d \n",\
			__L__, _gM14C0HDMIState,_gM14C0Force_thread_sleep, _gM14C0HDMI_thread_running);
		HDMI_VIDEO("[HDMI]  AVI State[%d] VSI State[%d]  SRC Mute State[%d] \n",  _gM14C0Intr_avi, _gM14C0Intr_vsi, _gM14C0Intr_src_mute );
		HDMI_VIDEO("[HDMI]  M14C0 : 20140916(BD-P1600) \n");
	} while (0);
...
int HDMI_M14C0_GetAudioInfo(LX_HDMI_AUDIO_INFO_T *pAudioInfo)
{
	...
	pAudioInfo->audioType	 = _gM14C0HdmiAudioInfo.audioType;
	pAudioInfo->samplingFreq = _gM14C0HdmiAudioInfo.samplingFreq;

	//Debug print
	if ((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_1S) == 0)
	{
		HDMI_AUDIO("HDMI_GetAudioInfo : type = %d, freq = %d, state = %d, mute = %d\n",	\
					pAudioInfo->audioType, pAudioInfo->samplingFreq, _gM14C0IntrAudioState, _gM14C0AudioMuteState);
	}
	else
	{
...
int HDMI_M14C0_GetAudioCopyInfo(LX_HDMI_AUDIO_COPY_INFO_T *pCopyInfo)
{
	...
	if (reg_hdmi_mode == 1)	//HDMI Mode
	{
		...
		if ((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_1S) == 0)
		{
			HDMI_AUDIO("HDMI_M14C0_GetAudioCopyInfo : CopyInfo = %d(mode = %d)(0x%X, 0x%X)\n", pCopyInfo->copyInfo, reg_hdmi_mode, reg_achst_byte0, reg_achst_byte1);
		}
		else
		{
			HDMI_PRINT("HDMI_M14C0_GetAudioCopyInfo : CopyInfo = %d(mode = %d)(0x%X, 0x%X)\n", pCopyInfo->copyInfo, reg_hdmi_mode, reg_achst_byte0, reg_achst_byte1);
		}
	}
	else	//DVI Mode
	{
	...
	HDMI_PRINT("HDMI_M14C0_GetAudioCopyInfo : CopyInfo = %d(mode = %d)\n", pCopyInfo->copyInfo, reg_hdmi_mode);
	return ret;
}
...
int HDMI_M14C0_SetArc(LX_HDMI_ARC_CTRL_T *pArcCtrl)
{
	...
	_gM14C0AudioArcStatus = pArcCtrl->bARCEnable;
...
int HDMI_M14C0_SetMute(LX_HDMI_MUTE_CTRL_T *pMuteCtrl)
{
	...
	if (pMuteCtrl->eMode == LX_HDMI_AUDIO_MUTE || pMuteCtrl->eMode == LX_HDMI_AV_MUTE)
	{
		...
		if (_gM14C0AudioMuteState != pMuteCtrl->bAudioMute)
		{
			...
			//Update _gM14C0AudioMuteState
			_gM14C0AudioMuteState = pMuteCtrl->bAudioMute;
		...
		if (pMuteCtrl->bAudioMute == FALSE)
		{
			HDMI_DEBUG("[%d]SetMute : type = %d, freq = %d, mute = %d, PSC = %d\n", \
						__L__, _gM14C0HdmiAudioInfo.audioType, _gM14C0HdmiAudioInfo.samplingFreq, _gM14C0AudioMuteState, _gM14C0HdmiPortStableCount);
		}
	}
...
*/
static void HDMI_M14C0_SetAudioDebugInterrupt(UINT8 enable)
{
	LINK_M14_RdFL(interrupt_enable_00);
	LINK_M14_Wr01(interrupt_enable_00, intr_cts_chg_enable, enable);		///< 14 intr_cts_chg_enable
	LINK_M14_Wr01(interrupt_enable_00, intr_n_chg_enable, enable);			///< 15 intr_n_chg_enable
	LINK_M14_Wr01(interrupt_enable_00, intr_fs_chg_enable, enable);			///< 16 intr_fs_chg_enable
	LINK_M14_Wr01(interrupt_enable_00, intr_2pcm_chg_enable, enable);		///< 17 intr_2pcm_chg_enable
	LINK_M14_Wr01(interrupt_enable_00, intr_2npcm_chg_enable, enable);		///< 18 intr_2npcm_chg_enable
	LINK_M14_Wr01(interrupt_enable_00, intr_aud_type_chg_enable, enable);	///< 19 intr_aud_type_chg_enable
	LINK_M14_Wr01(interrupt_enable_00, intr_spdif_err_enable, enable);		///< 20 intr_spdif_err_enable
	LINK_M14_Wr01(interrupt_enable_00, intr_afifo_undr_enable, enable);		///< 21 intr_afifo_undr_enable
	LINK_M14_Wr01(interrupt_enable_00, intr_afifo_ovrr_enable, enable);		///< 22 intr_afifo_ovrr_enable
	LINK_M14_WrFL(interrupt_enable_00);

	LINK_M14_RdFL(interrupt_enable_01);
	LINK_M14_Wr01(interrupt_enable_01, intr_ecc_err_enable, enable);		///< 17 intr_ecc_err_enable
	LINK_M14_Wr01(interrupt_enable_01, intr_terc4_err_prt3_enable, enable);	///< 18 intr_terc4_err_prt3_enable
	LINK_M14_Wr01(interrupt_enable_01, intr_terc4_err_prt2_enable, enable);	///< 19 intr_terc4_err_prt2_enable
	LINK_M14_Wr01(interrupt_enable_01, intr_terc4_err_prt1_enable, enable);	///< 20 intr_terc4_err_prt1_enable
	LINK_M14_Wr01(interrupt_enable_01, intr_terc4_err_prt0_enable, enable);	///< 21 intr_terc4_err_prt0_enable
	LINK_M14_WrFL(interrupt_enable_01);

	HDMI_NOTI("HDMI_M14C0_SetAudioDebugInterrupt : enable = %d\n", enable);
	return;
}

/**
 *  HDMI_M14C0_GetAudioDebugInfo
 *
 *  @parm LX_HDMI_AUDIO_INFO_T *
 *  @return int
*/
int HDMI_M14C0_GetAudioDebugInfo(LX_HDMI_DEBUG_AUDIO_INFO_T *pAudioDebugInfo)
{
	...
	if (link_num == (HDMI_LINK_0 + 10))
	{
		//Set a HDMI Audio Debug Enabled Status for toggle.
		if(_gM14C0AudioDebugState == FALSE)
		{
			_gM14C0AudioDebugState = TRUE;
			(void)HDMI_M14C0_SetAudioDebugInterrupt(1);
		}
		else
		{
			_gM14C0AudioDebugState = FALSE;
			(void)HDMI_M14C0_SetAudioDebugInterrupt(0);
		}
	}

	//Copy and Print HDMI Audio Debug Info.
	(void)_HDMI_M14C0_DebugAudioInfo(pAudioDebugInfo);

	HDMI_DEBUG("HDMI_M14C0_GetAudioDebugInfo : type = %d, freq = %d, state = %d, mute = %d, PSC = %d, video = %d\n", \
				_gM14C0HdmiAudioInfo.audioType, _gM14C0HdmiAudioInfo.samplingFreq, _gM14C0IntrAudioState, _gM14C0AudioMuteState, _gM14C0HdmiPortStableCount, _gM14C0HDMIState);
...
static void _HDMI_M14C0_CheckUDExtInfo(LX_HDMI_TIMING_INFO_T *info, UINT32 tmdsClock)
{
	...
	do {
		...
		if (_gM14C0Intr_avi > HDMI_AVI_INIT_STATE)
		{
			vic = ((_gM14C0PrevAVIPacket.dataBytes[1] &0xff) &0x7f);	// VIC6~VIC0
...
static int _HDMI_M14C0_GetExtendTimingInfo(LX_HDMI_TIMING_INFO_T *info, UINT32 tmdsClock)
{
	...
	do {
		...
		if (_gM14C0Intr_vsi < HDMI_VSI_INTERRUPT_STATE)
		{
			_HDMI_M14C0_CheckUDExtInfo(&bufTiming, tmdsClock);		// check UD format
			...
			{
				LINK_M14_RdFL(packet_13);		//AVI
				tmp32 = LINK_M14_Rd(packet_13);
		...
		switch(bufTiming.extInfo)		//if (eVideoFormat == LX_FORMAT_3D_FORMAT)
		{
			...
			case LX_HDMI_EXT_4K_2K:
				if ( !((bufTiming.hActive == 3840 && bufTiming.vActive == 2160) || (bufTiming.hActive == 4096 && bufTiming.vActive == 2160) ||\
					(bufTiming.hActive == 1920 && bufTiming.vActive == 2160 && _gM14C0HDMIPhyInform.tmds_clock[0] > 29000) || \
					(bufTiming.hActive == 2048 && bufTiming.vActive == 2160 && _gM14C0HDMIPhyInform.tmds_clock[0] > 29000) ) )
				{
					bufTiming.extInfo = LX_HDMI_EXT_2D_FORMAT;
	...
	if ( ( _gM14C0HDMI_thread_running == 0 &&  _gM14C0TimingReadCnt < 23 && _gM14C0TimingReadCnt > 20) ||
		(_gM14C0HDMIState >= HDMI_STATE_READ && _gM14C0TimingReadCnt < 6 && _gM14C0TimingReadCnt > 3) )
	{
		//HDMI_VIDEO(" VIC  = %d [gpreAVI - %d] \n", videoIdCode, __gM14C0PrevAVIPacket.eAviPacket.VideoIdCode);
...
static int _HDMI_M14C0_SetInternalMute(LX_HDMI_MUTE_CTRL_T interMute)
{
	...
	{
		//Check a previous state
		if (_gM14C0AudioMuteState != interMute.bAudioMute)
		{
			...
			//Update _gM14C0AudioMuteState
			_gM14C0AudioMuteState = interMute.bAudioMute;
...
static int _HDMI_M14C0_ClearSWResetAll(void)
{
...
static int _HDMI_M14C0_SetPortConnection(void)
{
	...
	{
		_HDMI_M14C0_Get_AN_Data(port_num, HDCP_AN_Data);
		...
		if ( memcmp(HDCP_AN_Data, Prev_HDCP_AN_Data, sizeof(HDCP_AN_Data) ) || (count_port_change == 1))
		{
			if (count_port_change == 1)
				_HDMI_M14C0_HDCP_ResetPort(port_num);

			count_port_change = 1;
			_HDMI_M14C0_PhyOn(port_num);
		}
		count_port_change --;
	}
	return RET_OK;
...
//	ULONG	flags = 0;
	...
	static int prev_cr_lock[4] = {0,};
	static int prev_cr_done[4] = {0,};
	static int cr_lock[4] = {0,};
	static int cr_done[4] = {0,};
...
//	UINT8 aksv_data[5];
	...
	_HDMI_M14C0_Get_AN_Data(0, HDCP_AN_Data);
	...
	*/
	_HDMI_M14C0_Get_HPD_Out_A4P(&_gM14C0HDMIPhyInform.hpd_out[0], &_gM14C0HDMIPhyInform.hpd_out[1] ,&_gM14C0HDMIPhyInform.hpd_out[2], &_gM14C0HDMIPhyInform.hpd_out[3]);
	_HDMI_M14C0_Get_HDMI5V_Info_A4P(&_gM14C0HDMIPhyInform.hdmi5v[0], &_gM14C0HDMIPhyInform.hdmi5v[1] ,&_gM14C0HDMIPhyInform.hdmi5v[2], &_gM14C0HDMIPhyInform.hdmi5v[3]);

//	if ( memcmp(_gM14C0HDMIPhyInform.hpd_out, _gM14C0HDMIPhyInform.hdmi5v, sizeof(_gM14C0HDMIPhyInform.hdmi5v)) )
	if ( ( _gM14C0HDMIPhyInform.hpd_out[0] != !(_gM14C0HDMIPhyInform.hdmi5v[0] ^ _gM14C0HDMIPhyInform.hpd_pol[0]) )\
	|| ( _gM14C0HDMIPhyInform.hpd_out[1] != !(_gM14C0HDMIPhyInform.hdmi5v[1] ^ _gM14C0HDMIPhyInform.hpd_pol[1]) )\
	|| ( _gM14C0HDMIPhyInform.hpd_out[2] != !(_gM14C0HDMIPhyInform.hdmi5v[2] ^ _gM14C0HDMIPhyInform.hpd_pol[2]) )\
	|| ( _gM14C0HDMIPhyInform.hpd_out[3] != !(_gM14C0HDMIPhyInform.hdmi5v[3] ^ _gM14C0HDMIPhyInform.hpd_pol[3]) ) )
	{
		for (port_num = 0; port_num < 4;port_num ++)
		{
#ifdef HPD_OFF_WORKAROUND_FOR_EXT_EDID
			if( (port_num == 1) && ( _gM14C0HDMIPhyInform.hdmi5v[1] == 1) &&  ( _g_HPD_Off_Time[1] != 0) )
			{
//				HDMI_DEBUG("---- HDMI_PHY:Port[%d] Ext EDID and cable connected !!! : HPD off", port_num);
			...
			hpd_out_polarity = !( _gM14C0HDMIPhyInform.hdmi5v[port_num] ^ _gM14C0HDMIPhyInform.hpd_pol[port_num])  ;	// hpd_out considering polarity
			if ( _gM14C0HDMIPhyInform.hpd_out[port_num] != hpd_out_polarity )
			{
				_gM14C0HDMIPhyInform.tcs_done[port_num] = 0;

				if (!_gM14C0HDMIPhyInform.hdmi5v[port_num])		// Cable Disconnected !!!
				{
					HDMI_NOTI("---- HDMI_PHY:DisConn port[%d] = [%d] => [%d]\n", port_num, _gM14C0HDMIPhyInform.hpd_out[port_num], hpd_out_polarity);
//					if (gHDMI_Phy_Control.link_reset_control)
//						_HDMI_M14C0_TMDS_HDCP_ResetPort(port_num);
						_HDMI_M14C0_HDCP_ResetPort(port_num);
#ifdef M14C0_BRINGUP_TODO
						_HDMI_M14C0_TMDS_ResetPort_Control(port_num ,0);
#endif

					if (gHDMI_Phy_Control.all_port_pdb_enable)
						_HDMI_M14C0_PhyOff(port_num);

					_HDMI_M14C0_Set_HPD_Out(port_num, hpd_out_polarity);	// disable HPD

#ifdef M14_HUMAX_SETTOP_HDCP_WORKAROUND
					/* HUMAX IR1020HD : snow noise when resolution change */
					/* Restore Default values for detached port */
					_HDMI_M14C0_Set_HDCP_Unauth(port_num, 1, 1);
#endif

					if (port_num == _gM14C0HDMIPhyInform.prt_sel)
						_gM14C0_tcs_fail_restart_count = 0;

					if (port_num == _gM14C0HDMIPhyInform.prt_sel) {
						_gM14C0HDMIPhyInform.ecc_error_count[port_num] = 0;
						_gM14C0HDMIPhyInform.hdcp_error_count[port_num] = 0;
					}
					_gM14C0HDMIPhyInform.terc4_error_count[port_num] = 0;
					_gM14C0HDMIPhyInform.loop_count[port_num] = 0;
					_gM14C0HDMIPhyInform.scdt_detect_count[port_num] = 0;
					_gM14C0HDMIPhyInform.scdt_fall_count[port_num] = 0;
				}
				else	// cable connected
				{
					_Port_Change_StartTime = jiffies_to_msecs(jiffies);

					//if (gHDMI_Phy_Control.all_port_pdb_enable && _gM14C0HDMIPhyInform.module_open)
					if (_gM14C0HDMIPhyInform.hpd_enable)
					{
						...
						/* Enable port_num */
						_HDMI_M14C0_EnablePort(port_num, 1);

						_HDMI_M14C0_Set_HPD_Out(port_num, hpd_out_polarity);	// Enable HPD

						OS_MsecSleep(10); // delay after enable port and phy register write

						/* Phy ON & phy run-reset */
						_HDMI_M14C0_PhyOn_5V(port_num);

						/* if port_num is not current selected port , disable port */
						if (port_num != _gM14C0HDMIPhyInform.prt_sel)
							_HDMI_M14C0_EnablePort(port_num, 0);

						/* Enable selected port */
						_HDMI_M14C0_EnablePort(_gM14C0HDMIPhyInform.prt_sel, 1);

						if ( ( port_num == 3) && (_HDMI_M14C0_Get_ManMHLMode() == 1) && ( _gM14C0_HDMI_MHLAutoModeCnt == 0) )// MHL port
						{
							_gM14C0_HDMI_MHLAutoModeCnt = 20; // MHL Mode to HDMI for 400msec
						}
#if 0
#ifdef M14_CODE_FOR_MHL_CTS
#ifdef M14_CBUS_PDB_CTRL
						if ( (port_num == 3) && (_gM14C0HDMIPhyInform.cd_sense == 0) )
						{
							...
							if (!phy_en_prt3)	// IF port3(C-BUS) is not enabled?
							{
								...
								_HDMI_M14C0_EnablePort(1, 0);
								_HDMI_M14C0_EnablePort(2, 0);
								_HDMI_M14C0_EnablePort(3, 1);
							}

							{
								/*
								HDMI_DEBUG("---- MHL Set ODT PDB Mode to 0x00\n");
								PHY_REG_M14C0_RdFL(eq_i2c_odt_pdb_mode);
								PHY_REG_M14C0_Wr01(eq_i2c_odt_pdb_mode,eq_i2c_odt_pdb,0x0);
								PHY_REG_M14C0_Wr01(eq_i2c_odt_pdb_mode,eq_i2c_odt_pdb_mode,0x0);
								PHY_REG_M14C0_WrFL(eq_i2c_odt_pdb_mode);
								*/
								HDMI_DEBUG("---- NO CD_SENSE for MHL,  eq_i2c_pdb_d0 :0x00, eq_i2c_pdb_dck :0x00\n");
								PHY_REG_M14C0_RdFL(eq_i2c_pdb_d0);
								PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_i2c_pdb_d0_mode,0x0);
								PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_man_pdb_d0,0x0);
								PHY_REG_M14C0_WrFL(eq_i2c_pdb_d0);

								PHY_REG_M14C0_RdFL(eq_i2c_pdb_dck);
								PHY_REG_M14C0_Wr01(eq_i2c_pdb_dck,eq_i2c_pdb_dck_mode,0x0);
								PHY_REG_M14C0_Wr01(eq_i2c_pdb_dck,eq_man_pdb_dck,0x0);
								PHY_REG_M14C0_WrFL(eq_i2c_pdb_dck);

							}

							if (!phy_en_prt3)
							{
								_HDMI_M14C0_EnablePort(0, phy_en_prt0);
								_HDMI_M14C0_EnablePort(1, phy_en_prt1);
								_HDMI_M14C0_EnablePort(2, phy_en_prt2);
								_HDMI_M14C0_EnablePort(3, phy_en_prt3);
							}
						}
#endif
#endif
...
//				_gM14C0HDMIPhyInform.hpd_out[port_num] = _gM14C0HDMIPhyInform.hdmi5v[port_num];
			}
		}

		//memcpy(_gM14C0HDMIPhyInform.hpd_out, _gM14C0HDMIPhyInform.hdmi5v, sizeof(_gM14C0HDMIPhyInform.hdmi5v));

		//_HDMI_M14C0_Set_HPD_Out_A4P(_gM14C0HDMIPhyInform.hpd_out[0], _gM14C0HDMIPhyInform.hpd_out[1],_gM14C0HDMIPhyInform.hpd_out[2], _gM14C0HDMIPhyInform.hpd_out[3]);
	}

	LINK_M14_RdFL(system_control_00);
	LINK_M14_Rd01(system_control_00, reg_prt_sel, _gM14C0HDMIPhyInform.prt_sel);

	LINK_M14_RdFL(phy_link_00);
	LINK_M14_Rd01(phy_link_00, phy_pdb_prt0, _gM14C0HDMIPhyInform.phy_pdb[0]);			//PHY PDB ON
	LINK_M14_Rd01(phy_link_00, phy_enable_prt0, _gM14C0HDMIPhyInform.phy_enable[0]);		//PHY Enable
	LINK_M14_Rd01(phy_link_00, phy_rstn_prt0, _gM14C0HDMIPhyInform.phy_rstn[0]);			//PHY RESET
	LINK_M14_Rd01(phy_link_00, hpd_in_prt0, _gM14C0HDMIPhyInform.hpd_in[0]);			//PHY HPD IN

	LINK_M14_RdFL(phy_link_02);
	LINK_M14_Rd01(phy_link_02, phy_pdb_prt1, _gM14C0HDMIPhyInform.phy_pdb[1]);			//PHY PDB ON
	LINK_M14_Rd01(phy_link_02, phy_enable_prt1, _gM14C0HDMIPhyInform.phy_enable[1]);		//PHY Enable
	LINK_M14_Rd01(phy_link_02, phy_rstn_prt1, _gM14C0HDMIPhyInform.phy_rstn[1]);			//PHY RESET
	LINK_M14_Rd01(phy_link_02, hpd_in_prt1, _gM14C0HDMIPhyInform.hpd_in[1]);			//PHY HPD IN

	LINK_M14_RdFL(phy_link_04);
	LINK_M14_Rd01(phy_link_04, phy_pdb_prt2, _gM14C0HDMIPhyInform.phy_pdb[2]);			//PHY PDB ON
	LINK_M14_Rd01(phy_link_04, phy_enable_prt2, _gM14C0HDMIPhyInform.phy_enable[2]);		//PHY Enable
	LINK_M14_Rd01(phy_link_04, phy_rstn_prt2, _gM14C0HDMIPhyInform.phy_rstn[2]);			//PHY RESET
	LINK_M14_Rd01(phy_link_04, hpd_in_prt2, _gM14C0HDMIPhyInform.hpd_in[2]);			//PHY HPD IN

	LINK_M14_RdFL(phy_link_06);
	LINK_M14_Rd01(phy_link_06, phy_pdb_prt3, _gM14C0HDMIPhyInform.phy_pdb[3]);			//PHY PDB ON
	LINK_M14_Rd01(phy_link_06, phy_enable_prt3, _gM14C0HDMIPhyInform.phy_enable[3]);		//PHY Enable
	LINK_M14_Rd01(phy_link_06, phy_rstn_prt3, _gM14C0HDMIPhyInform.phy_rstn[3]);			//PHY RESET
	LINK_M14_Rd01(phy_link_06, hpd_in_prt3, _gM14C0HDMIPhyInform.hpd_in[3]);			//PHY HPD IN

	LINK_M14_Rd01(system_control_00, reg_scdt_prt0, _gM14C0HDMIPhyInform.scdt[0]);
	LINK_M14_Rd01(system_control_00, reg_scdt_prt1, _gM14C0HDMIPhyInform.scdt[1]);
	LINK_M14_Rd01(system_control_00, reg_scdt_prt2, _gM14C0HDMIPhyInform.scdt[2]);
	LINK_M14_Rd01(system_control_00, reg_scdt_prt3, _gM14C0HDMIPhyInform.scdt[3]);

	for (i=0;i<4;i++) {
		if(_gM14C0HDMIPhyInform.scdt[i] > 0)
			_gM14C0HDMIPhyInform.scdt_detect_count[i] ++;
	}

	LINK_M14_Rd01(system_control_00, reg_hdmi_mode_prt0, _gM14C0HDMIPhyInform.hdmi_mode[0]);
	LINK_M14_Rd01(system_control_00, reg_hdmi_mode_prt1, _gM14C0HDMIPhyInform.hdmi_mode[1]);
	LINK_M14_Rd01(system_control_00, reg_hdmi_mode_prt2, _gM14C0HDMIPhyInform.hdmi_mode[2]);
	LINK_M14_Rd01(system_control_00, reg_hdmi_mode_prt3, _gM14C0HDMIPhyInform.hdmi_mode[3]);

	for (i=0;i<4;i++)
		access_phy[i] = _gM14C0HDMIPhyInform.phy_pdb[i] && _gM14C0HDMIPhyInform.phy_enable[i] && _gM14C0HDMIPhyInform.phy_rstn[i] ;

	_HDMI_M14C0_Get_HPD_Out_A4P(&_gM14C0HDMIPhyInform.hpd_out[0], &_gM14C0HDMIPhyInform.hpd_out[1] ,&_gM14C0HDMIPhyInform.hpd_out[2], &_gM14C0HDMIPhyInform.hpd_out[3]);
	_HDMI_M14C0_Get_HDMI5V_Info_A4P(&_gM14C0HDMIPhyInform.hdmi5v[0], &_gM14C0HDMIPhyInform.hdmi5v[1] ,&_gM14C0HDMIPhyInform.hdmi5v[2], &_gM14C0HDMIPhyInform.hdmi5v[3]);

	for (i=0;i<4;i++) {
		if(_gM14C0HDMIPhyInform.hdmi5v[i] > 0)
			_gM14C0HDMIPhyInform.loop_count[i] ++;
	}
	...
	if (access_phy[0] || access_phy[1] ||access_phy[2] ||access_phy[3] )
	{
		...
		_gM14C0HDMIPhyInform.tmds_clock[0] = ((up_freq << 8) + down_freq); 	// XXX.XX KHz

		if ( _gM14C0HDMIPhyInform.hpd_enable && (_gM14C0HDMIPhyInform.tmds_clock[0] == 0) && (_gM14C0HDMIPhyInform.hdmi5v[_gM14C0HDMIPhyInform.prt_sel] ))	// hdmi 5v , not no tmds clock
		{
			int cr_done, cr_lock;

			PHY_REG_M14C0_RdFL(cr_lock_done);
			PHY_REG_M14C0_Rd01(cr_lock_done, cr_lock, cr_lock);
			PHY_REG_M14C0_Rd01(cr_lock_done, cr_done, cr_done);

			if (cr_lock && cr_done)	// no tmds clock, but CR lock done ????
			{
				tmds_clock_abnormal_count++;
				if(tmds_clock_abnormal_count == 10) {

					HDMI_NOTI("@@@ Port[%d] Disable EQ Calibration Mode : abnormal count [%d]\n", \
							_gM14C0HDMIPhyInform.prt_sel, tmds_clock_abnormal_count);

					/* SS BD-P1600 workaround code */
					PHY_REG_M14C0_RdFL(eq_cal_mode);
					PHY_REG_M14C0_Wr01(eq_cal_mode,eq_cal_mode,0x0);
					PHY_REG_M14C0_WrFL(eq_cal_mode);
				}
			}
			else
			...
			if (tmds_clock_abnormal_count > 50)	//clock abnormal for long time
			{
				...
				HDMI_NOTI("#### HPD out off/on Port [%d] ############\n", _gM14C0HDMIPhyInform.prt_sel);
				HDMI_NOTI("#########################################\n");

				_HDMI_M14C0_Set_HPD_Out(_gM14C0HDMIPhyInform.prt_sel, !_gM14C0HDMIPhyInform.hpd_pol[_gM14C0HDMIPhyInform.prt_sel]);	// disable HPD
				_HDMI_M14C0_Phy_Reset(_gM14C0HDMIPhyInform.prt_sel);

				_g_Disable_EQ_Offset_Cal[_gM14C0HDMIPhyInform.prt_sel] = 1;

				OS_MsecSleep(100);
				/*
				PHY_REG_M14C0_RdFL(resetb_pdb_all);
				PHY_REG_M14C0_Wr01(resetb_pdb_all,resetb_all,0x0);		//active low reset
				PHY_REG_M14C0_WrFL(resetb_pdb_all);

				OS_MsecSleep(10);

				PHY_REG_M14C0_RdFL(resetb_pdb_all);
				PHY_REG_M14C0_Wr01(resetb_pdb_all,resetb_all,0x1);		//active low reset
				PHY_REG_M14C0_WrFL(resetb_pdb_all);
				*/
		...
		_gM14C0HDMIPhyInform.tmds_clock[0] = 0;

	if (_gM14C0HDMIPhyInform.prt_sel == 0)
	{
		...
		if ( prev_hdmi_mode_prt0 != mode)
		{
			HDMI_DEBUG("#### HDMI_Mode : Port [%d] Mode Changed [%d]=>[%d] \n",\
					_gM14C0HDMIPhyInform.prt_sel, prev_hdmi_mode_prt0, mode );
			prev_hdmi_mode_prt0 = mode;
		}
	}
	else if (_gM14C0HDMIPhyInform.prt_sel == 1)
	{
		...
		{
			HDMI_DEBUG("#### HDMI_Mode : Port [%d] Mode Changed [%d]=>[%d] \n",\
					_gM14C0HDMIPhyInform.prt_sel, prev_hdmi_mode_prt1, mode );
			prev_hdmi_mode_prt1 = mode;
		}
	}
	else if (_gM14C0HDMIPhyInform.prt_sel == 2)
	{
		...
		{
			HDMI_DEBUG("#### HDMI_Mode : Port [%d] Mode Changed [%d]=>[%d] \n",\
					_gM14C0HDMIPhyInform.prt_sel, prev_hdmi_mode_prt2, mode );
	...
	else
	{
		...
		{
			HDMI_DEBUG("#### HDMI_Mode : Port [%d] Mode Changed [%d]=>[%d] \n",\
					_gM14C0HDMIPhyInform.prt_sel, prev_hdmi_mode_prt3, mode );
	...
	for (port_num=0;port_num<4;port_num++)
	{
		...
		/*
		if ( (port_num ==3) && ( _gM14C0HDMIPhyInform.cd_sense) )	// for MHL
			break;
			*/
		...
		if ( _SCDT_Fall_Detected[port_num] )
		{
			...
			_gM14C0HDMIPhyInform.scdt_fall_count[port_num] ++;

			/* Polling SCDT sometimes miss detection of short SDCT fall */
			if( _gM14C0HDMIPhyInform.prt_sel == port_num)
				scdt = 0;

			_gM14C0HDMIPhyInform.tcs_done[port_num] = 0;
			_SCDT_Fall_Detected[port_num] = 0;
			if (_gM14C0HDMIPhyInform.phy_pdb[port_num] && _gM14C0HDMIPhyInform.phy_rstn[port_num])
			{
				/* All Phy Disabled except port_num */
				if (port_num != 0)
					_HDMI_M14C0_EnablePort(0, 0);
				...
				//PHY_REG_M14C0_RdFL(eq_cs_rs_sel);
				//PHY_REG_M14C0_Wr01(eq_cs_rs_sel, eq_cs_man_sel,0x0);
				//PHY_REG_M14C0_WrFL(eq_cs_rs_sel);

				PHY_REG_M14C0_RdFL(tcs_boundary);
				PHY_REG_M14C0_Wr01(tcs_boundary, tcs_boundary, _HDMI_M14C0_TCS_addtional_value_init);
				PHY_REG_M14C0_WrFL(tcs_boundary);

				PHY_REG_M14C0_RdFL(tcs_additional_value);
				PHY_REG_M14C0_Wr01(tcs_additional_value, tcs_additional_value, _HDMI_M14C0_TCS_addtional_value_init);
				PHY_REG_M14C0_WrFL(tcs_additional_value);
		...
		if ( _gM14C0HDMIPhyInform.hdmi5v[port_num] /* && (_gM14C0HDMIPhyInform.tcs_done[port_num] == 0)*/ \
			&& _gM14C0HDMIPhyInform.phy_pdb[port_num] && _gM14C0HDMIPhyInform.phy_rstn[port_num])		// port detected but TCS not done yet
		{
			int tcs_done_value, tcs_min_value[3], tcs_max_value[3];
			...
			/* If tcs_done is '0', TCS is in progress */
			PHY_REG_M14C0_RdFL(tcs_done);
			PHY_REG_M14C0_Rd01(tcs_done, tcs_done, tcs_done_value);

			if(tcs_done_value == 0) {
		//		HDMI_DEBUG("---- HDMI_PHY port [%d] : TCS Measuring !!! \n", port_num);
				_gM14C0HDMIPhyInform.tcs_done[port_num] = 0;
			}
			...
			{
				if(_gM14C0HDMIPhyInform.scdt[port_num] > 0)
				...
				if(tcs_done_value > 0) {
					HDMI_DEBUG("---- HDMI_PHY port [%d] : Clock Changed ,but TCS not restarted \n", port_num);
					_gM14C0HDMIPhyInform.tcs_done[port_num] = 0;
				}
				else
					HDMI_DEBUG("---- HDMI_PHY port [%d] : Clock Changed and TCS Measuring !!! \n", port_num);
			...
			else if(_gM14C0HDMIPhyInform.tcs_done[port_num] == 0)
			{
				...
				if (tcs_done_value)
				{
					...
					PHY_REG_M14C0_RdFL(tcs_min_ch0);
					PHY_REG_M14C0_Rd01(tcs_min_ch0, tcs_min_ch0, tcs_min_value[0]);

					PHY_REG_M14C0_RdFL(tcs_max_ch0);
					PHY_REG_M14C0_Rd01(tcs_max_ch0, tcs_max_ch0, tcs_max_value[0]);

					PHY_REG_M14C0_RdFL(tcs_min_ch1);
					PHY_REG_M14C0_Rd01(tcs_min_ch1, tcs_min_ch1, tcs_min_value[1]);

					PHY_REG_M14C0_RdFL(tcs_max_ch1);
					PHY_REG_M14C0_Rd01(tcs_max_ch1, tcs_max_ch1, tcs_max_value[1]);

					PHY_REG_M14C0_RdFL(tcs_min_ch2);
					PHY_REG_M14C0_Rd01(tcs_min_ch2, tcs_min_ch2, tcs_min_value[2]);

					PHY_REG_M14C0_RdFL(tcs_max_ch2);
					PHY_REG_M14C0_Rd01(tcs_max_ch2, tcs_max_ch2, tcs_max_value[2]);

					PHY_REG_M14C0_RdFL(tcs_ch0);
					PHY_REG_M14C0_Rd01(tcs_ch0, tcs_ch0, _HDMI_M14C0_TCS_result[port_num][0]);

					PHY_REG_M14C0_RdFL(tcs_ch1);
					PHY_REG_M14C0_Rd01(tcs_ch1, tcs_ch1, _HDMI_M14C0_TCS_result[port_num][1]);

					PHY_REG_M14C0_RdFL(tcs_ch2);
					PHY_REG_M14C0_Rd01(tcs_ch2, tcs_ch2, _HDMI_M14C0_TCS_result[port_num][2]);

					PHY_REG_M14C0_RdFL(tcs_additional_value);
					PHY_REG_M14C0_Rd01(tcs_additional_value, tcs_additional_value, _HDMI_M14C0_TCS_add[port_num]);

					//		if( (tcs_min_value == 0) && (tcs_max_value == 0) && (_g_tcs_min_max_zero_count[port_num] < 10 ) )
					if( ( ( abs(tcs_max_value[0] - tcs_min_value[0]) <= 2 ) || ( abs(tcs_max_value[1] - tcs_min_value[1]) <= 2 ) || ( abs(tcs_max_value[2] - tcs_min_value[2]) <= 2 ) ) && (_g_tcs_min_max_zero_count[port_num] < 10 ) )	//140515 : MHL CT min/max almost same value
					{
					...
					else
					{
						...
						_gM14C0HDMIPhyInform.tcs_done[port_num] = tcs_done_value;
						...
						//				PHY_REG_M14C0_RdFL(reg_tcs_man_mea_1);
						//				PHY_REG_M14C0_Wr01(reg_tcs_man_mea_1, reg_tcs_man_mea_1, 0x0);
						//				PHY_REG_M14C0_WrFL(reg_tcs_man_mea_1);
#if 0	// CS Auto Mode
#if WORK_NEEDS_TO_BE_DONE_BY_DOSA

						PHY_REG_M14C0_RdFL(eq_cs_man);
						if (tcs_min_value + 10 < 0x1f)		// for 15m cable
						{
							if(tcs_min_value == 0)
								PHY_REG_M14C0_Wr01(eq_cs_man, eq_cs_man,tcs_min_value + 12);
							else if(tcs_min_value == 4)		// 140409 : BD370V-N 1080P60Hz noise (to set cs value to 0x10(from 0xe))
								PHY_REG_M14C0_Wr01(eq_cs_man, eq_cs_man,tcs_min_value + 12);
							else
								PHY_REG_M14C0_Wr01(eq_cs_man, eq_cs_man,tcs_min_value + 10);
						}
						else
							PHY_REG_M14C0_Wr01(eq_cs_man, eq_cs_man, 0x1f);

						PHY_REG_M14C0_Rd01(eq_cs_man, eq_cs_man, _EQ_cs_man_value[port_num]);
						PHY_REG_M14C0_WrFL(eq_cs_man);

						PHY_REG_M14C0_RdFL(eq_cs_rs_sel);
						PHY_REG_M14C0_Wr01(eq_cs_rs_sel, eq_cs_man_sel,0x3);
						PHY_REG_M14C0_WrFL(eq_cs_rs_sel);

						PHY_REG_M14C0_RdFL(reset_pdb_sel);
						PHY_REG_M14C0_Wr01(reset_pdb_sel, reset_sel,0x1);	// Reset Manual Control
						PHY_REG_M14C0_WrFL(reset_pdb_sel);
#endif
						PHY_REG_M14C0_RdFL(bert_dtb_resetb);
						PHY_REG_M14C0_Wr01(bert_dtb_resetb, dtb_resetb,0x0);	// Data Retimer/BERT/TMDS Decoder reset
						PHY_REG_M14C0_WrFL(bert_dtb_resetb);

						//OS_MsecSleep(5);	// ms delay

						PHY_REG_M14C0_RdFL(bert_dtb_resetb);
						PHY_REG_M14C0_Wr01(bert_dtb_resetb, dtb_resetb,0x1);	// Data Retimer/BERT/TMDS Decoder reset
						PHY_REG_M14C0_WrFL(bert_dtb_resetb);
#if WORK_NEEDS_TO_BE_DONE_BY_DOSA
						PHY_REG_M14C0_RdFL(reset_pdb_sel);
						PHY_REG_M14C0_Wr01(reset_pdb_sel, reset_sel,0x0);	// Reset Manual Control
						PHY_REG_M14C0_WrFL(reset_pdb_sel);
#endif
#endif
						//_HDMI_M14C0_Enable_TMDS_Error_Interrupt();
...
#ifdef M14C0_BRINGUP_TODO
						_HDMI_M14C0_TMDS_ResetPort_Control(port_num ,1 );
#endif

						HDMI_DEBUG("---- HDMI_PHY : port [%d] TCS Done [%d]=>[%d]\n",port_num, _gM14C0HDMIPhyInform.tcs_done[port_num], tcs_done_value );
						...
						{
							_HDMI_M14C0_TCS_add[port_num] = 0x6;
							HDMI_NOTI("@@@@ HDMI_PHY : port [%d] May be Portable Tester ??? : Add value to [0x%x] \n",port_num, _HDMI_M14C0_TCS_add[port_num] );
							...
							PHY_REG_M14C0_RdFL(tcs_ch0);
							PHY_REG_M14C0_Rd01(tcs_ch0, tcs_ch0, _HDMI_M14C0_TCS_result[_gM14C0HDMIPhyInform.prt_sel][0]);

							PHY_REG_M14C0_RdFL(tcs_ch1);
							PHY_REG_M14C0_Rd01(tcs_ch1, tcs_ch1, _HDMI_M14C0_TCS_result[_gM14C0HDMIPhyInform.prt_sel][1]);

							PHY_REG_M14C0_RdFL(tcs_ch2);
							PHY_REG_M14C0_Rd01(tcs_ch2, tcs_ch2, _HDMI_M14C0_TCS_result[_gM14C0HDMIPhyInform.prt_sel][2]);
						}

						HDMI_NOTI("---- HDMI_PHY : TCS Result [0x%x] [0x%x] [0x%x] : Add value [0x%x] \n", _HDMI_M14C0_TCS_result[port_num][0] , _HDMI_M14C0_TCS_result[port_num][1] , _HDMI_M14C0_TCS_result[port_num][2], _HDMI_M14C0_TCS_add[port_num]);
						//	HDMI_DEBUG("---- HDMI_PHY : DTB reset port [%d] \n",port_num);
						_gM14C0HDMIPhyInform.tcs_min[port_num][0] = tcs_min_value[0];
						_gM14C0HDMIPhyInform.tcs_min[port_num][1] = tcs_min_value[1];
						_gM14C0HDMIPhyInform.tcs_min[port_num][2] = tcs_min_value[2];
						_gM14C0HDMIPhyInform.tcs_max[port_num][0] = tcs_max_value[0];
						_gM14C0HDMIPhyInform.tcs_max[port_num][1] = tcs_max_value[1];
						_gM14C0HDMIPhyInform.tcs_max[port_num][2] = tcs_max_value[2];
						_gM14C0HDMIPhyInform.tcs_add[port_num] = _HDMI_M14C0_TCS_add[port_num];
						_gM14C0HDMIPhyInform.tcs_result[port_num][0] = _HDMI_M14C0_TCS_result[port_num][0];
						_gM14C0HDMIPhyInform.tcs_result[port_num][1] = _HDMI_M14C0_TCS_result[port_num][1];
						_gM14C0HDMIPhyInform.tcs_result[port_num][2] = _HDMI_M14C0_TCS_result[port_num][2];
					}
				}
			}
			...
			{
				PHY_REG_M14C0_RdFL(cr_lock_done);
				PHY_REG_M14C0_Rd01(cr_lock_done, cr_lock, cr_lock[port_num]);
				PHY_REG_M14C0_Rd01(cr_lock_done, cr_done, cr_done[port_num]);
				...
				if( prev_cr_done[port_num] != cr_done[port_num] )
				{
					if(cr_done[port_num] == 1)
					{
						...
						PHY_REG_M14C0_RdFL(bert_tmds_sel);
						PHY_REG_M14C0_Wr01(bert_tmds_sel,tcs_en,0x0);
						PHY_REG_M14C0_WrFL(bert_tmds_sel);

						OS_MsecSleep(20);

						PHY_REG_M14C0_RdFL(bert_tmds_sel);
						PHY_REG_M14C0_Wr01(bert_tmds_sel,tcs_en,0x1);
						PHY_REG_M14C0_WrFL(bert_tmds_sel);

						_gM14C0HDMIPhyInform.tcs_restart_count[port_num] ++;
					}
	...
	if (_gM14C0_HDMI_MHLAutoModeCnt > 0)
	{
		_gM14C0_HDMI_MHLAutoModeCnt--;
		if ( ( _gM14C0_HDMI_MHLAutoModeCnt == 0) && (_g_mhl_enable_flag > 0) )
		{
			_HDMI_M14C0_Set_ManMHLMode(0, 0);
	...
	for (port_num = 0; port_num < 4;port_num ++)
	{
		_HDMI_M14C0_Get_EDID_Rd(port_num , &_gM14C0HDMIPhyInform.edid_rd_done[port_num], &temp);
		_HDMI_M14C0_Get_HDCP_info(port_num , &_gM14C0HDMIPhyInform.hdcp_authed[port_num], &_gM14C0HDMIPhyInform.hdcp_enc_en[port_num]);
	}

	/*
	if (_gM14C0HDMIPhyInform.hdcp_authed[_gM14C0HDMIPhyInform.prt_sel] && _gM14C0HDMIPhyInform.hdcp_enc_en[_gM14C0HDMIPhyInform.prt_sel] )
		hdcp_status = HDMI_HDCP_AUTH_DONE;
	else
	{
		HDMI_M14C0_Get_Aksv_Data(_gM14C0HDMIPhyInform.prt_sel, aksv_data);
		if ( (memcmp(aksv_data, (void*){0,}, sizeof(aksv_data)) ) && _gM14C0HDMIPhyInform.hdcp_authed[_gM14C0HDMIPhyInform.prt_sel])
			memcmp
	}
	*/
	current_time_eq = jiffies_to_msecs(jiffies);
	elasped_time_eq =  current_time_eq - _TCS_Done_Time[_gM14C0HDMIPhyInform.prt_sel];

	if ( (elasped_time_eq > 3000) && ( ( _gM14C0_TMDS_ERROR_EQ != -1) || ( _gM14C0_TMDS_ERROR_EQ_2nd != -1) ) )
	{
//		_HDMI_M14C0_Disable_TMDS_Error_Interrupt();
		HDMI_DEBUG("---- HDMI_PHY Stable??? for [%d]msec : Disable ECC ERROR check\n", elasped_time_eq);
		HDMI_DEBUG("---- 1st EQ ERROR[%d], 2nd EQ ERROR[%d]", _gM14C0_TMDS_ERROR_EQ, _gM14C0_TMDS_ERROR_EQ_2nd);
		_gM14C0_TMDS_ERROR_EQ = -1;
		_gM14C0_TMDS_ERROR_EQ_2nd = -1;

	}

	if(_gM14C0_TMDS_ERROR_intr_count > 0)
	{
		_gM14C0_TMDS_ERROR_intr_count = 0;
		_gM14C0_TMDS_ERROR_count ++;
		_gM14C0HDMIPhyInform.ecc_error_count[0] ++;

		if( (_gM14C0HDMIPhyInform.tcs_done[_gM14C0HDMIPhyInform.prt_sel] == 1) \
				&& ( ( _HDMI_M14C0_TCS_add[_gM14C0HDMIPhyInform.prt_sel] == _HDMI_M14C0_TCS_addtional_value_3rd) || (gHDMI_Phy_Control.sw_eq_algorithm_workaround == 0) )\
				&& (_gM14C0_tcs_fail_restart_count < 3) )
			_gM14C0_TMDS_ERROR_count_tcs_fail++;
		else
			_gM14C0_TMDS_ERROR_count_tcs_fail = 0;

		if(gHDMI_Phy_Control.hdcp_reset_ecc_err_workaround)
		{
			if(_gM14C0HDMIPhyInform.hdcp_authed[_gM14C0HDMIPhyInform.prt_sel] && _gM14C0HDMIPhyInform.hdcp_enc_en[_gM14C0HDMIPhyInform.prt_sel] && _gM14C0_HDCP_recovery_count < 1 )
				_gM14C0_TMDS_ERROR_count_HDCP_recovery ++;
			else
				_gM14C0_TMDS_ERROR_count_HDCP_recovery = 0;
		}

		if(_gM14C0_TMDS_ERROR_count > 3)
		{
			HDMI_NOTI("!!! TMDS Errors [%d] count[%d] : force scdt to zero , TCS[0x%x/0x%x/0x%x] Add[0x%x] !!!\n", _gM14C0_TMDS_ERROR_count, _gM14C0_TMDS_ERROR_count_tcs_fail, \
					_HDMI_M14C0_TCS_result[_gM14C0HDMIPhyInform.prt_sel][0], _HDMI_M14C0_TCS_result[_gM14C0HDMIPhyInform.prt_sel][1], _HDMI_M14C0_TCS_result[_gM14C0HDMIPhyInform.prt_sel][2], _HDMI_M14C0_TCS_add[_gM14C0HDMIPhyInform.prt_sel]);
			_gM14C0_TMDS_ERROR_count = 0;
			scdt = 0;

			if(_gM14C0_TMDS_ERROR_count_tcs_fail > 200) // TCS Failed (TMDS error for 4-sec and tcs value == 4)
			{
				HDMI_NOTI("!!!!! TCS Failed : count [%d] ,TCS[0x%x/0x%x/0x%x], restart_count[%d] !!!!!\n", _gM14C0_TMDS_ERROR_count_tcs_fail, \
						_HDMI_M14C0_TCS_result[_gM14C0HDMIPhyInform.prt_sel][0], _HDMI_M14C0_TCS_result[_gM14C0HDMIPhyInform.prt_sel][1], _HDMI_M14C0_TCS_result[_gM14C0HDMIPhyInform.prt_sel][2] ,_gM14C0_tcs_fail_restart_count);
				HDMI_NOTI("!!!!! Restart TCS  !!!!!\n");

				_gM14C0_tcs_fail_restart_count++;
				_gM14C0_TMDS_ERROR_count_tcs_fail = 0;
		...
		//
					...
					_gM14C0HDMIPhyInform.tcs_done[_gM14C0HDMIPhyInform.prt_sel] = 0;
			...
			if(gHDMI_Phy_Control.hdcp_reset_ecc_err_workaround)
			{
				if(_gM14C0_TMDS_ERROR_count_HDCP_recovery > 40) // ECC error more than 800msec
				{
					HDMI_NOTI("!!! ECC Errors [%d] : HDCP RESET count[%d] !!!\n", _gM14C0_TMDS_ERROR_count_HDCP_recovery, _gM14C0_HDCP_recovery_count);
					_HDMI_M14C0_HDCP_ResetPort(_gM14C0HDMIPhyInform.prt_sel);
					_gM14C0_TMDS_ERROR_count_HDCP_recovery = 0;
					_gM14C0_HDCP_recovery_count ++;
	...
	else
	{
		...
		_gM14C0_TMDS_ERROR_count_tcs_fail = 0;
		_gM14C0_TMDS_ERROR_count_HDCP_recovery = 0;
		_gM14C0_HDCP_recovery_count = 0;

	}

	if(_gM14C0_HDCP_ERROR_intr_count > 0)
	{
		_gM14C0_HDCP_ERROR_intr_count = 0;
		_gM14C0_HDCP_ERROR_count ++;
		_gM14C0HDMIPhyInform.hdcp_error_count[0] ++;

		if(gHDMI_Phy_Control.hdcp_reset_hdcp_err_workaround)
		{
			if(_gM14C0HDMIPhyInform.hdcp_authed[_gM14C0HDMIPhyInform.prt_sel] && _gM14C0HDMIPhyInform.hdcp_enc_en[_gM14C0HDMIPhyInform.prt_sel] && _gM14C0_HDCP_recovery_count < 1 )
				_gM14C0_HDCP_ERROR_count_HDCP_recovery ++;
			else
				_gM14C0_HDCP_ERROR_count_HDCP_recovery = 0;
		}

		if(_gM14C0_HDCP_ERROR_count > 3)
		{
			HDMI_NOTI("!!! HDCP Errors [%d][%d] ,HDCP[%d,%d] @@@\n", _gM14C0_HDCP_ERROR_count, _gM14C0_HDCP_ERROR_count_HDCP_recovery, _gM14C0HDMIPhyInform.hdcp_authed[_gM14C0HDMIPhyInform.prt_sel], _gM14C0HDMIPhyInform.hdcp_enc_en[_gM14C0HDMIPhyInform.prt_sel]);

			_gM14C0_HDCP_ERROR_count = 0;

			if(gHDMI_Phy_Control.hdcp_reset_hdcp_err_workaround)
			{
				if(_gM14C0_HDCP_ERROR_count_HDCP_recovery > 20) // ECC error more than 400msec
				{
					HDMI_NOTI("@@@ !!! HDCP Errors [%d] : HDCP RESET count[%d] !!! @@@ \n", _gM14C0_HDCP_ERROR_count_HDCP_recovery, _gM14C0_HDCP_recovery_count);
					_gM14C0_HDCP_ERROR_count_HDCP_recovery = 0;
					_gM14C0_HDCP_recovery_count ++;
					_HDMI_M14C0_HDCP_ResetPort(_gM14C0HDMIPhyInform.prt_sel);
					// _HDMI_M14C0_Set_HPD_Out(_gM14C0HDMIPhyInform.prt_sel, !_gM14C0HDMIPhyInform.hpd_pol[_gM14C0HDMIPhyInform.prt_sel]);	// disable HPD
	...
	if(_gM14C0_TERC4_ERROR_intr_count[0] > 0) {
		_gM14C0_TERC4_ERROR_intr_count[0] = 0;
		_gM14C0HDMIPhyInform.terc4_error_count[0] ++;
	}

	if(_gM14C0_TERC4_ERROR_intr_count[1] > 0) {
		_gM14C0_TERC4_ERROR_intr_count[1] = 0;
		_gM14C0HDMIPhyInform.terc4_error_count[1] ++;
	}

	if(_gM14C0_TERC4_ERROR_intr_count[2] > 0) {
		_gM14C0_TERC4_ERROR_intr_count[2] = 0;
		_gM14C0HDMIPhyInform.terc4_error_count[2] ++;
	}

	if(_gM14C0_TERC4_ERROR_intr_count[3] > 0) {
		_gM14C0_TERC4_ERROR_intr_count[3] = 0;
		_gM14C0HDMIPhyInform.terc4_error_count[3] ++;
	}
	/*
	if ( ( elasped_time_eq < 5000 ) && (_gM14C0_TMDS_ERROR_EQ != -1) )
		_HDMI_M14C0_Enable_TMDS_Error_Interrupt();
		*/

	/* more than 8 ECC Error interrrupt occured during 1.8 second,
	   change EQ CS value to 0xf */
	//if ( (_gM14C0_TMDS_ERROR_EQ > 30) && ( elasped_time_eq < 3000 ) )
	//if ( (_gM14C0_TMDS_ERROR_EQ > 8) && ( elasped_time_eq < 1800 ) )
	if ( (_gM14C0_TMDS_ERROR_EQ > 16) && ( elasped_time_eq < 3000 ) )	// 140403
	{
		/* Enable port_num */
		_HDMI_M14C0_EnablePort(_gM14C0HDMIPhyInform.prt_sel, 1);

		PHY_REG_M14C0_RdFL(tcs_boundary);
		PHY_REG_M14C0_Wr01(tcs_boundary, tcs_boundary, _HDMI_M14C0_TCS_addtional_value_2nd);
		PHY_REG_M14C0_WrFL(tcs_boundary);

		PHY_REG_M14C0_RdFL(tcs_additional_value);
		PHY_REG_M14C0_Wr01(tcs_additional_value, tcs_additional_value, _HDMI_M14C0_TCS_addtional_value_2nd);
		PHY_REG_M14C0_WrFL(tcs_additional_value);

#ifdef M14C0_BRINGUP_TODO
		PHY_REG_M14C0_RdFL(eq_cs_man_ch0);
		PHY_REG_M14C0_RdFL(eq_cs_man_ch1);
		PHY_REG_M14C0_RdFL(eq_cs_man_ch2);
		//PHY_REG_M14C0_Wr01(eq_cs_man, eq_cs_man, 0xf);
		PHY_REG_M14C0_Wr01(eq_cs_man_ch0, eq_cs_man_ch0, 0x11);		// 140403 : changed cs_man value for BP540 and 15M Cable
		PHY_REG_M14C0_Wr01(eq_cs_man_ch1, eq_cs_man_ch1, 0x11);		// 140403 : changed cs_man value for BP540 and 15M Cable
		PHY_REG_M14C0_Wr01(eq_cs_man_ch2, eq_cs_man_ch2, 0x11);		// 140403 : changed cs_man value for BP540 and 15M Cable
		PHY_REG_M14C0_Rd01(eq_cs_man_ch0, eq_cs_man_ch0, _EQ_cs_man_value[_gM14C0HDMIPhyInform.prt_sel]);
		PHY_REG_M14C0_WrFL(eq_cs_man_ch0);
		PHY_REG_M14C0_WrFL(eq_cs_man_ch1);
		PHY_REG_M14C0_WrFL(eq_cs_man_ch2);
		...
		_HDMI_M14C0_TMDS_ResetPort_Control(_gM14C0HDMIPhyInform.prt_sel ,1 );
#endif
		_SCDT_Fall_Detected[_gM14C0HDMIPhyInform.prt_sel] = 0;	// clear SCDT interrupt from above mode_sel reset
		...
		PHY_REG_M14C0_RdFL(tcs_additional_value);
		PHY_REG_M14C0_Rd01(tcs_additional_value, tcs_additional_value, _HDMI_M14C0_TCS_add[_gM14C0HDMIPhyInform.prt_sel]);
/*
		PHY_REG_M14C0_RdFL(reset_pdb_sel);
		PHY_REG_M14C0_Wr01(reset_pdb_sel, reset_sel,0x1);	// Reset Manual Control
		PHY_REG_M14C0_WrFL(reset_pdb_sel);
...
*/
//		_HDMI_M14C0_Disable_TMDS_Error_Interrupt();
		HDMI_NOTI("---- PHY port[%d] : ECC Err intr [%d] in [%d]msec : TCS Add to [0x%x], count[%d]\n",_gM14C0HDMIPhyInform.prt_sel, _gM14C0_TMDS_ERROR_EQ, elasped_time_eq, _HDMI_M14C0_TCS_addtional_value_2nd, _gM14C0_tcs_fail_restart_count );

		if( gHDMI_Phy_Control.sw_eq_algorithm_workaround)
		{
			_gM14C0_TMDS_ERROR_EQ = -1;
			_gM14C0_TMDS_ERROR_EQ_2nd = 0;
		}
		cs_man_changed_time_eq = jiffies_to_msecs(jiffies);

		_gM14C0HDMIPhyInform.tcs_add[_gM14C0HDMIPhyInform.prt_sel] = _HDMI_M14C0_TCS_add[_gM14C0HDMIPhyInform.prt_sel];
		_gM14C0HDMIPhyInform.tcs_result[_gM14C0HDMIPhyInform.prt_sel][0] = _HDMI_M14C0_TCS_result[_gM14C0HDMIPhyInform.prt_sel][0];
		_gM14C0HDMIPhyInform.tcs_result[_gM14C0HDMIPhyInform.prt_sel][1] = _HDMI_M14C0_TCS_result[_gM14C0HDMIPhyInform.prt_sel][1];
		_gM14C0HDMIPhyInform.tcs_result[_gM14C0HDMIPhyInform.prt_sel][2] = _HDMI_M14C0_TCS_result[_gM14C0HDMIPhyInform.prt_sel][2];

//		OS_MsecSleep(10);	// ms delay
//		_SCDT_Fall_Detected[_gM14C0HDMIPhyInform.prt_sel] = 0;	// clear SCDT interrupt
	}

	if(_gM14C0_TMDS_ERROR_EQ_2nd != -1)
	{
		HDMI_DEBUG("---- HDMI 2ND EQ Check [%d]\n", _gM14C0_TMDS_ERROR_EQ_2nd);
		current_time_eq = jiffies_to_msecs(jiffies);
	...
	//if ( (_gM14C0_TMDS_ERROR_EQ_2nd > 4) && ( elasped_2nd_time_eq < 900 ) )
	//if ( (_gM14C0_TMDS_ERROR_EQ_2nd > 8) && ( elasped_2nd_time_eq < 1800 ) )	//140404
	if ( (_gM14C0_TMDS_ERROR_EQ_2nd > 16) && ( elasped_2nd_time_eq < 3000 ) )	//140829
	{
//		int cs_man_value;
		...
		PHY_REG_M14C0_RdFL(tcs_boundary);
		PHY_REG_M14C0_Wr01(tcs_boundary, tcs_boundary, _HDMI_M14C0_TCS_addtional_value_3rd);
		PHY_REG_M14C0_WrFL(tcs_boundary);

		PHY_REG_M14C0_RdFL(tcs_additional_value);
		PHY_REG_M14C0_Wr01(tcs_additional_value, tcs_additional_value, _HDMI_M14C0_TCS_addtional_value_3rd);
		PHY_REG_M14C0_WrFL(tcs_additional_value);

#ifdef M14C0_BRINGUP_TODO
		PHY_REG_M14C0_RdFL(eq_cs_man_ch0);
		PHY_REG_M14C0_Rd01(eq_cs_man_ch0, eq_cs_man_ch0, cs_man_value);

		for(;cs_man_value > 0x3; cs_man_value--)
		{
			PHY_REG_M14C0_RdFL(eq_cs_man_ch0);
			PHY_REG_M14C0_Wr01(eq_cs_man_ch0, eq_cs_man_ch0, cs_man_value);
			PHY_REG_M14C0_RdFL(eq_cs_man_ch1);
			PHY_REG_M14C0_Wr01(eq_cs_man_ch1, eq_cs_man_ch1, cs_man_value);
			PHY_REG_M14C0_RdFL(eq_cs_man_ch2);
			PHY_REG_M14C0_Wr01(eq_cs_man_ch2, eq_cs_man_ch2, cs_man_value);
			PHY_REG_M14C0_Rd01(eq_cs_man_ch0, eq_cs_man_ch0, _EQ_cs_man_value[_gM14C0HDMIPhyInform.prt_sel]);
			PHY_REG_M14C0_WrFL(eq_cs_man_ch0);
			PHY_REG_M14C0_WrFL(eq_cs_man_ch1);
			PHY_REG_M14C0_WrFL(eq_cs_man_ch2);
...
*/
//		_HDMI_M14C0_Disable_TMDS_Error_Interrupt();
		HDMI_NOTI("---- PHY port[%d] : ECC Err 2ND intr [%d] in [%d]msec : TCS Add to [0x%x], count[%d] \n",_gM14C0HDMIPhyInform.prt_sel, _gM14C0_TMDS_ERROR_EQ_2nd, elasped_2nd_time_eq, _HDMI_M14C0_TCS_addtional_value_3rd, _gM14C0_tcs_fail_restart_count );
...
//		_SCDT_Fall_Detected[_gM14C0HDMIPhyInform.prt_sel] = 0;	// clear SCDT interrupt
	...
	/* more than 5 avi change interrupt occured during 5second,
	   change EQ CS value to 0xf */
	if ( (_gM14C0CHG_AVI_count_EQ > 5) && ( elasped_time_eq < 5000 ) )
	{
		/* Enable port_num */
		_HDMI_M14C0_EnablePort(_gM14C0HDMIPhyInform.prt_sel, 1);
		...
		PHY_REG_M14C0_RdFL(eq_cs_man_ch1);
		PHY_REG_M14C0_RdFL(eq_cs_man_ch2);
		PHY_REG_M14C0_Wr01(eq_cs_man_ch0, eq_cs_man_ch0, 0xf);
		PHY_REG_M14C0_Rd01(eq_cs_man_ch0, eq_cs_man_ch0, _EQ_cs_man_value[_gM14C0HDMIPhyInform.prt_sel]);
		PHY_REG_M14C0_WrFL(eq_cs_man_ch0);
		PHY_REG_M14C0_WrFL(eq_cs_man_ch1);
		PHY_REG_M14C0_WrFL(eq_cs_man_ch2);

		HDMI_NOTI("---- HDMI_PHY port[%d] : AVI_CHG intr [%d] in [%d]msec : EQ_CS_MAN to [%d]\n",_gM14C0HDMIPhyInform.prt_sel, _gM14C0CHG_AVI_count_EQ, elasped_time_eq, 0xf );

		_gM14C0CHG_AVI_count_EQ = -1;
	}
	...
	else
	{
		...
		if (portCnt < 3)
		{
			...
			*/
			...
			_HDMI_M14C0_SetInternalMute(muteCtrl);	// Video All Clear
	...
	_HDMI_M14C0_Enable_TMDS_Error_Interrupt();
	_HDMI_M14C0_Enable_HDCP_Error_Interrupt();
	_HDMI_M14C0_Enable_TERC4_Error_Interrupt(_gM14C0HDMIPhyInform.prt_sel);

// 130926 : wonsik.do spin lock needed ???
//	spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
	_gM14C0HDMIConnectState = scdt;
...
static int _HDMI_M14C0_PhyRunReset(void)
{
	...
	PHY_REG_M14C0_RdFL(eq_i2c_odt_pdb);
	PHY_REG_M14C0_Wr01(eq_i2c_odt_pdb,eq_i2c_odt_pdb_mode,0x1);
	PHY_REG_M14C0_Wr01(eq_i2c_odt_pdb,eq_man_odt_pdb,0x1);
	PHY_REG_M14C0_WrFL(eq_i2c_odt_pdb);

	PHY_REG_M14C0_RdFL(cr_mode_sel_resetb);
	PHY_REG_M14C0_Wr01(cr_mode_sel_resetb, cr_mode_sel_resetb,0x0);
	PHY_REG_M14C0_WrFL(cr_mode_sel_resetb);

	/* M14C0 New HDMI Phy Register Settings */
	PHY_REG_M14C0_RdFL(tcs_rs_mode_sel);
	PHY_REG_M14C0_Wr01(tcs_rs_mode_sel, tcs_mode_sel,_HDMI_M14C0_TCS_mode_sel);
//	PHY_REG_M14C0_Wr01(tcs_rs_mode_sel, eq_rs_mode_sel,0x1);
	PHY_REG_M14C0_WrFL(tcs_rs_mode_sel);

	if(_HDMI_M14C0_TCS_mode_sel == 0x2)
	{
		PHY_REG_M14C0_RdFL(eq_cs_man_ch0);
		PHY_REG_M14C0_Wr01(eq_cs_man_ch0, eq_cs_man_ch0,0x12);
		PHY_REG_M14C0_WrFL(eq_cs_man_ch0);

		PHY_REG_M14C0_RdFL(eq_cs_man_ch1);
		PHY_REG_M14C0_Wr01(eq_cs_man_ch1, eq_cs_man_ch1,0x12);
		PHY_REG_M14C0_WrFL(eq_cs_man_ch1);

		PHY_REG_M14C0_RdFL(eq_cs_man_ch2);
		PHY_REG_M14C0_Wr01(eq_cs_man_ch2, eq_cs_man_ch2,0x12);
		PHY_REG_M14C0_WrFL(eq_cs_man_ch2);
	}
	else if(_HDMI_M14C0_TCS_mode_sel == 0x3)
	{

		PHY_REG_M14C0_RdFL(tcs_err_sel);
		PHY_REG_M14C0_Wr01(tcs_err_sel, tcs_err_sel,0x3);
		PHY_REG_M14C0_WrFL(tcs_err_sel);

		PHY_REG_M14C0_RdFL(tcs_bcs_ch_sel);
		PHY_REG_M14C0_Wr01(tcs_bcs_ch_sel, bcs_ch_sel,0x0);
		PHY_REG_M14C0_Wr01(tcs_bcs_ch_sel, tcs_ch_sel,0x0);
		PHY_REG_M14C0_WrFL(tcs_bcs_ch_sel);
	}

	PHY_REG_M14C0_RdFL(tcs_repeat_off_divide_method);
	PHY_REG_M14C0_Wr01(tcs_repeat_off_divide_method, tcs_divide_method, 0x2);	//default : 0x2
	PHY_REG_M14C0_WrFL(tcs_repeat_off_divide_method);
	...
	PHY_REG_M14C0_RdFL(dr_clk_inv_ch);
	PHY_REG_M14C0_Wr01(dr_clk_inv_ch,dr_clk_inv_ch0,0x1);
	PHY_REG_M14C0_Wr01(dr_clk_inv_ch,dr_clk_inv_ch1,0x1);
	PHY_REG_M14C0_Wr01(dr_clk_inv_ch,dr_clk_inv_ch2,0x1);
	PHY_REG_M14C0_WrFL(dr_clk_inv_ch);

	/* M14 New HDMI Phy Register Settings */
#if WORK_NEEDS_TO_BE_DONE_BY_DOSA

	PHY_REG_M14C0_RdFL(eq_cs_rs_sel);
	PHY_REG_M14C0_Wr01(eq_cs_rs_sel,eq_rs_man_sel,0x1);
#ifdef M14_HDMI_SEMI_AUTO_EQ_CONTROL
	PHY_REG_M14C0_Wr01(eq_cs_rs_sel,eq_cs_man_sel,0x1);
#endif
	PHY_REG_M14C0_WrFL(eq_cs_rs_sel);
#endif

#ifdef M14_HDMI_SEMI_AUTO_EQ_CONTROL
	PHY_REG_M14C0_RdFL(ck_mode_cs_man_00);
	PHY_REG_M14C0_Wr01(ck_mode_cs_man_00,ck_mode_cs_man_00,0x6);
	PHY_REG_M14C0_WrFL(ck_mode_cs_man_00);
	PHY_REG_M14C0_RdFL(ck_mode_cs_man_01);
	PHY_REG_M14C0_Wr01(ck_mode_cs_man_01,ck_mode_cs_man_01,0x8);
	PHY_REG_M14C0_WrFL(ck_mode_cs_man_01);
	PHY_REG_M14C0_RdFL(ck_mode_cs_man_10);
	PHY_REG_M14C0_Wr01(ck_mode_cs_man_10,ck_mode_cs_man_10,0xA);
	PHY_REG_M14C0_WrFL(ck_mode_cs_man_10);
	PHY_REG_M14C0_RdFL(ck_mode_cs_man_11);
	PHY_REG_M14C0_Wr01(ck_mode_cs_man_11,ck_mode_cs_man_11,0xC);
	PHY_REG_M14C0_WrFL(ck_mode_cs_man_11);
#endif

	//140930 (4K, 15M, DVB)
	PHY_REG_M14C0_RdFL(eq_freq_div_ch0);
	PHY_REG_M14C0_Wr01(eq_freq_div_ch0,eq_freq_div_ch0,0x6); //141028 :0x6 (HDCP Error:MSPG-7100) //140930 : 0xF (4K,DVB)
	PHY_REG_M14C0_WrFL(eq_freq_div_ch0);
	PHY_REG_M14C0_RdFL(eq_freq_div_ch1);
	PHY_REG_M14C0_Wr01(eq_freq_div_ch1,eq_freq_div_ch1,0x6); //141028 :0x6 (HDCP Error:MSPG-7100) //140930 : 0xF (4K,DVB)
	PHY_REG_M14C0_WrFL(eq_freq_div_ch1);
	PHY_REG_M14C0_RdFL(eq_freq_div_ch2);
	PHY_REG_M14C0_Wr01(eq_freq_div_ch2,eq_freq_div_ch2,0x6); //141028 :0x6 (HDCP Error:MSPG-7100) //140930 : 0xF (4K,DVB)
	PHY_REG_M14C0_WrFL(eq_freq_div_ch2);

	//140930 : 4K,15M,DVB
	PHY_REG_M14C0_RdFL(tcs_rs_mode_sel);
	PHY_REG_M14C0_Wr01(tcs_rs_mode_sel,eq_rs_mode_sel,0x1); //140930(4K,DVB)
	PHY_REG_M14C0_WrFL(tcs_rs_mode_sel);

	PHY_REG_M14C0_RdFL(tmds_clk_ref_inv_muxed_clk_sel);
	PHY_REG_M14C0_Wr01(tmds_clk_ref_inv_muxed_clk_sel,muxed_clk_sel,0x0); 		//default
	PHY_REG_M14C0_Wr01(tmds_clk_ref_inv_muxed_clk_sel,tmds_clk_ref_inv,0x0); 	//default
	PHY_REG_M14C0_WrFL(tmds_clk_ref_inv_muxed_clk_sel);

	// For edison room XBOX360
	PHY_REG_M14C0_RdFL(idr_adj);
	PHY_REG_M14C0_Wr01(idr_adj,idr_adj,0x3); //140930(4K,DVB) //140829 by shyang
	PHY_REG_M14C0_WrFL(idr_adj);

#ifdef FOR_EDISON_XBOX360
	PHY_REG_M14C0_RdFL(cr_vbgr_ctrl);
	PHY_REG_M14C0_Wr01(cr_vbgr_ctrl,cr_vbgr_ctrl,0x3);
	PHY_REG_M14C0_WrFL(cr_vbgr_ctrl);

	PHY_REG_M14C0_RdFL(cr_tmr_scale);
	PHY_REG_M14C0_Wr01(cr_tmr_scale,cr_tmr_scale,0x02);
	PHY_REG_M14C0_WrFL(cr_tmr_scale);

	PHY_REG_M14C0_RdFL(eq_eval_time_1);
	PHY_REG_M14C0_Wr01(eq_eval_time_1,eq_eval_time_1, (_HDMI_M14C0_eq_eval_time_value >> 8) & 0xFF);
	PHY_REG_M14C0_WrFL(eq_eval_time_1);

	PHY_REG_M14C0_RdFL(eq_eval_time_2);
	PHY_REG_M14C0_Wr01(eq_eval_time_2,eq_eval_time_2, _HDMI_M14C0_eq_eval_time_value & 0xFF);
	PHY_REG_M14C0_WrFL(eq_eval_time_2);

	PHY_REG_M14C0_RdFL(eq_filter);
	PHY_REG_M14C0_Wr01(eq_filter,eq_filter,0x0f);	//0x1f : MSPG-4233 HDCP fail issue //0x3f // default 0x0f
	PHY_REG_M14C0_WrFL(eq_filter);
#endif
	// End of For edison room XBOX360

	PHY_REG_M14C0_RdFL(tmds_errec_detect);
	PHY_REG_M14C0_Wr01(tmds_errec_detect,tmds_errec_detect,0x1);
	PHY_REG_M14C0_WrFL(tmds_errec_detect);

	PHY_REG_M14C0_RdFL(bert_tmds_sel);
	PHY_REG_M14C0_Wr01(bert_tmds_sel,tcs_en,0x1);
	PHY_REG_M14C0_WrFL(bert_tmds_sel);

	PHY_REG_M14C0_RdFL(tmds_sync_recover_clk_inv_bypass);
	PHY_REG_M14C0_Wr01(tmds_sync_recover_clk_inv_bypass,tmds_clk_inv,0x1);
	PHY_REG_M14C0_WrFL(tmds_sync_recover_clk_inv_bypass);

	PHY_REG_M14C0_RdFL(reg_tcs_man_pre);
	PHY_REG_M14C0_Wr01(reg_tcs_man_pre,reg_tcs_man_pre,0x40);
	PHY_REG_M14C0_WrFL(reg_tcs_man_pre);

	PHY_REG_M14C0_RdFL(reg_tcs_man_mea_1);
	PHY_REG_M14C0_Wr01(reg_tcs_man_mea_1,reg_tcs_man_mea,0x44);
	PHY_REG_M14C0_WrFL(reg_tcs_man_mea_1);

	PHY_REG_M14C0_RdFL(tcs_scdt);
	PHY_REG_M14C0_Wr01(tcs_scdt,tcs_scdt,0x1);
	PHY_REG_M14C0_WrFL(tcs_scdt);

	/*
	PHY_REG_M14C0_RdFL(cr_mode_sel_resetb);
	PHY_REG_M14C0_Wr01(cr_mode_sel_resetb,cr_mode_sel_resetb,0x01);
	PHY_REG_M14C0_WrFL(cr_mode_sel_resetb);
	*/

	// set eq at 2012-11-17
	//PHY_REG_M14C0_RdFL(dr_n1);
	//PHY_REG_M14C0_Wr01(dr_n1,dr_n1,0x7);
	//PHY_REG_M14C0_WrFL(dr_n1);

	//PHY_REG_M14C0_RdFL(eq_rs);
	//PHY_REG_M14C0_Wr01(eq_rs,eq_rs,0x5);		//2012-11-17 change 3 -> 5
	//PHY_REG_M14C0_WrFL(eq_rs);

	//PHY_REG_M14C0_RdFL(cr_kvco_offset);
	//PHY_REG_M14C0_Wr01(cr_kvco_offset,cr_kvco_offset,0x1);		//2012-11-22 change 2 -> 1
	//PHY_REG_M14C0_WrFL(cr_kvco_offset);

/*	// HDCP Problem to the Master for L9 -- need cheak  : default value 0x1
	PHY_REG_M14C0_RdFL(eq_af_en_avg_width);
	PHY_REG_M14C0_Wr01(eq_af_en_avg_width,eq_avg_width,0x0);
	PHY_REG_M14C0_WrFL(eq_af_en_avg_width);
*/
	LINK_M14_RdFL(phy_link_06);
	...
	if(_g_Disable_EQ_Offset_Cal[prt_selected] == 1) {
		...
		_g_Disable_EQ_Offset_Cal[prt_selected] = 0;
	}
	PHY_REG_M14C0_RdFL(dr_filter_ch0);
	PHY_REG_M14C0_RdFL(eq_rs_man);
	...
	if ( (gHDMI_Phy_Control.all_port_pdb_enable && phy_enable_prt3) || (!gHDMI_Phy_Control.all_port_pdb_enable && (prt_selected==0x3) ) )
	{
		/*
		   ...
		   PHY_REG_M14C0_RdFL(eq_i2c_odt_pdb_mode);
		   PHY_REG_M14C0_Wr01(eq_i2c_odt_pdb_mode,eq_i2c_odt_pdb,0x0);
		   PHY_REG_M14C0_Wr01(eq_i2c_odt_pdb_mode,eq_i2c_odt_pdb_mode,0x1);
		   PHY_REG_M14C0_WrFL(eq_i2c_odt_pdb_mode);
		 */
		HDMI_DEBUG("---- PHY INIT for MHL,  eq_i2c_pdb_d0 :0x10, eq_i2c_pdb_dck :0x10\n");
		PHY_REG_M14C0_RdFL(eq_i2c_pdb_d0);
		PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_i2c_pdb_d0_mode,0x1);
		PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_man_pdb_d0,0x0);
		PHY_REG_M14C0_WrFL(eq_i2c_pdb_d0);

		PHY_REG_M14C0_RdFL(eq_i2c_pdb_dck);
		PHY_REG_M14C0_Wr01(eq_i2c_pdb_dck,eq_i2c_pdb_dck_mode,0x1);
		PHY_REG_M14C0_Wr01(eq_i2c_pdb_dck,eq_man_pdb_dck,0x0);
		PHY_REG_M14C0_WrFL(eq_i2c_pdb_dck);
	}
#endif
...
// GetMHLConnection also execute these settings
		...
		PHY_REG_M14C0_Wr01(dr_filter_ch0,dr_filter_ch0,0x1);
		HDMI_DEBUG("---- MHL EQ_RS_MAN  : [%d]\n", 0x5);
		PHY_REG_M14C0_Wr01(eq_rs_man,eq_rs_man,0x5);	//from 0x3(default)
#ifdef M14_CODE_FOR_MHL_CTS
		HDMI_DEBUG("---- MHL ODC CTRL  : [%d]\n", 0x0);
		PHY_REG_M14C0_RdFL(odt_ctrl);
		PHY_REG_M14C0_Wr01(odt_ctrl,odt_ctrl,0x0);
		PHY_REG_M14C0_WrFL(odt_ctrl);
#endif
#endif
	...
	else
	{
		...
		PHY_REG_M14C0_Wr01(dr_filter_ch0,dr_filter_ch0,0x7);
#ifdef M14_HDMI_SEMI_AUTO_EQ_CONTROL
		HDMI_DEBUG("---- HDMI EQ_RS_MAN  : [%d]\n", 0x3);
		PHY_REG_M14C0_Wr01(eq_rs_man,eq_rs_man,0x3);
#else
		HDMI_DEBUG("---- HDMI EQ_RS_MAN  : [%d]\n", 0x3);
		/* BD570 EQ error : EQ RS value 0x7 (default 0x3) , 131114 */
		PHY_REG_M14C0_Wr01(eq_rs_man,eq_rs_man,0x4);		//140930 : 4K, 15M , DVB
#endif
#ifdef M14_CODE_FOR_MHL_CTS
		HDMI_DEBUG("---- HDMI ODC CTRL  : [%d]\n", 0x1);
		PHY_REG_M14C0_RdFL(eq_odt_ctrl);
		PHY_REG_M14C0_Wr01(eq_odt_ctrl,eq_odt_ctrl,0x1);
		PHY_REG_M14C0_WrFL(eq_odt_ctrl);
#endif

		HDMI_DEBUG("---- HDMI DR_N1  : [%d]\n", 0x3);
		PHY_REG_M14C0_RdFL(dr_n1);
		PHY_REG_M14C0_Wr01(dr_n1,dr_n1, _HDMI_M14C0_hdmi_dr_n1_value); //121218(PC38363)	//140930(4K,dvb)	// for 15m cable (0x38/0x93)
		PHY_REG_M14C0_WrFL(dr_n1);

		// for HDMI : 0 for HPD out level 3.3Volt
		HDMI_DEBUG("---- HDMI CBUS_NORMAL_V1  : [%d]\n", 0x0);
		PHY_REG_M14C0_RdFL(cbus_normal_v1_pdb);
		PHY_REG_M14C0_Wr01(cbus_normal_v1_pdb,cbus_normal_v1,0x0);	// for HPD out Level
		PHY_REG_M14C0_WrFL(cbus_normal_v1_pdb);

		//141001 : HDMI : 0x26, MHL : 0x24
		HDMI_DEBUG("---- HDMI ICP_ADJ  : [0x%x]\n", 0x26);
		PHY_REG_M14C0_RdFL(cr_icp_adj);
		PHY_REG_M14C0_Wr01(cr_icp_adj,cr_icp_adj,0x6);		// 4K , DVB board (140930) //for HDMI (default 0x4)
		PHY_REG_M14C0_Wr01(cr_icp_adj,cr_mhl_icp_adj,0x1);	// 141023 : MHL CTS jitter test //for MHL (default 0x4)
		PHY_REG_M14C0_WrFL(cr_icp_adj);

		HDMI_DEBUG("---- HDMI EQ_WIDE_CKEX_BW  : [0x%x]\n", 0x0);
		PHY_REG_M14C0_RdFL(eq_wide_ckex_bw);
		PHY_REG_M14C0_Wr01(eq_wide_ckex_bw,eq_wide_ckex_bw,0x0); //141001 : MHL 24/PP mode
		PHY_REG_M14C0_WrFL(eq_wide_ckex_bw);
	}

	PHY_REG_M14C0_WrFL(dr_filter_ch0);
	PHY_REG_M14C0_WrFL(eq_rs_man);
	...
	PHY_REG_M14C0_RdFL(resetb_pdb_all);
	PHY_REG_M14C0_Wr01(resetb_pdb_all,resetb_all,0x1);
	PHY_REG_M14C0_WrFL(resetb_pdb_all);
...
static int _HDMI_M14C0_RunReset(void)
{
	...
	//0x298
	LINK_M14_RdFL(cbus_15);
	LINK_M14_Wr(cbus_15, 0x00223102); //141014 mhl version 2.2	//mhl version 2.1
	LINK_M14_WrFL(cbus_15);

	//0x29c
	LINK_M14_RdFL(cbus_16);
	LINK_M14_Wr(cbus_16, 0x64370100);	// 141008 disable PP mode // packed pixel support
...
/**
 *  _HDMI_M14C0_GetAudioTypeAndFreq
 *
 *  @return int
*/static int _HDMI_M14C0_GetAudioInfo(void)
{
	...
	//Increase _gM14C0HdmiAudioThreadCount;
	_gM14C0HdmiAudioThreadCount++;
	...
	if (reg_scdt == 0)	//Check HDMI, DVI Sync Detect
	{
		...
		if ((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_10S) == 0)
			HDMI_AUDIO("[%d]HDMI_GetAudioInfo : HDMI / DVI Not Connected(state = %d)!!!\n", __L__, _gM14C0IntrAudioState);
...
#ifdef M14_THREAD_READ_PHY_REG_VALUE
	ui64TmdsClock = (UINT64)(_gM14C0HDMIPhyInform.tmds_clock[0] ) * 10000;
#else
	PHY_REG_M14C0_RdFL(tmds_freq_1);
	PHY_REG_M14C0_RdFL(tmds_freq_2);

	PHY_REG_M14C0_Rd01(tmds_freq_1, tmds_freq, ui8TmdsClockHigh);
	PHY_REG_M14C0_Rd01(tmds_freq_2, tmds_freq, ui8TmdsClockLow);

	ui64TmdsClock = (UINT64)((ui8TmdsClockHigh << 8) | ui8TmdsClockLow) * 10000;
#endif

#if 1
	//check a UD(UHD) format(do support M14 Cx chip upper 25MHz TDMS Clock)
	if ((_gM14C0MHLContentOff == TRUE) && (_gM14C0HDMIPhyInform.prt_sel == 3) && (_gM14C0HDMIPhyInform.cd_sense == 1))
	{
		if ((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_10S) == 0)
			HDMI_AUDIO("[%d]HDMI_GetAudioInfo : MHL Contents Off(_gM14C0MHLContentOff = %d)!!!\n", __L__, _gM14C0MHLContentOff);

		goto func_exit;
	}
#else
	//check a UD(UHD) format(do not support M14 Bx chip upper 25MHz TDMS Clock)
	if (ui64TmdsClock > 250*1000*1000)
	{
		if ((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_10S) == 0)
			HDMI_AUDIO("[%d]HDMI_GetAudioInfo : HDMI UD(UHD) Format(ui64TmdsClock = %llu)!!!\n", __L__, ui64TmdsClock);

		goto func_exit;
	}
	//check a MHL Contents Off Status
	else if ((_gM14C0MHLContentOff == TRUE) && (_gM14C0HDMIPhyInform.prt_sel == 3) && (_gM14C0HDMIPhyInform.cd_sense == 1))
	{
		if ((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_10S) == 0)
			HDMI_AUDIO("[%d]HDMI_GetAudioInfo : MHL Contents Off(_gM14C0MHLContentOff = %d)!!!\n", __L__, _gM14C0MHLContentOff);
	...
	intrVideoState = _gM14C0HDMIState;
	//intrVideoState = HDMI_STATE_READ;	//temp for test

	//copy audio interrupt state
	intrAudioState = _gM14C0IntrAudioState;

	//spin lock for protection : unlock
	spin_unlock_irqrestore(&_gIntrHdmiM14C0AudioLock, flags);

	//Check _gM14C0IntrAudioState is stably changed.
	if ((intrAudioState == HDMI_AUDIO_STABLE_STATE && intrVideoState >= HDMI_STATE_READ)
	  ||(intrAudioState == HDMI_AUDIO_STABLE_STATE && _gM14C0HdmiPortStableCount > HDMI_AUDIO_PORT_STABLE_TIME_5S) )	//To protect a abnormal or transient video and audio info drop state.
	{
		if (_gM14C0HdmiAudioInfo.audioType == LX_HDMI_AUDIO_AC3_IEC60958)
		...
		if ((_gM14C0HdmiAudioThreadCount % audioRecheckTime) == 0)	// 0.5 or 1 seconds, Thread calls a every 20 ms.
		{
			//Check a current mute status for workaround
			if (_gM14C0AudioMuteState == TRUE)
			{
				//Unmute audio data for abnormal state
				muteCtrl.eMode		= LX_HDMI_AUDIO_MUTE;
				muteCtrl.bAudioMute = FALSE;
				_HDMI_M14C0_SetInternalMute(muteCtrl);
			...
			(void)_HDMI_M14C0_GetAudioTypeAndFreq(&audioType, &samplingFreq);

			//Get a sampling frequency from TMDS clock
			(void)_HDMI_M14C0_GetAudioFreqFromTmdsClock(&samplingFreqFromTmds);

			//M14 A0 IP Bug : non-PCM interrupt is not triggerred if non-PCM(AC-3) is changed to non-PCM(DTS).
			if ( (audioType			  != _gM14C0HdmiAudioInfo.audioType)
			  ||(samplingFreqFromTmds != _gM14C0HdmiAudioInfo.samplingFreq) )
			{
				HDMI_DEBUG("[%d]HDMI_GetAudioInfo : type = %d<-%d, tmds freq = %d<-%d, freq = %d, state = %d, mute = %d, PSC = %d(%d)\n", \
							__L__, audioType, _gM14C0HdmiAudioInfo.audioType, samplingFreqFromTmds, _gM14C0HdmiAudioInfo.samplingFreq,	\
							samplingFreq, _gM14C0IntrAudioState, _gM14C0AudioMuteState, _gM14C0HdmiPortStableCount, audioRecheckTime);

				//spin lock for protection for audio
				spin_lock_irqsave(&_gIntrHdmiM14C0AudioLock, flags);
				_gM14C0IntrAudioState = HDMI_AUDIO_INTERRUPT_STATE;
				_gM14C0HdmiFreqErrorCount = 0;
				spin_unlock_irqrestore(&_gIntrHdmiM14C0AudioLock, flags);
		...
		//Increase _gM14C0HdmiPortStableCount(If HDMI port is changed, this count is cleared.)
		spin_lock_irqsave(&_gIntrHdmiM14C0AudioLock, flags);
		_gM14C0HdmiPortStableCount++;
		spin_unlock_irqrestore(&_gIntrHdmiM14C0AudioLock, flags);
		...
		if ((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_10S) == 0)
		{
			HDMI_AUDIO("[%d]HDMI_GetAudioInfo : type = %d, freq = %d, state = %d, PSC = %d. mute = %d\n",	\
						__L__, _gM14C0HdmiAudioInfo.audioType, _gM14C0HdmiAudioInfo.samplingFreq,	\
						_gM14C0IntrAudioState, _gM14C0HdmiPortStableCount, _gM14C0AudioMuteState);
		}
	...
	else if (intrAudioState < HDMI_AUDIO_GET_INFO_STATE && intrVideoState >= HDMI_STATE_READ)
	{
		...
		//increase _gM14C0IntrAudioState
		_gM14C0IntrAudioState++;
	...
	if (reg_hdmi_mode)
	{
		...
		if (audioType == LX_HDMI_AUDIO_PCM)
		{
			   ...
			   &&(samplingFreqFromTmds != LX_HDMI_SAMPLING_FREQ_44_1KHZ) )
			{
				...
				if (_gM14C0HdmiFreqErrorCount < HDMI_AUDIO_FREQ_ERROR_TIME_500MS)
				{
					//Increase _gM14C0HdmiFreqErrorCount.
					spin_lock_irqsave(&_gIntrHdmiM14C0AudioLock, flags);
					_gM14C0HdmiFreqErrorCount++;
					spin_unlock_irqrestore(&_gIntrHdmiM14C0AudioLock, flags);

					//Set a no audio mode
					audioType = LX_HDMI_AUDIO_NO_AUDIO;

					if ((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0)
					{
		...
		if ((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0)
		{
			HDMI_AUDIO("[%d]GetAudioInfo : type = %d, freq = %d, mute = %d, PSC = %d\n", \
						__L__, audioType, samplingFreq, _gM14C0AudioMuteState, _gM14C0HdmiPortStableCount);
		}
	}
	else
	{
	...
	if ( (audioType	  == _gM14C0HdmiAudioInfo.audioType)
	  &&(samplingFreq == _gM14C0HdmiAudioInfo.samplingFreq)
	  /* &&(_gM14C0HdmiAudioInfo.audioType >= LX_HDMI_AUDIO_PCM) */
	  &&(intrVideoState >= HDMI_STATE_READ) )
	{
		//spin lock for protection : lock
		spin_lock_irqsave(&_gIntrHdmiM14C0AudioLock, flags);

		//Set a _gM14C0IntrAudioState
		_gM14C0IntrAudioState = HDMI_AUDIO_STABLE_STATE;
	...
	//For debug print
	if ( (audioType != _gM14C0HdmiAudioInfo.audioType) || (samplingFreq != _gM14C0HdmiAudioInfo.samplingFreq) )
	{
		if ( ((audioType == LX_HDMI_AUDIO_DVI) || (audioType == LX_HDMI_AUDIO_NO_AUDIO))	\
		  &&((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0) )
		{
			//Set a debug print status
			bDebugEnabled = TRUE;
		}
	...
	if (bDebugEnabled == TRUE)
	{
		...
		HDMI_AUDIO("[%d]GetAudioInfo : type = %d(%d), freq = %d, mute = %d, PSC = %d\n", \
					__L__, audioType, reg_burst_pc_0 & 0x1F, samplingFreq, _gM14C0AudioMuteState, _gM14C0HdmiPortStableCount);
	}
...
func_exit:
	if (audioType == LX_HDMI_AUDIO_NO_AUDIO)
	{
		...
		//Clear _gM14C0HdmiPortStableCount
		_gM14C0HdmiPortStableCount = 0;

		//Work-around code for M14 C0 chip clock bug in MHL mode.
		if (_gM14C0IntrAudioState == HDMI_AUDIO_CLOCK_SET_STATE)
		{
			LINK_M14_RdFL(audio_00);
			...
			//Check video vertical active size
			if (ui16VActive < 700)		//under 720P resolution
			{
				LINK_M14_Wr01(audio_00, reg_acr_n_fs,		 0x1);	//0 : 128Fs, 1 : 256 Fs, 2 : 512Fs(default : 256Fs)
				LINK_M14_Wr01(audio_00, reg_acr_clk_aud_div, 0x1);	//0 : 128Fs, 1 : 256 Fs, 2 : 512Fs(default : 256Fs)
			}
			else
			{
				...
				if( (_HDMI_M14C0_Check_MHL_PP_mode() > 0) && (_gM14C0HDMIPhyInform.prt_sel == 3) && (_gM14C0HDMIPhyInform.cd_sense == 1) )
				{
					LINK_M14_Wr01(audio_00, reg_acr_n_fs,		 0x2);	//0 : 128Fs, 1 : 256 Fs, 2 : 512Fs(default : 256Fs)
					LINK_M14_Wr01(audio_00, reg_acr_clk_aud_div, 0x0);	//0 : 128Fs, 1 : 256 Fs, 2 : 512Fs(default : 256Fs)
				}
				//For MHL 24bit Mode
				else if( (_HDMI_M14C0_Check_MHL_PP_mode() == 0) && (_gM14C0HDMIPhyInform.prt_sel == 3) && (_gM14C0HDMIPhyInform.cd_sense == 1) )
				{
					LINK_M14_Wr01(audio_00, reg_acr_n_fs,		 0x2);	//0 : 128Fs, 1 : 256 Fs, 2 : 512Fs(default : 256Fs)
					LINK_M14_Wr01(audio_00, reg_acr_clk_aud_div, 0x1);	//0 : 128Fs, 1 : 256 Fs, 2 : 512Fs(default : 256Fs)
				}
			...
			//For debug print
			HDMI_DEBUG("_M14C0_GetAudioInfo : pp_mode = %d, cd_sense = %d(acr_n_fs = %d, clk_aud_div = %d)\n",	\
						_HDMI_M14C0_Check_MHL_PP_mode(), _gM14C0HDMIPhyInform.cd_sense, LINK_M14_Rd_fld(audio_00, reg_acr_n_fs), LINK_M14_Rd_fld(audio_00, reg_acr_clk_aud_div));
		}
	...
	else
	{
		...
		_gM14C0HdmiAudioInfo.audioType	  = audioType;
		_gM14C0HdmiAudioInfo.samplingFreq = samplingFreq;
	}

	HDMI_ATASK("_M14C0_GetAudioInfo : type = %d, freq = %d(A = %d, V = %d)\n", audioType, samplingFreq, intrAudioState, intrVideoState);
	return RET_OK;
}
...
static int _HDMI_M14C0_GetAudioTypeAndFreq(LX_HDMI_AUDIO_TYPE_T *audioType, LX_HDMI_SAMPLING_FREQ_T *samplingFreq)
{
	...
	if (reg_hdmi_mode)
	{
		...
		if (reg_achst_byte0 & HDMI_AUDIO_SAMPLE_NON_PCM_MASK)	//bit 1, 0 : PCM, 1 : non-PCM
		{
			...
			if ((reg_burst_pc_0 & BURST_INFO_PAYLOAD_ERROR_BIT_MASK) == 0) //bit 7, 0 : No Error, 1 : Error
			{
				...
				switch(reg_burst_pc_0 & BURST_INFO_AUDIO_TYPE_BIT_MASK)	//bit 4 ~ 0
				{
					...
					case BURST_INFO_AUDIO_TYPE_NULL:
						*audioType = _gM14C0HdmiAudioInfo.audioType;
						if ((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0)
							HDMI_DEBUG("_HDMI_M14C0_GetAudioTypeAndFreq : BURST_INFO_AUDIO_TYPE_NULL(audioType= %d)\n", *audioType);
						break;
				...
				{
					if ((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0)
						HDMI_DEBUG("_HDMI_M14C0_GetAudioTypeAndFreq : BURST_INFO_AUDIO_TYPE_PAUSE(Pc = %d)!!!\n", reg_burst_pc_0);
				}
			...
			else
			{
				*audioType = LX_HDMI_AUDIO_PCM;
				...
				HDMI_ERROR("_HDMI_M14C0_GetAudioTypeAndFreq : Burst Info Error = %d\n", (reg_burst_pc_0 & BURST_INFO_PAYLOAD_ERROR_BIT_MASK));
			}
		...
		else
		{
			*audioType = LX_HDMI_AUDIO_PCM;
			...
			if (_gM14C0IntrBurstInfoCount != _gM14C0TaskBurstInfoCount)
			{
				//Update _gM14C0TaskBurstInfoCount value.
				_gM14C0TaskBurstInfoCount = _gM14C0IntrBurstInfoCount;
				...
				if (*audioType == LX_HDMI_AUDIO_AC3_IEC60958)
				{
					...
					if (_gM14C0IntrAudioState == HDMI_AUDIO_GET_INFO_STATE)
					{
						HDMI_DEBUG("_HDMI_M14C0_GetAudioTypeAndFreq : Forced AC-3.(byte0 = 0x%X, Pc = 0x%X)\n", reg_achst_byte0, reg_burst_pc_0);
					}
				}
			}
			else
		...
		switch(reg_achst_byte3 & HDMI_AUDIO_SAMPLE_BIT_MASK) 	//bit 0 ~ 3
		{
			...
			default:
				*samplingFreq = LX_HDMI_SAMPLING_FREQ_NONE;
				...
				HDMI_AUDIO("_HDMI_M14C0_GetAudioTypeAndFreq(Channel Status) : samplingFreq = %d\n", *samplingFreq);
				break;
		}
	}
	else
	...
	HDMI_ATASK("_M14C0_GetAudioTypeAndFreq : type = %d, freq = %d\n", *audioType, *samplingFreq);
	return ret;
}

/**
 *  _HDMI_M14C0_GetAudioFreqFromTmdsClock
 *
 *  @return int
*/
static int _HDMI_M14C0_GetAudioFreqFromTmdsClock(LX_HDMI_SAMPLING_FREQ_T *samplingFreqFromTmds)
{
	...
	if( ( _HDMI_M14C0_Check_MHL_PP_mode() > 0 ) && ( _gM14C0HDMIPhyInform.prt_sel == 3) && (_gM14C0HDMIPhyInform.cd_sense == 1) )
	{
		//Adjust ACR_CTS value in MHL PP mode.
		ui64AcrCts = ui64AcrCts >> 1;
	}
	...
	if(ui64TmdsClock)
	{
		HDMI_AUDIO("Get Audio Clock : TMDS = %10llu, N = %llu, CTS = %llu, Freq = %llu\n", \
					ui64TmdsClock, ui64AcrN, ui64AcrCts, ui64TmdsSamplingFreq);
	}

	//Check a HDMI Audio Debug Enabled Status.
	if(_gM14C0AudioDebugState == TRUE)
	{
		//Get video vertical & horizontal active size
		LINK_M14_RdFL(video_05);
		LINK_M14_Rd01(video_05, reg_h_av, ui16HActive);
		...
		HDMI_ERROR("%10llu N%llu C%llu F%llu H%d V%d\n", \
					ui64TmdsClock, ui64AcrN, ui64AcrCts, ui64TmdsSamplingFreq,	\
...
static int _HDMI_M14C0_DebugAudioInfo(LX_HDMI_DEBUG_AUDIO_INFO_T *pAudioDebugInfo)
{
	...
	HDMI_DEBUG("Get Audio Clock : TMDS = %10llu, N = %llu, CTS = %llu, Freq = %llu\n", \
				ui64TmdsClock, ui64AcrN, ui64AcrCts, ui64TmdsSamplingFreq);
...
static int _HDMI_M14C0_Thread(void *data)
{
	...
	//*  2012-06-15 : HDMI Initail ÁÖ?Ç »çÇ×
	//*  Ctop reset ?ÌÈÄ phy pdb & restn µ¿?Û ÇØ¾ß ÇÔ(Ctop ¿¡¼­ phy pdb¸¸ ÇÏ´Â °Í °°?½.
	//*  After phy pdb, phy register initi value °¡ ¿?¿°µÊ -> pdb ÈÄ¿¡´Â rstn resetÇØÁÖ¾î¾ß ÇÔ.
	...
	_HDMI_M14C0_ClearSWResetAll();			//M14C0D Chip - Ctop control

	OS_MsecSleep(10);	// ms delay

#ifdef ODT_PDB_OFF_ON_WORKAROUND

	_HDMI_M14C0_Get_HDMI5V_Info_A4P(&hdmi5v[0], &hdmi5v[1] ,&hdmi5v[2], &hdmi5v[3]);

	for (port_num = 0; port_num < 4;port_num ++)
	{
		if(hdmi5v[port_num])
		{
			_HDMI_M14C0_Phy_Enable_Register_Access(port_num);
			_HDMI_M14C0_EQ_PDB_control(1);
	...
	for (port_num = 0; port_num < 4;port_num ++)
	{
		if(hdmi5v[port_num])
		{
			_HDMI_M14C0_Set_HPD_Out(port_num, 1);	// enable HPD
		}
	}
	...
	if( hdmi5v[0] || hdmi5v[1] || hdmi5v[2] || hdmi5v[3] )
	{
		...
		for (port_num = 0; port_num < 4;port_num ++)
		{
			if(hdmi5v[port_num])
			{
				_HDMI_M14C0_Set_HPD_Out(port_num, 0);	// disable HPD
			}
		}
		_g_ODT_PDB_Off_Time = jiffies_to_msecs(jiffies);
	}
	...
	{
		_g_ODT_PDB_On_Time = jiffies_to_msecs(jiffies);
		if ( ( (_g_ODT_PDB_On_Time - _g_ODT_PDB_Off_Time) > 600 ) && (_gM14C0HDMIPhyInform.hpd_enable == 1) )
			break;
	...
	for (port_num = 0; port_num < 4;port_num ++)
	{
		...
		{
			_HDMI_M14C0_Phy_Enable_Register_Access(port_num);
			_HDMI_M14C0_EQ_PDB_control(0);
		}
	}
#ifdef HPD_OFF_WORKAROUND_FOR_EXT_EDID
	/* HDMI port2 connected and using External EDID */
	if ( ( _gM14C0HDMIPhyInform.hpd_pol[1] == 0 ) && ( hdmi5v[1] == 1) )
	{
	...
	// Force HDMI mode when MHL Cable Connected (to restart C-BUS wakeup sequence)
	// and When MHL disabled Mode
	if ( (cd_sense) || (_g_mhl_enable_flag == 0) )
		_HDMI_M14C0_Set_ManMHLMode(1, 0);

	//_HDMI_M14C0_Set_ManMHLMode(1, 0);
	_HDMI_M14C0_ResetPortControl(1);	// reset phy_link

	/* PHY_PDB=1, PHY_RSTN=1 ?» µ¿½Ã¿¡ ?Î°¡ registerµé?Ì Á¦´ë·Î pre-setµÇÁö ¾Ê´Â Çö»ó ¹ß»ýÇÔ. */
	/* PHY_PDB=1  -> 10us  -> PHY_RSTN=1  */
	OS_MsecSleep(2);	// ms delay

	//_HDMI_M14C0_PhyRunReset();	// init Phy register
	_HDMI_M14C0_RunReset();		// init Link register

	//audio setting
	_HDMI_M14C0_SetAudio();

	_HDMI_M14C0_ResetPortControl(0);	// Release reset phy_link

	_HDMI_M14C0_EnableInterrupt();

#if 0
	/* GPIO Pin 10 : Value High => ODT 55Ohm, Low => ODT 46Ohm(default) */
	if( GPIO_DevGetValue(_g_gpio_pin_number, (LX_GPIO_VALUE_T*)&_g_gpio_pin_value) < 0) {
		HDMI_ERROR("[%d] %s : ERROR Reading GPIO pin\n",__LINE__, __func__);
		_g_gpio_pin_value = -1;
	}
	else
		HDMI_NOTI("MHL ODT TOOL Option : pin[%d] value[%d]\n", _g_gpio_pin_number, _g_gpio_pin_value );
#endif
	/*
	_HDMI_M14C0_EnablePort(3, 1);	// Enable MHL Port
	...
	*/
	...
	while(1)
	{
		...
		_HDMI_M14C0_GetMHLConection();	//check MHL Connection
		if (gHDMI_Phy_Control.check_an_data)
			_HDMI_M14C0_SetPortConnection();	// port change after AN data valid
		_HDMI_M14C0_GetPortConnection();	//check connection
		_HDMI_M14C0_Periodic_Task();	//Video Task
		_HDMI_M14C0_GetAudioInfo();	//Audio Task
		_HDMI_M14C0_HDCP_Check_Status();	// check HDCP status

		OS_UnlockMutex(&g_HDMI_Sema);

		if ( _gM14C0HDMIPhyInform.hdmi5v[0] || _gM14C0HDMIPhyInform.hdmi5v[1] || _gM14C0HDMIPhyInform.hdmi5v[2] || _gM14C0HDMIPhyInform.hdmi5v[3])
		...
		if (_gM14C0Force_thread_sleep > 1)
		{
			HDMI_NOTI("Force sleep HDMI Thread\n");
			//_gM14C0Force_thread_sleep  = 1;
			_gM14C0HDMI_thread_running = 0;
			//interruptible_sleep_on(&WaitQueue_HDMI_M14C0);
			wait_event_interruptible(WaitQueue_HDMI_M14C0 , _gM14C0Force_thread_sleep == 0);
		}
		...
		if (_gHdmi_no_connection_count > 10)
		{
			...
			/*
			if ( _gM14C0HDMIState == HDMI_STATE_DISABLE )
			{
				interruptible_sleep_on(&WaitQueue_HDMI_M14C0);
			}
			else
			*/
			...
			if( wait_event_interruptible(WaitQueue_HDMI_M14C0 , _gM14C0HDMIState > HDMI_STATE_DISABLE ) < 0)
			{
...
static int _HDMI_M14C0_EnableInterrupt(void)
{
...
#endif
	LINK_M14_Wr01(interrupt_enable_00, intr_aud_type_chg_enable, 0x0);		///< 19 intr_aud_type_chg_enable
...
#ifndef G_PHONE_MHL_DEBUG
	LINK_M14_Wr01(interrupt_enable_04, intr_cbus_new_devcap_enable      , 0x0);		///< 7 intr_cbus_new_devcap_enable
#else
	LINK_M14_Wr01(interrupt_enable_04, intr_cbus_new_devcap_enable      , 0x1);		///< 7 intr_cbus_new_devcap_enable
#endif
	...
	if (_gM14C0HDMI_interrupt_registered < 1)
	{
		if (request_irq(M14_B0_IRQ_HDMI_LINK, _HDMI_M14C0_IRQHandler, 0, "hdmi", NULL))
		{
			HDMI_ERROR("request_irq is failed\n");
		}
		else
			_gM14C0HDMI_interrupt_registered = 1;
	}

	HDMI_NOTI("HDMI_M14C0_EnableInterrupt\n");
	return RET_OK;
}
...
int HDMI_M14C0_DisableInterrupt(void)
{
	...
	HDMI_NOTI("HDMI_M14C0_DisableInterrupt\n");
...
irqreturn_t _HDMI_M14C0_IRQHandler(int irq, void *dev)
{
	...
	if(_gM14C0AudioDebugState == TRUE)
	{
		if(LINK_M14_Rd_fld( interrupt_00, intr_afifo_undr))
			intr_afifo_undr = 1;
		else
			intr_afifo_undr = 0;

		if(LINK_M14_Rd_fld( interrupt_00, intr_afifo_ovrr))
			intr_afifo_ovrr = 1;
		else
			intr_afifo_ovrr = 0;

		if(LINK_M14_Rd_fld( interrupt_00, intr_spdif_err))
			intr_spdif_err = 1;
		else
			intr_spdif_err = 0;

		if(LINK_M14_Rd_fld( interrupt_00, intr_aud_type_chg))
			intr_aud_type_chg = 1;
		else
			intr_aud_type_chg = 0;

		if(LINK_M14_Rd_fld( interrupt_00, intr_cts_chg))
		{
			_gM14C0CtsInterruptCount++;

			if ((_gM14C0CtsInterruptCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_1S) == 0)
				intr_cts_chg = 1;
		}
		else
		{
		...
		if(LINK_M14_Rd_fld( interrupt_00, intr_n_chg))
			intr_n_chg = 1;
		else
			intr_n_chg = 0;
		...
		if(LINK_M14_Rd_fld( interrupt_01, intr_terc4_err_prt0))
			intr_terc4_err = 1;
		else
			intr_terc4_err = 0;
		...
		{
			HDMI_ERROR("un%d ov%d sp%d ty%d ct%d n%d fs%d 2p%d 2n%d ec%d te%d\n", \
						intr_afifo_undr, intr_afifo_ovrr, intr_spdif_err, intr_aud_type_chg, intr_cts_chg,	\
						intr_n_chg, intr_fs_chg, intr_2pcm_chg, intr_2npcm_chg, intr_ecc_err, intr_terc4_err);
		}
	}
	...
	if ( LINK_M14_Rd_fld(interrupt_01, intr_ecc_err) )
	{
		...
		if( gHDMI_Phy_Control.link_pkt_error_workaround == 1) {
			if (_gM14C0_TMDS_ERROR_EQ != -1)
				_gM14C0_TMDS_ERROR_EQ ++;
			if (_gM14C0_TMDS_ERROR_EQ_2nd != -1)
				_gM14C0_TMDS_ERROR_EQ_2nd ++;

			_gM14C0_TMDS_ERROR_intr_count ++;
		}

		HDMI_INTR("intr_ecc_err \n");
	}
	if ( LINK_M14_Rd_fld(interrupt_01, intr_hdcp_err) )
	{
		_HDMI_M14C0_Disable_HDCP_Error_Interrupt();

		if( gHDMI_Phy_Control.link_pkt_error_workaround == 1) {
			_gM14C0_HDCP_ERROR_intr_count ++;
		}
	...
	if ( LINK_M14_Rd_fld(interrupt_01, intr_terc4_err_prt0) )
	{
		_HDMI_M14C0_Disable_TERC4_Error_Interrupt(0);

		if( gHDMI_Phy_Control.link_pkt_error_workaround == 1) {
			_gM14C0_TERC4_ERROR_intr_count[0] ++;
		}

		HDMI_INTR("intr_terc4_err[0] \n");
	}
	if ( LINK_M14_Rd_fld(interrupt_01, intr_terc4_err_prt1) )
	{
		_HDMI_M14C0_Disable_TERC4_Error_Interrupt(1);

		if( gHDMI_Phy_Control.link_pkt_error_workaround == 1) {
			_gM14C0_TERC4_ERROR_intr_count[1] ++;
		}

		HDMI_INTR("intr_terc4_err[1] \n");
	}
	if ( LINK_M14_Rd_fld(interrupt_01, intr_terc4_err_prt2) )
	{
		_HDMI_M14C0_Disable_TERC4_Error_Interrupt(2);

		if( gHDMI_Phy_Control.link_pkt_error_workaround == 1) {
			_gM14C0_TERC4_ERROR_intr_count[2] ++;
		}

		HDMI_INTR("intr_terc4_err[2] \n");
	}
	if ( LINK_M14_Rd_fld(interrupt_01, intr_terc4_err_prt3) )
	{
		_HDMI_M14C0_Disable_TERC4_Error_Interrupt(3);

		if( gHDMI_Phy_Control.link_pkt_error_workaround == 1) {
			_gM14C0_TERC4_ERROR_intr_count[3] ++;
		}

		HDMI_INTR("intr_terc4_err[3] \n");
	}
	if (LINK_M14_Rd_fld(interrupt_01, intr_scdt_fedge_prt3))
	{
		if ( _gM14C0HDMI_thread_running > 0)
		{
#ifdef M14C0_BRINGUP_TODO
			_HDMI_M14C0_TMDS_ResetPort_Control(3 ,0);
#endif
	...
	if (LINK_M14_Rd_fld(interrupt_01, intr_scdt_fedge_prt2))
	{
		...
		{
#ifdef M14C0_BRINGUP_TODO
			_HDMI_M14C0_TMDS_ResetPort_Control(2 ,0);
#endif
	...
	if (LINK_M14_Rd_fld(interrupt_01, intr_scdt_fedge_prt1))
	{
		...
		{
#ifdef M14C0_BRINGUP_TODO
			_HDMI_M14C0_TMDS_ResetPort_Control(1 ,0);
#endif
	...
	if (LINK_M14_Rd_fld(interrupt_01, intr_scdt_fedge_prt0))
	{
		...
		{
#ifdef M14C0_BRINGUP_TODO
			_HDMI_M14C0_TMDS_ResetPort_Control(0 ,0);
#endif
	...
	if (LINK_M14_Rd_fld(interrupt_04, intr_cbus_disconn))
	{
		...
		_HDMI_M14C0_MHL_Check_Status(1);

#ifdef M14_CBUS_PDB_CTRL
		_MHL_PDB_Thread_wakeup = 1;
		wake_up_interruptible(&WaitQueue_MHL_PDB_M14C0);
#endif
...
#ifdef G_PHONE_MHL_DEBUG
	if (LINK_M14_Rd_fld(interrupt_04, intr_cbus_new_devcap))
	{
		LINK_M14_RdFL(cbus_04);
		...
		HDMI_NOTI("---- MHL New DEVCAP => SET HPD \n");
	}
#endif
	...
	if (LINK_M14_Rd_fld(interrupt_00, intr_vf_stable))
	{
		...
		spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
		_gM14C0Intr_vf_stable = HDMI_VIDEO_INTERRUPT_STATE;
		spin_unlock_irqrestore(&_gIntrHdmiM14C0VideoLock, flags);
		...
		if (_gM14C0HDMIState >= HDMI_STATE_READ)
		{
			HDMI_TASK("[%d] %s : -Changed -  _gM14C0Intr_vf_stable [STABLE_STATE] => [READ_STATE]     /   HDMI_STABLE[%d] / PSC[%d]\n", __L__, __F__, _gM14C0HDMIState, _gM14C0HdmiPortStableCount );
			spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
			_gM14C0Intr_vf_stable = HDMI_VIDEO_READ_STATE;
			//_gM14C0HDMIState = HDMI_STATE_READ;
			_gM14C0TimingReadCnt = 0;
			_gM14C0Intr_avi = HDMI_AVI_INTERRUPT_STATE;
			_gM14C0AVIReadState = FALSE;
			spin_unlock_irqrestore(&_gIntrHdmiM14C0VideoLock, flags);

			//intr_vf_stable is abnormally toggled from UP player source in UD(UHD) Model for abnormal chip only.(2013-06-25)
			//Check a HDMI Port Stable Count
			if ( (CHK_UHD_BD() && _gM14C0HdmiPortStableCount > HDMI_AUDIO_RECHECK_TIME_500MS)	\
			   ||(CHK_FHD_BD() && _gM14C0HdmiPortStableCount > HDMI_AUDIO_PORT_STABLE_TIME_5S) 	\
			   ||(lx_chip_rev() >= LX_CHIP_REV(M14, C0) && _gM14C0HdmiPortStableCount > HDMI_AUDIO_RECHECK_TIME_500MS) )
			{
				//spin lock for protection for audio
				spin_lock_irqsave(&_gIntrHdmiM14C0AudioLock, flags);
				_gM14C0IntrAudioState = HDMI_AUDIO_STABLE_STATE;
	...
	if (LINK_M14_Rd_fld(interrupt_01, intr_chg_avi_b12345))
	{
		...
		_gM14C0Intr_avi = HDMI_AVI_INTERRUPT_STATE;
		_gM14C0AVIReadState = FALSE;
		if (_gM14C0CHG_AVI_count_MHL != -1)
			_gM14C0CHG_AVI_count_MHL ++;
		if (_gM14C0CHG_AVI_count_EQ != -1)
			_gM14C0CHG_AVI_count_EQ ++;
		spin_unlock_irqrestore(&_gIntrHdmiM14C0VideoLock, flags);
		...
		HDMI_TASK("[%d][HDMI_AVI] state [%d]  preCSS[%d] CSC = %d \n", __L__, _gM14C0HDMIState, _gM14C0PrevPixelEncoding, csc);

		if (_gM14C0HDMIState >= HDMI_STATE_READ)//(_gM14C0Intr_vf_stable == HDMI_VIDEO_STABLE_STATE) 			// for PS3 CSC
		{
			if (_gM14C0PrevPixelEncoding != csc)
			{
				//spin lock for protection
				spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
				_gM14C0Intr_avi = HDMI_AVI_CHANGE_STATE;
				_gM14C0TimingReadCnt = 0;
				_gM14C0AVIReadState = FALSE;
				_gM14C0AVIChangeState = TRUE;
				spin_unlock_irqrestore(&_gIntrHdmiM14C0VideoLock, flags);

				HDMI_TASK("[%d] %s : -Changed  - _gM14C0Intr_avi [STABLE_STATE] => [CHANGE_STATE]     /   HDMI_STABLE[%d] \n", __L__, __F__,_gM14C0HDMIState );
	...
	if (LINK_M14_Rd_fld(interrupt_02, intr_set_mute))
	{
		...
		//if (_gM14C0Intr_vf_stable > HDMI_VIDEO_INIT_STATE)
		{
			if (_gM14C0Intr_src_mute != HDMI_SOURCE_MUTE_STATE)
			{
				muteCtrl.eMode		= LX_HDMI_VIDEO_MUTE;
				muteCtrl.bVideoMute = TRUE;
				_HDMI_M14C0_SetInternalMute(muteCtrl);
				...
				spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
				_gM14C0Intr_src_mute = HDMI_SOURCE_MUTE_STATE;
				_gM14C0TimingReadCnt = 0;
				spin_unlock_irqrestore(&_gIntrHdmiM14C0VideoLock, flags);

				HDMI_DEBUG("[%d] intr_set_mute : HDMI_SOURCE MUTE Enable[%d] \n", __L__, _gM14C0Intr_src_mute );
			}
		}
	}
	...
	if (LINK_M14_Rd_fld(interrupt_01, intr_no_vsi))
	{
		...
		HDMI_TASK("[%d] %s : -Changed  - _gM14C0Intr_vsi [STABLE_STATE] => [NO_DATA_STAT]     /   HDMI_STABLE[%d] \n", __L__, __F__,_gM14C0HDMIState );
		HDMI_TASK("[%d] %s : -Changed  - _gM14C0Intr_vf_stable [STABLE_STATE] => [READ_STATE]     /   HDMI_STABLE[%d] \n", __L__, __F__,_gM14C0HDMIState );
		spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
		_gM14C0Intr_vsi = HDMI_VSI_NO_DATA_STATE;
		_gM14C0Intr_vf_stable = HDMI_VIDEO_READ_STATE;
		_gM14C0TimingReadCnt = 0;
		_gM14C0Intr_avi = HDMI_AVI_INTERRUPT_STATE;
		_gM14C0AVIReadState = FALSE;
	...
	if (LINK_M14_Rd_fld(interrupt_00, intr_chg_vsi_vformat))
	{
		...
		spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
		_gM14C0Intr_vsi = HDMI_VSI_INTERRUPT_STATE;
		_gM14C0VSIState = TRUE;
...
//audio related. => start
	...
	if ( (CHK_FHD_BD() && lx_chip_rev() < LX_CHIP_REV(M14, C0)) && (intra0 & HDMI_AUDIO_INTERRUPT_BIT_MASK))
	{
		//spin lock for protection for audio
		spin_lock_irqsave(&_gIntrHdmiM14C0AudioLock, flags);
		_gM14C0IntrAudioState = HDMI_AUDIO_INTERRUPT_STATE;
	...
	//intr_fs_chg is abnormally toggled from UD(UHD) player source in UD(UHD) Model.
	if ( (CHK_UHD_BD() || lx_chip_rev() >= LX_CHIP_REV(M14, C0)) && (intra0 & HDMI_AUDIO_INTERRUPT_BIT_MASK) )
	{
		if (LINK_M14_Rd_fld(interrupt_00, intr_fs_chg))
			HDMI_INTR("intr_fs_chg\n");
	...
	if (LINK_M14_Rd_fld(interrupt_00, intr_burst_info))
	{
		...
		elapsedTime = jiffies_to_msecs(jiffies - _gM14C0BurstInfoPrevJiffies);

		//Save _gM14C0BurstInfoPrevJiffies
		_gM14C0BurstInfoPrevJiffies = jiffies;

		if (_gM14C0IntrBurstInfoCount == 0 && elapsedTime > HDMI_AUDIO_BURST_INFO_RECHECK_80MS)
		{
			//spin lock for protection for audio
			spin_lock_irqsave(&_gIntrHdmiM14C0AudioLock, flags);
			_gM14C0IntrAudioState = HDMI_AUDIO_INTERRUPT_STATE;
			...
			if (_gM14C0HdmiPortStableCount > HDMI_AUDIO_PORT_STABLE_TIME_5S)
			{
				//Check a vaild Pc data for payload
				LINK_M14_RdFL(audio_09);
				HDMI_DEBUG("intr_burst_info(%llu), Pd_Pc = 0x%X, elapsedTime = %llu\n",	\
							_gM14C0IntrBurstInfoCount, LINK_M14_Rd(audio_09), elapsedTime);
			}
		}

		//Increase _gM14C0IntrBurstInfoCount
		_gM14C0IntrBurstInfoCount++;

		HDMI_INTR("intr_burst_info(%llu)\n", _gM14C0IntrBurstInfoCount);

		//if ((_gM14C0HdmiAudioThreadCount % DEBUG_HDMI_AUDIO_MSG_PRINT_TIME_100S) == 0)
			//HDMI_DEBUG("intr_burst_info(%llu), elapsedTime(%llu)\n", _gM14C0IntrBurstInfoCount, elapsedTime);
	}
	else
	{
		//_gM14C0IntrBurstInfoCount is normal toggled in FHD Model.
		if (CHK_FHD_BD() && lx_chip_rev() < LX_CHIP_REV(M14, C0))
		{
			//Reset _gM14C0IntrBurstInfoCount
			_gM14C0IntrBurstInfoCount = 0;
		}
		else
		{
			//Check a vaild Pc data for payload
			LINK_M14_RdFL(audio_09);
			HDMI_INTR("_gM14C0IntrBurstInfoCount(%llu), Pd_Pc = 0x%X\n", _gM14C0IntrBurstInfoCount, LINK_M14_Rd(audio_09));
		}
	}
//audio related. => end.
...
static void _HDMI_M14C0_Periodic_Task(void)
{
	...
	do {
		if (_gM14C0HDMIState == HDMI_STATE_IDLE && _gM14C0HDMIState == HDMI_STATE_DISABLE)
			gPeriodicState = _gM14C0HDMIState;

		if (_gM14C0HDMIConnectState == HDMI_PORT_NOT_CONNECTED)			// change check connection in the thread function by 20121206
		{
		...
		{
			if (_gM14C0HDMIState <= HDMI_STATE_NO_SCDT)
			{
			...
			//if (_gM14C0Intr_vf_stable == HDMI_VIDEO_READ_STATE)
			if ( (_gM14C0Intr_vf_stable == HDMI_VIDEO_READ_STATE) \
					&& ( ( _gM14C0HDMIPhyInform.hdcp_authed[_gM14C0HDMIPhyInform.prt_sel] \
							&& _HDMI_M14C0_Check_Aksv_Exist(_gM14C0HDMIPhyInform.prt_sel) \
							&& (hdcp_check_count == HDMI_CHEAK_COUNT) )
						|| (_HDMI_M14C0_Check_Aksv_Exist(_gM14C0HDMIPhyInform.prt_sel) == 0) )
			  )
			{
				HDMI_TASK("[%d] %s : -Changed- HDMI_STATE_[%d]  => HDMI_STATE_READ && _gM14C0Intr_vf_stable[READ_STATE => INTERRUPT_STATE] \n", __L__, __F__, gPeriodicState);
			...
			{
				if ( ( memcmp(_gM14C0HDMIPhyInform.hdcp_authed, hdcp_authed_info, sizeof(hdcp_authed_info)) ) \
						|| ( memcmp(_gM14C0HDMIPhyInform.hdcp_enc_en, hdcp_enc_en_info, sizeof(hdcp_authed_info)) ) )
				{
					HDMI_TASK("HDMI HDCP Information Changed !!!\n");

					for (temp = 0; temp < 4;temp++)
						HDMI_TASK("Port[%d] : HDCP_ENC_EN[%d]=>[%d], HDCP_AUTHED[%d]=>[%d]\n", temp, hdcp_enc_en_info[temp], _gM14C0HDMIPhyInform.hdcp_enc_en[temp], \
								hdcp_authed_info[temp], _gM14C0HDMIPhyInform.hdcp_authed[temp]);

					memcpy(hdcp_authed_info, _gM14C0HDMIPhyInform.hdcp_authed, sizeof(hdcp_authed_info));
					memcpy(hdcp_enc_en_info, _gM14C0HDMIPhyInform.hdcp_enc_en, sizeof(hdcp_authed_info));
				}
		...
		switch(gPeriodicState)
		{
			...
			case HDMI_STATE_SCDT:		// if (HDMI_SCDT_COUNT == 4)  5 call
				{
					hdcp_check_count = HDMI_CHEAK_COUNT;
									// x2 for M14C0 (no external switch)
					if (gScdtCnt < (HDMI_SCDT_COUNT<<1) )	gScdtCnt++;
					else
					{
						gScdtCnt++;
						if (_gM14C0Intr_vf_stable == HDMI_VIDEO_INTERRUPT_STATE)
						{
							gPeriodicState = HDMI_STATE_CHECK_MODE;
							_HDMI_Check_State_StartTime = jiffies_to_msecs(jiffies);
						...
						if (gScdtCnt > 50)			// when no vf_stable Interrupt
						{
							...
							spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
							_gM14C0Intr_vf_stable = HDMI_VIDEO_INTERRUPT_STATE;
							if (_gM14C0Intr_avi == HDMI_AVI_INIT_STATE)		_gM14C0Intr_avi = HDMI_AVI_INTERRUPT_STATE;
							if (_gM14C0Intr_vsi == HDMI_VSI_INIT_STATE)		_gM14C0Intr_vsi = HDMI_VSI_INTERRUPT_STATE;
							spin_unlock_irqrestore(&_gIntrHdmiM14C0VideoLock, flags);
			...
			case HDMI_STATE_CHECK_AVI_NO:
				{
					...
					if (gM14BootData->mode == 1)
					{
						...
						if ( !_gM14C0HDMIPhyInform.hdcp_authed[_gM14C0HDMIPhyInform.prt_sel]	\
								&& (_HDMI_M14C0_Check_Aksv_Exist(_gM14C0HDMIPhyInform.prt_sel) != 0) )
						{
							hdcp_check_count = 100; //maximum 2sec
							HDMI_TASK("[%d] %s : HDCP Auth : HDMI_STATE [HDMI_STATE_CHECK_SOURCE_MUTE(%d)] \n", __L__, __F__, gPeriodicState);
						}
						else if ( (hdcp_check_count == 100) && _gM14C0HDMIPhyInform.hdcp_authed[_gM14C0HDMIPhyInform.prt_sel]	\
								&& (_HDMI_M14C0_Check_Aksv_Exist(_gM14C0HDMIPhyInform.prt_sel) != 0) )
						{
						...
						/*
						else if ( _(hdcp_check_count == HDMI_CHEAK_COUNT) && gM14C0HDMIPhyInform.hdcp_authed[_gM14C0HDMIPhyInform.prt_sel]	\
								&& (_HDMI_M14C0_Check_Aksv_Exist(_gM14C0HDMIPhyInform.prt_sel) != 0) )
						{
						...
						*/
						else if ( _HDMI_M14C0_Check_Aksv_Exist(_gM14C0HDMIPhyInform.prt_sel) == 0)
						{
							hdcp_check_count = HDMI_CHEAK_COUNT;	// No AKSV Received : HDCP not encoded?
							HDMI_TASK("HDMI_STATE [%d] : HDCP no encoded ??? \n", gPeriodicState);
						}
																					// x2 for M14C0 (no external switch)
						if (_gM14C0Intr_avi <= HDMI_AVI_INTERRUPT_STATE &&  gAviCnt < (50<<1) )		// No read AVI
						{
							LINK_M14_RdFL(packet_18);
			...
			case HDMI_STATE_READ:
				{
					...
					else if (_gM14C0TimingReadCnt > HDMI_READ_COUNT)
					{
...
#ifdef M14_HUMAX_SETTOP_HDCP_WORKAROUND
						/* HUMAX IR1020HD : snow noise when resolution change */
						if ( !((_gM14C0HDMIPhyInform.prt_sel == 3) && (_gM14C0HDMIPhyInform.cd_sense) ) )
							_HDMI_M14C0_Set_HDCP_Unauth(_gM14C0HDMIPhyInform.prt_sel, 0, 0);
#endif
					}

					HDMI_TASK("[%d] %s : HDMI State = [%d]  _gM14C0TimingReadCnt = %d \n", __L__, __F__, gPeriodicState, _gM14C0TimingReadCnt);
				}
				break;
			...
			case HDMI_STATE_STABLE:
				{
					spin_lock_irqsave(&_gIntrHdmiM14C0VideoLock, flags);
					_gM14C0Intr_vf_stable = HDMI_VIDEO_STABLE_STATE;
					spin_unlock_irqrestore(&_gIntrHdmiM14C0VideoLock, flags);
				}
				break;
	...
	_gM14C0HDMIState = gPeriodicState;

	if (_gM14C0HDMIState < HDMI_STATE_STABLE)
...
static int _HDMI_M14C0_EQ_PDB_control(int reset)
{
	...
	else
	{
		PHY_REG_M14C0_RdFL(eq_i2c_pdb_d0);
		PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_i2c_pdb_d0_mode,0x0);
		PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_man_pdb_d0,0x1);
		PHY_REG_M14C0_WrFL(eq_i2c_pdb_d0);

		PHY_REG_M14C0_RdFL(eq_i2c_pdb_dck);
		PHY_REG_M14C0_Wr01(eq_i2c_pdb_dck,eq_i2c_pdb_dck_mode,0x0);
		PHY_REG_M14C0_Wr01(eq_i2c_pdb_dck,eq_man_pdb_dck,0x1);
		PHY_REG_M14C0_WrFL(eq_i2c_pdb_dck);
	}
	return ret;
}
...
static int _HDMI_M14C0_ResetPortControl(int reset_enable)
{
	...
	if (reset_enable)
	{
		...
		if(_gM14C0HDMIPhyInform.hpd_pol[1])
...
*/
	...
	_HDMI_M14C0_HDCP_ResetPort_Control(0,0);
	_HDMI_M14C0_HDCP_ResetPort_Control(1,0);
	_HDMI_M14C0_HDCP_ResetPort_Control(2,0);
	_HDMI_M14C0_HDCP_ResetPort_Control(3,0);

	OS_MsecSleep(5);	// ms delay

	_HDMI_M14C0_HDCP_ResetPort_Control(0,1);
	_HDMI_M14C0_HDCP_ResetPort_Control(1,1);
	_HDMI_M14C0_HDCP_ResetPort_Control(2,1);
	_HDMI_M14C0_HDCP_ResetPort_Control(3,1);
...
int HDMI_M14C0_Get_Phy_Status(LX_HDMI_PHY_INFORM_T *sp_hdmi_phy_status)
{
	...
	if (!_gM14C0HDMI_thread_running)
	{
		...
		LINK_M14_RdFL(system_control_00);
		LINK_M14_Rd01(system_control_00, reg_cd_sense_prt3, _gM14C0HDMIPhyInform.cd_sense);
		LINK_M14_RdFL(cbus_01);
		LINK_M14_Rd01(cbus_01, reg_cbus_conn_done, _gM14C0HDMIPhyInform.cbus_conn_done);
		LINK_M14_Rd01(cbus_01, reg_cbus_warb_done, _gM14C0HDMIPhyInform.cbus_warb_done);
		LINK_M14_Rd01(cbus_01, reg_cbus_disconn, _gM14C0HDMIPhyInform.cbus_disconn);			// CBUS DisConnected ???
		LINK_M14_RdFL(cbus_00);
		LINK_M14_Rd01(cbus_00, reg_phy_sink_cbus_zdis, _gM14C0HDMIPhyInform.phy_sink_cbus_zdis);
		LINK_M14_Rd01(cbus_00, reg_cbus_st, _gM14C0HDMIPhyInform.cbus_st);

		for (port_num=0;port_num<4;port_num++)
			access_phy[port_num] = _gM14C0HDMIPhyInform.phy_pdb[port_num] && _gM14C0HDMIPhyInform.phy_enable[port_num] && _gM14C0HDMIPhyInform.phy_rstn[port_num] ;
		...
		LINK_M14_Rd01(cbus_13, reg_state_sink_rcp, _gM14C0HDMIPhyInform.state_sink_rcp);
	}

	LINK_M14_RdFL(phy_link_00);
	LINK_M14_Rd01(phy_link_00, link_sw_rstn_edid_prt0, _gM14C0HDMIPhyInform.rstn_edid[0]);
	LINK_M14_RdFL(phy_link_02);
	LINK_M14_Rd01(phy_link_02, link_sw_rstn_edid_prt1, _gM14C0HDMIPhyInform.rstn_edid[1]);
	LINK_M14_RdFL(phy_link_04);
	LINK_M14_Rd01(phy_link_04, link_sw_rstn_edid_prt2, _gM14C0HDMIPhyInform.rstn_edid[2]);
	LINK_M14_RdFL(phy_link_06);
	LINK_M14_Rd01(phy_link_06, link_sw_rstn_edid_prt3, _gM14C0HDMIPhyInform.rstn_edid[3]);

	_gM14C0HDMIPhyInform.mhl_pp_mode =  _HDMI_M14C0_Check_MHL_PP_mode();
	_gM14C0HDMIPhyInform.mhl_mode_enable =  _g_mhl_enable_flag;

	memcpy(sp_hdmi_phy_status, &_gM14C0HDMIPhyInform, sizeof(LX_HDMI_PHY_INFORM_T));
...
static int _HDMI_M14C0_GetMHLConection(void)
{
	...
	_gM14C0HDMIPhyInform.cbus_00 = LINK_M14_Rd(cbus_00);
	_gM14C0HDMIPhyInform.cbus_01 = LINK_M14_Rd(cbus_01);

	LINK_M14_RdFL(cbus_13);
	LINK_M14_Rd01(cbus_13, reg_state_sink_rcp, _gM14C0HDMIPhyInform.state_sink_rcp);

	if (cd_sense != _gM14C0HDMIPhyInform.cd_sense)
	{
		HDMI_NOTI("---- MHL CD SENSE : [%d] => [%d]\n", _gM14C0HDMIPhyInform.cd_sense, cd_sense);
		_gM14C0HDMIPhyInform.cd_sense = cd_sense;

		mhl_pp_mode_set = 0;

		if (!gHDMI_Phy_Control.all_port_pdb_enable && _gM14C0HDMIPhyInform.cd_sense && !_gM14C0HDMIPhyInform.phy_pdb[3] )
		{
			HDMI_DEBUG("---- MHL port 3 phy pdb force ON : [%d] => [%d]\n", _gM14C0HDMIPhyInform.phy_pdb[3], 1);
			LINK_M14_RdFL(phy_link_06);
	...
	if(cd_sense_changed)
	{
		...
		if (phy_pdb_prt3 && phy_rstn_prt3)
		{
			...
			if (cd_sense)
			{
				...
				PHY_REG_M14C0_Wr01(dr_filter_ch0,dr_filter_ch0,0x1);
				HDMI_DEBUG("---- MHL EQ_RS_MAN  : [%d]\n", 0x3);
				PHY_REG_M14C0_Wr01(eq_rs_man,eq_rs_man,0x4);	//140930 : 4K,DVB,15M	//from 0x3(default) // 0x5 //140514 : MHL CT B2 and new board


#ifdef M14_CODE_FOR_MHL_CTS
#ifdef M14_CBUS_PDB_CTRL
				// PDB off just after cd_sence detected : removed workaournd code for PRADA phone
				HDMI_DEBUG("---- CD_SENSE for MHL,  eq_i2c_pdb_d0 :0x10, eq_i2c_pdb_dck :0x10\n");
				PHY_REG_M14C0_RdFL(eq_i2c_pdb_d0);
				PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_i2c_pdb_d0_mode,0x1);
				PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_man_pdb_d0,0x0);
				PHY_REG_M14C0_WrFL(eq_i2c_pdb_d0);
				...
				HDMI_DEBUG("---- MHL ODT CTRL  : [%d]\n", _g_mhl_odt_value);
				PHY_REG_M14C0_RdFL(eq_odt_ctrl);
				PHY_REG_M14C0_Wr01(eq_odt_ctrl,eq_odt_ctrl,_g_mhl_odt_value);	//141014 : 46 Ohm// 0x0 //140514 : new board impedance 55=>50Ohm
				PHY_REG_M14C0_WrFL(eq_odt_ctrl);
#endif
				HDMI_DEBUG("---- MHL DR_N1  : [%d]\n", 0x0);
				PHY_REG_M14C0_RdFL(dr_n1);
				PHY_REG_M14C0_Wr01(dr_n1,dr_n1,0x0);	// for 15m cable (0x38/0x93)
				PHY_REG_M14C0_WrFL(dr_n1);

				// for MHL : 1 for MHL CTS
				HDMI_DEBUG("---- MHL CBUS_NORMAL_V1  : [%d]\n", 0x1);
				PHY_REG_M14C0_RdFL(cbus_normal_v1_pdb);
				PHY_REG_M14C0_Wr01(cbus_normal_v1_pdb,cbus_normal_v1,0x1);
				PHY_REG_M14C0_WrFL(cbus_normal_v1_pdb);

				//141001 : HDMI : 0x26, MHL : 0x24
				HDMI_DEBUG("---- MHL ICP_ADJ  : [0x%x]\n", 0x24);
				PHY_REG_M14C0_RdFL(cr_icp_adj);
				PHY_REG_M14C0_Wr01(cr_icp_adj,cr_icp_adj,0x4);		// 4K , DVB board (140930) //for HDMI (default 0x4)
				PHY_REG_M14C0_Wr01(cr_icp_adj,cr_mhl_icp_adj,0x1);	// 141023 : MHL CTS jitter test //for MHL (default 0x4)
				PHY_REG_M14C0_WrFL(cr_icp_adj);

				HDMI_DEBUG("---- MHL EQ_WIDE_CKEX_BW  : [0x%x]\n", 0x0);
				PHY_REG_M14C0_RdFL(eq_wide_ckex_bw);
				PHY_REG_M14C0_Wr01(eq_wide_ckex_bw,eq_wide_ckex_bw,0x0); //141030 : (0x1>0x0)MHL CTS NG(RCP/RAP) //141001 : MHL 24/PP mode (0x1)
				PHY_REG_M14C0_WrFL(eq_wide_ckex_bw);
			}
			else
			{
...
#ifdef M14_CBUS_PDB_CTRL
				// PDB off just after cd_sence detected : removed workaournd code for PRADA phone
				// change EQ pdb to auto mode when MHL to HDMI mode changed(master MSHG-700)
				HDMI_DEBUG("---- HDMI  eq_i2c_pdb_d0 :0x00, eq_i2c_pdb_dck :0x00\n");
				PHY_REG_M14C0_RdFL(eq_i2c_pdb_d0);
				PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_i2c_pdb_d0_mode,0x0);
				PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_man_pdb_d0,0x0);
				PHY_REG_M14C0_WrFL(eq_i2c_pdb_d0);
	...
	if (cbus_warb_done != _gM14C0HDMIPhyInform.cbus_warb_done)
	{
		HDMI_NOTI("---- MHL warb done : [%d] => [%d]\n", _gM14C0HDMIPhyInform.cbus_warb_done, cbus_warb_done);
		_gM14C0HDMIPhyInform.cbus_warb_done = cbus_warb_done;
		...
		if ( cbus_warb_done == 1)
		{
			...
			HDMI_DEBUG("---- MHL warb_done:1, eq_i2c_pdb_d0 :0x11, eq_i2c_pdb_dck :0x10\n");
			PHY_REG_M14C0_RdFL(eq_i2c_pdb_d0);
			PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_i2c_pdb_d0_mode,0x1);
			PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_man_pdb_d0,0x1);
			PHY_REG_M14C0_WrFL(eq_i2c_pdb_d0);
...
*/
		...
		else if (_gM14C0HDMIPhyInform.cd_sense)	// cd_sense, but not connection lost
#endif
		{
#if 0
			int phy_pdb_prt3, phy_rstn_prt3;
			...
			if (phy_pdb_prt3 && phy_rstn_prt3)
			{
				...
				HDMI_DEBUG("---- MHL warb_done:0, eq_i2c_pdb_d0 :0x10, eq_i2c_pdb_dck :0x10\n");
				PHY_REG_M14C0_RdFL(eq_i2c_pdb_d0);
				PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_i2c_pdb_d0_mode,0x1);
				PHY_REG_M14C0_Wr01(eq_i2c_pdb_d0,eq_man_pdb_d0,0x0);
				PHY_REG_M14C0_WrFL(eq_i2c_pdb_d0);
	...
	if (cbus_st != _gM14C0HDMIPhyInform.cbus_st)
	{
		HDMI_NOTI("---- MHL CBUS_ST : [%d] => [%d]\n", _gM14C0HDMIPhyInform.cbus_st, cbus_st);
		_gM14C0HDMIPhyInform.cbus_st = cbus_st;

		if (_gM14C0HDMIPhyInform.cbus_st == 3)
		{
			_gM14C0CHG_AVI_count_MHL = 0;
			_mhl_stable_count = 0;
		}
#ifdef M14_CODE_FOR_MHL_CTS
		else if (_gM14C0HDMIPhyInform.cbus_st  < 2)
			_HDMI_M14C0_Check_RAP_Content(1);
#endif
	}

	if ( _gM14C0HDMIPhyInform.cd_sense)	//MHL Mode
	{
		LINK_M14_RdFL(cbus_00);
...
#ifdef M14_CODE_FOR_MHL_CTS
		_HDMI_M14C0_Check_RAP_Content(0);
#if 0
		{
...
#endif
#endif
		if (_gM14C0HDMIPhyInform.cbus_warb_done && _gM14C0HDMIPhyInform.cbus_conn_done && (_gM14C0HDMIPhyInform.cbus_st == 3) )
		{
			/*
			   Workaround code for RCP Send Error
			 */
			 _HDMI_M14C0_MHL_Check_Status(0);
			/*
			   Workaround code for HTC beatsaudio phone
			 */
			if (_gM14C0CHG_AVI_count_MHL > 20)
			{
				HDMI_NOTI("---- MHL CHG_AVI_COUNT : [%d], mhl_stable_count : [%d] \n", _gM14C0CHG_AVI_count_MHL, _mhl_stable_count);

				_gM14C0CHG_AVI_count_MHL = -1;

				wake_up_interruptible(&WaitQueue_MHL_M14C0);
			}
			else if ( _gM14C0CHG_AVI_count_MHL != -1)
			{
				_mhl_stable_count ++;
...
#ifdef G_PHONE_MHL_DEBUG
		if (_gM14C0HDMIPhyInform.cbus_warb_done && _gM14C0HDMIPhyInform.cbus_conn_done )
		{
			int sink_set_hpd, msc_wrt_stat_pathen_set;

			LINK_M14_RdFL(cbus_05);
			LINK_M14_Rd01(cbus_05, reg_state_sink_set_hpd, sink_set_hpd);

			LINK_M14_RdFL(cbus_04);
			LINK_M14_Rd01(cbus_04, reg_man_msc_wrt_stat_pathen_set, msc_wrt_stat_pathen_set);

			if ( ( sink_set_hpd == 0x1) && ( msc_wrt_stat_pathen_set == 0) )
			{
				HDMI_NOTI("MHL Sink Set HPD : set PATHEN \n");
				LINK_M14_RdFL(cbus_04);
				LINK_M14_Wr01(cbus_04, reg_man_msc_wrt_stat_pathen_set, 0x1);
				LINK_M14_WrFL(cbus_04);
			}
		...
		//		if (hpd_in_prt3 != _gM14C0HDMIPhyInform.hpd_in[3])
		if (cbus_disconn)
		{
			//	HDMI_DEBUG("---- MHL HPD IN PORT3 : [%d] => [%d]\n",_gM14C0HDMIPhyInform.hpd_in[3], hpd_in_prt3);
			//	_gM14C0HDMIPhyInform.hpd_in[3] = hpd_in_prt3;

			//		if (_gM14C0HDMIPhyInform.hpd_in[3])
			if (1)
			{
				HDMI_DEBUG("---- MHL Clear cbus_disconn_en\n");
				LINK_M14_RdFL(cbus_00);
...
#ifdef M14C0_MHL_CONNECTION_BUG_WORKAROUND
	if ( _gM14C0HDMIPhyInform.cd_sense)	//MHL Mode
	{
		LINK_M14_RdFL(cbus_01);
		LINK_M14_Rd01(cbus_01, reg_cbus_disconn, cbus_disconn);			// CBUS DisConnected ???
	...
	{
		int phy_pdb_prt3, phy_rstn_prt3;
		if( (mhl_pp_mode_set == 0) && _HDMI_M14C0_Check_MHP_PP_link_mode() )
		{
			LINK_M14_RdFL(phy_link_06);
			LINK_M14_Rd01(phy_link_06, phy_pdb_prt3, phy_pdb_prt3);			//PHY PDB ON
			LINK_M14_Rd01(phy_link_06, phy_rstn_prt3, phy_rstn_prt3);			//PHY RESET
			if(phy_pdb_prt3 && phy_rstn_prt3)
			{
				...
				HDMI_DEBUG("---- MHL PP Mode Detected \n");
				PHY_REG_M14C0_RdFL(tmds_hdmi_mhl_pp_sel);
				PHY_REG_M14C0_Wr01(tmds_hdmi_mhl_pp_sel,tmds_man_packed_sel,0x1);
				PHY_REG_M14C0_Wr01(tmds_hdmi_mhl_pp_sel,tmds_man_packed_mode,0x1);
				PHY_REG_M14C0_WrFL(tmds_hdmi_mhl_pp_sel);

				PHY_REG_M14C0_RdFL(cr_i2c_mhl_mode);
				PHY_REG_M14C0_Wr01(cr_i2c_mhl_mode,cr_man_mhl_pp,0x1);
				PHY_REG_M14C0_Wr01(cr_i2c_mhl_mode,cr_man_mhl_mode,0x1);
				PHY_REG_M14C0_Wr01(cr_i2c_mhl_mode,cr_i2c_mhl_mode,0x1);
				PHY_REG_M14C0_WrFL(cr_i2c_mhl_mode);
		...
		else if( (mhl_pp_mode_set == 1) && ( _HDMI_M14C0_Check_MHP_PP_link_mode() == 0) )
		{
			LINK_M14_RdFL(phy_link_06);
			LINK_M14_Rd01(phy_link_06, phy_pdb_prt3, phy_pdb_prt3);			//PHY PDB ON
			LINK_M14_Rd01(phy_link_06, phy_rstn_prt3, phy_rstn_prt3);			//PHY RESET
			if(phy_pdb_prt3 && phy_rstn_prt3)
			{
				...
				HDMI_DEBUG("---- MHL PP Mode NOT Detected \n");
				PHY_REG_M14C0_RdFL(tmds_hdmi_mhl_pp_sel);
				PHY_REG_M14C0_Wr01(tmds_hdmi_mhl_pp_sel,tmds_man_packed_sel,0x0);
				PHY_REG_M14C0_Wr01(tmds_hdmi_mhl_pp_sel,tmds_man_packed_mode,0x0);
				PHY_REG_M14C0_WrFL(tmds_hdmi_mhl_pp_sel);

				PHY_REG_M14C0_RdFL(cr_i2c_mhl_mode);
				PHY_REG_M14C0_Wr01(cr_i2c_mhl_mode,cr_man_mhl_pp,0x0);
				PHY_REG_M14C0_Wr01(cr_i2c_mhl_mode,cr_man_mhl_mode,0x0);
				PHY_REG_M14C0_Wr01(cr_i2c_mhl_mode,cr_i2c_mhl_mode,0x0);
				PHY_REG_M14C0_WrFL(cr_i2c_mhl_mode);
...
#ifdef NOT_USED_NOW
static int _HDMI_M14C0_GetTMDSClock(int prt_selected, UINT32 *pTmds_clock)
{
	return RET_OK;
}
#endif	//#ifdef NOT_USED_NOW
...
int HDMI_M14C0_Module_Call_Type(LX_HDMI_CALL_TYPE_T	hdmi_call_type)
{
	...
	switch(hdmi_call_type)
	{
		case HDMI_CALL_TYPE_INIT :
			_gM14C0HDMIPhyInform.module_init = 1;
			break;
		case HDMI_CALL_TYPE_UNINIT :
			_gM14C0HDMIPhyInform.module_init = 0;
			break;
		case HDMI_CALL_TYPE_OPEN :
			_gM14C0HDMIPhyInform.module_open = 1;
	...
	//		_gM14C0HDMIPhyInform.hpd_enable = 1;
		...
		case HDMI_CALL_TYPE_CLOSE :
			_gM14C0HDMIPhyInform.module_open = 0;
...
/*
		...
		case HDMI_CALL_TYPE_CONN :
			_gM14C0HDMIPhyInform.module_conn = 1;
			break;
		case HDMI_CALL_TYPE_DISCONN :
			_gM14C0HDMIPhyInform.module_conn = 0;
			break;
		default :
			return RET_ERROR;
	}
...
int HDMI_M14C0_Reset_Internal_Edid(int port_num, int edid_resetn)
{
	...
	if (_gM14C0HDMIPhyInform.hpd_pol[port_num] != edid_resetn)
		HDMI_NOTI("HPD Polarity of port[%d] Changed [%d]=>[%d] \n", port_num, _gM14C0HDMIPhyInform.hpd_pol[port_num], edid_resetn);
	...
	switch(port_num)
	{
		case 0:
			...
			LINK_M14_WrFL(phy_link_00);
			_gM14C0HDMIPhyInform.hpd_pol[0] = edid_resetn;
			_HDMI_M14C0_Set_HPD_Out(0, !edid_resetn);	// disable HPD
			break;
		case 1:
			LINK_M14_RdFL(phy_link_02);
			LINK_M14_Wr01(phy_link_02, link_sw_rstn_edid_prt1, edid_resetn);
			LINK_M14_WrFL(phy_link_02);
			_gM14C0HDMIPhyInform.hpd_pol[1] = edid_resetn;
			_HDMI_M14C0_Set_HPD_Out(1, !edid_resetn);	// disable HPD
			break;
		case 2:
			LINK_M14_RdFL(phy_link_04);
			LINK_M14_Wr01(phy_link_04, link_sw_rstn_edid_prt2, edid_resetn);
			LINK_M14_WrFL(phy_link_04);
			_gM14C0HDMIPhyInform.hpd_pol[2] = edid_resetn;
			_HDMI_M14C0_Set_HPD_Out(2, !edid_resetn);	// disable HPD
			break;
		case 3:
			LINK_M14_RdFL(phy_link_06);
			LINK_M14_Wr01(phy_link_06, link_sw_rstn_edid_prt3, edid_resetn);
			LINK_M14_RdFL(phy_link_06);
			_gM14C0HDMIPhyInform.hpd_pol[3] = edid_resetn;
			_HDMI_M14C0_Set_HPD_Out(3, !edid_resetn);	// disable HPD
			break;
		default :
			ret = RET_ERROR;
			break;
...
static int _HDMI_M14C0_MHL_Reconnect_Thread(void *data)
{
	...
	while(1)
	{
		...
		//interruptible_sleep_on(&WaitQueue_MHL_M14C0);
		wait_event_interruptible(WaitQueue_MHL_M14C0 , _MHL_Reconnect_Thread_wakeup !=0 );
...
static int _HDMI_M14C0_MHL_PDB_Thread(void *data)
{
	...
	while(1)
	{
		...
		//interruptible_sleep_on(&WaitQueue_MHL_PDB_M14C0);
		wait_event_interruptible(WaitQueue_MHL_PDB_M14C0 , _MHL_PDB_Thread_wakeup !=0 );
		...
		HDMI_DEBUG("---- MHL disconn, eq_i2c_pdb_d0 :0x10, eq_i2c_pdb_dck :0x10\n");
	}
...
static int _HDMI_M14C0_Enable_TERC4_Error_Interrupt(int port_num)
{
	LINK_M14_RdFL(interrupt_enable_01);
	if(port_num == 3)
		LINK_M14_Wr01(interrupt_enable_01, intr_terc4_err_prt3_enable, 0x1);		///< 18 intr_terc4_err_prt3_enable
	else if (port_num == 2)
		LINK_M14_Wr01(interrupt_enable_01, intr_terc4_err_prt2_enable, 0x1);		///< 19 intr_terc4_err_prt2_enable
	else if (port_num == 1)
		LINK_M14_Wr01(interrupt_enable_01, intr_terc4_err_prt1_enable, 0x1);		///< 20 intr_terc4_err_prt1_enable
	else if (port_num == 0)
		LINK_M14_Wr01(interrupt_enable_01, intr_terc4_err_prt0_enable, 0x1);		///< 21 intr_terc4_err_prt0_enable
	else
		return RET_ERROR;
	LINK_M14_WrFL(interrupt_enable_01);
...
{
	LINK_M14_RdFL(interrupt_enable_01);
	LINK_M14_Wr01(interrupt_enable_01, intr_hdcp_err_enable, 0x1);				///< 16 intr_hdcp_err_enable
	LINK_M14_WrFL(interrupt_enable_01);
...
static int _HDMI_M14C0_Check_RAP_Content(int clear)
{
	...
	if (clear)
	{
		...
		_gM14C0MHLContentOff = 0;
	}
	else if ( (content_on_status != prev_content_on_status) && (_gM14C0HDMIPhyInform.cbus_st > 1) )
	{
		HDMI_DEBUG("---- MHL RAP Contents On CHG: [%d] => [%d] \n", prev_content_on_status, content_on_status);

		if (content_on_status)	// RAP : content ON
		{
			if (_gM14C0MHLContentOff)	// av mute state
			{
...
static int _HDMI_M14C0_MHL_Check_Status(int init)
{
	int state_sink_wrt_stat_pathen_set, seq_st, ddc_error;
...
#else
		LINK_M14_Wr(cbus_04, 0x00020100);	//initial CBUS-04 register value
#endif
	...
	else
	{
		...
		LINK_M14_RdFL(cbus_14);
		LINK_M14_Rd01(cbus_14, reg_tx_ddc_errcode, ddc_error);
		...
		else if ( (ddc_error == 0x2) && (mhl_status_invalid == 0) )
		{
			/* For G-Phone 4.4.2 MHL Connection Error (HDCP not authed) */

			HDMI_NOTI("MHL Connected BUT DDC ERROR : reg_tx_ddc_errcode : [0x%x] \n", ddc_error);
		...
		else if (mhl_status_invalid)
		{
			...
			if ( (man_msc_clr_hpd == 0x1) && (man_msc_set_hpd == 0) && !_gM14C0HDMIPhyInform.scdt[3])
			{
...
int HDMI_M14C0_Get_Driver_Status(LX_HDMI_DRIVER_STATUS_T *spHDMI_Driver_Status)
{
	...
	memcpy(&spHDMI_Driver_Status->PrevTiming, &_gM14C0PrevTiming, sizeof(LX_HDMI_TIMING_INFO_T));
	spHDMI_Driver_Status->TimingReadCnt[0] = _gM14C0TimingReadCnt;
	spHDMI_Driver_Status->HDMIState[0] = _gM14C0HDMIState;
	spHDMI_Driver_Status->PrevPixelEncoding[0] = _gM14C0PrevPixelEncoding;
	memcpy(&spHDMI_Driver_Status->PrevAVIPacket[0], &_gM14C0PrevAVIPacket, sizeof(LX_HDMI_INFO_PACKET_T));
	memcpy(&spHDMI_Driver_Status->PrevVSIPacket[0], &_gM14C0PrevVSIPacket, sizeof(LX_HDMI_INFO_PACKET_T));
	spHDMI_Driver_Status->AVIReadState[0] = _gM14C0AVIReadState;
	spHDMI_Driver_Status->VSIState[0] = _gM14C0VSIState;
	spHDMI_Driver_Status->AVIChangeState[0] = _gM14C0AVIChangeState;
	spHDMI_Driver_Status->PortSelected[0] = _gM14C0PortSelected;
	spHDMI_Driver_Status->MHLContentOff = _gM14C0MHLContentOff;

	spHDMI_Driver_Status->AudioMuteState[0] = _gM14C0AudioMuteState;
	spHDMI_Driver_Status->AudioArcStatus[0] = _gM14C0AudioArcStatus;
	spHDMI_Driver_Status->IntrAudioState[0] = _gM14C0IntrAudioState;
	spHDMI_Driver_Status->HdmiPortStableCount[0] = _gM14C0HdmiPortStableCount;
	spHDMI_Driver_Status->HdmiFreqErrorCount[0] = _gM14C0HdmiFreqErrorCount;
	spHDMI_Driver_Status->HdmiAudioThreadCount[0] = _gM14C0HdmiAudioThreadCount;
	spHDMI_Driver_Status->IntrBurstInfoCount[0] = _gM14C0IntrBurstInfoCount;
	spHDMI_Driver_Status->TaskBurstInfoCount[0] = _gM14C0TaskBurstInfoCount;
	memcpy(&spHDMI_Driver_Status->HdmiAudioInfo[0], &_gM14C0HdmiAudioInfo, sizeof(LX_HDMI_AUDIO_INFO_T));

	spHDMI_Driver_Status->Intr_vf_stable[0] = _gM14C0Intr_vf_stable;
	spHDMI_Driver_Status->Intr_avi[0] = _gM14C0Intr_avi;
	spHDMI_Driver_Status->Intr_src_mute[0] = _gM14C0Intr_src_mute;
	spHDMI_Driver_Status->Intr_vsi[0] = _gM14C0Intr_vsi;

	spHDMI_Driver_Status->Force_thread_sleep = _gM14C0Force_thread_sleep;
	spHDMI_Driver_Status->HDMI_thread_running = _gM14C0HDMI_thread_running;
	spHDMI_Driver_Status->HDMI_interrupt_registered = _gM14C0HDMI_interrupt_registered;

	spHDMI_Driver_Status->ChipPlatform = _gM14C0ChipPlatform;

	spHDMI_Driver_Status->CHG_AVI_count_MHL[0] = _gM14C0CHG_AVI_count_MHL;
	spHDMI_Driver_Status->CHG_AVI_count_EQ[0] = _gM14C0CHG_AVI_count_EQ;
	spHDMI_Driver_Status->TMDS_ERROR_EQ[0] = _gM14C0_TMDS_ERROR_EQ;

	spHDMI_Driver_Status->Driver_Revision = 0x14122200;
...
static int _HDMI_M14C0_HDCP_Check_Status(void)
{
	...
	static int hdcp_ri_value[4] = {0,};
	static int prev_hdcp_ri_value[4] = {0,};
	static unsigned int hdcp_ri_changed_time[4] = {0,};
	static unsigned int hdcp_ri_current_time[4] = {0,};
	static unsigned int hdcp_ri_elasped_time[4] = {0,};
	int port_num;

	for (port_num = 0; port_num <4; port_num++)
	{
		if ( (_gM14C0HDMIPhyInform.hdcp_authed[port_num] == 0) \
				&& (_HDMI_M14C0_Check_Aksv_Exist(port_num) )
				&& (_gM14C0HDMIPhyInform.scdt[port_num] ) )
		{
			...
			if (hdcp_unauthed_count[port_num] > 200)	//for about 4-sec
			{
				...
				if( (port_num == 3) &&  _gM14C0HDMIPhyInform.cd_sense && (_gM14C0HDMIPhyInform.cbus_st == 3) && _gM14C0HDMIPhyInform.cbus_conn_done)
				{
					int cbus_st_count, state_sink_clr_hpd, state_sink_set_hpd;
	...
	if( gHDMI_Phy_Control.hdcp_reset_ri_workaround == 1)
	{
		for (port_num = 0; port_num <4; port_num++)
		{
			_HDMI_M14C0_Get_HDCP_RI(port_num, &hdcp_ri_value[port_num]);
			if( hdcp_ri_value[port_num] != prev_hdcp_ri_value[port_num] )
			{
				HDMI_DEBUG("@ HDCP RI Changed [0x%x]=>[0x%x] @\n", prev_hdcp_ri_value[port_num], hdcp_ri_value[port_num]);
				hdcp_ri_changed_time[port_num] = jiffies_to_msecs(jiffies);
				prev_hdcp_ri_value[port_num] = hdcp_ri_value[port_num];
			}
			else if ( (_gM14C0HDMIPhyInform.hdcp_authed[port_num] == 1 ) && (_gM14C0HDMIPhyInform.hdcp_enc_en[port_num] == 1)\
					&& (hdcp_ri_value[port_num] != 0) )
			{
				hdcp_ri_current_time[port_num] = jiffies_to_msecs(jiffies);
				hdcp_ri_elasped_time[port_num] = hdcp_ri_current_time[port_num] - hdcp_ri_changed_time[port_num];

				if(hdcp_ri_elasped_time[port_num] > 8000) // RI Not Changed for 8-sec
				{
					HDMI_NOTI("@@@@@ HDCP RI[%d] Not Changed for 8-sec : Resetting HDCP !!!!! @@@@@\n", port_num);
					_HDMI_M14C0_HDCP_ResetPort(port_num);
				}
			}
		}
...
static int _HDMI_M14C0_Check_MHL_PP_mode(void)
{
	...
	LINK_M14_RdFL(cbus_00);
	LINK_M14_Rd01(cbus_00, reg_cbus_pp_mode, ret);	// 1: MHL PP mode
...
static int _HDMI_M14C0_Check_MHP_PP_link_mode(void)
{
	...
	LINK_M14_RdFL(cbus_20);
	LINK_M14_Rd01(cbus_20, reg_sink_stat_link_mode, value);	// 010 :PP Mode, 011 : 24bit
...
int HDMI_M14C0_EnableMHL(BOOLEAN bEnable)
{
	...
	if(bEnable) {
		_g_mhl_enable_flag = 1;
	}
	else {
		_g_mhl_enable_flag = 0;
		_HDMI_M14C0_Set_ManMHLMode(1, 0);
	}
...
int HDMI_M14C0_Set_MHL_ODT(LX_HDMI_MHL_ODT_IMPEDANCE_T odt_value)
{
	...
	if(odt_value == MHL_ODT_46_OHM)
		_g_mhl_odt_value = 0x3;
	else if(odt_value == MHL_ODT_50_OHM)
		_g_mhl_odt_value = 0x1;
	else if(odt_value == MHL_ODT_55_OHM)
		_g_mhl_odt_value = 0x0;
	else
		ret = RET_ERROR;
...
int HDMI_M14C0_Driver_Debug(LX_HDMI_DRV_DEBUG_T *sp_hdmi_drv_debug)
{
	...
	if(sp_hdmi_drv_debug->hdmi_debug_0 == 2) {
		...
		{
			_HDMI_M14C0_TCS_addtional_value_init = sp_hdmi_drv_debug->hdmi_debug_2;
			HDMI_NOTI("!!!!!!! TCS Add value changed to [0x%x] !!!!!!!!\n", _HDMI_M14C0_TCS_addtional_value_init );
		}
		else if(sp_hdmi_drv_debug->hdmi_debug_1 == 2)
		{
			_HDMI_M14C0_TCS_mode_sel = sp_hdmi_drv_debug->hdmi_debug_2;
			HDMI_NOTI("!!!!!!! TCS Mode Sel changed to [0x%x] !!!!!!!!\n", _HDMI_M14C0_TCS_mode_sel );
		}
		else if(sp_hdmi_drv_debug->hdmi_debug_1 == 3)
		{
			_HDMI_M14C0_eq_eval_time_value = sp_hdmi_drv_debug->hdmi_debug_2;
			HDMI_NOTI("!!!!!!! EQ EVAL TIME to [0x%x] !!!!!!!!\n", _HDMI_M14C0_eq_eval_time_value );
		}
		else if(sp_hdmi_drv_debug->hdmi_debug_1 == 4)
		{
			_HDMI_M14C0_hdmi_dr_n1_value = sp_hdmi_drv_debug->hdmi_debug_2;
			HDMI_NOTI("!!!!!!! HDMI DR_N1 changed to [0x%x] !!!!!!!!\n", _HDMI_M14C0_hdmi_dr_n1_value );
		}
	}
...
static int _HDMI_M14C0_Get_HDCP_RI(int port_num, int *hdcp_ri_value)
{
	...
	{
		LINK_M14_RdFL(hdcp_11);
		LINK_M14_Rd01(hdcp_11, reg_hdcp_ri_0_prt1, hdcp_ri_0);
		LINK_M14_Rd01(hdcp_11, reg_hdcp_ri_1_prt1, hdcp_ri_1);
	}
	else if(port_num == 2)
	{
		LINK_M14_RdFL(hdcp_17);
		LINK_M14_Rd01(hdcp_17, reg_hdcp_ri_0_prt2, hdcp_ri_0);
		LINK_M14_Rd01(hdcp_17, reg_hdcp_ri_1_prt2, hdcp_ri_1);
	}
	else if(port_num == 3)
	{
		LINK_M14_RdFL(hdcp_23);
		LINK_M14_Rd01(hdcp_23, reg_hdcp_ri_0_prt3, hdcp_ri_0);
		LINK_M14_Rd01(hdcp_23, reg_hdcp_ri_1_prt3, hdcp_ri_1);
	}
	else
		ret = RET_ERROR;

	*hdcp_ri_value = hdcp_ri_1 << 8 | hdcp_ri_1;
...
int HDMI_M14C0_RunResume(void)
{
	...
	_HDMI_M14C0_ClearSWResetAll();			//M14D Chip - Ctop control
