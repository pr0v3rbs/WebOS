...
#define GFX_WRITE_REG(reg,val)	\
    __raw_writel(val , (volatile void*)((ULONG)(g_gfx_reg_h14) + (ULONG)OFFSET(GFX_H14_REG_T,reg)))

#define GFX_READ_REG(reg)	\
    __raw_readl( (volatile void*)((ULONG)(g_gfx_reg_h14) + (ULONG)OFFSET(GFX_H14_REG_T,reg)))

//#define SHOW_GFX_REGISTER //only debugging
#if defined(SHOW_GFX_REGISTER)

#define PRINT_READ_REG(reg)	\
   do { printk("GFX REG 0x%x -> 0x%0.8x\n",((UINT32)OFFSET(GFX_H14_REG_T,reg)), *((UINT32 *)(&g_gfx_reg_h14_cache->reg)) ); } while(0);
...
GFX_H14_SCALE_VPHASE_DATA_T;

typedef struct
{
#define	GFX_SCALE_PHASE_DATA_NUM	16
	GFX_H14_SCALE_HPHASE_DATA_T		hphase[GFX_SCALE_PHASE_DATA_NUM];		/* hhpase data */
	GFX_H14_SCALE_VPHASE_DATA_T		vphase[GFX_SCALE_PHASE_DATA_NUM];		/* vhpase data */
}
GFX_H14_SCALE_FILTER_DATA_T;
...
/*----------------------------------------------------------------------------------------
	External Variables
----------------------------------------------------------------------------------------*/
extern volatile GFX_H14_REG_T	*g_gfx_reg_h14;
extern GFX_H14_REG_T 			*g_gfx_reg_h14_cache;

/*----------------------------------------------------------------------------------------
	global Variables
----------------------------------------------------------------------------------------*/
volatile UINT32 g_ui_ack_mode_h14 = 0;

#ifdef	KDRV_CONFIG_PM
unsigned int g_pm_cmd_delay_h14	= 0;
unsigned int g_pm_intr_ctrl_h14	= 0;
#endif
...
/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
static int	_g_gfx_h14_bw_val			= 0x7ff;
static int	_g_gfx_h14_bw_val_req		= 0x7ff;
module_param_named( gfx_h14_bw_val, _g_gfx_h14_bw_val_req, int, 0644 );

static int	_g_gfx_h14_cmd_delay		= 0x0;
static int	_g_gfx_h14_cmd_delay_req	= 0x0;
module_param_named( gfx_h14_cmd_delay, _g_gfx_h14_cmd_delay_req, int, 0644 );

#define	GFX_H14_CUSTOM_FILTER_MAX	4

static int _g_gfx_h14_scaler_index = -1;
module_param_named( gfx_h14_scaler_index, _g_gfx_h14_scaler_index, int, 0644 );

static GFX_H14_SCALE_FILTER_DATA_T 	g_gfx_h14_custom_scale_filter_data[GFX_H14_CUSTOM_FILTER_MAX] = 
{
 ...
 *
 */
static int		GFX_H14_GetBestHVScalerIndex ( GFX_SCALER_CONFIGURE_T* scaler )
{
	int	h_scale_ratio = 0;
	int	v_scale_ratio = 0;

	if ( _g_gfx_h14_scaler_index >= 0 ) return _g_gfx_h14_scaler_index;
 ...
 *
 */
void	GFX_H14_InitScaler(void)
{
	int	i,j;
	GFX_H14_SCALE_FILTER_DATA_T*	filter_data;

	GFX_WRITE_REG( scaler_frame_start, 0x1 );					/* scaler frame start (reset) */

	for ( j=0; j< GFX_H14_CUSTOM_FILTER_MAX; j++ )				/* download custome filter data */
	{
		filter_data = &(g_gfx_h14_custom_scale_filter_data[j]);	

		for ( i=0; i< GFX_SCALE_PHASE_DATA_NUM; i++ )
	{
			g_gfx_reg_h14->scaler_hphase_data0 = filter_data->hphase[i].data0;
			g_gfx_reg_h14->scaler_hphase_data1 = filter_data->hphase[i].data1;
			g_gfx_reg_h14->scaler_hphase_data2 = filter_data->hphase[i].data2;
			g_gfx_reg_h14->scaler_hphase_addr  = (j<<4) | filter_data->hphase[i].addr;
	}

		for ( i=0; i< GFX_SCALE_PHASE_DATA_NUM; i++ )
	{
			g_gfx_reg_h14->scaler_vphase_data0 = filter_data->vphase[i].data0;
			g_gfx_reg_h14->scaler_vphase_data1 = filter_data->vphase[i].data1;
			g_gfx_reg_h14->scaler_vphase_addr  = (j<<4) | filter_data->vphase[i].addr;
		}
	}
}
 ...
 *
 */
int GFX_H14_RunFlushCommand(void)
{
	GFX_TRACE_BEGIN();
 ...
 *
 */
int GFX_H14_RunStartCommand(void)
{
	GFX_TRACE_BEGIN();

	/* write B/W limit if changed */
	if ( _g_gfx_h14_bw_val != _g_gfx_h14_bw_val_req	)
	{
		_g_gfx_h14_bw_val_req &= 0x7ff;

		GFX_WARN("+++ bw_limit changed 0x%x -> 0x%x\n", _g_gfx_h14_bw_val, _g_gfx_h14_bw_val_req );
		_g_gfx_h14_bw_val = _g_gfx_h14_bw_val_req;
		OS_WrReg( 0xc000e4f8, _g_gfx_h14_bw_val );
	}

	/* write B/W limit if changed */
	if ( _g_gfx_h14_cmd_delay != _g_gfx_h14_cmd_delay_req	)
	{
		_g_gfx_h14_cmd_delay_req &= 0xffff;

		GFX_WARN("+++ cmd_delay changed 0x%x -> 0x%x\n", _g_gfx_h14_cmd_delay, _g_gfx_h14_cmd_delay_req );
		_g_gfx_h14_cmd_delay = _g_gfx_h14_cmd_delay_req;

		GFX_RdFL( gfx_cmd_dly );
		GFX_Wr01( gfx_cmd_dly, gfx_cmd_dly, _g_gfx_h14_cmd_delay );
 ...
 *
 */
int GFX_H14_PauseCommand(void)
{
	GFX_TRACE_BEGIN();
	g_gfx_reg_h14->gfx_reset.gfx_reset				= 0x0;
	g_gfx_reg_h14->gfx_batch_run.gfx_batch_run		= 0x0;
	g_gfx_reg_h14->gfx_start.gfx_start				= 0x0;
	g_gfx_reg_h14->gfx_pause.gfx_pause				= 0x1;
	GFX_TRACE_END();
 ...
 *
 */
int GFX_H14_ResumeCommand(void)
{
	GFX_TRACE_BEGIN();
	g_gfx_reg_h14->gfx_reset.gfx_reset				= 0x0;
	g_gfx_reg_h14->gfx_batch_run.gfx_batch_run		= 0x0;
	g_gfx_reg_h14->gfx_start.gfx_start				= 0x0;
	g_gfx_reg_h14->gfx_pause.gfx_pause				= 0x2;
	GFX_TRACE_END();
 ...
 */
int GFX_H14_SwResetCommand(void)
{
 ...
 *
     */
int GFX_H14_StopCommand(void)
{
	return GFX_H14_SwResetCommand();
}

/** get command queue status 
 *
 */
void GFX_H14_GetComQueueStatus(GFX_CMD_QUEUE_CTRL_T *queue)
{
	GFX_TRACE_BEGIN();
 ...
 */
BOOLEAN GFX_H14_IsGfxIdle(void)
{
    BOOLEAN ret;
	...
	ret = ( g_gfx_reg_h14_cache->gfx_status0.exec_status	== 0 &&
			g_gfx_reg_h14_cache->gfx_status1.cmd_que_status== 0 )? TRUE : FALSE;
//   ret = ( g_gfx_reg_h14->status0.bwCmdStatus == 0 &&
//          g_gfx_reg_h14->status0.abwProcessingLine == 0 &&
//          g_gfx_reg_h14->status1.bwCmdQueueStatus == 0 )? TRUE : FALSE ;

//	printk("GFX IDLE = (%s) %d-%d-%d\n", (ret)? "IDLE":"BUSY", 
//			g_gfx_reg_h14_cache->gfx_status0.exec_status, g_gfx_reg_h14_cache->gfx_status0.exec_line, g_gfx_reg_h14_cache->gfx_status1.cmd_que_status );
 ...
 *
 */
void GFX_H14_GetOperationStatus(GFX_CMD_OP_T *operate)
{
	GFX_TRACE_BEGIN();
	...
	operate->bChromaFilterEnable = 0 ; //g_gfx_reg_h14->op_mode.bwEnChromaFilter;
 ...
 *
 *	@note operation status should be read at GFX_H14_GetOperationStatus
 */
void GFX_H14_SetOperationStatus(GFX_CMD_OP_T *operate)
{
	GFX_TRACE_BEGIN();
	 ...
	 */
#if 0
    GFX_REG_MEMCPY( &g_gfx_reg_h14->op_mode, &g_gfx_reg_h14_cache->op_mode, 3 /* reg count = 3 */ );
#else
 ...
 *
 */
void GFX_H14_GetInputConfigure(int iPort, GFX_PORT_CONFIGURE_T *port)
{
	GFX_TRACE_BEGIN();
...
void GFX_H14_SetInputConfigure(GFX_PORT_CONFIGURE_T *port)
{
	GFX_TRACE_BEGIN();
 ...
 *
 */
void GFX_H14_GetBlendConfigure( GFX_BLEND_CONFIGURE_T *blend)
{
	UINT32	color_a;
	UINT32	color_c;
...
//	blend->ulBlendConstant	= g_gfx_reg_h14->out_const_ctrl.abwConstantCtrl;
	GFX_Rd01( rop_ctrl,		rop_ctrl,		blend->sRaster );
//	blend->sRaster			= g_gfx_reg_h14->out_rop_ctrl.abwRasterSel;
 ...
 *
 *	@note out_sel should be read before this function at GFX_H14_GetBlendConfigure
 */
void GFX_H14_SetBlendingOut(GFX_ENUM_OUT_T type)
{
	GFX_TRACE_BEGIN();
 ...
 *
 * 	@note all data should be read before this function at GFX_H14_GetBlendConfigure
 */
void GFX_H14_SetBlendConfigure( GFX_BLEND_CONFIGURE_T *blend)
{
	UINT32	color_a, color_c;
	GFX_TRACE_BEGIN();
	
	GFX_Wr01( out_sel, 	out_sel,	blend->sOut );
//	blend->sOut 		= g_gfx_reg_h14->out_sel.abwOutSel;
...
void GFX_H14_GetOutputConfigure(GFX_OUT_CONFIGURE_T *port)
{
	GFX_TRACE_BEGIN();
...
void GFX_H14_SetOutputConfigure(GFX_OUT_CONFIGURE_T *port)
{
	GFX_TRACE_BEGIN();
...
static int GFX_H14_SelectScalerOrder( GFX_SCALER_CONFIGURE_T* s )
{
    int vh_order;

    if ( ( s->in_dim.w < 4096 && s->in_dim.h < 4096 ) && ( s->out_dim.w >= 4096 || s->out_dim.h >= 4096 ) )
    {
...
void GFX_H14_SetScalerConfigure(GFX_SCALER_CONFIGURE_T* scaler)
{
	switch( scaler->cmd )
	{
		...
		{
			int scaler_set_idx = GFX_H14_GetBestHVScalerIndex( scaler );
			int vh_order= GFX_H14_SelectScalerOrder( scaler );
#if 1
...
#else
			g_gfx_reg_h14->scaler_line_init.scaler_line_init = 0x1; /* init scaler line */
			g_gfx_reg_h14->scaler_line_init.scaler_line_init = 0x0;
#endif
		...
		default:
		{
		    g_gfx_reg_h14->scaler_bypass.scaler_bypass = 0x1;
		}
		break;
	}
}

void GFX_H14_SetInterruptMode(UINT32 uiMode)
{
	/* do nothing */
}

/* what is it ? */
void GFX_H14_SetAckMode(UINT32 *mode)
{
	GFX_TRACE_BEGIN();
	g_ui_ack_mode_h14 = *mode;
	g_ui_ack_mode_h14 = 1;
	GFX_TRACE_END();
}

/* what is it ? */
void GFX_H14_GetAckMode(UINT32 *mode)
{
	*mode = g_ui_ack_mode_h14;
}

int GFX_H14_SetCLUT(int port, int size , UINT32 *data)
{
	int i , ret = 0;
	...
	switch(port)
	{
        case 0:
        {
            g_gfx_reg_h14->r0_clut_ctrl.clut0_addr	= 0;
            g_gfx_reg_h14->r0_clut_ctrl.clut0_rw	= 0;
#ifdef GFX_PALETTE_AUTO_INC
            g_gfx_reg_h14->r0_clut_ctrl.clut0_wai	= 1;
#else
            g_gfx_reg_h14->r0_clut_ctrl.clut0_wai	= 0;
#endif
            for(i=0 ; i<size ; i++)
            {
#ifdef GFX_PALETTE_AUTO_INC
...
#else
                g_gfx_reg_h14->r0_clut_ctrl.clut0_addr = i;
#endif
                g_gfx_reg_h14->r0_clut_data			= *data++;
            }
        }
        break;
        ...
        case 1:
        {
            g_gfx_reg_h14->r1_clut_ctrl.clut1_addr	= 0;
            g_gfx_reg_h14->r1_clut_ctrl.clut1_rw	= 0;
#ifdef GFX_PALETTE_AUTO_INC
            g_gfx_reg_h14->r1_clut_ctrl.clut1_wai	= 1;
#else
            g_gfx_reg_h14->r1_clut_ctrl.clut1_wai	= 0;
#endif
            for(i=0 ; i<size ; i++)
            {
#ifdef GFX_PALETTE_AUTO_INC
...
#else
                g_gfx_reg_h14->r1_clut_ctrl.clut1_addr = i;
#endif
                g_gfx_reg_h14->r1_clut_data			= *data++;
			}
        }
        break;
 ...
 *
 */
void GFX_H14_SetCommandDelayReg(UINT16 delay)
{
	GFX_TRACE_BEGIN();
	g_gfx_reg_h14->gfx_cmd_dly.gfx_cmd_dly = delay;
	GFX_TRACE_END();
...
*/
UINT16 GFX_H14_GetCommandDelayReg(void)
{
	UINT16	val;
	GFX_TRACE_BEGIN();
	val = g_gfx_reg_h14->gfx_cmd_dly.gfx_cmd_dly;
	GFX_TRACE_END();
	return val;
}
 ...
 */
void GFX_H14_DumpRegister(void)
{
    int i;
    UINT32* reg_ptr = (UINT32*)g_gfx_reg_h14;
...
int GFX_H14_RunSuspend(void)
{
	/* do nothing */
	return RET_OK;
}

int GFX_H14_RunResume(void)
{
	GFX_InitHW( );
	return RET_OK;
}
...
int GFX_H14_ClkCheck(void)
{
    ...
    CTOP_CTRL_H14A0_RdFL( ctr07 );
    CTOP_CTRL_H14A0_Rd01( ctr07, gfx_aclk_sel, gfx_clk );
 ...
 *
 */
int GFX_H14_RunClkOffCommand(void)
{
#if 1
	UINT64	start_ms;
	UINT64	curr_ms;
	...
	do
	{
		if ( GFX_H14_IsGfxIdle() ) { GFX_ERROR("gfx idle ok\n"); break; }
    ...
    CPU_TOP_H14A0_Wr01( flush_req, gfx_flush_en, 1 );
	...
	do
	{
		...
		CPU_TOP_H14A0_Rd01( flush_done_status, gfx_flush_done, gfx_flush_done );
	...
	/* step 3: reset */
    CTOP_CTRL_H14A0_RdFL( ctr07 );
    CTOP_CTRL_H14A0_Wr01( ctr07, swrst_gfx, 1 );
    CTOP_CTRL_H14A0_WrFL( ctr07 );

	/* step 4: clock gating off */
    CTOP_CTRL_H14A0_RdFL( ctr07 );
    CTOP_CTRL_H14A0_Wr01( ctr07, gfx_aclk_sel, 0x3 );
    CTOP_CTRL_H14A0_WrFL( ctr07 );
 ...
 *
 */
int GFX_H14_RunClkOnCommand(void)
{
#if 1
    ...
    CPU_TOP_H14A0_RdFL( flush_req );
    CPU_TOP_H14A0_Wr01( flush_req, gfx_flush_en, 0 );
    CPU_TOP_H14A0_WrFL( flush_req );

    // step 1: Clock gating on
    CTOP_CTRL_H14A0_RdFL( ctr07 );
    CTOP_CTRL_H14A0_Wr01( ctr07, gfx_aclk_sel, 0x0 );
    CTOP_CTRL_H14A0_WrFL( ctr07 );

    // 3. software reset back to normal
    CTOP_CTRL_H14A0_RdFL( ctr07 );
    CTOP_CTRL_H14A0_Wr01( ctr07, swrst_gfx, 0 );
    CTOP_CTRL_H14A0_WrFL( ctr07 );
