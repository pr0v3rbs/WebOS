...
int DEMOD_H14GBB_ResetHW(void)
{
	
	ACE_REG_H14A0_RdFL(soft_reset_0);
	//check chungiii swrst_f24m or swrst_f24m_g ?????
...
int DEMOD_H14GBB_SetInclk(LX_DEMOD_INCLK_SEL_T *inclk)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_RepeaterEnable(BOOLEAN bEnable)
{
	UINT8 ni2crpten;
	...
	DEMOD_PRINT("^r^[DEMOD H14GBB GBB] success!!  DVB_RepeaterEnable =  %s ", (bEnable == TRUE)?"ON":"OFF");
	return RET_OK;
}
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_SoftwareReset(void)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Serial_Control(BOOLEAN bEnable)
{
	...
	DEMOD_PRINT("^r^[DEMOD H13 DTV] success!!  DEMOD_H14GBB_VQI_Serial_Control =  %s ", (bEnable == TRUE)? "SERIAL":"PARALLEL");
...
int DEMOD_H14GBB_Power_Save(BOOLEAN bEnable)
{
	...
	DEMOD_PRINT("[DEMOD H14GBB GBB] success!!  DEMOD_H14GBB_Power_Save =  %s  %d\n ", (bEnable == TRUE)? "ON":"OFF", gbbpdb);
...
************************************************************************************/
/**
 * DEMOD_H14GBB_StdOperModeContrl
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_StdOperModeContrl( LX_DEMOD_OPER_MODE_T operMode, LX_DEMOD_BWMODE_T bandWd, LX_DEMOD_SYMRDET_MODE_T symbr)
{
	UINT8 	stdopmode_7_0;
	...
	switch(operMode)
	{
		...
		default :
			DEMOD_PRINT("[DEMOD H14GBB GBB](%s,%d) Invaild Opermode parameter !!!\n",__F__, __L__);
			return RET_ERROR;
	}
	...
	switch(bandWd)
	{
		...
		default :
			DEMOD_PRINT("[DEMOD H14GBB GBB](%s,%d) Invalid Bandwidth parameter !!!\n",__F__, __L__);
			return RET_ERROR;
	}
	...
	switch(symbr)
	{
		...
		default :
			DEMOD_PRINT("[DEMOD H14GBB GBB](%s,%d) Invalid Symbol rate parameter !!!\n",__F__, __L__);
			return RET_ERROR;
	}
	...
	*/
	...
	DEMOD_PRINT("[DEMOD H14GBB GBB] Sampling frequency is %d [MHz] (%d)!!!\n",samplingFreq/65536, samplingFreq );
 ...
 * @author 	Jeongpil.yun(jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_NeverlockScan(DEMOD_CTX_T *pDemodInfo, LX_DEMOD_FLAG_T ResetOn, LX_DEMOD_FLAG_T *pScanProcessing, LX_DEMOD_LOCK_STATE_T *pScanLock)
{
		...
		case LX_DEMOD_IDLE :
			...
			if(elapsedTime > _maxTimeOut)
			{
				*pScanProcessing	= LX_DEMOD_FLAG_FINISH;
				...
				DEMOD_PRINT( "[DEMOD H14GBB GBB] NEVERSCAN : LX_DEMOD_NEVERLOCK DETECTION is on initializing \n");
		...
		case LX_DEMOD_DETECTION_PROCESSING:
			{
				...
				if(elapsedTime > _maxTimeOut)
				{
					*pScanProcessing 	= LX_DEMOD_FLAG_FINISH;
					_demodScanLock		= LX_DEMOD_UNLOCKED;
					DEMOD_PRINT( "[DEMOD H14GBB GBB] NEVERSCAN : LX_DEMOD_NEVERLOCK DETECTION is on processing \n");
		...
		case LX_DEMOD_DETECTION_FAIL:
			{
				*pScanProcessing 	= LX_DEMOD_FLAG_FINISH;
				_demodScanLock		= LX_DEMOD_UNLOCKED;
				DEMOD_PRINT( "[DEMOD H14GBB GBB] NEVERSCAN :  LX_DEMOD_NEVERLOCK DETECTION FAIL  \n");
			}
			break;
		...
		case LX_DEMOD_DETECTED:
			{
				*pScanProcessing 	= LX_DEMOD_FLAG_FINISH;
				...
				DEMOD_PRINT( "[DEMOD H14GBB GBB] NEVERSCAN :  LX_DEMOD_NEVERLOCK DETECTED  !! \n");
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_TPOutEnable(BOOLEAN bEnable)
{

	UINT8 	tpouten;

	DEMOD_PRINT(" @DEMOD_H14GBB_TPOutCLKEnable\n");
	...
	DEMOD_TRACE(" DEMOD_H14GBB_TPOutEnable(%s)\n",(bEnable == TRUE) ? "ENABLE":"DISABLE");
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Set_IF_Frq(UINT32 ifFrq)
{
	...
	DEMOD_PRINT(" @DEMOD_H14GBB_Set_IF_Frq\n");
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Set_AGCPolarity(LX_DEMOD_AGC_POLAR_T agcPola)
{

	UINT8 	agcoutpol;

	DEMOD_PRINT(" @DEMOD_H14GBB_DVBT_Set_AGCPolarity\n");
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Set_SpectrumCtrl(BOOLEAN bAutoEnable, BOOLEAN bSpecInvEnable)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_Id(UINT16 *pId)
{
...
************************************************************************************/
/**
 * DEMOD_H14GBB_Get_SpectrumInvStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_SpectrumStatus(LX_DEMOD_LOCK_STATE_T *pSpectrumAuto, LX_DEMOD_LOCK_STATE_T *pSpectrumInv)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_IFAGC(UINT16 *pIfAgc)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_OperMode(LX_DEMOD_OPER_MODE_T *pOperMode)
{
...
************************************************************************************/
/**
 * DEMOD_H14GBB_Get_NeverLockStatus
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_NeverLockStatus(LX_DEMOD_OPER_MODE_T scanMode, LX_DEMOD_LOCK_STATE_T *pNeverLockStatus)
{
	UINT8 neverlock_1_0;
	UINT8 srdstatus_1_0;
	UINT8 tpslockeq_d;
	...
	*pNeverLockStatus = LX_DEMOD_UNKOWN_STATE;
	...
	DEMOD_PRINT("[DEMOD H14GBB GBB](%s) neverlock_1_0 = %d !!!\n", __F__,neverlock_1_0);
	DEMOD_PRINT("[DEMOD H14GBB GBB](%s) srdstatus_1_0 = %d !!!\n", __F__,srdstatus_1_0);
	DEMOD_PRINT("[DEMOD H14GBB GBB](%s) tpslockeq_d = %d !!!\n", __F__,tpslockeq_d);

	if(scanMode == LX_DEMOD_QAM)
	{
		if((neverlock_1_0 == 3) || (srdstatus_1_0 == 3))
		{
			DEMOD_PRINT("[DEMOD H14GBB GBB](%s)neverlock_1_0[%d] srdstatus_1_0[%d]!!!\n", __F__,neverlock_1_0,srdstatus_1_0);
			neverlock_1_0 = 3;
		}
	}
	else if(scanMode == LX_DEMOD_ISDBT)
	{
		...
		{
			DEMOD_PRINT("[DEMOD H14GBB GBB](%s)neverlock_1_0[%d] tpslockeq_d[%d]!!!\n", __F__,neverlock_1_0,tpslockeq_d);
			neverlock_1_0 = 1;
		}
	...
	DEMOD_PRINT("[DEMOD H14GBB GBB](%s) SYSLOCK = %s !!!\n", __F__,(syslock == 1) ? "LOCK" :"NOT LOCK");
...
************************************************************************************/
/**
 * DEMOD_H14GBB_Get_CarrierFreqOffset
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_CarrierFreqOffset(SINT32 *pFreqOff)
{
		...
		*pFreqOff = carrierOffset;
		...
		DEMOD_PRINT("[DEMOD H14GBB GBB] Carrier Offset freq = %d [KHz] !!!\n",*pFreqOff);
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_TPIFStatus(BOOLEAN *pbEnable)
{
	UINT8 	tpifen;
	...
	DEMOD_PRINT("[DEMOD H14GBB GBB] TP IF function is %s!!\n",(*pbEnable == TRUE) ? "Active" : "Inactive");
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_VABER(UINT32 *pVber)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_Packet_Error(UINT32 *pError)
{
...
************************************************************************************/
/**
 * DEMOD_H14GBB_Get_BandWidthMode
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_BandWidthMode( LX_DEMOD_BWMODE_T *ChannelBW )
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_QAMMode( LX_DEMOD_RF_MODE_T *pQamMode )
{
	...
	*pQamMode = LX_DEMOD_UNKOWN_MODE;
	...
	switch(mode)
	{
		...
		default :
			DEMOD_PRINT("[DEMOD H14GBB GBB] FAIL,(%s:%d) UNKNOWN RF MODE !!!\n", __F__, __L__);
			return RET_ERROR;
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_MseInfo( LX_DEMOD_MSE_T *pSigMse)
{
 ...
 * @author 	Jeongpil.yun (jeongpil.yun@lge.com)
*/
int DEMOD_H14GBB_Get_Lock(LX_DEMOD_LOCK_STATUS_T whatLock, LX_DEMOD_LOCK_STATE_T *lockState)
{
...
//	DEMOD_PRINT("[DEMOD H13 DTV] @ %s =====\n",__F__);
	...
	if (RET_OK != DEMOD_H14GBB_Get_OperMode(&operMode))
	{
		DEMOD_PRINT("[DEMOD H14GBB GBB] FAIL (%s:%d) DEMOD_H14GBB_GetOperMode() !!!\n", __F__, __L__);
		...
		case LX_DEMOD_PSYNC_LOCK:
			{
				...
				{
//					DEMOD_H14GBB_DVBT_Get_IFO_LOCK(&ifoLock);
...
int DEMOD_H14GBB_VSB_Get_SysLockTime(UINT16 *pSyslocktime)
{
...
int DEMOD_H14GBB_VSB_Get_FecLockTime(UINT16 *pFeclocktime)
{
