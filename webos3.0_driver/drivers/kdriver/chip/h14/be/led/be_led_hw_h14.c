...
#include "ipc_drv.h"
#include "be_led_hw_h14.h"
#include "../reg/be_reg_def_h14.h"
...
typedef struct {
	...
	UINT8	u8DRC_shp;
	UINT8	_rsvd;
} BE_LED_HDR_HW_H14_INFO_T;

typedef struct {
	UINT32	uWCGMode;
	UINT32	uWCGControl;
} BE_LED_WCG_HW_H14_INFO_T;
...
/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
static BE_LED_HDR_HW_H14_INFO_T _beHDRHwH14Info = {0,};
static BE_LED_WCG_HW_H14_INFO_T _beWCGHwH14Info = {0xFF,0xFF};

/*========================================================================================
	Implementation Group
========================================================================================*/

int BE_LED_HW_H14_Initialize(void)
{
	int ret = RET_OK;
...
int BE_LED_HW_H14_Get_Version(LX_BE_LED_VERSION_T *pstParams)
{
	int ret = RET_OK;
...
int BE_LED_HW_H14_Set_Mode(LX_BE_LED_MODE_T *pstParams)
{
	int ret = RET_OK;
...
int BE_LED_HW_H14_Control(LX_BE_LED_CONTROL_T *pstParams)
{
	int ret = RET_OK;
...
int BE_LED_HW_H14_Get_RegInfo(LX_BE_LED_REG_INFO_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		pstParams->u32CpuBaseAddr = DPPB_LED_REG_H14_BASE;
		pstParams->u32McuBaseAddr = DPPB_LED_REG_H14_BASE + 0x2FFE0000;
		pstParams->regOffset.AD_MEM_ADDR_CLR = offsetof(DPPB_LED_REG_H14A0_T, led_mem_addr_clr);
		pstParams->regOffset.AD_MEM_DL_SBI_HCOEFF = offsetof(DPPB_LED_REG_H14A0_T, led_mem_dl_sbi_hcoeff);
		pstParams->regOffset.AD_MEM_DL_SBI_VCOEFF = offsetof(DPPB_LED_REG_H14A0_T, led_mem_dl_sbi_vcoeff);
		pstParams->regOffset.AD_FC_TAB1 = offsetof(DPPB_LED_REG_H14A0_T, led_fc_tab1);
		pstParams->regOffset.AD_FC_TAB5 = offsetof(DPPB_LED_REG_H14A0_T, led_fc_tab5);
		pstParams->regOffset.AD_UPPER_DC_LUT = offsetof(DPPB_LED_REG_H14A0_T, led_upper_dc_lut);
		pstParams->regOffset.AD_LOWER_DC_LUT = offsetof(DPPB_LED_REG_H14A0_T, led_lower_dc_lut);
		pstParams->regOffset.AD_MEM_DL_PIXEL_COMP_A = offsetof(DPPB_LED_REG_H14A0_T, led_mem_dl_pixel_comp_a);
		pstParams->regOffset.AD_MEM_DL_PIXEL_COMP_B = offsetof(DPPB_LED_REG_H14A0_T, led_mem_dl_pixel_comp_b);
		pstParams->regOffset.AD_MEM_DL_PIXEL_COMP_C = offsetof(DPPB_LED_REG_H14A0_T, led_mem_dl_pixel_comp_c);
		pstParams->regOffset.AD_MEM_DL_PIXEL_COMP_D = offsetof(DPPB_LED_REG_H14A0_T, led_mem_dl_pixel_comp_d);
		pstParams->regOffset.AD_UMASK_WR_COEFF = offsetof(DPPB_LED_REG_H14A0_T, led_lsf_wr_coeff);
	} while (0);

	return ret;
}

int BE_HDR_HW_H14_Get_Version(LX_BE_HDR_VERSION_T *pstParams)
{
	int ret = RET_OK;
...
int BE_HDR_HW_H14_Set_Mode(LX_BE_HDR_MODE_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		if (stHwParams.u4DECMode != 0xF)
			_beHDRHwH14Info.u8DECMode = (UINT8)stHwParams.u4DECMode;

		if (stHwParams.u4DCMode != 0xF)
			_beHDRHwH14Info.u8DCMode	= (UINT8)stHwParams.u4DCMode;

		if (stHwParams.bStore != 0xF)
			_beHDRHwH14Info.u8Store 	= (UINT8)stHwParams.bStore;

		if (stHwParams.bOSD != 0xF)
			_beHDRHwH14Info.u8OSD 	= (UINT8)stHwParams.bOSD;

		if (stHwParams.bDRC_shp != 0xF)
			_beHDRHwH14Info.u8DRC_shp = (UINT8)stHwParams.bDRC_shp;

		if (_beHDRHwH14Info.u8DCBypass)
		{
			if (stHwParams.bDCBypass == LX_BE_HDR_DCR_BYPASS_OFF)
			{
				_beHDRHwH14Info.u8DCBypass = FALSE;

				stHwParams.u4DECMode= _beHDRHwH14Info.u8DECMode;
				stHwParams.u4DCMode	= _beHDRHwH14Info.u8DCMode ;
				stHwParams.bStore 	= _beHDRHwH14Info.u8Store;
				stHwParams.bOSD 		= _beHDRHwH14Info.u8OSD;
				stHwParams.bDRC_shp 	= _beHDRHwH14Info.u8DRC_shp;
			}
			else
			{
				if (stHwParams.bOSD == 0xF)
		...
		else
		{
			if (stHwParams.bDCBypass == LX_BE_HDR_DCR_BYPASS_ON)
				_beHDRHwH14Info.u8DCBypass = TRUE;
...
int BE_HDR_HW_H14_Control(LX_BE_HDR_CONTROL_T *pstParams)
{
	int ret = RET_OK;
...
int BE_WCG_HW_H14_Get_Version(LX_BE_WCG_VERSION_T *pstParams)
{
	int ret = RET_OK;
...
int BE_WCG_HW_H14_Set_Mode(LX_BE_WCG_MODE_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		if ( (stHwParams.u4PictureMode != 0xF) && (_beWCGHwH14Info.uWCGMode != pstParams->u32PictureMode) )
			_beWCGHwH14Info.uWCGMode = pstParams->u32PictureMode;
		else
...
int BE_WCG_HW_H14_Control(LX_BE_WCG_CONTROL_T *pstParams)
{
	int ret = RET_OK;
	...
	do {
		...
		if ( (stHwParams.bEnable != 0xF) && (_beWCGHwH14Info.uWCGControl != pstParams->u32Enable) )
			_beWCGHwH14Info.uWCGControl = pstParams->u32Enable;
		else
