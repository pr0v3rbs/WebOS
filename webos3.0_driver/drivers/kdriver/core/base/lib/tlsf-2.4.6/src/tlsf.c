 ...
 * - Used tlsf_ls_bit instead of ffs and tlsf_ms_bit instead of fls. I did this to 
 *   avoid confusion with the standard ffs function which returns 
 *   different values.
 * - Created tlsf_set_bit/tlsf_clear_bit fuctions because they are not present 
 *   on x86_64.
...
/*#define TLSF_USE_SBRK        (0) */
/*#define TLSF_USE_MMAP        (0) */
...
#ifndef TLSF_USE_MMAP
#define	TLSF_USE_MMAP 	(0)
#endif

#ifndef TLSF_USE_SBRK
#define	TLSF_USE_SBRK 	(0)
#endif
...
#if TLSF_USE_MMAP || TLSF_USE_SBRK
#include <unistd.h>
#endif
...
static __inline__ int tlsf_ls_bit(int x);
static __inline__ int tlsf_ms_bit(int x);
...
#if TLSF_USE_SBRK || TLSF_USE_MMAP
...
static __inline__ int tlsf_ls_bit(int i)
{
    unsigned int a;
    unsigned int x = i & -i;
...
static __inline__ int tlsf_ms_bit(int i)
{
    unsigned int a;
...
static __inline__ void MAPPING_SEARCH(size_t * _r, int *_fl, int *_sl)
{
    ...
    if (*_r < SMALL_BLOCK) {
        ...
        *_sl = *_r / (SMALL_BLOCK / MAX_SLI);
        ...
        _t = (1 << (tlsf_ms_bit(*_r) - MAX_LOG2_SLI)) - 1;
        *_r = *_r + _t;
        *_fl = tlsf_ms_bit(*_r);
...
static __inline__ bhdr_t *FIND_SUITABLE_BLOCK(tlsf_t * _tlsf, int *_fl, int *_sl)
{
    ...
    if (_tmp) {
        *_sl = tlsf_ls_bit(_tmp);
        _b = _tlsf->matrix[*_fl][*_sl];
    } else {
        *_fl = tlsf_ls_bit(_tlsf->fl_bitmap & (~0 << (*_fl + 1)));
        if (*_fl > 0) {         /* likely */
            *_sl = tlsf_ls_bit(_tlsf->sl_bitmap[*_fl]);
            _b = _tlsf->matrix[*_fl][*_sl];
        }
    }
    return _b;
