...
/*----------------------------------------------------------------------------------------
	Static Variables
----------------------------------------------------------------------------------------*/
static OS_PROC_DESC_TABLE_T	_g_ci_device_proc_table[] =
{
 ...
 *
*/
static int	_CI_ReadProcFunction(	UINT32 procId, char* buffer )
{
	int		ret;
	...
	switch( procId )
	{
		case PROC_ID_AUTHOR:
		{
			ret = sprintf( buffer, "%s\n", "hwajeong.lee (hwajeong.lee@lge.com)" );
		}
		break;
 ...
 *
*/
static int _CI_WriteProcFunction( UINT32 procId, char* command )
{
	/* TODO: add your proc_write implementation */
	switch( procId )
	{
...
/**
 * initialize proc utility for ci device
 *
 * @see CI_Init
*/
void	CI_PROC_Init (void)
{
	OS_PROC_CreateEntryEx ( CI_MODULE, _g_ci_device_proc_table,
											_CI_ReadProcFunction,
											_CI_WriteProcFunction );
}
 ...
 * @see CI_Cleanup
*/
void	CI_PROC_Cleanup (void)
{
	OS_PROC_RemoveEntry( CI_MODULE );
}
