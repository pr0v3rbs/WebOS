...
typedef struct _GPIO_Bus_Data
{
    MS_U16 u16byte;
    MS_U32 padSCL;
    MS_U32 padSDA;
    MS_U32 u32LoopDelay; // Delay Count
    MS_BOOL ack;
    MS_U8 *pBuf; //send
    MS_U16 u16Retry; //retry
    MS_BOOL *bRet;
}GPIO_Bus_Data;

typedef enum _GPIO_Mode_Str
{
    E_GPIO_STR_NONE,
    E_GPIO_STR_SetHigh,
    E_GPIO_STR_SetLow,
...
MS_BOOL _mdrv_gpio_send_bus_data(GPIO_Bus_Data *pBusData);
MS_BOOL _mdrv_gpio_get_bus_data(GPIO_Bus_Data *pBusData);
void _mdrv_gpio_bus_stop(GPIO_Bus_Data *pBusData);   // Stop
