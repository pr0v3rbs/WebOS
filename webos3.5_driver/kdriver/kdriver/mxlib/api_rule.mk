...
CFG_DEFS += -DSUPPORT_MSVP9=1
else
CFG_DEFS += -DSUPPORT_MSVP9=0
endif

.PHONY : all clean $(DRV_DIR) depend

$(DRV_DIR): | $(API_OBJ) $(API_IMPORT_OBJ)

all : $(API_OBJ) $(API_IMPORT_OBJ) $(DRV_DIR)

$(API_OBJ) $(API_IMPORT_OBJ) $(DRV_DIR) : | setup

lib : setup all


ifeq ($(LINK_TYPE),dynamic)
	$(CC) $(CFG_CC_OPTS) $(LD_OPTS) $(wildcard $(OBJ_PATH)/$(T_DIR_NAME)/*.T) $(API_OBJ) $(API_IMPORT_OBJ) -o $(DDI_PATH)/lib$(LIB_NAME).$(LIB_EXT) ;
	@rm -rf $(OBJ_PATH)/$(T_DIR_NAME)
endif
...
ifneq ($(APPEND_STRING),FALSE)
	...
	echo "++MSTAR::SW_VERSION_API_$(API_NAME)::$(SW_VERSION_API_$(API_NAME))::MSTAR++" >> $(DDI_PATH)/lib$(LIB_NAME).$(LIB_EXT)
endif
endif
endif
endif
...
$(DRV_DIR) :
ifeq ($(LINK_TYPE),dynamic)
	@$(MAKE) -j$(JOBS) BUILDING_API=1 -s -C $(DDIDRV)/$@
else
	@$(MAKE) -j$(JOBS) -C $(DDIDRV)/$@
endif
