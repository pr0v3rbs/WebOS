...
void MDrv_UART_Init(UART_DEVICE_TYPE uart_dev, MS_U32 baudrate)
{
    ...
    _uart_dev[E_UART_AEON_R2].device.flags = 0 ;
    ...
    _uart_dev[E_UART_SECURE_R2].device.flags = 0 ;
...
MS_U16 mdrv_uart_close(MS_U32 hUart)
{
    struct device *dev;
    ...
    if (hUart)
    {
        dev = &((ms_uart_dev_t)hUart)->device;
        if (dev->devio->close)
        {
...
MS_U16 mdrv_uart_set_rx_callback(MS_U32 hUart, ms_uart_rx_callback rx_cb)
{
    struct device *dev;
    ...
    if (hUart)
    {
        ...
        if (dev->devio->ioctl)
        {
            return dev->devio->ioctl((device_t)dev, IOCTL_SET_RXCALLBACK, (MS_U32)&rx_cb);
        }
    }
    _uartStatus.HwBusy = FALSE;
...
MS_U16 mdrv_uart_set_rx_callback_halreg(MS_U32 hUart, ms_uart_rx_callback_halreg rx_cb)
{
    struct device *dev;
    ...
    if (hUart)
    {
        ...
        if (dev->devio->ioctl)
        {
            return dev->devio->ioctl((device_t)dev, IOCTL_SET_RXCALLBACK_HALREG, (MS_U32)&rx_cb);
        }
    }
    _uartStatus.HwBusy = FALSE;
...
MS_U16 mdrv_uart_set_tx_buffer(MS_U32 hUart, void *tx_buffer, MS_U16 len)
{
    _uartStatus.HwBusy = TRUE;
    if (hUart)
    {
...
#endif
        ((ms_uart_dev_t)hUart)->tx_buf = tx_buffer;
        ((ms_uart_dev_t)hUart)->tx_buf_in = 0;
        ((ms_uart_dev_t)hUart)->tx_buf_out = 0;
        ((ms_uart_dev_t)hUart)->tx_buf_len = len;
#ifdef MSOS_TYPE_NOS
...
MS_U16 mdrv_uart_set_rx_buffer(MS_U32 hUart, void *rx_buffer, MS_U16 len)
{
    _uartStatus.HwBusy = TRUE;
    if (hUart)
    {
        ...
        MsOS_CPU_LockInterrupt();
#endif
        ((ms_uart_dev_t)hUart)->rx_buf = rx_buffer;
        ((ms_uart_dev_t)hUart)->rx_buf_in = 0;
        ((ms_uart_dev_t)hUart)->rx_buf_out = 0;
        ((ms_uart_dev_t)hUart)->rx_buf_len = len;
#ifdef MSOS_TYPE_NOS
        MsOS_CPU_UnLockInterrupt();
#endif
    }
...
MS_U16 mdrv_uart_rx_enable(MS_U32 hUart)
{
    _uartStatus.HwBusy = TRUE;
    if (hUart)
    {
        ((ms_uart_dev_t)hUart)->rx_flags &= ~RX_FLAG_DISABLED;
        return 0;
    }
    _uartStatus.HwBusy = FALSE;
...
MS_U16 mdrv_uart_tx_enable(MS_U32 hUart)
{
    _uartStatus.HwBusy = TRUE;
    if (hUart)
    {
        ((ms_uart_dev_t)hUart)->tx_flags &= ~TX_FLAG_DISABLED;
        return 1;
    }
    _uartStatus.HwBusy = FALSE;
...
MS_U16 mdrv_uart_rx_disable(MS_U32 hUart)
{
    _uartStatus.HwBusy = TRUE;
    if (hUart)
    {
...
MS_U16 mdrv_uart_tx_disable(MS_U32 hUart)
{
    _uartStatus.HwBusy = TRUE;
    if (hUart)
    {
...
MS_U16 mdrv_uart_write(MS_U32 hUart, const MS_U8 *buf, MS_U16 len)
{
    struct device *dev;
...
MS_U16 mdrv_uart_read(MS_U32 hUart, MS_U8 *buf, MS_U32 len)
{
    struct device *dev;
...
MS_U16 mdrv_uart_set_baudrate(MS_U32 hUart, MS_U32 baudrate)
{
    struct device *dev;
    ...
    if (hUart)
    {
        ...
        if (dev->devio->ioctl)
        {
            return dev->devio->ioctl((device_t)dev, IOCTL_SET_BAUDRATE, (MS_U32)&baudrate);
        }
    }
    _uartStatus.HwBusy = FALSE;
    _gu32baudrate = baudrate;
...
MS_U16 mdrv_uart_connect_mux(UART_DEVICE_TYPE uart_dev, UART_PORT_TYPE uart_port, UART_PAD_TYPE uart_pad)
{
	...
	if (u16Ret == FALSE)
	{
		printf("Error: %s, uart_connect fail\n", __FUNCTION__);
	}
	...
	if (u16Ret == FALSE)
	{
		printf("Error: %s, uart set port pad fail\n", __FUNCTION__);
	}
...
void Eywa_UART_RecvHandler(MS_U8 c)
{
	...
	//if( c == EYWA_PACKET_ID )
	{
		...
		_EywaPacket.count = 7;
		printf("[Eywa]packet.payload[0] = %x\n", _EywaPacket.payload[0]);
	}
	else
	{
		if( _EywaPacket.count > 0 )
		{
			...
			_EywaPacket.payload[inx] = c;
			printf("[Eywa]packet.payload[%d] = %x\n", inx, _EywaPacket.payload[inx]);
			_EywaPacket.count -= 1;
		}
#if 0
		if(_EywaPacket.payload[7])
		{
			printf("[Eywa]packet.data_type = %x\n", _EywaPacket.data_type);
			printf("[Eywa]packet.data_type_inv = %x\n", _EywaPacket.data_type_inv);
			printf("[Eywa]packet.len = %x\n", _EywaPacket.len);
			for(inx = 0; inx < 8; inx++)
			{
				printf("[Eywa]packet.payload[%d] = %x\n", inx, _EywaPacket.payload[inx]);
			}
			printf("[Eywa]packet.parity = %x\n", _EywaPacket.parity);
		}
#endif
	}
#endif
	...
	//mdrv_uart_set_rx_callback(g_EywaUART, Eywa_UART_RecvHandler);
		...
		while(1)
		{
			...
			if(p_buf[0]==0x52)
			{
				...
				{
					printf("[Eywa]packet checksum error!!\n");
				}
