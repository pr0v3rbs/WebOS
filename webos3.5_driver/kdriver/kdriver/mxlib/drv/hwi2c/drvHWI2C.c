...
static MS_U32 g_u32StartDelay = HWI2C_STARTDLY, g_u32StopDelay = HWI2C_STOPDLY;
...
static MS_BOOL _MDrv_HWI2C_ReadBytes(MS_U8 u8Port, MS_U16 u16SlaveCfg, MS_U32 uAddrCnt, MS_U8 *pRegAddr, MS_U32 uSize, MS_U8 *pData)
{
    ...
    if (!uSize)
    {
        ...
        //port 1 ursa stop command data size is 0
        if(u8Port != 1)
            return FALSE;
    }
    ...
    {
        if(gp_HWI2CinitCfg->sCfgPort[u8Port].u32PAlen>uSize)
        {
            _geState = E_HWI2C_DMA_READ_DATA;
            return HAL_HWI2C_DMA_ReadBytes(u32Offset, u16SlaveCfg, uAddrCnt, pRegAddr, uSize, pData);
        }
        HWI2C_DBG_INFO("HWIIC Read Size = %lu > MiuSize = %lu , go to polling mode\n",uSize,gp_HWI2CinitCfg->sCfgPort[u8Port].u32PAlen);
    }
    ...
    while (u16Dummy--)
    {
        if((g_HWI2CReadMode[u8Port]!=E_HWI2C_READ_MODE_DIRECT) && (uAddrCnt>0)&& (pRegAddr))
        {
            ...
            MsOS_DelayTaskUs_Poll(g_u32StartDelay);
            ...
            if(g_HWI2CReadMode[u8Port]==E_HWI2C_READ_MODE_DIRECTION_CHANGE_STOP_START)
            {
                ...
                //add extral delay by user configuration
                MsOS_DelayTaskUs_Poll(g_u32StopDelay);
            }
        }
...
static MS_BOOL _MDrv_HWI2C_WriteBytes(MS_U8 u8Port, MS_U16 u16SlaveCfg, MS_U32 uAddrCnt, MS_U8 *pRegAddr, MS_U32 uSize, MS_U8 *pData)
{
    ...
    if(g_bDMAEnable[u8Port])
    {
        ...
        HWI2C_DBG_INFO("HWIIC Write Size = %lu > MiuSize = %lu , go to polling mode\n",uSize,gp_HWI2CinitCfg->sCfgPort[u8Port].u32PAlen);
    }
...
{
    MS_BOOL bRet = TRUE;
    MS_U32 u32BaseAddr = 0xFFFFFFFF, u32BaseSize;
    MS_U8 u8Port=0;
	...
	gp_HWI2CinitCfg->sCfgPort[3] = psCfg->sCfgPort[3];
        gp_HWI2CinitCfg->sCfgPort[4] = psCfg->sCfgPort[4];
    if (_gbInit)
    {
...
#ifdef MSOS_TYPE_LINUX_KERNEL //(### REVIEW ###)
    bRet &= HAL_HWI2C_Init_Chip(ARM_MIU0_BUS_BASE);
    //#if 1//(### REVIEW ###)
    //if(0) //(HWI2C_DMA_BUF_ADDR > 0x20000000)
    #ifdef M2_NSS
        HAL_Set_MIU_Size(0x10000000); //MIU0 is 256MB
    #else
        HAL_Set_MIU_Size(0x40000000); //MIU0 is 1024MB
    #endif
#else
...
MS_BOOL MDrv_HWI2C_WriteBytes_U2K_CUST(MS_U16 u16SlaveCfg, MS_U32 uAddrCnt, MS_U8 *pRegAddr, MS_U32 uSize, MS_U8 *pData
                                     , HWI2C_PORT ePort, HWI2C_CLKSEL eClk)
{
    MS_BOOL bRet;
    MS_U8 u8Port;
    ...
    //Set Port and HWIIC reset
    MDrv_HWI2C_SelectPort_U2K(ePort);

    //Set Clk
    _MDrv_HWI2C_SetClk(u8Port, eClk);
...
MS_BOOL MDrv_HWI2C_ReadBytes_U2K_CUST(MS_U16 u16SlaveCfg, MS_U32 uAddrCnt, MS_U8 *pRegAddr, MS_U32 uSize, MS_U8 *pData
                                    , HWI2C_PORT ePort, HWI2C_CLKSEL eClk)
{
    MS_BOOL bRet;
    MS_U8 u8Port;
...
MS_BOOL MDrv_HWI2C_WriteBytesP1(MS_U16 u16SlaveCfg, MS_U32 uAddrCnt, MS_U8 *pRegAddr, MS_U32 uSize, MS_U8 *pData)
{
    ...
    bRet = MDrv_HWI2C_WriteBytes(HWI2C_PORT1<<8|u16SlaveCfg,uAddrCnt,pRegAddr,uSize,pData);
...
MS_BOOL MDrv_HWI2C_ReadBytesP1(MS_U16 u16SlaveCfg, MS_U32 uAddrCnt, MS_U8 *pRegAddr, MS_U32 uSize, MS_U8 *pData)
{
    ...
    HWI2C_MUTEX_LOCK(HWI2C_PORT1);
    bRet = MDrv_HWI2C_ReadBytes(HWI2C_PORT1<<8|u16SlaveCfg,uAddrCnt,pRegAddr,uSize,pData);
    HWI2C_MUTEX_UNLOCK(HWI2C_PORT1);
    return bRet;
}
...
MS_BOOL  MDrv_HWI2C_ConfigDelay(MS_U32 u32StartDelay, MS_U32 u32StopDelay)
{
    ...
    {
        if(u32StartDelay==0)
            printf("START Delay 0(us) will cause accessing error !\n");
        else
    ...
    {
        if(u32StopDelay==0)
            printf("STOP Delay 0(us) will cause accessing error !\n");
        else
            HWI2C_DBG_ERR("STOP Delay %d(us) will risk accessing error !\n",(int)u32StopDelay);
    }
    g_u32StartDelay = u32StartDelay;
    g_u32StopDelay = u32StopDelay;
...
void MDrv_HWI2C_Copy_STR(HWI2C_UnitCfg* pUnitCfg)
{
	if( _gbInit != TRUE ){
		HWI2C_DBG_INFO("\nI2C doesn't init\n");
		return;
	...
	HWI2C_DBG_INFO("\nI2C already init\n");
	memcpy(pUnitCfg,gp_HWI2CinitCfg,sizeof(HWI2C_UnitCfg));
}
...
MS_BOOL MDrv_HWI2C_WriteBytes(MS_U16 u16SlaveCfg, MS_U32 uAddrCnt, MS_U8 *pRegAddr, MS_U32 uSize, MS_U8 *pData)
{
    ...
    pHwi2cParam_WriteBytes->bReturn = &bResult;

    u32Ret = UtopiaIoctl(pInstantHwi2c,MDrv_CMD_HWI2C_WriteBytes,(MS_U32*)pHwi2cParam_WriteBytes);
...
MS_BOOL MDrv_HWI2C_ReadBytes(MS_U16 u16SlaveCfg, MS_U32 uAddrCnt, MS_U8 *pRegAddr, MS_U32 uSize, MS_U8 *pData)
{
    ...
    pHwi2cParam_ReadBytes->bReturn = &bResult;

    u32Ret = UtopiaIoctl(pInstantHwi2c,MDrv_CMD_HWI2C_ReadBytes,(MS_U32*)pHwi2cParam_ReadBytes);
...
MS_BOOL MDrv_HWI2C_WriteBytes_Customer(MS_U16 u16SlaveCfg, MS_U32 uAddrCnt, MS_U8 *pRegAddr, MS_U32 uSize, MS_U8 *pData
                                     , HWI2C_PORT ePort, HWI2C_CLKSEL eClk)
{
#if (HWI2C_UTOPIA20)
    MS_U32 u32Ret;
    MS_BOOL bResult = 1;
    HWI2C_PRIVATE_PARAM_WriteBytes_CUST Hwi2cParam_WriteBytes_Cust ;

    if (FALSE == _MDrv_HWI2C_CheckUtopia20Open(&pInstantHwi2c, 0, pAttributeHwi2c))
        return 0;

    //pHwi2cParam_WriteBytes = (PHWI2C_PRIVATE_PARAM_WriteBytes)malloc(sizeof(HWI2C_PRIVATE_PARAM_WriteBytes_CUST));
    Hwi2cParam_WriteBytes_Cust.u16SlaveCfg=u16SlaveCfg;
    Hwi2cParam_WriteBytes_Cust.uAddrCnt=uAddrCnt;
    Hwi2cParam_WriteBytes_Cust.pRegAddr=pRegAddr;
    Hwi2cParam_WriteBytes_Cust.uSize=uSize;
    Hwi2cParam_WriteBytes_Cust.pData=pData;
    Hwi2cParam_WriteBytes_Cust.ePort = ePort;
    Hwi2cParam_WriteBytes_Cust.eClk = eClk;
    Hwi2cParam_WriteBytes_Cust.bReturn = &bResult;

    u32Ret = UtopiaIoctl(pInstantHwi2c,MDrv_CMD_HWI2C_WriteBytes_CUST,(MS_U32*)&Hwi2cParam_WriteBytes_Cust);

    //free(pHwi2cParam_WriteBytes);

    return (MS_BOOL)bResult;
#else
    return MDrv_HWI2C_WriteBytes_U2K_CUST(u16SlaveCfg,uAddrCnt,pRegAddr,uSize,pData, ePort, eClk);
#endif
}

MS_BOOL MDrv_HWI2C_ReadBytes_Customer(MS_U16 u16SlaveCfg, MS_U32 uAddrCnt, MS_U8 *pRegAddr, MS_U32 uSize, MS_U8 *pData
                                    , HWI2C_PORT ePort, HWI2C_CLKSEL eClk)
{
#if (HWI2C_UTOPIA20)
    MS_U32 u32Ret;
    MS_BOOL bResult = 1;
    HWI2C_PRIVATE_PARAM_ReadBytes_CUST Hwi2cParam_ReadBytes_CUST;

    if (FALSE == _MDrv_HWI2C_CheckUtopia20Open(&pInstantHwi2c, 0, pAttributeHwi2c))
        return 0;
    //pHwi2cParam_ReadBytes = (PHWI2C_PRIVATE_PARAM_ReadBytes)malloc(sizeof(HWI2C_PRIVATE_PARAM_ReadBytes_CUST));
    Hwi2cParam_ReadBytes_CUST.u16SlaveCfg=u16SlaveCfg;
    Hwi2cParam_ReadBytes_CUST.uAddrCnt=uAddrCnt;
    Hwi2cParam_ReadBytes_CUST.pRegAddr=pRegAddr;
    Hwi2cParam_ReadBytes_CUST.uSize=uSize;
    Hwi2cParam_ReadBytes_CUST.pData=pData;
    Hwi2cParam_ReadBytes_CUST.ePort = ePort;
    Hwi2cParam_ReadBytes_CUST.eClk = eClk;
    Hwi2cParam_ReadBytes_CUST.bReturn = &bResult;

    u32Ret = UtopiaIoctl(pInstantHwi2c,MDrv_CMD_HWI2C_ReadBytes_CUST,(MS_U32*)&Hwi2cParam_ReadBytes_CUST);

    //free(pHwi2cParam_ReadBytes);

    return (MS_BOOL)bResult;
#else
    return MDrv_HWI2C_ReadBytes_U2K_CUST(u16SlaveCfg,uAddrCnt,pRegAddr,uSize,pData, ePort, eClk);
#endif
}
...
MS_U16 MDrv_HWI2C_SetPowerState_U2K(HWI2C_UnitCfg *psCfg, EN_POWER_MODE u32PowerState, void* pModule)
{
	MS_U16 u16Return = FALSE;
#if defined(MSOS_TYPE_LINUX_KERNEL)
		...
		case E_POWER_RESUME:
			{
				if(_gbInit == TRUE)
				{
					HWI2C_DBG_INFO("\nSTD I2C Resume Begin\n");
					_gbInit = FALSE;
					HAL_Resume_Flag(TRUE);
					MDrv_HWI2C_Init_U2K(psCfg);
				}
				u16Return = TRUE;
			}
		break;
...
int resume_i2c(void)
{
    if(_gbInit == TRUE)
    {
        ...
        #endif
        _gbInit = FALSE;
        MDrv_HWI2C_Init_U2K(gp_HWI2CinitCfg);
    }
    return 0;
}
