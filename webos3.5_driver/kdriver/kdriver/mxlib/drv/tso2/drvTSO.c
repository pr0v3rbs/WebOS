...
//  Local Defines
//-------------------------------------------------------------------------------------------------
#define TSO_MUTEX_WAIT_TIME                            0xFFFFFFFF
...
TSO_Result MDrv_TSO_InitLibResource(void* pResMemAddr)
{
    ...
    if(_bBankInit == FALSE)
    {
        ...
        {
            TSO_DBGERR(printf("MDrv_TSO_GetBASE failed\n"));
            return DRVTSO_FAIL;
        }
...
TSO_Result MDrv_TSO_Init(void)
{
    ...
    {
        TSO_DBGERR(printf("MsOS_CreateMutex failed\n"));
        return DRVTSO_FAIL;
    }
...
TSO_Result MDrv_TSO_Exit(void)
{
    ...
    MS_U16 u16Cfg = TSO_CFG1_TSO_OUT_EN|TSO_CFG1_TSO_TSIF1_EN|TSO_CFG1_TSO_TSIF5_EN|TSO_CFG1_TSO_TSIF6_EN;
#else
    MS_U8 u8TsIf[TSO_TSIF_NUM] = {HAL_TSO_TSIF_LIVE1, HAL_TSO_TSIF_LIVE2};
    MS_U16 u16Cfg = TSO_CFG1_TSO_OUT_EN|TSO_CFG1_TSO_TSIF1_EN|TSO_CFG1_TSO_TSIF5_EN;
#endif
    MS_BOOL bFlag = FALSE;
    MS_U16 u16Data = 0;
    ...
    {
        HAL_TSO_Cfg1_Enable(u8jj, u16Cfg, FALSE);
...
#endif
        default:
            printf("Not supported TSOCtrlMode = %d! \n", TSOCtrlMode);
...
TSO_Result MDrv_TSO_SetOutClk(MS_U8 u8Eng, DrvTSO_OutClk eOutClk, DrvTSO_OutClkSrc eOutClkSrc, MS_U16 u16DivNum, MS_BOOL bClkInv)
{
    ...
    //printf("[%s] eOutClk %x, eOutClkSrc %x, u16DivNum %x\n", __FUNCTION__, eOutClk,  eOutClkSrc, u16DivNum);
...
TSO_Result MDrv_TSO_PidFlt_Alloc(MS_U8 u8Eng, MS_U16 *pu16PidFltId)
{
    ...
    *pu16PidFltId = 0xFF;
    ...
    for(u16ii = 0; u16ii < TSO_PIDFLT_NUM; u16ii++)
    {
        ...
        {
            TSO_DBGERR(printf("Allocate PID Filter %d\n", u16ii));
...
TSO_Result MDrv_TSO_SetPacketMode(MS_U8 u8FileEng, DrvTSO_PacketMode ePktMode)
{
    ...
    MS_U16 u16ValidCount = 0xBC; //0xBB;
    MS_U16 u16InValidCount = 20 ; //0x0;
    MS_U8 u8Size = 0;
...
{
    TSO_DBGERR(printf("[%d]%s\n", __LINE__, __FUNCTION__));
    TSO_Result eRes = DRVTSO_FAIL;
    MS_U16 u16OpifValue = 0;
    if(pstTsoCfg->bSet == TRUE)
    {
        _TSO_ENTRY();
        switch(pstTsoCfg->eCfgCmd)
        {
            case E_DRVTSO_CFG_OPIF:
                u16OpifValue = (pstTsoCfg->u32CmdData0 & 0xFFFF);
    ...
    {// Get
        switch(pstTsoCfg->eCfgCmd)
        {
            case E_DRVTSO_CFG_OPIF:
                HAL_TSO_RW_OutputPktSize((MS_U8)pstTsoCfg->u32TsoEng, pstTsoCfg->bSet, &u16OpifValue);
                pstTsoCfg->u32CmdData0 = (MS_U32)u16OpifValue;
                eRes = DRVTSO_OK;
                break;
            default:
                printf("Not supported TSO configure Cmd = 0x%x! \n", pstTsoCfg->eCfgCmd);
...
TSO_Result MDrv_TSO_Filein_Stop(MS_U8 u8FileEng)
{
    ...
    #else
    MS_U8 u8TsIf = HAL_TSO_TSIF_FILE1;
    #endif
...
TSO_Result MDrv_TSO_SelPad(MS_U8 u8Eng, DrvTSO_If eIf, DrvTSO_If_Set* pIfSet)
{
    ...
    HAL_TSO_ChIf_Cfg(u8Eng, u8ChId, TSO_CHCFG_TS_SIN_C0, (pIfSet->bParallel == FALSE));
