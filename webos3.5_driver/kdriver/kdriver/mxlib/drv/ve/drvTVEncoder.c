...
typedef struct
{
    ...
    MS_PHY  u32MiuBaseAddr;  //base address without miu offset
...
#define MODULE_TVENCODER MODULE_TVENCODER|KERNEL_MODE
#endif
...
void MDrv_VE_SetMemoryBaseAddr(MS_PHY phyMIUAddress, MS_U32 u32MIUSize)
{
    ...
    g_VEInfo.u32MiuBaseAddr = phyStartOffset;
...
void MDrv_VE_Init(MS_PHYADDR u32MIUAddress)
{
     ...
     */
    if (u32MIUAddress == (MS_PHYADDR)NULL)
    {
        return;
    }
...
void _MDrv_VE_SetOutputCtrl(PMS_VE_Output_Ctrl pOutputCtrl)
{
    ...
    if(pOutputCtrl->bEnable)
    {
        ...
        if((g_VEInfo.InputSrcType == MS_VE_SRC_DTV_FROM_MVOP)
            || (g_VEInfo.InputSrcType == MS_VE_SRC_DTV_FROM_MVOP2))
        {
            Hal_VE_set_field_inverse(TRUE);
        }
        ...
        //For capture mode, make sure the capture window Hstart/Vstart can restore to original setting.
        //Otherwise, if VE power turn off and turn on, the capture window H/V start/size is set to (0x10,0x7FF,0x0,0x7FF)
        MDrv_VE_set_capture_window();
    }
    else
    {
    ...
    if(u16Out_Hsize != u16In_Hsize)
    {
        if(u16In_Hsize > u16Out_Hsize)
        {
            ...
            else
            {
                ...
                if((u16In_Hsize >= 0xF00)
                        &&(u16In_Vsize >= 0x870)
                        &&(g_VEInfo.u16inputVfreq >= 330))
                {
                    u32H_Ratio*=2;
                }
...
void MDrv_VE_set_capture_window(void)
{
    ...
    if(bVECManualCapWindow == FALSE) //For normal AV output case
    {
        ...
        if(g_VEInfo.u16V_CapSize < u16Out_Vsize)
        {
            ...
            u16Tmp = u16Out_Vsize - g_VEInfo.u16V_CapSize;
            if( (g_VEInfo.u16V_CapStart*2) < u16Tmp)
                u16Tmp = g_VEInfo.u16V_CapStart*2;
    ...
    */
    ...
    {
        if((g_VEInfo.u16H_CapSize >= 0xF00)
                &&(u16V_CapSize >= 0x870)
                &&(g_VEInfo.u16inputVfreq >= 330))
        {
...
VE_Result MDrv_VE_set_crop_window(MS_VE_WINDOW_TYPE stCropWin)
{
    ...
    phyMIUOffsetAddr += g_VEInfo.u32MiuBaseAddr;
...
void _MDrv_VE_SetMode(MS_VE_Set_Mode_Type *pVESetMode)
{
    ...
    g_VEInfo.u16inputVfreq = pVESetMode->u16InputVFreq;
...
VE_Result MDrv_VE_SetFrameLock(MS_VE_PLL_InCfg *pInCfg, MS_BOOL bEnable)
{
    ...
    if(pInCfg != NULL)
    {
        ...
        MSG_DRV_VE(printf("[VE_PLL] InCfg.u16HttIn=0x%x \t InCfg.u16VttIn=0x%x \t InCfg.u16HdeIn=0x%x \t InCfg.u16VdeIn=0x%x \t InCfg.u16Hde_St=0x%x \t InCfg.u16Vde_St=0x%x\n", _stVE_PllInfo.InCfg.u16HttIn, _stVE_PllInfo.InCfg.u16VttIn, _stVE_PllInfo.InCfg.u16HdeIn, _stVE_PllInfo.InCfg.u16VdeIn, _stVE_PllInfo.InCfg.u16Hde_St, _stVE_PllInfo.InCfg.u16Vde_St);)
        MSG_DRV_VE(printf("[VE_PLL] 32FreqIn=0x%lx \t u32FreqOut=0x%lx \n",u32FreqIn,u32FreqOut);)
        MSG_DRV_VE(printf("[VE_PLL] u32InitPll=0x%lx \n",_stVE_PllInfo.u32InitPll);)
...
VE_Result _MDrv_VE_Adjust_FrameStart(MS_VE_WINDOW_TYPE *stSrcWin,MS_S16 pixel_offset)
{
    ...
    if((stSrcWin) && (g_VEInfo.VideoSystem >= MS_VE_PAL_N )&&(g_VEInfo.VideoSystem <= MS_VE_PAL ) )   //PAL mode
    {
        if(!(((stSrcWin->width==704) && (stSrcWin->height==576)) &&
                (g_VEInfo.InputSrcType == MS_VE_SRC_DI)) )
        {
            MS_PHY u32AlignAddr = g_VEInfo.u32MiuBaseAddr + 16;
...
{
    MSG_DRV_VE(printf("Enable = %d, wss525data = %lu\n", bEn, u32WSSData));
#if 0
...
MS_U32 _MDrv_TVE_SetPowerState(EN_POWER_MODE u16PowerState)
{
    ...
    else if (u16PowerState == E_POWER_RESUME)
    {
        ...
        if(g_VEInfo.bMemAddrInMIU1)
        {
            u32MIUAddress = g_VEInfo.u32MiuBaseAddr + HAL_MIU1_BASE;
        }
        else
        {
            u32MIUAddress = g_VEInfo.u32MiuBaseAddr;
        }
