...
#define SC_WAIT_DELAY               20//20//50// TFCA //[TODO] tunable
#define SC_NULL_DELAY               10//500
#define SC_WRITE_TIMEOUT            500
#define SC_RST_HOLD_TIME            14//10                                  // Smart card reset should pull-low for 400 cycle
#define SC_RST_SWITCH_TIME          60//100
#define SC_CARD_STABLE_TIME         10//100

#define SC_T0_ATR_TIMEOUT           100
#define SC_T0_SEND_TIMEOUT          400//50

// add this  for TFCA some command will timeout
#define SC_T0_RECV_TIMEOUT          400//80
//add this for T14
#define SC_T14_RECV_TIMEOUT         400

#define SC_T0_RECV_RETRY_COUNT      (2000 / SC_T0_RECV_TIMEOUT)//(2000 / SC_T0_RECV_TIMEOUT)
#define SC_T14_ATR_TIMEOUT          100
#define SC_T14_SEND_TIMEOUT         100
...
static SC_Result _SC_GetATR(MS_U8 u8SCID, MS_U32 u32TimeoutMs)
{
    ...
    _SC_Recv(u8SCID, _scInfo[u8SCID].pu8Atr, &(_scInfo[u8SCID].u16AtrLen), u32TimeoutMs);
...
static SC_Result _SC_T1SendRecvBlock( MS_U8 u8SCID,MS_U8 *pu8Block, MS_U16 *pu16BlockLen, MS_U8 *pu8RspBlock, MS_U16 *pu16RspBlockLen)
{
    ...
    //receive data
    // wyc.rel-130516, divide the read flow in two parts. first read Prologue Field (3 bytes) and then the other (pu8RspBlock[2]+1)
#if 1
    //receive data
    ret_Result = _SC_Recv(u8SCID, pu8RspBlock, &PrologueSize, SC_T1_RECV_TIMEOUT);
        if(( PrologueSize >= 3 )) // Got length byte
    {
        if( pu8RspBlock[2] != 0 )
        {
            //printf("re-config, ori:%d. new:%d. Pro:%d\n",*pu16RspBlockLen, pu8RspBlock[2]+1, PrologueSize);
            *pu16RspBlockLen = pu8RspBlock[2]+1;
        }
        else
        {
            *pu16RspBlockLen = 1;
        }
        ret_Result = _SC_Recv(u8SCID, pu8RspBlock+3, pu16RspBlockLen, SC_T1_RECV_TIMEOUT);
        *pu16RspBlockLen += PrologueSize;
    }
    else // Didn't get pu8RspBlock[2]
    {
        ret_Result = _SC_Recv(u8SCID, pu8RspBlock+PrologueSize, pu16RspBlockLen, SC_T1_RECV_TIMEOUT);
    }
#else
    ret_Result = _SC_Recv(u8SCID, pu8RspBlock, pu16RspBlockLen, SC_T1_RECV_TIMEOUT);
#endif
    if( *pu16RspBlockLen > 0 )
    {
        ret_Result = E_SC_OK;
    }
    else
    {
        ret_Result = E_SC_NODATA;
    }
...
SC_Result MDrv_SC_Open(MS_U8 u8SCID, MS_U8 u8Protocol, SC_Param* pParam, P_SC_Callback pfSmartNotify)
{
    ...
    // wyc.test-140926
    //printf("MDrv_SC_Open:%d,%d,%d\n",_scInfo[u8SCID].eCardClk, _scInfo[u8SCID].u16ClkDiv, _scInfo[u8SCID].u8UartMode);
...
SC_Result MDrv_SC_Reset_ATR(MS_U8 u8SCID, SC_Param* pParam, MS_U8 *pu8Atr, MS_U16 *pu16AtrLen, MS_U8 *pu8His, MS_U16 *pu16HisLen)
{
    ...
    *pu16AtrLen = 0;
    ...
    {
        _scInfo[u8SCID].eCardClk = E_SC_CLK_4M;
...
SC_Result MDrv_SC_T1_SendRecv(MS_U8 u8SCID, MS_U8 *pu8SendData, MS_U16 *u16SendDataLen, MS_U8 *pu8ReadData, MS_U16 *u16ReadDataLen)
{
    ...
    *u16ReadDataLen = 0;
    ...
    #if 0
    // wyc.test-140926
    printf("MDrv_SC_T1_SendRecv,%d\n",*u16SendDataLen);
    for (i=0;i<*u16SendDataLen;i++)
    {
        printf("%02x ",pu8SendData[i]);
    }
    printf("\n");
    #endif
    ...
    u32CheckTimeOut = OS_SC_TIME();
    while (u16SentIdx <  *u16SendDataLen-u8EDCByteNum)
    {
        if (_scInfo[u8SCID].bCardIn == FALSE)
        {
            //printf("T1_SR_CARDOUT_ERR_01\n");
            OS_SC_RETURN(E_SC_CARDOUT);
        }
        if ((OS_SC_TIME() - u32CheckTimeOut) > 5000)
        {
            //printf("T1_SR_TIMEOUT_ERR_01\n");
            OS_SC_RETURN(E_SC_TIMEOUT);
        }
