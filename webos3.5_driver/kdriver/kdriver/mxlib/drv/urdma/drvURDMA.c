...
DRVURDMA_RESULT MDrv_UrDMA_TxInit(MS_PHYADDR *tx_buf, MS_U16 tx_size)//U32 hs_uart_dma_prepare_tx(void *tx_buf, U16 tx_size)
{
    ...
    // if (dma_dbug == 1)
    // {
    DEBUG_UART_DMA(E_URDMA_DBGLV_INFO, printf("prepare tx: 0x%08x 0x%x\n", (int)tx_buf, (int)tx_size));
    // }
    ...
    while (p_uart_dma->tx_busy)
    {
		...
		DEBUG_UART_DMA(E_URDMA_DBGLV_ERR_ONLY, printf("tx_busy!\n"));
        if (1000 == (i++))
        {
        	//reg dump, here
            goto dma_prepare_tx_end;
...
DRVURDMA_RESULT MDrv_UrDMA_RxInit(MS_PHYADDR *rx_buf, MS_U16 rx_size)//hs_uart_dma_prepare_rx(void *rx_buf, U16 rx_size)
{
    ...
    DEBUG_UART_DMA(E_URDMA_DBGLV_INFO, printf("prepare rx: 0x%08x 0x%x\n", (int)rx_buf, (int)rx_size));
    ...
    while (p_uart_dma->rx_busy)
    {
        ...
        DEBUG_UART_DMA(E_URDMA_DBGLV_ERR_ONLY, printf("rx_busy!\n"));
        if (1000 == (i++))
        {
            goto dma_prepare_rx_end;
        }
...
void MDrv_UrDMA_Init(void *dma_rx_cb, void *dma_tx_cb, URDMA_OPmode eOp_mode)
{
	...
	{
		DEBUG_UART_DMA(E_URDMA_DBGLV_ERR_ONLY, printf("%s():IOMap failure to get DRV_MMIO_NONPM_BANK\n",__FUNCTION__));
		return;
	}

	u32urdma_base = u32BaseAddr + FAST_UART_DMA_REG;
	DEBUG_UART_DMA(E_URDMA_DBGLV_INFO, printf("%s:UART DMA BASE = 0x%x\n",__FUNCTION__,(int)u32urdma_base));
...
void MDrv_UrDMA_DumpREG(void)
{
    if (p_uart_dma != NULL)
    {
        DEBUG_UART_DMA(E_URDMA_DBGLV_ALL, printf("0x%x:0x%x\n", (int)&p_uart_dma->reg00, (int)p_uart_dma->reg00));
