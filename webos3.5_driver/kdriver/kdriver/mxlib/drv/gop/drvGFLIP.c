...
MS_BOOL _MDrv_GFLIP_ProcessIRQ(GOP_CTX_DRV_LOCAL*pGOPDrvLocalCtx)
{
    ...
    for(u32GopIdx=GFLIP_GOP_IDX_4G; u32GopIdx<MAX_GOP_SUPPORT; u32GopIdx++)
    {
        ...
        for(u32GwinIdx=0; u32GwinIdx<MAX_GOP_GWIN; u32GwinIdx++)
        {
            ...
            u32Addr = pGOPDrvLocalCtx->GFlipInfo[u32GopIdx][u32GwinIdx][pGOPDrvLocalCtx->u32GFlipInfoReadPtr[u32GopIdx][u32GwinIdx]].u32Addr;
...
#if GFLIP_ENABLE
    extern int _MDrv_GFLIPIO_IOC_Init(struct file *filp, unsigned long arg);
    if(_MDrv_GFLIPIO_IOC_Init(NULL, &u32GopIdx))
        return FALSE;
#endif
        ...
        if(pGOPDrvLocalCtx->s32FdGFlip >= 0)
        {
            ...
            {
                GFLIP_PRINT("[%s][%d] MDRV_GFLIP_IOC_INIT failed!\n", __FUNCTION__, __LINE__);
...
#ifdef MSOS_TYPE_LINUX_KERNEL
#if GFLIP_ENABLE
    extern int _MDrv_GFLIPIO_IOC_SetMultiFlipInfo(struct file *filp, unsigned long arg);
    if(_MDrv_GFLIPIO_IOC_SetMultiFlipInfo(NULL, pMultiinfo))
        return FALSE;
#endif
#elif defined(MSOS_TYPE_LINUX)
        GOP_CTX_DRV_LOCAL*pGOPDrvLocalCtx = (GOP_CTX_DRV_LOCAL*)pGOPCtx;
...
#ifdef MSOS_TYPE_LINUX_KERNEL
#if GFLIP_ENABLE
    extern int _MDrv_GFLIPIO_IOC_SetTLBMultiFlipInfo(struct file *filp, unsigned long arg);
    if(_MDrv_GFLIPIO_IOC_SetTLBMultiFlipInfo(NULL, pMultiinfo))
        return FALSE;
#endif
#elif defined(MSOS_TYPE_LINUX)
        GOP_CTX_DRV_LOCAL*pGOPDrvLocalCtx = (GOP_CTX_DRV_LOCAL*)pGOPCtx;
...
MS_BOOL MDrv_GFLIP_SetFlipInfo(MS_GOP_CTX_LOCAL*pGOPCtx, MS_U32 u32GopIdx, MS_U32 u32GwinIdx, MS_U32 u32MainAddress, MS_U32 u32SubAddress, MS_U32 u32TagId, MS_U32 * u32QEntry)
{
   ...
   MS_U8 u8MiuSel;
    ...
    //GOP HW just read the relative offset of each MIU
    _phy_to_miu_offset(u8MiuSel, u32MainAddress, u32MainAddress);
    if(TRUE == pGOPDrvLocalCtx->bEnableVsyncIntFlip[u32GopIdx])
    {
        ...
        #if defined(MSOS_TYPE_LINUX) || defined(MSOS_TYPE_LINUX_KERNEL)
        {
            ...
            stFlinInfo.u32Addr= u32MainAddress;
               ...
               (u32SubAddress != 0))
            {
                ...
                //GOP HW just read the relative offset of each MIU
                _phy_to_miu_offset(u8MiuSel, u32SubAddress, u32SubAddress);
                memcpy(&st3DFlipInfo, &stFlinInfo, sizeof(MS_GFLIP_INFO));
                st3DFlipInfo.u32SubAddr = u32SubAddress;

                #ifdef MSOS_TYPE_LINUX_KERNEL
                #if GFLIP_ENABLE
                    extern int _MDrv_GFLIPIO_IOC_Set3DFlipInfo(struct file *filp, unsigned long arg);
                    if(_MDrv_GFLIPIO_IOC_Set3DFlipInfo(NULL, &st3DFlipInfo))
                        return FALSE;
                #endif
                #elif defined(MSOS_TYPE_LINUX)
                if((pGOPCtx->pGOPCtxShared->bTLB[u32GopIdx]))
                {
                    ...
                    stTLB3DFlipInfo.u32TLBAddr = pGOPCtx->pGOPCtxShared->u32TLBAddress[u32GopIdx];
            ...
            else
            {
                #ifdef MSOS_TYPE_LINUX_KERNEL
                #if GFLIP_ENABLE
                    extern int _MDrv_GFLIPIO_IOC_SetFlipInfo(struct file *filp, unsigned long arg);
                    _MDrv_GFLIPIO_IOC_SetFlipInfo(NULL, &stFlinInfo);
                #endif
                #elif defined(MSOS_TYPE_LINUX)
                if((pGOPCtx->pGOPCtxShared->bTLB[u32GopIdx]))
                {
                    ...
                    stTLBFlinInfo.u32TLBAddr = pGOPCtx->pGOPCtxShared->u32TLBAddress[u32GopIdx];
        ...
        #else
        {
            ...
            if(u32NextWritePtr !=pGOPDrvLocalCtx->u32GFlipInfoReadPtr[u32GopIdx][u32GwinIdx]) //can write
            {
                ...
                pGOPDrvLocalCtx->GFlipInfo[u32GopIdx][u32GwinIdx][pGOPDrvLocalCtx->u32GFlipInfoWritePtr[u32GopIdx][u32GwinIdx]].u32Addr = (u32MainAddress/u16WordUnit);
    ...
    else
    {
        ...
        if((pGOPCtx->pGOPCtxShared->bTLB[u32GopIdx]))
        {
            if(pGOPCtx->pGOPCtxShared->u32TLBAddress[u32GopIdx])
            {
                u32tmp = pGOPCtx->pGOPCtxShared->u32TLBAddress[u32GopIdx] + (u32MainAddress/PAGE_SIZE)*TLB_PER_ENTRY_SIZE;
                u32tmp /= ADDRESSING_8BYTE_UNIT;
                HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+REG_TLB_BASE_ADDR_L, u32tmp&GOP_REG_WORD_MASK, GOP_REG_WORD_MASK);
                HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+REG_TLB_BASE_ADDR_H, u32tmp>>16, GOP_REG_WORD_MASK);
                u32tmp = u32MainAddress % (PAGE_SIZE*PER_MIU_TLB_ENTRY_COUNT);
                u32tmp = u32tmp /u16WordUnit;
                HAL_GOP_Write32Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_DRAM_RBLK_L(u32GwinIdx), u32tmp);
                HAL_GOP_Write32Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_DRAM_RBLK_H(u32GwinIdx), 0);
            }

            if((pGOPCtx->pGOPCtxShared->GOP_StereoMode[u32GopIdx] != E_DRV_GOP_3D_DISABLE)&&(pGOPCtx->pGOPCtxShared->u32TLBAddress[u32GopIdx]!= 0))
            {
                u32tmp = pGOPCtx->pGOPCtxShared->u32TLBAddress[u32GopIdx] + (u32SubAddress/PAGE_SIZE)*TLB_PER_ENTRY_SIZE;
                u32tmp /= ADDRESSING_8BYTE_UNIT;
                HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+REG_TLB_BASE_ADDR_RVIEW_L, u32tmp&GOP_REG_WORD_MASK, GOP_REG_WORD_MASK);
                HAL_GOP_Write16Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet+REG_TLB_BASE_ADDR_RVIEW_H, u32tmp>>16, GOP_REG_WORD_MASK);
                u32tmp = u32SubAddress % (PAGE_SIZE*PER_MIU_TLB_ENTRY_COUNT);
                u32tmp = u32tmp /u16WordUnit;
                HAL_GOP_Write32Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_3DOSD_SUB_RBLK_L(u32GwinIdx), u32tmp);
                HAL_GOP_Write32Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_3DOSD_SUB_RBLK_H(u32GwinIdx), 0);
            }
        }
        else
        {
            u32tmp = u32MainAddress /u16WordUnit;
        HAL_GOP_Write32Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_DRAM_RBLK_L(u32GwinIdx), u32tmp);
            HAL_GOP_Write32Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_DRAM_RBLK_H(u32GwinIdx), u32tmp>>16);
            if((pGOPCtx->pGOPCtxShared->GOP_StereoMode[u32GopIdx] != E_DRV_GOP_3D_DISABLE) && (u32SubAddress != 0))
            {
                u32tmp = u32SubAddress /u16WordUnit;
                HAL_GOP_Write32Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_3DOSD_SUB_RBLK_L(u32GwinIdx), u32tmp);
                HAL_GOP_Write32Reg(&pGOPDrvLocalCtx->halCtxLocal, u32BankOffSet + GOP_4G_3DOSD_SUB_RBLK_H(u32GwinIdx), u32tmp>>16);
            }
        }
...
MS_BOOL MDrv_GFLIP_SetGwinInfo(MS_GOP_CTX_LOCAL*pGOPCtx, MS_U8 u8GopIdx, MS_U8 u8GwinIdx,DRV_GOP_GWIN_INFO* pinfo)
{
    ...
    MsGwinInfo.u32Addr = pinfo->u32DRAMRBlkStart;
...
#ifdef MSOS_TYPE_LINUX_KERNEL
#if GFLIP_ENABLE
    extern _MDrv_GFLIPIO_IOC_SetGwinInfo(struct file *filp, unsigned long arg);
    if(_MDrv_GFLIPIO_IOC_SetGwinInfo(NULL, &MsGwinInfo))
        return FALSE;
#endif
#elif defined(MSOS_TYPE_LINUX)
    GOP_CTX_DRV_LOCAL*pGOPDrvLocalCtx = (GOP_CTX_DRV_LOCAL*)pGOPCtx;
    ...
    if(0 > pGOPDrvLocalCtx->s32FdGFlip)
    {
        GFLIP_PRINT("GLIP device not opened!!!!\n");
        return FALSE;
    }
    if(ioctl(pGOPDrvLocalCtx->s32FdGFlip, MDRV_GFLIP_IOC_SETGWININFO, &MsGwinInfo))
    {
        GFLIP_PRINT("MDrv_GFLIP_SetGwinInfo fail!!!!\n");
        return FALSE;
    }
...
MS_BOOL MDrv_GFLIP_RestoreFromVsyncLimitation(MS_GOP_CTX_LOCAL *pGOPCtx)
{
    ...
    for(u32GopIdx=0; u32GopIdx<MAX_GOP_SUPPORT; u32GopIdx++)
    {
        ...
        if(TRUE == pGOPDrvLocalCtx->bEnableVsyncIntFlip[u32GopIdx])
        {
#ifdef MSOS_TYPE_LINUX_KERNEL
#if GFLIP_ENABLE
            extern _MDrv_GFLIPIO_IOC_SetInputSigStatus(struct file *filp, unsigned long arg);
            if(_MDrv_GFLIPIO_IOC_SetInputSigStatus(NULL, &bHasSignal))
                return FALSE;
#endif
#elif defined(MSOS_TYPE_LINUX)
            if(0 > pGOPDrvLocalCtx->s32FdGFlip)
            ...
            {
                GFLIP_PRINT("MDrv_GFLIP_RestoreFromVsyncLimitation fail!!!!\n");
                return FALSE;
            }
...
MS_BOOL MDrv_GFLIP_GetDWinIntInfo(MS_GOP_CTX_LOCAL *pGOPCtx, GFLIP_DWININT_INFO *pGFlipDWinIntInfo,MS_U32 u32Timeout)
{
...
#ifdef MSOS_TYPE_LINUX_KERNEL
#if GFLIP_ENABLE
    extern int _MDrv_GFLIPIO_IOC_GetDWinIntInfo2(struct file *filp, unsigned long arg);
    if(_MDrv_GFLIPIO_IOC_GetDWinIntInfo2(NULL, &stGFlipDWinIntInfo))
        return FALSE;
    ...
    {
        GFLIP_PRINT("MDrv_GFLIP_GetDWinIntInfo fail!!!!\n");
        return FALSE;
    }
...
MS_BOOL MDrv_GFLIP_EnaVECapture(MS_GOP_CTX_LOCAL *pGOPCtx, PMS_DRVGOP_VECAPTURESTATE pstVECapState)
{
    ...
    if(pGOPDrvLocalCtx != NULL)
    {
#ifdef MSOS_TYPE_LINUX_KERNEL
#if GFLIP_ENABLE
        extern int _MDrv_GFLIPIO_IOC_EnableVECapture(struct file *filp, unsigned long arg);
        if(_MDrv_GFLIPIO_IOC_EnableVECapture(NULL, (PMS_GFLIP_VECAPTURESTATE)pstVECapState))
            return FALSE;
#endif
#elif defined(MSOS_TYPE_LINUX)
        if(0 > pGOPDrvLocalCtx->s32FdGFlip)
        ...
        {
            GFLIP_PRINT("MDrv_GFLIP_EnaVECapture fail!!!!\n");
            return FALSE;
        }
...
#ifdef MSOS_TYPE_LINUX_KERNEL
#if GFLIP_ENABLE
    extern int _MDrv_GFLIPIO_IOC_GetVECaptureState(struct file *filp, unsigned long arg);
    if(_MDrv_GFLIPIO_IOC_GetVECaptureState(NULL, (PMS_GFLIP_VECAPTURESTATE)pstVECapState))
        return FALSE;
#endif
#elif defined(MSOS_TYPE_LINUX)
        if(0 > pGOPDrvLocalCtx->s32FdGFlip)
        ...
        {
            GFLIP_PRINT("MDrv_GFLIP_GetVECaptureState fail!!!!\n");
            return FALSE;
        }
...
#ifdef MSOS_TYPE_LINUX_KERNEL
#if GFLIP_ENABLE
        extern int _MDrv_GFLIPIO_IOC_VECaptureWaitOnFrame(struct file *filp, unsigned long arg);
        if(_MDrv_GFLIPIO_IOC_VECaptureWaitOnFrame(NULL, (PMS_GFLIP_VECAPTURESTATE)pstVECapState))
            return FALSE;
#endif
#elif defined(MSOS_TYPE_LINUX)
        if(0 > pGOPDrvLocalCtx->s32FdGFlip)
...
#ifdef MSOS_TYPE_LINUX_KERNEL
#if GFLIP_ENABLE
    extern int _MDrv_GFLIPIO_IOC_ClearFlipQueue(struct file *filp, unsigned long arg);
    if(_MDrv_GFLIPIO_IOC_ClearFlipQueue(NULL, &stFlipQueueIdx))
        return FALSE;
...
MS_BOOL MDrv_GFLIP_SetGPIO3DPin(MS_GOP_CTX_LOCAL*pGOPCtx, MS_U32 u32GPIO3DPin)
{
    ...
    stFlinInfo.u32Addr= u32GPIO3DPin;
...
#ifdef MSOS_TYPE_LINUX_KERNEL
#if GFLIP_ENABLE
    extern int _MDrv_GFLIPIO_IOC_SetGPIO3DPin(struct file *filp, unsigned long arg);
    if(_MDrv_GFLIPIO_IOC_SetGPIO3DPin(NULL, &stFlinInfo))
        return FALSE;
    else
        return TRUE;
#else
    return FALSE;
        ...
        if(0 > pGOPDrvLocalCtx->s32FdGFlip)
        {
            GFLIP_PRINT("GFLIP device not opened!!!!\n");
            return FALSE;
        }
        if (ioctl(pGOPDrvLocalCtx->s32FdGFlip, MDRV_GFLIP_IOC_SETGPIO3DPIN, &stFlinInfo))
        {
            GFLIP_PRINT("MDRV_GFLIP_IOC_SETGPIO3DPIN fail!!!!\n");
            return FALSE;
        }
        return TRUE;
#else
