...
TSO_Result MDrv_TSO_InitLibResource(void* pResMemAddr)
{
    ...
    if(_bBankInit == FALSE)
    {
        if (FALSE == MDrv_MMIO_GetBASE(&u32Bank, &u32BankSize, MS_MODULE_TSO))
        {
            TSO_DBGERR(printf("MDrv_TSO_GetBASE failed\n"));
            return DRVTSO_FAIL;
        }
        TSO_DBGINFO((printf("TSO Base Bank 0x%08x\n", (int)u32Bank)));
        HAL_TSO_SetBank(u32Bank);
        _bBankInit = TRUE;
    }
...
/// @return TSP_Result
//-------------------------------------------------------------------------------------------------
TSO_Result MDrv_TSO_Filein_GetReadAddr(MS_U8 u8Eng, MS_PHYADDR *pu32ReadAddr)
{
    TSO_DBGERR(printf("[%d]%s\n", __LINE__, __FUNCTION__));
...
/// @note
//-------------------------------------------------------------------------------------------------
TSO_Result MDrv_TSO_Filein_SetAddr(MS_U8 u8Eng, MS_PHYADDR u32StreamAddr)
{
    TSO_DBGERR(printf("[%d]%s\n", __LINE__, __FUNCTION__));
