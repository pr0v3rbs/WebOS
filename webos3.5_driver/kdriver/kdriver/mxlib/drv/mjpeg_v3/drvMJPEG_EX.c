...
static MS_BOOL bDisplayOutSideMode[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};
static MS_U32 u32FrameBuffStAddr[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};
static MS_U32 u32FrameBuffTotalSize[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};
static MS_U32 u32FrameBuffUnitSize[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};
static MS_U32 u32FrameBuffTotalNum[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};
static MS_U16 u16Width[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};
static MS_U16 u16Height[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};
static MS_U16 u16Pitch[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};
static MS_U32 u32FrameCnt[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};
static MS_U32 u32FrameRate[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};
static MS_U32 u32FrameRateBase[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};
static MS_U32 u32FrameTime[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};

void MDrv_MJPEG_SetFrameBuffStAddr(MS_U8 u8DrvId, MS_U32 u32Value)
{
    u32FrameBuffStAddr[u8DrvId] = u32Value;
}
MS_U32 MDrv_MJPEG_GetFrameBuffStAddr(MS_U8 u8DrvId)
{
...
MS_U32 MDrv_MJPEG_SendVPUCommand(MS_U32 u32Id, MJPEG_User_Cmd eCmd, MS_U32 u32Arg)
{
    ...
    if (bDisplayOutSideMode[u8DrvId])
    {
        ...
        case E_MJPEG_CMD_SET_FRAME_BUFF_START_ADDR:
            ...
            MJPEG_DEBUG_DRV_MSG("[MJPEG_FB][free(init) 0][%ld %ld %ld %ld %ld %ld %ld %ld]\n", pShareMem->DispQueue[0].u32Status, pShareMem->DispQueue[1].u32Status,
                pShareMem->DispQueue[2].u32Status, pShareMem->DispQueue[3].u32Status, pShareMem->DispQueue[4].u32Status,
                pShareMem->DispQueue[5].u32Status, pShareMem->DispQueue[6].u32Status, pShareMem->DispQueue[7].u32Status);
            MsOS_FlushMemory();
            break;
        ...
        case E_MJPEG_CMD_SET_FRAME_BUFF_TOTAL_NUM:
            ...
            MJPEG_DEBUG_DRV_MSG("[MJPEG_FB][free(init) all][%ld %ld %ld %ld %ld %ld %ld %ld]\n", pShareMem->DispQueue[0].u32Status, pShareMem->DispQueue[1].u32Status,
                pShareMem->DispQueue[2].u32Status, pShareMem->DispQueue[3].u32Status, pShareMem->DispQueue[4].u32Status,
                pShareMem->DispQueue[5].u32Status, pShareMem->DispQueue[6].u32Status, pShareMem->DispQueue[7].u32Status);
            MsOS_FlushMemory();
            break;
        ...
        case E_MJPEG_CMD_PUSH_QUEUE_PARA_SETTING:
            ...
            MJPEG_DEBUG_DRV_MSG("[MJPEG_FB][none %ld][%ld %ld %ld %ld %ld %ld %ld %ld]\n", pShareMem->u32MJPEGFrameBuffIdx, pShareMem->DispQueue[0].u32Status, pShareMem->DispQueue[1].u32Status,
                pShareMem->DispQueue[2].u32Status, pShareMem->DispQueue[3].u32Status, pShareMem->DispQueue[4].u32Status,
                pShareMem->DispQueue[5].u32Status, pShareMem->DispQueue[6].u32Status, pShareMem->DispQueue[7].u32Status);
            MsOS_FlushMemory();
            break;
        ...
        case E_MJPEG_CMD_SET_FRAME_BUFF_IDX_READY:
            ...
            MJPEG_DEBUG_DRV_MSG("[MJPEG_FB][init %ld][%ld %ld %ld %ld %ld %ld %ld %ld]\n", u32Arg, pShareMem->DispQueue[0].u32Status, pShareMem->DispQueue[1].u32Status,
                pShareMem->DispQueue[2].u32Status, pShareMem->DispQueue[3].u32Status, pShareMem->DispQueue[4].u32Status,
                pShareMem->DispQueue[5].u32Status, pShareMem->DispQueue[6].u32Status, pShareMem->DispQueue[7].u32Status);
            MsOS_FlushMemory();
            break;
        ...
        case E_MJPEG_CMD_SET_FRAME_BUFF_IDX_VALID:
            ...
            MJPEG_DEBUG_DRV_MSG("[MJPEG_FB][free(valid) %ld][%ld %ld %ld %ld %ld %ld %ld %ld]\n", u32Arg, pShareMem->DispQueue[0].u32Status, pShareMem->DispQueue[1].u32Status,
                pShareMem->DispQueue[2].u32Status, pShareMem->DispQueue[3].u32Status, pShareMem->DispQueue[4].u32Status,
                pShareMem->DispQueue[5].u32Status, pShareMem->DispQueue[6].u32Status, pShareMem->DispQueue[7].u32Status);
            break;
        ...
        case E_MJPEG_CMD_FLUSH_DISP_QUEUE:
            ...
            MJPEG_DEBUG_DRV_MSG("[MJPEG_FB][free(flush) all][%ld %ld %ld %ld %ld %ld %ld %ld]\n", pShareMem->DispQueue[0].u32Status, pShareMem->DispQueue[1].u32Status,
                pShareMem->DispQueue[2].u32Status, pShareMem->DispQueue[3].u32Status, pShareMem->DispQueue[4].u32Status,
                pShareMem->DispQueue[5].u32Status, pShareMem->DispQueue[6].u32Status, pShareMem->DispQueue[7].u32Status);
    ...
    case E_MJPEG_CMD_SET_DISPLAY_OUTSIDE_MODE:
        ...
        if (bDisplayOutSideMode[u8DrvId])
        {
            ...
            MJPEG_DEBUG_DRV_MSG("[MJPEG_FB][free all][%ld %ld %ld %ld %ld %ld %ld %ld]\n", pShareMem->DispQueue[0].u32Status, pShareMem->DispQueue[1].u32Status,
                pShareMem->DispQueue[2].u32Status, pShareMem->DispQueue[3].u32Status, pShareMem->DispQueue[4].u32Status,
                pShareMem->DispQueue[5].u32Status, pShareMem->DispQueue[6].u32Status, pShareMem->DispQueue[7].u32Status);
...
void MDrv_MJPEG_DispFrame(MS_U32 u32Id, MS_U32 u32DispIdx)
{
    ...
    MJPEG_DEBUG_DRV_MSG("[MJPEG_FB][disp %ld][%ld %ld %ld %ld %ld %ld %ld %ld]\n", u32DispIdx, pShareMem->DispQueue[0].u32Status, pShareMem->DispQueue[1].u32Status,
        pShareMem->DispQueue[2].u32Status, pShareMem->DispQueue[3].u32Status, pShareMem->DispQueue[4].u32Status,
        pShareMem->DispQueue[5].u32Status, pShareMem->DispQueue[6].u32Status, pShareMem->DispQueue[7].u32Status);
    MsOS_FlushMemory();
}
...
void MDrv_MJPEG_FreeFrame(MS_U32 u32Id, MS_U32 u32DispIdx)
{
    ...
    MJPEG_DEBUG_DRV_MSG("[MJPEG_FB][free %ld][%ld %ld %ld %ld %ld %ld %ld %ld]\n", u32DispIdx, pShareMem->DispQueue[0].u32Status, pShareMem->DispQueue[1].u32Status,
        pShareMem->DispQueue[2].u32Status, pShareMem->DispQueue[3].u32Status, pShareMem->DispQueue[4].u32Status,
        pShareMem->DispQueue[5].u32Status, pShareMem->DispQueue[6].u32Status, pShareMem->DispQueue[7].u32Status);
...
static MS_U16 _u16DispQPtr[E_MJPEG_MJPEG_TOTAL] = {0, 0, 0, 0};
void* MDrv_MJPEG_GetNextDispFrame(MS_U32 u32Id)
{
    ...
    if (bDisplayOutSideMode[u8DrvId])
    {
        ...
        if (found >= 0)
        {
            ...
            MsOS_FlushMemory();
            MJPEG_DEBUG_DRV_MSG("[MJPEG_FB][view %ld][%ld %ld %ld %ld %ld %ld %ld %ld]\n", found, pShareMem->DispQueue[0].u32Status, pShareMem->DispQueue[1].u32Status,
                pShareMem->DispQueue[2].u32Status, pShareMem->DispQueue[3].u32Status, pShareMem->DispQueue[4].u32Status,
                pShareMem->DispQueue[5].u32Status, pShareMem->DispQueue[6].u32Status, pShareMem->DispQueue[7].u32Status);
        ...
        {
            MJPEG_DEBUG_DRV_MSG("MDrv_MJPEG_GetNextDispFrame return NULL!!! [MJPEG_FB][%ld %ld %ld %ld %ld %ld %ld %ld]\n",
                pShareMem->DispQueue[0].u32Status, pShareMem->DispQueue[1].u32Status,
                pShareMem->DispQueue[2].u32Status, pShareMem->DispQueue[3].u32Status, pShareMem->DispQueue[4].u32Status,
                pShareMem->DispQueue[5].u32Status, pShareMem->DispQueue[6].u32Status, pShareMem->DispQueue[7].u32Status);
            return NULL;
    ...
    else
    {
        ...
        while (u16QNum != 0)
        {
            ...
            if (pFrm->u32Status == E_HVD_DISPQ_STATUS_INIT)
            {
                ...
                MJPEG_DEBUG_DRV_MSG("[MJPEG_FB][view %d][%ld %ld %ld %ld %ld %ld %ld %ld]\n", _u16DispQPtr[u8DrvId], pShareMem->DispQueue[0].u32Status, pShareMem->DispQueue[1].u32Status,
                pShareMem->DispQueue[2].u32Status, pShareMem->DispQueue[3].u32Status, pShareMem->DispQueue[4].u32Status,
                pShareMem->DispQueue[5].u32Status, pShareMem->DispQueue[6].u32Status, pShareMem->DispQueue[7].u32Status);
...
MJPEG_Return MDrv_MJPEG_InitFw(MS_U32 u32Id, MJPEG_FW_Info FWInfo)
{
    ...
    MJPEG_DEBUG_DRV_MSG("[MJPEG] FWInfo.u32MWCAddr=0x%lx, FWInfo.u32MWCSize=0x%lx\n", FWInfo.u32MWCAddr, FWInfo.u32MWCSize);

    MS_U8 MIUAllocation;
    _phy_to_miu_offset(MIUAllocation, stVpuInitPara.pFBCfg->u32FrameBufAddr, FWInfo.u32MWCAddr);
    UNUSED(MIUAllocation);
    stVpuInitPara.pFBCfg->u32FrameBufSize = FWInfo.u32MWCSize;
    MJPEG_DEBUG_DRV_MSG("[MJPEG] u32FrameBufAddr=0x%lx, u32FrameBufSize=0x%lx\n", stVpuInitPara.pFBCfg->u32FrameBufAddr, stVpuInitPara.pFBCfg->u32FrameBufSize);
#endif
...
MS_BOOL MDrv_MJPEG_GetFrameBuffer(MS_U32 u32Id)
{
    ...
    while(1)
    {
        ...
        {
            MJPEG_DEBUG_DRV_MSG("[%ld]ms, get ok\n", MsOS_GetSystemTime()-u32Timer);
            break;
        }
    }
    if(pShareMem->u32FrameBufAddr!=0xffffffff
        && pShareMem->u32FrameBufSize!=0xffffffff)
    {
        bRet = TRUE;
        u32FrameBuffStAddr[u8DrvId] = pShareMem->u32FrameBufAddr;
        u32FrameBuffTotalSize[u8DrvId] = pShareMem->u32FrameBufSize;
        MJPEG_DEBUG_DRV_MSG("Get addr=0x%lx, size=0x%lx\n", pShareMem->u32FrameBufAddr, pShareMem->u32FrameBufSize);
    }
    else
    {
        MJPEG_DEBUG_DRV_ERR("[fail]Get addr=0x%lx, size=0x%lx\n", pShareMem->u32FrameBufAddr, pShareMem->u32FrameBufSize);
    }
    return bRet;
}
...
MS_BOOL MDrv_MJPEG_IsFreeFinish(MS_U32 u32Id)
{
    ...
    while(1)
    {
        ...
        if(pShareMem->u32FrameBufAddr==0)
        {
            MJPEG_DEBUG_DRV_MSG("[%ld]ms, free ok\n", MsOS_GetSystemTime()-u32Timer);
            bRet = TRUE;
            break;
        }
    }
...
void MDrv_MJPEG_GetCMAInformation(MS_U32 u32Id, MS_U32* u32Addr,MS_U32* u32Size,MS_BOOL* bAllocDone)
{
    volatile HVD_ShareMem *pShareMem = NULL;
    pShareMem = (HVD_ShareMem *)MDrv_MJPEG_GetShareMemAddr(u32Id);
