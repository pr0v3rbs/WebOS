...
#include "osalMVOP.h"
#include "drvSYS.h"
#define MVOP_ERROR //MVOP_PRINTF("[Fail][%s,%d]\n",__FUNCTION__,__LINE__);
//-------------------------------------------------------------------------------------------------
...
typedef struct
{
    ...
    MVOP_VDEC_Source _eVDECSource;      //used by dynamic mvop checking
...
static void _MVOP_SubInitVar(MVOP_DevID eID)
{
    ...
    if (pstDrvCtrl)
    {
        ...
        pstDrvCtrl->_eVDECSource = E_MVOP_VDEC_NONE;
        memset((void*)&pstDrvCtrl->_mvopOutputCfg, 0, sizeof(MVOP_VidStat));
...
#elif (MVOP_BUS_WIDTH == _MVOP_256BIT_BUS)
    {
        u16HSize = ALIGN_UPTO_32(u16OriHSize);
        if (u16HSize != u16OriHSize)
        {
            *pu16SrcHSize = u16OriHSize;
        }
...
#else
    {
        u16HSize = ALIGN_DOWNTO_8(u16OriHSize); //bus 64
    }
#endif
...
static MS_U32 _MaxFactor(MS_U32 u32Num1, MS_U32 u32Num2)
{
    ...
    MVOP_DBG_VERBAL(MVOP_PRINTF("u32Num1=%lx u32Num2=%lx\n", u32Num1, u32Num2);)
    MS_ASSERT(u32Num1 != 0);
    ...
    while (1)
    {
        if (u32Num1 > u32Num2)
        {
            ...
            {
                MVOP_DBG_VERBAL(MVOP_PRINTF("ret0 u32Num1=%lx u32Num2=%lx\n", u32Num1, u32Num2);)
                return u32Num2;
            }
            else
            {
        ...
        else
        {
            ...
            {
                MVOP_DBG_VERBAL(MVOP_PRINTF("ret1 u32Num1=%lx u32Num2=%lx\n", u32Num1, u32Num2);)
                return u32Num1;
            }
            else
            {
                                ...
                                MS_BOOL b4k2k2p)
{
    ...
    u32Q += u32R/1000;
    u32Q = (u32Q%1000)==0?u32Q/1000:(u32Q/1000+1);
    MVOP_Clock = u32Q;
    u32MaxFactor = _MaxFactor(MVOP_Clock, DC_BaseClock);

    if (u32MaxFactor == 1)
    {
        pVOPTiming->u16Num = (MS_U16)(MVOP_Clock/2)+1;
                             ...
                             EN_MVOP_Output_3D_TYPE en3DType)
{
    ...
    if(bEnHDup && (pstVideoStatus->u16HorSize < 704))
    {
        u16HorSize_cal = pstVideoStatus->u16HorSize * 2;
        pVOPTiming->bHDuplicate = 1;
    }
    else
    {
        u16HorSize_cal = pstVideoStatus->u16HorSize;
        pVOPTiming->bHDuplicate = 0;
    }
    u16HSize = MVOP_CalculateHSize(pu16SrcHSize, u16HorSize_cal, pstVideoStatus->u16HorOffset);
    u16VSize = MVOP_CalculateVSize(eID, pu16SrcVSize, pstVideoStatus->u16VerSize, pstVideoStatus->u16VerOffset);
...
#endif
    MVOP_TIMING_DBG(MVOP_PRINTF(">>>>> u32PixClk=%ld, Htt=%d, Vtt=%d\n", u32PixClk,
                    pVOPTiming->u16H_TotalCount, pVOPTiming->u16V_TotalCount);)
                                ...
                                MS_BOOL bEnHDup)
{
    ...
    if(HAL_MVOP_SetDefaultClk && (MVOP_Clock <= MVOP_FREQ_160M))
    {
        MVOP_Clock = HAL_MVOP_SetDefaultClk();
        MVOP_TIMING_DBG(MVOP_PRINTF(">>>>> Set default clock");)
    }
    MVOP_TIMING_DBG(MVOP_PRINTF(">>>>> calculate need clk = %ld\n",MVOP_Clock);)
    if (0 == MVOP_Clock)
    {
    ...
    if(MVOP_Clock > MVOP_FREQ_320M)
    {
        if (bIsMain)
        {
            ...
            {
                MVOP_PRINTF("[Warning] MVOP_Clock == %ld Not Support!\n",MVOP_Clock);
                return FALSE;
            }
    ...
    else
    {
        ...
        if (bIsMain)
        {
            if(HAL_MVOP_CheckSTCCW)
            {
                ...
                {
                    MVOP_TIMING_DBG(printf(">>>>> STC cw = 0x0, MVOP enable freerun clock.\n");)
...
void MDrv_MVOP_Init_V2(void* pInstance)
{
    ...
    {
        if(p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._stMVOPDrvStat.bIsInit)
        {
...
MVOP_Result MDrv_MVOP_SetOutputCfg_V2(void* pInstance, MVOP_VidStat *pstVideoStatus, MS_BOOL bEnHDup)
{
    ...
    memcpy((void*)(&p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._mvopOutputCfg),(void*)pstVideoStatus,sizeof(MVOP_VidStat));
...
MS_BOOL MDrv_MVOP_GetDstInfo_V2(void* pInstance, MVOP_DST_DispInfo *pDstInfo, MS_U32 u32SizeofDstInfo)
{
    ...
    MVOP_TIMING_DBG(MVOP_PRINTF("bInterlace=%u,HTotal=%lu,VTotal=%lu\n", pDstInfo->bInterlaceMode, pDstInfo->HDTOT, pDstInfo->VDTOT);)
    MVOP_TIMING_DBG(MVOP_PRINTF("HDeStart=%lu,HDeEnd=%lu,VDeStart=%lu,VDeEnd=%lu\n", pDstInfo->DEHST, pDstInfo->DEHEND, pDstInfo->DEVST, pDstInfo->DEVEND);)
...
MS_BOOL MDrv_MVOP_SetBaseAdd_V2(void* pInstance, MS_U32 u32YOffset, MS_U32 u32UVOffset, MS_BOOL bProgressive, MS_BOOL b422pack)
{
    _DRV_MVOP_Entry(E_MVOP_DEV_0);
...
{
    MVOP_DBG_TRACE(MVOP_PRINTF("[TRACE] %s, line:%d, arg1:%ld, arg2:%ld, arg3:%d, arg4:%d \n", __FUNCTION__, __LINE__,
...
void MDrv_MVOP_SetStartPos_V2(void* pInstance, MS_U16 u16Xpos ,MS_U16 u16Ypos)
{
    ...
    u16Ypos = ALIGN_UPTO_2(u16Ypos);
#if STB_DC
    HAL_MVOP_SetStartX(u16Xpos);
    HAL_MVOP_SetStartY(u16Ypos, bIs4Align);
#else
        MVOP_Timing* pstTiming = NULL;
        MS_U16 u16Ypos4Align = 0;

        pstTiming = &p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._mvopTiming;
        if(pstTiming->bInterlace)
            u16Ypos4Align = ALIGN_DOWNTO_4(u16Ypos);

        if(u16Ypos4Align != u16Ypos)
            bIs4Align = TRUE;

        HAL_MVOP_SetStartX(u16Xpos);
        HAL_MVOP_SetStartY(u16Ypos4Align, bIs4Align);
#endif
    _DRV_MVOP_Release(E_MVOP_DEV_0);
...
void MDrv_MVOP_SetRegSizeFromMVD_V2(void* pInstance, MS_BOOL bEnable)
{
    ...
    HAL_MVOP_SetDeb2MVDFrameModeCtrl(p_gDrvMVOPCtx->_bRegSizeFromMVDStatus,E_MVOP_SIZE_FROM_MVD);
#endif
    _DRV_MVOP_Release(E_MVOP_DEV_0);
...
//-----------------------------------------------------------------------------
void MDrv_MVOP_GetBaseAdd_V2(void* pInstance, MS_U32* u32YOffset, MS_U32* u32UVOffset)
{
    _DRV_MVOP_Entry(E_MVOP_DEV_0);
    if (!MVOP_IsInit())
    {
...
MVOP_Result MDrv_MVOP_SubSetInputCfg_V2(void * pInstance, MVOP_InputSel in, MVOP_InputCfg * pCfg)
{
    ...
    switch (in)
    {
        ...
        case MVOP_INPUT_EVD_3DLR:
         ...
         case MVOP_INPUT_EVD:
            HAL_MVOP_SubSetInputMode(VOPINPUT_HARDWIRE, NULL, u16ECOVersion);
            HAL_MVOP_SubSetEVDHardwireMode();
            break;
#endif
#if SUPPORT_VP9_MODE
        case MVOP_INPUT_VP9:
            HAL_MVOP_SubSetInputMode(VOPINPUT_HARDWIRE, NULL, u16ECOVersion);
            HAL_MVOP_SubSetVP9HardwireMode();
            break;
#endif
        default:
...
MVOP_Result MDrv_MVOP_SubSetOutputCfg_V2(void* pInstance, MVOP_VidStat *pstVideoStatus, MS_BOOL bEnHDup)
{
    ...
    memcpy((void*)(&p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._mvopOutputCfg),(void*)pstVideoStatus,sizeof(MVOP_VidStat));
...
MS_BOOL MDrv_MVOP_SubSetBaseAdd_V2(void* pInstance, MS_U32 u32YOffset, MS_U32 u32UVOffset, MS_BOOL bProgressive, MS_BOOL b422pack)
{
#if MVOP_SUPPORT_SUB
    _DRV_MVOP_Entry(E_MVOP_DEV_1);
    if (!MVOP_SubIsInit())
...
//  Only support in Eagle.
        ...
        //-----------------------------------------------------------------------//
        // Please set before MDrv_MVOP_SetOutputCfg and after MDrv_MVOP_SetInputCfg
        //-----------------------------------------------------------------------//
        case E_MVOP_CMD_SET_420_BW_SAVING_MODE:
        {
            ...
            {
                HAL_MVOP_SubSet420BWSaveMode(bEnable);
        ...
        case E_MVOP_CMD_SET_EVD_FEATURE:
       {
        ...
        {
            if (HAL_MVOP_ResetMVOPSetting)
            {
                switch(eDevID)
                {
                    case E_MVOP_DEV_0:
                        HAL_MVOP_ResetMVOPSetting(E_MVOP_DEV_0);
                        break;
#if MVOP_SUPPORT_SUB
                    case E_MVOP_DEV_1:
                        HAL_MVOP_ResetMVOPSetting(E_MVOP_DEV_1);
                        break;
#endif
                    default:
                        MVOP_DBG(MVOP_PRINTF("\n %s not support this device 0x%x!! \n", __FUNCTION__, eCmd);)
        ...
        case E_MVOP_CMD_SET_VDEC_SOURCE:
        {
            ...
            {
                case E_MVOP_DEV_0:
                    p_gDrvMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_0]._eVDECSource = *(MVOP_VDEC_Source*)pPara;
                    break;
#if MVOP_SUPPORT_SUB
                case E_MVOP_DEV_1:
                    p_gDrvSubMVOPCtx->_stMVOPDrvCtrl[E_MVOP_DEV_1]._eVDECSource = *(MVOP_VDEC_Source*)pPara;
                    break;
#endif
                default:
                    MVOP_DBG(MVOP_PRINTF("\n %s not support this device 0x%x!! \n", __FUNCTION__, eCmd);)
    ...
    switch (eCmd)
    {
        ...
        case E_MVOP_CMD_GET_BASE_ADD_MULTI_VIEW:
            ...
            if (HAL_MVOP_GetYBaseAddMultiView && HAL_MVOP_GetUVBaseAddMultiView && (sizeof(MVOP_BaseAddInput) == u32ParaSize))
            {
                if (pPara)
                {
                    ...
                    pBaseAddInfo->u32YOffset = (MS_U32)HAL_MVOP_GetYBaseAddMultiView(pBaseAddInfo->eView);
                    pBaseAddInfo->u32UVOffset = (MS_U32)HAL_MVOP_GetUVBaseAddMultiView(pBaseAddInfo->eView);
                }
            }
            else
#endif
       ...
       case E_MVOP_CMD_GET_TOP_FIELD_IMAGE_VSTART:
        ...
        case E_MVOP_CMD_GET_VDEC_SOURCE:
            ...
            if(pstDrvCtrl)
            {
                *(MVOP_VDEC_Source*)pPara = pstDrvCtrl->_eVDECSource;
                eRet = E_MVOP_OK;
            }
            else
            {
        ...
        case E_MVOP_CMD_GET_OUTPUT_CFG:
            ...
            if(pstDrvCtrl)
            {
                memcpy(pPara,(void*)(&pstDrvCtrl->_mvopOutputCfg),sizeof(MVOP_VidStat));
                eRet = E_MVOP_OK;
            }
            else
            {
