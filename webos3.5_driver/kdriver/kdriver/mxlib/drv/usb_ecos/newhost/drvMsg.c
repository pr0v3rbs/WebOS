...
static int ms_usb_send_urb(struct urb *urb, int timeout, int* act_len)
{
    ...
    if (!timeout && !stAwd.done)
    {
        ...
        else
        {
            ...
            if (ms_usb_unlink_urb(urb) == -ETIMEDOUT)
            {
                diag_printf("timeout retry\n");
                ms_usb_unlink_urb(urb);
            }
        ...
        int timeout)
{
    ...
    if (ret < 0)
    {
        ms_debug_msg("ms_usb_send_urb retv:%d \n",-ret);
    }
    else
        ret = length;
...
int ms_usb_set_config(struct usb_device *pDev, int config_val)
{
    ...
    // Add string
    if (pDev->descriptor.iProduct)
    {
        ms_usb_string(pDev, pDev->descriptor.iProduct, usb_string_buf, 64);
        pDev->pProductString = usb_string_buf;
    }
    else
        pDev->pProductString = NULL;
