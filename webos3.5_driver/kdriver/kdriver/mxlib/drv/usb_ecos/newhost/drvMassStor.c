...
//==================Definition======================//
static int ms_usb_msc_match(struct usb_interface *iface, const struct usb_device_id *id);
...
int ms_usb_msc_control_thread(void * __us)
{
    ...
    ms_usbhost_msg("*** thread sleeping.%s","\n");
    ms_usbhost_msg("*** thread awakened.%s","\n");
    ...
    {
        ms_usbhost_msg("SCSI cmd done, result=0x%x\n", ms_us->srb.result);
    ...
    {
Stop_transfer:
        ms_usbhost_msg("SCSI command aborted%s","\n");
    }
    if (ms_us->usb_state == ABORTING_STATE)
...
{  
    ...
    {
        ms_usbhost_msg("[ms_usb_associate_dev] Allocating failed%s","\n");
        return -ENOMEM;
    }
    return 0;
}

void ms_usb_set_subclass(struct ms_usdata *ms_us,struct stUnusual_dev *unusual_dev, struct usb_interface_descriptor *idesc)
{
...
void ms_usb_set_protocol(struct ms_usdata *ms_us,struct stUnusual_dev *unusual_dev, struct usb_interface_descriptor *idesc)
{
...
static int ms_usb_get_protocol(struct ms_usdata *ms_us)
{
    switch (ms_us->subclass) 
    {
        ...
        case 0xFF:	// For Sony DC P8 (P series) and Sony DV
            ms_usbhost_msg("Transparent SCSI\n");
            ms_us->proto_handler = ms_usb_transparent_scsi_command;
            break;
        case MS_ATAPI_SC:
...
void ms_usb_set_pipe(struct ms_usdata *ms_us, 
...
static int ms_usb_get_pipes(struct ms_usdata *ms_us)
{
    ...
    {
        ms_usbhost_msg("EP sanity check failed! Rejecting dev.%s","\n");
        return -EIO;
    }
...
static int ms_usb_acquire_resources(struct ms_usdata *ms_us)
{
    ...
    {
        ms_usbhost_msg("USB Request Block allocates failed%s","\n");
        return -ENOMEM;
    }   
    osapi_down(&ms_us->dev_semaphore);
    ...
    if (ms_us->protocol == MS_BULK_PR)
    {
        regval = ms_usb_control_msg(ms_us, ms_us->ctrl_recv_pipe, BULK_GET_MAX_LUN,
                    ...
                    0, ms_us->ifnum, ms_us->iobuf, 1, HZ);

        ms_usbhost_msg("GetMaxLUN command regval is %d, data is %d\n", regval, ms_us->iobuf[0]);
...
static void ms_usb_unregister_dev(struct ms_usdata *ms_us)
{
    ...
    {
        ms_usbhost_msg("--> Call extra_destructor()%s","\n");
    ...
    for (tmp=0; tmp<NUM_OF_ROOT_HUB*MAX_USTOR; tmp++)
    {
        if (Mass_stor_us[tmp] == ms_us)
        {
            for (u8Lun=0; u8Lun<=Mass_stor_us[tmp]->max_lun; u8Lun++)
                RemoveUSBDiskPort(Mass_stor_us[tmp]->ustor_id, u8Lun);
        
            Mass_stor_us[tmp] = NULL;
        }    
    }
    if (ms_us->msc_device)
...
static int ms_usb_msc_match(struct usb_interface *ms_intf, const struct usb_device_id *id)
{
    ...
    ms_usbhost_msg("USB MSC device detected%s","\n");
    ms_usbhost_msg("act_altsetting = %d, idx = %d\n", ms_intf->act_altsetting, idx);
    ...
    if (startId >= NUM_OF_ROOT_HUB*MAX_USTOR)
        USB_ASSERT(0, "startId >= NUM_OF_ROOT_HUB*MAX_USTOR\n");
    ...
    Mass_stor_us[tmp]->host_id = ((struct usb_hcd *)pudev->bus->hcpriv)->host_id;
    for(tmp = 0; tmp<=ms_us->max_lun; tmp++)
        ms_us->msc_device[tmp].bDeviceReady = FALSE;

    ms_usbhost_msg("find USB MSC device at %d\n", ms_us->pusb_dev->u32DevNum);
...
static void ms_usb_msc_disconnect(struct usb_interface *ms_intf)
{
    ...
    ms_set_bit(MS_DISCONNECTING, &ms_us->flags, unsigned long);
    if (ms_test_and_clear_bit(MS_URB_ACTIVE, &ms_us->flags)) 
    {
        ms_usbhost_msg("--> cancel USB request block\n");
