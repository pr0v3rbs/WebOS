...
void ms_usb_disconnect(struct usb_device **pdev)
{
    ...
    *pdev = NULL;
    ...
    ms_usbhost_msg("%s - unregistering interface %s\n", pUdev->dev.dev_name, pUdev->dev.bus_id);
    ...
    ms_usbhost_debug ("unregistering device%s","\n");
...
int ms_usb_new_device(struct usb_device *pUdev, struct device_s *parent)
{
...
#if 0 // debug string only
    if (pUdev->descriptor.iProduct)
        ms_usb_show_string(pUdev, "Product", pUdev->descriptor.iProduct);
    if (pUdev->descriptor.iManufacturer)
        ms_usb_show_string(pUdev, "Manufacturer", pUdev->descriptor.iManufacturer);
    if (pUdev->descriptor.iSerialNumber)
        ms_usb_show_string(pUdev, "SerialNumber", pUdev->descriptor.iSerialNumber);
#endif
    ...
    ms_usbhost_msg("new device strings: Mfr=%d, Product=%d, SerialNumber=%d\n",
        pUdev->descriptor.iManufacturer, pUdev->descriptor.iProduct, pUdev->descriptor.iSerialNumber);
    ms_usbhost_msg("VID=0x%04X, PID=0x%04X", pUdev->descriptor.idVendor, pUdev->descriptor.idProduct);
