...
static void ms_usb_timeout_handler(U32 tmp)
{
    ...
    {
        diag_printf("Timeout.. Warning!! [Cancel USB request block %p]\n",ptr->current_urb);
        diag_printf("LOG: dev%d ep%d%s qtd --> status %d, len=%d",
            usb_pipedevice (ptr->current_urb->u32Pipe),
            usb_pipeendpoint (ptr->current_urb->u32Pipe),
            usb_pipein (ptr->current_urb->u32Pipe) ? "in" : "out", ptr->current_urb->s32Status, ptr->current_urb->u32TransferBufferLength);
        ...
        ms_usbhost_msg("unlink USB request block done...\n");
    }
}

#ifndef URB_TIMEOUT_BY_WAIT_EVENT
void ms_usb_wait_for_completion(struct stCompletion *cpt, S32 flag)
{
    BOOL    error;
    U32 u32Pattern;
    ...
    if (cpt->done != 1)
        ms_usbhost_warn("Completion fail %08lX %ld, error %d\n", u32Pattern, cpt->done, error);
}
#else
void ms_usb_wait_for_completion(struct stCompletion *cpt, S32 flag, struct ms_usdata *ptr, U32 wait_ms)
{
    BOOL    error;
    U32 u32Pattern;
    ...
    if (!error) // time out
    {
        ms_usbhost_msg("<ms_usb_wait_for_completion> Timeout.. Warning[Cancel USB request block%s]","\n");
        ms_usb_unlink_urb(ptr->current_urb);
        ms_usbhost_msg("<ms_usb_wait_for_completion> unlink USB request block done...\n");
    }
    if (cpt->done != 1)
        ms_usbhost_msg("Completion fail %08lX %ld\n", u32Pattern, cpt->done);
}
#endif
...
#endif    
    if (request_block_done.done!=1)
    {
        ms_usbhost_err("<ms_usb_msg_common> Timeout error..\n");
        // TODO: to kill the urb
        //return -ENODEV;                
    }
        ...
        U32 len, int regval, U32 partial)
{
    ...
    switch (regval) 
    {
        case 0:
            if (partial != len) 
            {
                ms_usbhost_msg("--> short transfer%s","\n");
                return XFER_SHORT;
            }

            ms_usbhost_msg("--> transfer complete%s","\n");
            return XFER_GOOD;
        case -EPIPE:
            if (usb_pipecontrol(pipe)) 
            {
                ms_usbhost_msg("--> stall on control pipe%s","\n");
                return XFER_STALLED;
            }
            ...
            return XFER_STALLED;
        case -ETIMEDOUT:
            ms_usbhost_msg("--> timeout or NAK%s","\n");
            return XFER_ERROR;
        case -ENODEV:
            ms_usbhost_msg("<interpret_urb_result> no dev\n");
            return XFER_NO_DEV;
        case -MS_ERR_OVERFLOW:
            ms_usbhost_msg("--> babble%s","\n");
            return XFER_LONG;
        case -ECONNRESET:
            ms_usbhost_msg("--> transfer cancelled%s","\n");
            return XFER_ERROR;
        case -MS_ERR_REMOTEIO:
            ms_usbhost_msg("--> short read transfer%s","\n");
            return XFER_SHORT;
        case -EIO:
            ms_usbhost_msg("--> abort or disconnect in progress%s","\n");
            return XFER_ERROR;
        default:
            ms_usbhost_msg("--> unknown error%s","\n");
            return XFER_ERROR;
    }
}
...
int ms_us_port_reset(struct ms_usdata *pUsdata)
{
    ...
    {   
		...
		if (ms_test_bit(MS_DISCONNECTING, &pUsdata->flags)) {
			...
			ms_usbhost_msg("usb_reset_device returns %d\n",	rval);
            if (rval == -ENODEV)
                rval = 0;
		}
	}
...
int ms_usb_bulk_transport(stCmd_scsi *request_block, struct ms_usdata *ptr)
{
    ...
    {
        ms_usbhost_msg("Bulk logical error%s","\n");
        return TRANSPORT_ERROR;
    }
        ...
        U16 val, U16 idx, void *data, U16 len)
{
    ...
    {
        ms_usbhost_msg("flag: MS_DISCONNECTING%s","\n");
        return FAILED;
    }
    ...
    {
        ms_usbhost_msg("clear reset failed%s","\n");
        return FAILED;
    }
    ms_usbhost_msg("reset command done%s","\n");
    return SUCCESS;
}
