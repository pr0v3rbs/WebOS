...
MS_U32 MIUOpen(void** ppInstance, MS_U32 u32ModuleVersion, void* pAttribute)
{
    ...
    pMIUPri->fpMIU_GetLoading = (IOCTL_MIU_GET_LOADING)_MDrv_MIU_GetLoading;
...
MS_U32 MIUIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    switch(u32Cmd)
    {
        ...
        case MDrv_CMD_MIU_GetLoading:
            MIUDBG("MIUIoctl - MDrv_CMD_MIU_GetLoading\n");
            pMIUGetLoading = (PMIU_MIU_GET_LOADING)pArgs;
            // when calling func occpuy h/w, need to call UtopiaResourceObtainToInstant to obtain res to lock other process
            if(UtopiaResourceObtain(pModule, MIU_POOL_ID_MIU0, &pResource) != 0)
            {
                printf("UtopiaResourceObtainToInstant fail\n");
            ...
            bRet = psMIUInstPri->fpMIU_GetLoading(pMIUGetLoading->u8MiuDev, pMIUGetLoading->u32Loading);
            pMIUGetLoading->bRet = bRet;
            UtopiaResourceRelease(pResource);
        ...
        case MDrv_CMD_MIU_Protect:
            ...
            bRet = psMIUInstPri->fpMIU_Protect(pMIUProtect->u8Blockx, pMIUProtect->pu8ProtectId, pMIUProtect->u32Start, pMIUProtect->u32End, pMIUProtect->bSetFlag);
