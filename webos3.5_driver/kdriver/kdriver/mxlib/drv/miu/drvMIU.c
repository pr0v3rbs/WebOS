...
/// @return             \b 0: Fail 1: OK
////////////////////////////////////////////////////////////////////////////////
MS_BOOL _MDrv_MIU_GetLoading(MS_U8 u8MiuDev, MS_U32 *u32Loading)
{
...
{
    PMIU_MIU_GET_LOADING pMIUGetLoading; 
    MS_BOOL bRet = TRUE;

    if (NULL != pInstantMIU)
    {
        pMIUGetLoading = (PMIU_MIU_GET_LOADING)malloc(sizeof(MIU_GET_LOADING));

        if (pMIUGetLoading == NULL)
        {
            return FALSE;
        }

        pMIUGetLoading->u8MiuDev = u8MiuDev;
        pMIUGetLoading->u32Loading = Loading;
        if(UtopiaIoctl(pInstantMIU,MDrv_CMD_MIU_GetLoading,(void*)pMIUGetLoading) != UTOPIA_STATUS_SUCCESS)
        {
            printf("Ioctl MDrv_CMD_MIU_GetLoading fail in IP %d\n",MDrv_CMD_MIU_GetLoading);
           // free(pMIUGetLoading);
    	    bRet = FALSE;
        }
        free(pMIUGetLoading);
...
void _MDrv_MIU_MaskReqUtopia20IOCtrl(eMIUIoctlCmd eMIUIOCmd, MS_U8 u8Mask, MS_U8 u8Miu)
{
    ...
    if (NULL != pInstantMIU)
    {
        ...
        {
            printf("Ioctl MDrv_CMD_MIU_Mask_Req fail in IP %d\n",eMIUIOCmd);
            free(pMIUMask);
    		return;
        }
        free(pMIUMask);
...
void  MDrv_MIU_MaskReq(MS_U8 u8Miu, eMIUClientID eClientID)
{
    ...
    if (NULL != pInstantMIU)
    {
        ...
        {
            printf("Ioctl MDrv_MIU_MaskReq fail\n");
            free(pMIUMaskReq);
            return;
        }
        free(pMIUMaskReq);
...
void MDrv_MIU_UnMaskReq(MS_U8 u8Miu, eMIUClientID eClientID)
{
    ...
    if (NULL != pInstantMIU)
    {
        ...
        {
            printf("Ioctl MDrv_CMD_MIU_UnMask_Req fail\n");
            free(pMIUMaskReq);
            return;
        }
        free(pMIUMaskReq);
...
MS_BOOL MDrv_MIU_SetSsc(MS_U16 u16Fmodulation, MS_U16 u16FDeviation,MS_BOOL bEnable)
{
    ...
    if (NULL != pInstantMIU)
    {
        ...
        {
            printf("Ioctl MDrv_CMD_MIU_SetSsc fail\n");
            bRet = FALSE;
        }
        else
        {
            bRet = pMIUSetSsc->bRet;
        }
...
MS_BOOL MDrv_MIU_SetSscValue(MS_U8 u8MiuDev, MS_U16 u16Fmodulation, MS_U16 u16FDeviation, MS_BOOL bEnable)
{
    ...
    if (NULL != pInstantMIU)
    {
        ...
        {
            printf("Ioctl MDrv_CMD_MIU_SetSscValue fail\n");
            bRet = FALSE;
        }
        else
        {
            bRet = pMIUSetSscValue->bRet;
        }
...
MS_BOOL MDrv_MIU_Protect( MS_U8 u8Blockx, MS_U8 *pu8ProtectId, MS_U32 u32Start, MS_U32 u32End, MS_BOOL bSetFlag )
{
    ...
    if (NULL != pInstantMIU)
    {
        ...
        pMIUProtect->u32Start = u32Start;
        pMIUProtect->u32End = u32End;
...
MS_BOOL MDrv_MIU_SelMIU(eMIUClientID eClientID, eMIU_SelType eType)
{
    ...
    if (NULL != pInstantMIU)
    {
        ...
        {
            printf("Ioctl MDrv_CMD_MIU_SelMIU fail\n");
            bRet = FALSE;
        }
        else
        {
            bRet = pMIUSelMIU->bRet;
        }
...
MS_BOOL MDrv_MIU_GetProtectInfo(MS_U8 u8MiuDev, MIU_PortectInfo *pInfo)
{
    ...
    if (NULL != pInstantMIU)
    {
        ...
        {
            printf("Ioctl MDrv_CMD_MIU_GetProtectInfo fail\n");
            bRet = FALSE;
        }
        else
        {
            bRet = pMIUGetProtectInfo->bRet;
        }
...
MS_BOOL MDrv_MIU_SetGroupPriority(MS_U8 u8MiuDev, MIU_GroupPriority sPriority)
{
    ...
    if (NULL != pInstantMIU)
    {
        ...
        {
            printf("Ioctl MDrv_CMD_MIU_SetGroupPriority fail\n");
            bRet = FALSE;
        }
        else
        {
            bRet = pMIUSetGroupPri->bRet;
        }
...
MS_BOOL MDrv_MIU_SetHPriorityMask(MS_U8 u8MiuDev, eMIUClientID eClientID, MS_BOOL bMask)
{
    ...
    if (NULL != pInstantMIU)
    {
        ...
        {
            printf("Ioctl MDrv_MIU_SetHPriorityMask fail\n");
            bRet = FALSE;
        }
        else
        {
            bRet = pMIUSetHighPri->bRet;
        }
...
MS_BOOL MDrv_MIU_Dram_Size(MS_U8 u8MiuDev, MS_U8 DramSize)
{
    ...
    if (NULL != pInstantMIU)
    {
        ...
        {
            printf("Ioctl MDrv_CMD_MIU_Dram_Size fail\n");
            bRet = FALSE;
        }
        else
        {
...
{
    PMIU_PROTECT_ALIGN pMIUProtectAlign;
    MS_BOOL bRet = TRUE;
