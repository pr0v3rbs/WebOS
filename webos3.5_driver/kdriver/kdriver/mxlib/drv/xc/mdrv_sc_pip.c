...
void MDrv_XC_EnableWindow(void *pInstance, MS_BOOL bEnable, SCALER_WIN eWindow)
{
    ...
    if ((MDrv_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED)
            &&(!MDrv_SC_GetSkipWaitVsync(pInstance,eWindow)))
    {
    ...
    if(((eWindow == SUB_WINDOW) && (bEnable == FALSE))
        &&(pXCResourcePrivate->bDualWinForAPVR == FALSE))
    {
...
void MApi_XC_Enable_DaulWindowForAPVR_U2(void* pInstance, MS_BOOL bEnable)
{
    ...
    _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
    _XC_ENTRY(pInstance);
    pXCResourcePrivate->bDualWinForAPVR = bEnable;
    _XC_RETURN(pInstance);
    _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
...
void MApi_XC_Enable_DaulWindowForAPVR(MS_BOOL bEnable)
{
    ...
    stXC_ENABLE_DAULWINDOWFORAPVR XCArgs;
    XCArgs.bEnable = bEnable;

    if(UtopiaIoctl(pu32XCInst, E_XC_CMD_ENABLE_DUALWINDOWFORAPVR, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return;
    }
...
MS_BOOL MApi_XC_Get_DaulWindowForAPVR_U2(void* pInstance)
{
    ...
    _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
    _XC_ENTRY(pInstance);
    bEnable = pXCResourcePrivate->bDualWinForAPVR;
    _XC_RETURN(pInstance);
    _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
...
MS_BOOL MApi_XC_Get_DaulWindowForAPVR(void)
{
    ...
    stXC_GET_DAULWINDOWFORAPVR XCArgs;
    XCArgs.bEnable = FALSE;

    if(UtopiaIoctl(pu32XCInst, E_XC_CMD_GET_DUALWINDOWFORAPVR, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return FALSE;
    }
