...
//--------3D static variables-----------------
//--------------------------------------------
static MS_U8 u8SRAMDataA[8][64];
static MS_U8 u8SRAMDataB[8][64];
static PIXEL Pixel10[15];
static PIXEL Pixel20[15];
static PIXEL Pixel30[15];
static PIXEL Pixel40[15];
static PIXEL Pixel10_h[15], Pixel30_h[15] ;
static PIXEL Pixel10_v[15], Pixel30_v[15] ;
static MS_U16 cplx10[15], cplx20[15], cplx30[15], cplx40[15];
static MS_U16 SSYdiff21[7];
static MS_U16 SSYdiff22[7];
static MS_U16 SSYdiff23[7];
static MS_U16 SSYdiff24[7];
static MS_U16 SSYdiff1_h[7];
static MS_U16 SSYdiff2_h[7];
static MS_U16 SSYdiff3_h[7];
static MS_U16 SSYdiff4_h[7];
static MS_U16 SScplxdiff1[7];
static MS_U16 SScplxdiff2[7];
static MS_U16 SScplxdiff3[7];
static MS_U16 SScplxdiff4[7];
static MS_U16 TBYdiff21[15];
static MS_U16 TBYdiff22[15];
static MS_U16 TBYdiff23[15];
static MS_U16 TBYdiff24[15];
static MS_U16 TBYdiff1_v[15];
static MS_U16 TBYdiff2_v[15];
static MS_U16 TBYdiff3_v[15];
static MS_U16 TBYdiff4_v[15];
static MS_U16 TBcplxdiff1[15];
static MS_U16 TBcplxdiff2[15];
static MS_U16 TBcplxdiff3[15];
static MS_U16 TBcplxdiff4[15];
...
void MDrv_SC_3D_Variable_Init(void *pInstance)
{
    ...
    pXCResourcePrivate->stdrvXC_3D._u32HW2DTO3D_AD_Buf = 0;
    ...
    pXCResourcePrivate->stdrvXC_3D.u16PreDiswidth = 0x00;
    pXCResourcePrivate->stdrvXC_3D.u16PreDisheight = 0x00;
    pXCResourcePrivate->stdrvXC_3D.u16PreCapwidth = 0x00;
    pXCResourcePrivate->stdrvXC_3D.u16PreCapheight = 0x00;
    pXCResourcePrivate->stdrvXC_3D.b3Dstate = 0x00;
}
...
MS_BOOL MDrv_SC_3D_IsOutputVfreq48hz(void *pInstance, SCALER_WIN eWindow, MS_U16 u16InputVFreq)
{
    ...
    //Coverity: Overrun: because gSrcInfo has only two element. if eWindow==2 then overrun happen
...
static MS_BOOL MDrv_SC_3D_Cus_Is2TapModeSupportedFormat(void *pInstance, XC_InternalStatus *pSrcInfo, SCALER_WIN eWindow)
{
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(0)],(void**)(&pXCResourcePrivate));
    MS_BOOL bIs2TapMode = 0;
    if ((pSrcInfo->stCapWin.width >= 2880) &&
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"3D:  bIs2TapMode = %d\n",bIs2TapMode);
    return bIs2TapMode;
}
...
MS_BOOL MDrv_SC_3D_Is2TapModeSupportedFormat(void *pInstance, XC_InternalStatus *pSrcInfo, SCALER_WIN eWindow)
{
    ...
    if (MDrv_SC_3D_Cus_Is2TapModeSupportedFormat(pInstance, pSrcInfo, eWindow))
    {
        return TRUE;
    }
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"3D:  MDrv_SC_3D_Is2TapModeSupportedFormat \n");
...
#endif
    switch(MDrv_XC_Get_3D_Output_Mode(pInstance))
    {
        case E_XC_3D_OUTPUT_FRAME_PACKING:
        {   
            if(pSrcInfo->Status2.u16PreVCusScalingSrc > pSrcInfo->Status2.u16PreVCusScalingDst)
            {
#if (HW_DESIGN_3D_VER <= 2)
                return FALSE;
#endif
            }       
            if(pSrcInfo->stCapWin.width <= MS_3D_LINE_BFF_MAX)
            {
                return FALSE;
            }
        ...
        {
            if((pSrcInfo->stCapWin.width > SCALER_LINE_BUFFER_MAX) // 4K2K or 2560x720 SBS-FULL
        ...
        default: //other output format
        {
            if(IS_INPUT_CHECK_BOARD(eWindow)
                || IS_INPUT_PIXEL_ALTERNATIVE(eWindow))
            {
                return TRUE;
            }
...
MS_BOOL MDrv_SC_3D_Set2TapMode(void *pInstance, XC_InternalStatus *pSrcInfo, SCALER_WIN eWindow)
{
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"3D:  MDrv_SC_3D_Set2TapMode \n");
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(0)],(void**)(&pXCResourcePrivate));
    ...
    {
        SC_W2BYTEMSK(0, REG_SC_BK20_10_L, BIT(5), BIT(5)); //Enable HD side by side line buffer mode
        SC_W2BYTEMSK(0, REG_SC_BK23_0B_L, 0x0300, 0xFF00); // If use 2Tap mode, need to disable PQ scaling filter
#if HW_2LINEMODE_DEINTERLACE_SUPPORT
        if(pSrcInfo->bInterlace)
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_18_L, BIT(5), BIT(5)); //Force DI 2-line mode at sc2lb mode
        }
#endif
        ...
        {
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK20_13_L,pSrcInfo->u16H_SizeAfterPreScaling,DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK20_1D_L,(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset&0xFF) || ((pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset&0xFF)<<8),DS_OP);
        }
        else
#endif
        {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK20_13_L, pSrcInfo->u16H_SizeAfterPreScaling, 0x1FFF);
        // When main and sub is start from 0, it means the main&sub win don't need Fill the sub windows line buffer in vertical blanking
        MDrv_XC_FilLineBuffer(pInstance, FALSE, MAIN_WINDOW);
        ...
        SC_W2BYTEMSK(0, REG_SC_BK20_1D_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset&0xFF), 0xFF);
        SC_W2BYTEMSK(0, REG_SC_BK20_1D_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset&0xFF)<<8, 0xFF00);
    }
    }
    else
    {
        SC_W2BYTEMSK(0, REG_SC_BK20_10_L, 0x0, BIT(5)); //Disable HD side by side line buffer mode
#if HW_2LINEMODE_DEINTERLACE_SUPPORT
        if(pSrcInfo->bInterlace)
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_18_L, 0x0, BIT(5)); //Force DI 2-line mode at sc2lb mode
        }
#endif
    }
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"3D: MDrv_SC_3D_Set2TapMode MDrv_SC_3D_Is2TapModeSupportedFormat(pInstance, pSrcInfo, eWindow) = %d\n",MDrv_SC_3D_Is2TapModeSupportedFormat(pInstance, pSrcInfo, eWindow));
    return TRUE;
}
...
MS_BOOL MDrv_SC_3D_PostPQSetting(void *pInstance, XC_InternalStatus *pSrcInfo, SCALER_WIN eWindow)
{
    ...
    if(MDrv_SC_3D_Is2TapModeSupportedFormat(pInstance, pSrcInfo, eWindow))
    {
        ...
        SC_W2BYTE(0, REG_SC_BK23_0B_L, 0x0303);
        SC_W2BYTE(0, REG_SC_BK23_2B_L, 0x0303);
        //disable snr_vertical vertical c low pass and spike NR
        SC_W2BYTEMSK(0, REG_SC_BK26_50_L, 0x0000, 0x00FF);
        //disable v cti
        SC_W2BYTEMSK(0, REG_SC_BK26_31_L, 0x0000, 0xF000);
        //disable anti pal
        SC_W2BYTEMSK(0, REG_SC_BK23_6A_L, 0x0000, BIT(0));
        //spf bypass
        SC_W2BYTEMSK(0, REG_SC_BK0C_70_L, BIT(0), BIT(0));
    }
    return TRUE;
}
                                  ...
                                  SCALER_WIN eWindow)
{
           ...
           )
    {
        ...
        if((gSrcInfo[eWindow].stCapWin.height == DOUBLEHD_1080X2I_VSIZE)    ||
                (gSrcInfo[eWindow].stCapWin.height == DOUBLEHD_1080X2I_50_VSIZE) ||
                (gSrcInfo[eWindow].stCapWin.height == DOUBLEHD_576X2I_VSIZE)     ||
                (gSrcInfo[eWindow].stCapWin.height == DOUBLEHD_480X2I_VSIZE))
        {
...
#define HW_2DTO3DBUFFERSIZE  0x200000
    MS_U32 u32HW2DTO3D_DD_Buf = pXCResourcePrivate->stdrvXC_3D._u32HW2DTO3D_DD_Buf;
    MS_U32 u32HW2DTO3D_DR_Buf = pXCResourcePrivate->stdrvXC_3D._u32HW2DTO3D_DR_Buf;
    ...
    {
        if(((u32HW2DTO3D_DD_Buf >= (MS_IPM_BASE0(pInstance, eWindow) * BYTE_PER_WORD))
        ...
        SCALER_WIN eWindow)
{
    ...
    return u8FBNumType;
#elif (HW_DESIGN_3D_VER > 2)
    if(IS_INPUT_FRAME_PACKING(eWindow) && ((gSrcInfo[eWindow].stCapWin.height == DOUBLEHD_1080X2I_VSIZE) || (gSrcInfo[eWindow].stCapWin.height == DOUBLEHD_1080X2I_50_VSIZE) ||
                                           (gSrcInfo[eWindow].stCapWin.height == DOUBLEHD_576X2I_VSIZE) || (gSrcInfo[eWindow].stCapWin.height == DOUBLEHD_480X2I_VSIZE)))
    {
        u8FBNumType = IMAGE_STORE_4_FRAMES;
    }
...
#endif
        (IS_INPUT_FRAME_PACKING(eWindow)
         && ((gSrcInfo[eWindow].stCapWin.height == DOUBLEHD_1080X2I_VSIZE) || (gSrcInfo[eWindow].stCapWin.height == DOUBLEHD_1080X2I_50_VSIZE) ||
             (gSrcInfo[eWindow].stCapWin.height == DOUBLEHD_576X2I_VSIZE) ||(gSrcInfo[eWindow].stCapWin.height == DOUBLEHD_480X2I_VSIZE)))
        ||
                                   ...
                                   SCALER_WIN eWindow)
{
    ...
    else  if((IS_INPUT_SIDE_BY_SIDE_HALF(eWindow) || IS_INPUT_SIDE_BY_SIDE_FULL(eWindow))
             && (!IS_OUTPUT_MODE_NONE()))
    {
        pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset = pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNROffset/2;
    }
...
#if (HW_DESIGN_3D_VER == 1)
    {
        ...
        if (SC_R2BYTEMSK(0, REG_SC_BK01_70_L, (BIT(15))) )
        {
...
MS_BOOL MDrv_SC_3D_FormatValidation(void *pInstance, XC_InternalStatus *pSrcInfo, SCALER_WIN eWindow)
{
    ...
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(0)],(void**)(&pXCResourcePrivate));
    MS_BOOL bFramePacking = (((pSrcInfo->stCapWin.width == DOUBLEHD_1080X2P_HSIZE)
                              && (pSrcInfo->stCapWin.height == DOUBLEHD_1080X2P_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_720X2P_HSIZE)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_720X2P_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_576X2P_HSIZE)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_576X2P_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_576X2P_HSIZE *2)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_576X2P_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_480X2P_HSIZE)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_480X2P_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_480X2P_HSIZE *2)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_480X2P_VSIZE))
                             || ((pSrcInfo->stCapWin.width == 640)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_480X2P_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_288X2P_HSIZE *2)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_288X2P_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_288X2P_HSIZE)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_288X2P_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_240X2P_HSIZE *2)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_240X2P_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_240X2P_HSIZE)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_240X2P_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_240X2P_1_HSIZE)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_240X2P_1_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_240X2P_1_HSIZE *2)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_240X2P_1_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_1080X2I_HSIZE)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_1080X2I_HSIZE)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_50_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_576X2I_HSIZE)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_576X2I_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_576X2I_HSIZE *2)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_576X2I_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_480X2I_HSIZE)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_480X2I_VSIZE))
                             || ((pSrcInfo->stCapWin.width == DOUBLEHD_480X2I_HSIZE * 2)
                                 && (pSrcInfo->stCapWin.height == DOUBLEHD_480X2I_VSIZE))
                            ) && (!pSrcInfo->bInterlace);
    MS_ASSERT(eWindow < MAX_WINDOW);
    MS_ASSERT(pSrcInfo);
    ...
    if(pXCResourcePrivate->stdrvXC_3D._bNeedRestore3DFormat)
    {
        pXCResourcePrivate->stdrvXC_3D._eInput3DMode[eWindow] = pXCResourcePrivate->stdrvXC_3D._ePreUsedInput3DMode[eWindow];
        pXCResourcePrivate->stdrvXC_3D._eOutput3DMode         = pXCResourcePrivate->stdrvXC_3D._ePreUsedOutput3DMode;
        pXCResourcePrivate->stdrvXC_3D._bNeedRestore3DFormat = FALSE;
    }
    ...
    if(!bRet)
    {
        ...
        pXCResourcePrivate->stdrvXC_3D._ePreUsedInput3DMode[eWindow]   = pXCResourcePrivate->stdrvXC_3D._eInput3DMode[eWindow];
        pXCResourcePrivate->stdrvXC_3D._ePreUsedOutput3DMode           = pXCResourcePrivate->stdrvXC_3D._eOutput3DMode;
        pXCResourcePrivate->stdrvXC_3D._bNeedRestore3DFormat = TRUE;
...
void MDrv_SC_3D_SetWriteMemory(void *pInstance, SCALER_WIN eWindow)
{
	...
	if(TRUE == pXCResourcePrivate->bRecord_Flag)//APVR record
	{
...
void MDrv_SC_3D_Adjust_PixelOffset(void *pInstance, XC_InternalStatus *pSrcInfo,MS_PHY *pu32FRCM_RPixelOffset,MS_U16 *pu16FRCM_ROffset,MS_U16 *pu16FRCM_ROffset3D,SCALER_WIN eWindow)
{
    MS_ASSERT(eWindow < MAX_WINDOW);
...
#else
    _XC_SEMAPHORE_ENTRY(_XC_SELECT_INTERNAL_VARIABLE(0));
...
#else
    _XC_SEMAPHORE_RETURN(_XC_SELECT_INTERNAL_VARIABLE(0));
        ...
        if(pXCResourcePrivate->stdrvXC_3D._u16TotalCount >= MAXFRAME_3DDETECT)
        {
                    ...
                    && (pXCResourcePrivate->stdrvXC_3D._u16SBSCount > pXCResourcePrivate->stdrvXC_3D._u162DCount))
            {
                if(pXCResourcePrivate->stdrvXC_3D._u16SBSCount >= PERCENTAGE_3DDETECT * pXCResourcePrivate->stdrvXC_3D._u16TotalCount / 100)
                {
					...
					if (pXCResourcePrivate->btvservice == TRUE)
					{
...
static MS_U32 _MDrv_SC_3D_2DTo3D_Art_Detect(void* pInstance)
{
    ...
    MS_U32 u32YEnd   =  896;
    MS_U32 u32PureColorBlkCnt = SC_R2BYTE(0, REG_SC_BK15_3D_L); //16bit 3d => 8bit 7a,7b
    MS_U32 u32GradLevelBlkCnt = SC_R2BYTE(0, REG_SC_BK15_3E_L); //16bit 3e => 8bit 7c,7d
    MS_U32 u32HighFreqBlkCnt  = SC_R2BYTE(0, REG_SC_BK15_3F_L); //16bit 3f => 8bit 7e,7f
    ...
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(0)],(void**)(&pXCResourcePrivate));
    // read histogram
    u32HisgrmArray[0] = SC_R2BYTEMSK(0, REG_SC_BK15_30_L, 0xFF00) >> 8;
    u32HisgrmArray[1] = SC_R2BYTEMSK(0, REG_SC_BK15_30_L, 0x00FF);
    u32HisgrmArray[2] = SC_R2BYTEMSK(0, REG_SC_BK15_31_L, 0xFF00) >> 8;
    u32HisgrmArray[3] = SC_R2BYTEMSK(0, REG_SC_BK15_31_L, 0x00FF);
    u32HisgrmArray[4] = SC_R2BYTEMSK(0, REG_SC_BK15_32_L, 0xFF00) >> 8;
    u32HisgrmArray[5] = SC_R2BYTEMSK(0, REG_SC_BK15_32_L, 0x00FF);
    u32HisgrmArray[6] = SC_R2BYTEMSK(0, REG_SC_BK15_33_L, 0xFF00) >> 8;
    u32HisgrmArray[7] = SC_R2BYTEMSK(0, REG_SC_BK15_33_L, 0x00FF);
    u32HisgrmArray[8] = SC_R2BYTEMSK(0, REG_SC_BK15_34_L, 0xFF00) >> 8;
    u32HisgrmArray[9] = SC_R2BYTEMSK(0, REG_SC_BK15_34_L, 0x00FF);
    u32HisgrmArray[10] = SC_R2BYTEMSK(0, REG_SC_BK15_35_L, 0xFF00) >> 8;
    u32HisgrmArray[11] = SC_R2BYTEMSK(0, REG_SC_BK15_35_L, 0x00FF);
    u32HisgrmArray[12] = SC_R2BYTEMSK(0, REG_SC_BK15_36_L, 0xFF00) >> 8;
    u32HisgrmArray[13] = SC_R2BYTEMSK(0, REG_SC_BK15_36_L, 0x00FF);
    u32HisgrmArray[14] = SC_R2BYTEMSK(0, REG_SC_BK15_37_L, 0xFF00) >> 8;
    u32HisgrmArray[15] = SC_R2BYTEMSK(0, REG_SC_BK15_37_L, 0x00FF);

    //------------- Set Detection Window --------------------------
    SC_W2BYTE(0, REG_SC_BK15_28_L, u32XStart);
    SC_W2BYTE(0, REG_SC_BK15_29_L, u32XEnd);
    SC_W2BYTE(0, REG_SC_BK15_2A_L, u32YStart);
    SC_W2BYTE(0, REG_SC_BK15_2B_L, u32YEnd);
...
void _MDrv_SC_3D_2DTo3D_DepthRefine(SC_INT_SRC eIntNum, void *pParam)
{
    ...
    MS_U32 u32DRBase = (MS_U32)MsOS_PA2KSEG1(pXCResourcePrivate->stdrvXC_3D._u32HW2DTO3D_DR_Buf);
    if(0 == u32DRBase)
    {
...
//                    _u8YCtrArray[u8Ptr][i * HBLOCKNUM + j]= u8YCtr_Cur;
        ...
        SC_W2BYTEMSK(0, REG_SC_BK15_65_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Offset, 0x00FF);//left depth offset
        SC_W2BYTEMSK(0, REG_SC_BK15_66_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Offset, 0x00FF);//right depth offset
#if (HW_2DTO3D_VER < 2)
        SC_W2BYTEMSK(0, REG_SC_BK15_65_L, ((pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Gain*u32NatWt)>>4)<<8, 0x3F00);//left depth gain:6bits
        SC_W2BYTEMSK(0, REG_SC_BK15_66_L, ((pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Gain*u32NatWt)>>4)<<8, 0x3F00);//right depth gain:6bits
#else
        SC_W2BYTEMSK(0, REG_SC_BK15_65_L, ((pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Gain*u32NatWt)>>4)<<8, 0x7F00);//left depth gain:7bits
        SC_W2BYTEMSK(0, REG_SC_BK15_66_L, ((pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Gain*u32NatWt)>>4)<<8, 0x7F00);//right depth gain:7bits
#endif
        if(SC_R2BYTEMSK(0, REG_SC_BK15_6C_L,BIT(1)))
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_65_L, 0x7F00, 0x7F00);//left depth offset
            SC_W2BYTEMSK(0, REG_SC_BK15_66_L, 0x3F00, 0x7F00);//right depth offset
        }
        if(pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16EdgeBlackWidth > 0)
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_72_L, BIT(7), BIT(7));                               //draw left/right black enable
            SC_W2BYTEMSK(0, REG_SC_BK15_72_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16EdgeBlackWidth, 0x007F);  //draw left/right black width
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_72_L, 0x0000, 0x00FF);  //draw left/right black enable, draw left/right black width
        }
                ...
                && pXCResourcePrivate->stdrvXC_3D._bEnableAutoDetect3D))
    {
        ...
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_41_L, 0x0001, 0x000F);   // DD force bank1
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_41_L, 0x0000, 0x000F);   // DD force bank0
        }
...
#define DISPLAY_DD_H 1920//MIN(gSrcInfo[MAIN_WINDOW].stDispWin.width, 1920)
#define DISPLAY_DD_V 1080//MIN(gSrcInfo[MAIN_WINDOW].stDispWin.height, 1080)
//int DISPLAY_DD_H = 0;
//int DISPLAY_DD_V = 0;
...
#define NBLK_DR_H ((3840 + PIXELNUMOFBLOCK - 1) / PIXELNUMOFBLOCK)  //dr h block
#define NBLK_DR_V ((2160 + PIXELNUMOFBLOCK - 1) / PIXELNUMOFBLOCK) //dr v block
...
#endif
static MS_U8 u8QueueDataBLKWeight[NBLK_DR_V][NBLK_DR_H];
static MS_U8 u8QueueDataBLKDepth[NBLK_DR_V][NBLK_DR_H];
static void _MDrv_SC_3D_2DTo3D_HW_Info_Write(void *pInstance)
{
...
#endif
    //MS_U8 u8QueueDataBLKWeight[NBLK_DR_V][NBLK_DR_H];
    //MS_U8 u8QueueDataBLKDepth[NBLK_DR_V][NBLK_DR_H];
    MS_S16 x_0 = 0, x_1 = 0, x_2 = 0, x_3 = 0, x_4 = 0, x_5 = 0, x_6 = 0, x_7 = 0;
    ...
    //if(gSrcInfo[MAIN_WINDOW].stDispWin.width==0xf00)
    {
        u8DRBLKLinePitch = 8*16;//256 bit alignment = 32 byte
        u8DRPixBLKLinePitch = 8*16;//256 bit alignment = 32 byte
    }
    //else //FHD
    //{
    //u8DRBLKLinePitch = 4*16;//256 bit alignment = 32 byte
    //u8DRPixBLKLinePitch = 4*16;//256 bit alignment = 32 byte
    //}
#if AUTO_ADJUST
    MS_S32 s32Total = 0;
    MS_S32 s32Count = 0;
    ...
    for ( y = 0; y < NBLK_DR_V; y++ )
    {
        for ( x = 0; x < NBLK_DR_H; x += 8 )
        {
            x_0 = MIN(x, NBLK_DR_H-1);
            x_1 = MIN(x+1, NBLK_DR_H-1);
            x_2 = MIN(x+2, NBLK_DR_H-1);
            x_3 = MIN(x+3, NBLK_DR_H-1);
            x_4 = MIN(x+4, NBLK_DR_H-1);
            x_5 = MIN(x+5, NBLK_DR_H-1);
            x_6 = MIN(x+6, NBLK_DR_H-1);
            x_7 = MIN(x+7, NBLK_DR_H-1);
            (*((volatile MS_U64*)((y*u8DRBLKLinePitch + x) + u32DRBase))) = (((MS_U64)u8QueueDataBLKWeight[y][x_7] << 56)
                    |((MS_U64)u8QueueDataBLKWeight[y][x_6] << 48)
                    |((MS_U64)u8QueueDataBLKWeight[y][x_5] << 40)
                    |((MS_U64)u8QueueDataBLKWeight[y][x_4] << 32)
                    |((MS_U64)u8QueueDataBLKWeight[y][x_3] << 24)
                    |((MS_U64)u8QueueDataBLKWeight[y][x_2] << 16)
                    |((MS_U64)u8QueueDataBLKWeight[y][x_1] << 8)
                    |((MS_U64)u8QueueDataBLKWeight[y][x_0])) ;          //dr pixel base address
            (*((volatile MS_U64*)((y*u8DRBLKLinePitch + x) + u32DRBase + 0x80000))) = (((MS_U64)u8QueueDataBLKDepth[y][x_7] << 56)
                    |((MS_U64)u8QueueDataBLKDepth[y][x_6] << 48)
                    |((MS_U64)u8QueueDataBLKDepth[y][x_5] << 40)
                    |((MS_U64)u8QueueDataBLKDepth[y][x_4] << 32)
                    |((MS_U64)u8QueueDataBLKDepth[y][x_3] << 24)
                    |((MS_U64)u8QueueDataBLKDepth[y][x_2] << 16)
                    |((MS_U64)u8QueueDataBLKDepth[y][x_1] << 8)
                    |((MS_U64)u8QueueDataBLKDepth[y][x_0])) ; //dr block base address

        }
        MsOS_FlushMemory();
    }

    UNUSED(u8DRPixBLKLinePitch);
}
#if (HW_2DTO3D_VER == 3)
...
static void _MDrv_SC_3D_2DTo3D_BGColorDetection(void *pInstance)
{
    ...
    for ( y = 0; y < NBLK_DD_V; y ++ )      //CID:116528
    {
        for ( x = 0; x < NBLK_DD_H; x ++ )
        {
            s16MaxEdgeRGB = pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y][x];
...
#elif (HW_2DTO3D_VER == 4)
int preCbTpMaxidx = 1, preCrTpMaxidx = 1, preCbBnMaxidx = 1, preCrBnMaxidx = 1;
int ppreCbTpMaxidx = 2, ppreCrTpMaxidx = 2, ppreCbBnMaxidx = 2, ppreCrBnMaxidx = 2;
int preCbTpMaxcnt = 1, preCrTpMaxcnt = 1, preCbDnMaxcnt = 1, preCrDnMaxcnt = 1;
static void _MDrv_SC_3D_2DTo3D_BGColorDetection_U9(void* pInstance)
{
    ...
    pXCResourcePrivate->stdrvXC_3D.BGColor_job_id = (pXCResourcePrivate->stdrvXC_3D.BGColor_job_id+1)%2;
    int x = 0;
    int y = 0;
    if(pXCResourcePrivate->stdrvXC_3D.BGColor_job_id == 0)
    {
        //aceGN
        static MS_U32 CbHistoTp[32];
        static MS_U32 CrHistoTp[32];
        static MS_U32 CbHistoBn[32];
        static MS_U32 CrHistoBn[32];
        static MS_U32 YHistoTp[32];
        static MS_U32 YHistoBn[32];

        MS_U32 XCbHistoTp[32];
        MS_U32 XCrHistoTp[32];
        MS_U32 XCbHistoBn[32];
        MS_U32 XCrHistoBn[32];
        MS_U32 XYHistoTp[32];
        MS_U32 XYHistoBn[32];

        PIXEL /*pp, */ppYCC;
        int cplx;
        int CbTpMaxidx=0,CrTpMaxidx=0,CbBnMaxidx=0,CrBnMaxidx=0,CbMaxidx=0,CrMaxidx=0;
        int CbTpMaxcnt=0,CrTpMaxcnt=0,CbBnMaxcnt=0,CrBnMaxcnt=0/*,CbMaxcnt=0,CrMaxcnt=0*/;
        int YTpMaxidx=0,YBnMaxidx=0/*,YTpMaxcnt=0,YBnMaxcnt=0*/;

        //int CbTpMaxinBncnt = 0, CrTpMaxinBncnt = 0;
        MS_U32 u32ScalingRatio = DISPLAY_DD_H*DISPLAY_DD_V*1000/FHD_W/FHD_H;
        for(x =0; x<32; x++)
        {
            XCbHistoTp[x] = 0;
        ...
        for ( y = 0; y < NBLK_DD_V/4; y ++ )
            for ( x = 0; x < NBLK_DD_H; x++ )
            {
                ...
                {
                    XCbHistoTp[ppYCC.B/8]++;
                    XCrHistoTp[ppYCC.R/8]++;
                    XYHistoTp[ppYCC.G/8]++;
                }
            }
        for(x =0; x<32; x++) //aceGN
        {
            CbHistoTp[x] = XCbHistoTp[x];
            CrHistoTp[x] = XCrHistoTp[x];
            YHistoTp[x] = XYHistoTp[x];
        }
        ...
        for ( y = NBLK_DD_V*3/4; y < NBLK_DD_V; y ++ )
            for ( x = 0; x < NBLK_DD_H; x++ )
            {
                ...
                {
                    XCbHistoBn[ppYCC.B/8]=XCbHistoBn[ppYCC.B/8]+1;
                    XCrHistoBn[ppYCC.R/8]=XCrHistoBn[ppYCC.R/8]+1;
                    XYHistoBn[ppYCC.G/8]=XYHistoBn[ppYCC.G/8]+1;
                }
            }
        for(x =0; x<32; x++) //aceGN
        {
            CbHistoBn[x] = XCbHistoBn[x];
            CrHistoBn[x] = XCrHistoBn[x];
            YHistoBn[x] = XYHistoBn[x];
        }

        for(x =0; x<32; x++)
        {
            CbTpMaxidx = (CbHistoTp[x]>(CbHistoTp[CbTpMaxidx]+((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x00E0)>>4)))?x:CbTpMaxidx;
            CrTpMaxidx = (CrHistoTp[x]>(CrHistoTp[CrTpMaxidx]+((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x00E0)>>4)))?x:CrTpMaxidx;
            CbBnMaxidx = (CbHistoBn[x]>(CbHistoBn[CbBnMaxidx]+((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x00E0)>>4)))?x:CbBnMaxidx;
            CrBnMaxidx = (CrHistoBn[x]>(CrHistoBn[CrBnMaxidx]+((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x00E0)>>4)))?x:CrBnMaxidx;
        ...
        if((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x0002)==2)
        {

            printf("[!CbTpMaxidx: %d, CrTpMaxidx:%d]\r\n",CbTpMaxidx,CrTpMaxidx);
        ...
        if((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x0002)==2)
        {
            ...
            printf("[!CbMaxCnt: %d, CrMaxCnt: %d ]\r\n",CbTpMaxcnt,CrTpMaxcnt);
        ...
        CntL2 = (CbTpMaxidx-2<0)?0:CbHistoBn[CbTpMaxidx-2];
        CntL1 = (CbTpMaxidx-1<0)?0:CbHistoBn[CbTpMaxidx-1];
        ...
        CntL2 = (CrTpMaxidx-2<0)?0:CrHistoBn[CrTpMaxidx-2];
        CntL1 = (CrTpMaxidx-1<0)?0:CrHistoBn[CrTpMaxidx-1];
        CntCur= CbHistoBn[CbTpMaxidx];
...
#if 1 //aceGN
        if(abs(CbTpMaxcnt-preCbTpMaxcnt)<=1)
            pXCResourcePrivate->stdrvXC_3D.BGColor_CbTp = preCbTpMaxidx*8;
        if(abs(CrTpMaxcnt-preCrTpMaxcnt)<=1)
            pXCResourcePrivate->stdrvXC_3D.BGColor_CrTp = preCrTpMaxidx*8;
        if(abs(CbBnMaxcnt-preCbDnMaxcnt)<=1)
            pXCResourcePrivate->stdrvXC_3D.BGColor_CbBn = preCbBnMaxidx*8;
        if(abs(CbBnMaxcnt-preCbDnMaxcnt)<=1)
            pXCResourcePrivate->stdrvXC_3D.BGColor_CrBn = preCrBnMaxidx*8;
#endif
#if 1 //aceGN


        //to avoid unstable BGColor
        if(CbTpMaxidx == preCbTpMaxidx)
            pXCResourcePrivate->stdrvXC_3D.BGColor_CbTp = CbTpMaxidx*8;
        else if(preCbTpMaxidx == ppreCbTpMaxidx) //Stable prev BG
            pXCResourcePrivate->stdrvXC_3D.BGColor_CbTp = preCbTpMaxidx*8;
        else //   3 consecutive diff BG
            pXCResourcePrivate->stdrvXC_3D.BGColor_CbTp = CbTpMaxidx*8;

        if(CrTpMaxidx == preCrTpMaxidx)
            pXCResourcePrivate->stdrvXC_3D.BGColor_CrTp = CrTpMaxidx*8;
        else if(preCrTpMaxidx == ppreCrTpMaxidx) //Stable prev BG
            pXCResourcePrivate->stdrvXC_3D.BGColor_CrTp = preCrTpMaxidx*8;
        else //   3 consecutive diff BG
            pXCResourcePrivate->stdrvXC_3D.BGColor_CrTp = CrTpMaxidx*8;

        if(CbBnMaxidx == preCbBnMaxidx)
            pXCResourcePrivate->stdrvXC_3D.BGColor_CbBn = CbBnMaxidx*8;
        else if(preCbBnMaxidx == ppreCbBnMaxidx) //Stable prev BG
            pXCResourcePrivate->stdrvXC_3D.BGColor_CbBn = preCbBnMaxidx*8;
        else //   3 consecutive diff BG
            pXCResourcePrivate->stdrvXC_3D.BGColor_CbBn = CbBnMaxidx*8;

        if(CrBnMaxidx == preCrBnMaxidx)
            pXCResourcePrivate->stdrvXC_3D.BGColor_CrBn = CrBnMaxidx*8;
        else if(preCrBnMaxidx == ppreCrBnMaxidx) //Stable prev BG
            pXCResourcePrivate->stdrvXC_3D.BGColor_CrBn = preCrBnMaxidx*8;
        else //   3 consecutive diff BG
            pXCResourcePrivate->stdrvXC_3D.BGColor_CrBn = CrBnMaxidx*8;
...
#endif
        //
        ppreCbTpMaxidx = preCbTpMaxidx;
        preCbTpMaxidx = CbTpMaxidx;
        ppreCrTpMaxidx = preCrTpMaxidx;
        preCrTpMaxidx = CrTpMaxidx;
        ppreCbBnMaxidx = preCbBnMaxidx;
        preCbBnMaxidx = CbBnMaxidx;
        ppreCrBnMaxidx = preCrBnMaxidx;
        preCrBnMaxidx = CrBnMaxidx;

        preCbTpMaxcnt = CbTpMaxcnt;
        preCrTpMaxcnt = CrTpMaxcnt;
        preCbDnMaxcnt = CbBnMaxcnt;
        preCrDnMaxcnt = CrBnMaxcnt;
        //pXCResourcePrivate->stdrvXC_3D.BGColor_CrTp = CrTpMaxidx*8;
        //pXCResourcePrivate->stdrvXC_3D.BGColor_CbBn = CbBnMaxidx*8;
        //pXCResourcePrivate->stdrvXC_3D.BGColor_CrBn = CrBnMaxidx*8;
    ...
    else //BGColor_job_id==1
    {
        MS_U16 _u16BGColorBlkCntTp = 0;
        MS_U16 _u16BGColorBlkCntBn = 0;
        _u16BGColorBlkCntTp = 0;
        _u16BGColorBlkCntBn = 0;
        for ( y = 0; y < NBLK_DD_V ; y ++ )
            for ( x = 0; x < NBLK_DD_H; x ++ )
            {

                if(y>=0&&y<(NBLK_DD_V>>2)&&( pXCResourcePrivate->stdrvXC_3D._bTpBGColorMarker[y][x] == true ))
                    _u16BGColorBlkCntTp++;
                if(y>=((NBLK_DD_V*3)>>2)&&y<NBLK_DD_V&&( pXCResourcePrivate->stdrvXC_3D._bTpBGColorMarker[y][x] == true ))
                    _u16BGColorBlkCntBn++;
            }

        pXCResourcePrivate->stdrvXC_3D.BGColorBlkCntTp = _u16BGColorBlkCntTp;
        pXCResourcePrivate->stdrvXC_3D.BGColorBlkCntBn = _u16BGColorBlkCntBn;
...
//  MS_U8 BGColorTrustWei_forTBABGColor;
    ...
    MS_S16 u8testval4 = 0x1C;//MFC_ReadByte(REG_0EB9);//0x20
#if DUMYGAINCTRL
    MS_U8 u8tmpval = (SC_R2BYTEMSK(0, REG_SC_BK23_5A_L, 0xff));//MFC_ReadByte(REG_1EFA);
    //MS_U8 u8tmpval2 = 0x00;

    MS_S16 u8testval7 = (SC_R2BYTEMSK(0, REG_SC_BK23_5C_L, 0xff)); //for edge depth
    MS_S16 u8testval8 = (SC_R2BYTEMSK(0, REG_SC_BK23_5D_L, 0xff00))>>8;//0x07; //for [0]:cbcr enable  [1]:y enable [2]:edge enable
    MS_S16 u8testval9 = (SC_R2BYTEMSK(0, REG_SC_BK23_5D_L, 0xff));//0x10; //for sat_wei
    MS_S16 u8testvalA = (SC_R2BYTEMSK(0, REG_SC_BK23_5E_L, 0xff00))>>8;//0x10 ; //for y_depth gain
#else
        ...
        for ( rIdx = 0; rIdx < 8; rIdx++ )
            for ( gIdx = 0; gIdx < 8; gIdx++ )
                for ( bIdx = 0; bIdx < 8; bIdx++ )
                {
                    ...
                    if(SC_R2BYTEMSK(0, REG_SC_BK23_5F_L, 0x1)==0x1)
                    ...
                    if((u8testval8&0x03)==0x01)
                        ...
                        depth = (CrCbDepth*sat_wei + yDepth *(32-sat_wei)+16)>>5;
#endif
                    //method3
                    pRGB.R = r;
                    pRGB.G = g;
                    ...
                    if((u8tmpval&0x40)==0x00)
                    {
                        ...
                        BGDepth = ( MIN(16,depth)*BGColorTrustWei+depth*(32-BGColorTrustWei)+16)>>5;
                        //if((SC_R2BYTEMSK(0, REG_SC_BK23_59_L,0x8)==8))
                        //BGDepth = 16;
                    ...
                    //LOCALDEPTH_ENHANCE
                    int skyRatio1, skyRatio2;
                    int skyRatioDiff_wei;

                    int gainU=0, gainD=0;
                    int offset;

                    skyRatio1 = pXCResourcePrivate->stdrvXC_3D._s16BLKblueSkyRatio1;
                    skyRatio2 = MIN(pXCResourcePrivate->stdrvXC_3D._s16BLKblueSkyRatio1,pXCResourcePrivate->stdrvXC_3D._s16BLKblueSkyRatio2);
        ...
        for ( rIdx = 0; rIdx < 8; rIdx++ )
            for ( gIdx = 0; gIdx < 8; gIdx++ )
                for ( bIdx = 0; bIdx < 8; bIdx++ )
                {
                    ...
                    {
                        int black_base_depth;
                        black_base_depth = u8testval2;//48//u8tmpval2;//
                    ...
                    pRGB.B = b;
                    pYCC = RGB2YUV(pRGB);
                    if((u8tmpval&0x40)==0x00)//aceGN
                        //if(0)//Off for unstable _u8TBABGColorWei(BGColorRatio2)
                    {
                        ...
                        BGDepth_forTpBGColor = ( MIN(16,depth)*pXCResourcePrivate->stdrvXC_3D._u8TBABGColorWei+depth*(32-pXCResourcePrivate->stdrvXC_3D._u8TBABGColorWei)+16)>>5;
                    ...
                    if(((SC_R2BYTE(0, REG_SC_BK23_59_L)&0xFF00)>>8)!=0)
                        egde_depth = ((SC_R2BYTE(0, REG_SC_BK23_59_L)&0xFF00)>>8);
                    depth = depth + egde_depth;
...
static void _MDrv_SC_3D_2DTo3D_UpdateRGB3DLUT(void *pInstance)
{
    ...
    SC_W2BYTE(0, REG_SC_BK62_73_L, 0x0000);//Choose in TBA,Choose in TBB
    SC_W2BYTEMSK(0, REG_SC_BK62_71_L,0x00,0x80);//SRAM write disable
    ...
    for(u8Idx =0; u8Idx<8; u8Idx++)
        for(u8AddrIdx =0; u8AddrIdx<16; u8AddrIdx++)
        {
            ...
            //Write SRAM A
            SC_W2BYTEMSK(0, REG_SC_BK62_71_L, u8AddrIdx<<8, 0xff00);//SRAM Addr
            SC_W2BYTEMSK(0, REG_SC_BK62_73_L, (0x1<<u8Idx)&0xFF, 0xff);//Choose SRAM
            SC_W2BYTE(0, REG_SC_BK62_74_L, (u8SRAMDataIn1<<8) | u8SRAMDataIn0);//SRAM Data
            SC_W2BYTE(0, REG_SC_BK62_75_L, (u8SRAMDataIn3<<8)|u8SRAMDataIn2);//SRAM Data
            SC_W2BYTEMSK(0, REG_SC_BK62_71_L,0x80,0x80);//SRAM write enable
            SC_W2BYTEMSK(0, REG_SC_BK62_71_L,0x00,0x80);//SRAM write disable
            SC_W2BYTEMSK(0, REG_SC_BK62_73_L, 0x00, 0xff);//Choose no SRAM
        }
    ...
    for(u8Idx =0; u8Idx<8; u8Idx++)
        for(u8AddrIdx =0; u8AddrIdx<16; u8AddrIdx++)
        {
            ...
            //Write SRAM B
            SC_W2BYTEMSK(0, REG_SC_BK62_71_L, u8AddrIdx<<8, 0xff00);//SRAM Addr
            SC_W2BYTEMSK(0, REG_SC_BK62_73_L, ((0x1<<u8Idx)&0xFF)<<8, 0xff00);//Choose SRAM
            SC_W2BYTE(0, REG_SC_BK62_74_L, (u8SRAMDataIn1<<8) | u8SRAMDataIn0);//SRAM Data
            SC_W2BYTE(0, REG_SC_BK62_75_L, (u8SRAMDataIn3<<8)|u8SRAMDataIn2);//SRAM Data
            SC_W2BYTEMSK(0, REG_SC_BK62_71_L,0x80,0x80);//SRAM write enable
            SC_W2BYTEMSK(0, REG_SC_BK62_71_L,0x00,0x80);//SRAM write disable
            SC_W2BYTEMSK(0, REG_SC_BK62_73_L, 0x0000, 0xff00);//Choose no SRAM
        }
}
#elif (HW_2DTO3D_VER == 4)
static void _MDrv_SC_3D_2DTo3D_UpdateRGB3DLUT_AutoDownload(void *pInstance)
{
    ...
    MS_U8 u8SRAMDataIn3 = 0;
    MS_U32 addr, u8Idx;
    //MS_U32 AD_BUFFER_OFFSET = 0x100000;
    MS_U32 AD_BUFFER_OFFSET = 0xf0000;//0x0000100000
    MS_U32 AD_TBLB_OFFSET = 0x200;
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    XC_RESOURCE_PRIVATE* pXCResourcePrivate = NULL;
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(0)],(void**)(&pXCResourcePrivate));

    pXCResourcePrivate->stdrvXC_3D.job_id = (pXCResourcePrivate->stdrvXC_3D.job_id+1)%3;
    MS_U32 u32DDBase = (MS_U32)MsOS_PA2KSEG1(pXCResourcePrivate->stdrvXC_3D._u32HW2DTO3D_DR_Buf);

    //MS_U32 u32DDBase = (MS_U32)MsOS_PA2KSEG1(pXCResourcePrivate->stdrvXC_3D._u32HW2DTO3D_AD_Buf);
    if(0 == u32DDBase)
    {
        printf("%s :u32DDBase = 0!,error!!!\n", __FUNCTION__);
        assert(u32DDBase != 0);
    ...
    if(pXCResourcePrivate->stdrvXC_3D.job_id==0)
    {
        MS_U8 u8AddrIdx;
        for(u8AddrIdx =0; u8AddrIdx<64; u8AddrIdx++)
        {
            MS_U8 u8IdxR = u8AddrIdx&0x03;
            MS_U8 u8IdxG = (u8AddrIdx>>2)&0x03;
            MS_U8 u8IdxB = (u8AddrIdx>>4)&0x03;
            /*if((SC_R2BYTE(0,REG_SC_BK23_59_L)&0x0020)==0x00)
            {
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[0][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxR*2+0][u8IdxG*2+0][u8IdxB*2+0];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[1][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxR*2+1][u8IdxG*2+0][u8IdxB*2+0];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[2][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxR*2+0][u8IdxG*2+1][u8IdxB*2+0];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[3][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxR*2+1][u8IdxG*2+1][u8IdxB*2+0];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[4][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxR*2+0][u8IdxG*2+0][u8IdxB*2+1];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[5][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxR*2+1][u8IdxG*2+0][u8IdxB*2+1];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[6][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxR*2+0][u8IdxG*2+1][u8IdxB*2+1];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[7][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxR*2+1][u8IdxG*2+1][u8IdxB*2+1];

            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[0][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxR*2+0][u8IdxG*2+0][u8IdxB*2+0];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[1][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxR*2+1][u8IdxG*2+0][u8IdxB*2+0];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[2][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxR*2+0][u8IdxG*2+1][u8IdxB*2+0];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[3][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxR*2+1][u8IdxG*2+1][u8IdxB*2+0];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[4][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxR*2+0][u8IdxG*2+0][u8IdxB*2+1];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[5][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxR*2+1][u8IdxG*2+0][u8IdxB*2+1];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[6][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxR*2+0][u8IdxG*2+1][u8IdxB*2+1];
            pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[7][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxR*2+1][u8IdxG*2+1][u8IdxB*2+1];
            }
            else*/
            {
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[0][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxB*2+0][u8IdxG*2+0][u8IdxR*2+0];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[1][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxB*2+1][u8IdxG*2+0][u8IdxR*2+0];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[2][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxB*2+0][u8IdxG*2+1][u8IdxR*2+0];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[3][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxB*2+1][u8IdxG*2+1][u8IdxR*2+0];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[4][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxB*2+0][u8IdxG*2+0][u8IdxR*2+1];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[5][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxB*2+1][u8IdxG*2+0][u8IdxR*2+1];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[6][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxB*2+0][u8IdxG*2+1][u8IdxR*2+1];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[7][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_A[u8IdxB*2+1][u8IdxG*2+1][u8IdxR*2+1];

                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[0][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxB*2+0][u8IdxG*2+0][u8IdxR*2+0];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[1][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxB*2+1][u8IdxG*2+0][u8IdxR*2+0];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[2][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxB*2+0][u8IdxG*2+1][u8IdxR*2+0];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[3][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxB*2+1][u8IdxG*2+1][u8IdxR*2+0];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[4][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxB*2+0][u8IdxG*2+0][u8IdxR*2+1];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[5][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxB*2+1][u8IdxG*2+0][u8IdxR*2+1];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[6][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxB*2+0][u8IdxG*2+1][u8IdxR*2+1];
                pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[7][u8AddrIdx] = pXCResourcePrivate->stdrvXC_3D._u8RGB3DLUT_B[u8IdxB*2+1][u8IdxG*2+1][u8IdxR*2+1];
            }
        }

    }
    else if(pXCResourcePrivate->stdrvXC_3D.job_id==1)
    {
        ...
        for(addr =0; addr<=15; addr++)
        {
            ...
            {
                //#if 0
                //if(SC_R2BYTEMSK(0, REG_SC_BK23_59_L,0x1)==1)
                {
                    u8SRAMDataIn0 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[u8Idx][addr*4+0];
                    u8SRAMDataIn1 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[u8Idx][addr*4+1];
                    u8SRAMDataIn2 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[u8Idx][addr*4+2];
                    u8SRAMDataIn3 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[u8Idx][addr*4+3];
                }
                //else
                //    {
//#else
                //    u8SRAMDataIn0 = 0x77+u8Idx+addr;
                //    u8SRAMDataIn1 = 0x22+u8Idx+addr;
                //    u8SRAMDataIn2 = 0x33+u8Idx+addr;
                //    u8SRAMDataIn3 = 0x44+u8Idx+addr;
                //    }

//#endif
                (*((volatile MS_U32*)(addr*32 + (u8Idx^0x4)*4 + u32DDBase + AD_BUFFER_OFFSET))) = ((MS_U32)u8SRAMDataIn3<<24|(MS_U32)u8SRAMDataIn2<<16|(MS_U32)u8SRAMDataIn1<<8|(MS_U32)u8SRAMDataIn0);
                /*if((SC_R2BYTEMSK(0, REG_SC_BK23_59_L,0x8)==8))
                {

                printf("\33[0;35m ace==================[@%x: 0x%x]===== \33[m \r\n",(addr*32 + (u8Idx^0x4)*4),((MS_U32)u8SRAMDataIn3<<24|(MS_U32)u8SRAMDataIn2<<16|(MS_U32)u8SRAMDataIn1<<8|(MS_U32)u8SRAMDataIn0));
        ...
        //SC_W2BYTEMSK(0, REG_SC_BK23_59_L,0,0x8);
        //On DMA_en
        SC_W2BYTEMSK(0, REG_SC_BK62_68_L, 0x60, 0x60);//On AutoDownload
    }
    else
    {
        ...
        for(addr =0; addr<=15; addr++)
        {
            for(u8Idx = 0; u8Idx < 8; u8Idx++)
            {
                ...
                u8SRAMDataIn0 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[u8Idx][addr*4+0];
                u8SRAMDataIn1 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[u8Idx][addr*4+1];
                u8SRAMDataIn2 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[u8Idx][addr*4+2];
                u8SRAMDataIn3 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[u8Idx][addr*4+3];



                (*((volatile MS_U32*)(addr*32 + (u8Idx^0x4)*4 + u32DDBase + AD_BUFFER_OFFSET + AD_TBLB_OFFSET))) =  ((MS_U32)u8SRAMDataIn3<<24|(MS_U32)u8SRAMDataIn2<<16|(MS_U32)u8SRAMDataIn1<<8|(MS_U32)u8SRAMDataIn0);
...
#if 1
static void _MDrv_SC_3D_2DTo3D_UpdateRGB3DLUT_tmp(void *pInstance)
{
    //_MLOAD_ENTRY();
    //MS_U8 u8AddrIdx = 0;
    //MS_U8 u8IdxR = 0, u8IdxG = 0, u8IdxB = 0;
...
//    MS_U8 prv_addr;
    ...
    pXCResourcePrivate->stdrvXC_3D.job_id = (pXCResourcePrivate->stdrvXC_3D.job_id+1)%17;//129;//9

#if 1
    if(pXCResourcePrivate->stdrvXC_3D.job_id == 16)
#else
    if(pXCResourcePrivate->stdrvXC_3D.job_id >= 1)
#endif
    {
        MS_U8 u8AddrIdx;
        for(u8AddrIdx =0; u8AddrIdx<64; u8AddrIdx++)
        ...
        //SC_W2BYTEMSK(REG_SC_BK62_71_L,0x80,0x80);//SRAM write enable(always)
    }
    else if(pXCResourcePrivate->stdrvXC_3D.job_id <= 7)
    {
        ...
        MS_U8 u8Idx;
#if 1
        u8Idx = pXCResourcePrivate->stdrvXC_3D.job_id;
#else
        if((SC_R2BYTE(REG_SC_BK23_5D_L) & 0x40) == 0x40)
        {
            u8Idx = min(pXCResourcePrivate->stdrvXC_3D.job_id,((SC_R2BYTE(REG_SC_BK23_5D_L) & 0xe000)>>13));
        }
        else
        {
            u8Idx = 0;
        ...
        for(loop_idx = 0; loop_idx <= 15; loop_idx++ )
        {
            //fill SW addr
            //if((SC_R2BYTE(REG_SC_BK23_5D_L) & 0x10) == 0x00)
            SC_W2BYTEMSK(0,REG_SC_BK62_71_L, (loop_idx)<<8, 0x0f00);


            //if((SC_R2BYTE(REG_SC_BK23_5D_L) & 0x20) == 0x00)
            {

                u8SRAMDataIn0 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[u8Idx][loop_idx*4+0];
                u8SRAMDataIn1 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[u8Idx][loop_idx*4+1];
                u8SRAMDataIn2 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[u8Idx][loop_idx*4+2];
                u8SRAMDataIn3 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataA[u8Idx][loop_idx*4+3];
            ...
            /*else
            {
               MS_U8 mask = (SC_R2BYTE(REG_SC_BK23_5D_L) & 0xf00) >> 8;
               SC_W2BYTE(REG_SC_BK62_74_L, ((((loop_idx&mask)<<12)) | (loop_idx<<8) | (((loop_idx&mask)<<4)) | loop_idx));//SRAM Data
               SC_W2BYTE(REG_SC_BK62_75_L, ((((loop_idx&mask)<<12)) | (loop_idx<<8) | (((loop_idx&mask)<<4)) | loop_idx));//SRAM Data
            }*/


            // set HW addr
            //if((SC_R2BYTE(REG_SC_BK23_5C_L)&0x200)==0x000)
            {
                SC_W2BYTEMSK(0,REG_SC_BK62_68_L,0x8000,0x8000);//enable rgb force

                // force R
                SC_W2BYTEMSK(0,REG_SC_BK62_68_L,0x4800,0x7800);//enable rgb force
                SC_W2BYTEMSK(0,REG_SC_BK62_72_L,(u8Idx & 0x01)<<5,0x00e0);

                SC_W2BYTEMSK(0,REG_SC_BK62_68_L,0x4000,0x7800);


                // force G
                SC_W2BYTEMSK(0,REG_SC_BK62_68_L,0x5000,0x7800);//enable rgb force
                SC_W2BYTEMSK(0,REG_SC_BK62_72_L,((loop_idx<<2)|(u8Idx & 0x02))<<4 ,0x00e0);

                SC_W2BYTEMSK(0,REG_SC_BK62_68_L,0x4000,0x7800);

                // force B
                SC_W2BYTEMSK(0,REG_SC_BK62_68_L,0x6000,0x7800);//enable rgb force
                SC_W2BYTEMSK(0,REG_SC_BK62_72_L,(((loop_idx>>2)<<3)|(u8Idx & 0x04))<<3,0x00e0);
                SC_W2BYTEMSK(0,REG_SC_BK62_68_L,0x4000,0x7800);
            }

            //fill en
            SC_W2BYTEMSK(0,REG_SC_BK62_73_L, (0x1<<u8Idx), 0xff);
            ...
            //if((SC_R2BYTE(REG_SC_BK23_5C_L)&0x200)==0x000)
            {
                SC_W2BYTEMSK(0,REG_SC_BK62_68_L,0x0000,0x7800);
            }
        }

        //if((SC_R2BYTE(REG_SC_BK23_5C_L)&0x200)==0x000)
        SC_W2BYTEMSK(0,REG_SC_BK62_68_L,0x0000,0x8000); //release rgb force
    }
    else //if(pXCResourcePrivate->stdrvXC_3D.job_id <= 7)
    {
        ...
        MS_U8 u8Idx;
#if 1
        u8Idx = pXCResourcePrivate->stdrvXC_3D.job_id & 0x07;
#else
        if((SC_R2BYTE(REG_SC_BK23_5D_L) & 0x40) == 0x40)
        {
            u8Idx = min(pXCResourcePrivate->stdrvXC_3D.job_id,((SC_R2BYTE(REG_SC_BK23_5D_L) & 0xe000)>>13));
        ...
        for(loop_idx = 0; loop_idx <= 15; loop_idx++ )
        {
            ...
            //if((SC_R2BYTE(REG_SC_BK23_5D_L) & 0x20) == 0x00)
            {
                ...
                u8SRAMDataIn0 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[u8Idx][loop_idx*4+0];
                u8SRAMDataIn1 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[u8Idx][loop_idx*4+1];
                u8SRAMDataIn2 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[u8Idx][loop_idx*4+2];
                u8SRAMDataIn3 = pXCResourcePrivate->stdrvXC_3D.u8SRAMDataB[u8Idx][loop_idx*4+3];
            ...
            //fill en
            SC_W2BYTEMSK(0,REG_SC_BK62_73_L, (0x1<<u8Idx)<<8, 0xff00);
...
static void _MDrv_SC_3D_2DTo3D_UpdateRGB3DLUT_U9(void *pInstance)
{
    ...
    MDrv_XC_wait_output_vsync(pInstance, 1, 40, MAIN_WINDOW);
...
static void MainObjectDetection_U9(void)//only for bule sky pattern (Golf pattern)
{
    ...
    int AvgDiff_SkyColor=0;
    int Avg_NonSkyColorIdx=0;

    MS_U8 MaxPosDiff_TpBGColor=0;
    MS_U8 MaxNegDiff_TpBGColor=0;
    int AvgDiff_TpBGColor=0;
    int Avg_NonTpBGColorIdx=0;
    ...
    if(SC_R2BYTEMSK(0, REG_SC_BK23_59_L,0x8)==8)
    {
        ...
        for ( x = 2; x < NBLK_DD_H-2; x ++ )
            pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x] = MED(pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x-2],pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x],pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x+2]);

        for ( x = 2; x < NBLK_DD_H-2; x ++ )
            pXCResourcePrivate->stdrvXC_3D._u8NonTpBGColorIdx[x] = MED(pXCResourcePrivate->stdrvXC_3D._u8NonTpBGColorIdx[x-2],pXCResourcePrivate->stdrvXC_3D._u8NonTpBGColorIdx[x],pXCResourcePrivate->stdrvXC_3D._u8NonTpBGColorIdx[x+2]);
    }
    else
    {
    ...
    //for ( x = NBLK_DD_H/3; x < NBLK_DD_H*2/3-7; x ++ )
    for ( x = NBLK_DD_H/4-2; x < NBLK_DD_H*2/3-7; x ++ )//aceGN
    {
        BLK_cnt++;

        if(SC_R2BYTEMSK(0, REG_SC_BK23_59_L,0x8)==8)
            Diff = ((pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x+4] -pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x+10])+(pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x+2] -pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x+8])+(pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x] -pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x+6]))/3;
        else
            Diff = ((pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x+2] -pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x+5])+(pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x+1] -pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x+4])+(pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x] -pXCResourcePrivate->stdrvXC_3D._u8NonSkyColorIdx[x+3]))/3;
    ...
    {
        printf("[MaxPosSky, MaxNegSky] = [%d %d]\r\n",MaxPosDiff_SkyColor,MaxNegDiff_SkyColor);
    ...
    for ( x = NBLK_DD_H/3; x < NBLK_DD_H*2/3; x ++ )
    {
        BLK_cnt++;
        if(SC_R2BYTEMSK(0, REG_SC_BK23_59_L,0x8)==8)
            Diff = pXCResourcePrivate->stdrvXC_3D._u8NonTpBGColorIdx[x] -pXCResourcePrivate->stdrvXC_3D._u8NonTpBGColorIdx[x+6];
        else
    ...
    if(pXCResourcePrivate->stdrvXC_3D.u8DBG_En==0x0E)
    {
        printf("[MaxPosBG, MaxNegBG] = [%d %d]\r\n",MaxPosDiff_TpBGColor,MaxNegDiff_TpBGColor);
    }
    ...
    _u8PreMainObjWei = pXCResourcePrivate->stdrvXC_3D._u8MainObjWei;
    ...
    if(pXCResourcePrivate->stdrvXC_3D.u8DBG_En==0x0E)
    {
        printf("[@@bSky, MainObjWeiSky, MainObjWeiBGC] = [%d %d %d]\r\n",(isBlue||isWCloud),MainObjWeiSky,MainObjWeiBGC);

    }

    //Keep MainObjDetectionWei stable after scaling
    if(ABS(_u8PreMainObjWei - pXCResourcePrivate->stdrvXC_3D._u8MainObjWei)<=4)
        pXCResourcePrivate->stdrvXC_3D._u8MainObjWei = _u8PreMainObjWei;
    if(SC_R2BYTEMSK(0, REG_SC_BK23_59_L,0x1)==1)
...
static void _MDrv_SC_3D_2DTo3D_TrendDetection(void *pInstance)
{
    ...
    MS_U8 VerTrend_1D[NBLK_V_MAX];
    MS_U8 HorTrend_1D[NBLK_H_MAX];
    ...
    for ( y = 0; y < NBLK_DD_V ; y ++ )
    {
        ...
        for ( x = 0; x < NBLK_DD_H; x ++ )
        {
            if((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x0004)==0)
                HorCplxSum = HorCplxSum + pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y][x];
            else
                HorCplxSum = HorCplxSum + pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y][x]*2/3;
        }
    ...
    for ( x = 0; x < NBLK_DD_H ; x ++ )
    {
        ...
        for ( y = 0; y < NBLK_DD_V; y ++ )
        {
            if((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x0004)==0)
                ...
                VerCplxSum = VerCplxSum + pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y][x]*2/3;
...
static void _MDrv_SC_3D_2DTo3D_DepthDetectionTmp(void *pInstance)
{
    ...
    for ( y = 0; y < NBLK_DR_V ; y ++ )
        for ( x = 0; x < NBLK_DR_H; x ++ )
        {
            RVal = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y/OUTIN_Ratio_Ver][x/OUTIN_Ratio_Hor].R;
            GVal = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y/OUTIN_Ratio_Ver][x/OUTIN_Ratio_Hor].G;
            BVal = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y/OUTIN_Ratio_Ver][x/OUTIN_Ratio_Hor].B;
            EDGEVal = pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y/OUTIN_Ratio_Ver][x/OUTIN_Ratio_Hor];
            YVal = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_YCCAvg[y/OUTIN_Ratio_Ver][x/OUTIN_Ratio_Hor].G;
            YMinMaxDiff = pXCResourcePrivate->stdrvXC_3D._field[(y/OUTIN_Ratio_Ver)*NBLK_DD_H+(x/OUTIN_Ratio_Hor)];
            ...
            {
                BLKWeight = (pXCResourcePrivate->stdrvXC_3D._bSkyMarker[y/OUTIN_Ratio_Ver][x/OUTIN_Ratio_Hor])?200:0;
                BLKDepth = (pXCResourcePrivate->stdrvXC_3D._bSkyMarker[y/OUTIN_Ratio_Ver][x/OUTIN_Ratio_Hor])?200:0;
            }
            else if(pXCResourcePrivate->stdrvXC_3D.u8DBG_En==0x09)
            {
                BLKWeight = (pXCResourcePrivate->stdrvXC_3D._bTpBGColorMarker[y/OUTIN_Ratio_Ver][x/OUTIN_Ratio_Hor])?200:0;
                BLKDepth = (pXCResourcePrivate->stdrvXC_3D._bTpBGColorMarker[y/OUTIN_Ratio_Ver][x/OUTIN_Ratio_Hor])?200:0;
            }
            ...
            else if(pXCResourcePrivate->stdrvXC_3D.u8DBG_En==0x0C)
            {
                ...
                BLKWeight =MINMAX(YMinMaxDiff,0,255);
                BLKDepth = MINMAX(YMinMaxDiff,0,255);
            }
            else// if(DBG_En==0x05)
            {
                BLKWeight = 0;
    ...
    if(pXCResourcePrivate->stdrvXC_3D.u8DBG_En==0x0D)
    {
        printf("\n[DDH:%d, DDV:%d]\n",NBLK_DD_H,NBLK_DD_V);
        printf("[DispH:%d, DispV:%d]\n",gSrcInfo[MAIN_WINDOW].stDispWin.height,gSrcInfo[MAIN_WINDOW].stDispWin.width);
    }
    if(pXCResourcePrivate->stdrvXC_3D.u8DBG_En==0x0F)
    {
        int dummy_x = (SC_R2BYTE(0, REG_SC_BK23_5A_L)&0x00FF);
        int dummy_y = (SC_R2BYTE(0, REG_SC_BK23_5A_L)&0xFF00)>>8;
...
#if (HW_2DTO3D_VER == 3)
static MS_U8 u8RegionBlkWeightFrame_Tmp[NBLK_V_MAX][NBLK_H_MAX];
...
static void _MDrv_SC_3D_2DTo3D_DepthDetectionPIX_U9(void *pInstance)
{
    ...
    MS_U8 _RegionBlkWeightFrame_tmp[NBLK_V_MAX][NBLK_H_MAX];
...
#if DUMYGAINCTRL
    MS_U8 u8tempval1EFC = (SC_R2BYTE(0, REG_SC_BK23_59_L)&0x00F0)>>4;
#else
    ...
    if(pXCResourcePrivate->stdrvXC_3D.u8DBG_En==0x0e)
    {
        printf("\nskycnt1:%d, Dn:%d, scalingR:%ld",blueAndFlatBlkCnt,blueAndFlatBlkCnt_Dn,u32ScalingRatio);
...
//DBG
    MS_U8 force_TBAB = SC_R2BYTEMSK(0, REG_SC_BK23_59_L,0x6)>>1;//0;
...
// keeps sky ratio stable
    {
        if(abs(blueSkyRatio1-pXCResourcePrivate->stdrvXC_3D._u8Pre_BlueSkyRatio1)<=4 &&abs(blueSkyRatio2-pXCResourcePrivate->stdrvXC_3D._u8Pre_BlueSkyRatio2)<=4)
        {
...
// LPF of _RegionBlkWeightFrame
    for ( y = 0; y < NBLK_DR_V; y ++ )
    {
        for ( x = 0; x < NBLK_DR_H; x ++ )
        {
            ...
            sumEdge = _RegionBlkWeightFrame_tmp[y>>id_v][x>>id_h];
            for ( i = -2; i <= 2; i ++ )
                for ( j = -2; j <= 2; j ++ )
                {
                    sumEdge = sumEdge+  _RegionBlkWeightFrame_tmp[MINMAX(y+j,0,NBLK_DR_V-1)>>id_v][MINMAX(x+i,0,NBLK_DR_H-1)>>id_h];
                }

            if((SC_R2BYTE(0,REG_SC_BK23_58_L)&0xC000)==0x0)
                sumEdge = MINMAX( sumEdge/25, 0, 255 );
            else if((SC_R2BYTE(0,REG_SC_BK23_58_L)&0xC000)==0x4000)
                sumEdge = MINMAX( sumEdge/20, 0, 255 );
            else if((SC_R2BYTE(0,REG_SC_BK23_58_L)&0xC000)==0x8000)
                sumEdge = MINMAX( sumEdge/16, 0, 255 );
            else
                sumEdge = MINMAX( sumEdge/9, 0, 255 );
...
#if (HW_2DTO3D_VER == 3)
static MS_U8 u8DepthBlkBased_Tmp[NBLK_V_MAX][NBLK_H_MAX];
static MS_U8 u8DepthBlkBased_Tmp2[NBLK_V_MAX][NBLK_H_MAX];
...
static void _MDrv_SC_3D_2DTo3D_DepthDetectionBLK_TEST(void *pInstance)
{
    ...
    MS_U8 _depthBlkBased_tmp[NBLK_V_MAX][NBLK_H_MAX];
    MS_U8 _depthBlkBased_tmp2[NBLK_V_MAX][NBLK_H_MAX];
    MS_U8 id_v=0,id_h=0;

    MS_U32 u32VRatio = 2160*10/DISPLAY_DD_V;
    MS_U32 u32HRatio = 3840*10/DISPLAY_DD_H;
    ...
    MS_U8 u8testval_0EBA = SC_R2BYTEMSK(0, REG_SC_BK23_5E_L, 0xff);//MFC_ReadByte(REG_0EBA);//default:0x00 for LGE 0x18
    MS_U8 u8testval_0EBB = SC_R2BYTEMSK(0, REG_SC_BK23_5F_L, 0xff00)>>8;//MFC_ReadByte(REG_0EBB);//default:0x20 for LGE 0x20
#else
    MS_U8 u8testval_1EFC = 0x00;//MFC_ReadByte(REG_1EFC);
    ...
    else
    {
        for ( y = 0; y < NBLK_DD_V; y ++ )
        {
            ...
            for ( x = 0; x < NBLK_DD_H; x ++ )
            {
                ...
                _depthBlkBased_tmp[y][x] = MINMAX(depth,0,255);
                //_depthBlkBased_tmp[y][x] = y*255/NBLK_DD_V;
...
static void _MDrv_SC_3D_SourceDetection(void *pInstance)
{
#if 1 //2014.07.14 ace new algorithm
    ...
    MS_U16 cplx1, cplx3, cplx2, cplx4 ;
    //MS_U16 cplxcount =0;

    UNUSED(m);

    MS_U16 SScount, TBcount, notSScount, notTBcount;

    memset(SSYdiff21, 0, 7*sizeof(MS_U16));
    memset(SSYdiff22, 0, 7*sizeof(MS_U16));
    memset(SSYdiff23, 0, 7*sizeof(MS_U16));
    memset(SSYdiff24, 0, 7*sizeof(MS_U16));

    memset(SSYdiff1_h, 0, 7*sizeof(MS_U16));
    memset(SSYdiff2_h, 0, 7*sizeof(MS_U16));
    memset(SSYdiff3_h, 0, 7*sizeof(MS_U16));
    memset(SSYdiff4_h, 0, 7*sizeof(MS_U16));

    memset(SScplxdiff1, 0, 7*sizeof(MS_U16));
    memset(SScplxdiff2, 0, 7*sizeof(MS_U16));
    memset(SScplxdiff3, 0, 7*sizeof(MS_U16));
    memset(SScplxdiff4, 0, 7*sizeof(MS_U16));

    memset(TBYdiff21, 0, 15*sizeof(MS_U16));
    memset(TBYdiff22, 0, 15*sizeof(MS_U16));
    memset(TBYdiff23, 0, 15*sizeof(MS_U16));
    memset(TBYdiff24, 0, 15*sizeof(MS_U16));

    memset(TBYdiff1_v, 0, 15*sizeof(MS_U16));
    memset(TBYdiff2_v, 0, 15*sizeof(MS_U16));
    memset(TBYdiff3_v, 0, 15*sizeof(MS_U16));
    memset(TBYdiff4_v, 0, 15*sizeof(MS_U16));
    
    memset(TBcplxdiff1, 0, 15*sizeof(MS_U16));
    memset(TBcplxdiff2, 0, 15*sizeof(MS_U16));
    memset(TBcplxdiff3, 0, 15*sizeof(MS_U16));
    memset(TBcplxdiff4, 0, 15*sizeof(MS_U16));

    MS_U16 minSSYdiff21, minSSYdiff22, minSSYdiff23, minSSYdiff24;
    MS_U16 minSSYdiff1_h, minSSYdiff2_h, minSSYdiff3_h, minSSYdiff4_h;
    MS_U16 minSScplxdiff1, minSScplxdiff2, minSScplxdiff3, minSScplxdiff4;

    MS_U16 minTBYdiff21, minTBYdiff22, minTBYdiff23, minTBYdiff24;
    MS_U16 minTBYdiff1_v, minTBYdiff2_v, minTBYdiff3_v, minTBYdiff4_v;
    MS_U16 minTBcplxdiff1, minTBcplxdiff2, minTBcplxdiff3, minTBcplxdiff4;

    MS_U16 minSSYdiff2_m, minSSYdiff_h_m, minSScplxdiff_m ;
    MS_U16 minTBYdiff2_m, minTBYdiff_v_m, minTBcplxdiff_m ;

    MS_U16 minSScplxin3, minSSYdiff2in3;
    MS_U16 minTBcplxin3,  minTBYdiff2in3 ;
    ...
    for(y = 0; y < NBLK_DD_V/2-1; y ++)
    {
        for(x = 8; x < NBLK_DD_H/2-8; x ++)
        {

            minTBcplxin3 = 0xffff;;
            minTBYdiff2in3 = 0xffff;

            for ( i = -1; i < 2 ; i++)
            {

                cplx1 =  pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y][x+i];
                cplx2 =  pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y][x+i+(NBLK_DD_H/2)];
                cplx3 =  pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y+(NBLK_DD_V/2)][x+i];
                cplx4 =  pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y+(NBLK_DD_V/2)][x+i+(NBLK_DD_H/2)];

                Pixel1 = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+i];
                Pixel2 = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+i+(NBLK_DD_H/2)];
                Pixel3 = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+i];
                Pixel4 = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+i+(NBLK_DD_H/2)];

                //horizontal postion pixels

                for ( k = 0; k< 15 ; k ++)
                {
                    Pixel10[k].R = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+k+i-7].R;
                    Pixel10[k].G = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+k+i-7].G;
                    Pixel10[k].B = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+k+i-7].B;

                    Pixel20[k].R = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+k+i+(NBLK_DD_H/2)-7].R;
                    Pixel20[k].G = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+k+i+(NBLK_DD_H/2)-7].G;
                    Pixel20[k].B = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+k+i+(NBLK_DD_H/2)-7].B;

                    Pixel30[k].R = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+k+i-7].R;
                    Pixel30[k].G = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+k+i-7].G;
                    Pixel30[k].B = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+k+i-7].B;

                    Pixel40[k].R = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+k+i+(NBLK_DD_H/2)-7].R;
                    Pixel40[k].G = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+k+i+(NBLK_DD_H/2)-7].G;
                    Pixel40[k].B = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+k+i+(NBLK_DD_H/2)-7].B;

                    // vertical shift pixel
                    Pixel10_v[k].R = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/4)][(x+i)+k-7].R;
                    Pixel10_v[k].G = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/4)][(x+i)+k-7].G;
                    Pixel10_v[k].B = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/4)][(x+i)+k-7].B;

                    Pixel30_v[k].R = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/4)][(NBLK_DD_H/2)+(x+i)+k-7].R;
                    Pixel30_v[k].G = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/4)][(NBLK_DD_H/2)+(x+i)+k-7].G;
                    Pixel30_v[k].B = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/4)][(NBLK_DD_H/2)+(x+i)+k-7].B;


                    //average edge
                    cplx10[k] = pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y][x+i+k-7];
                    cplx20[k] = pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y][(NBLK_DD_H/2)+x+i+k-7];
                    cplx30[k] = pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y+(NBLK_DD_V/2)][x+i+k-7];
                    cplx40[k] = pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y+(NBLK_DD_V/2)][(NBLK_DD_H/2)+x+i+k-7];
                ...
                minTBYdiff1_v = 0xffff;
                minTBYdiff2_v = 0xffff;
                minTBYdiff3_v = 0xffff;
                minTBYdiff4_v = 0xffff;

                minTBcplxdiff1 = 0xffff;
                minTBcplxdiff2 = 0xffff;
                minTBcplxdiff3 = 0xffff;
                minTBcplxdiff4 = 0xffff;

                for ( k = 0; k< 15 ; k++ )
                {

                    //TB distance square
                    TBYdiff21[k] = ( Pixel1.R - Pixel30[k].R)*( Pixel1.R - Pixel30[k].R) + ( Pixel1.G - Pixel30[k].G)*( Pixel1.G- Pixel30[k].G )+ ( Pixel1.B - Pixel30[k].B )*( Pixel1.B - Pixel30[k].B );
                    TBYdiff22[k] = ( Pixel2.R - Pixel40[k].R)*( Pixel2.R - Pixel40[k].R) + ( Pixel2.G - Pixel40[k].G)*( Pixel2.G - Pixel40[k].G) + ( Pixel2.B - Pixel40[k].B)*( Pixel2.B - Pixel40[k].B );
                    TBYdiff23[k] = ( Pixel3.R - Pixel10[k].R)*( Pixel3.R - Pixel10[k].R) + ( Pixel3.G - Pixel10[k].G)*( Pixel3.G - Pixel10[k].G) + ( Pixel3.B - Pixel10[k].B)*( Pixel3.B - Pixel10[k].B );
                    TBYdiff24[k] = ( Pixel4.R - Pixel20[k].R)*( Pixel4.R - Pixel20[k].R) + ( Pixel4.G - Pixel20[k].G)*( Pixel4.G - Pixel20[k].G) + ( Pixel4.B - Pixel20[k].B)*( Pixel4.B - Pixel20[k].B );

                    minTBYdiff21 = MIN( minTBYdiff21, TBYdiff21[k]);
                    minTBYdiff22 = MIN( minTBYdiff22, TBYdiff22[k]);
                    minTBYdiff23 = MIN( minTBYdiff23, TBYdiff23[k]);
                    minTBYdiff24= MIN( minTBYdiff24, TBYdiff24[k]);

                    // TB shift diff
                    TBYdiff1_v[k] = ( Pixel1.R - Pixel10_v[k].R)*( Pixel1.R - Pixel10_v[k].R) + ( Pixel1.G - Pixel10_v[k].G)*( Pixel1.G- Pixel10_v[k].G )+ ( Pixel1.B - Pixel10_v[k].B )*( Pixel1.B - Pixel10_v[k].B );
                    TBYdiff2_v[k] = ( Pixel2.R - Pixel30_v[k].R)*( Pixel2.R - Pixel30_v[k].R) + ( Pixel2.G - Pixel30_v[k].G)*( Pixel2.G- Pixel30_v[k].G )+ ( Pixel2.B - Pixel30_v[k].B )*( Pixel2.B - Pixel30_v[k].B );
                    TBYdiff3_v[k] = ( Pixel3.R - Pixel10_v[k].R)*( Pixel3.R - Pixel10_v[k].R) + ( Pixel3.G - Pixel10_v[k].G)*( Pixel3.G- Pixel10_v[k].G )+ ( Pixel3.B - Pixel10_v[k].B )*( Pixel3.B - Pixel10_v[k].B );
                    TBYdiff4_v[k] = ( Pixel4.R - Pixel30_v[k].R)*( Pixel4.R - Pixel30_v[k].R) + ( Pixel4.G - Pixel30_v[k].G)*( Pixel4.G- Pixel30_v[k].G )+ ( Pixel4.B - Pixel30_v[k].B )*( Pixel4.B - Pixel30_v[k].B );

                    minTBYdiff1_v = MIN( minTBYdiff1_v,  TBYdiff1_v[k] );
                    minTBYdiff2_v = MIN( minTBYdiff2_v,  TBYdiff2_v[k] );
                    minTBYdiff3_v = MIN( minTBYdiff3_v,  TBYdiff3_v[k] );
                    minTBYdiff4_v = MIN( minTBYdiff4_v,  TBYdiff4_v[k] );

                    // TB complex diff
                    TBcplxdiff1[k] =ABS( cplx1 - cplx30[k]);
                    TBcplxdiff2[k] =ABS( cplx2 - cplx40[k]);
                    TBcplxdiff3[k] =ABS( cplx3 - cplx10[k]);
                    TBcplxdiff4[k] =ABS( cplx4 - cplx20[k]);

                    minTBcplxdiff1 = MIN( minTBcplxdiff1,  TBcplxdiff1[k] );
                    minTBcplxdiff2 = MIN( minTBcplxdiff2,  TBcplxdiff2[k] );
                    minTBcplxdiff3 = MIN( minTBcplxdiff3,  TBcplxdiff3[k] );
                    minTBcplxdiff4 = MIN( minTBcplxdiff4,  TBcplxdiff4[k] );
                ...
                // max of min distance
                minTBYdiff2_m = MAX (MAX( minTBYdiff21,  minTBYdiff22), MAX( minTBYdiff23, minTBYdiff24 ))/10;

                // max of min distance in shifting
                minTBYdiff_v_m = MAX (MAX( minTBYdiff1_v,  minTBYdiff2_v), MAX( minTBYdiff3_v, minTBYdiff4_v ))/10;

                // max of min complex diff
                minTBcplxdiff_m = MAX (MAX( minTBcplxdiff1,  minTBcplxdiff2), MAX( minTBcplxdiff3, minTBcplxdiff4 ));

                // min complex diff in 3
                minTBcplxin3 = MIN(minTBcplxin3, minTBcplxdiff_m);

                // min distance in 3
                minTBYdiff2in3 = MIN(minTBYdiff2in3, minTBYdiff2_m);

            }

            if ( (minTBYdiff2_m < 500) && (minTBYdiff2in3 < 300) && (minTBcplxin3 < 25) && ( minTBYdiff2_m < minTBYdiff_v_m/2) && ( minTBYdiff_v_m > 20) )
                TBcount ++;

            if ( (minTBYdiff2_m > 500) || (minTBYdiff2in3 > 300) || (minTBcplxin3 > 25)  )
                notTBcount ++;

            //printf("\n  minSSYdiff_h_m %d,  minSSYdiff2_m %d,  minSSYdiff2in3 %d, minSScplxin3 %d ", minSSYdiff_h_m, minSSYdiff2_m, minSSYdiff2in3, minSScplxin3 );
            //printf("    minTBYdiff_v_m %d,  minTBYdiff2_m %d,  minTBYdiff2in3 %d, minTBcplxin3 %d ", minTBYdiff_v_m, minTBYdiff2_m, minTBYdiff2in3, minTBcplxin3 );
    ...
    for(y = 0; y < NBLK_DD_V/2-1; y ++)
    {
        for(x = 4; x < NBLK_DD_H/2-4; x ++)
        {
            minSScplxin3 = 0xffff;
            minTBcplxin3 = 0xffff;

            minSSYdiff2in3 = 0xffff;
            minTBYdiff2in3 = 0xffff;
            ...
            for ( i = -1; i < 2 ; i++)
            {
                ...
                for ( k = 0; k< 7 ; k ++)
                {
                    Pixel10[k].R = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+k+i-3].R;
                    Pixel10[k].G = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+k+i-3].G;
                    Pixel10[k].B = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+k+i-3].B;

                    Pixel20[k].R = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+k+i+(NBLK_DD_H/2)-3].R;
                    Pixel20[k].G = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+k+i+(NBLK_DD_H/2)-3].G;
                    Pixel20[k].B = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][x+k+i+(NBLK_DD_H/2)-3].B;

                    Pixel30[k].R = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+k+i-3].R;
                    Pixel30[k].G = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+k+i-3].G;
                    Pixel30[k].B = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+k+i-3].B;

                    Pixel40[k].R = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+k+i+(NBLK_DD_H/2)-3].R;
                    Pixel40[k].G = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+k+i+(NBLK_DD_H/2)-3].G;
                    Pixel40[k].B = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][x+k+i+(NBLK_DD_H/2)-3].B;

                    // horizontal shift pixel

                    Pixel10_h[k].R = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][(NBLK_DD_H/4)+(x+i)+k-3].R;
                    Pixel10_h[k].G = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][(NBLK_DD_H/4)+(x+i)+k-3].G;
                    Pixel10_h[k].B = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y][(NBLK_DD_H/4)+(x+i)+k-3].B;

                    Pixel30_h[k].R = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][(NBLK_DD_H/4)+(x+i)+k-3].R;
                    Pixel30_h[k].G = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][(NBLK_DD_H/4)+(x+i)+k-3].G;
                    Pixel30_h[k].B = pXCResourcePrivate->stdrvXC_3D._stBlockStatus_Avg[y+(NBLK_DD_V/2)][(NBLK_DD_H/4)+(x+i)+k-3].B;


                    //average edge
                    cplx10[k] = pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y][x+i+k-3];
                    cplx20[k] = pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y][(NBLK_DD_H/2)+x+i+k-3];
                    cplx30[k] = pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y+(NBLK_DD_V/2)][x+i+k-3];
                    cplx40[k] = pXCResourcePrivate->stdrvXC_3D._u16BlockStatus_Edge[y+(NBLK_DD_V/2)][(NBLK_DD_H/2)+x+i+k-3];
                ...
                minSSYdiff1_h = 0xffff;
                minSSYdiff2_h = 0xffff;
                minSSYdiff3_h = 0xffff;
                minSSYdiff4_h = 0xffff;

                minSScplxdiff1 = 0xffff;
                minSScplxdiff2 = 0xffff;
                minSScplxdiff3 = 0xffff;
                minSScplxdiff4 = 0xffff;
                ...
                for ( k = 0; k< 7 ; k++ )
                {
                    ...
                    SSYdiff21[k] = ( Pixel1.R - Pixel20[k].R)*( Pixel1.R - Pixel20[k].R) + ( Pixel1.G - Pixel20[k].G)*( Pixel1.G- Pixel20[k].G )+ ( Pixel1.B - Pixel20[k].B )*( Pixel1.B - Pixel20[k].B );
                    SSYdiff22[k] = ( Pixel2.R - Pixel10[k].R)*( Pixel2.R - Pixel10[k].R) + ( Pixel2.G - Pixel10[k].G)*( Pixel2.G - Pixel10[k].G) + ( Pixel2.B - Pixel10[k].B)*( Pixel2.B - Pixel10[k].B );
                    SSYdiff23[k] = ( Pixel3.R - Pixel40[k].R)*( Pixel3.R - Pixel40[k].R) + ( Pixel3.G - Pixel40[k].G)*( Pixel3.G - Pixel40[k].G) + ( Pixel3.B - Pixel40[k].B)*( Pixel3.B - Pixel40[k].B );
                    SSYdiff24[k] = ( Pixel4.R - Pixel30[k].R)*( Pixel4.R - Pixel30[k].R) + ( Pixel4.G - Pixel30[k].G)*( Pixel4.G - Pixel30[k].G) + ( Pixel4.B - Pixel30[k].B)*( Pixel4.B - Pixel30[k].B );

                    minSSYdiff21 = MIN( minSSYdiff21, SSYdiff21[k]);
                    minSSYdiff22 = MIN( minSSYdiff22, SSYdiff22[k]);
                    minSSYdiff23 = MIN( minSSYdiff23, SSYdiff23[k]);
                    minSSYdiff24= MIN( minSSYdiff24, SSYdiff24[k]);

                    // SS shift diff

                    SSYdiff1_h[k] = ( Pixel1.R - Pixel10_h[k].R)*( Pixel1.R - Pixel10_h[k].R) + ( Pixel1.G - Pixel10_h[k].G)*( Pixel1.G- Pixel10_h[k].G )+ ( Pixel1.B - Pixel10_h[k].B )*( Pixel1.B - Pixel10_h[k].B );
                    SSYdiff2_h[k] = ( Pixel2.R - Pixel10_h[k].R)*( Pixel2.R - Pixel10_h[k].R) + ( Pixel2.G - Pixel10_h[k].G)*( Pixel2.G- Pixel10_h[k].G )+ ( Pixel2.B - Pixel10_h[k].B )*( Pixel2.B - Pixel10_h[k].B );
                    SSYdiff3_h[k] = ( Pixel3.R - Pixel30_h[k].R)*( Pixel3.R - Pixel30_h[k].R) + ( Pixel3.G - Pixel30_h[k].G)*( Pixel3.G- Pixel30_h[k].G )+ ( Pixel3.B - Pixel30_h[k].B )*( Pixel3.B - Pixel30_h[k].B );
                    SSYdiff4_h[k] = ( Pixel4.R - Pixel30_h[k].R)*( Pixel4.R - Pixel30_h[k].R) + ( Pixel4.G - Pixel30_h[k].G)*( Pixel4.G- Pixel30_h[k].G )+ ( Pixel4.B - Pixel30_h[k].B )*( Pixel4.B - Pixel30_h[k].B );

                    minSSYdiff1_h = MIN( minSSYdiff1_h,  SSYdiff1_h[k] );
                    minSSYdiff2_h = MIN( minSSYdiff2_h,  SSYdiff2_h[k] );
                    minSSYdiff3_h = MIN( minSSYdiff3_h,  SSYdiff3_h[k] );
                    minSSYdiff4_h = MIN( minSSYdiff4_h,  SSYdiff4_h[k] );

                    // SS complex diff
                    SScplxdiff1[k] =ABS( cplx1 - cplx20[k]);
                    SScplxdiff2[k] =ABS( cplx2 - cplx10[k]);
                    SScplxdiff3[k] =ABS( cplx3 - cplx40[k]);
                    SScplxdiff4[k] =ABS( cplx4 - cplx30[k]);

                    minSScplxdiff1 = MIN( minSScplxdiff1,  SScplxdiff1[k] );
                    minSScplxdiff2 = MIN( minSScplxdiff2,  SScplxdiff2[k] );
                    minSScplxdiff3 = MIN( minSScplxdiff3,  SScplxdiff3[k] );
                    minSScplxdiff4 = MIN( minSScplxdiff4,  SScplxdiff4[k] );

                }


                // max of min distance
                minSSYdiff2_m = MAX (MAX( minSSYdiff21,  minSSYdiff23), MAX( minSSYdiff22, minSSYdiff24 ))/10;

                // max of min distance in shifting
                minSSYdiff_h_m = MAX (MAX( minSSYdiff1_h,  minSSYdiff2_h), MAX( minSSYdiff3_h, minSSYdiff4_h ))/10;

                // max of min complex diff
                minSScplxdiff_m = MAX (MAX( minSScplxdiff1,  minSScplxdiff2), MAX( minSScplxdiff3, minSScplxdiff4 ));

                // min complex diff in 3
                minSScplxin3 = MIN(minSScplxin3, minSScplxdiff_m);

                // min distance in 3
                minSSYdiff2in3 = MIN(minSSYdiff2in3, minSSYdiff2_m);

            }



            if ( (minSSYdiff2_m < 500) && (minSSYdiff2in3 < 300) && (minSScplxin3 < 25) && ( minSSYdiff2_m < minSSYdiff_h_m/2 ) && ( minSSYdiff_h_m > 50) )
                SScount ++;

            if ( (minSSYdiff2_m > 500) || (minSSYdiff2in3 > 300) || (minSScplxin3 > 25)  )
                notSScount ++;
...
#if 0
    printf("\n  Pixel1.R = %d  Pixel2.R = %d  Pixel3.R = %d  Pixel4.R = %d\n",Pixel1.R, m, Pixel2.R, m, Pixel3.R, m, Pixel4.R );
    printf("\n  Pixel1.G = %d  Pixel2.G = %d  Pixel3.G = %d  Pixel4.G = %d\n",Pixel1.G, m, Pixel2.G, m, Pixel3.G, m, Pixel4.G );
    printf("\n  Pixel1.B = %d  Pixel2.B = %d  Pixel3.B = %d  Pixel4.B = %d\n",Pixel1.B, m, Pixel2.B, m, Pixel3.B, m, Pixel4.B );


    for ( m= 0; m< 15 ; m ++)
    {

        printf("\n  Pixel10[%d].R = %d  Pixel20[%d].R = %d  Pixel30[%d].R = %d  Pixel40[%d].R = %d\n", m, Pixel10[k].R, m, Pixel20[k].R, m, Pixel30[k].R, m, Pixel40[k].R );
        printf("\n  Pixel10[%d].G = %d  Pixel20[%d].G = %d  Pixel30[%d].G = %d  Pixel40[%d].G = %d\n", m, Pixel10[k].G, m, Pixel20[k].G, m, Pixel30[k].G, m, Pixel40[k].G );
        printf("\n  Pixel10[%d].B = %d  Pixel20[%d].B = %d  Pixel30[%d].B = %d  Pixel40[%d].B = %d\n", m, Pixel10[k].B, m, Pixel20[k].B, m, Pixel30[k].B, m, Pixel40[k].B );
    }



    printf("\n  SScount %d,  notSScount %d,  TBcount %d, notTBcount %d\n "
           , SScount, notSScount, TBcount, notTBcount );
#endif
    SSratio= SScount/(notSScount+1);
    TBratio= TBcount/(notTBcount+1);

    if ( MAX( SScount, TBcount) < 5 )
        enFormat = E_3D_FORMAT_UNCERTAIN;// not taken into voting
    else if ( SSratio > MAX( TBratio, 9 ) )
        enFormat = E_3D_FORMAT_SS;
    else if ( TBratio > MAX( SSratio, 5 ) )
            ...
            && IS_OUTPUT_LINE_ALTERNATIVE())
    {
        ...
        MS_U8 u8G_Y_Avg = 0;
        MS_U8 u8B_Cb_Avg = 0;
        MS_U32 u32DDBase = (MS_U32)MsOS_PA2KSEG1(pXCResourcePrivate->stdrvXC_3D._u32HW2DTO3D_DD_Buf);
        if(0 == u32DDBase)
        {
            printf("%s :u32DDBase = 0!,error!!!\n", __FUNCTION__);
            assert(u32DDBase != 0);
        ...
        MS_U16 u16DDBlock_DramSize = DDBYTENUM;
        MS_U32 u32DDBlockLinePitch = (NBLK_DD_PITCH*DDBYTENUM);
        PIXEL stblockStatusEven[NBLK_DD_V][NBLK_DD_H]; // avg color value for even lines
        PIXEL stblockStatusOdd[NBLK_DD_V][NBLK_DD_H];  // avg color value for odd lines
        MS_U16 u16MinDiff = 0;
        //MS_U16 u16MaxDiff = 0;
            ...
            if (u16VLinesymmetryCount <= (NBLK_DD_H - 12)*(NBLK_DD_V-4)/4)
            {
                if(u16HBlockShift > 0)//overscan case
                {
                    ...
                    else if((u16SSCount > u16TBCount) && (u16SSCount > u162DCount))//ss predict
                    {
                        ...
                        else if(u16SSCount>3*u162DCount)//general case
                        {
                            enFormat = E_3D_FORMAT_SS;
                        }
...
static void _MDrv_SC_3D_2DTo3D_DepthRefine(SC_INT_SRC eIntNum, void *pParam)
{
    ...
    if(IS_INPUT_NORMAL_2D_HW(MAIN_WINDOW))
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK62_06_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Offset, 0x00FF);//left depth offset
        SC_W2BYTEMSK(0, REG_SC_BK62_0E_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Offset, 0x00FF);//right depth offset
        SC_W2BYTEMSK(0, REG_SC_BK62_06_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Gain<<8, 0x7F00);//left depth gain:7bits
        SC_W2BYTEMSK(0, REG_SC_BK62_0E_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Gain<<8, 0x7F00);//right depth gain:7bits
    }
...
static void _MDrv_SC_3D_2DTo3D_DepthRefine(SC_INT_SRC eIntNum, void *pParam)
{
    ...
    #if 0
    if((SC_R2BYTE(0,REG_SC_BK23_5F_L)&0x1E)>>1==0)
    pXCResourcePrivate->stdrvXC_3D.job_freq = 0;
    else if((SC_R2BYTE(0,REG_SC_BK23_5F_L)&0x1E)>>1==0xF)
        pXCResourcePrivate->stdrvXC_3D.job_freq = 1;
    else
    pXCResourcePrivate->stdrvXC_3D.job_freq = (( pXCResourcePrivate->stdrvXC_3D.job_freq +1 ) & ((SC_R2BYTE(0,REG_SC_BK23_5F_L)&0x1E)>>1));
    #else
    pXCResourcePrivate->stdrvXC_3D.job_freq = (( pXCResourcePrivate->stdrvXC_3D.job_freq +1 ) & 0x1);
    #endif
    if(IS_INPUT_NORMAL_2D_HW(MAIN_WINDOW))
    {

        pXCResourcePrivate->stdrvXC_3D.u8DBG_En = (SC_R2BYTE(0,REG_SC_BK23_58_L)&0x0F00)>>8;


        if((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x0001)==0)
        {

            _MDrv_SC_3D_2DTo3D_UpdateRGB3DLUT_AutoDownload(pInstance);
        }

//ace test for CPU resource occupied, these part don't need to update each time
    if(pXCResourcePrivate->stdrvXC_3D.job_freq == 0)
    {
        if((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x0001)==0)
            _MDrv_SC_3D_2DTo3D_HWInfo_Read_U9(pInstance);
        ...
        if((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x2000)==0)
            _MDrv_SC_3D_2DTo3D_BGColorDetection_U9(pInstance);

        _MDrv_SC_3D_2DTo3D_DepthDetectionPIX_U9(pInstance); //New


        if((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x0010)==0)
        {
        ...
        if((SC_R2BYTE(0,REG_SC_BK23_58_L)&0x1000)==0)
        {
            _MDrv_SC_3D_2DTo3D_HW_Info_Write(pInstance);
        }
        if(IS_INPUT_NORMAL_2D_HW(MAIN_WINDOW))
        {
...
#if DUMYGAINCTRL
            MS_U8 u8tmpval_FE = (SC_R2BYTEMSK(0, REG_SC_BK23_5B_L,0xff));//0EB8
            MS_U8 u8tmpval_FF = (SC_R2BYTEMSK(0, REG_SC_BK23_5C_L,0xff00))>>8;//0EB9
            MS_BOOL LR_SWAP = (SC_R2BYTEMSK(0, REG_SC_BK62_06_L,0x8000))>>15;
            ...
            if(Genable==1)
            {
                ...
                if(LR_SWAP)
                {
                    SC_W2BYTEMSK(0, REG_SC_BK62_0E_L, Ggain<<8 ,0xff00);
                    SC_W2BYTEMSK(0, REG_SC_BK62_06_L, (Ggain+0x80)<<8,0xff00);

                }
                else
                {
                    SC_W2BYTEMSK(0, REG_SC_BK62_06_L, Ggain<<8 ,0xff00);
                    SC_W2BYTEMSK(0, REG_SC_BK62_0E_L, (Ggain+0x80)<<8,0xff00);
            ...
            if(Penable==1)
            {
                ...
                if (LR_SWAP)
                {
                    SC_W2BYTEMSK(0, REG_SC_BK62_65_L, (~Pgain)+1 ,0x01ff);
                    SC_W2BYTEMSK(0, REG_SC_BK62_66_L, Pgain, 0x01ff);
                }
                else
                {
                    SC_W2BYTEMSK(0, REG_SC_BK62_66_L, (~Pgain)+1 ,0x01ff);
                    SC_W2BYTEMSK(0, REG_SC_BK62_65_L, Pgain, 0x01ff);
                               ...
                               SCALER_WIN eWindow)
{
    ...
    else //if not supported, display as 2D
    {
        ...
        pXCResourcePrivate->stdrvXC_3D._eInput3DMode[eWindow] = E_XC_3D_INPUT_MODE_NONE;
        if (e3dOutputMode == E_XC_3D_OUTPUT_FRAME_ALTERNATIVE_NOFRC)
        {
            pXCResourcePrivate->stdrvXC_3D._eOutput3DMode         = e3dOutputMode;
        }
...
MS_BOOL MDrv_XC_Set_3D_LR_Frame_Exchg_burst(void *pInstance, SCALER_WIN eWindow)
{
            ...
            || IS_OUTPUT_TOP_BOTTOM_HW() || IS_OUTPUT_SIDE_BY_SIDE_HALF_HW())
    {
        ...
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_17_L, Hal_SC_get_opm_fetch(SUB_WINDOW), 0x0FFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_57_L, u16Temp, 0x0FFF);
        ...
        if(IS_INPUT_FRAME_ALTERNATIVE(eWindow))
        {
            ...
            u16MainRWBank = SC_R2BYTE(0, REG_SC_BK12_07_L);
            u16SubRWBank = SC_R2BYTE(0, REG_SC_BK12_47_L);
           ...
           )
    {
        ...
        u16FRCM_W = SC_R2BYTE(0, REG_SC_BK32_30_L);
        u16FRCM_R = SC_R2BYTE(0, REG_SC_BK32_33_L);
        if(u16FRCM_R & BIT(7))
        {
            ...
            _MLOAD_ENTRY();
#endif //#if SC2_ENABLE
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_33_L, u16FRCM_R, BIT(8));
            MDrv_XC_MLoad_Fire(pInstance, TRUE);
#if SC2_ENABLE
            _MLOAD_RETURN(pInstance);
#else
        ...
        else if(u16FRCM_W & BIT(0))
        {
            ...
            _MLOAD_ENTRY();
#endif //#if SC2_ENABLE
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_30_L, u16FRCM_W, BIT(4));
            MDrv_XC_MLoad_Fire(pInstance, TRUE);
#if SC2_ENABLE
            _MLOAD_RETURN(pInstance);
#else
        ...
        u16IPM = SC_R2BYTE(0, REG_SC_BK12_30_L);
        u16OPM = SC_R2BYTE(0, REG_SC_BK12_33_L);
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "3D: OPM 3D LR invert\n");
        if(u16OPM & BIT(7))
        {
            //Recommend to use OP L/R switch as designer's advice
            u16OPM = u16OPM^BIT(8);
            ...
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_33_L, u16OPM, BIT(8));
#endif
            ...
            bRet = TRUE;
            XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "3D: F2 OPM 3D LR invert\n");
        }
        else if(u16IPM & BIT(0))
        {
...
#if (HW_DESIGN_3D_VER == 5)
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_30_L, u16IPM, BIT(4));
#endif
            MDrv_XC_MLoad_Fire(pInstance, TRUE);
            ...
            bRet = TRUE;
            XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "3D: F2 IPM 3D LR invert\n");
        }
        else
        {
    ...
    {
#if (HW_2DTO3D_VER >= 3)
        MS_U16 u16LeftGain = SC_R2BYTE(0, REG_SC_BK62_06_L);
        MS_U16 u16RightGain = SC_R2BYTE(0, REG_SC_BK62_0E_L);
        ...
        bRet = TRUE;
#else
        MS_U16 u16OutputFormat = SC_R2BYTEMSK(0, REG_SC_BK15_67_L, 0x0007);
...
MS_BOOL MDrv_XC_Set_3D_LR_Frame_Exchg(void *pInstance, SCALER_WIN eWindow)
{
            ...
            || IS_OUTPUT_TOP_BOTTOM_HW() || IS_OUTPUT_SIDE_BY_SIDE_HALF_HW())
    {
        ...
        if(IS_INPUT_FRAME_ALTERNATIVE(eWindow))
        {
            ...
            u16MainRWBank = u16MainRWBank^BIT(14);
            u16SubRWBank = u16SubRWBank^BIT(14);
            SC_W2BYTEMSK(0, REG_SC_BK12_07_L, u16MainRWBank, BIT(14));
            SC_W2BYTEMSK(0, REG_SC_BK12_47_L, u16SubRWBank, BIT(14));
        }
            ...
            (!IS_OUTPUT_FRAME_ALTERNATIVE()))
    {
        ...
        SC_W2BYTE(0, REG_SC_BK20_1C_L, pXCResourcePrivate->stdrvXC_3D._u163DHShift);//Main/sub display window offset in right direction
        bRet = TRUE;
    }
    else if(!(IS_INPUT_MODE_NONE(eWindow) || IS_OUTPUT_MODE_NONE())
            && (!(IS_INPUT_NORMAL_2D_HW(eWindow) && IS_OUTPUT_LINE_ALTERNATIVE_HW()))
           ...
           )
    {
        ...
        if(u16FRCM_R & BIT(7))
        {
            //Recommend to use OP L/R switch as designer's advice
            u16FRCM_R = u16FRCM_R^BIT(8);
            SC_W2BYTEMSK(0, REG_SC_BK32_33_L, u16FRCM_R, BIT(8));//F2 OPM 3D LR invert
            bRet = TRUE;
        }
        else if(u16FRCM_W & BIT(0))
        {
            u16FRCM_W = u16FRCM_W^BIT(4);
            SC_W2BYTEMSK(0, REG_SC_BK32_30_L, u16FRCM_R, BIT(4));//F2 IPM 3D LR invert
            bRet = TRUE;
        }
        else
        {
        ...
        if(u16OPM & BIT(7))
        {
            //Recommend to use OP L/R switch as designer's advice
            u16OPM = u16OPM^BIT(8);
            SC_W2BYTEMSK(0, REG_SC_BK12_33_L, u16OPM, BIT(8));//F2 OPM 3D LR invert
#if (HW_DESIGN_3D_VER == 5)
            SC_W2BYTEMSK(0, REG_SC_BK32_33_L, u16OPM, BIT(8));//F2 OPM 3D LR invert
#endif
            bRet = TRUE;
        }
        else if(u16IPM & BIT(0))
        {
            u16IPM = u16IPM^BIT(4);
            SC_W2BYTEMSK(0, REG_SC_BK12_30_L, u16IPM, BIT(4));//F2 IPM 3D LR invert
#if (HW_DESIGN_3D_VER == 5)
            SC_W2BYTEMSK(0, REG_SC_BK32_30_L, u16IPM, BIT(4));//F2 IPM 3D LR invert
#endif
            bRet = TRUE;
        }
        else
    ...
    if(IS_INPUT_NORMAL_2D_HW(eWindow) && IS_OUTPUT_LINE_ALTERNATIVE_HW())
    {
        ...
        u16LeftGain = u16LeftGain^BIT(15);
        u16RightGain = u16RightGain^BIT(15);
        SC_W2BYTEMSK(0, REG_SC_BK62_06_L, u16LeftGain, BIT(15));
        SC_W2BYTEMSK(0, REG_SC_BK62_0E_L, u16RightGain, BIT(15));
#else
        MS_U16 u16OutputFormat = SC_R2BYTEMSK(0, REG_SC_BK15_67_L, 0x0007);
        if(0x0001 == u16OutputFormat)
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0004, 0x0007);
            bRet = TRUE;
        }
        else if(0x0004 == u16OutputFormat)
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0001, 0x0007);
            bRet = TRUE;
        }
        else
        {
...
MS_BOOL MDrv_XC_3D_Is_LR_Frame_Exchged(void *pInstance, SCALER_WIN eWindow)
{
    ...
    if((IS_INPUT_NORMAL_2D(eWindow) || IS_INPUT_NORMAL_2D_INTERLACE(eWindow)) &&
            (!IS_OUTPUT_FRAME_ALTERNATIVE()))
    {
        bRet = ((SC_R2BYTEMSK(0, REG_SC_BK20_1C_L, 0xFF00) >> 8) > 0);
    }
    else if(!(IS_INPUT_MODE_NONE(eWindow) || IS_OUTPUT_MODE_NONE())
            && (!(IS_INPUT_NORMAL_2D_HW(eWindow) && IS_OUTPUT_LINE_ALTERNATIVE_HW()))
            && (!(IS_INPUT_NORMAL_2D_HW(eWindow) && IS_OUTPUT_FRAME_ALTERNATIVE_HW()))
           )
    {
        ...
        if(u16OPM & BIT(7))
        {
            bRet = (SC_R2BYTEMSK(0, REG_SC_BK12_33_L, BIT(8)) == BIT(8));
        }
        else if(u16IPM & BIT(0))
        {
            bRet = (SC_R2BYTEMSK(0, REG_SC_BK12_30_L, BIT(4)) == BIT(4));
        }
    ...
    {
#if (HW_2DTO3D_VER >= 3)
        bRet = (SC_R2BYTEMSK(0, REG_SC_BK62_06_L, BIT(15)) == BIT(15));
        bRet ^= (IsVMirrorMode(eWindow) || (MDrv_ReadRegBit(REG_MVOP_MIRROR, (BIT(0)|BIT(1))) == (BIT(0)|BIT(1))));
#else
        bRet = (SC_R2BYTEMSK(0, REG_SC_BK15_67_L, 0x0007) == 0x0001);
        bRet ^= (IsVMirrorMode(eWindow) || (MDrv_ReadRegBit(REG_MVOP_MIRROR, (BIT(0)|BIT(1))) == (BIT(0)|BIT(1))));
#endif
    }
    else if(IS_INPUT_NORMAL_2D_HW(eWindow) && IS_OUTPUT_FRAME_ALTERNATIVE_HW())
...
static void _MDrv_SC_3D_Load2DTo3DReg(void *pInstance, MS_BOOL bEnable, XC_InternalStatus *pSrcInfo, SCALER_WIN eWindow)
{
    ...
    if(bEnable)
    {
        ...
        if(pXCResourcePrivate->stdrvXC_3D._u32HW2DTO3D_DD_Buf >= HAL_MIU2_BASE)
        {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK15_01_L, 0x2000, 0x3000);//DD and DR both use miu2
            u8MIUSel = 2;
        }
        else
#endif
            ...
            {
                u32HW2DTO3D_DD_Buf -= HAL_MIU1_BASE;
                SC_W2BYTEMSK(0, REG_SC_BK15_01_L, 0x1000, 0x3000);//DD and DR both use miu1
                u8MIUSel = 1;
            }
            else
            {
                SC_W2BYTEMSK(0, REG_SC_BK15_01_L, 0x0000, 0x3000);//DD and DR both use miu0
                u8MIUSel =0;
            }
        ...
        //Get panel size
        MS_U32 u32OutputWidth = SC_R2BYTE(0, REG_SC_BK10_05_L) - SC_R2BYTE(0, REG_SC_BK10_04_L) + 1;
        MS_U32 PanelHeight= SC_R2BYTE(0, REG_SC_BK10_07_L) - SC_R2BYTE(0, REG_SC_BK10_06_L) + 1;
        if(u32OutputWidth==0)
        ...
        SC_W2BYTE(0, REG_SC_BK15_09_L, 0x3f00);
        SC_W2BYTE(0, REG_SC_BK15_0A_L, 0x5f00);
        SC_W2BYTE(0, REG_SC_BK15_0B_L, 0x7f00);
        SC_W2BYTE(0, REG_SC_BK15_0C_L, 0x9f00);
        SC_W2BYTE(0, REG_SC_BK15_0D_L, 0xbf00);
        SC_W2BYTE(0, REG_SC_BK15_0E_L, 0x5a1e);
        SC_W2BYTE(0, REG_SC_BK15_0F_L, 0xff00);
        SC_W2BYTE(0, REG_SC_BK15_11_L, 0x3f00);
        SC_W2BYTE(0, REG_SC_BK15_12_L, 0x5f00);
        SC_W2BYTE(0, REG_SC_BK15_13_L, 0x7f00);
        SC_W2BYTE(0, REG_SC_BK15_14_L, 0x9f00);
        SC_W2BYTE(0, REG_SC_BK15_15_L, 0xbf00);
        SC_W2BYTE(0, REG_SC_BK15_16_L, 0x2800);
        SC_W2BYTE(0, REG_SC_BK15_17_L, 0xc0a0);
        SC_W2BYTE(0, REG_SC_BK15_19_L, 0x3f00);
        SC_W2BYTE(0, REG_SC_BK15_1A_L, 0x5f00);
        SC_W2BYTE(0, REG_SC_BK15_1B_L, 0x7f00);
        SC_W2BYTE(0, REG_SC_BK15_1C_L, 0x9f00);
        SC_W2BYTE(0, REG_SC_BK15_1D_L, 0xbf00);
        SC_W2BYTE(0, REG_SC_BK15_1E_L, 0xfa78);
        SC_W2BYTE(0, REG_SC_BK15_1F_L, 0x6040);
        //coef
        SC_W2BYTE(0, REG_SC_BK15_20_L, 0x4d34);
        SC_W2BYTE(0, REG_SC_BK15_21_L, 0x540f);
        SC_W2BYTE(0, REG_SC_BK15_22_L, 0x5133);
        SC_W2BYTE(0, REG_SC_BK15_23_L, 0x171f);
        //mode select
        SC_W2BYTEMSK(0, REG_SC_BK15_24_L, 0x00fc, 0x00FF);
        SC_W2BYTEMSK(0, REG_SC_BK15_24_L, 0x9f00, 0xDF00);
        //filter program
        SC_W2BYTE(0, REG_SC_BK15_25_L, 0x8888);
        SC_W2BYTE(0, REG_SC_BK15_26_L, 0x10ff);
        //color quality initialize
        SC_W2BYTE(0, REG_SC_BK15_38_L, 0xffa4);
        SC_W2BYTE(0, REG_SC_BK15_39_L, 0x0210);
        SC_W2BYTE(0, REG_SC_BK15_3A_L, 0x1210);
        SC_W2BYTE(0, REG_SC_BK15_3B_L, 0x1002);
        SC_W2BYTE(0, REG_SC_BK15_3C_L, 0x0108);
#if (HW_2DTO3D_VER == 3)
        SC_W2BYTEMSK(0, REG_SC_BK15_01_L, MIN(u32OutputWidth, 1920), 0x07FF);//src width
#elif (HW_2DTO3D_VER == 4)
        SC_W2BYTEMSK(0, REG_SC_BK15_01_L, MIN(u32OutputWidth>>1, 1920), 0x07FF);//src width
#endif
        SC_W2BYTEMSK(0, REG_SC_BK15_02_L, MIN(pSrcInfo->stDispWin.height, 1080), 0x0FFF);//src height
        SC_W2BYTEMSK(0, REG_SC_BK15_02_L, 0x7000, 0xF000);//dd shift enable ext
        SC_W2BYTEMSK(0, REG_SC_BK15_05_L, 0x7878, 0x7F7F);//dram request number
        SC_W2BYTEMSK(0, REG_SC_BK15_06_L, (u32HW2DTO3D_DD_Buf/HW_2DTO3D_BYTE_PER_WORD)>>16, 0x07FF);//dd buffer high 11bit
        SC_W2BYTE(0, REG_SC_BK15_07_L, u32HW2DTO3D_DD_Buf/HW_2DTO3D_BYTE_PER_WORD);//dd buffer low 2 bytes
        SC_W4BYTE(0, REG_SC_BK63_44_L, (u32HW2DTO3D_DR_Buf+0x80000)/HW_2DTO3D_BYTE_PER_WORD);//dr buffer:block base address
        SC_W4BYTE(0, REG_SC_BK63_46_L, u32HW2DTO3D_DR_Buf/HW_2DTO3D_BYTE_PER_WORD);//dr buffer:pixel base address
        SC_W2BYTEMSK(0, REG_SC_BK63_4A_L, BIT(10),  BIT(10)); //disable YUV to RGB
        SC_W2BYTEMSK(0, REG_SC_BK15_40_L, BIT(6)|BIT(4),  BIT(6)|BIT(4)); // DD force bank enable,high priorty threshold
        SC_W2BYTEMSK(0, REG_SC_BK15_41_L, 0x0000, 0x000F);     // DD force bank0
        MDrv_XC_wait_output_vsync(pInstance, 1, 100, eWindow);
        SC_W2BYTEMSK(0, REG_SC_BK15_06_L, BIT(15), BIT(13)|BIT(15));//src domain select&start to write miu

        //dr block
        SC_W2BYTEMSK(0, REG_SC_BK63_55_L, u32OutputWidth, 0x1FFF);//pixel width
        SC_W2BYTEMSK(0, REG_SC_BK63_51_L, u32OutputWidth, 0x1FFF);//reg_ln_width
        SC_W2BYTEMSK(0, REG_SC_BK63_52_L, PanelHeight, 0x1FFF);//reg_col_height
        SC_W2BYTEMSK(0, REG_SC_BK63_49_L, (u32OutputWidth+31)/32, 0x00FF);//horizontal number of 32x32 block
        SC_W2BYTEMSK(0, REG_SC_BK63_49_L, BIT(8), BIT(8));//eco en
        SC_W2BYTEMSK(0, REG_SC_BK63_57_L, BIT(1), BIT(1));//dr miu eco en
        SC_W2BYTEMSK(0, REG_SC_BK63_42_L, BIT(7), BIT(7));//reg_clrmiurdy_n
        SC_W2BYTEMSK(0, REG_SC_BK63_48_L, BIT(2), BIT(2)|BIT(3)|BIT(4)|BIT(5)|BIT(6));//reg_blk_dpthptr_en,reg_blk_dpthptr_sel
        //SC_W2BYTE(0, REG_SC_BK63_50_L, 0x3C3C); //reg_dpcach_req_ctr,reg_dpcach_last_ctr
        if(u32OutputWidth==0xf00)
            SC_W2BYTE(0, REG_SC_BK63_50_L, 0x0808); //reg_dpcach_req_ctr,reg_dpcach_last_ctr
        else
            SC_W2BYTE(0, REG_SC_BK63_50_L, 0x0404); //reg_dpcach_req_ctr,reg_dpcach_last_ctr

        //fill sram
        SC_W2BYTE(0, REG_SC_BK62_73_L, 0xFFFF);//reg_fill_in
        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, BIT(7), BIT(7));//reg_fill_w

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0000, 0x0F00);//reg_fill_add 0
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0x5BFE5BFE);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0100, 0x0F00);//reg_fill_add 1
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0xA884A884);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0200, 0x0F00);//reg_fill_add 2
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0x38543854);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0300, 0x0F00);//reg_fill_add 3
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0xA487A487);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0400, 0x0F00);//reg_fill_add 4
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0x32123212);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0500, 0x0F00);//reg_fill_add 5
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0x57265726);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0600, 0x0F00);//reg_fill_add 6
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0xA823A823);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0700, 0x0F00);//reg_fill_add 7
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0xA878A878);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0800, 0x0F00);//reg_fill_add 8
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0xA826A826);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0900, 0x0F00);//reg_fill_add 9
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0x12841284);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0A00, 0x0F00);//reg_fill_add 10
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0x48484848);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0B00, 0x0F00);//reg_fill_add 11
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0x79237923);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0C00, 0x0F00);//reg_fill_add 12
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0x8A428A42);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0D00, 0x0F00);//reg_fill_add 13
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0x38423842);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0E00, 0x0F00);//reg_fill_add 14
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0xFF22FF22);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x0F00, 0x0F00);//reg_fill_add 15
        SC_W4BYTE(0, REG_SC_BK62_74_L, 0xAA88AA88);//reg_fill_sram_1,reg_fill_sram_2

        SC_W2BYTE(0, REG_SC_BK62_73_L, 0x0000);//reg_fill_in
        SC_W2BYTEMSK(0, REG_SC_BK62_71_L, 0x00, BIT(7));//reg_fill_w

        //dr pixel
        SC_W2BYTEMSK(0, REG_SC_BK62_60_L, 0x07, 0x3FFF);//reg_pipe_guard_cycle_pb
        SC_W2BYTEMSK(0, REG_SC_BK62_61_L, u32OutputWidth, 0x3FFF);//reg_ln_width
        SC_W2BYTEMSK(0, REG_SC_BK62_62_L, PanelHeight, 0x1FFF);//reg_col_height
        ...
        {
            SC_W2BYTEMSK(0, REG_SC_BK62_6B_L, BIT(4)|BIT(5), BIT(4)|BIT(5));//dd h 2:1 scaling down
        }
        ...
        {
            SC_W2BYTEMSK(0, REG_SC_BK62_6B_L, BIT(0), BIT(0));//dd v 2:1 scaling down
        }
        //SC_W2BYTEMSK(0, REG_SC_BK62_63_L, BIT(0)|BIT(1), BIT(0)|BIT(1));//reg_srclb_en,reg_depthlb_en
        SC_W2BYTEMSK(0, REG_SC_BK62_68_L, BIT(0)|BIT(2), BIT(0)|BIT(1)|BIT(2));//reg_render_control,reg_lr_control_l,reg_lr_control_r
        SC_W2BYTEMSK(0, REG_SC_BK62_08_L, BIT(5), BIT(5));//rgb_to_y
        SC_W2BYTE(0, REG_SC_BK62_67_L, 0x8080);//reg_pixelsub_l,reg_pixelsub_r
#if (HW_2DTO3D_VER == 4)
        if(u32OutputWidth==0xf00)
            SC_W2BYTEMSK(0, REG_SC_BK62_69_L, BIT(4), BIT(4));//DD v-sacling dn mode
#endif
        //AD
        SC_W2BYTEMSK(0, REG_SC_BK67_32_L, 0x3, 0x3);
        //SC_W2BYTEMSK(0, REG_SC_BK67_34_L, (pXCResourcePrivate->stdrvXC_3D._u32HW2DTO3D_AD_Buf>>20)&0xfff, 0xffff);//baseaddr H 0x3fc0000
        //SC_W2BYTEMSK(0, REG_SC_BK67_33_L, (pXCResourcePrivate->stdrvXC_3D._u32HW2DTO3D_AD_Buf>>4)&0xffff, 0xffff);//baseaddr L
        SC_W2BYTEMSK(0, REG_SC_BK67_34_L, ((u32HW2DTO3D_DR_Buf+0xf0000)>>20)&0xfff, 0xffff);//baseaddr H 0x3fc0000
        SC_W2BYTEMSK(0, REG_SC_BK67_33_L, ((u32HW2DTO3D_DR_Buf+0xf0000)>>4)&0xffff, 0xffff);//baseaddr L
        SC_W2BYTEMSK(0, REG_SC_BK67_35_L, 0x40, 0xff);//depth
        SC_W2BYTEMSK(0, REG_SC_BK67_36_L, 0x40, 0xff);//reg
        SC_W2BYTEMSK(0, REG_SC_BK67_37_L, 0x0, 0xff);
#if (HW_2DTO3D_VER == 4)
        //Tuning initial value
        SC_W2BYTEMSK(0, REG_SC_BK23_58_L, 0x006C, 0x00ff);
        SC_W2BYTEMSK(0, REG_SC_BK23_59_L, 0x7888, 0xffff);
        SC_W2BYTEMSK(0, REG_SC_BK23_5A_L, 0x0000, 0x00ff);
        SC_W2BYTEMSK(0, REG_SC_BK23_5B_L, 0x0010, 0x00ff);
        SC_W2BYTEMSK(0, REG_SC_BK23_5C_L, 0x0D30, 0xffff);
        SC_W2BYTEMSK(0, REG_SC_BK23_5D_L, 0x0710, 0xffff);
        SC_W2BYTEMSK(0, REG_SC_BK23_5E_L, 0x1010, 0xffff);
        SC_W2BYTEMSK(0, REG_SC_BK23_5F_L, 0x1801, 0xff01);
#endif
    }
    else
    {
        MS_U32 u32OutputWidth = pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width;
        SC_W2BYTEMSK(0, REG_SC_BK15_06_L, 0x0000, BIT(13)|BIT(15));//src domain select&start to write miu
        SC_W2BYTEMSK(0, REG_SC_BK15_01_L, 0x0780, 0x07FF);//src width
        SC_W2BYTEMSK(0, REG_SC_BK15_02_L, 0x0438, 0x0FFF);//src height
        SC_W2BYTEMSK(0, REG_SC_BK63_55_L, u32OutputWidth, 0x1FFF);//pixel width
        SC_W2BYTEMSK(0, REG_SC_BK63_51_L, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width, 0x1FFF);//reg_ln_width
        SC_W2BYTEMSK(0, REG_SC_BK63_52_L, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height, 0x1FFF);//reg_col_height
        SC_W2BYTEMSK(0, REG_SC_BK63_49_L, 0x0000, 0x00FF);//horizontal number of 32x32 block
        SC_W2BYTEMSK(0, REG_SC_BK63_49_L, 0x0000, BIT(8));//eco en
        SC_W2BYTEMSK(0, REG_SC_BK63_57_L, 0x0000, BIT(1));//dr miu eco en
        SC_W2BYTEMSK(0, REG_SC_BK63_4A_L, 0x0000, BIT(10));//enable YUV to RGB
        SC_W2BYTE(0, REG_SC_BK63_50_L, 0x8080); //reg_dpcach_req_ctr,reg_dpcach_last_ctr
        SC_W2BYTEMSK(0, REG_SC_BK62_61_L, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width, 0x3FFF);//reg_ln_width
        SC_W2BYTEMSK(0, REG_SC_BK62_62_L, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height, 0x1FFF);//reg_col_height
        SC_W2BYTEMSK(0, REG_SC_BK62_68_L, 0x0000, BIT(0)|BIT(1)|BIT(2));//reg_render_control,reg_lr_control_l,reg_lr_control_r
        SC_W2BYTE(0, REG_SC_BK62_67_L, 0x0000);//reg_pixelsub_l,reg_pixelsub_r
        SC_W2BYTEMSK(0, REG_SC_BK62_6B_L, 0x0000, BIT(0)|BIT(4)|BIT(5));//dd h/v 2:1 scaling down
        //SC_W2BYTEMSK(0, REG_SC_BK62_63_L, 0x0000, BIT(0));// disable T3D SRAM
    }
}
#endif
#endif
...
MS_BOOL MApi_XC_Set_3D_HShift_U2(void* pInstance, MS_U16 u16HShift)
{
                                        ...
                                        , pXCResourcePrivate->stdrvXC_3D._eOutput3DMode, MAIN_WINDOW))
    {
        ...
        {
            //original 3d depth
            SC_W2BYTE(0, REG_SC_BK62_07_L, 0x0000); //2  // 2nd offset L
            SC_W2BYTE(0, REG_SC_BK62_0F_L, 0x0000); //2  // 2nd offset R
        }
        else if(u16HShift > 0x80)
        {
            // |L-->      <--R|
            SC_W2BYTE(0, REG_SC_BK62_07_L, (0xFF - (u16ABSLevel-1))<<8); //2  // 2nd offset L
            SC_W2BYTE(0, REG_SC_BK62_0F_L, (u16ABSLevel)<<8); //2  // 2nd offset R
        }
        else
        {
            // |L<--      -->R|
            SC_W2BYTE(0, REG_SC_BK62_07_L, (u16ABSLevel)<<8); //2  // 2nd offset L
            SC_W2BYTE(0, REG_SC_BK62_0F_L, (0xFF - (u16ABSLevel-1))<<8); //2  // 2nd offset R
        }
#else
        if((u16HShift == 0x80) || (u16HShift == 0x00))
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_5D_L, 0x0000, 0x8000); //2  // 2nd offset enable
        }
        else if(u16HShift > 0x80)
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_5D_L, 0x8000, 0x8000); //2  // 2nd offset enable
            SC_W2BYTE(0, REG_SC_BK15_5E_L, 0x0000); //2  // 2nd offset L
            SC_W2BYTE(0, REG_SC_BK15_5F_L, (u16HShift-0x80)<<8); //2  // 2nd offset R
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_5D_L, 0x8000, 0x8000); //2  // 2nd offset enable
            SC_W2BYTE(0, REG_SC_BK15_5E_L, (0x80-u16HShift)<<8); //2  // 2nd offset L
            SC_W2BYTE(0, REG_SC_BK15_5F_L, 0x0000); //2  // 2nd offset R
        }
#endif
...
MS_BOOL MDrv_XC_Enable_3D_LR_Sbs2Line(void *pInstance, MS_BOOL bEnable)
{
    ...
    SC_W2BYTEMSK(0, REG_SC_BK23_53_L, (bEnable << 15), 0x8000);//dnr offset
...
MS_BOOL MApi_XC_Set_3D_HW2DTo3D_Buffer_U2(void* pInstance, MS_PHY u32HW2DTO3D_DD_Buf, MS_PHY u32HW2DTO3D_DR_Buf)
{
    ...
    //pXCResourcePrivate->stdrvXC_3D._u32HW2DTO3D_AD_Buf = u32HW2DTO3D_AD_Buf;
...
MS_BOOL MApi_XC_EnableAutoDetect3D_U2(void* pInstance, MS_BOOL bEnable, E_XC_3D_AUTODETECT_METHOD enDetectMethod)
{
            ...
            && (!pXCResourcePrivate->stdrvXC_3D._bCurEnableAutoDetect3D))//if disable hw auto detect 3d
    {
        ...
        if(TRUE == MDrv_XC_InterruptIsAttached(pInstance, SC_INT_VSINT, _MDrv_SC_3D_2DTo3D_DepthRefine, NULL) && (pXCResourcePrivate->btvservice == TRUE))
        {
            MDrv_XC_InterruptDeAttachWithoutMutex(pInstance, SC_INT_VSINT, _MDrv_SC_3D_2DTo3D_DepthRefine, NULL);
        }
#if SC2_ENABLE
...
MS_BOOL MApi_XC_DetectNL_U2(void* pInstance, SCALER_WIN eWindow, ST_DETECTNL_PARA* pstDetectNLatticePara)
{
    ...
    if (0 == u8MaxTryTimes || eWindow >= MAX_WINDOW-1)
    {
        return FALSE;
    }
    XC_INSTANCE_PRIVATE *psXCInstPri = NULL;
...
E_XC_3D_INPUT_MODE MApi_XC_Detect3DFormatByContent_U2(void* pInstance, SCALER_WIN eWindow)
{
    ...
    if((IS_INPUT_FRAME_PACKING(MAIN_WINDOW)
            ...
            && IS_OUTPUT_LINE_ALTERNATIVE())
    {
        SC_W2BYTEMSK(0, REG_SC_BK15_24_L, BIT(13), BIT(13));//reg_det3d_la_en
    }
    else
    {
        SC_W2BYTEMSK(0, REG_SC_BK15_24_L, 0x00, BIT(13));//reg_det3d_la_en
    }
#else
    ...
    else if((E_XC_3D_AUTODETECT_HW_COMPATIBLE == pXCResourcePrivate->stdrvXC_3D._enAutoDetectMethod) && pXCResourcePrivate->stdrvXC_3D._bEnableAutoDetect3D)
    {
        ...
        MDrv_WriteRegBit(REG_CKG_FICLK_F1, ENABLE, BIT(6)); //reg_ckg_ficlk_3d
        //dd
        SC_W2BYTEMSK(0, REG_SC_BK15_01_L, gSrcInfo[eWindow].stDispWin.width, 0x07FF);//src width
        SC_W2BYTEMSK(0, REG_SC_BK15_02_L, gSrcInfo[eWindow].stDispWin.height, 0x07FF);//src height
        SC_W2BYTEMSK(0, REG_SC_BK15_05_L, 0x7878, 0x7F7F);//dram request number
        SC_W2BYTEMSK(0, REG_SC_BK15_06_L, (u32HW2DTO3D_DD_Buf/HW_2DTO3D_BYTE_PER_WORD)>>16, 0x07FF);//dd buffer high 9bit
        SC_W2BYTE(0, REG_SC_BK15_07_L, u32HW2DTO3D_DD_Buf/HW_2DTO3D_BYTE_PER_WORD);//dd buffer low 2 bytes
        MDrv_XC_wait_output_vsync(pInstance, 1, 100, eWindow);
#if (HW_2DTO3D_VER < 2)
        SC_W2BYTEMSK(0, REG_SC_BK15_06_L, BIT(13)|BIT(15), BIT(13)|BIT(15));//src domain select&start to write miu
#else
        SC_W2BYTEMSK(0, REG_SC_BK15_06_L, BIT(15), BIT(13)|BIT(15));//src domain select&start to write miu
#endif
#if (HW_2DTO3D_VER > 0)
...
#endif
        SC_W2BYTE(0, REG_SC_BK15_21_L, 0x540f);
        SC_W2BYTE(0, REG_SC_BK15_24_L, 0x0430);
        SC_W2BYTEMSK(0, REG_SC_BK15_40_L, BIT(6),  BIT(6));   // DD force bank enable
        SC_W2BYTEMSK(0, REG_SC_BK15_41_L, 0x0000, 0x000F);     // DD force bank0
        ...
        pXCResourcePrivate->stdrvXC_3D._u16CheckedFrameCount = 0;
        pXCResourcePrivate->stdrvXC_3D._enCurDetectedRes = E_XC_3D_INPUT_MODE_NONE;
        if(FALSE == MDrv_XC_InterruptIsAttached(pInstance, SC_INT_VSINT, _MDrv_SC_3D_2DTo3D_DepthRefine, NULL) && (pXCResourcePrivate->btvservice == TRUE))
        {
        ...
        //dd
        SC_W2BYTEMSK(0, REG_SC_BK15_01_L, 0x0780, 0x07FF);//src width
        SC_W2BYTEMSK(0, REG_SC_BK15_02_L, 0x0438, 0x07FF);//src height
        SC_W2BYTEMSK(0, REG_SC_BK15_05_L, 0x7878, 0x7F7F);//dram request number
        SC_W2BYTE(0, REG_SC_BK15_21_L, 0x0000);
        SC_W2BYTE(0, REG_SC_BK15_24_L, 0x0000);
    ...
    else if((E_XC_3D_AUTODETECT_SW == pXCResourcePrivate->stdrvXC_3D._enAutoDetectMethod) && pXCResourcePrivate->stdrvXC_3D._bEnableAutoDetect3D)
    {
        ...
        MS_PIXEL_24BIT *stDest;

#ifdef MSOS_TYPE_LINUX_KERNEL
        stDest = kmalloc(sizeof(MS_PIXEL_24BIT)*(pXCResourcePrivate->stdrvXC_3D._stDetect3DFormatPara.u16HorSearchRange*pXCResourcePrivate->stdrvXC_3D._stDetect3DFormatPara.u16VerSearchRange), GFP_KERNEL);
#else
        ...
        stDest = malloc(sizeof(MS_PIXEL_24BIT)*(pXCResourcePrivate->stdrvXC_3D._stDetect3DFormatPara.u16HorSearchRange*pXCResourcePrivate->stdrvXC_3D._stDetect3DFormatPara.u16VerSearchRange));
#endif
        if(stDest==NULL)
        {
            printf("[%s][%d] request mem size[0x%x] fail to get memory!!\n", __func__, __LINE__, (sizeof(MS_PIXEL_24BIT)*(pXCResourcePrivate->stdrvXC_3D._stDetect3DFormatPara.u16HorSearchRange*pXCResourcePrivate->stdrvXC_3D._stDetect3DFormatPara.u16VerSearchRange)));
            return E_XC_3D_INPUT_MODE_NONE;
        }
        else
        {
    ...
    free(stDest);
            ...
            memset(stDest, 0 , (sizeof(MS_PIXEL_24BIT)*(pXCResourcePrivate->stdrvXC_3D._stDetect3DFormatPara.u16HorSearchRange*pXCResourcePrivate->stdrvXC_3D._stDetect3DFormatPara.u16VerSearchRange), 2));
...
//-------------------------------------------------------------------------------------------------
    ...
    {
        bRet = FALSE;
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "3D:this 3D format is not supported!e3dInputMode:%u;e3dOutputMode:%u\n",
                     e3dInputMode, e3dOutputMode);
    }
#else
    UNUSED(e3dInputMode);
...
void MDrv_SC_3D_GetFP_Info(void *pInstance, XC_InternalStatus *pSrcInfo, MS_U16 *pu16VactVideo, MS_U16 *pu16VactSpace, MS_BOOL bPreVscalingEna)
{
    ...
    else if(pSrcInfo->stCapWin.height == DOUBLEHD_480X2P_VSIZE)
    {
        *pu16VactVideo = DOUBLEHD_480X2P_FRAME_VSIZE;
        *pu16VactSpace = DOUBLEHD_480X2P_GARBAGE_VSIZE;
    }
    else if(pSrcInfo->stCapWin.height == DOUBLEHD_288X2P_VSIZE)
    {
        *pu16VactVideo = DOUBLEHD_288X2P_FRAME_VSIZE;
        *pu16VactSpace = DOUBLEHD_288X2P_GARBAGE_VSIZE;
    }
    else if(pSrcInfo->stCapWin.height == DOUBLEHD_240X2P_VSIZE)
    {
        *pu16VactVideo = DOUBLEHD_240X2P_FRAME_VSIZE;
        *pu16VactSpace = DOUBLEHD_240X2P_GARBAGE_VSIZE;
    }
    else if(pSrcInfo->stCapWin.height == DOUBLEHD_240X2P_1_VSIZE)
    {
        *pu16VactVideo = DOUBLEHD_240X2P_1_FRAME_VSIZE;
        *pu16VactSpace = DOUBLEHD_240X2P_1_GARBAGE_VSIZE;
    }
    ...
    else if(pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_50_VSIZE)
    {
        *pu16VactVideo = DOUBLEHD_1080X2I_FIELD_VSIZE;
        *pu16VactSpace = (DOUBLEHD_1080X2I_50_VACT_SPACE1*2+DOUBLEHD_1080X2I_50_VACT_SPACE2);
    }
    else if(pSrcInfo->stCapWin.height == DOUBLEHD_576X2I_VSIZE)
    {
        *pu16VactVideo = DOUBLEHD_576X2I_FIELD_VSIZE;
        *pu16VactSpace = (DOUBLEHD_576X2I_VACT_SPACE1*2+DOUBLEHD_576X2I_VACT_SPACE2);
    }
    else if(pSrcInfo->stCapWin.height == DOUBLEHD_480X2I_VSIZE)
    {
        *pu16VactVideo = DOUBLEHD_480X2I_FIELD_VSIZE;
        *pu16VactSpace = (DOUBLEHD_480X2I_VACT_SPACE1*2+DOUBLEHD_480X2I_VACT_SPACE2);
    }
    ...
    else if(pSrcInfo->stCapWin.height == FIELD_INTERLEAVE_480X2I_VISZE)
    {
        *pu16VactVideo = FIELD_INTERLEAVE_480X2I_FIELD_VISZE;
        *pu16VactSpace = FIELD_INTERLEAVE_480X2I_VACT_SPACE;
    }
    else if(pSrcInfo->stCapWin.height == FIELD_INTERLEAVE_576X2I_VISZE)
    {
        *pu16VactVideo = FIELD_INTERLEAVE_576X2I_FIELD_VISZE;
        *pu16VactSpace = FIELD_INTERLEAVE_576X2I_VACT_SPACE;
    }
    else
    {
                                  ...
                                  SCALER_WIN eWindow)
{
    ...
    else if(IS_INPUT_FRAME_PACKING(eWindow))
    {
        ...
        if ((pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_VSIZE) ||
                ...
                (pSrcInfo->stCapWin.height == DOUBLEHD_480X2I_VSIZE))
        {
            pSrcInfo->Status2.u16PreVCusScalingSrc = u16VactVideo * 2;
        }
        else if (eOutputMode == E_XC_3D_OUTPUT_MODE_NONE)
        {
        ...
        if (0)  //(u16ScaleDst != 0xFFFF)
        {
            ...
            if(1)//((g_XC_Pnl_Misc.FRCInfo.u8FRC3DPanelType == E_XC_3D_PANEL_PELLICLE)||(g_XC_Pnl_Misc.FRCInfo.u8FRC3DPanelType == E_XC_3D_PANEL_NONE))
            {
                ...
                else if(((pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_VSIZE) || (pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_50_VSIZE))&&
                        ((eOutputMode == E_XC_3D_OUTPUT_FRAME_L)||(eOutputMode == E_XC_3D_OUTPUT_FRAME_R)))
                {
                    b3DPreVScaling=TRUE;
                    pSrcInfo->Status2.u16PreVCusScalingSrc = (pSrcInfo->stCapWin.height - u16VactSpace);
            ...
            if(IS_INPUT_FRAME_PACKING(eWindow))
            {
                if(((pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_VSIZE) || (pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_50_VSIZE))
    ...
    else if(IS_INPUT_TOP_BOTTOM(eWindow) || IS_INPUT_LINE_ALTERNATIVE(eWindow))
    {
        if(pSrcInfo->Status2.bPreHCusScaling == FALSE)
        {
            if((IS_OUTPUT_SIDE_BY_SIDE_HALF()) ||
                    (IS_INPUT_LINE_ALTERNATIVE(eWindow) && (pSrcInfo->stCapWin.height >= 1440)))
            {
                b3DPreHScaling=TRUE;
                pSrcInfo->Status2.u16PreHCusScalingSrc = pSrcInfo->stCapWin.width;
    ...
    else if(IS_INPUT_FRAME_ALTERNATIVE(eWindow) && (!IS_OUTPUT_FRAME_ALTERNATIVE()) &&(pSrcInfo->stCapWin.width > 1280)
    ...
    else
    {
        ...
        {
#if (HW_DESIGN_3D_VER < 2)
            if(IS_INPUT_PIXEL_ALTERNATIVE(eWindow))//for cases cann't do h scaling, we just prescaling same ratio v instead
            {
                if(pSrcInfo->Status2.u16PreHCusScalingDst <= MST_LINE_BFF_MAX/2)
                {
                    MS_U16 u16Temp = (MS_U32)pSrcInfo->stCapWin.height * pSrcInfo->stDispWin.width / pSrcInfo->stCapWin.width;
                    b3DPreVScaling = TRUE;
                    pSrcInfo->Status2.u16PreVCusScalingSrc = pSrcInfo->stCapWin.height;

                    if(pSrcInfo->Status2.u16PreVCusScalingDst > u16Temp)
                    {
                        pSrcInfo->Status2.u16PreVCusScalingDst = u16Temp;
                    }
                }
            }
            else if(!IS_INPUT_CHECK_BOARD(eWindow))//keep 2d prescaling settings
#endif
            {
                b3DPreHScaling = FALSE;
                pSrcInfo->Status2.u16PreHCusScalingSrc = pSrcInfo->stCapWin.width;
        ...
        {
#if (HW_DESIGN_3D_VER < 2)
            if(IS_INPUT_LINE_ALTERNATIVE(eWindow))//for cases cann't do v scaling, we just prescaling same ratio h instead
            {
                MS_U16 u16Temp = (MS_U32)pSrcInfo->stCapWin.width * pSrcInfo->stDispWin.height / pSrcInfo->stCapWin.height;
                b3DPreHScaling = TRUE;
                pSrcInfo->Status2.u16PreHCusScalingSrc = pSrcInfo->stCapWin.width;

                if(pSrcInfo->Status2.u16PreHCusScalingDst > u16Temp)
                {
                    pSrcInfo->Status2.u16PreHCusScalingDst = u16Temp;
                }
            }
            else if(!IS_INPUT_CHECK_BOARD(eWindow))//keep 2d prescaling settings
#endif
            {
                if(!IS_INPUT_FIELD_ALTERNATIVE(eWindow))
                {
                    ...
                    pSrcInfo->Status2.u16PreVCusScalingSrc = pSrcInfo->stCapWin.height;
                    pSrcInfo->Status2.u16PreVCusScalingDst = pSrcInfo->u16V_SizeAfterPreScaling;
                    if(IS_INPUT_LINE_ALTERNATIVE(eWindow) && IS_OUTPUT_FRAME_ALTERNATIVE())
                    {
                        pSrcInfo->Status2.u16PreVCusScalingSrc = pSrcInfo->stCapWin.height;
                        pSrcInfo->Status2.u16PreVCusScalingDst = pSrcInfo->stDispWin.height*2;
                    }
            ...
            (!pSrcInfo->bInterlace))
    {
        ...
        if(pSrcInfo->Status2.u16PreHCusScalingDst*2 > u32TempWidth)
        {
            b3DPreHScaling=TRUE;
            pSrcInfo->Status2.u16PreHCusScalingSrc = pSrcInfo->stCapWin.width;
    ...
    if(b3DPreVScaling || b3DPreHScaling)
    {
        ...
        _XC_ENTRY(pInstance);
#else
        // this _XC_SEMAPHORE_RETURN(_XC_SELECT_INTERNAL_VARIABLE(0));
        // and _XC_SEMAPHORE_ENTRY(_XC_SELECT_INTERNAL_VARIABLE(0));
        // are unavoidable case under current scaler architecture
        // please be awared and do not remove these two unless
        // refactor the archiecture
        _XC_SEMAPHORE_RETURN(_XC_SELECT_INTERNAL_VARIABLE(0));
        ...
        if((pXCResourcePrivate->sthal_SC.bDynamicScalingEnable) && (pXCResourcePrivate->btvservice == 0))
        {
            ;
        }
        else
        {
            ...
            u32H_PreScalingRatio = SC_R4BYTE(0, REG_SC_BK02_04_L) & 0x40000000;
            u32H_PreScalingRatio |= (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PreScalingRatio & ~0x40000000);
            u32V_PreScalingRatio = SC_R4BYTE(0, REG_SC_BK02_08_L) & 0x40000000;
            ...
            if (pSrcInfo->b3DMenuload)
            {
                ...
                if (eWindow == MAIN_WINDOW)
                {
                    SC_W4BYTE(0, REG_SC_BK02_04_L, u32H_PreScalingRatio);   // H pre-scaling
                    SC_W4BYTE(0, REG_SC_BK02_08_L, u32V_PreScalingRatio);   // V pre-scaling
                }
                else // SUB_WINDOW
                {
                    SC_W4BYTE(0, REG_SC_BK04_04_L, u32H_PreScalingRatio);   // H pre-scaling
                    SC_W4BYTE(0, REG_SC_BK04_08_L, u32V_PreScalingRatio);   // V pre-scaling
                }
            }
                                  ...
                                  SCALER_WIN eWindow)
{
            ...
            IS_OUTPUT_FRAME_L() || IS_OUTPUT_FRAME_R())
    {
        if(IS_INPUT_FRAME_PACKING(eWindow))
        {
            ...
            if ((eOutputMode == E_XC_3D_OUTPUT_LINE_ALTERNATIVE) ||
                    ...
                    (eOutputMode == E_XC_3D_OUTPUT_FRAME_R)          ||
                    (eOutputMode == E_XC_3D_OUTPUT_TOP_BOTTOM))
            {
                    ...
                    (eOutputMode == E_XC_3D_OUTPUT_TOP_BOTTOM))
            {
                ...
                pSrcInfo->u16VCusScalingSrc = (pSrcInfo->ScaledCropWin.height)/2;
#endif
                pSrcInfo->u16VCusScalingDst = pSrcInfo->stDispWin.height/2-1;
            }
            else if((pSrcInfo->stCapWin.height == DOUBLEHD_1080X2P_VSIZE)&&
                    (eOutputMode == E_XC_3D_OUTPUT_TOP_BOTTOM))
            {
    ...
    else if(IS_OUTPUT_FRAME_ALTERNATIVE())
    {
               ...
               )
        {
            ...
            if(IS_INPUT_LINE_ALTERNATIVE(eWindow))
            {
            // FS output, one frame is L , one frame is R
                pSrcInfo->u16VCusScalingSrc = pSrcInfo->ScaledCropWin.height;
                pSrcInfo->u16VCusScalingDst = pSrcInfo->stDispWin.height * 2;
            }
            else
        ...
        else
        {
            ...
            if (pSrcInfo->b3DMenuload)
            {
                ...
                else
                {
                    ...
                    if (eWindow == MAIN_WINDOW)
                    {
                        SC_W4BYTE(0, REG_SC_BK23_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PostScalingRatio);   // H pre-scaling
                        SC_W4BYTE(0, REG_SC_BK23_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PostScalingRatio);   // V pre-scaling
                    }
                    else // SUB_WINDOW
                    {
                        SC_W4BYTE(0, REG_SC_BK23_27_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PostScalingRatio);   // H pre-scaling
                        SC_W4BYTE(0, REG_SC_BK23_29_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PostScalingRatio);   // V pre-scaling
                    }
...
MS_BOOL MDrv_XC_3D_CfgSubWin(void *pInstance, MS_BOOL bEn)
{
    ...
    {
        MS_U32 u32tmp;
        SC_W2BYTEMSK(0, REG_SC_BK20_10_L, 0x0002, 0x0002);//Enable sub window shown on the screen
        SC_W2BYTEMSK(0, REG_SC_BK20_18_L, 0x0002, 0x0002);//Force F1 use F2's register setting
        SC_W2BYTEMSK(0, REG_SC_BK03_02_L, 0x0000, 0x0080);//source sync enable
        //MDrv_ACE_3DClonePQMap(FALSE, FALSE);
        u32tmp = SC_R4BYTE(0, REG_SC_BK02_04_L);
        SC_W4BYTE(0, REG_SC_BK04_04_L, u32tmp);   // H pre-scaling
        u32tmp = SC_R4BYTE(0, REG_SC_BK02_08_L);
        SC_W4BYTE(0, REG_SC_BK04_08_L, u32tmp);   // V pre-scaling
        u32tmp = SC_R4BYTE(0, REG_SC_BK23_07_L);
        SC_W4BYTE(0, REG_SC_BK23_27_L, u32tmp);  // H post-scaling
        u32tmp = SC_R4BYTE(0, REG_SC_BK23_09_L);
        SC_W4BYTE(0, REG_SC_BK23_29_L, u32tmp);  // V post-scaling

        u32tmp = (MS_U32)(SC_R2BYTE(0, REG_SC_BK12_19_L));
        SC_W2BYTE(0, REG_SC_BK12_59_L, (MS_U16)u32tmp);//frame number and opm/ipm seperate.

        u32tmp = (MS_U32)(SC_R2BYTE(0, REG_SC_BK12_01_L));
        SC_W2BYTE(0, REG_SC_BK12_41_L, (MS_U16)u32tmp);//madi format

        u32tmp = (MS_U32)(SC_R2BYTE(0, REG_SC_BK12_02_L));
        SC_W2BYTE(0, REG_SC_BK12_42_L, (MS_U16)u32tmp);

        u32tmp = (MS_U32)(SC_R2BYTE(0, REG_SC_BK12_03_L));
        SC_W2BYTEMSK(0, REG_SC_BK12_43_L, (MS_U16)u32tmp, 0xCFFF);//don't copy h and v mirror

        u32tmp = (MS_U32)(SC_R2BYTE(0, REG_SC_BK12_04_L));
        SC_W2BYTEMSK(0, REG_SC_BK12_44_L, (MS_U16)u32tmp, 0xBFFF);

        u32tmp = (MS_U32)(SC_R2BYTE(0, REG_SC_BK12_05_L));
        SC_W2BYTEMSK(0, REG_SC_BK12_45_L, (MS_U16)u32tmp, 0xFDEF);
    }
    else
    {
        SC_W2BYTEMSK(0, REG_SC_BK20_10_L, 0x0000, 0x0002);//Enable sub window shown on the screen
        SC_W2BYTEMSK(0, REG_SC_BK20_18_L, 0x0000, 0x0002);//Force F1 use F2's register setting
        SC_W2BYTEMSK(0, REG_SC_BK03_02_L, 0x0080, 0x0080);//source sync disable
        //MDrv_ACE_3DClonePQMap(FALSE, FALSE);
    }
...
MS_BOOL MDrv_XC_3D_DynamicScaling_LoadReg(void *pInstance, E_XC_3D_INPUT_MODE eInputMode,
        E_XC_3D_OUTPUT_MODE eOutputMode,
        XC_InternalStatus *pSrcInfo,
        SCALER_WIN eWindow)
{
    ...
    //if((pSrcInfo->stDispWin.x-pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart+pSrcInfo->stDispWin.width)<=0x780)
    ...
    if(IS_INPUT_TOP_BOTTOM(eWindow))
    {
        ...
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK12_0F_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNRFetch>>00),DS_IP);
        ...
        //SCMI: OPM offset
        SC_W2BYTE(0, REG_SC_BK12_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset); // output offset
        ...
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_16_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset>>00),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_17_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch>>00),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_56_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset>>00),DS_OP);
        //SC_W2BYTE(0, REG_SC_BK32_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset); // output offset
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_57_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch>>00),DS_OP);
    ...
    else if(IS_INPUT_SIDE_BY_SIDE_HALF(eWindow))
    {
        ...
        //Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_0E_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_WOffset>>00),DS_IP);
        //Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_0F_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_WFetch>>00),DS_IP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_16_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset/2>>00),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_17_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch>>00),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_56_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset/2>>00),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_57_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch>>00),DS_OP);
    ...
    if(IS_OUTPUT_LINE_ALTERNATIVE())
    {
        ...
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK12_50_L,pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32OPMBase0,DS_IP);
        //SCMI: FRCM_R address 0
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_10_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32FRCM_RBase0>>00),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_11_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32FRCM_RBase0>>16),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_50_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32FRCM_RBase0>>00),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK32_51_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32FRCM_RBase0>>16),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK20_15_L,pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen,DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK20_16_L,pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen,DS_OP);

#if(HW_DESIGN_3D_VER == 5) //monaco mode
        pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset &= ~0x1;
        Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK33_32_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? (BIT(5) | BIT(4)):0x00), DS_OP);
#endif
        u16tempvalue = (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset | (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset&0xFF)<<8);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK20_1D_L,u16tempvalue,DS_OP);

        //H pre-scaling
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK02_04_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PreScalingRatio>>00),DS_IP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK02_05_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PreScalingRatio>>16),DS_IP);
        //V pre-scaling
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK02_08_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PreScalingRatio>>00),DS_IP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK02_09_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PreScalingRatio>>16),DS_IP);

        //H post-scaling
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_07_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PostScalingRatio>>00),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_08_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PostScalingRatio>>16),DS_OP);
        //V post-scaling
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_09_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PostScalingRatio>>00),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_0A_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PostScalingRatio>>16),DS_OP);

        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK04_04_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PreScalingRatio>>00),DS_IP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK04_05_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PreScalingRatio>>16),DS_IP);
        //V pre-scaling
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK04_08_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PreScalingRatio>>00),DS_IP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK04_09_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PreScalingRatio>>16),DS_IP);
        Hal_SC_UpdateDynamicScalingIPcommand(pInstance, MAIN_WINDOW, 1);

        //H post-scaling
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_27_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PostScalingRatio>>00),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_28_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PostScalingRatio>>16),DS_OP);
        //V post-scaling
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_29_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PostScalingRatio>>00),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK23_2A_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PostScalingRatio>>16),DS_OP);

        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_07_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_08_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_09_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_0A_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd),DS_OP);

        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_08_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_09_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_0A_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_0B_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd),DS_OP);
    ...
    else if(IS_OUTPUT_TOP_BOTTOM())
    {
        ...
        if (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart > stdeWin.y)
        {
            // V position
            u16V_detal = (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart - stdeWin.y +1) /2;
        }

        if ((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_EXTERNAL_URSA))
        {
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_09_L,(MS_U16)(stdeWin.y + stdeWin.height/2 + u16V_detal),DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_0A_L,(MS_U16)(stdeWin.y + stdeWin.height/2 + u16V_detal +pSrcInfo->stDispWin.height/2 -1),DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_0A_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart - u16V_detal),DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_0B_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart - u16V_detal + pSrcInfo->stDispWin.height/2 -1),DS_OP);
        }
        else
        {
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_09_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height/2),DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_0A_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height),DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_0A_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart),DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_0B_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height/2-1),DS_OP);
        }
        ...
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK63_55_L,u32OutputWidth,DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK63_52_L,pSrcInfo->stDispWin.height,DS_OP);
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B:%lu\n", pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B);
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "de V start:%u, de height: %u, display V start: %u, display height: %u\n", stdeWin.y, stdeWin.height, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, pSrcInfo->stDispWin.height);
    }
    else if (IS_OUTPUT_SIDE_BY_SIDE_HALF())
    {
        // URSA 3D for blank feature
        //MS_BOOL bchgdisplay = 0;
        MS_U16 u16H_detal = 0;

        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## 3D DS output is SBSH\n");
        ...
        if (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart > stdeWin.x)
        {
            // H position
            u16H_detal = (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart - stdeWin.x + 1) /2;
            //bchgdisplay = 1;
        }
        if ((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_EXTERNAL_URSA))
        {
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_07_L,(MS_U16)(stdeWin.x + stdeWin.width/2 + u16H_detal),DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_08_L,(MS_U16)(stdeWin.x + stdeWin.width/2 + u16H_detal +pSrcInfo->stDispWin.width/2-1),DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_08_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart - u16H_detal),DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_09_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart - u16H_detal + pSrcInfo->stDispWin.width/2 -1),DS_OP);
        }
        else
        {
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_07_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart + pSrcInfo->stDispWin.width/2),DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_08_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart+pSrcInfo->stDispWin.width-1),DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_08_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart),DS_OP);
            Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_09_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart + pSrcInfo->stDispWin.width/2-1),DS_OP);
        }
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B:%lu\n", pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B);
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "de H start:%u, de width: %u, display H start: %u, display width: %u\n", stdeWin.x, stdeWin.width, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, pSrcInfo->stDispWin.width);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_09_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK0F_0A_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_0A_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart),DS_OP);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK10_0B_L,(MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd),DS_OP);
...
#if(HW_DESIGN_3D_VER == 5) //monaco mode
        pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset &= ~0x1;
        Hal_SC_WriteSWDSCommand(pInstance, MAIN_WINDOW, REG_SC_BK33_32_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? BIT(5):0x00), DS_OP);
#endif
        u16tempvalue = (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset | (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset&0xFF)<<8);
        Hal_SC_WriteSWDSCommand(pInstance,MAIN_WINDOW,REG_SC_BK20_1D_L,u16tempvalue,DS_OP);
                           ...
                           SCALER_WIN eWindow)
{
...
#elif (HW_DESIGN_3D_VER >= 2)
    MS_BOOL b3Dstate =  pXCResourcePrivate->stdrvXC_3D.b3Dstate;
    MS_WINDOW_TYPE stdeWin;
    Hal_SC_get_disp_de_window(pInstance, &stdeWin);
    if(b3Dstate)
    {
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"Windows[%u] eInputMode=%u, eOutputMode=%u\n", eWindow, eInputMode, eOutputMode);
    XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW,"Windows[%u] ePreInputMode=%u, ePreOutputMode=%u\n", eWindow, pXCResourcePrivate->stdrvXC_3D.ePreInputMode, pXCResourcePrivate->stdrvXC_3D.ePreOutputMode);

    // 3D menuload for zoom
    if((pSrcInfo->b3DMenuload == 0) && (MDrv_XC_MLoad_GetStatus(pInstance) == E_MLOAD_ENABLED) && (!MDrv_SC_GetSkipWaitVsync(pInstance, MAIN_WINDOW)))
    {
        ...
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW," 3D input/output format is same, not need to set MDrv_XC_3D_LoadReg\n");
        ...
        _MLOAD_ENTRY();
#endif //#if SC2_ENABLE
        //MHal_SC_fill_main_sw_db_burst(pInstance, &pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg);
        if(IS_OUTPUT_TOP_BOTTOM())
        {
            XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## 3D output is TBP, ARC zoom mode moved\n");
            if (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart > stdeWin.y)
            {
                // V position
                u16V_detal = (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart - stdeWin.y +1) /2;
            ...
            XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## 3D output is TBP, ARC zoom mode moved menuload\n");

            if ((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_EXTERNAL_URSA))// &&
                //(bchgdisplay == 1))
            {
                // sub window
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK0F_09_L, (stdeWin.y + stdeWin.height/2 + u16V_detal), 0x0FFF);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK0F_0A_L, (stdeWin.y + stdeWin.height/2 + u16V_detal +pSrcInfo->stDispWin.height/2 -1), 0x0FFF);
                // main window
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0A_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart - u16V_detal), 0x0FFF);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0B_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart - u16V_detal + pSrcInfo->stDispWin.height/2 -1), 0x0FFF);
            }
            else
            {
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK0F_09_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height/2), 0x0FFF);
                pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd = pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height;
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//Main window display window
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height/2-1, 0x0FFF);
            }


            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK0F_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x0FFF);//Sub window display window
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, VOP_DISPLAY_HSTART_MASK); // Display H start
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, VOP_DISPLAY_HSTART_MASK);   // Display H end
#if (HW_DESIGN_3D_VER == 5) //monaco mode
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK33_32_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? (BIT(5) | BIT(4)):0x00), (BIT(5) | BIT(4)));
#else
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK08_32_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? (BIT(5) | BIT(4)):0x00), (BIT(5) | BIT(4)));
#endif
        }
        else if(IS_OUTPUT_SIDE_BY_SIDE_HALF())
        {
            XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## 3D output is SBS, ARC zoom mode moved\n");
            ...
            XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## 3D output is SBS, ARC zoom mode moved menuload\n");
            if ((pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B & E_XC_INIT_MISC_B_EXTERNAL_URSA)) //&&
                // (bchgdisplay == 1))
            {
                // sub window
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK0F_07_L, (stdeWin.x + stdeWin.width/2 + u16H_detal), 0x0FFF);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK0F_08_L, (stdeWin.x + stdeWin.width/2 + u16H_detal +pSrcInfo->stDispWin.width/2-1), 0x0FFF);
                // main window
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_08_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart - u16H_detal), 0x1FFF);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_09_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart - u16H_detal + pSrcInfo->stDispWin.width/2 -1), 0x1FFF);
            }
            else
            {
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK0F_07_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart + pSrcInfo->stDispWin.width/2), 0x0FFF);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart+pSrcInfo->stDispWin.width-1, 0x0FFF);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, VOP_DISPLAY_HSTART_MASK);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart + pSrcInfo->stDispWin.width/2-1, VOP_DISPLAY_HSTART_MASK);
            }
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//Main window display window
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
#if (HW_DESIGN_3D_VER == 5) //monaco mode
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK33_32_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? (BIT(5) | BIT(4)):0x00), (BIT(5) | BIT(4)));
#else
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK08_32_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? (BIT(5) | BIT(4)):0x00), (BIT(5) | BIT(4)));
        ...
        {
            XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## 3D output is FA, ARC zoom mode moved\n");
#if (HW_DESIGN_3D_VER == 4)
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, VOP_DISPLAY_HSTART_MASK); // Display H start
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);

            if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Pnl_Misc.FRCInfo.u83D_FI_out == E_XC_3D_OUTPUT_FI_960x1080)
            {
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart
                                       +(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd-pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart)/2, VOP_DISPLAY_HSTART_MASK);   // Display H end
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
                ...
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_07_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height - 1), 0x0FFF);
            }
            else if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Pnl_Misc.FRCInfo.u83D_FI_out == E_XC_3D_OUTPUT_FI_1920x540)
            {
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, VOP_DISPLAY_HSTART_MASK);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart+(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd-pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart)/2, 0x0FFF);
                ...
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_07_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height/2 - 1), 0x0FFF);
            }
            else if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Pnl_Misc.FRCInfo.u83D_FI_out == E_XC_3D_OUTPUT_FI_1280x720)
            {
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_09_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width - 1), VOP_DISPLAY_HSTART_MASK);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0B_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height - 1), 0x0FFF);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_05_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width - 1), 0x0FFF);
                MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_07_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height - 1), 0x0FFF);
            }
            else
            ...
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x1FFF); // Display H start
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x1FFF);   // Display H end
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
#endif
#if (HW_DESIGN_3D_VER == 5)
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK33_32_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? BIT(5):0x00), BIT(5));
#else
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK08_32_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? BIT(5):0x00), BIT(5));
#endif
        }
        else if(IS_OUTPUT_LINE_ALTERNATIVE())
        {
			...
			XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## 3D output is LA, ARC zoom mode moved\n");
        }
        // Sub OPM address
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_50_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32OPMBase0 & 0xFFFF), 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_51_L, (MS_U16)((pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32OPMBase0 >> 16) & 0xFFFF), 0xFFFF);
#if (HW_DESIGN_3D_VER == 5) //monaco mode
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_50_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32FRCM_RBase0 & 0xFFFF), 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_51_L, (MS_U16)((pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32FRCM_RBase0 >> 16) & 0xFFFF), 0xFFFF);
#endif
        if(IS_INPUT_LINE_ALTERNATIVE(eWindow))
        {
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen/2, 0x0FFF);
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK20_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen/2, 0x0FFF);
        }
        else
        {
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK20_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);
        }
#if (HW_DESIGN_3D_VER == 5) //monaco mode
        pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset &= ~0x1;
#endif
        u16LBOffset = (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset | (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset&0xFF)<<8);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK20_1D_L, u16LBOffset, 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_27_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PostScalingRatio & 0xFFFF), 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_28_L, (MS_U16)((pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PostScalingRatio >> 16) & 0xFFFF), 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_29_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PostScalingRatio & 0xFFFF), 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_2A_L, (MS_U16)((pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PostScalingRatio >> 16) & 0xFFFF), 0xFFFF);
        if(IS_INPUT_SIDE_BY_SIDE_HALF(eWindow)              ||
                IS_INPUT_SIDE_BY_SIDE_FULL(eWindow)              ||
                IS_INPUT_CHECK_BOARD(eWindow)                    ||
                IS_INPUT_LINE_ALTERNATIVE(eWindow)               ||
                ...
                IS_INPUT_PIXEL_ALTERNATIVE(eWindow))
        {
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_18_L, pSrcInfo->stCapWin.height*2, 0x1FFF);
#if (HW_DESIGN_3D_VER == 5) //monaco mode
            MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_18_L, pSrcInfo->stCapWin.height*2, 0x1FFF);
#endif
        }
        //MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_56_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset, 0x1FFF);//F1 OPM OFFSET
#if (HW_DESIGN_3D_VER == 5) //monaco mode
        //MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_0E_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNROffset, 0x1FFF);//dnr offset
        //MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_0F_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNRFetch, 0x1FFF);//dnr fetch
        //MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset, 0x1FFF);//F1 OPM OFFSET
        //MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_56_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset, 0x1FFF);//F1 OPM OFFSET
#endif
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_57_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch, 0x1FFF);//F1 OPM FETCH
#if (HW_DESIGN_3D_VER == 5) //monaco mode
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_57_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch, 0x1FFF);//F1 OPM FETCH
#endif
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_0B_L, pSrcInfo->u16BK23_0B, 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_2B_L, pSrcInfo->u16BK23_0B, 0xFFFF);

        // MHal_SC_fill_main_sw_db_burst, main window for crop feature
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_07_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PostScalingRatio & 0xFFFF), 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_08_L, (MS_U16)((pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32H_PostScalingRatio >> 16) & 0xFFFF), 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_09_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PostScalingRatio & 0xFFFF), 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK23_0A_L, (MS_U16)((pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PostScalingRatio >> 16) & 0xFFFF), 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_10_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32OPMBase0>>00), 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_11_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32OPMBase0>>16), 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_10_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32FRCM_RBase0 & 0xFFFF), 0xFFFF);
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_11_L, (MS_U16)((pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32FRCM_RBase0 >> 16) & 0xFFFF), 0xFFFF);
        //SCMI: OPM offset
        // MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_16_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset>>00), 0xFFFF);
        //SCMI: OPM fetch
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK12_17_L, (MS_U16)(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch>>00), 0xFFFF);
#if (HW_DESIGN_3D_VER == 5) //monaco mode
        MDrv_XC_MLoad_WriteCmd(pInstance, REG_SC_BK32_17_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch, 0x1FFF);//F1 OPM FETCH
#endif
        MDrv_XC_MLoad_Fire(pInstance, TRUE);
#if SC2_ENABLE
        _MLOAD_RETURN(pInstance);
...
#if 1
    // 3D DS load Reg
    if((b3Dstate == 1) && (pXCResourcePrivate->sthal_SC.bDynamicScalingEnable) && (pXCResourcePrivate->btvservice == 0))
    {
        MDrv_XC_3D_DynamicScaling_LoadReg(pInstance, eInputMode, eOutputMode, pSrcInfo, eWindow);
    ...
    if(   ((pXCResourcePrivate->stdrvXC_3D.ePreInputMode != E_XC_3D_INPUT_MODE_NONE) ||
            ((pXCResourcePrivate->stdrvXC_3D.ePreInputMode == E_XC_3D_INPUT_MODE_NONE) &&
             (pXCResourcePrivate->stdrvXC_3D.ePreOutputMode == E_XC_3D_OUTPUT_FRAME_ALTERNATIVE_NOFRC)))
    ...
    {
#if (HW_DESIGN_3D_VER == 5)
        //normal 2D and 3D format except checkboard should enable fclock
        Hal_SC_set_ficlk2(pInstance, CKG_FICLK2_F2_FCLK, ENABLE, MAIN_WINDOW);
        SC_W2BYTEMSK(0, REG_SC_BK57_29_L, 0x0404, 0xFFFF);//[4:0]reg_ini_max_bot_6tap,[12:8]reg_ini_max_top_6tap,should be 0x0404 in monaco 2D mode
        // for 3D 6Tap
        Hal_SC_3D_set_top_win_6Tap(pInstance, DISABLE);
        //Hal_SC_set_fmclk(psXCInstPri, DISABLE);
        SC_W2BYTEMSK(0, REG_SC_BK32_30_L, 0x0000, 0x0001);// 3d enable
        SC_W2BYTEMSK(0, REG_SC_BK32_33_L, 0x0000, 0x0082);//HDMI 3D field select toggle enable, OP 3D mode enable
        SC_W2BYTEMSK(0, REG_SC_BK32_41_L, 0x0000, BIT(7));
        SC_W2BYTEMSK(0, REG_SC_BK32_30_L, 0, BIT(4));//F2 IPM 3D LR invert
        SC_W2BYTEMSK(0, REG_SC_BK32_33_L, 0, BIT(11)|BIT(8)|BIT(1)|BIT(0));//[11]opm3d: OPM SBS using PIP

        SC_W2BYTEMSK(0, REG_SC_BK32_19_L, 0x0000, 0x8800);//clear opm/ipm frame number separate control
        SC_W2BYTEMSK(0, REG_SC_BK32_30_L, 0x0, BIT(8));//Enable free run 3D LR flag
        SC_W2BYTEMSK(0, REG_SC_BK32_06_L, 0x0000, 0x0F00);//[11]Enable, [10:8]F2 IPM user define read

        SC_W2BYTEMSK(0, REG_SC_BK48_23_L, 0x0000, 0x0003);//Enable free run 3D LR flag
        SC_W2BYTEMSK(0, REG_SC_BK48_22_L, 0x0, BIT(4));//reg_ext_lr_en
        if (pSrcInfo->bIsConnectOPM == 1)
        {
            SC_W2BYTEMSK(0, REG_SC_BK57_10_L, 0x0801, 0x0013);
            SC_W2BYTEMSK(0, REG_SC_BK58_10_L, 0x0001, 0x0001);
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK57_10_L, 0x0800, 0x0013);
            SC_W2BYTEMSK(0, REG_SC_BK58_10_L, 0x0000, 0x0001);
        }
        SC_W2BYTEMSK(0, REG_SC_BK57_18_L, 0x0000, 0x0002);//Force F1 use F2's register setting
        SC_W2BYTEMSK(0, REG_SC_BK57_47_L, 0x0000, 0x8000);//reg_sw_tg_en
        SC_W2BYTE(0, REG_SC_BK57_1C_L, 0x00); //Offset main/sub display window in right direction
        SC_W2BYTEMSK(0, REG_SC_BK57_10_L, 0x0, BIT(5)); //Enable HD side by side line buffer mode
        SC_W2BYTEMSK(0, REG_SC_BK57_18_L, 0x0, BIT(9));//fbl 3d enable

        //restore the register because several lines of garbage on the top for frame packing interlace
        SC_W2BYTEMSK(0, REG_SC_BK57_1A_L, 0x0003, 0x0FFF);//reg_atp_trig_dly,Generate TRAIN_TRIG_P from delayed line of Vsync
        SC_W2BYTEMSK(0, REG_SC_BK57_1B_L, 0x0005, 0x0FFF);//reg_disp_trig_dly,Generate DISP_TRIG_P from delayed line of Vsync
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0000, 0x0004);
        if(_bCopyMainOPBase)
        {
            _bCopyMainOPBase = FALSE;
            SC_W4BYTE(0, REG_SC_BK32_50_L, _u32SubBaseAddr1);
            SC_W4BYTE(0, REG_SC_BK32_52_L, _u32SubBaseAddr2);
        }
#endif
        SC_W2BYTEMSK(0, REG_SC_BK20_10_L, 0x0000, 0x0002);//Enable sub window shown on the screen
        SC_W2BYTEMSK(0, REG_SC_BK20_18_L, 0x0000, 0x0002);//Force F1 use F2's register setting
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0000, 0x0001);// 3d enable
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0x0000, 0x0082);//HDMI 3D field select toggle enable, OP 3D mode enable
        ...
        SC_W2BYTEMSK(0, REG_SC_BK23_53_L, 0x0000, 0x8000);// 3D LR side-by-side to line-to-line enable
        SC_W2BYTEMSK(0, REG_SC_BK02_62_L, 0x0000, 0x0130);//external lr signal enable and swap
#if SUPPORT_SEAMLESS_ZAPPING
        if(Hal_SC_GetSeamlessZappingStatus(pInstance, eWindow))
        {
            if(SC_R2BYTEMSK(0, REG_SC_BK02_63_L, BIT(0)|BIT(1)) == (BIT(0)|BIT(1)))
            {
                SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0000, 0xFFFC);//hw 3d TYPE, L/R split etc.
            }
        }
        else
#endif
        {
            SC_W2BYTE(0, REG_SC_BK02_63_L, 0x0000);//hw 3d TYPE, L/R split etc.
        }
        SC_W2BYTE(0, REG_SC_BK02_64_L, 0x0000);//FPP, FPI, FIdA related control
        SC_W2BYTEMSK(0, REG_SC_BK02_65_L, 0x0000, 0x1FFF);//v_active region
        SC_W2BYTEMSK(0, REG_SC_BK02_66_L, 0x0000, 0x1FFF);//v blanking between field1&field2 or field3&field4
        SC_W2BYTEMSK(0, REG_SC_BK02_67_L, 0x0000, 0x1FFF);//v blanking between field2&field3

        SC_W2BYTEMSK(0, REG_SC_BK02_6B_L, 0x00, BIT(0)|BIT(2)); //[0]pix_sep_en,[2]LR_Alt_line
        SC_W2BYTEMSK(0, REG_SC_BK02_6F_L, 0x00, 0x010F); //resample, [8]enable

        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0, BIT(4));//F2 IPM 3D LR invert
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0, BIT(11)|BIT(8)|BIT(1)|BIT(0));//[11]opm3d: OPM SBS using PIP
        //[8] F2 OPM 3D LR invert
        //[1] bsel_r_toggle_en
#if 1                                                        //[0] bsel_r_inv
        ///for 3D film issue
        SC_W2BYTE(0, REG_SC_BK12_14_L, 0x0000);
#endif
        SC_W2BYTEMSK(0, REG_SC_BK12_05_L, 0x0200, 0x0300);//reg_bk_field_sel_f2
#if(HW_DESIGN_3D_VER == 5) //monaco mode
        if(_bCopyMainOPBase == FALSE)
        {
            _bCopyMainOPBase = TRUE;
            _u32SubBaseAddr1 = SC_R4BYTE(0, REG_SC_BK32_50_L);
            _u32SubBaseAddr2 = SC_R4BYTE(0, REG_SC_BK32_52_L);

            SC_W4BYTE(0, REG_SC_BK32_50_L,SC_R4BYTE(0, REG_SC_BK32_10_L));
            SC_W4BYTE(0, REG_SC_BK32_52_L,SC_R4BYTE(0, REG_SC_BK32_12_L));
        }
        SC_W2BYTEMSK(0, REG_SC_BK32_05_L, 0x0200, BIT(9));//Enable OPM F1 register
#endif
        SC_W2BYTEMSK(0, REG_SC_BK12_19_L, 0x0000, 0x8800);//clear opm/ipm frame number separate control
        SC_W2BYTE(0, REG_SC_BK20_1C_L, 0x00); //Offset main/sub display window in right direction
        SC_W2BYTEMSK(0, REG_SC_BK20_10_L, 0x0, BIT(5)); //Enable HD side by side line buffer mode
#if HW_2LINEMODE_DEINTERLACE_SUPPORT
        if(pSrcInfo->bInterlace)
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_18_L, 0x0, BIT(5)); //Force DI 2-line mode at sc2lb mode
...
#endif
        SC_W2BYTEMSK(0, REG_SC_BK12_06_L, 0x0000, 0x0F00);//[11]Enable, [10:8]F2 IPM user define read
        MDrv_WriteByteMask(REG_CKG_FICLK_F2, CKG_FICLK_F2_FLK, CKG_FICLK_F2_MASK);
        SC_W2BYTEMSK(0, REG_SC_BK20_18_L, 0x0, BIT(9));//fbl 3d enable
        SC_W2BYTEMSK(0, REG_SC_BK00_34_L, 0x0, BIT(15));//fbl 3d eco
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0, BIT(8));//Enable free run 3D LR flag
        //HAL_SC_Set_FRCM_FB_Num(pInstance, MAIN_WINDOW, IMAGE_STORE_3_FRAMES, 0);
      ...
      )
    {
        ...
        {
	     if(TRUE == pXCResourcePrivate->bRecord_Flag)
	     {
	     }
	     else
	     {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x1FFF); // Display H start
            SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x1FFF);   // Display H end
            SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//Main window display window
            SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);


            if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Pnl_Misc.FRCInfo.u83D_FI_out == E_XC_3D_OUTPUT_FI_MODE_NONE)
            {
                //SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x1FFF);
                //SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
                //SC_W2BYTEMSK(0, REG_SC_BK10_05_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width - 1), 0x0FFF);
                //SC_W2BYTEMSK(0, REG_SC_BK10_07_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height - 1), 0x0FFF);
            }
        ...
        if (IS_OUTPUT_FRAME_ALTERNATIVE_NOFRC())
        {
            SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(1), BIT(1));
#if(HW_DESIGN_3D_VER == 5) //monaco mode
            SC_W2BYTEMSK(0, REG_SC_BK32_33_L, BIT(1), BIT(1));
#endif
        }
    }
    else
    {
        SC_W2BYTEMSK(0, REG_SC_BK12_05_L, 0x0000, BIT(9));//Enable OPM F1 register
#if(HW_DESIGN_3D_VER == 5) //monaco mode
        SC_W2BYTEMSK(0, REG_SC_BK32_05_L, 0x0000, BIT(9));//Enable OPM F1 register
#endif
    }
    ...
    if(IS_INPUT_NORMAL_2D(eWindow) && IS_OUTPUT_TOP_BOTTOM())
    {
        SC_W2BYTEMSK(0, REG_SC_BK08_30_L, BIT(14), BIT(14));

        if(SC_R2BYTEMSK(0, REG_SC_BK08_01_L, BIT(0)))
        {
            SC_W2BYTEMSK(0, REG_SC_BK08_01_L, BIT(1), BIT(1));
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK08_01_L, 0x00, BIT(1));
        }
    }
    else
    {
        SC_W2BYTEMSK(0, REG_SC_BK08_30_L, 0x00, BIT(14));
    }
    ...
    if(b3Dstate)
    {
        ...
        if (eWindow == MAIN_WINDOW)
        {
            SC_W4BYTE(0, REG_SC_BK12_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32DNRBase0);//DNR OPM base set the same
            SC_W4BYTE(0, REG_SC_BK12_10_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32OPMBase0);
#if(HW_DESIGN_3D_VER == 5) //monaco mode
            SC_W4BYTE(0, REG_SC_BK32_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32FRCM_WBase0);//DNR OPM base set the same
            SC_W4BYTE(0, REG_SC_BK32_10_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32FRCM_RBase0);//frcm_rbase0 for main
            SC_W4BYTE(0, REG_SC_BK32_50_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32FRCM_RBase0);//frcm_rbase0 for sub
#endif
            SC_W2BYTEMSK(0, REG_SC_BK20_12_L, 0x0000, 0x1FFF);//The starting address of f2 stored at line buffer
        }
#if (HW_DESIGN_3D_VER >= 4)
        if((IS_INPUT_FRAME_PACKING(eWindow)&&IS_OUTPUT_TOP_BOTTOM())||
                (IS_INPUT_FRAME_PACKING(eWindow)&&IS_OUTPUT_FRAME_L())||
          ...
          )
        {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK20_13_L, 0x00, 0x1FFF);
            ...
            SC_W2BYTEMSK(0, REG_SC_BK20_10_L, 0x00, BIT(10)); //[10]reg_vblank_main

            if (IsVMirrorMode(eWindow))
            {
                SC_W2BYTEMSK(0, REG_SC_BK20_10_L, BIT(11), BIT(11)); //[11]reg_vblank_sub
            }
            else //non mirror
            {
                SC_W2BYTEMSK(0, REG_SC_BK20_10_L, 0 , BIT(11)); //[11]reg_vblank_sub
            }
        }
        //else
#endif
...
#endif
            SC_W2BYTEMSK(0, REG_SC_BK20_13_L, MS_3D_LINE_BFF_MAX, 0x1FFF);//The starting address of f1 stored at line buffer
        }
    }
    ...
    if(IS_INPUT_NORMAL_2D(eWindow)
            || (IS_INPUT_NORMAL_2D_HW(eWindow) && (IS_OUTPUT_TOP_BOTTOM_HW() || IS_OUTPUT_SIDE_BY_SIDE_HALF_HW())))
    {
        SC_W2BYTEMSK(0, REG_SC_BK12_0E_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNROffset, 0x1FFF);//dnr offset
        SC_W2BYTEMSK(0, REG_SC_BK12_0F_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNRFetch, 0x1FFF);//dnr fetch
        SC_W2BYTEMSK(0, REG_SC_BK12_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset, 0x1FFF);//OPM offset
        SC_W2BYTEMSK(0, REG_SC_BK12_17_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch, 0x0FFF);//OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK12_56_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset, 0x1FFF);//F1 OPM OFFSET
        SC_W2BYTEMSK(0, REG_SC_BK12_57_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch, 0x0FFF);//F1 OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0x0800, 0x0800);//opm3d: OPM SBS using PIP
    }
    else if(IS_INPUT_FRAME_PACKING(eWindow))
    {
        MS_U16 u16VactVideo, u16VactSpace;
        ...
        SC_W2BYTEMSK(0, REG_SC_BK12_0F_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNRFetch, 0x1FFF);//dnr fetch
        SC_W2BYTEMSK(0, REG_SC_BK12_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset, 0x1FFF);//OPM offset
        SC_W2BYTEMSK(0, REG_SC_BK12_17_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch, 0x1FFF);//OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK12_56_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset, 0x1FFF);//F1 OPM OFFSET
        SC_W2BYTEMSK(0, REG_SC_BK12_57_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch, 0x1FFF);//F1 OPM FETCH

#if(HW_DESIGN_3D_VER == 5) //monaco mode
        SC_W2BYTEMSK(0, REG_SC_BK32_0E_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNROffset, 0x1FFF);//dnr offset
        SC_W2BYTEMSK(0, REG_SC_BK32_0F_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNRFetch, 0x1FFF);//dnr fetch
        SC_W2BYTEMSK(0, REG_SC_BK32_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset, 0x1FFF);//OPM offset
        SC_W2BYTEMSK(0, REG_SC_BK32_17_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch, 0x1FFF);//OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK32_56_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset, 0x1FFF);//F1 OPM OFFSET
        SC_W2BYTEMSK(0, REG_SC_BK32_57_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch, 0x1FFF);//F1 OPM FETCH

        if((pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_VSIZE) || (pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_50_VSIZE) ||
                (pSrcInfo->stCapWin.height == DOUBLEHD_576X2I_VSIZE)  || (pSrcInfo->stCapWin.height == DOUBLEHD_480X2I_VSIZE))
        {
            //interlace framepacking
            SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0002, 0x0113);//reg_lr_chg_mode[1:0],reg_ini_lr[4],reg_split_half[8]
            SC_W2BYTEMSK(0, REG_SC_BK02_64_L, 0x0237, 0xff37);//Blanking number in one vsync,enable gen pseudo vsync in 3d
            SC_W2BYTEMSK(0, REG_SC_BK02_65_L, u16VactVideo, 0x1FFF);//v_active region
            SC_W2BYTEMSK(0, REG_SC_BK02_66_L, DOUBLEHD_1080X2I_VACT_SPACE1, 0x1FFF);//v blanking between field1&field2 or field3&field4
            SC_W2BYTEMSK(0, REG_SC_BK02_67_L, DOUBLEHD_1080X2I_VACT_SPACE2, 0x1FFF);//v blanking between field2&field3
            SC_W2BYTEMSK(0, REG_SC_BK02_38_L, pSrcInfo->u16V_SizeAfterPreScaling/2, 0x1FFF);
            SC_W2BYTEMSK(0, REG_SC_BK02_38_L, BIT(15), BIT(15));
            SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, pSrcInfo->stCapWin.width, 0x1FFF);
            SC_W2BYTEMSK(0, REG_SC_BK12_05_L, 0x0100, 0x0100);//reg_bk_field_sel_f2
            SC_W2BYTEMSK(0, REG_SC_BK12_06_L, 0x0A00, 0x0F00);//[11]Enable, [10:8]F2 IPM user define read
            SC_W2BYTEMSK(0, REG_SC_BK12_30_L, BIT(0), BIT(0));//F2 IPM 3D input enable
            SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(7), BIT(7));//OP 3D ENABLE

            SC_W2BYTEMSK(0, REG_SC_BK48_23_L, 0x0002, 0x0003);//reg_lr_chg_mode
            SC_W2BYTEMSK(0, REG_SC_BK48_23_L, 0x0000, 0x0010);//reg_lr_chg_mode
            SC_W2BYTEMSK(0, REG_SC_BK48_38_L, pSrcInfo->u16V_SizeAfterPreScaling, 0x1FFF);//active_hight  after VSD
            SC_W2BYTEMSK(0, REG_SC_BK48_38_L, BIT(15), BIT(15));//reg_vsd_usr_vact_video_en

            SC_W2BYTEMSK(0, REG_SC_BK57_10_L, 0x0003, 0x0C03);//[0]:reg_main_en, [1]:reg_sw_sub_en,[11]:reg_vblank_sub
            SC_W2BYTEMSK(0, REG_SC_BK57_15_L, pSrcInfo->u16V_SizeAfterPreScaling, 0x0FFF);//active_hight  after VSD
            SC_W2BYTEMSK(0, REG_SC_BK57_16_L, pSrcInfo->u16V_SizeAfterPreScaling, 0x0FFF);//active_hight  after VSD
            //several lines of garbage on the top
            SC_W2BYTEMSK(0, REG_SC_BK57_1A_L, 0x0000, 0x0FFF);//reg_atp_trig_dly,Generate TRAIN_TRIG_P from delayed line of Vsync
            SC_W2BYTEMSK(0, REG_SC_BK57_1B_L, 0x0001, 0x0FFF);//reg_disp_trig_dly,Generate DISP_TRIG_P from delayed line of Vsync

            SC_W2BYTEMSK(0, REG_SC_BK57_47_L, BIT(15), BIT(15));//reg_vlen_f2
            SC_W2BYTEMSK(0, REG_SC_BK57_47_L, 0x0010, 0x3FFF);//reg_htot
            SC_W2BYTEMSK(0, REG_SC_BK57_48_L, 0x0002, 0x3FFF);//reg_hst_f2
            SC_W2BYTEMSK(0, REG_SC_BK57_49_L, 0x000a, 0x3FFF);//reg_hend_f2
            SC_W2BYTEMSK(0, REG_SC_BK57_4A_L, 0x0002, 0x3FFF);//reg_hst_f1
            SC_W2BYTEMSK(0, REG_SC_BK57_4B_L, 0x000a, 0x3FFF);//reg_hend_f1
            SC_W2BYTEMSK(0, REG_SC_BK57_4C_L, 0x0000, 0x0FFF);//reg_vst_f2
            SC_W2BYTEMSK(0, REG_SC_BK57_4D_L, (u16VactVideo*2 - 1), 0x0FFF);//reg_vend_f2
            SC_W2BYTEMSK(0, REG_SC_BK57_4E_L, u16VactVideo*2, 0x0FFF);//reg_vst_f1
            SC_W2BYTEMSK(0, REG_SC_BK57_4F_L, (4*u16VactVideo - 1), 0x0FFF);//reg_vend_f1

            SC_W2BYTEMSK(0, REG_SC_BK20_03_L,BIT(2), BIT(2));//reg_force_fe2_en
            SC_W2BYTEMSK(0, REG_SC_BK32_30_L,BIT(0), BIT(0));//reg_ipm_3d_en
            SC_W2BYTEMSK(0, REG_SC_BK32_33_L,BIT(7), BIT(7));//reg_opm_3d_en
            SC_W2BYTEMSK(0, REG_SC_BK32_41_L,BIT(7), BIT(7));
        }
        else
        {
            //progressive framepacking
            SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0002, 0x0113);//hw 3d TYPE, L/R split
            SC_W2BYTEMSK(0, REG_SC_BK02_64_L, 0x011, 0x0037);//Blanking number in one vsync,enable gen pseudo vsync in 3d
            SC_W2BYTEMSK(0, REG_SC_BK02_65_L, u16VactVideo, 0x1FFF);//v_active region=L Frame V size after prescaling
            SC_W2BYTEMSK(0, REG_SC_BK02_66_L, u16VactSpace, 0x1FFF);//v blanking between field1&field2 or field3&field4
            SC_W2BYTEMSK(0, REG_SC_BK02_38_L, pSrcInfo->u16V_SizeAfterPreScaling, 0x1FFF);
            SC_W2BYTEMSK(0, REG_SC_BK02_38_L, BIT(15), BIT(15));
            SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, pSrcInfo->stCapWin.width, 0x1FFF);
            ...
            SC_W2BYTEMSK(0, REG_SC_BK48_23_L, 0x0002, 0x0013);//[2:0]reg_lr_chg_mode,[4]:reg_ini_lr
            SC_W2BYTEMSK(0, REG_SC_BK48_38_L, pSrcInfo->u16V_SizeAfterPreScaling, 0x1FFF);//active_hight  after VSD
            SC_W2BYTEMSK(0, REG_SC_BK48_38_L, BIT(15), BIT(15));//reg_vsd_usr_vact_video_en
            ...
            SC_W2BYTEMSK(0, REG_SC_BK57_4B_L, 0x000a, 0x3FFF);//reg_hend_f1
            SC_W2BYTEMSK(0, REG_SC_BK57_4C_L, 0x0000, 0x0FFF);//reg_vst_f2
            SC_W2BYTEMSK(0, REG_SC_BK57_4D_L, (u16VactVideo - 1), 0x0FFF);//reg_vend_f2
            SC_W2BYTEMSK(0, REG_SC_BK57_4E_L, u16VactVideo, 0x0FFF);//reg_vst_f1
            SC_W2BYTEMSK(0, REG_SC_BK57_4F_L, (2*u16VactVideo - 1), 0x0FFF);//reg_vend_f1
        ...
        {
            //interlace framepacking
            SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0002, 0x0103);//hw 3d TYPE, L/R split
            SC_W2BYTEMSK(0, REG_SC_BK02_64_L, 0x0237, 0xff37);//Blanking number in one vsync,enable gen pseudo vsync in 3d
            SC_W2BYTEMSK(0, REG_SC_BK02_65_L, u16VactVideo, 0x1FFF);//v_active region
            SC_W2BYTEMSK(0, REG_SC_BK02_66_L, DOUBLEHD_1080X2I_VACT_SPACE1, 0x1FFF);//v blanking between field1&field2 or field3&field4
            SC_W2BYTEMSK(0, REG_SC_BK02_67_L, DOUBLEHD_1080X2I_VACT_SPACE2, 0x1FFF);//v blanking between field2&field3
        ...
        else
        {
            ...
            //progressive framepacking
            SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0002, 0x0103);//hw 3d TYPE, L/R split
            SC_W2BYTEMSK(0, REG_SC_BK02_64_L, 0x0011, 0x0037);//Blanking number in one vsync
            SC_W2BYTEMSK(0, REG_SC_BK02_65_L, u16VactVideo, 0x1FFF);//v_active region=L Frame V size after prescaling
            SC_W2BYTEMSK(0, REG_SC_BK02_66_L, u16VactSpace, 0x1FFF);//v blanking between field1&field2 or field3&field4
            SC_W2BYTEMSK(0, REG_SC_BK12_30_L, BIT(0), BIT(0));//F2 IPM 3D input enable
            SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(7), BIT(7));//OP 3D ENABLE
#if (HW_DESIGN_3D_VER >= 4)
            SC_W2BYTEMSK(0, REG_SC_BK02_38_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x1FFF);
#else
            MDrv_SC_3D_GetFP_Info(pInstance, pSrcInfo, &u16VactVideo, &u16VactSpace, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PreScalingRatio & BIT(31))>>31);
            SC_W2BYTEMSK(0, REG_SC_BK02_38_L, pSrcInfo->u16V_SizeAfterPreScaling, 0x1FFF);
#endif
            SC_W2BYTEMSK(0, REG_SC_BK02_38_L, BIT(15), BIT(15));
            SC_W2BYTEMSK(0, REG_SC_BK12_06_L, 0x0900, 0x0F00);//[11]Enable, [10:8]F2 IPM user define read
            SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, pSrcInfo->stCapWin.width, 0x1FFF);
        }
#else
        if(pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_VSIZE)
        {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0002, 0x0103);//hw 3d TYPE, L/R split
            SC_W2BYTEMSK(0, REG_SC_BK02_64_L, 0x0237, 0xff37);//Blanking number in one vsync,enable gen pseudo vsync in 3d
            SC_W2BYTEMSK(0, REG_SC_BK02_65_L, DOUBLEHD_1080X2I_FIELD_VSIZE, 0x1FFF);//v_active region
            SC_W2BYTEMSK(0, REG_SC_BK02_66_L, DOUBLEHD_1080X2I_VACT_SPACE1, 0x1FFF);//v blanking between field1&field2 or field3&field4
            SC_W2BYTEMSK(0, REG_SC_BK02_67_L, DOUBLEHD_1080X2I_VACT_SPACE2, 0x1FFF);//v blanking between field2&field3
            SC_W2BYTEMSK(0, REG_SC_BK12_05_L, 0x0100, 0x0100);//reg_bk_field_sel_f2
            SC_W2BYTEMSK(0, REG_SC_BK12_30_L, BIT(0), BIT(0));//F2 IPM 3D input enable
    ...
    else if(IS_INPUT_FIELD_ALTERNATIVE(eWindow))
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK12_05_L, 0x0000, 0x0100);//reg_local_field
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, BIT(0), BIT(0));//F2 IPM 3D input enable
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(7), BIT(7));//OP 3D ENABLE
        ...
        SC_W2BYTEMSK(0, REG_SC_BK57_4B_L, 0x000a, 0x3FFF);//reg_hend_f1
        SC_W2BYTEMSK(0, REG_SC_BK57_4C_L, 0x0000, 0x0FFF);//reg_vst_f2
        SC_W2BYTEMSK(0, REG_SC_BK57_4D_L, (pSrcInfo->u16V_SizeAfterPreScaling - 1), 0x0FFF);//reg_vend_f2
        SC_W2BYTEMSK(0, REG_SC_BK57_4E_L, pSrcInfo->u16V_SizeAfterPreScaling , 0x0FFF);//reg_vst_f1
        SC_W2BYTEMSK(0, REG_SC_BK57_4F_L, (2*pSrcInfo->u16V_SizeAfterPreScaling  - 1), 0x0FFF);//reg_vend_f1
        SC_W2BYTEMSK(0, REG_SC_BK32_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset, 0x1FFF);//OPM offset
        SC_W2BYTEMSK(0, REG_SC_BK32_17_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch, 0x1FFF);//OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK32_56_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset, 0x1FFF);//F1 OPM OFFSET
        SC_W2BYTEMSK(0, REG_SC_BK32_57_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch, 0x1FFF);//F1 OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK32_18_L, pSrcInfo->u16V_SizeAfterPreScaling*2, 0x1FFF);
    ...
    else if(IS_INPUT_FRAME_ALTERNATIVE(eWindow))
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK12_57_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch, 0x0FFF);//F1 OPM FETCH
#if(HW_DESIGN_3D_VER == 5) //monaco mode
        SC_W2BYTEMSK(0, REG_SC_BK02_62_L, 0x0030, 0x0030);// input Vsync exist between L and R frame
        SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0001, 0x0013);// hw 3d TYPE, L/R split=0
        SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, pSrcInfo->stCapWin.width, 0x1FFF);

        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, BIT(0), BIT(0));//F2 IPM 3D input enable
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(7), BIT(7));//OP 3D ENABLE

        SC_W2BYTEMSK(0, REG_SC_BK48_22_L, BIT(4), BIT(4));//reg_ext_lr_en
        SC_W2BYTEMSK(0, REG_SC_BK48_23_L, 0x0001, 0x0013);//reg_lr_chg_mode

        SC_W2BYTEMSK(0, REG_SC_BK57_10_L, 0x0001, 0x0C03);//[0]:reg_main_en, [1]:reg_sw_sub_en, [11]:reg_vblank_sub
        SC_W2BYTEMSK(0, REG_SC_BK57_15_L, pSrcInfo->u16V_SizeAfterPreScaling, 0x0FFF);//active_hight  after VSD
        SC_W2BYTEMSK(0, REG_SC_BK57_16_L, pSrcInfo->u16V_SizeAfterPreScaling, 0x0FFF);//active_hight  after VSD
        SC_W2BYTEMSK(0, REG_SC_BK57_47_L, 0x0000, 0x1000);//reg_sw_tg_en
        ...
        SC_W2BYTEMSK(0, REG_SC_BK32_57_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch, 0x1FFF);//F1 OPM FETCH
#elif (HW_DESIGN_3D_VER >= 3)
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, BIT(0), BIT(0));// 3d enable
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(7), BIT(7));
        SC_W2BYTEMSK(0, REG_SC_BK02_62_L, 0x0030, 0x0030);// input Vsync exist between L and R frame
        SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0001, 0x0103);// hw 3d TYPE, L/R split=0

        SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, pSrcInfo->stCapWin.width, 0x1FFF);
#else
        if(pSrcInfo->bInterlace == TRUE)
        {
            SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0001, 0x0001);// 3d enable
            SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0x0083, 0x0080);//[8] F2 OPM 3D LR invert
            SC_W2BYTEMSK(0, REG_SC_BK02_62_L, 0x0010, 0x0010);// input Vsync exist between L and R frame
            SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0001, 0x0103);// hw 3d TYPE, L/R split=0
        }
        else
        {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0x0083, 0x0080);//[8] F2 OPM 3D LR invert
            SC_W2BYTE(0, REG_SC_BK12_19_L, 0x8804);//input 4 frame, output 2 frame with 3d
        }
#endif
    }
    else if(IS_INPUT_TOP_BOTTOM(eWindow))
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK32_0E_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNROffset, 0x1FFF);//dnr offset
        SC_W2BYTEMSK(0, REG_SC_BK32_0F_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNRFetch, 0x1FFF);//dnr fetch
        SC_W2BYTEMSK(0, REG_SC_BK32_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset, 0x1FFF);//OPM offset
        SC_W2BYTEMSK(0, REG_SC_BK32_17_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch, 0x1FFF);//OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK32_56_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset, 0x1FFF);//F1 OPM OFFSET
        SC_W2BYTEMSK(0, REG_SC_BK32_57_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch, 0x1FFF);//F1 OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0002, 0x0113);//hw 3d TYPE, L/R split
        SC_W2BYTEMSK(0, REG_SC_BK02_65_L, pSrcInfo->stCapWin.height, 0x1FFF);
        SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, pSrcInfo->stCapWin.width, 0x1FFF);
        //SC_W2BYTEMSK(0, REG_SC_BK02_38_L, pSrcInfo->u16V_SizeAfterPreScaling/2, 0x1FFF);
        SC_W2BYTEMSK(0, REG_SC_BK02_38_L, BIT(15), BIT(15));
        ...
        {
            SC_W2BYTEMSK(0, REG_SC_BK02_38_L, pSrcInfo->u16V_SizeAfterPreScaling/4, 0x1FFF);
        }
        SC_W2BYTEMSK(0, REG_SC_BK02_38_L, BIT(15), BIT(15));

        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0001, 0x0001);// 3d enable
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0x0080, 0x0080);

        SC_W2BYTEMSK(0, REG_SC_BK48_23_L, 0x0002, 0x0013);//reg_lr_chg_mode
        SC_W2BYTEMSK(0, REG_SC_BK48_38_L, BIT(15), BIT(15));//reg_vsd_usr_vact_video_en
        SC_W2BYTEMSK(0, REG_SC_BK48_38_L, pSrcInfo->u16V_SizeAfterPreScaling/2, 0x1FFF);//reg_vsd_usr_vact_video
        if (pSrcInfo->bIsConnectOPM == 1)
        {
            SC_W2BYTEMSK(0, REG_SC_BK57_10_L, 0x0003, 0x0C03);//reg_hst_f2, [1]:reg_sw_sub_en, [11]:reg_vblank_sub
            SC_W2BYTEMSK(0, REG_SC_BK58_10_L, 0x0001, 0x0001);
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK57_10_L, 0x0000, 0x0C03);//reg_hst_f2, [1]:reg_sw_sub_en, [11]:reg_vblank_sub
            SC_W2BYTEMSK(0, REG_SC_BK58_10_L, 0x0000, 0x0001);
        }
        SC_W2BYTEMSK(0, REG_SC_BK57_47_L, 0x8010, 0xBFFF);//reg_hst_f2
        SC_W2BYTEMSK(0, REG_SC_BK57_48_L, 0x0002, 0x3FFF);//reg_hst_f2
        SC_W2BYTEMSK(0, REG_SC_BK57_49_L, 0x000a, 0x3FFF);//reg_hend_f2
        SC_W2BYTEMSK(0, REG_SC_BK57_4A_L, 0x0002, 0x3FFF);//reg_hst_f1
        SC_W2BYTEMSK(0, REG_SC_BK57_4B_L, 0x000a, 0x3FFF);//reg_hend_f1
        SC_W2BYTEMSK(0, REG_SC_BK57_4C_L, 0x0000, 0x0FFF);//reg_vst_f2
        SC_W2BYTEMSK(0, REG_SC_BK57_15_L, pSrcInfo->u16V_SizeAfterPreScaling/2, 0x0FFF);//active_hight  after VSD
        SC_W2BYTEMSK(0, REG_SC_BK57_16_L, pSrcInfo->u16V_SizeAfterPreScaling/2, 0x0FFF);//active_hight  after VSD
        SC_W2BYTEMSK(0, REG_SC_BK57_4D_L, (pSrcInfo->u16V_SizeAfterPreScaling/2 - 1), 0x0FFF);//reg_vend_f2
        SC_W2BYTEMSK(0, REG_SC_BK57_4E_L, pSrcInfo->u16V_SizeAfterPreScaling/2, 0x0FFF);//reg_vst_f1
        SC_W2BYTEMSK(0, REG_SC_BK57_4F_L, (pSrcInfo->u16V_SizeAfterPreScaling - 1), 0x0FFF);//reg_vend_f1
        ...
        {
            //progressive
            SC_W2BYTEMSK(0, REG_SC_BK02_65_L, pSrcInfo->stCapWin.height, 0x1FFF); //v_active region=L Frame V size after prescaling
        ...
        if (SC_R2BYTEMSK(0, REG_SC_BK20_10_L, BIT(5)) == BIT(5))
        {
            // If use 2Tap mode, need to disable PQ scaling filter
            SC_W2BYTE(0, REG_SC_BK23_0B_L, 0x303);
        }
        ...
        {
            //progressive
            SC_W2BYTEMSK(0, REG_SC_BK02_65_L, pSrcInfo->u16V_SizeAfterPreScaling/2, 0x1FFF); //v_active region=L Frame V size after prescaling
        }
        else
        {
            //interlace
            SC_W2BYTEMSK(0, REG_SC_BK02_65_L, pSrcInfo->u16V_SizeAfterPreScaling/4, 0x1FFF);
        }
#endif
    }
    else if(IS_INPUT_LINE_ALTERNATIVE(eWindow))
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0003, 0x0113);//hw 3d TYPE, L/R idx
        SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, pSrcInfo->stCapWin.width, 0x1FFF);
        if((pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PreScalingRatio & BIT(31))>>31)
        {
            SC_W2BYTEMSK(0, REG_SC_BK02_09_L, BIT(12), BIT(12));
        }
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0001, 0x0001);// 3d enable
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0x0080, 0x0080);//OP 3D ENABLE
        SC_W2BYTEMSK(0, REG_SC_BK12_18_L, pSrcInfo->stCapWin.height*2, 0x1FFF);

        SC_W2BYTEMSK(0, REG_SC_BK48_23_L, 0x0003, 0x0013);//reg_lr_chg_mode
        SC_W2BYTEMSK(0, REG_SC_BK48_38_L, pSrcInfo->u16V_SizeAfterPreScaling/2, 0x1FFF);//active_hight  after VSD
        SC_W2BYTEMSK(0, REG_SC_BK48_38_L, BIT(15), BIT(15));//reg_vsd_usr_vact_video_en

        SC_W2BYTEMSK(0, REG_SC_BK57_10_L, 0x0C03, 0x0C03);//[0]:reg_main_en, [1]:reg_sw_sub_en, [11]:reg_vblank_sub
        SC_W2BYTEMSK(0, REG_SC_BK57_29_L, 0x0000, 0xFFFF);//[4:0]reg_ini_max_bot_6tap,[12:8]reg_ini_max_top_6tap,should be 0 in monaco 3D mode
        // for 3D 6Tap
        Hal_SC_3D_set_top_win_6Tap(pInstance, ENABLE);
        SC_W2BYTEMSK(0, REG_SC_BK57_15_L, pSrcInfo->u16V_SizeAfterPreScaling/2, 0x0FFF);//active_hight  after VSD
        SC_W2BYTEMSK(0, REG_SC_BK57_16_L, pSrcInfo->u16V_SizeAfterPreScaling/2, 0x0FFF);//active_hight  after VSD
        SC_W2BYTEMSK(0, REG_SC_BK57_47_L, 0x8012, 0xBFFF);//reg_hst_f2
        SC_W2BYTEMSK(0, REG_SC_BK57_48_L, 0x0002, 0x3FFF);//reg_hst_f2
        SC_W2BYTEMSK(0, REG_SC_BK57_49_L, 0x0009, 0x3FFF);//reg_hend_f2
        SC_W2BYTEMSK(0, REG_SC_BK57_4A_L, 0x000a, 0x3FFF);//reg_hst_f1
        SC_W2BYTEMSK(0, REG_SC_BK57_4B_L, 0x0011, 0x3FFF);//reg_hend_f1
        SC_W2BYTEMSK(0, REG_SC_BK57_4C_L, 0x0000, 0x0FFF);//reg_vst_f2
        SC_W2BYTEMSK(0, REG_SC_BK57_4D_L, (pSrcInfo->u16V_SizeAfterPreScaling/2 - 1), 0x0FFF);//reg_vend_f2
        SC_W2BYTEMSK(0, REG_SC_BK57_4E_L, 0x0000, 0x0FFF);//reg_vst_f1
        SC_W2BYTEMSK(0, REG_SC_BK57_4F_L, (pSrcInfo->u16V_SizeAfterPreScaling/2 - 1), 0x0FFF);//reg_vend_f1

        SC_W2BYTEMSK(0, REG_SC_BK20_02_L,BIT(3), BIT(3));//reg_op1_frcm_cnnet_spthalf_f2
        SC_W2BYTEMSK(0, REG_SC_BK20_03_L,BIT(2), BIT(2));//reg_force_fe2_en
        ...
        SC_W2BYTEMSK(0, REG_SC_BK32_33_L,BIT(7), BIT(7));//reg_opm_3d_en
        SC_W2BYTEMSK(0, REG_SC_BK32_41_L,BIT(7), BIT(7));
        SC_W2BYTEMSK(0, REG_SC_BK32_18_L, pSrcInfo->stCapWin.height*2, 0x1FFF);
        SC_W2BYTEMSK(0, REG_SC_BK32_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset, 0x1FFF);//OPM offset
        SC_W2BYTEMSK(0, REG_SC_BK32_17_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch, 0x1FFF);//OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK32_56_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset, 0x1FFF);//F1 OPM OFFSET
        SC_W2BYTEMSK(0, REG_SC_BK32_57_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch, 0x1FFF);//F1 OPM FETCH
        ...
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0001, 0x0001);// 3d enable
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0x0080, 0x0080);//OP 3D ENABLE
        SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0003, 0x0013);//hw 3d TYPE, L/R idx
#if (HW_DESIGN_3D_VER >= 3)
        SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, pSrcInfo->stCapWin.width, 0x1FFF);
    ...
    else if(IS_INPUT_SIDE_BY_SIDE_HALF(eWindow) || IS_INPUT_SIDE_BY_SIDE_HALF_INTERLACE(eWindow))
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK12_0E_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNROffset, 0x1FFF);//dnr offset
        SC_W2BYTEMSK(0, REG_SC_BK12_0F_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNRFetch, 0x1FFF);//dnr fetch
        SC_W2BYTEMSK(0, REG_SC_BK12_16_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset + 1) & ~0x1, 0x1FFF);//OPM offset
        SC_W2BYTEMSK(0, REG_SC_BK12_56_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset + 1) & ~0x1, 0x1FFF);//F1 OPM OFFSET

#if(HW_DESIGN_3D_VER == 5) //monaco mode
        SC_W2BYTEMSK(0, REG_SC_BK12_17_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch  + 1) & ~0x1, 0x1FFF);//OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK12_18_L, pSrcInfo->stCapWin.height*2, 0x1FFF);
        SC_W2BYTEMSK(0, REG_SC_BK12_57_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch  + 1) & ~0x1, 0x1FFF);//F1 OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK32_0E_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNROffset, 0x1FFF);//dnr offset
        SC_W2BYTEMSK(0, REG_SC_BK32_0F_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNRFetch, 0x1FFF);//dnr fetch
        SC_W2BYTEMSK(0, REG_SC_BK32_16_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset/2 + 1) & ~0x1, 0x1FFF);//OPM offset
        SC_W2BYTEMSK(0, REG_SC_BK32_18_L, pSrcInfo->stCapWin.height*2, 0x1FFF);
        SC_W2BYTEMSK(0, REG_SC_BK32_56_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset/2 + 1) & ~0x1, 0x1FFF);//F1 OPM OFFSET
        SC_W2BYTEMSK(0, REG_SC_BK32_17_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch  + 1) & ~0x1, 0x1FFF);//OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK32_57_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch  + 1) & ~0x1, 0x1FFF);//F1 OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0103, 0x0113);//hw 3d TYPE

        if((pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PreScalingRatio & BIT(31))>>31)
        {
            SC_W2BYTEMSK(0, REG_SC_BK02_09_L, BIT(12), BIT(12));
            SC_W2BYTEMSK(0, REG_SC_BK02_65_L, pSrcInfo->stCapWin.height*2, 0x1FFF);//reg_vact_video
        }
        SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, pSrcInfo->stCapWin.width/2, 0x1FFF);

        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0001, 0x0001);//IPM 3D ENABLE
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0x0080, 0x0080);//OP 3D ENABLE
        ...
        {
            SC_W2BYTEMSK(0, REG_SC_BK57_10_L, 0x0C00, 0x0C03);//[0]:reg_main_en, [1]:reg_sw_sub_en, [11]:reg_vblank_sub
            SC_W2BYTEMSK(0, REG_SC_BK58_10_L, 0x0000, 0x0001);
        }
        SC_W2BYTEMSK(0, REG_SC_BK57_29_L, 0x0000, 0xFFFF);//[4:0]reg_ini_max_bot_6tap,[12:8]reg_ini_max_top_6tap,should be 0 in monaco 3D mode
        // for 3D 6Tap
        ...
        if(pSrcInfo->bFBL || pSrcInfo->bR_FBL)
        {
            if(MDrv_SC_3D_IsSupportFBL3D())
            {
                SC_W2BYTEMSK(0, REG_SC_BK12_17_L, pSrcInfo->u16H_SizeAfterPreScaling / 2, 0x0FFF);//OPM FETCH
                SC_W2BYTEMSK(0, REG_SC_BK12_57_L, pSrcInfo->u16H_SizeAfterPreScaling / 2, 0x0FFF);//F1 OPM FETCH
                SC_W2BYTEMSK(0, REG_SC_BK20_13_L, pSrcInfo->u16H_SizeAfterPreScaling / 2, 0x1FFF);
                SC_W2BYTEMSK(0, REG_SC_BK20_18_L, BIT(9), BIT(9));//fbl 3d enable
                SC_W2BYTEMSK(0, REG_SC_BK00_34_L, BIT(15), BIT(15));//fbl 3d eco
            }
            else
            {
        ...
        {
            SC_W2BYTEMSK(0, REG_SC_BK12_17_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch  + 1) & ~0x1, 0x0FFF);//OPM FETCH
            SC_W2BYTEMSK(0, REG_SC_BK12_57_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch  + 1) & ~0x1, 0x0FFF);//F1 OPM FETCH
            SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0103, 0x0103);//hw 3d TYPE
            SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0001, 0x0001);//IPM 3D ENABLE
            SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0x0080, 0x0080);//OP 3D ENABLE
            SC_W2BYTEMSK(0, REG_SC_BK12_18_L, 0x00, BIT(15));
            SC_W2BYTEMSK(0, REG_SC_BK12_18_L, pSrcInfo->stCapWin.height*2, 0x1FFF);
            SC_W2BYTEMSK(0, REG_SC_BK12_18_L, BIT(15), BIT(15));
#if (HW_DESIGN_3D_VER >= 3)
            SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, pSrcInfo->stCapWin.width/2, 0x1FFF);
            if((pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32V_PreScalingRatio & BIT(31))>>31)
                SC_W2BYTEMSK(0, REG_SC_BK02_09_L, BIT(12), BIT(12));
    ...
    else if(IS_INPUT_SIDE_BY_SIDE_FULL(eWindow))
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK12_0E_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_WOffset, 0x1FFF);//dnr offset
        SC_W2BYTEMSK(0, REG_SC_BK12_0F_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_WFetch *2, 0x1FFF);//dnr fetch
        SC_W2BYTEMSK(0, REG_SC_BK12_16_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset/2 + 1) & ~0x1, 0x1FFF);//OPM offset
        SC_W2BYTEMSK(0, REG_SC_BK12_17_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch  + 1) & ~0x1, 0x1FFF);//OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK12_56_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset/2 + 1) & ~0x1, 0x1FFF);//F1 OPM OFFSET
        SC_W2BYTEMSK(0, REG_SC_BK12_57_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch  + 1) & ~0x1, 0x1FFF);//F1 OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0103, 0x0103);//hw 3d TYPE
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0001, 0x0001);//IPM 3D ENABLE
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0x0080, 0x0080);//OP 3D ENABLE
        SC_W2BYTEMSK(0, REG_SC_BK12_18_L, 0x00, BIT(15));
        ...
        //SC_W2BYTEMSK(0, REG_SC_BK12_18_L, pSrcInfo->stCapWin.height*2, 0x1FFF);
        //SC_W2BYTEMSK(0, REG_SC_BK12_57_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch  + 1) & ~0x1, 0x0FFF);//F1 OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK32_0E_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_WOffset, 0x1FFF);//dnr offset
        SC_W2BYTEMSK(0, REG_SC_BK32_0F_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_WFetch * 2, 0x1FFF);//dnr fetch
        SC_W2BYTEMSK(0, REG_SC_BK32_16_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset/2 + 1) & ~0x1, 0x1FFF);//OPM offset
        SC_W2BYTEMSK(0, REG_SC_BK32_18_L, pSrcInfo->stCapWin.height*2, 0x1FFF);
        SC_W2BYTEMSK(0, REG_SC_BK32_56_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_ROffset/2 + 1) & ~0x1, 0x1FFF);//F1 OPM OFFSET
        SC_W2BYTEMSK(0, REG_SC_BK32_17_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16FRCM_RFetch  + 1) & ~0x1, 0x1FFF);//OPM FETCH
    ...
    else if(IS_INPUT_CHECK_BOARD(eWindow))
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK12_0E_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNROffset, 0x1FFF);//dnr offset
        SC_W2BYTEMSK(0, REG_SC_BK12_0F_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16DNRFetch, 0x1FFF);//dnr fetch
        SC_W2BYTEMSK(0, REG_SC_BK12_16_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset/2 + 1) & ~0x1, 0x1FFF);//OPM offset
        SC_W2BYTEMSK(0, REG_SC_BK12_17_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch  + 1) & ~0x1, 0x1FFF);//OPM FETCH
        SC_W2BYTEMSK(0, REG_SC_BK12_56_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMOffset/2 + 1) & ~0x1, 0x1FFF);//F1 OPM OFFSET
        SC_W2BYTEMSK(0, REG_SC_BK12_57_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16OPMFetch  + 1) & ~0x1, 0x1FFF);//F1 OPM FETCH
        ...
        {
            SC_W2BYTEMSK(0, REG_SC_BK02_09_L, BIT(12), BIT(12));
            SC_W2BYTEMSK(0, REG_SC_BK02_65_L, (pSrcInfo->stCapWin.height*2), 0x1FFF);
        }
        SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, pSrcInfo->stCapWin.width/2, 0x1FFF);
        SC_W2BYTEMSK(0, REG_SC_BK02_6B_L, 0x0005, 0x0005); //[0]pix_sep_en,[2]LR_Alt_line
        SC_W2BYTEMSK(0, REG_SC_BK02_6C_L, pSrcInfo->stCapWin.width/2, 0x7FF); //[10:0] pix_sep_half
        SC_W2BYTEMSK(0, REG_SC_BK02_6F_L, 0x0109, 0x010F); //resample line phase, [8]enable
        SC_W2BYTEMSK(0, REG_SC_BK02_6D_L, 0x8000, 0xFFFF); //resample phase0 coef0/1
        SC_W2BYTEMSK(0, REG_SC_BK02_6E_L, 0x4000, 0xFFFF); //resample phase1 coef0/1

        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, BIT(0), BIT(0)); //IPM 3D ENABLE
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(7), BIT(7)); //OP 3D ENABLE
        SC_W2BYTEMSK(0, REG_SC_BK12_18_L, pSrcInfo->stCapWin.height*2, 0x1FFF);
        ...
        SC_W2BYTEMSK(0, REG_SC_BK20_02_L,BIT(3), BIT(3));//reg_force_fe2_en
        SC_W2BYTEMSK(0, REG_SC_BK20_03_L,BIT(2), BIT(2));//reg_force_fe2_en
        SC_W2BYTEMSK(0, REG_SC_BK32_30_L,BIT(0), BIT(0));//reg_ipm_3d_en
        SC_W2BYTEMSK(0, REG_SC_BK32_33_L,BIT(7), BIT(7));//reg_opm_3d_en
        SC_W2BYTEMSK(0, REG_SC_BK32_41_L,BIT(7), BIT(7));
        ...
        SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0103, 0x0103);//hw 3d TYPE
        SC_W2BYTEMSK(0, REG_SC_BK02_6B_L, BIT(0)|BIT(2), BIT(0)|BIT(2)); //[0]pix_sep_en,[2]LR_Alt_line
        SC_W2BYTEMSK(0, REG_SC_BK02_6C_L, pSrcInfo->stCapWin.width/2, 0x7FF); //[10:0] pix_sep_half

#if (HW_DESIGN_3D_VER >= 3)
        // New HW design for chip after Agate/Eagle/M12, there is auto mode for width (bk02_6C[10:0])
        // under HDMI DE_by_pass mode, need to enable user mode (bk02_6C[15])
        if (SC_R2BYTEMSK(0, REG_SC_BK01_2F_L, BIT(15)) == BIT(15))
            SC_W2BYTEMSK(0, REG_SC_BK02_6C_L, BIT(15), BIT(15));
        else
            SC_W2BYTEMSK(0, REG_SC_BK02_6C_L, 0x00, BIT(15));
#endif

        // TODO: Need to check how to setup resample
        SC_W2BYTE(0, REG_SC_BK02_6D_L, 0x8000); //resample phase0 coef0/1
        SC_W2BYTE(0, REG_SC_BK02_6E_L, 0x4000); //resample phase1 coef0/1
        SC_W2BYTEMSK(0, REG_SC_BK02_6F_L, 0x0109, 0x010F); //resample line phase, [8]enable
    ...
    else if(IS_INPUT_PIXEL_ALTERNATIVE(eWindow))
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, pSrcInfo->stCapWin.width/2, 0x1FFF);
        SC_W2BYTEMSK(0, REG_SC_BK02_6B_L, 0x0001, 0x0005); //[0]pix_sep_en,[2]LR_Alt_line
        SC_W2BYTEMSK(0, REG_SC_BK02_6C_L, pSrcInfo->stCapWin.width/2, 0x7FF); //[10:0] pix_sep_half
        SC_W2BYTEMSK(0, REG_SC_BK02_6F_L, 0x0105, 0x010F); //resample line phase, [8]enable
        SC_W2BYTEMSK(0, REG_SC_BK02_6D_L, 0x8000, 0xFFFF); //resample phase0 coef0/1
        SC_W2BYTEMSK(0, REG_SC_BK02_6E_L, 0x4000, 0xFFFF); //resample phase1 coef0/1
        ...
        SC_W2BYTEMSK(0, REG_SC_BK02_63_L, 0x0103, 0x0103);//hw 3d TYPE
        SC_W2BYTEMSK(0, REG_SC_BK02_6B_L, BIT(0), BIT(0)|BIT(2)); //[0]pix_sep_en,[2]LR_Alt_line
        SC_W2BYTEMSK(0, REG_SC_BK02_6C_L, pSrcInfo->stCapWin.width/2, 0x7FF); //[10:0] pix_sep_half
...
#if (HW_DESIGN_3D_VER >= 3)
    // New HW design for chip after Agate/Eagle/M12, there is auto mode for width (bk02_7F[12:0])
    // under HDMI DE_by_pass mode, need to enable user mode (bk02_7F[15])
    if (SC_R2BYTEMSK(0, REG_SC_BK01_2F_L, BIT(15)) == BIT(15))
        SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, BIT(15), BIT(15));
    else
        SC_W2BYTEMSK(0, REG_SC_BK02_7F_L, 0x00, BIT(15));
#endif
    ...
    if(IS_OUTPUT_FRAME_ALTERNATIVE())
    {
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## 3D output is FA\n");
        {
#if (HW_DESIGN_3D_VER == 4)
            SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x1FFF); // Display H start
            SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);

            if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Pnl_Misc.FRCInfo.u83D_FI_out == E_XC_3D_OUTPUT_FI_960x1080)
            {
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart
                             +(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd-pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart)/2, 0x1FFF);   // Display H end
                SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_05_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width/2 - 1), 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_07_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height - 1), 0x0FFF);
            }
            else if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Pnl_Misc.FRCInfo.u83D_FI_out == E_XC_3D_OUTPUT_FI_1920x540)
            {
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x1FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart+(pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd-pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart)/2, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_05_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width - 1), 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_07_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height/2 - 1), 0x0FFF);
            }
            else if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Pnl_Misc.FRCInfo.u83D_FI_out == E_XC_3D_OUTPUT_FI_1280x720)
            {
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width - 1), 0x1FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height - 1), 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_05_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16HStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width - 1), 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_07_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height - 1), 0x0FFF);
            }
            else
            ...
            SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
#endif
            SC_W2BYTEMSK(0, REG_SC_BK12_30_L, BIT(8), BIT(8));//Enable free run 3D LR flag
            //HAL_SC_Set_FRCM_FB_Num(pInstance, MAIN_WINDOW, IMAGE_STORE_4_FRAMES, 0);
        }
        XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "## stDBreg.u16VLen =%x\n", pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen);
#if (HW_DESIGN_3D_VER == 4)
        if(IS_INPUT_TOP_BOTTOM(eWindow))
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen/2, 0x0FFF);//Set the maximum request lines for second channel
        }
        else
#endif
        {
            ...
            {
                SC_W2BYTEMSK(0, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);//Set the maximum request lines for second channel
            }
        }
        ...
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, FALSE, BIT(0)); // False: L flag is first, True: R flag is first
        //Set SRAM to linear mode
        if (IsVMirrorMode(eWindow))
        {
            SC_W2BYTE(0, REG_SC_BK23_0B_L, 0x303);
        ...
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(7), BIT(7));//[7]:reg_opm_3d_en
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(6), BIT(6));//[6]:reg_opm_ext_lr_sel
        SC_W2BYTEMSK(0, REG_SC_BK20_18_L, BIT(1), BIT(1));//reg_force_f2
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0004, 0x0004);
#endif
        // after timing change , we need to reset fclk for L/R alignment.
        ...
        if (IsVMirrorMode(eWindow))
        {
            ...
            if((IS_INPUT_FRAME_PACKING(eWindow))&&
                    ((pSrcInfo->stCapWin.height == DOUBLEHD_1080X2P_VSIZE)||
                     (pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_VSIZE)))
            {
                SC_W2BYTEMSK(0, REG_SC_BK0F_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x1FFF); // Display H start
                SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x1FFF);   // Display H end
                SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//Sub window display window
                SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height-1, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x0FFF);//Main window display window
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd*2-1, 0x0FFF);
            }
            else
#endif
            {
                ...
                SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x1FFF);   // Display H end
                SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//Sub window display window
                SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height/2-1, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x0FFF);//Main window display window
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height/2, 0x0FFF);
                pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd = pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart
                        + (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height/2 + pSrcInfo->stDispWin.height/2);
                SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
            }
        ...
        else
        {
            ...
            if((IS_INPUT_FRAME_PACKING(eWindow))&&
                    ((pSrcInfo->stCapWin.height == DOUBLEHD_1080X2P_VSIZE)||
                     (pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_VSIZE)))
            {
                SC_W2BYTEMSK(0, REG_SC_BK0F_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x0FFF);//Sub window display window
                SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd*2-1, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x1FFF); // Display H start
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x1FFF);   // Display H end
                SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//Main window display window
                SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height-1, 0x0FFF);
            }
            else
#endif
            {
                ...
                {
                    // sub window
                    SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, (stdeWin.y + stdeWin.height/2 + u16V_detal), 0x0FFF);
                    SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, (stdeWin.y + stdeWin.height/2 + u16V_detal +pSrcInfo->stDispWin.height/2 -1), 0x0FFF);
                    // main window
                    SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart - u16V_detal), 0x0FFF);
                    SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart - u16V_detal + pSrcInfo->stDispWin.height/2 -1), 0x0FFF);
                }
                else
                {
                    SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height/2), 0x0FFF);
                    pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd = pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height;
                    SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
                    SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//Main window display window
                    SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pSrcInfo->stDispWin.height/2-1, 0x0FFF);
                }
                ...
                SC_W2BYTEMSK(0, REG_SC_BK0F_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x0FFF);//Sub window display window
                SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height/2, 0x0FFF);
                pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd = pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart
                        + (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height/2 + pSrcInfo->stDispWin.height/2);
                SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x1FFF); // Display H start
...
#if(HW_DESIGN_3D_VER == 5) //monaco mode
        Hal_SC_set_fmclk(psXCInstPri, ENABLE);
        SC_W4BYTE(0, REG_SC_BK32_50_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32FRCM_RBase0);
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(7), BIT(7));//[7]:reg_opm_3d_en
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(11), BIT(11));
        SC_W2BYTEMSK(0, REG_SC_BK20_18_L, BIT(1), BIT(1));//reg_force_f2
        SC_W2BYTEMSK(0, REG_SC_BK10_13_L, BIT(10), BIT(10));//pip
        if(IS_INPUT_LINE_ALTERNATIVE(eWindow))
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen/2, 0x0FFF);//Set the maximum request lines for second channel
            SC_W2BYTEMSK(0, REG_SC_BK20_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen/2, 0x0FFF);//Set the maximum request lines for first channel
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);//Set the maximum request lines for second channel
            SC_W2BYTEMSK(0, REG_SC_BK20_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);//Set the maximum request lines for first channel
        }
        SC_W2BYTEMSK(0, REG_SC_BK57_1D_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset&0xFF), 0xFF);
        SC_W2BYTEMSK(0, REG_SC_BK57_1D_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset&0xFF)<<8, 0xFF00);
        SC_W2BYTEMSK(0, REG_SC_BK33_32_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? BIT(4):0x00), BIT(4));
        pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset &= ~0x1;
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0004, 0x0004);
#else
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(11), BIT(11)); //h3d_opm, sub win follow main's read bank
        SC_W4BYTE(0, REG_SC_BK12_50_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32OPMBase0);
        SC_W2BYTEMSK(0, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);//Set the maximum request lines for second channel
        SC_W2BYTEMSK(0, REG_SC_BK20_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);//Set the maximum request lines for first channel
        SC_W2BYTEMSK(0, REG_SC_BK08_32_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? BIT(4):0x00), BIT(4));
#endif
        SC_W2BYTEMSK(0, REG_SC_BK20_1D_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset&0xFF), 0xFF);
        SC_W2BYTEMSK(0, REG_SC_BK20_1D_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset&0xFF)<<8, 0xFF00);
//#endif
    ...
    else if(IS_OUTPUT_LINE_ALTERNATIVE())
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK0F_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x0FFF);//Sub window display window
        SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
        SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);
        SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
        ...
        SC_W2BYTEMSK(0, REG_SC_BK20_18_L, BIT(1), BIT(1));//reg_force_f2
        SC_W2BYTEMSK(0, REG_SC_BK10_13_L, BIT(10), BIT(10));//pip
        SC_W2BYTEMSK(0, REG_SC_BK32_33_L, BIT(11), BIT(11)); //h3d_opm, sub win follow main's read bank
        SC_W2BYTEMSK(0, REG_SC_BK33_32_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? BIT(4):0x00), BIT(4));
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0004, 0x0004);
#else
        SC_W2BYTEMSK(0, REG_SC_BK08_32_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? BIT(4):0x00), BIT(4));
...
#else
        ///for 3D film issue
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, 0x00, BIT(11)); //h3d_opm, sub win follow main's read bank
        SC_W2BYTE(0, REG_SC_BK12_14_L, 0x8000);
        //SC_W2BYTEMSK(0, REG_SC_BK20_10_L, 0x00, BIT(11)); // Sub ahead need to disable when memory allocate is SBS
#endif
        SC_W4BYTE(0, REG_SC_BK12_50_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32OPMBase0);//F1 OPM BASE0 sub
        if(IS_INPUT_LINE_ALTERNATIVE(eWindow))
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen/2, 0x0FFF);//Set the maximum request lines for second channel, added for protect
            SC_W2BYTEMSK(0, REG_SC_BK20_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen/2, 0x0FFF);//Set the maximum request lines for first channel
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);//Set the maximum request lines for second channel, added for protect
            SC_W2BYTEMSK(0, REG_SC_BK20_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);//Set the maximum request lines for first channel
        }
        SC_W2BYTEMSK(0, REG_SC_BK23_53_L, 0x8000, 0x8000);// 3D LR side-by-side to line-to-line enable
#if(HW_DESIGN_3D_VER == 5) //monaco mode
        pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset &= ~0x1;
#endif
        SC_W2BYTEMSK(0, REG_SC_BK20_1D_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset&0xFF), 0xFF);
    ...
    else if(IS_OUTPUT_FRAME_L())
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK10_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height- 1, 0x0FFF);
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0004, 0x0004);
        if(IS_INPUT_FRAME_PACKING(eWindow)&&((pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_VSIZE) || (pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_50_VSIZE)))
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen*2, 0x0FFF);//Set the maximum request lines for second channel, added for protect
        }
        else
        {
            if(IS_INPUT_LINE_ALTERNATIVE(eWindow))
            {
                SC_W2BYTEMSK(0, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen/2, 0x0FFF);
            }
            else
            {
                SC_W2BYTEMSK(0, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);//Set the maximum request lines for second channel, added for protect
    ...
    else if(IS_OUTPUT_FRAME_R())
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK12_33_L, BIT(11), BIT(11)); //h3d_opm, sub win follow main's read bank
        SC_W2BYTEMSK(0, REG_SC_BK32_33_L, BIT(11), BIT(11)); //h3d_opm, sub win follow main's read bank
        SC_W4BYTE(0, REG_SC_BK12_50_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u32OPMBase0);//F1 OPM BASE0
        SC_W2BYTEMSK(0, REG_SC_BK12_30_L, 0x0004, 0x0004);
        if(IS_INPUT_FRAME_PACKING(eWindow)&&(((pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_VSIZE) || (pSrcInfo->stCapWin.height == DOUBLEHD_1080X2I_50_VSIZE))))
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen*2, 0x0FFF);//Set the maximum request lines for second channel, added for protect
            if (IsVMirrorMode(eWindow))
            {
                SC_W2BYTEMSK(0, REG_SC_BK02_64_L, 0x00, 0x30); //Patch
            }
        }
        else
        {
    ...
    else if(IS_OUTPUT_SIDE_BY_SIDE_HALF() || IS_OUTPUT_SIDE_BY_SIDE_HALF_HW())
    {
        if (IsVMirrorMode(eWindow))
        {
            if (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_A & E_XC_INIT_MISC_A_FRC_INSIDE_240HZ)
            {
                SC_W2BYTEMSK(0, REG_SC_BK0F_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x1FFF);
                SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart + pSrcInfo->stDispWin.width/2-1, 0x1FFF);
                SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//sub window display window
                SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart+ pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd)/2, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width/2, 0x0FFF);
                pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd = pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart
                        + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width/2 + pSrcInfo->stDispWin.width/2;
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart+ pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd)/2, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_07_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height/2)-1, 0x0FFF);
            }
            else
            {
                ...
                SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//sub window display window
                SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart + pSrcInfo->stDispWin.width/2, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
            }
        ...
        else
        {
            if (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_A & E_XC_INIT_MISC_A_FRC_INSIDE_240HZ)
            {
                SC_W2BYTEMSK(0, REG_SC_BK0F_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart + pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width/2, 0x0FFF);
                pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd= pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart
                        + (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Width/2 + pSrcInfo->stDispWin.width/2)-1;
                SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart+ pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd)/2, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x1FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart + pSrcInfo->stDispWin.width/2-1, 0x1FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//Main window display window
                SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart+ pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd)/2, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_07_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16VStart+pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height/2)-1, 0x0FFF);
            }
            else
            {
                ...
                {
                    // sub window
                    SC_W2BYTEMSK(0, REG_SC_BK0F_07_L, (stdeWin.x + stdeWin.width/2 + u16H_detal), 0x0FFF);
                    SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, (stdeWin.x + stdeWin.width/2 + u16H_detal +pSrcInfo->stDispWin.width/2-1), 0x0FFF);
                    // main window
                    SC_W2BYTEMSK(0, REG_SC_BK10_08_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart - u16H_detal), 0x1FFF);
                    SC_W2BYTEMSK(0, REG_SC_BK10_09_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart - u16H_detal + pSrcInfo->stDispWin.width/2 -1), 0x1FFF);
                }
                else
                {
                    SC_W2BYTEMSK(0, REG_SC_BK0F_07_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart + pSrcInfo->stDispWin.width/2), 0x0FFF);
                    SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart+pSrcInfo->stDispWin.width-1, 0x0FFF);
                    SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x1FFF);
                    SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart + pSrcInfo->stDispWin.width/2-1, 0x1FFF);
                }
                XC_LOG_TRACE(XC_DBGLEVEL_SETWINDOW, "pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B:%lu\n", pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_Init_Misc.u32MISC_B);
        ...
        if(IS_INPUT_LINE_ALTERNATIVE(eWindow))
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen/2, 0x0FFF);
            SC_W2BYTEMSK(0, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen/2, 0x0FFF);
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK20_16_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);
            SC_W2BYTEMSK(0, REG_SC_BK20_15_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16VLen, 0x0FFF);
        }
        pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16LBOffset &= ~0x1;
        SC_W2BYTEMSK(0, REG_SC_BK33_32_L, (pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.bopm_lcnt_inv? BIT(4):0x00), BIT(4));
#else
    ...
    else if (IS_OUTPUT_FRAME_PACKING())
    {
        if (IsVMirrorMode(eWindow))
        {
            if (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height ==DOUBLEHD_1080X2P_VSIZE)
            {
                ...
                SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x1FFF);   // Display H end
                SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//Sub window display window
                SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + DOUBLEHD_1080X2P_FRAME_VSIZE, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x0FFF);//Main window display window
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + DOUBLEHD_1080X2P_FRAME_VSIZE + DOUBLEHD_1080X2P_GARBAGE_VSIZE, 0x0FFF);
            ...
            else if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height ==DOUBLEHD_720X2P_VSIZE)
            {
                ...
                SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x1FFF);   // Display H end
                SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//Sub window display window
                SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + DOUBLEHD_720X2P_FRAME_VSIZE, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x0FFF);//Main window display window
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + DOUBLEHD_720X2P_FRAME_VSIZE + DOUBLEHD_720X2P_GARBAGE_VSIZE, 0x0FFF);
        ...
        else
        {
            if (pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height ==DOUBLEHD_1080X2P_VSIZE)
            {
                SC_W2BYTEMSK(0, REG_SC_BK0F_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x0FFF);//Sub window display window
                SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + DOUBLEHD_1080X2P_FRAME_VSIZE + DOUBLEHD_1080X2P_GARBAGE_VSIZE, 0x0FFF);
                pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd = pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + DOUBLEHD_1080X2P_VSIZE -1;
                SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x1FFF); // Display H start
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x1FFF);   // Display H end
                SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//Main window display window
                SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + DOUBLEHD_1080X2P_FRAME_VSIZE-1, 0x0FFF);
            }
            else if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.u16Height ==DOUBLEHD_720X2P_VSIZE)
            {
                SC_W2BYTEMSK(0, REG_SC_BK0F_07_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x0FFF);//Sub window display window
                SC_W2BYTEMSK(0, REG_SC_BK0F_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK0F_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + DOUBLEHD_720X2P_FRAME_VSIZE + DOUBLEHD_720X2P_GARBAGE_VSIZE, 0x0FFF);
                pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd = pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + DOUBLEHD_720X2P_VSIZE -1;
                SC_W2BYTEMSK(0, REG_SC_BK0F_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisEnd, 0x0FFF);
                SC_W2BYTEMSK(0, REG_SC_BK10_08_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisStart, 0x1FFF); // Display H start
                SC_W2BYTEMSK(0, REG_SC_BK10_09_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16H_DisEnd, 0x1FFF);   // Display H end
                SC_W2BYTEMSK(0, REG_SC_BK10_0A_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart, 0x0FFF);//Main window display window
                SC_W2BYTEMSK(0, REG_SC_BK10_0B_L, pXCResourcePrivate->stdrvXC_MVideo_Context.stDBreg.u16V_DisStart + DOUBLEHD_720X2P_FRAME_VSIZE-1, 0x0FFF);
            }
    ...
    if(psXCInstPri->u32DeviceID == 0)
    {
                ...
                &&((MS_U8)pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.stPanelInfo.eLPLL_Type <= E_XC_PNL_LPLL_VBY1_8BIT_8LANE))
        {
...

            {
                ...
                MOD_W2BYTEMSK(REG_MOD_BK00_30_L, BIT(14), BIT(14));
            }
            else
            {
                MOD_W2BYTEMSK(REG_MOD_BK00_5A_L, 0, BIT(15));
                MOD_W2BYTEMSK(REG_MOD_BK00_30_L, 0, BIT(14));
            }
            //#endif
        }
    }
    ...
    if((E_XC_3D_AUTODETECT_HW == pXCResourcePrivate->stdrvXC_3D._enAutoDetectMethod) && pXCResourcePrivate->stdrvXC_3D._bCurEnableAutoDetect3D)
    {
        ...
        MDrv_WriteRegBit(REG_CKG_FICLK_F1, ENABLE, BIT(6)); //reg_ckg_ficlk_3d
        //dd
        SC_W2BYTEMSK(0, REG_SC_BK15_01_L, pSrcInfo->stDispWin.width, 0x07FF);//src width
        SC_W2BYTEMSK(0, REG_SC_BK15_02_L, pSrcInfo->stDispWin.height, 0x07FF);//src height
        SC_W2BYTEMSK(0, REG_SC_BK15_05_L, 0x7878, 0x7F7F);//dram request number
        SC_W2BYTEMSK(0, REG_SC_BK15_06_L, (u32HW2DTO3D_DD_Buf/HW_2DTO3D_BYTE_PER_WORD)>>16, 0x07FF);//dd buffer high 9bit
        SC_W2BYTE(0, REG_SC_BK15_07_L, u32HW2DTO3D_DD_Buf/HW_2DTO3D_BYTE_PER_WORD);//dd buffer low 2 bytes
        MDrv_XC_wait_output_vsync(pInstance, 1, 100, eWindow);
    ...
    {
        //dr
        SC_W2BYTEMSK(0, REG_SC_BK15_6C_L, 0x0000, 0x0001);//Enable Depth Render
        SC_W2BYTEMSK(0, REG_SC_BK15_64_L, 0x0000, 0x1FFF);//pixel width
        SC_W2BYTEMSK(0, REG_SC_BK15_60_L, 0x0000, 0x07FF);//MV width
        SC_W2BYTEMSK(0, REG_SC_BK15_62_L, 0x0000, 0x00FF);//horizontal number of 32x32 block
        SC_W2BYTEMSK(0, REG_SC_BK15_61_L, 0x0000, 0x007F);//vertical number of 32x32 block
        SC_W2BYTEMSK(0, REG_SC_BK15_62_L, 0x0F00, 0xFF00);//TOTAL FRAME
        SC_W2BYTEMSK(0, REG_SC_BK15_65_L, 0x0000, 0x00FF);//left depth offset
        SC_W2BYTEMSK(0, REG_SC_BK15_66_L, 0x0000, 0x00FF);//right depth offset
#if (HW_2DTO3D_VER < 2)
        SC_W2BYTEMSK(0, REG_SC_BK15_65_L, 0x0000, 0x3F00);//left depth gain:6bits
        SC_W2BYTEMSK(0, REG_SC_BK15_66_L, 0x0000, 0x3F00);//right depth gain:6bits
#else
        SC_W2BYTEMSK(0, REG_SC_BK15_65_L, 0x0000, 0x7F00);//left depth gain:7bits
        SC_W2BYTEMSK(0, REG_SC_BK15_66_L, 0x0000, 0x7F00);//right depth gain:7bits
#endif
#if (HW_2DTO3D_VER == 0)
        SC_W2BYTEMSK(0, REG_SC_BK15_73_L, 0x8000, 0xBF3F);//dp info software reset, last number,request number
#else
        SC_W2BYTE(0, REG_SC_BK15_73_L, 0x0000);          //last number,request number
        SC_W2BYTEMSK(0, REG_SC_BK15_71_L, BIT(11), BIT(11)); //dp info software reset
#endif
        SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0000, 0xC007);//Active mode, Passive mode Select,1:Line alternative
        SC_W2BYTEMSK(0, REG_SC_BK15_72_L, 0x0000, 0x00FF);  //draw left/right black enable, draw left/right black width
        if((IS_INPUT_MODE_NONE(eWindow) || IS_OUTPUT_MODE_NONE()))
        {
    ...
    if(IS_INPUT_NORMAL_2D_HW(eWindow))
    {
        ...
        if(IS_OUTPUT_SIDE_BY_SIDE_HALF_HW())
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_01_L, pSrcInfo->stDispWin.width/2, 0x07FF);//src width
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_01_L, pSrcInfo->stDispWin.width, 0x07FF);//src width
        ...
        SC_W2BYTEMSK(0, REG_SC_BK15_06_L, (u32HW2DTO3D_DD_Buf/HW_2DTO3D_BYTE_PER_WORD)>>16, 0x07FF);//dd buffer high 9bit
        SC_W2BYTE(0, REG_SC_BK15_07_L, u32HW2DTO3D_DD_Buf/HW_2DTO3D_BYTE_PER_WORD);//dd buffer low 2 bytes
        SC_W2BYTE(0, REG_SC_BK15_74_L, u32HW2DTO3D_DR_Buf/HW_2DTO3D_BYTE_PER_WORD);//dr buffer low 2 bytes
        SC_W2BYTEMSK(0, REG_SC_BK15_75_L, (u32HW2DTO3D_DR_Buf/HW_2DTO3D_BYTE_PER_WORD)>>16, 0x07FF);//dr buffer high 9bit
        MDrv_XC_wait_output_vsync(pInstance, 1, 100, eWindow);
#if (HW_2DTO3D_VER < 2)
        SC_W2BYTEMSK(0, REG_SC_BK15_06_L, BIT(13)|BIT(15), BIT(13)|BIT(15));//src domain select&start to write miu
#else
        ...
        if(IS_OUTPUT_SIDE_BY_SIDE_HALF_HW())
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_64_L, pSrcInfo->stDispWin.width/2, 0x1FFF);//pixel width
            SC_W2BYTEMSK(0, REG_SC_BK15_60_L, ((pSrcInfo->stDispWin.width+3)/4)/2, 0x07FF);//MV width
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_64_L, pSrcInfo->stDispWin.width, 0x1FFF);//pixel width
            SC_W2BYTEMSK(0, REG_SC_BK15_60_L, (pSrcInfo->stDispWin.width+3)/4, 0x07FF);//MV width
        }
        SC_W2BYTEMSK(0, REG_SC_BK15_62_L, (pSrcInfo->stDispWin.width+31)/32, 0x00FF);//horizontal number of 32x32 block
        SC_W2BYTEMSK(0, REG_SC_BK15_61_L, (pSrcInfo->stDispWin.height+31)/32, 0x007F);//vertical number of 32x32 block
        SC_W2BYTEMSK(0, REG_SC_BK15_62_L, 0x0F00, 0xFF00);//TOTAL FRAME
        SC_W2BYTEMSK(0, REG_SC_BK15_65_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Offset, 0x00FF);//left depth offset
        SC_W2BYTEMSK(0, REG_SC_BK15_66_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Offset, 0x00FF);//right depth offset
#if (HW_2DTO3D_VER < 2)
        SC_W2BYTEMSK(0, REG_SC_BK15_65_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Gain<<8, 0x3F00);//left depth gain:6bits
        SC_W2BYTEMSK(0, REG_SC_BK15_66_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Gain<<8, 0x3F00);//right depth gain:6bits
        if(IS_OUTPUT_TOP_BOTTOM_HW() || IS_OUTPUT_SIDE_BY_SIDE_HALF_HW())
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_65_L, 0x0000, BIT(14));//sign bit:bit14
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_65_L, BIT(14), BIT(14));//sign bit:bit14
        }
#else
        SC_W2BYTEMSK(0, REG_SC_BK15_65_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Gain<<8, 0x7F00);//left depth gain:7bits
        SC_W2BYTEMSK(0, REG_SC_BK15_66_L, pXCResourcePrivate->stdrvXC_3D._st3DHw2DTo3DPara.u16Gain<<8, 0x7F00);//right depth gain:7bits
        if(IS_OUTPUT_TOP_BOTTOM_HW() || IS_OUTPUT_SIDE_BY_SIDE_HALF_HW())
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_65_L, 0x0000, BIT(15));//sign bit:bit15
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_65_L, BIT(15), BIT(15));//sign bit:bit15
        }
#endif

        SC_W2BYTEMSK(0, REG_SC_BK15_5D_L, 0x0000, 0x8000); // 2nd offset enable
#if (HW_2DTO3D_VER == 0)
        SC_W2BYTEMSK(0, REG_SC_BK15_73_L, 0x0404, 0xBF3F);//dp info software reset, last number,request number
#else
        MDrv_WriteByteMask(REG_MIU0_BASE+0x2B, BIT(6), BIT(6)); //enable memory write
        SC_W2BYTEMSK(0, REG_SC_BK15_71_L, 0x0, BIT(11));           //dp info software reset
        SC_W2BYTE(0, REG_SC_BK15_73_L, 0x0404);                    //last number,request number
#endif
        if(IS_OUTPUT_SIDE_BY_SIDE_HALF_HW())
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_6C_L, 0x0821, 0x0821);//Enable Depth Render
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_6C_L, 0x0001, 0x0001);//Enable Depth Render
        }
        SC_W2BYTEMSK(0, REG_SC_BK15_67_L, BIT(12),  BIT(12)); // DR force bank enable
        SC_W2BYTEMSK(0, REG_SC_BK15_40_L, BIT(6),  BIT(6));   // DD force bank enable
        SC_W2BYTEMSK(0, REG_SC_BK15_41_L, 0x0000, 0x000F);     // DD force bank0
        SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0000, 0x0F00);     // DR force bank0
#if HW_2DTO3D_PATCH
            ...
            && (!IS_INPUT_NORMAL_2D(eWindow))) //real 3d adjust depth using hw 2d to 3d function
    {
        ...
        SC_W2BYTEMSK(0, REG_SC_BK15_64_L, u16Width, 0x1FFF);//pixel width
        SC_W2BYTEMSK(0, REG_SC_BK15_60_L, (u16Width+3)/4, 0x07FF);//MV width
        SC_W2BYTEMSK(0, REG_SC_BK15_62_L, (u16Width+31)/32, 0x00FF);//horizontal number of 32x32 block
        SC_W2BYTEMSK(0, REG_SC_BK15_61_L, (u16Height+31)/32, 0x007F);//vertical number of 32x32 block
#endif
        if(MDrv_XC_3D_IsHWDepthAdjSupported(pInstance, eInputMode, eOutputMode, eWindow)) //real 3d adjust depth using hw 2d to 3d function
        {
            ...
            else if(pXCResourcePrivate->stdrvXC_3D._u163DHShift > 0x80)
            {
                SC_W2BYTEMSK(0, REG_SC_BK15_5D_L, 0x8000, 0x8000); //2  // 2nd offset enable
                SC_W2BYTE(0, REG_SC_BK15_5E_L, 0x0000); //2  // 2nd offset L
                SC_W2BYTE(0, REG_SC_BK15_5F_L, (pXCResourcePrivate->stdrvXC_3D._u163DHShift-0x80)<<8); //2  // 2nd offset R
            }
            else
            {
                SC_W2BYTEMSK(0, REG_SC_BK15_5D_L, 0x8000, 0x8000); //2  // 2nd offset enable
                SC_W2BYTE(0, REG_SC_BK15_5E_L, (0x80-pXCResourcePrivate->stdrvXC_3D._u163DHShift)<<8); //2  // 2nd offset L
                SC_W2BYTE(0, REG_SC_BK15_5F_L, 0x0000); //2  // 2nd offset R
            }
        }
    }
    ...
    if(IS_OUTPUT_CHECKBOARD_HW())
    {
        ...
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0000, 0x0007);//Passive mode Select,0:Check Board, R first
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0003, 0x0007);//Passive mode Select,3:Check Board, L first
        }
    }
    else if(IS_OUTPUT_LINE_ALTERNATIVE_HW())
    {
        if (IsVMirrorMode(eWindow) || (MDrv_ReadRegBit(REG_MVOP_MIRROR, (BIT(0)|BIT(1))) == (BIT(0)|BIT(1))))
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0001, 0x0007);//Passive mode Select,1:Line Alternative, R first
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0004, 0x0007);//Passive mode Select,4:Line Alternative, L first
        }
    }
    else if(IS_OUTPUT_FRAME_ALTERNATIVE_HW())
    {
        SC_W2BYTEMSK(0, REG_SC_BK15_67_L, BIT(14), BIT(14));//active mode
    ...
    else if(IS_OUTPUT_PIXEL_ALTERNATIVE_HW())
    {
        if (IsVMirrorMode(eWindow) || (MDrv_ReadRegBit(REG_MVOP_MIRROR, (BIT(0)|BIT(1))) == (BIT(0)|BIT(1))))
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0002, 0x0007);//Passive mode Select,2:Pixel Alternative, R first
        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0005, 0x0007);//Passive mode Select,5:Pixel Alternative, L first
        }
    }
    else if(IS_OUTPUT_FRAME_L_HW())
    {
        SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0006, 0x0007);//Passive mode Select,6:L Only
    }
    else if(IS_OUTPUT_FRAME_R_HW())
    {
        SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0007, 0x0007);//Passive mode Select,7:R Only
    }
    else if(IS_OUTPUT_FRAME_ALTERNATIVE()) //real 3d adjust depth using hw 2d to 3d function:frame alternative output
    {
        SC_W2BYTEMSK(0, REG_SC_BK15_67_L, BIT(14), BIT(14)); //enable op2t3d_lr
    }
    ...
    {
        SC_W2BYTEMSK(0, REG_SC_BK15_67_L, 0x0001, 0x0007);//Passive mode Select,1:Line Alternative
    }
                ...
                || (eOutputMode != pXCResourcePrivate->stdrvXC_3D.ePreOutputMode)))
    {
        ...
        SC_W2BYTE(0, REG_SC_BK62_06_L, 0x0000);//left gain/offset
        SC_W2BYTE(0, REG_SC_BK62_0E_L, 0x0000);//right gain/offset
        SC_W2BYTE(0, REG_SC_BK62_07_L, 0x0000); //2  // 2nd offset L
        SC_W2BYTE(0, REG_SC_BK62_0F_L, 0x0000); //2  // 2nd offset R
        SC_W2BYTEMSK(0, REG_SC_BK62_65_L, 0x0000 ,0x01ff);
        SC_W2BYTEMSK(0, REG_SC_BK62_66_L, 0x0000, 0x01ff);
            ...
            && (!IS_INPUT_NORMAL_2D(eWindow))) //real 3d adjust depth using hw 2d to 3d function
    {
        if(MDrv_XC_3D_IsHWDepthAdjSupported(pInstance, eInputMode, eOutputMode, eWindow)) //real 3d adjust depth using hw 2d to 3d function
        {
            ...
            SC_W2BYTEMSK(0, REG_SC_BK63_55_L, u32OutputWidth, 0x1FFF);//pixel width
            SC_W2BYTEMSK(0, REG_SC_BK63_51_L, u32OutputWidth, 0x1FFF);//reg_ln_width
            SC_W2BYTEMSK(0, REG_SC_BK63_52_L, pSrcInfo->stDispWin.height, 0x1FFF);//reg_col_height
            SC_W2BYTEMSK(0, REG_SC_BK63_49_L, (u32OutputWidth+31)/32, 0x00FF);//horizontal number of 32x32 block
            SC_W2BYTEMSK(0, REG_SC_BK62_61_L, u32OutputWidth, 0x3FFF);//reg_ln_width
            SC_W2BYTEMSK(0, REG_SC_BK62_62_L, pSrcInfo->stDispWin.height, 0x1FFF);//reg_col_height
            if((pXCResourcePrivate->stdrvXC_3D._u163DHShift == 0x80) || (pXCResourcePrivate->stdrvXC_3D._u163DHShift == 0x00))
            {
                //original 3d depth
                SC_W2BYTE(0, REG_SC_BK62_07_L, 0x0000); //2  // 2nd offset L
    ...
    if(IS_OUTPUT_LINE_ALTERNATIVE_HW())
    {
        if (IsVMirrorMode(eWindow) || (MDrv_ReadRegBit(REG_MVOP_MIRROR, (BIT(0)|BIT(1))) == (BIT(0)|BIT(1))))
        {
            SC_W2BYTEMSK(0, REG_SC_BK62_06_L, BIT(15), BIT(15)); //sign bit:bit15
            SC_W2BYTEMSK(0, REG_SC_BK62_0E_L, 0x0000, BIT(15));   //sign bit:bit15

        }
        else
        {
            SC_W2BYTEMSK(0, REG_SC_BK62_06_L, 0x0000, BIT(15));  //sign bit:bit15
            SC_W2BYTEMSK(0, REG_SC_BK62_0E_L, BIT(15), BIT(15)); //sign bit:bit15
        }
    }
    else if(IS_OUTPUT_FRAME_ALTERNATIVE_HW())
    {
    ...
    pXCResourcePrivate->stdrvXC_3D.ePreInputMode = eInputMode;
    pXCResourcePrivate->stdrvXC_3D.ePreOutputMode = eOutputMode;
    pXCResourcePrivate->stdrvXC_3D.u16PreDiswidth = pSrcInfo->stDispWin.width;
    pXCResourcePrivate->stdrvXC_3D.u16PreDisheight = pSrcInfo->stDispWin.height;
    pXCResourcePrivate->stdrvXC_3D.u16PreCapwidth = pSrcInfo->stCapWin.width;
    pXCResourcePrivate->stdrvXC_3D.u16PreCapheight = pSrcInfo->stCapWin.height;
    pSrcInfo->u16BK23_0B  = SC_R2BYTE(0, REG_SC_BK23_0B_L);
...
MS_BOOL MDrv_XC_EnableT3D(void *pInstance, MS_BOOL bEnable)
{
    ...
    {
        SC_W2BYTEMSK(0, REG_SC_BK63_61_L, bEnable, BIT(0));
        SC_W2BYTEMSK(0, REG_SC_BK62_08_L, bEnable<<4, BIT(4));
        Hal_SC_set_T3D_setting(pInstance, bEnable);
    }
#endif
    return TRUE;
...
MS_BOOL gbIsKR3D = FALSE;
void MDrv_XC_Set_3D_KR3DMode(void *pInstance, MS_BOOL bOnOff)
{
    gbIsKR3D = bOnOff;
}

void MApi_XC_Set_3D_KR3DMode_U2(void *pInstance, MS_BOOL bOnOff)
{
    MDrv_XC_Set_3D_KR3DMode(pInstance, bOnOff);
}

void MApi_XC_Set_3D_KR3DMode(MS_BOOL bOnOff)
{
    if (pu32XCInst == NULL)
    {
        printf("[%s,%5d]No instance existed, please get an instance by calling MApi_XC_Init() first\n",__FUNCTION__,__LINE__);
    ...
    stXC_SET_3D_KR3DMODE XCArgs;
    XCArgs.bIsKR3D = bOnOff;

    if(UtopiaIoctl(pu32XCInst, E_XC_CMD_SET_3D_KR3DMODE, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return;
    }
...
MS_BOOL MDrv_XC_Get_3D_KR3DMode(void *pInstance)
{
    return gbIsKR3D;
}

MS_BOOL MApi_XC_Get_3D_KR3DMode_U2(void *pInstance)
{
    MS_BOOL bRet = FALSE;
    bRet = MDrv_XC_Get_3D_KR3DMode(pInstance);
    return bRet;
}

MS_BOOL MApi_XC_Get_3D_KR3DMode(void)
{
    if (pu32XCInst == NULL)
    {
        printf("[%s,%5d]No instance existed, please get an instance by calling MApi_XC_Init() first\n",__FUNCTION__,__LINE__);
    ...
    stXC_SET_3D_KR3DMODE XCArgs = {0};

    if(UtopiaIoctl(pu32XCInst, E_XC_CMD_GET_3D_KR3DMODE, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return FALSE;
    }
