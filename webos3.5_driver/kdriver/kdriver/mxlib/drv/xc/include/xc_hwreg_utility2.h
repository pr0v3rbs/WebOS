...
#define PCMonitor_MUTEX
#define PCMonitor_MUTEX_DBG 0
...
#define MAX_XC_DEVICE_NUM_SC2 2 //for muji coverity issue, pevent out-of-bounds access
...
#else //SC2_ENABLE
#define _dbg_semaphore E_XC_POOL_ID_INTERNAL_VARIABLE
...
#else //SC2_ENABLE
    #define _XC_SEMAPHORE_ENTRY(POOL_ID)                                                              \
            if (POOL_ID == _dbg_semaphore) \
                printf("1 [%s,%5d]==========================Prepare to get semaphore, ID = %d\n",__func__,__LINE__,POOL_ID);      \
            if(MDrv_XC_Get_Semaphore(POOL_ID) != UTOPIA_STATUS_SUCCESS)                               \
            {                                                                                         \
                printf("[%s,%5d] Get XC Semaphore failed\n",__FUNCTION__,__LINE__);                   \
            }                                                                                          \
            if (POOL_ID == _dbg_semaphore) \
...
#else //SC2_ENABLE
    #define _XC_SEMAPHORE_RETURN(POOL_ID)                                                             \
            if (POOL_ID == _dbg_semaphore) \
                printf("0 [%s,%5d]==========================prepare to release semaphore, ID = %d\n",__func__,__LINE__,POOL_ID);  \
            MDrv_XC_Release_Semaphore(POOL_ID);                                                             \
            if (POOL_ID == _dbg_semaphore) \
                printf("[%s,%5d]Semaphore Returned\n\n",__func__,__LINE__);
...
#define _XC_RETURN()    _XC_RETURN_MUTEX(_XC_Mutex)
        ...
        #define _XC_ENTRY_MUTEX(_mutex_)                                                                  \
                _XC_SEMAPHORE_ENTRY(E_XC_POOL_ID_INTERNAL_REGISTER);                                               \
                printf("1,==========================Prepare to get mutex\n");                             \
                printf("[%s][%s][%06d]\n",__FILE__,__FUNCTION__,__LINE__);    \
                if(!MsOS_ObtainMutex(_mutex_, _XC_MUTEX_TIME_OUT))                     \
                {                                                                        \
        ...
        #define _XC_RETURN_MUTEX(_mutex_)                                                                \
                ...
                printf("[%s][%s][%06d] \n",__FILE__,__FUNCTION__,__LINE__);    \
                MsOS_ReleaseMutex(_mutex_);                                                              \
                _XC_SEMAPHORE_RETURN(E_XC_POOL_ID_INTERNAL_REGISTER);
                                    //return _ret;
...
//PCmonitor mutex
#define _PCMonitor_MUTEX_TIME_OUT           MSOS_WAIT_FOREVER
#if SC2_ENABLE
#define _PCMonitor_ENTRY(pInstance)     _PCmonitor_ENTRY_MUTEX(pInstance,_PCMonitor_Mutex)
#define _PCMonitor_RETURN(pInstance)    _PCmonitor_RETURN_MUTEX(pInstance,_PCMonitor_Mutex)

#else //SC2_ENABLE


#define _PCMonitor_ENTRY()     _PCmonitor_ENTRY_MUTEX(_PCMonitor_Mutex)
#define _PCMonitor_RETURN()    _PCmonitor_RETURN_MUTEX(_PCMonitor_Mutex)

#endif //#ifdef SC2_ENABLE



#ifdef PCMonitor_MUTEX
         #if(PCMonitor_MUTEX_DBG)
                #if SC2_ENABLE
                       #define _PCmonitor_ENTRY_MUTEX(pInstance,_mutex_)                                                                  \
                        _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_REG);                                               \
                        printf("1,==========================Prepare to get mutex\n");                             \
                        printf("[%s][%s][%06d]\n",__FILE__,__FUNCTION__,__LINE__);    \
                        if(!MsOS_ObtainMutex(_mutex_, _PCMonitor_MUTEX_TIME_OUT))                     \
                        {                                                                        \
                            printf("==========================\n");                              \
                            printf("[%s][%s][%06d] Mutex taking timeout\n",__FILE__,__FUNCTION__,__LINE__);    \
                        }
                #else //SC2_ENABLE
                        #define _PCmonitor_ENTRY_MUTEX(_mutex_)                                                                  \
                        _XC_SEMAPHORE_ENTRY(E_XC_POOL_ID_INTERNAL_REGISTER);                                               \
                        printf("1,==========================Prepare to get mutex\n");                             \
                        printf("[%s][%s][%06d]\n",__FILE__,__FUNCTION__,__LINE__);    \
                        if(!MsOS_ObtainMutex(_mutex_, _PCMonitor_MUTEX_TIME_OUT))                     \
                ...
                #if SC2_ENABLE
                         #define _PCmonitor_RETURN_MUTEX(pInstance,_mutex_)                                                                \
                         printf("0,==========================prepare to release mutex\n");                        \
                         printf("[%s][%s][%06d] \n",__FILE__,__FUNCTION__,__LINE__);    \
                         MsOS_ReleaseMutex(_mutex_);                                                              \
                        _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_REG);
                ...
                #else //SC2_ENABLE
                         #define _PCmonitor_RETURN_MUTEX(_mutex_)                                                                \
                         printf("0,==========================prepare to release mutex\n");                        \
                         printf("[%s][%s][%06d] \n",__FILE__,__FUNCTION__,__LINE__);    \
                         MsOS_ReleaseMutex(_mutex_);                                                              \
                         _XC_SEMAPHORE_RETURN(E_XC_POOL_ID_INTERNAL_REGISTER);
           ...
           #endif
#else // #if not def PCMonitor_MUTEX
    #define _PCmonitor_ENTRY_MUTEX(_mutex_)     while(0)
    #define _PCmonitor_RETURN_MUTEX(_mutex_)    while(0)
#endif // #ifdef PCMonitor_MUTEX
...
INTERFACE MS_S32            _PCMonitor_Mutex = -1;
...
INTERFACE MS_S32            _PCMonitor_Mutex ;
#ifdef MSOS_TYPE_LINUX_KERNEL
INTERFACE wait_queue_head_t _XC_EventQueue;
...
INTERFACE MS_U32 _XC_Device_Offset[MAX_XC_DEVICE_NUM_SC2];
