...
void MApi_XC_ADC_SetInternalVoltage(void *pInstance, ADC_Internal_Voltage InternalVoltage)
{
    ...
    Hal_ADC_dtop_internaldc_setting(pInstance, InternalVoltage);
    _XC_RETURN();
    MsOS_DelayTask(35);  //** must delay n Vsync
...
{
	if( (enADC_SourceType != ADC_INPUTSOURCE_ONLY_MVOP ) && (enADC_SourceType != ADC_INPUTSOURCE_ONLY_DVI ) )
	{
...
#else
    _XC_ENTRY();
    if (MDrv_SC_GetSkipWaitVsync(MAIN_WINDOW) == FALSE)
    {
    MDrv_XC_wait_input_vsync(pInstance, 1, 50, MAIN_WINDOW);
    }
    Hal_ADC_set_phaseEx(pInstance, u16Value);
...
{
    ADC_INPUTSOURCE_TYPE adc_source_index[MAX_ADCSOUCE_NUM] = {ADC_INPUTSOURCE_UNKNOW,ADC_INPUTSOURCE_UNKNOW, ADC_INPUTSOURCE_UNKNOW};
    ...
    TRACE_ADC_INPUT(
        ...
        {
            printf("(src %d)enInputSource(%s), u8InputSourceCount(%d) source[0x%x]\n",
                    i,
                    ...
                    "Others Input",
                    u8InputSourceCount, enInputSource[i]);
        }
    );
    ...
    for(i=0;i <u8InputSourceCount; i++)
    {
		MS_BOOL bNewAdcSource = TRUE;
		ADC_INPUTSOURCE_TYPE enCurAdcIdx = MDrv_XC_ADC_ConvertSrcToADCSrc(pInstance, enInputSource[i]);
		
		for(j = 0; j < i; j++)
		{
			if(adc_source_index[j] == enCurAdcIdx)
			{
				bNewAdcSource = FALSE;
                //printf("[%s][%d] bNewAdcSource[%d] enCurAdcIdx[0x%x]\n", __func__, __LINE__, bNewAdcSource, enCurAdcIdx);
				break;
			}
		}
		if(bNewAdcSource)
        {
    		adc_source_index[u8AdcSourceCount] = enCurAdcIdx;
            
    		if(IsSrcTypeYPbPr(enInputSource[u8AdcSourceCount]))
            {
                bIsYPbPrSource |= TRUE;
            }
            u8AdcSourceCount++;
        ...
        if(u8AdcSourceCount > MAX_ADCSOUCE_NUM)
		{
			printf("[ASSERT]MDrv_XC_ADC_SetInputSource: u8InputSourceCount(%u) shouldn't be bigger than MAX_ADCSOUCE_NUM(%u)!\n",u8InputSourceCount,MAX_ADCSOUCE_NUM);
            MS_U8 k=0;
            for(k=0; k<u8AdcSourceCount; k++)
            {
                printf("(src %d)adc_source_index(%s), u8AdcSourceCount(%d) source[0x%x]\n",
                        k,
                        enInputSource[k] == INPUT_SOURCE_VGA?"VGA" :
                        enInputSource[k] == INPUT_SOURCE_TV? "TV"  :
                        enInputSource[k] == INPUT_SOURCE_YPBPR? "YPBPR"  :
                        enInputSource[k] == INPUT_SOURCE_YPBPR2? "YPBPR2"  :
                        enInputSource[k] == INPUT_SOURCE_YPBPR3? "YPBPR3"  :
                        "Others Input",
                        u8AdcSourceCount, enInputSource[k]);
            }
            MS_ASSERT(0);
			return;
		}
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_MUX, "enInputSource0[%d] enInputSource1[%d] enInputSource2[%d] enInputSource3[%d]\n", enInputSource[0], enInputSource[1], enInputSource[2], enInputSource[3]);
    ...
    if(isMultipleSource)
    {
        ...
        {
            MDrv_XC_ADC_poweron_source(pInstance, (ADC_INPUTSOURCE_TYPE)(adc_source_index[0]|adc_source_index[1]|adc_source_index[2]));
        }

        // set mux for main
        if(adc_source_index[0] != ADC_INPUTSOURCE_UNKNOW)
        {
            MApi_XC_Mux_GetPort(pInstance, enInputSource[0] , enPorts , &u8Port_count );
            MDrv_XC_ADC_Set_Source(pInstance, (ADC_INPUTSOURCE_TYPE)(adc_source_index[0] | adc_source_index[1]|adc_source_index[2]), enPorts, u8Port_count);
        }
    }
    else
    {
...
MS_BOOL MDrv_XC_ADC_SetCalibrationRefSource(void *pInstance,MS_U16 type)
{
    return Hal_ADC_SetCalibrationRefSource(pInstance,type);
}

void MDrv_XC_ADC_SelectRGBYDataToScaler(MS_U8 u8InputR, MS_U8 u8InputG, MS_U8 u8InputB, MS_U8 u8InputY)
{
	Hal_ADC_SelectRGBYDataToScaler(u8InputR, u8InputG, u8InputB, u8InputY);
}
