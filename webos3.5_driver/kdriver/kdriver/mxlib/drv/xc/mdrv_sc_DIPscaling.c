...
void MDrv_XC_DIP_Init(void* pInstance,SCALER_DIP_WIN eWindow)
{
    ...
    if (_XC_Mutex == -1)
    {
        msg_video(printf("[MAPI DWIN][%06d] Should call XC init first.\n", __LINE__));
        return;
    }
...
void MDrv_XC_DWIN_SetInputSource(void* pInstance,INPUT_SOURCE_TYPE_t enInputSourceType, SCALER_DIP_WIN eWindow)
{
    ...
    else if(IsSrcTypeHDMI(enInputSourceType))
    {
        ...
        u8SyncSel = (AUTO_DETECT << 1) | CSYNC;
#else
        if( MDrv_XC_GetHdmiSyncMode(pInstance,MAIN_WINDOW) == HDMI_SYNC_DE)
        {
            u8InputSrcSel = IP_HDMI;
            u8VideoSel = IP_HDMI;
            u8SyncSel = (AUTO_DETECT << 1) | CSYNC;
...
void MDrv_XC_DWIN_set_capture_window(void* pInstance,SCALER_DIP_WIN eWindow)
{
    ...
    if(g_DIPSrcInfo[eWindow].bInterlace)
    {
        ...
        stDIP_DBreg.u16V_CapStart = (g_DIPSrcInfo[eWindow].stCapWin.y)>>1;
        stDIP_DBreg.u16H_CapStart = g_DIPSrcInfo[eWindow].stCapWin.x;
        stDIP_DBreg.u16V_CapSize  = (g_DIPSrcInfo[eWindow].stCapWin.height)>>1;
        stDIP_DBreg.u16H_CapSize  = g_DIPSrcInfo[eWindow].stCapWin.width;
    }
    else
    {
        stDIP_DBreg.u16V_CapStart = g_DIPSrcInfo[eWindow].stCapWin.y;
        stDIP_DBreg.u16H_CapStart = g_DIPSrcInfo[eWindow].stCapWin.x;
        stDIP_DBreg.u16V_CapSize  = g_DIPSrcInfo[eWindow].stCapWin.height;
        stDIP_DBreg.u16H_CapSize  = g_DIPSrcInfo[eWindow].stCapWin.width;
    }
    XC_LOG_TRACE(XC_DBGLEVEL_DWIN_SETWINDOW,"Set DIP ClipWin x:%u, y:%u, w:%u, h:%u\n", stDIP_DBreg.u16H_CapStart, stDIP_DBreg.u16H_CapSize, stDIP_DBreg.u16V_CapStart, stDIP_DBreg.u16V_CapSize);
}
...
void MDrv_XC_DWIN_set_prescaling_ratio(void* pInstance,INPUT_SOURCE_TYPE_t enInputSourceType, XC_DIP_InternalStatus *pSrcInfo, SCALER_DIP_WIN eWindow)
{
    ...
    stDIP_DBreg.u32V_PreScalingRatio = u32TmpPre;
    ...
    stDIP_DBreg.u32H_PreScalingRatio = u32TmpPre;
...
void MDrv_XC_DWIN_set_fetch_number_limit(void* pInstance,XC_DIP_InternalStatus *pSrcInfo, SCALER_DIP_WIN eWindow)
{
    ...
    stDIP_DBreg.u16DNRFetch  = pSrcInfo->u16H_SizeAfterPreScaling;
	stDIP_DBreg.u16DNROffset = (stDIP_DBreg.u16DNRFetch + OFFSET_PIXEL_ALIGNMENT - 1) & ~(OFFSET_PIXEL_ALIGNMENT-1);  //TODO: T3 should be 32 alignment;

	XC_LOG_TRACE(XC_DBGLEVEL_DWIN_SETWINDOW,"u16DNRFetch=%u\n",stDIP_DBreg.u16DNRFetch);
    XC_LOG_TRACE(XC_DBGLEVEL_DWIN_SETWINDOW,"u16DNROffset=%u\n",stDIP_DBreg.u16DNROffset);
...
void MDrv_XC_DWIN_Scaler_Field_Detect(void* pInstance,INPUT_SOURCE_TYPE_t enInputSrcType, EN_XC_DWIN_SCAN_TYPE enScanType, SCALER_DIP_WIN eWindow)
{
    ...
    else if (IsSrcTypeHDMI(enInputSrcType))
    {
        ...
        if( (MDrv_XC_GetHdmiSyncMode(pInstance,MAIN_WINDOW) == HDMI_SYNC_HV)
...
void MDrv_XC_DIP_SetWindow(void* pInstance,XC_SETWIN_INFO *pstXC_SetWin_Info, MS_U32 u32InitDataLen, SCALER_DIP_WIN eWindow)
{
    ...
    if (g_DIPSrcInfo[eWindow].bHCusScaling)
    {
            if (g_DIPSrcInfo[eWindow].u16HCusScalingSrc == 0 || g_DIPSrcInfo[eWindow].u16HCusScalingDst == 0  )
            {
                //printf(XC_DBGLEVEL_DWIN_SETWINDOW,"u16HCusScalingSrc or u16HCusScalingDst can not be 0 \n");

                return;
            }
    }

    if (g_DIPSrcInfo[eWindow].bVCusScaling)
    {
            if (g_DIPSrcInfo[eWindow].u16VCusScalingSrc == 0 || g_DIPSrcInfo[eWindow].u16VCusScalingDst == 0  )
            {
                //printf(XC_DBGLEVEL_DWIN_SETWINDOW,"u16VCusScalingSrc or u16VCusScalingDst can not be 0 \n");
    ...
    MDrv_XC_DWIN_sw_db(pInstance,&stDIP_DBreg, eWindow );
...
void MDrv_XC_DIP_CaptureOneFrame(void* pInstance,SCALER_DIP_WIN eWindow)
{
    ...
    if( (bInit == 0) && (eWindow == DWIN1_WINDOW) )
    {
    	bInit = 1;
    	u16Count = 0;
    	HAL_XC_DIP_ClearIntr(pInstance,0xFF,eWindow);
	    HAL_XC_DIP_CpatureOneFrame2(pInstance, eWindow);
	    do{
	        regval = HAL_XC_DIP_GetIntrStatus(pInstance, eWindow);
	        if(regval > 0x0)
...

{
	...
	else
	{
		if( (fmt == XC_DWIN_DATA_FMT_YUV420) || (fmt == XC_DWIN_DATA_FMT_YUV420_H265)|| (fmt == XC_DWIN_DATA_FMT_YUV420_H265_10BITS)|| (fmt == XC_DWIN_DATA_FMT_YUV420_PLANER)|| (fmt == XC_DWIN_DATA_FMT_YUV420_SEMI_PLANER))
			u16LineOft = (pstDIPWinProperty->u16Pitch*2)/3;
		else
			u16LineOft = pstDIPWinProperty->u16Pitch/u16Bpp;

		if(u16LineOft < u16Width)
		{
			printf("[%s:%d] Pitch is not correct\n",__FUNCTION__,__LINE__);
        	return;
		}
	}
	...
	{
	    u16NewWidth = ALIGN_CHECK(u16Width,HAL_XC_DIP_GetBusSize(pInstance, eWindow)/u16Bpp);
		u16NewLineOft = ALIGN_CHECK(u16LineOft,HAL_XC_DIP_GetBusSize(pInstance, eWindow)/u16Bpp);
	}
    ...
    if( (fmt == XC_DWIN_DATA_FMT_YUV420) || (fmt == XC_DWIN_DATA_FMT_YUV420_H265) || (fmt == XC_DWIN_DATA_FMT_YUV420_H265_10BITS) || (fmt == XC_DWIN_DATA_FMT_YUV420_PLANER) || (fmt == XC_DWIN_DATA_FMT_YUV420_SEMI_PLANER))
    {
        if( (fmt == XC_DWIN_DATA_FMT_YUV420) || (fmt == XC_DWIN_DATA_FMT_YUV420_H265) || (fmt == XC_DWIN_DATA_FMT_YUV420_H265_10BITS))
        {
            ...
            u64CLen = u16NewLineOft*ALIGN_CHECK(u16NewHeight,DIP_HVD_TILE_BLOCK_V*2)/2;
        }
        else
        {
            u64YLen = u16NewLineOft*u16NewHeight;
            u64CLen = u16NewLineOft*u16NewHeight/2;
        }
    ...
    {
        if(eWindow == DWIN0_WINDOW)
        {
            //LGE M2 project, APVR uses DWIN0_WINDOW.
            //to make a alignment buffer address, it need to adj the u16NewHeight.
            //ex: SECAM width is 570 alignment => 576
            MS_U16 u16LayOutBufHeight = 0;
            u16LayOutBufHeight = ALIGN_CHECK(u16NewHeight,DIP_BYTE_PER_WORD); //make a alignment buf address.
            u64Offset = u16NewLineOft*u16LayOutBufHeight;
        }
        else
        {
            u64Offset = u16NewLineOft*u16NewHeight;
        }
    ...
    if( u64TotalLen > (u64BufEnd - u64BufStart))
    {
        ...
        printf("[%s:%d] Buffer size is not correct,need = %lx\n",__FUNCTION__,__LINE__,u64TotalLen);
    }
    ...
    //set win property
    HAL_XC_DIP_SetWinProperty(pInstance, u8BufCnt,u16NewWidth,u16NewLineOft,u16NewHeight,u64Offset,enSource,bPIP,eWindow);
    if( (fmt == XC_DWIN_DATA_FMT_YUV420) || (fmt == XC_DWIN_DATA_FMT_YUV420_H265) || (fmt == XC_DWIN_DATA_FMT_YUV420_H265_10BITS) || (fmt == XC_DWIN_DATA_FMT_YUV420_PLANER) || (fmt == XC_DWIN_DATA_FMT_YUV420_SEMI_PLANER) )
    {
    ...
    g_DIPSrcInfo[DIP_WINDOW].enDIPSourceType = enSource;
...
void MDrv_XC_DIP_SetDIPRProperty(void* pInstance,ST_XC_DIPR_PROPERTY *pstDIPRProperty, SCALER_DIP_WIN eWindow)
{
	...
	else
	{
	    ...
	    if( (fmt == XC_DWIN_DATA_FMT_YUV420) || (fmt == XC_DWIN_DATA_FMT_YUV420_H265) || (fmt == XC_DWIN_DATA_FMT_YUV420_H265_10BITS))
			//u16LineOft = pstDIPRProperty->u16Pitch/1.5;
			u16LineOft = pstDIPRProperty->u16Pitch*2/3;
		else
			u16LineOft = pstDIPRProperty->u16Pitch/u16Bpp;
	...
	HAL_XC_DIP_SetMiuSel(pInstance, u8MIUSel,MAX_DIP_WINDOW);
