...
void MDrv_XC_reset_ip(void *pInstance, SCALER_WIN eWindow)
{
    ...
    {
        Hal_SC_ip_software_reset(pInstance,REST_IP_OFFLINE, eWindow);
    }
    else
    {
...
void MDrv_XC_ip_get_sync_status(void *pInstance, XC_IP_SYNC_STATUS *sXC_Sync_Status, E_MUX_INPUTPORT enInputPort , SCALER_WIN eWindow)
{
    ...
    sXC_Sync_Status->u16Htotal =  MDrv_SC_ip_get_horizontalTotal(pInstance, eWindow);
    ...
    if ( IsDVIPort(enInputPort) ) // DVI
    {
        ...
        if ((sXC_Sync_Status->u8SyncStatus & XC_MD_SYNC_LOSS) != XC_MD_SYNC_LOSS)
        {
            ...
            {
                msg_sc_ip(printf("[%s][%d]HDMI 4K2K 2P mode 444/422\n",__FUNCTION__, __LINE__));
                if(eWindow == MAIN_WINDOW)
                MDrv_WriteByteMask(REG_IPMUX_01_L, SC_MAINWIN_IPMUX_HDMI_DVI << 4, BMASK(7:4));
                    //W2BYTEMSK(0x102e02,0x0010,BMASK(7:4)); // ipmux sel=0xb
                
                else if(eWindow == SUB_WINDOW)
                    MDrv_WriteByteMask(REG_IPMUX_01_L, SC_MAINWIN_IPMUX_HDMI_DVI, BMASK(3:0));
                //W2BYTEMSK(0x102e02,0x0001,BMASK(3:0)); // ipmux sel=0xb

            }
            else
            {
    #if 1//def SUPPORT_HDMI20_420
                msg_sc_ip(printf( "[%s][%d]HDMI 4K2K 2P mode 420\n",__FUNCTION__, __LINE__));
                        ...
                        &&(sXC_Sync_Status->u16Hperiod>= 0x50))
                {
                    ...
                    {
                        msg_sc_ip(printf( "[%s][%d]@@#@@4K2K@60 420\n",__FUNCTION__, __LINE__));
                        // clkgen2
                        ...
                        if(eWindow == MAIN_WINDOW)
                        {
                            ...
                            MDrv_WriteByteMask(REG_IPMUX_01_L, SC_MAINWIN_IPMUX_DC2 << 4, BMASK(7:4));
                        }
                        else if(eWindow == SUB_WINDOW)
                        {
                            //W2BYTEMSK(0x102e02,0x000b,BMASK(3:0)); // ipmux sel=0xb
                            MDrv_WriteByteMask(REG_IPMUX_01_L, SC_MAINWIN_IPMUX_DC2, BMASK(3:0));
                        }
                        ...
                        u16HDE_temp_size = _MDrv_HDMI_GetHDE();
                ...
                {
                    msg_sc_ip(printf( "[%s][%d] not 4K2K case\n",__FUNCTION__, __LINE__));
                    if(eWindow == MAIN_WINDOW)
                    {
                        MDrv_WriteByteMask(REG_IPMUX_01_L, SC_MAINWIN_IPMUX_HDMI_DVI << 4, BMASK(7:4));
                    }
...
MS_BOOL MDrv_SC_Get_DE_Bypass_Mode(void *pInstance, SCALER_WIN eWindow)
{
    ...
    {
        return ((SC_R2BYTEMSK( psXCInstPri->u32DeviceID,REG_SC_BK13_2F_L,  BIT(15) ))? TRUE : FALSE);
    }
    else
    {
...
void MApi_XC_SetCaptureWindowVstart_U2(void* pInstance, MS_U16 u16Vstart , SCALER_WIN eWindow)
{
    ...
    _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
    _XC_ENTRY(pInstance);
    MDrv_XC_SetCaptureWindowVstart(pInstance, u16Vstart, eWindow);
    _XC_RETURN(pInstance);
    _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
...
void MApi_XC_SetCaptureWindowHstart_U2(void* pInstance, MS_U16 u16Hstart , SCALER_WIN eWindow)
{
    ...
    _XC_SEMAPHORE_ENTRY(pInstance,E_XC_ID_VAR);
    _XC_ENTRY(pInstance);
    MDrv_XC_SetCaptureWindowHstart(pInstance, u16Hstart, eWindow);
    _XC_RETURN(pInstance);
    _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
...
E_APIXC_ReturnValue MDrv_SC_ForceInterlaceInPMode(void *pInstance, MS_BOOL bEnable, SCALER_WIN eWindow)
{
    ...
    if (MDrv_XC_GetForceiSupportType(pInstance) == ForceI_SW)
    {
        if (bEnable)
        {
            ...
            {
                SC_W2BYTEMSK(psXCInstPri->u32DeviceID,REG_SC_BK13_21_L, BIT(1) | BIT(0), BIT(1) | BIT(0) | BIT(8));
            }
            else
            {
...
void MDrv_XC_FRC_Rgb888_2_Abgr8888_Neon(MS_U32 u32DstAddr, MS_U32 u32SrcAddr, MS_U32 u32PixelNum)
{
    __asm__ __volatile__(
        " LDR             R0, %0 \t\n"
        " LDR             R1, %1 \t\n"
...
static void MDrv_XC_Access_BufferData(void *pInstance, E_XC_OUTPUTDATA_TYPE eBufType, MS_WINDOW_TYPE *pMemRect, void *pDataBuf, MS_WINDOW_TYPE *pSrcRect, SCALER_WIN eWindow, MS_BOOL bIsReadMode)
{
    ...
    memset(&FBInfo,0,sizeof(SC_FRAMEBUF_INFO_t));
    SC_MIUSEL_t FBMiuSelInfo;
    memset(&FBMiuSelInfo,0,sizeof(SC_MIUSEL_t));
    MS_U32 u32Main_FB_Start_Addr = pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.u32Main_FB_Start_Addr;
    MS_U32 u32Sub_FB_Start_Addr = pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.u32Sub_FB_Start_Addr;


    if(eBufType<=E_XC_OUTPUTDATA_4228BITS)
    {
        u32Main_FB_Start_Addr = pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.u32Main_FB_Start_Addr;
        u32Sub_FB_Start_Addr = pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.u32Sub_FB_Start_Addr;
        Hal_SC_get_framebuf_Info(pInstance, &FBInfo, eWindow);
    }
    else // FRCM
    {
        u32Main_FB_Start_Addr = pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.u32Main_FRCM_FB_Start_Addr;
        u32Sub_FB_Start_Addr = pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.u32Sub_FRCM_FB_Start_Addr;
        Hal_SC_get_Frc_framebuf_Info(pInstance, &FBInfo, eWindow);
    }
    ...
    {
#if FRC_INSIDE
        MS_U32 *pu32Dstaddr=NULL;
        MS_U32 u32PixelNum=0;
        MS_U8 *pu8Srcaddr=NULL;
        MS_U8 u8OutputBytePerPixel=4,u8FRCMemBytePerPixel=3, u8ReadBank=0; //read bank=Wbank number - 1;
        MS_U32 u32DstPhyAddr = (MS_U32)pDataBuf;
        MS_U32 u32SrcPhyAddr = 0, u32Offset = 0;
        if(u32DstPhyAddr==0)
        {
        ...
        else
        {
            ...
            u32PixelNum = gSrcInfo[MAIN_WINDOW].Status2.stXCPanelDefaultTiming.u16Width*
                          gSrcInfo[MAIN_WINDOW].Status2.stXCPanelDefaultTiming.u16Height*
                          u8FrameNum;
        }
        ...
        pu32Dstaddr = (MS_U32 *)MsOS_PA2KSEG1(u32DstPhyAddr);
        if(0 == pu32Dstaddr)
        {
            printf("%s :pu32Dstaddr_1 = 0!!,error!!!\n", __FUNCTION__);
            assert(pu32Dstaddr != 0);
        }
        pu8Srcaddr = (MS_U8 *)MsOS_PA2KSEG1(u32SrcPhyAddr);
        if(0 == pu8Srcaddr)
        {
...
#if DATA_CONVERT_NEON_OPTIMIZE_ENABLE
        MDrv_XC_FRC_Rgb888_2_Abgr8888_Neon((MS_U32)pu32Dstaddr, (MS_U32)pu8Srcaddr, u32PixelNum);
#else
    ...
    if(eBufType == E_XC_OUTPUTDATA_RGB10BITS || eBufType == E_XC_OUTPUTDATA_FRC_RGB10BITS)
    {
        u16PointSize = sizeof(MS_PIXEL_32BIT);
    }
    else if(eBufType == E_XC_OUTPUTDATA_RGB8BITS || eBufType == E_XC_OUTPUTDATA_FRC_RGB8BITS)
    {
        u16PointSize = sizeof(MS_PIXEL_24BIT);
    }
    else if(eBufType == E_XC_OUTPUTDATA_4228BITS || eBufType == E_XC_OUTPUTDATA_FRC_4228BITS)
    {
...
MS_U32 MApi_XC_CalculateHFreqx1K(MS_U16 u16HPeriod)
{
    ...
    if(UtopiaIoctl(pu32XC_Kernel_Inst, E_XC_CMD_GET_HFREQX1K, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return 0;
    }
...
MS_U16 MApi_XC_CalculateVFreqx10(MS_U16 u16HFreq, MS_U16 u16VTotal)
{
    ...
    if(UtopiaIoctl(pu32XC_Kernel_Inst, E_XC_CMD_GET_VFREQX10, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return 0;
    }
...
MS_U32 MApi_XC_CalculateVFreqx1K(MS_U32 u32HFreqx1K, MS_U16 u16VTotal)
{
    ...
    if(UtopiaIoctl(pu32XC_Kernel_Inst, E_XC_CMD_GET_VFREQX1K, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return 0;
    }
...
MS_U32 MDrv_XC_GetAccurateVFreqx1K(void* pInstance, SCALER_WIN eWindow)
{
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "MDrv_SC_ip_get_verticalperiod = %lu\n", u32AccuratePeriod);
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_SETTIMING, "u32AccurateVFreq = %lu\n", u32AccurateVFreq);
...
void MApi_XC_DisableInputSource_U2(void* pInstance, MS_BOOL bDisable, SCALER_WIN eWindow)
{
    ...
    // Blue screen is the same so just return;
    if (MDrv_XC_IsInputSourceDisabled(pInstance, eWindow) == bDisable && eWindow!=OFFLINE_WINDOW)
    {
        #if SC2_ENABLE
            _XC_RETURN(pInstance);
            _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
...
void MDrv_XC_DisableInputSource(void *pInstance, MS_BOOL bDisable, SCALER_WIN eWindow)
{
    ...
    else
    {
        ...
        {
            SC_W2BYTEMSK(psXCInstPri->u32DeviceID,REG_SC_BK13_02_L,   (bDisable ? BIT(7) : 0), BIT(7));
        }
        else
        {
            if (!MDrv_SC_GetSkipWaitVsync(pInstance, eWindow))
            {
...
MS_BOOL MDrv_XC_IsInputSourceDisabledByHwAutoNoSignal(void *pInstance, SCALER_WIN eWindow)
{
    ...
    if (eWindow == MAIN_WINDOW)
        ...
        return (SC_R2BYTEMSK(psXCInstPri->u32DeviceID,REG_SC_BK13_02_L, 0x0080) >> 7);
    else
...
#if SC2_ENABLE
    _XC_ENTRY(pInstance);
    Hal_SC_ip_Set_TestPattern(pInstance, u8Enable, u6Pattern_type, eWindow);
    _XC_RETURN(pInstance);
#else //SC2_ENABLE
    _XC_ENTRY();
    Hal_SC_ip_Set_TestPattern(pInstance, u8Enable, u6Pattern_type, eWindow);
...
void MApi_XC_InitRegInTimingChange_Post(void *pInstance, MS_U8 u8CurrentSyncStatus )
{
    ...
    //if( MP_IS_COMP() ) // just for component
    {
        ...
        if ( u8CurrentSyncStatus & (BIT(2)|BIT(3)|BIT(4)))// sync loss | MD_INTERLACE_BIT) )
            HAL_SC_ip_detect_mode(pInstance, ENABLE);
        else
            HAL_SC_ip_detect_mode(pInstance, DISABLE);
    }
...
E_APIXC_ReturnValue MApi_XC_OutputFrameCtrl_U2(void* pInstance, MS_BOOL bEnable,XC_OUTPUTFRAME_Info *pstOutFrameInfo,SCALER_WIN eWindow)
{
    ...
    if(!bEnable)
    {
        ...
        Hal_SC_set_skp_fd(pInstance, FALSE);
    ...
    else
    {
        ...
        else
        {
            ...
            else    //interlace
            {
                ...
                {
                    Hal_SC_set_skp_fd(pInstance, TRUE);
                }
                else
                {
                    Hal_SC_set_skp_fd(pInstance, FALSE);
...
MS_U32 MDrv_SC_ip_get_verticalperiod(void *pInstance, SCALER_WIN eWindow)
{
    ...
    UtopiaInstanceGetPrivate(pInstance, (void**)&psXCInstPri);
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(psXCInstPri->u32DeviceID)],(void**)(&pXCResourcePrivate));
    if(gSrcInfo[eWindow].enInputSourceType == INPUT_SOURCE_TV)
    {
        while (u8Count < (MAX_GET_VPERIOD_TIME*4))
        {
            u32AccurateVPeriod = HAL_SC_ip_get_verticalperiod(pInstance, eWindow);
            XC_LOG_TRACE(XC_DBGLEVEL_ATVTiming, "\33[0;35m  [%s][%d],AccurateVPeriod = [%ld] \33[m \n",__FUNCTION__,__LINE__,u32AccurateVPeriod);
...
MS_U32 MDrv_SC_ip_get_verticalperiod_isr(void* pInstance, SCALER_WIN eWindow)
{
    MS_U32 u32AccurateVPeriod = 0, u32PrevAccurateVPeriod = 0;
    MS_U8 u8Count = 0;
...
MS_BOOL MDrv_XC_PCMonitor_Init(void *pInstance, MS_U8 u8MaxWindowNum)
{
    ...
    {
        pXCResourcePrivate->stdrvXC_IP.psPCMONITOR_status[i] = &(pXCResourcePrivate->stdrvXC_IP.sPCMONITOR_status[i]);

        pXCResourcePrivate->stdrvXC_IP.psPCMONITOR_status[i]->eCurrentSrc = INPUT_SOURCE_NONE;
    }
...
void MDrv_XC_PCMonitor_Restart(void *pInstance, SCALER_WIN eWindow)
{
    ...
    pXCResourcePrivate->stdrvXC_IP.sPCMONITOR_status[eWindow].u16HTotal  = 0;
...
MS_BOOL MDrv_XC_PCMonitor_InvalidTimingDetect(void *pInstance, MS_BOOL bPollingOnly, SCALER_WIN eWindow)
{
    ...
    else // have signal
    {
        ...
        if ( bPollingOnly != TRUE )
            pXCResourcePrivate->stdrvXC_IP.sPCMONITOR_status[eWindow].u16HTotal= sXC_Sync_Status.u16Htotal;
        ...
        if( (IsSrcTypeHDMI(pXCResourcePrivate->stdrvXC_IP.sPCMONITOR_status[eWindow].eCurrentSrc) || IsSrcTypeDVI(pXCResourcePrivate->stdrvXC_IP.sPCMONITOR_status[eWindow].eCurrentSrc))
                &&(MDrv_XC_GetHdmiSyncMode(pInstance,eWindow) == HDMI_SYNC_DE) )
        {
            MS_WINDOW_TYPE sWin;
            ...
            if (abs( sWin.x - pXCResourcePrivate->stdrvXC_IP.sPCMONITOR_status[eWindow].sDVI_HDMI_DE_Info.x) > MD_DEHSTART_TORLANCE)
            {
                PCMSG_TIMING(printf("DE_X %x->%x, Tolerance=%x\n", pXCResourcePrivate->stdrvXC_IP.sPCMONITOR_status[eWindow].sDVI_HDMI_DE_Info.x, sWin.x, (MS_U16)MD_DEHSTART_TORLANCE));
                bResult = TRUE;
            }

            if (abs( sWin.y - pXCResourcePrivate->stdrvXC_IP.sPCMONITOR_status[eWindow].sDVI_HDMI_DE_Info.y) > MD_DEVSTART_TORLANCE)
            {
                PCMSG_TIMING(printf("DE_Y %x->%x, Tolerance=%x\n", pXCResourcePrivate->stdrvXC_IP.sPCMONITOR_status[eWindow].sDVI_HDMI_DE_Info.y, sWin.y, (MS_U16)MD_DEVSTART_TORLANCE));
                bResult = TRUE;
            }
...
void MApi_XC2_DisableInputSource(MS_BOOL bDisable, SCALER_WIN eWindow)
{
    ...
    if(UtopiaIoctl(pu32XCInst_1, E_XC_CMD_SET_DISABLE_INPUTSOURCE, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return;
    }
...
MS_U16 MApi_XC2_CalculateHFreqx10(MS_U16 u16HPeriod)
{
    if (pu32XCInst_1 == NULL)
    {
        printf("[%s,%5d]No instance existed, please get an instance by calling MApi_XC_Init() first\n",__FUNCTION__,__LINE__);
    ...
    if(UtopiaIoctl(pu32XCInst_1, E_XC_CMD_GET_HFREQX10, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return 0;
    }
