...
XC_ISR_INFO gstXCIsrInfo[MAX_SC_INT][MAX_ISR_NUM_OF_EACH_INT];
...
static MS_U8 _MApi_XC_GetIntSrc(SC_INT_SRC eIntNum)
{
 ...
 void MDrv_SC_ISR_fnptr_init(void )
 {
    MS_U8 i, j;
	        for(i=0; i<MAX_SC_INT; i++)
	        {
	            ...
	            {
	               gstXCIsrInfo[i][j].aryXC_ISR_Param = NULL;
	                gstXCIsrInfo[i][j].aryXC_ISR = NULL;
	            }
	        }
...
#endif
{
    ...
    u32IntSource = SC_R4BYTE(psXCInstPri->u32DeviceID, REG_SC_BK00_10_L);
    ...
    {
        MDrv_SC_set_interrupt(pInstance, SC_INT_VSINT, DISABLE);
        MDrv_SC_vsync_isr();
        MDrv_SC_set_interrupt(pInstance, SC_INT_VSINT, ENABLE);
    }
#endif
    ...
    for(eSCIntNum=SC_INT_DIPW; eSCIntNum<MAX_SC_INT; eSCIntNum++)
    {
        if(u32IntSource & (1 << eSCIntNum))
        {
            ...
            for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
            {
        	    ...
        	    if(gstXCIsrInfo[eSCIntNum][i].aryXC_ISR != NULL)
                {
                    // execute ISR
					...
					(gstXCIsrInfo[eSCIntNum][i].aryXC_ISR)((SC_INT_SRC)eSCIntNum, gstXCIsrInfo[eSCIntNum][i].aryXC_ISR_Param);
    ...
    for(eSCIntNum=SC_INT_DIPW; eSCIntNum<MAX_SC_INT; eSCIntNum++)
    {
        if(u32IntSource & (1 << eSCIntNum))
        {
            ...
            for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
            {
                ...
                {
                    // execute ISR
                    (void)(gstXCIsrInfo[eSCIntNum][i].aryXC_ISR) ((SC_INT_SRC)eSCIntNum,gstXCIsrInfo[eSCIntNum][i].aryXC_ISR_Param);                    
                    //(void)(pXCResourcePrivate->stdrvXC_ISR.stXCIsrInfo[eSCIntNum][i].aryXC_ISR) ((SC_INT_SRC)eSCIntNum, pXCResourcePrivate->stdrvXC_ISR.stXCIsrInfo[eSCIntNum][i].aryXC_ISR_Param);
                }
            }
        }
...
MS_BOOL MDrv_XC_InterruptAvaliable(void *pInstance, SC_INT_SRC eIntNum)
{
    ...
    for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
    {
	 if(gstXCIsrInfo[eIntNum][i].aryXC_ISR == NULL)
...
MS_BOOL MDrv_XC_InterruptIsAttached(void *pInstance, SC_INT_SRC eIntNum, SC_InterruptCb pIntCb, void * pParam)
{
    ...
    for(i=0; i<MAX_ISR_NUM_OF_EACH_INT; i++)
    {
   	 if((gstXCIsrInfo[eIntNum][i].aryXC_ISR == pIntCb) && (gstXCIsrInfo[eIntNum][i].aryXC_ISR_Param == pParam))
...
//        if(pXCResourcePrivate->stdrvXC_ISR.stXCIsrInfo[eIntNum][i].aryXC_ISR == NULL)
			...
			
        {
            ...
            gstXCIsrInfo[eIntNum][i].aryXC_ISR_Param = pParam;
            gstXCIsrInfo[eIntNum][i].aryXC_ISR = pIntCb;
          ...
          //  pXCResourcePrivate->stdrvXC_ISR.stXCIsrInfo[eIntNum][i].aryXC_ISR = pIntCb;
   		#ifdef MSOS_TYPE_LINUX_KERNEL	
            #else	
                gstXCIsrInfo[eIntNum][i].pidXCIsr = getpid();	
			
 	//               pXCResourcePrivate->stdrvXC_ISR.stXCIsrInfo[eIntNum][i].pidXCIsr = getpid();	
        //        (MDrv_SC_get_stXCIsrInfo_tmp()+(eIntNum*MAX_ISR_NUM_OF_EACH_INT+i))->pidXCIsr = getpid();
    	//    printf("ISRAttach %d successed to [%d][%d]\n", eIntNum, eIntNum, i);
		#endif	
            // enable interrupt
...
//            (pXCResourcePrivate->stdrvXC_ISR.stXCIsrInfo[eIntNum][i].aryXC_ISR_Param == pParam))
        if((gstXCIsrInfo[eIntNum][i].aryXC_ISR == pIntCb) &&
            (gstXCIsrInfo[eIntNum][i].aryXC_ISR_Param == pParam))
            
        {
            // disable interrupt first to avoid racing condition
            MDrv_SC_set_interrupt(pInstance, eIntNum, DISABLE);
            gstXCIsrInfo[eIntNum][i].aryXC_ISR = NULL;
            gstXCIsrInfo[eIntNum][i].aryXC_ISR_Param = NULL;
