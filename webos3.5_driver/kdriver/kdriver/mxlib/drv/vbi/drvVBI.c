...
static void _RingBuffer_Reset(void);
...
#endif
{
    ...
    if(_pFN!=NULL && _pStoreInfo._pCbVABuff!=NULL)
    {
        ...
        {
            case E_VBI_TELETEXT:
                if(MDrv_VBI_TTX_GetPackets(_pStoreInfo._cbBufferAddr, &length)==TRUE)
                {
...
#else
    // sec
    if (rb_handle.WritePacketNumber > _pStoreInfo._TTXBufLen)
    {
        MS_BOOL isOrgEnabled = VBI_TTX_IsSlicerEnabled();
		
        //reset
        VBI_TTX_EnableSlicer(FALSE);
        VBI_TTX_EnableSlicer(TRUE);
        if(!isOrgEnabled)
            VBI_TTX_EnableSlicer(FALSE);
...
MS_BOOL _MDrv_VBI_Init(EN_VBI_CMD cmd, VBI_INIT_TYPE type)
{
    ...
    if(cmd == VBI_INIT)
    {
        ...
        if(type==E_VBI_TELETEXT)
        {
            ...
            {
                printf("[VBI ERROR] TELETEXT not support in this chip !!!\n");
                return FALSE;
            }
        }
        else
...
MS_BOOL _MDrv_VBI_TTX_GetPacket(MS_PHY *packetAddress)
{
    ...
    if (_MDrv_VBI_TTX_PacketBufferIsEmpty()==FALSE)
    {
        ...
        if((_pStoreInfo._TTXBufAddr != 0) && (_pStoreInfo._TTXBufAddr != 0xFFFFFFFF))
...
void _MDrv_VBI_CC_InitSlicer(MS_U32 u32RiuAddr, MS_PHY bufferAddr, MS_U16 packetCount)
{
    MS_PHY phyNonPMBankSize;
...
MS_BOOL _MDrv_VBI_GetCompleteVPS_Data(MS_U8* pBuffer, MS_U32* dataLen)
{
    return VBI_GetCompleteVPS_Data(pBuffer, dataLen);
}
...
MS_U8 video_standard_vbi_settings[VIDEO_TYPE_CNT][VIDEO_REGISTER_CNT + 1] =
{
    // type             41h[7:0]   42h[5:0]    44h[7:0]    4Bh[5:0]    4Dh[7:0]     50h[4:0]     51h[4:0]
    {VBI_VIDEO_NTSC,      0x52,      0x1c,       0x39,       0x24,       0x8e,       0x12,       0x12},  // NTSC
    {VBI_VIDEO_PAL_M,     0x52,      0x1c,       0x39,       0x26,       0x8e,       0x12,       0x12},  // PAL-M
    {VBI_VIDEO_PAL_NC,    0x52,      0x1c,       0x39,       0x16,       0x8e,       0x12,       0x12},  // PAL-NC
    {VBI_VIDEO_NTSC_443,  0x52,      0x23,       0x47,       0x32,       0xb0,       0x12,       0x12},  // NTSC-443, PAL-60
    {VBI_VIDEO_PAL,       0xb5,      0x23,       0x47,       0x22,       0xb0,       0x15,       0x15},  // PAL
};

MS_BOOL _MDrv_VBI_CC_SetVideoStandard(VBI_VIDEO_STANDARD eStandard)
{
    int i;
    ...
    for (i = 0; i < VIDEO_TYPE_CNT; i++)
    {
        ...
        {
            //printf("video type...%d\n",i);
            MDrv_VBI_CC_SetDataRate((MS_U8*)video_standard_vbi_settings[i]);
            return TRUE;
        }
    }
...
MS_BOOL MDrv_VBI_GetStatus(VBI_DrvStatus *pDrvStatus)
{
    ...
    {
        printf("Ioctl MDrv_VBI_GetStatus fail\n");
        return FALSE;
    }
    return TRUE;
#else
...
MS_BOOL MDrv_VBI_Init(VBI_INIT_TYPE type)
{
    ...
    if (NULL == pInstantVbi)
    {        
        if(UtopiaOpen(MODULE_VBI | KERNEL_MODE, (void**)&pInstantVbi, 0, pAttributeVbi) ==  UTOPIA_STATUS_FAIL)
        {
            printf("UtopiaOpen VBI fail\n");
            return FALSE;
        }
    }
    else
    {
        printf("VBI already Opened\n");
        return FALSE;
    }
    ...
    {
        printf("Ioctl MDrv_VBI_Init fail\n");
        return FALSE;
    }
    return TRUE;
#else
...
{    
    ...
    {
        printf("Ioctl MDrv_VBI_Exit fail\n");
        return FALSE;
    } 
    return TRUE;
#else
...
void MDrv_VBI_RegisterCB(VBI_CB_FN pFN, MS_PHY bufferAddr, MS_U32 length)
{
    ...
    {
        printf("Ioctl MDrv_VBI_RegisterCB fail\n");
    }
#else
...
void MDrv_VBI_RingBuffer_Reset(void)
{
    ...
    {
        printf("Ioctl MDrv_VBI_RingBuffer_Reset fail\n");
    }
#else
    _MDrv_VBI_RingBuffer_Reset();
#endif
...
void MDrv_VBI_InitializeTTXSlicer(MS_PHY bufferAddr, MS_U16 packetCount)
{
    ...
    {
        printf("Ioctl MDrv_VBI_InitializeTTXSlicer fail\n");
    }
#else
...
void MDrv_VBI_EnableTTXSlicer(MS_BOOL bEnable)
{
    ...
    {
        printf("Ioctl MDrv_VBI_EnableTTXSlicer fail\n");
    }
#else
...
MS_BOOL MDrv_VBI_IsVPS_Ready(void)
{
    ...
    {
        printf("Ioctl MDrv_VBI_IsVPS_Ready fail\n");        
        return FALSE;
    }    
...
MS_BOOL MDrv_VBI_IsTTX_Ready(void)
{
    ...
    {
        printf("Ioctl MDrv_VBI_IsTTX_Ready fail\n");        
        return FALSE;
    }    
    return CheckTureFalse.bCheck;
#else
...
MS_BOOL MDrv_VBI_IsWSS_Ready(void)
{
    ...
    {
        printf("Ioctl MDrv_VBI_IsWSS_Ready fail\n");        
        return FALSE;
    }    
    return CheckTureFalse.bCheck;
#else
...
{        
    ...
    {
        printf("Ioctl MDrv_VBI_GetWSS_Data fail\n");        
        return FALSE;
    }    
    return GetData.data;
#else
...
void MDrv_VBI_GetVPS_Data(MS_U8 *lowerWord, MS_U8 *higherWord)
{
    ...
    {
        printf("Ioctl MDrv_VBI_GetVPS_Data fail\n");        
        return;
    }    
#else
...
{
#if (VBI_UTOPIA20)
    VBI_GET_COMPLETE_VPS_DATA GetCompleteVPSData;
    memset(&GetCompleteVPSData, 0 , sizeof(VBI_GET_COMPLETE_VPS_DATA));
	...
	GetCompleteVPSData.length  = (MS_U32*) dataLen;
                                
    if(UtopiaIoctl(pInstantVbi, MDrv_CMD_VBI_GetCompleteVPS_Data, (void*)&GetCompleteVPSData) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Ioctl MDrv_VBI_GetCompleteVPS_Data fail\n"); 
		return FALSE;
    }    

    if(*GetCompleteVPSData.length != 0)
    {
        memcpy((void*)_u8VPSData, (void*)&GetCompleteVPSData.pBuffer, VPS_PACKET_LEN);
        *pBuffer = (MS_U8*)_u8VPSData;
    }
	return TRUE;
#else
    return _MDrv_VBI_GetCompleteVPS_Data(pBuffer, dataLen);
#endif
}
...
{           
    ...
    {
        printf("Ioctl MDrv_VBI_SetVideoStandard fail\n");        
    }    
#else
...
MS_BOOL MDrv_VBI_TTX_PacketBufferIsEmpty(void)
{
    ...
    {
        printf("Ioctl MDrv_VBI_TTX_PacketBufferIsEmpty fail\n");  
        return FALSE;
    }        
    return CheckTureFalse.bCheck;
#else
...
MS_BOOL MDrv_VBI_TTX_CheckCircuitReady(void)
{
    ...
    {
        printf("Ioctl MDrv_VBI_TTX_CheckCircuitReady fail\n");  
        return FALSE;
    }        
    return CheckTureFalse.bCheck;
#else
...
MS_U16 MDrv_VBI_TTX_GetPacketCount(void)
{
    ...
    {
        printf("Ioctl MDrv_VBI_TTX_GetPacketCount fail\n");  
        return FALSE;
    }        
    return GetData.data;
#else
...
MS_BOOL MDrv_VBI_TTX_GetPackets(MS_PHY dataAddr, MS_U32 *length)
{
    ...
    {
        //printf("Ioctl MDrv_VBI_TTX_GetPackets fail\n");  
        return FALSE;
    }  
    return TRUE;
#else
...
MS_BOOL MDrv_VBI_TTX_PacketBufferIsOverflow( void )
{
    ...
    {
        printf("Ioctl MDrv_VBI_TTX_PacketBufferIsOverflow fail\n");  
        return FALSE;
    }  
    return CheckTureFalse.bCheck;
#else
...
MS_U16 MDrv_VBI_TTX_PacketBufferGetNoOfOverflows( void )
{
    ...
    {
        printf("Ioctl MDrv_VBI_TTX_PacketBufferGetNoOfOverflows fail\n");  
        return FALSE;
    }  
    return GetData.data;
#else
...
void MDrv_VBI_TTX_EnableLine(MS_U16 StartLine, MS_U16 EndLine)
{
    ...
    {
        printf("Ioctl MDrv_VBI_TTX_EnableLine fail\n");  
        return;
    }  
#else
...
MS_BOOL MDrv_VBI_SyncMemory( MS_U32 u32Start, MS_U32 u32Size )
{
    ...
    {
        printf("Ioctl MDrv_VBI_SyncMemory fail\n");  
        return FALSE;
    }
    return TRUE;
#else
 ...
 *@return
 ****************************************************************************/
void MDrv_VBI_CC_InitSlicer(MS_U32 u32RiuAddr, MS_PHY bufferAddr, MS_U16 packetCount)
{
#if (VBI_UTOPIA20)
    ...
    {
        printf("Ioctl MDrv_VBI_CC_InitSlicer fail\n");  
        return;
    }
#else
...
void MDrv_VBI_CC_InitYPbYr(MS_U8 cvbs_no)
{
    ...
    {
        printf("Ioctl MDrv_VBI_CC_InitYPbYr fail\n");  
        return;
    }
#else
...
MS_U8 MDrv_VBI_CC_SetDataRate(MS_U8 *ptable)
{
    ...
    {
        printf("Ioctl MDrv_VBI_CC_SetDataRate fail\n");  
        return FALSE;
    }
    return TRUE;
#else
...
MS_U32 MDrv_VBI_CC_GetInfo(MS_U32 selector)
{
    ...
    {
        printf("Ioctl MDrv_VBI_CC_GetInfo fail\n");  
        return FALSE;
    }
    return GetInfo.info;
#else
...
void MDrv_VBI_CC_SetFrameCnt(MS_U8 cnt)
{
    ...
    {
        printf("Ioctl MDrv_VBI_CC_SetFrameCnt fail\n");  
        return;
    }
#else
...
void MDrv_VBI_CC_EnableSlicer(MS_BOOL bEnable)
{
    ...
    {
        printf("Ioctl MDrv_VBI_CC_EnableSlicer fail\n");  
        return;
    }
#else
...
{
#if (VBI_UTOPIA20)
    VBI_SET_VIDEO_STANDARD CCSetVideoStandard;
    memset(&CCSetVideoStandard, 0 , sizeof(VBI_SET_VIDEO_STANDARD));
    ...
    CCSetVideoStandard.eStandard = eStandard;
                                
    if(UtopiaIoctl(pInstantVbi, MDrv_CMD_VBI_CC_SetVideoStandard, (void*)&CCSetVideoStandard) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Ioctl MDrv_VBI_CC_SetVideoStandard fail\n");        
		return FALSE;
    }    
	return TRUE;
#else
    return _MDrv_VBI_CC_SetVideoStandard(eStandard);
#endif
}
...
void MDrv_VBI_CC_EnableLine(MS_U16 StartLine, MS_U16 EndLine, MS_U8 mode)
{
    ...
    {
        printf("Ioctl MDrv_VBI_CC_EnableLine fail\n");  
        return;
    }
#else
...
void MDrv_VBI_WSS_SetVpsByteNum(MS_U8 cnt)
{
    ...
    {
        printf("Ioctl MDrv_VBI_WSS_SetVpsByteNum fail\n");  
        return;
    }
#else
...
MS_U32 MDrv_VBI_SetPowerState(EN_POWER_MODE u16PowerState)
{
    ...
    switch (u16PowerState)
    {
        case E_POWER_SUSPEND:
            _ret = MDrv_VBI_Suspend();
            break;

        case E_POWER_RESUME:
            _ret = MDrv_VBI_Resume();
            break;

        case E_POWER_MECHANICAL:
        case E_POWER_SOFT_OFF:
        default:
            printf("[%s] %d Power state not support!!\n", __FUNCTION__, __LINE__);
            break;
    }
...
MS_U32 MDrv_VBI_GetBuff(VBI_INIT_TYPE type)
{
	if (type == E_VBI_TELETEXT)
		return (MS_U32)(_pStoreInfo._TTXBufAddr);
	else //CC
		return (MS_U32)(_pStoreInfo._ptrVBIRiuBaseAddr);
}
