...
#define USB_HID_P1_DRV_VERSION                  /* Character String for DRV/API version             */  \
    MSIF_TAG,                           /* 'MSIF'                                           */  \
    MSIF_CLASS,                         /* '00'                                             */  \
    MSIF_CUS,                           /* 0x0000                                           */  \
    MSIF_MOD,                           /* 0x0000                                           */  \
...
static MSIF_Version _drv_usb_hid_p1_version = {
    .DDI = { USB_HID_P1_DRV_VERSION },
};

/*
 * Read data value from item.
 */
U8 UsbIntrData[16] __attribute__ ((aligned (16))) ;
U8 *pUsbIntrData;
...
U8 MDrv_USB_HID_GetLibVer(const MSIF_Version **ppVersion)
{
    if(!ppVersion)
        return 1;

    *ppVersion = &_drv_usb_hid_p1_version;
    return 0;
}
...
static int hid_chk_variable(struct hid_parser *parser, unsigned report_type)
{
    ...
    if (parser->global.usage_page == USEAGE_PAGE_BUTTON)
    {
        ...
        buttons.count=parser->global.report_size * rpt_count;
   ...
   // printf("  hid_parser_main   \n");
	...
	switch (item->tag) {
		...
		case HID_MAIN_ITEM_TAG_INPUT:
                        ret = hid_chk_variable(parser, HID_INPUT_REPORT);
			//ret = hid_add_field(parser, HID_INPUT_REPORT, data);
			break;
		case HID_MAIN_ITEM_TAG_OUTPUT:
                        ret = hid_chk_variable(parser, HID_OUTPUT_REPORT);
			//ret = hid_add_field(parser, HID_OUTPUT_REPORT, data);
			break;
		case HID_MAIN_ITEM_TAG_FEATURE:
                        ret = hid_chk_variable(parser, HID_FEATURE_REPORT);
...
void hid_parse_report(__u8 *start, unsigned size)
{
	__u8 *end;
       int result = 0;
...
//                start, gitem.format, gitem.size, gitem.type, gitem.tag, gitem.zdata.u8);
        ...
        if (gitem.type==0)
            result=hid_parser_main(&gparser,&gitem);
        else if (gitem.type==1)
            result=hid_parser_global(&gparser,&gitem);
               ...
               if (axis[i].size)                //this AXIS exist
                {
                        ...
                        if (axis[i].val!=tmp )
                            {
                                 USB_HID_DBG(printf(" axis idx %bx, offset:%x ,val %02bx  tmp:%02bx\n", i,axis[i].startbit,axis[i].val,tmp));
                                 axis[i].val=tmp;
...
// printf("t2");
		//printf("button start:%x",buttons.startbit);
            tmp2=GetValue(report,buttons.startbit,buttons.count);
           // printf("tmp2:%x\n",tmp2);
            if (buttons.val!=tmp2)
                {
...
BOOLEAN GetReportDescriptor(void)
{
    UINT8 bCMD[8];
    UINT8 i;

   memcpy(bCMD,GET_REPORT_DESCRIPTOR,8);
   if ( flib_Host20_Issue_Control (1,bCMD,0x65,pUsbData)>0)
       return FALSE;
   ...
   hid_parse_report(pUsbData, 0x65);

   return TRUE;

}
U8 drvUSBHost_HID_Initial(void)
{
    USB_HID_DBG(printf("HID device\n"););

    GetReportDescriptor();

    pUsbIntrData=(U8*) KSEG02KSEG1(UsbIntrData);
    MY_HAL_DCACHE_FLUSH((U32)UsbIntrData, 16);
    flib_Host20_Interrupt_Init(3);
...
U16  MDrv_GET_JOYSTICK_STATUS(void)
{
    ...
    memset(pUsbIntrData, 0, 8);
     if (flib_Host20_Issue_Interrupt((U32)pUsbIntrData,8)==HOST20_OK)
         tmp=Parse_Joystick_Report(pUsbIntrData);
// printf("x3");
...
U8*  MDrv_Get_HID_Report_Raw_Data(void)
{
    ...
    memset(ReportData, 0, 8);
    if (flib_Host20_Issue_Interrupt((U32)pUsbIntrData,8)==HOST20_OK)
    {
        for (i=0 ; i <8 ; i++)
        {
            ReportData[i]=pUsbIntrData[i];
