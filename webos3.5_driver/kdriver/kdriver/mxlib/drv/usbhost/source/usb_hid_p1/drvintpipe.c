...
Periodic_Frame_List_Structure   *pHost20_FramList;
qHD_Structure  Host20_qHD_List_Intr    __attribute__ ((aligned (32)));
qHD_Structure *pHost20_qHD_List_Intr;
extern  Host20_Attach_Device_Structure sAttachDevice;
extern UINT8 gSpeed;
extern U8 gUsbStatus;

extern UINT8 flib_Host20_CheckingForResult_QHD(qHD_Structure *spTempqHD);
extern void ResetUsbHardware(void);
extern U8 usb_temp_buf[Scsi_Max_Transfer_Len];

void SetFrameList(UINT8 i)
{
    pHost20_FramList->sCell[i].bTerminal=0;
    pHost20_FramList->sCell[i].bType=HOST20_HD_Type_QH;
    //SetPointer(&(Host20_FramList.sCell[i].bLinkPointer3),VirtoPhyAddr((UINT16)pHost20_qHD_List_Intr));
    pHost20_FramList->sCell[i].bLinkPointer = (VirtoPhyAddr((U32)pHost20_qHD_List_Intr)>>5);
}
...
// * OutPut:
//====================================================================
void  flib_Host20_Interrupt_Init(UINT8 bAddr)
{
    UINT32 i,j;
    // UINT32 wSize;
    UINT8 *pData;
    ...
    pHost20_qHD_List_Intr=(qHD_Structure*)KSEG02KSEG1(&Host20_qHD_List_Intr);
    pHost20_FramList = (Periodic_Frame_List_Structure*) KSEG02KSEG1(&Host20_FramList);
    //printf("pHost20_qHD_List_Intr: %X\n", pHost20_qHD_List_Intr);
    MY_HAL_DCACHE_FLUSH((U32)&Host20_qHD_List_Intr, sizeof(qHD_Structure));
    MY_HAL_DCACHE_FLUSH((U32)&Host20_FramList, sizeof(Periodic_Frame_List_Structure));
    ...
    EndPaddr=  sAttachDevice.saCD[0].sInterface[0].sED[0].bED_EndpointAddress & 0x7f;
    USB_HID_INT_DBG(printf("interrupt addr:%02x\n",EndPaddr));
    flib_Host20_Allocate_QHD(pHost20_qHD_List_Intr,HOST20_HD_Type_QH,bAddr,0,EndPaddr,8);//Address=0,Head=1,EndPt=0,Size
    ...
    //SetPointer(&(pHost20_qHD_List_Intr->bNextQHDPointer3),VirtoPhyAddr((UINT16)pHost20_qHD_List_Intr));
     pHost20_qHD_List_Intr->bNextQHDPointer=(VirtoPhyAddr((U32)pHost20_qHD_List_Intr)>>5);
    ...
    for (i=0;i< Host20_Preiodic_Frame_List_MAX ;i++)
        pHost20_FramList->sCell[i].bTerminal=1;
    //Find the Interval-X
    ...
    mwHost20_PeriodicBaseAddr_Set(VirtoPhyAddr((U32)pHost20_FramList));
...
UINT8 flib_Host20_Send_qTD2(qTD_Structure  *spHeadqTD ,qHD_Structure  *spTempqHD,U16 wTimeOutSec)
{
...
// * OutPut:
//====================================================================
U8  flib_Host20_Issue_Interrupt(U32 buf,U32 hwSize)
{
    ...
    //<3>.Send TD
    bResult = flib_Host20_Send_qTD2(spTempqTD ,pHost20_qHD_List_Intr,3);
