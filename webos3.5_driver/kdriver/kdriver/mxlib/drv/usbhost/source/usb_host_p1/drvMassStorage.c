...
UINT8 Host20_MSCD_issue_CBW(struct us_data *psMassStorage)
{
  ...
  OTGH_PT_BLK.sCBW.u32DataTransferLength=psMassStorage->srb->request_bufflen;
  ...
  //memcpy(buf,(U8 xdata *)&(OTGH_PT_BLK.sCBW.u32Signature),31);
      buf=((U32)&(OTGH_PT_BLK.sCBW.u32Signature));
...
// Status:P-OK
//***************************************************************************************
UINT8 Host20_MSCD_issue_CSW(void)
{
  ...
  //printf("Get CSW");
  // buf=(U8 xdata *)sAttachDevice.bDataBuffer;
    buf=(U32)&(OTGH_PT_BLK.sCSW.u32Signature);
	  ...
	  while(wTotalLengthRemain>0)
	  {
        ...
        while (result==HOST20_TRANSACTION_ERROR)
        {
	      ...
	      if(OTGH_PT_BLK.bOutQHDArrayNum)
	        pHost20_qHD_List_Bulk1->bDataToggleControl=1;
		  else
		    pHost20_qHD_List_Bulk0->bDataToggleControl=1;
...
code UINT8 CBI_SET_COMMAND[]    = {0x21,0x00,0x00,0x00,0x00,0x00,0x0c,0x00};

BOOLEAN Host20_MSDCD_usb_stor_CBI_protocol(struct us_data *psMassStorage)
{
    UINT8 bCMD[8];
    UINT8 bValue,result;
    //printf("CBI protocol\n");
    memcpy(bCMD,CBI_SET_COMMAND,8);

    result=flib_Host20_Issue_Control_CBI (bCMD,12,(UINT8 *)&(OTGH_PT_BLK.sCBW.u8CB[0]));
    ...
    if (psMassStorage->srb->request_bufflen>0)
    {
        ...
        {
            DRV_MASS_STORAGE_DBG(printf("data phase stall\n"));
            Host20_MSCD_ClearFeature(psMassStorage);
        }
        else if (bValue==HOST20_FATAL)
        {
            DRV_MASS_STORAGE_DBG(printf("data phase fatal\n"));
            return FALSE;
        }
    }
...
BOOLEAN Host20_MSCD_usb_stor_control_thread(struct us_data *psMassStorage)
{
  ...
  {
      return Host20_MSDCD_usb_stor_CBI_protocol(psMassStorage);
  }
#endif
