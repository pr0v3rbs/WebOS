...
UINT8 Host20_MSCD_issue_CBW_Port2(struct us_data *psMassStorage)
{
  ...
  pOTGH_PT_BLK_Port2->sCBW.u32DataTransferLength=psMassStorage->srb->request_bufflen;
  ...
  buf=(U32)&(pOTGH_PT_BLK_Port2->sCBW.u32Signature);
...
// Status:P-OK
//***************************************************************************************
UINT8 Host20_MSCD_issue_CSW_Port2(void)
{
  ...
  //printf("Get CSW");
  // buf=(U8 xdata *)psAttachDevice_Port2->bDataBuffer;
    buf=(U32)&(pOTGH_PT_BLK_Port2->sCSW.u32Signature);
  ...
  redo2:
       ...
       if (result==HOST20_TRANSACTION_ERROR)
       {
	     ...
	     if(retry<=ErrorRetryCount1)
         {
	       ...
	       {
	         //printf("\r\n CSW toggle!!",0);
                UTMI2_SETXBYTE(0x0a, UTMI2_READXBYTE(0x0a)^0x10); //invert CDR_CLOCK
                UTMI2_ORXBYTE(0x06,0x03);    //reset UTMI
                UTMI2_ANDXBYTE(0x06,0xfc);
	       }
         }
	     #endif
...
code UINT8 CBI_SET_COMMAND_PORT2[]    = {0x21,0x00,0x00,0x00,0x00,0x00,0x0c,0x00};

BOOLEAN Host20_MSDCD_usb_stor_CBI_protocol_Port2(struct us_data *psMassStorage)
{
    UINT8 bCMD[8];
    UINT8 bValue,result;
    //printf("CBI protocol\n");
    memcpy(bCMD,CBI_SET_COMMAND_PORT2,8);

    result=flib_Host20_Issue_Control_CBI_Port2(bCMD,12,(UINT8 *)&(pOTGH_PT_BLK_Port2->sCBW.u8CB[0]));
...
BOOLEAN Host20_MSCD_usb_stor_control_thread_Port2(struct us_data *psMassStorage)
{
  ...
  //<1>.CBW issue
#ifdef ENABLE_CBI_HOST
  if ((psAttachDevice_Port2->saCD[0].sInterface[0].bInterfaceProtocol==0x00)||(psAttachDevice_Port2->saCD[0].sInterface[0].bInterfaceProtocol==0x01))
  {
      return Host20_MSDCD_usb_stor_CBI_protocol_Port2(psMassStorage);
  }
#endif
