...
Periodic_Frame_List_Structure  Host20_FramList_Port2 __attribute__ ((aligned (4096)));
Periodic_Frame_List_Structure   *pHost20_FramList_Port2;
qHD_Structure  Host20_qHD_List_Intr_Port2    __attribute__ ((aligned (32)));
qHD_Structure *pHost20_qHD_List_Intr_Port2;
extern  Host20_Attach_Device_Structure sAttachDevice_Port2;
//extern Host20_Attach_Device_Structure  *psAttachDevice_Port2;
extern UINT8 gSpeed_Port2;
extern U8 gUsbStatus_Port2;

extern UINT8 flib_Host20_CheckingForResult_QHD_Port2(qHD_Structure *spTempqHD);
extern void ResetUsbHardwarePort2(void);
extern U8 usb_temp_buf_Port2[Scsi_Max_Transfer_Len];

void SetFrameList_Port2(UINT8 i)
{
    pHost20_FramList_Port2->sCell[i].bTerminal=0;
    pHost20_FramList_Port2->sCell[i].bType=HOST20_HD_Type_QH;
    //SetPointer(&(Host20_FramList.sCell[i].bLinkPointer3),VirtoPhyAddr((UINT16)pHost20_qHD_List_Intr));
    pHost20_FramList_Port2->sCell[i].bLinkPointer = (VirtoPhyAddr((U32)pHost20_qHD_List_Intr_Port2)>>5);
}
...
// * OutPut:
//====================================================================
void  flib_Host20_Interrupt_Init_Port2(UINT8 bAddr)
{
    UINT32 i,j;
    // UINT32 wSize;
    UINT8 *pData;
    ...
    pHost20_qHD_List_Intr_Port2=(qHD_Structure*)KSEG02KSEG1(&Host20_qHD_List_Intr_Port2);
    pHost20_FramList_Port2= (Periodic_Frame_List_Structure*) KSEG02KSEG1(&Host20_FramList_Port2);
    //printf("pHost20_qHD_List_Intr: %X\n", pHost20_qHD_List_Intr);
    MY_HAL_DCACHE_FLUSH((U32)&Host20_qHD_List_Intr_Port2, sizeof(qHD_Structure));
    MY_HAL_DCACHE_FLUSH((U32)&Host20_FramList_Port2, sizeof(Periodic_Frame_List_Structure));
    ...
    EndPaddr=  sAttachDevice_Port2.saCD[0].sInterface[0].sED[0].bED_EndpointAddress & 0x7f;
    USB_HID_INT_DBG(printf("interrupt addr:%02x\n",EndPaddr));
    flib_Host20_Allocate_QHD_Port2(pHost20_qHD_List_Intr_Port2,HOST20_HD_Type_QH,bAddr,0,EndPaddr,8);//Address=0,Head=1,EndPt=0,Size
    ...
    //SetPointer(&(pHost20_qHD_List_Intr->bNextQHDPointer3),VirtoPhyAddr((UINT16)pHost20_qHD_List_Intr));
     pHost20_qHD_List_Intr_Port2->bNextQHDPointer=(VirtoPhyAddr((U32)pHost20_qHD_List_Intr_Port2)>>5);
    ...
    pData=(UINT8 *)pHost20_FramList_Port2;
    ...
    for (i=0;i< Host20_Preiodic_Frame_List_MAX ;i++)
        pHost20_FramList_Port2->sCell[i].bTerminal=1;
    //Find the Interval-X
    ...
    USB_HID_INT_DBG(printf("pHost20_FramList: %lX\n", (U32)pHost20_FramList_Port2));
    mwHost20_PeriodicBaseAddr_Set(VirtoPhyAddr((U32)pHost20_FramList_Port2));
...
UINT8 flib_Host20_Send_qTD2_Port2(qTD_Structure  *spHeadqTD ,qHD_Structure  *spTempqHD,U16 wTimeOutSec)
{
  ...
  sAttachDevice_Port2.bSendStatusError=0;
      ...
      if (MsOS_Timer_DiffTimeFromNow(LastTime) >(U32)300)     //over 1 mini sec
        {
            ...
            if (mwHost20_FrameIndex_Rd()==FrameIdx)
            {
                ...
                gUsbStatus_Port2=USB_TIMEOUT;         //ESD , USB hang,should be timeout
                bReturnValue=HOST20_FATAL;
                break;
            }
            FrameIdx=mwHost20_FrameIndex_Rd();
  ...
  do {
    //spReleaseqTDNext=((UINT32)(spReleaseqTD->bNextQTDPointerL))<<5 + ((UINT32)(spReleaseqTD->bNextQTDPointerH))<<16  ;
     spReleaseqTDNext=(qTD_Structure*)PhytoVirAddr(((U32)(spReleaseqTD->bNextQTDPointer))<<5);
        ...
        flib_Host20_ReleaseStructure_Port2(Host20_MEM_TYPE_qTD,(U32)spReleaseqTD);
...
// * OutPut:
//====================================================================
U8  flib_Host20_Issue_Interrupt_Port2(U32 buf,U32 hwSize)
{
    ...
    if ( buf !=(U32) KSEG02KSEG1(buf) )
    {
        if (buf & 0x7)			//flush should be 8 bytes aligned
        {
            ...
            #else
            mybuf = (U32)KSEG02KSEG1(usb_temp_buf_Port2);
            #endif
        }
        else
        {
    ...
    //<3>.Send TD
    bResult = flib_Host20_Send_qTD2_Port2(spTempqTD ,pHost20_qHD_List_Intr_Port2,3);
