...
#define USB_HID_P2_DRV_VERSION                  /* Character String for DRV/API version             */  \
    MSIF_TAG,                           /* 'MSIF'                                           */  \
    MSIF_CLASS,                         /* '00'                                             */  \
    MSIF_CUS,                           /* 0x0000                                           */  \
    MSIF_MOD,                           /* 0x0000                                           */  \
...
static MSIF_Version _drv_usb_hid_p2_version = {
    .DDI = { USB_HID_P2_DRV_VERSION },
};
/*
 * Read data value from item.
 */
U8 UsbIntrData_Port2[16] __attribute__ ((aligned (16))) ;
U8 *pUsbIntrData_Port2;
extern UINT8  pUsbData_Port2[1024];
struct   axis_struct  axis_Port2[MAX_AXIS_NUM];
__u8     axis_idx_Port2;
struct   button_struct  buttons_Port2;
__u16  gOffsetBit_Port2;

U8 MDrv_USB_HID_GetLibVer_Port2(const MSIF_Version **ppVersion)
{
    if(!ppVersion)
        return 1;

    *ppVersion = &_drv_usb_hid_p2_version;
    return 0;
}
...
static int hid_chk_variable_Port2(struct hid_parser *parser, unsigned report_type)
{
    ...
    {
        buttons_Port2.startbit=gOffsetBit_Port2;
        buttons_Port2.count=parser->global.report_size * rpt_count;
        USB_HID_DBG(printf("Usage Page (buttons): %X size: %X\n", buttons_Port2.startbit, buttons_Port2.count));

        gOffsetBit_Port2+=buttons_Port2.count;
        goto Func_Done;
    }
    ...
    if (usg_count > 0)
    {
        for(ii=usg_count-rpt_count; ii<usg_count; ii++)
        {
            ...
            if (report_type == HID_INPUT_REPORT)
            {
                ...
                {
                     axis_Port2[axis_idx_Port2].startbit=gOffsetBit_Port2;
                     axis_Port2[axis_idx_Port2].size=parser->global.report_size;
                     axis_Port2[axis_idx_Port2].type=TYPE_AXIS_X;
                     gOffsetBit_Port2+=axis_Port2[axis_idx_Port2].size;
                ...
                else if (parser->local.usage[ii] == USEAGE_Y)
                {
                     axis_Port2[axis_idx_Port2].startbit=gOffsetBit_Port2;
                     axis_Port2[axis_idx_Port2].size=parser->global.report_size;
                     axis_Port2[axis_idx_Port2].type=TYPE_AXIS_Y;
                     gOffsetBit_Port2+=axis_Port2[axis_idx_Port2].size;
 ...
 * Process a main item.
 */
 int hid_parser_main_Port2(struct hid_parser *parser, struct hid_item *item)
{
	__u32 adata;
	int ret=0;
   ...
   // printf("  hid_parser_main   \n");
	...
	switch (item->tag) {
		...
		case HID_MAIN_ITEM_TAG_INPUT:
                        ret = hid_chk_variable_Port2(parser, HID_INPUT_REPORT);
			//ret = hid_add_field(parser, HID_INPUT_REPORT, data);
			break;
		case HID_MAIN_ITEM_TAG_OUTPUT:
                        ret = hid_chk_variable_Port2(parser, HID_OUTPUT_REPORT);
			//ret = hid_add_field(parser, HID_OUTPUT_REPORT, data);
			break;
		case HID_MAIN_ITEM_TAG_FEATURE:
                        ret = hid_chk_variable_Port2(parser, HID_FEATURE_REPORT);
 ...
 */
 ...
 int hid_parser_global_Port2(struct hid_parser *parser, struct hid_item *item)
{
...
void hid_parse_report_Port2(__u8 *start, unsigned size)
{
	__u8 *end;
       int result = 0;
...
//                start, gitem.format, gitem.size, gitem.type, gitem.tag, gitem.zdata.u8);
        ...
        if (gitem.type==0)
            result=hid_parser_main_Port2(&gparser,&gitem);
        else if (gitem.type==1)
            result=hid_parser_global_Port2(&gparser,&gitem);
...
// printf("t2");
		//printf("button start:%x",buttons.startbit);
            tmp2=GetValue_Port2(report,buttons_Port2.startbit,buttons_Port2.count);
           // printf("tmp2:%x\n",tmp2);
            if (buttons_Port2.val!=tmp2)
                {
                    USB_HID_DBG(printf("buttons:%x %x\n",buttons_Port2.val, tmp2));
                    buttons_Port2.val=tmp2;
...
BOOLEAN GetReportDescriptor_Port2(void)
{
    UINT8 bCMD[8];
    UINT8 i;

   memcpy(bCMD,GET_REPORT_DESCRIPTOR_PORT2,8);
   if ( flib_Host20_Issue_Control_Port2 (1,bCMD,0x65,pUsbData_Port2)>0)
       return FALSE;
    ...
    buttons_Port2.count = 0;
    buttons_Port2.startbit = 0;
    buttons_Port2.val = 0;

   hid_parse_report_Port2(pUsbData_Port2, 0x65);

   return TRUE;

}
U8 drvUSBHost_HID_Initial_Port2(void)
{
    USB_HID_DBG(printf("HID device\n"));

    GetReportDescriptor_Port2();

    pUsbIntrData_Port2=(U8*) KSEG02KSEG1(UsbIntrData_Port2);
    MY_HAL_DCACHE_FLUSH((U32)UsbIntrData_Port2, 16);
    flib_Host20_Interrupt_Init_Port2(3);

    return 0;
}
U16  MDrv_GET_JOYSTICK_STATUS_Port2(void)
{
    __u16 tmp=0;

    //printf("x1");

    memset(pUsbIntrData_Port2, 0, 8);
     if (flib_Host20_Issue_Interrupt_Port2((U32)pUsbIntrData_Port2,8)==HOST20_OK)
         tmp=Parse_Joystick_Report_Port2(pUsbIntrData_Port2);
// printf("x3");

     return tmp;
}

U8  ReportData_Port2[8];
U8*  MDrv_Get_HID_Report_Raw_Data_Port2(void)
{
    U8  i;

    memset(ReportData_Port2, 0, 8);
    if (flib_Host20_Issue_Interrupt_Port2((U32)pUsbIntrData_Port2,8)==HOST20_OK)
    {
        for (i=0 ; i <8 ; i++)
        {
            ReportData_Port2[i]=pUsbIntrData_Port2[i];
            // printf("%02bx ",ReportData[i]);
        }
    }

    return ReportData_Port2;
}
