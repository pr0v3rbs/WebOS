...
Periodic_Frame_List_Structure  Host20_FramList_Port3 __attribute__ ((aligned (4096)));
Periodic_Frame_List_Structure   *pHost20_FramList_Port3;
qHD_Structure  Host20_qHD_List_Intr_Port3    __attribute__ ((aligned (32)));
qHD_Structure *pHost20_qHD_List_Intr_Port3;
extern  Host20_Attach_Device_Structure sAttachDevice_Port3;
//extern Host20_Attach_Device_Structure  *psAttachDevice_Port3;
extern UINT8 gSpeed_Port3;
extern U8 gUsbStatus_Port3;

extern UINT8 flib_Host20_CheckingForResult_QHD_Port3(qHD_Structure *spTempqHD);
extern void ResetUsbHardwarePort3(void);
extern U8 usb_temp_buf_Port3[Scsi_Max_Transfer_Len];

void SetFrameList_Port3(UINT8 i)
{
    pHost20_FramList_Port3->sCell[i].bTerminal=0;
    pHost20_FramList_Port3->sCell[i].bType=HOST20_HD_Type_QH;
    //SetPointer(&(Host20_FramList.sCell[i].bLinkPointer3),VirtoPhyAddr((UINT16)pHost20_qHD_List_Intr));
    pHost20_FramList_Port3->sCell[i].bLinkPointer = (VirtoPhyAddr((U32)pHost20_qHD_List_Intr_Port3)>>5);
}
...
// * OutPut:
//====================================================================
void  flib_Host20_Interrupt_Init_Port3(UINT8 bAddr)
{
    UINT32 i,j;
    // UINT32 wSize;
    UINT8 *pData;
    ...
    flib_Host20_Periodic_Setting_Port3(HOST20_Disable);

    //<2>.Init qHD for Interrupt(7~9) Scan the ED

    mbHost20_USBCMD_FrameListSize_Set(HOST20_USBCMD_FrameListSize_256);
    pHost20_qHD_List_Intr_Port3=(qHD_Structure*)KSEG02KSEG1(&Host20_qHD_List_Intr_Port3);
    pHost20_FramList_Port3= (Periodic_Frame_List_Structure*) KSEG02KSEG1(&Host20_FramList_Port3);
    //printf("pHost20_qHD_List_Intr: %X\n", pHost20_qHD_List_Intr);
    MY_HAL_DCACHE_FLUSH((U32)&Host20_qHD_List_Intr_Port3, sizeof(qHD_Structure));
    MY_HAL_DCACHE_FLUSH((U32)&Host20_FramList_Port3, sizeof(Periodic_Frame_List_Structure));

    pData=(UINT8*)pHost20_qHD_List_Intr_Port3;
    for (j=0;j<sizeof(qHD_Structure);j++)
        *(pData+j)=0;

    //  printf("battribute:%02bx\n", sAttachDevice.saCD[0].sInterface[0].sED[0].bED_bmAttributes);
    EndPaddr=  sAttachDevice_Port3.saCD[0].sInterface[0].sED[0].bED_EndpointAddress & 0x7f;
    USB_HID_INT_DBG(printf("interrupt addr:%02x\n",EndPaddr));
    flib_Host20_Allocate_QHD_Port3(pHost20_qHD_List_Intr_Port3,HOST20_HD_Type_QH,bAddr,0,EndPaddr,8);//Address=0,Head=1,EndPt=0,Size

    pHost20_qHD_List_Intr_Port3->bHighBandwidth=1;
    pHost20_qHD_List_Intr_Port3->bInterruptScheduleMask=1;
    pHost20_qHD_List_Intr_Port3->bEdSpeed= gSpeed_Port3;
    pHost20_qHD_List_Intr_Port3->bDataToggleControl=0;

    //SetPointer(&(pHost20_qHD_List_Intr->bNextQHDPointer3),VirtoPhyAddr((UINT16)pHost20_qHD_List_Intr));
     pHost20_qHD_List_Intr_Port3->bNextQHDPointer=(VirtoPhyAddr((U32)pHost20_qHD_List_Intr_Port3)>>5);
    ...
    pHost20_qHD_List_Intr_Port3->bTerminate=1;          //terminated
    ...
    pData=(UINT8 *)pHost20_FramList_Port3;
    ...
    for (i=0;i< Host20_Preiodic_Frame_List_MAX ;i++)
        pHost20_FramList_Port3->sCell[i].bTerminal=1;
    //Find the Interval-X
    //  Host20_FramList.sCell[0].bLinkPointer=((UINT32)(pHost20_qHD_List_Intr))>>5;


    SetFrameList_Port3(0);
    SetFrameList_Port3(50);
    SetFrameList_Port3(100);
    SetFrameList_Port3(150);
    SetFrameList_Port3(200);
    ...
    mwHost20_PeriodicBaseAddr_Set(VirtoPhyAddr((U32)pHost20_FramList_Port3));


    //<5>.Enable the periodic
    flib_Host20_Periodic_Setting_Port3(HOST20_Enable);
...
// * OutPut:
//====================================================================
void flib_Host20_Periodic_Setting_Port3(UINT8 bOption)
{
...
UINT8 flib_Host20_Send_qTD2_Port3(qTD_Structure  *spHeadqTD ,qHD_Structure  *spTempqHD,U16 wTimeOutSec)
{
  ...
  sAttachDevice_Port3.bSendStatusError=0;
      ...
      if (MsOS_Timer_DiffTimeFromNow(LastTime) >(U32)300)     //over 1 mini sec
        {
            ...
            if (mwHost20_FrameIndex_Rd()==FrameIdx)
            {
                ...
                gUsbStatus_Port3=USB_TIMEOUT;         //ESD , USB hang,should be timeout
                bReturnValue=HOST20_FATAL;
                break;
            }
            FrameIdx=mwHost20_FrameIndex_Rd();
...
// * OutPut:
//====================================================================
U8  flib_Host20_Issue_Interrupt_Port3(U32 buf,U32 hwSize)
{
    ...
    //<3>.Send TD
    bResult = flib_Host20_Send_qTD2_Port3(spTempqTD ,pHost20_qHD_List_Intr_Port3,3);
