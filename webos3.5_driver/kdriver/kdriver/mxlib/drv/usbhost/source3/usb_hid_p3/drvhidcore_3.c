...
#if 0
U8 code USBHID_P3_VBuf[32] = {'M', 'S', 'V', 'C', '0', '0',			    // 0, 0 fixed
                        'U', '3',						            // Library ID
...
#define USB_HID_P3_DRV_VERSION                  /* Character String for DRV/API version             */  \
    MSIF_TAG,                           /* 'MSIF'                                           */  \
    MSIF_CLASS,                         /* '00'                                             */  \
    MSIF_CUS,                           /* 0x0000                                           */  \
    MSIF_MOD,                           /* 0x0000                                           */  \
...
static MSIF_Version _drv_usb_hid_p3_version = {
    .DDI = { USB_HID_P3_DRV_VERSION },
};

/*
 * Read data value from item.
 */
U8 UsbIntrData_Port3[16] __attribute__ ((aligned (16))) ;
U8 *pUsbIntrData_Port3;
extern UINT8  pUsbData_Port3[1024];
struct   axis_struct  axis_Port3[MAX_AXIS_NUM];
__u8     axis_idx_Port3;
struct   button_struct  buttons_Port3;
__u16  gOffsetBit_Port3;

U8 MDrv_USB_HID_GetLibVer_Port3(const MSIF_Version **ppVersion)
{
    if(!ppVersion)
        return 1;

    *ppVersion = &_drv_usb_hid_p3_version;
    return 0;
}

__u32 item_udata_Port3(struct hid_item *item)
{
...
static  __s32 item_sdata_Port3(struct hid_item *item)
{
...
static __u8 *fetch_item_Port3(__u8 *start, __u8 *end, struct hid_item *item)
{
...
static int open_collection_Port3(struct hid_parser *parser, unsigned type)
{
...
static int close_collection_Port3(struct hid_parser *parser)
{
...
static int hid_chk_variable_Port3(struct hid_parser *parser, unsigned report_type)
{
    ...
    {
        buttons_Port3.startbit=gOffsetBit_Port3;
        buttons_Port3.count=parser->global.report_size * rpt_count;
        USB_HID_DBG(printf("Usage Page (buttons): %X size: %X\n", buttons_Port3.startbit, buttons_Port3.count));

        gOffsetBit_Port3+=buttons_Port3.count;
        goto Func_Done;
    }
    ...
    if (usg_count > 0)
    {
        for(ii=usg_count-rpt_count; ii<usg_count; ii++)
        {
            ...
            if (report_type == HID_INPUT_REPORT)
            {
                ...
                {
                     axis_Port3[axis_idx_Port3].startbit=gOffsetBit_Port3;
                     axis_Port3[axis_idx_Port3].size=parser->global.report_size;
                     axis_Port3[axis_idx_Port3].type=TYPE_AXIS_X;
                     gOffsetBit_Port3+=axis_Port3[axis_idx_Port3].size;
                ...
                else if (parser->local.usage[ii] == USEAGE_Y)
                {
                     axis_Port3[axis_idx_Port3].startbit=gOffsetBit_Port3;
                     axis_Port3[axis_idx_Port3].size=parser->global.report_size;
                     axis_Port3[axis_idx_Port3].type=TYPE_AXIS_Y;
                     gOffsetBit_Port3+=axis_Port3[axis_idx_Port3].size;
 ...
 * Process a main item.
 */
 int hid_parser_main_Port3(struct hid_parser *parser, struct hid_item *item)
{
	__u32 adata;
	int ret=0;
...
#endif
			ret = open_collection_Port3(parser, adata & 3);
			break;
		case HID_MAIN_ITEM_TAG_END_COLLECTION:
			ret = close_collection_Port3(parser);
			break;
		case HID_MAIN_ITEM_TAG_INPUT:
                        ret = hid_chk_variable_Port3(parser, HID_INPUT_REPORT);
			//ret = hid_add_field(parser, HID_INPUT_REPORT, data);
			break;
		case HID_MAIN_ITEM_TAG_OUTPUT:
                        ret = hid_chk_variable_Port3(parser, HID_OUTPUT_REPORT);
			//ret = hid_add_field(parser, HID_OUTPUT_REPORT, data);
			break;
		case HID_MAIN_ITEM_TAG_FEATURE:
                        ret = hid_chk_variable_Port3(parser, HID_FEATURE_REPORT);
 ...
 */
 ...
 int hid_parser_global_Port3(struct hid_parser *parser, struct hid_item *item)
{
   // printf("  hid_parser_global   \n");
	switch (item->tag) {
		...
		case HID_GLOBAL_ITEM_TAG_USAGE_PAGE:
			parser->global.usage_page = item_udata_Port3(item);
		...
		case HID_GLOBAL_ITEM_TAG_LOGICAL_MINIMUM:
			parser->global.logical_minimum = item_sdata_Port3(item);
			return 0;

		case HID_GLOBAL_ITEM_TAG_LOGICAL_MAXIMUM:
			if (parser->global.logical_minimum < 0)
				parser->global.logical_maximum = item_sdata_Port3(item);
			else
				parser->global.logical_maximum = item_udata_Port3(item);
			return 0;

		case HID_GLOBAL_ITEM_TAG_PHYSICAL_MINIMUM:
			parser->global.physical_minimum = item_sdata_Port3(item);
			return 0;

		case HID_GLOBAL_ITEM_TAG_PHYSICAL_MAXIMUM:
			if (parser->global.physical_minimum < 0)
				parser->global.physical_maximum = item_sdata_Port3(item);
			else
				parser->global.physical_maximum = item_udata_Port3(item);
			return 0;

		case HID_GLOBAL_ITEM_TAG_UNIT_EXPONENT:
			parser->global.unit_exponent = item_udata_Port3(item);
			return 0;

		case HID_GLOBAL_ITEM_TAG_UNIT:
			parser->global.unit = item_udata_Port3(item);
			return 0;

		case HID_GLOBAL_ITEM_TAG_REPORT_SIZE:
			if ((parser->global.report_size = item_udata_Port3(item)) > 32) {
		...
		case HID_GLOBAL_ITEM_TAG_REPORT_COUNT:
			if ((parser->global.report_count = item_udata_Port3(item)) > HID_MAX_USAGES) {
		...
		case HID_GLOBAL_ITEM_TAG_REPORT_ID:
			if ((parser->global.report_id = item_udata_Port3(item)) == 0) {
 ...
 int hid_parser_local_Port3(struct hid_parser *parser, struct hid_item *item)
{
	__u32 adata;
...
void hid_parse_report_Port3(__u8 *start, unsigned size)
{
	__u8 *end;
       int result = 0;
  ...
  // printf("@@@ >>> Enter hid-core.c file --> hid_parse_report function \n");
	...
	while ((start = fetch_item_Port3(start, end, &gitem)) != 0)
        {
...
//                start, gitem.format, gitem.size, gitem.type, gitem.tag, gitem.zdata.u8);
        ...
        if (gitem.type==0)
            result=hid_parser_main_Port3(&gparser,&gitem);
        else if (gitem.type==1)
            result=hid_parser_global_Port3(&gparser,&gitem);
        else if (gitem.type==2)
            result=hid_parser_local_Port3(&gparser,&gitem);
...
U16 Parse_Joystick_Report_Port3(__u8 *report)
{
__u8 i,tmp;
__u16  tmp2,result;
 //printf("t1");
		...
		//printf("%02bx ", report[i]);
            for (i=0 ; i < MAX_AXIS_NUM ; i ++)
           {
               if (axis_Port3[i].size)                //this AXIS exist
                {
                        tmp=GetValue_Port3(report,axis_Port3[i].startbit,axis_Port3[i].size);
                        if (axis_Port3[i].val!=tmp )
                            {
                                 USB_HID_DBG(printf(" axis idx %x, offset:%x ,val %02x  tmp:%02x\n", i,axis_Port3[i].startbit,axis_Port3[i].val,tmp));
                                 axis_Port3[i].val=tmp;
                                 result=(axis_Port3[i].type << 12) | tmp;
                                 return result;
...
// printf("t2");
		//printf("button start:%x",buttons.startbit);
            tmp2=GetValue_Port3(report,buttons_Port3.startbit,buttons_Port3.count);
           // printf("tmp2:%x\n",tmp2);
            if (buttons_Port3.val!=tmp2)
                {
...
__u32 GetValue_Port3(__u8 *buf, __u16 offset,__u8 size)
{
...
code UINT8  GET_REPORT_DESCRIPTOR_PORT3[]        = {0x81,0x06,0x00,0x22,0x00,0x00,0x65,0x00};

BOOLEAN GetReportDescriptor_Port3(void)
{
    UINT8 bCMD[8];
    UINT8 i;

   memcpy(bCMD,GET_REPORT_DESCRIPTOR_PORT3,8);
   if ( flib_Host20_Issue_Control_Port3 (1,bCMD,0x65,pUsbData_Port3)>0)
       return FALSE;
    ...
    buttons_Port3.count = 0;
    buttons_Port3.startbit = 0;
    buttons_Port3.val = 0;

   hid_parse_report_Port3(pUsbData_Port3, 0x65);

   return TRUE;

}
U8 drvUSBHost_HID_Initial_Port3(void)
{
    USB_HID_DBG(printf("HID device\n"));

    GetReportDescriptor_Port3();

    pUsbIntrData_Port3=(U8*) KSEG02KSEG1(UsbIntrData_Port3);
    MY_HAL_DCACHE_FLUSH((U32)UsbIntrData_Port3, 16);
    flib_Host20_Interrupt_Init_Port3(3);

    return 0;
}
U16  MDrv_GET_JOYSTICK_STATUS_Port3(void)
{
    __u16 tmp=0;

    //printf("x1");

    memset(pUsbIntrData_Port3, 0, 8);
     if (flib_Host20_Issue_Interrupt_Port3((U32)pUsbIntrData_Port3,8)==HOST20_OK)
         tmp=Parse_Joystick_Report_Port3(pUsbIntrData_Port3);
// printf("x3");

     return tmp;
}

U8  ReportData_Port3[8];
U8*  MDrv_Get_HID_Report_Raw_Data_Port3(void)
{
    U8  i;

    memset(ReportData_Port3, 0, 8);
    if (flib_Host20_Issue_Interrupt_Port3((U32)pUsbIntrData_Port3,8)==HOST20_OK)
    {
        for (i=0 ; i <8 ; i++)
        {
            ReportData_Port3[i]=pUsbIntrData_Port3[i];
            // printf("%02bx ",ReportData[i]);
        }
    }

    return ReportData_Port3;
}
