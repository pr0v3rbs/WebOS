...
#define _TSP_FLAG_FREE_TIMEOUT                              1000
...
#define _TSP_ENTRY()                                    do { \
                                                            if (!OSAL_TSP_MutexObtain(_ptsp_res->_s32TspMutexId , OSAL_TSP_MUTEX_TIMEOUT))      \
                                                            {                                                                       \
                                                                _ptsp_res->_u32LastErr = DRVTSP_FUNC_ERROR;                                    \
                                                                return DRVTSP_FUNC_ERROR;                                           \
                                                            }                                                                       \
                                                            if (_ptsp_res->_bSuspend)                                                          \
                                                            {                                                                       \
...
#define _TSP_RETURN(_ret)                               do { \
                                                            _TSP_IND_UNLOCK();                                                      \
                                                            OSAL_TSP_MutexRelease(_ptsp_res->_s32TspMutexId );                                  \
                                                            if (_ret != DRVTSP_OK) { _ptsp_res->_u32LastErr = _ret; }                          \
                                                               return _ret;                                                         \
                                                        }while(0);   // Should be optimized by compiler

#define _TSP_IND_LOCK()                                 OSAL_TSP_MutexObtain(_ptsp_res->_s32IndMutexId, OSAL_TSP_MUTEX_TIMEOUT)
#define _TSP_IND_UNLOCK()                               OSAL_TSP_MutexRelease(_ptsp_res->_s32IndMutexId)

#define _TSP_INT_LOCK()                                 OSAL_TSP_MutexObtain(_ptsp_res->_s32IntRegMutexId, OSAL_TSP_MUTEX_TIMEOUT)
#define _TSP_INT_UNLOCK()                               OSAL_TSP_MutexRelease(_ptsp_res->_s32IntRegMutexId)
...
static TSP_RESOURCE_PRIVATE     _tsp_res = {
    ...
    #ifdef VQ_ENABLE
    ._u32NULLFileFilterID               = 0xFFFFFFFF,
    #endif
    ...
    .TSPPvrcaParams                     = {
        ...
        ._u32Flt1Flag_DscmbPVR          = 0,
        ._u32Flt1Flag_DscmbRef          = 0,
...
static MS_U32                   _u32SecNotifyId[(TSP_SECFLT_NUM_All+31)>>5] = {0}; //for ISR processing
...
#if (defined(HWPCR_ENABLE) && (TSP_HWPCR_BY_HK == 1))
static MS_U32                   _u32HwPcrNotifyId[(TSP_PCRFLT_NUM+31)>>5] = {0};
static MS_U8                    _u8ResetPcr[TSP_PCRFLT_NUM] = {[0 ... (TSP_PCRFLT_NUM-1)] = 3};
static MS_U32                   _u32PrevStcBase[TSP_PCRFLT_NUM] = {[0 ... (TSP_PCRFLT_NUM-1)] = 0};
static MS_U32                   _u32PrevPllControl[TSP_PCRFLT_NUM] = {[0 ... (TSP_PCRFLT_NUM-1)] = 0};
#endif

static MS_S32                   _s32TspEventId = -1;
static MS_S32                   _s32TspSecRdyId[(TSP_SECFLT_NUM_All+31)>>5] = {[0 ... (((TSP_SECFLT_NUM_All+31)>>5)-1)] = -1};
static MS_S32                   _s32TspSecOvfId[(TSP_SECFLT_NUM_All+31)>>5] = {[0 ... (((TSP_SECFLT_NUM_All+31)>>5)-1)] = -1};
static MS_S32                   _s32TspSecCrcId[(TSP_SECFLT_NUM_All+31)>>5] = {[0 ... (((TSP_SECFLT_NUM_All+31)>>5)-1)] = -1};
static MS_S32                   _s32TspFltDisableId[(TSP_SECFLT_NUM_All+31)>>5] = {[0 ... (((TSP_SECFLT_NUM_All+31)>>5)-1)] = -1};
...
static MS_BOOL _TSP_InitResource(TSP_RESOURCE_PRIVATE* presource)
{
    ...
    #ifdef VQ_ENABLE
    presource->_u32NULLFileFilterID = 0xFFFFFFFF;
    #endif
    ...
    presource->TSPPvrcaParams._u32Flt1Flag_DscmbPVR = 0;
    presource->TSPPvrcaParams._u32Flt1Flag_DscmbRef = 0;
...
static void _TSP_SW_Exit(void)
{
    ...
    while(_s32TspTaskId > 0)
    {
        ...
        {
            MS_DEBUG_MSG(TSP_DBG("[%s][%d] TSP task exit timeout!!!!!!\n", __FUNCTION__, __LINE__));
            break;
        }
...
static MS_BOOL _TSP_DumpDscmbDupArray(void)
{
    ...
    printf("_u32Flt1Flag_DscmbRef %lx , _u32Flt1Flag_DscmbPVR %lx\n",
...
static MS_BOOL _TSP_FreeDupDscmbPidFlt(MS_U32 u32EngId, MS_U32 u32FltGroupId, MS_U32 u32CaMode)
{
    ...
    {
        PVRCA_DBG(printf("[%s][%d] Free FLT0 list \n", __FUNCTION__, __LINE__));        
    ...
    {
        PVRCA_DBG(printf("[%s][%d] Free FLT2 list \n", __FUNCTION__, __LINE__));
...
static MS_BOOL _TSP_DupDscmbPidFlt(MS_U32 u32EngId, MS_U32 OrgFltId, MS_U32 DupFltId)
{
    ...
    PVRCA_DBG(printf("[%s][%d] OrgFltId %ld, DupFltId %ld, FltType %lx\n",
    ...
    PVRCA_DBG(printf("[%s][%d] Org pid %lx\n", __FUNCTION__, __LINE__, u32pid));
    ...
    if(OrgFltId <=  DSCMB_FLT_END_ID)
    {
        ...
        PVRCA_DBG(printf("[%s][%d] #1 u32OrgFltListIdx %ld \n" ,__FUNCTION__, __LINE__, u32OrgFltListIdx));
        ...
        for(u32ii = 0; u32ii < u32DscmbFltNum; u32ii++)
        {
            if(_ptsp_res->TSPPvrcaParams._u16Flt1PidList[u32ii] == u32pid)
            {
                ...
                PVRCA_DBG(printf("[%s] Repeate: OrgFltId %ld, DupFltId %ld, u32pid 0x%lx\n", __FUNCTION__, OrgFltId, u32ii+TSP_SECFLT_NUM, u32pid));
                return TRUE;
            }
        }
    ...
    else if((OrgFltId >=  (TSP_SECFLT_NUM+TSP_PIDFLT1_NUM)) &&  (OrgFltId < (TSP_SECFLT_NUM+TSP_PIDFLT1_NUM + DSCMB_FLT_NUM)))//Group TS2 -> Group TS1
    {
        ...
        PVRCA_DBG(printf("[%s][%d] #2 u32OrgFltListIdx %ld \n" , __FUNCTION__, __LINE__, u32OrgFltListIdx));
    ...
    {
        PVRCA_DBG(printf("[%s][%d] 333 OrgFltId %ld \n" , __FUNCTION__, __LINE__, OrgFltId));
        return FALSE;
    }
    ...
    PVRCA_DBG(printf("[%s][%d] OrgFltId %ld, DupFltId %ld, u32pid 0x%lx\n", __FUNCTION__, __LINE__, OrgFltId, DupFltId, u32pid));
...
static MS_BOOL _TSP_FLT1_DupFLT0_Pid(MS_U32 u32EngId, MS_U32 u32Flt0Id, MS_U32 u32Pid)
{
    ...
    if(_ptsp_res->TSPPvrcaParams._bFLT1Dup_Enable == FALSE)
    {
        ...
        PVRCA_DBG(printf("[%s] u32Flt0Id %ld, New PID 0x%lx, Old PID 0x%x\n",
        ...
        if(u32Pid == DRVTSP_PID_NULL)
        {
            ...
            for(u32ii = 0; u32ii < u32DscmbFltNum; u32ii++)
            {
                ...
                if(_ptsp_res->TSPPvrcaParams._u16Flt0DscmbPidList[u32Flt0ListId] == _ptsp_res->TSPPvrcaParams._u16Flt1PidList[u32ii])
                {
                    ...
                    PVRCA_DBG(printf("[%s] Free Null pid, u32Flt0Id %ld, Flt1Id 0x%ld\n", __FUNCTION__, u32Flt0Id, u32ii));
                    return TRUE;
                }
            }
        ...
        if(u32ii < u32DscmbFltNum)
        {
            ...
            PVRCA_DBG(printf("[%s]Du32Flt0ListId %ld , u32Flt1_id %ld ---------------------\n", __FUNCTION__, u32Flt0ListId, u32ii));
...
UPDATE_PARAM:
        ...
        PVRCA_DBG(printf("[%s] u32Flt0Id %ld, u32Flt1_id %ld, PID 0x%lx\n", __FUNCTION__, u32Flt0Id, u32Flt1_id, u32Pid));

        return TRUE;
    }

    PVRCA_DBG(printf("[%s] u32Flt0Id %ld, PID 0x%lx\n", __FUNCTION__, u32Flt0Id, u32Pid));
    ...
    PVRCA_DBG(printf("[%s][%d] u32Flt0Id %ld, PID 0x%lx\n", __FUNCTION__, __LINE__, u32Flt0Id, u32Pid));
    ...
    if(bFreeDscmbPid)
    {
        ...
        PVRCA_DBG(printf("_u32Flt1Flag_DscmbRef 0x%lx, _u32Flt1Flag_DscmbPVR 0x%lx\n", \
                         _ptsp_res->TSPPvrcaParams._u32Flt1Flag_DscmbRef, _ptsp_res->TSPPvrcaParams._u32Flt1Flag_DscmbPVR));
        PVRCA_DBG(printf("#1 _u16Flt0DscmbPidList[%ld] 0x%x\n", u32Flt1_id, _ptsp_res->TSPPvrcaParams._u16Flt0DscmbPidList[u32Flt1_id]));
        return TRUE;
    }
    ...
    for(u32ii = 0; u32ii < u32DscmbFltNum; u32ii++)
    {
        ...
        if(_ptsp_res->TSPPvrcaParams._u16Flt1PidList[u32ii] == u32Pid)
        {
            ...
            PVRCA_DBG(printf("_u32Flt1Flag_DscmbRef 0x%lx, _u32Flt1Flag_DscmbPVR 0x%lx\n", \
                             _ptsp_res->TSPPvrcaParams._u32Flt1Flag_DscmbRef, _ptsp_res->TSPPvrcaParams._u32Flt1Flag_DscmbPVR));
            PVRCA_DBG(printf("#2  _u16Flt0DscmbPidList[%ld] 0x%x, _u16Flt1PidList[%ld] 0x%x\n", \
    ...
    if(u32ii == u32DscmbFltNum)
    {
        ...
        if(u32ii < u32DscmbFltNum)
        {
            ...
            PVRCA_DBG(printf("_u32Flt1Flag_DscmbRef 0x%lx, _u32Flt1Flag_DscmbPVR 0x%lx\n", \
                             _ptsp_res->TSPPvrcaParams._u32Flt1Flag_DscmbRef, _ptsp_res->TSPPvrcaParams._u32Flt1Flag_DscmbPVR));
            PVRCA_DBG(printf("#3  _u16Flt0DscmbPidList[%ld] 0x%x, _u16Flt1PidList[%ld] 0x%x\n", \
                             u32Flt1_id, _ptsp_res->TSPPvrcaParams._u16Flt0DscmbPidList[u32Flt1_id], u32ii, _ptsp_res->TSPPvrcaParams._u16Flt1PidList[u32ii]));
            return TRUE;
        }
        else
...
static MS_BOOL _TSP_FLT1_DupFLT2_Pid(MS_U32 u32EngId, MS_U32 u32Flt2Id, MS_U32 u32Pid)
{
    ...
    PVRCA_DBG(printf("[%s] u32Flt2Id %ld, New PID 0x%lx, Old PID 0x%x\n",
    ...
    if(u32Pid == DRVTSP_PID_NULL)
    {
        ...
        for(u32ii = 0; u32ii < DSCMB_FLT_NUM; u32ii++)
        {
            ...
            if(_ptsp_res->TSPPvrcaParams._u16Flt2PidList[u32ArrayIdx] == _ptsp_res->TSPPvrcaParams._u16Flt1PidList[u32ii])
            {
                ...
                if(((_ptsp_res->TSPPvrcaParams._u32Flt1Flag_DscmbPVR|_ptsp_res->TSPPvrcaParams._u32Flt1Flag_DscmbRef) & (1 << u32ii)) != 0)
                {
                    PVRCA_DBG(printf("[%s][%d] Free Null pid used, u32Flt2Id %ld, Flt1Id 0x%ld\n", __FUNCTION__, __LINE__, u32ArrayIdx, u32ii));
                    return FALSE;
                }
                PVRCA_DBG(printf("[%s] Free Null pid, u32Flt2Id %ld, Flt1Id 0x%ld\n", __FUNCTION__, u32ArrayIdx, u32ii));
                HAL_TSP_PidFlt_SetPid(HAL_TSP_GetPidFltReg(u32EngId, u32ii + TSP_SECFLT_NUM), DRVTSP_PID_NULL);
                _ptsp_res->TSPPvrcaParams._u16Flt1PidList[u32ii] = DRVTSP_PID_NULL;
                return TRUE;
            }
    ...
    if(u32ii < DSCMB_FLT_NUM)
    {
        ...
        PVRCA_DBG(printf("[%s][%d] u32Flt2ListId %ld , u32Flt1_id %ld ---------------------\n",
            __FUNCTION__, __LINE__, u32ArrayIdx, u32ii));
        return TRUE;
    }
    //---  1st, end...
...
SET_PID:
    ...
    _ptsp_res->TSPPvrcaParams._u16Flt1PidList[u32FLt1Id] = u32Pid;
    _ptsp_res->TSPPvrcaParams._u16Flt2PidList[u32ArrayIdx] = u32Pid;
    PVRCA_DBG(printf("[%s][%d] u32Flt2Id %ld, u32Flt1Id %ld, PID 0x%lx\n", __FUNCTION__, __LINE__, u32Flt2Id, u32FLt1Id, u32Pid));
    return TRUE;
...
static MS_BOOL _TSP_PidFlt_Alloc(MS_U32 u32EngId, DrvTSP_FltType eFilterType, MS_U32 *pu32PidFltId)
{
    ...
    *pu32PidFltId = 0xFFFFFFFF;
    ...
    {
        TSP_ASSERT(0, TSP_DBG("[TSP_ERROR][%06d] (EngId, FilterType)= (%ld %lx)\n", __LINE__, u32EngId, eFilterType));
        return FALSE;
    }
...
static MS_BOOL _TSP_SecFlt_StateGet(MS_U32 u32EngId, MS_U32 u32SecFltId, DrvTSP_FltState *peState)
{
    ...
    TSP_ASSERT(TSP_ENGINE_NUM> u32EngId, TSP_DBG("[TSP_ERROR][%06d] Bad Engine Id %ld\n", __LINE__, u32EngId));
    TSP_ASSERT(TSP_SECFLT_NUM_All> u32SecFltId, TSP_DBG("[TSP_ERROR][%06d] Bad SecFltId %ld\n", __LINE__, u32SecFltId));
    TSP_ASSERT(peState, TSP_DBG("[TSP_ERROR][%06d] NULL pointer\n", __LINE__));
...
static void _TSP_SecBuf_Init(MS_U32 u32EngId, MS_U32 u32SecBufId)
{
    ...
    HAL_TSP_SecBuf_SetBuffer(pSecBuf, 0, 0);
...
static MS_BOOL _TSP_FLT1_Duplicate_FLT0(MS_U32 u32EngId)
{
    ...
    for(u32ii = 0; u32ii < TSP_PIDFLT1_NUM; u32ii++)
    {
        ...
        PVRCA_DBG(printf("[%s]u32Flt0Id %ld\n", __FUNCTION__, u32Flt0Id));
        ...
        PVRCA_DBG(printf("[%s]u32Flt0Id %ld, u32PID %lx \n", __FUNCTION__, u32Flt0Id, u32PID));
...
#endif
{
    ...
    //printf("u32HwInt %lx, u32SwInt %lx\n", u32HwInt, u32SwInt);
    ...
    if (u32SwInt & TSP_SWINT_CTRL_FIRE)
    {
        ...
        case TSP_SWINT_STATUS_SEC_CRCERR:
            ...
            {
                break;
               // _TSP_SetSecCrcId(u32EngId, (1 << u32SecFltId));             // Set section event flag
            }
            else
            {
...
static MS_BOOL _TSP_Isr_Proc_SecRdyCrc(MS_U32 u32EngId, MS_U32 u32FltId, MS_U32 u32CbOpt, MS_U32* pu32Evt)
{
    ...
    *pu32Evt = (MS_U32)E_DRVTSP_EVENT_DATA_INIT;
    ...
    if (TSP_PIDFLT_NUM_ALL == u32PIDFlt)
        goto jump_bad_state;
    ...
    u32Read = HAL_TSP_SecBuf_GetBufRead(pSecFilter);
    bData = (u32Read != HAL_TSP_SecBuf_GetBufWrite(pSecFilter))? TRUE: FALSE; // critical section
    _TSP_IND_UNLOCK();
    ...
    {       
        ...
        else
        {
            ...
            if ((u32EvtNotify & E_DRVTSP_EVENT_DATA_READY) && pfEvtCallback)
            {
                ...
                _TSP_IND_LOCK();
                if (u32Read == HAL_TSP_SecBuf_GetBufRead(pSecFilter))
                {
                    _TSP_GetSecRdyId(u32FltId, &u32Event, OSAL_EVENT_OR_CLEAR);
                }
                _TSP_IND_UNLOCK();              
...
static MS_BOOL _TSP_Isr_Proc_SecOvf(MS_U32 u32EngId, MS_U32 u32SecFltId, MS_U32 u32CbOpt, MS_U32* pu32Evt)
{
    ...
    *pu32Evt = (MS_U32)E_DRVTSP_EVENT_DATA_INIT;
    ...
    for (u32PidFltId = 0; u32PidFltId < TSP_PIDFLT_NUM_ALL; u32PidFltId++)
    {
        ...
        for (j = 0; j < u32GroupNum; j++)
        {
            ...
            while (u32AskFreeFltList)
            {
                ...
                if (_ptsp_res->_TspState[0].FltState[u32PidFltId] != E_DRVTSP_FLT_STATE_FREE)
                {
                    ...
                    _TSP_PidFlt_Free(0, u32PidFltId, TRUE);
...
static TSP_Result _TSP_Init(MS_BOOL bResetSwState, MS_BOOL bRestoreRegState)
{
    ...
    MS_U32 u32IRBank, u32IrBankSize;
    MS_BOOL bAlive;
    ...
    HAL_TSP_SetBank(u32TspBank, u32IRBank);
...
//#endif
    ...
    {
#ifdef VQ_ENABLE
        _ptsp_res->_u32NULLFileFilterID = 0xFFFFFFFF;
#endif
...
TSP_Result MDrv_TSP_Suspend(void)
{
    ...
    MDrv_TSP_GetTSIFStatus(E_DRVTSP_IF_PLAYBACK, &(_ptsp_res->_ifset_save));
...
TSP_Result MDrv_TSP_Resume(MS_U32 u32FWAddr, MS_U32 u32FWSize)
{
    ...
    _TSP_ENTRY();
    _TSP_SelPad(0, E_DRVTSP_IF_PLAYBACK, &(_ptsp_res->_ifset_save));
    _TSP_RETURN(ret);
}
...
// For new VQ architecture with file VQ, not need to create dummy file filter for switch section buffer path
    ...
    case E_DRVTSP_CTRL_MODE_TS0:
        ...
        PVRCA_DBG(printf("[%s][%d] Org u32CaMode %lx\n", __FUNCTION__, __LINE__, u32CaMode));
        if((u32CaMode & (TSP_CA_INPUT_TSIF1|TSP_CA_OUTPUT_PLAY_LIVE)) != (TSP_CA_INPUT_TSIF1|TSP_CA_OUTPUT_PLAY_LIVE)) //if TS1 dscmb, not change it...
        {
            if((u32CaMode & TSP_CA_OUTPUT_REC) == 0)
            {
                ...
                PVRCA_DBG(printf("[%s][%d] u32CaMode %lx\n", __FUNCTION__, __LINE__, u32CaMode));
            }
        }
    ...
    case E_DRVTSP_CTRL_MODE_MEM_NOCA:
        ...
        PVRCA_DBG(printf("[%s][%d] _ptsp_res->_u32OrgCAMode 0x%lx\n", __FUNCTION__, __LINE__, _ptsp_res->_u32OrgCAMode));
...
{
    _TSP_ENTRY();
    TSP_ASSERT2(TSP_PIDFLT_NUM_ALL> u32FltId, TSP_DBG("[TSP_ERROR][%06d] Bad FltId %ld\n", __LINE__, u32FltId));
...
TSP_Result MDrv_TSP_PidFlt_Alloc(MS_U32 u32EngId, DrvTSP_FltType eFilterType, MS_U32 *pu32PidFltId)
{
    ...
    if(MDrv_SYS_Query(E_SYS_QUERY_DTV_SUPPORTED) == FALSE)
    {
        ...
        {
            // block DTV function
            printf("[TSP_ERROR][%06d] DTV function not supported on this chip\n", __LINE__);
...
TSP_Result MDrv_TSP_PidFlt_Free(MS_U32 u32EngId, MS_U32 u32PidFltId)
{
    ...
    else if ((_ptsp_res->_TspState[u32EngId].SecFltEvtNotify[u32SecFltId] & E_DRVTSP_EVENT_CB_MASK) == E_DRVTSP_EVENT_CB_SELF)
    {
        ...
        if(_TSP_FLAG_GetDone(u32PidFltId, &u32Flag, OSAL_EVENT_OR_CLEAR) == TRUE)
        {
            tRes = DRVTSP_OK;                        
        }
        else
        {
            printf("Free filter fail %ld, %ld\n", u32PidFltId, MsOS_GetSystemTime()); 
        }
    }
...
TSP_Result MDrv_TSP_PidFlt_SelSecFlt(MS_U32 u32EngId, MS_U32 u32PidFltId, MS_U32 u32SecFltId)
{
    ...
    TSP_ASSERT2(TSP_PIDFLT_NUM_ALL> u32PidFltId, TSP_DBG("[TSP_ERROR][%06d] Bad PidFltId %ld\n", __LINE__, u32PidFltId));
    TSP_ASSERT2(E_DRVTSP_FLT_STATE_FREE!= _ptsp_res->_TspState[u32EngId].FltState[u32PidFltId], TSP_DBG("[TSP_ERROR][%06d] Bad Flt state\n", __LINE__));
    TSP_ASSERT2(TSP_SECFLT_NUM_All> u32SecFltId, TSP_DBG("[TSP_ERROR][%06d] SecFltId= %ld\n", __LINE__, u32SecFltId));
...
TSP_Result MDrv_TSP_PidFlt_Enable(MS_U32 u32EngId, MS_U32 u32PidFltId, MS_BOOL bEnable)
{
    ...
    else if (E_DRVTSP_FLT_TYPE_CB == _ptsp_res->_TspState[u32EngId].FltType[u32PidFltId])
    {
        ...
        #endif
        TSP_ASSERT2((TSP_PIDFLT_BASE0_NUM+TSP_PIDFLT_BASE1_NUM) <= u32PidFltId, TSP_DBG("[TSP_ERROR][%06d] Bad PidFltId %ld\n", __LINE__, u32PidFltId));
    }
#endif
    ...
    else
    {
        ...
        case E_DRVTSP_FLT_TYPE_TELETEXT:
        case E_DRVTSP_FLT_TYPE_SECTION_VER:
            TSP_ASSERT2(TSP_SECFLT_NUM_All> u32SecBufId, TSP_DBG("[TSP_ERROR][%06d] SecBufId= %ld\n", __LINE__, u32SecBufId));
...
TSP_Result MDrv_TSP_SecFlt_Alloc(MS_U32 u32EngId, MS_U32 *pu32SecFltId)
{
    ...
    TSP_ASSERT2(TSP_SECFLT_NUM_All> *pu32SecFltId, TSP_DBG("[TSP_ERROR][%06d] Bad SecFltId %ld\n", __LINE__, *pu32SecFltId));
...
TSP_Result MDrv_TSP_SecFlt_Free(MS_U32 u32EngId, MS_U32 u32SecFltId)
{
    ...
    TSP_ASSERT2(TSP_ENGINE_NUM> u32EngId, TSP_DBG("[TSP_ERROR][%06d] Bad Engine Id %ld\n", __LINE__, u32EngId));
    TSP_ASSERT2(TSP_SECFLT_NUM_All> u32SecFltId, TSP_DBG("[TSP_ERROR][%06d] Bad SecFltId %ld\n", __LINE__, u32SecFltId));
...
TSP_Result MDrv_TSP_SecFlt_SetBuffer(MS_U32 u32EngId, MS_U32 u32SecFltId, MS_PHYADDR u32StartAddr, MS_U32 u32BufSize)
{
    ...
    HAL_TSP_SecBuf_SetBuffer(pSecFilter, u32StartAddr, u32BufSize);
    _TSP_RETURN(DRVTSP_OK);
}
...
TSP_Result MDrv_TSP_SecFlt_GetBufStart(MS_U32 u32EngId, MS_U32 u32SecFltId, MS_PHYADDR *pu32BufStart)
{
    ...
    *pu32BufStart= HAL_TSP_SecBuf_GetBufStart(pSecFilter);
    _TSP_RETURN(DRVTSP_OK);
}
...
TSP_Result MDrv_TSP_SecFlt_GetBufSize(MS_U32 u32EngId, MS_U32 u32SecFltId, MS_U32 *pu32BufSize)
{
    ...
    *pu32BufSize=  HAL_TSP_SecBuf_GetBufEnd(pSecFilter)- HAL_TSP_SecBuf_GetBufStart(pSecFilter);
    _TSP_RETURN(DRVTSP_OK);
}
...
TSP_Result MDrv_TSP_SecFlt_GetReadAddr(MS_U32 u32EngId, MS_U32 u32SecFltId, MS_PHYADDR *pu32ReadAddr)
{
    ...
    *pu32ReadAddr= (MS_PHYADDR)HAL_TSP_SecBuf_GetBufRead(pSecFilter);
    _TSP_RETURN(DRVTSP_OK);
}
...
TSP_Result MDrv_TSP_SecFlt_GetWriteAddr(MS_U32 u32EngId, MS_U32 u32SecFltId, MS_PHYADDR *pu32WriteAddr)
{
    ...
    *pu32WriteAddr= (MS_PHYADDR)HAL_TSP_SecBuf_GetBufWrite(pSecFilter);
    _TSP_RETURN(DRVTSP_OK);
}
...
TSP_Result MDrv_TSP_SecFlt_SetReadAddr(MS_U32 u32EngId, MS_U32 u32SecFltId, MS_PHYADDR u32ReadAddr)
{
    ...
    HAL_TSP_SecBuf_SetBufRead(pSecFilter, (MS_U32)u32ReadAddr);
    _TSP_RETURN(DRVTSP_OK);
}
...
TSP_Result MDrv_TSP_TTX_SecFlt_GetWriteAddr(MS_U32 u32EngId, MS_U32 u32SecFltId, MS_PHYADDR *pu32WriteAddr)
{
    ...
    *pu32WriteAddr= (MS_PHYADDR)HAL_TSP_SecBuf_GetBufWrite_tmp(pSecFilter);
    return DRVTSP_OK;
}
...
#ifndef NO_PVR
    _TSP_ENTRY();
    HAL_TSP_PVR_SetBuffer((MS_U32)u32BufStart0, (MS_U32)u32BufStart1, u32BufSize0, u32BufSize1);
...
TSP_Result MDrv_TSP_PVR_PidFlt_Reserved(MS_U32 Pid, MS_U8* pu8DmxId, MS_BOOL bReserved)
{
    ...
    if(!bReserved)
    {
        ...
        PVRCA_DBG(printf("[%s] Unreserved FLT %ld _u32Flt1Flag_DscmbPVR 0x%lx, _u32Flt1Flag_DscmbRef %lx \n", \
            __FUNCTION__, u32ii, _ptsp_res->TSPPvrcaParams._u32Flt1Flag_DscmbPVR, _ptsp_res->TSPPvrcaParams._u32Flt1Flag_DscmbRef));
        PVRCA_DBG(printf("_u16Flt1PidList %x , _u16Flt0DscmbPidList %x \n", \
    ...
    *pu8DmxId = (MS_U8)(u32ii & 0xFF)+TSP_SECFLT_NUM;
    ...
    PVRCA_DBG(printf("[%s] FLT %ld PID 0x%lx, _u32Flt1Flag_DscmbPVR 0x%lx\n", __FUNCTION__, u32ii, Pid, _ptsp_res->TSPPvrcaParams._u32Flt1Flag_DscmbPVR));
    PVRCA_DBG(printf("_u32Flt1Flag_DscmbRef %lx, _u16Flt1PidList %x , _u16Flt0DscmbPidList %x \n", _ptsp_res->TSPPvrcaParams._u32Flt1Flag_DscmbRef, \
...
TSP_Result MDrv_TSP_PVR_Eng_SetBuffer(MS_U8 u8PVRId, MS_PHYADDR u32BufStart0, MS_PHYADDR u32BufStart1, MS_U32 u32BufSize0, MS_U32 u32BufSize1)
{
    ...
    {
        case 0:
            return (MDrv_TSP_PVR_SetBuffer(u32BufStart0, u32BufStart1, u32BufSize0, u32BufSize1));

#ifdef TS2_IF_SUPPORT
        case 1:
            _TSP_ENTRY();
            #ifndef TS2_LIVE_SUPPORT
            if((u32BufSize1 > 0) && ((u32BufStart0 + u32BufSize0) == u32BufStart1))
                u32BufSize0 += u32BufSize1;
            #endif
            HAL_TSP_PVR1_SetBuffer((MS_U32)u32BufStart0, u32BufSize0, (MS_U32)u32BufStart1, u32BufSize1);
...
#ifdef TS2_IF_SUPPORT
        case 1:
            *pu32WriteAddr = (MS_PHYADDR)HAL_TSP_PVR1_GetBufWrite();
            break;
#endif
        default:
...
#ifdef VQ_ENABLE
    _TSP_ENTRY();
    HAL_TSP_SetVQBuffer(u32VQBaseAddr, u32VQBufLen);
    _TSP_RETURN(DRVTSP_OK);
#else
    return DRVTSP_NOT_SUPPORTED;
#endif
