...
MS_BOOL OSAL_GE_GetMapBase(MS_U32 *pu32Addr, MS_U32 *puSize)
{
    if( !MDrv_MMIO_GetBASE(pu32Addr, puSize, MS_MODULE_GE))
        return FALSE;

    return TRUE;
}

MS_BOOL OSAL_GE_GetMapBase2(MS_U32 *pu32Addr, MS_U32 *puSize)
{
    //get chip bank for identify chip version
    if( !MDrv_MMIO_GetBASE(pu32Addr, puSize, MS_MODULE_PM))
        return FALSE;
...
MS_BOOL OSAL_GE_Dcache_Flush( MS_U32 u32Base, MS_U32 u32Size )
{
    GE_DCACHE_FLUSH(u32Base, u32Size);
    return TRUE;
}
