...
#else
    #define getpid()                0xFFFFUL
    #define MsOS_GetOSThreadID()    0xFFFFUL
#endif
...
#define GE_BIT15                        0x8000UL
    ...
    #define _GE_MUXTEX_ENTRY(pGECtx , POOL_ID)\
        if(POOL_ID == E_GE_POOL_ID_INTERNAL_VARIABLE)\
            printf("[1] [%s, %d] GE Prepare Get Mutex ID=%d \n",__FUNCTION__,__LINE__, POOL_ID);\
                GE_Get_Resource(pGECtx);\
            printf("    [%s, %d] GE Got Mutex ID=%d \n",__FUNCTION__,__LINE__, POOL_ID);


    #define _GE_MUXTEX_RETURN(pGECtx, POOL_ID)\
        if(POOL_ID == E_GE_POOL_ID_INTERNAL_VARIABLE)\
            printf("[0] [%s, %d] GE Prepare Free Mutex ID=%d \n",__FUNCTION__,__LINE__, POOL_ID);\
                GE_Free_Resource(pGECtx);\
            printf("    [%s, %d] GE Free Mutex ID=%d \n",__FUNCTION__,__LINE__, POOL_ID);
#else
    #define _GE_MUXTEX_ENTRY(pGECtx, POOL_ID)\
            GE_Get_Resource(pGECtx);
    ...
    #define _GE_SEMAPHORE_ENTRY(pGECtx , POOL_ID)\
        if(POOL_ID == E_GE_POOL_ID_INTERNAL_REGISTER)\
            printf("[1] [%s, %d] GE Prepare Get Semaphore ID=%d \n",__FUNCTION__,__LINE__, POOL_ID);\
            MDrv_GE_Get_Semaphore(pGECtx, POOL_ID);\
            printf("    [%s, %d] GE Got Semaphore ID=%d \n",__FUNCTION__,__LINE__, POOL_ID);


    #define _GE_SEMAPHORE_RETURN(pGECtx, POOL_ID)\
        if(POOL_ID == E_GE_POOL_ID_INTERNAL_REGISTER)\
            printf("[0] [%s, %d] GE Prepare Free Semaphore ID=%d \n",__FUNCTION__,__LINE__, POOL_ID);\
            MDrv_GE_Free_Semaphore(pGECtx, POOL_ID);\
            printf("    [%s, %d] GE Free Semaphore ID=%d \n",__FUNCTION__,__LINE__, POOL_ID);
#else
    #define _GE_SEMAPHORE_ENTRY(pGECtx, POOL_ID)\
        MDrv_GE_Get_Semaphore(pGECtx, POOL_ID);\
