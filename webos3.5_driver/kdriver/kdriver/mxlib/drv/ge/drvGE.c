...
#define DRV_GE_ENTRY(pGECtx)       do{\
   if(GE_SWTABLE==FALSE){\
    ...
    } /*\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\Table is stored as Intance\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*/\
    else{\
                                                       GE_CTX_LOCAL *pGECtxLocal = (GE_CTX_LOCAL*)pGECtx; \
                                                       if(pGECtxLocal->u32CTXInitMagic != 0x55aabeef) {return;}  \
                                                       if (0 > pGECtxLocal->s32GEMutex)    \
                                                       { return;  }                  \
                                                        if((pGECtxLocal->bGELockStatus ==FALSE)&& (pGECtxLocal->s32GELock == MsOS_GetOSThreadID()))   \
                                                        {   \
                                                            pGECtxLocal->s32GE_Recursive_Lock_Cnt++;   \
                                                        }   \
                                                        else   \
                                                        {   \
                                                           ...
                                                           pGECtxLocal->s32GELock = MsOS_GetOSThreadID();     \
                                                            pGECtxLocal->s32GE_Recursive_Lock_Cnt++;        \
                                                            pGECtxLocal->bGELockStatus =TRUE;\
                                                           if(pGECtxLocal->pSharedCtx->u32LstGEClientId!=pGECtxLocal->u32GEClientId)  \
                                                           {   \
                                                               if(!pGECtxLocal->pSharedCtx->bNotFirstInit)   \
                                                               {   \
...
#define DRV_GE_RETURN(pGECtx)     do{    \
        if(GE_SWTABLE==FALSE){\
                                                             GE_CTX_LOCAL *pGECtxLocal = (GE_CTX_LOCAL*)pGECtx;   \
                                                             GE_ASSERT_CHECK((pGECtxLocal->s32GELock==MsOS_GetOSThreadID()), (printf("\nGE_RETURN: Fatal Error, Task ID mismatch[%d]->[%d]\n", pGECtxLocal->s32GELock, (MS_S32)MsOS_GetOSThreadID())));    \
                                                             GE_ASSERT_CHECK((0<pGECtxLocal->s32GE_Recursive_Lock_Cnt), (printf("\nGE_RETURN: Fatal Error, No Mutex to release[Cnt=%d]\n", pGECtxLocal->s32GE_Recursive_Lock_Cnt)));    \
                                                       ...
                                                       {   \
                                                           ...
                                                           {   \
                                                               ...
                                                               {\
                                                                   ...
                                                                   printf("\33[0;31m [Entry While][%s][%d]GE_GetHWSemphoreID=%x; pGECtxLocal->u32GESEMID=%x \33[m;\n",__FUNCTION__,__LINE__,GE_GetHWSemaphoreID, pGECtxLocal->u32GESEMID);\
                                                             ...
                                                             {   \
                                                                 pGECtxLocal->s32GE_Recursive_Lock_Cnt--;  \
                                                                     ...
                                                                     {\
                                                                        ...
                                                                        {\
                                                                            MDrv_SEM_Get_ResourceID(GESEMID, &GE_GetHWSemaphoreID);\
                                                                            printf("\33[0;31m [Return While][%s][%d] GE_GetHWSemphoreID=%x; u32GESEMID=%x\33[m;\n",__FUNCTION__,__LINE__,GE_GetHWSemaphoreID,pGECtxLocal->u32GESEMID);\
                                                                            GE_HWSemaphore_count =0;\
                                                                        }\
                                                                     }\
                                                             }\
...
#if GE_DEBUG_ENABLE
#define GE_DBG(_fmt, _args...)      MS_DEBUG_MSG(printf(_fmt, ##_args))
#else
#define GE_DBG(_fmt, _args...)      { }
#endif

#ifdef MS_DEBUG //---------------------------------------------------------------------------------
#define GE_ERR(_fmt, _args...)      printf(_fmt, ##_args)
...
#endif
static MS_U32* pTLBSRCStartVA=NULL;
static MS_U32* pTLBDSTStartVA=NULL;
...
//  Local Functions
//-------------------------------------------------------------------------------------------------
static void GE_BeginDraw(GE_Context *pGECtx)
{
    DRV_GE_ENTRY(pGECtx);
}

static void GE_EndDraw(GE_Context *pGECtx)
{
    DRV_GE_RETURN(pGECtx);
}
...
static GE_Result GE_SetPalette(GE_CTX_LOCAL *pGECtxLocal)
{
    ...
    //printf("Input Palette (%d, 0x%lx )\n",(int) 0 ,pGECtxLocal->halLocalCtx.u32Palette[0]);
    for(u32Idx = GE_PALETTE_NUM - 1; u32Idx >= 0; u32Idx--)
    {
        ...
        MS_U32 u32Count = 0;
        GE_WriteReg(&pGECtxLocal->halLocalCtx, REG_GE_CLUT_CTRL, ((u32Idx)&GE_CLUT_CTRL_IDX_MASK) | GE_CLUT_CTRL_RD);
        u32Color = GE_ReadReg(&pGECtxLocal->halLocalCtx, REG_GE_CLUT_H);
        u32Color = ByteSwap16(u32Color);
        u32Color2 = GE_ReadReg(&pGECtxLocal->halLocalCtx, REG_GE_CLUT_L);
        u32Color |= ByteSwap16(u32Color2)<<16;

        if(u32Color != pGECtxLocal->halLocalCtx.u32Palette[u32Idx])
        {
            printf("%s(),%d: >>> (%d,0x%lx)\n",__FUNCTION__,__LINE__,(int)u32Idx,u32Color);
        }

        if(u32Count >= 50)
        {
            //printf("%s(),%d: u32Count over %d\n",__FUNCTION__,__LINE__,u32Count);
            break;
        }
        u32Count++;
    }while(u32Color != pGECtxLocal->halLocalCtx.u32Palette[u32Idx]);
#endif
...
GE_Result MDrv_GE_Get_Semaphore(GE_Context *pGECtx, MS_U32 u32RPoolID)
{
    ...
    if(pGECtxLocal->ctxHeader.bGEMode4MultiProcessAccess ==TRUE)
    {
        ...
        {
            printf("[%s %d]Unkown GE Pool ID!",__FUNCTION__,__LINE__);
            return E_GE_FAIL;
        }
        ...
        {
            GFX_CRITIAL_MSG(printf("[%s:%s:%d]UtopiaResourceObtainToInstant fail\n",__FILE__,__FUNCTION__,__LINE__));
                return E_GE_FAIL;
        }
    }
#endif
...
GE_Result MDrv_GE_SetItalic(GE_Context *pGECtx, MS_U8 x_offset, MS_U8 y_offset, MS_U8 delta)
{
    ...
    GE_WriteReg(&pGECtxLocal->halLocalCtx, REG_GE_ITALIC_OFFSET, ((y_offset<<GE_ITALIC_Y_SHFT) | x_offset));
...
GE_Result MDrv_GE_SetAlphaTest(GE_Context *pGECtx, MS_BOOL enable, GE_ATestOp eATestOp, MS_U8 a_low, MS_U8 a_high)
{
    ...
    {
        u16en |= GE_EN_ATEST;
        GE_WriteReg(&pGECtxLocal->halLocalCtx, REG_GE_ATEST_TH, (a_low<<8) | (a_high));
...
GE_Result MDrv_GE_SetYUVMode(GE_Context *pGECtx, GE_YUVMode *mode)
{
    ...
    GE_WriteReg(&pGECtxLocal->halLocalCtx, REG_GE_YUV_MODE, u16YuvMode);
...
/// @sa @ref MDrv_GE_GetFmtCaps
//-------------------------------------------------------------------------------------------------
GE_Result MDrv_GE_BitBltByTwoSourceBuffer(GE_Context *pGECtx, GE_BufFmt dst_fmt, GE_Rect *pSrc0Rect, GE_Rect *pSrc1Rect, GE_Rect *pDestRect, MS_PHYADDR addr, MS_U16 pitch, MS_U32 flags, GE_ScaleInfo* scaleinfo)
{
    GE_Result           ret;
    GE_FmtCaps          caps;
    MS_U16              u16fmt = 0;
...
GE_Result MDrv_GE_BitBltEX(GE_Context *pGECtx, GE_Rect *src, GE_DstBitBltType *dst, MS_U32 flags, GE_ScaleInfo* scaleinfo)
{
    ...
    MS_U16              u16tmp_dy;
    MS_U16               srcMiu, dstMiu;
    MS_U16               u16enGeAlphaBlending;
...
void MDrv_GE_Exit(void* pInstance)
{
    ...
    #endif
#else
    printf("not enable GE_LOCK_SUPPORT\n");
#endif
}
...
GE_Result MDrv_GE_Init(void* pInstance, GE_Config *cfg, GE_Context **ppGECtx)
{
    ...
    *ppGECtx = (GE_Context *)pGECtxLocal;
    ...
    //STR restored
    MS_U8 i=0UL;
    GE_STR_SAVE_AREA* pGFX_STRPrivate;
    void* pModule = NULL;
    UtopiaInstanceGetModule(pInstance, &pModule);
    UtopiaModuleGetSTRPrivate(pModule, (void**)&pGFX_STRPrivate);
    for(i =0;i<GE_TABLE_REGNUM;i++ )
    {
        pGFX_STRPrivate->GE0_Reg[i]  = GE_ReadReg(&pGECtxLocal->halLocalCtx, i+GE_TABLE_REGNUM);//pGECtxLocal->halLocalCtx.u16RegGETable[pGECtxLocal->pSharedCtx->halSharedCtx.pVirualTableInfo.u16WRTableIndex][i];
        pGFX_STRPrivate->GETLB_Reg[i]  = GE2_ReadReg(&pGECtxLocal->halLocalCtx, i+GE_TABLE_REGNUM);//pGECtxLocal->halLocalCtx.u16RegGETableEX[pGECtxLocal->pSharedCtx->halSharedCtx.pVirualTableInfo.u16WRTableIndex][i];
    }
#endif
...
/// @return @ref GE_Result
//-------------------------------------------------------------------------------------------------
GE_Result MDrv_GE_WriteProtect(GE_Context *pGECtx, MS_U8 miu, MS_PHYADDR addr_low, MS_U32 addr_high, GE_WPType eWPType)
{
    GE_Result           ret;
    GE_CTX_LOCAL *pGECtxLocal = (GE_CTX_LOCAL*)pGECtx;
...
GE_Result MDrv_GE_SetAlpha_ARGB1555(GE_Context *pGECtx, MS_U8 coef)
{
    ...
    u16tmp |= coef<<8; ///- Set Alpha for ARGB1555
...
/// @return @ref GE_Result
//-------------------------------------------------------------------------------------------------
GE_Result MDrv_GE_SetVCmdBuffer(GE_Context *pGECtx, MS_PHYADDR u32Addr, GE_VcmqBufSize enBufSize)
{
    GE_Result           ret;
    GE_CTX_LOCAL *pGECtxLocal = (GE_CTX_LOCAL*)pGECtx;
    ...
    ret = GE_SetVCmdBuffer(&pGECtxLocal->halLocalCtx, u32Addr, enBufSize);
...
GE_Result MDrv_GE_SetPowerState(GE_Context *pGECtx, EN_POWER_MODE u16PowerState, void* pModule)
{
    ...
    UtopiaModuleGetSTRPrivate(pModule, (void**)&pGFX_STRPrivate);
    if( !MDrv_MMIO_GetBASE(&u32MMIOBaseAdr, &u32NonPMBankSize, MS_MODULE_GE))
    {
    ...
    switch(u16PowerState)
    {
        ...
        case E_POWER_RESUME:
        {
            for(i=0;i<GE_TABLE_REGNUM; i++)
                {
                //resume reg
                (*((volatile MS_U16 *)(u32MMIOBaseAdr+ 0x62800*2 + ((i)<<2) ))) = pGFX_STRPrivate->GETLB_Reg[i];

                if(i == REG_GE_CMD )
                {
                    continue;
                }
                (*((volatile MS_U16 *)(u32MMIOBaseAdr+ 0x2800*2 + ((i)<<2) ))) = pGFX_STRPrivate->GE0_Reg[i];
            }
        }
        break;
    }
...
GE_Result MDrv_GE_GetTLBSRCADDR(GE_Context *pGECtx, MS_PHYADDR* addr)
{
    GE_CTX_LOCAL *pGECtxLocal = (GE_CTX_LOCAL*)pGECtx;
...
GE_Result MDrv_GE_GetTLBDSTADDR(GE_Context *pGECtx, MS_PHYADDR* addr)
{
    GE_CTX_LOCAL *pGECtxLocal = (GE_CTX_LOCAL*)pGECtx;
...
GE_Result MDrv_GE_SetTLBSrcBaseAddr(GE_Context *pGECtx, MS_PHYADDR u32addr)
{
    GE_CTX_LOCAL *pGECtxLocal = (GE_CTX_LOCAL*)pGECtx;

    GE_ENTRY(pGECtx);

    GE_SetTLBSrcBaseAddr(&pGECtxLocal->halLocalCtx, u32addr);
    GE_RETURN(pGECtx,E_GE_OK);
}

GE_Result MDrv_GE_SetTLBDstBaseAddr(GE_Context *pGECtx, MS_PHYADDR u32addr)
{
    GE_CTX_LOCAL *pGECtxLocal = (GE_CTX_LOCAL*)pGECtx;

    GE_ENTRY(pGECtx);

    GE_SetTLBDstBaseAddr(&pGECtxLocal->halLocalCtx, u32addr);
    GE_RETURN(pGECtx,E_GE_OK);
}
...
#define REG_ADDR(addr)              (*((volatile MS_U16*)(pGECtxLocal->halLocalCtx.u32_mmio_base2 + ((addr) << 1))))
#define REG_CLK_BASE                 (pGECtxLocal->halLocalCtx.u32_mmio_base2+0x201600)
...
#define REG_GE_MIUCLIENT    (0x1006F4)

#define MIU_LOG    0        //miu flow rate enable  --> we could use this to check GE miu_request percentage

#define REG_MIU0_CTRL       (0x10121A)
#define REG_MIU0_CNT        (0x10121C)

#define REG_MIU1_CTRL       (0x10061A)
#define REG_MIU1_CNT        (0x10061C)

#define REG_MIU0_GROUP0_MASK (0x101246)
#define REG_MIU0_GROUP1_MASK (0x101266)
#define REG_MIU0_GROUP2_MASK (0x101286)
#define REG_MIU0_GROUP3_MASK (0x1012A6)

#define REG_MIU1_GROUP0_MASK (0x100646)
#define REG_MIU1_GROUP1_MASK (0x100666)
#define REG_MIU1_GROUP2_MASK (0x100686)
#define REG_MIU1_GROUP3_MASK (0x1006A6)


#define MAX_TEST_NUM 9
int test_rect_w[MAX_TEST_NUM] = { 10,100,100,100,500,500,1280, 1920, 3840};
int test_rect_h[MAX_TEST_NUM] = {100, 10,100,500,100,500, 720, 1080, 2160};

//int test_rect_w[MAX_TEST_NUM] = { 1280,1280,1280,100,500,500,1280};
//int test_rect_h[MAX_TEST_NUM] = {720, 720,720,500,100,500, 720};

#define BLTCNT 1000
#define CMD_BLT  0x40
#define CMD_FILL 0x30

#define GE_TEST_CMD CMD_BLT
#define GE_1P_MODE 1

#define GE_MIU_SEL 0x000   //0: miu0  0x8000: miu1

#define NORMAL_BLT   0x0
#define Stretch_BLT  0x1
#define BLENDING_BLT 0x4

#define STRETCH_RATIO 1.2
#define GE_STRETCH  BLENDING_BLT

#define GE_ARGB8888 0x1
#define GE_ARGB4444 0x10
...
GE_Result MDrv_GE_BitbltPerformance(GE_Context *pGECtx)
{
    ...
    REG_WR(0x101268, 0x0000);
       ...
       printf("\n[MIU Sel]       Type '0'(miu0) or '1'(miu1) or '2'(miu2) to sel MIU Base\n");
        ...
        printf("[1P/4P(8P) mode]    Type '1'(1P) or '4'(4P/8P) to sel engine mode\n");
        c = getchar();
        if (c=='1')
        ...
        printf("[BLT mode]      Type 'b'(Bitblt) or 'f'(Rectfill) to sel BLT mode\n");
        c = getchar();
        if (c=='f')
            GE_Cmd = CMD_FILL;
        else
        ...
        printf("[BLT Type]      Type 'n'(Normal_BLT) or 's'(Stretch_BLT) or 'b'(Blending_BLT) to sel BLT type\n");
        c = getchar();
        if (c == 's')
        ...
        printf("[THRESHOLD]     Type '7'(0x7) or 'f'(0xf) or Others to set engine threshold\n\n");
        c = getchar();
...
#ifdef GE_EN_BURST
        c = getchar();
        printf("[Burst Type]      Type 'b'(Burst) or 'n'(ormal) to speed up\n");
        c = getchar();
        if (c == 'b')
            GE_Burst = 0x4000;
        else
    ...
    {
        c = getchar();
        printf("[Spilt Type]      Type 's'(Spilt) or 'n'(ormal) to speed up\n");
        c = getchar();
        if (c == 's')
            GE_Spilt = 0x4000;
        else
...
TEST_START:
    ...
    if(GE_ReadReg(&pGECtxLocal->halLocalCtx,REG_GE_EN)& 0x4000)
        printf("\33[0\34m Burst Enable \33[m\n");
    else
        printf("\33[0\34m Burst Disable \33[m\n");


    if(GE_ReadReg(&pGECtxLocal->halLocalCtx,REG_GE_CFG)& 0x4000)
        printf("\33[0\34m Spilt Enable \33[m\n");
    else
        printf("\33[0\34m Spilt Disable \33[m\n");
    ...
    {
        pitch=stretch_w*4;
        printf("[alignment]SRC pitch=%d\n",pitch);
    }
    else
    {
        pitch=(((stretch_w*4)/32)+1)*32;
        printf("[Not alignment]SRC pitch=%d\n",pitch);
    }
    ...
    {
        pitch=stretch_w*4;
        printf("[alignment]DST pitch=%d\n",pitch);
    }
    else
    {
        pitch=(((stretch_w*4)/32)+1)*32;
        printf("[Not alignment]DST pitch=%d\n",pitch);
    }
    GE_WriteReg(&pGECtxLocal->halLocalCtx,REG_GE_DST_PITCH, pitch);
    ...
    printf("=== cnt:%d  MIU_SEL_L_bit:0x%x  MIU_SEL_H_bit:0x%x===\n\n",BLTCNT,GE_BASE,GE_BASE_H);
    ...
    if(GE_Cmd == CMD_BLT)
    {
        ...
        printf("GE_BLTTest ARGB4444 time:%ld sec\n", u32Timer_ms);
    }
    else
    {
        printf("\n GE_RectFill ARGB4444 cnt:%u total time:%lu us\n",BLTCNT, u32Timer/12);
        printf("GE_RectFill ARGB4444 time:%ld ms\n", u32Timer_ms);
    }
    ...
    printf("\n\33[0;32m=== ARGB4444 Result: %ld  MB/s ===\33[m\n\n",(MS_U32)avgtime);
