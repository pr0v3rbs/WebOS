...
//  Local Defines
//-------------------------------------------------------------------------------------------------
#define MMFI_MUTEX_WAIT_TIME                            0xFFFFFFFF
...
#define _MMFI_GET_ENGID(x)                              (x <  MMFI_PIDFLT0_NUM ? 0 : 1);
#define _MMFI_GET_FLTID(x)                              (x %  MMFI_PIDFLT0_NUM);
#define _MMFI_GET_FLTLISTID(e,f)                        (e == 0 ? f : (MMFI_PIDFLT0_NUM+f))
//-------------------------------------------------------------------------------------------------
//  Local Structures
//-------------------------------------------------------------------------------------------------
...
MMFI_Result MDrv_MMFI_InitLibResource(void* pResMemAddr)
{
    ...
    if(_bBankInit == FALSE)
    {
        if (FALSE == MDrv_MMIO_GetBASE(&u32Bank, &u32BankSize, MS_MODULE_MMFILEIN))
        {
            MMFI_DBGERR(printf("MDrv_MMIO_GetBASE failed\n"));
            return DRVMMFI_FAIL;
        }
        MMFI_DBGINFO((printf("MMFI Base Bank 0x%08x\n", (int)u32Bank)));
        HAL_MMFI_SetBank(u32Bank);
        _bBankInit = TRUE;
    }
...
MMFI_Result MDrv_MMFI_Init(void)
{
    ...
    for(u8jj = 0; u8jj < MMFI_ENGINE_NUM; u8jj++)
    {
        for(u8ii = 0; u8ii < MMFI_PIDFLT0_NUM; u8ii++)
        {
...
MMFI_Result MDrv_MMFI_File_CmdQ_GetSlot(DrvMMFI_Path epath, MS_U32 *pu32EmptySlot)
{
    ...
    *pu32EmptySlot = 0;
    *pu32EmptySlot = MMFI_CMDQ_SIZE - HAL_MMFI_CmdQ_FIFO_Get_WRCnt((MS_U8)epath);
    return DRVMMFI_OK;
}
...
MMFI_Result MDrv_MMFI_File_SetAddr(DrvMMFI_Path epath, MS_PHYADDR u32StreamAddr)
{
    ...
    _MMFI_ENTRY();
    HAL_MMFI_Set_Filein_ReadAddr((MS_U8)epath, u32StreamAddr);
    _MMFI_RETURN(DRVMMFI_OK);
}
...
/// @param  epath                            \b IN: MM Filein Path
/// @param  pu32ReadAddr                     \b OUT: pointer to store read address of file
/// @return MMFI_Result
/// @note
//-------------------------------------------------------------------------------------------------
MMFI_Result MDrv_MMFI_File_GetReadAddr(DrvMMFI_Path epath, MS_PHYADDR* pu32ReadAddr)
{
    *pu32ReadAddr = HAL_MMFI_Get_Filein_WriteAddr((MS_U8)epath);
    ...
    *((MS_U32*)pOutput) = 0;
    ...
    switch (eCap)
    {
        case E_DRVMMFI_CAP_RESOURCE_SIZE:
            *((MS_U32*)pOutput) = sizeof(MMFI_RESOURCE_PRIVATE);
            return DRVMMFI_OK;
...
MMFI_Result MDrv_MMFI_File_ModeEnable(DrvMMFI_FileinMode fileinmode, MS_BOOL bEnable)
{
    ...
    if(fileinmode == E_DRVMMFI_MODE_PS_AUC)
        u16cfg2 |= MMFI_CFG2_MMFI_PS_AUDC_EN;
    if(fileinmode == E_DRVMMFI_MODE_PS_AUD)
        u16cfg2 |= MMFI_CFG2_MMFI_PS_AUDD_EN;
    if(fileinmode == E_DRVMMFI_MODE_APIDC_BYPASS)
        u16cfg2 |= MMFI_CFG2_MMFI_APIDC_BYPASS;
    if(fileinmode == E_DRVMMFI_MODE_APIDD_BYPASS)
    ...
    if(fileinmode == E_DRVMMFI_MODE_VDPID_BYPASS)
        u32cfg |= MMFI_CFG_VPID_BYPASS;
    if(fileinmode == E_DRVMMFI_MODE_V3DPID_BYPASS)
        u32cfg |= MMFI_CFG_VPID3D_BYPASS;
    if(fileinmode == E_DRVMMFI_MODE_PS_AU)
        u32cfg |= MMFI_CFG_PS_AUD_EN;
    if(fileinmode == E_DRVMMFI_MODE_PS_AUB)
        u32cfg |= MMFI_CFG_PS_AUDB_EN;
    if(fileinmode == E_DRVMMFI_MODE_PS_VD)
        u32cfg |= MMFI_CFG_PS_VD_EN;
    if(fileinmode == E_DRVMMFI_MODE_PS_V3D)
