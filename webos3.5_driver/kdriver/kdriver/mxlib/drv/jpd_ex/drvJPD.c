...
void MDrv_JPD_Rst_Patch(MS_BOOL bSVLD, MS_U8 u8ScalingFactor, MS_U8 com_num, MS_BOOL bUV_en, MS_U8 Y_VSF, MS_U8 Y_HSF, MS_U16 _u16Restart_interval)
{

    MS_U16 u16SConfigReg;
    u16SConfigReg = ((MS_U16) u8ScalingFactor) << 4 |((MS_U16) bUV_en) << 3 | ( Y_VSF << 2 ) | Y_HSF;

    if(_u16Restart_interval)
    {
        u16SConfigReg = u16SConfigReg | JPD_RST_EN;
    }

    if(com_num>2)
    {
        u16SConfigReg = u16SConfigReg | JPD_SUVQ;
    }

    if(bSVLD)
    {
        u16SConfigReg = u16SConfigReg | JPD_SVLD;
    }

    HAL_JPD_Rst_Patch(u16SConfigReg);
}
...
{
//    HAL_JPD_Debug();
    MDrv_JPD_PrintGTable();
    MDrv_JPD_PrintHTable();
    MDrv_JPD_PrintQTable();
}

#ifdef MSOS_TYPE_LINUX_KERNEL
MS_BOOL MDrv_JPD_Get_ISR(void)
{
    return gbCallbackFlag;
}

void MDrv_JPD_Set_ISR(MS_BOOL val)
{
    gbCallbackFlag = val;
}
#endif
