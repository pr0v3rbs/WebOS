...
MMFI_Result MDrv_MMFI_Init(void)
{
    ...
    for(u8ii = 0; u8ii < MMFI_PIDFLT_NUM_ALL; u8ii++)
    ...
    for(u8ii = 0; u8ii < MMFI_V3DPIDFLT_NUM; u8ii++)
    {
...
MMFI_Result MDrv_MMFI_File_CmdQ_GetSlot(DrvMMFI_Path epath, MS_U32 *pu32EmptySlot)
{
    ...
    *pu32EmptySlot = 0;
    ...
    if(epath == E_DRVMMFI_PTH_AUDIO)
        *pu32EmptySlot = MMFI_CMDQ_SIZE - HAL_MMFI_AU_CmdQ_FIFO_Get_WRCnt();
#ifdef MMFI_VD3D
    else if(epath == E_DRVMMFI_PATH_VD3D)
        *pu32EmptySlot = MMFI_CMDQ_SIZE - HAL_MMFI_V3D_CmdQ_FIFO_Get_WRCnt();
#endif
...
MMFI_Result MDrv_MMFI_File_SetAddr(DrvMMFI_Path epath, MS_PHYADDR u32StreamAddr)
{
    ...
    if(epath == E_DRVMMFI_PTH_AUDIO)
        HAL_MMFI_AU_Set_Filein_ReadAddr(u32StreamAddr);
#ifdef MMFI_VD3D
    else if(epath == E_DRVMMFI_PATH_VD3D)
        HAL_MMFI_V3D_Set_Filein_ReadAddr(u32StreamAddr);
#endif
