...
extern MS_BOOL MApi_XC_IsBlackVideoEnable( SCALER_WIN eWindow );
...
static void _MDrv_PNL_Init_XC_VOP(void *pInstance, PNL_InitData *pstPanelInitData)
{
    ...
    MS_U16 u16VsyncStart = pstPanelInitData->u16VTotal - pstPanelInitData->u16VSyncStart;// vsync start = Vtt - (VSyncWidth + BackPorch)
    PNL_INSTANCE_PRIVATE *pPNLInstancePrivate = NULL;
    static MS_BOOL bFirstTime = TRUE;
    UtopiaInstanceGetPrivate(pInstance, (void**)&pPNLInstancePrivate);
    if(!bFirstTime)
    {
       //SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK00_01_L, BIT(0), BIT(0));
       // SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK00_01_L, 0, BIT(1));
     //   SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK00_01_L, BIT(2), BIT(2));
  //   }
    // Htotal/Vtotal
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_0C_L, (pstPanelInitData->u16HTotal - 1), 0x1FFF);         // output htotal
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_0D_L, (pstPanelInitData->u16VTotal - 1), 0xFFF);         // output vtotal
    ...
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_06_L, u16DE_VStart, 0xFFF);                                       // DE V start
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_07_L, u16DE_VEND  , 0xFFF);  // DE V end
    ...
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_0A_L, u16DE_VStart , 0xFFF);                                               // Display V start
    SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_0B_L, u16DE_VEND   , 0xFFF);  // Display V end
    ...
    // dither control
    SC_W2BYTE(pPNLInstancePrivate->u32DeviceID,REG_SC_BK10_1B_L, pstPanelInitData->u16DITHCTRL);

  //  if(bFirstTime)
     // {
       // SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK00_01_L, 0, BIT(0));
       // SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK00_01_L, 0, BIT(1));
     //   SC_W2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK00_01_L, 0, BIT(2));
     }
    bFirstTime = FALSE;
}
...
MS_BOOL MDrv_PNL_Control_Vreg_Level(void *pInstance, MS_U16 u16Vreg_Level)
{
    return (MS_BOOL)MHal_PNL_MOD_Control_Vreg_Level(pInstance, u16Vreg_Level);
}


MS_BOOL MDrv_PNL_Control_Preemphasis_Level(void *pInstance, MS_U16 u16Preemphasis_Level)
{
    return (MS_BOOL)MHal_PNL_MOD_Control_Preemphasis_Level(pInstance, u16Preemphasis_Level);
}
...
MS_BOOL MDrv_PNL_MOD_Control_Out_PE_Current (void *pInstance,MS_U16 u16Current_Level)
{
    return (MS_BOOL)MHal_PNL_MOD_Control_Out_PE_Current (pInstance,u16Current_Level);
}

MS_BOOL MDrv_PNL_MOD_Control_Out_PE_Current_Partial (void *pInstance,MS_U8 *u8Current_Level_array)
{
    return (MS_BOOL)MHal_PNL_MOD_Control_Out_PE_Current_Partial (pInstance, u8Current_Level_array);
}
...
MS_U32 MDrv_PNL_CalculateLPLLSETbyDClk(void *pInstance, MS_U64 u64LdHz, MS_BOOL bHighAccurate)
{
    ...
    if(!((pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type == E_PNL_TYPE_TTL)||
        ...
        (pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type == E_PNL_LPLL_VBY1_10BIT_1LANE)||
        (pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type == E_PNL_LPLL_VBY1_8BIT_1LANE)))
    {
        //u64LdHz = u64LdHz/2;
        do_div(u64LdHz,2);
    }
    ...
    switch(pPNLResourcePrivate->stdrvPNL._stPnlInitData.eLPLL_Type)
    {
        ...
        default:
            ...
            do_div(u64LdPllSet, u64LdHz);
				...
				printf("[LPLL setting error ]u64LdHz =0\n");
            ...
            do_div(u64LdPllSet, u32Div);
				...
				printf("[LPLL setting error ] u32Div =0\n");
            break;
    }

    PNL_DBG(PNL_DBGLEVEL_INIT, "CalculateLPLLSETbyDClk u32LpllSet = %lx\n", (MS_U32)u64LdPllSet);
...
PNL_Result MDrv_PNL_SetPanelType(void *pInstance, PNL_InitData *pstPanelInitData)
{
    ...
    if(pPNLResourcePrivate->stapiPNL._bPNL_InitByCus != TRUE)
    {
        _MDrv_PNL_Init_XC_VOP(pInstance, &pPNLResourcePrivate->stdrvPNL._stPnlInitData);
    }
                                 ...
                                 DRVPNL_GAMMA_MAPPEING_MODE GammaMapMode)
{
    ...
    PNL_INSTANCE_PRIVATE *pPNLInstancePrivate = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&pPNLInstancePrivate);
    MS_BOOL bvideomute = 0;
    //bvideomute = MApi_XC_IsBlackVideoEnable(MAIN_WINDOW);
    ...
    // For performance
    bvideomute = MApi_XC_IsBlackVideoEnable(MAIN_WINDOW);
    if((MApi_XC_MLG_GetStatus() == E_MLG_ENABLED) && (bvideomute == 0))
...
// SSC
//-----------------------------------------------------------------------------
// SPAN value, recommend value is 10KHz ~ 40KHz
// STEP percent value, recommend is under 4%
...
PNL_Result MDrv_PNL_SetSSC(void *pInstance, MS_U16 u16Fmodulation, MS_U16 u16Rdeviation, MS_BOOL bEnable)
{
    ...
    // Set SPAN
    if( u16Fmodulation < 100 )
    {
        u16Fmodulation = 100;
    }
    else if( u16Fmodulation > 400)
    {
        u16Fmodulation = 400;
    }
    ...
    {
        //if u16Fmodulation is too small (< 150), (MS_U32)LVDS_MPLL_CLOCK_MHZ*LVDS_SPAN_FACTOR ) / (u16Fmodulation) ) * 10000) will overflow
        //so we multiply by 2 for u16Fmodulation here to avoid overflow
        u16Span =( ( (((MS_U32)LVDS_MPLL_CLOCK_MHZ*LVDS_SPAN_FACTOR ) / (u16Fmodulation*2)) * 10000) / ((MS_U32)u32PLL_SET)*2 );
    }
    // Set STEP
    if(u16Rdeviation > 400)
    {
        u16Rdeviation = 400;
    }
    //printf("u32PLL_SET=%d,u16Rdeviation(%)=%d,u16Span=%d,\n",u32PLL_SET,u16Rdeviation,u16Span);
...
void MDrv_PNL_TCON_DumpGENRegTab(void *pInstance, MS_U8* pu8TconTab)
{
    ...
    {
        u32Addr = ((pu8TconTab[u32tabIdx]<<8) + pu8TconTab[(u32tabIdx +1)]) & 0xFFFF;
        u8Mask  = pu8TconTab[(u32tabIdx +2)] & 0xFF;
        u8Value = pu8TconTab[(u32tabIdx +3)] & 0xFF;
...
MS_BOOL MDrv_PNL_GetDataFromRegister(void *pInstance, PNL_TimingInfo *PNLTiming)
{
    ...
    //Vtotal
    PNLTiming->u16VTotal    = SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_0D_L, 0xFFF);         // output vtotal
    PNLTiming->u16DEVStart  = SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_06_L, 0xFFF);                                       // DE V start
    PNLTiming->u16DEVEnd    = SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_07_L, 0xFFF);       // DE V end
    PNLTiming->u16VSyncStart= SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_02_L, 0xFFF);       // DE V end
    PNLTiming->u16VSyncEnd  = SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_03_L, 0xFFF);       // DE V end


    // Htotal
    PNLTiming->u16HTotal    = SC_R2BYTEMSK(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_0C_L, 0x1FFF);         // output htotal
...
MS_U16 MDrv_PNL_Get_DEVstart(void *pInstance)
{
    PNL_INSTANCE_PRIVATE *pPNLInstancePrivate = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&pPNLInstancePrivate);
    return SC_R2BYTE(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_06_L) & 0xFFF; //pnl de vstart, B[11:0]
}
...
MS_U16 MDrv_PNL_Get_DEHstart(void *pInstance)
{
    PNL_INSTANCE_PRIVATE *pPNLInstancePrivate = NULL;
    UtopiaInstanceGetPrivate(pInstance, (void**)&pPNLInstancePrivate);
    return SC_R2BYTE(pPNLInstancePrivate->u32DeviceID, REG_SC_BK10_04_L) & 0xFFF; //pnl de vstart, B[11:0]
}
...
MS_U32 MDrv_PNL_Get_Semaphore(E_PNL_POOL_ID eID)
{
    ...
    void* pModule = NULL;
    UtopiaInstanceGetModule(pu32PNLInst_private, &pModule);
...
MS_U32 MDrv_PNL_Release_Semaphore(E_PNL_POOL_ID eID)
{
    MS_U32 u32Return = UTOPIA_STATUS_FAIL;
...
void MDrv_PNL_VB1_Video_Protect(void *pInstance,MS_BOOL enable)
{

    MHal_PNL_VB1_Video_Protect(pInstance, enable);

}


void MDrv_PNL_VBY1_Packer_Check(void *pInstance,MS_U16 eLPLL_Type)
{
    	MHal_PNL_VBY1_Packer_Check( pInstance,eLPLL_Type);
}



PNL_Result MDrv_PNL_OSDC_Init_Status(void *pInstance,E_DRVPNL_VIDEO_OSD_CONFIG econfig)
{
  	MHal_PNL_OSDC_Init_Status(pInstance,econfig);
    	return E_PNL_OK;
}

PNL_Result MDrv_PNL_VideoOSDConfig(void *pInstance,E_DRVPNL_VIDEO_OSD_CONFIG econfig)
{
    MHal_PNL_VideoOSDConfig(pInstance,econfig);
    return E_PNL_OK;
}

PNL_Result MDrv_PNL_DivisionModeConfig(void *pInstance,E_DRVPNL_DIVISIONMODE_CONFIG econfig)
{
    MHal_PNL_DivisionModeConfig(pInstance,econfig);
    return E_PNL_OK;
}

void MDrv_PNL_SetSkipInitFlag(MS_BOOL bEnable)
{
	MHal_PNL_SetSkipInitFlag(bEnable);
}

MS_BOOL MDrv_PNL_GetSkipInitFlag(void )
{
	return MHal_PNL_GetSkipInitFlag();
}

void MDrv_PQ_Clock_Gen_For_Gamma(void *pInstance)
{
    MHal_PQ_Clock_Gen_For_Gamma(pInstance);
}

void MDrv_PNL_SetModPattern(MS_BOOL b)
{
    MHal_PNL_SetModPattern(b);
}
#undef _DRVPNL_C_
