...
//#define MBX_DEBUG
#ifdef MBX_DEBUG
    #define MBX_ERROR(fmt, args...)  printf("[MBX Driver USER ERR][%06d]     " fmt, __LINE__, ## args)
    #define MBX_WARN(fmt, args...)   printf("[MBX Driver WARN][%06d]    " fmt, __LINE__, ## args)
    #define MBX_PRINT(fmt, args...)  printf("[MBX Driver][%06d]     " fmt, __LINE__, ## args)
...
{
    MS_U32 u32RIUBaseAddr, u32RIUBaseSize;
    MBX_Result mbxResult;

    if(_mgrMBX.s32RefCnt == 0)
    {
        if( !MDrv_MMIO_GetBASE(&u32RIUBaseAddr, &u32RIUBaseSize, MS_MODULE_MBX))
        {
        ...
        //init mbx hardware:
        MHAL_MBX_Init(eHostRole, u32RIUBaseAddr);

        //init mbx int hardware:
        mbxResult = MHAL_MBXINT_Init(_mgrMBX.eMbxRole2Cpu[eHostRole], _MDrv_MBX_MsgRecvCb, u32RIUBaseAddr);
        if(E_MBX_SUCCESS != mbxResult)
        {
...
MBX_Result  MDrv_MBX_SetInformation(MBX_ROLE_ID eTargetRole, MS_U8 *pU8Info, MS_U8 u8Size)
{
    ...
    return MHAL_MBX_SetInformation(eTargetRole, u32RIUBaseAddr, pU8Info, u8Size);
}
...
MBX_Result  MDrv_MBX_GetInformation(MBX_ROLE_ID eTargetRole, MS_U8 *pU8Info, MS_U8 u8Size)
{
    ...
    return MHAL_MBX_GetInformation(eTargetRole, u32RIUBaseAddr, pU8Info, u8Size);
}
