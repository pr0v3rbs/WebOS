...
static void mfe_alloc_addr(MFE_U32 u32MIUOffset, MFE_U32* ppBufStart, MFE_U32 pBufEnd, MEMMAP_t Buf[2], int sizeY, int sizeC)
{
	MMAPMalloc(u32MIUOffset, ppBufStart, pBufEnd, sizeY, &Buf[0], 256,"Y");
	MMAPMalloc(u32MIUOffset, ppBufStart, pBufEnd, sizeC, &Buf[1], 256,"C");
}
...
MFE_U32 jpeAllocDRAM(MFE_CONFIG* pConfig)
{
    ...
    for (i=0; i<pBufInfo->m_nOutBuffer; i++)
        MMAPMalloc(pConfig->MFE_U32MFEMIUOffset, &pConfig->dram_ptr, pBufEnd, pBufInfo->m_OutBufferSize, &pBufInfo->m_nOutBufAddr[i], 256,"out buf");

    ms_dprintk(DRV_L4, "[jpeAllocDRAM] Start=%08x, End=%08x\n", (unsigned int) pConfig->dram_base, (unsigned int)pConfig->dram_ptr);
...
// 	dram_base = (dram_base+7) & ~7;
	MMAPMalloc(pConfig->MFE_U32MFEMIUOffset, &pConfig->dram_ptr, pBufEnd, nMbCount*16, &pBufInfo->m_nMvStoreAddr, 8,"Mbcount*16");
...
// 	dram_base = (dram_base+7) & ~7;
	MMAPMalloc(pConfig->MFE_U32MFEMIUOffset, &pConfig->dram_ptr, pBufEnd, (pConfig->nBufWidth>>4)*64, &pBufInfo->m_nGNAddr, 8,"(nBufWidth>>4)*6");
...
// 	mfe_alloc_addr((U32*)&pConfig->dram_ptr, pBufEnd, pInfo->m_FrameBuf[M4VE_FRM_REF1_ORI], sizeY, sizeC);	// m_pvopcRefOrig1
	mfe_alloc_addr(pConfig->MFE_U32MFEMIUOffset, &pConfig->dram_ptr, pBufEnd, pInfo->m_FrameBuf[M4VE_FRM_CUR_REC], sizeY, sizeC);	// m_pvopcCurrQ
	mfe_alloc_addr(pConfig->MFE_U32MFEMIUOffset, &pConfig->dram_ptr, pBufEnd, pInfo->m_FrameBuf[M4VE_FRM_REF0_REC], sizeY, sizeC);	// m_pvopcRefQ0
#ifndef _MFE_M1_
	mfe_alloc_addr(pConfig->MFE_U32MFEMIUOffset, &pConfig->dram_ptr, pBufEnd, pInfo->m_FrameBuf[M4VE_FRM_REF1_REC], sizeY, sizeC);	// m_pvopcRefQ1
#endif
...
// 	g_dram_base = (g_dram_base+7) & ~7;
	...
	MMAPMalloc(pConfig->MFE_U32MFEMIUOffset, &pConfig->dram_ptr, pBufEnd, (pConfig->nBufWidth/16)*128, &pBufInfo->m_nGNAddr, 8,"(pConfig->nBufWidth>>4)*64");
#else
    MMAPMalloc(pConfig->MFE_U32MFEMIUOffset, &pConfig->dram_ptr, pBufEnd, (pConfig->nBufWidth/16)*64, &pBufInfo->m_nGNAddr, 8,"(pConfig->nBufWidth>>4)*64");
#endif
	MFE_ASSERT((pBufInfo->m_nGNAddr.miuAddress&0x7)==0);
    ...
    if(pConfig->bColorFormat == YUVTILE) {
        ...
        int interlace_h = pConfig->nBufHeight;
        if (pConfig->bInterlace != PROGRESSIVE){
	...
	for (i=0; i<total; i++) {
		mfe_alloc_addr(pConfig->MFE_U32MFEMIUOffset, &pConfig->dram_ptr, pBufEnd, pInfo->BufPool.addr[i], sizeY, sizeC);
...
static void bufpool_get(FrameBufPool* pPool, MEMMAP_t Buf[2])
{
    ...
    for (i=0; i<pPool->FrameBufPoolSize; i++) {
    	if (pPool->available[i]) {
    		...
    		pPool->available[i] = 0;
            ms_dprintk(DRV_L4, "[bufpool_get] %d",i);
    		break;
    	}
    }
...
void h264DpbHandling(MFE_CONFIG* pConfig)
{
    ...
    ms_dprintk(DRV_L4, "empty_idx = %d",empty_idx);
...

    {
        int i;
        for(i=0;i<pInfo->dpb_size;i++)
            ms_dprintk(DRV_L4, "pInfo->dpb[%d].is_used = %d,structure = %d,addr=0x%x",i,pInfo->dpb[i].is_used,
                pInfo->dpb[i].structure,pInfo->dpb[i].addr[0].miuAddress);
    }
