...
static __inline int iabs(int x)
{
    return ((x) < 0) ? -(x) : (x);
}
...
int RBSPtoEBSP(MFE_BYTE *streamBuffer, int begin_bytepos, int end_bytepos, int min_num_bytes)
{
    ...
    int i, j, count;
    MFE_BYTE *NAL_Payload_buffer = (MFE_BYTE *)MfeDrvMemMalloc(sizeof(MFE_BYTE)*(MAXRBSPSIZE<<1), "NAL_Payload_buffer");
    if(NAL_Payload_buffer == NULL)
    {
        printf("NAL_Payload_buffer NULL error\n");
        return 0;
    }
    ...
    MfeDrvMemFree(&NAL_Payload_buffer, "NAL_Payload_buffer");
    return j;
}
...
*/
        ...
        se.bitpattern = (0xff >> (8 - currStream->bits_to_go));
