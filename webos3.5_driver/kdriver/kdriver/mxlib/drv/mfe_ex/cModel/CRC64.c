...
void CRC_Generator(CRC_INFO* pInfo)
{
	...
	N_bits -= i*8;
    offset = i;
	for (i=0;i<N_bits/64;i++) {
		// generate 64 bit at a time
		for (j=0; j<8; j++)
			data_new[j] = pInfo->pBuf[8*i+j+offset];
	...
	if(N_bits%64 != 0) {
		if (!pInfo->bIsLast) {
			for (j=0; j<m/8; j++)
				pInfo->TempBuf[j] = pInfo->pBuf[8*n+j+offset];
			pInfo->nLastBytes = j;
			return;
		}
		// The last bytes
		for (j=0; j<m/8; j++)
			data_new[j] = pInfo->pBuf[8*n+j+offset];
		for (j=m/8; j<8; j++)
			data_new[j] = 0;
		// call bit array CRC64 generator
		CRC64_BIT((BitArray*)data_new, (BitArray*)pInfo->CRCValue, 1);
