...
{
    pConfig->active_sps = MfeDrvMemMalloc(sizeof(seq_parameter_set_rbsp_t),"active_sps");
    ...
    pConfig->active_pps = MfeDrvMemMalloc(sizeof(pic_parameter_set_rbsp_t),"active_pps");
    ...
    {
        ms_dprintk(DRV_L0, "Failed to allocate pps!\m");
    }
}

void FreeSPSPPSBuf(MFE_CONFIG* pConfig)
{
    MfeDrvMemFree(&pConfig->active_sps, "active_sps");
    MfeDrvMemFree(&pConfig->active_pps, "active_pps");
}
...
static NALU_t* _GenerateSeq_parameter_set_NALU (MFE_CONFIG* pConfig)
{
    ...
    int RBSPlen = 0;
//    int NALUlen;
    MFE_BYTE *rbsp = (MFE_BYTE *)MfeDrvMemMalloc(sizeof(MFE_BYTE)*MAXRBSPSIZE, "rbsp");
    if(rbsp == NULL)
    {
        printf("rbsp NULL error\n");
        return NULL;
    }
...
ms_dprintk(DRV_L4,"In GenerateSequenceParameterSet");
    // *************************************************************************
    // Sequence Parameter Set
    // *************************************************************************
    MFE_ASSERT (sps != NULL);
ms_dprintk(DRV_L4,"In GenerateSequenceParameterSet 2");
...
*/
    ...
    sps->frame_mbs_only_flag = (MFE_BOOL) (pConfig->bInterlace == PROGRESSIVE);
    ...
    int ntmpDispHeight = pConfig->nDispHeight;
...
static void _GenerateVUIParameters(MFE_CONFIG *pConfig, seq_parameter_set_rbsp_t *sps)
{
  ...
  vui->aspect_ratio_info_present_flag      = (MFE_BOOL) (pConfig->setVUI_aspect_ratio_info_present_flag==1);
  vui->aspect_ratio_idc                    = (unsigned int) 1;
  vui->sar_width                           = (unsigned int) 0;
  vui->sar_height                          = (unsigned int) 0;

  if(pConfig->setVUI_aspect_ratio_info_present_flag==1)
  {
    vui->aspect_ratio_idc                    = (unsigned int) pConfig->setVUI_aspect_ratio_idc;
    vui->sar_width                           = (unsigned int) pConfig->setVUI_sar_width;
    vui->sar_height                          = (unsigned int) pConfig->setVUI_sar_height;
  }
  ...
  case 2997:
	  ...
	  break;
  default:
	  vui->num_units_in_tick = (unsigned int) 50;
	  vui->time_scale        = (unsigned int) pConfig->FrameRatex100;
	  //MFE_ASSERT(0);
  }
...
static int SliceHeader(MFE_CONFIG* pConfig, MFE_BYTE *rbsp)
{
    ...
    len += ue_v("SH: pic_parameter_set_id" , pConfig->active_pps->pic_parameter_set_id ,bitstream);
...
*/
	...
	if (pConfig->vopPredType == P_VOP)
	{
        ...
        ms_dprintk(DRV_L4,"override_flag = %d,num_ref_idx_l0_active=%d,num_ref_idx_l0_active_minus1=%d",override_flag,pInfo->num_ref_idx_l0_active,pConfig->active_pps->num_ref_idx_l0_active_minus1);
    ...
    {
        len += ue_v("SH: cabac_init_idc", 0, bitstream);
    }

	len += se_v("SH: slice_qp_delta", (pInfo->intQP - 26 - pConfig->active_pps->pic_init_qp_minus26), bitstream);
