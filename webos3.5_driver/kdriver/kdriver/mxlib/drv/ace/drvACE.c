...
static MS_U16 SRAM1_IHC[MAX_SRAM_SIZE];
static MS_U16 SRAM2_IHC[MAX_SRAM_SIZE];
static MS_U16 SRAM3_IHC[MAX_SRAM_SIZE];
static MS_U16 SRAM4_IHC[MAX_SRAM_SIZE];
...
void MDrv_ACE_PicSetContrast(void* pInstance, MS_BOOL bScalerWin, MS_BOOL bUseYUVSpace, MS_U8 u8Contrast )
{
    ...
    {
        msAdjustPCContrast(pInstance, bScalerWin, u8Contrast);
    }
}


void MDrv_ACE_PicSetHSC(void* pInstance, MS_BOOL bScalerWin, MS_BOOL bUseYUVSpace, MS_U8 u8Hue,MS_U8 u8Saturation,MS_U8 u8Contrast )
{
    if(bUseYUVSpace)
    {
        msAdjustHSC(pInstance, bScalerWin, u8Hue, u8Saturation, u8Contrast);
    }
    else
    {
        msAdjustPCContrast(pInstance, bScalerWin, u8Contrast);
...
MS_U16 MDrv_ACE_GetACEInfo(void* pInstance, MS_BOOL bWindow, MS_U16 u16ACEinfo )
{
    ...
    //else if( SC_R2BYTEMSK(psACEInstPri->u32DeviceID, REG_SC_BK25_01_L, BIT(4)) && ((eACEInfoType == ACE_INFO_TYPE_COLOR_CORRECTION_OFFSET_R) || (eACEInfoType == ACE_INFO_TYPE_COLOR_CORRECTION_OFFSET_G) || (eACEInfoType == ACE_INFO_TYPE_COLOR_CORRECTION_OFFSET_B)))
    //{
...
void MDrv_ACE_Write_BT2020_ColorMatrixBurst(void* pInstance, MS_BOOL bWindow, MS_U16* psMatrix )
{
    Hal_ACE_Write_BT2020_Matrix_Burst(pInstance, bWindow,psMatrix);
}
...
MS_BOOL MDrv_ACE_SetBT2020_Colormetry(void* pInstance, XC_ACE_BT2020_TYPE eValue)
{
    u8BT2020_Colormetry = eValue;
    return TRUE;
}
MS_U8 MDrv_ACE_GetBT2020_Colormetry(void)
{
    return u8BT2020_Colormetry;
}
