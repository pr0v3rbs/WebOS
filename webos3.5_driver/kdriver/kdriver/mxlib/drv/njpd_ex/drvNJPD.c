...
static MS_U32  u32MRC0_Start[E_NJPD_NJPD_TOTAL];
static MS_U32  u32MRC0_End[E_NJPD_NJPD_TOTAL];
static MS_U32  u32MRC1_Start[E_NJPD_NJPD_TOTAL];
static MS_U32  u32MRC1_End[E_NJPD_NJPD_TOTAL];
static MS_U32  u32READ_Start[E_NJPD_NJPD_TOTAL];
...
{
    NJPD_DEBUG_DRV_MSG("_NJPD_ISRHandler: IsrNum = %ld\n", IsrNum);
#ifdef MSOS_TYPE_LINUX_KERNEL
    gbCallbackFlag = TRUE;
#else
    if(_pNJPDIsrCb)
...
void _NJPD_PrintMem(MS_U32 u32Addr, MS_U32 u32Size)
{
    MS_U32 u32i;
    NJPD_DEBUG_DRV_MSG("===========================================================\n");
    NJPD_DEBUG_DRV_MSG("print memory addr=0x%lx, size=0x%lx\n", u32Addr, u32Size);
    NJPD_DEBUG_DRV_MSG("===========================================================\n");
...
void MDrv_NJPD_SetReadBuffer0(MS_PHYADDR u32BufAddr, MS_U32 u32BufSize)
{
    MS_U8 u8MiuSel = 0;
    ...
    HAL_NJPD_SetMRCBuf0_StartLow(u32BufAddr & 0xffff);
    HAL_NJPD_SetMRCBuf0_StartHigh(u32BufAddr >> 16);

    u32MRC0_End[eNJPDNum] = (( u32BufAddr + u32BufSize ) - 1);
    // set end address of read buffer
    HAL_NJPD_SetMRCBuf0_EndLow((( u32BufAddr + u32BufSize ) - 1)& 0xffff);
    HAL_NJPD_SetMRCBuf0_EndHigh((( u32BufAddr + u32BufSize ) - 1) >> 16);

}

void MDrv_NJPD_SetReadBuffer1(MS_PHYADDR u32BufAddr, MS_U32 u32BufSize)
{
    MS_U8 u8MiuSel = 0;
    _phy_to_miu_offset(u8MiuSel, u32BufAddr, u32BufAddr);
    HAL_NJPD_SetMIU(u8MiuSel);
    ...
    // set start address of read buffer
    HAL_NJPD_SetMRCBuf1_StartLow(u32BufAddr & 0xffff);
    HAL_NJPD_SetMRCBuf1_StartHigh(u32BufAddr >> 16);

    u32MRC1_End[eNJPDNum] = (( u32BufAddr + u32BufSize ) - 1);
    // set end address of read buffer
    HAL_NJPD_SetMRCBuf1_EndLow((( u32BufAddr + u32BufSize ) - 1)& 0xffff);
    HAL_NJPD_SetMRCBuf1_EndHigh((( u32BufAddr + u32BufSize ) - 1) >> 16);

}


void MDrv_NJPD_SetMRCStartAddr(MS_PHYADDR u32ByteOffset)
{
    MS_U8 u8MiuSel = 0;
    ...
    HAL_NJPD_SetMRCStart_Low((u32ByteOffset & 0xffff));
...
MS_BOOL MDrv_NJPD_SetReadBuffer_Patch(MS_PHYADDR u32BufAddr, MS_U32 u32BufSize, MS_PHYADDR u32BufStartAddr)
{
    ...
    HAL_NJPD_SetMRCBuf0_StartLow(u32MRC0_Start[eNJPDNum] & 0xffff);
    HAL_NJPD_SetMRCBuf0_StartHigh(u32MRC0_Start[eNJPDNum] >> 16);
    HAL_NJPD_SetMRCBuf0_EndLow(u32MRC0_End[eNJPDNum]& 0xffff);
    HAL_NJPD_SetMRCBuf0_EndHigh(u32MRC0_End[eNJPDNum] >> 16);
    HAL_NJPD_SetMRCBuf1_StartLow(u32MRC1_Start[eNJPDNum] & 0xffff);
    HAL_NJPD_SetMRCBuf1_StartHigh(u32MRC1_Start[eNJPDNum] >> 16);
    HAL_NJPD_SetMRCBuf1_EndLow(u32MRC1_End[eNJPDNum] & 0xffff);
...
#else
MS_BOOL MDrv_NJPD_SetReadBuffer_New(MS_PHYADDR u32BufAddr, MS_U32 u32BufSize, MS_PHYADDR u32BufStartAddr)
{
    ...
    NJPD_DEBUG_DRV_MSG("MRC0 start:%lx, MRC0 end:%lx, Offset:%lx\n",
        u32MRC0_Start[eNJPDNum], u32MRC0_End[eNJPDNum] , u32READ_Start[eNJPDNum]);

    NJPD_DEBUG_DRV_MSG("[offset: MRC0: MRC1]=[%ld, %ld, %ld]\n",
        (u32READ_Start[eNJPDNum]/16 - u32MRC0_Start[eNJPDNum]/16 +1)%16,
        (u32MRC0_End[eNJPDNum]/16 - u32MRC0_Start[eNJPDNum]/16 +1)%16,
        (u32MRC1_End[eNJPDNum]/16 - u32MRC1_Start[eNJPDNum]/16 +1)%16
        );
...
///@return none
/******************************************************************************/
void MDrv_NJPD_SetOutputFrameBuffer(MS_PHYADDR u32BufAddr, MS_U16 u16LineNum)
{
    ...
    HAL_NJPD_SetMWCBuf_StartLow(u32BufAddr & 0xffff);
...
MS_BOOL MDrv_NJPD_InitBuf(NJPD_BufCfg in, MS_BOOL bMJPEG)
{
    ...
    else
    {
        ...
        {
            NJPD_DEBUG_DRV_MSG("MRC start:%lx, MRC end:%lx, Offset:%lx\n", in.u32MRCBufAddr, in.u32MRCBufAddr + in.u32MRCBufSize, in.u32MRCBufAddr + in.u32MRCBufOffset);
...
///@return none
/******************************************************************************/
void MDrv_NJPD_SetWPENEndAddr(MS_PHYADDR u32ByteOffset)
{
    MS_U8 u8MiuSel = 0;
    _phy_to_miu_offset(u8MiuSel, u32ByteOffset, u32ByteOffset);
    UNUSED(u8MiuSel);

    NJPD_DEBUG_DRV_MSG("MDrv_NJPD_SetWPENEndAddr=0x%lx\n", u32ByteOffset);
    HAL_NJPD_SetWPENUBound_0_L(( u32ByteOffset >> 3 ) & 0xffff);
    HAL_NJPD_SetWPENUBound_0_H(( u32ByteOffset >> 3 ) >> 16);
}

void MDrv_NJPD_SetWPENStartAddr(MS_PHYADDR u32ByteOffset)
{
    MS_U8 u8MiuSel = 0;
    _phy_to_miu_offset(u8MiuSel, u32ByteOffset, u32ByteOffset);
    UNUSED(u8MiuSel);

    NJPD_DEBUG_DRV_MSG("MDrv_NJPD_SetWPENStartAddr=0x%lx\n", u32ByteOffset);
    HAL_NJPD_SetWPENLBound_0_L(( u32ByteOffset >> 3 ) & 0xffff);
...
void MDrv_NJPD_SetSoftwareVLD(MS_BOOL bEnable)
{
    ...
    if(bEnable)
    {
        ...
        HAL_NJPD_SetSpare00(HAL_NJPD_GetSpare00() & 0xfffe);
    }
}
...
void MDrv_NJPD_SetHTableStartAddr(MS_PHYADDR u32ByteOffset)
{
    MS_U8 u8MiuSel = 0;
    _phy_to_miu_offset(u8MiuSel, u32ByteOffset, u32ByteOffset);
    UNUSED(u8MiuSel);

    HAL_NJPD_SetHTableStart_Low((u32ByteOffset & 0xffff));
    HAL_NJPD_SetHTableStart_High((u32ByteOffset >> 16));
}

void MDrv_NJPD_SetQTableStartAddr(MS_PHYADDR u32ByteOffset)
{
    MS_U8 u8MiuSel = 0;
    _phy_to_miu_offset(u8MiuSel, u32ByteOffset, u32ByteOffset);
    UNUSED(u8MiuSel);

    HAL_NJPD_SetQTableStart_Low((u32ByteOffset & 0xffff));
    HAL_NJPD_SetQTableStart_High((u32ByteOffset >> 16));
}


void MDrv_NJPD_SetGTableStartAddr(MS_PHYADDR u32ByteOffset)
{
    MS_U8 u8MiuSel = 0;
    _phy_to_miu_offset(u8MiuSel, u32ByteOffset, u32ByteOffset);
    UNUSED(u8MiuSel);

    HAL_NJPD_SetGTableStart_Low((u32ByteOffset & 0xffff));
