...
MS_U32 BDMAIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    switch(u32Cmd)
    {
        case MDrv_CMD_BDMA_MemCopy:
            ...
            u32Ret = psBDMAInstPri->fpBDMAMemCpy(pMemCpyParam->u32SrcAddr,pMemCpyParam->u32DstAddr, pMemCpyParam->u32Len);
        ...
        case MDrv_CMD_BDMA_Search:
            ...
            u32SearchAddr = psBDMAInstPri->fpBDMASearch(pSearchParam->u32Addr, pSearchParam->u32Len, pSearchParam->u32Pattern, pSearchParam->u32ExcluBit, pSearchParam->eDev);
            *(pSearchParam->pSearchAddr) = u32SearchAddr;
            UtopiaResourceRelease(pResource);
            if (0xFFFFFFFF != u32SearchAddr)
                return UTOPIA_STATUS_SUCCESS;
            else
                return UTOPIA_STATUS_FAIL;
        case MDrv_CMD_BDMA_CRC32:
            ...
            u32CRC32 = psBDMAInstPri->fpBDMACRC32(pCRC32Param->u32Addr,pCRC32Param->u32Len,pCRC32Param->u32Poly,pCRC32Param->u32Seed,pCRC32Param->eDev,pCRC32Param->bReflect);
            UtopiaResourceRelease(pResource);
        ...
        case MDrv_CMD_BDMA_MOBFSearch:
            ...
            u32Ret = psBDMAInstPri->fpBDMAMOBFSearch(pMOBFSearchParam->u32Addr,pMOBFSearchParam->u32Len,pMOBFSearchParam->pMobfPsCfg,pMOBFSearchParam->eDev);
            UtopiaResourceRelease(pResource);
            pMOBFSearchParam->u32SearchAddr = u32Ret;
            if (0xFFFFFFFF != u32Ret)
                return UTOPIA_STATUS_SUCCESS;
            else
                return UTOPIA_STATUS_FAIL;
#else
