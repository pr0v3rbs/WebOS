...
////////////////////////////////////////////////////////////////////////////////
#define BDMA_UNUSED(expr) do { (void)(expr); } while (0)
...
static void _BDMA_MemFlush(BDMA_Dev eDev, MS_U32 u32Base, MS_U32 u32Size)
{
    BDMA_DBG_FUNC();
    BDMA_UNUSED(eDev);
#if 0// hank fixed Unhandled fault
...
static BDMA_Result _BDMA_CmnHnd(BDMA_OP_CB sOpCB, MS_U8 *pu8Ch)
{
    ...
    //Set start address
    HAL_BDMA_SetSrcAddr(sOpCB.eAct, sOpCB.u32SrcAddr, *pu8Ch);
    HAL_BDMA_SetDstAddr(sOpCB.eAct, sOpCB.u32DstAddr, *pu8Ch);
    //Set size
...
static BDMA_Result _DMA_FlashCopy(BDMA_OP_CB sOpCB)
{
    ...
    //Set start address
    HAL_BDMA_SetSrcAddr(sOpCB.eAct, sOpCB.u32SrcAddr, u8Ch);
    HAL_BDMA_SetDstAddr(sOpCB.eAct, sOpCB.u32DstAddr, u8Ch);
    //Set size
    ...
    if (_BDMA_Start(sOpCB.u8SrcDevCfg, sOpCB.u8DstDevCfg, u8Ch))
    {
        ...
        if (E_BDMA_OK == eRet)
        {
            ...
            if (!_gsHwInfo.bEnDmyWrCnt && (E_BDMA_DEV_MIU0 == (sOpCB.u8DstDevCfg&0x0F) || E_BDMA_DEV_MIU1 == (sOpCB.u8DstDevCfg&0x0F) || E_BDMA_DEV_MIU2 == (sOpCB.u8DstDevCfg&0x0F) || E_BDMA_DEV_MIU3 == (sOpCB.u8DstDevCfg&0x0F)))
                MDrv_BDMA_CRC32(sOpCB.u32DstAddr, HAL_BDMA_DMY_WRCNT, BDMA_CRC32_POLY, BDMA_CRC_SEED_0, (BDMA_SrcDev)(sOpCB.u8DstDevCfg&0x0F), FALSE);
        }
        return eRet;
    }
    else
...
/// @param <GLOBAL>     \b None :
////////////////////////////////////////////////////////////////////////////////
static BDMA_Dev _BDMA_Check_MIUDev(BDMA_Dev eDev, MS_U32 u32Addr)
{
    MS_U8 u8MiuSel;
    MS_U32 u32Offset;
    BDMA_DBG_FUNC();
...
BDMA_Result MDrv_BDMA_Init(MS_U32 u32Miu1Base)
{
    ...
    if (!_gsInfo.bInit)
    {
        ...
        _gsInfo.u32MIU1Base = HAL_MIU1_BASE;
...
BDMA_Result _MDrv_BDMA_PatternFill(MS_PHYADDR u32Addr, MS_U32 u32Len, MS_U32 u32Pattern, BDMA_DstDev eDev)
{
    ...
    _BDMA_MemFlush((BDMA_Dev)eDev, sOpCB.u32DstAddr, sOpCB.u32DataSize);
...
(
    MS_PHYADDR u32SrcAddr, MS_PHYADDR u32DstAddr, MS_U32 u32Len, BDMA_CpyType eCpyType, MS_U8 u8OpCfg
)
{
    MS_U8 u8MiuSel;
    MS_U8 u8Ch;
    ...
    if (eSrcDev == eDstDev)
    {
        if ((u32SrcAddr < u32DstAddr) && ((u32SrcAddr+u32Len-1) >= u32DstAddr))
        {
    ...
    _BDMA_MemFlush(eSrcDev, u32SrcAddr, sOpCB.u32DataSize);
    _BDMA_MemFlush(eDstDev, u32DstAddr, sOpCB.u32DataSize);
    ...
    {
        _phy_to_miu_offset(u8MiuSel, u32Offset, u32DstAddr);
        u32DstAddr = u32Offset;

        _phy_to_miu_offset(u8MiuSel, u32Offset, sOpCB.u32DstAddr);
        sOpCB.u32DstAddr = u32Offset;
    }

    if((E_BDMA_SRCDEV_MIU1 == (BDMA_SrcDev)eSrcDev) || (E_BDMA_DSTDEV_MIU2 == (BDMA_DstDev)eDstDev) || (E_BDMA_DSTDEV_MIU3 == (BDMA_DstDev)eDstDev))
    {
        _phy_to_miu_offset(u8MiuSel, u32Offset, u32SrcAddr);
        u32SrcAddr = u32Offset;

        _phy_to_miu_offset(u8MiuSel, u32Offset, sOpCB.u32SrcAddr);
        sOpCB.u32SrcAddr = u32Offset;
    }
    ...
    {
        u32CRCVal = MDrv_BDMA_CRC32(u32SrcAddr, u32Len, BDMA_CRC32_POLY, BDMA_CRC_SEED_0, (BDMA_SrcDev)eSrcDev, FALSE);
    }
    if(E_BDMA_OK == (eRet = _BDMA_CmnHnd(sOpCB, &u8Ch)))
    {
        ...
        {
            MDrv_BDMA_CRC32(sOpCB.u32DstAddr, HAL_BDMA_DMY_WRCNT, BDMA_CRC32_POLY, BDMA_CRC_SEED_0, (BDMA_SrcDev)eDstDev, FALSE);
        }

        if (u8OpCfg & BDMA_OPCFG_CRC_COPY)
        {
            u32DstAddr = MDrv_BDMA_CRC32(u32DstAddr, u32Len, BDMA_CRC32_POLY, BDMA_CRC_SEED_0, (BDMA_SrcDev)eDstDev, FALSE);
            if ( (u32CRCVal != u32DstAddr) )
            {
        ...
        {
            MS_DEBUG_MSG(printf("bdma copy:%lu ms\n", MsOS_GetSystemTime() - u32Time));
        }
        return E_BDMA_OK;
    }
...
BDMA_Result _MDrv_BDMA_MemCopy(MS_PHYADDR u32SrcAddr, MS_PHYADDR u32DstAddr, MS_U32 u32Len)
{
    ...
    eSrc = (MS_U8)_BDMA_Check_MIUDev(E_BDMA_DEV_MIU0, u32SrcAddr);
    eDst = (MS_U8)_BDMA_Check_MIUDev(E_BDMA_DEV_MIU0, u32DstAddr);
...
{
    MS_U8 eDst = (MS_U8)_BDMA_Check_MIUDev(E_BDMA_DEV_MIU0, u32DramAddr);
    BDMA_CpyType eCpyType;
...
MS_U32 _MDrv_BDMA_Search(MS_PHYADDR u32Addr, MS_U32 u32Len, MS_U32 u32Pattern, MS_U32 u32ExcluBit, BDMA_SrcDev eDev)
{
    ...
    if (E_BDMA_OK == _BDMA_CmnHnd(sOpCB, &u8Ch))
    {
        ...
        {
            return HAL_BDMA_GetMatched(u32Addr, u8Ch);
        }
        else
        {
...
MS_U32 _MDrv_BDMA_MOBFSearch(MS_PHYADDR u32Addr, MS_U32 u32Len, BDMA_MOBF_PS* pMobfPsCfg, BDMA_SrcDev eDev)
{
	...
	if (HAL_BDMA_Is_Found(u8Ch))
        {
            u32RetAddr = (HAL_BDMA_GetMatched(u32Addr, u8Ch)-u32Addr);
        }
      else
        {
...
void MDrv_BDMA_DumpCB(void *pvOpCB)
{
    ...
    if (pvOpCB)
    {
        ...
        MS_DEBUG_MSG(printf("\tDump BDMA CB.......\n"));
        MS_DEBUG_MSG(printf("\tAct: %u Src dev: 0x%x Dst dev: 0x%x\n",psOpCB->eAct, psOpCB->u8SrcDevCfg, psOpCB->u8DstDevCfg));
        MS_DEBUG_MSG(printf("\tSrc addr: %lx Dst addr: %lx\n",psOpCB->u32SrcAddr, psOpCB->u32DstAddr));
        MS_DEBUG_MSG(printf("\tData len: %lx\n",psOpCB->u32DataSize));
        MS_DEBUG_MSG(printf("\tCmd0: %lx Cmd1: %lx Cmd2: %lx\n",psOpCB->u32Cmd0, psOpCB->u32Cmd1, psOpCB->u32Cmd2));
        MS_DEBUG_MSG(printf("\tDmyWrCnt: %u OpCfg: %u Cbf:%lx\n",psOpCB->u8DmyWrCnt, psOpCB->u8OpCfg, (MS_U32)psOpCB->pCbf));
    }
}
...
void MDrv_BDMA_Internal_Test(void)
{
    ...
    _BDMA_Check_MIUDev(E_BDMA_DEV_MIU0, _gsInfo.u32MIU1Base);
...
BDMA_Result MDrv_BDMA_MemCopy(MS_PHYADDR u32SrcAddr, MS_PHYADDR u32DstAddr, MS_U32 u32Len)
{
    ...
    return E_BDMA_OK;
#else
    return _MDrv_BDMA_MemCopy(u32SrcAddr,u32DstAddr,u32Len);
#endif
}
...
MS_U32 MDrv_BDMA_Search(MS_PHYADDR u32Addr, MS_U32 u32Len, MS_U32 u32Pattern, MS_U32 u32ExcluBit, BDMA_SrcDev eDev)
{
    ...
    if (NULL == pSearchParam)
        return E_BDMA_FAIL;
    pSearchParam->u32Addr = u32Addr;
    pSearchParam->u32Len = u32Len;
    pSearchParam->u32Pattern = u32Pattern;
    pSearchParam->u32ExcluBit = u32ExcluBit;
    pSearchParam->eDev = eDev;
    pSearchParam->pSearchAddr = &u32SearchAddr;
    if (UtopiaIoctl(pInstantBdma,MDrv_CMD_BDMA_Search,(void*)pSearchParam) != UTOPIA_STATUS_SUCCESS)
    {
        free(pSearchParam);
        return 0xFFFFFFFF;
    }
...
#else
    return _MDrv_BDMA_Search(u32Addr, u32Len, u32Pattern, u32ExcluBit, eDev);
#endif
}
...
MS_U32 MDrv_BDMA_CRC32(MS_PHYADDR u32Addr, MS_U32 u32Len, MS_U32 u32Poly, MS_U32 u32Seed, BDMA_SrcDev eDev, MS_BOOL bReflect)
{
    ...
    return u32CRC32;
#else
    return _MDrv_BDMA_CRC32(u32Addr, u32Len, u32Poly, u32Seed, eDev, bReflect);
#endif
}
...
BDMA_Result MDrv_BDMA_PatternFill(MS_PHYADDR u32Addr, MS_U32 u32Len, MS_U32 u32Pattern, BDMA_DstDev eDev)
{
    ...
    if (NULL == pPattFillParam)
        return E_BDMA_FAIL;
    pPattFillParam->u32Addr = u32Addr;
    ...
    {
        printf("Ioctl MDrv_BDMA_PatternFill fail\n");
        free(pPattFillParam);
        return E_BDMA_FAIL;
    }
    free(pPattFillParam);
    return E_BDMA_OK;
#else
    return _MDrv_BDMA_PatternFill(u32Addr, u32Len, u32Pattern, eDev);
#endif
}
...
BDMA_Result MDrv_BDMA_FlashCopy2Dram(MS_PHYADDR u32FlashAddr, MS_PHYADDR u32DramAddr, MS_U32 u32Len)
{
    ...
    if (NULL == pFlash2DramParam)
        return E_BDMA_FAIL;
    pFlash2DramParam->u32FlashAddr = u32FlashAddr;
    pFlash2DramParam->u32DramAddr = u32DramAddr;
    ...
    {
        printf("Ioctl MDrv_BDMA_FlashCopy2Dram fail\n");
    ...
    return E_BDMA_OK;
#else
    return _MDrv_BDMA_FlashCopy2Dram(u32FlashAddr, u32DramAddr, u32Len);
#endif
}
...
MS_U32 MDrv_BDMA_MOBFSearch(MS_PHYADDR u32Addr, MS_U32 u32Len, BDMA_MOBF_PS* pMobfPsCfg, BDMA_SrcDev eDev)
{
    ...
    if (NULL == pMOBFSearchParam)
        return E_BDMA_FAIL;
    pMOBFSearchParam->u32Addr = u32Addr;
    ...
    u32Ret = pMOBFSearchParam->u32SearchAddr;
    free(pMOBFSearchParam);
    return u32Ret;
#else
...
void MDrv_BDMA_GetStatus(BDMA_Status *pStatus)
{
    ...
    {
        printf("Ioctl MDrv_BDMA_GetStatus fail\n");
    }
...
BDMA_Result MDrv_BDMA_WaitFlashDone(void)
{
    ...
    {
        printf("Ioctl MDrv_BDMA_WaitFlashDone fail\n");
        return E_BDMA_FAIL;
    }
    return E_BDMA_OK;
#else
...
#if defined(MSOS_TYPE_LINUX_KERNEL)
static MS_U8 _pm_dat[] =
{
    #include "pmlite.dat"
};
#endif

void MDrv_BDMA_LoadPM2SRAM(void)
{
#if defined(MSOS_TYPE_LINUX_KERNEL)
    MS_U8 *pmVAaddress = 0 ; 

    #define PM_DRAM_ADDRESS 0x00100000
    #define PM_SIZE 0x6000

    if(MsOS_MPool_Mapping(0, PM_DRAM_ADDRESS, PM_SIZE, 1) == false)
    {
        printf("[%s][%d] mapping fail. \n",__FUNCTION__, __LINE__);
        return;
    }

    pmVAaddress = (MS_U8 *)MsOS_MPool_PA2KSEG1(PM_DRAM_ADDRESS);

    if( NULL == pmVAaddress )
    {
        printf("[%s][%d] Fail to convert PA to VA. \n",__FUNCTION__, __LINE__);
        return;
    }
    
    memset((void *)pmVAaddress, 0, PM_SIZE);    
    memcpy((void *)pmVAaddress, _pm_dat, PM_SIZE);
    
    MDrv_BDMA_Init(0);
    MDrv_BDMA_CopyHnd(PM_DRAM_ADDRESS, 0x0, (PM_SIZE-1), E_BDMA_SDRAM2SRAM1K_HK51, 0);
#endif    
}
