...
static unsigned int DGifGetWord(GifFileType *GifFile, unsigned int *Word);
...
static int GIFmode = NO_SUPPORT_MGIF;

int MstDGifGetDuration(void)
{
    return GifFile->GraphicCtrl.time;
}

void MstDGifSetMode (int mode)
{
    GIFmode = mode;
}


int read_byte(GifFileType* gif,GifByteType* buf,int len)
{
    int i=0,cnt=len;
    unsigned short tmp;
...
int READ_ram(GifFileType* gif,GifByteType* buf,int len)
{
...
void rgb2yuv(unsigned char r_in, unsigned char g_in, unsigned char b_in,
             unsigned char *y_out_ptr, unsigned char *cb_out_ptr, unsigned char *cr_out_ptr)
{
    int y_term_r;
    int y_term_g;
    int y_term_b;
...
void FillBkColor(GifFileType *GifFile, GifByteType* framebuff, int ocolor,
                 unsigned int width, unsigned int height, unsigned int pitch)
{
    ...
    GifByteType y, cb, cr;
    unsigned long u32color;
    unsigned short u16color;
    unsigned char u8color;
    unsigned char colorbuff[4];
    unsigned int i, pixelbytes;
...
{
    unsigned int left   = GifFile->Image.Left;
    unsigned int right  = GifFile->Image.Left + GifFile->Image.Width;
    unsigned int top    = GifFile->Image.Top;
    unsigned int bottom = GifFile->Image.Top + GifFile->Image.Height;
    ...
    //printf("image roi:  %d, %d, %d, %d\n", image_roi.hstart, image_roi.vstart, image_roi.width, image_roi.height);
    printf("frame roi:  %ld, %ld, %ld, %ld\n", frame_roi.hstart, frame_roi.vstart, frame_roi.width, frame_roi.height);
...
******************************************************************************/
GifFileType *MstDGifProcess(void *ptr, int ocolor_typ, U32 mode, void *roi)
{
    ...
    unsigned int vdata, image_pitch, frame_pitch, pixelbytes;
    unsigned int FrameBuffAddr, TempBuffOffset, StreamBuffAddr;
    unsigned int vstart_shift = 0;
    ...
    if(mode != 3)
    {
        ...
        GifFile = MstDGifOpenFileHandle(FileHandle, READ_ram, (GifByteType*)StreamBuffAddr);
        ...
        switch (RecordType)
        {
          case IMAGE_DESC_RECORD_TYPE:
            ...
            if (GifFile->Image.ColorMap)
                ...
                drv_gpd_write_cmap(GifFile->SColorMap->ColorCount, (unsigned char *)GifFile->SColorMap->Colors, trans_idx);
...
*   info record. _MstGifError is cleared if succesfull.                          *
******************************************************************************/
GifFileType *MstDGifOpenFileHandle(int FileHandle, InputFunc read_fun, GifByteType* bitstream)
{
    ...
    GifFile->UserData = (unsigned int)bitstream;//0;  /* TVT */
...
ColorMapObject *MstMakeMapObject(int ColorCount, const GifColorType *ColorMap)
/*
...
******************************************************************************/
int MstDGifGetScreenDesc(GifFileType *GifFile)
{
    int BitsPerPixel;
...
******************************************************************************/
int MstDGifGetRecordType(GifFileType *GifFile, GifRecordType *Type)
{
    GifByteType Buf = 0;
...
******************************************************************************/
int MstDGifGetImageDesc(GifFileType *GifFile)
{
    int BitsPerPixel;
...
******************************************************************************/
int MstDGifGetExtension(GifFileType *GifFile, int *ExtCode,
                            GifByteType **Extension)
{
    GifByteType Buf = 0;
    GifFilePrivateType *Private = (GifFilePrivateType *) GifFile->Private;
...
*   The Extension should NOT be freed by the user (not dynamically allocated).*
******************************************************************************/
int MstDGifGetExtensionNext(GifFileType *GifFile, GifByteType **Extension)
{
    GifByteType Buf = 0;
    GifFilePrivateType *Private = (GifFilePrivateType *) GifFile->Private;
...
******************************************************************************/
#ifdef SW_DEC
int MstDGifGetLine(GifFileType *GifFile, GifPixelType *Line, int LineLen)
{
    GifByteType *Dummy;
    GifFilePrivateType *Private = (GifFilePrivateType *) GifFile->Private;
...
******************************************************************************/
int MstDGifGetPixel(GifFileType *GifFile, GifPixelType Pixel)
{
    GifByteType *Dummy;
    GifFilePrivateType *Private = (GifFilePrivateType *) GifFile->Private;
...
******************************************************************************/
int MstDGifGetCode(GifFileType *GifFile, int *CodeSize, GifByteType **CodeBlock)
{
    GifFilePrivateType *Private = (GifFilePrivateType *) GifFile->Private;
...
*   The block should NOT be freed by the user (not dynamically allocated).    *
******************************************************************************/
int MstDGifGetCodeNext(GifFileType *GifFile, GifByteType **CodeBlock)
{
    GifByteType Buf;
    GifFilePrivateType *Private = (GifFilePrivateType *) GifFile->Private;
...
******************************************************************************/
int MstDGifGetLZCodes(GifFileType *GifFile, int *Code)
{
...
*
 ******************************************************************************/
int MstDGifSlurp(GifFileType *GifFile)
{
    int ImageSize;
    int gif_err = 0;
    GifRecordType RecordType;
...
******************************************************************************/
int MstDGifCloseFile(GifFileType *GifFile)
{
    GifFilePrivateType *Private;
    //FILE *File;
