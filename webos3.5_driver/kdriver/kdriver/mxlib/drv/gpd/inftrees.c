...
static unsigned short mincode_tmp[MAXBITS+1];
static unsigned short offset_tmp[MAXBITS+2];
static unsigned short count[MAXBITS+1];    /* number of codes of each length */
static unsigned short offs[MAXBITS+2];     /* offsets in table for each length */
...
int gpdinflate_table(type, lens, codes, table, bits, work)
codetype type;
...
unsigned short FAR *work;
{
    ...
    unsigned short mincode=0;
    //unsigned short mincode_tmp[288];
    //unsigned short offset_tmp[MAXBITS+1];
    unsigned short  mincode_valid=0;
   ...
   if(type==CODES)
   {
        ...
        for (;;) {
            ...
            if (len > root && (huff & mask) != low) {
                ...
                while ((curr + drop) < max)
                {
                    unsigned index = (curr + drop)%(MAXBITS+1);
