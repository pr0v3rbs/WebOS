...
extern unsigned short read_fifo(int n);
extern unsigned short show_fifo(int n);
...
#if 0
int ZEXPORT gpdinflateReset(strm)
z_streamp strm;
{
...
int ZEXPORT gpdinflateReset(z_streamp strm)
{
    struct inflate_state FAR *state;
...
int ZEXPORT gpdinflateInit2_(strm, windowBits, version, stream_size)
z_streamp strm;
int windowBits;
const char *version;
int stream_size;
...
int ZEXPORT gpdinflateInit_(strm, version, stream_size)
z_streamp strm;
const char *version;
int stream_size;
{
...
int num = 0, blk_typ;
static unsigned char prev,curr;
extern int nocompress, fix, dynamic;

int ZEXPORT gpdinflate(strm, flag)
z_streamp strm;
int flag;
{
    int i;
    //extern unsigned int huff_reg_time;
    unsigned long ncodebits;
    int ncodetmp;
    //int test1, test2;
    unsigned int dbport, bitoffset = 0, tmpxx;
    //unsigned long hold;
    //unsigned bits;
    unsigned char readdata1/*, readdata2*/;
    unsigned short readtmp;
    unsigned short readdata3;
    struct inflate_state FAR *state;
    unsigned int copy;
...
#endif
    static const unsigned short order[20] = /* permutation of code lengths */
