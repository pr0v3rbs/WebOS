...
#define FIXED_LOGICAL_ADDRESS 1
...
MS_BOOL gRIU_Addr_Ready = FALSE;
MS_BOOL gCEC_Is_Source = FALSE;
...
MS_U8 g_u8CecVendorID[3] = {0x00, 0x00, 0x00};
MS_U8 g_u8RetryCnt = RETRY_CNT;
static MS_U8 u8CecResponse = 0xFF;
//-------------------------------------------------------------------------------------------------
//  Local Functions
//-------------------------------------------------------------------------------------------------
...
void _MDrv_CEC_init_riu_base(void)
{
    ...
    MS_U32 u32NonPMBankSize = 0, u32PMBankSize = 0;
...
//
//**************************************************************************
void MDrv_CEC_PortSelect(MsCEC_INPUT_PORT InputPort)
{
...
//                   error_code:  return status
//**************************************************************************
CEC_ERROR_CODE MDrv_CEC_TxApi2(MsCEC_DEVICELA dst_address, MsCEC_MSGLIST msg, MS_U8* operand_ptr, MS_U8 len)
{
    MS_U8 res, header;
    CEC_ERROR_CODE error_code = E_CEC_FEATURE_ABORT;
#if ENABLE_CEC_MULTIPLE
...
void MDrv_CEC_SetRetryCount(MS_U8 u8RetryCount)
{
    g_u8RetryCnt = u8RetryCount;
}

void MDrv_CEC_ConfigWakeUp(void)
{
    if ( gRIU_Addr_Ready!=TRUE )
        _MDrv_CEC_init_riu_base(); // Get base address to prevent core dump

    mhal_CEC_ConfigWakeUp();
}

void MDrv_CEC_ConfigWakeupInfoVendorID(MS_U8* u8CecVendorID)
{
    memcpy(&g_u8CecVendorID, u8CecVendorID, sizeof(g_u8CecVendorID)/sizeof(MS_U8));
}
...
{
    return gCEC_Is_Source;
}

MS_U32 MDrv_CEC_SetPowerState(EN_POWER_MODE u16PowerState)
{
