...
/*                       Macro                                               */
/*****************************************************************************/
#define PCM_ASSERT_RETURN(x, y) if(!(x)) {printf("Assert Fail : %s, %d\n", __FUNCTION__, __LINE__); return y;}

#if CI_DBG
    #define PCM_PRINT(x, a...) printf(x, ##a)
#else    
...
void CIRegisterToUtopia(FUtopiaOpen ModuleType)
{
    ...
    {
        printf("[%s] create module CI failed\n", __FUNCTION__);
        return;
    }
    ...
    {
        printf("[%s] create resource CI failed\n", __FUNCTION__);
    }
    ...
    {
        printf("[%s][%d]UtopiaResourceObtain PCMCIA fail\n", __FUNCTION__, __LINE__);
        return;
    }
...
MS_U32 PCMCIAIoctl(void* pInstantTmp, MS_U32 u32Cmd, void* const pu32Args)
{
    ...
    switch(u32Cmd)
    {
        ...
        default :
            printf("[PCMCIA] unknown IOCTL 0x%x\n", (int)u32Cmd);
            break;
    }
