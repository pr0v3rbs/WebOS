...
#if defined ( MSOS_TYPE_LINUX )
    #if PCMCIA_UTOPIA2
        #define PCM_ENTER()     if(1 == UtopiaResourceObtain(pModulePcm, E_PCMCIA_RESOURCE, &psResource)) \
                                {                                                                       \
                                    printf("[PCMCIA][%06d] Mutex taking timeout\n", __LINE__);          \
                                }               
...
/*                       Local Functions                                     */
/*****************************************************************************/
static MS_BOOL _MDrv_PCMCIA_ReadReg( MS_U8 u8Addr, MS_U8 *pu8Value );
static MS_BOOL _MDrv_PCMCIA_WriteReg( MS_U8 u8Addr, MS_U8 u8Value );
...
static void _MDrv_PCMCIA_Isr( void )
{
    ...
    {
        printf("[PCMCIA] IRQ but nothing happen\n");
        //MS_ASSERT( 0 );
    }
...
void _MDrv_PCMCIA_Exit( MS_BOOL bSuspend )
{
    if(FALSE == bSuspend)
    {
        ...
        if(Pcmcia_Mutex == -1)
        {
            printf("[%s] PCMCIA mutex not exist!\n", __FUNCTION__);        
            return;
        }
...
{
    MS_U32 u32NonPMBankSize, u32PCMCIA_RiuBaseAddr = 0;
    MS_U32 i = 0;
    ...
    {
        MS_ASSERT(0);
        printf("[%s][%d] open fail /dev/system\n", __FUNCTION__, __LINE__);
    }
    else
    {
        printf ("PCMCIA open /dev/system success >> SYS_fd= %ld\n", SYS_fd);
    }    
#endif
    ...
    if ( MDrv_MMIO_GetBASE( &u32PCMCIA_RiuBaseAddr, &u32NonPMBankSize, MS_MODULE_PCMCIA ) != TRUE )
    {
        ...
        #else
            printf("PCMCIA error RIU base address assigned \n");
        #endif

        printf( "MDrv_PCMCIA_Init GetBASE failure, use default base address: [ %lX ]\n", u32PCMCIA_RiuBaseAddr );
    }
    else
    {
        PCMCIA_DEBUG( ( "PCMCIA Base Addr: [ %lX ]\n", u32PCMCIA_RiuBaseAddr ) );
    }
    ...
    if(-1 != Pcmcia_Mutex)
    {
        printf("[%s] open PCMCIA mutex more than once\n", __FUNCTION__);
        return;
    }
    ...
    if(-1 == Pcmcia_Mutex)
    {
        printf("[%s] open PCMCIA mutex fail\n", __FUNCTION__);
        return;
    }
#endif
...
void _MDrv_PCMCIA_InitHW( MS_BOOL bResume )
{
    ...
    u8Reg = ( BIT6 | BIT4 | BIT0 ); // reg_module_sel(BIT 1~0): module select
                                    //                          01: select module A
...
//    {
//        printf("ERROR: Module 0x%x not support\n", (int)eModule);
//        return FALSE;
//    }
...
MS_BOOL _MDrv_PCMCIA_WriteDataV2( PCMCIA_MODULE eModule, MS_U8* u8pWriteBuffer, MS_U16 u16DataLen )
{
    ...
    {
        printf( "MDrv_PCMCIA_WriteData: not \"free\", retry %d times fail!\r\n", u16TryLoop);
        return FALSE;
    }
...
void _MDrv_PCMCIA_WriteBufferSizeV2( PCMCIA_MODULE eModule, MS_U16 u16BufferSize )
{
    ...
    if ( PCMCIA_HW_MAX_RETRY_COUNT == u16TryLoop )
    {
        printf( "ERROR: MDrv_PCMCIA_WriteBufferSize NG!\n" );
    }
...
MS_U8 MDrv_PCMCIA_ReadyStatus( void )
{
    ...
    _MDrv_PCMCIA_ReadReg( REG_PCMCIA_STAT_INT_RAW_INT, (MS_U8 *) &u8Reg );
        printf("REG_PCMCIA_STAT_INT_RAW_INT %x\n",u8Reg );

    _MDrv_PCMCIA_ReadReg( REG_PCMCIA_STAT_INT_RAW_INT1, (MS_U8 *)&u8Reg2 );
        printf("REG_PCMCIA_STAT_INT_RAW_INT1 %x\n",u8Reg2 );
