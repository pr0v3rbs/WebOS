...
//  Macro and Define
//-------------------------------------------------------------------------------------------------
extern MS_U32 _DLC_RIU_BASE;      // This should be inited before XC library starting.
...
#define RIU_READ_BYTE(addr)         ( READ_BYTE( _DLC_RIU_BASE + (addr) ) )
#define RIU_READ_2BYTE(addr)        ( READ_WORD( _DLC_RIU_BASE + (addr) ) )
#define RIU_WRITE_BYTE(addr, val)   { WRITE_BYTE( _DLC_RIU_BASE + (addr), val) }
#define RIU_WRITE_2BYTE(addr, val)  { WRITE_WORD( _DLC_RIU_BASE + (addr), val) }


//=============================================================
// Standard Form
#if SCALER_REGISTER_SPREAD
#define MDrv_ReadByte( u32Reg )   RIU_READ_BYTE((((REG_SCALER_BASE + ((u32Reg) & 0xFFFF))) << 1) - ((u32Reg) & 1))
#else
...
#define MDrv_WriteRegBit( u32Reg, bEnable, u8Mask )                                     \
    RIU_MACRO_START                                                                     \
    RIU_WRITE_BYTE( ((((REG_SCALER_BASE + ((u32Reg) & 0xFFFF))) <<1) - ((u32Reg) & 1)) , (bEnable) ? (RIU_READ_BYTE(  ((((REG_SCALER_BASE + ((u32Reg) & 0xFFFF))) <<1) - ((u32Reg) & 1))  ) |  (u8Mask)) :                           \
                                (RIU_READ_BYTE( ((((REG_SCALER_BASE + ((u32Reg) & 0xFFFF))) <<1) - ((u32Reg) & 1)) ) & ~(u8Mask)));                            \
    RIU_MACRO_END

#define MDrv_WriteByte( u32Reg, u8Val )                                                 \
    RIU_MACRO_START                                                                     \
    RIU_WRITE_BYTE((((REG_SCALER_BASE + ((u32Reg) & 0xFFFF))) << 1) - ((u32Reg) & 1), u8Val);   \
    RIU_MACRO_END
