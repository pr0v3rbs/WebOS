...
SAR_Result MDrv_SAR_Config(SAR_RegCfg *pSARRegCfg)
{
    ...
    MB_Command.u8Parameters[3] = (MS_S8)(u32SrcAddr&0x000000FF);
    MbxResult = MDrv_MBX_SendMsg(&MB_Command);
    while(E_MBX_SUCCESS != MbxResult)
    {
        //Error Handling here:
...
const SAR_DrvInfo* MDrv_SAR_GetInfo(void)
{
    ...
    //(1) set address to get final result
    u32SrcAddr = (MS_U32)&gSARDrvInfo;
    SAR_DBG_INFO("u32SrcAddr = 0x%08lX\n", u32SrcAddr);
...
static MS_U8 gu8SARCheckCount, gu8SARPrevCMD, gu8SARPressKey;
static MS_U32 gu32SARTimePeriod;
...
static MS_U8 MDrv_SAR_Kpd_GetCHKey(MS_U8 u8Channel, MS_U8 *pu8Key , MS_U8 *pu8Repeat)
{
    ...
    *pu8Repeat = 0;
    ...
    for(i=0; i<MDrv_SAR_Kpd_GetChLevels(u8Index); i++)
    {
        if(KEY_LV[i] >= SAR_LEVEL_STABLE_NUM_MIN)
        {
            ...
            *pu8Key = MDrv_SAR_Kpd_GetChIdxKey(u8Index,i);
            ...
            if (gu8SARPrevCMD != *pu8Key)
            {
                gu8SARPrevCMD = *pu8Key;
                gu8SARCheckCount = 0;
            }
            else
            {
                if (gu8SARCheckCount < SAR_KEY_VALIDATION)
                {
                    gu8SARCheckCount++;
                    return FALSE;
                }
                else if (gu8SARCheckCount == SAR_KEY_VALIDATION)
                {
                    gu8SARCheckCount++;
                    gu32SARTimePeriod = _GetSystemTime(100);
                    return TRUE;
                }

                if (gu8SARCheckCount == SAR_REPEAT_KEY_CHECK)    //SAR_KEY_VALIDATION+2
                {
                    if (_GetSystemTime(100) > (gu32SARTimePeriod + SAR_REPEAT_PERIOD/3))
                    {
                        gu32SARTimePeriod = _GetSystemTime(100);
                        gu8SARCheckCount = SAR_REPEAT_KEY_CHECK_1;
                        *pu8Repeat = 0x01;
                    }
                    else
                    {
                ...
                else if (gu8SARCheckCount == SAR_REPEAT_KEY_CHECK_1) //SAR_KEY_VALIDATION+3
                {
                    if (_GetSystemTime(100) > gu32SARTimePeriod)
                    {
                        gu32SARTimePeriod = _GetSystemTime(100);
                        gu8SARCheckCount = SAR_REPEAT_KEY_CHECK_1;
                        *pu8Repeat = 0x01;
                ...
                if (_GetSystemTime(100) > (gu32SARTimePeriod + SAR_REPEAT_PERIOD))  //if 700ms
                {
                    gu32SARTimePeriod = _GetSystemTime(100);
                    gu8SARCheckCount = SAR_REPEAT_KEY_CHECK; //SAR_KEY_VALIDATION+2
                    *pu8Repeat = 0x01;
                    return TRUE;
                }
                else
    ...
    u8LastCh = g_SARKpdDrvInfo.u8ConfigUsedChs - 1;
    if(u8Index == u8LastCh)
    {
        if (gu8SARPressKey)
            gu8SARPressKey = FALSE;
        else
            gu8SARPrevCMD = 0xFF;
    }
    return ret_val;
...
SAR_KpdResult MDrv_SAR_Kpd_MMIO_Init(void)
{
    ...
    HAL_SAR_SetIOMapBase(u32BaseAddr);
    SAR_KPD_DBG_INFO("u32BaseAddr(PM) = %lx\n", u32BaseAddr);

    if(!MDrv_MMIO_GetBASE(&u32BaseAddr, &u32BaseSize, MS_MODULE_HW))
    {
        SAR_KPD_DBG_ERR("Get NPM IOMAP Base faill!\n");
        return E_SAR_KPD_FAIL;
    }
    HAL_SAR_SetIOMapBase_NPM(u32BaseAddr);
    SAR_KPD_DBG_INFO("u32BaseAddr(NPM) = %lx\n", u32BaseAddr);
...
SAR_KpdResult MDrv_SAR_Kpd_GetKeyCode_U2K(MS_U8 *pu8Key, MS_U8 *pu8Repeat)
{
    ...
    {
		u8Channel = MDrv_SAR_Kpd_GetChID(i) ;
...
//--------------------------------------------------------------------------------------------------
// Utopia2.0 will call this function to get a instance to use SAR
