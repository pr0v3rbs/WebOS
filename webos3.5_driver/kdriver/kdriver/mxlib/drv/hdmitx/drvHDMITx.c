...
#else
static MS_U16 _u16DbgHDMITXSwitch = 0x00;
#endif
...
void MDrv_HDMITx_InitVariable(void)
{
    ...
    gHDMITxInfo.hdmitx_CECEnable_flag = TRUE; // CEC
...
#endif
    g_bHDMITxTask = TRUE;
    _s32HDMITxTaskId = MsOS_CreateTask((TaskEntry)_HDMITx_Task,
                                    (MS_U32)NULL,
                                    E_TASK_PRI_HIGH,
                                    TRUE,
                                    _pHDMITxTaskStack,
...
void MDrv_HDMITx_Set_VS_InfoFrame(void)
{
    ...
    MDrv_HDMITx_SetAVMUTE(TRUE);
    MHal_HDMITx_SetTMDSOnOff(gHDMITxInfo.hdmitx_RB_swap_flag, FALSE);
    MsOS_DelayTask(20); //delay 1 frame/field interval
    MHal_HDMITx_SetTMDSOnOff(gHDMITxInfo.hdmitx_RB_swap_flag, TRUE);
    MDrv_HDMITx_SetAVMUTE(FALSE);
}
...
void MDrv_HDMITx_SetVideoOutputMode(void)
{
    ...
    if( (gHDMITxInfo.output_video_timing == E_HDMITX_RES_1920x1080p_24Hz) || (gHDMITxInfo.output_video_timing == E_HDMITX_RES_1920x1080p_25Hz) || (gHDMITxInfo.output_video_timing == E_HDMITX_RES_1920x2205p_24Hz)) // 24/25 Hz
