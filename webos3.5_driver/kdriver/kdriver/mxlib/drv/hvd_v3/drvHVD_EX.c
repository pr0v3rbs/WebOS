...
#endif
#define HVD_U64_MAX                 0xffffffffffffffffULL
#define _MS_TO_90K(x)         (x*90)  //ms ==> 90k counter
#define _90K_TO_MS(x)         (x/90)  //90k counter ==> ms
#define _90K_TO_MS_U64(x)   ((x!=HVD_U64_MAX)?(do_div(x,90)):(x))  //90k counter ==> ms
...
#ifdef VDEC3
static HVD_EX_Result   _HVD_EX_InitRegCPU(MS_U32 u32Id, MS_BOOL bFWdecideFB);
#else
...
static MS_U32 _HVD_EX_GetMIUBase(MS_U8 u8DrvId, MS_U32 u32MIU);

MS_U32 u32UartCtrl = (E_HVD_UART_CTRL_ERR | E_HVD_UART_CTRL_MUST);
#ifdef MSOS_TYPE_LINUX_KERNEL
extern MS_BOOL MsOS_MPool_VAError(unsigned long u32VA);
#endif
typedef struct
{
    ...
    struct CMA_Pool_Free_Param cmaFreeParam[2][HVD_MAX_STREAMS];
    MS_BOOL bCMATwoMIU[HVD_MAX_STREAMS];
#endif
    MS_BOOL bHVDIsrAttached;  //HVD ISR is attach or not
    MS_BOOL bHVDMSOSIsrEnable;  //HVD msos isr enable or not
    MS_BOOL bEVDIsrAttached;  //EVD ISR is attach or not
    MS_BOOL bEVDMSOSIsrEnable;  //EVD msos isr enable or not
...
static int _HVD_ExStreamTypeToInt(HVD_EX_DRV_StreamType eType)
{
    int res = 0;
    switch(eType)
    {
        case E_HVD_EX_DRV_STREAM_NONE   : res = 0; break;
        case E_HVD_EX_DRV_MAIN_STREAM   : res = 1; break;
        case E_HVD_EX_DRV_SUB_STREAM    : res = 2; break;
        case E_HVD_EX_DRV_MVC_STREAM    : res = 3; break;
#ifdef VDEC3
        case E_HVD_EX_DRV_N_STREAM      : res = 4; break;
#endif
        default:  break;
    }
    return res;
...
static int _VPU_StreamTypeToInt(HAL_VPU_StreamType eType)
{
    int res = 0;
    switch(eType)
    {
        case E_HAL_VPU_MAIN_STREAM   : res = 0; break;
        case E_HAL_VPU_SUB_STREAM    : res = 1; break;
        case E_HAL_VPU_MVC_STREAM    : res = 2; break;
#ifdef VDEC3
        case E_HAL_VPU_N_STREAM      : res = 3; break;
#endif
        default:  break;
    }
    return res;
...
static int _HVD_StreamTypeToInt(HAL_HVD_StreamType eType)
{
    int res = 0;
    switch(eType)
    {
        case E_HAL_HVD_MAIN_STREAM   : res = 0; break;
        case E_HAL_HVD_SUB_STREAM    : res = 1; break;
        case E_HAL_HVD_MVC_STREAM    : res = 2; break;
#ifdef VDEC3
        case E_HAL_HVD_N_STREAM      : res = 3; break;
#endif
        default:  break;
    }
    return res;
...
static HVD_EX_Result _HVD_MapRetEx(HVD_Return eHvd_Ret)
{
    HVD_EX_Result eRes = E_HVD_EX_FAIL;
    switch(eHvd_Ret)
    {
        case E_HVD_RETURN_FAIL              : eRes = E_HVD_EX_FAIL;break;
        case E_HVD_RETURN_SUCCESS           : eRes = E_HVD_EX_OK;break;
        case E_HVD_RETURN_INVALID_PARAMETER : eRes = E_HVD_EX_RET_INVALID_PARAMETER;break;
        case E_HVD_RETURN_ILLEGAL_ACCESS    : eRes = E_HVD_EX_RET_ILLEGAL_ACCESS;break;
        case E_HVD_RETURN_HARDWARE_BREAKDOWN: eRes = E_HVD_EX_RET_HARDWARE_BREAKDOWN;break;
        case E_HVD_RETURN_OUTOF_MEMORY      : eRes = E_HVD_EX_RET_OUTOF_MEMORY;break;
        case E_HVD_RETURN_UNSUPPORTED       : eRes = E_HVD_EX_RET_UNSUPPORTED;break;
        case E_HVD_RETURN_TIMEOUT           : eRes = E_HVD_EX_RET_TIMEOUT;break;
        case E_HVD_RETURN_NOTREADY          : eRes = E_HVD_EX_RET_NOTREADY;break;
        case E_HVD_RETURN_MEMORY_OVERWIRTE  : eRes = E_HVD_EX_RET_MEMORY_OVERWIRTE;break;
        case E_HVD_RETURN_ES_FULL           : eRes = E_HVD_EX_RET_QUEUE_FULL;break;
        case E_HVD_RETURN_RE_INIT           : eRes = E_HVD_EX_RET_RE_INIT;break;
        case E_HVD_RETURN_NOT_RUNNING       : eRes = E_HVD_EX_RET_NOT_RUNNING;break;
        default: break;
    }
    return eRes;
}
...
static void _HVD_EX_SetShareInfoAddr(MS_U32 u32Id)
{
    MS_U32 u32ShmAddr = HAL_VPU_EX_GetSHMAddr();
...
static HVD_EX_Result _HVD_EX_InitVariables(MS_U32 u32Id, HVD_EX_MemCfg *pStMemCfg, HVD_Init_Params *pStInitParams)
{
    ...
    {
        HVD_EX_MSG_ERR("HVD struct define is diff: HVD_Init_Params(%lu) vs HVD_Init_Settings(%lu)\n",
...
#ifdef VDEC3
    pCtrl->MemMap.u32TotalBitstreamBufAddr = pStMemCfg->u32TotalBitstreamBufAddr;
    pCtrl->MemMap.u32TotalBitstreamBufSize = pStMemCfg->u32TotalBitstreamBufSize;
#endif
    ...
    MDrv_SYS_Init();
    const SYS_Info* sysInfo = MDrv_SYS_GetInfo();
    pCtrl->InitParams.u16ChipECONum = (MS_U16)sysInfo->Chip.Version;//MDrv_SYS_GetChipRev();
...
#ifdef VDEC3
HVD_EX_Result _HVD_EX_InitRegCPU(MS_U32 u32Id, MS_BOOL bFWdecideFB)
#else
HVD_EX_Result _HVD_EX_InitRegCPU(MS_U32 u32Id)
#endif
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
...
#ifdef VDEC3
    return (HVD_EX_Result) HAL_HVD_EX_InitRegCPU(u32Id,bFWdecideFB);
#else
...
static HVD_EX_Result _HVD_EX_RstVariables(MS_U32 u32Id)
{
    ...
    MS_U8 u8BitMiuSel = 0 ;
    MS_U32 u32BitStartOffset = 0 ;
    MS_U8 u8CodeMiuSel = 0 ;
    MS_U32 u32CodeStartOffset = 0 ;
    MS_U8 u8FrmMiuSel = 0 ;
    MS_U32 u32FrmStartOffset = 0 ;
    MS_U8 u8DrvProccMiuSel = 0 ;
    MS_U32 u32DrvProccStartOffset = 0 ;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_Drv_Ctrl *pCtrl = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
    ...
    pCtrl->u8Frm2MiuSel = u8FrmMiuSel;
    ...
    HVD_EX_MSG_COVERITY("u32BitStartOffset = 0x%lx u32CodeStartOffset = 0x%lx u32DrvProccStartOffset = 0x%lx u32FrmStartOffset = 0x%lx",u32BitStartOffset,u32CodeStartOffset,u32DrvProccStartOffset,u32FrmStartOffset);
    #else
    ...
    #endif
    if(u32DrvProccStartOffset == 0 || u32FrmStartOffset == 0 || u32CodeStartOffset == 0 || u32BitStartOffset == 0)
    {
    }
...
static HVD_EX_Result _HVD_EX_SetSyncMode(MS_U32 u32Id, HVD_Sync_Tbl_Type eMode)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SYNC_TYPE, eMode));
...
static HVD_EX_Result _HVD_EX_InitFW_AVC(MS_U32 u32Id)
{
    ...
    if (pHVDDrvContext->gHVDPreCtrl[u8DrvId].bEnableThumbnailMode)
    {
        HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_THUMBNAIL_MODE, TRUE);
        HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SKIP_DEC, E_HVD_EX_SKIP_DECODE_I);
    }
...
static MS_BOOL _HVD_EX_IsAllBufferEmpty(MS_U32 u32Id)
{
    ...
    {
        #ifdef VDEC3
        MS_U32 u32VBBUAddr = MDrv_HVD_EX_GetData(u32Id, E_HVD_EX_GDATA_TYPE_VBBU_ADDR);
        if (!HAL_VPU_EX_IsVBBUEmpty(u32VBBUAddr))
        #else
...
#endif
    {
        ...
        if ((HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DEC_Q_NUMB) == 0) && HAL_HVD_EX_DispFrameAllViewed(u32Id)
            && (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_FLUSH_STATUS) == E_HVD_EX_FLUSH_DONE
                || HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_IS_PLAYBACK_FINISH)
                ))
        #else
        if ((HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DEC_Q_NUMB) == 0 && HAL_HVD_EX_DispFrameAllViewed(u32Id)) &&
        ...
        if ((HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_VPU_IDLE_CNT) > u32FwIdleThreshold)) {
            printf("E_HVD_GDATA_VPU_IDLE_CNT > 5000, _HVD_EX_IsAllBufferEmpty=true\n");
            return TRUE;
        }
        return FALSE;
    }
...
static void _HVD_EX_ISRHandler(void)
{
    ...
    {
        if(pHVDDrvContext->gHVDCtrl_EX[u8Idx].bUsed == FALSE)
        {
            continue;
        }

        if(pHVDDrvContext->gHVDCtrl_EX[u8Idx].HVDISRCtrl.eHWDecIsr != E_HWDEC_ISR_HVD)
        {
            continue;
        }
        ...
        if (pfnCb)
        {
            ...
            {
                HAL_HVD_EX_EnableISR(pHVDDrvContext->gHVDCtrl_EX[u8Idx].u32Sid, FALSE);
                pfnCb(pHVDDrvContext->gHVDCtrl_EX[u8Idx].u32Sid);
                HAL_HVD_EX_EnableISR(pHVDDrvContext->gHVDCtrl_EX[u8Idx].u32Sid, TRUE);
            }
        }
        else
        {
    ...
    HAL_HVD_EX_SetClearISR(E_HWDEC_ISR_HVD);
    OSAL_HVD_ISR_Enable(E_HWDEC_ISR_HVD);
}

static void _EVD_EX_ISRHandler(void)
{
    MS_U8 u8Idx;
    HVD_ISRCallBack pfnCb = NULL;
    ...
    for (u8Idx = 0; u8Idx < HVD_MAX_STREAMS; u8Idx++)
    {
        ...
        if(pHVDDrvContext->gHVDCtrl_EX[u8Idx].HVDISRCtrl.eHWDecIsr != E_HWDEC_ISR_EVD)
        {
            continue;
        }
    ...
    HAL_HVD_EX_SetClearISR(E_HWDEC_ISR_EVD);
    OSAL_HVD_ISR_Enable(E_HWDEC_ISR_EVD);
}
...
// TODO: complete this function. and consider more cases. step decoder, display?
    ...
    // temp solution
        ...
        #if defined(CHIP_EINSTEIN) || defined(CHIP_EINSTEIN3) || defined(CHIP_NAPOLI) || defined(CHIP_MONACO) || defined(CHIP_MADISON) || defined(CHIP_MONET) || defined(CHIP_MIAMI) || defined(CHIP_KERES) || defined(CHIP_MUJI) || defined(CHIP_MUNICH) || defined(CHIP_CLIPPERS) || defined(CHIP_MULAN) || defined(CHIP_MUSTANG)
...
MS_BOOL MDrv_HVD_checkISR(MS_U32 u32Id, MS_U8 u8DrvId, HWDEC_ISR_TYPE hwdec_Isr_type)
{
    MS_U8 i = 0;
    MS_BOOL bUsed = FALSE;
    for(i = 0; i < HVD_MAX_STREAMS; i++)
    {
        if(i != u8DrvId)
        {
            if((hwdec_Isr_type == pHVDDrvContext->gHVDCtrl_EX[i].HVDISRCtrl.eHWDecIsr)
            && (TRUE == pHVDDrvContext->gHVDCtrl_EX[i].bUsed))
            {
                bUsed = TRUE;
                break;
            }
...
HVD_EX_Result _HVD_EX_Rst(MS_U32 u32Id, MS_BOOL bErrHandle)
{
    ...
    // disable ISR when there is no one using ISR
#if defined(CHIP_EINSTEIN) || defined(CHIP_EINSTEIN3) || defined(CHIP_NAPOLI) || defined(CHIP_MONACO) || defined(CHIP_MADISON) || defined(CHIP_MONET) || defined(CHIP_MIAMI) || defined(CHIP_KERES) || defined(CHIP_MUJI) || defined(CHIP_MUNICH) || defined(CHIP_CLIPPERS) || defined(CHIP_MULAN) || defined(CHIP_MUSTANG)
    MS_BOOL bUsed = FALSE;
    if ((pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_HEVC)
    {
        bUsed = MDrv_HVD_checkISR(u32Id, u8DrvId, E_HWDEC_ISR_EVD);
    }
    else
    {
        bUsed = MDrv_HVD_checkISR(u32Id, u8DrvId, E_HWDEC_ISR_HVD);
    }
    if (bUsed == FALSE)
    {
...
#ifdef VDEC3
    eRst = (HVD_EX_Result) HAL_HVD_EX_InitShareMem(u32Id, pHVDDrvContext->bFWdecideFB, pHVDDrvContext->bCMAUsed);
#else
...
#endif
#ifdef VDEC3
    eRst = _HVD_EX_InitRegCPU(u32Id,pHVDDrvContext->bFWdecideFB);
#else
...
#if defined(SUPPORT_CMA)
    if (pHVDDrvContext->bCMAUsed)
    {
        HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_CMA_USED, 1);
        /*if (((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_HW_MASK) == HVD_INIT_HW_HEVC) ||
            ((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_HW_MASK) == HVD_INIT_HW_VP9))
        {
            pHVDDrvContext->bCMATwoMIU[u8DrvId] = 1;
            HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_CMA_TWO_MIU, 1);

            HAL_HVD_EX_SetCmd(u32Id, E_DUAL_R2_CMD_FB2ADDR, pHVDDrvContext->cmaInitParam[1].heap_miu_start_offset);
            HAL_HVD_EX_SetCmd(u32Id, E_DUAL_R2_CMD_FB2SIZE, pHVDDrvContext->cmaInitParam[1].heap_length);
        }
        else
        {
            pHVDDrvContext->bCMATwoMIU[u8DrvId] = 0;
            HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_CMA_TWO_MIU, 0);
        }*/

        HAL_HVD_EX_SetCmd(u32Id, E_DUAL_R2_CMD_FBADDR, pHVDDrvContext->cmaInitParam[0].heap_miu_start_offset);
        HAL_HVD_EX_SetCmd(u32Id, E_DUAL_R2_CMD_FBSIZE, pHVDDrvContext->cmaInitParam[0].heap_length);
    }
#endif
    return eRst;
}
...
static MS_U32 _HVD_EX_Map2HVDErrCode(MS_U32 u32ErrCode)
{
    ...
    switch (u32ErrCode)
    {
        ...
        case E_HVD_ERR_RM_SIZE_OUT_FB_LAYOUT:
            ...
            break;
        case E_HVD_ERR_HEVC_RES:
        case E_HVD_ERR_VP9_RES:
            u32Ret = E_HVD_EX_ERRCODE_OUT_OF_SPEC;
            break;
        default:
            break;
...
{
    if (u32MIU == 1)
        return pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32MIU1BaseAddr;
    else if (u32MIU == 2)
        return pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32MIU2BaseAddr;
    else
        return 0;
}
...
void MDrv_HVD_EX_MJPEG_InitSharemem(MS_U32 u32Id, MS_U32 u32CodeBufVAddr,
    MS_BOOL bPreConnEnable, MS_BOOL bConnect, HVD_EX_DISPLAY_PATH   eMvopPath)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag |= HVD_INIT_HW_MJPEG;
    ...
    if (bPreConnEnable)
    {
        MDrv_HVD_EX_PreConnectDispPath(u32Id, bPreConnEnable, bConnect, eMvopPath);
    }


#ifdef VDEC3
    HAL_HVD_EX_InitShareMem(u32Id, FALSE, pHVDDrvContext->bCMAUsed); // FIXME
#else
...
{
#if ((defined(MSOS_TYPE_LINUX) || defined(MSOS_TYPE_LINUX_KERNEL)|| defined(MSOS_TYPE_ECOS)) && (!defined(SUPPORT_X_MODEL_FEATURE)))
    MS_U32 u32ShmId;
    MS_U32 u32Addr;
    MS_U32 u32BufSize;
...
#if defined(SUPPORT_CMA)
#if defined(CHIP_MONACO) || defined(CHIP_CLIPPERS)
            //pHVDDrvContext->bCMAUsed = 1; // enable in the future
#endif

            if (pHVDDrvContext->bCMAUsed)
            {
                pHVDDrvContext->cmaInitParam[0].heap_id = ION_VDEC_HEAP_ID; // 19
                pHVDDrvContext->cmaInitParam[0].flags = CMA_FLAG_MAP_VMA;
                if (MApi_CMA_Pool_Init(&pHVDDrvContext->cmaInitParam[0]) == FALSE)
                    return E_HVD_EX_RET_CMA_ERROR;;
                HVD_EX_MSG_DBG("MApi_CMA_Pool_Init: pool_handle_id=0x%lx, miu=%ld, offset=0x%lx, length=0x%lx\n", pHVDDrvContext->cmaInitParam[0].pool_handle_id, pHVDDrvContext->cmaInitParam[0].miu, pHVDDrvContext->cmaInitParam[0].heap_miu_start_offset, pHVDDrvContext->cmaInitParam[0].heap_length);

                pHVDDrvContext->cmaInitParam[1].heap_id = ION_VDEC_HEAP_ID + 1; // 20
                pHVDDrvContext->cmaInitParam[1].flags = CMA_FLAG_MAP_VMA;
                if (MApi_CMA_Pool_Init(&pHVDDrvContext->cmaInitParam[1]) == FALSE)
                    return E_HVD_EX_RET_CMA_ERROR;
                HVD_EX_MSG_DBG("MApi_CMA_Pool_Init: pool_handle_id=0x%lx, miu=%ld, offset=0x%lx, length=0x%lx\n", pHVDDrvContext->cmaInitParam[1].pool_handle_id, pHVDDrvContext->cmaInitParam[1].miu, pHVDDrvContext->cmaInitParam[1].heap_miu_start_offset, pHVDDrvContext->cmaInitParam[1].heap_length);
            }
#endif
...
#endif
#ifdef VDEC3
    else if ((eStreamType >=  E_HVD_EX_DRV_N_STREAM) && (eStreamType < E_HVD_EX_DRV_N_STREAM + HVD_MAX_STREAMS))
    {
        u32VPUStreamId = (MS_U32) HAL_VPU_EX_GetFreeStream(E_HAL_VPU_N_STREAM + (eStreamType - E_HVD_EX_DRV_N_STREAM));
        u32HVDStreamId = (MS_U32) HAL_HVD_EX_GetFreeStream(E_HAL_HVD_N_STREAM + (eStreamType - E_HVD_EX_DRV_N_STREAM));
        printf("u32VPUStreamId:%lx  u32HVDStreamId:%lx\n",u32VPUStreamId,u32HVDStreamId);
        if (u32VPUStreamId && u32HVDStreamId)
        {
            *pu32Id = ((u32HVDStreamId & 0x0F) << 16 | u32HVDStreamId << 8 | u32VPUStreamId);
            eRet = E_HVD_EX_OK;
        }
    }
#endif
...
//-----------------------------------------------------------------------------
#ifdef VDEC3
HVD_EX_Result MDrv_HVD_EX_Init(MS_U32 u32Id, HVD_EX_MemCfg *pStMemCfg, HVD_EX_InitSettings *pStInitSettings, MS_BOOL bFWdecideFB)
#else
...
#endif
{
...
#ifdef VDEC3
    pHVDDrvContext->bFWdecideFB = bFWdecideFB;
    pCtrl->bNStreamMode = bFWdecideFB; // FIXME: use the FWdecideFB as NStream mode temporarily
#endif
    ...
    if ((pStInitSettings->u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_HEVC)
    {
        pCtrl->HVDISRCtrl.eHWDecIsr = E_HWDEC_ISR_EVD;
    }
#if SUPPORT_G2VP9
    else if((pStInitSettings->u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_VP9)
    {
        pCtrl->HVDISRCtrl.eHWDecIsr = E_HWDEC_ISR_G2VP9;
    }
#endif
#if SUPPORT_MSVP9
    else if((pStInitSettings->u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_VP9)
...
#endif
    {
        pCtrl->HVDISRCtrl.eHWDecIsr = E_HWDEC_ISR_HVD;
    }
...
#ifdef VDEC3
#if SUPPORT_G2VP9
    HAL_VPU_EX_SetBitstreamBufAddress(u32Id,pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32TotalBitstreamBufAddr);
#endif

    pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32BBUId = HAL_HVD_EX_GetBBUId(u32Id);

    if (pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32BBUId == HAL_HVD_INVALID_BBU_ID)
    {
        eRet = E_HVD_EX_FAIL;

        HVD_EX_MSG_ERR("driver init failed due to no valid bbu id.\n");
...
HVD_EX_Result MDrv_HVD_EX_Play(MS_U32 u32Id)
{
...
#endif
    {
        eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_PLAY, 0));
    ...
    // step display off
    eRet = _HVD_MapRetEx ( HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_TRIGGER_DISP, 0));
...
HVD_EX_Result MDrv_HVD_EX_Exit(MS_U32 u32Id)
{
    ...
    if (pHVDDrvContext->bCMAUsed)
    {
        if (MApi_CMA_Pool_PutMem(&pHVDDrvContext->cmaFreeParam[0][u8DrvId]) == FALSE)
            return E_HVD_EX_RET_CMA_ERROR;
        HVD_EX_MSG_DBG("[0x%lx]MApi_CMA_Pool_PutMem: offset=0x%lx, len=0x%lx\n", u32Id, pHVDDrvContext->cmaFreeParam[0][u8DrvId].offset_in_pool, pHVDDrvContext->cmaFreeParam[0][u8DrvId].length);
        if (pHVDDrvContext->bCMATwoMIU[u8DrvId])
        {
            if (MApi_CMA_Pool_PutMem(&pHVDDrvContext->cmaFreeParam[1][u8DrvId]) == FALSE)
                return E_HVD_EX_RET_CMA_ERROR;
    ...
    pHVDDrvContext->gHVDCtrl_EX[u8DrvId].bUsed = FALSE;
#ifdef VDEC3
    if (!HAL_HVD_EX_FreeBBUId(u32Id, pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32BBUId))
    {
        HVD_EX_MSG_ERR("free bbu id (%ld) failed\n", pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u32BBUId);
    }
#endif
    ...
    pHVDDrvContext->bHVDIsInited[u8DrvId] = FALSE;
    // close HVD FW
    eRet = _HVD_MapRetEx( HAL_HVD_EX_DeInit(u32Id));
...
HVD_EX_Result MDrv_HVD_EX_Pause(MS_U32 u32Id)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_PAUSE, 0));
...
HVD_EX_Result MDrv_HVD_EX_Flush(MS_U32 u32Id, MS_BOOL bShowLast)
{
    ...
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_Drv_Ctrl *pCtrl = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
    HVD_FW_State eFwState = E_HVD_FW_INIT;
    ...
    if (MDrv_HVD_EX_GetPlayState(u32Id) != E_HVD_EX_GSTATE_PAUSE)
    {
        ...
        while (1)
        {
            ...
            //handle stop process
            eFwState = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_STATE);
            if((E_HVD_FW_STOP <= eFwState)
            && (E_HVD_FW_STOP_DONE >= eFwState))
            {
                eRet = E_HVD_EX_OK;
                HVD_EX_MSG_INF("In fw stopping process, no need to flush...\n");
                _DRV_HVD_EX_RET(u8DrvId, eRet);
            }
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FLUSH, (MS_U32) bShowLast));
...
HVD_EX_Result MDrv_HVD_EX_SetDynmcDispPath(MS_U32 u32Id, MS_BOOL bConnect, HVD_EX_DISPLAY_PATH eDispPath)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    MS_U32 u32Arg = 0;
    ...
    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_DYNMC_DISP_PATH_STATUS, E_HVD_EX_DISP_PATH_DYNMC_HANDLING);

    u32Arg = (((MS_U32)(eDispPath)) << 1) & (~0x1);
    u32Arg |= (bConnect & 0x1);

    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DYNAMIC_CONNECT_DISP_PATH, u32Arg));
    printf("%s waiting command\n",__FUNCTION__);
    MS_U32 u32TimeOut = MsOS_GetSystemTime() + 500; // it can not block
    while (1)
    {
        if(bConnect)
        {
            if (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DYNMC_DISP_PATH_STATUS) == E_HVD_EX_DISP_PATH_DYNMC_CONNECTTED)
            {
                eRet = E_HVD_EX_OK;
                break;
            }
        ...
        {
            if (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DYNMC_DISP_PATH_STATUS) == E_HVD_EX_DISP_PATH_DYNMC_DISCONNECT)
            {
                eRet = E_HVD_EX_OK;
                break;
            }
        ...
        {
            eRet = E_HVD_EX_FAIL;
            HVD_EX_MSG_ERR("Set dynamic display path timeout!\n");
            break;
        }
    }
    printf("%s command finish eRet:%d\n",__FUNCTION__,eRet);
    return eRet;
}

//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_SetAVSyncDispAutoDrop()
/// @brief \b Function \b Description: enable/disable avsync display auto drop when mvop is disconnectted
/// @param -bEnable \b IN :enable/disable
/// @return -E_HVD_EX_OK for success; E_HVD_EX_FAIL for failure.
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_SetAVSyncDispAutoDrop(MS_U32 u32Id, MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_AVSYNC_DISP_AUTO_DROP, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_StepDisp(MS_U32 u32Id)
{
    ...
    if (_DRV_HVD_Ctrl(u8DrvId, HVD_CTRL_DISPLAY_CTRL))
    {
        ...
        {
            eRet = _HVD_MapRetEx( HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_TRIGGER_DISP, 1));
        }
        else
        {
...
HVD_EX_Result MDrv_HVD_EX_StepDecode(MS_U32 u32Id)
{
    ...
    // step decode
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_STEP_DECODE, 0));
...
HVD_EX_Result MDrv_HVD_EX_PushQueue(MS_U32 u32Id, HVD_EX_PacketInfo *pInfo)
{
    ...
    if (pCtrl->bNStreamMode)
        stVpuPkt.u32Offset    = pInfo->u32Staddr + (pCtrl->MemMap.u32BitstreamBufAddr - pCtrl->MemMap.u32TotalBitstreamBufAddr); // NStream will share the same ES buffer
    else
    ...
    return (HVD_EX_Result)HAL_VPU_EX_Push2VBBU(u32Id, &stVpuPkt, u32VBBUAddr);
#else
    HVD_EX_Result eRet      = E_HVD_EX_RET_ILLEGAL_ACCESS;
...
HVD_EX_Result MDrv_HVD_EX_DecodeIFrame(MS_U32 u32Id, MS_PHYADDR u32SrcSt, MS_U32 u32SrcSize)
{
    ...
    // skip decode I
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SKIP_DEC, E_HVD_EX_SKIP_DECODE_I));
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DISP_I_DIRECT, TRUE));
...
HVD_EX_Result MDrv_HVD_EX_SetDataEnd(MS_U32 u32Id, MS_BOOL bEnd)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_PLAYBACK_FINISH, bEnd));
...
HVD_EX_Result MDrv_HVD_EX_SetDispErrFrm(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DISP_ERR_FRM, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_SetDispRepeatField(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_REPEAT_LAST_FIELD, bEnable));
...
#endif
    {
        if (pHVDDrvContext->gHVDPreCtrl[u8DrvId].bEnableThumbnailMode && eDecType != E_HVD_EX_SKIP_DECODE_I)
            HVD_EX_MSG_ERR("[ERROR] Set wrong decode type 0x%x in thumbnail mode\n", eDecType);
        else
            HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SKIP_DEC, eDecType);

        eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SKIP_DEC, eDecType));
        ...
        {
            eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DISP_I_DIRECT, FALSE));
        }
    }
...
#ifdef VDEC3_FB
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_SetFrmBuffAddr()
/// @brief \b Function \b Description:  specify the frame buffer address.
/// @param -u32FrmBuffAddr \b IN : frame buffer address.
/// @return -The result of command set frame buffer address.
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_SetFrmBuffAddr(MS_U32 u32Id, MS_U32 u32FrmBuffAddr)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
...
//    _DRV_HVD_Inited(u8DrvId,eRet);
    ...
    HVD_EX_MSG_DBG("Drv: set u32FrmBuffAddr = 0x%lX\n", (MS_U32) u32FrmBuffAddr);

//#if HVD_ENABLE_CHECK_STATE_BEFORE_SET_CMD
//    if (HAL_HVD_EX_GetData(E_HVD_GDATA_DISPLAY_DURATION) != u32FrmBuffAddr)
//#endif
    {
        eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_DUAL_R2_CMD_FBADDR, u32FrmBuffAddr));
    }
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_SetFrmBuffSize()
/// @brief \b Function \b Description:  specify the frame buffer size.
/// @param -u32FrmBuffSize \b IN : frame buffer size.
/// @return -The result of command set frame buffer size.
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_SetFrmBuffSize(MS_U32 u32Id, MS_U32 u32FrmBuffSize)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
...
//    _DRV_HVD_Inited(u8DrvId,eRet);
    ...
    HVD_EX_MSG_DBG("Drv: set FrmBuffSize = 0x%lX\n", (MS_U32) u32FrmBuffSize);

//#if HVD_ENABLE_CHECK_STATE_BEFORE_SET_CMD
//    if (HAL_HVD_EX_GetData(E_HVD_GDATA_DISPLAY_DURATION) != u32FrmBuffSize)
//#endif
    {
        eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_DUAL_R2_CMD_FBSIZE, u32FrmBuffSize));
    }
/*#if HVD_ENABLE_WAIT_CMD_FINISHED
    if (eRet == E_HVD_EX_OK)
    {
...
/// @return -The result of command set frame buffer address.
//-----------------------------------------------------------------------------
HVD_EX_Result MDrv_HVD_EX_SetFrmBuff2(MS_U32 u32Id, MS_U32 u32FrmBuffAddr, MS_U32 u32FrmBuffSize)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    MS_U8 u8Frm2MiuSel;
    ...
    HVD_EX_MSG_DBG("Drv: set u32FrmBuff2Addr = 0x%lX\n", (MS_U32) u32FrmBuffAddr);

    _phy_to_miu_offset(u8Frm2MiuSel, u32FrmStartOffset, u32FrmBuffAddr);
    pHVDDrvContext->gHVDCtrl_EX[u8DrvId].u8Frm2MiuSel = u8Frm2MiuSel;
    u32MIUSel = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_MIU_SEL);

    u32MIUSel &= (~(VDEC_MIUSEL_MASK << VDEC_LUMA2_MIUSEL));
    u32MIUSel &= (~(VDEC_MIUSEL_MASK << VDEC_CHROMA2_MIUSEL));
    u32MIUSel &= (~(VDEC_MIUSEL_MASK << VDEC_BUF2_MIUSEL));
    u32MIUSel |= (u8Frm2MiuSel << VDEC_LUMA2_MIUSEL);
    u32MIUSel |= (u8Frm2MiuSel << VDEC_CHROMA2_MIUSEL);
    u32MIUSel |= (u8Frm2MiuSel << VDEC_BUF2_MIUSEL);

#if SUPPORT_G2VP9
    if ((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_HW_MASK) == HVD_INIT_HW_VP9)
    {
        u32MIUSel &= (~(VDEC_MIUSEL_MASK << VDEC_LUMA8_MIUSEL));
        u32MIUSel &= (~(VDEC_MIUSEL_MASK << VDEC_CHROMA8_MIUSEL));
        u32MIUSel |= (u8Frm2MiuSel << VDEC_LUMA8_MIUSEL);
        u32MIUSel |= (u8Frm2MiuSel << VDEC_CHROMA8_MIUSEL);
    }
#endif

    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_MIU_SEL, u32MIUSel);

    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_FRAMEBUF2_ADDR, u32FrmStartOffset);

    HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_FRAMEBUF2_SIZE, u32FrmBuffSize);
...
#endif
    {
        eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DISP_SPEED, eSpeed));
    }
#if HVD_ENABLE_WAIT_CMD_FINISHED
    if (eRet == E_HVD_EX_OK)
    {
...
#endif
    {
        eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SYNC_ACTIVE, bEnable));
    }
#if HVD_ENABLE_WAIT_CMD_FINISHED
    if (eRet == E_HVD_EX_OK)
    {
...
HVD_EX_Result MDrv_HVD_EX_SetDropMode(MS_U32 u32Id, HVD_EX_DropDisp eMode, MS_U32 u32Arg)
{
    ...
    {
        eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DROP_DISP_AUTO, u32Arg));
    }
    else if (eMode == E_HVD_EX_DROP_DISPLAY_ONCE)
    {
        eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DROP_DISP_ONCE, u32Arg));
    }
    else
    {
        eRet = E_HVD_EX_RET_INVALID_PARAMETER;
...
HVD_EX_Result MDrv_HVD_EX_RstPTS(MS_U32 u32Id, MS_U32 u32PTS)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_RESET_PTS, u32PTS));
...
HVD_EX_Result MDrv_HVD_EX_SetFrcMode(MS_U32 u32Id, HVD_EX_FrmRateConvMode eMode)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FRC_MODE, eMode));
...
HVD_EX_Result MDrv_HVD_EX_SetSyncTolerance(MS_U32 u32Id, MS_U32 u32Arg)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SYNC_TOLERANCE, u32Arg));
...
HVD_EX_Result MDrv_HVD_EX_SetSyncVideoDelay(MS_U32 u32Id, MS_U32 u32Arg)
{
    ...
    if ((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_MAIN_MASK) == HVD_INIT_MAIN_LIVE_STREAM)
    {
        ...
        {
            eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SYNC_VIDEO_DELAY, (MS_U32) (u32Arg + HVD_DTV_VIDEO_DELAY)));
        }
        else
        {
            eRet =
                _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SYNC_VIDEO_DELAY, u32Arg + ((MS_U32) HVD_DTV_VIDEO_DELAY * 90)));
        }
    }
    else
    {
        eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SYNC_VIDEO_DELAY, u32Arg));
    }
...
HVD_EX_Result MDrv_HVD_EX_SetSyncFreeRunTH(MS_U32 u32Id, MS_U32 u32Arg)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FREERUN_THRESHOLD, u32Arg));
...
HVD_EX_Result MDrv_HVD_EX_SetSyncRepeatTH(MS_U32 u32Id, MS_U32 u32Arg)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SYNC_THRESHOLD, u32Arg));
...
HVD_EX_Result MDrv_HVD_EX_SetErrConceal(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_ERR_CONCEAL, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_SeekToPTS(MS_U32 u32Id, MS_U32 u32PTS)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_JUMP_TO_PTS, u32PTS));
...
HVD_EX_Result MDrv_HVD_EX_SkipToPTS(MS_U32 u32Id, MS_U32 u32PTS)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SKIP_TO_PTS, u32PTS));
...
HVD_EX_Result MDrv_HVD_EX_SetFreezeImg(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FREEZE_IMG, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_SetBlueScreen(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_BLUE_SCREEN, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_SetDispOneField(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    if (bEnable)
    {
        ...
        eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DISP_ONE_FIELD, E_HVD_FIELD_CTRL_TOP));
    }
    else
    {
        eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DISP_ONE_FIELD, E_HVD_FIELD_CTRL_OFF));
    }
...
HVD_EX_Result MDrv_HVD_EX_SetISREvent(MS_U32 u32Id, MS_U32 u32Event, HVD_InterruptCb fnISRHandler)
{
    ...
    if (u32Event == E_HVD_EX_ISR_NONE)
    {
        ...
        if(pCtrl->HVDISRCtrl.eHWDecIsr == E_HWDEC_ISR_HVD) // hvd case
        {
            for(i = 0; i < HVD_MAX_STREAMS; i++)
            {
                if(i != u8DrvId)
                {
                    if((E_HWDEC_ISR_HVD == pHVDDrvContext->gHVDCtrl_EX[i].HVDISRCtrl.eHWDecIsr)
                    && (TRUE == pHVDDrvContext->gHVDCtrl_EX[i].bUsed))
                    {
                        bUsed = TRUE;
                        break;
        ...
        //OSAL_HVD_ISR_Detach();
//!defined(SUPPORT_X_MODEL_HVD_FEATURE) && !defined(DISABLE_ISR_DETACH)
        ...
        else if(pCtrl->HVDISRCtrl.eHWDecIsr == E_HWDEC_ISR_EVD) // evd case
        {
            for(i = 0; i < HVD_MAX_STREAMS; i++)
            {
                if(i != u8DrvId)
                {
                    if((E_HWDEC_ISR_EVD == pHVDDrvContext->gHVDCtrl_EX[i].HVDISRCtrl.eHWDecIsr)
                    && (TRUE == pHVDDrvContext->gHVDCtrl_EX[i].bUsed))
                    {
                        bUsed = TRUE;
                        break;
...
//!defined(SUPPORT_X_MODEL_HVD_FEATURE) && !defined(DISABLE_ISR_DETACH)
    ...
    else
    {
        if (fnISRHandler != NULL)
        {
            if(pCtrl->HVDISRCtrl.eHWDecIsr == E_HWDEC_ISR_HVD) //hvd case
            {
                if (FALSE == pHVDDrvContext->bHVDIsrAttached)
                {
                    if (OSAL_HVD_ISR_Attach(E_HWDEC_ISR_HVD, (void*)_HVD_EX_ISRHandler) != TRUE)
                    {
                        HVD_EX_MSG_ERR("fail to attach HVD_IsrProc!\n");
                        return E_HVD_EX_FAIL;
                    }

                    pHVDDrvContext->bHVDIsrAttached = TRUE;
                }

                if (FALSE == pHVDDrvContext->bHVDMSOSIsrEnable)
                {
                    if (OSAL_HVD_ISR_Enable(E_HWDEC_ISR_HVD) != TRUE)
                    {
                        HVD_EX_MSG_ERR("fail to OSAL_HVD_IntEnable!\n");
                        return E_HVD_EX_FAIL;
                    }

                    pHVDDrvContext->bHVDMSOSIsrEnable = TRUE;
                }
            }

            if(pCtrl->HVDISRCtrl.eHWDecIsr == E_HWDEC_ISR_EVD) //evd case
            {
                if (FALSE == pHVDDrvContext->bEVDIsrAttached)
                {
                    if (OSAL_HVD_ISR_Attach(E_HWDEC_ISR_EVD, (void*)_EVD_EX_ISRHandler) != TRUE)
                    {
                        HVD_EX_MSG_ERR("fail to attach HVD_IsrProc!\n");
                        return E_HVD_EX_FAIL;
                    }

                    pHVDDrvContext->bEVDIsrAttached = TRUE;
                }

                if (FALSE == pHVDDrvContext->bEVDMSOSIsrEnable)
                {
                    if (OSAL_HVD_ISR_Enable(E_HWDEC_ISR_EVD) != TRUE)
                    {
                        HVD_EX_MSG_ERR("fail to OSAL_HVD_IntEnable!\n");
                        return E_HVD_EX_FAIL;
                    }

                    pHVDDrvContext->bEVDMSOSIsrEnable = TRUE;
                }
            }

            eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_ISR_TYPE, u32Event));
...
MS_BOOL MDrv_HVD_EX_SetMVOPDone(MS_U32 u32Id)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_BLUE_SCREEN, FALSE));
...
HVD_EX_Result MDrv_HVD_EX_SetDynScalingParam(MS_U32 u32Id, void *pStAddr, MS_U32 u32Size)
{
    ...
    else
    {
        ...
        // 2. while till FW finish it.
         eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SCALER_INFO_NOTIFY, ((MS_U8 *)pStAddr)[0]));
    }
...
HVD_EX_Result MDrv_HVD_EX_SetDispInfoTH(MS_U32 u32Id, HVD_EX_DispInfoThreshold *DispInfoTH)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_UPDATE_DISP_THRESHOLD, 0));
...
HVD_EX_Result MDrv_HVD_EX_SetIgnoreErrRef(MS_U32 u32Id, MS_BOOL bIgnore)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_IGNORE_ERR_REF, bIgnore));
...
HVD_EX_Result MDrv_HVD_EX_ForceFollowDTVSpec(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FORCE_DTV_SPEC, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_SetFastDisplay(MS_U32 u32Id, MS_BOOL bFastDisplay)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FAST_DISP, bFastDisplay));
...
//change to open max setting ,wait f/w release new fwHVD_if
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_ENABLE_DISP_OUTSIDE, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_CheckDispInfoRdy(MS_U32 u32Id)
{
    ...
    #if defined(SUPPORT_CMA)
    if (pHVDDrvContext->bCMAUsed && !HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_CMA_ALLOC_DONE))
    {
        HVD_EX_Drv_Ctrl *pCtrl = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
        struct CMA_Pool_Alloc_Param alloc_param, alloc_param2;
        MS_U32 framebuf_size, framebuf_size2 = 0;
        MS_U8 u8BitMiuSel = 0;
        MS_U8 u8CodeMiuSel = 0;
        MS_U8 u8FBMiuSel = 0, u8FB2MiuSel = 0;
        MS_U32 u32BitStartOffset;
        MS_U32 u32CodeStartOffset;

        framebuf_size = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FRAMEBUF_SIZE);
        if (framebuf_size == 0)
            return E_HVD_EX_RET_NOTREADY;

        if (pHVDDrvContext->bCMATwoMIU[u8DrvId])
        {
            framebuf_size2 = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FRAMEBUF2_SIZE);
            if (framebuf_size2 == 0)
                return E_HVD_EX_RET_NOTREADY;
        }
        alloc_param.pool_handle_id = pHVDDrvContext->cmaInitParam[0].pool_handle_id;
        alloc_param.offset_in_pool = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FRAMEBUF_ADDR) - pHVDDrvContext->cmaInitParam[0].heap_miu_start_offset;
        alloc_param.length = framebuf_size;
        alloc_param.flags = CMA_FLAG_VIRT_ADDR;
        if (MApi_CMA_Pool_GetMem(&alloc_param) == FALSE)
            return E_HVD_EX_RET_CMA_ERROR;
        pHVDDrvContext->cmaFreeParam[0][u8DrvId].pool_handle_id = alloc_param.pool_handle_id;
        pHVDDrvContext->cmaFreeParam[0][u8DrvId].offset_in_pool = alloc_param.offset_in_pool;
        pHVDDrvContext->cmaFreeParam[0][u8DrvId].length = alloc_param.length;
        u8FBMiuSel = pHVDDrvContext->cmaInitParam[0].miu;
        HVD_EX_MSG_DBG("[0x%lx]MApi_CMA_Pool_GetMem:offset=0x%lx, len=0x%lx, miu=%ld\n", u32Id, alloc_param.offset_in_pool, alloc_param.length, pHVDDrvContext->cmaInitParam[0].miu);

        if (pHVDDrvContext->bCMATwoMIU[u8DrvId])
        {
            alloc_param2.pool_handle_id = pHVDDrvContext->cmaInitParam[1].pool_handle_id;
            alloc_param2.offset_in_pool = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FRAMEBUF2_ADDR) - pHVDDrvContext->cmaInitParam[1].heap_miu_start_offset;
            alloc_param2.length = framebuf_size2;
            alloc_param2.flags = CMA_FLAG_VIRT_ADDR;
            if (MApi_CMA_Pool_GetMem(&alloc_param2) == FALSE)
                return E_HVD_EX_RET_CMA_ERROR;
            pHVDDrvContext->cmaFreeParam[1][u8DrvId].pool_handle_id = alloc_param2.pool_handle_id;
            pHVDDrvContext->cmaFreeParam[1][u8DrvId].offset_in_pool = alloc_param2.offset_in_pool;
            pHVDDrvContext->cmaFreeParam[1][u8DrvId].length = alloc_param2.length;
            u8FB2MiuSel = pHVDDrvContext->cmaInitParam[1].miu;
            HVD_EX_MSG_DBG("[0x%lx]MApi_CMA_Pool_GetMem:offset=0x%lx, len=0x%lx, miu=%ld\n", u32Id, alloc_param2.offset_in_pool, alloc_param2.length, pHVDDrvContext->cmaInitParam[1].miu);
        }
        else
        {
            u8FB2MiuSel = u8FBMiuSel;
        }

        _phy_to_miu_offset(u8BitMiuSel, u32BitStartOffset, pCtrl->MemMap.u32BitstreamBufAddr);
        _phy_to_miu_offset(u8CodeMiuSel, u32CodeStartOffset, pCtrl->MemMap.u32CodeBufAddr);

        MS_U32 u32MIUSel = (u8BitMiuSel << VDEC_BS_MIUSEL) |
                            (u8FBMiuSel << VDEC_LUMA8_MIUSEL) |
                            (u8FBMiuSel << VDEC_CHROMA8_MIUSEL) |
                            (u8FB2MiuSel << VDEC_LUMA2_MIUSEL) |
                            (u8FB2MiuSel << VDEC_CHROMA2_MIUSEL) |
                            (u8FBMiuSel << VDEC_HWBUF_MIUSEL) |
                            (u8FBMiuSel << VDEC_BUF1_MIUSEL) |
                            (u8FB2MiuSel << VDEC_BUF2_MIUSEL) |
        ...
        if ((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_HW_MASK) == HVD_INIT_HW_VP9)
        {
            u32MIUSel &= (~(VDEC_MIUSEL_MASK << VDEC_LUMA8_MIUSEL));
            u32MIUSel &= (~(VDEC_MIUSEL_MASK << VDEC_CHROMA8_MIUSEL));
            u32MIUSel |= (u8FB2MiuSel << VDEC_LUMA8_MIUSEL);
            u32MIUSel |= (u8FB2MiuSel << VDEC_CHROMA8_MIUSEL);
        }
#endif

        HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_MIU_SEL, u32MIUSel);

        HVD_EX_MSG_DBG("E_HVD_SDATA_MIU_SEL = 0x%lx u8CodeMiuSel = %d  u32BitStartOffset = 0x%lx  u32CodeStartOffset = 0x%lx\n", u32MIUSel,u8CodeMiuSel,u32BitStartOffset,u32CodeStartOffset);

        HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_CMA_ALLOC_DONE, 1);
    }
    #endif
...
#ifdef VDEC3
    MS_U32 u32VBBUAddr = MDrv_HVD_EX_GetData(u32Id, E_HVD_EX_GDATA_TYPE_VBBU_ADDR);
    return HAL_VPU_EX_GetVBBUVacancy(u32VBBUAddr);
#else
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
...
//-----------------------------------------------------------------------------
/// @brief \b Function \b Name: MDrv_HVD_EX_GetU64PTS_PreParse()
/// @brief \b Function \b Description:  get the 33 bits pts which is latest added into PTS table. unit: ms
/// @return - PTS
//-----------------------------------------------------------------------------
MS_U64 MDrv_HVD_EX_GetU64PTS_PreParse(MS_U32 u32Id)
{
    MS_U64 u64PTS = 0;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    _DRV_HVD_Inited(u8DrvId,FALSE);

    u64PTS = *((MS_U64*)(HAL_HVD_EX_GetData(u32Id,E_HVD_GDATA_U64PTS_PRE_PARSE)));
    _90K_TO_MS_U64(u64PTS); //ms
...
MS_U32 MDrv_HVD_EX_GetESWritePtr(MS_U32 u32Id)
{
    ...
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
        HVD_EX_Drv_Ctrl *pCtrl  = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
    if ((pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_INPUT_MASK) == E_HVD_INIT_INPUT_DRV) // File mode
    {
        MS_U32 u32VBBUAddr = MDrv_HVD_EX_GetData(u32Id, E_HVD_EX_GDATA_TYPE_VBBU_ADDR);
        MS_U32 u32Wptr = HAL_VPU_EX_GetESWritePtr(u32Id, u32VBBUAddr);

        if (pCtrl->bNStreamMode)
            return (u32Wptr == 0) ? u32Wptr : (u32Wptr - (pCtrl->MemMap.u32BitstreamBufAddr - pCtrl->MemMap.u32TotalBitstreamBufAddr)); // NStream will share the same ES buffer
        else
            return u32Wptr;
    }
    else // TSP mode
...
MS_U32 MDrv_HVD_EX_GetESReadPtr(MS_U32 u32Id)
{
    ...
    {
        MS_U32 u32VBBUAddr = MDrv_HVD_EX_GetData(u32Id, E_HVD_EX_GDATA_TYPE_VBBU_ADDR);
        MS_U32 u32Rptr = HAL_VPU_EX_GetESReadPtr(u32Id, u32VBBUAddr);

        if (pCtrl->bNStreamMode)
            return (u32Rptr == 0) ? u32Rptr : (u32Rptr - (pCtrl->MemMap.u32BitstreamBufAddr - pCtrl->MemMap.u32TotalBitstreamBufAddr)); // NStream will share the same ES buffer
        else
            return u32Rptr;
    }
    else // TSP mode
...
// TODO: define driver error code  for upper layer
    MS_U32 u32Ret = 0;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    extern MS_BOOL bModeSupportDivXPlus;
    extern MS_BOOL bModeSupportDivXHEVC_4K;
    extern MS_BOOL bModeSupportDivXHEVC_1080P;
    if(pHVDDrvContext->gHVDPreCtrl[u8DrvId].bDisableDivx)
    {
        bModeSupportDivXPlus = FALSE;
        bModeSupportDivXHEVC_4K = FALSE;
        bModeSupportDivXHEVC_1080P = FALSE;
    }
    ...
    if((!u32Ret) && ((HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_IS_DIVX_PLUS_STREAM)&E_DIVX_PROFILE_DIVX_PLUS) && (bModeSupportDivXPlus == FALSE)))
    {
        u32Ret = E_HVD_EX_ERRCODE_DIVX_PLUS_UNSUPPORTED;
    }
    else if((!u32Ret) && (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_IS_DIVX_PLUS_STREAM)&E_DIVX_PROFILE_DIVX_HEVC))
    {
        HVD_EX_DispInfo stInfo;
        if (E_HVD_EX_OK == _HVD_EX_GetDispInfo(u32Id, &stInfo, TRUE))
        {
            if(((stInfo.u16HorSize*stInfo.u16VerSize) > (1920 * 1088)) && (bModeSupportDivXHEVC_4K == FALSE) )
            {
                u32Ret = E_HVD_EX_ERRCODE_DIVX_PLUS_UNSUPPORTED;
            }
            else if(((stInfo.u16HorSize*stInfo.u16VerSize) <= (1920 * 1088)) && (bModeSupportDivXHEVC_4K == FALSE) && (bModeSupportDivXHEVC_1080P == FALSE))
            {
                u32Ret = E_HVD_EX_ERRCODE_DIVX_PLUS_UNSUPPORTED;
            }
        }
...
#if 1//def SUPPORT_X_MODEL_FEATURE //We using the common compile option to handle X model
...
HVD_EX_Result MDrv_HVD_EX_GetFrmInfo(MS_U32 u32Id, HVD_EX_GetFrmInfoType eType, HVD_EX_FrameInfo * pInfo)
{
...
#ifdef SUPPORT_DISP_INFO_EXT
    HVD_Frm_Information_EXT_Entry *pFrmInfo_ext = NULL;
#endif
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    else if (eType == E_HVD_EX_GFRMINFO_NEXT_DISPLAY)
    {
        ...
        if(pFrmInfo != NULL)
        {
            pFrmInfo_ext = (HVD_Frm_Information_EXT_Entry *)HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_NEXT_DISP_FRM_INFO_EXT);
        }
#endif
    ...
    else if (eType == E_HVD_EX_GFRMINFO_LAST_DISPLAY_EX)
    {
        pFrmInfo = (HVD_Frm_Information *) pHVDDrvContext->gHVDCtrl_EX[u8DrvId].pLastFrmInfo;
#ifdef SUPPORT_DISP_INFO_EXT
        pFrmInfo_ext = (HVD_Frm_Information_EXT_Entry *) pHVDDrvContext->gHVDCtrl_EX[u8DrvId].pLastFrmInfo_ext;
#endif
    }
    else if (eType == E_HVD_EX_GFRMINFO_DECODE_SIMPLE)
    {
        pFrmInfo = (HVD_Frm_Information *) HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_HVD_DECODE_SIMPLE_INFO);
        if(pFrmInfo)
        {
    ...
    if (pFrmInfo != NULL)
    {
        MS_U32 u32MIUSel = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_MIU_SEL);
        MS_U32 u32Luma8MiuSel = (u32MIUSel >> VDEC_LUMA8_MIUSEL) & VDEC_MIUSEL_MASK;
        MS_U32 u32Luma2MiuSel = (u32MIUSel >> VDEC_LUMA2_MIUSEL) & VDEC_MIUSEL_MASK;
        MS_U32 u32Chroma8MiuSel = (u32MIUSel >> VDEC_CHROMA8_MIUSEL) & VDEC_MIUSEL_MASK;
        MS_U32 u32Chroma2MiuSel = (u32MIUSel >> VDEC_CHROMA2_MIUSEL) & VDEC_MIUSEL_MASK;

#if SUPPORT_G2VP9
        if ((pHVDDrvContext->gHVDCtrl_EX[u8DrvId].InitParams.u32ModeFlag & HVD_INIT_HW_MASK) == HVD_INIT_HW_VP9)
        {
            // FIXME: workaround for VP9 SW detile path
            if (pFrmInfo->u16Width <= 2560)
                u32Luma8MiuSel = u32Chroma8MiuSel = (u32MIUSel >> VDEC_BUF1_MIUSEL) & VDEC_MIUSEL_MASK;
            else
                u32Luma8MiuSel = u32Chroma8MiuSel = (u32MIUSel >> VDEC_BUF2_MIUSEL) & VDEC_MIUSEL_MASK;
        }
#endif

        pInfo->u32LumaAddr = (MS_PHYADDR) (pFrmInfo->u32LumaAddr + _HVD_EX_GetMIUBase(u8DrvId, u32Luma8MiuSel));
        pInfo->u32ChromaAddr = (MS_PHYADDR) (pFrmInfo->u32ChromaAddr + _HVD_EX_GetMIUBase(u8DrvId, u32Chroma8MiuSel));
        pInfo->u32TimeStamp = pFrmInfo->u32TimeStamp;
        ...
        pInfo->u32LumaAddr_2bit = (MS_PHYADDR) (pFrmInfo->u32LumaAddr_2bit + _HVD_EX_GetMIUBase(u8DrvId, u32Luma2MiuSel)) ;
        pInfo->u32ChromaAddr_2bit = (MS_PHYADDR) (pFrmInfo->u32ChromaAddr_2bit + _HVD_EX_GetMIUBase(u8DrvId, u32Chroma2MiuSel));
...
#ifdef SUPPORT_DISP_INFO_EXT
        if(pFrmInfo_ext != NULL)
        {
            pInfo->u32LumaAddr_2bit = (MS_PHY)(pFrmInfo_ext->u32LumaAddrExt[HVD_FRM_INFO_EXT_TYPE_10BIT]
                                                + _HVD_EX_GetMIUBase(u8DrvId, u32Luma2MiuSel));
            pInfo->u32ChromaAddr_2bit = (MS_PHY)(pFrmInfo_ext->u32ChromaAddrExt[HVD_FRM_INFO_EXT_TYPE_10BIT]
                                                + _HVD_EX_GetMIUBase(u8DrvId, u32Chroma2MiuSel));
            pInfo->u32LumaAddrI = (MS_PHY)(pFrmInfo_ext->u32LumaAddrExt[HVD_FRM_INFO_EXT_TYPE_INTERLACE]
                                                + _HVD_EX_GetMIUBase(u8DrvId, u32Luma2MiuSel));
            pInfo->u32LumaAddrI_2bit = (MS_PHY)(pFrmInfo_ext->u32LumaAddrExt[HVD_FRM_INFO_EXT_TYPE_10BIT_INTERLACE]
                                                + _HVD_EX_GetMIUBase(u8DrvId, u32Luma2MiuSel));
            pInfo->u32ChromaAddrI = (MS_PHY)(pFrmInfo_ext->u32ChromaAddrExt[HVD_FRM_INFO_EXT_TYPE_INTERLACE]
                                                + _HVD_EX_GetMIUBase(u8DrvId, u32Chroma2MiuSel));
            pInfo->u32ChromaAddrI_2bit = (MS_PHY)(pFrmInfo_ext->u32ChromaAddrExt[HVD_FRM_INFO_EXT_TYPE_10BIT_INTERLACE]
                                                + _HVD_EX_GetMIUBase(u8DrvId, u32Chroma2MiuSel));
            pInfo->u32MFCodecInfo = pFrmInfo_ext->MFCodecInfo;
            pInfo->u32LumaMFCbitlen = pFrmInfo_ext->u32LumaAddrExt[HVD_FRM_INFO_EXT_TYPE_MFCBITLEN];
            pInfo->u32ChromaMFCbitlen = pFrmInfo_ext->u32ChromaAddrExt[HVD_FRM_INFO_EXT_TYPE_MFCBITLEN];

            pInfo->maxLuminance = pFrmInfo_ext->maxLuminance;
            pInfo->minLuminance = pFrmInfo_ext->minLuminance;
            pInfo->primaries[0][0] = pFrmInfo_ext->primaries[0][0];
            pInfo->primaries[0][1] = pFrmInfo_ext->primaries[0][1];
            pInfo->primaries[1][0] = pFrmInfo_ext->primaries[1][0];
            pInfo->primaries[1][1] = pFrmInfo_ext->primaries[1][1];
            pInfo->primaries[2][0] = pFrmInfo_ext->primaries[2][0];
            pInfo->primaries[2][1] = pFrmInfo_ext->primaries[2][1];
            pInfo->whitePoint[0] = pFrmInfo_ext->whitePoint[0];
            pInfo->whitePoint[1] = pFrmInfo_ext->whitePoint[1];

            pInfo->Frm_Info_Ext_avail = pFrmInfo_ext->Frm_Info_Ext_avail;
            pInfo->colour_primaries = pFrmInfo_ext->colour_primaries;
            pInfo->transfer_characteristics = pFrmInfo_ext->transfer_characteristics;
            pInfo->matrix_coefficients = pFrmInfo_ext->matrix_coefficients;

            if (eType == E_HVD_EX_GFRMINFO_NEXT_DISPLAY)
            {
                pHVDDrvContext->gHVDCtrl_EX[u8DrvId].pLastFrmInfo_ext = pFrmInfo_ext;
            }
        }
#endif
...
MS_U8 MDrv_HVD_EX_GetDSBufMiuSelect(MS_U32 u32Id)
{
    ...
    if(u32StartOffset == 0 || u8MiuSel == 0)
    {
    }
    return u8MiuSel;
}
...
MS_U32 MDrv_HVD_EX_GetData(MS_U32 u32Id, HVD_EX_GDataType eType)
{
    ...
    switch (eType)
    {
        ...
        {
            u32Ret = 0;//pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32CodeBufAddr + HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_VSYNC_BRIDGE_ADDR);
            break;
        }
...
#ifdef VDEC3
        case E_HVD_EX_GDATA_TYPE_VBBU_ADDR:
        {
            u32Ret = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_FW_VBBU_ADDR);
            break;
        }
#endif
        case E_HVD_EX_GDATA_TYPE_IS_DIVX_PLUS_STREAM:
        {
            u32Ret = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_IS_DIVX_PLUS_STREAM);
            break;
        }
        case E_HVD_EX_GDATA_TYPE_VIDEO_FULL_RANGE_FLAG:
        {
            u32Ret = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_VIDEO_FULL_RANGE_FLAG);
            break;
        }
        default:
            break;
...
//-----------------------------------------------------------------------------
MS_U32 MDrv_HVD_EX_GetMem_Dbg(MS_U32 u32Id, MS_U32 u32Addr)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    MS_U32 u32Ret = 0;
...
//-----------------------------------------------------------------------------
void MDrv_HVD_EX_SetMem_Dbg(MS_U32 u32Id, MS_U32 u32Addr, MS_U32 u32Arg)
{
    HVD_EX_MSG_TRACE();
...
HVD_EX_Result MDrv_HVD_EX_SetCmd_Dbg(MS_U32 u32Id, MS_U32 u32Cmd, MS_U32 u32Arg)
{
    ...
    // todo: consider more...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, (HVD_User_Cmd) u32Cmd, u32Arg));
...
HVD_EX_Result MDrv_HVD_EX_Set_Err_Tolerance(MS_U32 u32Id, MS_U32 u32Arg)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SET_DISP_ERROR_TOLERANCE, u32Arg));
...
HVD_EX_Result MDrv_HVD_EX_SetSettings_Pro(MS_U32 u32Id, HVD_EX_SSettingsType eType, MS_U32 u32Arg)
{
    ...
    {
        eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, eCmd, u32Arg));
    }
    else
    {
        eRet = E_HVD_EX_RET_INVALID_PARAMETER;
...
#if SUPPORT_MSVP9 && defined(VDEC3)
    if(E_HVD_EX_VP9 == u32Type)
        return TRUE;
#endif
...
HVD_EX_Result MDrv_HVD_EX_SetBalanceBW(MS_U32 u32Id, MS_U8 u8QPCnt, MS_U8 u8DBCnt, MS_U8 u8Upper)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_BALANCE_BW, u8QPCnt | (u8DBCnt << 8) | (u8Upper << 16)));
...
HVD_EX_Result MDrv_HVD_EX_SetFdMaskDelayCnt(MS_U32 u32Id, MS_U8 u8DelayCnt)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FDMASK_DELAY_CNT, u8DelayCnt));
...
HVD_EX_Result MDrv_HVD_EX_SetOutputFRCMode(MS_U32 u32Id, MS_U8 u8FrameRate, MS_U8 u8Interlace)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FRC_OUTPUT_FRAMERATE, u8FrameRate));
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FRC_OUTPUT_INTERLACE, u8Interlace));
...
HVD_EX_Result MDrv_HVD_EX_ShowDecodeOrder(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SHOW_DECODE_ORDER, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_Disp_Ignore_Crop(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DISP_IGNORE_CROP, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_SuspendDynamicScale(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SUSPEND_DYNAMIC_SCALE, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_RmEnablePtsTbl(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_RM_ENABLE_PTS_TBL, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_AutoExhaustESMode(MS_U32 u32Id, MS_U32 u32ESbound)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_AUTO_EXHAUST_ES_MODE, u32ESbound));
...
HVD_EX_Result MDrv_HVD_EX_SetMinTspSize(MS_U32 u32Id, MS_U32 u32Size)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SET_MIN_TSP_DATA_SIZE, u32Size));
...
HVD_EX_Result MDrv_HVD_EX_SetFRCDropType(MS_U32 u32Id, MS_U8 u8DropType)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FRC_DROP_MODE, u8DropType));
...
HVD_EX_Result MDrv_HVD_EX_ForceInterlaceMode(MS_U32 u32Id, MS_U8 u8Mode)
{
    ...
    eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FORCE_INTERLACE, u8Mode));
...
HVD_EX_Result MDrv_HVD_EX_PushDispQWithRefNum(MS_U32 u32Id, MS_U8 u8Mode)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_PUSH_DISPQ_WITH_REF_NUM, u8Mode));
...
HVD_EX_Result MDrv_HVD_EX_IgnorePicOverrun(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_IGNORE_PIC_OVERRUN, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_CtlSpeedInDispOnly(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_CTRL_SPEED_IN_DISP_ONLY, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_AVCSupportRefNumOverMaxDBPSize(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_AVC_SUPPORT_REF_NUM_OVER_MAX_DPB_SIZE, bEnable));

    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_ReturnInvalidAFD(MS_U32 u32Id, MS_U32 u32Mode)
{
    HVD_EX_Result eRet = E_HVD_EX_RET_ILLEGAL_ACCESS;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_RETURN_INVALID_AFD, u32Mode));
...
HVD_EX_Result MDrv_HVD_EX_AVCForceBrokenByUs(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_AVC_FORCE_BROKEN_BY_US, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_Support_AVC2MVC(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SUPPORT_AVC_TO_MVC, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_3DLR_View_Exchange(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_3DLR_VIEW_EXCHANGE, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_Enable_New_Slow_Motion(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_ENABLE_NEW_SLOW_MOTION, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_GetUserDataInfo(MS_U32 u32Id,HVD_EX_UserData_Info* pUsrInfo)
{
    ...
    u32UserDataAddr = MS_PA2KSEG1((MS_U32)(pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32CodeBufAddr+u32UserDataAddr));
#ifdef MSOS_TYPE_LINUX_KERNEL
    if(MsOS_MPool_VAError(u32UserDataAddr))
    {
        printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32UserDataAddr,__LINE__);
        eRet = E_HVD_EX_FAIL;
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }
#endif
    if(u32UserDataAddr == NULL)
    {
        eRet = E_HVD_EX_FAIL;
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }
...
HVD_EX_Result MDrv_HVD_EX_SetDTVUserDataMode(MS_U32 u32Id,MS_U8 u8UserDataMode)
{
    ...
    if (u8UserDataMode == 0)
    {
        ...
        eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id,E_HVD_CMD_SET_USERDATA_MODE, 0));
    }
    else if (u8UserDataMode == 1)
    {
        eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id,E_HVD_CMD_SET_USERDATA_MODE, 1));
    }
    else if (u8UserDataMode == 2)
    {
        eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id,E_HVD_CMD_SET_USERDATA_MODE, 2));
    }
    else if (u8UserDataMode == 3)
    {
        eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id,E_HVD_CMD_SET_USERDATA_MODE, 3));
    }
    else if (u8UserDataMode == 7)
    {
        eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id,E_HVD_CMD_SET_USERDATA_MODE, 7));
    }
    else
    {
        eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id,E_HVD_CMD_SET_USERDATA_MODE, u8UserDataMode));
...
HVD_EX_Result MDrv_HVD_EX_GetFrmPackingArrSEI(MS_U32 u32Id, HVD_EX_FrmPackingSEI *pFrmPacking)
{
...
#ifdef MSOS_TYPE_LINUX_KERNEL
    if(MsOS_MPool_VAError(u32CodeVaddr))
    {
        printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32CodeVaddr,__LINE__);
        printk("%s  PA:%lx\n",__FUNCTION__,pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32CodeBufAddr);
        eRet = E_HVD_EX_FAIL;
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }
#endif
    ...
    //u32FPAFrameAddr += u32CodeVaddr; // change to virtual address
...
#ifdef MSOS_TYPE_LINUX_KERNEL
    if(MsOS_MPool_VAError(u32FPAFrameAddr))
    {
        printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32FPAFrameAddr,__LINE__);
        eRet = E_HVD_EX_FAIL;
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }
#endif
...
HVD_EX_Result MDrv_HVD_EX_GetDisplayColourVolumeArrSEI(MS_U32 u32Id, HVD_EX_DisplayColourVolumeSEI *pDisplayColourVolume)
{
    HVD_EX_Result eRet = E_HVD_EX_FAIL;
    MS_U32 u32DCVFrameAddr = 0 ;

    HVD_EX_DisplayColourVolumeSEI *pDisplayColourVolumeSrc = NULL;
    MS_BOOL bIn = FALSE;
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    ...
    u32DCVFrameAddr = HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_DisplayColourVolume_SEI_DATA);//get data

    if(u32DCVFrameAddr == 0x0)
    {
        eRet = E_HVD_EX_FAIL;
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }

    u32DCVFrameAddr = MS_PA2KSEG1((MS_U32)(pHVDDrvContext->gHVDCtrl_EX[u8DrvId].MemMap.u32CodeBufAddr+u32DCVFrameAddr)); // change to virtual address
#ifdef MSOS_TYPE_LINUX_KERNEL
    if(MsOS_MPool_VAError(u32DCVFrameAddr))
    {
        printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32DCVFrameAddr,__LINE__);
        eRet = E_HVD_EX_FAIL;
        _DRV_HVD_EX_RET(u8DrvId, eRet);
    }
#endif
    pDisplayColourVolumeSrc = (HVD_EX_DisplayColourVolumeSEI *)u32DCVFrameAddr;

    if(pDisplayColourVolume != NULL)
    {
        bIn = TRUE;

        pDisplayColourVolume->bColourVolumeSEIEnabled         = pDisplayColourVolumeSrc->bColourVolumeSEIEnabled;
        pDisplayColourVolume->u32MaxLuminance                 = pDisplayColourVolumeSrc->u32MaxLuminance;
        pDisplayColourVolume->u32MinLuminance                 = pDisplayColourVolumeSrc->u32MinLuminance;
        pDisplayColourVolume->u16Primaries[0][0]              = pDisplayColourVolumeSrc->u16Primaries[0][0];
        pDisplayColourVolume->u16Primaries[0][1]              = pDisplayColourVolumeSrc->u16Primaries[0][1];
        pDisplayColourVolume->u16Primaries[1][0]              = pDisplayColourVolumeSrc->u16Primaries[1][0];
        pDisplayColourVolume->u16Primaries[1][1]              = pDisplayColourVolumeSrc->u16Primaries[1][1];
        pDisplayColourVolume->u16Primaries[2][0]              = pDisplayColourVolumeSrc->u16Primaries[2][0];
        pDisplayColourVolume->u16Primaries[2][1]              = pDisplayColourVolumeSrc->u16Primaries[2][1];
        pDisplayColourVolume->u16WhitePoint[0]                = pDisplayColourVolumeSrc->u16WhitePoint[0];
        pDisplayColourVolume->u16WhitePoint[1]                = pDisplayColourVolumeSrc->u16WhitePoint[1];
...
HVD_EX_Result MDrv_HVD_EX_DynamicScalingResvNBuffer(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DYNAMIC_SCALE_RESV_N_BUFFER, bEnable));
...
{
#if (defined(CHIP_EDISON) || defined(CHIP_EINSTEIN) || defined(CHIP_EINSTEIN3)) || defined(CHIP_NAPOLI) || defined(CHIP_MONACO) || defined(CHIP_MUJI) || defined(CHIP_CLIPPERS)
...
HVD_EX_Result MDrv_HVD_EX_DSReportDispInfoChange(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DS_RESET_XC_DISP_WIN, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_RVU_Setting_Mode(MS_U32 u32Id, MS_U32 u32Param)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_RVU_SETTING_MODE, u32Param));
...
HVD_EX_Result MDrv_HVD_EX_FramerateHandling(MS_U32 u32Id, MS_U32 u32FrameRate)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_FRAMERATE_HANDLING, u32FrameRate));
...
HVD_EX_Result MDrv_HVD_EX_DualNonBlockMode(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_DUAL_NON_BLOCK_MODE, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_IgnorePicStructDisplay(MS_U32 u32Id, MS_U32 param)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_IGNORE_PIC_STRUCT_DISPLAY, param));
...
HVD_EX_Result MDrv_HVD_EX_InputPtsFreerunMode(MS_U32 u32Id, MS_U32 param)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_INPUT_PTS_FREERUN_MODE, param));
...
HVD_EX_Result MDrv_HVD_EX_ErrConcealStartSlice1stMB(MS_U32 u32Id, MS_U32 param)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_ERR_CONCEAL_SLICE_1ST_MB, param));
...
#define SIZE_3K 0xC00
    ...
    HVD_EX_Drv_Ctrl *pCtrl = &(pHVDDrvContext->gHVDCtrl_EX[u8DrvId]);
    MS_U32 u32DSBufAddr = pExternalBuf->u32DSBufAddr;
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_EXTERNAL_DS_BUF, TRUE));
...
HVD_EX_Result MDrv_HVD_EX_CC_Init(MS_U32 u32Id)
{
    ...
    eRet = _HVD_MapRetEx(HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SET_USERDATA_MODE, pCCInfo->u8UserDataMode));
...
HVD_EX_Result MDrv_HVD_EX_CC_Set_RB_StartAddr(MS_U32 u32Id, MS_U32 u32StartPAddress, MS_U8 u8CC608)
{
    ...
    HVD_EX_MSG_DBG("u32StartPAddress: 0x%lx , u32StartOffset = 0x%lx ,u8CC608: 0x%x, pCCInfo->u8BufMiuSel[u8CC608]: %d, pCtrl->MemMap.u32MIU1BaseAddr: 0x%lx, pCtrl->MemMap.u32MIU2BaseAddr: 0x%lx\n",
        u32StartPAddress , u32StartOffset , u8CC608,pCCInfo->u8BufMiuSel[u8CC608], pCtrl->MemMap.u32MIU1BaseAddr, pCtrl->MemMap.u32MIU2BaseAddr);
...
HVD_EX_Result MDrv_HVD_EX_SetExternal_CC608_Buffer(MS_U32 u32Id, MS_U32 u32Addr, MS_U32 u32Len)
{
    ...
    *u32FWBinaryAddr = pCtrl->MemMap.u32FWBinaryAddr;
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_ENABLE_EXTERNAL_CC_608_BUF, u32CCinfo));
...
HVD_EX_Result MDrv_HVD_EX_SetExternal_CC708_Buffer(MS_U32 u32Id, MS_U32 u32Addr, MS_U32 u32Len)
{
    ...
    *u32FWBinaryAddr = pCtrl->MemMap.u32FWBinaryAddr;
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_ENABLE_EXTERNAL_CC_708_BUF, u32CCinfo));
...
HVD_EX_Result MDrv_HVD_EX_ShowFirstFrameDirect(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SHOW_FIRST_FRAME_DIRECT, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_AVCResizeDosDispPendBuf(MS_U32 u32Id, MS_U32 u32Size)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_AVC_RESIZE_DOS_DISP_PEND_BUF, u32Size));
...
{
    return _HVD_MapRetEx( HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_DMX_FRAMERATE, u32Value));
}
...
{
    return _HVD_MapRetEx( HAL_HVD_EX_SetData(u32Id, E_HVD_SDATA_DMX_FRAMERATEBASE, u32Value));
}
...
HVD_EX_Result MDrv_HVD_EX_ThumbnailMode(MS_U32 u32Id,MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;

    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    pHVDDrvContext->gHVDPreCtrl[u8DrvId].bEnableThumbnailMode = bEnable;
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_DisableDivx(MS_U32 u32Id,MS_BOOL bDisable)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    ...
    pHVDDrvContext->gHVDPreCtrl[u8DrvId].bDisableDivx = bDisable;
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

MS_BOOL MDrv_HVD_EX_IsMoreFrame(MS_U32 u32Id)
{
    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);
    HVD_EX_MSG_TRACE();
    _DRV_HVD_Inited(u8DrvId,FALSE);

    if (HAL_HVD_EX_GetData(u32Id, E_HVD_GDATA_HVD_IS_MORE_FRAME))
    {
        return TRUE;
    }
    else
...
HVD_EX_Result MDrv_HVD_EX_PreConnectDispPath(MS_U32 u32Id,MS_BOOL bEnable, MS_BOOL bConnect, HVD_EX_DISPLAY_PATH   eMvopPath)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;

    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    pHVDDrvContext->gHVDPreCtrl[u8DrvId].stPreConnectDispPath.bEnable = bEnable;
    pHVDDrvContext->gHVDPreCtrl[u8DrvId].stPreConnectDispPath.stDynmcDispPath.bConnect = bConnect;
    pHVDDrvContext->gHVDPreCtrl[u8DrvId].stPreConnectDispPath.stDynmcDispPath.eMvopPath = eMvopPath;
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}

HVD_EX_Result MDrv_HVD_EX_PreSetMFCodecMode(MS_U32 u32Id, HVD_EX_MFCodec_mode eMFCodecMode)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;
    ...
    switch(eMFCodecMode)
    {
        case E_HVD_EX_MFCODEC_DEFAULT:
            pHVDDrvContext->gHVDPreCtrl[u8DrvId].eMFCodecMode = E_HVD_DEF_MFCODEC_DEFAULT;
            break;
        case E_HVD_EX_MFCODEC_FORCE_ENABLE:
            pHVDDrvContext->gHVDPreCtrl[u8DrvId].eMFCodecMode = E_HVD_DEF_MFCODEC_FORCE_ENABLE;
            break;
        case E_HVD_EX_MFCODEC_FORCE_DISABLE:
            pHVDDrvContext->gHVDPreCtrl[u8DrvId].eMFCodecMode = E_HVD_DEF_MFCODEC_FORCE_DISABLE;
            break;
        default:
            pHVDDrvContext->gHVDPreCtrl[u8DrvId].eMFCodecMode = E_HVD_DEF_MFCODEC_DEFAULT;
            break;
...
HVD_EX_Result MDrv_HVD_EX_EnableForceStartNonISlice(MS_U32 u32Id,MS_BOOL bEnable)
{
    HVD_EX_Result eRet = E_HVD_EX_OK;

    MS_U8 u8DrvId = _HVD_EX_GetDrvId(u32Id);

    pHVDDrvContext->gHVDPreCtrl[u8DrvId].bEnableForceStartNonISlice = bEnable;
    HAL_HVD_EX_SetPreCtrlVariables(u32Id,(MS_U32)(&pHVDDrvContext->gHVDPreCtrl[u8DrvId]));
    _DRV_HVD_EX_RET(u8DrvId, eRet);
}
...
HVD_EX_Result MDrv_HVD_EX_SetPTSUsecMode(MS_U32 u32Id, MS_BOOL bEnable)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_SET_PTS_US_MODE, bEnable));
...
HVD_EX_Result MDrv_HVD_EX_PVRTimeShiftSeamlessMode(MS_U32 u32Id, MS_U8 u8Arg)
{
    ...
    eRet = _HVD_MapRetEx( HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_PVR_SEAMLESS_TIMESHIFT, u8Arg));
...
MS_U32 MDrv_HVD_EX_GetSHM_Offset(void)
{
    return HAL_VPU_EX_GetSHM_Offset();
}
