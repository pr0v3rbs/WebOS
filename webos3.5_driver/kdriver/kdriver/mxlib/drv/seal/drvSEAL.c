...
// Local Defines
//-------------------------------------------------------------------------------------------------
#define SEAL_ERROR(fmt, args...)           printf("[SEAL USER ERR][%06d]     " fmt, __LINE__, ## args)
#define SEAL_WARN(fmt, args...)            printf("[SEAL USER WARN][%06d]    " fmt, __LINE__, ## args)
#define SEAL_PRINT(fmt, args...)           printf("[SEAL USER][%06d]         " fmt, __LINE__, ## args)
...
{
    MS_U32 u32NonPmBaseAddr = 0;
    MS_U32 u32NonPmBankSize = 0;
    MS_U32 u32PmBaseAddr = 0;
    MS_U32 u32PmBankSize = 0;
...
MS_BOOL MDrv_Seal_SecureRangeSet(MS_U8 u8SecureRangeId ,MS_U64 u64StartAddr, MS_U64 u64EndAddr, MS_U32 u32Attribute)
{
    ...
    {
        SEAL_ERROR("Secure range address should be aligned to %x\n",SEAL_SECURE_RANGE_ALIGN);
        bret = FALSE;
        return bret;
    }
...
MS_BOOL MDrv_Seal_IMI_RangeSet(MS_U32 u32StartAddr, MS_U32 u32EndAddr, MS_BOOL bEnable)
{
    ...
    {
        SEAL_ERROR("IMI range address should be aligned to %x\n",SEAL_IMI_RANGE_ALIGN);
        bret = FALSE;
        return bret;
    }
...
MS_BOOL MDrv_Seal_GetHittedInfo(MS_U8 u8MiuDev, Seal_PortectInfo *pInfo)
{
    ...
    if(pInfo->bIsHit == TRUE)
    {
        SEAL_PRINT("MIU:%u SecureRangeId:%u ClientId:0x%x IsSecure:%u IsWrite:%u HitAddr:0x%llx\n", u8MiuDev, 
                   pInfo->u8SecureRangeId, pInfo->u8ClientId, pInfo->bIsSecure, pInfo->bIsWrite, pInfo->u64HitAddr);
    }
