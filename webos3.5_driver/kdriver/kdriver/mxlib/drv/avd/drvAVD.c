...
{
    static AVD_COPYTOUSER spArgs = {0};
    if((u32AVDopen!=-1)&&(u8AVDUtopiaOpen==1))
    {
...
void MDrv_AVD_SetAnaDemod(MS_BOOL bEnable)
{
    MS_BOOL *pArgs = &bEnable;
    if((u32AVDopen!=-1)&&(u8AVDUtopiaOpen==1))
        UtopiaIoctl(ppAVDInstant, MDrv_CMD_AVD_SetAnaDemod, pArgs);
}

void MDrv_AVD_SetHsyncSensitivityForDebug(MS_U8 u8AFEC99 ,MS_U8 u8AFEC9C)
{
    AVD_SETHSYNCSENSITIVITYFORDEBUG _eAVD_SETHSYNCSENSITIVITYFORDEBUG;
    _eAVD_SETHSYNCSENSITIVITYFORDEBUG.u8AFEC99= u8AFEC99;
    _eAVD_SETHSYNCSENSITIVITYFORDEBUG.u8AFEC9C = u8AFEC9C;
    if((u32AVDopen!=-1)&&(u8AVDUtopiaOpen==1))
        UtopiaIoctl(ppAVDInstant, MDrv_CMD_AVD_SetHsyncSensitivityForDebug, &_eAVD_SETHSYNCSENSITIVITYFORDEBUG);
}