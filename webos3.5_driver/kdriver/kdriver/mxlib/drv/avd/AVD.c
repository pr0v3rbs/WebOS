...
#define LM15U_GetStandard 1    //for Lm15u, after setcolorsystem, GetStandardDetection need to return the standard which is enable
//-------------------------------------------------------------------------------------------------
//  Debug Functions
//-------------------------------------------------------------------------------------------------
#ifdef MS_DEBUG
...
MS_U32 AVDOpen(void** ppInstance, MS_U32 u32ModuleVersion, void* pAttribute)
{
    ...
    pAvdPri->fpAVDSetAnaDemod= Drv_AVD_SetAnaDemod;
    pAvdPri->fpAVDSetHsyncSensitivityForDebug= Drv_AVD_SetHsyncSensitivityForDebug;
...
MS_U32 AVDIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    PAVD_SETHSYNCSENSITIVITYFORDEBUG pSetHsyncSensitivityForDebug = NULL;
    ...
    switch(u32Cmd)
    {
        case MDrv_CMD_AVD_Init:
            AVD_debug(printf("MDrv_CMD_AVD_Init\n"));
        ...
        case MDrv_CMD_AVD_ResetMCU:
            AVD_debug(printf("MDrv_CMD_AVD_ResetMCU\n"));
        ...
        case MDrv_CMD_AVD_StartAutoStandardDetection:
            AVD_debug(printf("MDrv_CMD_AVD_StartAutoStandardDetection\n"));
            (psAvdInstPri->fpAVDStartAutoStandardDetection)(psAvdResPri);
            break;
        case MDrv_CMD_AVD_ForceVideoStandard:
            AVD_debug(printf("MDrv_CMD_AVD_ForceVideoStandard: Force standard=%d\n",*(AVD_VideoStandardType *)pArgs));
        ...
        case MDrv_CMD_AVD_3DCombSpeedup:
            AVD_debug(printf("MDrv_CMD_AVD_3DCombSpeedup\n"));
            (psAvdInstPri->fpAVD3DCombSpeedup)(psAvdResPri);
            break;
        case MDrv_CMD_AVD_LoadDSP:
            AVD_debug(printf("MDrv_CMD_AVD_LoadDSP \n"));
        ...
        case MDrv_CMD_AVD_BackPorchWindowPositon:
            AVD_debug(printf("MDrv_CMD_AVD_BackPorchWindowPositon\n"));
        ...
        case MDrv_CMD_AVD_MBX_ReadByteByVDMbox:
            AVD_debug(printf("MDrv_CMD_AVD_MBX_ReadByteByVDMbox\n"));
        ...
        case MDrv_CMD_AVD_SetVideoStandard:
            ...
            AVD_debug(printf("MDrv_CMD_AVD_SetVideoStandard: Standard=%d, IsInAutoTuning= %d\n",pSetVideoStandard->eStandard,pSetVideoStandard->bIsInAutoTuning));
        ...
        case MDrv_CMD_AVD_SetFactoryPara:
            ...
            AVD_debug(printf("MDrv_CMD_AVD_SetFactoryPara: FactoryPara= %d , value= 0x%x\n", pSetFactorypara->FactoryPara,pSetFactorypara->u8Value));
            (psAvdInstPri->fpAVDSetFactoryPara)(pSetFactorypara->FactoryPara, pSetFactorypara->u8Value);
            break;
            */
        case MDrv_CMD_AVD_Set_Htt_UserMD:
            AVD_debug(printf("MDrv_CMD_AVD_Set_Htt_UserMD: 0x%x\n", *(MS_U16*)pArgs));
            psAvdResPri->_u16Htt_UserMD = *(MS_U16*)pArgs;
            break;
        case MDrv_CMD_AVD_SetDbgLevel:
            pSetDbgLevel = (PAVD_SETDBGLEVEL)pArgs;
            AVD_debug(printf("MDrv_CMD_AVD_SetDbgLevel: %d\n",*(AVD_DbgLv*)pArgs));
        ...
        case MDrv_CMD_AVD_GetInfo:
            ...
            pCopyToUser->eAVDInfo.eAnaDemodType= psAvdResPri->g_VD_InitData.eDemodType;
            break;
        ...
        case MDrv_CMD_AVD_GetCGMSDetect:
            ...
            pCopyToUser->bEnable= (MS_BOOL)u32Ret;
            */
        case MDrv_CMD_AVD_SetAnaDemod:
            AVD_debug(printf("MDrv_CMD_AVD_SetAnaDemod: Enanle=%d\n", *(MS_BOOL *)pArgs));
            (psAvdInstPri->fpAVDSetAnaDemod)(*(MS_BOOL *)pArgs, psAvdResPri);
            break;
        case MDrv_CMD_AVD_SetHsyncSensitivityForDebug:
            pSetHsyncSensitivityForDebug= (PAVD_SETHSYNCSENSITIVITYFORDEBUG)pArgs;
            (psAvdInstPri->fpAVDSetHsyncSensitivityForDebug)(pSetHsyncSensitivityForDebug->u8AFEC99, pSetHsyncSensitivityForDebug->u8AFEC9C, psAvdResPri);
            break;
        default:
            break;
    }
...
static void _Drv_AVD_VDMCU_Init(AVD_RESOURCE_PRIVATE *pResource)
{
    ...
    #if (VD_CHIP_VERSION==AVD_TITANIA3_A) // weicheng 20121003
    ...
    // Load code
    //--------------------------------------------------------
    #if (VD_CHIP_VERSION==AVD_TITANIA4_A)//weicheng 20121003
    if (1)
    #else
...
static void _Drv_AVD_SyncRangeHandler(AVD_RESOURCE_PRIVATE *pResource)
{
    ...
    XC_SETWIN_INFO sXC_SetWinInfoTemp;
...
*/
    ...
    if(u8SyncStableCounter == 5)
    {
        ...
        MApi_XC_GetWinInfo(&sXC_SetWinInfoTemp, MAIN_WINDOW);
        // reset changed part
        sXC_SetWinInfo.bPreHCusScaling = sXC_SetWinInfoTemp.bPreHCusScaling;
        sXC_SetWinInfo.u16PreHCusScalingSrc = sXC_SetWinInfoTemp.u16PreHCusScalingSrc;
        sXC_SetWinInfo.u16PreHCusScalingDst = sXC_SetWinInfoTemp.u16PreHCusScalingDst;
...
AVD_Result Drv_AVD_Init(VD_INITDATA *pVD_InitData, MS_U32 u32InitDataLen, AVD_RESOURCE_PRIVATE *pResource)
{
    ...
    #if (VD_CHIP_VERSION!=AVD_TITANIA3_A) //20121003 weicheng
...
*/    
    ...
    {  // burst
        ...
        if ( pResource->_u8Comb10Bit3Flag )
        {
            if ((pResource->_u8Comb10Bit3Flag&(BIT(0))) && (MsOS_GetSystemTime() -pResource->_u32VideoSystemTimer >1000))
            {
                ...
                if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
                {
                    printf("Drv_AVD_3DCombSpeedup Disable\n");
                }
                #endif
            }
            if ((pResource->_u8Comb10Bit3Flag&(BIT(1))) && (MsOS_GetSystemTime() -pResource->_u32VideoSystemTimer >500))
            {
                if (!(pResource->_u8AfecD4Factory & BIT(4)))
                {
            ...
            if ((pResource->_u8Comb10Bit3Flag&(BIT(2))) && (MsOS_GetSystemTime() -pResource->_u32VideoSystemTimer >300))
            {
...
*/
    ...
    if (eSource == E_INPUT_SOURCE_ATV)
    {
        if (pResource->g_VD_InitData.bRFGainSel==VD_USE_FIX_GAIN)
        {
            ...
            {
                HAL_AVD_AFEC_AGCSetCoarseGain(VD_AGC_COARSE_GAIN_X_1);
                HAL_AVD_AFEC_AGCSetFineGain(pResource->g_VD_InitData.u8RFGain);
            }
            else
            {
                HAL_AVD_AFEC_AGCSetCoarseGain(VD_AGC_COARSE_GAIN_SAMSUNG);
                HAL_AVD_AFEC_AGCSetFineGain(pResource->g_VD_InitData.u8RFGain);
            }
        }
        else
    ...
    #if (VD_CHIP_VERSION==AVD_TITANIA3_A) //weicheng 20121003
    if ((pResource->g_VD_InitData.u32VDPatchFlag & AVD_PATCH_SAMSUNG_DSP)&&((pResource->g_VD_InitData.u32VDPatchFlag & AVD_PATCH_INTERNAL_DEMOD) ==0)) // use external demodulator
        {
...
MS_BOOL Drv_AVD_SetVideoStandard(AVD_VideoStandardType eStandard, MS_BOOL bIsInAutoTuning, AVD_RESOURCE_PRIVATE *pResource)
{
   ...
   if (eStandard>=E_VIDEOSTANDARD_MAX)
    ...
    if(eStandard == E_VIDEOSTANDARD_NTSC_M && pResource->_eVDInputSource == E_INPUT_SOURCE_ATV)
        {
        HAL_AVD_SetBurstWinStart(0x6E);
        }
    else
        {
        HAL_AVD_SetBurstWinStart(0x62);
        }
    #endif
    switch (eStandard)
    {
    ...
    if (eStandard == E_VIDEOSTANDARD_SECAM)
    {
        ...
        if (pResource->g_VD_InitData.u32VDPatchFlag & AVD_PATCH_DSPCODE_LG)
            {
            HAL_AVD_SetReg(BK_SECAM_1E, HAL_AVD_GetReg(BK_SECAM_1E)|((BIT_(4))));
            }
    }
    else
    {
    ...
    if (pResource->g_VD_InitData.u32VDPatchFlag & AVD_PATCH_FINE_TUNE_COMB_F2)
    {
        ...
        if(((pResource->_eVDInputSource == E_INPUT_SOURCE_CVBS1) ||(pResource->_eVDInputSource == E_INPUT_SOURCE_CVBS2) ||( pResource->_eVDInputSource == E_INPUT_SOURCE_CVBS3)) && (eStandard == E_VIDEOSTANDARD_NTSC_M))
        {
...
void Drv_AVD_SetChannelChange(AVD_RESOURCE_PRIVATE *pResource)
{
    ...
    if (pResource->_u8AVDDbgLevel == AVD_DBGLV_DEBUG)
    {
        printf("Drv_AVD_SetChannelChange\n");
    }
    #endif
...
void Drv_AVD_Set3dComb(MS_BOOL bEnable, AVD_RESOURCE_PRIVATE *pResource)
{
    ...
    HAL_AVD_COMB_Set3dComb(bEnable);
    AVD_UNLOCK(pResource->_s32AVD_Mutex,pResource->_u8AVDDbgLevel);
}
/*
...
/*
void Drv_AVD_SetFactoryPara(AVD_Factory_Para FactoryPara,MS_U8 u8Value)
{
    MS_U8 u8tmp;
...
//for LM15U WebOS, after SetColorSystem, GetStandardDetection need to return the standard which is enable          
...
//if only 576i is enable and input signal is 480i , must return 576i standard (PAL)
            ...
            {
                if((HAL_AVD_GetReg(BK_AFEC_CB))&(BIT(0)))     // PAL-BGHI disable
                {
                    if(!((HAL_AVD_GetReg(BK_AFEC_CB))&(BIT(6))))   //   PAL disable; PAL-N enable
                    {
                        eVideoStandard = E_VIDEOSTANDARD_PAL_N;
                    }
                    else if(!((HAL_AVD_GetReg(BK_AFEC_CB))&(BIT(1))))   // PAL, PAL-N disable; SECAM enable
                    {
                        eVideoStandard = E_VIDEOSTANDARD_SECAM;
                    }
                    else                                                                    // All 576i disable
                    {
                        eVideoStandard = E_VIDEOSTANDARD_NTSC_M;      // Set 480i standard
                    }
                }    
            }
            else if(eVideoStandard == E_VIDEOSTANDARD_NTSC_M)
            {
                if((HAL_AVD_GetReg(BK_AFEC_CB))&(BIT(2)))    // NTSC disable
                {
                    if(!((HAL_AVD_GetReg(BK_AFEC_CB))&(BIT(3))))   //   NTSC disable; NTSC443 enable
                    {
                        eVideoStandard = E_VIDEOSTANDARD_NTSC_44;
                    }
                    else if(!((HAL_AVD_GetReg(BK_AFEC_CB))&(BIT(4))))   // NTSC, NTSC443 disable; PAL-M enable
                    {
                        eVideoStandard = E_VIDEOSTANDARD_PAL_M;
                    }
                    else                                                                    // All 480i disable
                    {
                        eVideoStandard = E_VIDEOSTANDARD_PAL_BGHI;      // Set 576i standard
                    }
                }
            }
#endif
...
*/
void Drv_AVD_SetAnaDemod(MS_BOOL bEnable, AVD_RESOURCE_PRIVATE *pResource)
{
    if(bEnable)
    {
...
void Drv_AVD_SetHsyncSensitivityForDebug(MS_U8 u8AFEC99, MS_U8 u8AFEC9C, AVD_RESOURCE_PRIVATE *pResource)
{
