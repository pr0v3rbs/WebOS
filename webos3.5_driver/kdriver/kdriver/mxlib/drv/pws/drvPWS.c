...
MS_U8 MDrv_PWS_SetIOMapBase(void)
{
    ...
    {
        DEBUG_INFO(printf("Get IOMAP Base faill!\n"));
        return 0;
    }
    DEBUG_INFO(printf("Get IOMAP ID:%u Base:%lx!\n", MS_MODULE_PWS, u32BaseAddr));
...
E_PWS_Result MDrv_PWS_Init(E_PWS_VIF_type eVifType)
{
    ...
    if(!PWS_Init)
    {
        ...
        {
            DEBUG_INFO(printf("Get IO base fail\n"));
            return PWS_FAIL;
        }
...
{  
    ...
    if(sockfd < 0)
    {
        DEBUG_INFO(printf("sockfd is negative %s\n",__FUNCTION__));
        return PWS_FAIL;
    }
    ...
    if(bSetFlag == PHY_OFF)
    {
        ...
        {
            DEBUG_INFO(printf("IOCTL FAIL %s\n",__FUNCTION__));
            close(sockfd);
            return PWS_FAIL;
        }
    }
...
#ifdef PRINT
    printf("\033[35mFunction = %s, Line = %d, PWS Register finished\033[m\n", __PRETTY_FUNCTION__, __LINE__); 
#endif
}
...
MS_U32 PWSOpen(void** ppInstance, MS_U32 u32ModuleVersion, void* pAttribute)
{
    ...
    if(u32Ret == 0)
    {
        ...
        if (u32RetTmp == 0)
        {
            printf("[%s error] something wrong in MsOS_SHM_GetId\n", __FUNCTION__);
            printf("is SHM_SIZE reached?\n");
            printf("is MAX_SHM_CLIENT_NUM reached?\n");
            MS_ASSERT(0);
            PWS_MUTEX_UNLOCK();
            return FALSE;
        }
    ...
    {
        printf("\033[35mFunction = %s, Line = %d, [Multi-process Safe] A Instance is existed!!\033[m\n", __PRETTY_FUNCTION__, __LINE__);
        PWS_MUTEX_UNLOCK();
        return FALSE;
    }
    else
    ...
    printf("\033[35mFunction = %s, Line = %d, [PWS INFO] OPEN INSTANCE...\033[m\n", __PRETTY_FUNCTION__, __LINE__); // joe.liu
...
MS_U32 PWSIoctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    switch(u32Cmd)
    {
        case MDrv_CMD_PWS_GetLibVer:
            printf("PWSIoctl - MDrv_CMD_PWS_GetLibVer\n");
        ...
        case MDrv_CMD_PWS_SetDbgLevel:
            printf("PWSIoctl - MDrv_CMD_PWS_SetDbgLevel\n");
        ...
        case MDrv_CMD_PWS_HandleSource:
            printf("PWSIoctl - MDrv_CMD_PWS_HandleSource\n");
        ...
        case MDrv_CMD_PWS_PHYPower:
            printf("PWSIoctl - MDrv_CMD_PWS_PHYPower\n");
...
MS_U32 PWSClose(void* pInstance)
{
    ...
    {
        printf("\033[35mFunction = %s, Line = %d, [Multi-process Safe] u32IsInstanceCreated value is incorrect, which is %lu!!\033[m\n", __PRETTY_FUNCTION__, __LINE__, *(MS_U32 *)u32RetAddr);
        return FALSE;
    }
    else
    {
        printf("\033[35mFunction = %s, Line = %d, [Multi-process Safe] Release a Instance!!\033[m\n", __PRETTY_FUNCTION__, __LINE__);
        PWS_MUTEX_LOCK();
        *(MS_U32 *)u32RetAddr = 0;
        PWS_MUTEX_UNLOCK();
        return TRUE;
