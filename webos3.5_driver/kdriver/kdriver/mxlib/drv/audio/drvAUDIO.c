...
#define DRVAUDIO_PRINT(fmt, args...)    LOGI("<<android>>      " fmt, ## args)
#define DRVAUDIO_ERROR(fmt, args...)    LOGE("<<android>>      " fmt, ## args)
#else
    #ifdef CONFIG_MBOOT   //For Speed up boot time
        #define DRVAUDIO_PRINT(fmt, args...)
        #define DRVAUDIO_ERROR(fmt, args...)
    #else
        #define DRVAUDIO_PRINT(fmt, args...)
        #define DRVAUDIO_ERROR(fmt, args...)    printf("[[utopia]]      " fmt, ## args)
#endif
#endif
...
#define DRVAUDIO_CHECK_SHM_INIT \
    do { \
        ...
        { \
            DRVAUDIO_ERROR("%s() : Warning! g_AudioVars2 should not be NULL !!!\n", __FUNCTION__); \
            DRVAUDIO_ERROR("%s() : Perform SHM Init here !!!\n", __FUNCTION__); \
...
static MS_U32 _gu32AudioRiuBaseAddr;

// Global variables for VoIP applications to register the CallBack.
static void (*audio_dec_app_callbk)(MS_U8) = NULL;
static void (*audio_se_app_callbk)(MS_U8) = NULL;
...
MS_U32 (* gGetDDRInfoFuncPtr)(AUDIO_DEC_ID DecId, EN_AUDIO_DDRINFO DDRInfo) = NULL;

#define AUDIO_ASSERT(_cnd, _fmt, _args...)                                      \
                if (!(_cnd)) {                                                  \
                    AUD_ERRMSG(printf("AUDIO ASSERT: "_fmt, ##_args));          \
                    while(1);                                                   \
                }
...
#endif
void MDrv_AUDIO_R2ISR_0( InterruptNum eIntNum );
void MDrv_AUDIO_R2ISR_1( InterruptNum eIntNum );
...
extern MS_BOOL bReBoot_All_Aud_Proc;
...
void MDrv_AUDIO_InitMMIO(void)
{
    ...
    {
        DRVAUDIO_ERROR("Audio get base address failed\n");
        return;
    }
    else
    {
        AUD_DBGMSG(printf("MDrv_AUDIO_Init u32PnlRiuBaseAddr = %lx\n", _gu32AudioRiuBaseAddr));
    }
...
MS_BOOL MDrv_AUDIO_AttachInterrupt(MS_BOOL bEnable)
{
    ...
    if (bEnable)
    {
        ...
        // Attached R2 interrupt
        MsOS_AttachInterrupt( E_INT_FIQ_R2TOMCU_INT0, MDrv_AUDIO_R2ISR_0);

        // Currently E_INT_FIQ_R2TOMCU_INT1 us R2 while 1 counter, not attached E_INT_FIQ_R2TOMCU_INT1
        //MsOS_AttachInterrupt( E_INT_FIQ_R2TOMCU_INT1, MDrv_AUDIO_R2ISR_1);
        MsOS_EnableInterrupt(E_INT_FIQ_R2TOMCU_INT0);
        //MsOS_EnableInterrupt(E_INT_FIQ_R2TOMCU_INT1);
    }
    else
    {
        ...
        // detatch R2 interrupt
        MsOS_DisableInterrupt( E_INT_FIQ_R2TOMCU_INT0);
        MsOS_DisableInterrupt( E_INT_FIQ_R2TOMCU_INT1);
    }
#endif
    return TRUE;
}
...
{
    HAL_AUDIO_SetDspBaseAddr(u8Index, u32Bin_Base_Address, u32Mad_Base_Buffer_Adr);
}
...
/// @return MS_U32     \b     :
////////////////////////////////////////////////////////////////////////////////
MS_U32 MDrv_AUDIO_GetDspBinBaseAddr(MS_U8 u8Index)
{
...
{
    HAL_AUDIO_R2ISR_0();
    MsOS_EnableInterrupt(E_INT_FIQ_R2TOMCU_INT0);   // enable interrput
}

void MDrv_AUDIO_R2ISR_1( InterruptNum eIntNum )
{
    HAL_AUDIO_R2ISR_1();
    MsOS_EnableInterrupt(E_INT_FIQ_R2TOMCU_INT1);   // enable interrput
}
...
void MDrv_AUDIO_SetInputPath( AUDIO_INPUT_TYPE u8Input, AUDIO_PATH_TYPE u8Path)
{
    ...
    if((u8Input != AUDIO_HDMI_INPUT) && (u8Input != AUDIO_DSP1_HDMI_INPUT) && (u8Input != AUDIO_DSP3_HDMI_INPUT))
    {
        ...
        //In LM15U, need to consider DTV using ADEC1 and go to CH6,
        //need to disable DSP HDMI Auto mute.
        if(u8Path == AUDIO_PATH_5)
        {
            HAL_AUDIO_WriteMaskByte((REG_AUDIO_MUTE_CTRL1+1), 0x78, 0x00);
        }
    }
    else if(u8Input == AUDIO_HDMI_INPUT)  // HDMI pcm in
    {
            HAL_AUDIO_WriteMaskByte((REG_AUDIO_MUTE_CTRL1+1) + (path-4)*2, 0x78, 0x58);
    }
    else if((u8Input == AUDIO_DSP1_HDMI_INPUT) ||(u8Input == AUDIO_DSP3_HDMI_INPUT))  // HDMI non-PCM in
    {
        HAL_AUDIO_WriteMaskByte((REG_AUDIO_MUTE_CTRL1+1) + (path-4)*2, 0x78, 0x50);
    }
}
...
// [LM14]
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: MDrv_AUDIO_ARC_HWEN()
/// @brief \b Function \b Description:  This routine is used to enable ARC output (Hardware)
/// @param bEnable     \b : 0--Disable ARC out,
///                         1--Enable ARC out
////////////////////////////////////////////////////////////////////////////////
void MDrv_AUDIO_ARC_HWEN(MS_U8 bEnable)
{
    _HAL_AUDIO_ARC_HWEN((MS_BOOL)bEnable);
}
//[M2]
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: MDrv_AUDIO_ARC_HWEN_BY_PORT()
/// @brief \b Function \b Description:  This routine is used to enable ARC output by port(Hardware)
/// @param bEnable     \b : 0--Disable ARC out,
///                         1--Enable ARC out
////////////////////////////////////////////////////////////////////////////////
void MDrv_AUDIO_ARC_HWEN_BY_PORT(AUDIO_HDMI_INDEX_T hdmiIndex, MS_BOOL bOnOff)
{
    _HAL_AUDIO_ARC_HWEN_BY_PORT(hdmiIndex, bOnOff);
}
...
MS_BOOL MDrv_AUDIO_DigitalOut_GetChannelStatus(DIGITAL_OUTPUT_TYPE ePath, Digital_Out_Channel_Status_t *stChannelStatus)
{
    return (HAL_AUDIO_DigitalOut_GetChannelStatus(ePath, stChannelStatus));
}
...
/// @param <RET>       \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void MDrv_AUDIO_DmaReader_Init(PCM_OUTPUT_FORMAT* pSettings)
{
    HAL_AUDIO_DmaReader_Init(pSettings);
}
...
/// @param <RET>       \b NONE    :
////////////////////////////////////////////////////////////////////////////////
MS_BOOL MDrv_AUDIO_DmaReader_WritePCM(MS_U8 u8Idx, void* buffer, MS_U32 bytes)
{
    MS_BOOL bRet = FALSE;
    bRet = HAL_AUDIO_DmaReader_WritePCM(u8Idx, buffer,bytes);
    return bRet;
}
// [LM14]
////////////////////////////////////////////////////////////////////////////////
...
/// @param <RET>       \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void MDrv_AUDIO_SwDmaReader_Init(PCM_OUTPUT_FORMAT* pSettings)
{
    HAL_AUDIO_SwDmaReader_Init(pSettings);
}
...
/// @param <RET>       \b NONE    :
////////////////////////////////////////////////////////////////////////////////
MS_BOOL MDrv_AUDIO_SwDmaReader_WritePCM(MS_U8 u8Idx, void* buffer, MS_U32 bytes)
{
    return(HAL_AUDIO_SwDmaReader_WritePCM(u8Idx, buffer,bytes));
}
...
MS_BOOL MDrv_AUDIO_VoIP_Config(AUDIO_VoIP_CONFIG_TYPE configType, MS_U32 param1, MS_U32 param2)
{
    ...
    switch(configType)
    {
        ...
        default:
        {
            AUD_ERRMSG(printf("%s: Invalid VoIP Config Type (%d) !!\n", __func__, (int)configType));
            break;
        }
    }
...
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: MDrv_AUDIO_SetCaptureEnable()
/// @brief \b Function \b Description:  For Samsung PCM capture
////////////////////////////////////////////////////////////////////////////////
void MDrv_AUDIO_SetCaptureEnable(MS_BOOL bEnable, void * PCM_CB)
{
    HAL_AUDIO_SetCaptureEnable(bEnable, PCM_CB);
}
...
// [LM14]
MS_U32 MDrv_AUDIO_Decoded_Frame_Cnt(MS_BOOL bDspType)
{
    if(bDspType == DSP_DEC)
    {
        //return(HAL_MAD_Read_DSP_sram((DSP1DmAddr_sys_DecStatus+SYS_DECSTATUS_OK_CNT), DSP_MEM_TYPE_DM));
        return (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_OK_FRMCNT, ADEC1));
    }
    else
    {
        //return(HAL_MAD_Read_DSP_sram((DSP1DmAddr_sys_DecStatus+SYS_DECSTATUS_OK_CNT2), DSP_MEM_TYPE_DM));
        return (HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_OK_FRMCNT, ADEC2));
    }
}

MS_U16 MDrv_AUDIO_HDMI_RX_GetHdmiInChannelStatus(void)
{
    return( HAL_AUDIO_HDMI_RX_GetHdmiInChannelStatus());
}
...
MS_S32 MDrv_AUDIO_PCM_converter(void *pDstData, MS_U32 u32DstChannel, MS_U32 u32DstSize, const void *pSrcData, AUDIO_PCM_INFO_t *pSrcPCMInfo)
{
    ...
    if (pDstData == NULL)
    {
        DRVAUDIO_ERROR("%s: Error! pDstData should not be NULL!\n", __FUNCTION__);
        return -A_EFAULT;
    }

    if (pSrcData == NULL)
    {
        DRVAUDIO_ERROR("%s: Error! pSrcData should not be NULL!\n", __FUNCTION__);
        return -A_EFAULT;
    }

    if (pSrcPCMInfo == NULL)
    {
        DRVAUDIO_ERROR("%s: Error! pSrcPCMInfo should not be NULL!\n", __FUNCTION__);
        return -A_EFAULT;
    }

    if ((u32DstChannel != 2) && (u32DstChannel != 10))
    {
        DRVAUDIO_ERROR("%s: Error! u32DstChannel(%lu) is invalid!\n", __FUNCTION__, u32DstChannel);
        return -A_EINVAL;
    }

    if (u32DstSize == 0)
    {
        DRVAUDIO_ERROR("%s: Error! u32Size(%lu) is invalid!\n", __FUNCTION__, u32DstSize);
        return -A_EINVAL;
    }
    ...
    {
        DRVAUDIO_ERROR("%s: Error! u32Channel(%lu) is invalid!\n", __FUNCTION__, u32Channel);
        return -A_EINVAL;
    }
    ...
    {
        DRVAUDIO_ERROR("%s: Error! u32BitWidth(%lu) is invalid!\n", __FUNCTION__, u32BitWidth);
        return -A_EINVAL;
    }
    ...
    {
        DRVAUDIO_ERROR("%s: Error! u32BigEndian(%u) is invalid!\n", __FUNCTION__, (unsigned int)u32BigEndian);
        return -A_EINVAL;
    }
    ...
    else
    {
        ...
        u32PcmSample = (u32DstSize / u32DstChannel) / u32DstBitWidth;
...
void MDrv_AUDIO_PCM_Init(void)
{
    ...
    DRVAUDIO_ERROR("%s: Init PCM device\n", __FUNCTION__);

    DRVAUDIO_CHECK_SHM_INIT;

    if (g_Common_PCM_IO_Init == TRUE)
    {
        DRVAUDIO_ERROR("%s: already initialized\n", __FUNCTION__);
        return;
    }
...
void MDrv_AUDIO_PCM_DeInit(void)
{
    ...
    DRVAUDIO_ERROR("%s: DeInit PCM device\n", __FUNCTION__);

    DRVAUDIO_CHECK_SHM_INIT;

    if (g_Common_PCM_IO_Init == FALSE)
    {
        DRVAUDIO_ERROR("%s: no need to de-initialize\n", __FUNCTION__);
        return;
    }
...
MS_S32 MDrv_AUDIO_PCM_Open(void *pData)
{
    ...
    MS_S32 s32DeviceId = 0;
    MS_S32 s32Ret = 0;
    DRVAUDIO_ERROR("%s: Open PCM device\n", __FUNCTION__);

    DRVAUDIO_CHECK_SHM_INIT;

    if (pData == NULL)
    {
        DRVAUDIO_ERROR("%s: Error! pData should not be NULL!\n", __FUNCTION__);
        return -A_EFAULT;
    }
...
#if 1 /* A patch here! it's for GST, to avoid incorrect PCM settings from GST */
        if (pUserPcmInfo->u32Channel != 10)
        {
            pUserPcmInfo->u32Channel = 2;
        }
        pUserPcmInfo->u32BitWidth = 16;
#endif
    ...
    DRVAUDIO_ERROR("%s: return device-id %ld\n", __FUNCTION__, s32DeviceId);
...
MS_S32 MDrv_AUDIO_PCM_Close(MS_S32 s32DeviceId)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_S32 s32Ret = 0;
    DRVAUDIO_ERROR("%s: Close PCM device-id %ld\n", __FUNCTION__, s32DeviceId);

    DRVAUDIO_CHECK_SHM_INIT;

    if ((s32DeviceId < 0) || (s32DeviceId >= AUDIO_PCM_MAXNUM))
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) is invalid!\n", __FUNCTION__, s32DeviceId);
        return -A_EINVAL;
    }
    ...
    if (pPCM == NULL)
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) doesn't support PCM I/O!\n", __FUNCTION__, s32DeviceId);
        return -A_EPERM;
    }

    if (pPCM->tPcmOps.close == NULL)
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) doesn't register close()!\n", __FUNCTION__, s32DeviceId);
        return -A_EPERM;
    }

    if (pPCM->pPcmInfo->u8ConnectFlag == FALSE)
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) is not opened yet!\n", __FUNCTION__, s32DeviceId);
        return -A_EPERM;
    }
...
MS_S32 MDrv_AUDIO_PCM_Start(MS_S32 s32DeviceId)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_S32 s32Ret = 0;
    DRVAUDIO_ERROR("%s: Start PCM device-id %ld\n", __FUNCTION__, s32DeviceId);
    ...
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) doesn't register start()!\n", __FUNCTION__, s32DeviceId);
        return -A_EPERM;
    }
...
MS_S32 MDrv_AUDIO_PCM_Stop(MS_S32 s32DeviceId)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_S32 s32Ret = 0;
    DRVAUDIO_ERROR("%s: Stop PCM device-id %ld\n", __FUNCTION__, s32DeviceId);
    ...
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) doesn't register stop()!\n", __FUNCTION__, s32DeviceId);
        return -A_EPERM;
    }
...
MS_S32 MDrv_AUDIO_PCM_Set(MS_S32 s32DeviceId, MS_U32 u32Cmd, const void *pData)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_S32 s32Ret = 0;
    //DRVAUDIO_ERROR("%s: Set command %lu to PCM device-id %ld\n", __FUNCTION__, u32Cmd, s32DeviceId);
    ...
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) doesn't register set()!\n", __FUNCTION__, s32DeviceId);
        return -A_EPERM;
    }
    ...
    if (pPCM->pPcmInfo->u8StartFlag != FALSE)
    {
        switch (u32Cmd)
        {
            ...
            default:
            {
                DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) is running, can not be configured now!\n", __FUNCTION__, s32DeviceId);
                return -A_EBUSY;
            }
        }
    }
...
MS_S32 MDrv_AUDIO_PCM_Get(MS_S32 s32DeviceId, MS_U32 u32Cmd, void *pData)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_S32 s32Ret = 0;
    //DRVAUDIO_ERROR("%s: Get command %lu from PCM device-id %ld\n", __FUNCTION__, u32Cmd, s32DeviceId);
    ...
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) doesn't register get()!\n", __FUNCTION__, s32DeviceId);
        return -A_EPERM;
    }
...
MS_U32 MDrv_AUDIO_PCM_Read(MS_S32 s32DeviceId, void *pBuf, MS_U32 u32Size)
{
    ...
    MS_U32 u32Loop = 0;
    //DRVAUDIO_ERROR("%s: Read %lu bytes from PCM device-id %ld\n", __FUNCTION__, u32Size, s32DeviceId);

    DRVAUDIO_CHECK_SHM_INIT;

    if (pBuf == NULL)
    {
        DRVAUDIO_ERROR("%s: Error! pBuf should not be NULL!\n", __FUNCTION__);
        return u32Size;
    }
    ...
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) doesn't register read()!\n", __FUNCTION__, s32DeviceId);
        return u32Size;
    }
    ...
    if (pPCM->pPcmInfo->u8StartFlag == FALSE)
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) is not started yet!\n", __FUNCTION__, s32DeviceId);
        return u32Size;
    }
    ...
    do
    {
        ...
        if (u32ReadSize == 0)
        {
            ...
            if (u32Loop > 250)
            {
                DRVAUDIO_ERROR("%s: s32DeviceId(%ld) read PCM timeout!\n", __FUNCTION__, s32DeviceId);
                break;
            }
...
MS_U32 MDrv_AUDIO_PCM_Write(MS_S32 s32DeviceId, const void *pBuf, MS_U32 u32Size)
{
    ...
    MS_U32 u32Loop = 0;
    MS_S32 s32Ret = 0;
    //DRVAUDIO_ERROR("%s: Write %lu bytes to PCM device-id %ld\n", __FUNCTION__, u32Size, s32DeviceId);
    ...
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) doesn't register write()!\n", __FUNCTION__, s32DeviceId);
        return u32Size;
    }
    ...
    {
        DRVAUDIO_ERROR("%s: Error! u32Size(%u) is not a sample base size at %u channels!\n", __FUNCTION__, (unsigned int)u32Size, (unsigned int)pPCM->pPcmInfo->u32Channel);
        return u32Size;
    }
    ...
    do
    {
        ...
        if (u32WriteSize == 0)
        {
            ...
            if (u32Loop > 250)
            {
                DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) write PCM timeout!\n", __FUNCTION__, s32DeviceId);
                break;
            }
        ...
        else
        {
            if (pConvertedPcmBuf == NULL)
            {
                ...
                if (pConvertedPcmBuf == NULL)
                {
                    DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) can't allocate %lu bytes for PCM buffer!\n", __FUNCTION__, s32DeviceId, u32TargetSizeToWrite);
                    return u32Size;
                }
                ...
                if (s32Ret < 0)
                {
                    DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) fail to convert PCM data!\n", __FUNCTION__, s32DeviceId);
                }
...
MS_S32 MDrv_AUDIO_PCM_Flush(MS_S32 s32DeviceId)
{
    AUDIO_PCM_t *pPCM = NULL;
    MS_S32 s32Ret = 0;
    DRVAUDIO_ERROR("%s: Flush PCM device-id %ld\n", __FUNCTION__, s32DeviceId);
    ...
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) doesn't register flush()!\n", __FUNCTION__, s32DeviceId);
        return -A_EPERM;
    }
    ...
    if (pPCM->pPcmInfo->u8StartFlag != FALSE)
    {
        DRVAUDIO_ERROR("%s: Error! s32DeviceId(%ld) is running, can not do flush now!\n", __FUNCTION__, s32DeviceId);
        return -A_EBUSY;
    }
