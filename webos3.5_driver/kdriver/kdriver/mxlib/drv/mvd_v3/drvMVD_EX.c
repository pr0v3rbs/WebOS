...
typedef struct
{
    ...
    MS_U32 u32Id[MVD_MAX_STREAMS];

#ifdef VDEC3
    MS_U32 u32FrameBaseMode[MVD_MAX_STREAMS];
#endif
#if defined(SUPPORT_CMA)
    MS_BOOL bCMAUsed;
    MS_BOOL bCMAAllocDone;
    struct CMA_Pool_Init_Param cmaInitParam; // support two MIU
    struct CMA_Pool_Free_Param cmaFreeParam[MVD_MAX_STREAMS];
    MS_BOOL bCMATwoMIU[MVD_MAX_STREAMS];
#endif
} MVD_Drv_CTX;
...
static MS_BOOL MVD_SaveStreamId(MS_U8 u8DrvIdx, MS_U32 u32MvdSid, MS_U32 u32VpuSid)
{
    ...
    {
        printf("%s err IsUsed: drv=%x mvd=%lx vpu=%lx\n", __FUNCTION__,
...
void MVD_RecordStreamId(MS_U32 u32Id)
{
    ...
    pMVDDrvContext->u32Id[u8DrvIdx] = u32Id;
}

MS_U32 MVD_GetStreamId(MS_U8 u8Idx)
{
    return pMVDDrvContext->u32Id[u8Idx];
}
...
static int _HAL_MVD_StreamTypeToInt(HAL_MVD_StreamType eType)
{
    int res = 0;
    switch(eType)
    {
        case E_HAL_MVD_MAIN_STREAM   : res = 0; break;
        case E_HAL_MVD_SUB_STREAM   : res = 1; break;
#ifdef VDEC3
        case E_HAL_MVD_N_STREAM      : res = 2; break;
#endif
        default:  break;
    }
    return res;
...
static int _MVD_StreamTypeToInt(MVD_DRV_StreamType eType)
{
    int res = 0;
    switch(eType)
    {
        case E_MVD_DRV_STREAM_NONE   : res = 0; break;
        case E_MVD_DRV_MAIN_STREAM   : res = 1; break;
        case E_MVD_DRV_SUB_STREAM    : res = 2; break;
#ifdef VDEC3
        case E_MVD_DRV_N_STREAM      : res = 3; break;
#else
        case E_MVD_DRV_STREAM_MAX    : res = 3; break;
#endif
        default:  break;
    }
    return res;
...
static void MVD_InitVar(MS_U8 u8DrvIdx,MS_U8 u8HalIdx)
{
    ...
    pMVDDrvContext->u32FrameBaseMode[u8DrvIdx] = FALSE;
    return;
}
...
E_MVD_Result MDrv_MVD_SetCfg(MS_U32 u32Id, MVD_FWCfg* fwCfg, MVD_MEMCfg* memCfg)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s u32Id=0x%lx, u8HalIdx=0x%x\n", __FUNCTION__, u32Id, u8HalIdx));
    //1.1 Determine framebuffer mode, frame number, used size.
    #if defined(SUPPORT_CMA)
    if(pMVDDrvContext->bCMAUsed == FALSE)
    #endif
    {
    ...
    _phy_to_miu_offset(u8MiuSel, u32StartOffset, memCfg->u32BSAddr);
    ...
    MVD_DEBUGVERBAL(MVD_PRINT("u32StartOffset = 0x%lx \n",u32StartOffset));
    ...
    {
        memCfg->u32FWSrcVAddr = HAL_MVD_PA2NonCacheSeg((MS_U32)memCfg->u32FWBinAddr);
    }
    else
    {
        memCfg->u32FWSrcVAddr = NULL;
    }
    if( 0 == u32StartOffset || 0 == u8MiuSel)
    {
    }
...
MS_BOOL MDrv_MVD_Init(MS_U32 u32Id,MVD_CodecType eCodecType)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_Init:start u32Id=0x%lx, sid=0x%x, halIdx=0x%x\n",
...
MS_BOOL MDrv_MVD_Exit(MS_U32 u32Id)
{
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s u8DrvIdx=0x%x u8HalIdx=0x%x u32Id=0x%lx\n",__FUNCTION__, u8DrvIdx, u8HalIdx, u32Id));
...
#if defined(SUPPORT_CMA)
    if (pMVDDrvContext->bCMAUsed == TRUE && pMVDDrvContext->bCMAAllocDone == TRUE)
    {
        if (MApi_CMA_Pool_PutMem(&pMVDDrvContext->cmaFreeParam[u8DrvIdx]) == FALSE)
        {
            return FALSE;
        }
        else
        {
            pMVDDrvContext->bCMAAllocDone = FALSE;
        }
    }
#endif
#ifdef MVD_ENABLE_ISR
...
_HW_TILE_DOWN:
    ...
    MVD_DEBUGINFO(MVD_PRINT("%s i=0x%x u32Id=0x%lx\n", __FUNCTION__, u8DrvIdx, u32Id));
...
{
	  MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    MVD_SrcMode eSrcMode = HAL_MVD_GetSrcMode(u8HalIdx);
    MVD_FUNC_ENTRY();
    ...
    extern const MS_U8 bMvdParserDisable[5];
    if(E_MVD_SRC_UNKNOWN > eSrcMode)
        MDrv_MVD_SetCodecInfo(u32Id, eCodecType, eSrcMode, bMvdParserDisable[eSrcMode]);
...
//------------------------------------------------------------------------------
void MDrv_MVD_SetFrameBuffAddr(MS_U32 u32Id, MS_U32 u32addr)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
...
MS_BOOL MDrv_MVD_IsMoreDecFrame(MS_U32 u32Id)
{
    MVD_FUNC_ENTRY();
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_IsMoreDecFrame(u8HalIdx);
}
...
//------------------------------------------------------------------------------
/// Get MVD dropped picture counter
/// @return -dropped picture counter
//------------------------------------------------------------------------------
MS_U32 MDrv_MVD_GetDropPicCounter(MS_U32 u32Id)
{
    MVD_FUNC_ENTRY();
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_GetDropPicCounter(u8HalIdx);
}
...
//------------------------------------------------------------------------------
MS_BOOL MDrv_MVD_DecodeIFrame(MS_U32 u32Id, MS_PHYADDR u32FrameBufAddr, MS_PHYADDR u32StreamBufAddr, MS_PHYADDR u32StreamBufEndAddr )
{
    MS_BOOL bRet = FALSE;
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
...
//------------------------------------------------------------------------------
/// Get the original scanType
/// @return -TRUE for progressive or FALSE for interlace
//------------------------------------------------------------------------------
MS_BOOL MDrv_MVD_IsProgressiveStream(MS_U32 u32Id)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_IsProgressiveStream(u8HalIdx);
}
...
MS_U64 MDrv_MVD_GetU64PTS(MS_U32 u32Id,MVD_PtsType eType)
{
    MVD_FUNC_ENTRY();
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_GetU64PTS(u8HalIdx,eType);
}
...
MS_U8 MDrv_MVD_GetDispRdy(MS_U32 u32Id)
{
    ...
    if(pMVDDrvContext->bCMAUsed == TRUE && pMVDDrvContext->bCMAAllocDone == FALSE)
    {
        MS_U32 u32Addr,u32Size;
        MS_BOOL bAllocateDone;
        struct CMA_Pool_Alloc_Param alloc_param;
        HAL_MVD_GetCMAInformation(u8HalIdx,&u32Addr,&u32Size,&bAllocateDone);
        ...
        {
            alloc_param.pool_handle_id = pMVDDrvContext->cmaInitParam.pool_handle_id;
            alloc_param.offset_in_pool = u32Addr - pMVDDrvContext->cmaInitParam.heap_miu_start_offset;
            ...
            pMVDDrvContext->cmaFreeParam[u8DrvIdx].pool_handle_id = alloc_param.pool_handle_id;
            pMVDDrvContext->cmaFreeParam[u8DrvIdx].offset_in_pool = alloc_param.offset_in_pool;
            pMVDDrvContext->cmaFreeParam[u8DrvIdx].length = alloc_param.length;

            //Run time set frame buffer miu client
            HAL_MVD_SetFrameBufferMiu(u8HalIdx,pMVDDrvContext->cmaInitParam.miu);

            HAL_MVD_SetCMAAllocateDone(u8HalIdx);

            pMVDDrvContext->bCMAAllocDone = TRUE;
        }
    }
#endif
...
MS_U32 MDrv_MVD_GetQueueVacancy(MS_U32 u32Id, MS_BOOL bCached)
{
    ...
    if(pMVDDrvContext->u32FrameBaseMode[u8DrvIdx] == TRUE)
    {
...
E_MVD_Result MDrv_MVD_SetSpeed(MS_U32 u32Id, MVD_SpeedType eSpeedType, MS_U8 u8Multiple)
{
    ...
    {   //save avsync config when normal play for restoring it later
        ...
        MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_SetSpeed save avsync: bEnable=0x%x u32Delay=0x%lx u16Tolerance=0x%x\n",
    ...
    if (HAL_MVD_SetSpeed(u8HalIdx, eSpeedType, u8Multiple) == TRUE)
    {
        if (E_MVD_SPEED_DEFAULT == eSpeedType) //Normal Play
        {
            if (pstCtrlCfg->ePreSpeedType != E_MVD_SPEED_DEFAULT)
            {
                ...
                MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_SetSpeed reset avsync: bEnable=0x%x u32Delay=0x%lx u16Tolerance=0x%x\n",
                              pstNFPSyncCfg->bEnable, pstNFPSyncCfg->u32Delay, pstNFPSyncCfg->u16Tolerance));
...
//------------------------------------------------------------------------------
E_MVD_Result MDrv_MVD_DbgSetData(MS_U32 u32Id, MS_U32 u32Addr, MS_U32 u32Data)
{
    return E_MVD_RET_FAIL;
}
...
//------------------------------------------------------------------------------
E_MVD_Result MDrv_MVD_DbgGetData(MS_U32 u32Id, MS_U32 u32Addr, MS_U32* u32Data)
{
    if (!u32Data)
    {
        return E_MVD_RET_INVALID_PARAM;
...
void MVD_IsrProc(void)
{
    ...
    for (u8DrvIdx=0; u8DrvIdx<MVD_MAX_STREAMS; u8DrvIdx++)
    {
        ...
        u32Id = MVD_GetStreamId(u8DrvIdx);
        ...
        if (u32IntStat != 0)
        {
            ...
            *pu32CurEvent = E_MVD_EVENT_DISABLE_ALL;
            ...
            if ((*pu32EventFlag & E_MVD_EVENT_PTS_DISCONTINUE) == E_MVD_EVENT_PTS_DISCONTINUE)
            {
                if (MVD_IntHasAbnormalPTS(u32IntStat))
                {
                    *pu32CurEvent |= E_MVD_EVENT_PTS_DISCONTINUE;
                    MVD_DEBUGINFO(MVD_PRINT("===> E_MVD_EVENT_PTS_DISCONTINUE!!!\n"));
                }
            }
            ...
            {
                MVD_DEBUGINFO(MVD_PRINT("%s Callback for *pu32CurEvent=0x%lx, cbData=0x%lx\n",
...
E_MVD_Result MDrv_MVD_SetIsrEvent(MS_U32 u32Id, MS_U32 eEvent, MVD_InterruptCb fnHandler)
{
    ...
    if (eEvent == E_MVD_EVENT_DISABLE_ALL)
    {
        ...
        MS_U32 i = 0;
        MS_BOOL bUsed = FALSE;
        for(i = 0; i < MVD_MAX_STREAMS; i++)
        {
            if((i != u8DrvIdx)
            && (TRUE == pMVDDrvContext->gMVDCtrl_EX[i].bUsed))
            {
                bUsed = TRUE;
                break;
            }
        ...
        if((pMVDDrvContext->_bDisableFlag != TRUE) && (FALSE == bUsed))
        {
            ...
            //!defined(SUPPORT_X_MODEL_FEATURE) && !defined(DISABLE_ISR_DETACH)
    ...
    else
    {
        if(fnHandler != NULL)
        {
            ...
            MVD_DEBUGINFO(MVD_PRINT("%s Hook callback u32Id=0x%lx u8HalIdx=0x%x eEvent=0x%lx\n",
                __FUNCTION__, u32Id, u8DrvIdx, eEvent));
            pMVDDrvContext->_eEventFlag[u8DrvIdx] = eEvent;

#if 0//!defined(SUPPORT_X_MODEL_FEATURE) && !defined(DISABLE_ISR_DETACH)
            ...
            if (FALSE == pMVDDrvContext->_bMVDMSOSIsrEnable)
            {
                if (OSAL_MVD_IntEnable() != TRUE)
                {
                    MVD_DEBUGERROR(MVD_ERR("fail to OSAL_MVD_IntEnable!\n"));
                ...
                pMVDDrvContext->_bDisableFlag = FALSE;
                pMVDDrvContext->_bMVDMSOSIsrEnable = TRUE;
            }
#endif
            MVD_DEBUGINFO(MVD_PRINT("MDrv_MVD_SetIsrEvent eEventFlag=0x%lx\n", pMVDDrvContext->_eEventFlag[u8DrvIdx]));
        ...
        {
            MVD_DEBUGERROR(MVD_ERR( "SetISREvent with NULL pointer. ISR type:%lu\n", (MS_U32)eEvent));
            return E_MVD_RET_INVALID_PARAM;
        }
    }
    return E_MVD_RET_OK;
...
//-----------------------------------------------------------------------------
MS_BOOL MDrv_MVD_GenPattern(MS_U32 u32Id, MVD_PatternType ePattern, MS_PHYADDR u32PAddr, MS_U32* pu32Size)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    if (!pu32Size)
    {
...
//-----------------------------------------------------------------------------
E_MVD_Result MDrv_MVD_SetDynScalingParam(MS_U32 u32Id, MS_PHYADDR u32StAddr, MS_U32 u32Size)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
...
/// @return -E_MVD_RET_OK for success; E_MVD_RET_FAIL for failure.
//------------------------------------------------------------------------------
MS_U32 MDrv_MVD_GetXcLowDelayIntState(MS_U32 u32Id)
{
    MVD_FUNC_ENTRY();
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
...
void MDrv_MVD_DbgDump(MS_U32 u32Id)
{
    ...
    MVD_PRINT("avsync on=%x, delay=%ld, tolerance=%d, done=%x, skip=%x, repeat=%x, pts=%ldms\n",
    ...
    MVD_PRINT("seq(%d): w=%d, h=%d, i/p=%x, fps=%ld\n", MDrv_MVD_GetDispRdy(u32Id),
...
//------------------------------------------------------------------------------
void MDrv_MVD_DbgDumpBits(MS_U32 u32Id, MS_PHYADDR u32base, MS_U32 u32size)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
...
#ifdef VDEC3
    MS_U8 u8DrvIdx = _MVD_GET_IDX(u32Id);
    pMVDDrvContext->u32FrameBaseMode[u8DrvIdx] = u8Mode;
#endif
...
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_ForceInterlaceMode(u8HalIdx, u8Mode);
}
...
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_ForceProgressiveMode(u8HalIdx, u8Mode);
}
...
E_MVD_Result MDrv_MVD_GetFreeStream(MS_U32 *pu32Id, MVD_DRV_StreamType eStreamType)
{
...
#ifdef VDEC3
    else if ((eStreamType >= E_MVD_DRV_N_STREAM) && (eStreamType < (E_MVD_DRV_N_STREAM+MVD_MAX_STREAMS)))
    {
        eVpuStreamType = E_HAL_VPU_N_STREAM + (eStreamType - E_MVD_DRV_N_STREAM);
        eHalStreamType = E_HAL_MVD_N_STREAM + (eStreamType - E_MVD_DRV_N_STREAM);
    }
#endif
    ...
    {
       printf("%s  %d  u32VPUStreamId:%lx\n",__FUNCTION__,__LINE__,u32VPUStreamId);
       return E_MVD_RET_FAIL;
    }
    ...
    MVD_DEBUGVERBAL(MVD_PRINT("%s:: i=0x%x, mvd=0x%lx, vpu=0x%lx\n", __FUNCTION__,
            i, u32MVDStreamId, u32VPUStreamId));
    printf("u32VPUStreamId:%lx  u32MVDStreamId:%lx\n",u32VPUStreamId,u32MVDStreamId);
    if (u32VPUStreamId && u32MVDStreamId)
    {
        MVD_DEBUGINFO(MVD_PRINT("%s   i=0x%x, mvd=0x%lx, vpu=0x%lx\n", __FUNCTION__,
            i, u32MVDStreamId, u32VPUStreamId));
#ifdef VDEC3
        *pu32Id = ((u32MVDStreamId & 0x0F) << 16 | u32MVDStreamId << 8 | u32VPUStreamId);
        if (FALSE == MVD_SaveStreamId(u32MVDStreamId&0x0F, u32MVDStreamId, u32VPUStreamId))
        {
            return E_MVD_RET_FAIL;
        }
#else
    ...
    {
#ifdef VDEC3
        MVD_DEBUGINFO(MVD_PRINT("%s(%d) NG  i=0x%lx, mvd=0x%lx, vpu=0x%lx\n", __FUNCTION__, __LINE__,
            u32MVDStreamId&0xf, u32MVDStreamId, u32VPUStreamId));
#else
        MVD_DEBUGINFO(MVD_PRINT("%s(%d) NG  i=0x%lx, mvd=0x%lx, vpu=0x%lx\n", __FUNCTION__, __LINE__,
            i, u32MVDStreamId, u32VPUStreamId));
#endif
...
//------------------------------------------------------------------------------
/// get Stereo Type
/// @return Stereo Type. 0000011(3) : 3d side-by-side   0001000(8) : 2d
//------------------------------------------------------------------------------
...
MS_U8 MDrv_MVD_GetStereoType(MS_U32 u32Id)
{
    MVD_FUNC_ENTRY();
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_GetStereoType(u8HalIdx);
}
...
#if (defined(CHIP_EDISON) || defined(CHIP_EINSTEIN) || defined(CHIP_NAPOLI) || defined(CHIP_MONACO)) || defined(CHIP_MUNICH) || defined(CHIP_MUSTANG)
    MVD_CtrlCfg* pstCtrlCfg = HAL_MVD_GetCtrlCfg(u8HalIdx);
    MS_BOOL bRet = FALSE;
    MS_BOOL bExternalInMIU1 = FALSE;
    MS_U32 u32DSAddr = pExternalBuf->u32DSBufAddr;
    MS_U32 u32VPUAddr = NULL;
...
void MDrv_MVD_REE_GetSHMInformation(MS_U32* u32SHMAddr,MS_U32* u32VsyncSHMOffset)
{
    *u32SHMAddr = HAL_VPU_EX_GetSHMAddr();
    *u32VsyncSHMOffset = HAL_MVD_GetVsyncAddrOffset();
}

E_MVD_Result MDrv_MVD_REE_GetVsyncExtShm(MS_U32* u32SHMAddr, MS_U32* u32VsyncExtShmOffset)
{
    *u32SHMAddr = HAL_VPU_EX_GetSHMAddr();
    *u32VsyncExtShmOffset = HAL_MVD_GetVsyncExtAddrOffset();
    if(0 == *u32VsyncExtShmOffset)
    {
        return E_MVD_RET_FAIL;
    }
...
//------------------------------------------------------------------------------
/// enable/disable ThumbNail Decode Mode
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------
void MDrv_MVD_EnableThumbNailMode(MS_U32 u32Id, MS_BOOL bEnable)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_EnableThumbNailMode(u8HalIdx,bEnable);
}

void MDrv_MVD_DisableDivx(MS_U32 u32Id, MS_BOOL bDisable)
{
    if(bDisable)
    {
    ...
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_DisableDivx(u8HalIdx,bDisable);
}
...
E_MVD_Result MDrv_MVD_PTS_Discontinue_Auto_Detect(MS_U32 u32Id, MS_BOOL bEnable)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_PTS_Discontinue_Auto_Detect(u8HalIdx,bEnable);
}
...
{
    return HAL_VPU_Set_MBX_param(u8APIMbxMsgClass);
}

void MDrv_MVD_SetDmxFrameRate(MS_U32 u32Id,MS_U32 u32Value)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    HAL_MVD_SetDmxFrameRate(u8HalIdx,u32Value);
}

void MDrv_MVD_SetDmxFrameRateBase(MS_U32 u32Id,MS_U32 u32Value)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    HAL_MVD_SetDmxFrameRateBase(u8HalIdx,u32Value);
}

void MDrv_MVD_SetCMAInformation(void* cmaInitParam)
{
#if defined(SUPPORT_CMA)
    pMVDDrvContext->bCMAUsed = TRUE;
    memcpy((void*)(&pMVDDrvContext->cmaInitParam),cmaInitParam,sizeof(struct CMA_Pool_Init_Param));
    HAL_MVD_SetCMAInformation(cmaInitParam);
#endif
}
...
E_MVD_Result MDrv_MVD_EnablePTSDetector(MS_U32 u32Id, MS_BOOL bEn)
{
    MVD_FUNC_ENTRY();
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_EnablePTSDetector(u8HalIdx, bEn);
}

MS_BOOL MDrv_MVD_IsSTRResume(MS_U32 u32Id)
{
    MVD_FUNC_ENTRY();
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_IsSTRResume(u8HalIdx);
}

MS_BOOL MDrv_MVD_IsSTR(MS_U32 u32Id)
{
    MVD_FUNC_ENTRY();
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_IsSTR(u8HalIdx);
}

MS_BOOL MDrv_MVD_SetAVSyncDispAutoDrop(MS_U32 u32Id,MS_BOOL bEnable)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_SetAVSyncDispAutoDrop(u8HalIdx,bEnable);
}

MS_BOOL MDrv_MVD_SetDynmcDispPath(MS_U32 u32Id,MS_BOOL bConnect,MVD_DISPLAY_PATH eValue,MS_BOOL bPreSet)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_SetDynmcDispPath(u8HalIdx,bConnect,eValue,bPreSet);
}

MS_BOOL MDrv_MVD_SetPUSISetting(MS_U32 u32Id,MS_BOOL bEnable)
{
    MS_U8 u8HalIdx = MVD_GetHalIdx(u32Id);
    return HAL_MVD_SetPUSISetting(u8HalIdx,bEnable);
}
