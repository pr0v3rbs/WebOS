...
{
    MS_U32 u32GOPSCRiuBaseAddr,u32NonPMBankSize;
    ...
    // get MMIO base
    if(MDrv_MMIO_GetBASE( &u32GOPSCRiuBaseAddr, &u32NonPMBankSize, MS_MODULE_GE ) != TRUE)
    {
        printf("GOP_SC GetBase failed\n");
        return En_GOPSC_FAIL;
    }
    MDrv_GOPSC_Init_riu_base(_GetDeviceIDFromInstance(pInstance),u32GOPSCRiuBaseAddr);
