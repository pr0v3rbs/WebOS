...
#ifdef _MFE_UTOPIA_
static MFE_S32 madp_sem_input = -1;
...
#else
static int mfe_irq_fd = -1;
#endif

//void mfe_wait_int(int isIsrMode)
void mfe_wait_int(int isIsrMode, int wait_time)
{
    MS_BOOL bRet = FALSE;
    if(!isIsrMode)
        return;
...
#else
    if(wait_time < 100) {
        ms_dprintf(ADP_L2, "Wait MFE ISR...MSOS_WAIT_FOREVER");
        bRet = MsOS_ObtainSemaphore(madp_sem_input, MSOS_WAIT_FOREVER);
    } else {
        ms_dprintf(ADP_L2, "Wait MFE ISR..(%d)",wait_time);
        bRet = MsOS_ObtainSemaphore(madp_sem_input, wait_time);
    }
...
void mfe_open_int(void)
{
    ...
    if (mfe_irq_fd < 0)
    {
        ...
        if (mfe_irq_fd < 0)
        {
            ms_dprintf(ADP_L2,"ERR: MFE IRQ Open Failed\n");
            return;
        }

        ms_dprintf(ADP_L2,"NOTE: MFE IRQ Open OK\n");
    }
#endif
}
...
void mfe_close_int(void)
{
    ...
    if (mfe_irq_fd >= 0)
    {
        ...
        ms_dprintf(ADP_L2,"NOTE: MFE IRQ Close OK\n");
    }
#endif
}
