...
void MApi_HDMITx_SetVideoOutputTiming(HDMITX_VIDEO_TIMING mode)
{
    switch (mode)
    {
        ...
        case HDMITX_RES_1920x2205p_24Hz:
            gHDMITxInfo.output_video_timing = E_HDMITX_RES_1920x2205p_24Hz;
            break;
        case HDMITX_RES_1280X1470p_50Hz:
            gHDMITxInfo.output_video_timing = E_HDMITX_RES_1280x1470p_50Hz;
            break;
        case HDMITX_RES_1280X1470p_60Hz:
            gHDMITxInfo.output_video_timing = E_HDMITX_RES_1280x1470p_60Hz;
            break;
        default:
    ...
    if((gHDMITxInfo.output_video_timing == E_HDMITX_RES_1920x2205p_24Hz) ||
        (gHDMITxInfo.output_video_timing == E_HDMITX_RES_1280x1470p_50Hz) ||
        (gHDMITxInfo.output_video_timing == E_HDMITX_RES_1280x1470p_60Hz))
    {
        if(gHDMITxInfo.output_colordepth_val >= E_HDMITX_VIDEO_CD_30Bits)
        {
            gHDMITxInfo.output_colordepth_val = E_HDMITX_VIDEO_CD_24Bits;
            MDrv_HDMITx_SetHDMITxMode();
        }
    }
...
HDMITX_INT_HDCP_STATUS MApi_HDMITx_GetHDCP_PreStatus(void)
{
    ...
    if(gHDMITxInfo.hdmitx_fsm_prestate == E_HDMITX_FSM_HDCP_AUTH_DONE)
        res = HDMITX_INT_HDCP_PASS;
    else if (gHDMITxInfo.hdmitx_fsm_prestate == E_HDMITX_FSM_HDCP_AUTH_FAIL)
        res = HDMITX_INT_HDCP_FAIL;
    else
        res = HDMITX_INT_HDCP_PROCESS;
