...
static int _DMX_DBG_Pvr(char* param1)
{
    ...
    //printf("pfPVR = 0x%x\n", (unsigned int)pfPVR);
    //printf("bWrongParam = 0x%x\n", (int)bWrongParam);
    //printf("u32NumPids = 0x%x\n", (unsigned int)u32NumPids);
    if((u32TimeStartPvr == 0) && (bWrongParam == FALSE))
    {
        printf("_DMX_DBG_Record_Start\n");
            ...
            else
            {
                ...
                else
                {
                    for(u32Addr = u32Addr1; u32Addr < u32Addr2; u32Addr++)
                    {
                        ...
                        printf("%02X ", *(volatile unsigned char*)u32Addr);
                    }
                }
                printf("\n");
            }
