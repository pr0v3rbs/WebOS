...
MS_U32 VDEC_Pid[MAX_STREAM_NUM];
MS_BOOL bVDECUsed[MAX_STREAM_NUM];
VDEC_StreamId VDEC_Streamid[MAX_STREAM_NUM];
...
MS_U32 VDEC_EX_V2_Open(void** ppInstance, const void* const pAttribute)
{
    ...
    psVDEC_EX_InstPri->fpVDEC_EX_KillProcess    = MApi_VDEC_EX_V2_KillProcess;
...
MS_U32 VDEC_EX_V2_IOctl(void* pInstance, MS_U32 u32Cmd, void* pArgs)
{
    ...
    if(pArgs == NULL)
    {
        VPRINTF("[Error]%s,pArgs is NULL",__FUNCTION__);
        return UTOPIA_STATUS_FAIL;
    }
    UtopiaInstanceGetPrivate(pInstance, (void*)&psVDEC_EX_InstPri);
    if(psVDEC_EX_InstPri == NULL)
    {
        VPRINTF("[Error]%s,pArgs is NULL",__FUNCTION__);
        return UTOPIA_STATUS_FAIL;
    }
    ...
    if(u32Cmd == CMD_USIGNAL_PROCESS_KILL)
    {
        int i = 0;
        for( i= 0; i<MAX_STREAM_NUM;i++)
        {
            if(VDEC_Pid[i] == *((MS_U32*)pArgs) && bVDECUsed[i] == TRUE)
            {
                VDEC_Pid[i] = 0;
                bVDECUsed[i] = FALSE;
                psVDEC_EX_InstPri->fpVDEC_EX_KillProcess(&VDEC_Streamid[i]);
            }
        }
        return UTOPIA_STATUS_SIGNAL_OK;
    }
    else
#endif
    {
        ...
        *pRet = E_VDEC_EX_FAIL;
        switch(IOcmd)
        {
...
#ifdef MSOS_TYPE_LINUX_KERNEL
                    if(*pRet == E_VDEC_EX_OK)
                    {
                        unsigned char u8Id = (pLocal_Args->StreamID->u32Id)>>24;
                        if(u8Id < MAX_STREAM_NUM)
                        {
                            VDEC_Pid[u8Id] = UtopiaInstanceGetPid(pInstance);
                            memcpy((void*)(&VDEC_Streamid[u8Id]),(void*)(pLocal_Args->StreamID),sizeof(VDEC_StreamId));
                            bVDECUsed[u8Id] = TRUE;
                        }
                    }
#endif
            ...
            case E_VDEC_EX_V2_CMD_SET_CONTROL:
                    if(pLocal_Args->eUserCmd == E_VDEC_EX_V2_USER_CMD_EXIT)
                    {
                        unsigned char u8Id = (pLocal_Args->StreamID->u32Id)>>24;
                        if(u8Id < MAX_STREAM_NUM)
                        {
                            printf("cmd:%d  u8Id:%d  exit ok\n",pLocal_Args->eUserCmd,u8Id);
                            VDEC_Pid[u8Id] = 0;
                            bVDECUsed[u8Id] = FALSE;
                        }
                    }
