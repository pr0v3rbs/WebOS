...
#include "drvMJPEG.h"
#include "apiJPEG.h"
#include "../jpeg_ex/apiJPEG_priv.h"
...
#define MJPEG_MAX_SUPPORT_WIDTH     1920
#define MJPEG_MAX_SUPPORT_HEIGHT     1080

#define MAX_SUPPORTED_FRAME_RATE_1080P   30000

#define DISPLAY_QUEUE_NUM           8
#define MJPEG_INTERNAL_SIZE                         0x20000
...
void _MApi_MJPEG_InitPara(MS_U32 u32Id)
{
    ...
    MDrv_MJPEG_SetFrameRate(u8DrvId, 600);
    MDrv_MJPEG_SetFrameRateBase(u8DrvId, 20);
...
static MJPEG_Result __MApi_MJPEG_CheckDecodeDone(MS_U32 u32Id)
{
    ...
    if (__bIsFirstFrameRdy[u8DrvId])
    {
        ...
        {
            MJPEG_DEBUG_API_MSG("   JPD Decode Error\n");
            //MApi_MJPEG_Debug();
        ...
        else if(E_JPEG_EVENT_DEC_DONE & reg_val)
        {
            ...
            if (__bIsDecodeDone[u8DrvId] == FALSE)
            {
                ...
                MJPEG_DEBUG("[MJPEG]Decode done...Frame Buffer Index %ld ready\n", u32NextFrameBuffIdx[u8DrvId]);
                __bIsDecodeDone[u8DrvId] = TRUE;
            }
#endif
            MJPEG_DEBUG_API_MSG("\n");
            //__u32ReadPtr = u32EndAddr;
            MJPEG_DEBUG_API_MSG("Count %ld\n", MDrv_MJPEG_GetFrameCnt(u8DrvId));
        }
        else
        if(E_JPEG_EVENT_DEC_ERROR_MASK & reg_val)
        {
...
MS_BOOL _MApi_MJPEG_DropFrameWhenFF(MS_U8 u32Id)
{
    if(MApi_JPEG_IsNJPD()==TRUE)
    {
        ...
        else if(_MApi_JPEG_GetWidth()<=MJPEG_HD_800x600_WIDTH && _MApi_JPEG_GetHeight()<=MJPEG_HD_800x600_HEIGHT
            && MDrv_MJPEG_GetFrameRate(u8DrvId) <= MAX_SUPPORTED_FRAME_RATE_1080P)
        {
            ...
            && MDrv_MJPEG_GetFrameRate(u8DrvId) <= (MAX_SUPPORTED_FRAME_RATE_1080P/2))
        {
            return FALSE;
        }
        return TRUE;
...
#endif
{
    ...
    if(bCMAUsed && u8DrvId == 0)
    {
        ...
        MJPEG_DEBUG("CMA used, reset address/size to [0x%lx, 0x%lx]\n", FWInfo.u32MWCAddr, FWInfo.u32MWCSize);
#ifdef VDEC3
        bFWdecideFB = TRUE;
#endif
    }
...
#endif
    {
        MJPEG_DEBUG("%s...FW decide FB!!! [%ldx%ld]\n", __FUNCTION__, pInitParam->u32Width, pInitParam->u32Height);
        ...
        else
        {
             if(pInitParam->u32Width*pInitParam->u32Height ==0)
            {
                pInitParam->u32Width = MJPEG_SD_WIDTH;
                pInitParam->u32Height = MJPEG_SD_HEIGHT;
            }
       }
        ...
        if(bCMAUsed && u8DrvId == 0)
        {
            ...
            MJPEG_DEBUG("MDrv_MJPEG_GetCMAInformation() u32Addr=0x%lx, u32Size=0x%lx, bAllocateDone=%d!!!\n", u32Addr, u32Size, bAllocateDone);
...
MJPEG_Result MApi_MJPEG_Stop(MS_U32 u32Id)
{
    ...
    if (bCMAUsed && u8DrvId == 0)
    {
        ...
        MJPEG_DEBUG("[MJPEG][0x%lx]MApi_CMA_Pool_PutMem: offset=0x%lx, len=0x%lx\n", u32Id, cmaFreeParam[u8DrvId].offset_in_pool, cmaFreeParam[u8DrvId].length);
    }
#endif
    ...
    for(u16DecodeCnt=0; u16DecodeCnt<=u16NullCnt[u8DrvId]; u16DecodeCnt++)
    {
        ...
        if(__u32ReadPtr[u8DrvId] > __u32WritePtr[u8DrvId])
        {
            MJPEG_DEBUG("\033[1;31m[MWC][%dx%d.yuyv]addr=0x%lx, size=0x%x, [r:0x%lx, w:0x%lx]\033[m\n",
                _MApi_JPEG_GetWidth(), _MApi_JPEG_GetHeight(), infoDecFrame[u8DrvId].u32LumaAddr,
                _MApi_JPEG_GetWidth()*_MApi_JPEG_GetHeight()*2,
        ...
        MJPEG_DEBUG_API_MSG("%s: Count %ld, __bSkipToPts=%d, __bSeekToPts=%d, __u32Skip2PtsBase=%ld, __u32TargetPts=%ld\n",
            __FUNCTION__, MDrv_MJPEG_GetFrameCnt(u8DrvId), __bSkipToPts[u8DrvId], __bSeekToPts[u8DrvId], __u32Skip2PtsBase[u8DrvId], __u32TargetPts[u8DrvId]);
        ...
        if (__bIsFirstFrameRdy[u8DrvId])
        {
            if (eCurSpeedType[u8DrvId] == E_MJPEG_SPEED_FAST)
            {
                ...
                {
                    MJPEG_DEBUG_API_MSG("       Skip frame %ld\n", (MDrv_MJPEG_GetFrameCnt(u8DrvId) - __u32SpeedCtrlFrameCntBase[u8DrvId]));
                    u16NullCnt[u8DrvId]=0;
        ...
        else
        {
            ...
            for(u8Cnt=0; u8Cnt<(pInfo->u32StAddr + __u32MRCBufAddr[u8DrvId]-jpegInitParam.u32MRCBufAddr); u8Cnt++)
            {
                if((*((MS_U8*)MS_PA2KSEG1(jpegInitParam.u32MRCBufAddr+u8Cnt))==0xff && *((MS_U8*)MS_PA2KSEG1(jpegInitParam.u32MRCBufAddr+u8Cnt+1))==0xd9) ||
                    (*((MS_U8*)MS_PA2KSEG1(jpegInitParam.u32MRCBufAddr+u8Cnt))==0xff && *((MS_U8*)MS_PA2KSEG1(jpegInitParam.u32MRCBufAddr+u8Cnt+1))==0xd8))
                {
...
//      memset((void *)MS_PA2KSEG1(jpegInitParam.u32MRCBufAddr), 0, (pInfo->u32StAddr + __u32MRCBufAddr[u8DrvId]-jpegInitParam.u32MRCBufAddr));
        ...
        u32EndAddr[u8DrvId] = pInfo->u32StAddr + pInfo->u32Size - 1;
#if 1
//        MJPEG_DEBUG("[MWC]: PA=0x%lx, size=0x%lx, VA=0x%lx\n", jpegInitParam.u32MWCBufAddr, jpegInitParam.u32MWCBufSize, MS_PA2KSEG1(jpegInitParam.u32MWCBufAddr));
        MJPEG_DEBUG("[MRC]: PA=0x%lx, size=0x%lx, VA=0x%lx\n", jpegInitParam.u32MRCBufAddr, jpegInitParam.u32MRCBufSize, MS_PA2KSEG1(jpegInitParam.u32MRCBufAddr));
        MJPEG_DEBUG("[INTER]: PA=0x%lx, size=0x%lx, VA=0x%lx\n", jpegInitParam.u32InternalBufAddr, jpegInitParam.u32InternalBufSize, MS_PA2KSEG1(jpegInitParam.u32InternalBufAddr));
#endif
#if 0
...
MJPEG_Result MApi_MJPEG_FlushQueue(MS_U32 u32Id)
{
    ...
    u32Ret = MDrv_MJPEG_SendVPUCommand(u32Id, E_MJPEG_CMD_FLUSH_DISP_QUEUE, 0xFFFFFFFF);
    if (u32Ret == 0)
    {
...
//    ret = __MApi_MJPEG_CheckDecodeDone(u32Id);
    ...
    if(E_MJPEG_RET_SUCCESS != __MApi_MJPEG_CheckDecodeDone(u32Id)
        && E_MJPEG_RET_DECODE_ERROR != __MApi_MJPEG_CheckDecodeDone(u32Id))
    {
    ...
    if (MDrv_MJPEG_SendVPUCommand(u32Id, E_MJPEG_CMD_IS_DISP_FINISH, 0xFFFFFFFF) == 0
        && E_MJPEG_RET_SUCCESS == __MApi_MJPEG_CheckDecodeDone(u32Id))
    {
...
MJPEG_Result MApi_MJPEG_GetNextDispFrame(MS_U32 u32Id, MJPEG_FrameInfo *pFrmInfo)
{
    ...
    if(E_MJPEG_RET_DECODE_ERROR == __MApi_MJPEG_CheckDecodeDone(u32Id))
    {
        MJPEG_DEBUG_API_MSG("%s decode error\n",__FUNCTION__);
    }
...
MJPEG_Result MApi_MJPEG_SetDynmcDispPath(MS_U32 u32Id, MS_BOOL bConnect, HVD_EX_DISPLAY_PATH eDispPath)
{
    MJPEG_Result eRet = E_MJPEG_RET_INVALID_PARAMETER;
//    MS_U8 u8DrvId = MDrv_MJPEG_GetDrvId(u32Id);
    MS_U32 u32Arg = 0;
    //MS_U32 u32Times = 0;
    MDrv_MJPEG_SetConnectStatus(u32Id, E_HVD_EX_DISP_PATH_DYNMC_HANDLING);
    u32Arg = (((MS_U32)(eDispPath)) << 1) & (~0x1);
    u32Arg |= (bConnect & 0x1);
    MDrv_MJPEG_SendVPUCommand(u32Id, E_MJPEG_CMD_DYNAMIC_CONNECT_DISP_PATH, u32Arg);
    MS_U32 u32TimeOut = MsOS_GetSystemTime() + 500; // it can not block
    while (1)
    {
        if(bConnect)
        {
            if (MDrv_MJPEG_GetConnectStatus(u32Id) == E_HVD_EX_DISP_PATH_DYNMC_CONNECTTED)
            {
        ...
        {
            if (MDrv_MJPEG_GetConnectStatus(u32Id) == E_HVD_EX_DISP_PATH_DYNMC_DISCONNECT)
            {
                eRet = E_MJPEG_RET_SUCCESS;
                break;
            }
        ...
        if (MsOS_GetSystemTime() > u32TimeOut)
        {
            ...
            MJPEG_DEBUG_API_ERR("Set dynamic display path timeout!\n");
            break;
        }
    }
    return eRet;
...
void MApi_MJPEG_SetCMAInformation(void* pcmaInitParam)
{
    ...
    MJPEG_DEBUG("[MJPEG!!] set bCMAUsed=TRUE, cmaInitParam.heap_miu_start_offset=0x%lx, cmaInitParam.heap_length=0x%lx\n", cmaInitParam.heap_miu_start_offset, cmaInitParam.heap_length);
}
#endif
