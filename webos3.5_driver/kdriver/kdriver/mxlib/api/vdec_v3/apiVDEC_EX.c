...
#define MApi_VDEC_EX_CHECK_NULL_PTR(vbbu_addr) (((void *)(vbbu_addr)) == NULL)
...
#if defined(MSOS_TYPE_LINUX) || defined(MSOS_TYPE_ECOS) || defined(VDEC_UTOPIA_2K)
...
#define VDEC_MVD_SUPPORT_CC  //Enable cc with display order
#define VDEC_NOT_SUPPORT_DIVX 1//added by math for not supporting divx without checking hashkey
#ifndef VDEC_NOT_SUPPORT_DIVX
...
typedef struct
{
    //******** Handle for Dynamic MVOP to check Seamless is enable or not *****//
    MS_BOOL             bDSEnable;
    //******** Frame Buffer Handling********//
    MS_BOOL  bOnePendingBuffer;
    MS_BOOL  bEnableThumbnailMode;
    //******** HW Buffer ReMapping ********//
    ...
    MS_BOOL bEnableForceStartNonISlice;
...
#endif
#endif
    VDEC_EX_MVOPInfo stMVOPInfo[VDEC_MAX_SUPPORT_STREAM_NUM];
} VDEC_EX_CTX;
...
VDEC_EX_DEC_MODE eVdecDecMode[VDEC_MAX_SUPPORT_STREAM_NUM] = {E_VDEC_EX_DEC_MODE_DUAL_INDIE,E_VDEC_EX_DEC_MODE_DUAL_INDIE};
VDEC_EX_DispFrame g_stDispFrame[VDEC_MAX_SUPPORT_STREAM_NUM];
#if AUTO_ASSIGN_FRAMEBUFFER
static MS_U32 u32FrmBufAutoAddr = 0;
static MS_U32 u32FrmBufAutoSize = 0;
#define MM_VP9_MFCODEC_SIZE 0x3100000
#define DTV_BUFFER_SIZE_FHD 0x2B00000 // H.264/MPEG2/AVS/HEVC:43 mb
#define DTV_BUFFER_SIZE_UHD 0xA000000 // HEVC Case
#define MM_BUFFER_SIZE_FHD  0x2B00000 // VP9 Case test for 44 mb
#define MM_BUFFER_SIZE_UHD  0xA000000 // VP9 Case
#define THUMB_BUFFER_SIZE_UHD  0x2000000
#define THUMB_BUFFER_SIZE_FHD  0x600000
#endif
...
MS_BOOL  bIsSupportDivxPlus = FALSE;
...
#define FW_LOG_FILENAME       "/data/fw_log.txt"
...
void DbgDump()
{
    unsigned char u8Cnt;
    DbgArea* darea = 0;
    MS_U32 DBG_DUMP_ADDR = MsOS_PA2KSEG1(u32FWCodeAddr + 0xF9300);
    u32DbgIdx = *((volatile unsigned int*)MsOS_PA2KSEG1(u32FWCodeAddr + 0xDFFF0));
    ...
    while (u32RdIdx != u32DbgIdx)
    {
        if (darea == 0)
        {
            ...
            if(g_pFile != NULL)
            {
                fprintf(g_pFile, "YY %d:", darea->u16Id);
            }
            u8Cnt = 0;
        }
        else
        if (u8Cnt < (unsigned char)darea->u4Cnt)
        {
            if (darea->bHex & (1<<u8Cnt))
            {
                if(g_pFile != NULL)
                {
                  fprintf(g_pFile, " %x", *((unsigned int*)(u32RdIdx + DBG_DUMP_ADDR)));
                }
            }
            else
            {
                if(g_pFile != NULL)
                {
                  fprintf(g_pFile, " %d", *((unsigned int*)(u32RdIdx + DBG_DUMP_ADDR)));
                }
            }
            u8Cnt++;
        }
        ...
        u32RdIdx += 4;
        if (u32RdIdx >= 0x06500) u32RdIdx = 0;
    }
...
static VDEC_EX_Result _VDEC_EX_CheckStatus(MS_U8 u8Idx, MS_U32 u32DrvId, MS_U32 u32CallerId)
{
    ...
    {
        VDEC_INFO(u8Idx, VDEC_PRINT("%s %ld err: invalid streamID u8Idx(0x%x), u32DrvId(0x%lx)!=0x%lx\n", __FUNCTION__, u32CallerId, u8Idx, u32DrvId, pVdecExContext->_Attr[u8Idx].u32DrvId));
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
VDEC_EX_Result MApi_VDEC_EX_DbgCmd(VDEC_StreamId *pStreamId, MS_U32 u8Cmd, MS_U32 u32CmdArg)
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    MVD_CmdArg stCmdArg;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId)) {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pVdecExContext)) {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return;
    }
    MS_U8  u8Idx = _VDEC_GET_IDX(pStreamId->u32Id);
...
extern void HAL_VPU_EX_InitRegBase(MS_U32 u32RegBase) __attribute__((weak));
...
static MS_U32 _VDEC_Map2HVDErrCode(MS_U32 u32ErrCode)
{
    ...
    switch (u32ErrCode)
    {
        ...
        case E_HVD_EX_ERRCODE_RM_SIZE_OUT_FB_LAYOUT:
            ...
            break;
        case E_HVD_EX_ERRCODE_DIVX_PLUS_UNSUPPORTED:
            //report not support spec to avoid continue play
            printf("HVD not support divx plus\n");
            u32Ret = E_VDEC_EX_ERR_CODE_NOT_SUPPORT;
            break;
        default:
            break;
    }
...
static VDEC_EX_FrameType _VDEC_MapFrameType_HVD(HVD_EX_FrmType eHvdType)
{
    VDEC_EX_FrameType eType = E_VDEC_EX_FRM_TYPE_OTHER;
    switch(eHvdType)
    {
        case E_HVD_EX_FRM_TYPE_I                : eType = E_VDEC_EX_FRM_TYPE_I ;break;
        case E_HVD_EX_FRM_TYPE_P                : eType = E_VDEC_EX_FRM_TYPE_P ;break;
        case E_HVD_EX_FRM_TYPE_B                : eType = E_VDEC_EX_FRM_TYPE_B ;break;
        case E_HVD_EX_FRM_TYPE_OTHER            : eType = E_VDEC_EX_FRM_TYPE_OTHER ;break;
        default                                 : break;
    }
    return eType;
}
...
static VDEC_EX_FieldType _VDEC_MapFieldType_HVD(HVD_EX_FieldType eHvdType)
{
    VDEC_EX_FieldType eType = E_VDEC_EX_FIELDTYPE_NONE;
    switch(eHvdType)
    {
        case E_HVD_EX_FIELDTYPE_NONE            : eType = E_VDEC_EX_FIELDTYPE_NONE ;break;
        case E_HVD_EX_FIELDTYPE_TOP             : eType = E_VDEC_EX_FIELDTYPE_TOP ;break;
        case E_HVD_EX_FIELDTYPE_BOTTOM          : eType = E_VDEC_EX_FIELDTYPE_BOTTOM ;break;
        case E_HVD_EX_FIELDTYPE_BOTH            : eType = E_VDEC_EX_FIELDTYPE_BOTH ;break;
        default                                 : break;
    }
    return eType;
}

static VDEC_EX_Result _VDEC_MapRet_MVD(E_MVD_Result eMvd_Ret)
{
    VDEC_EX_Result eRes = E_VDEC_EX_FAIL;
    switch(eMvd_Ret)
    {
        case E_MVD_RET_FAIL                     : eRes = E_VDEC_EX_FAIL ;break;
        case E_MVD_RET_OK                       : eRes = E_VDEC_EX_OK ;break;
        case E_MVD_RET_INVALID_PARAM            : eRes = E_VDEC_EX_RET_INVALID_PARAM ;break;
        case E_MVD_RET_QUEUE_FULL               : eRes = E_VDEC_EX_FAIL ;break;
        case E_MVD_RET_TIME_OUT                 : eRes = E_VDEC_EX_RET_TIMEOUT ;break;
        default                                 : break;
    }
    return eRes;
}

static VDEC_EX_Result _VDEC_MapRet_HVD(HVD_EX_Result eHvd_Ret)
{
    VDEC_EX_Result eRes = E_VDEC_EX_FAIL;
    switch(eHvd_Ret)
    {
        case E_HVD_EX_FAIL                  : eRes = E_VDEC_EX_FAIL ;break;
        case E_HVD_EX_OK                    : eRes = E_VDEC_EX_OK ;break;
        case E_HVD_EX_RET_INVALID_PARAMETER : eRes = E_VDEC_EX_RET_INVALID_PARAM ;break;
        case E_HVD_EX_RET_ILLEGAL_ACCESS    : eRes = E_VDEC_EX_RET_ILLEGAL_ACCESS ;break;
        case E_HVD_EX_RET_HARDWARE_BREAKDOWN: eRes = E_VDEC_EX_RET_HARDWARE_BREAKDOWN ;break;
        case E_HVD_EX_RET_OUTOF_MEMORY      : eRes = E_VDEC_EX_FAIL ;break;
        case E_HVD_EX_RET_UNSUPPORTED       : eRes = E_VDEC_EX_RET_UNSUPPORTED ;break;
        case E_HVD_EX_RET_TIMEOUT           : eRes = E_VDEC_EX_RET_TIMEOUT ;break;
        case E_HVD_EX_RET_NOTREADY          : eRes = E_VDEC_EX_RET_NOT_READY ;break;
        case E_HVD_EX_RET_MEMORY_OVERWIRTE  : eRes = E_VDEC_EX_FAIL ;break;
        case E_HVD_EX_RET_QUEUE_FULL        : eRes = E_VDEC_EX_FAIL ;break;
        case E_HVD_EX_RET_RE_INIT           : eRes = E_VDEC_EX_FAIL ;break;
        case E_HVD_EX_RET_NOT_RUNNING       : eRes = E_VDEC_EX_RET_NOT_RUNNING ;break;
        case E_HVD_EX_RET_CMA_ERROR         : eRes = E_VDEC_EX_FAIL ;break;
        default                             : break;
    }
    return eRes;
}
...
static VDEC_EX_V2_User_Cmd _VDEC_MapUserCmd_V2(VDEC_EX_User_Cmd eUserCmd)
{
    ...
    switch (eUserCmd)
    {
        ...
        case E_VDEC_EX_USER_CMD_PTS_DISCONTINUE_AUTO_DETECT                : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_PTS_DISCONTINUE_AUTO_DETECT;break;
        ...
        case E_VDEC_EX_USER_CMD_SET_GST_TIMING                : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_SET_GST_TIMING;break;
        //case E_VDEC_EX_USER_CMD_DISABLE_DIVX                : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_DISABLE_DIVX;break;
        case E_VDEC_EX_USER_CMD_SET_PUSI_SETTING                : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_SET_PUSI_SETTING;break;
        ...
        case E_VDEC_EX_USER_CMD_GET_EVENT_FLAG                : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_GET_EVENT_FLAG;break;
        ...
        case E_VDEC_EX_USER_CMD_GET_IS_STR_RESUME                : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_GET_IS_STR_RESUME;break;
        case E_VDEC_EX_USER_CMD_GET_IS_STR                : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_GET_IS_STR;break;
        case E_VDEC_EX_USER_CMD_GET_IS_SEAMLESS_MODE                : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_GET_IS_SEAMLESS_MODE;break;
        case E_VDEC_EX_USER_CMD_GET_GST_TIMING                : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_GET_GST_TIMING;break;
        case E_VDEC_EX_USER_CMD_IS_MORE_FRAME             : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_IS_MORE_FRAME;break;
        case E_VDEC_EX_USER_CMD_GET_SIMPLE_FRAME              : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_GET_SIMPLE_FRAME;break;
        ...
        case E_VDEC_EX_USER_CMD_GET_SECURE_MODE                : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_GET_SECURE_MODE;break;
        ...
        case E_VDEC_EX_USER_CMD_FORCE_START_NON_I_SLICE                : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_FORCE_START_NON_I_SLICE;break;
        ...
        case E_VDEC_EX_USER_CMD_ENABLE_SEAMLESS_MODE                : eEXUserCmd = E_VDEC_EX_V2_USER_CMD_ENABLE_SEAMLESS_MODE;break;
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(param)) {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
    MS_U8  u8Idx = _VDEC_GET_IDX(pStreamId->u32Id);
...
static VDEC_EX_Result _VDEC_EX_SetMinTspDataSize(VDEC_StreamId *pStreamId, MS_U32 u32Size)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        ...
        case E_VDEC_EX_DECODER_HVD:
            ...
            if((pVdecExContext->_Attr[u8Idx].eCodecType != E_VDEC_EX_CODEC_TYPE_H264) && (pVdecExContext->_Attr[u8Idx].eCodecType != E_VDEC_EX_CODEC_TYPE_AVS))
            {
                return E_VDEC_EX_RET_UNSUPPORTED;
            }
...
static VDEC_EX_Result _VDEC_EX_ReturnInvalidAFD(VDEC_StreamId *pStreamId, MS_U32 u32mode)
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId))
    {
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
            ...
            break;
        case E_VDEC_EX_DECODER_HVD:
            _HVD_RET_HANDLE(MDrv_HVD_EX_ReturnInvalidAFD(u32Id, u32mode));
            eRet = E_VDEC_EX_OK;
            break;
        case E_VDEC_EX_DECODER_MJPEG:
            eRet = E_VDEC_EX_RET_UNSUPPORTED;
...
static VDEC_EX_Result _VDEC_EX_SetDecodeMode(VDEC_StreamId *pStreamId, VDEC_EX_DecModCfg *pDecModCfg)
{
    ...
    MS_U8 i = 0;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pDecModCfg))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
    ...
    if(u8Idx < VDEC_MAX_SUPPORT_STREAM_NUM)
    {
        eVdecDecMode[u8Idx] = pDecModCfg->eDecMod;
    }
    ...
    switch (pDecModCfg->eDecMod)
    {
        ...
        case E_VDEC_EX_DEC_MODE_DUAL_INDIE:
            ...
            break;
        case E_VDEC_EX_DEC_MODE_AUTO_MCU:
            stHvdDecModCfg.u8DecMod   = (MS_U8)pDecModCfg->eDecMod;
            stHvdDecModCfg.u8CodecCnt = pDecModCfg->u8CodecCnt;
            for (i=0; ((i<pDecModCfg->u8CodecCnt)&&(i<VDEC_MAX_DEC_NUM)); i++)
            {
...
static VDEC_EX_Decoder _VDEC_EX_GetDecoderByCodecType(VDEC_EX_CodecType eCodecType)
{
    ...
    switch (eCodecType)
    {
        ...
        case E_VDEC_EX_CODEC_TYPE_VC1_MAIN:
            ...
            #ifdef VDEC_NOT_SUPPORT_DIVX
            //remove this part on 2015/12/15 for disable parsing BitStream
            //bIsSupportDivxPlus = FALSE;
            #else
            MS_BOOL bNoUse = FALSE;
            //need to check the logic, not support divx 311 means not support whole divx
            bIsSupportDivxPlus = MDrv_MVD_AUTH_IPCheck(E_MVD_CODEC_DIVX311,&bNoUse);
            #endif
            break;
            ...
            case E_VDEC_EX_DECODER_MVD:
            {
                ...
                if (u32MVDIsrEvent != E_MVD_EVENT_DISABLE_ALL)
                {
                    ...
                    if (u32MVDIsrEvent & E_MVD_EVENT_USER_DATA)
                    {
                        ...
                        //exclude this for CC for this is UserData in "decoding" order
                    ...
                    if (u32MVDIsrEvent & E_MVD_EVENT_PTS_DISCONTINUE)
                    {
                        eEventFlag |= E_VDEC_EX_EVENT_PTS_DISCONTINUE;
                    }
                }
                break;
            }
...
static MS_U32 _VDEC_EX_GetESBuffVacancy(VDEC_StreamId *pStreamId)
{
    ...
    MS_U32 u32ESRdPtr = 0;
    MS_U32 u32ESWtPtr = 0;
    VDEC_EX_SysCfg *pCfg = &(pVdecExContext->_Attr[u8Idx].vdecExInitParam.SysConfig);
    ...
    if (u32ESRdPtr > u32ESWtPtr)
    {
        return (u32ESRdPtr - u32ESWtPtr);
    }
    else
    {
        return (pCfg->u32BitstreamBufSize-pCfg->u32DrvProcBufSize-u32ESWtPtr+u32ESRdPtr);
    }
}
...
{
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pPacketInfo)) {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
static VDEC_EX_Result _VDEC_EX_Field_Polarity_Display_One_Field(VDEC_StreamId *pStreamId, VDEC_EX_Field_Polarity* pFieldPority)
{
    ...
    if(MApi_VDEC_EX_CHECK_NULL_PTR(pFieldPority))
    {
        return E_VDEC_EX_FAIL;
    }
...
static VDEC_EX_Result _MApi_VDEC_EX_MVC_SetControl(VDEC_StreamId *pStreamId, VDEC_MVC_Control_type eType, void** param)
{
    ...
    switch(eType)
    {
        case E_VDEC_EX_MVC_SET_BBU2_PUSH_PACKET:
        {
            ...
            pInput = (VDEC_EX_DecCmd *)(*((MS_U32*)(param[0])));
...
static VDEC_EX_Result _VDEC_EX_GetIsOriInterlaceMode(VDEC_StreamId *pStreamId, MS_U32 *param)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
        {
            *param = MDrv_MVD_IsProgressiveStream(u32Id) == TRUE?FALSE:TRUE;
            eRet = E_VDEC_EX_OK;
            break;
        }
        case E_VDEC_EX_DECODER_HVD:
...
static VDEC_EX_Result _VDEC_EX_GetVsyncBridgeAddr(VDEC_StreamId *pStreamId, MS_U32 *param)
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(param))
    {
...
static VDEC_EX_Result _VDEC_EX_GetVsyncBridgeExtAddr(VDEC_StreamId *pStreamId, MS_U32 *param)
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    MS_U8  u8Idx = _VDEC_GET_IDX(pStreamId->u32Id);
    MS_U32 u32SHMaddr = 0;
    MS_U32 u32VsyncExtSHMOffset = 0;
...
static VDEC_EX_Result _VDEC_Init_Share_Mem(VDEC_EX_Stream eStreamType)
{
                                          ...
                                          MSOS_SHM_QUERY))
    {
        ...
        else
        {
            ...
            pVdecExContext->bCMAUsed = FALSE; // enable in the future
#endif

            if (pVdecExContext->bCMAUsed)
            {
                pVdecExContext->cmaInitParam[0].heap_id = ION_VDEC_HEAP_ID; // 19
                pVdecExContext->cmaInitParam[0].flags = CMA_FLAG_MAP_VMA;
                if (MApi_CMA_Pool_Init(&pVdecExContext->cmaInitParam[0]) == FALSE)
                {
                    return E_VDEC_EX_FAIL;
                }

                VPRINTF("MApi_CMA_Pool_Init: pool_handle_id=0x%lx, miu=%ld, offset=0x%lx, length=0x%lx\n",
                                        pVdecExContext->cmaInitParam[0].pool_handle_id,
                                        pVdecExContext->cmaInitParam[0].miu,
                                        pVdecExContext->cmaInitParam[0].heap_miu_start_offset,
                                        pVdecExContext->cmaInitParam[0].heap_length);
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pCrcValue))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
#define SIZE_6K 0x1800
    ...
    if(pExternalDSBuf->u32DSBufSize < SIZE_8K)
    {
        return E_VDEC_EX_RET_UNSUPPORTED;
    }
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
        {
            ...
            stMVDExternalDSBuf.u32DSBufSize = SIZE_6K; //pExternalDSBuf->u32DSBufSize; /// MVD only use 3K for IP/OP buffer.
        ...
        case E_VDEC_EX_DECODER_HVD:
        {
            ...
            stExternalDSBuf.u32DSBufSize = SIZE_6K;// pExternalDSBuf->u32DSBufSize;
...
static VDEC_EX_Result _VDEC_EX_SetDbgLevel(VDEC_StreamId *pStreamId, VDEC_EX_DbgLevel eDbgLevel)
{
    ...
    MS_U8 u8Idx = _VDEC_GET_IDX(pStreamId->u32Id);
    //glen: comment out; to solve uart-switching problem
    //eDbgLevel = E_VDEC_EX_DBG_LEVEL_FW;
...
void _VDEC_Callback_Proc(MS_U32 u32streamIdx)
{
    ...
    while(gVDEC_EX_2k_CbData[u8Idx].bRunning)
    {
        ...
        {
            printf("EventMap:0x%lx\n",gVDEC_EX_2k_CbData[u8Idx].u32EventMap[0]);
        }
        #if (EVENT_PATCH == 0)
        if(gVDEC_EX_2k_CbData[u8Idx].bVdecInit &&
          (gVDEC_EX_2k_CbData[u8Idx].u32EventMap[0]||gVDEC_EX_2k_CbData[u8Idx].u32EventMap[1]))
        #endif
        {
            u32VdecEventFlag = 0;
            ...
            for(cb_type=0;cb_type<E_VDEC_EX_CB_MAX_NUM;cb_type++)
            {
                ...
                if(gVDEC_EX_2k_CbData[u8Idx].u8Dbglevl == 2)
                {
                    printf("Find event:0x%lx\n",u32Event);
                }
                if (gVDEC_EX_2k_CbData[u8Idx].pfn[cb_type] && u32Event && gVDEC_EX_2k_CbData[u8Idx].bVdecInit)
                {
...
MS_BOOL _VDEC_EX_Create_Callback_task(VDEC_StreamId *pStreamId, VDEC_EX_CB_TYPE cb_type, VDEC_EX_EventCb pfn, void *param, MS_U32 u32EventFlag)
{
    ...
    gVDEC_EX_2k_CbData[u8Idx].bVdecInit = TRUE;
    //char task_name[100] = {0,};
    //snprintf(task_name,sizeof(task_name),"VDEC_Callback_Task_%d",u8Idx);
...
void _VDEC_EX_Delete_Callback_Task(VDEC_StreamId *pStreamId)
{
    ...
    if(u8Idx < VDEC_MAX_SUPPORT_STREAM_NUM)
    {
        ...
        gVDEC_EX_2k_CbData[u8Idx].bVdecInit = FALSE;
    }
    #if 0
    if(_s32VDEC_Callback_TaskId[u8Idx]!=-1)
    {
...
VDEC_EX_Result _MApi_VDEC_EX_V2_Exit(VDEC_StreamId *pStreamId)
{
    ...
    _VDEC_Memset((void*) &pVdecExContext->stMVOPInfo[u8Idx], 0, sizeof(VDEC_EX_MVOPInfo));
    ...
    pVdecExContext->_Pre_Ctrl[u8Idx].bEnableThumbnailMode = FALSE;
    pVdecExContext->_Pre_Ctrl[u8Idx].bDSEnable = FALSE;
    ...
    //_VDEC_EX_API_MutexUnlock();
    if(eVdecDecMode[u8Idx] == E_VDEC_EX_DEC_MODE_AUTO_MCU)
    {
        eVdecDecMode[u8Idx] = E_VDEC_EX_DEC_MODE_DUAL_INDIE;
    }
#ifdef MSOS_TYPE_LINUX_KERNEL
    printk("%s  exit Done  eRet:%d\n",__FUNCTION__,eRet);
#endif
    _VDEC_EX_API_MutexUnlock();
    #ifdef FW_LOG_DUMPTOFILE
...
VDEC_EX_Result _MApi_VDEC_EX_V2_SetFrcMode(VDEC_StreamId *pStreamId, VDEC_EX_FrcMode eFrcMode)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
        {
            ...
            {
                if(MDrv_MVD_DispCtrl(u32Id, FALSE, pVdecExContext->bDropErrFrm[u8Idx], FALSE, eMvdFrcMode))
                    eRet = E_VDEC_EX_OK;
                else
                    eRet = E_VDEC_EX_FAIL;
            }
        ...
        case E_VDEC_EX_DECODER_HVD:
        {
            ...
            if(E_VDEC_EX_RET_INVALID_PARAM != eRet)
            {
                eRet = _VDEC_MapRet_HVD(MDrv_HVD_EX_SetFrcMode(u32Id, eHvdFrcMode));
            }
            break;
        }
        case E_VDEC_EX_DECODER_MJPEG:
...
VDEC_EX_Result _MApi_VDEC_EX_V2_SetDynScalingParams(VDEC_StreamId *pStreamId, MS_PHYADDR u32Addr, MS_U32 u32Size)
{
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pCmd))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
VDEC_EX_Result _MApi_VDEC_EX_V2_SetFrmBuffAddr(VDEC_StreamId *pStreamId, MS_U32 u32FrmBuffAddr)
{
#ifdef VDEC3_FB
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId))
...
VDEC_EX_Result _MApi_VDEC_EX_V2_SetEvent_MultiCallback(VDEC_StreamId *pStreamId, VDEC_EX_CB_TYPE cb_type, MS_U32 u32EventFlag, VDEC_EX_EventCb pfn, void *param)
{
    ...
    VDEC_INFO(u8Idx, VDEC_PRINT("%s: flag=0x%lx\n", __FUNCTION__, u32EventFlag));
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
        {
            ...
            {
#if defined(VDEC_MVD_SUPPORT_CC)
            ...
            if (pVdecExContext->_u32VdecExIsrEventFlag[u8Idx] & E_VDEC_EX_EVENT_PTS_DISCONTINUE)
            {
                u32Event |= E_MVD_EVENT_PTS_DISCONTINUE;
            }
...
VDEC_EX_Result _MApi_VDEC_EX_V2_GenPattern(VDEC_StreamId *pStreamId, VDEC_EX_PatternType ePatternType, MS_PHYADDR u32Addr, MS_U32* u32Size)
{
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pDispFrm))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
    ...
    {
        VDEC_INFO(u8Idx, VDEC_PRINT("%s invalid pDispFrm 0x%lx\n", __FUNCTION__, pDispFrm->u32PriData));
        return E_VDEC_EX_RET_ILLEGAL_ACCESS;
    }
...
VDEC_EX_Result _MApi_VDEC_EX_V2_CaptureFrame(VDEC_StreamId *pStreamId, MS_U32 u32FrmPriData, MS_BOOL bEnable)
{
    ...
    {
        VDEC_PRINT("%s: Release an uncaptured frame(0x%lx != 0x%lx)!\n",
...
VDEC_EX_Result _MApi_VDEC_EX_V2_CC_Init(VDEC_StreamId *pStreamId, MS_U32 *pIn)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        ...
        case E_VDEC_EX_DECODER_HVD:
            {
                eRet = _VDEC_MapRet_HVD(MDrv_HVD_EX_CC_Init(u32Id));
            }
            break;
        ...
        case E_VDEC_EX_DECODER_MVD:
            {
                ...
                if (pInput->u32Ver != 0)
                {
                    VDEC_PRINT("%s not supported ver %ld\n", __FUNCTION__, pInput->u32Ver);
                    return E_VDEC_EX_FAIL;
                }
        ...
        case E_VDEC_EX_DECODER_HVD:
            {
                ...
                u16BufferSize = (MS_U16)pInput->u32Val;
                eRet = _VDEC_MapRet_HVD(MDrv_HVD_EX_CC_SetCfg(u32Id, u8Opt, u16BufferSize, u8Type));
        ...
        case E_VDEC_EX_DECODER_HVD:
            {
                ...
                u32StartAdd = pInput->u32Val;
                eRet = _VDEC_MapRet_HVD(MDrv_HVD_EX_CC_Set_RB_StartAddr(u32Id, u32StartAdd, u8Type));
...
VDEC_EX_Result _MApi_VDEC_EX_V2_CC_UpdateWriteAdd(VDEC_StreamId *pStreamId, VDEC_EX_CCFormat eFmt, MS_U32 *pIn)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        ...
        case E_VDEC_EX_DECODER_HVD:
            {
                MS_U8 u8Type = _VDEC_Map2HVDCCFmt(eFmt);
                eRet = _VDEC_MapRet_HVD(MDrv_HVD_EX_CC_SyncRB_RdAddr2WrAddr(u32Id, u8Type));
        ...
        case E_VDEC_EX_DECODER_HVD:
            {
                ...
                u32EachPktSize = pInput->u32Val;
                eRet = _VDEC_MapRet_HVD(MDrv_HVD_EX_CC_Adv_RB_ReadAddr(u32Id, u32EachPktSize, u8Type));
...
VDEC_EX_Result _MApi_VDEC_EX_V2_CC_DisableParsing(VDEC_StreamId *pStreamId, VDEC_EX_CCFormat eFmt)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        ...
        case E_VDEC_EX_DECODER_HVD:
            {
                MS_U8 u8Type = _VDEC_Map2HVDCCFmt(eFmt);
                eRet = _VDEC_MapRet_HVD(MDrv_HVD_EX_CC_DisableParsing(u32Id, u8Type));
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pDispinfo))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
    ...
    if(u32Id != pVdecExContext->_Attr[u8Idx].u32DrvId || pVdecExContext->_Attr[u8Idx].bInit == FALSE)
    {
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
...
VDEC_EX_Result _MApi_VDEC_EX_V2_IsMoreDecFrame(VDEC_StreamId *pStreamId)
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId))
    {
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
        {
            _BOOL_TO_VDEC_RESULT(eRet, MDrv_MVD_IsMoreDecFrame(u32Id));
            break;
        }
        case E_VDEC_EX_DECODER_HVD:
        {
            _BOOL_TO_VDEC_RESULT(eRet, MDrv_HVD_EX_IsMoreFrame(u32Id));
            break;
        }
        case E_VDEC_EX_DECODER_MJPEG:
        default:
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pExtDispinfo))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
VDEC_EX_Result _MApi_VDEC_EX_V2_GetSimpleDecFrame(VDEC_StreamId *pStreamId, VDEC_EX_FrameInfo *pFrmInfo)
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pFrmInfo))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
        {
            ...
            if (E_MVD_RET_OK == MDrv_MVD_GetFrmInfo(u32Id, E_MVD_FRMINFO_DECODE_SIMPLE, &stFrm))
            {
        ...
        case E_VDEC_EX_DECODER_HVD:
        {
            HVD_EX_FrameInfo info;
            _VDEC_Memset(&info, 0, sizeof(HVD_EX_FrameInfo));
            if (E_HVD_EX_OK == MDrv_HVD_EX_GetFrmInfo(u32Id, E_HVD_EX_GFRMINFO_DECODE_SIMPLE, &info))
            {
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pTimeCode))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u32EventFlag))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u8ActFmt))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u8ColourPrimaries))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
VDEC_EX_Result _MApi_VDEC_EX_V2_GetFwVersion(VDEC_StreamId *pStreamId, VDEC_EX_FwType eFwType,MS_U32* u32FWVer)
{
    ...
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u32FWVer))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u8Cnt))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u32Ptr))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u32Pts))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(s64Pts))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(eErrCode))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
        {
            ...
            if (E_MVD_ERR_UNKNOWN == errCode)
            {
                ...
                {
                    //report not support spec to avoid continue play
                    printf("MVD not support divx plus\n");
                    pVdecExContext->_Attr[u8Idx].eErrCode = E_VDEC_EX_ERR_CODE_NOT_SUPPORT;
                }
            }
    ...
    static VDEC_EX_ErrCode prevErrCode = E_VDEC_EX_ERR_CODE_BASE;
    if(pVdecExContext->_Attr[u8Idx].eErrCode && prevErrCode != pVdecExContext->_Attr[u8Idx].eErrCode)
    {
        printf("[MMM]  pVdecExContext->_Attr[u8Idx].eErrCode:%x\n",pVdecExContext->_Attr[u8Idx].eErrCode);
    }
    prevErrCode = pVdecExContext->_Attr[u8Idx].eErrCode;
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u32Cnt))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u32Bitrate))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
VDEC_EX_Result _MApi_VDEC_EX_V2_GetDropCnt(VDEC_StreamId *pStreamId,MS_U32* u32Cnt)
{
    ...
    *u32Cnt = 0;
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
        {
            *u32Cnt = MDrv_MVD_GetDropPicCounter(u32Id);
            break;
        }
        case E_VDEC_EX_DECODER_HVD:
        {
...
//------------------------------------------------------------------------------
VDEC_EX_Result _MApi_VDEC_EX_V2_GetDecQVacancy(VDEC_StreamId *pStreamId,MS_U32* u32Vacancy)
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u32Vacancy))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(bIs32PullDown))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
VDEC_EX_Result _MApi_VDEC_EX_V2_IsAlive(VDEC_StreamId *pStreamId)
{
    ...
    {
        case E_VDEC_EX_DECODER_MVD:
            eRet = _VDEC_MapRet_MVD(MDrv_MVD_IsAlive(u32Id));
            break;
        case E_VDEC_EX_DECODER_HVD:
        {
...
VDEC_EX_Result _MApi_VDEC_EX_V2_IsCCAvailable(VDEC_StreamId *pStreamId,MS_BOOL* is_available)
{
    ...
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(is_available))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pInfo))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(eTrick))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(CodecType))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(length))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_RET_UNSUPPORTED;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pu8Key))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u32ESBuffVacancy))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
/// @return -VDEC_EX_Result
//------------------------------------------------------------------------------
VDEC_EX_Result _MApi_VDEC_EX_V2_GetESBuff(VDEC_StreamId *pStreamId, MS_U32 u32ReqSize, MS_U32 *u32AvailSize, MS_PHYADDR *u32Addr)
{
    MS_U32 u32ReadPtr = 0;
    ...
    MS_U32 u32ESSafeSize = ES_SAFE_SIZE;

    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u32Addr) || MApi_VDEC_EX_CHECK_NULL_PTR(u32AvailSize))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }

    if (pVdecExContext->_bEsBufMgmt[u8Idx] == 0)
    {
        return E_VDEC_EX_RET_ILLEGAL_ACCESS;
    }
...
#endif
    {
        ...
        u32ESSafeSize = 0x80000;
    }

    u32ReqSize += (u32paddingsize+u32ESSafeSize);

    pCfg = &(pVdecExContext->_Attr[u8Idx].vdecExInitParam.SysConfig);

    if (pVdecExContext->bFlush[u8Idx] == TRUE)
    {
        if((pCfg->u32BitstreamBufSize-pCfg->u32DrvProcBufSize) < u32ReqSize)
        {
        ...
        *u32AvailSize = u32ReqSize-(u32paddingsize+u32ESSafeSize);
        *u32Addr = pCfg->u32BitstreamBufAddr+pCfg->u32DrvProcBufSize;
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        ...
        default:
            u32ReadPtr = u32WritePtr = pCfg->u32DrvProcBufSize;
            break;
    }
    ...
    if (u32ReadPtr == 0)
    {
        u32ReadPtr = u32BuffSt;
    }

    u32WritePtr = ((u32WritePtr+3)>>2)<<2;   //4 byte alignment

    MS_U32 u32EsVacancy = 0;

    if(u32ReadPtr > u32WritePtr)
    {
       u32EsVacancy =  u32ReadPtr - u32WritePtr;
    }
    else
    {
        u32EsVacancy = pCfg->u32BitstreamBufSize-pCfg->u32DrvProcBufSize-u32WritePtr+u32ReadPtr;
    }

    if(u32EsVacancy < u32ReqSize)
    {
        VDEC_INFO(u8Idx, VDEC_PRINT("ES buffer full\n"));
        goto _NO_ES_BUFF;
    }
    ...
    if (u32ReadPtr <= u32WritePtr)
    {
        if (u32WritePtrNew >= u32BuffEnd)
        {
            ...
            {
#ifdef VDEC3
                if(((pVdecExContext->_Attr[u8Idx].eDecoder) == E_VDEC_EX_DECODER_MVD || (pVdecExContext->_Attr[u8Idx].eDecoder) == E_VDEC_EX_DECODER_HVD) &&
                    (MAX_VDEC_VBBU_ENTRY_COUNT == u32Vacancy )
                    )
                {
                    VDEC_INFO(u8Idx, VDEC_PRINT("xRd=0x%lx, Wr=0x%lx, WrNew=0x%lx, BsBuff=0x%lx, ReqSize=0x%lx\n", u32ReadPtr, u32WritePtr,
    ...
    *u32Addr = u32WritePtr + pCfg->u32BitstreamBufAddr;
    VDEC_INFO(u8Idx, VDEC_PRINT("Rd=0x%lx, Wr=0x%lx, WrNew=0x%lx, BsBuff=0x%lx, ReqSize=0x%lx\n", u32ReadPtr, u32WritePtr,
           u32WritePtrNew, pCfg->u32BitstreamBufAddr, u32ReqSize));
...
VDEC_EX_Result _MApi_VDEC_EX_V2_GetNextDispFrame(VDEC_StreamId *pStreamId, VDEC_EX_DispFrame *pDispFrm)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_MVD:
        {
            ...
            VDEC_EX_DispFrame *pDispFrm_tmp = NULL;
            if(eVdecDecMode[u8Idx] == E_VDEC_EX_DEC_MODE_AUTO_MCU)
            {
                pDispFrm_tmp = &g_stDispFrame[u8Idx];
            }
            else
            {
                pDispFrm_tmp = _VDEC_EX_MallocDq(u8Idx);
            }
        ...
        case E_VDEC_EX_DECODER_HVD:
        {
            ...
            if(eVdecDecMode[u8Idx] == E_VDEC_EX_DEC_MODE_AUTO_MCU)
            {
                if((pVdecExContext->_Attr[u8Idx].eCodecType == E_VDEC_EX_CODEC_TYPE_MVC) || (pVdecExContext->_Attr[u8Idx].eCodecType == E_VDEC_EX_CODEC_TYPE_HEVC))
                {
                    static unsigned int toggle_mvc_view = 0;
                    pDispFrm_tmp = &g_stDispFrame[toggle_mvc_view];
                    toggle_mvc_view = 1-toggle_mvc_view;
                }
                else
                {
                    pDispFrm_tmp = &g_stDispFrame[u8Idx];
            ...
            static MS_U16 u16Height = 0, u16Width = 0;
            if(u16Height != info.u16Height || u16Width != info.u16Width)
            {
                printf("[MMM] resolution change [%d %d] to [%d %d]\n",u16Width,u16Height,info.u16Width,info.u16Height);
            }
            u16Height = info.u16Height;
            u16Width = info.u16Width;
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pFrmInfoEx))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        case E_VDEC_EX_DECODER_HVD:
        {
            ...
            pFrmInfoEx->sFrameInfo.eFrameType = _VDEC_MapFrameType_HVD(info.eFrmType);
            pFrmInfoEx->sFrameInfo.eFieldType = _VDEC_MapFieldType_HVD(info.eFieldType);
...
VDEC_EX_Result _MApi_VDEC_EX_V2_GetNextDispFrameInfoExt(VDEC_StreamId *pStreamId, void *param)
{
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        ...
        case E_VDEC_EX_DECODER_HVD:
        {
            VDEC_EX_FrameInfoExt *pVerCtl = (VDEC_EX_FrameInfoExt *)param;
            if((pVerCtl->stVerCtl.u32version == 0)
            && (pVerCtl->stVerCtl.u32size == sizeof(VDEC_EX_FrameInfoExt)))
            {
                VDEC_EX_FrameInfoExt *pFrmInfoEx = (VDEC_EX_FrameInfoExt *)param;
...
#if defined(VDEC_CAP_HEVC_HDR)
            else if((pVerCtl->stVerCtl.u32version == 1)
            && (pVerCtl->stVerCtl.u32size== sizeof(VDEC_EX_FrameInfoExt)))
            {
                VDEC_EX_FrameInfoExt *pFrmInfoEx = (VDEC_EX_FrameInfoExt *)param;
                ...
                pFrmInfoEx->maxLuminance = info.maxLuminance;
                pFrmInfoEx->minLuminance = info.minLuminance;
                pFrmInfoEx->primaries[0][0] = info.primaries[0][0];
                pFrmInfoEx->primaries[0][1] = info.primaries[0][1];
                pFrmInfoEx->primaries[1][0] = info.primaries[1][0];
                pFrmInfoEx->primaries[1][1] = info.primaries[1][1];
                pFrmInfoEx->primaries[2][0] = info.primaries[2][0];
                pFrmInfoEx->primaries[2][1] = info.primaries[2][1];
                pFrmInfoEx->whitePoint[0] = info.whitePoint[0];
                pFrmInfoEx->whitePoint[1] = info.whitePoint[1];
                pFrmInfoEx->Frm_Info_Ext_avail = info.Frm_Info_Ext_avail;  //bit1: SEI_Enabled,  bit0=colur_description_present_flag
                pFrmInfoEx->colour_primaries = info.colour_primaries;
                pFrmInfoEx->transfer_characteristics = info.transfer_characteristics;
                pFrmInfoEx->matrix_coefficients = info.matrix_coefficients;
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pOut))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
VDEC_EX_Result _MApi_VDEC_EX_V2_CC_GetWriteAdd(VDEC_StreamId *pStreamId, VDEC_EX_CCFormat eFmt,MS_PHYADDR* u32CCWrPtr)
{
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
...
/// @return MS_PHYADDR
//------------------------------------------------------------------------------
VDEC_EX_Result _MApi_VDEC_EX_V2_CC_GetReadAdd(VDEC_StreamId *pStreamId, VDEC_EX_CCFormat eFmt,MS_PHYADDR* u32CCRdPtr)
{
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u32CCRdPtr))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
VDEC_EX_Result _MApi_VDEC_EX_V2_CheckCaps(VDEC_StreamId *pStreamId, VDEC_EX_CodecType eCodecType,MS_BOOL* bCaps)
{
    ...
    if (MApi_VDEC_EX_CHECK_NULL_PTR(bCaps))
        return E_VDEC_EX_RET_INVALID_PARAM;
    *bCaps = FALSE;
...
{
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pStatus))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
    MS_U8  u8Idx = _VDEC_GET_IDX(pStreamId->u32Id);
    VDEC_EX_Result bRet = E_VDEC_EX_FAIL;
...
void _MApi_VDEC_EX_V2_StoreUserCmd(VDEC_EX_RESUME_SET_CMD* pData)
{
    ...
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pData))
        return;
...
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(u32Para))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
{
    VDEC_EX_Result eRet  = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pstErrTolerance))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
VDEC_EX_Result _MApi_VDEC_EX_V2_GetEventFlag(VDEC_StreamId *pStreamId,MS_U32* u32EventFlag,  MS_U32 cb_idx)
{
    ...
    #if EVENT_PATCH
    MsOS_WaitEvent_Interrupt(s32VdecExEventId[u8Idx],
        pVdecExContext->_u32PreVdecExIsrEventFlag[u8Idx],
        &u32VdecEventFlag,
        E_OR_CLEAR,
        MSOS_WAIT_FOREVER);
...
static VDEC_EX_Result  _MApi_VDEC_EX_PTS_Discontinue_Auto_Detect(VDEC_StreamId *pStreamId,MS_BOOL bEnable)
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId))
    {
    ...
    {
        case E_VDEC_EX_DECODER_MVD:
            if(MDrv_MVD_PTS_Discontinue_Auto_Detect(u32Id,bEnable) != E_MVD_RET_OK)
            {
                eRet = E_VDEC_EX_FAIL;
            }
            break;
...
static VDEC_EX_Result  _MApi_VDEC_EX_V2_IsSTRResume(VDEC_StreamId *pStreamId)
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId))
    {
    ...
    MS_U32 u32Id = _VDEC_GET_DRV_ID(pStreamId->u32Id);
    _BOOL_TO_VDEC_RESULT(eRet,  (MS_BOOL) (MDrv_MVD_IsSTRResume(u32Id)==TRUE));
    return eRet;

}

static VDEC_EX_Result  _MApi_VDEC_EX_V2_IsSTR(VDEC_StreamId *pStreamId)
{
    VDEC_EX_Result eRet = E_VDEC_EX_FAIL;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId))
    {
    ...
    MS_U32 u32Id = _VDEC_GET_DRV_ID(pStreamId->u32Id);
    _BOOL_TO_VDEC_RESULT(eRet,  (MS_BOOL) (MDrv_MVD_IsSTR(u32Id)==TRUE));
    return eRet;
...
{
#ifdef VDEC_UTOPIA_2K_KDRV
    printk("[_MApi_VDEC_EX_V2_Set_MBX_param] u8APIMbxMsgClass = %d\n",u8APIMbxMsgClass);
#endif
...
/// @return VDEC_Result
//------------------------------------------------------------------------------
static VDEC_EX_Result _MApi_VDEC_EX_SetFrameBuff2(VDEC_StreamId *pStreamId, MS_U32 u32FrmBuffAddr, MS_U32 u32FrmBuffSize)
{
    VDEC_EX_Result ret = E_VDEC_EX_FAIL;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId))
    {
...
#if 0
static VDEC_EX_Result  _MApi_VDEC_EX_DisableDivx(VDEC_StreamId *pStreamId,MS_BOOL bDisable)
{
    VDEC_EX_Result eRet = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId))
    {
    ...
    {
        case E_VDEC_EX_DECODER_HVD:
            _HVD_RET_HANDLE(MDrv_HVD_EX_DisableDivx(u32Id,bDisable));
            break;
        case E_VDEC_EX_DECODER_MVD:
            MDrv_MVD_DisableDivx(u32Id,bDisable);
            break;;
        case E_VDEC_EX_DECODER_MJPEG:
        default:
             eRet = E_VDEC_EX_RET_UNSUPPORTED;
...
{
    VDEC_EX_Result eRet  = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pstDynmcDispPath))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
static VDEC_EX_Result _MApi_VDEC_EX_SetPUSISetting(VDEC_StreamId *pStreamId, MS_BOOL bEnable)
{
    VDEC_EX_Result eRet  = E_VDEC_EX_OK;
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId))
    {
    ...
    {
        case E_VDEC_EX_DECODER_MVD:
            if(MDrv_MVD_SetPUSISetting(u32Id,bEnable) == FALSE)
            {
                eRet = E_VDEC_EX_FAIL;
            }
            break;
...
VDEC_EX_Result MApi_VDEC_EX_SetDbgLevel(VDEC_StreamId *pStreamId, VDEC_EX_DbgLevel eDbgLevel)
{
    ...
    {
        MS_U8  u8Idx = _VDEC_GET_IDX(pStreamId->u32Id);
        gVDEC_EX_2k_CbData[u8Idx].u8Dbglevl = (MS_U8)eDbgLevel;
    }
    IO_arg.StreamID = (VDEC_EX_V2_StreamId*) pStreamId;
...
VDEC_EX_Result MApi_VDEC_EX_SetFrmBuff2(VDEC_StreamId *pStreamId, MS_U32 u32FrmBuffAddr, MS_U32 u32FrmBuffSize)
{

    VDEC_EX_V2_IO_Param IO_arg;
    VDEC_EX_Result ret = E_VDEC_EX_FAIL;
    VDEC_FRAMEBuff stframebuff;
    if (pu32VDEC_EX_Inst == NULL || u32FrmBuffSize == 0)
    {
        VPRINTF("[Fail][%s,%d]\n",__FUNCTION__,__LINE__);
        return E_VDEC_EX_FAIL;
    }
    stframebuff.u32Version = 0;
    stframebuff.u32Addr = u32FrmBuffAddr;
    stframebuff.u32Size = u32FrmBuffSize;
    IO_arg.StreamID = (VDEC_EX_V2_StreamId*) pStreamId;
    IO_arg.eUserCmd = E_VDEC_EX_V2_USER_CMD_SET_FRAMEBUFF2;
    IO_arg.param[0] = (void*)(&stframebuff);
    IO_arg.pRet     = (void*)(&ret);
...
VDEC_EX_Result MApi_VDEC_EX_IsSTRResume(VDEC_StreamId *pStreamId)
{
    VDEC_EX_V2_IO_Param IO_arg;
    VDEC_EX_Result ret;
    ...
    IO_arg.StreamID = (VDEC_EX_V2_StreamId*) pStreamId;
    IO_arg.eUserCmd = E_VDEC_EX_V2_USER_CMD_GET_IS_STR_RESUME;
    IO_arg.pRet     = (void*)(&ret);
...
VDEC_EX_Result MApi_VDEC_EX_IsSeamless(VDEC_StreamId *pStreamId)
{
    VDEC_EX_V2_IO_Param IO_arg;
    VDEC_EX_Result ret;
    ...
    IO_arg.StreamID = (VDEC_EX_V2_StreamId*) pStreamId;
    IO_arg.eUserCmd = E_VDEC_EX_V2_USER_CMD_GET_IS_SEAMLESS_MODE;
    IO_arg.pRet     = (void*)(&ret);
...
VDEC_EX_Result MApi_VDEC_EX_IsSTR(VDEC_StreamId *pStreamId)
{
    VDEC_EX_V2_IO_Param IO_arg;
    VDEC_EX_Result ret;
    ...
    IO_arg.StreamID = (VDEC_EX_V2_StreamId*) pStreamId;
    IO_arg.eUserCmd = E_VDEC_EX_V2_USER_CMD_GET_IS_STR;
    IO_arg.pRet     = (void*)(&ret);
...
VDEC_EX_Result MApi_VDEC_EX_IsMoreDecFrame(VDEC_StreamId *pStreamId)
{
    VDEC_EX_V2_IO_Param IO_arg;
    VDEC_EX_Result ret;
    ...
    IO_arg.StreamID = (VDEC_EX_V2_StreamId*) pStreamId;
    IO_arg.eUserCmd = E_VDEC_EX_V2_USER_CMD_IS_MORE_FRAME;
    IO_arg.pRet     = (void*)(&ret);
...
VDEC_EX_Result MApi_VDEC_EX_GetSimpleDecFrame(VDEC_StreamId *pStreamId, VDEC_EX_FrameInfo *pFrmInfo)
{
    VDEC_EX_V2_IO_Param IO_arg;
    VDEC_EX_Result ret;
    ...
    IO_arg.StreamID = (VDEC_EX_V2_StreamId*) pStreamId;
    IO_arg.eUserCmd = E_VDEC_EX_V2_USER_CMD_GET_SIMPLE_FRAME;
    IO_arg.param[0] = (void*)(pFrmInfo);
    IO_arg.pRet     = (void*)(&ret);
...
/// @return VDEC_EX_Result
//------------------------------------------------------------------------------
VDEC_EX_Result MApi_VDEC_EX_GenPattern(VDEC_StreamId *pStreamId, VDEC_EX_PatternType ePatternType, MS_PHYADDR u32Addr, MS_U32* u32Size)
{
    VDEC_EX_V2_IO_Param IO_arg;
    VDEC_EX_Result ret;
...
#ifdef VDEC_UTOPIA_2K
    _VDEC_EX_Delete_Callback_Task(pStreamId);
#endif
    IO_arg.StreamID = (VDEC_EX_V2_StreamId*) pStreamId;
...
VDEC_EX_Result MApi_VDEC_GetCaps(VDEC_HwCap *pVDECHWCap)
{
    ...
    memset(&stVDECHWCap, 0, sizeof(VDEC_HwCap));

    if(MApi_VDEC_EX_CHECK_NULL_PTR(pVDECHWCap))
    {
    ...
    {
        VPRINTF("[VDEC] old lib version has only length:%x\n",sizeof(VDEC_HwCap));
...
#ifndef MSOS_TYPE_LINUX_KERNEL
    if(_MApi_VDEC_EX_IsSecureModeEnable())// jeffrey move mailbox register/enable to user mode
    {
...
VDEC_EX_Result MApi_VDEC_EX_V2_KillProcess(VDEC_StreamId *pStreamId)
{
#ifdef VDEC_UTOPIA_2K
    return _MApi_VDEC_EX_V2_Exit(pStreamId);
#else
    return E_VDEC_EX_OK;
#endif
}
...
VDEC_EX_Result MApi_VDEC_EX_V2_GetFreeStream(void *pInfo, MS_U32 u32Size, VDEC_EX_Stream eStreamType, VDEC_EX_CodecType eCodecType)
{
...
#if VDEC_NOT_SUPPORT_DIVX
    if(eCodecType == E_VDEC_EX_CODEC_TYPE_DIVX311 || eCodecType == E_VDEC_EX_CODEC_TYPE_DIVX412)
    {
        VDEC_PRINT("Not Support Divx Codec:%d\n",eCodecType);
        return E_VDEC_EX_FAIL;
    }
#endif
    ...
    else if (pVdecExContext->bFWdecideFB && eStreamType != E_VDEC_EX_N_STREAM)
    {
    ...
    if (u32Version == 0 && u32Size == sizeof(VDEC_StreamId))
    {
        ...
        {
            VDEC_PRINT("eStreamType(%x) is used! pVdecExContext->_Attr[%d].bUsed = %x\n", eStreamType,u8Idx ,pVdecExContext->_Attr[u8Idx].bUsed);
            _VDEC_EX_API_MutexUnlock();
            return E_VDEC_EX_FAIL;
        }
        ...
        switch (eDecoder)
        {
            case E_VDEC_EX_DECODER_MVD:
            {
                ...
                if (eMvdStreamType != E_MVD_DRV_STREAM_NONE)
                {
                    ...
                    VDEC_PRINT("=========> vdec_u8Idx=0x%x, u32VdecStreamId=0%lx\n", u8Idx, u32VdecStreamId);
...
#ifdef VDEC3
static void _MApi_VDEC_EX_BS_Init(MS_PHYADDR addr, MS_U32 size)
{
    if (!pVdecExContext)
        return;
...
static MS_PHYADDR _MApi_VDEC_EX_BS_GetAddr(MS_U8 u8Idx)
{
...
VDEC_EX_Result MApi_VDEC_EX_V2_Init(VDEC_StreamId *pStreamId, VDEC_EX_InitParam *pInitParam)
{
    ...
    if (MApi_VDEC_EX_CHECK_NULL_PTR(pStreamId) || MApi_VDEC_EX_CHECK_NULL_PTR(pInitParam))
    {
        VPRINTF("ERROR!!! Input Null pointer at %s:%d", __FUNCTION__, __LINE__);
        return E_VDEC_EX_RET_INVALID_PARAM;
    }
...
#if VDEC_NOT_SUPPORT_DIVX
    if(pInitParam->eCodecType == E_VDEC_EX_CODEC_TYPE_DIVX311 || pInitParam->eCodecType == E_VDEC_EX_CODEC_TYPE_DIVX412)
    {
        VDEC_PRINT("Not Support Divx Codec:%d\n",pInitParam->eCodecType);
        return E_VDEC_EX_FAIL;
    }
#endif
    ...
    if(pInitParam->SysConfig.u32FrameBufSize < MM_VP9_MFCODEC_SIZE && pInitParam->eCodecType == E_VDEC_EX_CODEC_TYPE_VP9)
    {
        pVdecExContext->_Pre_Ctrl[u8Idx].eMFCodecMode = E_VDEC_EX_MFCODEC_FORCE_DISABLE;
    }
    else
    {
        pVdecExContext->_Pre_Ctrl[u8Idx].eMFCodecMode = E_VDEC_EX_MFCODEC_DEFAULT;
    }
#ifdef MSOS_TYPE_LINUX_KERNEL
    printk("%s  u8Idx:%d  eSrcMode:%d  CodecType:%d  FB:[%lx,%lx]  ES:[%lx,%lx]  VPU:[%lx,%lx]\n",__FUNCTION__,u8Idx,pInitParam->VideoInfo.eSrcMode,pInitParam->eCodecType,
    pInitParam->SysConfig.u32FrameBufAddr,pInitParam->SysConfig.u32FrameBufSize,
    pInitParam->SysConfig.u32BitstreamBufAddr,pInitParam->SysConfig.u32BitstreamBufSize,
    pInitParam->SysConfig.u32CodeBufAddr,pInitParam->SysConfig.u32CodeBufSize);
#endif
#ifdef VDEC3
    if (pVdecExContext->bDRVdecideBS)
    {
        _MApi_VDEC_EX_BS_Init(pInitParam->SysConfig.u32BitstreamBufAddr, pInitParam->SysConfig.u32BitstreamBufSize);
        pVdecExContext->_Attr[u8Idx].vdecExInitParam.SysConfig.u32BitstreamBufAddr = _MApi_VDEC_EX_BS_GetAddr(u8Idx);
        pVdecExContext->_Attr[u8Idx].vdecExInitParam.SysConfig.u32BitstreamBufSize = _MApi_VDEC_EX_BS_GetSize(u8Idx);
    }
#endif
    ...
    switch (pVdecExContext->_Attr[u8Idx].eDecoder)
    {
        ...
        // remove this part to fix coverity first
        #if 0
        if (!MDrv_MVD_Init)
        {
            _VDEC_EX_API_MutexUnlock();
        ...
        #endif
        {
            ...
            if (pVdecExContext->_Pre_Ctrl[u8Idx].bEnableThumbnailMode)
            {
                MDrv_MVD_EnableThumbNailMode(u32Id, pVdecExContext->_Pre_Ctrl[u8Idx].bEnableThumbnailMode);
            }
...
#ifdef VDEC_NOT_SUPPORT_DIVX
                //remove this part on 2015/12/15 for disable parsing BitStream
                //MDrv_MVD_DisableDivx(u32Id,TRUE);
#else
                MS_BOOL bNoUse = FALSE;
                //need to check the logic, not support divx 311 means not support whole divx
                if(FALSE == MDrv_MVD_AUTH_IPCheck(E_MVD_CODEC_DIVX311,&bNoUse))
                {
                    MDrv_MVD_DisableDivx(u32Id,TRUE);
                }
#endif
        ...
        #endif
        {
            if (pVdecExContext->_Pre_Ctrl[u8Idx].bEnableThumbnailMode)
            {
                MDrv_HVD_EX_ThumbnailMode(u32Id, pVdecExContext->_Pre_Ctrl[u8Idx].bEnableThumbnailMode);
            }
            ...
            if(pVdecExContext->_Pre_Ctrl[u8Idx].bEnableForceStartNonISlice)
            {
                MDrv_HVD_EX_EnableForceStartNonISlice(u32Id,pVdecExContext->_Pre_Ctrl[u8Idx].bEnableForceStartNonISlice);
            }
...
#ifdef VDEC3
            if(MDrv_HVD_EX_Init(u32Id, &stMemCfg, &stInitSettings, pVdecExContext->bFWdecideFB) != E_HVD_EX_OK)
#else
...
#ifdef VDEC_NOT_SUPPORT_DIVX
                //remove this part on 2015/12/15 for disable parsing BitStream
                //MDrv_HVD_EX_DisableDivx(u32Id,TRUE);
#else
                MS_BOOL bNoUse = FALSE;
                //need to check the logic, not support divx 311 means not support whole divx
                if(FALSE == MDrv_MVD_AUTH_IPCheck(E_MVD_CODEC_DIVX311,&bNoUse))
...
VDEC_EX_Result MApi_VDEC_EX_SetControl(VDEC_StreamId *pStreamId, VDEC_EX_User_Cmd cmd_id, MS_U32 param)
{
    ...
    else
    {
        ...
        if((cmd_id == E_VDEC_EX_USER_CMD_SET_DYNAMIC_DISP_PATH) ||
           (cmd_id == E_VDEC_EX_USER_CMD_SET_GST_TIMING) ||
           (cmd_id == E_VDEC_EX_USER_CMD_SET_EXTERNAL_DS_BUFFER) ||
           (cmd_id == E_VDEC_EX_USER_CMD_FIELD_POLARITY_DISPLAY_ONE_FIELD))
        {
            IO_arg.param[0] = (void*)(param);
        }
        else
...
VDEC_EX_Result MApi_VDEC_EX_V2_SetControl(VDEC_StreamId* pStreamId, VDEC_EX_V2_User_Cmd cmd_id, void** param,MS_BOOL IsResume)
{
    ...
    switch (cmd_id)
    {
        case E_VDEC_EX_V2_USER_CMD_REPEAT_LAST_FIELD:
            ...
            {
                ret = _VDEC_EX_RepeatLastField(pStreamId,(*((MS_BOOL*)(param[0]))));
        ...
        case E_VDEC_EX_V2_USER_CMD_FRC_OUTPUT:
            ...
            {
                ret = _VDEC_EX_FrameRateConvert(pStreamId, (VDEC_EX_FRC_OutputParam*)(*((MS_U32*)(param[0]))));
        ...
        case E_VDEC_EX_V2_USER_CMD_FAST_DISPLAY:
            ...
            {
                ret = _VDEC_EX_SetFastDisplay(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_IGNORE_ERR_REF:
            ...
            {
                ret = _VDEC_EX_SetIgnoreErrRef(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_FORCE_FOLLOW_DTV_SPEC:
            ...
            {
                ret = _VDEC_EX_ForceFollowDTVSpec(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_DISABLE_SEQ_CHG:
            ...
            {
                ret = _VDEC_EX_SetDisableSeqChange(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_DTV_RESET_MVD_PARSER:
            if(param != NULL)
            ret = _VDEC_EX_ResetMvdParser(pStreamId,(*((MS_BOOL*)(param[0]))));
            break;
        ...
        case E_VDEC_EX_V2_USER_CMD_SET_DISP_OUTSIDE_CTRL_MODE:
            ...
            {
                ret = _VDEC_EX_DISP_OUTSIDE_MODE(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_RELEASE_FD_MASK:
            if(param != NULL)
            ret = _VDEC_EX_ReleaseFdMask(pStreamId,(*((MS_BOOL*)(param[0]))));
            break;
        case E_VDEC_EX_V2_USER_CMD_FLUSH_PTS_BUF:
            if(param != NULL)
            ret = _VDEC_EX_FlushPTSBuf(pStreamId,(*((MS_BOOL*)(param[0]))));
            break;
        case E_VDEC_EX_V2_USER_CMD_SUPPORT_AVC_TO_MVC:
            ...
            {
                ret = _VDEC_EX_Support_AVC2MVC(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_3DLR_VIEW_EXCHANGE:
            ...
            {
                ret = _VDEC_EX_3DLR_View_Excahnge(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_AVC_NEW_SLOW_MOTION:
            ...
            {
                ret = _VDEC_EX_AVC_Enable_New_Slow_Motion(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_SET_VSIZE_ALIGN:
            ...
            {
                ret = _VDEC_EX_SetVSizeAlign(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_SHOW_DECODE_ORDER:
            ...
            {
                ret = _VDEC_EX_ShowDecodeOrder(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_AVC_DISP_IGNORE_CROP:
            ...
            {
                ret = _VDEC_EX_Disp_Ignore_Crop(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_SUSPEND_DYNAMIC_SCALE:
            ...
            {
                ret = _VDEC_EX_SuspendDynamicScale(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_FORCE_AUTO_MUTE:
            ...
            {
                ret = _VDEC_EX_ForceAutoMute(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_PUSH_DISPQ_WITH_REF_NUM:
            ...
            {
                ret = _VDEC_EX_PushDispQWithRefNum(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_DS_RESV_N_BUFFER:
            ...
            {
                ret = _VDEC_EX_DynamicScalingResvNBuffer(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_RM_ENABLE_PTS_TBL:
            ...
            {
                ret = _VDEC_EX_RmEnablePtsTbl(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_DROP_ERR_FRAME:
            ...
            {
                ret = _VDEC_EX_DropErrFrame(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_SET_CC608_INFO_ENHANCE_MODE:
            ...
            {
                ret = _VDEC_EX_SetCcInfoEnhanceMode(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_IGNORE_PIC_OVERRUN:
            ...
            {
                ret = _VDEC_EX_IgnorePicOverrun(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_SET_SELF_SEQCHANGE:
            ...
            {
                ret = _VDEC_EX_SetSelfSeqChange(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_CTL_SPEED_IN_DISP_ONLY:
            ...
            {
                ret = _VDEC_EX_CtlSpeedInDispOnly(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_AVC_SUPPORT_REF_NUM_OVER_MAX_DPB_SIZE:
            ...
            {
                ret = _VDEC_EX_AVCSupportRefNumOverMaxDBPSize(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_RETURN_INVALID_AFD:
            ...
            {
                ret = _VDEC_EX_ReturnInvalidAFD(pStreamId, (MS_U32)(*((MS_U32*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_FIELD_POLARITY_DISPLAY_ONE_FIELD:
            ...
            {
                ret = _VDEC_EX_Field_Polarity_Display_One_Field(pStreamId,(VDEC_EX_Field_Polarity*)param[0]);
        ...
        case E_VDEC_EX_V2_USER_CMD_AVC_FORCE_BROKEN_BY_US:
            ...
            {
                ret = _VDEC_EX_AVCForceBrokenByUs(pStreamId,(*((MS_BOOL*)(param[0]))));
                //_VDEC_PREPARE_RESUME_DATA(u8Idx,cmd_id,param,1,0,0,0,1,2,3,4);
                resume_data.u8Idx=u8Idx;
                resume_data.cmd_id=cmd_id;
                resume_data.param=param;
        ...
        case E_VDEC_EX_V2_USER_CMD_DUAL_NON_BLOCK_MODE:
            if(param != NULL)
            ret = _VDEC_EX_DualNonBlockMode(pStreamId,(*((MS_BOOL*)(param[0]))));
            break;
        ...
        case E_VDEC_EX_V2_USER_CMD_SET_EXTERNAL_DS_BUFFER:
            if(param != NULL)
            ret = _VDEC_EX_Set_External_DS_Buffer(pStreamId,(VDEC_EX_EXTERNAL_DS_BUFFER*)param[0]);
            break;
        ...
        case E_VDEC_EX_V2_USER_CMD_SET_DYNSCALING_PARAMS:
            if(IsResume == TRUE)
            {
                ret = _MApi_VDEC_EX_V2_SetDynScalingParams(pStreamId,
                                                          (MS_PHYADDR)(pVdecExContext->store_set_cmd[pVdecExContext->store_set_cmd_read_pointer[u8Idx]].u32Arg[0]),
        ...
        case E_VDEC_EX_V2_USER_CMD_ENABLE_DECODE_ENGINE_TIMEOUT:
            if(param != NULL)
            ret = _VDEC_EX_SetDecodeTimeoutParam(pStreamId,(VDEC_EX_Decode_Timeout_Param*)(*((MS_U32*)(param[0]))));
            break;

        case E_VDEC_EX_V2_USER_CMD_FRAMEBUFFER_AUTO_MODE:
            if(param != NULL)
            ret = _MApi_VDEC_EX_SetFramebufferAutoMode(pStreamId,(*((MS_BOOL*)(param[0]))));
            break;
        ...
        case E_VDEC_EX_V2_USER_CMD_SET_ERROR_TOLERANCE:
            if(param != NULL)
            ret = _MApi_VDEC_EX_SetError_Tolerance(pStreamId,(VDEC_EX_Err_Tolerance*)(*((MS_U32*)(param[0]))));
            break;

        case E_VDEC_EX_V2_USER_CMD_SET_PTS_US_MODE:
            if(param != NULL)
            ret = _MApi_VDEC_EX_Set_PTS_USec_Mode(pStreamId,(*((MS_BOOL*)(param[0]))));
            break;

        case E_VDEC_EX_V2_USER_CMD_AUTO_INSERT_DUMMY_DATA:
            if(param != NULL)
            ret = _MApi_VDEC_EX_Enable_AutoInsertDummyPattern(pStreamId,(*((MS_BOOL*)(param[0]))));
            break;
        ...
        case E_VDEC_EX_V2_USER_CMD_PVR_TIMESHIFT_SEAMLESS_MODE:
            ...
            break;
        case E_VDEC_EX_V2_USER_CMD_SET_MBX_PARAM://jeffrey
            ret = _MApi_VDEC_EX_V2_Set_MBX_param((MS_U8)(*((MS_U8*)(param[0]))));
            break;
       case E_VDEC_EX_V2_USER_CMD_SET_FRAMEBUFF2:
            if(param != NULL)
            {
                VDEC_FRAMEBuff *framebuff = (VDEC_FRAMEBuff *)(param[0]);
       ...
       case E_VDEC_EX_V2_USER_CMD_PTS_DISCONTINUE_AUTO_DETECT:
            ret = _MApi_VDEC_EX_PTS_Discontinue_Auto_Detect(pStreamId,(*((MS_BOOL*)(param[0]))));
            break;
       ...
       case E_VDEC_EX_V2_USER_CMD_SET_AVSYNC_DISP_AUTO_DROP:
            if(param != NULL)
                ret = _MApi_VDEC_EX_SetAVSyncDispAutoDrop(pStreamId,(*((MS_BOOL*)(param[0]))));
            break;
       case E_VDEC_EX_V2_USER_CMD_SET_DYNAMIC_DISP_PATH:
            if(param != NULL)
            {
                ret = _MApi_VDEC_EX_SetDynmcDispPath(pStreamId,(VDEC_EX_DynmcDispPath*)(param[0]));
            }
            break;
       case E_VDEC_EX_V2_USER_CMD_SET_GST_TIMING:
            ...
            {
                MS_U8  u8Idx = _VDEC_GET_IDX(pStreamId->u32Id);
                VDEC_EX_MVOPInfo* pMVOPInfo = (VDEC_EX_MVOPInfo*)(param[0]);
                if(pMVOPInfo)
                {
                    memcpy((void*)(&pVdecExContext->stMVOPInfo[u8Idx]),(void*)(pMVOPInfo),sizeof(VDEC_EX_MVOPInfo));
                }
            }
            else
                ret = E_VDEC_EX_FAIL;
       ...
       case E_VDEC_EX_V2_USER_CMD_DISABLE_DIVX:
            if(param != NULL)
                ret = _MApi_VDEC_EX_DisableDivx(pStreamId,(MS_BOOL)(*((MS_U32*)(param[0]))));
            break;
       #endif
       case E_VDEC_EX_V2_USER_CMD_SET_PUSI_SETTING:
            ret = _MApi_VDEC_EX_SetPUSISetting(pStreamId,(*((MS_BOOL*)(param[0]))));
            break;
       default:
...
VDEC_EX_Result MApi_VDEC_EX_V2_GetControl(VDEC_StreamId* pStreamId,VDEC_EX_V2_User_Cmd cmd_id,void** param)
{
    ...
    _VDEC_SHM_POINTER_CHECK(E_VDEC_EX_FAIL);
#if EVENT_PATCH
    if(cmd_id == E_VDEC_EX_V2_USER_CMD_GET_EVENT_FLAG)
    {
        return _MApi_VDEC_EX_V2_GetEventFlag(pStreamId,param[0] ,(*(MS_U32*)param[1]) );
    }
#endif
    _VDEC_NOT_INIT_RET(u8Idx);
    ...
    switch (cmd_id)
    {
        ...
        case E_VDEC_EX_V2_USER_CMD_GET_VSYNC_BRIDGE_ADDR:
            eRet = _VDEC_EX_GetVsyncBridgeAddr(pStreamId, ((MS_U32*)(param[0])));
            break;
        ...
        case E_VDEC_EX_V2_USER_CMD_GET_SIMPLE_FRAME:
            eRet = _MApi_VDEC_EX_V2_GetSimpleDecFrame(pStreamId,((VDEC_EX_FrameInfo*)(param[0])));
            break;
        ...
        case E_VDEC_EX_V2_USER_CMD_GET_SECURE_MODE:
            eRet = _MApi_VDEC_EX_V2_IsSecureMode((MS_BOOL*)(param[0]));
            break;
        ...
        case E_VDEC_EX_V2_USER_CMD_GET_VSYNC_BRIDGE_EXT_ADDR:
            eRet = _VDEC_EX_GetVsyncBridgeExtAddr(pStreamId, ((MS_U32*)(param[0])));
            break;
        case E_VDEC_EX_V2_USER_CMD_GET_IS_STR_RESUME:
            eRet = _MApi_VDEC_EX_V2_IsSTRResume(pStreamId);
            break;
        case E_VDEC_EX_V2_USER_CMD_GET_IS_STR:
            eRet = _MApi_VDEC_EX_V2_IsSTR(pStreamId);
            break;
        case E_VDEC_EX_V2_USER_CMD_GET_IS_SEAMLESS_MODE:
            u8Idx = _VDEC_GET_IDX(pStreamId->u32Id);
            _VDEC_SHM_POINTER_CHECK(E_VDEC_EX_FAIL);
            eRet = (pVdecExContext->_Pre_Ctrl[u8Idx].bDSEnable == TRUE)?E_VDEC_EX_OK:E_VDEC_EX_FAIL;
            break;
        case E_VDEC_EX_V2_USER_CMD_GET_GST_TIMING:
            if(param[0] != NULL)
            {
                u8Idx = _VDEC_GET_IDX(pStreamId->u32Id);
                memcpy((void*)(param[0]),(void*)&pVdecExContext->stMVOPInfo[u8Idx],sizeof(VDEC_EX_MVOPInfo));
            }
            else
                eRet = E_VDEC_EX_FAIL;
            break;
        case E_VDEC_EX_V2_USER_CMD_IS_MORE_FRAME:
            eRet = _MApi_VDEC_EX_V2_IsMoreDecFrame(pStreamId);
            break;
        default:
            eRet = E_VDEC_EX_RET_ILLEGAL_ACCESS;
            break;
...
VDEC_EX_Result MApi_VDEC_EX_PreSetControl(VDEC_StreamId *pStreamId, VDEC_EX_User_Cmd cmd_id, MS_U32 param)
{
    ...
    IO_arg.StreamID = (VDEC_EX_V2_StreamId*) pStreamId;
    IO_arg.eUserCmd = _VDEC_MapUserCmd_V2(cmd_id);
    if((IO_arg.eUserCmd == E_VDEC_EX_V2_USER_CMD_CONNECT_DISPLAY_PATH) ||
       (IO_arg.eUserCmd == E_VDEC_EX_V2_USER_CMD_HVD_IAPGN_BUF_SHARE_BW_MODE))
    {
        IO_arg.param[0] = (void*)(param);
    }
    else
...
VDEC_EX_Result MApi_VDEC_EX_V2_PreSetControl(VDEC_StreamId* pStreamId,VDEC_EX_V2_User_Cmd cmd_id,void** param)
{
    ...
    switch (cmd_id)
    {
        ...
        case E_VDEC_EX_V2_USER_CMD_PRESET_DECODE_MODE:
             eRet = _VDEC_EX_SetDecodeMode(pStreamId,(VDEC_EX_DecModCfg *)param[0]);
             break;
        ...
        case E_VDEC_EX_V2_USER_CMD_HVD_IAPGN_BUF_SHARE_BW_MODE:
            ...
            VDEC_EX_IapGnBufShareBWMode* pBufShareBW = (VDEC_EX_IapGnBufShareBWMode*)param[0];
            pVdecExContext->_Pre_Ctrl[u8Idx].stIapGnShBWMode.bEnable = pBufShareBW->bEnable;
            pVdecExContext->_Pre_Ctrl[u8Idx].stIapGnShBWMode.u32IapGnBufAddr= pBufShareBW->u32IapGnBufAddr;
            pVdecExContext->_Pre_Ctrl[u8Idx].stIapGnShBWMode.u32IapGnBufSize= pBufShareBW->u32IapGnBufSize;
         ...
         case E_VDEC_EX_V2_USER_CMD_FORCE_START_NON_I_SLICE:
             ...
             pVdecExContext->_Pre_Ctrl[u8Idx].bEnableForceStartNonISlice = (MS_BOOL)(*((MS_U32*)(param[0])));
             break;
         ...
         case E_VDEC_EX_V2_USER_CMD_THUMBNAIL_MODE:
            ...
            pVdecExContext->_Pre_Ctrl[u8Idx].bEnableThumbnailMode = (MS_BOOL)(*((MS_U32*)(param[0])));
            break;
         case E_VDEC_EX_V2_USER_CMD_ENABLE_SEAMLESS_MODE:
            u8Idx = _VDEC_GET_IDX(pStreamId->u32Id);
            _VDEC_SHM_POINTER_CHECK(E_VDEC_EX_FAIL);
            pVdecExContext->_Pre_Ctrl[u8Idx].bDSEnable = (MS_BOOL)(*((MS_U32*)(param[0])));
            break;
         case E_VDEC_EX_V2_USER_CMD_CONNECT_DISPLAY_PATH:
            u8Idx = _VDEC_GET_IDX(pStreamId->u32Id);
            _VDEC_SHM_POINTER_CHECK(E_VDEC_EX_FAIL);
            VDEC_EX_DynmcDispPath* pParam = ((VDEC_EX_DynmcDispPath*)(param[0]));
            pVdecExContext->_Pre_Ctrl[u8Idx].stPreConnectDispPath.bEnable = TRUE;
            pVdecExContext->_Pre_Ctrl[u8Idx].stPreConnectDispPath.stDynmcDispPath.bConnect = pParam->bConnect;
            pVdecExContext->_Pre_Ctrl[u8Idx].stPreConnectDispPath.stDynmcDispPath.eMvopPath = pParam->eMvopPath;
            break;
         default:
             eRet = E_VDEC_EX_RET_ILLEGAL_ACCESS;
             break;
...
MS_BOOL _VDEC_EX_TEE_GetVPUProtect(VDEC_EX_TEE_VPU_Param* param)
{
    ...
    else
    {
        if(Bitstream_StartAddr_Main != 0 && Bitstream_StartAddr_Sub != 0)
        {
            ...
            param->ProtectInfo[0].u32Size = MDrv_HVD_EX_GetSHM_Offset()-0x1000;
...
void MApi_VDEC_EX_SetFrameBuffer(MS_U32 u32FrmBufAddr,MS_U32 u32FrmBufSize)
{
    ...
    printf("%s  u32FrmBufAutoAddr:%lx  u32FrmBufAutoSize:%lx\n",__FUNCTION__,u32FrmBufAutoAddr,u32FrmBufAutoSize);
}
//bFileMode:[0,DTV] [1,MM] [2,THUMBNAIL]
MS_BOOL MApi_VDEC_EX_GetFrmInfoByTask(MS_U8 u8TaskId,MS_BOOL bUHD,MS_BOOL bFileMode,MS_U32* u32FrmBufAddr,MS_U32* u32FrmBufSize)
{
    if(u32FrmBufAutoSize < MM_BUFFER_SIZE_UHD || u32FrmBufAutoSize < DTV_BUFFER_SIZE_UHD)
    {
        bUHD = FALSE;
    }
    if((u32FrmBufAddr && u32FrmBufSize)&&(u32FrmBufAutoAddr))
    {
        switch(bFileMode)
        {
            case E_VDEC_EX_MEM_DTV:
                *u32FrmBufSize = (bUHD)?DTV_BUFFER_SIZE_UHD:DTV_BUFFER_SIZE_FHD;
                break;
            case E_VDEC_EX_MEM_MM:
                *u32FrmBufSize = (bUHD)?MM_BUFFER_SIZE_UHD:MM_BUFFER_SIZE_FHD;
                break;
            case E_VDEC_EX_MEM_THUMBNAIL:
                *u32FrmBufSize = (bUHD)?THUMB_BUFFER_SIZE_UHD:THUMB_BUFFER_SIZE_FHD;
                break;
            default:
                break;
        }
        if(*u32FrmBufSize > u32FrmBufAutoSize)
        {
            *u32FrmBufSize = u32FrmBufAutoSize;
        }
        if(u8TaskId == 0)
        {
    ...
    {
        printf("%s  u8TaskId:%d  bFileMode:%d NULL pointer\n",__FUNCTION__,u8TaskId,bFileMode);
        return FALSE;
    }
}
#endif
