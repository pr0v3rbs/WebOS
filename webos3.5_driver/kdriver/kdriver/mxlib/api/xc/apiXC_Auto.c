...
#define   AUTO_GAIN_RANGE_Y_INTERNAL         643   //(235-16)*4 *0.5/0.7,
...
static MS_AUTOADC_TYPE _GetAverageDataFromMemory(void *pInstance, MS_WINDOW_TYPE* mem_win, SCALER_WIN eWindow)
{
    ...
    memset(&pMemInfo, 0, sizeof(MS_IMAGE_MEM_INFO));
    ...
    MS_PIXEL_32BIT *sMemBuf;
    
#ifdef MSOS_TYPE_LINUX_KERNEL
    sMemBuf = kmalloc(sizeof(MS_PIXEL_32BIT)*((pMemInfo.eCal_Win_Size.u16Xsize)*(pMemInfo.eCal_Win_Size.u16Ysize)), GFP_KERNEL);
#else
    //MS_PIXEL_32BIT sMemBuf[(pMemInfo.eCal_Win_Size.u16Xsize)*(pMemInfo.eCal_Win_Size.u16Ysize)];
    //memset(&sMemBuf, 0 , sizeof(sMemBuf) );
    sMemBuf = malloc(sizeof(MS_PIXEL_32BIT)*((pMemInfo.eCal_Win_Size.u16Xsize)*(pMemInfo.eCal_Win_Size.u16Ysize)));
#endif
    if(sMemBuf==NULL)
    {
        printf("[%s][%d] request mem size[0x%x] fail to get memory!!\n", __func__, __LINE__, (sizeof(MS_PIXEL_32BIT)*((pMemInfo.eCal_Win_Size.u16Xsize)*(pMemInfo.eCal_Win_Size.u16Ysize))));
    }
    else
    {
        memset(sMemBuf, 0 , sizeof(MS_PIXEL_32BIT)*((pMemInfo.eCal_Win_Size.u16Xsize)*(pMemInfo.eCal_Win_Size.u16Ysize)) );
...
#else
        _XC_ENTRY();
        MDrv_XC_wait_input_vsync(pInstance, 1, 40, eWindow);
        _XC_RETURN();
#endif //#if SC2_ENABLE
...
#else
        _XC_ENTRY();
        MDrv_XC_wait_input_vsync(pInstance, 1, 200, eWindow);
        _XC_RETURN();
#endif //#if SC2_ENABLE
...
static MS_BOOL _MApi_XC_Auto_Internal_AutoCablication(void* pInstance, XC_Auto_TuneType enAutoTuneType, APIXC_AdcGainOffsetSetting *pstADCSetting , SCALER_WIN eWindow)
{
    ...
    if ((MIRROR_V_ONLY == enMirrorBackup[eWindow]) || (MIRROR_HV == enMirrorBackup[eWindow]))
    {
        ...
        {
        MDrv_XC_SetFrameBufferAddress(pInstance, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.u32Main_FB_Start_Addr, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.u32Main_FB_Size-0x10, eWindow);
        }
        else
        {
             MDrv_XC_SetFrameBufferAddress(pInstance, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.u32Sub_FB_Start_Addr, pXCResourcePrivate->stdrvXC_MVideo_Context.g_XC_InitData.u32Sub_FB_Size-0x10, eWindow);
        }
    ...
    if (bResult)
    {
        ...
        if (bResult)
        {
            ...
            if (bResult)
            {
                ...
                if(u8FlowFlag == 0x3f)
                {
                    AUTOMSG(printf("VGA calibration succeed!!\n"));
                ...
                {
                    AUTOMSG(printf("VGA calibration fail!!\n"));
            ...
            {
                AUTOMSG(printf("External offset Calibration initial fail !! \n"));
            }
        }
        else
        {
            AUTOMSG(printf("ExternalTuneVgaOffset GetOffsetSetting fail!!\n"));
        }
    }
    else
    {
        AUTOMSG(printf("ExternalTuneVgaOffset GetGainSetting fail!!\n"));
    }
    return bResult;
}
#endif
    ...
    if (bResult)
    {
        ...
        if (bResult)
        {
            ...
            if (bResult)
            {
                ...
                {
                    AUTOMSG(printf("VGA calibration succeed!!\n"));
                    AUTOMSG(printf(" Gain R: %d	G: %d  B: %d \n",pstADCSetting->u16RedGain,
            ...
            {
                AUTOMSG(printf("External VGA Calibration initial fail !! \n"));
            }
        }
        else
        {
            AUTOMSG(printf("ExternalTuneVgaRGBGain GetOffsetSetting fail!!\n"));
        }
    }
    else
    {
        AUTOMSG(printf("ExternalTuneVgaRGBGain GetGainSetting fail!!\n"));
    }
    return bResult;
}
    ...
    if (bResult)
    {
        ...
        if (bResult)
        {
            ...
            if (bResult)
            {
                ...
                if (u8FlowFlag != 0x3f)
                {
                    AUTOMSG(printf("Calibration fail !! \n"));
            ...
            {
                AUTOMSG(printf("External Scart Calibration initial fail !! \n"));
            }
        }
        else
        {
            AUTOMSG(printf("ExternalTuneScartRGBGain GetOffsetSetting fail!!\n"));
        }
    }
    else
    {
        AUTOMSG(printf("ExternalTuneScartRGBGain GetGainSetting fail!!\n"));
    }
    return bResult;
}
...
static MS_BOOL _MApi_XC_Auto_ExternalTuneComponetYpbprGain(void *pInstance, MS_U8 u8VSyncTime, APIXC_AdcGainOffsetSetting *pstADCSetting)
{
    ...
    if (bResult)
    {
        ...
        if (bResult)
        {
            ...
            {
                AUTOMSG(printf("External YPbPr Calibration initial fail !! \n"));
            }
        }
        else
        {
            AUTOMSG(printf("ExternalTuneComponetYpbprGain GetOffsetSetting fail!!\n"));
        }
    }
    else
    {
        AUTOMSG(printf("ExternalTuneComponetYpbprGain GetGainSetting fail!!\n"));
    }
    return bResult;
}
...
MS_BOOL MApi_XC_Auto_GainOffset_U2(void* pInstance, XC_Auto_CalibrationType type, XC_Auto_TuneType enAutoTuneType,  APIXC_AdcGainOffsetSetting *pstADCSetting, SCALER_WIN eWindow)
{
    ...
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(psXCInstPri->u32DeviceID)],(void**)(&pXCResourcePrivate));
   if (E_XC_HW_BG_CALIBRATION == pXCResourcePrivate->stapiXC_Auto.g_enCalbrationMode)
    {
        MDrv_XC_ADC_EnableHWCalibration(pInstance,ENABLE);
        bReturn = MDrv_XC_ADC_SetCalibrationRefSource(pInstance,1);
        MDrv_XC_ADC_Source_Calibrate(pInstance,MDrv_XC_ADC_ConvertSrcToADCSrc(pInstance,INPUT_SOURCE_YPBPR));
#if SC2_ENABLE
                    _XC_SEMAPHORE_RETURN(pInstance,E_XC_ID_VAR);
...
MS_BOOL MApi_XC_Auto_SetCalibrationMode_U2(void* pInstance, XC_Auto_CalibrationMode eMode)
{
	...
	else if ((E_XC_HW_BG_CALIBRATION == eMode) && MDrv_XC_ADC_CheckHWCalibrationSupport(pInstance))
    {
        pXCResourcePrivate->stapiXC_Auto.g_enCalbrationMode = eMode;
...
MS_BOOL MApi_XC_GetAutoPositionWindow_U2(void* pInstance, MS_U8 u8ValidData, SCALER_WIN eWindow, MS_WINDOW_TYPE *pstAutoPositionWindow)
{
    ...
    if(IsSrcTypeHDMI(enInputSourceType) && (MDrv_XC_GetHdmiSyncMode(pInstance,eWindow) == HDMI_SYNC_DE))
    {
        bHdmiDeMode = TRUE;
...
void MApi_XC_SetADC_RB_Swap_U2(void* pInstance, MS_BOOL bEnable)
{
	...
	if(bEnable)
	{
    	MDrv_XC_ADC_SelectRGBYDataToScaler(E_ADC_COLOR_BLUE, E_ADC_COLOR_GREEN, E_ADC_COLOR_RED, E_ADC_COLOR_Y);
	}
	else
	{
    	MDrv_XC_ADC_SelectRGBYDataToScaler(E_ADC_COLOR_RED, E_ADC_COLOR_GREEN, E_ADC_COLOR_BLUE, E_ADC_COLOR_Y);
	}
...
void MApi_XC_SetADC_RB_Swap(MS_BOOL bEnable)
{
    if (pu32XCInst == NULL)
    {
        printf("[%s,%5d]No instance existed, please get an instance by calling MApi_XC_Init() first\n",__FUNCTION__,__LINE__);
    ...
    MS_BOOL XCArgs = bEnable;

    if(UtopiaIoctl(pu32XCInst, E_XC_ADC_CMD_SET_RB_SWAP, (void*)&XCArgs) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Obtain XC engine fail\n");
        return;
    }
...
void MApi_XC_AUTO_GetSyncInfo_U2(void* pInstance,XC_AUTO_SYNC_INFO *pSyncInfo)
{
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_AUTOGEOMETRY,"\n======== Get SyncInfo:Polarity=%u,HSyncStart=%ld,VSyncStart=%ld ======== \n",pSyncInfo->eSyncPolarity, pSyncInfo->u32HSyncStart,pSyncInfo->u32HSyncEnd);
}
