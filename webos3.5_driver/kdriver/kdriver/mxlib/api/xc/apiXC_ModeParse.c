...
#if 0 //test AP for detect the similar resolution index
{
    ...
    for(ucIndex=0; ucIndex<u8NumberOfItems; ucIndex++, pStdModeTbl++)
    {
        ...
        MODE_DBG(printf("[%d][%d][%d][%d][%d][%d][%d][%d][%d]\n",pStdModeTbl->enResolutionIndex,pStdModeTbl->u16HorizontalFrequency,pStdModeTbl->u16VerticalFrequency,pStdModeTbl->u16HorizontalStart,pStdModeTbl->u16VerticalStart,pStdModeTbl->u16HorizontalTotal,pStdModeTbl->u16VerticalTotal,pStdModeTbl->u8VTotalTolerance,pStdModeTbl->u8AdcPhase);)
...
//#endif
    ...
    #if 0 // LG not need to use user mode
    else
    {
...
//          printf("This mode is USER MODE \n");
    ...
    {
        MODE_DBG(printf("did not support user mode ucMatchIndex = %d\n",ucMatchIndex));
        return ucMatchIndex;
    }
    #endif
...
MS_U16 MApi_XC_ModeParse_GetHResolution(EN_RESOLUTION_TYPE enReolution)
{
    return astStandardModeResolution[enReolution].u16DisplayWidth;
}

MS_U16 MApi_XC_ModeParse_GetVResolution(EN_RESOLUTION_TYPE enReolution)
{
    return astStandardModeResolution[enReolution].u16DisplayHeight;
}
