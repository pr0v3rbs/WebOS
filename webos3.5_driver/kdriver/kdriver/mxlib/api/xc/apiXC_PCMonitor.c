...
#define STABLE_COUNTER        20

static MS_BOOL gIsADC_Freerun = FALSE;
...
static void _MApi_XC_PCMonitor_FPi_Patch(void *pInstance, SCALER_WIN eWindow)
{
    ...
    else if ( ( sWin.height >  ( DOUBLEHD_576X2I_VSIZE - 5) )
              && ( sWin.height <  ( DOUBLEHD_576X2I_VSIZE + 5) )
              && ( sWin.width <  ( DOUBLEHD_576X2I_HSIZE + 5) )
              && ( sWin.width >  ( DOUBLEHD_576X2I_HSIZE - 5) ) )
    {
        MDrv_SC_ForceInterlaceInPMode(pInstance, ENABLE,eWindow);
        MsOS_DelayTask(150);
    }
    else if ( ( sWin.height >  ( DOUBLEHD_480X2I_VSIZE - 5) )
              && ( sWin.height <  ( DOUBLEHD_480X2I_VSIZE + 5) )
              && ( sWin.width <  ( DOUBLEHD_480X2I_HSIZE + 5) )
              && ( sWin.width >  ( DOUBLEHD_480X2I_HSIZE - 5) ) )
    {
        MDrv_SC_ForceInterlaceInPMode(pInstance, ENABLE,eWindow);
        MsOS_DelayTask(150);
...
static void _MApi_XC_PCMonitor_InitRegInTimingChange(void *pInstance, SCALER_WIN eWindow)
{
    ...
    {
        MDrv_XC_SoftwareReset(pInstance,REST_IP_OFFLINE, eWindow);
    }
    else
    {
...
static void _MApi_XC_PCMonitor_PrepareForTimingChange(void *pInstance, SCALER_WIN eWindow)
{
    ...
    {
        MApi_XC_InitRegInTimingChange_Post(pInstance, stPCMonitorInfo.u8SyncStatus );
    }
...
MS_U16 MDrv_XC_PCMonitor_Get_VFreqx10(void *pInstance, SCALER_WIN eWindow)
{
    ...
    UtopiaResourceGetPrivate(g_pXCResource[_XC_SELECT_INTERNAL_VARIABLE(psXCInstPri->u32DeviceID)],(void**)(&pXCResourcePrivate));
    stPCMonitorInfo = MDrv_XC_PCMonitor_GetStatus(pInstance, eWindow);
    static MS_U16 u16AccurateVFreqback = 0;
    ...
    else
    {
        if(FALSE == (pXCResourcePrivate->stdrvXC_MVideo_Context.g_bFast_Get_VFreq))
        {
...
#if 1//debouce small diff from frontend.
            if(abs(u16AccurateVFreqback-u16AccurateVFreq)>1)
            {
                u16AccurateVFreqback = u16AccurateVFreq;
            }
            else
            {
                u16AccurateVFreq = u16AccurateVFreqback;
            }
#endif
            ...
            if((abs(u16VFreq - u16AccurateVFreq) < ACCURATE_VFREQ_TOLERANCE )
                ...
                &&( !IsSrcTypeHDMI(stPCMonitorInfo.eCurrentSrc))
                )
            {
...
MS_U32 MDrv_XC_PCMonitor_Get_FrameRatex1K(void* pInstance, SCALER_WIN eWindow)
{
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_MODEPARSE, "u32VFreq = %lu\n", u32VFreq);

    // for more correct VPeriod
    u8VSyncTime = stPCMonitorInfo.u16VSyncTime;
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_MODEPARSE, "u32AccurateVPeriod = %lu\n", u32AccurateVPeriod);
    ...
    XC_LOG_TRACE(XC_DBGLEVEL_MODEPARSE, "u32AccurateVFreq = %lu\n", u32AccurateVFreq);

    XC_LOG_TRACE(XC_DBGLEVEL_MODEPARSE, "InputVFreq=%lu->%lu, Tolerance=%lu\n", u32VFreq, u32AccurateVFreq, (MS_U32)ACCURATE_VFREQ_TOLERANCE * 100);
...
MS_U16 MDrv_XC_PCMonitor_Get_Htotal(void *pInstance,SCALER_WIN eWindow)
{
    XC_PCMONITOR_HK_INFO stPCMonitorInfo;
    stPCMonitorInfo = MDrv_XC_PCMonitor_GetStatus(pInstance,eWindow);
    if((stPCMonitorInfo.eCurrentState == E_XC_PCMONITOR_UNSTABLE) || (stPCMonitorInfo.eCurrentState == E_XC_PCMONITOR_STABLE_NOSYNC))
    ...
    {
        return stPCMonitorInfo.u16HTotal;
    }
}
...
MS_U16 MApi_XC_PCMonitor_Get_Htotal_U2(void* pInstance, SCALER_WIN eWindow)
{
    ...
    u16Return = MDrv_XC_PCMonitor_Get_Htotal(pInstance, eWindow);
#if PCMONITOR_REFINE
    _PCMonitor_RETURN(pInstance);
#else
#if SC2_ENABLE
...
MS_U16 MApi_XC_PCMonitor_Get_Htotal(SCALER_WIN eWindow)
{
    if (pu32XCInst == NULL)
    {
        printf("[%s,%5d]No instance existed, please get an instance by calling MApi_XC_Init() first\n",__FUNCTION__,__LINE__);
    ...
    stXC_PCMONITOR_GET_HTOTAL XCArgs;
    XCArgs.eWindow = eWindow;
    XCArgs.u16ReturnValue = 0;

    if(UtopiaIoctl(pu32XCInst, E_XC_PCMONITOR_CMD_GET_HTOTAL, (void*)&XCArgs) != 0)
    {
        printf("Obtain XC engine fail\n");
        return 0;
    }
...
//#endif
    ...
    {
        PCMSG(printf("Restart src %d, t=%lu\n", eCurrentSrc, MsOS_GetSystemTime()));
#if PCMONITOR_REFINE
    _PCMonitor_ENTRY(pInstance);
#else
#if SC2_ENABLE
    ...
    if( (IsSrcTypeHDMI(stPCMonitorInfo.eCurrentSrc) || IsSrcTypeDVI(eCurrentSrc) )
            &&(MDrv_XC_GetHdmiSyncMode(pInstance,eWindow) == HDMI_SYNC_HV)
            &&(eWindow == MAIN_WINDOW) )
    {
    ...
    if ( stPCMonitorInfo.bInputTimingChange == FALSE ) // check timing change flag have be setting
    {
        if ( MDrv_XC_PCMonitor_SyncLoss(pInstance, eWindow) ) // no sync
        {
            ...
            if ( _MApi_XC_PCMonitor_DetectSync(pInstance, stPCMonitorInfo.eCurrentSrc,eWindow) ) // check signal active
            {
                ...
                stPCMonitorInfo.bInputTimingChange = TRUE;
                MDrv_XC_PCMonitor_SetStatus(pInstance, stPCMonitorInfo, eWindow);
                PCMSG(if(bNoSyncStatus) printf("NewSync t=%lu\n", MsOS_GetSystemTime()); bNoSyncStatus = FALSE);
            }
            else
            {
                PCMSG(if(!bNoSyncStatus) printf("NoSync t=%lu\n", MsOS_GetSystemTime()); bNoSyncStatus = TRUE);
            }
        }
        else //have sync?
        {
            ...
            if((gSrcInfo[eWindow].bAutoNoSignalEnabled && pXCResourcePrivate->stapiXC_PCMonitor._bEnableAutoNoSignalDetect[eWindow])
                ||(gSrcInfo[eWindow].bBlackscreenEnabled && pXCResourcePrivate->stapiXC_PCMonitor._bEnableAutoNoSignalDetect[eWindow]))
            {
                // check invalid
                ...
                else // time stable
                {
                    if( gSrcInfo[eWindow].bAutoNoSignalEnabled )
                    {
                        ...
                        if( MDrv_XC_IsInputSourceDisabled(pInstance, eWindow)&&(pXCResourcePrivate->stapiXC_PCMonitor.u8DeNoiseCount[eWindow] != 0)
                                &&(stPCMonitorInfo.eCurrentState == E_XC_PCMONITOR_STABLE_SYNC)
                                && ((pXCResourcePrivate->stapiXC_PCMonitor.u8DeStableCnt[eWindow]>8)))
                        {
                            MDrv_XC_DisableInputSource(pInstance, DISABLE, eWindow); //enable the input source
                            //pXCResourcePrivate->stapiXC_PCMonitor.u8DeStableCnt[eWindow] = 0;
                        }
                        ...
                        {
                            if(pXCResourcePrivate->stapiXC_PCMonitor.u8DeStableCnt[eWindow] < STABLE_COUNTER)
                            {
                    ...
                    if(pXCResourcePrivate->stapiXC_PCMonitor.u8DeStableCnt[eWindow] >= STABLE_COUNTER) // to check the input source status
                    {
                        ...
                        else//For mantis 81460, counter doesn't reset
                        {

                            if((pXCResourcePrivate->stapiXC_PCMonitor.u8DeStableCnt[eWindow] == STABLE_COUNTER) && (IsSrcTypeYPbPr(stPCMonitorInfo.eCurrentSrc)))
                            {
                                pXCResourcePrivate->stapiXC_PCMonitor.u8DeStableCnt[eWindow] = 0;
                            }
                        }
            ...
            {
	                PCMSG(printf("InvalidTimingDetect GO TO UNSTABLE! t=%lu\n", MsOS_GetSystemTime())) ;
	                stPCMonitorInfo = MDrv_XC_PCMonitor_GetStatus(pInstance, eWindow);
	                stPCMonitorInfo.u8SyncStatus |= XC_MD_SYNC_LOSS;
	                stPCMonitorInfo.bInputTimingChange = TRUE;
	                MDrv_XC_PCMonitor_SetStatus(pInstance, stPCMonitorInfo, eWindow);
    ...
    {
        PCMSG(printf("TimingChg , t=%lu\n", MsOS_GetSystemTime()));
        stPCMonitorInfo = MDrv_XC_PCMonitor_GetStatus(pInstance, eWindow);
        stPCMonitorInfo.bInputTimingChange = FALSE;
        stPCMonitorInfo.ucTimingChangeDebounce++;

        PCMSG(printf("Deb=%d, t=%lu\n", stPCMonitorInfo.ucTimingChangeDebounce, MsOS_GetSystemTime()));

        if ( !MDrv_XC_PCMonitor_SyncLoss(pInstance, eWindow) ) // have signal
        {
            if ( IsSrcTypeYPbPr(stPCMonitorInfo.eCurrentSrc ) && (stPCMonitorInfo.ucTimingChangeDebounce <= 3) ) // prevent from SOG
            {
                stPCMonitorInfo.u8SyncStatus |= XC_MD_SYNC_LOSS;
                MDrv_XC_PCMonitor_SetStatus(pInstance, stPCMonitorInfo, eWindow);
#if PCMONITOR_REFINE
    ...
    else // input timing is stable
    {
        ...
        if ( !stPCMonitorInfo.bInputTimingStable )
        {
                    ...
                    (MDrv_XC_PCMonitor_SyncLoss(pInstance, eWindow) && (stPCMonitorInfo.ucInputTimingStableCounter >= pXCResourcePrivate->stapiXC_PCMonitor._u16TimingNosyncCounter[eWindow])) )
            {
                ...
                PCMSG(printf("pXCResourcePrivate->stapiXC_PCMonitor._u16TimingStableCounter[eWindow]) = %d\n", pXCResourcePrivate->stapiXC_PCMonitor._u16TimingStableCounter[eWindow]));
                PCMSG(printf("pXCResourcePrivate->stapiXC_PCMonitor._u16TimingNosyncCounter[eWindow]) = %d\n", pXCResourcePrivate->stapiXC_PCMonitor._u16TimingNosyncCounter[eWindow]));
                ...
                else
                {
                    ...
                    if (( IsSrcTypeYPbPr(stPCMonitorInfo.eCurrentSrc ) ||
                            IsSrcTypeVga(stPCMonitorInfo.eCurrentSrc ) ) && gIsADC_Freerun == TRUE )
                    {
                        // When timing is stable , ADC needs to change to non-freerun mode.
                        MDrv_XC_ADC_Set_Freerun(pInstance, FALSE);
                        gIsADC_Freerun = FALSE;
                    }
	            ...
	            MDrv_XC_PCMonitor_Get_HFreq(pInstance, ENABLE,eWindow);
                    //_MApi_XC_PCMonitor_ModeParse(eWindow);
                }

                PCMSG(printf("*** Do Mode In Analog Path: stable time =%lu ***\n", MsOS_GetSystemTime()));
#if PCMONITOR_REFINE
    _PCMonitor_RETURN(pInstance);
#else
#if SC2_ENABLE
...
XC_PCMONITOR_STATUS MApi_XC_PCMonitor(INPUT_SOURCE_TYPE_t eCurrentSrc, SCALER_WIN eWindow)
{
 ...
 MS_U16 MDrv_XC_PCMonitor_Get_HFreq(void *pInstance,MS_BOOL bSet,SCALER_WIN eWindow)
{
	static MS_U16 u16StableHfrq=0;

	if(bSet)
	{
		u16StableHfrq = Hal_SC_ip_get_horizontalPeriod(pInstance, eWindow);

		if( (u16StableHfrq == MST_H_PERIOD_MASK) || (u16StableHfrq < 10) )
			u16StableHfrq = 0;
	}
	
	return u16StableHfrq;
