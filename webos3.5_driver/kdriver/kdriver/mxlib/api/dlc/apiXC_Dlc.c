...
inline void MApi_XC_DLC_DecodeExtCmd_U2(void * pInstance,tDLC_CONTROL_PARAMS * params)
{
...
inline void MApi_XC_DLC_DecodeExtCmd(tDLC_CONTROL_PARAMS* params)
{
    if (pu32DLCInst == NULL)
    {
        if(UtopiaOpen(MODULE_DLC, &pu32DLCInst, 0, NULL) != UTOPIA_STATUS_SUCCESS)
...
inline void MApi_XC_DBC_DecodeExtCmd_U2(void * pInstance,tDBC_CONTROL_PARAMS * pValue)
{
    msDBC_ParametersTool(pValue->pCmdBuff, pValue->fnDBC_AP_OnOff, pValue->fnDBC_AdjustBacklight, pValue->fnDBC_Putchar);
}

inline void MApi_XC_DBC_DecodeExtCmd(tDBC_CONTROL_PARAMS* pValue )
{
    if (pu32DBCInst == NULL)
    {
        if(UtopiaOpen(MODULE_DLC, &pu32DBCInst, 0, NULL) != UTOPIA_STATUS_SUCCESS)
...
MS_U32 MApi_XC_DLC_SetPowerState_U2(void* pInstance, EN_POWER_MODE enPowerState)
{
    ...
    else if (enPowerState == E_POWER_RESUME)
    {
        ...
        {
            MApi_XC_DLC_InitCurve(g_DlcCapRange_Suspend.wHStart, g_DlcCapRange_Suspend.wHEnd, g_DlcCapRange_Suspend.wVStart, g_DlcCapRange_Suspend.wVEnd);
            _enPrevPowerState = enPowerState;
            u32Ret = UTOPIA_STATUS_SUCCESS;
        }
        else
