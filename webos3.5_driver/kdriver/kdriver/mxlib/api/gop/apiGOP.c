...
void* pAttributeGOP_XC = NULL;
XC_INITDATA XC_InitData;

#define CheckXCOpen()   do{\
     if(pInstantGOP_XC==NULL)\
     {\
        if(UtopiaOpen(MODULE_XC, &pInstantGOP_XC, 0, &pAttributeGOP_XC) != UTOPIA_STATUS_SUCCESS)\
        {\
...
#define GOP_ENTRY()                 _GOP_Entry()
//{if(TRUE != MDrv_GOP_GWIN_BeginDraw()){GOP_ASSERT(FALSE, printf("\nError: Obtain GOP mutex fail\n"));}}
#define GOP_RETURN(_ret)             return _ret;//{if(TRUE == MDrv_GOP_GWIN_EndDraw()){return _ret;}else{GOP_ASSERT(FALSE, printf("\nError: Release GOP mutex fail\n"));return _ret;}}
...
E_GOP_API_Result Mapi_GOP_GWIN_ResetGOP(MS_U32 u32Gop)
{
    ...
    ioctl_info.pSetting = (MS_U32*)&i;
    ioctl_info.u32Size  = sizeof(int);
...
int _GOP_RegisterAllCBFunc(void);
...
E_GOP_API_Result MApi_GOP_GWIN_SetTransClr_8888_EX(MS_U8 u8GOP,MS_U32 clr, MS_U32 mask)
{
    ...
    ioctl_info.pSetting = (MS_U32*)&trans_set;
...
E_GOP_API_Result MApi_GOP_GWIN_OutputColor_EX(MS_U8 u8GOP,EN_GOP_OUTPUT_COLOR type)
{
    ...
    ioctl_info.gop_idx  = u8GOP;
    ioctl_info.pSetting = (MS_U32*)&output;
...
MS_U16 MApi_GOP_DWIN_GetIntrStatus(void)
{
    ...
    ioctl_info.pIntrInfo = (MS_U32*)&intrStatus;
...
E_GOP_API_Result MApi_GOP_GWIN_SetAlphaInverse_EX(MS_U8 u8GOP,MS_BOOL bEnable)
{
    ...
    ioctl_info.gop_idx  = u8GOP;
    ioctl_info.pSetting = (MS_U32*)&bAlphaInv;
    ioctl_info.u32Size  = sizeof(MS_BOOL);
...
E_GOP_API_Result MApi_GOP_GWIN_SetBnkForceWrite(MS_U8 u8GOP, MS_BOOL bEnable)
{
    ...
    ioctl_info.gop_idx  = u8GOP;
    ioctl_info.pSetting = (MS_U32*)&bEna;
    ioctl_info.u32Size  = sizeof(MS_BOOL);
...
E_GOP_API_Result MApi_GOP_GWIN_2GSetPaletteOpt_EX(MS_U8 u8GOP,GOP_PaletteEntry *pPalArray, MS_U16 u32PalStart, MS_U16 u32PalEnd)
{
    ...
    printf("\33[0;36m   %s: #define CONFIG_GOP_2ND_PAL not opened\33[m \n",__FUNCTION__);
...
E_GOP_API_Result MApi_GOP_SetGOPClk(MS_U8 gopNum, EN_GOP_DST_TYPE eDstType)
{
    ...
    ioctl_info.pSetting = (MS_U32*)&eType;
...
E_GOP_API_Result MApi_GOP_GWIN_EnableProgressive_EX(MS_U8 u8GOP,MS_BOOL bEnable)
{
    ...
    ioctl_info.gop_idx  = u8GOP;
    ioctl_info.pSetting = (MS_U32*)&Progressive;
    ioctl_info.u32Size  = sizeof(MS_BOOL);
...
E_GOP_API_Result MApi_GOP_DWIN_EnableR2YCSC(MS_BOOL bEnable)
{
    ...
    ioctl_info.pDwinProp = (MS_U32*)&bEna;
    ioctl_info.u32Size = sizeof(MS_BOOL);
...
E_GOP_API_Result MApi_GOP_DWIN_SetAlphaValue(MS_U8 u8AlphaVal)
{
    ...
    ioctl_info.pDwinProp = (MS_U32*)&u8Value;
...
MS_U8 MApi_GOP_FB_Get(void)
{
    ...
    ioctl_info.FBId = 0x0;
    ioctl_info.pSet = (MS_U32*)&u8PoolId;
    ioctl_info.u32Size = sizeof(MS_U8);
...
E_GOP_API_Result MApi_GOP_GWIN_SetGWinShared(MS_U8 winId, MS_BOOL bIsShared)
{
    ...
    ioctl_info.pSet = (MS_U32*)&bShared;
    ioctl_info.u32Size = sizeof(MS_BOOL);
...
MS_U8 MApi_GOP_GWIN_CreateStaticWin(MS_U8 u8WinId, MS_U16 width, MS_U16 height, MS_U16 fbFmt)
{
    ...
    u8WinId = MApi_GOP_GWIN_GetFreeWinID();
...
MS_U8 MApi_GOP_GWIN_IsGwinExist(MS_U8 gId)
{
    ...
    ioctl_info.pSet = (MS_U32*)&u8Exist;
    ioctl_info.u32Size = sizeof(MS_U8);
...
E_GOP_API_Result MApi_GOP_GWIN_Enable(MS_U8 winId, MS_BOOL bEnable)
{
    ...
    ioctl_info.GwinId = winId;
    ioctl_info.pSet = (MS_U32*) &bEnable;
    ioctl_info.u32Size = sizeof(MS_BOOL);
...
E_GOP_API_Result MApi_GOP_GWIN_SetWinProperty(GOP_GwinDispProperty WinProperty)
{
    ...
    MApi_GOP_GWIN_GetWinInfo(WinProperty.gId,&gWin);
    MApi_GOP_GWIN_Get32FBInfo(u32fbId,  &fbInfo);
...
E_GOP_API_Result MApi_GOP_GWIN_Switch2Gwin(MS_U8 winId)
{
    ...
    memset(pCBFmtInfo,0x0,sizeof(GOP_CBFmtInfo)); //Coverity 213966,213967,213968
...
#if GOP_UTOPIA2K
    GFX_BufferInfo bufInfo;
    memset(&bufInfo, 0x0, sizeof(GFX_BufferInfo));//Coverity 213965
    bufInfo.u32Addr = pCBFmtInfo->u32Addr;
    bufInfo.u32Pitch = pCBFmtInfo->u16Pitch;
    bufInfo.u32ColorFmt = pCBFmtInfo->u16Fmt;
    MApi_GFX_SetDstBufferInfo(&bufInfo, 0);
#endif
    free(pCBFmtInfo);
...
E_GOP_API_Result MApi_GOP_GWIN_SwitchGOP(MS_U8 u8GOP)
{
    ...
    GOP_ENTRY();
    pCBFmtInfo = (PGOP_CBFmtInfo)malloc(sizeof(GOP_CBFmtInfo));
    memset(pCBFmtInfo,0x0,sizeof(GOP_CBFmtInfo)); //Coverity 213969,213970
...
#if GOP_UTOPIA2K
    GFX_BufferInfo bufInfo;
	memset(&bufInfo, 0x0, sizeof(GFX_BufferInfo));
    bufInfo.u32Addr = pCBFmtInfo->u32Addr;
    bufInfo.u32Pitch = pCBFmtInfo->u16Pitch;
    bufInfo.u32ColorFmt = pCBFmtInfo->u16Fmt;
    MApi_GFX_SetDstBufferInfo(&bufInfo, 0);
...
MS_U8 MApi_GOP_GWIN_GetFreeWinID(void)
{
    ...
    ioctl_info.pSet = (MS_U32*)&u8FreeId;
    ioctl_info.u32Size = sizeof(MS_U8);
...
MS_U8 MApi_GOP_GWIN_GetFreeFBID(void)
{
    ...
    {
        printf( "[Warning] %s( u32FreeFBId:%ld....)out of MS_U8 bound, please to use MApi_GOP_GWIN_GetFree32FBID()\n",__FUNCTION__,u32FreeFBId );
    }
...
MS_U32 MApi_GOP_GWIN_GetFree32FBID(void)
{
    ...
    {
        if(!MApi_GOP_GWIN_Is32FBExist(i))
        {
            u32FBId = i;
            break;
        }
...
MS_U8 MApi_GOP_GWIN_GetMAXFBID(void)
{
    ...
    {
        printf( "[Warning] %s( u32MaxFBID:%ld....)out of MS_U8 bound, please to use MApi_GOP_GWIN_GetMAX32FBID()\n",__FUNCTION__,u32MaxFBID );
    }
    return u32MaxFBID;
}
...
MS_U8 MApi_GOP_GWIN_GetMaxGOPNum(void)
{
    ...
    ioctl_info.pStatus = (MS_U32*) &u8MaxGop;
...
MS_U8 MApi_GOP_GWIN_GetTotalGwinNum(void)
{
    ...
    ioctl_info.pStatus = (MS_U32*)&u8TotalWin;
    ioctl_info.u32Size = sizeof(MS_U8);
    if (UtopiaIoctl(pInstantGOP,MAPI_CMD_GOP_GET_STATUS,(void*)&ioctl_info) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Ioctl %s fail\n",__FUNCTION__);
...
E_GOP_API_Result MApi_GOP_GWIN_EnableMultiAlpha(MS_U32 u32GopIdx, MS_BOOL bEnable)
{
    ...
    ioctl_info.pSetting = (MS_U32*)&bEnable;
    ioctl_info.u32Size = sizeof(MS_BOOL);
    if (UtopiaIoctl(pInstantGOP,MAPI_CMD_GOP_SET_PROPERTY,(void*)&ioctl_info) != UTOPIA_STATUS_SUCCESS)
    {
        printf("Ioctl %s fail\n",__FUNCTION__);
...
E_GOP_API_Result MApi_GOP_GWIN_SetPreAlphaMode(MS_U8 u8GOP, MS_BOOL bEnble)
{
    ...
    ioctl_info.gop_idx  = u8GOP;
    ioctl_info.pSetting = (MS_U32*)&bPreAlpha;
    ioctl_info.u32Size  = sizeof(MS_BOOL);
...
E_GOP_API_Result MApi_GOP_GWIN_SetPalette(void)
{
    ...
    for (i=0;i<(256/PALETTE_ENTRY_NUM);i++)
    {
        ...
        palEntry.pClut = (MS_U32*)&PaletteEntry;
        palEntry.start = i;
...
MS_U8 MApi_GOP_GWIN_GetFBfromGWIN(MS_U8 gwinId)
{
    ...
    {
        printf( "[Warning] %s( u32CurFBId:%ld....)out of MS_U8 bound, please to use MApi_GOP_GWIN_Get32FBfromGWIN()\n",__FUNCTION__,u32CurFBId );
    }
    return u32CurFBId;
}
...
MS_U16 MApi_GOP_GWIN_Get32FBFmt(MS_U32 u32fbId)
{
   ...
   GOP_ENTRY();
    ...
    {
        MS_DEBUG_MSG( printf("MApi_GOP_GWIN_Get32FBFmt: u32fbId=%ld is not in existence\n",u32fbId));
...
MS_U8 MApi_GOP_GWIN_GetCurrentFBID(void)
{
    ...
    if( u32CurFBID > 256) //Out of MS_U8
    {
        printf( "[Warning] %s( u32CurFBID:%ld....)out of MS_U8 bound, please to use MApi_GOP_GWIN_GetCurrent32FBID()\n",__FUNCTION__,u32CurFBID );
    }
    return u32CurFBID;
}
...
E_GOP_API_Result MApi_GOP_GWIN_SetFieldInver_EX(MS_U8 u8GOP,MS_BOOL enable)
{
    ...
    ioctl_info.gop_idx  = u8GOP;
    ioctl_info.pSetting = (MS_U32*)&bFieldInv;
    ioctl_info.u32Size  = sizeof(MS_BOOL);
...
E_GOP_API_Result MApi_GOP_Initialize_StretchWindow(MS_U8 u8GOPNum)
{
    ...
    printf("\33[0;36m   %s: #define CONFIG_GOP_GWIN_MISC not opened\33[m \n",__FUNCTION__);
    return GOP_API_FUN_NOT_SUPPORTED;
#else
...
E_GOP_API_Result MApi_GOP_GWIN_SetHMirror_EX(MS_U8 u8GOP,MS_BOOL bEnable)
{
    ...
    MS_BOOL bHMirror=0, bVMirror=0;
    GOP_ENTRY();
    memset(&ioctl_info, 0x0, sizeof(GOP_SETMIRROR_PARAM));

    MApi_GOP_GWIN_IsMirrorOn(&bHMirror, &bVMirror);

    if(bVMirror == TRUE && bEnable == TRUE)
        ioctl_info.dir = E_GOP_MIRROR_HV;
    else if(bVMirror == FALSE && bEnable == TRUE)
        ioctl_info.dir = E_GOP_MIRROR_H_ONLY;
    else if(bVMirror == TRUE && bEnable == FALSE)
        ioctl_info.dir = E_GOP_MIRROR_V_ONLY;
    else
        ioctl_info.dir = E_GOP_MIRROR_NONE;
...
E_GOP_API_Result MApi_GOP_GWIN_SetVMirror_EX(MS_U8 u8GOP,MS_BOOL bEnable)
{
    ...
    if(bHMirror == TRUE && bEnable == TRUE)
        ioctl_info.dir = E_GOP_MIRROR_HV;
    else if(bHMirror == FALSE && bEnable == TRUE)
        ioctl_info.dir = E_GOP_MIRROR_V_ONLY;
    else if(bHMirror == TRUE && bEnable == FALSE)
        ioctl_info.dir = E_GOP_MIRROR_H_ONLY;
    else
        ioctl_info.dir = E_GOP_MIRROR_NONE;
...
void MApi_GOP_Exit()
{
    ...
    DBGCHECK_INFO(printf("---%s %d: PID[%lu], TID[%ld] tries to exit\n", __FUNCTION__, __LINE__, (MS_U32)getpid(), MsOS_GetOSThreadID()));
    GOP_ENTRY();
#ifdef  MSOS_TYPE_LINUX
    GOP_MISC_PARAM ioctl_info;
    int  value = 0;
...
E_GOP_API_Result MApi_GOP_GWIN_GetWinAttr(MS_U8 gId, MS_U16 *x0, MS_U16 *y0, MS_U16 *width, MS_U16 *height,  MS_U16 *fbPitch, MS_U16 *fbFmt, MS_U32 *addr, MS_U32 *size)
{
    ...
    *addr = BuffInfo.addr;
    *size = BuffInfo.width * BuffInfo.height * GOP_GetBPP(BuffInfo.fbFmt)/8;
...
MS_U8 MApi_GOP_GWIN_GetMaxFBNum(void)
{
    ...
    {
        printf( "[Warning] %s( u32MaxFBNum:%ld....)out of MS_U8 bound, please to use MApi_GOP_GWIN_GetMax32FBNum()\n",__FUNCTION__,u32MaxFBNum );
    }
    return u32MaxFBNum;
}
...
{
    return MApi_GOP_GWIN_Create32FBbyStaticAddr2((MS_U32)fbId, dispX, dispY, width, height, fbFmt, u32FbAddr, FBString);
}
...
MS_U8 MApi_GOP_GWIN_CreateFBFrom3rdSurf(MS_U16 width, MS_U16 height,  MS_U16 fbFmt, MS_U32 u32FbAddr, MS_U16 pitch, MS_U8* ret_FBId)
{
    ...
    MS_U8 u8result = 0;
    u8result = MApi_GOP_GWIN_Create32FBFrom3rdSurf(width, height, fbFmt, u32FbAddr, pitch, &u32ret_FBID);
    *ret_FBId = (MS_U8)u32ret_FBID;
    if(u32ret_FBID > 256)//Out of MS_U8
    {
        printf( "[Warning] %s( ret_FBID:%ld....)out of MS_U8 bound, please to use MApi_GOP_GWIN_Create32FBFrom3rdSurf()\n",__FUNCTION__,u32ret_FBID );
    }
    return u8result;
}
...
{
     return MApi_GOP_GWIN_Create32FBbyStaticAddr((MS_U32)fbId, dispX, dispY, width, height,  fbFmt, u32FbAddr);
}
...
{
     return MApi_GOP_GWIN_Create32FBbyStaticAddr2(u32fbId, dispX, dispY, width, height,  fbFmt, u32FbAddr, E_GOP_FB_NULL);
}
...
E_GOP_API_Result MApi_GOP_GWIN_Switch2_32FB(MS_U32 u32fbId)
{
    ...
    GOP_ENTRY();
    pCBFmtInfo = (PGOP_CBFmtInfo)malloc(sizeof(GOP_CBFmtInfo));
    memset(pCBFmtInfo,0x0,sizeof(GOP_CBFmtInfo)); //Coverity
...
MS_BOOL MApi_GOP_Switch_GWIN_2_FB(MS_U8 gWinId, MS_U8 fbId, MS_U16 u16WaitTagID, MS_U16 *pU16QueueCnt)
{
    ...
    *pU16QueueCnt = (MS_U16)u32QueueCnt;
    ...
    {
        printf( "[Warning] %s( QueueCnt:%ld....)out of MS_U16 bound, please to use MApi_GOP_Switch_GWIN_2_32FB()\n",__FUNCTION__,u32QueueCnt);
    }
...
MS_BOOL MApi_GOP_Switch_GWIN_2_32FB(MS_U8 gWinId, MS_U32 u32fbId, MS_U32 u32WaitTagID, MS_U32* pU32QueueCnt)
{
    ...
    ioctl_info.pFlipInfo = (MS_U32*)&flipInfo;
...
MS_BOOL MApi_GOP_Switch_Multi_GWIN_2_FB_BY_ADDR(GOP_MultiFlipInfo GopMultiFlipInfo)
{
    ...
    ioctl_info.pMultiFlipInfo = (MS_U32*)&multiflipInfo;
...
MS_BOOL MApi_GOP_Switch_GWIN_2_FB_BY_ADDR(MS_U8 gWinId, MS_U32 u32FlipAddr, MS_U16 u16WaitTagID, MS_U16 *pU16QueueCnt)
{
    ...
    flipInfo.b3DEnable = FALSE;
    flipInfo.GwinId    = gWinId;
    flipInfo.FlipAddr  = u32FlipAddr;
    flipInfo.SubFlipAddr = u32FlipAddr;
...
MS_BOOL MApi_GOP_Switch_3DGWIN_2_FB_BY_ADDR(MS_U8 gWinId, MS_U32 u32MainFlipAddr, MS_U32 u32SubFlipAddr, MS_U16 u16WaitTagID, MS_U16 *pu16QueueCnt)
{
    ...
    flipInfo.GwinId    = gWinId;
    flipInfo.FlipAddr  = u32MainFlipAddr;
    flipInfo.SubFlipAddr = u32SubFlipAddr;
    flipInfo.WaitTagID = u16WaitTagID;
    flipInfo.pQueueCnt = &QueueCnt;
...
E_GOP_API_Result MApi_GOP_GWIN_SwapOverlapWin(MS_U8 u8GOP, MS_U8 u8win)
{
    ...
    ioctl_info.gop_idx  = u8GOP;
    ioctl_info.pSetting = (MS_U32*)&u8WinId;
    ioctl_info.u32Size  = sizeof(MS_U8);
...
E_GOP_API_Result MApi_GOP_GWIN_SetRelativeWinPrio(MS_U8 u8GOP, GOP_GwinPri *pGwinPri)
{
    ...
    ioctl_info.gop_idx  = u8GOP;
    ioctl_info.pSetting = (MS_U32*)&GwinPri;
...
E_GOP_API_Result MApi_GOP_GetLibVer(const MSIF_Version **ppVersion)
{
    ...
    if (!ppVersion)
    {
        GOP_RETURN(GOP_API_INVALID_PARAMETERS);
    }
...
E_GOP_API_Result MApi_GOP_MIXER_EnableVfilter(MS_BOOL bEnable)
{
    ...
    ioctl_info.pMixerProp = (MS_U32*)&bEna;
    ioctl_info.u32Size = sizeof(MS_BOOL);
...
E_GOP_API_Result MApi_GOP_MIXER_EnableOldBlendMode(MS_U8 u8GOP, MS_BOOL bEnable)
{
    ...
    ioctl_info.pMixerProp = (MS_U32*)&MixerOldBlendingMode;
...
{
    GOP_ENTRY();
    printf("[%s] Not SUPPORT bEn:%d type:%d\n",__FUNCTION__,bEnable, tiletype);
    GOP_RETURN(GOP_API_SUCCESS);
}
...
E_GOP_API_Result MApi_GOP_SetPINPON(MS_U8 winId, MS_BOOL bEn,GOP_PINPON_MODE PINPON_Mode)
{
    ...
    switch(PINPON_Mode)
    {
        ...
        case GOP_PINPON_DIP:
            ...
            break;
        default:
            printf("MApi_GOP_SetPINPON Enum not mapping!\n");
...
E_GOP_API_Result MApi_GOP_DWIN_SetBufferPINPON(MS_U32 u32fbaddr0,MS_U32 u32fbaddr1)
{
    ...
    stPinpon.u32Addr0 = u32fbaddr0;
    stPinpon.u32Addr1 = u32fbaddr1;
...
{
    GOP_ENTRY();
    printf("[%s] Not SUPPORT in utopia2\n",__FUNCTION__);
...
E_GOP_API_Result MApi_GOP_GWIN_SetScrollRate(MS_U8 u8rate)
{
    ...
    ioctl_info.pCfg = (MS_U32*)&u8ScrollRate;
    ioctl_info.u32Size  = sizeof(MS_U8);
...
E_GOP_API_Result MApi_GOP_IsRegUpdated(MS_U8 u8GopType)
{
    ...
    ioctl_info.pSetting = (MS_U32*)&bUpdate;
    ioctl_info.u32Size  = sizeof(MS_BOOL);
...
{
    static MS_U16 u16ActWin = 0xFFFF;
    MS_U8 u8GwinIdx = 0;
    GOP_ENTRY();
    switch (enPowerState)
    {
        ...
        {
            MS_U16 u16WinNum = MApi_GOP_GWIN_GetTotalGwinNum();
            u16ActWin = MApi_GOP_GWIN_GetActiveGWIN();
        ...
        case E_POWER_RESUME:
        {
            ...
            {
                printf("[%s] Error!!! Need suspend first!!!\n",__FUNCTION__);
            }
            u16ActWin = 0xFFFF;
            break;
        }
