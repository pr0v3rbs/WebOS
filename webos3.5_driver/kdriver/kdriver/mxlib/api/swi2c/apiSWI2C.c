...
#define I2C_CHECK_PIN_DUMMY     3600
...
static SWI2C_BusCfg g_SWI2CCBusCfg[IIC_BUS_MAX];
static MS_U8 g_u8CfgBusNum = 0;
...
static MS_BOOL MApi_SWI2C_Burst_SendBytes(MS_U8 u8BusNum, MS_U16 u16Cnt , MS_U8 *pu8Buff);
static MS_BOOL MApi_SWI2C_Burst_ReadBytes(MS_U8 u8BusNum, MS_U16 u16Cnt , MS_U8 *pu8Buff);
static void MApi_SWI2C_Burst_Stop(MS_U8 u8BusNum);
static MS_BOOL MApi_SWI2C_Burst_Start(MS_U8 u8BusNum);
...
static MS_BOOL IIC_AccessStart(MS_U8 u8BusNum, MS_U8 u8SlaveID, MS_U8 trans_t)
{
    ...
    while (u8Dummy--)
    {
        if ( MApi_SWI2C_Burst_Start(u8BusNum) == FALSE)
        {
            continue;
        }
        if (MApi_SWI2C_Burst_SendBytes(u8BusNum, 1, (MS_U8 *)&u8SlaveID) == TRUE)
        {
            return TRUE;
        }
        MApi_SWI2C_Burst_Stop(u8BusNum);
    }
...
static MS_BOOL IIC_CfgSpeedParam(MS_U8 u8BusNum, MS_U32 u32Speed_K)
{
#define DELAY_CNT(SpeedKHz)  ((u32FactorDelay/(SpeedKHz))-((u32Parameter1+u32AdjParam)-((SpeedKHz)/u32AdjParam))+((1<<((u32Parameter2-SpeedKHz)/40))))
    ...
    MS_U32 u32AdjParam = 14;

    //(1) assign primary parameters
    u32FactorDelay = u32CpuSpeedMHz * 268;
    ...
    {
        SWI2C_DBG_ERR("%s, Error parameter u32CpuSpeedMHz=%ld",__FUNCTION__, u32CpuSpeedMHz);
        return FALSE;
    }
    u32Parameter2 = 440UL;
    ...
    else if(u32Speed_K >= 250) u32Parameter1 = u32ParamBase1 +0 ;//+ 10; //300K evel
    else if(u32Speed_K >= 150) u32Parameter1 = u32ParamBase1 - (u32ParamBase1>100?100:0); //+ 60; //200K level
    else if(u32Speed_K >= 75) u32Parameter1 = u32ParamBase1;//250; //100K level //160
    else u32Parameter1 = u32ParamBase1 - (u32ParamBase1>160?160:0);//560; //50K level
    //(4) compute delay counts
    if ((u32Speed_K > 100) && (u32AdjParam > 0))
    {
    	u32DelayCount[u8BusNum] = 40;
    }
    else if ((u32Speed_K <= 100) && (u32AdjParam > 0))
    {
    	u32DelayCount[u8BusNum] = DELAY_CNT(u32Speed_K);
    }
    else
    {
        SWI2C_DBG_ERR("%s, Error parameter u32Speed_K=%ld , u32AdjParam=%ld",__FUNCTION__,  u32Speed_K, u32AdjParam);
        return FALSE;
    }
...
static MS_BOOL  IIC_UseBus( MS_U8 u8BusNum )
{
	MS_BOOL bReturn = FALSE;
    if ( u8BusNum < COUNTOF( g_I2CBus ) )
    {
        u8BusSel = u8BusNum;
        bReturn = IIC_CfgSpeedParam(u8BusNum, g_I2CBus[u8BusNum].defDelay);
...
static MS_BOOL MApi_SWI2C_Burst_Start(MS_U8 u8BusNum)
{
	return IIC_Start(u8BusNum);
}

static void MApi_SWI2C_Burst_Stop(MS_U8 u8BusNum)
{
	GPIO_Bus_Data sBusData;
	MS_BOOL bRet = TRUE;

	sBusData.u16byte=0;
	sBusData.pBuf = NULL;
	sBusData.padSCL = g_I2CBus[u8BusNum].padSCL;
	sBusData.padSDA = g_I2CBus[u8BusNum].padSDA;
	sBusData.u32LoopDelay = u32DelayCount[u8BusNum];
	sBusData.ack = TRUE;
	sBusData.u16Retry = 5;
	sBusData.bRet = &bRet;

	mdrv_gpio_bus_stop(&sBusData);
}
...
static MS_BOOL MApi_SWI2C_Burst_SendBytes(MS_U8 u8BusNum, MS_U16 u16Cnt , MS_U8 *pu8Buff)
{
	...
	if (pu8Buff == NULL)
	{
		printf("[Error] %s, line:%d, u16Cnt=%d, pu8Buff is null\n", __FUNCTION__, __LINE__, u16Cnt);
		return FALSE;
	}

	sBusData.u16byte=u16Cnt;
	sBusData.pBuf = pu8Buff;
	sBusData.padSCL = g_I2CBus[u8BusNum].padSCL;
	sBusData.padSDA = g_I2CBus[u8BusNum].padSDA;
	sBusData.u32LoopDelay = u32DelayCount[u8BusNum];
	sBusData.ack = TRUE;
	...
	return mdrv_gpio_send_bus_data(&sBusData);
}
...
static MS_BOOL MApi_SWI2C_Burst_ReadBytes(MS_U8 u8BusNum, MS_U16 u16Cnt , MS_U8 *pu8Buff)
{
	...
	return mdrv_gpio_get_bus_data(&sBusData);
}
...
MS_U32 MApi_SWI2C_Speed_Setting(MS_U8 u8BusNum, MS_U32 u32Speed_K)
{
    ...
    {
		g_I2CBus[u8BusNum].defDelay = (MS_U8) u32OriginalValue;
    }
...
void MApi_SWI2C_Init(SWI2C_BusCfg SWI2CCBusCfg[],MS_U8 u8CfgBusNum)
{
    ...
    g_u8CfgBusNum = u8CfgBusNum;
    for(u8Bus=0;u8Bus<u8CfgBusNum;u8Bus++)
    {
        g_SWI2CCBusCfg[u8Bus] = SWI2CCBusCfg[u8Bus];
    }
...
MS_BOOL MApi_SWI2C_WriteBytes(MS_U16 u16BusNumSlaveID, MS_U8 AddrCnt, MS_U8* pu8addr, MS_U16 u16size, MS_U8* pBuf)
{
    ...
    #if (SWIIC_RETRY_FLAG)
        u8Dummy = 3;  //input your retry number
    #endif
    ...
    while (u8Dummy--)
    {
        ...
        //Send Address Data
        bRet = MApi_SWI2C_Burst_SendBytes(u8BusNum, (MS_U16)AddrCnt, pu8addr);
        if (bRet == FALSE)
        {
            #if (SWIIC_RETRY_FLAG)
                if( u8Dummy )
        ...
        //Send  Write Buffer
        bRet = MApi_SWI2C_Burst_SendBytes(u8BusNum, u16size, pBuf);
...
MS_BOOL MApi_SWI2C_ReadBytes(MS_U16 u16BusNumSlaveID, MS_U8 ucSubAdr, MS_U8* paddr, MS_U16 ucBufLen, MS_U8* pBuf)
{
    ...
    while (u8Dummy--)
    {
        if((g_I2CReadMode[u8BusNum]!=E_SWI2C_READ_MODE_DIRECT) && (ucSubAdr>0) && (paddr))
        {
            ...
            if (MApi_SWI2C_Burst_SendBytes(u8BusNum, ucSubAdr, paddr)== FALSE)
            {
                    goto fail;
            }
        ...
        MApi_SWI2C_Burst_ReadBytes(u8BusNum, ucBufLen, pBuf);
...
{
	IIC_MUTEX_LOCK(u8BusChn);
    IIC_UseBus(u8BusChn);
	IIC_MUTEX_UNLOCK(u8BusChn);
}
...
MS_U16 MApi_SWI2C_SetPowerState(EN_POWER_MODE u16PowerState)
{
    ...
    else if (u16PowerState == E_POWER_RESUME)
    {
        ...
        {
            MApi_SWI2C_Init(g_SWI2CCBusCfg,g_u8CfgBusNum);
            _prev_u16PowerState = u16PowerState;
            u16Return = 1;//RESUME_OK;
        }
        else
        {
            SWI2C_DBG_INFO("[%s,%5d]It is not suspended yet. We shouldn't resume\n",__FUNCTION__,__LINE__);
            u16Return = 3;//SUSPEND_FAILED;
        }
    }
    else
    {
        SWI2C_DBG_INFO("[%s,%5d]Do Nothing: %d\n",__FUNCTION__,__LINE__,u16PowerState);
        u16Return = FALSE;
    }
