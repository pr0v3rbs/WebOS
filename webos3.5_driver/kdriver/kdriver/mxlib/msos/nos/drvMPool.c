...
MS_BOOL MsOS_MPool_Mapping(MS_U8 u8MiuSel, MS_SIZE u64Offset, MS_SIZE u64MapSize, MS_BOOL bNonCache){
	...
	_MPool_Check_aligned(u64Offset, u64MapSize);
	 MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("u64Offset = %lx, u64MapSize = %lx, bNonCache = %02x\n", u64Offset, u64MapSize, bNonCache));
		...
		else
        	{
	        	...
	        	if ( (mpool_info[i].pPhyaddr <= Phyaddr && Phyaddr < mpool_info[i].pPhyaddr + mpool_info[idx].u64MpoolSize) ||
        	           (mpool_info[i].pPhyaddr <= PhyaddrEnd && PhyaddrEnd < mpool_info[i].pPhyaddr + mpool_info[idx].u64MpoolSize) )
            		{
        	                ...
        	                MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("[Utopia2_nos] Duplicated PA(0x%lx ~ 0x%lx) mapping with Table[%d](0x%lx ~0x%lx).\n",
                                Phyaddr, PhyaddrEnd, i, mpool_info[i].pPhyaddr, (mpool_info[i].pPhyaddr + mpool_info[idx].u64MpoolSize -1)));
	...
	MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("The phy and the bus address is:%#lx and :%#lx\n,offset is:%#lx!!!!!!",Phyaddr,Busaddr,mpool_info[idx].s32V2Poff));
	/***************************************/
	...
	MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%d].pVirtStart =%lx\n", idx,mpool_info[idx].pVirtStart));
        MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%d].pVirtEnd = %lx\n", idx,mpool_info[idx].pVirtEnd));
        MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%d].pPhyaddr =%lx\n", idx,mpool_info[idx].pPhyaddr));
        MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%d].u64MpoolSize =%lx\n", idx,mpool_info[idx].u64MpoolSize));
        MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%d].s32V2Poff = %lx\n", idx,mpool_info[idx].s32V2Poff));
Mapping_End:
	return bRet;
}
...
MS_VIRT _MPool_PA2VA(MS_PHY pAddrPhys, MS_BOOL bNonCache)
{
    ...
    for(idx=0;idx<MAX_MAPPINGSIZE;idx++)
    {
        if(!mpool_info[idx].bIsUsed)
            continue;
        MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%d].u8MiuSel = %d\n", idx,mpool_info[idx].u8MiuSel));
        MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%d].bNonCache = %d\n", idx,mpool_info[idx].bNonCache));
	MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%d].pVirtStart =%lx\n",idx, mpool_info[idx].pVirtStart));
        MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%d].pVirtEnd = %lx\n", idx,mpool_info[idx].pVirtEnd));
        MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%d].pPhyaddr =%lx\n", idx,mpool_info[idx].pPhyaddr));
        MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("mpool_info[%d].u64MpoolSize =%lx\n", idx,mpool_info[idx].u64MpoolSize));
    ...
    MPOOL_DBG_MSG(E_MsOSMPool_DBG_L1, printf("[Utopia2_nos] ERROR!!! CAN'T SUCCESSFULLY CONVERT FROM PA(%lx) TO VA.\n", pAddrPhys));
