...
#define LOGI ALOGI
#endif
#define MSOS_PRINT(fmt, args...)    LOGI("[MSOS_PRINT][%06d]     " fmt, __LINE__, ## args)
#else
...
#define SHM_SIZE                ((10 * 1024 - 124) * 1024)
...
typedef struct
{
    ...
    MS_U32      u32Offset;
    MS_U32      u32Size;
    MS_U16      u16ClientId;    // 0 means "never used"
    MS_U8       u8RefCnt;
    MS_U8       u8Dummy[2];
} MsOS_SHM_Context;
...
MS_BOOL MsOS_Init (void)
{
    ...
    if (-1 == _shm_id)
    {
        ...
        {
            MSOS_PRINT("[MsOS_Init] MsOS_SHM_Init failed!\n");
            bRet = FALSE;
            goto ret;
        }
    }
    ...
    if (SEM_FAILED == (pMutexMutex = sem_open(MUTEX_MUTEX_NAME, O_CREAT | O_EXCL, 0666, 1)))
    {
        ...
        {
            MSOS_PRINT("[MsOS_Init] sem_open failed!\n");
            bRet = FALSE;
            goto ret;
        }
    }
    if (FALSE == MsOS_SHM_GetId((MS_U8*)"_proc_mutex", MSOS_MUTEX_MAX* sizeof(MsOS_Mutex_Info), &u32ShmId, &u32Addr, &u32Size, MSOS_SHM_QUERY))
    {
        ...
        {
            MSOS_PRINT("[MsOS_Init] MsOS_SHM_GetId _proc_mutex failed!\n");
            bRet = FALSE;
            goto ret;
        }
    }
    ...
    if (_s32FdSYS < 0)   //First time open
    {
        ...
        {
            MSOS_PRINT("[MsOS_Init]open device node failed!\n");
            bRet = FALSE;
            goto ret;
        }
    }
...
MS_BOOL MsOS_DeleteMemoryPool (MS_S32 s32PoolId)
{
    ...
    if ( (s32PoolId >= MSOS_MEMPOOL_MAX) )
    {
        ...
        MSOS_PRINT("Invalid memory pool ID:%d, you must use the ID of the mpool you created\n",s32PoolId);
        return TRUE;
    ...
    {
        MSOS_PRINT("MEMORY POOL WITH MEMORYPOOL_ID:0x%x NOT EXIST\n",s32PoolId);
        return FALSE;
    }
    #endif
...
void * MsOS_AllocateMemory (MS_U32 u32Size, MS_S32 s32PoolId)
{
    ...
    if ( s32PoolId >= MSOS_MEMPOOL_MAX)
    {
        ...
        {
            MSOS_PRINT("Invalid memory pool ID:%d, you must use the default ID: MSOS_MALLOC_ID ,or the ID of the mpool you created\n",s32PoolId);
            return NULL;
        }
    ...
    {
        MSOS_PRINT("Invalid memory pool ID:%d, you must use the default ID: MSOS_MALLOC_ID ,or the ID of the mpool you created\n",s32PoolId);
        MSOS_PRINT("System will use default mpool to allocate memory here\n");
        return malloc(u32Size);
    }
...
MS_BOOL MsOS_FreeMemory (void *pAddress, MS_S32 s32PoolId)
{
    ...
    if (FALSE== _MsOS_MemoryPool_Info[s32PoolId].bMPool)
    {
        MSOS_PRINT("Invalid mpool ID:%d, you must use the default ID macro: MSOS_MALLOC_ID ,or the ID of the mpool you created\n",s32PoolId);
        MSOS_PRINT("System will free default mpool memory here\n");
        free(pAddress);
...
#if USE_DLMALLOC
        PTH_RET_CHK(pthread_mutex_lock(&_MsOS_MemoryPool_Mutex));
        // my_info = mspace_mallinfo(_MsOS_MemoryPool_Info[s32PoolId].stMemoryPool);
        // printf("Total size %ld, used size %ld, free size %ld\n", (unsigned long) my_info.usmblks, (unsigned long) my_info.uordblks, (unsigned long) my_info.fordblks);
...
void * MsOS_ReallocateMemory (void *pOrgAddress, MS_U32 u32NewSize, MS_S32 s32PoolId)
{
    ...
    {
        MSOS_PRINT("Invalid mpool ID:%d, you must use the default ID macro: MSOS_MALLOC_ID ,or the ID of the mpool you created\n",s32PoolId);
        MSOS_PRINT("System will reallocate memory from default pookl\n");
        return realloc(pOrgAddress, u32NewSize);
    }
                     ...
                     char *pTaskName)
{
    ...
    //set thread name
    if(strlen((const char*)pTaskName) > MAX_NAME)
    {
        MSOS_PRINT("the length of the thread name exceeds %d, it will be truncated to %d chars\n", MAX_NAME, MAX_NAME);
    }

    strncpy((char*)_MsOS_Task_Info[s32Id].u8Name, (const char*)pTaskName, MIN(strlen((const char*)pTaskName),(MAX_NAME - 1)));
...
MS_BOOL MsOS_DeleteTask (MS_S32 s32TaskId)
{
...
#if defined (ANDROID)
    MSOS_PRINT("ERROR!!! pthread_cancel() is not supported by BIONIC libc\n");
#else
    ...
    {
        MSOS_PRINT("TASK WITH TASK_ID:0x%x NOT EXIST\n",s32TaskId);
        return FALSE ;
    }
#endif
    ...
    while(1)
    {
        ...
        if(err==-1)
        {
            switch(errno)
            {
                ...
                default:
                    if(errno !=(int)NULL)
                        MSOS_PRINT("nanosleep is interrupted: %d\n", errno);
                    break;
            }
...
MS_BOOL MsOS_DeleteMutex (MS_S32 s32MutexId)
{
    ...
    {
        MSOS_PRINT("MUTEX WITH MUTEX_ID:0x%x NOT EXIST\n",s32MutexId );
        return FALSE;
    }
    #endif
...
MS_BOOL MsOS_DeleteSemaphore (MS_S32 s32SemaphoreId)
{
    ...
    {
        MSOS_PRINT("SEMAPHORE WITH SEMAPHORE_ID:0x%x NOT EXIST\n",s32SemaphoreId );
        return FALSE;
    }
    #endif
...
MS_BOOL MsOS_DeleteEventGroup (MS_S32 s32EventGroupId)
{
    ...
    {
        MSOS_PRINT("EVENTGROUP WITH EVENTGROUP:0x%x NOT EXIST\n",s32EventGroupId);
        return FALSE;
    }
    #endif
...
void Timer_Thread(void *data)
{
    ...
    {
        MSOS_PRINT("Failed to Create the timer.\n");
        // @FIXME: should not fail here. Do error handling later
        MS_ASSERT(0);
    }
    /* Start the timer */
                      ...
                      char *pTimerName)
{
    ...
    if(ret!=0){
        MSOS_PRINT ("Create pthread error!\n");
        MS_ASSERT(0);
    }
    else
    {
...
MS_BOOL MsOS_DeleteTimer (MS_S32 s32TimerId)
{
    ...
    {
        MSOS_PRINT("TIMER WITH TIMER_ID:0x%x NOT EXIST\n",s32TimerId );
        PTH_RET_CHK(pthread_mutex_unlock(&_MsOS_Timer_Info[s32TimerId].TimerMutex));
        return FALSE;
    }
}
                      ...
                      char         *pQueueName)
{
    ...
    {
        MSOS_PRINT("E_MSG_VAR_SIZE has not been supported yet\n");
        return -1;
    }
...
MS_BOOL MsOS_DeleteQueue (MS_S32 s32QueueId)
{
    ...
    {
        MSOS_PRINT("QUEUE WITH QUEUEID:0x%x NOT EXIST\n",s32QueueId);
        return FALSE;
    }
    #endif
...
MS_BOOL MsOS_SHM_Init(void)
{
    ...
    if(pMutexShmIdx < 0)
    {
        ...
        if(pMutexShmIdx < 0)
        {
            MSOS_PRINT("create namedmutex %d fail ",pMutexShmIdx);
            return FALSE;
        }
    }
...
MS_BOOL MsOS_SHM_GetId(MS_U8* pu8ClientName, MS_U32 u32BufSize, MS_U32* pu32ShmId, MS_VIRT* pu32Addr, MS_U32* pu32BufSize, MS_U32 u32Flag)
{
    ...
    if (!_pu8ShareMem)
    {
        MSOS_PRINT("[%s][%d] MsOS_SHM_Init should be invoked first\n", __FUNCTION__, __LINE__);
        return FALSE;
    }
    ...
    if ((NULL == pClient) && (MSOS_SHM_CREATE == u32Flag))
    {
        ...
        for (i= 0; i< MAX_SHM_CLIENT_NUM; i++, pContext++)
        {
            if (pContext->u16ClientId)
            {
                continue;
            }
            if ((_ShmHdr.u32Offset + u32BufSize)> _ShmHdr.u32ShmSize)
            {
                MSOS_PRINT("[%s][%d] MsOS_SHM_GetId: shared memory buffer overflow\n", __FUNCTION__, __LINE__);
#if defined (TV_OS)
                _MSOS_SHM_UNLOCK(pMutexShmIdx);
#else
                _MSOS_SHM_UNLOCK(pMutexShm);
...
void* _MsOS_LinuxTaskWrapper (void *argv)
{
    ...
    snprintf(thd_name, MAX_NAME-1,"%s", pMsOS_Task_Info->u8Name);
...
{
    MSOS_PRINT("[%s][%d] %s is not supported\n", __FUNCTION__, __LINE__, __FUNCTION__);
    return FALSE;
}
...
MS_U32 MsOS_GetKattribute(char *pAttr)
{
    ...
    {
        MSOS_PRINT("Can not open file at: %s", path);
        return -1;
    }

    if (fscanf(pFile, "%s", str) < 0)
    {
        u32val = -1;
        goto out;
    }
