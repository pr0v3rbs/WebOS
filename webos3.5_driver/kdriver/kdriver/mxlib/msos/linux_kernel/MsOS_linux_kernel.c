...
#define SEMA_MUTEX_LOCK()       spin_lock(&_MsOS_Semaphore_Mutex)
#define SEMA_MUTEX_UNLOCK()     spin_unlock(&_MsOS_Semaphore_Mutex)
...
MS_S32 MsOS_CreateEventGroup (char *pEventName)
{
    ...
    spin_lock(&_MsOS_EventGroup_Info[s32Id].stMutexEvent);
    _MsOS_EventGroup_Info[s32Id].u32EventGroup= 0;
    spin_unlock(&_MsOS_EventGroup_Info[s32Id].stMutexEvent);
            ...
            if (bAnd)
            {
				...
				do {
					s32Ret = wait_event_interruptible(_MsOS_EventGroup_Info[s32EventGroupId].stEventWaitQueue,
								((_MsOS_EventGroup_Info[s32EventGroupId].u32EventGroup & u32WaitEventFlag) == u32WaitEventFlag));
					if (s32Ret == -ERESTARTSYS && signal_pending(current))
                    {
					_MsOS_EventGroup_Info[s32EventGroupId].u32Waiting--;
                    return -ERESTARTSYS;
                    }
        ...
        else
        {
            if(u32WaitMs != 0)
            {
                ...
                if (bAnd)
                {
                    spin_unlock(&_MsOS_EventGroup_Info[s32EventGroupId].stMutexEvent);
					do {
						s32Ret = wait_event_interruptible_timeout(_MsOS_EventGroup_Info[s32EventGroupId].stEventWaitQueue,
											((_MsOS_EventGroup_Info[s32EventGroupId].u32EventGroup & u32WaitEventFlag) == u32WaitEventFlag),
											u32WaitMs);
						if (s32Ret == -ERESTARTSYS && signal_pending(current))
							return s32Ret;
						if (unlikely(s32Ret >= u32WaitMs))
							break;
						u32WaitMs -= s32Ret;
					} while (u32WaitMs > 0 && !((_MsOS_EventGroup_Info[s32EventGroupId].u32EventGroup & u32WaitEventFlag) == u32WaitEventFlag));
                    spin_lock(&_MsOS_EventGroup_Info[s32EventGroupId].stMutexEvent);
                }
                else
                {
					...
					do {
						...
						if (unlikely(s32Ret >= u32WaitMs))
							break;
						u32WaitMs -= s32Ret;
					} while (u32WaitMs > 0 && !((_MsOS_EventGroup_Info[s32EventGroupId].u32EventGroup & u32WaitEventFlag) != 0));
                    spin_lock(&_MsOS_EventGroup_Info[s32EventGroupId].stMutexEvent);
                }
            }
        }
...
{
#if 0
    Chip_Inv_Cache_Range(u32Start, u32Size);
    return TRUE;
#else
    return FALSE;
#endif
...
MS_BOOL MsOS_SHM_GetId(MS_U8* pu8ClientName, MS_U32 u32BufSize, MS_U32* pu32ShmId, MS_U32* pu32Addr, MS_U32* pu32BufSize, MS_U32 u32Flag)
{
    ...
    if(pu8ClientName == NULL)
        return result;

    //skip size = 0 to avoid registering module fail
    if(u32BufSize == 0)
        return TRUE;

    //check name length, should not over than 50
    if(strlen(pu8ClientName) > MAX_CLIENT_NAME_LENGTH)
    {
        printk("[%s][%d] SHM name length should not over than 50 !!\n", __FUNCTION__, __LINE__);
        return result;
    }
	...
	if (match) {
		// if match, fill info
		*pu32ShmId = i;
		*pu32Addr = (MS_U32)(g_shm_array.shm_entry[i].addr);
		...
		*pu32Addr = (MS_U32)vmalloc(u32BufSize);
		if(NULL == *pu32Addr) {
			SHM_MUTEX_UNLOCK();
			return result;
		}
