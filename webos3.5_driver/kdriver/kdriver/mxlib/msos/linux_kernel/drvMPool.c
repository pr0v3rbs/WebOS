...
static MS_U32 u32MpoolRegBase;
static MS_U32 u32UtopiaVAStart = 0;
static MS_U32 u32UtopiaVAEnd = 0;
static MS_U32 map_kdriver_mem(MS_U32 u32BusStart, MS_U32 u32MapSize, MS_BOOL bNonCache);
...
static unsigned long driver_mem_va_start;
...
//--------------------------------------------------------------------------------------------------
static MS_U32 _MPool_PA2VA(MS_U32 pAddrPhys, MS_BOOL bNonCache);
...
//  Implementation
//--------------------------------------------------------------------------------------------------
MS_BOOL MsOS_MPool_VAError(unsigned long u32VA)
{
    return (u32VA < u32UtopiaVAStart)||(u32VA >= u32UtopiaVAEnd);
}
...
MS_PHY MsOS_MPool_VA2PA(MS_VIRT pAddrVirt)
{
    ...
    printk("ERROR!!! CAN'T SUCCESSFULLY CONVERT FROM VA(%lx) TO PA\n", pAddrVirt);
...
void MsOS_MPool_InfoMsg(MS_U8 u8MsgType)
{
    ...
    for (i=0;i<MAX_MAPPINGSIZE; i++)
    {
        ...
        printk("map miu:%d-pa:0x%x with size:0x%x to VA(0x%x ~ 0x%x)\n",(unsigned int)mpool_info[i].u8MiuSel,(unsigned int)mpool_info[i].pPhyaddr,(unsigned int)mpool_info[i].u32MpoolSize,(unsigned int)mpool_info[i].pVirtStart,(unsigned int)mpool_info[i].pVirtEnd);
    }
    printk("======================================================================\n");
    return;
}
...
static MS_U32 _MPool_PA2VA(MS_U32 pAddrPhys, MS_BOOL bNonCache)
{
    ...
    for(i=0;i<MAX_MAPPINGSIZE;i++)
    {
        ...
        {
            if((mpool_info[i].s32V2Poff == 0) && (mpool_info[i].pVirtStart == 0))
            {
                if(!_MPOOL_DELAY_BINDING(i))
                {
                    printk("[Utopia2K] PA2KSEG delay binding fail!, pa = %x", (int)mpool_info[i].pPhyaddr);
        ...
        if(u32UtopiaVAStart == 0 || mpool_info[i].pVirtStart <= u32UtopiaVAStart)
            u32UtopiaVAStart = mpool_info[i].pVirtStart;
        if(mpool_info[i].pVirtEnd >= u32UtopiaVAEnd)
            u32UtopiaVAEnd = mpool_info[i].pVirtEnd;
        ...
        pAddrVirt = pAddrPhys + mpool_info[i].s32V2Poff;
    ...
    printk("[Utopia2K] ERROR!!! CAN'T SUCCESSFULLY CONVERT FROM PA(%lx) TO VA.\n", pAddrPhys);
...
MS_BOOL MsOS_MPool_Dcache_Flush(MS_U32 pAddrVirt, MS_U32 u32Size)
{
    ...
    Chip_Flush_Cache_Range(pAddrVirt, u32Size);
...
static MS_BOOL _MPOOL_DELAY_BINDING(int idx)
{
    ...
    driver_mem_va_start = (unsigned int)map_kdriver_mem(ba_mapping_start, mpool_info[idx].u32MpoolSize, mpool_info[idx].bNonCache);
...
MS_BOOL MsOS_MPool_Mapping(MS_U8 u8MiuSel, MS_U32 u32Offset, MS_U32 u32MapSize, MS_BOOL bNonCache)
{
    ...
    {
        //BUG();
        printk("[Kernel][WARNING][%s] Mapping size should not <= 0 !!!!! \n\n", __FUNCTION__);
        return TRUE;
    }
    ...
    driver_mem_va_start = (unsigned int)map_kdriver_mem(ba_mapping_start, u32MapSize, bNonCache);
...
{
	MS_U32 VirtAddr = 0;
//	printk("Jway...u32BusStart = 0x%lx  u32MapSize=0x%lx \n",u32BusStart,u32MapSize);

	if (pfn_valid(__phys_to_pfn(u32BusStart & PAGE_MASK)))
		VirtAddr = __va(u32BusStart);
	else {
		if (bNonCache)
			VirtAddr = (unsigned long)ioremap(u32BusStart, u32MapSize);
		else
			VirtAddr = (unsigned long)ioremap_cached(u32BusStart, u32MapSize);
	}

//	printk("Jway2...u32BusStart = 0x%lx  VirtAddr =0x%lx \n",u32BusStart,VirtAddr);
	return VirtAddr;
}
