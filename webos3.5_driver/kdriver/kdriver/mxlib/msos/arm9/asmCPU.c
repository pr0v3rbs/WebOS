...
{
    #define CPU_LOOP_MSEC(_msec)        ((_msec)*(900000000/1000/3)) // 3 cycles / loop
...
void MAsm_CPU_TimerInit(void)
{
    ...
    periphclk = ARM_CLOCK_FREQ / (1000000*2);
    prescale = 99;
    regctl = (prescale << 8) | 0x06; //set auto increment mode and enable interrupt

   *(volatile MS_U32*)(PERIBASE + (0x0608)) = regctl; //Set Timer control register
   *(volatile MS_U32*)(PERIBASE + (0x0600)) = periphclk*10 - 1; //Set Timer load register
}

void MAsm_CPU_TimerStart(void)
{
    *(volatile MS_U32*)(PERIBASE + (0x0608)) |= 0x0001;
}

void MAsm_CPU_TimerClean(void)
{
    *(volatile MS_U32*)(PERIBASE + (0x0608)) = 0x0000;
}
