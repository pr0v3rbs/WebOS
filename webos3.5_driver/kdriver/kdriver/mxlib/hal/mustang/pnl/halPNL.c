    ...
    switch (eLPLL_Type)
    {
        case E_PNL_TYPE_LVDS:
            switch (eLPLL_Mode)
            {
                ...
                case E_PNL_MODE_DUAL:
                    if ((ldHz >= 500000000) && (ldHz < 1100000000))
                    {
        ...
        case E_PNL_TYPE_HS_LVDS:
            ...
            switch (eLPLL_Mode)
            {
                ...
                default:
                case E_PNL_MODE_DUAL:
                    if((ldHz >= 500000000) && (ldHz < 1145000000))
                    {
                        u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_HS_LVDS_2ch_50to114_5MHz;
                    }
                    else if((ldHz >= 1145000000) && (ldHz < 2300000000UL))
                    {
        ...
        case E_PNL_TYPE_TTL:
              ...
              else if((ldHz >= 500000000) && (ldHz < 1000000000))
              {
        ...
        //    u8SupportedLPLLIndex = E_PNL_SUPPORTED_LPLL_EPI28_8BIT_12PAIR_330to330MHz;
        //break;
...
// Turn OD function
////////////////////////////////////////////////////////////////////////
void MHal_PNL_OverDriver_Init(void *pInstance,MS_U32 u32OD_MSB_Addr, MS_U32 u32OD_MSB_limit, MS_U32 u32OD_LSB_Addr, MS_U32 u32OD_LSB_limit,MS_U8 u8MIUSel)
{
    UNUSED(u8MIUSel);
    ...
    SC_W2BYTE(0,REG_SC_BK16_39_L, (MS_U16)(u32OD_LSB_limit & 0xFFFF)); // OD frame buffer write address limit
    SC_W2BYTEMSK(0,REG_SC_BK16_3A_L, (MS_U16)((u32OD_LSB_limit >> 16) & 0x00FF), 0x00FF); // OD frame buffer write address limit
    SC_W2BYTE(0,REG_SC_BK16_3B_L, (MS_U16)(u32OD_LSB_limit & 0xFFFF)); // OD frame buffer read address limit
    SC_W2BYTEMSK(0,REG_SC_BK16_3C_L, (MS_U16)((u32OD_LSB_limit >> 16) & 0x00FF), 0x00FF); // OD frame buffer read address limit
...
////////////////////////////////////////////////////////////////////////
// Turn Pre-Emphasis Current function partial setting
////////////////////////////////////////////////////////////////////////
MS_BOOL MHal_PNL_MOD_Control_Out_PE_Current_Partial (void *pInstance,MS_U8 *u8Current_Level_array)
{
    MS_BOOL bStatus = FALSE;
    //MS_U16 u16ValidCurrent = u16Current_Level & 0x07;

    MOD_W2BYTEMSK(REG_MOD_BK00_23_L,
        ( (u8Current_Level_array[0] ) |(u8Current_Level_array[1] << 3 )|(u8Current_Level_array[2] << 6 )
        |(u8Current_Level_array[3] << 9 ) |(u8Current_Level_array[4] << 12 )) , 0x7FFF);

    MOD_W2BYTEMSK(REG_MOD_BK00_24_L,
        ( (u8Current_Level_array[5]  ) |(u8Current_Level_array[6] << 3 )|(u8Current_Level_array[7] << 6 )
        |(u8Current_Level_array[8] << 9 ) |(u8Current_Level_array[9] << 12 )) , 0x7FFF);

    MOD_W2BYTEMSK(REG_MOD_BK00_25_L,
        ( (u8Current_Level_array[10]  ) |(u8Current_Level_array[11] << 3 )|(u8Current_Level_array[12] << 6 )
        |(u8Current_Level_array[13] << 9 ) |(u8Current_Level_array[14] << 12 )) , 0x7FFF);

    MOD_W2BYTEMSK(REG_MOD_BK00_26_L,
        ( (u8Current_Level_array[15]  ) |(u8Current_Level_array[16] << 3 )|(u8Current_Level_array[17] << 6 )
        |(u8Current_Level_array[18] << 9 ) |(u8Current_Level_array[19] << 12 )) , 0x7FFF);

    MOD_W2BYTEMSK(REG_MOD_BK00_29_L, u8Current_Level_array[20]  ,0x0007);
    ...
    else
    {
        switch(eOutputMode)
        {
            ...
            default:
                ...
                u16PE_Current = (MOD_R2BYTEMSK(REG_MOD_BK00_23_L, 0x07));
                ...
                MHal_PNL_MOD_Control_Out_PE_Current (pInstance, u16PE_Current);
                //8. set Desired Pairs: GCR_ICON[5:0]    (current all init);
                MHal_PNL_MOD_Control_Out_Swing(pInstance,(u16ValidSwing2+2)*10);
...
void MHal_PNL_Init_MOD(void *pInstance,PNL_InitData *pstPanelInitData)
{
    ...
    // patch: for LG HD panel
    if (strcmp(pstPanelInitData->pPanelName, "WXGA_AU20_T200XW02") == 0)
    {
        SC_W2BYTEMSK(0,REG_SC_BK24_3F_L, 0x00, BIT(0) | BIT(1) | BIT(2) | BIT(3));
    }
...
// =========
    ...
    if (u8CheckTimes <=3)
    {
        ...
        for(x=0;x<14;x++)
        {
            ...
            if((u16cur_ibcal[x]>=(u8cur_ibcal*8/5))||(u16cur_ibcal[x]<=(u8cur_ibcal*2/5)))
            {
...
PNL_Result MHal_PNL_En(void *pInstance,MS_BOOL bPanelOn, MS_BOOL bCalEn)
{
    ...
    if(bPanelOn)
    {
        ...
        if(!MHal_PNL_MOD_Control_Out_Swing(pInstance, pPNLResourcePrivate->sthalPNL._u16PnlDefault_SwingLevel))
            printf(">>Swing Level setting error!!\n");
        if(!MHal_PNL_MOD_Control_Out_PE_Current(pInstance, pPNLResourcePrivate->sthalPNL._u16PnlDefault_PECurrent))
            printf(">>PECurrent setting error!!\n");
    }
    else
{
...
{
    MS_U8 u8CheckModBK_Count = 0;

    while(MOD_R2BYTE(REG_MOD_BK00_00_L) && u8CheckModBK_Count<20)
    {
        u8CheckModBK_Count++;
        MsOS_DelayTask(5);
    }
...
void MHal_PNL_SetModPattern(MS_BOOL b)
{
     MOD_W2BYTEMSK(REG_MOD_BK00_01_L, b?BIT(15):0, BIT(15));
}
#endif
