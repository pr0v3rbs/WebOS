...
static MS_BOOL bhal_sys_wol_int=0;
//-------------------------------------------------------------------------------------------------
//  Debug Functions
//-------------------------------------------------------------------------------------------------
...
static MS_U8 HAL_SYS_ReadByte(MS_U32 u32RegAddr)
{
    ...
    {
        printf("[SYS]%s access RIU register error!!\r\n",__FUNCTION__);
    }
    else
    {
...
{
    HAL_SYS_WriteByte(0x3C4E, (HAL_SYS_ReadByte(0x3C4E) | BIT0));
    return TRUE;
}
...
void HAL_SYS_SetEfuseIOMapBase(MS_U32 u32Base)
{
...
static void HAL_SYS_WOL_ISR(InterruptNum eIntNum)
{
    if((HAL_SYS_ReadByte(0x0e1c)&(BIT2))==(BIT2))//reg_wk_irq_final_status[2]: WOL
    {
        ...
        //2-1 WOL part
        HAL_SYS_WriteByte(0x3446, HAL_SYS_ReadByte(0x3446)&(~BIT7));
        HAL_SYS_WriteByte(0x3446, HAL_SYS_ReadByte(0x3446)|(BIT7));
        //2-2 PM part
...
void HAL_SYS_EnableWkEventWOL(void)
{
    ...
    // WOL enable
    HAL_SYS_WriteByte(0x3447, HAL_SYS_ReadByte(0x3447) | BIT5); //avoid to wake-up in unexpect condition
    HAL_SYS_WriteByte(0x3446, (BIT0)); // Enable WOL
}
...
void HAL_SYS_DisableWkEventWOL(void)
{
    ...
    MsOS_DetachInterrupt(E_INT_IRQ_PMSLEEP);
    bhal_sys_wol_int = 0;

    HAL_SYS_WriteByte(0x0e10, HAL_SYS_ReadByte(0x0e10)|(BIT2)); //WOL disable interrupt
    HAL_SYS_WriteByte(0x3446, HAL_SYS_ReadByte(0x3446)&(~BIT7)); //<<WOL INT clear>>
    HAL_SYS_WriteByte(0x3446, HAL_SYS_ReadByte(0x3446)|(BIT7));
    HAL_SYS_WriteByte(0x3446, HAL_SYS_ReadByte(0x3446) & ~BIT0); // Disable WOL
}
...
MS_BOOL HAL_SYS_GetStatusWOL(void)
{
    ...
    if((HAL_SYS_ReadByte(0x3446) & BIT0) == BIT0 )
    {
        bRET = 1;
    }
