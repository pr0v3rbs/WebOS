...
static MS_U32 _gUART1_Status =0;
static MS_U32 _gUART0_Status =0;
//-------------------------------------------------------------------------------------------------
//  Local Variables
//-------------------------------------------------------------------------------------------------
...
int HAL_UART_Aeon_Set_Baudrate(ms_uart_dev_t u, MS_U32 baudrate)
{
    ...
    switch((_HAL_REG_ReadByte(0x0B94)>>2)&0x7)
    {
        case 0x0:
            Fre = 240000000;
            break;
...
int HAL_UART_PIU_Write(device_t dev, const char *buf, MS_U32 len, int off)
{
    ...
    if (!u->tx_buf)
    {
        ...
        {
			timeout_count = 0;
            while ((!(UART_REG8(UART_LSR) & UART_LSR_THRE)) && (timeout_count++ < UART_TIMEOUT));
...
int HAL_UART_PIU_Read(device_t dev, char *buf, MS_U32 len, int off)
{
    ...
    if (!u->rx_buf)
    {
        ...
        {
			timeout_count = 0;
            while ((!(UART_REG8(UART_LSR) & UART_LSR_DR)) && (timeout_count++ < UART_TIMEOUT));
...
int HAL_UART_PIU_Set_Baudrate(ms_uart_dev_t u, MS_U32 baudrate)
{
    ...
    //	Clear FIFO Buffer
	#ifdef MSOS_TYPE_NOS
    UART_REG8(UART_FCR) |= 0x07; //mark for OS system hang
	#endif

    //ii.	Poll "reg_usr_busy" till 0;
	#ifdef MSOS_TYPE_NOS
	while(UART_REG8(UART_USR) & 0x01); //mark for OS system hang
	#endif
	...
	#ifdef MSOS_TYPE_NOS
    UART_REG8(UART_DLM) = (divisor >> 8) & 0xFF; //mark for OS system hang
    #endif
    UART_REG8(UART_LCR) &= ~(UART_LCR_DLAB);
...
static void _HAL_UART_PIU_Isr(UART_DEVICE_TYPE uart_dev)//ISR Body
{
    ...
    if (!(iir & UART_IIR_NO_INT))
    {
       ...
       if ( (iir & UART_IIR_TOI)||(iir & UART_IIR_NO_INT) )// UART time out & busy
       {

	         if( uart_dev == E_UART_PIU_UART0 )
	         {
	               _gUART0_Status = UART_IIR_TOI;
			 }
	         else
	         {
	               _gUART1_Status = UART_IIR_TOI;
	         }
...
void HAL_UART0_PIU_Isr(MS_U32 *pHalReg, MS_U32 u32Data)
{
       ...
       if (_gUART0_Status != UART_IIR_TOI )
       {
...
void HAL_UART1_PIU_Isr(MS_U32 *pHalReg, MS_U32 u32Data)
{
        ...
        if (_gUART1_Status != UART_IIR_TOI )
        {
...
int HAL_UART_PIU_Open(device_t dev, int mode)
{
    ...
    if (u->device.refcnt == 0)
    {
		...
		#ifdef MSOS_TYPE_NOS
        UART_REG8(UART_IER) = 0x00; //mark for OS system hang
		#endif

        // card_reset_reg must set to 1 for rx
		#ifdef MSOS_TYPE_NOS
        UART_REG8(8 * 2) = 0x10; //makr for OS system hang
		#endif
