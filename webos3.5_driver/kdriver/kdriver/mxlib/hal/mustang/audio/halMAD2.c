...
void HAL_MAD2_SetMemInfo(void)
{
    ...
    if((MIUBank&0x01)==0x00)  //MIU0
    {
        HAL_AUR2_WriteMaskByte((REG_R2_RIU_MAIL_00 + 1), 0x60, 0x20);
        HAL_AUR2_WriteMaskByte((REG_R2_1_RIU_MAIL_00 + 1), 0x60, 0x20);
    }
    else if((MIUBank&0x01)==0x01)   //MIU1
    {
        HAL_AUR2_WriteMaskByte((REG_R2_RIU_MAIL_00 + 1), 0x60, 0x40);
        HAL_AUR2_WriteMaskByte((REG_R2_1_RIU_MAIL_00 + 1), 0x60, 0x40);
    }
    else //default  //MIU0
    {
        HAL_AUR2_WriteMaskByte((REG_R2_RIU_MAIL_00 + 1), 0x60, 0x20);
  ...
  ********************************************************/
    HAL_AUDIO_WriteMaskByte(REG_AUDIO_SYNTH_EXPANDER, 0x02, 0x00);        //BANK 1130 control by MCU
    HALMAD2_PRINT(" ======> Audio DEC_R2 Base : %x\n", (unsigned int) DEC_R2_ADDR);
    ...
    HAL_AUR2_WriteReg(REG_DECR2_IO3_MAPPING_BASE_HI, 0xB000);
  ...
  ********************************************************/
    HAL_AUDIO_WriteMaskByte(REG_AUDIO_SYNTH_EXPANDER, 0x01, 0x00);        //BANK 1129 control by MCU
    HALMAD2_PRINT(" ======> Audio SND_R2 Base : %08x\n", (unsigned int) SND_R2_ADDR);
...
MS_U32 HAL_MAD2_Read_DSP_sram(MS_U16 dsp_addr,AUDIO_DSP_MEMORY_TYPE dsp_memory_type)
{
    ...
    {
        HALMAD2_ERROR("ERROR: SE DSP IDMA Busy \n");
...
MS_BOOL HAL_MAD2_ReLoadCode(MS_U8 u8Type)
{
    ...
    if(u8Type == AU_SIF_BTSC) // only BTSC need to stop play command when reload DSP codec
    {
        Dvb2DecCmd_tmp = HAL_AUDIO_Se_Status();
        HAL_MAD2_SetDecCmd(0);                      // Stop
    }
    ...
    if(u8Type == AU_SIF_BTSC) // only BTSC need to restore play command when reload DSP codec
    {
        HAL_MAD2_SetDecCmd((MS_U16)Dvb2DecCmd_tmp);
    }
...
void HAL_MAD2_RebootDsp(void)
{
    ...
    MS_U8 dspcmd_status, R2dec1cmd_status, R2dec2cmd_status;
    MS_U8 u8SifCodeType = HAL_SIF_GetDspCodeType();
    MS_U8 u8EncCodeType = HAL_AUDIO_GetDsp2CodeType();
    ...
    dspcmd_status = HAL_MAD2_GetDecCmd();
    R2dec1cmd_status = HAL_MAD_GetDecCmd();
    R2dec2cmd_status = HAL_MAD_GetDSP2DecCmd();

    HAL_MAD2_SetDecCmd(0);
    ...
    // load DSP2 code
    HAL_AUDSP_DspLoadCode(u8SifCodeType);  // Sif
    HAL_AUDSP_DspLoadCode(u8EncCodeType);  // Encoder
    ...
    HAL_MAD2_SetDecCmd((MS_U16)dspcmd_status);
    HAL_MAD_SetDecCmd(R2dec1cmd_status);
    HAL_MAD_SetDSP2DecCmd(R2dec2cmd_status);
