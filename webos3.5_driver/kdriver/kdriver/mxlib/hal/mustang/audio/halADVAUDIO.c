...
void  HAL_SOUND_SRS_APOLLO_SetPara(MS_U8 u8mode, MS_U32 u32param, MS_U16 u16value2)
{
    ...
    if(u16value2 == 0)//for previous user, u16 parameter
        FXP24Value = long_div((8388607*(MS_U64)u32param), 1000);
...
MS_BOOL HAL_ADVSOUND_ProcessEnable(AUDIO_DSP_CODE_TYPE type)
{
    ...
    MS_BOOL ret_status = FALSE;
        ...
        else if((type == AU_DVB2_ADVSND_TSHD) ||(type == AU_DVB2_ADVSND_THEATERSOUND) ||(type == AU_DVB2_ADVSND_STUDIOSOUND_3D))
        {
            ...
            {
                HAL_AUDIO_WriteMaskByte(REG_SOUND_ADV_CFG2,0xFF,0x00);         //Reset the pure snd setting
                HAL_AUDIO_WriteMaskByte(REG_SOUND_ADV_CFG3,0xFF,0x00);         //Reset the pure snd setting
...
MS_BOOL HAL_ADVSOUND_SubProcessEnable(ADVFUNC proc, MS_BOOL enable)
{
    ...
    switch (proc)
    {
        ...
        //LGSE
        case LG_SOUNDENGINE_EN:
            if(enable)
                HAL_AUR2_WriteMaskReg(REG_DECR2_LGSE_SEL, 0x0001, 0x0001);
            else
                HAL_AUR2_WriteMaskReg(REG_DECR2_LGSE_SEL, 0x0001, 0x0000);
            status = TRUE;
            break;
...
MS_BOOL HAL_ADVSOUND_SetParam(ADVSND_PARAM param, MS_U32 u32value1, MS_U16 u16value2)
{
    ...
    switch (param)
    {
        ...
        case DBXTV_SET_MODE:
            {
                MS_U8 u8param1 = (EN_DBXTV_TotSonMode)((MS_U8)(u32value1 >> 16));
                MS_U8 u8param2 = (EN_DBXTV_TotVolMode)((MS_U8)(u32value1 >> 8));
                MS_U8 u8param3 = (EN_DBXTV_TotSurMode)((MS_U8)u32value1);              	
                HAL_DBXTV_SetMode(u8param1, u8param2, u8param3, (MS_U32)(u16value2));
            }
            status = TRUE;
            break;
...
void HAL_AUDIO_COPY_Parameter(ADVSND_PARAM type, void* Parameter_ptr, MS_U32 size)
{
    switch (type)
    {
        ...
        case DBX_COPY_TotSurDM:     //update for bugs
        case DBX_COPY_TotSurPM:
            HAL_SOUND_DBX_COPY_PARAMETERS_FROM_APP(type, (MS_U32 *)Parameter_ptr, size);
            break;
        ...
        case SONICEMOTION_ABS3D_SET_PARAMS:
            {
                MS_U32 offset, size;
