...
//  Macro and Define
//-------------------------------------------------------------------------------------------------
//SW DMA Reader configuration
#define NUM_OF_SW_DMA  1
#define NUM_OF_HW_DMA  2
...
#define R1BYTE(u32Addr, u8mask) \
    (READ_BYTE (_gMIO_MapBase + ((u32Addr) << 1) - ((u32Addr) & 1)) & (u8mask))

#define W1BYTE(u32Addr, u8Val, u8mask)      \
    (WRITE_BYTE(_gMIO_MapBase + ((u32Addr) << 1) - ((u32Addr) & 1), (R1BYTE(u32Addr, 0xFF) & ~(u8mask)) | ((u8Val) & (u8mask))))

// u32Addr must be 16bit aligned
#define R2BYTE(u32Addr, u16mask)  \
    (READ_WORD (_gMIO_MapBase + ((u32Addr) << 1)) & (u16mask))

// u32Addr must be 16bit aligned
#define W2BYTE(u32Addr, u16Val, u16mask)    \
    (WRITE_WORD(_gMIO_MapBase + ((u32Addr) << 1), (R2BYTE(u32Addr, 15:0) & ~(u16mask)) | ((u16Val) & (u16mask))))

///////////////////////////////////////////////////////////////
#define _AU_AbsWriteByte( u32Reg, u8Val )   \
     do {   \
     (WRITE_BYTE(_gMIO_MapBase + ((u32Reg) << 1) - ((u32Reg) & 1), u8Val)); \
          }while(0)

#define _AU_AbsWrite2Byte( u32Reg, u16Val ) \
     do {   \
     (WRITE_WORD(_gMIO_MapBase + ((u32Reg) << 1), u16Val)); \
          }while(0)

#define _AU_AbsWriteMaskByte( u32Reg, u8Mask, u8Val )   \
    do {    \
    (WRITE_BYTE(_gMIO_MapBase + ((u32Reg) << 1) - ((u32Reg) & 1), (R1BYTE((u32Reg), 0xFF) & ~(u8Mask)) | ((u8Val) & (u8Mask))));    \
        }while(0)

#define _AU_AbsReadByte( u32Reg) (READ_BYTE (_gMIO_MapBase + ((u32Reg) << 1) - ((u32Reg) & 1)))
#define _AU_AbsRead2Byte( u32Reg)  (READ_WORD (_gMIO_MapBase + ((u32Reg) << 1)) )
...
#else
    #define AUDIO_DELAY1US(x)                MsOS_DelayTaskUs_Poll(x)
    #define AUDIO_DELAY1MS(x)                MsOS_DelayTask(x)
#endif

#define AUDIO_HW_DMA_READER1_SUPPORT    1
#define AUDIO_HW_DMA_READER2_SUPPORT    1
#define AUDIO_SW_DMA_READER1_SUPPORT    1
#define AUDIO_R2_DMA_READER1_SUPPORT    1
#define AUDIO_R2_DMA_READER2_SUPPORT    1
#define AUDIO_PCM_CAPTURE1_SUPPORT      1
#define AUDIO_PCM_CAPTURE2_SUPPORT      1
#define AUDIO_HW_DMA_WRITER1_SUPPORT    1
...
void    HAL_AUDIO_R2ISR_0(void); // [LM14]
void    HAL_AUDIO_R2ISR_1(void); // [LM14]
...
void    HAL_AUDIO_SetCaptureEnable(MS_BOOL bEnable, void* PCM_CB); // [LM14]
...
void    _HAL_AUDIO_ARC_HWEN(MS_BOOL bEnable); // [LM14]
void    _HAL_AUDIO_ARC_HWEN_BY_PORT(AUDIO_HDMI_INDEX_T hdmiIndex, MS_BOOL bOnOff);//[M2]
...
MS_U16  HAL_AUDIO_HDMI_RX_GetHdmiInChannelStatus(void); // [LM14]
//================================================================
...
void    HAL_AUDIO_DmaReader_Init(PCM_OUTPUT_FORMAT* pSettings); // [LM14]
void    HAL_AUDIO_DmaReader_AllInput_Init(void);
...
//MS_U32 HAL_AUDIO_SwDmaReader_GeRemainingBufferSize(void);
...
void    HAL_AUDIO_PTS_info( MS_U32 );
...
extern MS_BOOL HAL_AUDSP_DspLoadCode(MS_U8 u8Type);
