...
#define AUR2_DBGMSG(msg)  MS_DEBUG_MSG(msg)
    ...
    #define HALAUR2_PRINT(fmt, args...)    LOGI("<<android>>      " fmt, ## args)
    #define HALAUR2_ERROR(fmt, args...)    LOGE("<<android>>      " fmt, ## args)
#else
    #ifdef CONFIG_MBOOT
        #define HALAUR2_PRINT(fmt, args...)
        #define HALAUR2_ERROR(fmt, args...)    printf("[[utopia]]      " fmt, ## args)
    #else
        #define HALAUR2_PRINT(fmt, args...)  //DBG_PREFIX("[[utopia]]      " fmt, ## args)
        #define HALAUR2_ERROR(fmt, args...)   printf("[[utopia]]      " fmt, ## args)
    #endif
#endif
...
#define REG_R2_1_CTRL_BASE           0x113000
...
#define DEC_R2_COMMON_INFO_COUNTER        500
#define SND_R2_COMMON_INFO_COUNTER        500
//-------------------------------------------------------------------------------------------------
//  Local Structures
//-------------------------------------------------------------------------------------------------
...
void HAL_DEC_R2_EnableR2(MS_BOOL bEnable )
{
    if (bEnable)
    {
        ...
        HAL_AUR2_WriteMaskByte(0x113040, 0xFF, 0x10);
        HAL_AUDIO_WriteMaskByte(REG_AUDIO_SYNTH_EXPANDER, 0x02, 0x02);        //BANK 1130 control by R2
    }
    else
    {
        HAL_AUDIO_WriteMaskByte(REG_AUDIO_SYNTH_EXPANDER, 0x02, 0x00);        //BANK 1130 control by MIU
...
void HAL_DEC_R2_init_SHM_param( void )
{
    ...
    for (dec_id = 0; dec_id < MAX_ADEC; dec_id++ )
    {
        ...
        if (p_shm_param->bootMagicID != SHM_PARAM_BOOT_MAGIC_ID)
        {
            ...
            p_shm_param->decCtrl = 0x0020; //bit 5 for 33 bit pts report under MM-TS
            ...
            HALAUR2_PRINT("[======= initialize DEC-R2[%d] SHM =======]\n", dec_id);
        }
    }

 // MsOS_Dcache_Flush((unsigned int)p_shm, sizeof(DEC_R2_SHARE_MEM));
 // audio SHM is non-cache, so no need for D-cache flush
    MsOS_FlushMemory();
...
void HAL_SND_R2_SetCommInfo(R2_AudioParamType infoType, Audio_id id, MS_U16 param1, MS_U16 param2)
{
    ...
    while( i < SND_R2_COMMON_INFO_COUNTER )
    {
...
void HAL_DEC_R2_SetCommInfo(R2_AudioParamType infoType, Audio_id id, MS_U16 param1, MS_U16 param2)
{
    ...
    while( i < DEC_R2_COMMON_INFO_COUNTER )
    {
    ...
    #if 0
    if (i >= DEC_R2_COMMON_INFO_COUNTER)
    {
        HAL_AUDIO_WriteMaskByte(REG_AUDIO_SYNTH_EXPANDER, 0x02, 0x00);        //BANK 1130 control by MIU
        printf("R2 PC:");
        for( i=0; i<10; i++)
        {
            MS_U16 tmp_H, tmp_L;
            tmp_H = HAL_AUR2_ReadReg(0x113012);
            tmp_L = HAL_AUR2_ReadReg(0x113010);
...
MS_U32 HAL_SND_R2_GetCommInfo(R2_AudioInfoType infoType, Audio_id id, MS_U16 param1, MS_U16 param2)
{
    ...
    if ( i >= SND_R2_COMMON_INFO_COUNTER )
    {
        HALAUR2_ERROR("%s:%x, %x, %x, %x cmd fail\n",__FUNCTION__, infoType, id, param1, param2);
        return 0;
    }
...
MS_U32 HAL_DEC_R2_GetCommInfo(R2_AudioInfoType infoType, Audio_id id, MS_U16 param1, MS_U16 param2)
{
    ...
    if ( i >= DEC_R2_COMMON_INFO_COUNTER )
    {
        HALAUR2_ERROR("%s: 0x%x, 0x%x, 0x%x, 0x%x cmd fail\n",__FUNCTION__, infoType, id, token, HAL_AUR2_ReadByte( REG_R2_1_CMD_TOKEN ));
        OS_RELEASE_MUTEX(_s32AUDIOMutexIDMA);
        return 0;
    }
...
MS_U32 HAL_SND_R2_Get_SHM_INFO(R2_SHM_INFO_TYPE decInfoType, MS_U8 dec_id )
{
    ...
    //D-cache Invalidate
    MsOS_Dcache_Flush((MS_U32) ptr, sizeof(MS_U32));
    ret_value = *ptr;
#endif
...
MS_U32 HAL_DEC_R2_Get_SHM_INFO(R2_SHM_INFO_TYPE decInfoType, MS_U8 dec_id )
{
    ...
    volatile MS_U32 *ptr;
    DEC_R2_SHARE_MEM  *p_shm = (DEC_R2_SHARE_MEM *)(MS_PA2KSEG1(R2_1_DDR_BASE+DEC_R2_SHM_DDR_OFFSET));
    AUR2_ADEC_INFO_SHM *p_shm_info = NULL;
    AUR2_ADEC_ES_INFO_SHM   *p_shm_esinfo = NULL;
    MS_U8 es_id = 0;

    if (dec_id >= MAX_ADEC)
    {
        AUR2_DEBUG_PRINT(printf("[%s] invalid dec_id %d \n", __FUNCTION__, dec_id));
        return 0; //invalid index
    }

    p_shm_info = &p_shm->adec_info_shm[dec_id];

    if(dec_id == 1)
    {
        es_id = 2;
    }
    p_shm_esinfo = &p_shm->adec_esInfo_shm[es_id];
    ...
    switch( decInfoType )
    {
        ...
        case R2_SHM_INFO_DTS_PCM_FRAME_SIZE:        ptr = (volatile MS_U32 *) &p_shm_info->dts_info.dts_PCM_frame_size;   break;
        case R2_SHM_INFO_DTS_ES_TYPE:               ptr = (volatile MS_U32 *) &p_shm_info->dts_info.dts_ES_Type;          break;
        case R2_SHM_INFO_SPDIF_INFO_FLAG:           ptr = (volatile MS_U32 *) &p_shm_info->common_info.spdif_info_flag;   break;
        ...
        case R2_SHM_INFO_SECURITY_INFO:             ptr = (volatile MS_U32 *) &p_shm->Security_Check;                     break;
#endif
        default:
            //ret_value = 0;
            return 0;
    ...
    // HALAUR2_PRINT("[%s] shmType:%4x, id:%x ==> %x\n", __FUNCTION__, decInfoType, dec_id, ret_value);
...
MS_BOOL HAL_DEC_R2_Set_SHM_PARAM(R2_SHM_PARAM_TYPE decParamType, MS_U8 dec_id, MS_U32 param, MS_U32 param2 )
{
    ...
    volatile MS_U32 *ptr;
    DEC_R2_SHARE_MEM  *p_shm = (DEC_R2_SHARE_MEM *)(MS_PA2KSEG1(R2_1_DDR_BASE+DEC_R2_SHM_DDR_OFFSET));
    AUR2_ADEC_PARAM_SHM     *p_shm_param = NULL;
    AUR2_ADEC_ES_PARAM_SHM  *p_shm_esparam = NULL;
    MS_U32 offset_R2_to_DSP, offset_R2_to_CommDDR;
    ...
    switch( decParamType )
    {
        case R2_SHM_PARAM_MM_TRICK:
                ptr = (volatile MS_U32 *) &p_shm_param->fast_forward;
                switch(param)
                {
                    case 15:    // 1.5x
                    case 20:    // 2x
                ...
                // audio SHM is non-cache, so no need for D-cache flush
                MsOS_FlushMemory();
                // MsOS_Dcache_Flush((MS_U32)ptr,4);
                break;
        ...
        case R2_SHM_PARAM_EXT_SMPRATE_CTRL:
                ptr = (volatile MS_U32 *) &p_shm_param->extSynthsizer_en;
                *ptr = param;
                MsOS_FlushMemory();
                ptr = (volatile MS_U32 *) &p_shm_param->extSynthsizer_value;
                *ptr = param2;
                MsOS_FlushMemory();
                break;


        case R2_SHM_PARAM_MPG_AVOFFSET:     ptr = (volatile MS_U32 *) &p_shm_param->dtv_mpg_avDelay;     *ptr = param;  MsOS_FlushMemory();     break;
        case R2_SHM_PARAM_AC3_AVOFFSET:     ptr = (volatile MS_U32 *) &p_shm_param->dtv_ac3_avDelay;     *ptr = param;   MsOS_FlushMemory();    break;
        case R2_SHM_PARAM_AAC_AVOFFSET:     ptr = (volatile MS_U32 *) &p_shm_param->dtv_aac_avDelay;     *ptr = param;   MsOS_FlushMemory();    break;
        case R2_SHM_PARAM_MM_FILE_REQ_SIZE: ptr = (volatile MS_U32 *) &p_shm_param->mmFileReqSize;     *ptr = param;   MsOS_FlushMemory();    break;
        ...
        case R2_SHM_PARAM_UNI_SETUP_DECODE_FRMCNT:  ptr = (volatile MS_U32 *) &p_shm_param->mmUni_needDecodeFrmCnt;   *ptr = param; MsOS_FlushMemory();  break;
        ...
        case R2_SHM_PARAM_DOLBY_DMX_MODE:   ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.dmxMode;      *ptr = param;   MsOS_FlushMemory();    break;
        case R2_SHM_PARAM_DOLBY_HIGH_CUT:   ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.high_cut;      *ptr = param;  MsOS_FlushMemory();     break;
        case R2_SHM_PARAM_DOLBY_LOW_BOOST:  ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.low_boost;      *ptr = param;  MsOS_FlushMemory();     break;
        case R2_SHM_PARAM_DOLBY_GAIN:       ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.gain;      *ptr = param;  MsOS_FlushMemory();     break;
        case R2_SHM_PARAM_DOLBY_MUL_FRAME_HEADER:       ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.mul_frame_header;      *ptr = param;  MsOS_FlushMemory();     break;
        case R2_SHM_PARAM_DOLBY_DDP_HDMI_BYPASS:   ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.DDP_HDMI_bypass;      *ptr = param;  MsOS_FlushMemory();     break;
        case R2_SHM_PARAM_DOLBY_AAC_BYPASS:       ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.AAC_HDMI_bypass;      *ptr = param;  MsOS_FlushMemory();     break;
        case R2_SHM_PARAM_DOLBY_TB11_ENABLE:      ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.TB11_enable;      *ptr = param;  MsOS_FlushMemory();     break;
        case R2_SHM_PARAM_DOLBY_MIXER_BALANCE:
                {
                    MS_S32 temp = 0;
                    switch((DOLBY_MIXER_BALANCE_MODE)param)
                    {
                        case BALANCE_MAIN_0:
                            temp = -32;
                            break;

                        case BALANCE_MAIN_25:
                            temp = -16;
                            break;

                        case BALANCE_MAIN_75:
                            temp = 16;
                            break;

                        case BALANCE_MAIN_100:
                            temp = 32;
                            break;
                    ...
                    ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.mixer_balance;      *ptr = temp;  MsOS_FlushMemory();
                }
                break;
        ...
        case R2_SHM_PARAM_MPEG_SOUNDMODE:        ptr = (volatile MS_U32 *) &p_shm_param->mpeg_param.MPEG_soundMode;     *ptr = param;      break;
        case R2_SHM_PARAM_MPEG_GAIN:             ptr = (volatile MS_U32 *) &p_shm_param->mpeg_param.gain;               *ptr = param;      break;

        case R2_SHM_PARAM_OMX_SPDIF_CTRL:        ptr = (volatile MS_U32 *) &p_shm_param->OMX_SPDIF_Ctrl;                *ptr = param;      break;
        case R2_SHM_PARAM_OMX_SPDIF_PCM_LEVEL:   ptr = (volatile MS_U32 *) &p_shm_param->OMX_SPDIF_PCM_Level;           *ptr = param;      break;

        case R2_SHM_PARAM_MAIN_VOLUME:           ptr = (volatile MS_U32 *) &p_shm_param->adMix_mainVol;                 *ptr = param;      break;
        case R2_SHM_PARAM_MAIN_VOLUME_MUTE:
                ...
                if(param)
                    ...
                    *ptr &= 0x000003FF;
                break;
        case R2_SHM_PARAM_AD_VOLUME:             ptr = (volatile MS_U32 *) &p_shm_param->adMix_asocVol;                 *ptr = param;      break;
        ...
        case R2_SHM_PARAM_COOK_NUMCODECS:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.mNumCodecs;        *ptr = param;    break;
        case R2_SHM_PARAM_COOK_SAMPLES:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.mSamples;          *ptr = param;    break;
        case R2_SHM_PARAM_COOK_SAMPLERATES:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.mSampleRate;       *ptr = param;    break;
        case R2_SHM_PARAM_COOK_CHANNELS:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.Channels[param2];  *ptr = param;    break;
        case R2_SHM_PARAM_COOK_REGIONS:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.Regions[param2];   *ptr = param;    break;
        case R2_SHM_PARAM_COOK_CPLSTART:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.cplStart[param2];   *ptr = param;    break;
        case R2_SHM_PARAM_COOK_CPLQBITS:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.cplQbits[param2];   *ptr = param;    break;
        case R2_SHM_PARAM_COOK_FRAMESIZE:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.FrameSize[param2];  *ptr = param;    break;

        case R2_SHM_PARAM_DTS_DMX_LFE_ENABLE:
            ptr = (volatile MS_U32 *) &p_shm_param->dts_param.dmxLfeEnable;      *ptr = param;      break;
        case R2_SHM_PARAM_DTS_DMX_MODE:
            ptr = (volatile MS_U32 *) &p_shm_param->dts_param.dmxMode;           *ptr = param;      break;
        case R2_SHM_PARAM_DTS_SEAMLESS_MODE:
            ptr = (volatile MS_U32 *) &p_shm_param->dts_param.seamless;    *ptr = param;      break;
        case R2_SHM_PARAM_DTS_HD_HDMI_BYPASS:
            ptr = (volatile MS_U32 *) &p_shm_param->dts_param.DTSHD_HDMI_bypass;      *ptr = param;     break;

        case R2_SHM_PARAM_ASF_VERSION:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.asf_version;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_CHANNELS:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.nChannels;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_SAMPLERATE:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.smpRate;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_BYTERATE:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.byteRate;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_BLOCKALIGN:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.blockAlign;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_ENCOPT:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.encOpt;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_PARSINGBYAPP:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.parsingByApp;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_BITS_PER_SAMPLE:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.bitsPerSmp;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_CHANNELMASK:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.channelMask;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_DRC_PARAM_EXIST:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.drcParamExist;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_DRC_RMS_AMP_REF:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.drcRmsAmpRef;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_DRC_RMS_AMP_TARGET:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.drcRmsAmpTarget;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_DRC_PEAK_AMP_REF:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.drcPeakAmpRef;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_DRC_PEAK_AMP_TARGET:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.drcPeakAmpTarget;      *ptr = param;  break;
        case R2_SHM_PARAM_ASF_MAX_PACKET_SIZE:
            ptr = (volatile MS_U32 *) &p_shm_param->asf_param.maxPacketSize;      *ptr = param;  break;

        case R2_SHM_PARAM_XPCM_TYPE:
            ptr = (volatile MS_U32 *) &p_shm_param->xpcm_param.xPcmType;      *ptr = param;  break;
        case R2_SHM_PARAM_XPCM_CHANNELS:
            ptr = (volatile MS_U32 *) &p_shm_param->xpcm_param.nChannels;      *ptr = param;  break;
        case R2_SHM_PARAM_XPCM_SAMPLERATES:
            ptr = (volatile MS_U32 *) &p_shm_param->xpcm_param.smpRate;      *ptr = param;  break;
        case R2_SHM_PARAM_XPCM_BITS_PER_SAMPLE:
            ptr = (volatile MS_U32 *) &p_shm_param->xpcm_param.bitsPerSmp;      *ptr = param;  break;
        case R2_SHM_PARAM_XPCM_BLOCK_SIZE:
            ptr = (volatile MS_U32 *) &p_shm_param->xpcm_param.blockSize;      *ptr = param;  break;
        case R2_SHM_PARAM_XPCM_SAMPLE_PER_BLOCK:
            ptr = (volatile MS_U32 *) &p_shm_param->xpcm_param.samplePerBlock;      *ptr = param;  break;
        case R2_SHM_PARAM_SPDIF_TYPE:
            ptr = (volatile MS_U32 *) &p_shm_param->spdif_type;      *ptr = param;   break;

        case R2_SHM_PARAM_RFSIGNAL_TYPE:
            ptr = (volatile MS_U32 *) &p_shm->RfSignalType;      *ptr = param;   break;

        case R2_SHM_PARAM_VORBIS_HEADER_SIZE:
            ptr = (volatile MS_U32 *) &p_shm_param->vorbis_header_size;      *ptr = param;     break;

        case R2_SHM_PARAM_TTS_EN:
            ptr = (volatile MS_U32 *) &p_shm_param->TTS_On_flag;      *ptr = param;      break;

#if 1
        case R2_SHM_PARAM_DAP_EN:
            ptr = (volatile MS_U32 *) &p_shm_param->DAP_enable;      *ptr = param;    break;

        case R2_SHM_PARAM_SPEAKER_EN:
            ptr = (volatile MS_U32 *) &p_shm_param->speaker_enable;      *ptr = param;    break;

        case R2_SHM_PARAM_DDP_ENCODE_EN:
            ptr = (volatile MS_U32 *) &p_shm_param->DDP_encode_enable;      *ptr = param;    break;
#endif

        case R2_SHM_PARAM_DPGA_FLAG:
            ptr = (volatile MS_U32 *) &p_shm_param->DPGA_Disable_flag;      *ptr = param;  break;

        case R2_SHM_PARAM_ES_Limiter_EN:
            ptr = (volatile MS_U32 *) &p_shm_param->ES_Limiter_EN;      *ptr = param;  break;

        case R2_SHM_PARAM_ES_Limiter_Threshold:
            ptr = (volatile MS_U32 *) &p_shm_param->ES_Limiter_Threshold;      *ptr = param;   break;

        case R2_SHM_PARAM_PCM_Limiter_EN:
            ptr = (volatile MS_U32 *) &p_shm_param->PCM_Limiter_EN;      *ptr = param;    break;

        case R2_SHM_PARAM_PCM_Limiter_Threshold:
            ptr = (volatile MS_U32 *) &p_shm_param->PCM_Limiter_Threshold;      *ptr = param;   break;

        case R2_SHM_PARAM_MULTI_CHANNEL:
            ptr = (volatile MS_U32 *) &p_shm_param->force_mmuni_mch_pcm;    *ptr = param;    break;
    ...
    MsOS_FlushMemory();
    UNUSED(offset_R2_to_DSP);
    UNUSED(offset_R2_to_CommDDR);
    UNUSED(ret_value);

    //  HALAUR2_PRINT("[%s] shmType:%4x, id:%x, param: [%08lx, %08lx]\n", __FUNCTION__, decParamType, dec_id, param, param2);
    return TRUE;
}
...
MS_U32 HAL_DEC_R2_Get_SHM_PARAM(R2_SHM_PARAM_TYPE decParamType, MS_U8 dec_id, MS_U32 param )
{
    ...
    p_shm_param = &p_shm->adec_param_shm[dec_id];
    p_shm_esparam = &p_shm->adec_esParam_shm[dec_id];
    ...
    switch( decParamType )
    {
        ...
        case R2_SHM_PARAM_MPG_AVOFFSET:     ptr = (volatile MS_U32 *) &p_shm_param->dtv_mpg_avDelay;     ret_value = *ptr;       break;
        case R2_SHM_PARAM_AC3_AVOFFSET:     ptr = (volatile MS_U32 *) &p_shm_param->dtv_ac3_avDelay;     ret_value = *ptr;       break;
        case R2_SHM_PARAM_AAC_AVOFFSET:     ptr = (volatile MS_U32 *) &p_shm_param->dtv_aac_avDelay;     ret_value = *ptr;       break;
        case R2_SHM_PARAM_MM_FILE_REQ_SIZE: ptr = (volatile MS_U32 *) &p_shm_param->mmFileReqSize;       ret_value = *ptr;       break;
        case R2_SHM_PARAM_MM_INPUT_PTS:     ptr = (volatile MS_U32 *) &p_shm_esparam->MM_inputPTS;        ret_value = *ptr;       break;
        case R2_SHM_PARAM_DOLBY_DRC_MODE:
        case R2_SHM_PARAM_DOLBY_RF_MODE:
                ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.drcMode;
                ret_value = *ptr;
        ...
        case R2_SHM_PARAM_DOLBY_DMX_MODE:   ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.dmxMode;      ret_value = *ptr;       break;
        case R2_SHM_PARAM_DOLBY_HIGH_CUT:   ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.high_cut;      ret_value = *ptr;       break;
        case R2_SHM_PARAM_DOLBY_LOW_BOOST:  ptr = (volatile MS_U32 *) &p_shm_param->dolby_param.low_boost;  ret_value = *ptr;       break;
        case R2_SHM_PARAM_UNI_SETUP_DECODE_FRMCNT:  ptr = (volatile MS_U32 *) &p_shm_param->mmUni_needDecodeFrmCnt;      ret_value = *ptr;       break;

        case R2_SHM_PARAM_OMX_SPDIF_CTRL:
                ptr = (volatile MS_U32 *) &p_shm_param->OMX_SPDIF_Ctrl;      ret_value = *ptr;       break;
      ...
      case R2_SHM_PARAM_COOK_NUMCODECS:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.mNumCodecs;     ret_value = *ptr;       break;
        case R2_SHM_PARAM_COOK_SAMPLES:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.mSamples;      ret_value = *ptr;       break;
        case R2_SHM_PARAM_COOK_SAMPLERATES:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.mSampleRate;     ret_value = *ptr;       break;
        case R2_SHM_PARAM_COOK_CHANNELS:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.Channels[param];     ret_value = *ptr;       break;
        case R2_SHM_PARAM_COOK_REGIONS:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.Regions[param];      ret_value = *ptr;       break;
        case R2_SHM_PARAM_COOK_CPLSTART:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.cplStart[param];      ret_value = *ptr;       break;
        case R2_SHM_PARAM_COOK_CPLQBITS:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.cplQbits[param];     ret_value = *ptr;       break;
        case R2_SHM_PARAM_COOK_FRAMESIZE:
            ptr = (volatile MS_U32 *) &p_shm_param->cook_param.FrameSize[param];      ret_value = *ptr;       break;
        case R2_SHM_PARAM_HASH_KEY:
            ptr = (volatile MS_U32 *) &p_shm_param->hashKey;      ret_value = *ptr;       break;
...
//     HALAUR2_PRINT("[%s] shmType:%4x, id:%x, param: [%08lx] ==> %x\n", __FUNCTION__, decParamType, dec_id, param, ret_value);

    return ret_value;
}

MS_BOOL HAL_AUR2_LGSE_Set_param(MS_U32 infoType, MS_U32 index, MS_U32 *pParam, MS_U16 noParam)
{
    MS_U32 offset;

    //printf("\033[0;31m @@@@@@@@@@@@@@@@@@[HAL_AUR2_LGSE_Set_param] [0x%X] [0x%X] [0x%X] [%d] \033[0m \n", infoType, index, *(pParam), noParam);
    offset = HAL_SND_R2_GetCommInfo(infoType, 0, 0, 0);
    if(offset == 0)
    {
        HALAUR2_ERROR("@@ LGSE_Set_param 0x%lx fail !\n", infoType);
        return FALSE;
    }


    //printf("\n\033[0;31m@@@@@@@@@@@@@@@@@@@@ addr = [0x%X] + [0x%X] + [0x%X] \033[0m \n", HAL_AUDIO_GetDspMadBaseAddr(2) + ADEC__R2_DDR_SIZE, offset, index);

    //printf("\n\033[0;31m@@@@@@@@@@@@@@@@@@@@ addr = [0x%X], vaddr = [0x%X]\033[0m \n", HAL_AUDIO_GetDspMadBaseAddr(2) + ADEC__R2_DDR_SIZE + offset + index, MsOS_PA2KSEG1(HAL_AUDIO_GetDspMadBaseAddr(2) + ADEC__R2_DDR_SIZE + offset + index));

    memcpy((void *)MsOS_PA2KSEG1(HAL_AUDIO_GetDspMadBaseAddr(2)+ADEC__R2_DDR_SIZE+offset + index), (void *)pParam, noParam);

    MsOS_FlushMemory();

    HAL_SND_R2_SetCommInfo(infoType, 0, 0, noParam);

    return TRUE;
}

MS_BOOL HAL_AUR2_LGSE_Get_param(MS_U32 infoType, MS_U32 index, MS_U32 *pParam, MS_U16 noParam)
{
    MS_U32 offset;

    infoType = infoType + 0x100;
    offset  = HAL_SND_R2_GetCommInfo(infoType, 0, 0, 0);
    if(offset == 0)
    {
        HALAUR2_ERROR("@@ LGSE_Get_param 0x%lx fail !\n", infoType);
        return FALSE;
    }

    memcpy((void *)pParam, (void *)MsOS_PA2KSEG1(HAL_AUDIO_GetDspMadBaseAddr(2)+ADEC__R2_DDR_SIZE+offset + index), noParam);
    MsOS_FlushMemory();
