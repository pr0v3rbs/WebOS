...
//[LM14A]
extern AUDIO_OMX_ARENDER_PATH_ID g_OMX_ArenderPathID;
...
//[LM14A]
extern MS_BOOL g_TV_Service_CH5_bMute;
extern MS_BOOL g_TV_Service_CH6_bMute;
void HAL_SOUND_SetMute(MS_U8 u8Path, MS_BOOL bEnable)
{
    //printf("\n\033[1;36m [AUDIO][UTOPIA][%s] [%s] [%d] [u8Path = %d] [bEnable = %d] \033[0m \n", __FILE__, __FUNCTION__, __LINE__, u8Path, bEnable);/*@audio*/
    DBG_SOUND(printf("HAL_SOUND_SetMute = %x\n", (MS_U8)bEnable ));

    switch(u8Path)
    {
        //M2: CH5: HW DMA, CH6: SW DMA
        //M2: only have one SW DMA reader "0"
        case AUDIO_PATH_SW_DMA_0:
        case AUDIO_PATH_SW_DMA_1:
        {
            printf("\r\n \033[1;36m === [u8Path: 0x%x]mute sw dma rdr:  (%d) === \033[0m \r\n", u8Path, (int)bEnable);
            if(bEnable == TRUE)
            {
            ...
            else
            {
                ...
                HAL_AUDIO_AbsWriteMaskReg(0x112D5C, 0x0002, 0x0000);
                HAL_AUDIO_AbsWriteMaskReg(0x112D5E, 0x0002, 0x0000);
            }
        }
        break;
        ...
        {
            printf("\r\n \033[1;36m === [u8Path: 0x%x]mute HW dma 1 rdr:  (%d) === \033[0m \r\n", u8Path, (int)bEnable);
            if(bEnable)
            {
        ...
        case AUDIO_PATH_HW_DMA_2:
        {
            printf("\r\n \033[1;36m === [u8Path: 0x%x]mute HW dma 2 rdr:  (%d) === \033[0m \r\n", u8Path, (int)bEnable);
            if(bEnable)
            {
                HAL_AUDIO_WriteMaskByte(REG_SOUND_CH6_MIX_VOL_FRC, 0x01, 0x01);
            }
            else
            {
                HAL_AUDIO_WriteMaskByte(REG_SOUND_CH6_MIX_VOL_FRC, 0x01, 0x00);
                HAL_AUDIO_WriteMaskByte(REG_SOUND_CH6_MIX_VOL_INT, 0x80, 0x00);
            }
        }
        break;

        case AUDIO_PATH_R2_DMA_1:
        {
            if(bEnable)
            {
                HAL_AUR2_WriteMaskByte(R2_DMARDR1_EN + 1, 0x30, 0x30);
            }
            else
            {
                HAL_AUR2_WriteMaskByte(R2_DMARDR1_EN + 1, 0x30, 0x00);
            }
        }
        break;

        case AUDIO_PATH_R2_DMA_2:
        {
            if(bEnable)
            {
                HAL_AUR2_WriteMaskByte(R2_DMARDR2_EN + 1, 0x30, 0x30);
            }
            else
            {
                HAL_AUR2_WriteMaskByte(R2_DMARDR2_EN + 1, 0x30, 0x00);
            }
        }
        break;
...
void HAL_SOUND_SetBalanceMask(AUDIO_OUTPUT_TYPE u8Output, MS_BOOL u8Enable)
{
    ...
    MS_BOOL ctrl_flag = FALSE; //DMR doesn't use MStar balance
    ...
    if (ctrl_flag)
    {
        g_BalanceMask = ((g_BalanceMask & (~(BalanceMaskBit))) | (u8Enable<<BalanceMaskBit));
...
void HAL_SOUND_SetCH1AudioDelay(MS_U16 u16Delay)
{
    ...
    {
        DBG_SOUND(printf("Warning! Invalid value (%u), main audio delay should be less than %u ms. \r\n", u16Delay, AUDIO_DELAY_UPPER_BOUND));
        u16Delay = AUDIO_DELAY_UPPER_BOUND;
    }

    if (u16Delay < AUDIO_DELAY_LOWER_BOUND)
    {
        DBG_SOUND(printf("Warning! Invalid value (%u), main audio delay should be larger than %u ms. \r\n", u16Delay, AUDIO_DELAY_LOWER_BOUND));
        u16Delay = AUDIO_DELAY_LOWER_BOUND;
    }
...
////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_SOUND_SetMultiCHAudioDelay()
/// @brief \b Function \b Description: This routine is used to set the u16Value of audio delay.
/// @param <IN>        \b delay    :   in ms unit
/// @param <OUT>       \b NONE    :
/// @param <RET>       \b NONE    :
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void HAL_SOUND_SetMultiCHAudioDelay(MS_U16 u16Delay)
{
    HAL_MAD2_Write_DSP_sram(DSP2PmAddr_multiChInputDelay, u16Delay, DSP_MEM_TYPE_PM);
}

////////////////////////////////////////////////////////////////////////////////
/// @brief \b Function \b Name: HAL_SOUND_GetMultiCHAudioDelay()
/// @brief \b Function \b Description: This routine is used to get the u16Value of audio delay.
/// @param <RET>        \b delay    :  in ms unit
////////////////////////////////////////////////////////////////////////////////
MS_U16 HAL_SOUND_GetMultiCHAudioDelay(void)
{
    return(HAL_MAD2_Read_DSP_sram(DSP2PmAddr_multiChInputDelay, DSP_MEM_TYPE_PM));
}
...
void HAL_SOUND_SetSpdifAudioDelay(MS_U16 u16Delay)
{
    ...
    {
        DBG_SOUND(printf("Warning! Invalid value (%u), spdif audio delay should be less than %u ms. \r\n", u16Delay, SPDIF_DELAY_UPPER_BOUND));
        u16Delay = AUDIO_DELAY_UPPER_BOUND_SPDIF;
    }

    if (u16Delay < AUDIO_DELAY_LOWER_BOUND_SPDIF)
    {
        DBG_SOUND(printf("Warning! Invalid value (%u), spdif audio delay should be larger than %u ms. \r\n", u16Delay, SPDIF_DELAY_LOWER_BOUND));
...
void HAL_SOUND_SetMixModeVolume(AUDIO_SOURCE_INFO_TYPE eSourceType, AUDIO_MIX_VOL_TYPE VolType, MS_U8 u8Vol1, MS_U8 u8Vol2  )
{
    switch (eSourceType)
    {
        case E_AUDIO_INFO_KTV_IN:
            ...
            {
                case PCM_VOL:
                    HAL_AUDIO_WriteMaskByte(0x2D5B, 0x7F, u8Vol1);
                    HAL_AUDIO_WriteMaskByte(0x2D5A, 0xE0, (u8Vol2<<5));
                    break;
                case MIC_VOL:
                    HAL_AUDIO_WriteMaskByte(0x2D5D, 0x7F, u8Vol1);
                    HAL_AUDIO_WriteMaskByte(0x2D5C, 0xE0, (u8Vol2<<5));
                    break;
                case MP3_VOL:
                    HAL_AUDIO_WriteMaskByte(0x2D5F, 0x7F, u8Vol1);
                    HAL_AUDIO_WriteMaskByte(0x2D5E, 0xE0, (u8Vol2<<5));
		     break;
		 case ECHO1_VOL:
		 	if(u8Vol1<14)
		 	    u8Vol1 = 14;
...
{
    //printf("\n\033[1;36m [AUDIO][UTOPIA][%s] [%s] [%d] [eSourceType = %d] [VolType = %d] [EnMute = %d] \033[0m \n", __FILE__, __FUNCTION__, __LINE__, eSourceType, VolType, EnMute);/*@audio*/
    switch (eSourceType)
    {
        case E_AUDIO_INFO_KTV_IN:
            switch(VolType)
            {
                case PCM_VOL:
                    if(EnMute)  // Mute
                        HAL_AUDIO_WriteMaskByte(0x2D5B, 0x80, 0x80);
                    else        // UnMute
                        HAL_AUDIO_WriteMaskByte(0x2D5B, 0x80, 0x00);
                    break;
                case MIC_VOL:
                    if(EnMute)  // Mute
                        HAL_AUDIO_WriteMaskByte(0x2D5D, 0x80, 0x80);
                    else        // UnMute
                        HAL_AUDIO_WriteMaskByte(0x2D5D, 0x80, 0x00);
                    break;
                case MP3_VOL:
                    if(EnMute)  // Mute
                        HAL_AUDIO_WriteMaskByte(0x2D5F, 0x80, 0x80);
                    else        // UnMute
                        HAL_AUDIO_WriteMaskByte(0x2D5F, 0x80, 0x00);
                    break;
                default:
                    break;
            }
...
MS_BOOL HAL_SND_SetParam(Sound_SET_PARAM_Type Type, MS_U16 param1, MS_U16 param2)
{
    ...
    switch(Type)
    {
        ...
        case Sound_SET_PARAM_AbsoluteEQ:                // absolute GEQ, param1: band 0~4, param2: 0x30(+12dB)~0x00(0dB)~0xD0(-12dB)
            if(param1 > 4)
            {
                DBG_SOUND(printf("Invalid value, GEQ Band should be 0~4\n"));
            }
            else
            {
        ...
        case Sound_SET_PARAM_PEQ_48K_A0:
            if((param1>>8) > MAX_PEQ_BAND-1)
                DBG_SOUND(printf("PEQ paramater Band should be 0~4\n"));
            else
