...
static MS_U8   u8sifSystemType = AU_SIF_NONE;
...
static MS_U32  _Convert_SIF_Reg(MS_U32 u32RegAddr)
{
    ...
    switch(u32RegAddr)
    {
       ...
       //     break;
        ...
        case REG_MB_DE_ACK1:
            resultReg = REG_MB_SE_ACK1;
            break;

        case REG_MB_SE_ACK2:
            resultReg = REG_MB_SE_ACK2;
            break;
...
void HAL_SIF_AbsWriteMaskByte(MS_U32 u32RegAddr, MS_U8 u8Mask, MS_U8 u8Val)
{
    u32RegAddr = _Convert_SIF_Reg(u32RegAddr);
    HAL_AUDIO_AbsWriteMaskByte(u32RegAddr, u8Mask, u8Val);
}

MS_U8 HAL_SIF_ReadByte(MS_U32 u32RegAddr)
{
    u32RegAddr = _Convert_SIF_Reg(u32RegAddr);
    return (HAL_AUDIO_ReadByte(u32RegAddr));
}

void HAL_SIF_WriteByte(MS_U32 u32RegAddr, MS_U8 u8Val)
{
    u32RegAddr = _Convert_SIF_Reg(u32RegAddr);
    HAL_AUDIO_WriteByte(u32RegAddr, u8Val);
}

void HAL_SIF_WriteMaskByte(MS_U32 u32RegAddr, MS_U8 u8Mask, MS_U8 u8Val)
{
    u32RegAddr = _Convert_SIF_Reg(u32RegAddr);
    HAL_AUDIO_WriteMaskByte(u32RegAddr, u8Mask, u8Val);
}

MS_U16 HAL_SIF_ReadReg(MS_U32 u32RegAddr)
{
    u32RegAddr = _Convert_SIF_Reg(u32RegAddr);
    return (HAL_AUDIO_ReadReg(u32RegAddr));
}

void HAL_SIF_WriteReg(MS_U32 u32RegAddr, MS_U16 u16Val)
{
    u32RegAddr = _Convert_SIF_Reg(u32RegAddr);
...
void HAL_SIF_TriggerSifPLL(void)
{
    ...
    if (bADCFromVifPathEnabled == TRUE) //enable VIF
    {
        ...
        HAL_SIF_AbsWriteMaskByte(0x112CB0, 0xE0, 0xE0);
    ...
    else    //enable SIF
    {
        ...
        HAL_SIF_AbsWriteMaskByte(0x103314, 0xFF, 0x00);  // start demod CLKGEN setting
        HAL_SIF_AbsWriteMaskByte(0x103315, 0xFF, 0x00);

        //HAL_SIF_AbsWriteMaskByte(0x112002, 0xFF, 0x74);
        HAL_SIF_AbsWriteMaskByte(0x112003, 0xFF, 0x00);  // start demod top setting

        HAL_SIF_AbsWriteMaskByte(0x1128C0, 0x02, 0x02);

        HAL_SIF_AbsWriteMaskByte(0x11286A, 0xFF, 0x83);  // start demod atop MPLL setting
        HAL_SIF_AbsWriteMaskByte(0x11286B, 0xFF, 0x00);
        AUDIO_DELAY1MS(1);
        HAL_SIF_AbsWriteMaskByte(0x11286A, 0xFF, 0x03);
        HAL_SIF_AbsWriteMaskByte(0x11286B, 0xFF, 0x00);

        HAL_SIF_AbsWriteMaskByte(0x112866, 0xFF, 0x01);
        HAL_SIF_AbsWriteMaskByte(0x112867, 0xFF, 0x12);

        HAL_SIF_AbsWriteMaskByte(0x112860, 0xFF, 0x00);  // Set MPLL_ADC_DIV_SEL
        HAL_SIF_AbsWriteMaskByte(0x112861, 0xFF, 0x12);

        HAL_SIF_AbsWriteMaskByte(0x112865, 0xFF, 0x09);

        HAL_SIF_AbsWriteMaskByte(0x1128AB, 0xFF, 0x01);

        HAL_SIF_AbsWriteMaskByte(0x1128AE, 0xFF, 0x00);  // start demod atop ADC setting
        HAL_SIF_AbsWriteMaskByte(0x1128AF, 0xFF, 0x00);

        HAL_SIF_AbsWriteMaskByte(0x1128B1, 0x03, 0x00);

        HAL_SIF_AbsWriteMaskByte(0x1128B2, 0xFF, 0x00);
        HAL_SIF_AbsWriteMaskByte(0x1128B3, 0xFF, 0x00);

        HAL_SIF_AbsWriteMaskByte(0x112802, 0xFF, 0x20);  // Set IMUXS QMUXS
        HAL_SIF_AbsWriteMaskByte(0x112803, 0xFF, 0x02);

        HAL_SIF_AbsWriteMaskByte(0x112818, 0xFF, 0x02);  // Set enable ADC clock
        HAL_SIF_AbsWriteMaskByte(0x112819, 0xFF, 0x00);

        HAL_SIF_AbsWriteMaskByte(0x112840, 0xFF, 0x00);  // Disable PWDN_REF
        HAL_SIF_AbsWriteMaskByte(0x112841, 0xFF, 0x00);

        HAL_SIF_AbsWriteMaskByte(0x112816, 0xFF, 0x05);  // Set ADC gain is 1
        ...
        HAL_SIF_AbsWriteMaskByte(0x112830, 0xFF, 0x00);  // Disable agc enable
...
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
void HAL_SIF_SetDspCodeType(MS_U8 u8Type)
{
    if (g_AudioVars2 == NULL)
    {
        DBG_SIF(printf("%s() : invalid g_AudioVars2!\n", __FUNCTION__));
    ...
    g_AudioVars2->g_u8SifCodeType = u8Type;
...
/// @param <GLOBAL>    \b NONE    :
////////////////////////////////////////////////////////////////////////////////
MS_U8 HAL_SIF_GetDspCodeType(void)
{
    if (g_AudioVars2 == NULL)
    {
        DBG_SIF(printf("%s() : invalid g_AudioVars2!\n", __FUNCTION__));
    ...
    return g_AudioVars2->g_u8SifCodeType;
...
MS_BOOL HAL_SIF_StartAutoDetect(void)
{
    ...
    {
        HAL_SIF_WriteMaskByte(AU_CMD_PFIRBANDWIDTH, 0x30, 0x10);
        if(bDetect_Main_Std_Only_Flag == 0)
            HAL_SIF_WriteByte(AU_CMD_STANDARD, 0xF0);
        else
            HAL_SIF_WriteByte(AU_CMD_STANDARD, 0xD0);
    }
    return TRUE;
}
...
MS_U8 HAL_SIF_SendCmd(AU_SIF_CMD u8DecCmd, MS_U8 comm_arg1, MS_U8 comm_arg2)
{
    ...
    switch( u8DecCmd )
    {
        ...
        case AU_SIF_CMD_SET_ADC_FROM_VIF_PATH:
            ...
            {
                //printf("[AUDIO UTOPIA SIF] comm_arg1(%d), bADCFromVifPathEnabled(%d) \r\n", comm_arg1, bADCFromVifPathEnabled);
                if ((MS_BOOL)comm_arg1 == TRUE)
                {
        ...
        case AU_SIF_CMD_ENABLE_CHANNEL:
            if ((MS_BOOL)comm_arg1 == TRUE)
            {
                if (HAL_SIF_SendCmd(AU_SIF_CMD_GET_ADC_FROM_VIF_PATH, 0, 0) == TRUE) // VIF mode
                {
                    ...
                    //printf("[AUDIO UTOPIA SIF] VIF mode CLK\r\n");
                ...
                else // SIF mode
                {
                    ...
                    //printf("[AUDIO UTOPIA SIF] SIF mode CLK\r\n");
                }
            }
            break;
        ...
        case AU_SIF_CMD_GET_ADC_FROM_VIF_PATH:
            ...
            DBG_SIF(("[AUDIO UTOPIA SIF] u8Return(0x%x)\r\n", u8Return));
            break;
...
MS_BOOL HAL_SIF_ReLoadCode(MS_U8 u8Type)
{
...
MS_BOOL HAL_SIF_SetSystem(MS_U8 u8SystemType)
{
