...
#define HVD_HW_MAX_PIXEL            (1920*1088*61000ULL) // FHD@60p
#define HEVC_HW_MAX_PIXEL           (1920*1088*61000ULL) // FHD@60p
...
//------------------------------ MIU SETTINGS ----------------------------------
#define _MaskMiuReq_MVD_RW( m )         _HVD_WriteRegBit(MIU0_REG_RQ1_MASK, m, BIT(2))
#define _MaskMiuReq_MVD_BBU_R( m )      _HVD_WriteRegBit(MIU0_REG_RQ0_MASK+1, m, BIT(4))
#define _MaskMiuReq_HVD_RW( m )         _HVD_WriteRegBit(MIU0_REG_RQ4_MASK, m, BIT(1))
#define _MaskMiuReq_HVD_BBU_R( m )      _HVD_WriteRegBit(MIU0_REG_RQ4_MASK, m, BIT(0))

#define _MaskMiu1Req_MVD_RW( m )        _HVD_WriteRegBit(MIU1_REG_RQ1_MASK, m, BIT(2))
#define _MaskMiu1Req_MVD_BBU_R( m )     _HVD_WriteRegBit(MIU1_REG_RQ0_MASK+1, m, BIT(4))
#define _MaskMiu1Req_HVD_RW( m )        _HVD_WriteRegBit(MIU1_REG_RQ4_MASK, m, BIT(1))
#define _MaskMiu1Req_HVD_BBU_R( m )     _HVD_WriteRegBit(MIU1_REG_RQ4_MASK, m, BIT(0))

#define HVD_MVD_RW_ON_MIU1              ((_HVD_Read2Byte(MIU0_REG_SEL1) & BIT(2)) == BIT(2))
#define HVD_MVD_BBU_R_ON_MIU1           ((_HVD_Read2Byte(MIU0_REG_SEL0) & BIT(12)) == BIT(12))
#define HVD_HVD_RW_ON_MIU1              ((_HVD_Read2Byte(MIU0_REG_SEL4) & BIT(1)) == BIT(1))
#define HVD_HVD_BBU_R_ON_MIU1           ((_HVD_Read2Byte(MIU0_REG_SEL4) & BIT(0)) == BIT(0))

#if SUPPORT_EVD
#define _MaskMiuReq_EVD_RW1( m )         _HVD_WriteRegBit(MIU0_REG_RQ4_MASK,   m, BIT(2))
#define _MaskMiuReq_EVD_RW2( m )         _HVD_WriteRegBit(MIU0_REG_RQ4_MASK,   m, BIT(4))
#define _MaskMiuReq_EVD_BBU_R( m )       _HVD_WriteRegBit(MIU0_REG_RQ4_MASK,   m, BIT(3))

#define _MaskMiu1Req_EVD_RW1( m )        _HVD_WriteRegBit(MIU1_REG_RQ4_MASK,   m, BIT(2))
#define _MaskMiu1Req_EVD_RW2( m )        _HVD_WriteRegBit(MIU1_REG_RQ4_MASK,   m, BIT(4))
#define _MaskMiu1Req_EVD_BBU_R( m )      _HVD_WriteRegBit(MIU1_REG_RQ4_MASK,   m, BIT(3))

#define HVD_EVD_RW1_ON_MIU0              (((_HVD_Read2Byte(MIU0_REG_SEL4) & BIT(2)) == 0))
#define HVD_EVD_RW2_ON_MIU0              (((_HVD_Read2Byte(MIU0_REG_SEL4) & BIT(4)) == 0))
#define HVD_EVD_BBU_R_ON_MIU0            (((_HVD_Read2Byte(MIU0_REG_SEL4) & BIT(3)) == 0))

#define HVD_EVD_RW1_ON_MIU1              (((_HVD_Read2Byte(MIU0_REG_SEL4) & BIT(2)) == BIT(2)))
#define HVD_EVD_RW2_ON_MIU1              (((_HVD_Read2Byte(MIU0_REG_SEL4) & BIT(4)) == BIT(4)))
#define HVD_EVD_BBU_R_ON_MIU1            (((_HVD_Read2Byte(MIU0_REG_SEL4) & BIT(3)) == BIT(3)))
#endif
...
static HVD_Frm_Information _stSimpleFrm[HAL_HVD_EX_MAX_SUPPORT_STREAM];
...
MS_U32 HAL_HVD_EX_GetBBUId(MS_U32 u32Id)
{
    ...
    ret = HAL_VPU_EX_GetBBUId(u32Id,&taskInfo, pCtrl->bNStreamMode);
...
static void _HVD_EX_Context_Init_HAL(void)
{
    ...
    pHVDHalContext->u32VPUClockType = 320; ///Note : Max VD_R2 clock in Monaco is only 320MHz
                                             ///Note : Max EVD_R2 clock in Monaco is only 384MHz

    const SYS_Info* sysInfo;
    sysInfo = MDrv_SYS_GetInfo();

    pHVDHalContext->u32HVDClockType = 240;//160;

#if SUPPORT_EVD
    pHVDHalContext->u32EVDClockType = 240;
#endif
#if SUPPORT_G2VP9 && defined(VDEC3)
...
static MS_U16 _HVD_EX_GetBBUReadptr(MS_U32 u32Id)
{
    ...
    {
        MS_U16 u16Ret0 = _HVD_Read2Byte(HVD_REG_NAL_RPTR_HI_BS4);
...
static MS_U16 _HVD_EX_GetBBUWritedptr(MS_U32 u32Id)
{
    ...
    {
        MS_U16 u16Ret0 = _HVD_Read2Byte(HVD_REG_NAL_WPTR_HI_BS4);
...
static MS_BOOL _HVD_EX_SetCMD(MS_U32 u32Id, MS_U32 u32Cmd)
{
    ...
    HVD_EX_MSG_DBG("Send CMD 0x%lx to HVD u8TaskId = %X\n", u32Cmd,u8TaskId);
...
static HVD_Return _HVD_EX_SendCmd(MS_U32 u32Id, MS_U32 u32Command, MS_U32 u32CmdArg)
{
    ...
    HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    if(pShm == NULL && (HAL_VPU_EX_IsDisplayQueueCMD(u32Command) || FALSE == HAL_VPU_EX_IsMailBoxCMD(u32Command)))
    {
        return E_HVD_RETURN_FAIL;
    }
    if (HAL_VPU_EX_IsDisplayQueueCMD(u32Command))
    {
        do {
            ...
            if (SentRet == E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL)
                HVD_EX_MSG_DBG("^^^Display command ARG return=0x%X cmd=0x%lx arg=0x%lx\n", SentRet,u32Command, u32CmdArg);
            ...
            {
                 HVD_EX_MSG_ERR("^^^Display command ARG timeout: cmd=0x%lx arg=0x%lx\n", u32Command, u32CmdArg);
                 break;
            }
        }while (SentRet != E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL);
    }
    else if (!HAL_VPU_EX_IsMailBoxCMD(u32Command))
    {
        do {
            SentRet = HAL_VPU_EX_DRAMStreamCMDQueueSend(u32Id, &pShm->cmd_queue, E_HVD_CMDQ_ARG, u32CmdArg);
            if (!SentRet)
                HVD_EX_MSG_DBG("^^^Dram command ARG return=0x%X cmd=0x%lx arg=0x%lx\n", SentRet,u32Command, u32CmdArg);
            if (SentRet == E_HVD_COMMAND_QUEUE_NOT_INITIALED)
                break;
            else if (SentRet == E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL) {
                IsSent = TRUE;
            ...
            {
                 HVD_EX_MSG_ERR("^^^Dram command ARG timeout: cmd=0x%lx arg=0x%lx\n", u32Command, u32CmdArg);
                 break;
            }
        }while (SentRet != E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL);
    }
...
#endif
    {
        ...
        if((HVD_User_Cmd)u32Command < E_DUAL_CMD_BASE)
        {
            //_HVD_EX_GetPC();
    ...
    IsSent = FALSE;
    u32timeout = HVD_GetSysTime_ms() + pHVDHalContext->u32HVDCmdTimeout;
    if (HAL_VPU_EX_IsDisplayQueueCMD(u32Command) && !IsMailBox)
    {
        do {
            SentRet = HAL_VPU_EX_DRAMStreamDispCMDQueueSend(u32Id, &pShm->cmd_queue, E_HVD_CMDQ_CMD,u32Command);
            if (!SentRet)
                HVD_EX_MSG_DBG("^^^Display command CMD return=0x%X cmd=0x%lx arg=0x%lx\n", SentRet,u32Command, u32CmdArg);
            if (SentRet == E_HVD_COMMAND_QUEUE_NOT_INITIALED)
                break;
            else if (SentRet == E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL){
                IsSent = TRUE;
             ...
             {
                 HVD_EX_MSG_ERR("^^^Display command CMD timeout: cmd=0x%lx arg=0x%lx\n", u32Command, u32CmdArg);
                 break;
             }
        } while (SentRet != E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL);
    }
    else if(!HAL_VPU_EX_IsMailBoxCMD(u32Command) && !IsMailBox)
    {
        do {
            SentRet = HAL_VPU_EX_DRAMStreamCMDQueueSend(u32Id, &pShm->cmd_queue, E_HVD_CMDQ_CMD,u32Command);
            if (SentRet != E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL) {
                HVD_EX_MSG_ERR("^^^Dram command CMD return=0x%X cmd=0x%lx arg=0x%lx\n", SentRet,u32Command, u32CmdArg);
            }
            if (SentRet == E_HVD_COMMAND_QUEUE_NOT_INITIALED)
                break;
            else if (SentRet == E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL){
             ...
             {
                 HVD_EX_MSG_ERR("^^^Dram command CMD timeout: cmd=0x%lx arg=0x%lx\n", u32Command, u32CmdArg);
                 break;
             }
        } while (SentRet != E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL);
    }
    ...
    {
        u32timeout = HVD_GetSysTime_ms() + pHVDHalContext->u32HVDCmdTimeout;
        while (!_HVD_EX_SetCMD(u32Id, u32Command))
#else
    u32timeout = HVD_GetSysTime_ms() + pHVDHalContext->u32HVDCmdTimeout;
    ...
    if(pCtrl)
    {
        ...
        MS_U8* u8Data = 0;
        MS_U32* u32Data = 0;
        if(pCtrl->MemMap.u32CodeBufAddr)
        {
            u8Data = (MS_U8*)MsOS_PA2KSEG1(u32StrAddrOffset+pCtrl->MemMap.u32CodeBufAddr);
            u32Data = (MS_U32*)MsOS_PA2KSEG1(pHVDHalContext->u32strcmdbuf[u8Idx]+pCtrl->MemMap.u32CodeBufAddr);
        }
        if(HAL_VPU_EX_CheckSTRState(u8Idx) && u8Data)
        {
            if(pHVDHalContext->u8strcmdcnt[u8Idx]<VDEC_STR_CMD_RESERVERD)
            {
                if(u32Data)
                {
                    u32Data[0] = VDEC_STR_HVD;
                    u32Data[1] = u32Command;
                    u32Data[2] = u32CmdArg;
                }
                pHVDHalContext->u8strcmdcnt[u8Idx]++;
                pHVDHalContext->u32strcmdbuf[u8Idx]+=VDEC_STR_ALIGN;
                 if(u8Idx == 0)
                    u8Data[VDEC_STR_MAIN_CMD_COUNT_OFFSET] = pHVDHalContext->u8strcmdcnt[u8Idx];
                else
                    u8Data[VDEC_STR_SUB_CMD_COUNT_OFFSET] = pHVDHalContext->u8strcmdcnt[u8Idx];
                //printf("\n[STR] [%d]HVD_CMD: %lx; ARG %lx",pHVDHalContext->u8strcmdcnt, u32Cmd, u32CmdArg);
                HAL_HVD_EX_FlushMemory();
            }
        }
    }
...
#if SUPPORT_EVD
    _HVD_MIU_SetReqMask(EVD_RW1, bEnable);
    _HVD_MIU_SetReqMask(EVD_RW2, bEnable);
...
static MS_BOOL _HAL_EX_IS_EVD(MS_U32 u32ModeFlag)
{
    ...
    if (u32CodecType == E_HVD_INIT_HW_HEVC
#if SUPPORT_MSVP9
...
static MS_BOOL _HAL_EX_IS_HVD(MS_U32 u32ModeFlag) // VP8 isn't included
{
    MS_U32 u32CodecType = u32ModeFlag & E_HVD_INIT_HW_MASK;

    if ((u32CodecType == E_HVD_INIT_HW_MVC) ||
        (u32CodecType == E_HVD_INIT_HW_AVC) ||
        (u32CodecType == E_HVD_INIT_HW_AVS) ||
        (u32CodecType == E_HVD_INIT_HW_RM))
        return TRUE;

    return FALSE;
}

static MS_BOOL _HAL_EX_BBU_EVD_InUsed(void)
{
    if (!pHVDHalContext)
        return FALSE;
    ...
    for (i = 0; i < HAL_HVD_EX_MAX_SUPPORT_STREAM; i++)
    {
        if (pHVDHalContext->_stHVDStream[i].bUsed &&
            ((pHVDHalContext->_stHVDStream[i].u32CodecType == E_HAL_HVD_HEVC)
#if SUPPORT_MSVP9
           ||(pHVDHalContext->_stHVDStream[i].u32CodecType == E_HAL_HVD_VP9)
#endif
            ))
        {
            bRet = TRUE;
...
static MS_BOOL _HAL_EX_BBU_HVD_InUsed(void) // VP8 isn't included
{
    if (!pHVDHalContext)
        return FALSE;
    ...
    {
        if (pHVDHalContext->_stHVDStream[i].bUsed &&
            ((pHVDHalContext->_stHVDStream[i].u32CodecType == E_HAL_HVD_AVC) ||
            (pHVDHalContext->_stHVDStream[i].u32CodecType == E_HAL_HVD_AVS) ||
            (pHVDHalContext->_stHVDStream[i].u32CodecType == E_HAL_HVD_RM)))
        {
            bRet = TRUE;
            break;
        }
...
//    MS_U32 u32TmpStartOffset;
...
#endif
        {
            //HVD_EX_MSG_ERR("u32FBStartOffset = 0x%lx  ESB start addr=%lx\n", u32FBStartOffset, u32StAddr);
    ...
    HVD_EX_MSG_COVERITY("[%d]NAL start addr=%lx\n",u8TaskId, u32StAddr);

#ifdef VDEC3
    if (!pCtrl->bNStreamMode || ((_HAL_EX_IS_EVD(pCtrl->InitParams.u32ModeFlag) && !_HAL_EX_BBU_EVD_InUsed()) ||
        (_HAL_EX_IS_HVD(pCtrl->InitParams.u32ModeFlag) && !_HAL_EX_BBU_HVD_InUsed()) ||
         (E_HVD_INIT_INPUT_TSP == (pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_INPUT_MASK))))
    {
        if (pCtrl->u32BBUId == 0)
        {
    ...
    // ES buffer
#ifdef VDEC3
    if(!pCtrl->bNStreamMode || E_HVD_INIT_INPUT_TSP == (pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_INPUT_MASK))
    {
    ...
    HVD_EX_MSG_COVERITY("ESB start addr=%lx, len=%lx\n", u32StAddr, pCtrl->MemMap.u32BitstreamBufSize);
         ...
         (E_HVD_INIT_INPUT_TSP == (pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_INPUT_MASK))))
    {
        ...
        if (_HVD_EX_IS_BBU_TSP_MODE(u32Id))
        {
            HVD_PRINT("\033[1;32m[%s] %d u16Reg &= ~HVD_REG_BBU_TSP_INPUT\033[m\n",__FUNCTION__,__LINE__);
            u16Reg &= ~HVD_REG_BBU_TSP_INPUT;
        }
        ...
        if (E_HVD_INIT_HW_RM == (pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK))        // RM
        {
            if (pCtrl->u32BBUId == 0)    // force BBU to remove nothing, RM only
                u16Reg |= HVD_REG_BBU_PASER_DISABLE;
            else
                u16Reg |= HVD_REG_BBU_PASER_DISABLE_BS2;
        }
#if SUPPORT_MSVP9
        else if (E_HVD_INIT_HW_VP9 == (pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK))
        {
            if (pCtrl->u32BBUId == 0)
    ...
    if (0 == u8TaskId)
    {
        ...
        if (_HVD_EX_IS_BBU_TSP_MODE(u32Id))
        {
            HVD_PRINT("\033[1;31m][%s][%s] %d u16Reg &= ~HVD_REG_BBU_TSP_INPUT\033[m\n",__FILE__,__FUNCTION__,__LINE__);
            u16Reg &= ~HVD_REG_BBU_TSP_INPUT;
        }
...
#ifdef VDEC3
#if SUPPORT_MSVP9
    if ((pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_VP9)
    {
...
static HVD_Frm_Information *_HVD_EX_GetNextDispFrame(MS_U32 u32Id)
{
    ...
    if (pHVDHalContext->_stHVDStream[u8Idx].bDispOutSide &&
        (pShm->u32CodecType == E_HVD_Codec_HEVC && pShm->u8FieldPicFlag))
    {
        MS_U32 first_field = pHVDHalContext->_stHVDStream[u8Idx].u32DispQIndex == 1 ? 0 : 1;
        volatile HVD_Frm_Information *pHvdFrm_first = NULL;
            ...
            if (pHvdFrm->u32Status == E_HVD_DISPQ_STATUS_INIT) // Must Be
            {
                if (!first_field) // second get frame, we will check at least one paired in disp queue.
                {
                    ...
                    if(pHvdFrm->u8FieldType == EVD_TOP_FIELD || pHvdFrm->u8FieldType == EVD_TOP_WITH_PREV || pHvdFrm->u8FieldType == EVD_TOP_WITH_NEXT)
                        pHvdFrm->u8FieldType = 0;
                    else
                        pHvdFrm->u8FieldType = 1;
                ...
                else // first get frame, we will check at least one paired in disp queue.
                {
                    ...
                    else
                    {
                        ...
                        //After flush, we cannot get the correct field type of first field from sei, so we use second field type to decide first field type.
                        if (pHVDHalContext->_stHVDStream[u8Idx].u32DispQIndex == 0xff)
                        {
                            if (pHvdFrm->u8FieldType == EVD_TOP_WITH_PREV)
                                pHvdFrm_first->u8FieldType = EVD_BOTTOM_WITH_NEXT;
                            else if (pHvdFrm->u8FieldType == EVD_BOTTOM_WITH_PREV)
                                pHvdFrm_first->u8FieldType = EVD_TOP_WITH_NEXT;
                            else if (pHvdFrm->u8FieldType == EVD_BOTTOM_WITH_PREV)
                                pHvdFrm_first->u8FieldType = EVD_TOP_WITH_NEXT;
                            else if (pHvdFrm->u8FieldType == EVD_TOP_FIELD)
                                pHvdFrm_first->u8FieldType = EVD_BOTTOM_FIELD;
                            else if (pHvdFrm->u8FieldType == EVD_BOTTOM_FIELD)
                                pHvdFrm_first->u8FieldType = EVD_TOP_FIELD;
                            else
                            {
                                HAL_HVD_EX_SetCmd(u32Id, E_HVD_CMD_RELEASE_DISPQ, pHvdFrm_first->u32PrivateData);
                                return NULL;
                            }
                            if ((pHvdFrm_first->u32ID_L >> 16) & 0x1)
                                pHvdFrm_first->u32ID_L |= (1 << 16);
                            else
                                pHvdFrm_first->u32ID_L &= (~(1 << 16));
                        }
                        else if (pHvdFrm_first->u8FieldType == EVD_TOP_WITH_PREV || pHvdFrm_first->u8FieldType == EVD_BOTTOM_WITH_PREV)
                        {
                        ...
                        if (pHvdFrm_first->u8FieldType == EVD_TOP_FIELD || pHvdFrm_first->u8FieldType == EVD_TOP_WITH_PREV || pHvdFrm_first->u8FieldType == EVD_TOP_WITH_NEXT)
                            pHvdFrm_first->u8FieldType = 0;
                        else
                            pHvdFrm_first->u8FieldType = 1;
                        return (HVD_Frm_Information*)(MS_U32)pHvdFrm_first;
                    }
                }
            }
            u16QNum--;
...
static MS_U64 _HAL_EX_GetHwMaxPixel(MS_U32 u32Id)
{
    ...
    MS_U64 u64Ret = 0;
#if SUPPORT_EVD
    MS_BOOL isEVD = (E_HVD_INIT_HW_HEVC == (pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK));
#if SUPPORT_MSVP9
    isEVD = isEVD || (E_HVD_INIT_HW_VP9 == (pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK));
#endif
#endif
...
MS_U32 HAL_HVD_EX_GetShmAddr(MS_U32 u32Id)
{
    ...
    u32ShmAddr = (MS_U32)MsOS_PA2KSEG1(u32PhyAddr);
#ifdef MSOS_TYPE_LINUX_KERNEL
...
MS_U32 HAL_HVD_EX_GetDispQExtShmAddr(MS_U32 u32Id)
{
    HVD_EX_Drv_Ctrl *pCtrl = _HVD_EX_GetDrvCtrl(u32Id);
    HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    ...
    u32PhyAddr += pShm->u32DISPQUEUE_EXT_ST_ADDR;
    #else
    u32PhyAddr = pCtrl->MemMap.u32CodeBufAddr + pShm->u32DISPQUEUE_EXT_ST_ADDR;
    #endif
...
{
    // The cache should already been overwritten by other address so we ignore the flush call to improve performance
    //MsOS_MPool_Dcache_Flush((MS_U32)pVA, u32Size);
}
...
static void _HAL_HVD_EX_Flush_Cache(void *pVA, MS_U32 u32Size)
{
    ...
    MsOS_MPool_Dcache_Flush(((MS_U32)pVA) + u32SkipSize, u32Size);
}
...
static MS_BOOL _HAL_HVD_EX_PostProc_Task(MS_U32 u32Id)
{
    ...
    while (pstHVDStream->ePpTaskState != E_HAL_HVD_STATE_STOP)
    {
        ...
        while (pShm->u8PpQueueRPtr != pShm->u8PpQueueWPtr)
        {
            ...
            // Luma
            u32SrcMiuSel = pFrmInfo->u2Luma1Miu;
            u32DstMiuSel = pFrmInfo->u2Luma0Miu;
            ...
            // Chroma
            u32SrcMiuSel = pFrmInfo->u2Chroma1Miu;
            u32DstMiuSel = pFrmInfo->u2Chroma0Miu;
...
RESET:
    ...
    // This is for Clippers, Miami and Munich using 128 bits bbu
    if (isEVD)
    {
        printf("EVD BBU 256 bits\n");
        _HVD_Write2Byte(EVD_REG_BBU_MIU_WIDTH, EVD_REG_BBU_MIU_256);
    }
#endif
...
void HAL_HVD_EX_PowerCtrl(MS_BOOL bEnable)
{
    ...
    switch (pHVDHalContext->u32HVDClockType)
    {
        ...
        case 288: // for maldives only
        {
            _HVD_WriteByteMask(REG_TOP_HVD, TOP_CKG_HVD_288MHZ, TOP_CKG_HVD_CLK_MASK);
            _HVD_WriteWordMask(REG_TOP_HVD_IDB, TOP_CKG_HVD_IDB_172MHZ, TOP_CKG_HVD_IDB_CLK_MASK);
            break;
        ...
        default:
        {
                                                                                                            // TODO://if(hvd_clk > miu_clk) set to TOP_CKG_HVD_IDB_CLK_HVD_MIU = 0
            _HVD_WriteByteMask(REG_TOP_HVD, TOP_CKG_HVD_240MHZ, TOP_CKG_HVD_CLK_MASK);
            _HVD_WriteWordMask(REG_TOP_HVD_IDB, TOP_CKG_HVD_IDB_288MHZ, TOP_CKG_HVD_IDB_CLK_MASK);
            break;
        }
...
#endif
{
    ...
    MS_U32 u32TmpStartOffset = 0;
    MS_U8  u8TmpMiuSel = 0;

    memset(pShm, 0, sizeof(HVD_ShareMem));
#ifdef SUPPORT_VDEC_STR
    MS_U32 u32StrAddrOffset = HAL_VPU_EX_GetStrOffset(pCtrl->MemMap.u32CodeBufSize);
    ...
    //MS_U8 u8Idx = _HVD_EX_GetStreamIdx(u32Id);
    if(u8Idx==0)
        pHVDHalContext->u32strcmdbuf[u8Idx] = VDEC_STR_MAIN_CMD_BUF;
    else if(u8Idx==1)
        pHVDHalContext->u32strcmdbuf[u8Idx] = VDEC_STR_SUB_CMD_BUF;

    pHVDHalContext->u8strcmdcnt[u8Idx] = 0;
    ...
    if (_HVD_EX_IS_BBU_TSP_MODE(u32Id))
    {
        pShm->bUseTSPInBBUMode = TRUE;
        HVD_PRINT("\033[1;36m[%s] %d set PRESET_TSP_IN_BBU_MODE   pShm->u32PreSetControl = %x\033[m\n",__FUNCTION__,__LINE__,(unsigned int)(pShm->u32PreSetControl));
    }
    else
        ...
        ((pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK) == E_HVD_INIT_HW_AVC))
    {
        ...
        _phy_to_miu_offset(u8TmpMiuSel, u32Addr, pHVDHalContext->pHVDPreCtrl_Hal[_HVD_EX_GetStreamIdx(u32Id)]->stIapGnShBWMode.u32IapGnBufAddr);
    ...
    if ((pHVDHalContext->pHVDPreCtrl_Hal[_HVD_EX_GetStreamIdx(u32Id)]->stPreConnectDispPath.bEnable))
    {
        pShm->u32PreSetControl |= PRESET_CONNECT_DISPLAY_PATH;

        pShm->stDynmcDispPath.u8Connect = pHVDHalContext->pHVDPreCtrl_Hal[_HVD_EX_GetStreamIdx(u32Id)]->stPreConnectDispPath.stDynmcDispPath.bConnect;
        pShm->stDynmcDispPath.u8DispPath = (MS_U8)(pHVDHalContext->pHVDPreCtrl_Hal[_HVD_EX_GetStreamIdx(u32Id)]->stPreConnectDispPath.stDynmcDispPath.eMvopPath);
        pShm->stDynmcDispPath.u8ConnectStatus = E_DISP_PATH_DYNMC_HANDLING;
    }

    if(u8TmpMiuSel == 0 || u32TmpStartOffset == 0)
    {
    }

    if (pHVDHalContext->pHVDPreCtrl_Hal[_HVD_EX_GetStreamIdx(u32Id)]->bEnableForceStartNonISlice)
    {
        pShm->u32PreSetControl |= PRESET_FORCE_START_NON_I_SLICE;
    }
...
#if SUPPORT_G2VP9
    // Enable SW detile support for G2 VP9 except thumbnail mode
    if (E_HVD_INIT_HW_VP9 == (pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK))
    {
        if (!pHVDHalContext->pHVDPreCtrl_Hal[u8Idx]->bEnableThumbnailMode)
        {
...
MS_U32 HAL_HVD_EX_GetData(MS_U32 u32Id, HVD_GetData eType)
{
    ...
    // share memory
        ...
        {
            u32Ret = (MS_U32) _HVD_EX_GetNextDispFrameExt(u32Id);
            break;
        }
#endif
        ...
        case E_HVD_GDATA_FRM_PACKING_SEI_DATA:
            ...
            break;
        case E_HVD_GDATA_DisplayColourVolume_SEI_DATA:
        ...
        case E_HVD_GDATA_FW_STATUS_FLAG:
            ...
            break;
        case E_HVD_GDATA_IS_DIVX_PLUS_STREAM:
            u32Ret=(MS_U32)(pShm->u8DivxProfile);
            break;
        // internal control
    ...
    // FW def
        ...
        case E_HVD_GDATA_FW_VSYNC_BRIDGE_ADDR:
            u32Ret = 0;//pShm->u32VSYNC_BRIGE_SHM_ADDR;
            break;
        ...
        case E_HVD_GDATA_HVD_HW_MAX_PIXEL:
            ...
            break;
        case E_HVD_GDATA_HVD_IS_MORE_FRAME:
            if(pShm->u32DecodeCnt)
                u32Ret = (pShm->u32FrameWrtPtr != pShm->u32FrameReadPtr);
            else
                u32Ret = 0;
            break;
        case E_HVD_GDATA_HVD_DECODE_SIMPLE_INFO:
            memset((void*)(&_stSimpleFrm[u8Idx]),0,sizeof(HVD_Frm_Information));
            if(pShm->u32FrameReadPtr >= FRAMEQ_SIZE )
            {
                pShm->u32FrameReadPtr = 0;
            }
            _stSimpleFrm[u8Idx].u8FrmType = pShm->u8FrameType[pShm->u32FrameReadPtr];
            _stSimpleFrm[u8Idx].u32TimeStamp = pShm->u32DecTimeStamp[pShm->u32FrameReadPtr];
            pShm->u32FrameReadPtr = (pShm->u32FrameReadPtr + 1) % FRAMEQ_SIZE;
            HAL_HVD_EX_FlushMemory();
            u32Ret = (MS_U32) (&_stSimpleFrm[u8Idx]);
            break;
#ifdef VDEC3
        case E_HVD_GDATA_FW_VBBU_ADDR:
            u32Ret = pShm->u32HVD_VBBU_DRAM_ST_ADDR;
            break;
#endif
        default:
            break;
...
HVD_Return HAL_HVD_EX_SetCmd(MS_U32 u32Id, HVD_User_Cmd eUsrCmd, MS_U32 u32CmdArg)
{
    ...
    if (E_HVD_INIT_HW_HEVC == (pCtrl->InitParams.u32ModeFlag & E_HVD_INIT_HW_MASK) && u32Cmd == E_HVD_CMD_FLUSH)
    {
        // for MCU interlace mode
        pHVDHalContext->_stHVDStream[u8Idx].u32DispQIndex = 0xff;
    }
...
void HAL_HVD_EX_UartSwitch2FW(MS_BOOL bEnable)
{
    if (bEnable)
    {
        ...
        if (HAL_VPU_EX_IsEVDR2())
            _HVD_WriteWordMask(REG_TOP_UART_SEL, REG_TOP_UART_SEL_0_MHEG5, REG_TOP_UART_SEL_0_MASK);
        else
            _HVD_WriteWordMask(REG_TOP_UART_SEL, REG_TOP_UART_SEL_0_VD_MHEG5, REG_TOP_UART_SEL_0_MASK);
        #else
        if (HAL_VPU_EX_IsEVDR2())
            _HVD_WriteWordMask(REG_TOP_UART_SEL, REG_TOP_UART_SEL_1_MHEG5, REG_TOP_UART_SEL_1_MASK);
        else
            _HVD_WriteWordMask(REG_TOP_UART_SEL, REG_TOP_UART_SEL_1_VD_MHEG5, REG_TOP_UART_SEL_1_MASK);
        #endif
    }
    else
    {
        ...
        _HVD_WriteWordMask(REG_TOP_UART_SEL, REG_TOP_UART_SEL_0_PIU_0, REG_TOP_UART_SEL_0_MASK);
#endif
        #else
#if defined (__aeon__)
        _HVD_WriteWordMask(REG_TOP_UART_SEL, REG_TOP_UART_SEL_1_MHEG5, REG_TOP_UART_SEL_1_MASK);
#else // defined (__mips__)
        _HVD_WriteWordMask(REG_TOP_UART_SEL, REG_TOP_UART_SEL_1_PIU_0, REG_TOP_UART_SEL_1_MASK);
#endif
        #endif
    }
}
...
void HAL_HVD_EX_UpdateESWptr_Fire(MS_U32 u32Id)
{
    ...
    _phy_to_miu_offset(u8BitMiuSel, u32BitStartOffset, pCtrl->MemMap.u32BitstreamBufAddr);
    _phy_to_miu_offset(u8CodeMiuSel, u32CodeStartOffset, pCtrl->MemMap.u32CodeBufAddr);
    HVD_EX_MSG_COVERITY("u32BitStartOffset = 0x%lx u32CodeStartOffset = 0x%lx \n", u32BitStartOffset,u32CodeStartOffset);
...
MS_BOOL HAL_HVD_EX_GetFrmRateIsSupported(MS_U32 u32Id, MS_U16 u16HSize, MS_U16 u16VSize, MS_U32 u32FrmRate)
{
    ...
    HVD_EX_MSG_DBG("%s w:%d, h:%d, fr:%ld, MAX:%lld\n", __FUNCTION__,
                    u16HSize, u16VSize, u32FrmRate, _hw_max_pixel);
...
void HAL_EVD_EX_PowerCtrl(MS_BOOL bEnable)
{
    ...
    switch (pHVDHalContext->u32EVDClockType)
    {
        ...
        case 216:
        {
            ...
            _HVD_WriteWordMask(REG_TOP_CKG_EVD, TOP_CKG_EVD_216MHZ, TOP_CKG_EVD_MASK);
            break;
        }
        case 172:
        {
            _HVD_WriteWordMask(REG_TOP_CKG_EVD_PPU, TOP_CKG_EVD_PPU_216MHZ, TOP_CKG_EVD_PPU_MASK);
            _HVD_WriteWordMask(REG_TOP_CKG_EVD, TOP_CKG_EVD_172MHZ, TOP_CKG_EVD_MASK);
            break;
        }
        case 160:
        {
            _HVD_WriteWordMask(REG_TOP_CKG_EVD_PPU, TOP_CKG_EVD_PPU_172MHZ, TOP_CKG_EVD_PPU_MASK);
            _HVD_WriteWordMask(REG_TOP_CKG_EVD, TOP_CKG_EVD_160MHZ, TOP_CKG_EVD_MASK);
            break;
        }
        default:
        {
            _HVD_WriteByteMask(REG_TOP_CKG_EVD_PPU, TOP_CKG_EVD_PPU_288MHZ, TOP_CKG_EVD_PPU_MASK);
            _HVD_WriteByteMask(REG_TOP_CKG_EVD, TOP_CKG_EVD_240MHZ, TOP_CKG_EVD_MASK);
            break;
        }
    }
...
static void HAL_VP9_EX_PowerCtrl(MS_BOOL bEnable)
{
    ...
    switch (pHVDHalContext->u32VP9ClockType)
    {
        ...
        case 456:
        {
            _HVD_WriteWordMask(REG_TOP_VP9, TOP_CKG_VP9_EVDPLL, TOP_CKG_VP9_CLK_MASK);
            break;
        }
        case 480:
        {
...
{
    MS_U8 u8Idx = _HVD_EX_GetStreamIdx(u32Id);
    return pHVDHalContext->bBBU_running[u8Idx];
}

void _HVD_EX_BBU_Set_TaskRunning(MS_U32 u32Id,MS_BOOL val)
{
    MS_U8 u8Idx = _HVD_EX_GetStreamIdx(u32Id);
    pHVDHalContext->bBBU_running[u8Idx] = val;
}

MS_U32 _HVD_EX_GetESOffsetIncrease(MS_U32 u32Id, MS_U32 inc, MS_U32 offset)
{
    HVD_EX_Drv_Ctrl *pCtrl = _HVD_EX_GetDrvCtrl(u32Id);
    return (offset + inc < pCtrl->MemMap.u32BitstreamBufSize)?(offset + inc):(offset + inc - pCtrl->MemMap.u32BitstreamBufSize);
}

MS_U32 _HVD_EX_GetESOffsetMinus(MS_U32 u32Id, MS_U32 mis, MS_U32 offset)
{
    HVD_EX_Drv_Ctrl *pCtrl = _HVD_EX_GetDrvCtrl(u32Id);
    return (offset >= mis)?(offset - mis):(offset + pCtrl->MemMap.u32BitstreamBufSize - mis);
}
...
void _HVD_EX_ES_DBG_PRINT(MS_U32 u32Id, MS_U32 offset, MS_U32 length)
{
    ...
    int idx = 0;
    MS_U32 newOffset = 0;
    HVD_PRINT("\033[1;33m [%s][%s] %d offset = %x \n\033[1;36m",__FILE__,__FUNCTION__,__LINE__,(unsigned int)offset);
    for(idx = 0;idx<length;idx++)
    {
        newOffset = _HVD_EX_GetESOffsetIncrease(u32Id, idx, offset);
        char* tempPtr = (char*)(pCtrl->MemMap.u32BitstreamBufVAddr + newOffset);
        HVD_PRINT("%02x ",(unsigned int)(*tempPtr));
...
void HAL_HVD_EX_BBU_Task(MS_U32 u32Id)
{
