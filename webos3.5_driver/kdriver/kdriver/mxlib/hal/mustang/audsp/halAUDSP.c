...
MS_BOOL HAL_AUDSP_DspLoadCode(MS_U8 u8Type)
{
   ...
   REE_TO_TEE_MBX_MSG_TYPE msg_type = REE_TO_TEE_MBX_MSG_NULL;
    ...
    if((u8Type == 0x61) && (bIsSTR == 0))
    {
        bIsSTR = 1;             // turn on STR flag
        u8Type = AU_SE_SYSTEM;  // reset to original type

        //printk("\033[1;31m [AUDIO][%s] [%d] [turn on STR flag] \033[0m \n", __FUNCTION__, __LINE__);/*@audio*/
    }
    if((u8Type == 0x61) && (bIsSTR == 1))
    {
        bIsSTR = 0;             // turn off STR flag and return immediately

        //printk("\033[1;32m [AUDIO][%s] [%d] [turn off STR flag] \033[0m \n", __FUNCTION__, __LINE__);/*@audio*/
    ...
    switch(u8Type & 0xF0)
    {
        ...
        case 0x30://AU_DVB2_STANDARD_MAD_TYPE
           if(u8Type >= AU_DVB2_STANDARD_MAX )
            {
                HALAUDSP_ERROR("    [audio_se_dec_info]:decoder type out of range.\r\n");
                return FALSE;
            }
        ...
        case 0x50:
            ...
            {
                HALAUDSP_ERROR("    [audio_adv_sndeff_info]:decoder type out of range.\r\n");
                return FALSE;
            }
        ...
        case 0xb0://AU_DVB_STANDARD_SIF_TYPE
            ...
            {
                HALAUDSP_ERROR("    [audio_sif_info]:decoder type out of range.\r\n");
                return FALSE;
            }
...
MS_BOOL HAL_AUDSP_DspLoadSystemCode2(MS_U8 DSP_select, AUDIO_ALG_INFO * pau_info)
{
    ...
    HALAUDSP_ERROR("HAL_AUDSP_DspLoadSystemCode2 finished(type=%s)\r\n", pau_info->AlgName);
...
MS_BOOL HAL_AUDSP_DspVerifySegmentCode(MS_U32 dsp_addr, MS_U8 *dspCode_buf, MS_U32 dspCode_buflen, MS_U8 DSP_select)
{
    ...
    for (i=0; i<dspCode_buflen; i+=3)
    {
     ...
     if(DSP_select == DSP_DEC)
        ...
        {
            HALAUDSP_ERROR("check data %lx\n", i);
            HALAUDSP_ERROR("dat0 %X <===> ",dspCode_buf[i]);
            HALAUDSP_ERROR("%x \n", dat[0]);
            HALAUDSP_ERROR("dat1 %X <===> ",dspCode_buf[i+1]);
            HALAUDSP_ERROR("%x \n", dat[1]);
            HALAUDSP_ERROR("dat2 %x <===> ",dspCode_buf[i+2]);
            HALAUDSP_ERROR("%X \n", dat[2]);
            HALAUDSP_ERROR("  Dsp code verify error!!\r\n");
