...
#define HALVIFDBG2_BIT      (DBB1_REG_BASE+0xF6)  // Bit 0
//-------------------------------------------------------------------------------------------------
//  extern function
//-------------------------------------------------------------------------------------------------
...
#define PM_RIU_READ_BYTE(addr)         ( READ_BYTE(u32PMBank + (addr) ) )
#define PM_RIU_READ_2BYTE(addr)        ( READ_WORD( u32PMBank+ (addr) ) )
#define PM_RIU_WRITE_BYTE(addr, val)   { WRITE_BYTE( u32PMBank + (addr), val) }
#define PM_RIU_WRITE_2BYTE(addr, val)  { WRITE_WORD( u32PMBank + (addr), val) }
...
int SeriousACI_Index = 0;
BYTE ADC_Index= 0;
BYTE ADC_Underflow_Index = 0;
BYTE ADC_Overflow_Index= 0;
BYTE PGA = 0;
WORD VGA = 0;
////////////////////////////////////////////////////
...
void msVifAdcInitial(void)
{
    ...
    // only support silicon tuner, internal single SAW and SAW-less architectures
...
void msVifInitial(void)
{
    ...
    msWriteBit(BYPASS_SOS11, 0, _BIT0);                         // SOS11 not bypass
    msWriteBit(BYPASS_SOS12, 0, _BIT1);                         // SOS12 not bypass
   ...
   // version control
    msWriteByte(FIRMWARE_VERSION_L, 0x1D);                          // 29(dd)
    msWriteByte(FIRMWARE_VERSION_H, 0xCE);                          // 12/14 (mm/yy)  firmware version control
