...
#if defined(MCU_AEON)
#define _u32MIO_MapBase     0xFA200000
#elif defined(__arm__)
#ifdef MBOOT
#define _u32MIO_MapBase     0x1f200000
    #else
...
MS_BOOL CHIP_EnableIRQ(InterruptNum eIntNum)
{
    ...
    if ((irq_to_desc(irq)->irq_data).common->state_use_accessors & IRQD_IRQ_DISABLED)
        enable_irq(irq);
#else
