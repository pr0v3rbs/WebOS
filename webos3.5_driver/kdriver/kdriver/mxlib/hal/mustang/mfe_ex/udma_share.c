...
MS_BOOL MMAPInit(MS_PHY buf_base, MS_U32 buf_size, MS_PHY* ppBufStart)
{
    *ppBufStart = buf_base;
    ...
    if(buf_base == (MS_VIRT)NULL) {
        ms_dprintk(DRV_L0, "MMAPInit: start:0x%x is NULL\n", buf_base);
        return FALSE;
    }
    else
        return TRUE;
...
MS_BOOL MMAPMalloc(MS_U32 u32MIUOffset, MS_PHY* ppBufStart, MS_PHY pBufEnd, MS_U32 size, MEMMAP_t * memmap, MS_PHY nAlignBytes, char* msg)
{
    //assert(memmap != NULL);
    ...
    *ppBufStart += size;
    if (*ppBufStart > pBufEnd) {
        ms_dprintk(DRV_L1, "Warning!! MFE MMAPMalloc over size!!, 0x%x 0x%x 0x%x\n"
            , *ppBufStart, pBufEnd, size);
        MFE_ASSERT(0);
        return FALSE;
    }
    memmap->miuPointer = (MS_U8*)addr_phy2log(memmap->miuAddress + u32MIUOffset);

    //ms_dprintk(DRV_L1,"MMAPMalloc:\n");
    ms_dprintk(DRV_L1,"u32MIUOffset: 0x%08x,", u32MIUOffset);
    ms_dprintk(DRV_L1,"memmap:miuPointer: 0x%08x,", memmap->miuPointer);
    ms_dprintk(DRV_L1,"size  : 0x%08x,", memmap->size);
