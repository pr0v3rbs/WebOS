...
#elif defined(__UBOOT__)||defined(MSOS_TYPE_LINUX_KERNEL)
...
MS_U32 u32MFERegOSBase;
MS_BOOL bSetRegBase = FALSE;
...
void MHAL_MFE_InitRegBase(MS_U32 U32RegBase)
{
    ...
    bSetRegBase = TRUE;
    if((U32RegBase != 0xfd200000) || (U32RegBase == 0x00))
    {
        printf("[i][%s][%d] U32RegBase = 0x%lx\n",__FUNCTION__,__LINE__,U32RegBase);
    }
}
...
#if defined(__MOBILE_CASE__)
//#define CMU_MMP_ASIC_CLK_MFE           170UL //clk_occmux_sel  CMU_CLK_OFF, CMU_CLK_32K, CMU_CLK_12M, CMU_CLK_24M, CMU_CLK_64M, CMU_CLK_85P3M, CMU_CLK_109P6M, CMU_CLK_128M, CMU_CLK_153P6M, CMU_CLK_170P6M
#ifndef CMU_CLK_24M
...
#define CMU_CLK_170P6M    CMU_MMP_ASIC_CLK_170P6M
    ...
    if (is_off) {
        *(MS_U16*)(0xbf206600+(0x18)*4) = 1;//disable MFE clock
    } else {
        //*(MS_U16*)(0xbf206600+(0x18)*4) = 0; // 4'b0000 123
        //*(MS_U16*)(0xbf206600+(0x18)*4) = 4; // 4'b0100 144
        //*(MS_U16*)(0xbf206600+(0x18)*4) = 8; // 4'b1000 172
        //*(MS_U16*)(0xbf206600+(0x18)*4) = 12; // 4'b1100 192

        if((clock_level >>2) == 0 )
            *(MS_U16*)(0xbf206600+(0x18)*4) = clock_level*4;
        else
            *(MS_U16*)(0xbf206600+(0x18)*4) = 8;
    }
    //MFE clock;
    //*(MS_U16*)(0xbf200000+(0x1980+0x18)*4) = 2<<2; //2<<2
#elif defined(_MFE_T8_) &&defined(_MIPS_PLATFORM_)&&defined(_MFE_UTOPIA_)

    printf("[i][%s][%d] is_off = %d, T8_RIU_BASE = 0x%0x, bSetRegBase =%u\n", __FUNCTION__, __LINE__, (int)is_off, (unsigned int)T8_RIU_BASE, bSetRegBase);

    if((T8_RIU_BASE != 0xfd200000) || (T8_RIU_BASE == 0x00))
    {
        printf("[i][%s][%d] is_off = %d, T8_RIU_BASE = 0x%0x\n", __FUNCTION__, __LINE__, (int)is_off, (unsigned int)T8_RIU_BASE);
    }
    if (is_off) {

        if(bSetRegBase == TRUE)
        {
            *(MS_U16*)(T8_RIU_BASE+(0x1980+0x18)*4) = 1;//disable MFE clock
            bSetRegBase = FALSE;
        }
    } else {
        printf("[i][%s][%d] is_off = %d, T8_RIU_BASE = 0x%0x, bSetRegBase =%u\n", __FUNCTION__, __LINE__, (int)is_off, (unsigned int)T8_RIU_BASE, bSetRegBase);
        if(bSetRegBase == TRUE)
        {
            ...
            {
                case MFE_CLK_VERY_SLOW:
                    *(MS_U16*)(T8_RIU_BASE+(0x1980+0x18)*4) = 0; break;
                case MFE_CLK_SLOW:
                    *(MS_U16*)(T8_RIU_BASE+(0x1980+0x18)*4) = 4; break;
                case MFE_CLK_MEDIUM:
                    *(MS_U16*)(T8_RIU_BASE+(0x1980+0x18)*4) = 8; break;
                case MFE_CLK_FAST:
                    *(MS_U16*)(T8_RIU_BASE+(0x1980+0x18)*4) = 12; break;
                default:
                    *(MS_U16*)(T8_RIU_BASE+(0x1980+0x18)*4) = 12; break;
            }
        }
...
{
    MS_U16 reg_mfe_s_bsp_bit_cnt_high=0;
    MS_U16 reg_mfe_s_bsp_bit_cnt_low=0;
    MS_U32 nHwBytes;
    ...
    nHwBytes = (((MS_U32)reg_mfe_s_bsp_bit_cnt_high<<16) + reg_mfe_s_bsp_bit_cnt_low ) >> 3;
	return nHwBytes;
}
...
#if 1//str
void MHal_MFE_ClearPowerSuspend(void)
{
    MFE_U16 pwr_bits;
    ReadRegMFE_BANK1(0x7f, &pwr_bits);
    pwr_bits = pwr_bits & 0xfff7;
    WriteRegMFE_BANK1(0x7f, pwr_bits, "", 0, "");
}

void MHal_MFE_GetPowerSuspend(MFE_U16 *pwr_bits)
{
    if(pwr_bits != NULL)
    {
        ReadRegMFE_BANK1(0x7f, pwr_bits);
...
#else
	MS_U16 sadr_low, sadr_high, eadr_low, eadr_high;
...
{
   WriteRegMFE(0x0a, (MS_U16)mfe_reg->reg0a, "", 0, "");
}
...
void MHal_MFE_GetCRC(MS_U8 checksum_HW[8])
{
	...
	for(i=0;i<4;i++){
		...
		checksum_HW[2*i] = (MS_U8)(u16Reg1&0xFF);
