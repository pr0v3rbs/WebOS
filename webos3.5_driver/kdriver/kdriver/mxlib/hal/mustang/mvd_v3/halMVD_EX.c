...
//-------------------------------------------------------------------------------------------------
#define MVD_SUPPORT_CC  //Enable cc with display order
...
#define MVDHW_ON_MIU1      ((HAL_MVD_RegReadByte(MIU0_SEL1_L) & BIT2) == BIT2)
...
#define _90K_TO_MS_U64(x)   ((x!=MVD_U64_MAX)?(do_div(x,90)):(x))  //90k counter ==> ms
...
#define HAL_MVD_InvalidBuffRetFalse(x)                                                              \
    do {                                                                                            \
        if ((x)==0)                                                                                 \
        {                                                                                           \
            MVD_DEBUGERROR(printf("%s(%d) error: NULL buffer address.\n", __FUNCTION__, __LINE__)); \
            return FALSE;                                                                           \
        }                                                                                           \
    } while(0)
...
{
    MS_U32 pu8MVDGetVolBufStart;
    MS_U32 u32VolAdd;
    MS_U32 pu8MVDGetFrameInfoBufStart;
    MS_U32 pu8MVDSetHeaderBufStart;
    MS_U32 u32UserDataBuf;
    MS_U32 u32MVDFWSLQTABTmpbufAdr;
    MS_U32 u32MVDFWPtsTblAddr;
    MS_U32 u32DynScalingAdd;
    MS_U32 u32ScalerInfoAdd;
    MS_U32 u32DecFrmInfoAdd;
    MS_U32 u32VBBUTableAdd;
} MVD_FWBuff;
...
typedef struct _MVD_SLQTBLInfo
{
    ...
    MS_U32 u32ESBuffEnd;    ///< miu offset of ES buffer end, used for report TS read/write ptr.
    MS_U32 u32FileEndPtr;   ///< write ptr of file-end
    MS_U32 u32PreEsRd;      ///< previous ES read ptr
    MS_U32 u32PreEsWr;      ///< previous ES write ptr
...
typedef struct _MVDMiuCfg
{
    ...
    MS_PHYADDR u32Miu2BaseAddr;
    MS_PHYADDR u32Miu3BaseAddr;
    MS_U8    u8FWMiuSel;
    MS_U8    u8HWMiuSel;
} MVDMiuCfg;
...
typedef struct
{
    ...
    MS_BOOL bEnableThumbnailMode[MAX_DEC_NUM];
    ...
    MS_U32 u32strcmdbuf[MAX_DEC_NUM];
    MS_U8 u8strcmdcnt[MAX_DEC_NUM];
    MS_U32 u32strFWBinAddr;
#endif
} MVD_Hal_CTX;

static MS_U32 _u32FWCodeBufStartAddr = 0;
static MS_U32 _u32FWCodeBufEndAddr = 0;
//global variables
...
static MS_U32 MVD_GetFWBuffData(MS_U8 u8Idx, FW_BUFF_TYPE eBuffType, MS_U16 u16Offset, MS_U8 u8Size)
{
    MS_U32 u32Val = 0;
    MS_U32 u32BufStart = NULL;
    switch (eBuffType)
    {
    ...
    if (NULL == u32BufStart)
    {
        printf("%s err: u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d\n",
               __FUNCTION__, u8Idx, eBuffType, u16Offset, u8Size);
        return 0;
    }

    if (u8Size == sizeof(MS_U8))
    {
        u32Val = (MS_U32)HAL_MVD_MemReadByte(u32BufStart+u16Offset);
    }
    else if (u8Size == sizeof(MS_U32))
    {
        u32Val = HAL_MVD_MemRead4Byte(u32BufStart+u16Offset);
    }
    else if (u8Size == sizeof(MS_U16))
    {
        u32Val = HAL_MVD_MemRead2Byte(u32BufStart+u16Offset);
    }

    MVD_DEBUGVERBAL(printf("%s: u32Val=%ld for u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d\n",
                          __FUNCTION__, u32Val, u8Idx, eBuffType, u16Offset, u8Size));
    return u32Val;
}
...
static void MVD_SetFWBuffData(MS_U8 u8Idx, FW_BUFF_TYPE eBuffType, MS_U8 u8Offset, MS_U8 u8Size, MS_U32 u32Value)
{
    ...
    if (NULL == u32BufStart)
    {
        printf("%s err: u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d, value=%d\n",
    ...
    MVD_DEBUGVERBAL(printf("%s: u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d, value=%d\n",
                          __FUNCTION__,  (unsigned int)u8Idx, (unsigned int)eBuffType, (unsigned int)u8Offset, (unsigned int)u8Size,(unsigned int)u32Value));
}
#endif
...
void HAL_MVD_MemGetMap(MS_U8 u8Idx, MS_U8 u8type, MS_U32* pu32addr, MS_U32* pu32len)
{
...
MS_U32 HAL_MVD_MemRead4Byte(MS_U32 u32Address)
{
    ...
    HAL_MVD_CPU_Sync();
    HAL_MVD_ReadMemory();
    MS_U32 u32phyAddress = 0;

    _miu_offset_to_phy(pMVDHalContext->stMiuCfg.u8FWMiuSel,u32Address,u32phyAddress);
#ifdef MSOS_TYPE_LINUX_KERNEL
    MS_U32 u32VAddr = (MS_U32)HAL_MVD_PA2NonCacheSeg(u32phyAddress);
    if(MsOS_MPool_VAError(u32VAddr))
    {
        printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32VAddr,__LINE__);
        return FALSE;
    }
#endif
    u32Val = *(volatile MS_U32*) HAL_MVD_PA2NonCacheSeg(u32phyAddress);
...
#endif
    *(volatile MS_U32 *) HAL_MVD_PA2NonCacheSeg(u32phyAddress) = u32Value;
...
void HAL_MVD_SetReqMask(MS_BOOL bEnMask)
{
    ...
    if (MVDHW_ON_MIU1)
    {
        HAL_MVD_RegWriteBit(MIU1_RQ1_MASK_L, bEnMask, BIT2);  //MVD R/W
        HAL_MVD_RegWriteBit(MIU1_RQ0_MASK_H, bEnMask, BIT4);  //MVD bbu R/W
    }
    else
    {
        HAL_MVD_RegWriteBit(MIU0_RQ1_MASK_L, bEnMask, BIT2);  //MVD R/W
...
static MS_BOOL HAL_MVD_Memset4Byte(MS_U32 u32Address, MS_U32 u32Value)
{
    MS_U32 U32_PhyAddr = 0;

    _miu_offset_to_phy(pMVDHalContext->stMiuCfg.u8FWMiuSel,u32Address,U32_PhyAddr);
#ifdef MSOS_TYPE_LINUX_KERNEL
    MS_U32 u32VAddr = (MS_U32)HAL_MVD_PA2NonCacheSeg(U32_PhyAddr);
    if(MsOS_MPool_VAError(u32VAddr))
    {
        printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32VAddr,__LINE__);
        return FALSE;
...
#endif
    *(volatile MS_U32 *) HAL_MVD_PA2NonCacheSeg(U32_PhyAddr) = u32Value;

    return TRUE;
}

static MVD_PicType _MVD_ExStreamTypeToNumber(unsigned short eType)
{
    MVD_PicType res = E_MVD_PIC_UNKNOWN;
    switch(eType)
    {
        case 0   : res = E_MVD_PIC_I; break;
        case 1   : res = E_MVD_PIC_P; break;
        case 2   : res = E_MVD_PIC_B; break;
        default  : res = E_MVD_PIC_UNKNOWN; break;
    }
    return res;
}
...
MS_BOOL HAL_MVD_ReleaseFW(void)
{
    ...
    {
        MVD_DEBUGINFO(printf("%s VPU_IsRsted\n", __FUNCTION__));
        return TRUE;
    }
...
MS_U8 HAL_MVD_GetFBMode(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGINFO(printf("FBMode=0x%x\n", u8Mode));
    return u8Mode;
}
...
void HAL_MVD_ResetHandShake(MS_U8 u8Idx, MVD_HANDSHAKE_CMD cmd)
{
    ...
    switch (cmd)
    {
        ...
        case MVD_HANDSHAKE_FLUSHQUEUE_COMMAND:
            ...
            break;
        case MVD_HANDSHAKE_VSYNC_CONTROL:
            (u32CmdState.mvdcmd_handshake_vsync_control) = 0;
            break;
        default:
            break;
    }
...
MS_BOOL HAL_MVD_MVDCommand ( MS_U8 u8cmd, MVD_CmdArg *pstCmdArg )
{
...
#ifdef VDEC3
    MS_U32 u32CmdArg;
    HVD_DRAM_COMMAND_QUEUE_SEND_STATUS u32Ret = E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL;
    MS_U32 u32Timeout;
    ...
    CMD_QUEUE *pShm = (CMD_QUEUE *)MsOS_PA2KSEG1(pstMemCfg->u32FWCodeAddr + VCOMMANDQ_INFO_START + u8VPUIdx*0x100000);
#ifdef MSOS_TYPE_LINUX_KERNEL
    MS_U32 u32ShmAddr = (MS_U32)pShm;
    if(MsOS_MPool_VAError(u32ShmAddr))
    {
        printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32ShmAddr,__LINE__);
        printk("u8cmd:%x  FWCodeAddr:%lx  u8VPUIdx:%d\n",u8cmd,pstMemCfg->u32FWCodeAddr,u8VPUIdx);
        return FALSE;
    }
#endif
    MS_U32 u32BufStart2 = NULL;
    ...
    if ((CMD_GET_AFD != u8cmd) && (CMD_SLQ_GET_TBL_RPTR != u8cmd) &&
            ...
            (CMD_DECODE_STATUS != u8cmd) && (CMD_RD_PTS != u8cmd) && (CMD_GET_INT_STAT != u8cmd) &&
            (CMD_RD_IO != u8cmd)&& (CMD_SEND_UNI_PTS != u8cmd)&& (CMD_GET_SYNC_STAT != u8cmd)&&
            (CMD_WD_USER_RP != u8cmd) && (CMD_RD_USER_WP!= u8cmd) && (CMD_PARSER_READ_POSITION != u8cmd))
    {
        MS_U8 u8Idx = pstCmdArg->Arg5;
        MS_U32 u32StrAddrOffset = HAL_VPU_EX_GetStrOffset(0);
        #define VDEC_STR_MAIN_CMD_COUNT u32StrAddrOffset+VDEC_STR_MAIN_CMD_COUNT_OFFSET
        #define VDEC_STR_SUB_CMD_COUNT u32StrAddrOffset+VDEC_STR_SUB_CMD_COUNT_OFFSET
        if(HAL_VPU_EX_CheckSTRState(u8Idx) && pMVDHalContext->u8strcmdcnt[u8Idx]<VDEC_STR_CMD_RESERVERD)
        {
            HAL_MVD_MemWrite4Byte(pMVDHalContext->u32strcmdbuf[u8Idx] + pMVDHalContext->u32strFWBinAddr, VDEC_STR_MVD);  // 1:mvd 2:hvd
            HAL_MVD_MemWrite4Byte(pMVDHalContext->u32strcmdbuf[u8Idx] + pMVDHalContext->u32strFWBinAddr + VDEC_STR_CMD,u8cmd);
            HAL_MVD_MemWriteByte(pMVDHalContext->u32strcmdbuf[u8Idx] + pMVDHalContext->u32strFWBinAddr + VDEC_STR_ARG0,pstCmdArg->Arg0);
            HAL_MVD_MemWriteByte(pMVDHalContext->u32strcmdbuf[u8Idx] + pMVDHalContext->u32strFWBinAddr + VDEC_STR_ARG1,pstCmdArg->Arg1);
            HAL_MVD_MemWriteByte(pMVDHalContext->u32strcmdbuf[u8Idx] + pMVDHalContext->u32strFWBinAddr + VDEC_STR_ARG2,pstCmdArg->Arg2);
            HAL_MVD_MemWriteByte(pMVDHalContext->u32strcmdbuf[u8Idx] + pMVDHalContext->u32strFWBinAddr + VDEC_STR_ARG3,pstCmdArg->Arg3);
            HAL_MVD_MemWriteByte(pMVDHalContext->u32strcmdbuf[u8Idx] + pMVDHalContext->u32strFWBinAddr + VDEC_STR_ARG4,pstCmdArg->Arg4);
            HAL_MVD_MemWriteByte(pMVDHalContext->u32strcmdbuf[u8Idx] + pMVDHalContext->u32strFWBinAddr + VDEC_STR_ARG5,pstCmdArg->Arg5);
            pMVDHalContext->u8strcmdcnt[u8Idx]++;
            pMVDHalContext->u32strcmdbuf[u8Idx]+=VDEC_STR_ALIGN;
            if(u8Idx==0)
                HAL_MVD_MemWriteByte(VDEC_STR_MAIN_CMD_COUNT + pMVDHalContext->u32strFWBinAddr , pMVDHalContext->u8strcmdcnt[u8Idx]);
            else
                HAL_MVD_MemWriteByte(VDEC_STR_SUB_CMD_COUNT + pMVDHalContext->u32strFWBinAddr , pMVDHalContext->u8strcmdcnt[u8Idx]);

            //printf("\n[%d][STR] [%d]MVD_CMD: %x; ARG: %x, %x, %x, %x, %x, %x", u8Idx,pMVDHalContext->u8strcmdcnt[u8Idx],u8cmd,
            //pstCmdArg->Arg0,pstCmdArg->Arg1, pstCmdArg->Arg2, pstCmdArg->Arg3, pstCmdArg->Arg4, pstCmdArg->Arg5);
        }
    }
#endif
    ...
    if(HAL_MVD_IsDisplayCommand(u8cmd) == TRUE)
    {
        if(HAL_MVD_IsNeedResponseCommand(u8cmd) == TRUE)
        {
            ...
            _miu_offset_to_phy(pMVDHalContext->stMiuCfg.u8FWMiuSel,GET_VOL_BUFFADD(u8HalIdx),u32BufStart2);
        ...
        do
        {
            ...
            else if(u32Ret == E_HVD_COMMAND_QUEUE_SEND_FAIL)
            {
                bRet = FALSE;
                break;
            }
    ...
    else if(HAL_MVD_IsNormalCommand(u8cmd) == TRUE)
    {
        ...
        do
        {
            ...
            else if (MsOS_GetSystemTime() > u32Timeout)
            {
                bRet = FALSE;
                break;
            }
...
_CMD_DONE:
    ...
    if (!bRet)
    {
        MVD_DEBUG_FWCMD(printf("%s timeout dump pc\n", __FUNCTION__));
        MS_U32 x=0;
        for (x=0; x<30; x++)
            MVD_DEBUG_FWCMD(printf("0x%lx\n", HAL_VPU_EX_GetProgCnt()));
        MVD_DEBUG_FWCMD(printf("###\n"));
    }
...
MS_U32 HAL_MVD_GetFWVer(MS_U32 u32VpuSid)
{
    ...
    {
        MVD_ERROR( printf( "Command: 0x%x fail!!\r\n", CMD_GET_FW_VERSION ) );
        return 0;
    }
...
static MS_BOOL _MVD_Check_FW_Rdy(MS_U32 u32VpuSid)
{
    ...
    if (ret == FALSE)
    {
        MVD_ERROR(printf("MVD f/w header version is wrong,%x,%x,%x,%x\n",(*(pVersion+4)), INTERFACE_VERSION,(*pVersion), FW_VERSION));
    }
...
MS_BOOL _DumpCtrl(MS_U8 u8Idx)
{
...
#ifdef MSOS_TYPE_LINUX_KERNEL
    MS_U32 u32VAddr = (MS_U32)ctl_ptr;
    if(MsOS_MPool_VAError(u32VAddr))
    {
        printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32VAddr,__LINE__);
        return FALSE;
    ...
    MVD_DEBUGINFO(printf("version=0x%x, statue=0x%x, last_ctl_cmd=0x%x, last_ctl_arg=0x%x\n",
    ...
    while (CTL_TASK_CMDRDY != ctl_ptr->statue)
    {
        ...
        {
            MVD_DEBUGERROR(printf("CTRL timeout!!! %d\n", __LINE__));
            return FALSE;
        }
    }
    ...
    for (i = 0; i < 4; i++)
    {
        MVD_DEBUGINFO(printf("%s: Task %d, status=%d\n", __FUNCTION__, i, ctl_ptr->task_statue[i]));
    }
    return TRUE;
}
...
MS_BOOL HAL_MVD_CreateTask(MS_U8 u8Idx, HAL_VPU_StreamId eVpuId)
{
    ...
    {
        stVpuInitPara.pFBCfg->u32FrameBufAddr = pMVDHalContext->cmaInitParam.heap_miu_start_offset;
        stVpuInitPara.pFBCfg->u32FrameBufSize = pMVDHalContext->cmaInitParam.heap_length;
        bFWdecideFB = TRUE;

    }
#endif
    stTaskInfo.u8HalId = u8Idx;
    pMVDHalContext->_stMVDStream[u8Idx].u32SLQId = HAL_VPU_EX_GetBBUId(stTaskInfo.u32Id, &stTaskInfo, FALSE); //FIXME: MVD how to know Nstream?
    if(pMVDHalContext->_stMVDStream[u8Idx].u32SLQId == HAL_VPU_INVALID_BBU_ID)
    {
        MVD_DEBUGINFO(printf("ERROR!!!MVD GetBBU Id Fail \n"));
    }
    else
    {
    ...
    if(bRet == FALSE)
    {
        ...
        {
            MVD_DEBUGERROR(printf("[%s][%d]ERROR!!! Free BBU Id Fail !!!\n",__FUNCTION__,__LINE__));
        }
    }
#else
    bRet = HAL_VPU_EX_TaskCreate((MS_U32)eVpuId, (void*)&stVpuInitPara);
#endif
    MVD_DEBUGINFO(printf(" Create Task!!! bRet=%x ###\n\n", bRet));
    _DumpCtrl(u8Idx);
...
MS_BOOL HAL_MVD_DeleteTask(MS_U8 u8Idx, HAL_VPU_StreamId eVpuId)
{
    ...
    if(!HAL_VPU_EX_FreeBBUId(stTaskInfo.u32Id, pMVDHalContext->_stMVDStream[u8Idx].u32SLQId, &stTaskInfo))
    {
        MVD_DEBUGINFO(printf("ERROR!!! Free BBU Id Fail !!!\n"));
    }
#endif
    bRet = HAL_VPU_EX_TaskDelete((MS_U32) eVpuId, &stVpuInitPara);
    MVD_DEBUGINFO(printf(" Delete Task!!! ###\n\n"));
    _DumpCtrl(u8Idx);
    return bRet;
}
...
MS_BOOL HAL_MVD_Init(MS_U8 u8HalIdx,MVD_CodecType eCodecType, MS_U32 u32VpuSid)
{
    ...
    MS_BOOL ret;
    MVD_MEMCfg* pMemCfg = HAL_MVD_GetMEMCfg(u8HalIdx);
#ifdef SUPPORT_VDEC_STR
    MS_U32 u32StrAddrOffset = HAL_VPU_EX_GetStrOffset(pMemCfg->u32FWCodeSize);
    #define VDEC_STR_MAIN_CMD_BUF u32StrAddrOffset+VDEC_STR_MAIN_CMD_OFFSET
    #define VDEC_STR_SUB_CMD_BUF u32StrAddrOffset+VDEC_STR_SUB_CMD_OFFSET
    if(u8HalIdx==0)
        pMVDHalContext->u32strcmdbuf[u8HalIdx] = VDEC_STR_MAIN_CMD_BUF;
    else if(u8HalIdx==1)
        pMVDHalContext->u32strcmdbuf[u8HalIdx] = VDEC_STR_SUB_CMD_BUF;

    pMVDHalContext->u32strFWBinAddr= _PA2Offset(pMemCfg->u32FWCodeAddr);
#endif
    _u32FWCodeBufStartAddr = pMemCfg->u32FWCodeAddr;
    _u32FWCodeBufEndAddr = _u32FWCodeBufStartAddr + pMemCfg->u32FWCodeSize;
    ...
    {
        MVD_DEBUGERROR(printf("%s:HAL_MVD_InitFW(%x) failed\n", __FUNCTION__, u8HalIdx));
        if (!HAL_MVD_DeleteTask(u8HalIdx, (HAL_VPU_StreamId)u32VpuSid))
        {
            MVD_DEBUGERROR(printf("%s:HAL_MVD_DeleteTask failed\n", __FUNCTION__));
        }
    ...
    {
        MVD_DEBUGINFO(printf("%s:HAL_MVD_InitFW(%x) success\n", __FUNCTION__, u8HalIdx));
    }
...
//------------------------------------------------------------------------------
void HAL_MVD_SetFrameBuffAddr(MS_U8 u8Idx, MS_U32 u32addr, MS_U8 u8fbMode)
{
    MVD_CmdArg mvdcmd;
...
/// @param -u32addr \b IN : start address
//------------------------------------------------------------------------------
void HAL_MVD_SetHeaderBufferAddr (MS_U8 u8Idx, MS_U32 u32addr )
{
    MVD_CmdArg mvdcmd;
...
/// @param -u32addr \b IN : start address
//------------------------------------------------------------------------------
void HAL_MVD_SetVolInfoBufferAddr (MS_U8 u8Idx, MS_U32 u32addr )
{
    MVD_CmdArg mvdcmd;
...
/// @param -u32addr \b IN : start address
//------------------------------------------------------------------------------
void HAL_MVD_SetFrameInfoBufferAddr (MS_U8 u8Idx, MS_U32 u32addr )
{
    MVD_CmdArg mvdcmd;
...
/// @param -u32addr \b IN : start address
//------------------------------------------------------------------------------
void HAL_MVD_SetIAPBufferAddr (MS_U8 u8Idx, MS_U32 u32addr )
{
    MVD_CmdArg mvdcmd;
...
/// @param -u32addr \b IN : start address
//------------------------------------------------------------------------------
void HAL_MVD_SetDPBufferAddr (MS_U8 u8Idx, MS_U32 u32addr )
{
    MVD_CmdArg mvdcmd;
    MS_ASSERT((u32addr%8)==0);
    u32addr >>= 3;
...
/// @param -u32addr \b IN : start address
//------------------------------------------------------------------------------
void HAL_MVD_SetMVBufferAddr (MS_U8 u8Idx, MS_U32 u32addr )
{
    MVD_CmdArg mvdcmd;
...
static void _MVD_SetUserDataBufStart(MS_U8 u8Idx, MS_U32 u32addr, MS_U8 u8arg3)
{
    MVD_CmdArg mvdcmd;
...
/// @param -u32addr \b IN : start address
//------------------------------------------------------------------------------
void HAL_MVD_SetUserDataBuf(MS_U8 u8Idx, MS_U32 u32addr, MS_U32 u32size)
{
    MS_U8 u8ccType = 0;
    ...
    u8ccType = 2;
#elif defined(MVD_SUPPORT_CC)
    u8ccType = 4;   //display order
#else
    u8ccType = 2;// 2 for testing 0;
#endif

#if defined(MVD_SUPPORT_CC)
...
void HAL_MVD_SetSLQTblBufStartEnd(MS_U8 u8Idx, MS_U32 u32start, MS_U32 u32end)
{
    MVD_CmdArg mvdcmd;
...
//------------------------------------------------------------------------------
MS_U32 HAL_MVD_GetTsFileESReadPtr(MS_U8 u8Idx)
{
    MS_U32 u32Add = 0;
    MVD_CmdArg mvdcmd;
...
//------------------------------------------------------------------------------
MS_U32 HAL_MVD_GetTsFileESWritePtr(MS_U8 u8Idx)
{
    MS_U32 u32Diff = 0;
    MS_U32 u32WrPtr = 0;
    MVD_CmdArg mvdcmd;
    ...
    {
        MVD_DEBUGINFO(printf("ES wrapping Wr=0x%lx ==> ", u32WrPtr));
        u32WrPtr -= pstMemCfg->u32BSSize;
        MVD_DEBUGINFO(printf("0x%lx\n", u32WrPtr));
    }
    return u32WrPtr;
}
...
MS_BOOL HAL_MVD_EnableLastFrameShow(MS_U8 u8Idx, MS_BOOL bEnable)
{
    ...
    MVD_SLQTBLInfo* pstSlqTblInfo = HAL_MVD_GetSlqTblInfo(u8Idx);
    MS_U32* pu32FileEndPtr = &pstSlqTblInfo->u32FileEndPtr;
    ...
    if (E_MVD_SLQ_TBL_MODE == curSrcMode)
    {
        ...
        MVD_DEBUGINFO(printf("fe=%lx, rd=%lx, wr=%lx\n", *pu32FileEndPtr,
                        pstSlqTblInfo->pDrvSlqTbl->u32RdPtr, pstSlqTblInfo->pDrvSlqTbl->u32WrPtr));
    }
...
MS_BOOL HAL_MVD_FlushDisplayBuf(MS_U8 u8Idx)
{
    ...
    while(!HAL_MVD_IsCmdFinished(u8Idx, MVD_HANDSHAKE_FLUSHQUEUE_COMMAND))
    {
        ...
        {
            MVD_DEBUGERROR( printf( "Ctrl: 0x%x fail timeout!!\r\n", CMD_FLUSH_DISP_QUEUE ) );
            break;
        }
    }
...
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------
MS_BOOL HAL_MVD_SetPtsTblAddr(MS_U8 u8Idx, MS_U32 u32addr)
{
    MVD_CmdArg mvdcmd;
...
MS_BOOL HAL_MVD_SkipData(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGINFO(printf("====> %s (t1=%lu t2=%lu diff=%lu)\n", __FUNCTION__,
...
MS_BOOL HAL_MVD_PauseDisp(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR( printf( "Command: 0x%x(ON) fail!!\r\n", CMD_DISPLAY_PAUSE) );
        bRst = FALSE;
    }
    return bRst;
}
...
//------------------------------------------------------------------------------
MS_BOOL HAL_MVD_SetScalerInfoAddr(MS_U8 u8Idx, MS_U32 u32addr,MS_U8 u8Arg4)
{
    MVD_CmdArg mvdcmd;
...
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------
MS_BOOL HAL_MVD_SetDynamicScaleAddr(MS_U8 u8Idx, MS_U32 u32addr)
{
    MVD_CmdArg mvdcmd;
...
/// @param -u32addr \b IN : start address (units in byte)
//------------------------------------------------------------------------------
void HAL_MVD_SetDecFrmInfoAddr(MS_U8 u8Idx, MS_U32 u32addr)
{
    MVD_CmdArg mvdcmd;
...
MS_U32 HAL_MVD_GetIntState(MS_U8 u8Idx)
{
    ...
    else
    {
        ...
        {
            MVD_DEBUGINFO(printf("Cmd 0x%x normal timeout.\n", CMD_GET_INT_STAT));
        }
    }
...
MS_BOOL MVD_IntHasAbnormalPTS(MS_U32 u32IntStat)
{
    return (((u32IntStat&INT_PTS_DISCONTINUE)==INT_PTS_DISCONTINUE) ? TRUE : FALSE);
}
...
static MS_U8 MVD_GetSyncStat(MS_U8 u8Idx, MS_U8 u8ArgIdx)
{
    ...
    MVD_DEBUGINFO(printf("Sync On/Off %x, Done %x, Stat %x, diff=%ld\n",
                  mvdcmd.Arg0, mvdcmd.Arg1, mvdcmd.Arg2, HAL_MVD_GetPtsStcDiff(u8Idx)));
...
void HAL_MVD_SlqTblLoadWrPtr(MS_U8 u8Idx, MS_U32 u32WrPtr)
{
#if 1
    MVD_SLQTBLInfo* pstSlqTblInfo = HAL_MVD_GetSlqTblInfo(u8Idx);
    MVD_CmdArg mvdcmd;
...
MS_U32 HAL_MVD_SlqTblProbeWrPtr(MS_U8 u8Idx)
{
    return HAL_MVD_SlqTblProbe(MVD_HKSLQ_GET_WRITEPTR);
}

MS_U32 HAL_MVD_SlqTblProbeRdPtr(MS_U8 u8Idx)
{
...
//------------------------------------------------------------------------------
/// Set firmware as ForceInterlace mode
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------
MS_BOOL HAL_MVD_ForceInterlaceMode(MS_U8 u8Idx, MS_U8 u8Mode)
{
    MVD_CmdArg mvdcmd;

    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = u8Mode; //1: enable, 0:disable
    mvdcmd.Arg1 = 0;
    SET_DECNUM(mvdcmd, u8Idx);
    SET_CMD_RET_FALSE(CMD_VC1_FORCE_INTLACE_DISP, &mvdcmd);

    return TRUE;
}

//------------------------------------------------------------------------------
/// Set firmware as Progressive mode
/// @return -TRUE for success; FALSE for failure.
//------------------------------------------------------------------------------
MS_BOOL HAL_MVD_ForceProgressiveMode(MS_U8 u8Idx, MS_U8 u8Mode)
{
    MVD_CmdArg mvdcmd;

    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = 0;
    mvdcmd.Arg1 = u8Mode; //1: enable, 0:disable
    SET_DECNUM(mvdcmd, u8Idx);
    SET_CMD_RET_FALSE(CMD_VC1_FORCE_INTLACE_DISP, &mvdcmd);
...
//------------------------------------------------------------------------------
MS_U32 HAL_MVD_SetHWBuffer(MS_U8 u8Idx, MS_U32 u32Add)
{
    MS_U32 tmpAdr;
...
static MS_BOOL MVD_WriteFWBuffData(MS_U8 u8Idx, FW_BUFF_TYPE eBuffType, MS_U8 u8Offset, MS_U8 u8Size, MS_U32 u32Val)
{
    ...
    MVD_DEBUGINFO(printf("%s: u32Val=%ld for u8Idx=0x%x, bufType=0x%x, offset=%d, size=%d\n",
...
MS_BOOL HAL_MVD_IsMoreDecFrame(MS_U8 u8Idx)
{
    unsigned int u32FrmCnt = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_FRAME_COUNT, sizeof(MS_U32));
    if(u32FrmCnt)
    {
        unsigned int u32FrmWrtPtr = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_FRAME_WRTPTR, sizeof(MS_U32));
        unsigned int u32FrmReadPtr = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_FRAME_READPTR, sizeof(MS_U32));
        return (u32FrmWrtPtr != u32FrmReadPtr);
    }
    return FALSE;
}
...
MS_U32 HAL_MVD_GetDropPicCounter(MS_U8 u8Idx)
{
    return MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_DROP_FRAME_COUNT, sizeof(MS_U32));
}
...
MS_BOOL HAL_MVD_IsCmdFinished(MS_U8 u8Idx, MVD_HANDSHAKE_CMD eCmd)
{
    ...
    switch (eCmd)
    {
        ...
        case MVD_HANDSHAKE_FLUSHQUEUE_COMMAND:
            ...
            break;
        case MVD_HANDSHAKE_VSYNC_CONTROL:
            bCmdRdy = (MS_BOOL)(u32CmdState.mvdcmd_handshake_vsync_control);
            break;
        default:
            bCmdRdy = FALSE;
            break;
...
{
    MS_BOOL bRet = FALSE;
    MS_U32 pu8MVDGetFrameInfoBufStart = GET_FRMINFO_BUFFADD(u8Idx);
    if (NULL == pInfo)
    {
        return FALSE;
    }
...
MS_U32 HAL_MVD_GetXcLowDelayIntState(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR(printf("MDrv_MVD_GetXcLowDelayIntState error: pu8MVDGetFrameInfoBufStart=NULL\n"));
        return 0;
    }

    MVD_DEBUGINFO(printf("MDrv_MVD_GetXcLowDelayIntState()=%lx,%lx,%lx,%lx,%lx\n",
...
MS_U32 HAL_MVD_GetMemOffset(MS_PHYADDR u32PhyAdd)
{
...
MS_BOOL HAL_MVD_SLQTblSendPacket(MS_U8 u8Idx, MVD_PacketInfo *pstVideoPKT)
{
    MVD_SLQTBLInfo* pstSlqTblInfo = HAL_MVD_GetSlqTblInfo(u8Idx);
    MS_U32 u32EsLast;
    MS_U32* u32LastEntry = &pstSlqTblInfo->pDrvSlqTbl->u32WrPtr;
    MS_U32* pu32EsNew = &pstSlqTblInfo->pDrvEsTbl->u32WrPtr;
    MS_U32 u32EntryWord = 0;
    MS_U32 u32Index = 0;
    MS_U32 u32Pts;
    MS_U32 u32MVDFWPtsTblAddr = NULL;
    MVD_CtrlCfg* pstCtrlCfg = HAL_MVD_GetCtrlCfg(u8Idx);
...
MS_BOOL HAL_MVD_IsProgressiveStream(MS_U8 u8Idx)
{
     ...
     u32VAddr = (MS_U32)HAL_MVD_PA2NonCacheSeg(temp);
     #ifdef MSOS_TYPE_LINUX_KERNEL
     if(MsOS_MPool_VAError(u32VAddr))
     {
         printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32VAddr,__LINE__);
     ...
     #endif
     memcpy((void*)(&gvolInfo),(void*)(u32VAddr),sizeof(FW_VOL_INFO));
     return gvolInfo.original_progressive;
...
void HAL_MVD_GetFrameInfo(MS_U8 u8Idx, MVD_FrameInfo *pinfo)
{
    ...
    MVD_DEBUGVERBAL(printf("===> MVD: frameRate=%ld  curCodecType=0x%x\n", pinfo->u32FrameRate, curCodecType));
    ...
    if(pMVDHalContext->stMiuCfg.u8FWMiuSel >= E_CHIP_MIU_2)
        {
        MVD_DEBUGERROR(printf("u8MiuSel is %d !!, but return type is only Boolean. Please use MDrv_HVD_EX_GetDSBufMiuSelect() to get right miu select of DS buffer!\n",pMVDHalContext->stMiuCfg.u8FWMiuSel));
        }

    if (pstMemCfg->bEnableDynScale)
        {
        #ifdef ENABLE_EXTERNAL_DS_BUFFER
        if(pMVDHalContext->stCtrlCfg[u8Idx].bExternalDSBuf)
        {
            if(pinfo->u32DynScalingAddr > HAL_MVD_GetMiu1BaseAdd())
            {
                pinfo->bEnableMIUSel = 1;
            }
            else
            {
                pinfo->bEnableMIUSel = 0;
            }
            pinfo->u32DynScalingAddr = pMVDHalContext->stFWBuff[u8Idx].u32DynScalingAdd;
        }
        else
        #endif
        {
...
E_MVD_Result MVD_RstFrmInfo(MS_U8 u8Idx, MVD_FrmInfoType eType)
{
    ...
    MS_U32 u32DecFrmInfoAdd = GET_DECFRMINFO_BUFFADD(u8Idx);
...
#define FW_SHM_SIZE 0x20000  //this size use in vdec 3.1
static MS_BOOL MVD_GetFWBuffAdd(MS_U32 u32Start, MS_U32 u32AvailLen, MVD_FWBuff* pBuff)
{
    MS_U32 tmpAdr = u32Start;

    if(pBuff == NULL)
    {
        MVD_DEBUGERROR(printf("%s err: NULL pBuff\n", __FUNCTION__));
        return FALSE;
    }
    ...
    //MVD3_FW_USER_DATA_BUF_BACKUP_LEN is used as CC decoding buffer for MVD_SUPPORT_CC
    ...
    MVD_DEBUGINFO(printf("u32UserDataBuf start=%lx\n", pBuff->u32UserDataBuf));
    MVD_DEBUGINFO(printf("u32MVDFWSLQTABTmpbufAdr start=%lx\n", pBuff->u32MVDFWSLQTABTmpbufAdr));
    MVD_DEBUGINFO(printf("PtsTblAddr start=%lx\n", pBuff->u32MVDFWPtsTblAddr));
    MVD_DEBUGINFO(printf("u32DynScalingAdd start=%lx\n", pBuff->u32DynScalingAdd));
    MVD_DEBUGINFO(printf("ScalerInfo start=%lx end=%lx\n",
           pBuff->u32ScalerInfoAdd, (pBuff->u32ScalerInfoAdd+MVD_FW_SCALER_INFO_BUF_LEN)));
    MVD_DEBUGINFO(printf("DecFrmInfo start=%lx end=%lx\n",
           pBuff->u32DecFrmInfoAdd, (pBuff->u32DecFrmInfoAdd+MVD_FW_DECFRM_INFO_BUF_LEN)));

    if ((pBuff->u32VBBUTableAdd+MVD_FW_VBBU_TABLE_LEN-u32Start) > FW_SHM_SIZE)
    {
        //shared memory should not overlap with FW memory pool.
        MVD_DEBUGERROR(printf("%s err: 0x%lx out of memory boundary!\n", __FUNCTION__,
...
static MS_BOOL MVD_SetFWBuffAdd(MS_U8 u8Idx, MVD_FWBuff* pBuff)
{
    ...
    _miu_offset_to_phy(pMVDHalContext->stMiuCfg.u8FWMiuSel,pBuff->pu8MVDGetVolBufStart,pBuff->u32VolAdd);

    pBuff->u32VolAdd = HAL_MVD_PA2NonCacheSeg(pBuff->u32VolAdd);
    MVD_DEBUGINFO(printf("gvolInfo = 0x%lx, volBuf=0x%lx\n", pBuff->u32VolAdd, pBuff->pu8MVDGetVolBufStart));
    ...
    && (curSrcMode != E_MVD_TS_MODE))
    {
        ...
        MVD_DEBUGINFO(printf("PTS tbl start=%lx end=%lx\n",
               pBuff->u32MVDFWPtsTblAddr, (pBuff->u32MVDFWPtsTblAddr+MVD_FW_SLQTBL_PTS_BUF_LEN)));
    ...
    if (pstMemCfg->bEnableDynScale)
    {
        ...
        pCtrlCfg->u8DynScalingDepth = 16; //HAL_MVD_GetDynamicScaleDepth
        MVD_DEBUGINFO(printf("u8EnableMIUSel    = 0x%x\n", pMVDHalContext->stMiuCfg.u8FWMiuSel));
    ...
    MVD_DEBUGINFO(printf("DynScaling start=%lx end=%lx\n",
           pBuff->u32DynScalingAdd, (pBuff->u32DynScalingAdd+MVD_FW_DYN_SCALE_BUF_LEN)));
...
E_MVD_Result HAL_MVD_GetFrmInfo(MS_U8 u8Idx, MVD_FrmInfoType eType, MVD_FrmInfo* pInfo)
{
    ...
    {
#if v3_temp
        DISPQ_IN_DRAM* pTemp = (DISPQ_IN_DRAM*)(MS_PA2KSEG1(pstMemCfg->u32FWCodeAddr+DISP_QUEUE_START+ u8VPUIdx*0x100000));
#ifdef MSOS_TYPE_LINUX_KERNEL
        MS_U32 u32VAddr = (MS_U32)pTemp;
        if(MsOS_MPool_VAError(u32VAddr))
        {
            printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32VAddr,__LINE__);
            return E_MVD_RET_FAIL;
        ...
        pInfo->eFrmType = _MVD_ExStreamTypeToNumber(pTemp->disp_info[pTemp->dispQ_rd].u16NextDispeFrameType);
    ...
    else if(E_MVD_FRMINFO_DECODE_SIMPLE == eType)
    {
        unsigned int u32FrmReadPtr = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_FRAME_READPTR, sizeof(MS_U32));
        if(u32FrmReadPtr >= FRAMEQ_SIZE )
        {
            u32FrmReadPtr = 0;
        }
        MS_U32 u32BufStart = GET_FRMINFO_BUFFADD(u8Idx);
        pInfo->eFrmType = (MVD_PicType)MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_FRAME_TYPE+u32FrmReadPtr, sizeof(MS_U8));
        pInfo->u32TimeStamp = MVD_GetFWBuffData(u8Idx, FW_BUFF_FRMINFO, OFFSET_FRAME_TIMESTAMP+(u32FrmReadPtr*sizeof(MS_U32)), sizeof(MS_U32));
        u32FrmReadPtr = (u32FrmReadPtr + 1) % FRAMEQ_SIZE;
        HAL_MVD_MemWrite4Byte((u32BufStart+OFFSET_FRAME_READPTR),u32FrmReadPtr);
        return eRet;
    }
    else
    {
...
E_MVD_Result HAL_MVD_SetDynScalingParam(MS_U8 u8Idx, MS_PHYADDR u32StAddr, MS_U32 u32Size)
{
    #define SCALER_INFO_TIMEOUT 0x1000
    MS_U32 u32TimeOut = 0;
    MS_U32 u32SrcAdd = NULL;
    MS_U32 i;
    MS_U32 u32ScalerInfoAdd = GET_XCINFO_BUFFADD(u8Idx);
    MVD_MEMCfg* pstMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
...
#ifdef MSOS_TYPE_LINUX_KERNEL
    if(MsOS_MPool_VAError(u32SrcAdd))
    {
        printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32SrcAdd,__LINE__);
        return E_MVD_RET_FAIL;
    }
#endif
...
static MS_BOOL MVD_CheckFrmBuffSizeMin(MVD_FWCfg* fwCfg, MVD_MEMCfg* memCfg)
{
    ...
    if (_IS_VC1(fwCfg->eCodecType))
    {
        MVD_DEBUGERROR(printf("Framebuffer size(0x%lx) < (0x%x+0x%lx)!\n",
            memCfg->u32FBSize, (MVD_HD_FBSIZE*MVD_FBNUM_MIN), MVD_HW_BUF_TOTAL_LEN));
        ret = FALSE;
    }
    else if (memCfg->u32FBSize < (MVD4_MPEG_FBSIZE_SDMIN+MVD_HW_BUF_TOTAL_LEN))
    {
        MVD_DEBUGERROR(printf("Framebuffer size(0x%lx) < (0x%x+0x%lx)\n",
            memCfg->u32FBSize, MVD4_MPEG_FBSIZE_SDMIN, MVD_HW_BUF_TOTAL_LEN));
        ret = FALSE;
    }
    return ret;
}
    ...
    *pu8FBMode = MVD3_SD_MODE;
    ...
    if (MVD3_SD_MODE == *pu8FBMode)
    {
        ...
        *pu8FBNum = MVD_FBNUM_MIN;
        MVD_DEBUGINFO(printf("[MVD_SD_MODE] u8FBNum=%d, FBSize=0x%lx\n", *pu8FBNum, memCfg->u32FBSize));
        goto _GET_USED_SIZE;
    }
    ...
    else if(pMVDHalContext->bEnableThumbnailMode[u8Idx] && u32AvailFrmBuffSize >= (MVD_HD_FBSIZE*MVD_FBNUM_THUMBNAIL))
    {
        MVD_DEBUGINFO(printf("%s(%d) ThumbNail HD*1\n", __FUNCTION__, __LINE__));
        *pu8FBMode = MVD3_HD_MODE;
        *pu8FBNum = MVD_FBNUM_THUMBNAIL;
    }
    else
    {
        ...
        if (TRUE == ret)
        {
            *pu8FBMode = MVD3_SD_MODE;
            *pu8FBNum = MVD_FBNUM_MIN;
            MVD_DEBUGINFO(printf("Framebuffer [SD] mode\n"));
        }
    }
...
_GET_USED_SIZE:
    ...
    MVD_DEBUGINFO(printf("%s u8FBMode=0x%x, u8FBNum=%d, FBSize=0x%lx, used=0x%lx\n",
                  __FUNCTION__, fwCfg->u8FBMode, fwCfg->u8FBNum, memCfg->u32FBSize, fwCfg->u32FBUsedSize));
    return ret;
}
...
MS_BOOL HAL_MVD_SetCodecInfo(MS_U8 u8Idx, MVD_CodecType eCodecType, MVD_SrcMode eSrcMode, MS_U8 bDisablePESParsing)
{
    ...
    if (u32Len==0) printf("%s err: u32Len=0!\n", __FUNCTION__);
    MS_U32 i=0;
    i = u32Addr >> 3;
    SET_CMDARG(stCmdArg, i, u8Idx);
    SET_CMD_RET_FALSE(CMD_CODE_OFFSET, &stCmdArg);

    #define _mvdAssert(x) if(!x) printf("%s(%d) inconsistent cfg!\n", __FUNCTION__, __LINE__);
    ...
    // add the 6mb thumbnail command
    if ((eCodecType == E_MVD_CODEC_MPEG2) && pMVDHalContext->bEnableThumbnailMode[u8Idx])
    {
        MVD_CmdArg mvdcmd;
        SETUP_CMDARG(mvdcmd);
        mvdcmd.Arg0 = 1;
        SET_DECNUM(mvdcmd, u8Idx);
        if (HAL_MVD_MVDCommand(CMD_THUMBNAIL_LESS_FB_MODE, &stCmdArg) == FALSE)
        {
            MVD_DEBUGERROR( printf( "Command: 0x%x fail!!\r\n", CMD_THUMBNAIL_LESS_FB_MODE ) );
            return FALSE;
        }
    }
...
MS_BOOL HAL_MVD_Exit(MS_U8 u8Idx)
{
    ...
    pMVDHalContext->bEnableThumbnailMode[u8Idx] = FALSE;
    pMVDHalContext->bDropOnePTS[u8Idx] = FALSE;
    ...
    #define VDEC_STR_SUB_CMD_BUF u32StrAddrOffset+VDEC_STR_SUB_CMD_OFFSET
    if(u8Idx==0)
        pMVDHalContext->u32strcmdbuf[u8Idx] = VDEC_STR_MAIN_CMD_BUF;
    else if(u8Idx==1)
        pMVDHalContext->u32strcmdbuf[u8Idx] = VDEC_STR_SUB_CMD_BUF;

    pMVDHalContext->u8strcmdcnt[u8Idx] =0;
#endif
...
{
    MS_U32 pu8MVDSetHeaderBufStart = GET_HDR_BUFFADD(u8Idx);
...
///////////////////////////////////////////////////////////////////////////////
MS_U32 HAL_CC_CM_GetMVDRB_HWAddr(MS_U8 u8Idx, MS_U8 u8CC608)
{
#if 1
    MVD_CmdArg mvdcmd;
    MS_U32 u32CCWrPtr = 0;
    ...
    u32CCWrPtr = COMBU32(mvdcmd.Arg3, mvdcmd.Arg2, mvdcmd.Arg1, mvdcmd.Arg0);
    return u32CCWrPtr;
#else
...
MS_BOOL HAL_MVD_GetUsrDataIsAvailable(MS_U8 u8Idx)
{
    ...
    MS_U32 u32UsrDataWr = 0;
    MS_BOOL bIsAvail = FALSE;

#if defined(MVD_SUPPORT_CC)
    u32UsrDataWr = HAL_CC_CM_GetMVDRB_HWAddr(u8Idx,4);
#else
    u32UsrDataWr = HAL_CC_CM_GetMVDRB_HWAddr(u8Idx,2);
#endif
...
MS_BOOL HAL_MVD_GetUsrDataInfo(MS_U8 u8Idx, MVD_UsrDataInfo* pUsrInfo)
{
    ...
    {
        MVD_DEBUGERROR(printf("%s: no data? Rd=0x%lx Wr=0x%lx\n", __FUNCTION__, pCtrlCfg->u32UsrDataRd, u32UsrDataWr));
        return FALSE;
    }
...
#ifdef MSOS_TYPE_LINUX_KERNEL
    if(MsOS_MPool_VAError(u32UsrData))
    {
        printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32UsrData,__LINE__);
        return FALSE;
    }
#endif
...
MS_U32 HAL_MVD_Map2DrvSlqTbl(MS_U8 u8Idx, MS_U32 u32HWPtr)
{
    MVD_MEMCfg* pstMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
    MVD_CtrlCfg* pstCtrlCfg = HAL_MVD_GetCtrlCfg(u8Idx);
    MVD_SLQTBLInfo* pstSlqTblInfo = HAL_MVD_GetSlqTblInfo(u8Idx);
    MS_U32 u32HWSt = HAL_MVD_GetMemOffset(pstMemCfg->u32BSAddr);
    MS_U32 u32DrvPtr;
...
{
    MVD_MEMCfg* pstMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
    MS_U32 u32EndPattern = HAL_MVD_GetMemOffset(pstMemCfg->u32DrvBufAddr+SLQ_TBL_SIZE*3);
...
MS_BOOL HAL_MVD_SLQTblInsertPattern(MS_U8 u8Idx, MVD_PatternType ePattern)
{
    ...
    {   //insert dummy pattern
        ...
        #endif
        //printf("WrPtr 0x%lx ", pstSlqTblInfo->pDrvSlqTbl->u32WrPtr);
...
E_MVD_Result HAL_MVD_IsDispFinish(MS_U8 u8Idx)
{
    ...
    MVD_SLQTBLInfo* pstSlqTblInfo = HAL_MVD_GetSlqTblInfo(u8Idx);
    MS_U32 u32FileEndPtr = pstSlqTblInfo->u32FileEndPtr;
    MS_U32 u32Id = MVD_GetStreamId(u8Idx);
    MS_U8 u8VPUIdx = HAL_VPU_EX_GetTaskId(u32Id);
#ifdef VDEC3
    MS_U32 u32VBBUAddr = HAL_MVD_GetVBBUTableAddr(u8VPUIdx);
    MVD_CtrlCfg* pstCtrlCfg = HAL_MVD_GetCtrlCfg(u8Idx);
    #endif
    ...
    if ((E_MVD_SLQ_TBL_MODE == curSrcMode) && (TRUE != FLAG_LAST_FRM_SHOW))
    {
        ...
        if(pstCtrlCfg->u8McuMode == TRUE)
        {
            ...
            if (TRUE != FLAG_LAST_FRM_SHOW)
            {
                MVD_DEBUGERROR(printf("MDrv_MVD_IsDispFinish TIMEOUT\n"));
            ...
            (HAL_MVD_IsMStreamerMode(u8Idx) == FALSE))
        {
            ...
            if ((HAL_MVD_GetTime() - u32TimeCnt) >= CMD_TIMEOUT_MS)
            {
                ...
                MVD_DEBUGERROR(printf("***** fe=%lx, rd=%lx(%lx,%lx), wr=%lx, empty=%lx, u32FeByteCnt=%lx\n",
                    u32FileEndPtr, pstSlqTblInfo->pDrvSlqTbl->u32RdPtr, HAL_MVD_Map2DrvSlqTbl(u8Idx, HAL_MVD_GetSLQReadPtr(u8Idx)),
                    HAL_MVD_GetSLQReadPtr(u8Idx), pstSlqTblInfo->pDrvSlqTbl->u32WrPtr, pstSlqTblInfo->pDrvSlqTbl->u32Empty, u32FeByteCnt));
                return E_MVD_RET_TIME_OUT;
            }
            else
            {
        ...
        else
        {
            ...
            MVD_DEBUGINFO(printf("fe=%lx, rd=%lx(%lx), wr=%lx, empty=%lx\n", u32FileEndPtr, pstSlqTblInfo->pDrvSlqTbl->u32RdPtr,
...
MS_BOOL HAL_MVD_DecodeIFrame(MS_U8 u8Idx, MS_PHYADDR u32FrameBufAddr, MS_PHYADDR u32StreamBufAddr, MS_PHYADDR u32StreamBufEndAddr )
{
    MS_U32 u32deley = 0;
    MS_U32 u32time = 0;
    MVD_CmdArg mvdcmd;
    MS_U32 u32MVDFWSLQTABTmpbufAdr = GET_SLQ_BUFFADD(u8Idx);
    ...
    {
        MVD_DEBUGERROR( printf( "HAL_MVD_StepDecode fail!!\r\n") );
        return FALSE;
    }
...
#define WAIT_DECODE_DONE_TIME 33 //To decode 1 frame should take less than 33ms
    ...
    {
        MVD_DEBUGERROR(printf ("MDrv_MVD_DecodeIFrame time out(du=%ld, st=%ld, now=%ld)\n", u32time, u32deley, HAL_MVD_GetTime()));
...
/// @param -u32end \b IN : end address
//------------------------------------------------------------------------------
static void MVD_SetBitStreamAddr(MS_U8 u8Idx, MS_U32 u32start, MS_U32 u32end)
{
    MVD_CmdArg mvdcmd;
...
MS_BOOL HAL_MVD_SetInternalBuffAddr(MS_U8 u8Idx, MS_U32 u32start, MS_U32 u32len)
{
    ...
    MS_U32 u32Id = MVD_GetStreamId(u8Idx);
    MS_U8 u8VPUIdx = HAL_VPU_EX_GetTaskId(u32Id);
    MS_U32 u32BuffStart = u32start+(MVD_FW_TASK_OFFSET*u8VPUIdx);
    MS_U32 VPUSHMAddr = HAL_VPU_EX_GetSHMAddr();
    MS_U32 u32StartOffset;
    MS_U8  u8MiuSel;
    ...
    MVD_DEBUGINFO(printf("MVD FW shared mem start = 0x%lx\n", u32BuffStart));
...
static void _SLQTbl_DumpPtsTbl(MS_U8 u8Idx, MS_U32 u32EntryStart, MS_U32 u32EntryEnd)
{
    ...
    MS_U32 u32MVDFWPtsTblAddr = GET_PTSTBL_BUFFADD(u8Idx);
...
{
    MVD_SLQTBLInfo* pstSlqTblInfo = HAL_MVD_GetSlqTblInfo(u8Idx);
    MS_U32 u32DivXPattern = pstSlqTblInfo->pDrvDivxTbl->u32WrPtr;
...
{
    MVD_MEMCfg* pstMemCfg = HAL_MVD_GetMEMCfg(u8Idx);
    //MS_U8* temp = (MS_U8*)MS_PA2KSEG1(pstMemCfg->u32DrvBufAddr+SLQ_TBL_SIZE*3);;
    MS_U32 u32EndPattern = HAL_MVD_GetMemOffset(pstMemCfg->u32DrvBufAddr+SLQ_TBL_SIZE*3);
    MS_U32 temp = 0;
    MVD_CtrlCfg* pstCtrlCfg = HAL_MVD_GetCtrlCfg(u8Idx);

#if 0
    memset(temp, 0xff, END_PATTERN_SIZE);
#else
...
static void MVD_SLQTblInitDrvSlqTbl(MVD_SLQ_TBL_ST* pDrvSlqTbl, MS_U32 u32Addr)
{
    MS_U32 temp = 0;
...
static void MVD_SLQTblInitSlqStatus(MVD_SLQ_STATUS* pSlqStatus, MS_U32 u32WrPtr)
{
#if SLQ_NEW_PUSH
...
static void MVD_SLQTblInitDrvDivxTbl(MVD_SLQ_ES_ST* pDrvDivxTbl, MS_U32 u32StAdd, MS_U32 u32Val)
{
    MS_U32 temp = 0;
...
static void MVD_SLQTblInitDrvEsTbl(MVD_SLQ_ES_ST* pDrvEsTbl, MS_U32 u32StAdd, MS_U32 u32Val)
{
    MS_U32 temp = 0;
    UNUSED(u32Val);
    ///// init ES table
...
MS_BOOL HAL_MVD_SLQTblInit(MS_U8 u8Idx)
{
    ...
    {
        MVD_DEBUGERROR(printf("%s: unknow codec type!\n", __FUNCTION__));
        return FALSE;
    }
...
MS_U32 HAL_MVD_GetQueueVacancy(MS_U8 u8Idx, MS_BOOL bCached)
{
    ...
    //printf("QV=0x%lx rd=0x%lx==>", u32Empty, pstSlqTblInfo->pDrvSlqTbl->u32RdPtr);
    if (pstSlqTblInfo->pDrvSlqTbl->u32RdPtr >= (pstSlqTblInfo->pDrvSlqTbl->u32StAdd+SLQ_ENTRY_LEN))
    {
        if (pstSlqTblInfo->pDrvSlqTbl->u32RdPtr >= pstSlqTblInfo->pDrvSlqTbl->u32EndAdd)
        {
            MVD_DEBUGERROR(printf("%s: readPtr 0x%lx out of range: too large!\n",
                           __FUNCTION__, pstSlqTblInfo->pDrvSlqTbl->u32RdPtr));
        }
        if(pstSlqTblInfo->pDrvSlqTbl->u32RdPtr >= SLQ_ENTRY_LEN)//coverity
...
#endif
    {
        MS_U8* temp = (MS_U8*)(MS_PA2KSEG1((MS_U32) pInfo->u32StAddr+pstMemCfg->u32BSAddr));
        MS_U32 i;
        MS_U32 u32DummyPattern[3];
        MS_U32 u32PatternSize;
...
#define MAX_VALIDLEN    0x200000    //2M
    ...
    else
    {
        ...
        MVD_DEBUGINFO(printf("(%x) ValidLen=0x%lx CurLen=0x%lx\n",
...
static E_MVD_Result MVD_FlushSlqTblQueue(MS_U8 u8Idx)
{
           ...
           (TRUE != MVD_PatternLenIsValid(u8Idx, u32PatternByteCnt)))
    {
        ...
        {
            MVD_DEBUGERROR(printf("RCV timeout!!! pl=%ld, pbc=%ld, emp=%ld\n",
...
MS_BOOL HAL_MVD_EnableInt(MS_U8 u8Idx, MS_U32 bEn)
{
    ...
    u32IntFlag = (((bEn & E_MVD_EVENT_USER_DATA) == E_MVD_EVENT_USER_DATA) ? INT_USER_DATA : 0) |
                 ...
                 (((bEn & E_MVD_EVENT_PTS_DISCONTINUE) == E_MVD_EVENT_PTS_DISCONTINUE) ? INT_PTS_DISCONTINUE : 0);
    ...
    MVD_DEBUGINFO(printf("u32IntFlag = 0x%lx\n", u32IntFlag));
    SETUP_CMDARG(mvdcmd);
...
E_MVD_Result HAL_MVD_EnablePTSDetector(MS_U8 u8Idx, MS_BOOL bEn)
{
    ...
    SET_DECNUM(stCmdArg, u8Idx);
    if (HAL_MVD_MVDCommand(CMD_PTS_DETECTOR_EN, &stCmdArg) == FALSE)
    {
        MVD_DEBUGERROR( printf( "Command: 0x%x fail!!\r\n", CMD_PTS_DETECTOR_EN ) );
        return E_MVD_RET_FAIL;
    }
...
E_MVD_Result HAL_MVD_DbgGetData(MS_U32 u32Addr, MS_U32* u32Data)
{
    MVD_CmdArg mvdcmd;
    MS_U32 u32Val;
...
void HAL_MVD_DbgDumpBits(MS_U8 u8Idx, MS_PHYADDR u32base, MS_U32 u32size)
{
    MVD_CmdArg mvdcmd;
...
//------------------------------------------------------------------------------
MS_U32 MVD_GetSlqTblESReadPtr(MS_U8 u8Idx)
{
    MS_U32 u32Idx;
...
/// @return -the read pointer
//------------------------------------------------------------------------------
MS_U32 MVD_GetSlqTblESWritePtr(MS_U8 u8Idx)
{
    MS_U32 u32EsWp;
    MS_U32 u32EsEnd;
    MS_U32 u32Idx;
...
MS_U32 HAL_MVD_GetSLQReadPtr(MS_U8 u8Idx)
{
    MS_U32 u32RdPtr = 0;
    ...
    //printf("##### HAL_MVD_GetSLQReadPtr 0x%lx\n", readPtr);
...
MS_BOOL HAL_MVD_SetSLQWritePtr(MS_U8 u8Idx, MS_BOOL bCheckData)
{
    ...
    if (FALSE == pstSlqTblInfo->bRdyToFireCmd)
    {
        ...
        while ((HAL_MVD_GetTime() - u32TimeCnt) < CMD_TIMEOUT_MS)
        {
            if (HAL_MVD_GetDecodeStatus(u8Idx) != E_MVD_STAT_IDLE)
            {
                ...
                MVD_DEBUGVERBAL(printf("time=%ld ms, ", (HAL_MVD_GetTime() - u32TimeCnt)));
                break;
            }
        }
        if (FALSE == pstSlqTblInfo->bRdyToFireCmd)
        ...
        {
            MVD_DEBUGVERBAL(printf("%s: ready to update WrPtr.\n", __FUNCTION__));
        }
    }
    ...
    {
        MVD_DEBUGINFO(printf("**** pstSlqTblInfo->pSlqStatus->bSlqFireRdy is not Ready ** \n"));
        return FALSE;
    }
    pstSlqTblInfo->pSlqStatus->bSlqFireRdy = FALSE;
#endif
    if ((bCheckData==TRUE) && (FALSE==pstSlqTblInfo->bSlqTblHasValidData))
    {
        MVD_DEBUGINFO(printf("**** SlqWrPtr(0x%lx) is not update!(%x, %x) ****\n",
            pstSlqTblInfo->pDrvSlqTbl->u32WrPtr, bCheckData, pstSlqTblInfo->bSlqTblHasValidData));
        return FALSE;
    }
    pstSlqTblInfo->bSlqTblHasValidData = FALSE;
    ...
    if (pstCtrlCfg->bSlqTblSync)
    {
        ...
        MS_U32 u32SrcOffset = pstSlqTblInfo->pDrvSlqTbl->u32StAdd;
        MS_U32 u32SrcAdd = u32SrcOffset;
        MS_U32 u32DstAdd = pstMemCfg->u32BSAddr;
        MS_U32 u32DstOffset = HAL_MVD_GetMemOffset(pstMemCfg->u32BSAddr);
        MS_U32 u32TblWr;
        ...
        MVD_DEBUGINFO(printf("SlqWrPtr_BDMA src=0x%lx dst=0x%lx size=0x%lx cpyType=0x%x\n",
            u32SrcAdd, u32DstAdd, pstMemCfg->u32DrvBufSize, pstSlqTblInfo->bdmaCpyType));
...
MS_BOOL HAL_MVD_GenPattern(MS_U8 u8Idx, MVD_PatternType ePattern, MS_PHYADDR u32PAddr, MS_U32* pu32Size)
{
...
void HAL_MVD_DbgDump(MS_U8 u8Idx)
{
    ...
    {
        #ifndef VDEC3
        printf("fe=%lx, rd=%lx(%lx), wr=%lx, empty=%lx; ", pstSlqTblInfo->u32FileEndPtr, pstSlqTblInfo->pDrvSlqTbl->u32RdPtr,
                HAL_MVD_Map2DrvSlqTbl(u8Idx, HAL_MVD_GetSLQReadPtr(u8Idx)), pstSlqTblInfo->pDrvSlqTbl->u32WrPtr, pstSlqTblInfo->pDrvSlqTbl->u32Empty);
...
void MVD_DumpMVDStream(void)
{
    ...
    for (i=0; i< MAX_DEC_NUM; i++)
    {
        MVD_DEBUGINFO(printf("[%d] eStreamId=0x%x(%s) bUsed=0x%x\n", i, pMVDHalContext->_stMVDStream[i].eStreamId,
...
MS_BOOL HAL_MVD_Init_Share_Mem(void)
{
                                          ...
                                          MSOS_SHM_QUERY))
    {
        if (FALSE == MsOS_SHM_GetId((MS_U8*)"Linux MVD HAL",
                                             ...
                                             MSOS_SHM_CREATE))
        {
            printf("[%s]SHM allocation failed!!! use global structure instead!!!\n",__FUNCTION__);
            if(pMVDHalContext == NULL)
            {
...
HAL_MVD_StreamId HAL_MVD_GetFreeStream(HAL_MVD_StreamType eStreamType)
{
    ...
    else if ((eStreamType >= E_HAL_MVD_N_STREAM) && (eStreamType < E_HAL_MVD_N_STREAM + MAX_DEC_NUM))
    {
        ...
        {
            // reset the thumbNail mode to avoid the exit fail
            pMVDHalContext->bEnableThumbnailMode[i] = FALSE;
            return pMVDHalContext->_stMVDStream[i].eStreamId;
        }
    }
#endif
...
MS_BOOL HAL_MVD_ReleaseFdMask(MS_U8 u8Idx, MS_BOOL bRls)
{
    ...
    MVD_DEBUGINFO(printf("%s: release=0x%x\n", __FUNCTION__, bRls));
    SET_DECNUM(mvdcmd, u8Idx);
...
MS_BOOL HAL_MVD_ParserRstDone(MS_U8 u8Idx, MS_BOOL bEnable)
{
    ...
    u32TimeCnt = HAL_MVD_GetTime();
    while ((HAL_MVD_GetTime() - u32TimeCnt) < CMD_TIMEOUT_MS*2)
    {
        bRet = HAL_MVD_IsCmdFinished(u8Idx, MVD_HANDSHAKE_PARSER_RST);
        if (bRet == TRUE)
        {
...
{
    MVD_SLQTBLInfo* pstSlqTblInfo = HAL_MVD_GetSlqTblInfo(u8Idx);
    MS_U32 u32RdPtr = HAL_MVD_GetSLQReadPtr(u8Idx);
    MS_U32 u32Diff = 0;
...
MS_BOOL _HAL_MVD_CrcTimeOut(MS_U8 u8Idx)
{
    ...
    MVD_DEBUGERROR( printf("_HAL_MVD_CrcTimeOut=%lx\n", i) );
    return TRUE;
}
...
E_MVD_Result HAL_MVD_GetCrcValue(MS_U8 u8Idx, MVD_CrcIn *pCrcIn, MVD_CrcOut *pCrcOut)
{
    ...
    MVD_DEBUGINFO(printf("%s width=0x%lx, height=0x%lx, pitch=0x%lx, luma=0x%lx, chroma=0x%lx\n",
                                    __FUNCTION__, pCrcIn->u32HSize, pCrcIn->u32VSize, pCrcIn->u32Strip, pCrcIn->u32YStartAddr, pCrcIn->u32UVStartAddr));
...
MS_BOOL HAL_MVD_SetExternalDSBuff(MS_U8 u8Idx, MS_U32 u32VPUAddr, MS_U32 u32DrvAddr)
{
    MS_BOOL bRet = TRUE;
    MVD_FWBuff* pBuff = &(pMVDHalContext->stFWBuff[u8Idx]);
...
MS_BOOL HAL_MVD_Field_Polarity_Display_One_field(MS_U8 u8Idx, MS_BOOL bEn,MS_U8 top_bottom)
{
    ...
    MVD_DEBUGINFO(printf("%s: bEn=%x,top_bottom=%x\n", __FUNCTION__, bEn,top_bottom));
    SET_DECNUM(mvdcmd, u8Idx);
...
MS_BOOL HAL_MVD_SetShareMemoryBase(MS_U8 u8Idx, MS_U32 u32base, MS_U8 u8sel)
    {
    u8SHMMiuSel = u8sel;
...
MS_BOOL HAL_MVD_GetShareMemoryOffset(MS_U8 u8Idx, MS_U32 *u32base)
{
...
MS_BOOL HAL_MVD_SetExternal_CC_Buffer(MS_U8 u8Idx, MS_U32 u32base, MS_U8 u8size, MS_U8 cc_type)
{
    MVD_CmdArg mvdcmd;
    MS_U32 u32cc_size=0;
    MS_U32 u32StartOffset;
...
#ifdef MSOS_TYPE_LINUX_KERNEL
    MS_U32 u32VAddr0 = (MS_U32)HAL_MVD_PA2NonCacheSeg(u32base);
    MS_U32 u32VAddr1 = (MS_U32)HAL_MVD_PA2NonCacheSeg(u32base+4);
    MS_U32 u32VAddr2 = (MS_U32)HAL_MVD_PA2NonCacheSeg(u32base+8);
    MS_U32 u32VAddr3 = (MS_U32)HAL_MVD_PA2NonCacheSeg(u32base+12);
    if(MsOS_MPool_VAError(u32VAddr0)||MsOS_MPool_VAError(u32VAddr1)||MsOS_MPool_VAError(u32VAddr2)||MsOS_MPool_VAError(u32VAddr3))
    {
        printk("%s  Error VA [%lx] Mapping %d\n",__FUNCTION__,u32VAddr0,__LINE__);
        return FALSE;
    }
#endif
    MS_U32 *u32ReadPtr = (MS_U32 *)(HAL_MVD_PA2NonCacheSeg(u32base));
    MS_U32 *u32WritePtr = (MS_U32 *)(HAL_MVD_PA2NonCacheSeg(u32base+4));
    MS_U32 *u32LatchOverflow = (MS_U32 *)(HAL_MVD_PA2NonCacheSeg(u32base+8));
    MS_U32 *u32CpuBaseAddr = (MS_U32 *)(HAL_MVD_PA2NonCacheSeg(u32base+12));
...
MS_U32 HAL_MVD_GetVsyncAddrOffset(void)
{
    MS_U32 VPUSHMAddr = HAL_VPU_EX_GetSHMAddr();
    MS_U32 VsyncBridgeOffset = 0;
...
MS_U32 HAL_MVD_GetVsyncExtAddrOffset(void)
{
    MS_U32 VsyncBridgeExtOffset = 0;
    MS_U32 VPUSHMAddr = HAL_VPU_EX_GetSHMAddr();
...
E_MVD_Result HAL_MVD_SetTimeIncPredictParam(MS_U8 u8Idx, MS_U32 u32time)
{
    ...
    MVD_DEBUGINFO(printf("CMD_TIME_INCR_PREDICT=%lx\n",u32time));
...
E_MVD_Result HAL_MVD_Set_Smooth_Rewind(MS_U8 u8Idx, MS_U8 btype)
{
    ...
    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = btype;
    MVD_DEBUGINFO(printf("%s: btype=%x\n", __FUNCTION__, btype));
    SET_DECNUM(mvdcmd, u8Idx);
...
void HAL_MVD_EnableThumbNailMode(MS_U8 u8Idx, MS_BOOL bEnable)
{
    pMVDHalContext->bEnableThumbnailMode[u8Idx] = bEnable;
}

void HAL_MVD_DisableDivx(MS_U8 u8Idx, MS_BOOL bDisable)
{
    MVD_CmdArg mvdcmd;

    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = bDisable;

    SET_DECNUM(mvdcmd, u8Idx);

    MVD_DEBUGINFO(printf("%s CMD_DIVX_DISABLE arg0=%x\n", __FUNCTION__, bDisable));
    if (HAL_MVD_MVDCommand(CMD_DIVX_DISABLE, &mvdcmd)== FALSE)
    {
        MVD_DEBUGERROR( printf( "Command: 0x%x fail!!\r\n", CMD_DIVX_DISABLE) );
    }
}
...
E_MVD_Result HAL_MVD_PTS_Discontinue_Auto_Detect(MS_U8 u8Idx, MS_BOOL bEnable)
{
    MVD_CmdArg mvdcmd;
    ...
    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = bEnable;
    MVD_DEBUGINFO(printf("%s: arg=%d\n", __FUNCTION__, (unsigned int)bEnable));
    SET_DECNUM(mvdcmd, u8Idx);

    if (HAL_MVD_MVDCommand(CMD_PTS_DETECTOR_EN, &mvdcmd) == FALSE)
    {
        MVD_DEBUGERROR( printf( "Command: 0x%x fail!!\r\n", CMD_PTS_DETECTOR_EN ) );
        return E_MVD_RET_FAIL;
    }
...
E_MVD_Result HAL_MVD_PVR_Seamless_mode(MS_U8 u8Idx, MS_U8 u8Arg)
{
    ...
    if(u8Arg != 2)
    {
        ...
        _miu_offset_to_phy(pMVDHalContext->stMiuCfg.u8FWMiuSel,GET_FRMINFO_BUFFADD(u8Idx),u32BufStart);
    ...
    if(u8Arg == 1)
    {
        ...
        while(1)
        {
            ...
            if((HAL_MVD_GetTime()-u32StartTime)>STOP_TIMEOUT)
            {
                MVD_DEBUGERROR(printf("PVR seamless pause timeout\n" ) );
                ret = E_MVD_RET_FAIL;
                break;
            }
        }
    ...
    else if(u8Arg == 2)
    {
        while(1)
        {
            ...
            if((HAL_MVD_GetTime()-u32StartTime)>STOP_TIMEOUT)
            {
                MVD_DEBUGERROR(printf("PVR seamless hw reset timeout\n" ) );
                ret = E_MVD_RET_FAIL;
                break;
            }
        }
...
MS_U32 HAL_MVD_GetVBBUTableAddr(MS_U8 u8Idx)
{
...
MS_BOOL HAL_MVD_SetAVSyncDispAutoDrop(MS_U8 u8Idx, MS_BOOL bEnable)
{
    MVD_CmdArg mvdcmd;
    ...
    MVD_DEBUGINFO(printf("%s CMD_AUTO_DROP_FRAME_IN_DECODE arg0=%x\n", __FUNCTION__, bEnable));
    if (HAL_MVD_MVDCommand(CMD_AUTO_DROP_FRAME_IN_DECODE, &mvdcmd)== FALSE)
    {
...
MS_BOOL HAL_MVD_SetDynmcDispPath(MS_U8 u8Idx,MS_BOOL bConnect,MVD_DISPLAY_PATH eValue,MS_BOOL bPreSet)
{
    MS_U32 u32TimeCnt = 0;
    MVD_CmdArg mvdcmd;

    HAL_MVD_ResetHandShake(u8Idx, MVD_HANDSHAKE_VSYNC_CONTROL);

    SETUP_CMDARG(mvdcmd);
    mvdcmd.Arg0 = bConnect;
    mvdcmd.Arg1 = eValue;
    mvdcmd.Arg2 = bPreSet;

    SET_DECNUM(mvdcmd, u8Idx);

    MVD_DEBUGINFO(printf("%s CMD_DYNAMIC_MVOP_CONNECT arg0=%x, arg1=%x\n", __FUNCTION__, bConnect,eValue));
    if (HAL_MVD_MVDCommand(CMD_DYNAMIC_MVOP_CONNECT, &mvdcmd)== FALSE)
    {
        MVD_DEBUGERROR( printf( "Command: 0x%x fail!!\r\n", CMD_DYNAMIC_MVOP_CONNECT) );
        return FALSE;
    }

    if(bConnect == FALSE) // disconnect, need to do handshake with fw
    {
        u32TimeCnt = HAL_MVD_GetTime();
        while ((HAL_MVD_GetTime() - u32TimeCnt) < CMD_TIMEOUT_MS)
        {
            if (HAL_MVD_IsCmdFinished(u8Idx, MVD_HANDSHAKE_VSYNC_CONTROL))
            {
        ...
        if (TRUE != HAL_MVD_IsCmdFinished(u8Idx, MVD_HANDSHAKE_VSYNC_CONTROL))
        {
...
MS_BOOL HAL_MVD_SetPUSISetting(MS_U8 u8Idx,MS_BOOL bEnable)
{
    MVD_CmdArg mvdcmd;
    ...
    MVD_DEBUGINFO(printf("%s CMD_PUSI_CONTROL arg0=%x\n", __FUNCTION__, bEnable));
    if (HAL_MVD_MVDCommand(CMD_PUSI_CONTROL, &mvdcmd)== FALSE)
    {
        MVD_DEBUGERROR( printf( "Command: 0x%x fail!!\r\n", CMD_PUSI_CONTROL) );
        return FALSE;
    }
...
void HAL_MVD_GetCMAInformation(MS_U8 u8HalIdx,MS_U32* u32Addr,MS_U32* u32Size,MS_BOOL* bAllocDone)
{
    FW_VOL_INFO gvolInfo;
    MS_U32 u32VolAddr;
    ...
    *u32Addr = gvolInfo.CMA_FB_Address;
    *u32Size = gvolInfo.CMA_FB_Size;
...
MS_BOOL HAL_MVD_IsSTRResume(MS_U8 u8TaskId)
{
    return HAL_VPU_EX_IsSTRResume(u8TaskId);
}

MS_BOOL HAL_MVD_IsSTR(MS_U8 u8TaskId)
{
    return HAL_VPU_EX_IsSTR(u8TaskId);
}
