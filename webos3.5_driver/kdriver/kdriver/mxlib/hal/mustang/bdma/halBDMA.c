...
{
    MS_PHY phy64Reg = _HAL_BDMA_Get_RegAddr(BDMA_REG_CH0_CMD2_L, u8Ch);
    return HAL_BDMA_Write4Byte(phy64Reg, u32Cmd);
}
...
{
    MS_PHY phy64Reg = _HAL_BDMA_Get_RegAddr(BDMA_REG_CH0_MISC, u8Ch);
    return HAL_BDMA_WriteRegBit(phy64Reg, BDMA_CH_MOBF_EN, bEnable);
}
...
void HAL_BDMA_SetSPIOffsetForMCU()
{
     ...
     HAL_BDMA_WriteRegBit(0x000e41UL, __BIT6, DISABLE);  //spi_clk=xtal

     //HAL_BDMA_WriteRegBit(0x000e40, DISABLE, __BIT7);  //mcu51 clk=xtal
     HAL_BDMA_WriteRegBit(0x000e40UL, __BIT7, DISABLE);  //mcu51 clk=xtal

     //HAL_BDMA_WriteRegBit(0x001018, DISABLE, __BIT3);  // i_cache rstz
     HAL_BDMA_WriteRegBit(0x001018UL, __BIT3, DISABLE);  // i_cache rstz

     //HAL_BDMA_WriteRegBit(0x002ba0, ENABLE,  __BIT0);  //disable i cache
     HAL_BDMA_WriteRegBit(0x002ba0UL, __BIT0,  ENABLE);  //disable i cache

     HAL_BDMA_Write2Byte(0x0e24UL,0x0000);
     HAL_BDMA_Write2Byte(0x0e26UL,0x0000);

     HAL_BDMA_Write2Byte(0x2e52UL,0x403F);   //PM 51 rst

     //HAL_BDMA_Write2Byte(0x2e3eUL,0x44);    // Chip_overwrite to mips boot
    ...
    HAL_BDMA_Write2Byte(0x2e54UL,0x829f);


    //reg_fire[3:0]                   0x000e53 = 0x01

    //reg_fire[3:0]                   0x000e53 = 0x00
    HAL_BDMA_WriteByte(0x2e53UL,0x41);
    for(u32temCnt=0; u32temCnt<0xffff; u32temCnt++);
    HAL_BDMA_WriteByte(0x2e53UL,0x40);

    for(u32temCnt=0; u32temCnt<0xffff; u32temCnt++);
    HAL_BDMA_WriteByte(0x2e53UL,0x50);

    printf("Wait for PM51 standby...........\n");
    while(HAL_BDMA_Read2Byte(0x0e24UL)!=0x02);
    printf("PM51 run ok...........\n");
}
