...
void VBI_TTXInit(MS_VIRT ptrAddr)
{
    ...
    MS_U8 u8value = 0;
    u8value = HAL_VBI_ReadByte(BK_VBI_89);
    HAL_VBI_WriteByte(BK_VBI_89, (u8value & 0x20)|0xC2); //skip [5]
...
MS_BOOL VBI_TTX_IsSlicerEnabled(void)
{
    return ((HAL_VBI_ReadByte(TT_ENABLE) & _BIT0) != 0);
}
...
MS_BOOL VBI_GetCompleteVPS_Data(MS_U8* pData, MS_U32* dataLen)
{
    if(HAL_VBI_ReadByte(BK_VBI_AF)&BIT(4))
    {
        //memset(_u8VPSData, 0, VPS_PACKET_LEN);
        pData[0x0D - 1] = HAL_VBI_ReadByte(BK_VBI_AD);
        pData[0x0E - 1] = HAL_VBI_ReadByte(BK_VBI_AE);
        pData[0x05 - 1] = HAL_VBI_ReadByte(BK_VBI_A6);
        pData[0x0B - 1] = HAL_VBI_ReadByte(BK_VBI_A7);
...
MS_BOOL VBI_ProtectMemory(MS_BOOL bEnable, MS_PHY phyAddr, MS_U32 u32Size)
{
    ...
    MS_U32 u32MiuAddrMax = ( ( (MS_U32)phyAddr + u32Size ) >> 3 ) + 1;
    if(bEnable)
    {
