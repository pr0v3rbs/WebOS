...
void HAL_TSP_PVR_Enable(MS_U8 u8PVRId, MS_BOOL bEnable)
{
    ...
    if (bEnable)
    {
        ...
        //_HAL_REG32_W(&_TspCtrl[0].HW2_Config3, SET_FLAG1(_HAL_REG32_R(&_TspCtrl[0].HW2_Config3), TSP_DMA_FLUSH_EN));
...
void HAL_TSP_HwPatch(void)
{
    ...
    //enable ECO bit for or write fix for new abt disable
    ///_HAL_REG16_W(&_TspCtrl5[0].Miu_MCM_Cfg,_HAL_REG16_R(&_TspCtrl5[0].Miu_MCM_Cfg) | TS_OR_WRITE_FIX);
