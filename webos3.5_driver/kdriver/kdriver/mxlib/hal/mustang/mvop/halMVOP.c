...
void _HAL_MVOP_InitVarCtx(void)
{
    ...
    g_pHalMVOPCtx->bIsEnable = FALSE;
    g_pHalMVOPCtx->bSubIsEnable = FALSE;
...
void HAL_MVOP_SetMIUReqMask(MS_BOOL bEnable)
{
	...
	{
		// mask msb mvop
		u8Miu = (HAL_ReadByte(VOP_MIU_SEL) & 0x30) >> 4;
	}
	else
	{
        u8Miu = VOP_ON_MIU1;
...
void HAL_MVOP_SetInputMode( VOPINPUTMODE mode, MVOP_InputCfg *pparam, MS_U16 u16ECOVersion )
{
    ...
    else if (mode == VOPINPUT_MCUCTRL)
    {
        ...
        if ( pparam->bDramRdContd == 0 )
        {
            ...
            MVOP_DBG(MVOP_PRINTF("[MVOP]%s enVideoType = 0x%x\n", __FUNCTION__,pparam->enVideoType);)

            if(pparam->enVideoType == MVOP_H264)
            {
                HAL_WriteRegBit(VOP_INPUT_SWITCH0, 1, VOP_TILE_32x32);//mantis:1144345 mustang h264 default 32x32.
            }
            else
            {
                HAL_WriteRegBit(VOP_INPUT_SWITCH0, 0, VOP_TILE_32x32);//MVD can not 32x32.
            }
        }
        else
        {
...
void HAL_MVOP_SetOutputTiming( MVOP_Timing *ptiming )
{
    ...
    HAL_WriteByte((VOP_BF_VS   + 1), HIGHBYTE( ptiming->u16BottomField_VS ));
     ...
     //patch for new vsync mode, t/b field opposite
    if((ptiming->bInterlace == 1) && (g_pHalMVOPCtx->bNewVSyncMode == TRUE))
    {
        HAL_MVOP_SetFieldInverse(DISABLE, ENABLE);
        HAL_WriteByte(VOP_DMA0, 0x18);
        HAL_WriteByteMask(VOP_DMA1, 0x01, VOP_BURST_EXT);
    }
    else
    {
        HAL_WriteByte(VOP_DMA0, 0x08);
        HAL_WriteByteMask(VOP_DMA1, 0x02, VOP_BURST_EXT);
    }

    if(((ptiming->u16H_TotalCount > 1920) && (ptiming->u16V_TotalCount > 1080))
       && (ptiming->u8Framerate >= 59)
       && (true == ptiming->bInterlace))
    {
       g_pHalMVOPCtx->bNewVSyncMode = true;
       HAL_MVOP_SetFieldInverse(DISABLE, ENABLE);
    }
...
void HAL_MVOP_SetH264HardwireMode(void)
{
    ...
    // H264 use WISHBONE(R2) interface
    //HAL_WriteRegBit(VOP_INPUT_SWITCH0, 1, VOP_R2_WISHBONE);
...
void HAL_MVOP_SetEVDHardwireMode(void)
{
    ...
    HAL_WriteByteMask(VOP_REG_MASK, 0, VOP_LSB_REQ_MASK);//10 bits bw control by vdec fw
        ...
        case E_MVOP_DEV_0:
            ...
            switch(stEVDInput->eEVDBit[Y_INFO])
            {
                ...
                case E_MVOP_EVD_10BIT:
                    // muji default not support 10 bits
                    #if 1
...
/* Y and UV / msb and lsb buffer can not be seperated in Mustang, but support hw select*/
void HAL_MVOP_SelMIU(MVOP_DevID eDevID, HALMVOPMIUSEL eMiuMSB0, HALMVOPMIUSEL eMiuMSB1, HALMVOPMIUSEL eMiuLSB0, HALMVOPMIUSEL eMiuLSB1)
{
    ...
    u8MSBVlue |= (eMiuMSB0 << 6); // not support y/c separate.

    u8LSBVlue = u8MSBVlue; //not support msb/lsb separate.
    ...
    switch(eDevID)
    {
        case E_MVOP_DEV_0:
        {
            ...
            // LSB
            HAL_WriteByteMask(VOP_MIU_SEL_LSB, u8MSBVlue, VOP_LSB_BUF0_MIU_SEL | VOP_LSB_BUF1_MIU_SEL);
            break;
        }
#if MVOP_SUPPORT_SUB
        case E_MVOP_DEV_1:
        {
            // MSB
            HAL_WriteByteMask(SUB_REG(VOP_MIU_SEL), u8LSBVlue, VOP_MSB_BUF0_MIU_SEL | VOP_MSB_BUF1_MIU_SEL);
            // LSB
...
void HAL_MVOP_GetMaxFramerate(MVOP_DevID eDevID, MVOP_GetMaxFps* stStreamInfo)
{
    ...
    switch(eDevID)
    {
...
#endif
        {
            ...
            do_div(u64MaxClk, ((MS_U32)u16HsizeTiming*(MS_U32)u16VsizeTiming));
            stStreamInfo->u32Framerate = (MS_U32)(u64MaxClk)*1000;
        }
            break;
        default:
            break;
...
void HAL_MVOP_ResetReg(MVOP_DevID eDevID)
{
    ...
    HAL_WriteRegBit(VOP_INPUT_SWITCH0, 0, VOP_TILE_32x32);
    HAL_MVOP_LoadReg();
    /*****************************************************/
        break;
    }
    ...
    case E_MVOP_DEV_1:
    {
        ...
        //the DUMMY is double buffer, it shall call HAL_MVOP_SubLoadReg() right away
        HAL_WriteRegBit(SUB_REG(VOP_REG_DUMMY), 1, VOP_32x32_WB); //32x32 from wb
        HAL_WriteRegBit(SUB_REG(VOP_INPUT_SWITCH0), 0, VOP_TILE_32x32);
        HAL_MVOP_SubLoadReg();
        /*****************************************************/
        break;
    }
...
#if MVOP_SUPPORT_SUB
    case E_MVOP_DEV_1:
        if((g_pHalMVOPCtx->eSubInterlaceType == E_MVOP_INT_TB_SEP_FRAME) && (g_pHalMVOPCtx->bIsH265))
        {
            HAL_WriteRegBit(SUB_REG(VOP_INT_TYPE), 1, VOP_EVD_INT_SEP);
        }
		else if((g_pHalMVOPCtx->eSubInterlaceType == E_MVOP_INT_TB_ONE_FRAME) && (g_pHalMVOPCtx->bIsH265))
		{
            HAL_WriteRegBit(SUB_REG(VOP_INT_TYPE), 0, VOP_EVD_INT_SEP);
...
void HAL_MVOP_SetSramPower(MVOP_DevID eID ,MS_BOOL bEnable)
{
    ...
    case E_MVOP_DEV_0:
#if MVOP_SUPPORT_SUB
        if((bEnable== FALSE) && (g_pHalMVOPCtx->bSubIsEnable == FALSE)) //check sub disable -> mfdec sram disable
        {
...
void HAL_MVOP_SubRst(void)
{
    ...
    g_pHalMVOPCtx->bSubIsEnable = TRUE;
}
...
void HAL_MVOP_SubSetInputMode( VOPINPUTMODE mode, MVOP_InputCfg *pparam, MS_U16 u16ECOVersion )
{
    ...
    else if (mode == VOPINPUT_MCUCTRL)
    {
        ...
        if (pparam->bDramRdContd == 0)
        {
            ...
            if(pparam->enVideoType == MVOP_H264)
            {
                HAL_WriteRegBit(SUB_REG(VOP_INPUT_SWITCH0), 1, VOP_TILE_32x32);//mantis:1144345 mustang h264 default 32x32.
            }
            else
            {
                HAL_WriteRegBit(SUB_REG(VOP_INPUT_SWITCH0), 0, VOP_TILE_32x32);//MVD can not 32x32.
            }
        }
        else
        {
...
void HAL_MVOP_SubSetOutputTiming( MVOP_Timing *ptiming )
{
    ...
    //patch for new vsync mode, t/b field opposite
    if((ptiming->bInterlace == 1) && (g_pHalMVOPCtx->bSubNewVSyncMode == TRUE))
    {
        HAL_MVOP_SubSetFieldInverse(DISABLE, ENABLE);
        HAL_WriteByte(SUB_REG(VOP_DMA0), 0x18);
        HAL_WriteByteMask(SUB_REG(VOP_DMA1), 0x01, VOP_BURST_EXT);
    }
    else
    {
        HAL_WriteByte(SUB_REG(VOP_DMA0), 0x08);
        HAL_WriteByteMask(SUB_REG(VOP_DMA1), 0x02, VOP_BURST_EXT);
    }
    ...
    else
    {
        ...
        HAL_WriteByte((VOP_TF_VS_MVD    ), LOWBYTE( u16TopField_VS2MVD ));
        HAL_WriteByte((VOP_TF_VS_MVD + 1), HIGHBYTE( u16TopField_VS2MVD ));
        HAL_WriteRegBit(SUB_REG(VOP_CTRL1), 0, VOP_MVD_VS_MD);
    }
...
/// Set MVOP for EVD Hardwire Mode
/******************************************************************************/
void HAL_MVOP_SubSetEVDHardwireMode(void)
{
    // Hardwire mode
    HAL_WriteByte(SUB_REG(VOP_MPG_JPG_SWITCH), 0x00);
    ...
    HAL_WriteByteMask(SUB_REG(VOP_REG_MASK), 0, VOP_LSB_REQ_MASK);//10 bits bw control by vdec fw
