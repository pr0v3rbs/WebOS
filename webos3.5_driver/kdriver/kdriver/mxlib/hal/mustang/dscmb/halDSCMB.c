...
void inline _pid_key_table_w(MS_U32 eng, MS_U32 addr, MS_U32 key_idx)
{
    MS_U32 engbit = (eng == 0) ? REG_DSCMB2_ARRAY_ENG0_VLD : 
                    ...
                    (eng == 2) ? REG_DSCMB2_ARRAY_ENG2_VLD : 0;
...
{
    MS_U32 addr = 0;        
    for(;addr < REG_DSCMB_MAX_PIDFLT; addr++)
        {
...
MS_BOOL _HAL_DSCMB_Mode(MS_U32 u32EngId, HAL_DSCMB_Type type)
{
    ...
    else if(E_HAL_DSCMB_TYPE_OC == type) // open cable
    {
        ...
        if(u32EngId == 0)
        {
            ...
            REG16_W(REG_DSCMB_KT_CTRL_EXT, REG16_R(REG_DSCMB_KT_CTRL_EXT) | REG_DSCMB_ESA_OC_MODE);
        }
        else if(u32EngId == 1)
        {
            REG16_W(REG_CSA1_CTRL, 0x0);
            ...
            REG16_W(REG_DSCMB1_KT_CTRL_EXT, REG16_R(REG_DSCMB1_KT_CTRL_EXT) | REG_DSCMB1_ESA_OC_MODE);
    ...
    if(bEnable)
    {
        ...
        Drv_Clkm_Set_Clk_Source(s32ClkHandle , "CLK_TSP_FAST");
            ...
            REG16_W(CKG_TSP_STC0, REG16_R(CKG_TSP_STC0) & ~(CLK_TSP_CLK_MASK |
                                                            CLK_TSP_INVERT   |
                                                            CLK_TSP_DISABLE));
            REG16_W(CKG_TS2_TSGP, REG16_R(CKG_TS2_TSGP) & ~(CKG_TSP_GPONLY_MASK));
        #endif
    }
    else
    {
...
MS_BOOL HAL_DSCMB_Init(void)
{
    ...
    // PES layer descramble fix
    REG16_W(REG_DSCMB_KT_CTRL_EXT,  REG_DSCMB_ESA_HDCP_RESIDUE_FIX_EN  | REG_DSCMB_ESA_PES_FIX_EN);
    REG16_W(REG_DSCMB1_KT_CTRL_EXT, REG_DSCMB1_ESA_HDCP_RESIDUE_FIX_EN | REG_DSCMB1_ESA_PES_FIX_EN);
    REG16_W(REG_DSCMB2_KT_CTRL_EXT, REG_DSCMB2_ESA_HDCP_RESIDUE_FIX_EN | REG_DSCMB2_ESA_PES_FIX_EN);
    REG16_W(REG_DSCMB3_KT_CTRL_EXT, REG_DSCMB3_ESA_HDCP_RESIDUE_FIX_EN | REG_DSCMB3_ESA_PES_FIX_EN);
...
MS_BOOL HAL_DSCMB_GetCap(MS_U32 u32EngId, HAL_DSCMB_Query_Type eQueryType, void* pInData, void* pOutData)
{
   ...
   case E_HAL_DSCMB_CAP_SHARE_KEY_REGION_NUM:
    ...
    case E_HAL_DSCMB_CAP_SHARE_KEY_REGION_START: // this region is fix on TSP.
        ...
        {
            if(*((MS_U32*)pInData) == 0)
                *((MS_U32*)pOutData) = DSCMB2_SHARE2_FLT_START_ID;
            else
                *((MS_U32*)pOutData) = 0xFFFFFFFFUL;
        }
        else
    ...
    case E_HAL_DSCMB_CAP_SHARE_KEY_REGION_END:
        ...
        {
            if(*((MS_U32*)pInData) == 0)
                *((MS_U32*)pOutData) = DSCMB2_SHARE2_FLT_END_ID + 1;
            else
                *((MS_U32*)pOutData) = 0xFFFFFFFFUL;
        }
        else
    ...
    case E_HAL_DSCMB_CAP_PIDFILTER_MAP_START:
        if(u32EngId == 0)
            ...
            *((MS_U32*)pOutData) = GROUP2_MAP_PID_START;
            break;
    case E_HAL_DSCMB_CAP_PIDFILTER_MAP_END:
        if(u32EngId == 0)
            ...
            *((MS_U32*)pOutData) = (GROUP2_MAP_PID_START + GROUP2_MAP_PID_LENGTH - 1);
            break;
