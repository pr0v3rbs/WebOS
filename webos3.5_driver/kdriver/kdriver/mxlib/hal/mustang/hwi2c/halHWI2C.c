...
#define INTERRUPT_DMA_MODE 1
////////////////////////////////////////////////////////////////////////////////
// Local variable
////////////////////////////////////////////////////////////////////////////////
static MS_U32 _gMIO_MapBase = 0;
static MS_BOOL g_bLastByte[HAL_HWI2C_PORTS];
static MS_U32 g_u32DmaPhyAddr[HAL_HWI2C_PORTS];
...
/// @param <RET>        \b None :
////////////////////////////////////////////////////////////////////////////////
void HAL_HWI2C_SetIOMapBase(MS_U32 u32Base)
{
    HWI2C_HAL_FUNC();
...
/// @param <RET>        \b MS_U8
////////////////////////////////////////////////////////////////////////////////
MS_U8 HAL_HWI2C_ReadByte(MS_U32 u32RegAddr)
{
...
{
#if defined(MSOS_TYPE_LINUX_KERNEL) && (INTERRUPT_DMA_MODE == 1)
    ...
    bResult = HAL_HWI2C_Write4Byte(REG_HWI2C_DMA_MIU_ADR+u32PortOffset, u32MiuAddr);
    ...
    if(HAL_HWI2C_GetPortIdxByOffset(u32PortOffset,&u8Port)==FALSE)
        return FALSE;
    g_u32DmaPhyAddr[u8Port] = u32MiuAddr;
    return HAL_HWI2C_Write4Byte(REG_HWI2C_DMA_MIU_ADR+u32PortOffset, u32MiuAddr);
#endif
}
...
static MS_BOOL HAL_HWI2C_DMA_SetMiuData(MS_U32 u32PortOffset, MS_U32 u32Length, MS_U8* pu8SrcData)
{
    ...
    pMiuData = (MS_U8*)_PA2VA((MS_U32)u32PhyAddr);
...
//defined(MSOS_TYPE_LINUX_KERNEL) && (INTERRUPT_DMA_MODE == 1)if
#if 1
static MS_BOOL HAL_HWI2C_DMA_WaitDone(MS_U32 u32PortOffset, MS_U8 u8ReadWrite, MS_BOOL bWait)
{
    ...
    //clear transfer done first for savfty
    ...
    return FALSE;
        ...
        /* no need to wait with interrupt mode */
        return TRUE;
    }
}
#else
static MS_BOOL HAL_HWI2C_DMA_WaitDone(MS_U32 u32PortOffset, MS_U8 u8ReadWrite)
{
    MS_U16 volatile u16Timeout = HWI2C_DMA_WAIT_TIMEOUT;
    MS_U8 u8Port;
...
#if defined(MSOS_TYPE_LINUX_KERNEL) && (INTERRUPT_DMA_MODE == 1)
//IIC DMA interrupt handler and related functions --by hank.lai
...
//  Global Functions
//-------------------------------------------------------------------------------------------------
void HAL_Set_MIU_Size(MS_U32 u32Size)
{
    _gu32MIU0_SIZE = u32Size +_gu32MIU0_BUS_BASE;
    HWI2C_HAL_INFO("[IIC][%s] MIU0 size is 0x%X \n\n", __FUNCTION__, _gu32MIU0_SIZE);
}
#endif
////////////////////////////////////////////////////////////////////////////////
...
MS_BOOL HAL_HWI2C_SetPortRegOffset(HAL_HWI2C_PORT ePort, MS_U32* pu32PortOffset)
{
    ...
    if((ePort>=E_HAL_HWI2C_PORT0_0)&&(ePort<=E_HAL_HWI2C_PORT0_2))
    {//port 0 : bank register address 0x102B00
        *pu32PortOffset = (MS_U32)0x00;
    }
    else if((ePort>=E_HAL_HWI2C_PORT1_0)&&(ePort<=E_HAL_HWI2C_PORT1_2))
    {//port 1 : bank register address 0x102B80
        *pu32PortOffset = (MS_U32)0x80;
    }
    else if((ePort>=E_HAL_HWI2C_PORT2_0)&&(ePort<=E_HAL_HWI2C_PORT2_2))
    {//port 2 : bank register address 0x102C00
        *pu32PortOffset = (MS_U32)0x100; 
    }
    else if((ePort>=E_HAL_HWI2C_PORT3_0)&&(ePort<=E_HAL_HWI2C_PORT3_1))
    {//port 3 : bank register address 0x102C80
        *pu32PortOffset = (MS_U32)0x180; 
    }
    else if((ePort>=E_HAL_HWI2C_PORT4_0)&&(ePort<=E_HAL_HWI2C_PORT4_1))
    {//port 4 : bank register address 0x110500
        *pu32PortOffset = (MS_U32)0xDA00; 
    }
    else if((ePort>=E_HAL_HWI2C_PORT5_0)&&(ePort<=E_HAL_HWI2C_PORT5_1))
    {//port 5 : bank register address 0x110580
        *pu32PortOffset = (MS_U32)0xDA80; 
    }
    else
    {
        *pu32PortOffset = (MS_U32)0x00;
...
/// @param <IN>         \b u32PortOffset : port register offset
...
MS_BOOL HAL_HWI2C_GetPortIdxByOffset(MS_U32 u32PortOffset, MS_U8* pu8Port)
{
    ...
    else if(u32PortOffset==(MS_U32)0x80)
    {//port 1 : bank register address 0x102B80
    ...
    else if(u32PortOffset==(MS_U32)0x180)
    {//port 3 : bank register address 0x102C80
        *pu8Port = HAL_HWI2C_PORT3;
    }
    else if(u32PortOffset==(MS_U32)0xDA00)
    {//port 4 : bank register address 0x110500
        *pu8Port = HAL_HWI2C_PORT4;
    }
    else if(u32PortOffset==(MS_U32)0xDA80)
    {//port 5 : bank register address 0x110580
...
MS_BOOL HAL_HWI2C_SelectPort(HAL_HWI2C_PORT ePort)
{
    ...
    //decide port mask
    if((ePort>=E_HAL_HWI2C_PORT0_0)&&(ePort<=E_HAL_HWI2C_PORT0_2))//port 0
    {
        switch(ePort)
        {
            case E_HAL_HWI2C_PORT0_0: //miic0 disable
                ...
                break;
            case E_HAL_HWI2C_PORT0_1: //miic0 using PAD_GPIO17/PAD_GPIO18
                u8Value1 = CHIP_MIIC0_PAD_1;
                break;
            case E_HAL_HWI2C_PORT0_2: //miic0 using PAD_TS1_CLK /PAD_TS1_SYNC
    ...
    else if((ePort>=E_HAL_HWI2C_PORT1_0)&&(ePort<=E_HAL_HWI2C_PORT1_2))//port 1
    {
        switch(ePort)
        {
            ...
            case E_HAL_HWI2C_PORT1_1: //miic1 using PAD_TGPIO2/PAD_TGPIO3
                ...
                break;
            case E_HAL_HWI2C_PORT1_2: //miic1 using  PAD_TS1_VLD/PAD_TS1_D[0]
    ...
    else if((ePort>=E_HAL_HWI2C_PORT2_0)&&(ePort<=E_HAL_HWI2C_PORT2_2))//port 2
    {
        switch(ePort)
        {
            ...
            case E_HAL_HWI2C_PORT2_1: //miic2 using PAD_I2S_IN_BCK/PAD_I2S_IN_SD
                ...
                break;
            case E_HAL_HWI2C_PORT2_2: //miic2 using PAD_GPIO15/PAD_GPIO14
                u8Value1 = CHIP_MIIC2_PAD_2;
		  break;
            default:
                return FALSE;
        }
        HAL_HWI2C_WriteByteMask(CHIP_REG_HWI2C_MIIC2, u8Value1, CHIP_MIIC2_PAD_MSK);
    }
    else if((ePort>=E_HAL_HWI2C_PORT3_0)&&(ePort<=E_HAL_HWI2C_PORT3_1))//port 3
    {
        switch(ePort)
        {
            case E_HAL_HWI2C_PORT3_0: //miic3 disable
        	  u8Value1 = CHIP_MIIC3_PAD_0;
                break;
            case E_HAL_HWI2C_PORT3_1: //miic3 using PAD_TCON0/PAD_TCON1
                u8Value1 = CHIP_MIIC3_PAD_1; 
                break;
            default:
                return FALSE;
        }
        HAL_HWI2C_WriteByteMask(CHIP_REG_HWI2C_MIIC3, u8Value1, CHIP_MIIC3_PAD_MSK);
    }
	else if((ePort>=E_HAL_HWI2C_PORT4_0)&&(ePort<=E_HAL_HWI2C_PORT4_1))//port 4
    {
        switch(ePort)
        {
            case E_HAL_HWI2C_PORT4_0: //miic4 disable
                ...
                break;
            case E_HAL_HWI2C_PORT4_1: //miic4 using PAD_TCON4/PAD_TCON5
	...
	else if((ePort>=E_HAL_HWI2C_PORT5_0)&&(ePort<=E_HAL_HWI2C_PORT5_1))//port 5
    {
        switch(ePort)
        {
            case E_HAL_HWI2C_PORT5_0: //miic5 disable
                ...
                break;
            case E_HAL_HWI2C_PORT5_1: //miic5 using PAD_I2S_OUT_SD1/PAD_I2S_OUT_SD2
...
MS_BOOL HAL_HWI2C_DMA_Enable(MS_U32 u32PortOffset, MS_BOOL bEnable)
{
    ...
    //let SDA delay 2T
    //HAL_HWI2C_WriteRegBit(REG_HWI2C_DMA_CMDDAT7+u32PortOffset, _DMA_CMDDAT7_MSK, TRUE); //(### REVIEW ###)
    bRet &= HAL_HWI2C_DMA_SetINT(u32PortOffset, bEnable); //(### REVIEW ###)
...
MS_BOOL HAL_HWI2C_DMA_Init(MS_U32 u32PortOffset, HAL_HWI2C_PortCfg* pstPortCfg)
{
    ...
    if(u8ResumeFlag == 0)
    {
        if(g_stPortCfg[u8Port].bDmaEnable != 0)
        {
            ...
            if ( g_stPortCfg[u8Port].irq > 0 ) {
                ...
                /* IRQF_DISABLED is depreciated */
                result = request_irq(g_stPortCfg[u8Port].irq, miic_handler, 0,
	...
	//(5) MIU 16Byte Setting
	//HAL_HWI2C_WriteRegBit(0x01E45,__BIT7, 0);
...
{
#if defined(MSOS_TYPE_LINUX_KERNEL) && (INTERRUPT_DMA_MODE == 1)//(### REVIEW ###)
    int ret = 0;
    MS_U8 u8SlaveAddr = LOW_BYTE(u16SlaveCfg)>>1;
...
#ifdef MSOS_TYPE_LINUX_KERNEL //(### REVIEW ###)
void HAL_Resume_Flag(MS_U8 u8Resume)
{
    u8ResumeFlag = u8Resume;
}
#endif
