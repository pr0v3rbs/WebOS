...
MS_BOOL HAL_DMD_RegInit (void)
{
    ...
    if (!MDrv_MMIO_GetBASE( &virtNonPMBank, &u32NonPMBankSize, MS_MODULE_PM))
    {
        #ifdef MS_DEBUG
...
MS_U8 HAL_DMD_RIU_ReadByte(MS_U32 u32Addr)
{
    ...
    {
        #ifdef MS_DEBUG
        printf("%s base address un-initialized\n", __FUNCTION__);
        #endif
    }
    return 0;
}
    ...
    {      
        ...
        if (fRFPowerDbm>=100.0) // unreasonable input value, get RF level from RFAGG
        {
            ...
            if (rf_agc_val >=pRfagcSsi[0].sar3_val)
            {
                ...
                #ifdef MS_DEBUG
                printf("RF Level from SAR:%f\n", ch_power_rf);
                printf("SSI_RFAGC (SAR-4) = 0x%x\n", rf_agc_val);
...
void HAL_DMD_RFAGC_Tristate(MS_BOOL bEnable)
{
    ...
    #ifdef MS_DEBUG
    printf("HAL_DMD_RFAGC_Tristate %d\n",bEnable);
    #endif
...
void HAL_DMD_IFAGC_Tristate(MS_BOOL bEnable)
{
    ...
    #ifdef MS_DEBUG
    printf("HAL_DMD_IFAGC_Tristate %d\n",bEnable);
    #endif
    u8RegMuxBackup = HAL_DMD_RIU_ReadByte(0x101E39);
    HAL_DMD_RIU_WriteByte(0x101E39, 0x00); //mux from DMD MCU to HK
    if (bEnable)
...
{
    #ifdef MS_DEBUG
    printf("HAL_DMD_IFAGC_TS_Tristate %d\n",bEnable);
    #endif
...
MS_BOOL HAL_DMD_TSO_Clk_Control(MS_U8 *u8cmd_array)
{
    ...
    if ( (u8Temp&0x01) == 0x00)
    {
        printf("[utopia][halDMD]Error!!!, we shall select clk_dmplldiv3\n");
        return false;
    }
    switch (u8cmd_array[0])
    {
        ...
        case 0x03:
            ...
            break;
        default:
            printf("[utopia][halDMD]Error!!!, cmd invalid\n");
            break;

    }
#ifdef MS_DEBUG
    printf("0x103300: 0x%x\n",HAL_DMD_RIU_ReadByte(0x103300));
    printf("0x103301: 0x%x\n",HAL_DMD_RIU_ReadByte(0x103301));
    printf("0x10330B: 0x%x\n",HAL_DMD_RIU_ReadByte(0x10330B));    
#endif
    return true;
}
