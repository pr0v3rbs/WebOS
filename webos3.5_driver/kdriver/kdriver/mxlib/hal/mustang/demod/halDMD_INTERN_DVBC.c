...
#define INNC_REG_BASE 0x2600UL
#define EQE_REG_BASE  0x2c00UL
#define EQE2_REG_BASE 0x2d00UL
...
MS_U16 INTERN_DVBC_DSPReg_Init(const MS_U8 *u8DVBC_DSPReg,  MS_U8 u8Size)
{
    ...
    DBG_INTERN_DVBC(printf("INTERN_DVBC_DSPReg_Init\n"));

    #if 0//def MS_DEBUG
    {
        MS_U8 u8buffer[256];
        printf("INTERN_DVBC_DSPReg_Init Reset\n");
        ...
        printf("INTERN_DVBC_DSPReg_Init ReadBack, should be all 0\n");
        for (idx = 0; idx<sizeof(INTERN_DVBC_DSPREG); idx++)
            printf("%x ", u8buffer[idx]);
        printf("\n");

        printf("INTERN_DVBC_DSPReg_Init Value\n");
        for (idx = 0; idx<sizeof(INTERN_DVBC_DSPREG); idx++)
            printf("%x ", INTERN_DVBC_DSPREG[idx]);
        printf("\n");
    }
    #endif
    ...
    for (idx = 0; idx<sizeof(INTERN_DVBC_DSPREG); idx++)
    {
        ...
        {
            printf("[Error]INTERN_DVBC_DSPReg_Init, idx=%d, drv_val=0x%x, firmware_val=0x%x\n",idx,INTERN_DVBC_DSPREG[idx],u8RegRead);
        }
    }
    #endif
    ...
    if (u8DVBC_DSPReg != NULL)
    {
        if (1 == u8DVBC_DSPReg[0])
        {
            ...
            for (idx = 0; idx<u8Size; idx++)
            {
                ...
                u8DVBC_DSPReg++;
                DBG_INTERN_DVBC(printf("DSP addr:%x mask:%x read:%x write:%x\n", u16DspAddr, u8Mask, u8RegRead, u8RegWrite));
    ...
    #if 0//def MS_DEBUG
    {
        ...
        printf("INTERN_DVBC_DSPReg_Init ReadReg 0x2000~0x207F\n");
        for (idx = 0; idx<128; idx++)
        {
            printf("%x ", u8buffer[idx]);
...
MS_BOOL INTERN_DVBC_Reset ( void )
{
    ...
    DBG_INTERN_DVBC_TIME(printf("INTERN_DVBC_Reset, t = %ld\n",MsOS_GetSystemTime()));
...
//    HAL_DMD_RIU_WriteByte(0x11208E, (HAL_DMD_RIU_ReadByte(0x11208E)&(~(BIT(0)))));
    ...
    // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h14, 2'b01, 16'h0007);
    // `RIU_W((`RIUBASE_CLKGEN_DMD>>1)+7'h14, 2'b01, 16'h0007);
...
{
    MS_U8            status = true;
    DBG_INTERN_DVBC(printf("INTERN_DVBC_Power_On_Initialization\n"));
...
void INTERN_DVBC_Driving_Control(MS_BOOL bEnable)
{
    ...
    DBG_INTERN_DVBC(printf("---> INTERN_DVBC_Driving_Control(Bit0) = 0x%x \n",u8Temp));
...
void INTERN_DVBC_Clk_Inversion_Control(MS_BOOL bInversionEnable)
{
    ...
    DBG_INTERN_DVBC(printf("---> Inversion(Bit9) = 0x%x \n",u8Temp));
...
MS_BOOL INTERN_DVBC_Serial_Control(MS_BOOL bEnable, MS_U8 u8TSClk)
{
    MS_U8   status = true;
    MS_U8   temp_val;
    DBG_INTERN_DVBC(printf(" @INTERN_DVBC_ts... u8TSClk=%d\n", u8TSClk));
...
#if(INTERN_DVBC_TS_SERIAL_INVERSION == 0)
    DBG_INTERN_DVBC(printf("---> Inversion(Bit5) = %d \n",0 ));
#else
    DBG_INTERN_DVBC(printf("---> Inversion(Bit5) = %d \n",1 ));
#endif
...
{
    DBG_INTERN_DVBC(printf(" @INTERN_DVBC_TS1_Enable... \n"));
...
MS_BOOL INTERN_DVBC_Config(MS_U16 u16SymbolRate, DMD_DVBC_MODULATION_TYPE eQamMode, MS_U32 u32IFFreq, MS_BOOL bSpecInv, MS_BOOL bSerialTS, MS_U8 u8TSClk, MS_U16 *pu16_symbol_rate_list,MS_U8 u8_symbol_rate_list_num)
{
    ...
    DBG_INTERN_DVBC(printf(" @INTERN_DVBC_config, SR=%d, QAM=%d, u32IFFreq=%ld, bSpecInv=%d, bSerialTS=%d, u8TSClk=%d\n",u16SymbolRate,eQamMode,u32IFFreq,bSpecInv,bSerialTS, u8TSClk));
    DBG_INTERN_DVBC_TIME(printf("INTERN_DVBC_Config, t = %ld\n",MsOS_GetSystemTime()));
...
/*
    switch(u32IFFreq)
    {
        ...
        case 4560:
            ...
            DBG_INTERN_DVBC(printf("Fc freq = %ld\n", DVBC_FS - u32IFFreq));
            break;
        case 44000:
        default:
...
*/
    ...
    {
        DBG_INTERN_DVBC(printf("DMD_DVBC_QAMAUTO\n"));
    ...
    {
        DBG_INTERN_DVBC(printf("DMD_DVBC_QAM %d\n", eQamMode));
...

    {
        switch( eType )
        {
            case DMD_DVBC_GETLOCK_FEC_LOCK:
                ...
                INTERN_DVBC_info();
                #endif
                DBG_INTERN_DVBC(printf(" @INTERN_DVBC_GetLock FSM 0x%x\n",cData));
                if (cData == 0x0C)
                {
                    if(g_dvbc_lock == 0)
                    {
                      g_dvbc_lock = 1;
                      DBG_INTERN_DVBC(printf("[T12][DVBC]lock++++\n"));
                ...
                else
                {
                    ...
                    {
                      g_dvbc_lock = 0;
                      DBG_INTERN_DVBC(printf("[T12][DVBC]unlock----\n"));
                    }
                    return FALSE;
                }
                break;
            ...
            case DMD_DVBC_GETLOCK_NO_CHANNEL:
                ...
                #ifdef MS_DEBUG
                {
                    ...
                    u16Timer=(u16Timer<<8)+reg_frz;
                    printf("DMD_DVBC_GETLOCK_NO_CHANNEL %d %d %x\n",FSM,u16Timer,cData);
                }
                #endif
                break;
...
MS_BOOL INTERN_DVBC_GetPostViterbiBer(float *ber)
{
    ...
    DBG_GET_SIGNAL_DVBC(printf("INTERN_DVBC PostVitBER = %8.3e \n ", *ber));
...
MS_BOOL INTERN_DVBC_GetPacketErr(MS_U16 *pktErr)
{
    ...
    DBG_GET_SIGNAL_DVBC(printf("INTERN_DVBC PktErr = %d \n ", (int)PktErr));
...
MS_BOOL INTERN_DVBC_GetSignalStrength(MS_U16 *strength, const DMD_DVBC_InitData *sDMD_DVBC_InitData, MS_U8 u8SarValue, float fRFPowerDbm)
{
    ...
    DBG_INTERN_DVBC_TIME(printf("INTERN_DVBC_GetSignalStrength, t=%ld, RF level=%f, Table=%lx\n",MsOS_GetSystemTime(), fRFPowerDbm, (MS_U32)(sDMD_DVBC_InitData->pTuner_RfagcSsi)));
    ...
    DBG_GET_SIGNAL_DVBC(printf(">>> SSI_CH_PWR(dB) = %f , Score = %d<<<\n", ch_power_db, *strength));
    DBG_GET_SIGNAL_DVBC(printf(">>> SSI = %d <<<\n", (int)*strength));
...
MS_BOOL INTERN_DVBC_GetSignalQuality(MS_U16 *quality, const DMD_DVBC_InitData *sDMD_DVBC_InitData, MS_U8 u8SarValue, float fRFPowerDbm)
{
    ...
    //DBG_GET_SIGNAL_DVBC(printf("SNR = %f, QAM = %d, code Rate = %d\n", cn_rec, tps_cnstl, tps_cr));
    DBG_GET_SIGNAL_DVBC(printf("BER = %8.3e\n", fber));
    DBG_GET_SIGNAL_DVBC(printf("Signal Quility = %d\n", *quality));
    return TRUE;
}
...
MS_BOOL INTERN_DVBC_Get_FreqOffset(float *pFreqOff, MS_U8 u8BW)
{
    ...
    DBG_INTERN_DVBC(printf("INTERN_DVBC_Get_FreqOffset\n"));
    ...
    DBG_INTERN_DVBC_LOCK(printf("[dvbc]Freq_Offset = %f KHz, Reg_offset = 0x%lx, Reg_Fc_over_Fs=0x%lx, SR = %d KS/s, Fc = %f %d\n",
...
MS_BOOL INTERN_DVBC_GetCurrentModulationType(DMD_DVBC_MODULATION_TYPE *pQAMMode)
{
    ...
    DBG_INTERN_DVBC(printf("INTERN_DVBC_GetCurrentModulationType\n"));

    MDrv_SYS_DMD_VD_MBX_ReadReg(EQE2_REG_BASE + 0xC4, &u8Data);

    switch(u8Data&0x07)
    {
        case 0:
            *pQAMMode = DMD_DVBC_QAM16;
            DBG_INTERN_DVBC_LOCK(printf("[dvbc]QAM=16\n"));
            return TRUE;
             break;
        case 1:
            *pQAMMode = DMD_DVBC_QAM32;
            DBG_INTERN_DVBC_LOCK(printf("[dvbc]QAM=32\n"));
            return TRUE;
            break;
        case 2:
            *pQAMMode = DMD_DVBC_QAM64;
            DBG_INTERN_DVBC_LOCK(printf("[dvbc]QAM=64\n"));
            return TRUE;
            break;
        case 3:
            *pQAMMode = DMD_DVBC_QAM128;
            DBG_INTERN_DVBC_LOCK(printf("[dvbc]QAM=128\n"));
            return TRUE;
            break;
        case 4:
            *pQAMMode = DMD_DVBC_QAM256;
            DBG_INTERN_DVBC_LOCK(printf("[dvbc]QAM=256\n"));
            return TRUE;
            break;
        default:
            *pQAMMode = DMD_DVBC_QAMAUTO;
            DBG_INTERN_DVBC_LOCK(printf("[dvbc]QAM=invalid\n"));
            return FALSE;
    }
}
...
MS_BOOL INTERN_DVBC_GetCurrentSymbolRate(MS_U16 *u16SymbolRate)
{
    ...
    *u16SymbolRate = u16SymbolRateTmp;
    ...
    DBG_INTERN_DVBC_LOCK(printf("[dvbc]SR=%d\n",*u16SymbolRate));
...
MS_BOOL INTERN_DVBC_GetCurrentSymbolRateOffset(MS_U16 *pData)
{
    ...
    printf("[dvbc]u32_symb_offset = 0x%x\n",u32Data);
    ...
    *pData = (MS_U16)(f_symb_offset + 0.5f);
    ...
    DBG_INTERN_DVBC_LOCK(printf("[dvbc]sfo_offset = %d,%f\n",*pData, f_symb_offset));
...
MS_BOOL INTERN_DVBC_Show_AGC_Info(void)
{
    ...
    printf("[dvbc]agc_k=0x%x, agc_ref=0x%x, d1_k=0x%x, d1_ref=0x%x, d2_k=0x%x, d2_ref=0x%x\n",
        agc_k,agc_ref,d1_k,d1_ref,d2_k,d2_ref);

    printf("[dvbc]agc_g=0x%x, d1_g=0x%x, d2_g=0x%x, agc_err=0x%x\n",if_agc_gain,d1_gain,d2_gain,if_agc_err);
...
void INTERN_DVBC_info(void)
{
    ...
    printf("[MStar_1][1]0x%x,[2]0x%lx,[3]0x%lx,[4]0x%lx,[5]0x%lx,[6]0x%x,[7]%d\n",version,fb_fs,fc_fs,tr_error,crv,qam,packetErr);
    printf("[MStar_2][1]%f,[2]0x%lx,[3]%d,[4]%f,[5]%d,[6]%d,[7]%d\n",f_snr,intp,quality,f_freq,symb_rate,symb_offset,packetErr);
    printf("[Mstar_3][1]0x%x,[2]0x%x,[3]0x%x,[4]0x%x,[5]%d,[6]0x%x,[7]0x%x\n",fft_u16bw,fft_u8,f_end,f_start,s0_count,sc3,sc4);
    printf("[Mstar_4][1]0x%x,[2]0x%x,[3]0x%x,[4]0x%x,[5]0x%x,[6]0x%x,[7]0x%x\n",kp0,kp1,kp2,kp3,kp4,fmax,era_th);
    printf("[Mstar_5][1]0x%x,[2]0x%x,[3]0x%x,[4]0x%x,[5]0x%x,[6]0x%x,[7]0x%x\n",aci_e0,aci_e1,aci_e2,aci_e3,fb_i_1,fb_q_1,era_th);
    printf("[Mstar_6][1]%f,[2]%f,[3]0x%x,[4]0x%x,[5]0x%x,[6]0x%x,[7]0x%x\n",freq,mag,aci_e2,aci_e3,fb_i_1,fb_q_1,era_th);
    return;
}
