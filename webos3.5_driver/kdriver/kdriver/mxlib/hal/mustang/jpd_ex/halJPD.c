...
static void _HAL_JPD_SetMIUProtectMask(MS_BOOL bEnable)
{
    ...
    __HAL_JPD_WriteBit(JPD_MIU0_RQ1_MASK, bEnable, JPD_MIU0_CLIENT_JPD);
    __HAL_JPD_WriteBit(JPD_MIU1_RQ1_MASK, bEnable, JPD_MIU1_CLIENT_JPD);

    JPD_DEBUG_HAL_MSG("_HAL_JPD_SetMIUProtectMask MIU0 : 0x%04X\n", __HAL_JPD_Read2Byte(JPD_MIU0_RQ1_MASK));
    JPD_DEBUG_HAL_MSG("_HAL_JPD_SetMIUProtectMask MIU1 : 0x%04X\n", __HAL_JPD_Read2Byte(JPD_MIU1_RQ1_MASK));
#endif
    return;
}
...
void HAL_JPD_SetMIU(MS_U8 u8Idx)
{
    ...
    __HAL_JPD_WriteBit(JPD_MIU1_MIU_SEL1, 1,JPD_MIU1_CLIENT_JPD);
//        __HAL_JPD_WriteBit(BK_JPD_MCONFIG+1, 0, JPD_MIU_SEL);     // 0 1
    ...
    #endif
#else
    __HAL_JPD_WriteBit(JPD_MIU1_MIU_SEL1, 0,JPD_MIU1_CLIENT_JPD);

    if(0 == __HAL_JPD_ReadBit(JPD_MIU1_MIU_SEL1, JPD_MIU1_CLIENT_JPD))
    {
        if(u8Idx)
        {
            __HAL_JPD_WriteBit(JPD_MIU0_MIU_SEL1, 1, JPD_MIU0_CLIENT_JPD);
            JPD_DEBUG_HAL_MSG("HAL_JPD_SetMIU : JPD_MIU0_MIU_SEL1 : 0x%04X\n", __HAL_JPD_Read2Byte(JPD_MIU0_MIU_SEL1));
        }
        else
        {
            __HAL_JPD_WriteBit(JPD_MIU0_MIU_SEL1, 0, JPD_MIU0_CLIENT_JPD);
            JPD_DEBUG_HAL_MSG("HAL_JPD_SetMIU : JPD_MIU0_MIU_SEL1 : 0x%04X\n", __HAL_JPD_Read2Byte(JPD_MIU0_MIU_SEL1));
        }
    }
    else
...
#if 0
    printf("Set JPD_NONPM_SECURE_21 secure bit to 0\n");
    __HAL_JPD_WriteBit(JPD_NONPM_SECURE_21, 0, JPD_SECURE_RANGE_JPD);

    printf("Set i64 bit to 1\n");
    __HAL_JPD_WriteBit(JPD_MIU_GROUP1_I64, 1, JPD_MIU_CLIENT_JPD);
#endif
...
void HAL_JPD_Rst_SVLD(void)
{
    JPD_DEBUG_HAL_MSG("HAL_JPD_Rst_SVLD : start!!\n");
    _HAL_JPD_SetMIUProtectMask(TRUE);
    //mif reset is high active
    HAL_JPD_Set_M_Config(HAL_JPD_Get_M_Config() | JPD_MIF_RST);
    HAL_JPD_Set_M_Config(HAL_JPD_Get_M_Config() & ~JPD_MIF_RST);
    ...
    __HAL_JPD_Write2Byte(BK_JPD_EVENTFLAG, JPD_EVENT_ALL);
    // reset : low active
    JPD_DEBUG_HAL_MSG("HAL_JPD_Rst_SVLD: set S-config: 0x%lx\n", (MS_U32)JPD_SVLD);
    __HAL_JPD_Write2Byte(BK_JPD_SCONFIG, JPD_SVLD);
    JPD_DEBUG_HAL_MSG("HAL_JPD_Rst_SVLD: set S-config: 0x%lx\n", (MS_U32)(JPD_SVLD | JPD_SWRST));
    __HAL_JPD_Write2Byte(BK_JPD_SCONFIG, (JPD_SVLD | JPD_SWRST));
    ...
    JPD_DEBUG_HAL_MSG("HAL Get EVENTFLAG : 0x%04X\n",  __HAL_JPD_Read2Byte(BK_JPD_EVENTFLAG));
    _HAL_JPD_SetMIUProtectMask(FALSE);
    JPD_DEBUG_HAL_MSG("HAL_JPD_Rst_SVLD : end!!\n");
}

void HAL_JPD_Rst_Patch(MS_U16 u16SConfigReg)
{
    JPD_DEBUG_HAL_MSG("HAL_JPD_Rst_Patch : start!!\n");
    _HAL_JPD_SetMIUProtectMask(TRUE);
    //mif reset is high active
    HAL_JPD_Set_M_Config(HAL_JPD_Get_M_Config() | JPD_MIF_RST);
    HAL_JPD_Set_M_Config(HAL_JPD_Get_M_Config() & ~JPD_MIF_RST);
    ...
    JPD_DEBUG_HAL_MSG("HAL_JPD_Rst_Patch: set S-config: 0x%x\n", u16SConfigReg);
    __HAL_JPD_Write2Byte(BK_JPD_SCONFIG, u16SConfigReg);
    JPD_DEBUG_HAL_MSG("HAL_JPD_Rst_Patch: set S-config: 0x%x\n", u16SConfigReg | JPD_SWRST);
    __HAL_JPD_Write2Byte(BK_JPD_SCONFIG, u16SConfigReg | JPD_SWRST);
    __HAL_JPD_Write2Byte(BK_JPD_INTEN, JPD_EVENT_ALL);
    JPD_DEBUG_HAL_MSG("Get S-config : 0x%04X\n", __HAL_JPD_Read2Byte(BK_JPD_SCONFIG));
    JPD_DEBUG_HAL_MSG("Get M-config : 0x%04X\n", __HAL_JPD_Read2Byte(BK_JPD_MCONFIG));
    JPD_DEBUG_HAL_MSG("HAL Get INTEN : 0x%04X\n",  __HAL_JPD_Read2Byte(BK_JPD_INTEN));
    JPD_DEBUG_HAL_MSG("HAL Get EVENTFLAG : 0x%04X\n",  __HAL_JPD_Read2Byte(BK_JPD_EVENTFLAG));
    _HAL_JPD_SetMIUProtectMask(FALSE);
    JPD_DEBUG_HAL_MSG("HAL_JPD_Rst_Patch : end!!\n");
}
...
void HAL_JPD_Debug(void)
{
        MS_U8 u8i;

        MS_U16 u16Spare = HAL_JPD_GetSpare();
        MDrv_JPD_SetSpare(u16Spare | JPD_BIT(9));

        MS_U16 u16DbgCtrl = __HAL_JPD_Read2Byte(BK_JPD_DBG_CTRL);
        u16DbgCtrl &= 0xfff0;


        MS_U16 u16i;
        for(u16i=0; u16i<=0xf; u16i++)
        {
            __HAL_JPD_Write2Byte(BK_JPD_DBG_CTRL, u16DbgCtrl | u16i);
            JPD_DEBUG_HAL_MSG("BK_JPD_DEBUG_BUS[H:L] = [0x%02x][0x%04x:0x%04x]\n", u16i,
            __HAL_JPD_Read2Byte(BK_JPD_DEBUG_BUS_H),
            __HAL_JPD_Read2Byte(BK_JPD_DEBUG_BUS_L));
        }

        JPD_DEBUG_HAL_MSG("=======================================================\n");
        JPD_DEBUG_HAL_MSG("JPD   | 00/08 01/09 02/0A 03/0B 04/0C 05/0D 06/0E 07/0F\n");
        JPD_DEBUG_HAL_MSG("=======================================================\n");
        for(u8i=0; u8i<0x80; u8i+=8)
        {
            JPD_DEBUG_HAL_MSG("%02x    | %04x  %04x  %04x  %04x  %04x  %04x  %04x  %04x \n",u8i,
            __HAL_JPD_Read2Byte(BK_JPD_GENERAL(u8i)),
            __HAL_JPD_Read2Byte(BK_JPD_GENERAL(u8i+1)),
            __HAL_JPD_Read2Byte(BK_JPD_GENERAL(u8i+2)),
            __HAL_JPD_Read2Byte(BK_JPD_GENERAL(u8i+3)),
            __HAL_JPD_Read2Byte(BK_JPD_GENERAL(u8i+4)),
            __HAL_JPD_Read2Byte(BK_JPD_GENERAL(u8i+5)),
            __HAL_JPD_Read2Byte(BK_JPD_GENERAL(u8i+6)),
            __HAL_JPD_Read2Byte(BK_JPD_GENERAL(u8i+7))
            );
        }
        JPD_DEBUG_HAL_MSG("=======================================================\n");

        JPD_DEBUG_HAL_MSG("=======================================================\n");
        JPD_DEBUG_HAL_MSG("MIU0  | 00/08 01/09 02/0A 03/0B 04/0C 05/0D 06/0E 07/0F\n");
        JPD_DEBUG_HAL_MSG("=======================================================\n");
        for(u8i=0; u8i<0x80; u8i+=8)
        {
            JPD_DEBUG_HAL_MSG("%02x    | %04x  %04x  %04x  %04x  %04x  %04x  %04x  %04x \n",u8i,
            __HAL_JPD_Read2Byte(BK_MIU0_GENERAL(u8i)),
            __HAL_JPD_Read2Byte(BK_MIU0_GENERAL(u8i+1)),
            __HAL_JPD_Read2Byte(BK_MIU0_GENERAL(u8i+2)),
            __HAL_JPD_Read2Byte(BK_MIU0_GENERAL(u8i+3)),
            __HAL_JPD_Read2Byte(BK_MIU0_GENERAL(u8i+4)),
            __HAL_JPD_Read2Byte(BK_MIU0_GENERAL(u8i+5)),
            __HAL_JPD_Read2Byte(BK_MIU0_GENERAL(u8i+6)),
            __HAL_JPD_Read2Byte(BK_MIU0_GENERAL(u8i+7))
            );
        }
        JPD_DEBUG_HAL_MSG("=======================================================\n");

        JPD_DEBUG_HAL_MSG("=======================================================\n");
        JPD_DEBUG_HAL_MSG("MIU1  | 00/08 01/09 02/0A 03/0B 04/0C 05/0D 06/0E 07/0F\n");
        JPD_DEBUG_HAL_MSG("=======================================================\n");
        for(u8i=0; u8i<0x80; u8i+=8)
        {
            JPD_DEBUG_HAL_MSG("%02x    | %04x  %04x  %04x  %04x  %04x  %04x  %04x  %04x \n",u8i,
            __HAL_JPD_Read2Byte(BK_MIU1_GENERAL(u8i)),
            __HAL_JPD_Read2Byte(BK_MIU1_GENERAL(u8i+1)),
            __HAL_JPD_Read2Byte(BK_MIU1_GENERAL(u8i+2)),
            __HAL_JPD_Read2Byte(BK_MIU1_GENERAL(u8i+3)),
            __HAL_JPD_Read2Byte(BK_MIU1_GENERAL(u8i+4)),
            __HAL_JPD_Read2Byte(BK_MIU1_GENERAL(u8i+5)),
            __HAL_JPD_Read2Byte(BK_MIU1_GENERAL(u8i+6)),
            __HAL_JPD_Read2Byte(BK_MIU1_GENERAL(u8i+7))
            );
        }
        JPD_DEBUG_HAL_MSG("=======================================================\n");

        JPD_DEBUG_HAL_MSG("=======================================================\n");
        JPD_DEBUG_HAL_MSG("CHIP  | 00/08 01/09 02/0A 03/0B 04/0C 05/0D 06/0E 07/0F\n");
        JPD_DEBUG_HAL_MSG("=======================================================\n");
        for(u8i=0; u8i<0x80; u8i+=8)
        {
            JPD_DEBUG_HAL_MSG("%02x    | %04x  %04x  %04x  %04x  %04x  %04x  %04x  %04x \n",u8i,
            __HAL_JPD_Read2Byte(BK_CHIPTOP_GENERAL(u8i)),
            __HAL_JPD_Read2Byte(BK_CHIPTOP_GENERAL(u8i+1)),
            __HAL_JPD_Read2Byte(BK_CHIPTOP_GENERAL(u8i+2)),
            __HAL_JPD_Read2Byte(BK_CHIPTOP_GENERAL(u8i+3)),
            __HAL_JPD_Read2Byte(BK_CHIPTOP_GENERAL(u8i+4)),
            __HAL_JPD_Read2Byte(BK_CHIPTOP_GENERAL(u8i+5)),
            __HAL_JPD_Read2Byte(BK_CHIPTOP_GENERAL(u8i+6)),
            __HAL_JPD_Read2Byte(BK_CHIPTOP_GENERAL(u8i+7))
            );
        }
        JPD_DEBUG_HAL_MSG("=======================================================\n");
        ...
        JPD_DEBUG_HAL_MSG("=======================================================\n");
        JPD_DEBUG_HAL_MSG("TZPC  | 00/08 01/09 02/0A 03/0B 04/0C 05/0D 06/0E 07/0F\n");
        JPD_DEBUG_HAL_MSG("=======================================================\n");
        for(u8i=0; u8i<0x80; u8i+=8)
        {
            JPD_DEBUG_HAL_MSG("%02x    | %04x  %04x  %04x  %04x  %04x  %04x  %04x  %04x \n",u8i,
            __HAL_JPD_Read2Byte(BK_TZPC_GENERAL(u8i)),
            __HAL_JPD_Read2Byte(BK_TZPC_GENERAL(u8i+1)),
            __HAL_JPD_Read2Byte(BK_TZPC_GENERAL(u8i+2)),
            __HAL_JPD_Read2Byte(BK_TZPC_GENERAL(u8i+3)),
            __HAL_JPD_Read2Byte(BK_TZPC_GENERAL(u8i+4)),
            __HAL_JPD_Read2Byte(BK_TZPC_GENERAL(u8i+5)),
            __HAL_JPD_Read2Byte(BK_TZPC_GENERAL(u8i+6)),
            __HAL_JPD_Read2Byte(BK_TZPC_GENERAL(u8i+7))
            );
        }
        JPD_DEBUG_HAL_MSG("=======================================================\n");
