...
static MS_BOOL bSetModeDone = FALSE;
static MS_U16 u16ATOP_09_copy = 0;
...
static MS_BOOL _bUseBandgap = FALSE;
static MS_U16 _u16IdacCurrrenMode = ADC_IDAC_CURRENT_MODE_FULL;
...
static void _Hal_ADC_Set_RGBYPbPr_Calibration(void *pInstance)
{
    ...
    if(_bUseBandgap)
    {
        ...
        if(Hal_ADC_GetBandgapTrimData(pInstance,&efuseBandgap))
        {
            ...
            W2BYTEMSK(L_BK_ADC_ATOP(0x5B) , BIT(13), BIT(13));
            W2BYTEMSK(L_BK_ADC_ATOP(0x5B) ,((MS_U16)efuseBandgap & 0x1F) << 8, BMASK(12:8) );
        }
        else
        {
    ...
    {
        // Use LDO
        W2BYTEMSK(L_BK_ADC_ATOP(0x5B) , 0, BIT(13));
    }
    ...
    switch (enADCSrcType)
    {
        ...
        case ADC_TABLE_SOURCE_SCART:
            if(Hal_ADC_is_scart_rgb(pInstance))
            {
                ...
                // Disable ISOG reset signal in normal mode (have input signal), to prevent gargabe lines.
...
static void Hal_ADC_SetADCBwFilter(void *pInstance,ADC_INPUTSOURCE_TYPE enADCInput,MS_U16 u16PixelClk)
{
    ...
    if ( enADCInput & ADC_INPUTSOURCE_ONLY_YPBPR ) // YPbPr
    {
        ...
        Tab_info.u8TabRows = sizeof(MST_ADCSetModeYUV_TBL)/Tab_info.u8TabCols;
             ...
             && ((SC_R2BYTE(0,REG_SC_BK01_02_L) & 0x70)==0x70)
    ...
    bSetModeDone = TRUE;
    u16ATOP_09_copy = R2BYTEMSK(REG_ADC_ATOP_09_L, 0x1800);
...
void Hal_ADC_InitInternalCalibration(void *pInstance,ADC_INPUTSOURCE_TYPE enAdcSource, SCALER_WIN eWindow)
{
    ...
    SC_W2BYTEMSK(0,REG_SC_BK12_01_L, BIT(5), BIT(4) | BIT(5));
    //SC_W2BYTEMSK(0,REG_SC_BK12_03_L, FALSE, BIT(4));
    ...
    SC_W2BYTEMSK(0,REG_SC_BK12_01_L, 0x0100, BIT(8)|BIT(9)|BIT(10));
    SC_W2BYTEMSK(0,REG_SC_BK12_01_L, 0x0000, BIT(12)|BIT(13)|BIT(14));
    SC_W2BYTEMSK(0,REG_SC_BK12_04_L, 0x02, BIT(1));

    // Disable user memory format
    //MDrv_WriteByteMask(H_BK_SCMI(0x02), 0x00, BIT(0)|BIT(1));
    SC_W2BYTEMSK(0,REG_SC_BK12_02_L, 0x0000, BIT(8)|BIT(9));

    //MDrv_WriteByte( BK_SELECT_00, REG_BANK_IP1F2 );
    //MDrv_WriteByte(L_BK_IP1F2(0x0E), 0x11); // enable auto gain function
    SC_W2BYTE(0,REG_SC_BK01_0E_L, 0x0011); // enable auto gain function

    MDrv_Write2Byte(L_BK_ADC_ATOP(0x00) ,0x0001 );

    // Disable NR
    SC_W2BYTEMSK(0,REG_SC_BK06_21_L, 0, BIT(1)|BIT(0));
    SC_W2BYTEMSK(0,REG_SC_BK06_01_L, 0, BIT(1)|BIT(0));

    //framebuffer number
    //SC_W2BYTEMSK(0,REG_SC_BK12_04_L, 0, BIT(6)|BIT(7));
//    SC_W2BYTEMSK(0,REG_SC_BK12_07_L, 0, BIT(13));
    // Two frame mode
    //SC_W2BYTEMSK(0,REG_SC_BK12_07_L, 0, BIT(13));
    //SC_W2BYTEMSK(0,REG_SC_BK12_44_L, 0, BIT(6)|BIT(7));
    //SC_W2BYTEMSK(0,REG_SC_BK12_47_L, 0, BIT(13));
...
void Hal_ADC_init(void *pInstance,MS_U16 u16XTAL_CLK,MS_BOOL IsShareGrd,  MS_U16 eScartIDPortSelection)
{
    ...
    // Eiffel specify setting for New LDO power down control
    // Handles the case that ENABLE_PWS was not enabled.
    W2BYTEMSK(REG_ADC_ATOP_5A_L, 0, BIT(5)|BIT(6)|BIT(7));

    // Disable iternal SOG
    Hal_ADC_ISOG_Enable(pInstance,TRUE);
    //Hal_ADC_ISOG_Enable(TRUE);
    //  1/4 current mode for China package.
...
static void _Hal_ADC_set_freerun(void *pInstance,MS_BOOL bEnable)
{
    ...
    if(bEnable==FALSE && bSetModeDone==FALSE)
    {
        W2BYTEMSK(REG_ADC_ATOP_09_L, u16ATOP_09_copy, 0x1800);
        bSetModeDone = TRUE;
    }
}
...
void Hal_ADC_Set_Source(void *pInstance,ADC_INPUTSOURCE_TYPE enADC_SourceType, E_MUX_INPUTPORT* enInputPortType, MS_U8 u8PortCount)
{
    ...
    case ADC_INPUTSOURCE_MULTI_MVOP_ATV:
        stADCSrc = ADC_TABLE_SOURCE_MVOP_ATV;
        break;
    ...
    case ADC_INPUTSOURCE_MULTI_RGB_MVOP_DVI:
        stADCSrc = ADC_TABLE_SOURCE_RGB_DVI;
        break;

    case ADC_INPUTSOURCE_MULTI_RGB_MVOP_ATV:
    case ADC_INPUTSOURCE_MULTI_RGB_MVOP_CVBS:
    case ADC_INPUTSOURCE_MULTI_RGB_DVI_ATV:
    case ADC_INPUTSOURCE_MULTI_RGB_DVI_CVBS:
    case ADC_INPUTSOURCE_MULTI_RGB_CVBS_ATV:
       stADCSrc = ADC_TABLE_SOURCE_RGB_AV;
        break;

    case ADC_INPUTSOURCE_MULTI_RGB_MVOP_SCART:
    case ADC_INPUTSOURCE_MULTI_RGB_DVI_SCART:
    case ADC_INPUTSOURCE_MULTI_RGB_ATV_SCART:
    case ADC_INPUTSOURCE_MULTI_RGB_SCART_SVIDEO:
        stADCSrc = ADC_TABLE_SOURCE_RGB_SC;
        break;

    case ADC_INPUTSOURCE_MULTI_YPBPR_MVOP_DVI:
        stADCSrc = ADC_TABLE_SOURCE_YUV_MV;
        break;

    case ADC_INPUTSOURCE_MULTI_YPBPR_MVOP_ATV:
    case ADC_INPUTSOURCE_MULTI_YPBPR_MVOP_CVBS:
    case ADC_INPUTSOURCE_MULTI_YPBPR_DVI_ATV:
    case ADC_INPUTSOURCE_MULTI_YPBPR_DVI_CVBS:
    case ADC_INPUTSOURCE_MULTI_YPBPR_ATV_CVBS:
        stADCSrc = ADC_TABLE_SOURCE_YUV_AV;
        break;

    case ADC_INPUTSOURCE_MULTI_YPBPR_MVOP_SCART:
    case ADC_INPUTSOURCE_MULTI_YPBPR_DVI_SCART:
    case ADC_INPUTSOURCE_MULTI_YPBPR_SCART_ATV:
    case ADC_INPUTSOURCE_MULTI_YPBPR_SCART_CVBS:
    case ADC_INPUTSOURCE_MULTI_SCART_YPBPR_RGB:
        stADCSrc = ADC_TABLE_SOURCE_YUV_SC;
        break;

    case ADC_INPUTSOURCE_MULTI_MVOP_DVI_ATV:
    case ADC_INPUTSOURCE_MULTI_MVOP_DVI_CVBS:
    case ADC_INPUTSOURCE_MULTI_MVOP_ATV_CVBS:
        stADCSrc = ADC_TABLE_SOURCE_MVOP_AV;
        break;

    case ADC_INPUTSOURCE_MULTI_MVOP_DVI_SVIDEO:
    case ADC_INPUTSOURCE_MULTI_MVOP_ATV_SVIDEO:
        stADCSrc = ADC_TABLE_SOURCE_MVOP_SV;
        break;

    case ADC_INPUTSOURCE_MULTI_MVOP_DVI_SCART:
    case ADC_INPUTSOURCE_MULTI_MVOP_ATV_SCART:
        stADCSrc = ADC_TABLE_SOURCE_MVOP_SC;
        break;

    case ADC_INPUTSOURCE_MULTI_ATV_DVI_CVBS:
        stADCSrc = ADC_TABLE_SOURCE_DVI_CVBS;
        break;

    case ADC_INPUTSOURCE_MULTI_SVIDEO_DVI_ATV:
        stADCSrc = ADC_TABLE_SOURCE_SVIDEO;
        break;

    case ADC_INPUTSOURCE_MULTI_SCART_DVI_ATV:
        stADCSrc = ADC_TABLE_SOURCE_SCART;
        break;
...
void Hal_ADC_auto_adc_backup(void *pInstance,SCALER_WIN eWindow)
{
    ...
    //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F2);
    _stAutoAdcSetting.u8L_BkAtop_00    = MDrv_ReadByte(L_BK_ADC_ATOP(0x00) );
    ...
    _stAutoAdcSetting.u16SC_BK1_21    = SC_R2BYTE(0,REG_SC_BK01_21_L);
    ...
    _stAutoAdcSetting.u16SC_BK1_02     = SC_R2BYTE(0,REG_SC_BK01_02_L);
    _stAutoAdcSetting.u16SC_BK1_03     = SC_R2BYTE(0,REG_SC_BK01_03_L);
    _stAutoAdcSetting.u16SC_BK1_04     = SC_R2BYTE(0,REG_SC_BK01_04_L);
    _stAutoAdcSetting.u16SC_BK1_05     = SC_R2BYTE(0,REG_SC_BK01_05_L);
    _stAutoAdcSetting.u16SC_BK1_06     = SC_R2BYTE(0,REG_SC_BK01_06_L);
    _stAutoAdcSetting.u16SC_BK1_07     = SC_R2BYTE(0,REG_SC_BK01_07_L);
    _stAutoAdcSetting.u16SC_BK1_0E     = SC_R2BYTE(0,REG_SC_BK01_0E_L);

    _stAutoAdcSetting.u16SC_BK12_01    = SC_R2BYTE(0,REG_SC_BK12_01_L);
    _stAutoAdcSetting.u16SC_BK12_03    = SC_R2BYTE(0,REG_SC_BK12_03_L);
    _stAutoAdcSetting.u16SC_BK12_04    = SC_R2BYTE(0,REG_SC_BK12_04_L);
    _stAutoAdcSetting.u16SC_BK12_0E    = SC_R2BYTE(0,REG_SC_BK12_0E_L);
    _stAutoAdcSetting.u16SC_BK12_0F    = SC_R2BYTE(0,REG_SC_BK12_0F_L);
    _stAutoAdcSetting.u16SC_BK12_16    = SC_R2BYTE(0,REG_SC_BK12_16_L);
    _stAutoAdcSetting.u16SC_BK12_17    = SC_R2BYTE(0,REG_SC_BK12_17_L);

    //scaling
    _stAutoAdcSetting.u16SC_BK02_04    = SC_R2BYTE(0,REG_SC_BK02_04_L);
    _stAutoAdcSetting.u16SC_BK02_05    = SC_R2BYTE(0,REG_SC_BK02_05_L);
    _stAutoAdcSetting.u16SC_BK02_08    = SC_R2BYTE(0,REG_SC_BK02_08_L);
    _stAutoAdcSetting.u16SC_BK02_09    = SC_R2BYTE(0,REG_SC_BK02_09_L);
    _stAutoAdcSetting.u16SC_BK23_07    = SC_R2BYTE(0,REG_SC_BK23_07_L);
    _stAutoAdcSetting.u16SC_BK23_08    = SC_R2BYTE(0,REG_SC_BK23_08_L);
    _stAutoAdcSetting.u16SC_BK23_09    = SC_R2BYTE(0,REG_SC_BK23_09_L);
    _stAutoAdcSetting.u16SC_BK23_0A    = SC_R2BYTE(0,REG_SC_BK23_0A_L);

    //DNR base
    _stAutoAdcSetting.u32SC_BK12_08    = SC_R4BYTE(0,REG_SC_BK12_08_L);
    _stAutoAdcSetting.u32SC_BK12_0A    = SC_R4BYTE(0,REG_SC_BK12_0A_L);
    _stAutoAdcSetting.u32SC_BK12_0C    = SC_R4BYTE(0,REG_SC_BK12_0C_L);
    //OPM Base
    _stAutoAdcSetting.u32SC_BK12_10    = SC_R4BYTE(0,REG_SC_BK12_10_L);
    _stAutoAdcSetting.u32SC_BK12_12    = SC_R4BYTE(0,REG_SC_BK12_12_L);
    _stAutoAdcSetting.u32SC_BK12_14    = 0;//SC_R4BYTE(0,REG_SC_BK12_14_L);

    _stAutoAdcSetting.u16SC_BK06_01    = SC_R2BYTE(0,REG_SC_BK06_01_L);
    _stAutoAdcSetting.u16SC_BK06_21    = SC_R2BYTE(0,REG_SC_BK06_21_L);
    _stAutoAdcSetting.u16SC_BK12_07    = SC_R2BYTE(0,REG_SC_BK12_07_L);
    _stAutoAdcSetting.u16SC_BK12_44    = SC_R2BYTE(0,REG_SC_BK12_44_L);
    _stAutoAdcSetting.u16SC_BK12_47    = SC_R2BYTE(0,REG_SC_BK12_47_L);
    _stAutoAdcSetting.u16SC_BK12_1A     = SC_R2BYTE(0,REG_SC_BK12_1A_L);
    _stAutoAdcSetting.u16SC_BK12_1B     = SC_R2BYTE(0,REG_SC_BK12_1B_L);

    //MDrv_WriteByte(BK_SELECT_00, REG_BANK_VOP);
    _stAutoAdcSetting.u16SC_BK10_19   = SC_R2BYTE(0,REG_SC_BK10_19_L);

    MDrv_WriteByteMask(L_BK_ADC_ATOP(0x5c), 0x30,0x30);  //ldo

    SC_W2BYTEMSK(0,REG_SC_BK12_02_L, BIT(15), BIT(15));
...
void Hal_ADC_auto_adc_restore(void *pInstance,SCALER_WIN eWindow)
{
   ...
   // u8Bank = MDrv_ReadByte(BK_SELECT_00);
   // MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F2);
    MDrv_WriteByte(L_BK_ADC_ATOP(0x00), _stAutoAdcSetting.u8L_BkAtop_00);
    ...
    SC_W2BYTE(0,REG_SC_BK01_21_L, _stAutoAdcSetting.u16SC_BK1_21);
    ...
    SC_W2BYTE(0,REG_SC_BK01_02_L, _stAutoAdcSetting.u16SC_BK1_02);
    SC_W2BYTE(0,REG_SC_BK01_03_L, _stAutoAdcSetting.u16SC_BK1_03);
    SC_W2BYTE(0,REG_SC_BK01_04_L, _stAutoAdcSetting.u16SC_BK1_04);
    SC_W2BYTE(0,REG_SC_BK01_05_L, _stAutoAdcSetting.u16SC_BK1_05);
    SC_W2BYTE(0,REG_SC_BK01_06_L, _stAutoAdcSetting.u16SC_BK1_06);
    SC_W2BYTE(0,REG_SC_BK01_07_L, _stAutoAdcSetting.u16SC_BK1_07);
    SC_W2BYTE(0,REG_SC_BK01_0E_L, _stAutoAdcSetting.u16SC_BK1_0E);
    /*
    ...
    */
    //MDrv_WriteByte(BK_SELECT_00, REG_BANK_SCMI);
    SC_W2BYTE(0,REG_SC_BK12_01_L, _stAutoAdcSetting.u16SC_BK12_01);
    SC_W2BYTE(0,REG_SC_BK12_03_L, _stAutoAdcSetting.u16SC_BK12_03);
    SC_W2BYTE(0,REG_SC_BK12_04_L, _stAutoAdcSetting.u16SC_BK12_04);
    SC_W2BYTE(0,REG_SC_BK12_0E_L, _stAutoAdcSetting.u16SC_BK12_0E);
    SC_W2BYTE(0,REG_SC_BK12_0F_L, _stAutoAdcSetting.u16SC_BK12_0F);
    SC_W2BYTE(0,REG_SC_BK12_16_L, _stAutoAdcSetting.u16SC_BK12_16);
    SC_W2BYTE(0,REG_SC_BK12_17_L, _stAutoAdcSetting.u16SC_BK12_17);

    //scaling
    SC_W2BYTE(0,REG_SC_BK02_04_L, _stAutoAdcSetting.u16SC_BK02_04);
    SC_W2BYTE(0,REG_SC_BK02_05_L, _stAutoAdcSetting.u16SC_BK02_05);
    SC_W2BYTE(0,REG_SC_BK02_08_L, _stAutoAdcSetting.u16SC_BK02_08);
    SC_W2BYTE(0,REG_SC_BK02_09_L, _stAutoAdcSetting.u16SC_BK02_09);
    SC_W2BYTE(0,REG_SC_BK23_07_L, _stAutoAdcSetting.u16SC_BK23_07);
    SC_W2BYTE(0,REG_SC_BK23_08_L, _stAutoAdcSetting.u16SC_BK23_08);
    SC_W2BYTE(0,REG_SC_BK23_09_L, _stAutoAdcSetting.u16SC_BK23_09);
    SC_W2BYTE(0,REG_SC_BK23_0A_L, _stAutoAdcSetting.u16SC_BK23_0A);

    //DNR,OPM Base
    SC_W4BYTE(0,REG_SC_BK12_08_L, _stAutoAdcSetting.u32SC_BK12_08);
    SC_W4BYTE(0,REG_SC_BK12_0A_L, _stAutoAdcSetting.u32SC_BK12_0A);
    SC_W4BYTE(0,REG_SC_BK12_0C_L, _stAutoAdcSetting.u32SC_BK12_0C);
    SC_W4BYTE(0,REG_SC_BK12_10_L, _stAutoAdcSetting.u32SC_BK12_10);
    SC_W4BYTE(0,REG_SC_BK12_12_L, _stAutoAdcSetting.u32SC_BK12_12);
//    SC_W4BYTE(0,REG_SC_BK12_14_L, _stAutoAdcSetting.u32SC_BK12_14);
    SC_W2BYTE(0,REG_SC_BK12_1A_L, _stAutoAdcSetting.u16SC_BK12_1A);
    SC_W2BYTE(0,REG_SC_BK12_1B_L, _stAutoAdcSetting.u16SC_BK12_1B);

    SC_W2BYTE(0,REG_SC_BK06_01_L, _stAutoAdcSetting.u16SC_BK06_01);
    SC_W2BYTE(0,REG_SC_BK06_21_L, _stAutoAdcSetting.u16SC_BK06_21);
    SC_W2BYTE(0,REG_SC_BK12_07_L, _stAutoAdcSetting.u16SC_BK12_07);
    SC_W2BYTE(0,REG_SC_BK12_44_L, _stAutoAdcSetting.u16SC_BK12_44);
    SC_W2BYTE(0,REG_SC_BK12_47_L, _stAutoAdcSetting.u16SC_BK12_47);
/*
...
*/
    ...
    SC_W2BYTE(0,REG_SC_BK10_19_L, _stAutoAdcSetting.u16SC_BK10_19);

    SC_W2BYTEMSK(0,REG_SC_BK12_02_L, 0x00, BIT(15));
...
static MS_BOOL Hal_ADC_GetBandgapTrimData(void *pInstance, MS_U8 *pu8BandgapTrimData)
{
    ...
    MDrv_WriteByteMask(0x00204E, 0x24, 0xFF);
    MDrv_WriteByteMask(0x00204F, 0x00, 0xFF);
    MDrv_WriteByteMask(0x00204C, 0x01, 0xFF);

    while( MDrv_ReadByte(0x00204C) & BIT(0) ) //waiting data  ready
    {
    ...
    *pu8BandgapTrimData = (MS_U8)MDrv_ReadByte(0x002080);
...
void Hal_ADC_SelectRGBYDataToScaler(MS_U8 u8InputR, MS_U8 u8InputG, MS_U8 u8InputB, MS_U8 u8InputY)
{
    u8InputR = u8InputR > E_ADC_COLOR_Y ? E_ADC_COLOR_RED : u8InputR;
    u8InputG = u8InputG > E_ADC_COLOR_Y ? E_ADC_COLOR_GREEN : u8InputG;
    u8InputB = u8InputB > E_ADC_COLOR_Y ? E_ADC_COLOR_BLUE : u8InputB;
    u8InputY = u8InputY > E_ADC_COLOR_Y ? E_ADC_COLOR_Y : u8InputY;

    W2BYTEMSK(REG_ADC_DTOP_10_L, ((MS_U16)u8InputR), BIT(0)|BIT(1)|BIT(2));     // R
    W2BYTEMSK(REG_ADC_DTOP_10_L, ((MS_U16)u8InputG << 3), BIT(3)|BIT(4)|BIT(5));    // G
    W2BYTEMSK(REG_ADC_DTOP_10_L, ((MS_U16)u8InputB << 6), BIT(6)|BIT(7)|BIT(8));    // B
    W2BYTEMSK(REG_ADC_DTOP_10_L, ((MS_U16)u8InputY << 9), BIT(9)|BIT(10)|BIT(11));  // Y
}
#undef MHAL_ADC_C
