...
//
//**************************************************************************
MS_BOOL _Hal_HDMI_GetMHLAttachPort(MS_U8 ucPortSelect)
{
    ...
    if(_Hal_HDMI_GetMHLCableDetect())
    {
        switch(ucPortSelect)
        {
            case INPUT_PORT_DVI0:
                if(pXCResourcePrivate->stdrvXC_MVideo_Context._u8MhlSupportInfo == E_INPUT_SUPPORT_MHL_PORT_DVI0)
                {
                    bMHLAttach = TRUE;
                }
...
MS_BOOL Hal_HDCP22_PollingWriteDone(MS_U8 ucPortIdx)
{
	...
	if(R2BYTE(REG_HDCP22_P0_34_L + dwBKOffset22) & BIT(2)) //check write done
	{
		//clear status
		W2BYTEMSK(REG_HDCP22_P0_34_L + dwBKOffset22, BIT(4)| BIT(2), BIT(4)| BIT(2)); //clear write done, write start
...
MS_BOOL Hal_HDCP22_PollingReadDone(MS_U8 ucPortIdx)
{
	...
	if(R2BYTE(REG_HDCP22_P0_34_L + dwBKOffset22) & BIT(3)) //check read done
	{
		//clear status
		W2BYTEMSK(REG_HDCP22_P0_34_L + dwBKOffset22, BIT(3), BIT(3)); //clear read done
        ...
        switch(enInputPortType)
        {
            ...
            case INPUT_PORT_DVI2://C
                if(pXCResourcePrivate->stdrvXC_MVideo_Context._u8MhlSupportInfo==E_INPUT_SUPPORT_MHL_PORT_DVI2)
                {
					...
					if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_bIsMhlHotPlugInverse)
                ...
                {
                    PM_W2BYTE(REG_PM_SLEEP_73_L, BIT(1) , BIT(1) );
                    PM_W2BYTE(REG_PM_SLEEP_27_L, bHighLow ? BIT(6)|BIT(2) : 0, BIT(6)|BIT(2));
                }
                break;
            case INPUT_PORT_DVI3://D
                if(pXCResourcePrivate->stdrvXC_MVideo_Context._u8MhlSupportInfo==E_INPUT_SUPPORT_MHL_PORT_DVI3)
                {
                ...
                {
                    PM_W2BYTE(REG_PM_SLEEP_73_L, BIT(9) , BIT(9) );
            ...
            default: // For system first power on with the unknow HDMI port.
                {
					...
					if(pXCResourcePrivate->stdrvXC_MVideo_Context._u8MhlSupportInfo != E_INPUT_NOT_SUPPORT_MHL)
					{
					    if(pXCResourcePrivate->stdrvXC_MVideo_Context.g_bIsMhlHotPlugInverse)
                            PM_W2BYTE(REG_PM_MHL_CBUS_20, bHighLow ? 0 : BIT(4), BMASK(4:3)); // [4]: output val, [3]: oen
					    else
...
void Hal_HDMI_init(void *pInstance)// void )
{
    ...
    PM_W2BYTE(REG_PM_SLEEP_73_L, BIT(9)| BIT(1), BIT(14)| BIT(9)| BIT(6)| BIT(1)); // [14]: reg_hpluga_gpio_en, [9]: reg_hpluga_pu1k, [6]: reg_hplugc_gpio_en, [1]: reg_hpluga_pu1k

    if(pXCResourcePrivate->stdrvXC_MVideo_Context._u8MhlSupportInfo == E_INPUT_SUPPORT_MHL_PORT_DVI0)
    {
        PM_W2BYTE(REG_PM_SLEEP_73_L, BIT(8), BIT(8)| BIT(0)); // [8]: reg_hpluga_mhl_en
    }
    else if(pXCResourcePrivate->stdrvXC_MVideo_Context._u8MhlSupportInfo == E_INPUT_SUPPORT_MHL_PORT_DVI2)
    {
...
{
    MS_BOOL bClockStableFlag[4] = {FALSE, FALSE, FALSE, FALSE};
    MS_U8 ucDivValue = 0;
    ...
    {
        if(!_Hal_HDMI_GetMHLAttachPort(INPUT_PORT_DVI0))
        {
            ...
            {
                W2BYTEMSK(REG_DVI_ATOP_32_L, (HDMI_HF_SETTING_VALUE_LOW << 8), BMASK(9:8));
                W2BYTEMSK(REG_DVI_ATOP_5E_L, BIT(4)| HDMI_I_CONTROL_VALUE_LOW_FOR_LG_BOARDISSUE, BMASK(4:0)); //[4]: ICtrl overwrite, [3:0]: ICtrl value
            }
        }
        ...
        if(!bClockStableFlag[0])
        {
            ...
            bClockStableFlag[0] = TRUE;
        }
    }
    else if(bClockStableFlag[0])
    {
        bClockStableFlag[0] = FALSE;
    }
    ...
    if(R2BYTE(REG_DVI_DTOP1_16_L) & BIT(9)) // clk stable
    {
        ...
        {
            W2BYTEMSK(REG_DVI_ATOP1_32_L, (HDMI_HF_SETTING_VALUE_LOW << 8), BMASK(9:8));
            W2BYTEMSK(REG_DVI_ATOP1_5E_L, BIT(4)| HDMI_I_CONTROL_VALUE_LOW_FOR_LG_BOARDISSUE, BMASK(4:0)); //[4]: ICtrl overwrite, [3:0]: ICtrl value
        }
        ...
        if(!bClockStableFlag[1])
        {
            ...
            bClockStableFlag[1] = TRUE;
        }
    }
    else if(bClockStableFlag[1])
    {
        bClockStableFlag[1] = FALSE;
    }

    // port C
    if(R2BYTE(REG_DVI_DTOP3_16_L) & BIT(9)) // clk stable
    {
        if(!_Hal_HDMI_GetMHLAttachPort(INPUT_PORT_DVI2))
        {
        ...
        if(!bClockStableFlag[3])
        {
            ...
            bClockStableFlag[3] = TRUE;
        }
    }
    else if(bClockStableFlag[3])
    {
        bClockStableFlag[3] = FALSE;
    }
    ...
    if(R2BYTE(REG_DVI_DTOP2_16_L) & BIT(9)) // clk stable
    {
        ...
        if(!bClockStableFlag[2])
        {
            ...
            bClockStableFlag[2] = TRUE;
        }
    }
    else if(bClockStableFlag[2])
    {
        bClockStableFlag[2] = FALSE;
    }
}
...
MS_U16 Hal_HDMI_GetHTT(void)
{
    return R2BYTEMSK(REG_HDMI2_32_L, 0x3FFF);
}

MS_U16 Hal_HDMI_GetVDE(void)
{
    return R2BYTEMSK(REG_HDMI2_31_L, 0x3FFF);
}

MS_U16 Hal_HDMI_GetVTT(void)
{
    return 0;
}

void Hal_HDMI_DDCChannelEnable(E_MUX_INPUTPORT enInputPortType, MS_BOOL bEnable)
{
    if(bEnable)
    {
        switch(enInputPortType)
