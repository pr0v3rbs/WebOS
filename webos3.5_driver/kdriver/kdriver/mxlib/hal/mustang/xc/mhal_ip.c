...
//  Global Variables
//-------------------------------------------------------------------------------------------------
static MS_BOOL _bSetVGASogEn = FALSE;
static MS_U8   _u8SetSogDetectStatus = 0;
...
static MS_U8 u8Do_SC_SWreset = 0;
...
{
	if(u8Do_SC_SWreset == u8Reset)
	{
		printf(" [%s][%d] , u8Do_SC_SWreset = [%d] skip!! \n",__FUNCTION__,__LINE__, u8Do_SC_SWreset);
		return;
	}
	else
	{
		u8Do_SC_SWreset = u8Reset;
	}
    MS_U16  u16RWRequest=0;
    MS_U16  u16OPWRequestDisable=0;
...
/*  if ((u8Reset & RST_SCALER_ALL) || (u8Reset & RST_IP_ALL) || (u8Reset & RST_IP_F2))
	...
	if(u8Reset != REST_IP_OFFLINE) // OFFLINE window without access MIU
	{
    // Step 1: before MIU mask, stop the XC W/R request first
       	 if(eWindow == MAIN_WINDOW)
            {
            	u16RWRequest = SC_R2BYTEMSK(0, REG_SC_BK12_07_L, 0x03);
            	u16OPWRequestDisable = SC_R2BYTEMSK(0, REG_SC_BK12_27_L, 0x01);
            	SC_W2BYTEMSK(0,REG_SC_BK12_07_L, 0x03, 0x03);
            	SC_W2BYTEMSK(0,REG_SC_BK12_27_L, 0x01, 0x01);
            }
    // Step 2: Wait the privous request to finish
    	    MDrv_XC_wait_output_vsync(pInstance, 1, 50, eWindow);
        #if USE_MIU_MSK
        /// Before reset need to mask the XC R/W
            MDrv_MIU_MaskReq(0, MIU_CLIENT_SC_IPMAIN_RW);
            MDrv_MIU_MaskReq(0, MIU_CLIENT_SC_OPMAIN_RW);
            MsOS_DelayTask(1);
        #endif	
	}
    ...
    SC_W2BYTEMSK(0, REG_SC_BK00_02_L, u8Reset, 0xFF );
    MsOS_DelayTask(1);
...
#if USE_MIU_MSK
        	MsOS_DelayTask(1);
        	MDrv_MIU_UnMaskReq(0, MIU_CLIENT_SC_IPMAIN_RW);
        	MDrv_MIU_UnMaskReq(0, MIU_CLIENT_SC_OPMAIN_RW);

        	MsOS_DelayTask(1);

        //Restore the W/R request
        	if(eWindow == MAIN_WINDOW)
        	{
            		SC_W2BYTEMSK(0,REG_SC_BK12_07_L, 0x0, 0x03);
            		SC_W2BYTEMSK(0,REG_SC_BK12_27_L, 0x0, 0x01);
        	}
#else
        	MDrv_Write2ByteMask(0x16121E, 0x8000, 0x8000);
        	MsOS_DelayTask(1);
        	MDrv_Write2ByteMask(0x16121E, 0x0000, 0x8000);
#endif
	}	
	u8Do_SC_SWreset = 0;
...
MS_U8 Hal_SC_ip_get_sync_detect_status(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        ...
        u8DetectStatus = (SC_R2BYTE(0,REG_SC_BK01_1E_L) & 0xFF00)>>8;
    }
    else if(eWindow == OFFLINE_WINDOW)
    {
	u8DetectStatus = (SC_R2BYTE(0,REG_SC_BK13_1E_L) & 0xFF00)>>8;
    }
    else if(eWindow == SUB_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F1);
        //u8DetectStatus = MDrv_ReadByte(H_BK_IP1F1(0x1E));
        u8DetectStatus = (SC_R2BYTE(0,REG_SC_BK03_1E_L) & 0xFF00)>>8;
    }
...
{
    return (SC_R2BYTE(0,REG_SC_BK01_1E_L) & BIT(8)) ? 1:0;
}
...
void Hal_SC_ip_set_reg_usr_vspolmd(void *pInstance,MS_U8 u8Enable, SCALER_WIN eWindow )
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        ...
        SC_W2BYTEMSK(0,REG_SC_BK01_21_L, u8Enable ? BIT(12) : 0, BIT(12));
    }
    else if(eWindow == OFFLINE_WINDOW)
    {
	SC_W2BYTEMSK(0, REG_SC_BK13_21_L, u8Enable ? BIT(12) : 0, BIT(12));
    }
    else if(eWindow == SUB_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F1);
        //MDrv_WriteRegBit(H_BK_IP1F1(0x21), u8Enable, BIT(4));
        SC_W2BYTEMSK(0,REG_SC_BK03_21_L, u8Enable ? BIT(12) : 0, BIT(12));
    }
...
void Hal_SC_ip_init_reg_in_timingchange(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        ...
        SC_W2BYTEMSK(0,REG_SC_BK01_21_L,0x0000,0x01FF);

        // reset 3D setting
        SC_W2BYTE(0,REG_SC_BK02_63_L, 0x0000);
        SC_W2BYTEMSK(0,REG_SC_BK02_7F_L, 0x00, BIT(15));
        SC_W2BYTEMSK(0,REG_SC_BK12_30_L, 0x00, BIT(0));
    }
    else if(eWindow == OFFLINE_WINDOW)
    {
	SC_W2BYTEMSK(0,REG_SC_BK13_21_L,0x0000,0x01FF);
    }
    else if(eWindow == SUB_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F1);
        ...
        SC_W2BYTEMSK(0,REG_SC_BK03_21_L,0x0000,0x01FF);
    }
...
MS_U16 Hal_SC_ip_get_horizontalPeriod(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        ...
        u16HorizontalPeriod = ( SC_R2BYTE(0, REG_SC_BK01_20_L) & MST_H_PERIOD_MASK );
    }
    else if(eWindow == OFFLINE_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F2);
        //u16HorizontalPeriod = ( MDrv_Read2Byte( L_BK_IP1F2(0x20)) & MST_H_PERIOD_MASK );
        u16HorizontalPeriod = ( SC_R2BYTE( 0,REG_SC_BK13_20_L) & MST_H_PERIOD_MASK );
    }
    else if(eWindow == SUB_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F1);
        //u16HorizontalPeriod = ( MDrv_Read2Byte( L_BK_IP1F1(0x20)) & MST_H_PERIOD_MASK );
        u16HorizontalPeriod = ( SC_R2BYTE(0, REG_SC_BK03_20_L) & MST_H_PERIOD_MASK );
    }
...
MS_U16 Hal_SC_ip_get_verticaltotal(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        ...
        u16VerticalTotal = ( SC_R2BYTE(0, REG_SC_BK01_1F_L) & MST_V_TOTAL_MASK );
    }
    else if(eWindow == OFFLINE_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F2);
        //u16VerticalTotal = ( MDrv_Read2Byte( L_BK_IP1F2(0x1f)) & MST_V_TOTAL_MASK );
        u16VerticalTotal = ( SC_R2BYTE( 0,REG_SC_BK13_1F_L) & MST_V_TOTAL_MASK );
    }
    else if(eWindow == SUB_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F1);
        //u16VerticalTotal = ( MDrv_Read2Byte( L_BK_IP1F1(0x1f)) & MST_V_TOTAL_MASK );
        u16VerticalTotal = ( SC_R2BYTE(0, REG_SC_BK03_1F_L) & MST_V_TOTAL_MASK );
    }
...
{
    MS_U16 u16de_hstart = 0;
    if(eWindow == MAIN_WINDOW)
        u16de_hstart = SC_R2BYTEMSK(0,REG_SC_BK01_13_L, IP_DE_HSTART_MASK);
    else if(eWindow == OFFLINE_WINDOW)
        u16de_hstart = SC_R2BYTEMSK(0,REG_SC_BK13_13_L, IP_DE_HSTART_MASK);
    else if(eWindow == SUB_WINDOW)
        u16de_hstart = SC_R2BYTEMSK(0,REG_SC_BK03_13_L, IP_DE_HSTART_MASK);
    return u16de_hstart;
}
//=========================================================//
...
MS_U16 Hal_SC_ip_de_hend_info(void *pInstance,SCALER_WIN eWindow)
{
    MS_U16 u16de_hend = 0;
    if(eWindow == MAIN_WINDOW)
        u16de_hend = SC_R2BYTEMSK(0,REG_SC_BK01_15_L, IP_DE_HEND_MASK);
     else if(eWindow == OFFLINE_WINDOW)
        u16de_hend = SC_R2BYTEMSK(0,REG_SC_BK13_15_L, IP_DE_HEND_MASK);
    else if(eWindow == SUB_WINDOW)
        u16de_hend = SC_R2BYTEMSK(0,REG_SC_BK03_15_L, IP_DE_HEND_MASK);
    return u16de_hend;
}
...
{
    MS_U16 u16de_vstart = 0;
    if(eWindow == MAIN_WINDOW)
        u16de_vstart = SC_R2BYTEMSK(0,REG_SC_BK01_12_L, IP_DE_VSTART_MASK);
    else if(eWindow == OFFLINE_WINDOW)
        u16de_vstart = SC_R2BYTEMSK(0,REG_SC_BK13_12_L, IP_DE_VSTART_MASK);
    else if(eWindow == SUB_WINDOW)
        u16de_vstart = SC_R2BYTEMSK(0,REG_SC_BK03_12_L, IP_DE_VSTART_MASK);
    return u16de_vstart;
}
//=========================================================//
...
MS_U16 Hal_SC_ip_de_vend_info(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        u16de_vend = SC_R2BYTEMSK(0,REG_SC_BK01_14_L, IP_DE_VEND_MASK);
    }
    else if(eWindow == OFFLINE_WINDOW)
    {
       u16de_vend = SC_R2BYTEMSK(0,REG_SC_BK13_14_L, IP_DE_VEND_MASK);
    }
    else if(eWindow == SUB_WINDOW)
    {
        u16de_vend = SC_R2BYTEMSK(0,REG_SC_BK03_14_L, IP_DE_VEND_MASK);
    }
    return u16de_vend;
}
...
void Hal_SC_ip_set_pc_h_position (void *pInstance, MS_U16 u16Position, SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        ...
        SC_W2BYTE(0,REG_SC_BK01_05_L, u16Position) ;
    }
    else if(eWindow == SUB_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F1);
        //MDrv_Write2Byte(L_BK_IP1F1(0x05), u16Position) ;
        SC_W2BYTE(0,REG_SC_BK03_05_L, u16Position) ;
    }
...
void Hal_SC_ip_set_pc_v_position (void *pInstance, MS_U16 u16Position, SCALER_WIN eWindow )
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        ...
        SC_W2BYTE(0,REG_SC_BK01_04_L, u16Position) ;
    }
    else if(eWindow == SUB_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F1);
        //MDrv_Write2Byte(L_BK_IP1F1(0x04), u16Position );
        SC_W2BYTE(0,REG_SC_BK03_04_L, u16Position);
    }
...
void Hal_SC_ip_set_ms_filter(void *pInstance,MS_BOOL bEnable, MS_U16 u16FilterRange, SCALER_WIN eWindow )
{
    ...
    if ( bEnable )
    {
        if( eWindow == MAIN_WINDOW )
        {
            SC_W2BYTEMSK(0, REG_SC_BK01_35_L, BIT(12) , BIT(12) | BIT(13) );
        }
        else if(eWindow == SUB_WINDOW)
        {
            SC_W2BYTEMSK(0, REG_SC_BK03_35_L, BIT(12) , BIT(12) | BIT(13) );
        }
    }
    else
    {
        if( eWindow == MAIN_WINDOW )
        {
            SC_W2BYTEMSK(0, REG_SC_BK01_35_L, 0x00 , BIT(12) | BIT(13) );
        }
        else if(eWindow == SUB_WINDOW)
        {
            SC_W2BYTEMSK(0, REG_SC_BK03_35_L, 0x00 , BIT(12) | BIT(13) );
        }
    }
...
void Hal_SC_ip_set_coast_window(void *pInstance, MS_U8 u8Start, MS_U8 u8End, SCALER_WIN eWindow )
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_24_L, u8Start<<8, 0xFF00 );
        SC_W2BYTEMSK(0, REG_SC_BK01_25_L, u8End, 0x00FF );
    }
    else if( eWindow == OFFLINE_WINDOW )
    {
	SC_W2BYTEMSK( 0,REG_SC_BK13_24_L, u8Start<<8, 0xFF00 );
        SC_W2BYTEMSK( 0,REG_SC_BK13_25_L, u8End, 0x00FF );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_24_L, u8Start<<8, 0xFF00 );
        SC_W2BYTEMSK(0, REG_SC_BK03_25_L, u8End, 0x00FF );
    }
}
...
void Hal_SC_ip_sog_detect(void *pInstance)
{
    ...
    u8SOGState = SC_R2BYTE(0,REG_SC_BK01_02_L)& 0x0070;
    ...
    SC_W2BYTEMSK(0,REG_SC_BK01_02_L,u8SOGState,0x0070);
    ...
    if((u8SOGState & 0x70) == 0x70)
    {
        ...
        SC_W2BYTEMSK(0,REG_SC_BK01_03_L, 0 ,BIT(5));
...
MS_U8 Hal_SC_ip_get_interlace_status (void *pInstance, SCALER_WIN eWindow )
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        ...
        if ( SC_R2BYTE(0,REG_SC_BK01_1E_L) & BIT(11) )
        {
            bInterlace = 1;
        }
    }
    else if(eWindow == OFFLINE_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F2);
        //if ( MDrv_ReadByte(H_BK_IP1F2(0x1E)) & BIT(3) )
        if ( SC_R2BYTE(0,REG_SC_BK13_1E_L) & BIT(11) )
        {
            bInterlace = 1;
        }
    }
    else if(eWindow == SUB_WINDOW)
    {
        ...
        if ( SC_R2BYTE(0,REG_SC_BK03_1E_L) & BIT(11) )
        {
            bInterlace = 1;
        }
    }
...
void Hal_SC_ip_set_input_source(void *pInstance, MS_U8 u8InputSrcSel, MS_U8 u8SyncSel, MS_U8 u8VideoSel, MS_U8 u8isYPbPr, SCALER_WIN eWindow )
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_02_L, u16RegVal,  0x0777);
    }
    else if( eWindow == OFFLINE_WINDOW )
    {
        SC_W2BYTEMSK( 0,REG_SC_BK13_02_L, u16RegVal,  0x0777);
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_02_L, u16RegVal,  0x0777);
    }
...
void hal_ip_set_input_10bit(void *pInstance, MS_BOOL bInput10Bit, SCALER_WIN eWindow )
{
    if( eWindow == MAIN_WINDOW )
    {
        if(bInput10Bit)
        {
            SC_W2BYTEMSK(0, REG_SC_BK01_03_L, BIT(11), BIT(11));
        }
        else //Input is 8bit
        {
            //when BIT(11)=0 --> BIT(10)=1, 8bit.5;  BIT(10)=0, 8bit.0
            SC_W2BYTEMSK(0, REG_SC_BK01_03_L, BIT(10), (BIT(11)|BIT(10)) );
        }
    }
    else
    {
        if(bInput10Bit)
        {
            SC_W2BYTEMSK(0, REG_SC_BK03_03_L, BIT(11), BIT(11));
        }
        else //Input is 8bit
        {
            //when BIT(11)=0 --> BIT(10)=1, 8bit.5;  BIT(10)=0, 8bit.0
            SC_W2BYTEMSK(0, REG_SC_BK03_03_L, BIT(10), (BIT(11)|BIT(10)) );
        }
    }
}
...
void Hal_SC_ip_set_image_wrap(void *pInstance, MS_BOOL bHEnable, MS_BOOL bVEnable, SCALER_WIN eWindow )
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_03_L, ((bVEnable<<1)|bHEnable), (BIT(1)|BIT(0)) );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_03_L, ((bVEnable<<1)|bHEnable), (BIT(1)|BIT(0)) );
    }
}
...
void Hal_SC_ip_set_input_sync_reference_edge(void *pInstance, MS_BOOL bHRef, MS_BOOL bVRef, SCALER_WIN eWindow )
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_03_L, ((bHRef<<4)|(bVRef<<3)), (BIT(4)|BIT(3)) );
    }
    else if( eWindow == SUB_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_03_L, ((bHRef<<4)|(bVRef<<3)), (BIT(4)|BIT(3)) );
    }
}
...
void Hal_SC_ip_set_input_vsync_delay(void *pInstance, MS_BOOL bDelay, SCALER_WIN eWindow )
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_03_L, bDelay<<5, BIT(5) );
    }
    else if( eWindow == OFFLINE_WINDOW )
    {
        SC_W2BYTEMSK( 0,REG_SC_BK13_03_L, bDelay<<5, BIT(5) );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_03_L, bDelay<<5, BIT(5) );
    }
}
...
void Hal_SC_ip_set_de_only_mode(void *pInstance, MS_BOOL bEnable, SCALER_WIN eWindow )
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_03_L, (BIT(7)|(bEnable<<6)), (BIT(7)|BIT(6)) );
        SC_W2BYTEMSK(0, REG_SC_BK01_2F_L, bEnable<< 4, BIT(4) );
        MDrv_WriteRegBit(REG_DE_ONLY_F2, bEnable, DE_ONLY_F2_MASK);
    }
    else if( eWindow == OFFLINE_WINDOW )
    {
        SC_W2BYTEMSK( 0,REG_SC_BK13_03_L, (BIT(7)|(bEnable<<6)), (BIT(7)|BIT(6)) );
        SC_W2BYTEMSK(0, REG_SC_BK01_2F_L, bEnable<< 4, BIT(4) );
        MDrv_WriteRegBit(REG_DE_ONLY_F2, bEnable, DE_ONLY_F0_MASK);
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_03_L, (BIT(7)|(bEnable<<6)), (BIT(7)|BIT(6)) );
        SC_W2BYTEMSK(0, REG_SC_BK01_2F_L, bEnable<< 4, BIT(4) );
...
void Hal_SC_ip_set_coast_input(void *pInstance, MS_BOOL bInputSel, SCALER_WIN eWindow )
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_24_L, ((bInputSel<<5)|BIT(0)), 0xFF );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_24_L, ((bInputSel<<5)|BIT(0)), 0xFF );
    }
}
...
void Hal_SC_ip_set_DE_Mode_Glitch(void *pInstance,MS_U8 u8Setting , SCALER_WIN eWindow )
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_27_L, u8Setting, 0xFF );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_27_L, u8Setting, 0xFF );
    }
}
MS_BOOL Hal_SC_ip_get_DE_mode_glitch_protect_enabled(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        bEnable = (MS_BOOL)(SC_R2BYTEMSK(0,REG_SC_BK01_27_L, BIT(2)) >> 2);
    }
    else if(eWindow == SUB_WINDOW)
    {
        bEnable = (MS_BOOL)(SC_R2BYTEMSK(0,REG_SC_BK03_27_L, BIT(2)) >> 2);
    }
    return bEnable;
}
...
void Hal_SC_ip_set_input_sync_sample_mode(void *pInstance, MS_BOOL bMode, SCALER_WIN eWindow )
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_26_L, (bMode<<2), BIT(2) );
    }
    else if( eWindow == OFFLINE_WINDOW )
    {
        SC_W2BYTEMSK( 0,REG_SC_BK13_26_L, (bMode<<2), BIT(2) );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_26_L, (bMode<<2), BIT(2) );
    }
}
...
void Hal_SC_ip_set_de_bypass_mode(void *pInstance, MS_BOOL bEnable, SCALER_WIN eWindow )
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_2F_L, (bEnable<<15), BIT(15) );
    }
    else if( eWindow == OFFLINE_WINDOW )
    {
        SC_W2BYTEMSK( 0,REG_SC_BK13_2F_L, (bEnable<<15), BIT(15) );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_2F_L, (bEnable<<15), BIT(15) );
    }
}
...
MS_U16 Hal_SC_get_hsd_pixel_count(void *pInstance, SCALER_WIN eWindow )
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        u16Count = SC_R2BYTEMSK(0, REG_SC_BK02_3E_L, BITMASK(11 : 0) );
    }
    else if(eWindow == SUB_WINDOW)
    {
        u16Count = SC_R2BYTEMSK(0, REG_SC_BK04_3E_L, BITMASK(11 : 0) );
    }
    return u16Count;
}
MS_U16 Hal_SC_get_vsd_line_count(void *pInstance, SCALER_WIN eWindow )
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        u16Count = SC_R2BYTEMSK(0, REG_SC_BK02_3F_L, BITMASK(10 : 0) );
    }
    else if(eWindow == SUB_WINDOW)
    {
        u16Count = SC_R2BYTEMSK(0, REG_SC_BK04_3F_L, BITMASK(10 : 0) );
    }
...
void Hal_SC_ip_set_post_glitch_removal(void *pInstance, MS_BOOL bEnble, MS_U8 u8Range, SCALER_WIN eWindow )
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_26_L, ((bEnble<<7)|((u8Range&0x07)<<4)), 0xF0 );
    }
    else if( eWindow == OFFLINE_WINDOW )
    {
        SC_W2BYTEMSK( 0,REG_SC_BK13_26_L, ((bEnble<<7)|((u8Range&0x07)<<4)), 0xF0 );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_26_L, ((bEnble<<7)|((u8Range&0x07)<<4)), 0xF0 );
    }
}
...
MS_U8 Hal_SC_ip_get_post_glitch_removal(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        bEnable = (MS_BOOL)(SC_R2BYTEMSK(0,REG_SC_BK01_26_L, BIT(7)) >> 7);
        if (bEnable)
        {
            bEnable = (MS_U8)(SC_R2BYTEMSK(0,REG_SC_BK01_26_L, BIT(6)|BIT(5)|BIT(4)) >> 4);
        }
        else
        {
            bEnable = FALSE;
    ...
    {
        bEnable = (MS_BOOL)(SC_R2BYTEMSK(0,REG_SC_BK03_26_L, BIT(7)) >> 7);
        if (bEnable)
        {
            bEnable = (MS_U8)(SC_R2BYTEMSK(0,REG_SC_BK03_26_L, BIT(6)|BIT(5)|BIT(4)) >> 4);
        }
        else
        {
            bEnable = FALSE;
...
void Hal_SC_ip_get_capture_window(void *pInstance,MS_WINDOW_TYPE* capture_win,SCALER_WIN eWindow)
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        ...
        */
        capture_win->y =  SC_R2BYTE(0, REG_SC_BK01_04_L ) ;  // Vstart
        capture_win->x =  SC_R2BYTE(0, REG_SC_BK01_05_L ) ;  // Hstart
        capture_win->height=  SC_R2BYTE(0, REG_SC_BK01_06_L ) ;  // Vsize
        capture_win->width=  SC_R2BYTE(0, REG_SC_BK01_07_L ) ;  // Hsize
    }
    else if(eWindow == SUB_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F1);

        capture_win->y =  SC_R2BYTE(0, REG_SC_BK03_04_L ) ;  // Vstart
        capture_win->x =  SC_R2BYTE(0, REG_SC_BK03_05_L ) ;  // Hstart
        capture_win->height=  SC_R2BYTE(0, REG_SC_BK03_06_L ) ;  // Vsize
        capture_win->width=  SC_R2BYTE(0, REG_SC_BK03_07_L ) ;  // Hsize
    }
...
static void Hal_SC_ip_set_capture_window(MS_U32 u32Reg, MS_U16 u16Value , SCALER_WIN eWindow)
{
    if( eWindow == MAIN_WINDOW )
    {
        ...
        SC_W2BYTEMSK(0, u32Reg , u16Value , 0x07FF );
    }
    else
    {
        MS_ASSERT(0);
...
{
    if(eWindow == MAIN_WINDOW)
        SC_W2BYTEMSK(0,REG_SC_BK01_04_L, u16Vstart, 0x1FFF);
    else if(eWindow == SUB_WINDOW)
        SC_W2BYTEMSK(0,REG_SC_BK03_04_L, u16Vstart, 0x1FFF);
}

void Hal_SC_ip_set_capture_h_start(void *pInstance,MS_U16 u16Hstart , SCALER_WIN eWindow)
{
    if(eWindow == MAIN_WINDOW)
        SC_W2BYTEMSK(0,REG_SC_BK01_05_L, u16Hstart, 0x1FFF);
    else if(eWindow == SUB_WINDOW)
        SC_W2BYTEMSK(0,REG_SC_BK03_05_L, u16Hstart, 0x1FFF);
}

void Hal_SC_ip_set_capture_v_size(void *pInstance,MS_U16 u16Vsize , SCALER_WIN eWindow)
{
    if(eWindow == MAIN_WINDOW)
        SC_W2BYTEMSK(0,REG_SC_BK01_06_L, u16Vsize, 0x1FFF);
    else if(eWindow == SUB_WINDOW)
        SC_W2BYTEMSK(0,REG_SC_BK03_06_L, u16Vsize, 0x1FFF);
}

void Hal_SC_ip_set_capture_h_size(void *pInstance,MS_U16 u16Hsize , SCALER_WIN eWindow)
{
    if(eWindow == MAIN_WINDOW)
        SC_W2BYTEMSK(0,REG_SC_BK01_07_L, u16Hsize, 0x1FFF);
    else if(eWindow == SUB_WINDOW)
        SC_W2BYTEMSK(0,REG_SC_BK03_07_L, u16Hsize, 0x1FFF);
}
...
void Hal_SC_ip_set_fir_down_sample_divider(void *pInstance, MS_U8 u8Enable, MS_U16 u16OverSampleCount,SCALER_WIN eWindow)
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_0D_L, (u8Enable?( BIT(7) | (u16OverSampleCount & 0xF ) ):0) , (BIT(7) | BIT(0) |BIT(1) | BIT(2) |BIT(3)) );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_0D_L, (u8Enable?( BIT(7) | (u16OverSampleCount & 0xF ) ):0) , (BIT(7) | BIT(0) |BIT(1) | BIT(2) |BIT(3)) );
    }
}
...
void Hal_SC_ip_set_IPAutoNoSignal(void *pInstance,MS_BOOL bEnable, SCALER_WIN eWindow )
{
    if (eWindow == MAIN_WINDOW)
    {
        //SC_W2BYTEMSK(0, REG_SC_BK01_0B_L, bEnable? BIT(4)|BIT(5):0, BIT(4)|BIT(5));
        SC_W2BYTEMSK(0, REG_SC_BK01_0B_L, bEnable? BIT(5):0, BIT(5));
        SC_W2BYTEMSK(0,REG_SC_BK01_1D_L, bEnable? BIT(12):0, BIT(12));
    }
    else if(eWindow == OFFLINE_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK13_17_L, bEnable? BIT(8):0, BIT(8));
        SC_W2BYTEMSK(0, REG_SC_BK13_19_L, bEnable? BIT(8):0, BIT(8));
        SC_W2BYTEMSK(0, REG_SC_BK13_0B_L, bEnable? BIT(0)|BIT(1)|BIT(4)|BIT(5):0, BIT(0)|BIT(1)|BIT(4)|BIT(5));
        SC_W2BYTEMSK(0, REG_SC_BK13_1D_L, bEnable? BIT(12):0, BIT(12));
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0,REG_SC_BK03_0B_L, bEnable? BIT(4)|BIT(5):0, BIT(4)|BIT(5));
        SC_W2BYTEMSK(0,REG_SC_BK03_1D_L, bEnable? BIT(12):0, BIT(12));
    }
}
MS_BOOL Hal_SC_ip_get_IPAutoNoSignal(void *pInstance, SCALER_WIN eWindow )
{
    ...
    if (eWindow == MAIN_WINDOW)
    {
        bEnable = ( SC_R2BYTEMSK(0,REG_SC_BK01_1D_L, BIT(12) ) )? TRUE:FALSE;
    }
    else if(eWindow == SUB_WINDOW)
    {
        bEnable = ( SC_R2BYTEMSK(0,REG_SC_BK03_1D_L, BIT(12) ) )? TRUE:FALSE;
    }
    return bEnable;
}
...
void Hal_SC_ip_set_auto_gain_function(void *pInstance, MS_U8 u8Enable, SCALER_WIN eWindow)
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_0E_L, (u8Enable?(BIT(0) | BIT(4)):0) , (BIT(0) | BIT(4)) );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_0E_L, (u8Enable?(BIT(0) | BIT(4)):0) , (BIT(0) | BIT(4)) );
    }
}
MS_BOOL Hal_SC_ip_is_auto_gain_result_ready(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        bEnable = ( SC_R2BYTEMSK(0,REG_SC_BK01_0E_L, BIT(1) ) )? TRUE:FALSE;
    }
    else if(eWindow == SUB_WINDOW)
    {
        bEnable = ( SC_R2BYTEMSK(0,REG_SC_BK03_0E_L, BIT(1) ) )? TRUE:FALSE;
    }
    return bEnable;
}
...
MS_U8 Hal_SC_ip_auto_gain_min_value_status(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        u8status = ( (SC_R2BYTEMSK(0,REG_SC_BK01_0E_L, ( BIT(10) | BIT(9) | BIT(8) ) ) ) >> 8 );
    }
    else if(eWindow == SUB_WINDOW)
    {
        u8status = ( (SC_R2BYTEMSK(0,REG_SC_BK03_0E_L, ( BIT(10) | BIT(9) | BIT(8) ) ) ) >> 8 );
    }
    return u8status;
}
...
MS_U8 Hal_SC_ip_auto_gain_max_value_status(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        u8Status = ( (SC_R2BYTEMSK(0,REG_SC_BK01_0E_L, ( BIT(5) | BIT(6) | BIT(7) ) ) ) >> 5 );
    }
    else if(eWindow == SUB_WINDOW)
    {
        u8Status = ( (SC_R2BYTEMSK(0,REG_SC_BK03_0E_L, ( BIT(5) | BIT(6) | BIT(7) ) ) ) >> 5 );
    }
    return u8Status;
}
...
MS_U32 Hal_SC_ip_get_auto_phase_value(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        u32value = ( SC_R4BYTE(0,REG_SC_BK01_1A_L) );
    }
    else if(eWindow == SUB_WINDOW)
    {
        u32value = ( SC_R4BYTE(0,REG_SC_BK03_1A_L) );
    }
    return u32value;
}
MS_BOOL Hal_SC_ip_is_auto_phase_result_ready(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        bEnable = ( SC_R2BYTEMSK(0,REG_SC_BK01_19_L, BIT(1) ) )? TRUE:FALSE;
    }
    else if(eWindow == SUB_WINDOW)
    {
        bEnable = ( SC_R2BYTEMSK(0,REG_SC_BK03_19_L, BIT(1) ) )? TRUE:FALSE;
    }
    return bEnable;
}
...
MS_BOOL Hal_SC_ip_is_auto_position_result_ready(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if( eWindow == MAIN_WINDOW )
    {
        bEnable = ( SC_R2BYTEMSK(0,REG_SC_BK01_10_L, BIT(1) ) )? TRUE:FALSE;
    }
    else if(eWindow == SUB_WINDOW)
    {
        bEnable = ( SC_R2BYTEMSK(0,REG_SC_BK03_10_L, BIT(1) ) )? TRUE:FALSE;
    }
    return bEnable;
}
void Hal_SC_ip_set_auto_position_function(void *pInstance, MS_U8 u8Enable, SCALER_WIN eWindow)
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_10_L, (u8Enable?BIT(0):0) , BIT(0) );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_10_L, (u8Enable?BIT(0):0) , BIT(0) );
    }
}
...
void Hal_SC_ip_set_valid_data_threshold(void *pInstance, MS_U8 u8Threshold, SCALER_WIN eWindow )
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_10_L, (u8Threshold & 0x0F) << 12 , 0xF000 );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_10_L, (u8Threshold & 0x0F) << 12 , 0xF000 );
    }
}
...
static MS_U16 Hal_SC_ip_get_auto_position(MS_U32 u32Reg, SCALER_WIN eWindow)
{
    if( eWindow == MAIN_WINDOW )
    {
        return ( SC_R2BYTE(0,u32Reg) & 0xFFF );
    }
    else
    {
        MS_ASSERT(0);
...
MS_U16 Hal_SC_ip_get_auto_position_v_start(void *pInstance, SCALER_WIN eWindow )
{
    MS_U16 u16v_start = 0;
    if(eWindow == MAIN_WINDOW)
        u16v_start = (SC_R2BYTE(0,REG_SC_BK01_12_L) & 0xFFF);
    else if(eWindow == SUB_WINDOW)
        u16v_start = (SC_R2BYTE(0,REG_SC_BK03_12_L) & 0xFFF);
    return u16v_start;
}
MS_U16 Hal_SC_ip_get_auto_position_h_start(void *pInstance, SCALER_WIN eWindow )
{
    MS_U16 u16h_start = 0;
    if(eWindow == MAIN_WINDOW)
        u16h_start = (SC_R2BYTE(0,REG_SC_BK01_13_L) & 0xFFF);
    else if(eWindow == SUB_WINDOW)
        u16h_start = (SC_R2BYTE(0,REG_SC_BK03_13_L) & 0xFFF);
    return u16h_start;
}
MS_U16 Hal_SC_ip_get_auto_position_v_end(void *pInstance, SCALER_WIN eWindow )
{
    MS_U16 u16v_end = 0;
    if(eWindow == MAIN_WINDOW)
        u16v_end = (SC_R2BYTE(0,REG_SC_BK01_14_L) & 0xFFF);
    else if(eWindow == SUB_WINDOW)
        u16v_end = (SC_R2BYTE(0,REG_SC_BK03_14_L) & 0xFFF);
    return u16v_end;
}
MS_U16 Hal_SC_ip_get_auto_position_h_end(void *pInstance, SCALER_WIN eWindow )
{
    MS_U16 u16h_end = 0;
    if(eWindow == MAIN_WINDOW)
        u16h_end = (SC_R2BYTE(0,REG_SC_BK01_15_L) & 0xFFF);
    else if(eWindow == SUB_WINDOW)
        u16h_end = (SC_R2BYTE(0,REG_SC_BK03_15_L) & 0xFFF);
    return u16h_end;
}
...
void Hal_SC_ip_set_auto_range_function(void *pInstance, MS_U8 u8Enable, SCALER_WIN eWindow )
{
    ...
    {
        // Debug mode.
        SC_W2BYTEMSK(0, REG_SC_BK01_60_L, 0xFF00 , 0xFF00); // Bit(15) is enable.
    }
    else
    {
        // Debug mode.
        SC_W2BYTEMSK(0, REG_SC_BK01_60_L, 0x0000 , 0xFF00); // Bit(15) is enable.
    }
    #endif

    if(eWindow == MAIN_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK01_29_L, (u8Enable?BIT(8):0) , BIT(8) );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_29_L, (u8Enable?BIT(8):0) , BIT(8) );
    }
}
...
void Hal_SC_ip_set_auto_range_window(void *pInstance,MS_U16 u16Vstart, MS_U16 u16Hstart, MS_U16 u16Vsize , MS_U16 u16Hsize, SCALER_WIN eWindow)
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTE(0,REG_SC_BK01_2A_L, u16Vstart  );  //vstart
        SC_W2BYTE(0,REG_SC_BK01_2B_L, u16Hstart  );  //hstart
        SC_W2BYTE(0,REG_SC_BK01_2C_L, u16Vsize  );  //vsize
        SC_W2BYTE(0,REG_SC_BK01_2D_L, u16Hsize  );  //hsize
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTE(0,REG_SC_BK03_2A_L, u16Vstart  );  //vstart
        SC_W2BYTE(0,REG_SC_BK03_2B_L, u16Hstart  );  //hstart
        SC_W2BYTE(0,REG_SC_BK03_2C_L, u16Vsize  );  //vsize
        SC_W2BYTE(0,REG_SC_BK03_2D_L, u16Hsize  );  //hsize
    }
...
{
    return (MS_U8)SC_R2BYTEMSK(0,REG_SC_BK01_02_L, 0x0007);
}

void HAL_SC_ip_set_capturesource(void *pInstance,MS_U8 u8Val)
{
    SC_W2BYTEMSK(0,REG_SC_BK01_02_L, (MS_U16)u8Val, 0x0007);
}

MS_U8 Hal_SC_ip_get_user_def_interlace_status(void *pInstance)
{
    return (MS_U8)SC_R2BYTEMSK(0,REG_SC_BK01_21_L, 0x0003);
}
void Hal_SC_ip_set_user_def_interlace_status(void *pInstance,MS_BOOL bEnable, MS_BOOL bIsInterlace,SCALER_WIN eWindow)
{
	if(eWindow == MAIN_WINDOW)
	{
    	SC_W2BYTEMSK(0,REG_SC_BK01_21_L, (bEnable?BIT(0):0),BIT(0));
    	SC_W2BYTEMSK(0,REG_SC_BK01_21_L, (bIsInterlace?BIT(1):0),BIT(1));
	}
	else if(eWindow == SUB_WINDOW)
	{
		SC_W2BYTEMSK(0,REG_SC_BK03_21_L, (bEnable?BIT(0):0),BIT(0));
    	SC_W2BYTEMSK(0,REG_SC_BK03_21_L, (bIsInterlace?BIT(1):0),BIT(1));
	}
}

MS_U8 HAL_SC_ip_get_capture_method(void *pInstance)
{
    return (MS_U8)SC_R2BYTEMSK(0,REG_SC_BK01_03_L, 0x00FF);
}

MS_BOOL HAL_SC_ip_get_capture_format(void *pInstance)
{
    MS_BOOL bIsRGBfmt;
    if(SC_R2BYTEMSK(0,REG_SC_BK01_02_L, 0x0008))
        bIsRGBfmt = TRUE;
    else
        bIsRGBfmt = FALSE;
...
{
    SC_W2BYTEMSK(0,REG_SC_BK01_02_L, (bIsRGB?BIT(3):0), 0x0008);
}

MS_BOOL HAL_SC_ip_get_h_predown_info(void *pInstance)
{
    if(SC_R2BYTEMSK(0,REG_SC_BK02_05_L, 0x8000))
        return TRUE;
    else
        return FALSE;
}
...
{
    if(SC_R2BYTEMSK(0,REG_SC_BK02_09_L, 0x8000))
        return TRUE;
    else
        return FALSE;
}
...
{
    SC_W2BYTEMSK(0,REG_SC_BK02_05_L, (bEnable?BIT(15):0), BIT(15));
}
void HAL_SC_ip_set_v_predown_info(void *pInstance,MS_BOOL bEnable)
{
    SC_W2BYTEMSK(0,REG_SC_BK02_09_L, (bEnable?BIT(15):0),BIT(15));
}

MS_BOOL HAL_SC_ip_get_444to422_filter_mod(void *pInstance)
{
    if(SC_R2BYTEMSK(0,REG_SC_BK02_0A_L, BIT(7)))
        return TRUE;
    else
        return FALSE;
}
...
{
    SC_W2BYTEMSK(0,REG_SC_BK02_0A_L, (bEnable?BIT(7):0), BIT(7));
}

//F2 memory data format, BK12_01[10:8]
MS_U8 HAL_SC_get_memory_fmt(void *pInstance)
{
    return (MS_U8)(SC_R2BYTEMSK(0,REG_SC_BK12_01_L, 0x0700)>>8);
}

//F2 IP memory data format, BK12_2[3:0]
MS_U8 HAL_SC_get_IPM_memory_fmt(void *pInstance)
{
    return (MS_U8)(SC_R2BYTEMSK(0,REG_SC_BK12_02_L, 0x000F));
}
void HAL_SC_set_IPM_memory_fmt(void *pInstance,MS_U8 u8Value)
{
    SC_W2BYTEMSK(0,REG_SC_BK12_02_L, (((MS_U16)u8Value)|BIT(8)),0x010F);
}

//F2 OP memory data format, BK12_2[7:4]
MS_U8 HAL_SC_get_OPM_memory_fmt(void *pInstance)
{
    return (MS_U8)(SC_R2BYTEMSK(0,REG_SC_BK12_02_L, 0x00F0));
}
void HAL_SC_set_OPM_memory_fmt(void *pInstance,MS_U8 u8Value)
{
    SC_W2BYTEMSK(0,REG_SC_BK12_02_L, (((MS_U16)u8Value)<<4|(BIT(9))),0x02F0);
}

//capture image to IP enable
void HAL_SC_set_capture_image2ip(void *pInstance,MS_BOOL bEnable)
{
    SC_W2BYTEMSK(0,REG_SC_BK0F_57_L, (bEnable ? BIT(11):0), BIT(11));
}
MS_U8 HAL_SC_get_VOP_mux(void *pInstance)
{
    return (MS_U8)SC_R2BYTEMSK(0,REG_SC_BK0F_46_L, 0x00F0);
}

void HAL_SC_set_VOP_mux(void *pInstance,MS_U8 u8Val)
{
    SC_W2BYTEMSK(0,REG_SC_BK0F_46_L, (MS_U16)u8Val, 0x00F0);
}
MS_U8 HAL_SC_get_memory_bit_fmt(void *pInstance,SCALER_WIN eWindow)
{
    MS_U8 u8regval = 0;
    if(eWindow == MAIN_WINDOW)
        u8regval = (MS_U8)SC_R2BYTEMSK(0,REG_SC_BK12_01_L, BIT(5)|BIT(4));
    else if(eWindow == SUB_WINDOW)
        u8regval = (MS_U8)SC_R2BYTEMSK(0,REG_SC_BK12_41_L, BIT(5)|BIT(4));
    return u8regval;
}
...
{
    SC_W2BYTEMSK(0,REG_SC_BK12_01_L, (MS_U16)((bEnable_10bit<<5)|bEnable_8bit<<4), (BIT(5)|BIT(4)));
}

void HAL_SC_set_IPM_capture_start(void *pInstance,MS_BOOL bEnable)
{
    SC_W2BYTEMSK(0,REG_SC_BK12_02_L, (MS_U16)(bEnable?BIT(15):0), BIT(15));
}

MS_U32 HAL_SC_get_IRQ_mask_0_31(void *pInstance)
{
    return (MS_U32)SC_R4BYTE(0,REG_SC_BK00_14_L);
}

void HAL_SC_set_IRQ_mask_0_31(void *pInstance,MS_U32 u32Val)
{
    SC_W4BYTE(0,REG_SC_BK00_14_L, u32Val);
}
MS_BOOL HAL_SC_set_GOP_Enable(void *pInstance,MS_U8 MaxGOP ,MS_U8 UseNum, MS_U8 u8MuxNum, MS_BOOL bEnable)
{
    /* GOP OP Path enable to SC Setting
        ...
        mux0-->mux2-->mux3
    */
    MS_U16 u16regval = FALSE;;

    if(UseNum > MaxGOP)
        return FALSE;
    u16regval = (MS_U16)SC_R2BYTE(0,REG_SC_BK00_0C_L);

    if(UseNum == (u8MuxNum & (BIT(0)|BIT(1))))  // Enable Mux0 to XC
    {
        SC_W2BYTEMSK(0,REG_SC_BK00_0C_L, bEnable?(u16regval|BIT(15)):(u16regval & (~ BIT(15))), BIT(15));
    }
    else if(UseNum == ((u8MuxNum & (BIT(4)|BIT(5)))>>4))  // Enable Mux2 to XC
    {
        SC_W2BYTEMSK(0,REG_SC_BK00_0C_L, bEnable?(u16regval|BIT(14)):(u16regval & (~ BIT(14))), BIT(14));
    }
    else if(UseNum == ((u8MuxNum & (BIT(6)|BIT(7)))>>6))  // Enable Mux3 to XC
    {
        SC_W2BYTEMSK(0,REG_SC_BK00_0C_L, bEnable?(u16regval|BIT(13)):(u16regval & (~ BIT(13))), BIT(13));
    }
    else
    {
        return FALSE;
...
void HAL_SC_ip_sel_for_gop(void *pInstance,MS_U8 u8MuxNum ,MS_XC_IPSEL_GOP ipSelGop)
{
    switch(ipSelGop)
    {
        ...
        case MS_IP0_SEL_GOP0:
            SC_W2BYTEMSK(0,REG_SC_BK00_0A_L, 0xA0, 0xA0);
            if(0 == (u8MuxNum & 0x0003))
                SC_W2BYTEMSK(0,REG_SC_BK00_0A_L, BIT(12), BIT(12)); // mux 0
            else
                SC_W2BYTEMSK(0,REG_SC_BK00_0A_L, BIT(13), BIT(13)); // mux 1
            break;
        ...
        case MS_NIP_SEL_GOP0:
            if(0==(u8MuxNum &0x0003))
                SC_W2BYTEMSK(0,REG_SC_BK00_0A_L, 0, BIT(12)); // mux 0
            else
                SC_W2BYTEMSK(0,REG_SC_BK00_0A_L, 0, BIT(13)); // mux 1
            break;
        ...
        case MS_MVOP_SEL:
            SC_W2BYTEMSK(0,REG_SC_BK00_0A_L, 0xA0, 0xA0);
            SC_W2BYTEMSK(0,REG_SC_BK00_0A_L, BIT(12), BIT(12)|BIT(13)); // mux 0
            break;
    }
}
...
{
    SC_W2BYTEMSK(0,REG_SC_BK00_46_L, 0x20, 0x20);
}
...
void Hal_SC_ip_Set_TestPattern(void *pInstance,MS_U8 u8Enable, MS_U16 u6Pattern_type, SCALER_WIN eWindow)
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK(0,REG_SC_BK01_40_L, u8Enable, 0xFF);
        SC_W2BYTE(0,REG_SC_BK01_42_L, u6Pattern_type);
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK(0,REG_SC_BK03_40_L, u8Enable, 0xFF);
        SC_W2BYTE(0,REG_SC_BK03_42_L, u6Pattern_type);
    }
}
...
void HAL_SC_ip_3DMainSub_IPSync(void *pInstance)
{
    ...
    value = SC_R2BYTE(0,REG_SC_BK01_08_L);
    SC_W2BYTE(0,REG_SC_BK03_08_L, value);

    value = SC_R2BYTE(0,REG_SC_BK01_09_L);
    SC_W2BYTE(0,REG_SC_BK03_09_L, value);

    value = SC_R2BYTE(0,REG_SC_BK01_21_L);
    SC_W2BYTE(0,REG_SC_BK03_21_L, value);

    value = SC_R2BYTE(0,REG_SC_BK01_24_L);
    SC_W2BYTE(0,REG_SC_BK03_24_L, value);

    value = SC_R2BYTE(0,REG_SC_BK01_25_L);
    SC_W2BYTE(0,REG_SC_BK03_25_L, value);

    value = SC_R2BYTE(0,REG_SC_BK01_26_L);
    SC_W2BYTE(0,REG_SC_BK03_26_L, value);

    value = SC_R2BYTE(0,REG_SC_BK01_2F_L);
    SC_W2BYTE(0,REG_SC_BK03_2F_L, value);

    SC_W2BYTEMSK(0,REG_SC_BK00_02_L, 1 <<6, BIT(6));
    MsOS_DelayTask(100);
    SC_W2BYTEMSK(0,REG_SC_BK00_02_L, 0x0,     BIT(6));
}

void HAL_SC_ip_detect_mode(void *pInstance,MS_BOOL bEnable)
{
    //MS_U8 u8Bank;
    ...
    if(bEnable)
    {
        ...
        SC_W2BYTEMSK(0,REG_SC_BK01_29_L,BIT(5), BIT(5)|BIT(4));
    }
    else
    {
        //MDrv_WriteByteMask(L_BK_IP1F2(0x29), 0, BIT(5)|BIT(4));
        SC_W2BYTEMSK(0,REG_SC_BK01_29_L, 0 , BIT(5)|BIT(4));
    }
...
MS_U8 Hal_SC_ip_get_h_v_sync_active(void *pInstance,SCALER_WIN eWindow)
{
    ...
    SC_W2BYTEMSK(0,REG_SC_BK00_33_L, BIT(8), BIT(8));

    u8val = (SC_R2BYTE(0,REG_SC_BK01_1E_L) & 0xC000) >> 8;

    //SC_W2BYTEMSK(0,REG_SC_BK00_33_L, 0, BIT(8));

    return u8val;
}

void Hal_SC_set_skp_fd(void *pInstance,MS_BOOL bEnable)
{
}
...
{
    _u8SetSogDetectStatus = u8SogDetectStatus;
}
...
MS_U32 HAL_SC_ip_get_verticalperiod(void *pInstance,SCALER_WIN eWindow)
{
    ...
    if(eWindow == MAIN_WINDOW)
    {
        u32verticalperiod = SC_R4BYTE(0, REG_SC_BK01_3B_L );
    }
    else if(eWindow == SUB_WINDOW)
    {
        u32verticalperiod = SC_R4BYTE(0, REG_SC_BK03_3B_L );
    }
...
/******************************************************************************/
///Set xc fd mask on/off
///@return
/******************************************************************************/
void HAL_SC_ip_set_FD_Mask(void *pInstance,MS_BOOL bEnable)
{
    SC_W2BYTEMSK(0,REG_SC_BK01_26_L, (bEnable?BIT(8):0), BIT(8));
    return;
}
...
/******************************************************************************/
MS_BOOL HAL_SC_ip_Get_FD_Mask(void *pInstance)
{
    if(SC_R2BYTEMSK(0,REG_SC_BK01_26_L, BIT(8)))
        return TRUE;
    else
        return FALSE;
}
...
void HAL_SC_ip_Set_FD_Mask_ByWin(void *pInstance, MS_BOOL bEnable, SCALER_WIN eWindow)
{
    ...
    {
        SC_W2BYTEMSK(0, REG_SC_BK03_26_L, (bEnable?BIT(8):0), BIT(8));
    }
}
...
MS_BOOL HAL_SC_ip_Get_FD_Mask_ByWin(void *pInstance, SCALER_WIN eWindow)
{
    ...
    {
        if(SC_R2BYTEMSK(0, REG_SC_BK03_26_L, BIT(8)))
            bEnable = TRUE;
        else
            bEnable = FALSE;
    }
...
void Hal_SC_ip_set_de_lock_mode(void *pInstance, MS_BOOL bEnable, SCALER_WIN eWindow )
{
    if( eWindow == MAIN_WINDOW )
    {
        SC_W2BYTEMSK( 0, REG_SC_BK01_1D_L, (bEnable<<6), BIT(6) );
    }
    else if(eWindow == SUB_WINDOW)
    {
        SC_W2BYTEMSK( 0, REG_SC_BK03_1D_L, (bEnable<<6), BIT(6) );
    }
}
MS_U16 Hal_SC_ip_get_horizontalTotal(void *pInstance, SCALER_WIN eWindow)
{
    //MS_U8 u8Bank;
    MS_U16 u16HorizontalPeriod = 0;
    ...
    if(eWindow == MAIN_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F2);
        //u16HorizontalPeriod = ( MDrv_Read2Byte( L_BK_IP1F2(0x20)) & MST_H_PERIOD_MASK );
        u16HorizontalPeriod = ( SC_R2BYTE( 0, REG_SC_BK01_28_L) & MST_H_PERIOD_MASK );
    }
    else if(eWindow == OFFLINE_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F2);
        //u16HorizontalPeriod = ( MDrv_Read2Byte( L_BK_IP1F2(0x20)) & MST_H_PERIOD_MASK );
        u16HorizontalPeriod = ( SC_R2BYTE( 0,REG_SC_BK13_28_L) & MST_H_PERIOD_MASK );
    }
    else if(eWindow == SUB_WINDOW)
    {
        //MDrv_WriteByte(BK_SELECT_00, REG_BANK_IP1F1);
        //u16HorizontalPeriod = ( MDrv_Read2Byte( L_BK_IP1F1(0x20)) & MST_H_PERIOD_MASK );
        u16HorizontalPeriod = ( SC_R2BYTE( 0, REG_SC_BK03_28_L) & MST_H_PERIOD_MASK );
    }
    else
    {
    }
