...
void Hal_XC_SetOffLineToSog_YUV(void *pInstance, MS_U8 Channel )
{
    ...
    {
        // Internal SOG & re-route SOG signal to channel 11~13 (Original is 0~2)
        MDrv_WriteByte(L_BK_ADC_ATOP(0x3d), (MDrv_ReadByte(L_BK_ADC_ATOP(0x3d)) & 0xf0) | (Channel + 11) );
        AIS_HalDebug(printf("%s: SOG offline mux -> %d\n", __FUNCTION__, (Channel + 11)));
    }
    else
    {
