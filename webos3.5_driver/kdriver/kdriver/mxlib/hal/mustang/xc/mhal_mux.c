...
void Hal_SC_mux_set_dvi_mux(void *pInstance, MS_U8 PortId, E_OUTPUT_PORT_TYPE enDstPort)
{
    ...
    {
        if(enDstPort == HAL_OUTPUTPORT_SCALER_SUB_WINDOW1 && pXCResourcePrivate->bIsSmartzoomOpen)
        {
            //smart zoom: main and sub connect to same source, so we don't need reset hdmi again.
        }
        else
        {
...
void Hal_SC_mux_set_offline_ip_mux( void *pInstance,MS_U8 u8Data_Mux, MS_U8 u8Clk_Mux)
{
   // MDrv_WriteByte(REG_IPMUX_01_H, (MDrv_ReadByte(REG_IPMUX_01_H) & 0xf0) | u8Data_Mux);
    MDrv_WriteByteMask(H_BK_IPMUX(0x01), u8Data_Mux, 0x0f);
    if ((u8Data_Mux == SC_DWIN_IPMUX_ADC_A) ||
        (u8Data_Mux == SC_DWIN_IPMUX_HDMI_DVI) ||
        (u8Data_Mux == SC_DWIN_IPMUX_MHL))
    {
        MDrv_Write2ByteMask(REG_CKG_PRE_IDCLK0, u8Clk_Mux , CKG_PRE_IDCLK0_MASK);
    }  
    MDrv_WriteByteMask(REG_CKG_IDCLK0, u8Data_Mux << 2, CKG_IDCLK0_MASK);   
}
...
void Hal_SC_mux_dispatch(void *pInstance,E_MUX_INPUTPORT src_port , E_MUX_OUTPUTPORT dest_port)
{
    ...
    else if ( src >= HAL_INPUTPORT_ANALOG0  && src < (HAL_INPUTPORT_ANALOG0 + NUMBER_OF_ANALOG_PORT ))
    {
        ...
        else if (dest == HAL_OUTPUTPORT_OFFLINE)
	    {
	        Hal_SC_mux_set_offline_ip_mux(pInstance,SC_DWIN_IPMUX_ADC_A, SC_PRE_CLK_ADC_A);
	    }
        else
        {
            //Undefined
    ...
    else if ( src >= HAL_INPUTPORT_DVI0 && src < (HAL_INPUTPORT_DVI0 + NUMBER_OF_DVI_PORT) )
    {
        ...
        {
            Hal_SC_mux_set_offline_ip_mux(pInstance,SC_DWIN_IPMUX_HDMI_DVI , SC_PRE_CLK_HDMI_DVI);
	    }
        else
        {
            // Undefeined
