...
extern MS_U32  HAL_HVD_EX_GetShmAddr(MS_U32 u32Id);
...
static MS_U32 _u32VDECFWAddr = 0;
static MS_U32 _u32VDECFWSize = 0;
static MS_U32 _u32StrAddrOffset = 0;
static MS_BOOL _bSuspend = FALSE;
static VPU_EX_ClockSpeed _eClkSpeed = E_VPU_EX_CLOCK_320MHZ;
#endif
//-------------------------------------------------------------------------------------------------
//  Local Variables
//-------------------------------------------------------------------------------------------------
...
typedef struct
{
    ...
    MS_U32  u32FWShareInfoAddr[MAX_SUPPORT_DECODER_NUM];
    #ifdef VDEC3
    MS_U32  u32FWCodeAddr;
    MS_U32  u32BitstreamAddress;
...
#endif
    {
     //MVD in mustang will use the WB MVOP and HVD no need to power on
#if 0
...
#if defined(MSOS_TYPE_LINUX) || defined(MSOS_TYPE_LINUX_KERNEL) || defined(VDEC_UTOPIA_2K)
//For REE
...
MS_BOOL HAL_VPU_Set_MBX_param(MS_U8 u8APIMbxMsgClass)
{
    ...
    printk("[HAL_VPU_Set_MBX_param]  u8APIMbxMsgClass = %d \n",u8APIMbxMsgClass);
#endif
    bVPUMbxInitFlag = TRUE;
...
static MS_BOOL _VPU_EX_InitAll(VPU_EX_NDecInitPara *pInitPara)
{
    ...
    #ifdef SUPPORT_VDEC_STR
    _u32VDECFWAddr = pFWCodeCfg->u32DstAddr;
    _u32VDECFWSize = pFWCodeCfg->u32DstSize;
    HAL_VPU_EX_GetStrOffset(_u32VDECFWSize);
    HAL_VPU_EX_EnableSTRResume(0,FALSE);
    HAL_VPU_EX_EnableSTRResume(1,FALSE);
    #endif
#if MBX_2K
    ...
    #ifdef SUPPORT_VDEC_STR
    _eClkSpeed = eClkSpeed;
    ctl_ptr->u32StrAddrOffset = _u32StrAddrOffset;
    #endif
    MsOS_FlushMemory();
...
MS_BOOL HAL_VPU_EX_LoadVLCTable(VPU_EX_VLCTblCfg *pVlcCfg, MS_U8 u8FwSrcType)
{
    ...
    else
    {
        ...
        else
        {
            ...
            VPU_MSG_DBG("Load VLC inD2D: dest:0x%lx source:%lx size:%lx\n",
                        pVlcCfg->u32DstAddr, ((MS_U32) pu8HVD_VLC_Binary),
                        (MS_U32) sizeof(u8HVD_VLC_Binary));
...
#endif
{
    ...
    else
    {
        ...
        #ifdef SUPPORT_VDEC_STR
        HAL_VPU_EX_EnableSTRResume(u8Offset,FALSE);
        #endif
    ...
    //VPRINTF("[%s][%d] create task : id 0x%lx, cmd 0x%x, arg 0x%lx, bbuID %ld, offset %d \n", __FUNCTION__, __LINE__, u32Id, eCmd, u32Arg, u32BBUId, u8Offset);
...
MS_U32 HAL_VPU_EX_GetVBBUVacancy(MS_U32 u32VBBUAddr)
{
...
#ifdef MSOS_TYPE_LINUX_KERNEL
    MS_U32 u32VAddr = (MS_U32)pstVBBU;
    if(MsOS_MPool_VAError(u32VAddr))
    {
        ...
        {
            printk("%s  Error VA [%lx] Mapping %d  PA:%lx\n",__FUNCTION__,u32VAddr,__LINE__,(pVPUHalContext->u32FWCodeAddr + u32VBBUAddr));
            printk("pVPUHalContext->u32FWCodeAddr:%lx  u32VBBUAddr:%x\n",pVPUHalContext->u32FWCodeAddr,(unsigned int)u32VBBUAddr);
            print_count = 0;
        }
        return 0;
    }
    ...
    else
    {
        ...
        if (E_VPU_EX_DECODER_G2VP9 == pVPUHalContext->_stVPUStream[u8OffsetIdx].eDecodertype)
        {
            if (stEntry->u32Offset == 0)
                ...
                return stEntry->u32Offset - MsOS_VA2PA(pVPUHalContext->u32BitstreamVAddress);
        }
...
MS_U32 HAL_VPU_EX_GetESWritePtr(MS_U32 u32Id, MS_U32 u32VBBUAddr)
{
    ...
    if (E_VPU_EX_DECODER_G2VP9 == pVPUHalContext->_stVPUStream[u8OffsetIdx].eDecodertype)
    {
        if (stEntry->u32Offset == 0)
            ...
            return stEntry->u32Offset + stEntry->u32Length - MsOS_VA2PA(pVPUHalContext->u32BitstreamVAddress);
    }
    else
    #endif
    {
...
MS_BOOL HAL_VPU_EX_Push2VBBU(MS_U32 u32Id, HAL_VPU_EX_PacketInfo *stVpuPkt, MS_U32 u32VBBUAddr)
{
    ...
    if (E_VPU_EX_DECODER_G2VP9 == pVPUHalContext->_stVPUStream[u8OffsetIdx].eDecodertype)
    {
        stEntry->u32Offset += pVPUHalContext->u32BitstreamAddress;
    }
#endif
...
MS_BOOL HAL_VPU_EX_DRAMCMDQueueSend(MS_U32 u32DramAddr, MS_U32 u32Msg)
{
    ...
    VPU_MSG_DBG("Send to Command Queue Address=0x%lx, msg=0x%lx\n", u32DramAddr, u32Msg);
...
//    HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    ...
    if (!cmd_q)
    {
        VPU_MSG_ERR("Invalid parameter with share memory address=0x%x %s:%d \n", (unsigned int)cmd_q, __FUNCTION__, __LINE__);
        return FALSE;
    }
...
HVD_DRAM_COMMAND_QUEUE_SEND_STATUS HAL_VPU_EX_DRAMStreamCMDQueueSend(MS_U32 u32Id, void *cmd_queue, HVD_COMMAND_QUEUE_TYPE u8CmdType, MS_U32 u32Msg)
{
    ...
    MS_U32 u32CmdQWdPtr = 0;

    if (CHECK_NULL_PTR(cmd_q->u32HVD_CMDQ_DRAM_ST_ADDR))
    ...
    #ifdef MSOS_TYPE_LINUX_KERNEL
    if(MsOS_MPool_VAError(u32CmdQWdPtr))
    {
        static int print_count = 0;
        print_count++;
        if((print_count & 0xF) == 0)
        {
            printk("%s  Error VA [%lx] Mapping %d  PA:%lx\n",__FUNCTION__,u32CmdQWdPtr,__LINE__,(pVPUHalContext->u32FWCodeAddr + cmd_q->u32HVD_CMDQ_DRAM_ST_ADDR + cmd_q->u32HVD_STREAM_CMDQ_WD));
            printk("FWCodeAddr:%lx CMDQ_DRAM_ST_ADDR:%x STREAM_CMDQ_WD:%x u32Msg:%x u8CmdType:%d\n",pVPUHalContext->u32FWCodeAddr,
            (unsigned int)cmd_q->u32HVD_CMDQ_DRAM_ST_ADDR,
            (unsigned int)cmd_q->u32HVD_STREAM_CMDQ_WD,
            (unsigned int)u32Msg,u8CmdType);
            print_count = 0;
        }
    ...
    switch (u8CmdType)
    {
        ...
        {
            u32Msg |= (u8TaskID << 24);
            MS_BOOL bRes = HAL_VPU_EX_DRAMCMDQueueSend(u32CmdQWdPtr, u32Msg);
            MsOS_FlushMemory();
        ...
        case E_HVD_CMDQ_ARG:
        {
            MS_BOOL bRes = HAL_VPU_EX_DRAMCMDQueueSend(u32CmdQWdPtr + HVD_DRAM_CMDQ_CMD_SIZE, u32Msg);
            if (bRes)
                bResult = E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL;
            break;
        }
...
//    HVD_ShareMem *pShm = (HVD_ShareMem *) HAL_HVD_EX_GetShmAddr(u32Id);
    ...
    MS_U32 u32DISPCMDQWdPtr = 0;

    if (CHECK_NULL_PTR(cmd_q->u32HVD_DISPCMDQ_DRAM_ST_ADDR))
    {
        return E_HVD_COMMAND_QUEUE_NOT_INITIALED;
    }
    ...
    #ifdef MSOS_TYPE_LINUX_KERNEL
    if(MsOS_MPool_VAError(u32DISPCMDQWdPtr))
    {
        static int print_count = 0;
        print_count++;
        if((print_count & 0xF) == 0)
        {
            printk("%s  Error VA [%lx] Mapping %d  PA:%lx\n",__FUNCTION__,u32DISPCMDQWdPtr,__LINE__,(pVPUHalContext->u32FWCodeAddr + cmd_q->u32HVD_DISPCMDQ_DRAM_ST_ADDR + cmd_q->u32HVD_STREAM_DISPCMDQ_WD));
            printk("FWCodeAddr:%lx CMDQ_DRAM_ST_ADDR:%x STREAM_CMDQ_WD:%x u32Msg:%x u8CmdType:%d\n",pVPUHalContext->u32FWCodeAddr,
    ...
    switch (u8CmdType)
    {
        ...
        {
            u32Msg |= (u8TaskID << 24);
            MS_BOOL res = HAL_VPU_EX_DRAMCMDQueueSend(u32DISPCMDQWdPtr, u32Msg);
        ...
        case E_HVD_CMDQ_ARG:
        {
            MS_BOOL res = HAL_VPU_EX_DRAMCMDQueueSend(u32DISPCMDQWdPtr + HVD_DRAM_CMDQ_CMD_SIZE, u32Msg);
            if (res)
                bResult = E_HVD_COMMAND_QUEUE_SEND_SUCCESSFUL;
            break;
        }
...
{
    pVPUHalContext->u32BitstreamAddress = u32BsAddr;
    return TRUE;
}
#endif
...
void HAL_VPU_EX_ResumeFW(void)
{
    if(_u32VDECFWAddr)
    {
        HVD_memcpy(_u32VDECFWAddr, (MS_U32)u8HVD_FW_Binary, sizeof(u8HVD_FW_Binary));
        struct _ctl_info *ctl_ptr = (struct _ctl_info *)MsOS_PA2KSEG1(MsOS_VA2PA(_u32VDECFWAddr) + CTL_INFO_ADDR);
        ctl_ptr->statue = CTL_STU_NONE;
        //notify controller the interface version of VPU driver.
        ctl_ptr->ctl_interface = VPU_CTL_INTERFACE_VER;
        ctl_ptr->vpu_clk = _VPU_EX_InClock(_eClkSpeed);
        ctl_ptr->u32StrAddrOffset = _u32StrAddrOffset;
        MsOS_FlushMemory();
    }
}
...
{
    MS_BOOL bNeedSTR  = FALSE;
    if(pVPUHalContext)
    {
        switch(pVPUHalContext->_stVPUStream[u8TaskId].eDecodertype)
        {
            case E_VPU_EX_DECODER_MVD:
                bNeedSTR  = pVPUHalContext->stMVD_SLQ_STATE[u8TaskId].bTSP;
                break;
            case E_VPU_EX_DECODER_EVD:
                bNeedSTR  = pVPUHalContext->stEVD_BBU_STATE[u8TaskId].bTSP;
                break;
            case E_VPU_EX_DECODER_HVD:
                bNeedSTR  = pVPUHalContext->stHVD_BBU_STATE[u8TaskId].bTSP;
                break;
            case E_VPU_EX_DECODER_NONE:
            case E_VPU_EX_DECODER_MJPEG:
            case E_VPU_EX_DECODER_RVD:
            case E_VPU_EX_DECODER_MVC:
            case E_VPU_EX_DECODER_VP8:
            #if SUPPORT_G2VP9
            case E_VPU_EX_DECODER_G2VP9:
            #endif
            default:
...
MS_U32 HAL_VPU_EX_GetStrOffset(MS_U32 u32CodeBufSize)
{
    if(u32CodeBufSize == 0)
    {
        u32CodeBufSize = _u32VDECFWSize;
    }
    if( _u32StrAddrOffset == 0)
    {
        _u32StrAddrOffset = (u32CodeBufSize < VDEC_STR_BUFFER_DUAL_OFFSET)?VDEC_STR_BUFFER_SINGLE_OFFSET:VDEC_STR_BUFFER_DUAL_OFFSET;
    }
    return _u32StrAddrOffset;
}
...
void HAL_VPU_EX_EnableSTRResume(MS_U8 u8TaskId,MS_BOOL bEnable)
{
    if(pVPUHalContext)
    {
        #define VDEC_STR_MAIN_RESUME _u32StrAddrOffset+VDEC_STR_MAIN_RESUME_OFFSET
        #define VDEC_STR_SUB_RESUME _u32StrAddrOffset+VDEC_STR_SUB_RESUME_OFFSET
        MS_U32 u32ResumeOffSet = (u8TaskId == 0) ? VDEC_STR_MAIN_RESUME:VDEC_STR_SUB_RESUME;
        MS_U8* u8Resume = (MS_U8*)MsOS_PA2KSEG1(pVPUHalContext->u32FWCodeAddr+u32ResumeOffSet);
        if(u8Resume)
        {
            u8Resume[0] = bEnable;
            _bSuspend = bEnable;
...
MS_BOOL HAL_VPU_EX_IsSTRResume(MS_U8 u8TaskId)
{
    ...
    if(pVPUHalContext)
    {
        ...
        {
            MsOS_ReadMemory();
            bIsResume = u8Resume[0];
        }
    }
    #endif
    return bIsResume;
}
//This function is designed for the HVD part, it needs to disable the blueScreen
MS_U8 HAL_VPU_EX_IsSTR(MS_U8 u8TaskId)
{
    MS_BOOL bIsSTR = FALSE;
    #ifdef SUPPORT_VDEC_STR
    bIsSTR = _bSuspend;
    if(_bSuspend)
    {
        _bSuspend = FALSE;// it is just to use for setting bluescreen after getting decode one event
    }
    #endif
    return bIsSTR;
}
