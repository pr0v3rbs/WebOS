...
#define SEAL_NON_PM_GROUP11 \
    ...
    E_SEAL_USBC1_PROT_NONPM, \
    E_SEAL_USBC2_PROT_NONPM, \
    E_SEAL_USBC3_PROT_NONPM, \
...
)
{
    ...
    // Set start address
    HAL_SEAL_NonPmWrite4Byte(u32RegStartAddr, u64StartAddr);

    // Set end address
    HAL_SEAL_NonPmWrite4Byte(u32RegEndAddr, u64EndAddr);
...
MS_BOOL HAL_Seal_SecureRangeLock(MS_U8 u8MiuDev, MS_U8 u8SecureRangeId)
{
    ...
    u16Val = HAL_SEAL_NonPmReadByte(u32RegAddr);
    u16Val = u16Val | (1 << u8SecureRangeId);
    HAL_SEAL_NonPmWriteByte(u32RegAddr, u16Val);
...
MS_BOOL HAL_Seal_SecureSlaveSet(MS_U32 u32SlaveId, MS_BOOL bSecure)
{
    ...
    if(s32HwIdx == -1)
    {
        ...
        if(s32HwIdx == -1)
        {
            HAL_SEAL_ERR("Slave ID:%ld doesn't exist !\n", u32SlaveId);
            return FALSE;
        }
    }
...
MS_BOOL HAL_Seal_SecureMasterSet(MS_U32 u32MasterId, MS_BOOL bSecure)
{
    ...
    if(s32HwIdx == -1)
    {
        HAL_SEAL_ERR("Master ID:%ld doesn't exist !\n", u32MasterId);
        return FALSE;
    }
...
MS_BOOL HAL_Seal_SecureMasterQuery(MS_U32 u32MasterId, MS_BOOL *pSecure)
{
    ...
    if(s32HwIdx == -1)
    {
        HAL_SEAL_ERR("PM Master ID:%ld doesn't exist !\n", u32MasterId);
        return FALSE;
    }
...
MS_BOOL HAL_Seal_BufferLock(MS_U8 u8BufferLockId, MS_BOOL bLock)
{
    ...
    if(s32HwIdx == -1)
    {
        HAL_SEAL_ERR("Secure buffer lock ID:%d doesn't exist !\n", u8BufferLockId);
        return FALSE;
    }
