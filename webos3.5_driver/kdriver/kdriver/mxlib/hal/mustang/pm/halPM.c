...
void HAL_PM_SetSPIOffsetForMCU(MS_U32 BANK)
{
    ...
    HAL_PM_Write2Byte(0x2e52UL,0x403FUL);   //PM 51 rst
...
void HAL_PM_SetSRAMOffsetForMCU(void)
{
     ...
     HAL_PM_WriteByte(0x0eabUL, 0x00UL);//Clear 0x87
    ...
    while(HAL_PM_ReadByte(0x0eabUL)!=0x87UL);
    printf("PM51 run ok...........\n");
}
...
void HAL_PM_SPI(MS_BOOL bEN)
{
    if(bEN > 0)
        HAL_PM_WriteRegBit(0x001018UL, ENABLE, __BIT1); //SPI  enable
    else
        HAL_PM_WriteRegBit(0x001018UL, DISABLE, __BIT1); //SPI  disable
}
//2016.08.18 hank added end
//2016.08.22 kelvin added start
void HAL_PM_WholeChipReset(void)
{
    HAL_PM_WriteByte(0x002e5cUL, 0x79); //whole chip reset
}
