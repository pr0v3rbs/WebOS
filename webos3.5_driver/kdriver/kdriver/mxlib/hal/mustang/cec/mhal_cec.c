...
MS_U8 mhal_CEC_SendFrame(MS_U8 header, MS_U8 opcode, MS_U8* operand, MS_U8 len)
{
    ...
    MDrv_WriteByte(H(REG_HDMI_INT_CLEAR), 0x0E);
    MDrv_WriteByte(H(REG_HDMI_INT_CLEAR), 0x00);

    MDrv_WriteByte( L(REG_HDMI_CEC_TX_DATA0) , header );
    MDrv_WriteByte( H(REG_HDMI_CEC_TX_DATA0) , opcode );
    ...
    if(len > 0)
    {
        ...
        for(i=0;i<len;i++)
        {
            MDrv_WriteByte( REG_HDMI_CEC_TX_DATA1+i , *(ptr+i) );
            CEC_DPRINTF(" 0x%x", *(operand+i));
        }
        CEC_DPUTSTR("\r\n/**************************/\r\n");
    }

    if((MDrv_ReadByte(L(__REG_HDMI(0x6D))) == 1) && (MDrv_ReadByte(H(__REG_HDMI(0x6D))) == 0)) // CEC idle
    {
        // CEC transmit length
        //if((opcode==0x00)&&(operand==NULL)&&(len==0))
        if((opcode==0x00)&&(len==0))
        {
            MDrv_WriteByte(L(__REG_HDMI(0x68)),0);                   //polling message
            u8waitcnt = 5;
        }
        else
        {
            MDrv_WriteByte(L(__REG_HDMI(0x68)),(len+1));
            u8waitcnt = 4 * (len+2);
        }
        ...
        do
        {
            ...
            if(cnt++>=u8waitcnt)
                break;
        } while((MDrv_ReadByte(H(REG_HDMI_INT_STATUS))&0x0E)==0);

        res = (MDrv_ReadByte(H(REG_HDMI_INT_STATUS))&0x0E);
...
{
    MDrv_WriteByteMask(L(__REG_HDMI(0x6A)), mylogicaladdress<<4, 0xF0);
}

void mhal_CEC_INTEn(MS_BOOL bflag)
{
    if(bflag) // unmask
        W2BYTEMSK(REG_HDMI_INT_MASK, 0x0000, 0x0100);  //REG_HDMI_INT_MASK
    else // Mask CEC interrupt
        W2BYTEMSK(REG_HDMI_INT_MASK, 0x0100, 0x0100);  //REG_HDMI_INT_MASK
}
...
void mhal_CEC_Init(MS_U32 u32XTAL_CLK_Hz)
{
...
#if ENABLE_CEC_INT
    // DVI irq clear control.
    MDrv_WriteByte( REG_DVI_ATOP_71_H, 0xFF);
    MDrv_WriteByte( REG_DVI_ATOP_71_H, 0);
    MDrv_WriteByte( REG_DVI_ATOP_73_H, 0xFF);
    MDrv_WriteByte( REG_DVI_ATOP_73_H, 0);
    MDrv_WriteByte( REG_DVI_ATOP1_71_H, 0xFF);
    MDrv_WriteByte( REG_DVI_ATOP1_71_H, 0);
    MDrv_WriteByte( REG_DVI_ATOP2_71_H, 0xFF);
    MDrv_WriteByte( REG_DVI_ATOP2_71_H, 0);
    // DVI irq mask control - disable all DVI irq.
    MDrv_WriteByte( REG_DVI_ATOP_70_H, 0xFF);
    MDrv_WriteByte( REG_DVI_ATOP_72_H, 0xFF);
    MDrv_WriteByte( REG_DVI_ATOP1_70_H, 0xFF);
    MDrv_WriteByte( REG_DVI_ATOP2_70_H, 0xFF);

    // DVI EQ irq clear control
    MDrv_WriteByte( REG_DVI_EQ_00_H, 0x0F);
    MDrv_WriteByte( REG_DVI_EQ_00_H, 0);
    MDrv_WriteByte( REG_DVI_EQ1_00_H, 0x0F);
    MDrv_WriteByte( REG_DVI_EQ1_00_H, 0);
    MDrv_WriteByte( REG_DVI_EQ2_00_H, 0x0F);
    MDrv_WriteByte( REG_DVI_EQ2_00_H, 0);
    MDrv_WriteByte( REG_DVI_EQ3_00_H, 0x0F);
    MDrv_WriteByte( REG_DVI_EQ3_00_H, 0);
    // DVI EQ mask control
    MDrv_WriteByte( REG_DVI_EQ_00_L, 0x0F);
    MDrv_WriteByte( REG_DVI_EQ1_00_L, 0x0F);
    MDrv_WriteByte( REG_DVI_EQ2_00_L, 0x0F);
    MDrv_WriteByte( REG_DVI_EQ3_00_L, 0x0F);

    // HDCP IRQ clear
    MDrv_WriteByte( REG_HDCP_01_H, 0xFF); // HDCP status clear
    MDrv_WriteByte( REG_HDCP_15_L, 0x03); // DDC 0x74/0x76 clear
    MDrv_WriteByte( REG_HDCP_15_L, 0x00);
    // HDCP IRQ mask
    MDrv_WriteByte( REG_HDCP_15_H, 0xFF); // HDCP event mask
    MDrv_WriteByte( REG_HDCP_15_L, 0x30); // DDC 0x74/0x76 mask
    ...
    MDrv_WriteByte(REG_HDMI2_27_H, 0xFF); // HDMI2 interrupt clear
    MDrv_WriteByte(REG_HDMI2_27_H, 0x00);
    MDrv_WriteByte(REG_HDMI2_26_H, 0xFF); // HDMI2 interrupt mask
    // MHL TMDS irq clear
    MDrv_Write2Byte( REG_MHL_TMDS_63_L, 0xFFFF);
    MDrv_Write2Byte( REG_MHL_TMDS_63_L, 0);
    // MHL TMDS irq mask control -only enable CEC rx irq
    MDrv_Write2Byte( REG_MHL_TMDS_60_L, 0xFEFF);

    gCECIRQ = E_INT_IRQ_DVI_HDMI_HDCP; // IRQ52
#endif

    // CEC Clock not gating
    reg_val=MDrv_ReadByte(L(__REG_HDMI(0x69)));
    MDrv_WriteByte(L(__REG_HDMI(0x69)), reg_val&(0x7F));

     reg_val=(u32XTAL_CLK_Hz %100000l)*0.00016+0.5;

     MDrv_Write2Byte(H(__REG_HDMI(0x6A)), (reg_val<<8)+u32XTAL_CLK_Hz/100000l);

    //reg_val=MDrv_ReadByte(H(__REG_HDMI(0x68)))&(~0x07);
    MDrv_WriteByte(H(__REG_HDMI(0x68)), 0x10|g_u8RetryCnt);
    reg_val=(FrameInterval<<8)|(BusFreeTime<<4)|(ReTxInterval);
    MDrv_Write2Byte(H(__REG_HDMI(0x69)), reg_val);

    MDrv_WriteByte(L(REG_HDMI_CEC_STATUS1), 0x07);
}

MS_BOOL mhal_CEC_IsMessageReceived(void)
{
    return (MDrv_ReadByte(H(REG_HDMI_INT_STATUS)) & 0x01 ? TRUE : FALSE);
}

MS_U8 mhal_CEC_ReceivedMessageLen(void)
{
    return ((MDrv_ReadByte(H(REG_HDMI_CEC_STATUS1)) & 0x1F) + 1);
}

MS_U8 mhal_CEC_GetMessageByte(MS_U8 idx)
{
    return (MDrv_ReadByte(L(REG_HDMI_CEC_RX_DATA0) + idx));
}

void mhal_CEC_ClearRxStatus(void)
{
    // clear RX INT status
    MDrv_WriteByte( H(REG_HDMI_INT_CLEAR), 0x01);
    MDrv_WriteByte( H(REG_HDMI_INT_CLEAR), 0);
    // clear RX NACK status
    MDrv_WriteByte(L(REG_HDMI_CEC_STATUS1), 0x04);
}
...
void mhal_CEC_ConfigWakeUp(void)
{
    ...
    //XBYTE[0x250C] &= ~BIT0;
    MDrv_WriteByte( REG_DVI_ATOP_06_L, MDrv_ReadByte(REG_DVI_ATOP_06_L) &(~ BIT(0)));
...
{
    if(bEnableFlag)
        MDrv_WriteByteMask(L(__REG_HDMI(0x69)), 0x00, 0x80);//No gate.
    else
        MDrv_WriteByteMask(L(__REG_HDMI(0x69)), 0x80, 0x80);//Gate.
}

MS_U8 mhal_CEC_TxStatus(void)
{
    return (MDrv_ReadByte(H(REG_HDMI_INT_STATUS))&0x0E);
}
