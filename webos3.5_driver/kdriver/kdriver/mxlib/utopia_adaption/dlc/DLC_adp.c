...
MS_U32 DLC_adp_Init(void* pIoctl)
{
...
//set table
    ...
    UADPBypassSetSPT(&spt_DBC_DECODE_EXT_CMD[1],UADP_SPT_POINTER_TO_NEXT, ((unsigned int)(&(pDrvDLC_CONTROL_PARAMS->params)) - (unsigned int)pDrvDLC_CONTROL_PARAMS), &spt_DrvDLC_CONTROL_PARAMS);
    UADPBypassSetSPT(&spt_DBC_DECODE_EXT_CMD[2],UADP_SPT_END , 0, 0);

    pIoctl= DLC_adp_Ioctl;
