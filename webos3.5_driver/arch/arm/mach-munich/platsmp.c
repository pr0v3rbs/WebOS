...
extern int platform_boot_secondary(unsigned int cpu, struct task_struct *idle);
#else
...
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
    writew(0xbabe, (volatile void __iomem *)SECOND_MAGIC_NUMBER_ADRESS);
    writew(virt_to_phys(vexpress_secondary_startup) & 0xFFFF, (volatile void __iomem *)(SECOND_START_ADDR));
    writew(virt_to_phys(vexpress_secondary_startup) >> 16, (volatile void __iomem *)(SECOND_START_ADDR + 4));
#else
    writew(0xbabe, SECOND_MAGIC_NUMBER_ADRESS);
    writew(BSYM(virt_to_phys(vexpress_secondary_startup)) & 0xFFFF, SECOND_START_ADDR);
    writew(BSYM(virt_to_phys(vexpress_secondary_startup)) >> 16, SECOND_START_ADDR + 4);
#endif /* CONFIG_MSTAR_PORTING_KERNEL_4_4_3 */
...
{
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
    writew(0, (volatile void __iomem *)SECOND_MAGIC_NUMBER_ADRESS);
    writew(0, (volatile void __iomem *)SECOND_START_ADDR);
    writew(0, (volatile void __iomem *)SECOND_START_ADDR + 4);
#else
    writew(0, SECOND_MAGIC_NUMBER_ADRESS);
    writew(0, SECOND_START_ADDR);
    writew(0, SECOND_START_ADDR + 4);
#endif /* CONFIG_MSTAR_PORTING_KERNEL_4_4_3 */
    __cpuc_flush_kern_all();
    return 0;
}
