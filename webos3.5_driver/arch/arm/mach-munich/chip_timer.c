...
#define LG_PRECISELY_TIMER

//ARM Global Timer
#ifdef LG_PRECISELY_TIMER
static unsigned int GLB_TIMER_FREQ_HZ;
#else
...
void __init chip_clocksource_init(unsigned int base)
{
	 ...
	 */
#ifdef LG_PRECISELY_TIMER
	cs->mult = clocksource_hz2mult((u32)(GLB_TIMER_FREQ_HZ), cs->shift);
#else
...
#ifdef LG_PRECISELY_TIMER
    fclk = clk_register_fixed_rate(NULL, "fclk", NULL, CLK_IS_ROOT, (GLB_TIMER_FREQ_HZ * 2) - 1);
...
void __init chip_init_timer(void)
{
...
#elif defined(CONFIG_MP_STATIC_TIMER_CLOCK_SOURCE)
	...
	switch (cpu_freq) {
		...
		break;
	    default:
		GLB_TIMER_FREQ_HZ = cpu_freq*1000*(1000/2);
...
#ifdef LG_PRECISELY_TIMER
/* mstar_fix_0001: type casting (long unsigned int) */
	printk("\033[35m--Global Timer Frequency = %lu Hz\033[m\n", (long unsigned int)GLB_TIMER_FREQ_HZ);
/* mstar_fix_0001: type casting (long unsigned int) */
	printk("\033[35m--CPU Clock Frequency = %lu Hz\033[m\n", (long unsigned int)GLB_TIMER_FREQ_HZ*2);
#else
...
#else
#ifdef LG_PRECISELY_TIMER
	clocks_calc_mult_shift(&mult, &shift, (u32)GLB_TIMER_FREQ_HZ, NSEC_PER_SEC, 0);
...
#ifdef LG_PRECISELY_TIMER
	mstar_sched_clock_init((void __iomem *)(PERI_VIRT+0x200), (unsigned long)GLB_TIMER_FREQ_HZ);
#else
