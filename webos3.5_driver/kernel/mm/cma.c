...
struct page *cma_alloc_direct(struct cma *cma, size_t count, unsigned int align,
		long *retlen)
{
	...
	for (;;) {
		...
		*/
		...
		mutex_lock(&cma_mutex);
		ret = alloc_contig_range(pfn, pfn + len);
		mutex_unlock(&cma_mutex);
		if (ret == 0) {
			page = pfn_to_page(pfn);
			break;
...
struct page *cma_alloc_addr(struct cma *cma, unsigned long start, size_t count,
		unsigned int align)
{
	unsigned long mask;
	struct page *page = NULL;
	...
	mutex_lock(&cma_mutex);
	ret = alloc_contig_range(start_pfn, start_pfn + count);
	mutex_unlock(&cma_mutex);
...
unsigned long cma_get_base_pfn(const struct cma *cma)
{
	return cma->base_pfn;
}

unsigned long cma_get_count(const struct cma *cma)
{
	return cma->count;
}
...
struct mutex *cma_get_lock(const struct cma *cma)
{
	return (struct mutex *)&cma->lock;
}
