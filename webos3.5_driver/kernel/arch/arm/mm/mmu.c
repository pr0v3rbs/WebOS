...
#endif /* CONFIG_MP_SYNC_3_1_10_SETTING_VMALLOC_SETTING */
...
// cyli add to fix lowmem size as miu0 size
#ifdef CONFIG_VMSPLIT_2G_MSTAR_PATCH
        if (reg == memblock.memory.regions) {
            vmalloc_min = (void *)(PAGE_OFFSET + reg->size);
            ...
            printk("fix lowmem size as miu0 size, vmalloc_min = 0x%p\n"
                   "                            vmalloc_limit = 0x%p\n",
                   (void *)vmalloc_min, (void *)vmalloc_limit);
        }
#endif /* CONFIG_VMSPLIT_2G_MSTAR_PATCH */
...
void __init map_driver_mem(unsigned long drv_ba, unsigned long len, unsigned long va, bool non_cache)
#else
