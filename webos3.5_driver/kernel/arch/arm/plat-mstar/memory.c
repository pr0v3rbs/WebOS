...
unsigned int  uPanelType_Innolux=0;
...
EXPORT_SYMBOL(uPanelType_Innolux);
...
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
/* migrate to using memblock directly instead of meminfo as an intermediate */
#else
struct meminfo {
        int nr_banks;
...
static int __init str_innolux_setup(char *str)
{
    if( str != NULL )
    {
        uPanelType_Innolux = simple_strtol(str, NULL, 16);
    }
    else
    {
        printk("uPanelType_Innolux is disable\n");
    }
    printk("uPanelType_Innolux = %d\n", uPanelType_Innolux);
    return 0;
}
...
early_param("innolux", str_innolux_setup);
...
int get_str_innolux(void)
{
    return uPanelType_Innolux;
}
EXPORT_SYMBOL(get_str_innolux);
