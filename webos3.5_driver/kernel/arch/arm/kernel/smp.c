...
#ifdef CONFIG_MP_PLATFORM_ARM
static inline void arch_virt_counter_set_user_access(void)
{
#ifdef CONFIG_ARCH_MSTAR_CA9
	/*
	enable PMUSERENR.EN, bit[0]
	*/
        u32 cntkctl;
        /* enable user-mode access to the performance counter */
        asm volatile("mrc p15, 0, %0, c9, c14, 0" : "=r" (cntkctl));
        cntkctl |= 1;
        asm volatile("mcr p15, 0, %0, c9, c14, 0" : : "r" (cntkctl));
	/* enable all counters */
...
#ifdef CONFIG_MP_PLATFORM_ARM
    arch_virt_counter_set_user_access();
#endif /* CONFIG_MP_PLATFORM_ARM */
	local_irq_enable();
	local_fiq_enable();
	local_abt_enable();
