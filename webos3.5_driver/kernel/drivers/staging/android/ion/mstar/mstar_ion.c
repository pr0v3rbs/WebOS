...
struct page* mali_alloc_page(void)
{
	...
	if(s_current_mali_alloc_strategy != mali_alloc_strategy){
		//glen_test
		printk("cma_mali: s_current_mali_alloc_strategy != mali_alloc_strategy\n");
	...
	if(s_mali_current_heap){
		printk("cma_mali: s_mali_current_heap\n");
...
static int cmarunlist_proc_show(struct seq_file *m, void *v)
{
	...
	unsigned long cma_count, *cma_bitmap;

	if(!cma)
		return 0;

	printk("mutex_lock_4");
	mutex_lock(cma_get_lock(cma));

	cma_count = cma_get_count(cma);
	cma_bitmap = cma_get_bitmap(cma);

	frst_bit = bitmap_find_next_zero_area(cma_bitmap, cma_count, 0, 1, 0);
	if (this_bit >= cma_count){
		goto out;
	}
	this_bit = frst_bit;
	last_bit = frst_bit;
	...
	while(1){
		this_bit = bitmap_find_next_zero_area(cma_bitmap, cma_count, this_bit + 1, 1, 0);
		if (this_bit >= cma_count){
...
out:
	mutex_unlock(cma_get_lock(cma));
	printk("mutex_unlock_4");
	return 0;
}
...
static int cmarunlist_proc_open(struct inode *inode, struct file *file)
{
	...
	if(cma){
		freebits = get_free_bit_count(cma, 0, cma_get_count(cma));
		printk("cma %s, total page %lu, free page %lu\n", heap->name,
			cma_get_count(cma), freebits);
...
static int cmabitmap_proc_show(struct seq_file *m, void *v)
{
	...
	set_fs(get_ds());
	ret = vfs_write(filp, (char __user *)cma_get_bitmap(cma),
		cma_get_count(cma)/8, &pos);
