...
unsigned long  get_free_bit_count(struct cma *cma, unsigned long start, unsigned long len)
{
	...
	unsigned long cma_count = cma_get_count(cma);
	unsigned long *cma_bitmap = cma_get_bitmap(cma);

	if(len==0 || start >= cma_count)
		return 0;

	end = start+len;
	end = min(end, cma_count);

	offset_zero = start;

	while(offset_zero < cma_count){
		unsigned offset_bit;
		offset_zero = find_next_zero_bit(cma_bitmap, end, offset_zero);
		if(offset_zero>= end)
			break;
		offset_bit = find_next_bit(cma_bitmap, end, offset_zero+1);
...
static int cma_increase_pool_freelist_threshold(struct ion_mstar_cma_heap *mstar_cma_heap, unsigned long len)
{
	...
	struct page *page;
	unsigned long pfn, totalfreebits, cma_base_pfn, cma_count;
	int ret = -ENOMEM;
	...
	while(mstar_cma->cache_size < len){
		...
		cma_base_pfn = cma_get_base_pfn(mstar_cma->cma);
		cma_count = cma_get_count(mstar_cma->cma);

		CMA_BUG_ON((page && !retlen) || (!page && retlen),
			"alloc out of cma area\n");
		CMA_BUG_ON(pfn < cma_base_pfn, "alloc out of cma arean");
		CMA_BUG_ON(pfn >= cma_base_pfn + cma_count, "alloc out of cma area\n");
...
int __mstar_free_one_page(struct ion_heap *heap,struct page *page)
{
	...
	if(heap){
		if(!test_and_clear_bit(pfn - cma_get_base_pfn(mstar_cma->cma),
			cma_get_bitmap(mstar_cma->cma)))
			BUG();
	}
...
ALLOC:
	...
	if (!buffer_info->page) {
		if (retry < CMA_CONTIG_RTYCNT)
		{
			...
			freepg = get_free_bit_count(mstar_cma->cma, 0,
						cma_get_count(mstar_cma->cma))
					+ mstar_cma->cache_page_count;
					...
					int nr_to_scan)
{
	...
	struct page *page;
	struct cma_cache_node *node, *tmp;
	unsigned long cma_base_pfn = cma_get_base_pfn(mstar_cma->cma);
	unsigned long cma_count = cma_get_count(mstar_cma->cma);

	if (gfpflags_to_migratetype(gfp_mask) != MIGRATE_MOVABLE)
		return 0;
	if(!mutex_trylock(cma_get_lock(mstar_cma->cma)))
		goto OUT_NO_LOCK;
	...
	list_for_each_entry_safe(node, tmp, &mstar_cma->cache_head, list){
		...
		CMA_BUG_ON(pfn < cma_base_pfn || pfn >= cma_base_pfn + cma_count,
			"pfn=%lx,cma[%lx,%lx] \n", pfn, cma_base_pfn, cma_count);

		/*
		 *Free entire node, so freed may be a little larger than @nr_to_scan
		 */
		while(len < node->len){
			if(!test_and_clear_bit(pfn - cma_base_pfn,
				cma_get_bitmap(mstar_cma->cma))){
				BUG();
			}
			page = pfn_to_page(pfn);
	...
	spin_unlock(&mstar_cma->cache_lock);
	mutex_unlock(cma_get_lock(mstar_cma->cma));
...
static int ion_mstar_cma_shrink_lock(struct ion_heap *heap)
{
	...
	mutex_lock(cma_get_lock(mstar_cma->cma));
	spin_lock(&mstar_cma->cache_lock);
...
struct ion_heap *ion_mstar_cma_heap_create(struct ion_platform_heap *data)
{
	...
	struct mstar_cma *mstar_cma = NULL;
	...
	if (!dev_get_cma_area(mstar_cma_heap->dev))
		goto err;

	mstar_cma->cma = dev_get_cma_area(mstar_cma_heap->dev);
	mstar_cma->zone = page_zone(pfn_to_page(
		cma_get_base_pfn(mstar_cma->cma)));
	init_waitqueue_head(&mstar_cma->cma_swap_wait);
	mutex_init(&mstar_cma->contig_rtylock);

	mstar_cma->cache_size = 0;
	mstar_cma->cache_page_count = 0;
	mstar_cma->fail_alloc_count = 0;
	INIT_LIST_HEAD(&mstar_cma->cache_head);
	spin_lock_init(&mstar_cma->cache_lock);
...
static int get_cma_freebits(struct cma *cma)
{
	...
	while(1){
		pos = find_next_zero_bit(cma_bitmap, cma_count, start);

		if(pos >= cma_count)
			break;
		start = pos + 1;
		next = find_next_bit(cma_bitmap, cma_count, start);

		if(next >= cma_count){
			total_free += (cma_count - pos);
			break;
		}
...
{
	int freebits;
	struct mutex *cma_lock = cma_get_lock((const struct cma *)system_cma);

	mutex_lock(cma_lock);
	freebits = get_cma_freebits(system_cma);
	mali_heap_info[0] = cma_get_count(system_cma) - freebits;	// alloc
	mali_heap_info[1] = 0;
	mali_heap_info[2] = 0;
...
void get_cma_heap_info(struct ion_heap *heap, int *mali_heap_info, char *name)
{
	...
	spin_lock(&mstar_cma->cache_lock);
	freebits = get_cma_freebits(mstar_cma->cma);
	mali_heap_info[0] = cma_get_count(mstar_cma->cma) - freebits;
	mali_heap_info[1] = mstar_cma->cache_page_count;
	mali_heap_info[2] = mstar_cma->fail_alloc_count;
