...
#ifdef CONFIG_MSTAR_MMC
#include "eMMC.h"
#endif /* CONFIG_MSTAR_MMC */
...
#ifdef CONFIG_MSTAR_PORTING_PLAT
extern void mmc_set_clock(struct mmc_host *host, unsigned int hz);
extern int mmc_send_cxd_data_export(struct mmc_card *card,
    struct mmc_host *host, u32 opcode, void *buf, unsigned len);
	...
	unsigned int user_cmd, unsigned long user_arg)
{
			...
			(_IOC_NR(user_cmd) == 15))
	{
	    ...
	    if(_IOC_NR(user_cmd) == 4)
	    {
             ...
             err = mmc_send_cxd_data_export(card, card->host, MMC_SEND_EXT_CSD, buff,
                                     512);

             if(err)
			     goto cmd_rel_host;

			 if(copy_to_user((void __user*) user_arg,(void *)buff, 512))
			 {
				err = -EFAULT;
				goto cmd_rel_host;
			 }
		 ...
		 else if(_IOC_NR(user_cmd) == 7)
	 {
		 ...
		 if(copy_to_user((void __user*) user_arg,(void *) card->raw_cid, sizeof(card->raw_cid)))
		 {
			err = -EFAULT;
			goto cmd_rel_host;
		 }
	     ...
	     else if(_IOC_NR(user_cmd) == 8)
	     {
			 ...
			 if(copy_from_user((void *)buff,(void __user*) user_arg, 512))
			 {
				 err = -EFAULT;
				 goto cmd_rel_host;
			 }
         ...
         else if(_IOC_NR(user_cmd) == 15)
	 {
		 printk(KERN_ERR "mmc_blk_ioctl_cmd_simple -> Get CSD\n");

		 if(copy_to_user((void __user*) user_arg,(void *) card->raw_csd, sizeof(card->raw_csd)))
		 {
			err = -EFAULT;
			goto cmd_rel_host;
		 }
	      ...
	      (_IOC_NR(user_cmd) == 11))
	{
		...
		if (!buff)
		{
		  printk(KERN_ERR "alloc 0x%08lu bytes buffer failed!\n", normal_cmd[2]);
			  err = -ENOMEM;
			  return err;
		}
		...
		{
		    if (copy_from_user((void *)buff, (void __user*)normal_cmd[1], normal_cmd[2]))
		    {
			    err = -EFAULT;
			    goto cmd_rel_host;
		    }
		   ...
		   ((_IOC_NR(user_cmd) == 11)&&(normal_cmd[0] & 0x1)))
		{
		    ...
		    if (copy_to_user((void __user*)normal_cmd[1], (void *)buff, normal_cmd[2]))
			{
			    err = -EFAULT;
			    goto cmd_rel_host;
		    }
	...
	else if(_IOC_NR(user_cmd) == 13)
	{
	    printk(KERN_ERR "mmc_blk_ioctl_cmd_simple -> mmc_release_host\n");
	    mmc_release_host(card->host);
	    return err;
	}
	else if(_IOC_NR(user_cmd) == 14)
	{
	    printk(KERN_ERR "mmc_blk_ioctl_cmd_simple -> mmc_clock_set:%lu\n", user_arg);
	    mmc_set_clock(card->host, user_arg);
	    return err;
	}
    else if(_IOC_NR(user_cmd) >= 64)
    {
        err = mmc_blk_ioctl_cmd_rpmb(bdev, user_cmd, user_arg);
        return err;
    }
...
#endif /* CONFIG_MSTAR_PORTING_PLAT */
...
struct mmc_ext_ioctl_subcommand {
	unsigned char type;
	unsigned char mmc_opcode;
	unsigned char mmc_flags;
	unsigned char data_rw;
...
struct mmc_ext_ioctl {
	struct mmc_ext_ioctl_header header;
	struct mmc_ext_ioctl_subcommand subcmd[15];
	unsigned char reserved[16];
} __attribute__((packed));

#ifndef CONFIG_MSTAR_PORTING_PLAT
	...
	unsigned int cmd, unsigned long arg)
{
	...
	default:
#ifdef CONFIG_MSTAR_PORTING_PLAT
		if(_IOC_TYPE(cmd) == MMC_BLOCK_MAJOR) {
			return mmc_blk_ioctl_cmd_simple(bdev, cmd, arg);
		} else {
			return -EINVAL;
		}
#else
