...
#if defined(CONFIG_MTD_UBI_ENHANCE_INIT) && defined(CONFIG_MP_NAND_UBI)
#include <linux/init.h>
#endif /* CONFIG_MTD_UBI_ENHANCE_INIT && CONFIG_MP_NAND_UBI */

#if defined(CONFIG_MTD_UBI_BACKUP_LSB) && defined(CONFIG_MP_NAND_UBI)
#include <linux/mtd/nand.h>
#include "drv_unfd.h"

struct ubi_paired_page_map ga_tUbiPairedPageMap[512];
#endif /* CONFIG_MTD_UBI_BACKUP_LSB && CONFIG_MP_NAND_UBI */
...
#if defined (CONFIG_MTD_UBI_BITFLIPS) && defined(CONFIG_MP_NAND_UBI)
	if (err && err != UBI_IO_BITFLIPS &&  err != UBI_IO_BITFLIPS_BAD && err != UBI_IO_BITFLIPS_TORTURE && !mtd_is_eccerr(err))
#else
	if (err && err != UBI_IO_BITFLIPS && !mtd_is_eccerr(err))
#endif /* CONFIG_MTD_UBI_BITFLIPS && CONFIG_MP_NAND_UBI */
		goto out_unlock;
...
int process_tbl_page(struct ubi_device *ubi, struct ubi_attach_info *ai, __u8 *pu8_DataBuf, __u32 page_idx)
{
	...
	for(idx = 0; idx < ttl_rcd; idx ++)
	{
		...
		if (vol_id > UBI_MAX_VOLUMES && vol_id != UBI_LAYOUT_VOLUME_ID) {
			...
			case UBI_COMPAT_RO:
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
				ubi_msg(ubi, "read-only compatible internal volume %d:%d"
			...
			case UBI_COMPAT_PRESERVE:
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
				ubi_msg(ubi, "\"preserve\" compatible internal volume %d:%d"
					" found", vol_id, lnum);
#else
...
//		ubi_msg("[%d] lsb(%d), paired_page(%d)\r\n", j, ga_tUbiPairedPageMap[j].lsb, ga_tUbiPairedPageMap[j].paired_page);
	...
	if (!sv) {
		 ...
		 */
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
		ubi_msg(ubi, "the backup volume was not found");
#else
		ubi_msg("the backup volume was not found");
#endif /* CONFIG_MSTAR_PORTING_KERNEL_4_4_3 */
	} else {
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
		ubi_msg(ubi, "check backup volume(%d):%d", UBI_BACKUP_VOLUME_ID, sv->vol_id);
#else
...
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
				ubi_msg(ubi, "the backup volume was scrubbed or WL, no need to restore");
#else
...
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
		ubi_msg(ubi, "Spare Strut page: %X, sqnum: %X, vol_id: %X, pnum: %X, lnum: %X",
		...
		ubi_msg(ubi, "backup @pnum %d, offset %d", pnum, ubi->backup_next_offset);
		ubi_msg(ubi, "backup source @pnum %d, lnum %d, vol_id %d, page %d, sq %d",
		...
		for(i=0 ; i<sqnum ; i++) {
			// read backup page
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
			ubi_msg(ubi, "check backup @pnum %d, offset %d", pnum, ubi->backup_next_offset-(i+1)*ubi->min_io_size);
#else
			...
			// read source page
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
			ubi_msg(ubi, "check source @pnum %d, offset %d", source_pnum, source_page*ubi->min_io_size);
#else
			...
			if(err < 0) {
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
				ubi_msg(ubi, "source @pnum %d, offset %d correct=%d", source_pnum, source_page*ubi->min_io_size, err);
#else
		...
		if(corrupt) {
			...
			if (!sv) {
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
				ubi_msg(ubi, "volume id %d was not found", source_vol_id);
#else
			...
			if(recovery != RECOVERY_NONE) {
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
				ubi_msg(ubi, "recovery from %d", recovery);
#else
				...
				for(offset=0 ; offset<data_size; offset+=ubi->min_io_size) {
					...
					if(err < 0)
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
						ubi_warn(ubi, "error %d while reading data from PEB %d:%d", err, old_seb->pnum, offset);
#else
				...
				for(i=0 ; i<sqnum ; i++) {
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
					ubi_msg(ubi, "read backup(%d) from %d", pnum, ubi->backup_next_offset-(i+1)*ubi->min_io_size);
#else
...
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
						ubi_msg(ubi, "copy backup page %d to offset %d", source_page, (source_page*ubi->min_io_size)-ubi->leb_start);
#else
...
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
				ubi_msg(ubi, "calc CRC data size %d", data_size);
#else
			...
			else
			{
				...
				if (old_seb->pnum == source_pnum) {
					list_del(&old_seb->u.list);
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
					ubi_msg(ubi, "add corrept peb %d, ec %d from free to erase list", old_seb->pnum, old_seb->ec);
#else
				...
				if (old_seb->pnum == source_pnum) {
					list_del(&old_seb->u.list);
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
					ubi_msg(ubi, "add corrept peb %d, ec %d from alien to erase list", old_seb->pnum, old_seb->ec);
#else
