...
void Titania3_series_start_ehc(uintptr_t UTMI_base, uintptr_t USBC_base, uintptr_t UHC_base, unsigned int flag)
{
	...
	writeb(readb((void*)(UTMI_base))|0xC2, (void*)(UTMI_base));
	...
	writeb(readb((void*)(UTMI_base))&0x3D, (void*)(UTMI_base));
...
static int ehci_hcd_mstar_drv_probe(struct platform_device *dev)
{
	...
	struct comp_port ehc0_comp = {_MSTAR_EHC0_COMP_PORT, _MSTAR_U3TOP_BASE, _MSTAR_EHC0_COMP_CTRL_ADDR, _MSTAR_EHC0_COMP_PORT_INDEX};
	#endif
	#ifdef _MSTAR_EHC1_COMP_PORT
	struct comp_port ehc1_comp = {_MSTAR_EHC1_COMP_PORT, _MSTAR_U3TOP_BASE, _MSTAR_EHC1_COMP_CTRL_ADDR, _MSTAR_EHC1_COMP_PORT_INDEX};
	#endif
	#ifdef _MSTAR_EHC2_COMP_PORT
	struct comp_port ehc2_comp = {_MSTAR_EHC2_COMP_PORT, _MSTAR_U3TOP_BASE, _MSTAR_EHC2_COMP_CTRL_ADDR, _MSTAR_EHC2_COMP_PORT_INDEX};
	#endif
	#ifdef _MSTAR_EHC3_COMP_PORT
	struct comp_port ehc3_comp = {_MSTAR_EHC3_COMP_PORT, _MSTAR_U3TOP_BASE, _MSTAR_EHC3_COMP_CTRL_ADDR, _MSTAR_EHC3_COMP_PORT_INDEX};
	#endif
	#ifdef _MSTAR_EHC4_COMP_PORT
	struct comp_port ehc4_comp = {_MSTAR_EHC4_COMP_PORT, _MSTAR_U3TOP_BASE, _MSTAR_EHC4_COMP_CTRL_ADDR, _MSTAR_EHC4_COMP_PORT_INDEX};
	#endif
...
#if (USB_LG_USERRESUME == 1)
	/* add for LG user reusme */
...
#ifdef CONFIG_MSTAR_STR_TIME
	REG_WR_USB(REG_PIU_TIMER0_USB(0x00), (REG_RR_USB(REG_PIU_TIMER0_USB(0x00)) & (~(0x1))));
	REG_WR_USB(REG_PIU_TIMER0_USB(0x02), 0xFFFF);
	REG_WR_USB(REG_PIU_TIMER0_USB(0x03), 0xFFFF);
	REG_WR_USB(REG_PIU_TIMER0_USB(0x00), (REG_RR_USB(REG_PIU_TIMER0_USB(0x00)) | (0x1)));
#endif
...
#ifdef CONFIG_MSTAR_STR_TIME
	u32Timer = ((unsigned long)REG_RR_USB(REG_PIU_TIMER0_USB(0x04))) | ((unsigned long)REG_RR_USB(REG_PIU_TIMER0_USB(0x05)) << 16);
	u32Timer_us = ((u32Timer))/12;
	printk("usb port %d resume time : %d.%06ds\n\n", hcd->port_index, u32Timer_us/1000000, u32Timer_us%1000000);
#endif
	return 0;
}
#endif
