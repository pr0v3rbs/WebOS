...
#if defined(CONFIG_MP_USB_MSTAR) && (XHCI_FLUSHPIPE_PATCH)
extern void Chip_Flush_Memory(void);
#endif /* CONFIG_MP_USB_MSTAR && XHCI_FLUSHPIPE_PATCH */
...
int xhci_reset(struct xhci_hcd *xhci)
{
	...
	if (xhci->quirks & XHCI_INTEL_HOST)
		udelay(1000);
#if defined(CONFIG_MP_USB_MSTAR) && defined(XHCI_HC_RESET_PATCH)
	/*
...
int xhci_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	if (!(ep->ep_state & EP_HALT_PENDING)) {
		...
		printk("[XHCI] delay for stop cmd...\n");
		mdelay(100);
#endif
...
#if defined(CONFIG_MP_USB_MSTAR) && (MSTAR_LOST_SLOT_PATCH)
...
#endif /* CONFIG_MP_USB_MSTAR && MSTAR_LOST_SLOT_PATCH */
...
{
#if defined(CONFIG_MP_USB_MSTAR) // temp xhci init
	int retval;
...
int usb_xhci_hcd_init(void)
{
	return xhci_hcd_init();
}
user_initcall_grp("USB", usb_xhci_hcd_init);
#else
