		...
		if (!event_seg) {
			if (!ep->skip ||
				...
				trb_in_td(xhci, ep_ring->deq_seg,
					  ...
					  event_dma, true);
#if defined(CONFIG_MP_USB_MSTAR) && defined(XHCI_TX_ERR_EVENT_PATCH)
				if (trb_comp_code == COMP_TX_ERR) {
...
#endif /* CONFIG_MP_USB_MSTAR && XHCI_TX_ERR_EVENT_PATCH */
...
#endif /* CONFIG_MP_USB_MSTAR && XHCI_FLUSHPIPE_PATCHevent_ring_deq = xhci->event_ring->dequeue */
...
#if defined(CONFIG_MP_USB_MSTAR)  //XHCI_CROSS64K_PATCH
		running_total = 0;
#else
