...
#if defined(CONFIG_MP_USB_MSTAR) && (_USB_SPLIT_MDATA_BLOCKING_PATCH)
...
#endif /* CONFIG_MP_USB_MSTAR && _USB_SPLIT_MDATA_BLOCKING_PATCH */
...
#if defined(CONFIG_MP_USB_MSTAR) && (_USB_SHORT_PACKET_LOSE_INT_PATCH)
		if (is_input && usb_pipebulk (urb->pipe) &&
			(!(urb->transfer_flags & URB_NO_INTERRUPT)) && (!(urb->transfer_flags & URB_SHORT_NOT_OK)))
		{
			token |= QTD_IOC;
		}
#endif /* CONFIG_MP_USB_MSTAR && _USB_SHORT_PACKET_LOSE_INT_PATCH */
