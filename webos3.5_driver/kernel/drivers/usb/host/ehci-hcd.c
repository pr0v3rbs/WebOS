...
#if defined(CONFIG_MP_USB_MSTAR) && (_USB_TURN_ON_TT_THROTTLE_MODE_PATCH)
#define	EHCI_TUNE_RL_TT		4
#else
#define	EHCI_TUNE_RL_TT		0
#endif /* CONFIG_MP_USB_MSTAR && _USB_TURN_ON_TT_THROTTLE_MODE_PATCH */
...
#if defined(CONFIG_MP_USB_MSTAR)	//tony ignore oc
...
#if defined(CONFIG_MP_USB_MSTAR) && defined(ENABLE_UHC_RUN_BIT_ALWAYS_ON_ECO)
	/* Don't close RUN bit when device disconnect */
	ehci_writel(ehci, ehci_readl(ehci, &ehci->regs->hcmisc) | BIT7, &ehci->regs->hcmisc);
#endif /* CONFIG_MP_USB_MSTAR && ENABLE_UHC_RUN_BIT_ALWAYS_ON_ECO */
...
static void ehci_work (struct ehci_hcd *ehci)
{
	 ...
	 */
#if defined(CONFIG_MP_USB_MSTAR) && (_USB_T3_WBTIMEOUT_PATCH)
	Chip_Read_Memory();	//Flush Read buffer when H/W finished
#endif /* CONFIG_MP_USB_MSTAR && _USB_T3_WBTIMEOUT_PATCH */
	if (ehci->scanning) {
...
#if defined(CONFIG_MP_USB_MSTAR)	//tony add for FUSB200
	ehci->i_thresh = 8;
...
int usb_ehci_hcd_init(void)
{
	return ehci_hcd_init();
}
user_initcall_grp("USB", usb_ehci_hcd_init);
