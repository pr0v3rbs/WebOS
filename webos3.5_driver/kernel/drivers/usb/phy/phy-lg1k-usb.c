...
int lg1k_usbphy_parse_dt(struct lg1k_usbphy *lgphy)
{
	...
	ret = of_property_read_u32(np, "loslvl_val", &regs[0]);
	if (ret) {
		lgphy->loslvl_val = 0x0;
	} else {
		lgphy->loslvl_val = regs[0];
		dev_info(lgphy->dev, "loslvl_val=%d\n", lgphy->loslvl_val);
	}
	return 0;
...
int lg1k_usbphy_set_reset_ctl_mask(struct usb_phy *phy, int on)
{
	struct lg1k_usbphy *lgphy;
	...
	if (lgphy->phy_rst) {
		...
		if (ret) {
			dev_warn(lgphy->dev,
					"phy_rst register is exist, but won't be set\n");
		} else {
			ret = lg1k_usb_phy_set_reset_ctl(lgphy->phy_rst,
						regs[0], regs[1], on);
			if (ret) {
