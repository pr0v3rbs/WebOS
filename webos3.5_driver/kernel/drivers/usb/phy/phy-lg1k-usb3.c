...
static int lg1k_usb3_phy_init_v2(struct lg1k_usbphy *lgphy)
{
	...
	if (lgphy->loslvl_val)
		parm2.reg.phy_los_level = lgphy->loslvl_val;
...
static int lg1k_usb3_rx_equalizer(struct lg1k_usbphy *lgphy)
{
	...
	if (lgphy->rx_eq_val) {
		...
		if ((val & FIXED_RX_EQ) != FIXED_RX_EQ)
			dev_warn(lgphy->dev,
				"adaptive rx equalization used (0x%x)\n", val);

		lg1k_usb3_cr_write(lgphy, addr, val);
		out = lg1k_usb3_cr_read(lgphy, addr);

		if (val != out) {
			dev_err(lgphy->dev, "rx_eq=0x%x, val=0x%x, out=0x%x\n",
					lgphy->rx_eq_val, val, out);
			return -EFAULT;
		}
	} else {
		dev_warn(lgphy->dev, "no rx equalizer setting\n");
	}
	dev_info(lgphy->dev, "rx_eq_val=0x%x, val=0x%x, out=0x%x, RX_EQ=0x%x\n",
			lgphy->rx_eq_val, val, out, val & RX_EQ_MASK);
