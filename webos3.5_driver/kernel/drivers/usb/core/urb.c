...
#if defined(CONFIG_MP_USB_MSTAR) //tony add for hotplug (FS)
				struct usb_hcd *hcd;
...
void usb_poison_urb(struct urb *urb)
{
	...
	long timeleft = wait_event_timeout(usb_kill_urb_queue,
		atomic_read(&urb->use_count) <= 0, (long)msecs_to_jiffies(2000)); //120314, modify for wifi load/unload repeatly
	if (timeleft == 0)
		printk("usb_poison_urb time out!\n");
	}
#else
	wait_event(usb_kill_urb_queue, atomic_read(&urb->use_count) == 0);
#endif /* CONFIG_MP_USB_MSTAR */
