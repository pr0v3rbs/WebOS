...
static U32  nand_WritePages_Raw(U32 u32_PhyRowIdx, U8 *pu8_DataBuf, U8 *pu8_SpareBuf, U32 u32_PageCnt)
{
    ...
    if( ((U32)pu8_DataBuf) >= 0xC0000000 || ((U32)pu8_DataBuf) % CACHE_LINE )   // For
...
#endif
    {
        for(u32_i = 0; u32_i < u32_PageCnt; u32_i ++)
        {
            ...
            u32_Err = NC_WritePages(u32_PhyRowIdx + u32_i, au8_PageBuf, pu8_SpareBuf, 1);
...
static void nand_ECC_ErrorHandler(U16 u16_ErrPBA)
{
    ...
    LABEL_START_MOVE:
    if(u16_ErrPBA >= pNandDrv->u16_FirstPBA_Zone0)
        u16_FreePBA = nand_get_FreeBlk_Zone0();
    else if(u16_ErrPBA >= pNandDrv->u16_FirstPBA_Zone1)
