...
#define _MAD_SND_DRIVER    "MStar ALSA"
...
#define _MAD_DEVICE    8
#define _MAD_MAX_PCM_BUFFER_SIZE    64*1024


#define ALSA_AUDIO_DRIVER_DEBUG 0
...
//lesterlee
#if ALSA_AUDIO_DRIVER_DEBUG
#define PLAYBACK_FILE_PATH      "/tmp/alsa_drv.log"
...
static int _mdrv_alsa_playback_copy(struct snd_pcm_substream *substream, int channel, snd_pcm_uframes_t pos, void __user *buf, snd_pcm_uframes_t count)
{
	...
	diff  = (time3.tv_sec * 1000000 + time3.tv_usec) - (time2.tv_sec * 1000000 + time2.tv_usec);
	diff1 = (time3.tv_sec * 1000000 + time3.tv_usec) - (time1.tv_sec * 1000000 + time1.tv_usec);
	sprintf(str1, "[%010lu] request_byte [%d] writen size [%d] time_all[%ld] time_write[%ld] usec\n", jiffies_to_msecs(jiffies), request_size, copied_size, diff1, diff);
	audio_write_file(PLAYBACK_FILE_PATH, str1, strlen(str1));
#endif
...
static int _mdrv_alsa_probe(int card_no)
{
	...
	if ((err = snd_card_new(NULL, index[card_no], id[card_no], THIS_MODULE, sizeof(struct MStar_Device_Struct), &card)) != 0) {
