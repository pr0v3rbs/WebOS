...
static ssize_t _mod_ir_read(struct file *filp, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	DEFINE_WAIT(wait_ir_read);
#endif /* CONFIG_MSTAR_PORTING_KERNEL_4_4_3 */


        /* 2008,12,03:(dreamer@lge.com) --> removed non-ASCII char by glen.chang */
 ...
 *        wait-queue usage. */
	prepare_to_wait(&key_wait_q, &wait_ir_read, TASK_INTERRUPTIBLE);
	irParam.u32Timeout = schedule_timeout(timeout);
	finish_wait(&key_wait_q, &wait_ir_read);
#else
	...
	if (bRet)
	{
		...
		else if( u8Flag == (_IR_READ_TYPE_REPEAT - 1) )	/* REPEAT KEY */
		{
			irParam.u8IRType        =       _IR_READ_TYPE_REPEAT;
                        memcpy( &(irParam.u8IRValue[0]), pU8Key, IR_TV_LINK_DATA_NUM );
		}
		else	/* SINGLE KEY */
		{
