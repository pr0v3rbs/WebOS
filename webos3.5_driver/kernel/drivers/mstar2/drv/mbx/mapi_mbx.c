...
static MBX_Result MApi_SendDynamicRequest(MBX_ROLE_ID eRoleID, const char *name)
{
    MS_U16 i;
    MBX_Msg smsg;
    MBX_Result ret = E_MBX_UNKNOW_ERROR;

        memset(&smsg,0,sizeof(MBX_Msg));
...
#define MAX_RETRY_RECV (5000)
    ...
    MBX_Result ret = E_MBX_UNKNOW_ERROR;
        ...
        memset(&rmsg,0,sizeof(MBX_Msg));
    while(E_MBX_SUCCESS!= (ret=MApi_MBX_RecvMsg(E_MBX_CLASS_SECUREBOOT_WAIT, &rmsg, 1500, MBX_CHECK_INSTANT_MSG)) && (u16Retry++<MAX_RETRY_RECV) )
    {
...
MBX_Result  MApi_MBX_QueryDynamicClass(MBX_ROLE_ID eRoleID, const char *name, MS_U8 *pmbx_class)
{
    MBX_Result ret = E_MBX_UNKNOW_ERROR;
...
void  MApi_MBX_NotifyTeetoSuspend(void)
{
    ...
    stMsg.u8Index=0x2e; //EN_CTA_CMD_STR_SUSPEND_NO_ACK
    stMsg.u8ParameterCount=0xa;
    stMsg.u8Parameters[0]=0x0;
    stMsg.u8Parameters[1]=0x0;
    stMsg.u8Parameters[2]=0x0;
    stMsg.u8Parameters[3]=0x0;
    stMsg.u8Parameters[4]=0x0;
    stMsg.u8Parameters[5]=0x0;
    stMsg.u8Parameters[6]=0x0;
    stMsg.u8Parameters[7]=0x0;
