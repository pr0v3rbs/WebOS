...
MBX_Result  _MDrv_MBX_RecvMsg(MS_S16 mbxAsyncNotifierID, MBX_Class eTargetClass, MBX_Msg *pMsg, MS_U32 u32Flag)
{
   ...
   // DRV_MBX_LockRecv();
    ...
    else
    {
        ...
        if(!IS_VALID_PTR(MDrv_MSGQ_GetNotiferIDByQID((MS_S16)eTargetClass)))
        {
            //DRV_MBX_UnLockRecv();
        ...
        */
        ...
        {
            //check Instance Msg for All registered class:
            mbxResult = MDrv_MSGQ_RecvMsg((MS_S16)eTargetClass, pMsg, TRUE);
            if(mbxResult == E_MBX_SUCCESS)
            {
                //DRV_MBX_UnLockRecv();
                return mbxResult;
        ...
        {
            //check Instance Msg for All registered class:
            mbxResult = MDrv_MSGQ_RecvMsg((MS_S16)eTargetClass, pMsg, FALSE);
        }
    }
...
//  g_u32AMBXAsyncID = u32AsyncID;
	...
	#ifdef CONFIG_MSTAR_MBX_U2K
#ifdef CONFIG_MSTAR_PORTING_KERNEL_4_4_3
    /* move upward to avoid compiler warning */
#else
	    g_u32AMBXAsyncID = u32AsyncID;
