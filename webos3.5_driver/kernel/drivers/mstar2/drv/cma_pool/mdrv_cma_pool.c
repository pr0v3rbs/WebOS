    ...
    , filp_private *pdev)
{
    ...
    *virt_addr = 0;
...
#ifdef CMA_POOL_DUMP
    printk(KERN_ERR "\n%s:%d heapid %u  flags=%lu   ", __FUNCTION__,__LINE__, heap_id,flags);
    #ifdef CONFIG_MP_CMA_PATCH_POOL_UTOPIA_TO_KERNEL
    printk(KERN_ERR "flags &CMA_FLAG_MAP_KERNEL=%d\n",!!(flags &CMA_FLAG_MAP_KERNEL));
    #else
    printk("\n");
    #endif
#endif
    ...
    if(heap_info)
    {
        ...
        *miu = heap_info->miu;
        *heap_length = cma_get_size(dev_get_cma_area(heap_info->dev));
        pdev->temp_heap_id = heap_id;
        proc_info->heap_id = heap_id;
        proc_info->base_phy = cma_get_base(dev_get_cma_area(heap_info->dev));
    ...
    pfn = cma_get_base_pfn(dev_get_cma_area(dev));
    ...
    *miu = heap_info->miu;
    *heap_length = cma_get_size(dev_get_cma_area(dev));
    *heap_start_offset = offset;
    *bus_addr = proc_info->base_phy;
    ...
    unsigned long length, unsigned long align, unsigned int flags, filp_private *pdev)
{
    ...
    struct heap_proc_info * proc_info = NULL;
	signed long timeout;
    unsigned long cma_area_size;
    phys_addr_t cma_area_base;
...
#ifdef CMA_POOL_DUMP
    printk(KERN_ERR "\n%s:%d, heapid %u, flags=%u\n", __FUNCTION__,__LINE__, heap_id, flags);
#endif
    ...
    cma_area_base = cma_get_base(dev_get_cma_area(heap_info->dev));
    cma_area_size = cma_get_size(dev_get_cma_area(heap_info->dev));
    ...
    if(!buffer)
    {
        ...
        if(((phy_addr + length) > (cma_area_base + cma_area_size)) || (phy_addr < cma_area_base))
        {
            ret = -EINVAL;
...
static long CMA_Pool_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    {
        ret = CMA_Pool_Init(data.heap_info.heap_id, data.heap_info.flags,
					(unsigned int *)&data.heap_info.miu,
                    (unsigned long *)&data.heap_info.bus_addr,
                    (unsigned long *)&data.heap_info.heap_miu_start_offset,
                    (unsigned long *)&data.heap_info.heap_length,
#ifdef CONFIG_MP_CMA_PATCH_POOL_UTOPIA_TO_KERNEL
                    (unsigned long *)&data.heap_info.virt_addr,
#endif
...
static void init_glob_miu_kranges(void)
{
	...
	/* workaround: lx_mem3_size of size 0 declared in boot arg */
    lx_mem3_size = INVALID_PHY_ADDR;
#endif
...
{
	printk("\033[0;32;31m [Ian] %s %i to %lu\033[m\n",__func__,__LINE__,data);
	printk("\033[0;32;31m [Ian] %s %i to %lu\033[m\n",__func__,__LINE__,data);
	printk("\033[0;32;31m [Ian] %s %i to %lu\033[m\n",__func__,__LINE__,data);
	printk("\033[0;32;31m [Ian] %s %i to %lu\033[m\n",__func__,__LINE__,data);
