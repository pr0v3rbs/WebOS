...
BOOL msGetHistogramHandler(BOOL bWindow)
{
    ...
    if (g_DlcParameters.ucDlcTimeOut)
    {
        ...
        for (ucTmp = 0, wTmpHistogramSum = 0, ucHistogramOverflowIndex = 0xFF; ucTmp < 32; ucTmp++)
        {
            ...
            if((MApi_XC_R2BYTEMSK(REG_SC_BK30_01_L, 0xFF))==0x1A)
            {
...
BOOL msDLCMSTVToolReadWriteValue( void )
{
    ...
    //Check R/C bit
    if ( MApi_XC_R2BYTEMSK(REG_SC_BK30_03_L, BIT(0))==BIT(0) )
    {
        u8IDValue=(BYTE)MApi_XC_R2BYTEMSK(REG_SC_BK30_02_L, 0xFF);
        u8DataByte=(BYTE)MApi_XC_R2BYTEMSK(REG_SC_BK30_04_L, 0xFF);
        ...
        if((u8DataByte>16)|(u8IDValue>181)|((u8IDValue+u8DataByte)>182))
        {
            ...
            MApi_XC_W2BYTEMSK(REG_SC_BK30_03_L, 0xFE, 0xFF);
            return FALSE;
        }

        //MSTV Tool want to read the data
        if( MApi_XC_R2BYTEMSK(REG_SC_BK30_03_L, BIT(8))==BIT(8) )
        {
        ...
        MApi_XC_W2BYTEMSK(REG_SC_BK30_03_L, 0, 0xFF);
    }
    bReturn = TRUE;
