...
static int get_hib_part_size(void)
{
	...
	ret = lgemmc_get_partsize(partnum);
	if (!ret)
		return -EINVAL;
	else
		return ret;
...
int lgsnap_write_magic(void)
{
	...
	info = (struct swsusp_info *)__get_free_pages(GFP_KERNEL,
					get_order(sizeof(struct swsusp_info)));
	if (info == NULL) {
	...
	free_pages((unsigned long)info, get_order(sizeof(struct swsusp_info)));
out_finish:
